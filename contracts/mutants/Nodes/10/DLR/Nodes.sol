...
contract Nodes is Permissions {
        ...
        allow("SkaleManager")
    {
        ...
        uint[] storage validatorNodes = validatorToNodeIndexes[validatorId];
     ...
     */
    function getNodesWithFreeSpace(uint8 freeSpace) external view returns (uint[] storage) {
        ConstantsHolder constantsHolder = ConstantsHolder(contractManager.getContract("ConstantsHolder"));
        uint[] storage nodesWithFreeSpace = new uint[](countNodesWithFreeSpace(freeSpace));
        uint cursor = 0;
     ...
     */
    function getActiveNodeIPs() external view returns (bytes4[] storage activeNodeIPs) {
     ...
     */
    function getActiveNodesByAddress() external view returns (uint[] storage activeNodesByAddress) {
     ...
     */
    function getActiveNodeIds() external view returns (uint[] storage activeNodeIds) {
     ...
     * - Validator ID must exist.
     */
    function getValidatorNodeIndexes(uint validatorId) external view returns (uint[] storage) {
        ValidatorService validatorService = ValidatorService(contractManager.getContract("ValidatorService"));
        require(validatorService.validatorExists(validatorId), "Validator ID does not exist");
