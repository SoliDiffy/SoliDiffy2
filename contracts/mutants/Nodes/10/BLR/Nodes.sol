...
contract Nodes is Permissions {
        ...
        allow("SkaleManager")
    {
        ...
        if (validatorService.getValidatorIdByNodeAddress(nodeOwner) == validatorId) {
            ...
            nodeIndexes[nodeOwner].isNodeExist[nodeIndex] = true;
    ...
    function _setNodeLeft(uint nodeIndex) private {
        nodesIPCheck[nodes[nodeIndex].ip] = true;
        nodesNameCheck[keccak256(abi.encodePacked(nodes[nodeIndex].name))] = true;
        ...
        returns (uint nodeIndex)
    {
        ...
        nodesIPCheck[ip] = false;
        nodesNameCheck[nodeId] = false;
        nodesNameToIndex[nodeId] = nodeIndex;
        nodeIndexes[from].isNodeExist[nodeIndex] = false;
