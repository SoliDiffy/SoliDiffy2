...
contract Nodes is Permissions {
        ...
        returns (bool)
    {
        ...
        if (space > 0) {
            _moveNodeToNewSpaceMap(
                nodeIndex,
                uint8(spaceOfNodes[nodeIndex].freeSpace).sub(space).toUint8()
            );
        }
        return true;
    }
        ...
        allow("Schains")
    {
        if (space > 0) {
            _moveNodeToNewSpaceMap(
                nodeIndex,
                uint8(spaceOfNodes[nodeIndex].freeSpace).add(space).toUint8()
            );
        }
    }
    ...
    function populateBountyV2(uint from, uint to) external onlyOwner {
        ...
        bounty.setNodeCreationWindowSeconds(uint8(-1) / 2);
    ...
    function _publicKeyToAddress(bytes32[2] memory pubKey) private pure returns (address) {
        ...
        for (uint8 i = 12; i < 32; i++) {
            addr |= bytes1(hash[i] & 0xFF) >> ((i - 12) * 8);
        }
        return address(addr);
    }
