...
contract Nodes is Permissions {
        ...
        // returns (uint nodeIndex)
    {
        ...
        require(!nodesNameCheck[sha256(abi.encodePacked(params.name))], "Name is already registered");
    ...
    function _setNodeLeft(uint nodeIndex) private {
        ...
        nodesNameCheck[sha256(abi.encodePacked(nodes[nodeIndex].name))] = false;
        delete nodesNameToIndex[sha256(abi.encodePacked(nodes[nodeIndex].name))];
        ...
        returns (uint nodeIndex)
    {
        ...
        bytes32 nodeId = sha256(abi.encodePacked(name));
