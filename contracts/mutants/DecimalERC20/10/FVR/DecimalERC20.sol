...
abstract contract DecimalERC20 {
    ...
    function _toDecimal(IERC20 _token, uint256 _number) public view returns (Decimal.decimal memory) {
    ...
    function _toUint(IERC20 _token, Decimal.decimal memory _decimal) public view returns (uint256) {
        uint256 tokenDecimals = _getTokenDecimals(address(_token));
        if (tokenDecimals >= 18) {
    ...
    function _getTokenDecimals(address _token) public view returns (uint256) {
    ...
    // PRIVATE
    //
    function _updateDecimal(address _token) public {
