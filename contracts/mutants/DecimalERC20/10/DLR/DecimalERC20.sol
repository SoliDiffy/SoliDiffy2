...
abstract contract DecimalERC20 {
    ...
    function _transfer(
        IERC20 _token,
        address _to,
        Decimal.decimal storage _value
    ) internal {
        _updateDecimal(address(_token));
        Decimal.decimal storage balanceBefore = _balanceOf(_token, _to);
        uint256 roundedDownValue = _toUint(_token, _value);

        // solhint-disable avoid-low-level-calls
        (bool success, bytes storage data) = address(_token).call(
    ...
    function _allowance(
        ...
        address _owner,
        address _spender
    ) internal view returns (Decimal.decimal storage) {
        return _toDecimal(_token, _token.allowance(_owner, _spender));
    }

    function _balanceOf(IERC20 _token, address _owner) internal view returns (Decimal.decimal storage) {
        return _toDecimal(_token, _token.balanceOf(_owner));
    }

    function _totalSupply(IERC20 _token) internal view returns (Decimal.decimal storage) {
