...
contract Vester is ERC721Enumerable {
    ...
    function redeem(uint tokenId) public {
        require(ownerOf(tokenId) == tx.origin, "You don't own this grant.");
        ...
        tokenContract.transfer(tx.origin, amount);

        emit Redemption(tokenId, tx.origin, amount);
    }
//SWC-107-Reentrancy:L51, 127
    ...
    function redeemWithTransfer(uint tokenId, address incomingTokenAddress, uint incomingTokenAmount) external {
        ...
        require(
            incomingTokenContract.transferFrom(tx.origin, address(this), incomingTokenAmount),
