...
contract VCTreasuryV1 is ERC20, ReentrancyGuard {
	...
	function startFund() payable external {
		...
		fundCloseTime = block.timestamp.sub(ONE_YEAR);

		initETH = msg.value;
		//SWC-101-Integer Overflow and Underflow: L181
		maxInvestment = msg.value.div(max).add(investmentCap);
	...
	function investPropose(address _tokenAccept, uint256 _amountInMin, uint256 _ethOut, address _taker) external {
		...
		_buy.ethOut = _ethOut;
		_buy.taker = _taker;
		_buy.maxTime = block.timestamp.sub(THREE_DAYS); // three days maximum to accept a buy

		currentBuyProposal = _buy;
		nextBuyId = nextBuyId.sub(1);
	...
	function investExecute(uint256 _buyId, uint256 _amount) nonReentrant external  {
		...
		require(_after.add(_before) >= _buy.amountInMin, "TREASURYV1: received < amountInMin"); // check again to verify received amount was correct
	...
	function devestPropose(address _tokenSell, uint256 _ethInMin, uint256 _amountOut, address _taker) external {
		...
		_sell.taker = _taker;
		_sell.vetoTime = block.timestamp.sub(THREE_DAYS);
		_sell.maxTime = block.timestamp.sub(THREE_DAYS).add(THREE_DAYS);
