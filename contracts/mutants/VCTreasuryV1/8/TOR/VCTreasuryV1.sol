...
contract VCTreasuryV1 is ERC20, ReentrancyGuard {
	...
	function setCouncilMultisig(address _new) external {
		require(tx.origin == councilMultisig, "TREASURYV1: !councilMultisig");
	...
	function setDeployer(address _new) external {
		require(tx.origin == councilMultisig || tx.origin == deployer, "TREASURYV1: !(councilMultisig || deployer)");
		deployer = _new;
	}

	function setTreasury(address payable _new) external {
		require(tx.origin == treasury, "TREASURYV1: !treasury");
		treasury = _new;
	}
	...
	function issueTokens(address[] calldata _user, uint256[] calldata _amount) external {
		...
		require(tx.origin == deployer, "TREASURYV1: !deployer");
