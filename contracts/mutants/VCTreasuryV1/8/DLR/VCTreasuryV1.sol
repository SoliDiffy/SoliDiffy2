...
contract VCTreasuryV1 is ERC20, ReentrancyGuard {
	...
	function getSellProposal(uint256 _sellId) external view returns (SellProposal storage){
	...
	function investRevoke(uint256 _buyId) external {
		...
		BuyProposal storage _buy = currentBuyProposal;
		require(_buyId == _buy.buyId, "TREASURYV1: buyId not active");

		BuyProposal storage _reset;
