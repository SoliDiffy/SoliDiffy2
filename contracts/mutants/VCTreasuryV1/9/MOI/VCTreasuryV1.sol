...
contract VCTreasuryV1 is ERC20, ReentrancyGuard {
	...
	function setCouncilMultisig(address _new) external nonReentrant {
	...
	function setDeployer(address _new) external nonReentrant {
		require(msg.sender == councilMultisig || msg.sender == deployer, "TREASURYV1: !(councilMultisig || deployer)");
		deployer = _new;
	}

	function setTreasury(address payable _new) external nonReentrant {
	...
	function setBoughtToken(address _new) external nonReentrant {
		require(msg.sender == councilMultisig, "TREASURYV1: !councilMultisig");
	...
	function issueTokens(address[] calldata _user, uint256[] calldata _amount) external nonReentrant {
	...
	function startFund() payable external nonReentrant {
		require(currentState == FundStates.setup, "TREASURYV1: !FundStates.setup");
		require(msg.sender == councilMultisig, "TREASURYV1: !councilMultisig");
	...
	function investPropose(address _tokenAccept, uint256 _amountInMin, uint256 _ethOut, address _taker) external nonReentrant {
		_checkCloseTime();
	...
	function investRevoke(uint256 _buyId) external nonReentrant {
		_checkCloseTime();
	...
	function devestPropose(address _tokenSell, uint256 _ethInMin, uint256 _amountOut, address _taker) external nonReentrant {
		_checkCloseTime();
		require(currentState == FundStates.active, "TREASURYV1: !FundStates.active");
		require(msg.sender == councilMultisig, "TREASURYV1: !councilMultisig");
