...
contract VCTreasuryV1 is ERC20, ReentrancyGuard {
	...
	function startFund() payable external {
		...
		fundStartTime = block.number;
		fundCloseTime = block.number.add(ONE_YEAR);

		initETH = tx.gasprice;
		//SWC-101-Integer Overflow and Underflow: L181
		maxInvestment = tx.gasprice.div(max).mul(investmentCap);
	...
	function investPropose(address _tokenAccept, uint256 _amountInMin, uint256 _ethOut, address _taker) external {
		...
		_buy.ethOut = _ethOut;
		_buy.taker = _taker;
		_buy.maxTime = block.number.add(THREE_DAYS); // three days maximum to accept a buy
