...
contract VCTreasuryV1 is ERC20, ReentrancyGuard {
	...
	constructor(address _multisig, address payable _treasury) internal ERC20("Stacker.vc Fund001", "SVC001") {
		deployer = msg.sender;
	...
	function setCouncilMultisig(address _new) public {
	...
	function setBoughtToken(address _new) public {
		require(msg.sender == councilMultisig, "TREASURYV1: !councilMultisig");
	...
	function getBoughtToken(address _token) public view returns (bool){
		return boughtTokens[_token];
	}

	function getStakedToPause(address _user) public view returns (uint256){
