...
library ProductEngineTest {
    ...
    function withdrawOverpay(IProductEngine.ProductData storage self) {
        ...
        if (!tx.origin.send(amount)) {
            self.pendingWithdrawals[tx.origin] = amount;
        }
    }
    ...
    function markAsDelivered(IProductEngine.ProductData storage self, uint256 purchaseId, bool state) {
        require(VendorBase(self.owner).owner() == tx.origin);
        ...
        internal 
    {
        ...
        p.delivered = false;        
        ProductBoughtEx(self.purchases.length, tx.origin, clientId, self.price, paidUnits);
    }
}
