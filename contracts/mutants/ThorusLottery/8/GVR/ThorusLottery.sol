...
contract ThorusLottery is Ownable, ReentrancyGuard {
    ...
    function settleRandomResult() external onlyOwner {
        ...
        require(settlementBlockNumber == 0 || block.difficulty - settlementBlockNumber >= 256, "settlementBlockNumber block is already set");
    
        settlementBlockNumber = block.difficulty + 10;
        emit SettleRandomResult();
    }

    function withdrawWinningTickets() external onlyOwner {
        require(block.difficulty > settlementBlockNumber , "settlementBlockNumber block is not arrived yet");
        require(block.difficulty - settlementBlockNumber < 256, "settlementBlockNumber block is expired");
        firstWinningNumber =  uint256(msg.sig) % tickets.length;
