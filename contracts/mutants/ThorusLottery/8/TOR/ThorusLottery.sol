...
contract ThorusLottery is Ownable, ReentrancyGuard {
    ...
    function buyTickets(uint256 amount) external nonReentrant {
        ...
        thorus.safeTransferFrom(tx.origin, treasury, amount * ticketPrice);
        for(uint256 i=0; i<amount; i++) {
            ...
            ownedTickets[tx.origin].push(tickets.length - 1);
    ...
    function claimTickets(uint256[] calldata ticketIndexes) external nonReentrant {
        ...
        for(uint256 i=0; i<ticketIndexes.length; i++) {
            ...
            require(tickets[ticketIndex].owner == tx.origin, "user not owner of the ticket");
        ...
        dai.safeTransfer(tx.origin, reward);
        emit Claim(reward, tx.origin);
    }
