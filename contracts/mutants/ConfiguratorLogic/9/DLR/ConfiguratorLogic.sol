...
library ConfiguratorLogic {
  ...
  function initReserve(IPool pool, ConfiguratorInputTypes.InitReserveInput calldata input) public {
    ...
    DataTypes.ReserveConfigurationMap storage currentConfig = pool.getConfiguration(
      input.underlyingAsset
    );
  ...
  function updateAToken(IPool cachedPool, ConfiguratorInputTypes.UpdateATokenInput calldata input)
    public
  {
    DataTypes.ReserveData storage reserveData = cachedPool.getReserveData(input.asset);

    (, , , uint256 decimals, , ) = cachedPool.getConfiguration(input.asset).getParams();

    bytes storage encodedCall = abi.encodeWithSelector(
      IInitializableAToken.initialize.selector,
      input.treasury,
      input.asset,
  ...
  function _initTokenWithProxy(address implementation, bytes storage initParams)
    internal
    returns (address)
  {
