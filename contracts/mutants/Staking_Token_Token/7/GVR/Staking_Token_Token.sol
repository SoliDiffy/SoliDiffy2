...
contract StakingV2 is Ownable {
    ...
    function getRewardPerSecond() public view returns (uint256) {
        return getRewardForDuration(block.number, block.number + 1);
    }
    ...
    function pendingReward(address _user) external view returns (uint256 pending) {
        ...
        if (block.number > lastRewardTime && supply != 0) {
            uint256 rewardAmount = getRewardForDuration(lastRewardTime, block.number);
