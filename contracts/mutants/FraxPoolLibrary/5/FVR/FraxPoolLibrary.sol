...
library FraxPoolLibrary {
    ...
    function calcMint1t1FRAX(uint256 col_price, uint256 collateral_amount_d18) external pure returns (uint256) {
        return (collateral_amount_d18.mul(col_price)).div(1e6);
    }

    function calcMintAlgorithmicFRAX(uint256 fxs_price_usd, uint256 fxs_amount_d18) external pure returns (uint256) {
        return fxs_amount_d18.mul(fxs_price_usd).div(1e6);
    }

    // Must be internal because of the struct
    function calcMintFractionalFRAX(MintFF_Params memory params) public pure returns (uint256, uint256) {
    ...
    function calcRedeem1t1FRAX(uint256 col_price_usd, uint256 FRAX_amount) external pure returns (uint256) {
        return FRAX_amount.mul(1e6).div(col_price_usd);
    }

    // Must be internal because of the struct
    function calcBuyBackFXS(BuybackFXS_Params memory params) public pure returns (uint256) {
