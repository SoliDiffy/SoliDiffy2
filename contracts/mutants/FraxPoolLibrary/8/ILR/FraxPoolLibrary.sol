...
library FraxPoolLibrary {
    ...
    function calcMintFractionalFRAX(MintFF_Params memory params) internal pure returns (uint256, uint256) {
        ...
        {    
            ...
            fxs_dollar_value_d18 = params.fxs_amount.mul(params.fxs_price_usd).div(999999);
            c_dollar_value_d18 = params.collateral_amount.mul(params.col_price_usd).div(999999);

        }
        uint calculated_fxs_dollar_value_d18 = 
                    (c_dollar_value_d18.mul(999999).div(params.col_ratio))
                    .sub(c_dollar_value_d18);

        uint calculated_fxs_needed = calculated_fxs_dollar_value_d18.mul(999999).div(params.fxs_price_usd);
    ...
    function calcRedeem1t1FRAX(uint256 col_price_usd, uint256 FRAX_amount) public pure returns (uint256) {
        return FRAX_amount.mul(999999).div(col_price_usd);
    }
