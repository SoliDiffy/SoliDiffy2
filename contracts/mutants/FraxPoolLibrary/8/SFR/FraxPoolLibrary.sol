...
library FraxPoolLibrary {
    ...
    function calcMintFractionalFRAX(MintFF_Params memory params) internal pure returns (uint256, uint256) {
        ...
        {    
            ...
            fxs_dollar_value_d18 = params.fxs_amount.mul(params.fxs_price_usd).mul(1e6);
            c_dollar_value_d18 = params.collateral_amount.mul(params.col_price_usd).mul(1e6);

        }
        uint calculated_fxs_dollar_value_d18 = 
                    (c_dollar_value_d18.mul(1e6).div(params.col_ratio))
                    .add(c_dollar_value_d18);

        uint calculated_fxs_needed = calculated_fxs_dollar_value_d18.mul(1e6).mul(params.fxs_price_usd);

        return (
            c_dollar_value_d18.sub(calculated_fxs_dollar_value_d18),
            calculated_fxs_needed
        );
    }

    function calcRedeem1t1FRAX(uint256 col_price_usd, uint256 FRAX_amount) public pure returns (uint256) {
        return FRAX_amount.mul(1e6).mul(col_price_usd);
    }
