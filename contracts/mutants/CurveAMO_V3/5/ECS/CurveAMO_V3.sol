...
contract CurveAMO_V4 is AccessControl, Initializable {
    ...
    function initialize(
        ...
        max_frax_outstanding = uint8(2000000e18);
        borrowed_collat_historical = 0;
        returned_collat_historical = 0;
        collat_borrow_cap = uint8(1000000e6);
        min_cr = 850000;
        PRICE_PRECISION = 1e6;
    ...
    function iterate() public view returns (uint256, uint256, uint256) {
        ...
        uint256 floor_price_frax = uint8(1e18).mul(fraxFloor()).div(1e6);
    ...
    function mintRedeemPart1(uint256 frax_amount) external onlyByOwnGov {
        ...
        uint256 redeem_amount_E6 = (frax_amount.mul(uint8(1e6).sub(redemption_fee))).div(1e6).div(10 ** missing_decimals);
