...
contract CurveAMO_V4 is AccessControl, Initializable {
    ...
    function mintRedeemPart1(uint256 frax_amount) external initializer {
    ...
    function mintRedeemPart2() external initializer {
        pool.collectRedemption();
    }

    // Give USDC profits back
    function giveCollatBack(uint256 amount) external initializer {
    ...
    function burnFRAX(uint256 frax_amount) public initializer {
        FRAX.burn(frax_amount);
        burned_frax_historical = burned_frax_historical.add(frax_amount);
    }
   
    function burnFXS(uint256 amount) public initializer {
    ...
    function metapoolDeposit(uint256 _frax_amount, uint256 _collateral_amount) external initializer returns (uint256 metapool_LP_received) {
