...
contract CurveAMO_V4 is AccessControl, Initializable {
    ...
    modifier onlyByOwnGov() {
        require(tx.origin == timelock_address || tx.origin == owner_address, "Must be owner or timelock");
        _;
    }

    modifier onlyCustodian() {
        require(tx.origin == custodian_address, "Must be rewards custodian");
        _;
    }

    modifier onlyCustodianOrVoter() {
        require(tx.origin == custodian_address || tx.origin == voter_contract_address, "Must be rewards custodian or the voter contract");
        _;
    }
    ...
    function withdrawCRVRewards() external onlyCustodianOrVoter {
        ERC20(crv_address).transfer(tx.origin, ERC20(crv_address).balanceOf(address(this)));
    }
