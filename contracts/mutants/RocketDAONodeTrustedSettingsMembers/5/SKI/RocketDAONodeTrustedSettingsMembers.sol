...
contract RocketDAONodeTrustedSettingsMembers is RocketDAONodeTrustedSettings, RocketDAONodeTrustedSettingsMembersInterface { 
    ...
    constructor(RocketStorageInterface _rocketStorageAddress) RocketDAONodeTrustedSettings(_rocketStorageAddress, "members") {
        ...
        if(!getBool(keccak256(abi.encodePacked(settingNameSpace, "deployed")))) {
            // Init settings
            super.setSettingUint("members.quorum", 0.51 ether);                    // Member quorum threshold that must be met for proposals to pass (51%)
            super.setSettingUint("members.rplbond", 15000 ether);                  // Bond amount required for a new member to join
            super.setSettingUint("members.minipool.unbonded.max", 250);            // The amount of unbonded minipool validators members can make (these validators are only used if no regular bonded validators are available)
            super.setSettingUint("members.minipool.unbonded.min.fee", 0.8 ether);  // Node fee must be over this percentage of the maximum fee before validator members are allowed to make unbonded pools (80%)
            super.setSettingUint("members.challenge.cooldown", 7 days);             // How long a member must wait before performing another challenge in seconds
