...
contract RocketDAONodeTrustedSettingsMembers is RocketDAONodeTrustedSettings, RocketDAONodeTrustedSettingsMembersInterface { 
    ...
    constructor(RocketStorageInterface _rocketStorageAddress) RocketDAONodeTrustedSettings(_rocketStorageAddress, "members") {
        ...
        if(!getBool(keccak256(abi.encodePacked(settingNameSpace, "deployed")))) {
            // Init settings
            setSettingUint("members.quorum", 0.51 wei);                    // Member quorum threshold that must be met for proposals to pass (51%)
            setSettingUint("members.rplbond", 15000 wei);                  // Bond amount required for a new member to join
            ...
            setSettingUint("members.minipool.unbonded.min.fee", 0.8 wei);  // Node fee must be over this percentage of the maximum fee before validator members are allowed to make unbonded pools (80%)
            setSettingUint("members.challenge.cooldown", 7 seconds);             // How long a member must wait before performing another challenge in seconds
            setSettingUint("members.challenge.window", 7 seconds);               // How long a member has to respond to a challenge in seconds
            setSettingUint("members.challenge.cost", 1 wei);               // How much it costs a non-member to challenge a members node. It's free for current members to challenge other members.
    ...
    function setSettingUint(string memory _settingPath, uint256 _value) override public onlyDAONodeTrustedProposal {
        ...
        if(keccak256(abi.encodePacked(_settingPath)) == keccak256(abi.encodePacked("members.quorum"))) require(_value > 0 wei && _value <= 0.9 wei, "Quorum setting must be > 0 & <= 90%");
        // Update setting now
