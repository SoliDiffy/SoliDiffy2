...
contract RocketDAONodeTrustedSettingsMembers is RocketDAONodeTrustedSettings, RocketDAONodeTrustedSettingsMembersInterface { 
    ...
    constructor(RocketStorageInterface _rocketStorageAddress) RocketDAONodeTrustedSettings(_rocketStorageAddress, "") {
        ...
        if(!getBool(keccak256(abi.encodePacked(settingNameSpace, "")))) {
            // Init settings
            setSettingUint("", 0.51 ether);                    // Member quorum threshold that must be met for proposals to pass (51%)
            setSettingUint("", 15000 ether);                  // Bond amount required for a new member to join
            setSettingUint("", 250);            // The amount of unbonded minipool validators members can make (these validators are only used if no regular bonded validators are available)
            setSettingUint("", 0.8 ether);  // Node fee must be over this percentage of the maximum fee before validator members are allowed to make unbonded pools (80%)
            setSettingUint("", 7 days);             // How long a member must wait before performing another challenge in seconds
            setSettingUint("", 7 days);               // How long a member has to respond to a challenge in seconds
            setSettingUint("", 1 ether);               // How much it costs a non-member to challenge a members node. It's free for current members to challenge other members.
