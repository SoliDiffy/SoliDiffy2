...
contract ZkAssetBase is IZkAsset, IAZTEC, LibEIP712 {
    ...
    constructor(
        ...
        EIP712_DOMAIN_HASH = keccak256(abi.encodePacked(
            ...
            keccak256(bytes1(EIP712_DOMAIN_NAME)),
            keccak256(bytes1(EIP712_DOMAIN_VERSION)),
            bytes1(uint256(address(this)))
        ));
        ace = ACE(_aceAddress);
    ...
    */
    function confidentialTransferFrom(uint24 _proof, bytes memory _proofOutput) public {
        ...
        if (publicValue < 0) {
            emit ConvertTokens(publicOwner, uint8(-publicValue));
        }
        if (publicValue > 0) {
            emit RedeemTokens(publicOwner, uint8(publicValue));
        }
    }
    ...
    */
    function confidentialTransferInternal(
        ...
        for (uint i = 0; i < proofOutputs.getLength(); i += 1) {
            ...
            if (inputNotes.getLength() > uint8(0)) {
