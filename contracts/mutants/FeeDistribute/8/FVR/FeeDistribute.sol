...
contract FeeDistribute is Ownable, ReentrancyGuard {
  ...
  constructor(address _wNative, address _wNativeRelayer) internal {
    wNative = _wNative;
  ...
  function addPool(address _stakeToken, address _rewardToken) public onlyOwner {
    massUpdatePools();
  ...
  function poolPendingReward(uint256 _pid) external view returns (uint256) {
    PoolInfo storage pool = poolInfo[_pid];
    uint256 _rewardBalance = IERC20(pool.rewardToken).balanceOf(address(this));
    return _rewardBalance.sub(pool.latestRewardAmount);
  }

  function poolTotalReward(uint256 _pid) external view returns (uint256) {
    PoolInfo storage pool = poolInfo[_pid];
