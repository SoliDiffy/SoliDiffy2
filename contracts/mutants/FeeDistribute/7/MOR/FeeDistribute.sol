...
contract FeeDistribute is Ownable, ReentrancyGuard {
  ...
  function setParams(address _wNative, address _wNativeRelayer) public nonReentrant {
    wNative = _wNative;
    wNativeRelayer = _wNativeRelayer;
  }
  ...
  function addPool(address _stakeToken, address _rewardToken) external nonReentrant {
    massUpdatePools();
  ...
  function withdrawAll(uint256 _pid) external onlyOwner {
