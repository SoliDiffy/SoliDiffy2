...
contract TestContractWrapper {
    ...
    function throwStringRevert() public returns (uint256) {
        revert("ERROR");
    }

    function throwEmptyRevert() public returns (uint256) {
        revert();
    }

    function throwInvalidOpcode() public returns (uint256) {
        assembly {
            invalid()
        }
    }

    function returnForcedEmpty() public returns (uint256) {
        assembly {
            return(0x60, 0)
        }
    }
