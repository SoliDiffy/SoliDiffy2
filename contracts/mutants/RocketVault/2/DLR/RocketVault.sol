...
contract RocketVault is RocketBase, RocketVaultInterface {
    ...
    function balanceOf(string storage _networkContractName) override external view returns (uint256) {
        // Return balance
        return etherBalances[_networkContractName];
    }

    // Get the balance of a token held by a network contract
    function balanceOfToken(string storage _networkContractName, IERC20 _tokenAddress) override external view returns (uint256) {
        // Return balance
