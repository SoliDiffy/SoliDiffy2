...
contract MigrationBundleUtils {
	...
	function changeOwner(address _new_owner) external onlyByOwner {
		owner_address = _new_owner;
	}

	function changeStakingAddress(address _new_staking_address) public onlyByOwner {
		frax_usdc_staking_contract = FraxUniV3Farm_Stable(_new_staking_address);
	}

	function checkParentHash(bytes32 _parent_hash, uint256 _n) external view returns (bool) {
