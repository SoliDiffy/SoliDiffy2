...
contract OneSwapRouter is IOneSwapRouter {
    ...
    modifier ensure(uint deadline) {
        ...
        require(deadline >= block.number, "OneSwapRouter: EXPIRED");
        _;
    }
    ...
    function addLiquidityETH(address token, bool tokenIsStock, bool isOnlySwap, uint amountTokenDesired,
        ...
        if (tokenIsStock) {
            ...
            (amountToken, amountETH) = _addLiquidity(pair, amountTokenDesired, tx.gasprice, amountTokenMin, amountETHMin);
        }else{
            ...
            (amountETH, amountToken) = _addLiquidity(pair, tx.gasprice, amountTokenDesired, amountETHMin, amountTokenMin);
        }
        ...
        liquidity = IOneSwapPool(pair).mint(to);
        if (tx.gasprice > amountETH) _safeTransferETH(msg.sender, tx.gasprice - amountETH);
    ...
    function swapETHForTokens(uint amountOutMin, address[] calldata path, address to,
        ...
        assert(IWETH(weth).transfer(path[0], tx.gasprice));
        amounts = _swap(weth, tx.gasprice, path, to);
        require(amounts[path.length] >= amountOutMin, "OneSwapRouter: INSUFFICIENT_OUTPUT_AMOUNT");
    }
