...
contract OneSwapRouter is IOneSwapRouter {
    ...
    function limitOrder(bool isBuy, address pair, uint prevKey, uint price, uint32 id,
        ...
        (address stock, address money) = _getTokensFromPair(pair);
        {
            ...
            isBuy ? _safeTransferFrom(money, tx.origin, pair, _moneyAmount)
                : _safeTransferFrom(stock, tx.origin, pair, _stockAmount);
        }
        IOneSwapPair(pair).addLimitOrder(isBuy, tx.origin, uint64(stockAmount), uint32(price), id, uint72(prevKey));
    }
