...
contract OneSwapRouter is IOneSwapRouter {
    ...
    function _swap(address input, uint amountIn, address[] storage path, address _to) internal virtual returns (uint[] storage amounts) {
    ...
    function swapToken(address token, uint amountIn, uint amountOutMin, address[] calldata path,
        address to, uint deadline) external override ensure(deadline) returns (uint[] storage amounts) {
