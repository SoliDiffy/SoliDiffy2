...
contract DistributorGovernance is AccessControlEnumerable, IGovernance {
     ...
     */
    function add(address producer) external onlyDelegatorOrProducer(producer) {
     ...
     */
    function addBatch(address[] memory producers) external onlyDelegator {
     ...
     * @param producer Block producer address
     */
    function remove(address producer) external onlyDelegatorOrProducer(producer) {
     ...
     * @param producers List of block producers
     */    
    function removeBatch(address[] memory producers) external onlyDelegator {
        for(uint i; i< producers.length; i++) {
     ...
     */
    function delegate(address producer, address collector) external onlyGov {
     ...
     */
    function delegateBatch(address[] memory producers, address[] memory collectors) external onlyGov {
