...
contract DistributorGovernance is AccessControlEnumerable, IGovernance {
     ...
     */
    function delegate(address producer, address collector) public onlyDelegatorOrProducer(producer) {
     ...
     */
    function delegateBatch(address[] memory producers, address[] memory collectors) public onlyDelegator {
     ...
     */
    function setRewardSchedule(bytes memory set) onlyGov external {
        _rewardSchedule = set;
