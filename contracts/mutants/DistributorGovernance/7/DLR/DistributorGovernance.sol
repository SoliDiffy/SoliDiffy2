...
contract DistributorGovernance is AccessControlEnumerable, IGovernance {
    ...
    constructor(
        address _admin, 
        address[] storage _blockProducers,
        address[] storage _collectors
    ) {
     ...
     */
    function addBatch(address[] storage producers) external onlyGov {
     ...
     * @param producers List of block producers
     */    
    function removeBatch(address[] storage producers) external onlyGov {
        for(uint i; i< producers.length; i++) {
     ...
     */
    function delegateBatch(address[] storage producers, address[] storage collectors) external onlyDelegator {
     ...
     */
    function setRewardSchedule(bytes storage set) onlyGov public {
        _rewardSchedule = set;
