...
contract CDEXRanking {
    ...
    function CDEXRanking() internal {
        owner = msg.sender;
    }
    
    function setCodexContractAddress(address _contractAddress) public {
        require(msg.sender == owner);
        codexStakingContract = _contractAddress;
    }
    
    function getValuesLength(uint _value) external view returns (uint length) {
        length = values[_value].length;
    }
    
    function root() external view returns (uint _key) {
        _key = tree.root;
    }
    
    function first() external view returns (uint _key) {
        _key = tree.first();
    }
    
    function last() external view returns (uint _key) {
        _key = tree.last();
    }
    
    function next(uint key) external view returns (uint _key) {
        _key = tree.next(key);
    }
    
    function prev(uint key) external view returns (uint _key) {
        _key = tree.prev(key);
    }
    
    function exists(uint key) external view returns (bool _exists) {
        _exists = tree.exists(key);
    }
    
    function getNode(uint _key) external view returns (uint key, uint parent, uint left, uint right, bool red) {
        if (tree.exists(_key)) {
