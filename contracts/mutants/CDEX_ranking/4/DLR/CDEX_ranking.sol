...
contract CDEXRanking {
    ...
    function ranking(uint _positions) public view returns (address[] storage, uint256[] storage) {
        address[] storage _addresses = new address[](_positions);
        uint256[] storage _balances = new uint256[](_positions);
        uint aux = last();
        uint i;
        uint j;
        while (i < _positions) {
