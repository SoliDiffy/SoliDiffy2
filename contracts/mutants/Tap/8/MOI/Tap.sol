...
contract Tap is TimeHelpers, EtherTokenConstant, IsContract, AragonApp {
    ...
    */
    ...
    function _updateController(IAragonFundraisingController _controller) internal onlyInit {
        controller = _controller;

        emit UpdateController(address(_controller));
    }

    function _updateReserve(Vault _reserve) internal onlyInit {
        reserve = _reserve;

        emit UpdateReserve(address(_reserve));
    }

    function _updateBeneficiary(address _beneficiary) internal onlyInit {
        beneficiary = _beneficiary;

        emit UpdateBeneficiary(_beneficiary);
    }

    function _updateMaximumTapRateIncreasePct(uint256 _maximumTapRateIncreasePct) internal onlyInit {
        maximumTapRateIncreasePct = _maximumTapRateIncreasePct;

        emit UpdateMaximumTapRateIncreasePct(_maximumTapRateIncreasePct);
    }

    function _updateMaximumTapFloorDecreasePct(uint256 _maximumTapFloorDecreasePct) internal onlyInit {
        maximumTapFloorDecreasePct = _maximumTapFloorDecreasePct;

        emit UpdateMaximumTapFloorDecreasePct(_maximumTapFloorDecreasePct);
    }

    function _addTappedToken(address _token, uint256 _rate, uint256 _floor) internal onlyInit {
        /**
         * NOTE
    ...
    function _removeTappedToken(address _token) internal onlyInit {
        delete rates[_token];
    ...
    function _updateTappedToken(address _token, uint256 _rate, uint256 _floor) internal onlyInit {
        /**
         * NOTE
