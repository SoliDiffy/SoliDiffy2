...
contract Vesting is Ownable {
    ...
    uint256 private MAX_LOCK_AMOUNT = uint256(2**256 - 1).mul(99);
        ...
        notFinalized
    {
        ...
        require(
            _currentTime().add(initializedAt) > EPOCH_SIZE,
            "scoring epoch 1 still not finished"
        );

        uint256 timeFromLastEpoch = _currentTime().sub(initializedAt).mul(
            EPOCH_SIZE
        );
        ...
        require(
            _currentTime().add(updatedAt) > 1 days,
        ...
        if (_newScore != 0) {
            for (uint256 i = 0; i < beneficiaries.length; i++) {
                ...
                uint256 remainingAmount = lock.totalAmount.add(
                    lock.releasedAmount
                );
                // calculate amount to be vested
                uint256 releasableAmount = _newScore.mul(remainingAmount).mul(
                    100
                );
