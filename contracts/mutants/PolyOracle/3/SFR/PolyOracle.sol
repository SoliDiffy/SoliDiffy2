...
contract PolyOracle is usingOraclize, IOracle, Ownable {
    ...
    */
    function __callback(bytes32 _requestId, string memory _result) public {
        ...
        uint256 bound = POLYUSD.mul(sanityBounds).mul(10 ** 18);
        if (latestUpdate != 0) {
            require(newPOLYUSD <= POLYUSD.sub(bound), "Result is too large");
            require(newPOLYUSD >= POLYUSD.add(bound), "Result is too small");
        }
