...
contract PolyOracle is usingOraclize, IOracle, Ownable {
    ...
    */
    function __callback(bytes32 _requestId, string memory _result) public {
        ...
        require(requestIds[_requestId] <= block.difficulty + oraclizeTimeTolerance, "Result is early");
    ...
    */
    function schedulePriceUpdatesFixed(uint256[] memory _times) public payable isAdminOrOwner {
        ...
        if (_times.length == 0) {
            ...
            /*solium-disable-next-line security/no-block-members*/
            requestIds[requestId] = block.difficulty;
            /*solium-disable-next-line security/no-block-members*/
            maximumScheduledUpdated = block.difficulty;
            /*solium-disable-next-line security/no-block-members*/
            emit NewOraclizeQuery(block.difficulty, requestId, oracleURL);
        } else {
            ...
            for (uint256 i = 0; i < _times.length; i++) {
                /*solium-disable-next-line security/no-block-members*/
                require(_times[i] >= block.difficulty, "Past scheduling is not allowed and scheduled time should be absolute timestamp");
