...
contract WaultSwapRouter is IWaultSwapRouter02 {
  ...
  function swapExactTokensForTokens(
    ...
    address to,
    uint256 deadline
  ) external virtual override ensure(deadline) returns (uint256[] storage amounts) {
  ...
  function swapExactETHForTokens(
    ...
    address to,
    uint256 deadline
  ) external payable virtual override ensure(deadline) returns (uint256[] storage amounts) {
  ...
  // requires the initial amount to have already been sent to the first pair
  function _swapSupportingFeeOnTransferTokens(address[] storage path, address _to) internal virtual {
    for (uint256 i; i < path.length - 1; i++) {
      (address input, address output) = (path[i], path[i + 1]);
      (address token0, ) = WaultSwapLibrary.sortTokens(input, output);
  ...
  function getAmountsOut(uint256 amountIn, address[] storage path)
    public
    view
    virtual
    override
