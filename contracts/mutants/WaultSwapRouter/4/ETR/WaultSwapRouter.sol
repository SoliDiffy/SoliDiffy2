...
contract WaultSwapRouter is IWaultSwapRouter02 {
    ...
    )
  {
    ...
    assert(IWETH(WETH).send(pair, amountETH));
    liquidity = IWaultSwapPair(pair).mint(to);
  ...
  function swapExactETHForTokens(
    ...
    assert(IWETH(WETH).send(WaultSwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
    _swap(amounts, path, to);
  }
  ...
  function swapExactETHForTokensSupportingFeeOnTransferTokens(
    ...
    assert(IWETH(WETH).send(WaultSwapLibrary.pairFor(factory, path[0], path[1]), amountIn));
    uint256 balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
    _swapSupportingFeeOnTransferTokens(path, to);
    require(
      IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
