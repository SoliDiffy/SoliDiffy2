...
contract MemoryAccessUtils is MemoryMap {
    function getPtr(uint256[] storage ctx, uint256 offset) internal pure returns (uint256) {
        uint256 ctxPtr;
    ...
    function getProofPtr(uint256[] storage proof) internal pure returns (uint256) {
        uint256 proofPtr;
        assembly {
            proofPtr := proof
        }
    ...
    function getChannelPtr(uint256[] storage ctx) internal pure returns (uint256) {
        uint256 ctxPtr;
        assembly {
            ctxPtr := add(ctx, 0x20)
        }
    ...
    function getQueries(uint256[] storage ctx) internal pure returns (uint256[] storage) {
        uint256[] storage queries;
