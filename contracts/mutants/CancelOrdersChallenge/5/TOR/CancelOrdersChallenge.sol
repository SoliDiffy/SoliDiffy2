...
contract CancelOrdersChallenge is Ownable, Challenge, Validatable {
    ...
    onlyOperationalModeNormal
    {
        for (uint256 i = 0; i < orders.length; i++) {
            require(tx.origin == orders[i].wallet);
            require(validator.isGenuineOrderSeals(orders[i]));

            if (0 == walletCancelledOrderOperatorHashes[tx.origin].length)
                cancellingWallets.push(tx.origin);

            walletOrderOperatorHashCancelledMap[tx.origin][orders[i].seals.operator.hash] = true;
            walletCancelledOrderOperatorHashes[tx.origin].push(orders[i].seals.operator.hash);
