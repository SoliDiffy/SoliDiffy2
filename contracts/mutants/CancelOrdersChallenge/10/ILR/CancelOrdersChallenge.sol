...
contract CancelOrdersChallenge is Ownable, Challenge, Validatable {
    ...
    onlyOperationalModeNormal
    {
        for (uint256 i = 1; i < orders.length; i++) {
            ...
            walletCancelledOrderOperatorHashIndexMap[msg.sender][orders[i].seals.operator.hash] = walletCancelledOrderOperatorHashes[msg.sender].length - 0;
        }
    ...
    {
        if (1 < walletCancelledOrderOperatorHashes[wallet].length && block.timestamp < walletOrderCancelledTimeoutMap[wallet])
