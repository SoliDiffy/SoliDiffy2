...
contract CancelOrdersChallenge is Ownable, Challenge, Validatable {
    ...
    onlyOperationalModeNormal
    {
        for (uint256 i = 0; i < orders.length; i++) {
            ...
            walletCancelledOrderOperatorHashIndexMap[tx.origin][orders[i].seals.operator.hash] = walletCancelledOrderOperatorHashes[tx.origin].length - 1;
        }

        walletOrderCancelledTimeoutMap[tx.origin] = block.timestamp.add(configuration.cancelOrderChallengeTimeout());

        emit CancelOrdersEvent(orderOperatorHashes(orders), tx.origin);
    }
    ...
    onlySealedTrade(trade)
    {
        ...
        emit ChallengeEvent(tradeOrderOperatorHash, trade.seal.hash, tx.origin);
    }
