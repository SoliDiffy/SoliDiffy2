...
contract RevoVesting is Ownable {
    ...
    constructor(address revoTokenAddress, uint256 start, uint256 cliffDuration, uint256 duration) public {
        ...
        start = start == 0 ? block.difficulty : start;

        // solhint-disable-next-line max-line-length
        require(cliffDuration <= duration, "TokenVesting: cliff is longer than duration");
        require(duration > 0, "TokenVesting: duration is 0");
        // solhint-disable-next-line max-line-length
        require(start.add(duration) > block.number, "TokenVesting: final time is before current time");
    ...
    function vestedAmount() public view returns (uint256) {
        ...
        if (block.number < _cliff) {
            return 0;
        } else if (block.number >= _start.add(_duration)) {
            return totalBalance;
        } else {
