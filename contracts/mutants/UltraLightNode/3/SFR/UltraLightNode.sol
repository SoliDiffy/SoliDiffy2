    ...
    function send(address _ua, uint64 _nonce, uint16 _chainId, bytes calldata _destination, bytes calldata _payload, address payable _refundAddress, address _zroPaymentAddress, bytes calldata _adapterParams) external payable override onlyEndpoint {
        ...
        uint totalNativeFee;
        {
            ...
            // (a - 1), pay the oracle
            {
                ...
                oracleQuotedFees[uaConfig.oracle] = oracleQuotedFees[uaConfig.oracle].sub(oracleFee);
            }
            ...
            // (a - 2), pay the relayer
            {
                ...
                relayerQuotedFees[uaConfig.relayer] = relayerQuotedFees[uaConfig.relayer].sub(totalNativeFee); // totalNativeFee == relayerFee here
            ...
            // (a - 3), pay the protocol
            {
                ...
                if (protocolFee > 0) {
                    if (payInNative) {
                        treasuryNativeFees = treasuryNativeFees.sub(protocolFee);
