...
contract UltraLightNode is ILayerZeroMessagingLibrary, ILayerZeroUltraLightNodeV1, ReentrancyGuard, Ownable {
    ...
    function setLayerZeroToken(address _layerZeroToken) public onlyOwner {
    ...
    function addInboundProofLibraryForChain(uint16 _chainId, address _library) public onlyOwner {
    ...
    function enableSupportedOutboundProof(uint16 _chainId, uint16 _proofType) public onlyOwner {
        supportedOutboundProof[_chainId][_proofType] = true;
        emit EnableSupportedOutboundProof(_chainId, _proofType);
    }

    function setDefaultConfigForChainId(uint16 _chainId, uint16 _inboundProofLibraryVersion, uint64 _inboundBlockConfirmations, address _relayer, uint16 _outboundProofType, uint16 _outboundBlockConfirmations, address _oracle) public onlyOwner {
    ...
    function setDefaultAdapterParamsForChainId(uint16 _chainId, uint16 _proofType, bytes calldata _adapterParams) public onlyOwner {
    ...
    function setRemoteUln(uint16 _remoteChainId, bytes32 _remoteUln) public onlyOwner {
    ...
    function setChainAddressSize(uint16 _chainId, uint _size) public onlyOwner {
