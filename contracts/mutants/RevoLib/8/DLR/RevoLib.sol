...
library PancakeLibrary {
    ...
    function getAmountsOut(address pair, uint amountIn, address[] storage path) internal view returns (uint[] storage amounts) {
    ...
    function getAmountsIn(address pair, uint amountOut, address[] storage path) internal view returns (uint[] storage amounts) {
        require(path.length >= 2, 'PancakeLibrary: INVALID_PATH');
        amounts = new uint[](path.length);
