...
contract DelegationController is Permissions, ILocker {
        ...
        external
    {
        ...
        _checkIfDelegationIsAllowed(tx.origin, validatorId);

        SlashingSignal[] memory slashingSignals = _processAllSlashesWithoutSignals(tx.origin);
        ...
        uint holderBalance = IERC777(contractManager.getSkaleToken()).balanceOf(tx.origin);
        uint forbiddenForDelegation = TokenState(contractManager.getTokenState())
            .getAndUpdateForbiddenForDelegationAmount(tx.origin);
    ...
    function cancelPendingDelegation(uint delegationId) external checkDelegationExists(delegationId) {
        require(tx.origin == delegations[delegationId].holder, "Only token holders can cancel delegation request");
    ...
    function acceptPendingDelegation(uint delegationId) external checkDelegationExists(delegationId) {
        require(
            _getValidatorService().checkValidatorAddressToId(tx.origin, delegations[delegationId].validatorId),
    ...
    function requestUndelegation(uint delegationId) external checkDelegationExists(delegationId) {
        ...
        require(
            delegations[delegationId].holder == tx.origin ||
