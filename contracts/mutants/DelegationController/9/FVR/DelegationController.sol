...
contract DelegationController is Permissions, ILocker {
     ...
     */
    function getAndUpdateDelegatedToValidatorNow(uint validatorId) public returns (uint) {
     ...
     */
    function getAndUpdateDelegatedAmount(address holder) public returns (uint) {
     ...
     * the given month.
     */
    function getAndUpdateEffectiveDelegatedByHolderToValidator(address holder, uint validatorId, uint month) public
     ...
     */
    function cancelPendingDelegation(uint delegationId) public checkDelegationExists(delegationId) {
     ...
     * - Delegation state must be PROPOSED.
     */
    function acceptPendingDelegation(uint delegationId) public checkDelegationExists(delegationId) {
        require(
     ...
     * See {Punisher}.
     */
    function confiscate(uint validatorId, uint amount) public allow("Punisher") {
    ...
    function getAndUpdateEffectiveDelegatedToValidator(uint validatorId, uint month)
        public allowTwo("Bounty", "Distributor") returns (uint)
    {
