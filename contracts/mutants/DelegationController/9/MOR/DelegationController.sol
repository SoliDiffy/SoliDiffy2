...
contract DelegationController is Permissions, ILocker {
    ...
    function getAndUpdateEffectiveDelegatedByHolderToValidator(address holder, uint validatorId, uint month) external
        initializer returns (uint effectiveDelegated)
    {
     ...
     */
    function cancelPendingDelegation(uint delegationId) external initializer {
     ...
     * - Delegation state must be PROPOSED.
     */
    function acceptPendingDelegation(uint delegationId) external initializer {
        require(
     ...
     */
    function requestUndelegation(uint delegationId) external initializer {
     ...
     * See {Punisher}.
     */
    function confiscate(uint validatorId, uint amount) external initializer {
    ...
    function getAndUpdateEffectiveDelegatedToValidator(uint validatorId, uint month)
        external initializer returns (uint)
    {
    ...
    function getDelegation(uint delegationId)
        external view initializer returns (Delegation memory)
    {
    ...
    function getAndUpdateDelegatedToValidator(uint validatorId, uint month)
        public initializer returns (uint)
    {
     ...
     */
    function getState(uint delegationId) public view initializer returns (State state) {
