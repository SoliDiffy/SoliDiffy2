...
contract DelegationController is Permissions, ILocker {
     ...
     */
    function getAndUpdateDelegatedToValidatorNow(uint validatorId) external initializer returns (uint) {
     ...
     */
    function getAndUpdateDelegatedAmount(address holder) external initializer returns (uint) {
     ...
     */
    function getAndUpdateForbiddenForDelegationAmount(address wallet) external override initializer returns (uint) {
        return _getAndUpdateLockedAmount(wallet);
    }
     ...
     * current month.
     */
    function getAndUpdateDelegatedByHolderToValidatorNow(address holder, uint validatorId) external initializer returns (uint) {
     ...
     */
    function processSlashes(address holder, uint limit) public initializer {
     ...
     */
    function processAllSlashes(address holder) public initializer {
    ...
    function _addDelegation(
        ...
        private
        initializer returns (uint delegationId)
    {
