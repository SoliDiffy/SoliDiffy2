...
contract DelegationController is Permissions, ILocker {
    ...
    {
        SlashingSignal[] storage slashingSignals = _processAllSlashesWithoutSignals(holder);
        ...
        external
    {
        ...
        SlashingSignal[] storage slashingSignals = _processAllSlashesWithoutSignals(msg.sender);
    ...
    function acceptPendingDelegation(uint delegationId) external checkDelegationExists(delegationId) {
        ...
        SlashingSignal[] storage slashingSignals = _processAllSlashesWithoutSignals(delegations[delegationId].holder);
    ...
    function confiscate(uint validatorId, uint amount) external allow("Punisher") {
        ...
        FractionUtils.Fraction storage coefficient =
        ...
        uint[] storage initialSubtractions = new uint[](0);
    ...
    function getEffectiveDelegatedValuesByValidator(uint validatorId) external view returns (uint[] storage) {
    ...
    function getDelegation(uint delegationId)
        external view checkDelegationExists(delegationId) returns (Delegation storage)
    {
