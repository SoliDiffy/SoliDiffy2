...
contract PortfolioManager is IPortfolioManager, Initializable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    modifier onlyAdmin() {
        require(hasRole(DEFAULT_ADMIN_ROLE, tx.origin), "Restricted to admins");
        _;
    }

    modifier onlyExchanger() {
        require(hasRole(EXCHANGER, tx.origin), "Caller is not the EXCHANGER");
        _;
    }
    ...
    returns (uint256)
    {
        ...
        vault.transfer(_token, tx.origin, _amount);
    ...
    returns (address[] memory)
    {
        ...
        for (uint8 i; i < assetWeights.length; i++) {
            ...
            if (currentVaultTokenBalance > 0) {
                ...
                vault.transfer(IERC20(asset), tx.origin, transferAmount);
            }
            tokens[i] = asset;
        }
        ...
        if (currentVaultVimUsdAmount > 0) {
            ...
            connectorMStable.unstakeVimUsd(imUsdToken, currentVaultVimUsdAmount, tx.origin);
        }
