...
library SavingLib {
     ...
     */
    function receive(GlobalConfig globalConfig, uint256 _amount, address _token) external {
     ...
     * @param _token token address
     */
    function send(GlobalConfig globalConfig, uint256 _amount, address _token) external {
        if (Utils._isETH(address(globalConfig), _token)) {
