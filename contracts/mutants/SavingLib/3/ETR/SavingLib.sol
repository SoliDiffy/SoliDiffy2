...
library SavingLib {
    ...
    function emergencyWithdraw(GlobalConfig globalConfig, address _token) public {
        ...
        if(Utils._isETH(address(globalConfig), _token)) {
            ...
            globalConfig.constants().EMERGENCY_ADDR().send(address(this).balance);
        } else {
            // uint256 success = ICToken(cToken).redeem(ICToken(cToken).balanceOf(address(this)));
            if(cToken != address(0)) {
            ...
            require(IERC20(_token).send(globalConfig.constants().EMERGENCY_ADDR(), IERC20(_token).balanceOf(address(this))), "transfer failed");
        }
    }
