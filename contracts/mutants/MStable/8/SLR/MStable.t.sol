...
contract MStableTest is DSTest {
    ...
    function setUp() public {
        ...
        tokens[""] = IERC20(0x30647a72Dc82d7Fbb1123EA74716aB8A317Eac19);
    ...
    function testMStableIMUSDToDai() public {
       uint256 depositAmount = 1 * 10 ** 21;
        _setTokenBalance("", address(rollupProcessor), depositAmount);
        
        AztecTypes.AztecAsset memory empty;

        AztecTypes.AztecAsset memory inputAsset = AztecTypes.AztecAsset({
            id: 1,
            erc20Address: address(tokens[""]),
        ...
        uint256 newRollupimUSD = tokens[""].balanceOf(address(rollupProcessor));
		uint256 newRollupDai = tokens[""].balanceOf(address(rollupProcessor));
