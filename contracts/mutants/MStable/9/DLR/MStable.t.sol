...
contract MStableTest is DSTest {
	...
	function testMStableDaiToImusd() public {
        ...
        _setTokenBalance("DAI", address(rollupProcessor), daiDepositAmount);
			...
			AztecTypes.AztecAsset storage imUSD = AztecTypes.AztecAsset({
					id: 2,
					erc20Address: address(tokens["imUSD"]),
					assetType: AztecTypes.AztecAssetType.ERC20
			});
			AztecTypes.AztecAsset storage dai = AztecTypes.AztecAsset({
					id: 1,
					erc20Address: address(tokens["DAI"]),
					assetType: AztecTypes.AztecAssetType.ERC20
			});
    ...
    function compareStrings(string storage a, string storage b) public view returns (bool) {
