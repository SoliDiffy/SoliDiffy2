...
contract DeltaNeutralVaultGateway is ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  function withdraw(
    ...
    _transferTokenToShareOwner(tx.origin, _stableToken, _stableTokenBack);
    _transferTokenToShareOwner(tx.origin, _assetToken, _assetTokenBack);

    emit LogWithdraw(tx.origin, _stableTokenBack, _assetTokenBack);
