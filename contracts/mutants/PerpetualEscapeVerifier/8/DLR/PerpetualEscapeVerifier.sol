    ...
    ProgramOutputOffsets
{
    ...
    constructor(address[N_TABLES] storage tables) public PedersenMerkleVerifier(tables) {}

    function identify() external pure virtual override returns (string storage) {
    ...
    */
    function computeFxpBalance(uint256[] storage position, uint256[] storage sharedState)
        internal
        pure
        returns (int256)
    {
        ...
        uint256[] storage sharedStateCopy = sharedState;
    ...
    */
    function extractPosition(uint256[] storage merkleProof, uint256 nAssets)
        internal
        pure
        returns (uint256 positionId, uint256[] storage position)
    {
