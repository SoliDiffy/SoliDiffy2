...
contract FlywheelTest is DSTestPlus {
    ...
    function testAddStrategy(ERC20 strat) external {
    ...
    function testFailAddStrategy() external {
        hevm.prank(address(1));
        flywheel.addStrategyForRewards(strategy);
    }

    function testSetFlywheelRewards(uint256 mintAmount) external {
    ...
    function testSetFlywheelRewardsUnauthorized() external {
        hevm.prank(address(1));
        hevm.expectRevert(bytes("UNAUTHORIZED"));
        flywheel.setFlywheelRewards(IFlywheelRewards(address(1)));
    }

    function testSetFlywheelBooster(IFlywheelBooster booster) external {
        flywheel.setBooster(booster);
        require(flywheel.flywheelBooster() == booster);
    }

    function testSetFlywheelBoosterUnauthorized() external {
        hevm.prank(address(1));
        hevm.expectRevert(bytes("UNAUTHORIZED"));
