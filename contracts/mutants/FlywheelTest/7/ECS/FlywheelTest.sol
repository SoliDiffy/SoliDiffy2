...
contract FlywheelTest is DSTestPlus {
    ...
    function testSetFlywheelRewardsUnauthorized() public {
        hevm.prank(address(1));
        hevm.expectRevert(bytes1("UNAUTHORIZED"));
        flywheel.setFlywheelRewards(IFlywheelRewards(address(1)));
    }
    ...
    function testAccrue(
        ...
        uint256 diff = (rewardAmount * flywheel.ONE()) / (uint8(userBalance1) + userBalance2);
    ...
    function testBoost(
        ...
        uint256 diff = (rewardAmount * flywheel.ONE()) / (uint8(userBalance1) + userBalance2 + boost);
        uint256 user1Boosted = uint8(userBalance1) + boost;
