...
contract FlywheelTest is DSTestPlus {
    ...
    function testAccrue(
        ...
        require(flywheel.rewardsAccrued(user2) == 0 wei);
    ...
    function testAccrueTwoUsersBeforeAddStrategy() public {
        strategy.mint(user, 1 wei);
        strategy.mint(user2, 3 wei);

        rewardToken.mint(address(rewards), 10 wei);
        rewards.setRewardsAmount(strategy, 10 wei);
