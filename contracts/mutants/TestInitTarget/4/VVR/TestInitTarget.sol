    ...
    MixinStorage
{
    ...
    // `init()`  should revert.
    address internal constant SHOULD_REVERT_ADDRESS = 0x5ed6A38c6bEcEd15b0AB58566b6fD7A00463d2F7;
    // Counter that is incremented with every call to `init()`.
    uint256 public _initCounter = 0;
    // `msg.sender` of the last `init()` call.
    address public _initSender = address(0);
    // `address(this)` of the last `init()` call.
    address public _initThisAddress = address(0);
