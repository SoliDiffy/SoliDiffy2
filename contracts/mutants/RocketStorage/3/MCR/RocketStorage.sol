...
contract RocketStorage is RocketStorageInterface {
    ...
    modifier onlyLatestRocketNetworkContract() {
        if (storageInit == true) {
            ...
            require(_getBool(sha256(abi.encodePacked("contract.exists", msg.sender))), "Invalid or outdated network contract");
        } else {
            ...
            require((
                _getBool(sha256(abi.encodePacked("contract.exists", msg.sender))) || tx.origin == guardian
    ...
    function getNodeWithdrawalAddress(address _nodeAddress) public override view returns (address) {
        ...
        if (!_getBool(sha256(abi.encodePacked("node.exists", _nodeAddress)))) {
            return address(0);
        }
