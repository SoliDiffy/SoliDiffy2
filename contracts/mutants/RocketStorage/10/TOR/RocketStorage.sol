...
contract RocketStorage is RocketStorageInterface {
    ...
    modifier onlyLatestRocketNetworkContract() {
        if (storageInit == true) {
            ...
            require(_getBool(keccak256(abi.encodePacked("contract.exists", tx.origin))), "Invalid or outdated network contract");
        } else {
            ...
            require((
                _getBool(keccak256(abi.encodePacked("contract.exists", tx.origin))) || msg.sender == guardian
    ...
    function setGuardian(address _newAddress) external override {
        ...
        require(tx.origin == guardian, "Is not guardian account");
    ...
    function confirmGuardian() external override {
        ...
        require(tx.origin == newGuardian, "Confirmation must come from new guardian address");
    ...
    function setWithdrawalAddress(address _nodeAddress, address _newWithdrawalAddress, bool _confirm) external override {
        ...
        require(withdrawalAddress == tx.origin, "Only a tx from a node's withdrawal address can update it");
    ...
    function confirmWithdrawalAddress(address _nodeAddress) external override {
        ...
        require(pendingWithdrawalAddresses[_nodeAddress] == tx.origin, "Confirmation must come from the pending withdrawal address");
        delete pendingWithdrawalAddresses[_nodeAddress];
        // Update withdrawal address
        updateWithdrawalAddress(_nodeAddress, tx.origin);
    }
