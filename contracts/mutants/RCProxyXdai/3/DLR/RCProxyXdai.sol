...
contract RCProxyXdai is Ownable
{
    ...
    function postQuestionToBridge(address _marketAddress) public {
        ...
        bytes storage data = abi.encodeWithSelector(_methodSelector, _marketAddress, questions[_marketAddress].question, questions[_marketAddress].oracleResolutionTime);
    ...
    function postCardToUpgrade(uint256 _tokenId) public {
        ...
        bytes storage data = abi.encodeWithSelector(_methodSelector, _tokenId, upgradedNfts[_tokenId].tokenURI, upgradedNfts[_tokenId].owner);
        bridge.requireToPassMessage(proxyMainnetAddress,data,200000);
    }
    ...
    function confirmDaiDeposit(address _user, uint256 _amount, uint256 _nonce) external {
        ...
        if(deposits[_nonce].user == address(0)) {
            Deposit storage newDeposit = Deposit(_user, _amount, 0, false, false);
