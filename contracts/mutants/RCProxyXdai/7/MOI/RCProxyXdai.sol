...
contract RCProxyXdai is Ownable
{
    ...
    function addMarket(address _newMarket) external onlyOwner returns(bool) {
    ...
    function saveQuestion(address _marketAddress, string calldata _question, uint32 _oracleResolutionTime) external onlyOwner {
        require(msg.sender == factoryAddress, "Not factory");
    ...
    function postQuestionToBridge(address _marketAddress) public onlyOwner {
    ...
    function setWinner(address _marketAddress, uint256 _winningOutcome) external onlyOwner {
    ...
    function saveCardToUpgrade(uint256 _tokenId, string calldata _tokenUri, address _owner) external onlyOwner {
    ...
    function postCardToUpgrade(uint256 _tokenId) public onlyOwner {
    ...
    function confirmDaiDeposit(address _user, uint256 _amount, uint256 _nonce) external onlyOwner {
