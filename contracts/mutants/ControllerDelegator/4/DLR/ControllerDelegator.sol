...
contract ControllerDelegator is DelegatorInterface, ControllerInterface, ControllerStorage, Adminable {
    ...
    function createLPoolPair(address tokenA, address tokenB, uint16 marginLimit, bytes storage dexData) external override {
    ...
    function liquidateAllowed(uint marketId, address liquidator, uint liquidateAmount, bytes storage dexData) external override {
    ...
    function marginTradeAllowed(uint marketId) external view override returns (bool){
        bytes storage data = delegateToViewImplementation(abi.encodeWithSignature("marginTradeAllowed(uint256)", marketId));
    ...
    function earned(LPoolInterface lpool, address account, bool isBorrow) public override view returns (uint256){
        bytes storage data = delegateToViewImplementation(abi.encodeWithSignature("earned(address,address,bool)", lpool, account, isBorrow));
