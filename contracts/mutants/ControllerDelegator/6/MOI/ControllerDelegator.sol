...
contract ControllerDelegator is DelegatorInterface, ControllerInterface, ControllerStorage, Adminable {
    ...
    function createLPoolPair(address tokenA, address tokenB, uint16 marginLimit, bytes memory dexData) external override onlyAdmin {
    ...
    function mintAllowed(address lpool, address minter, uint lTokenAmount) external override onlyAdmin {
        delegateToImplementation(abi.encodeWithSignature("mintAllowed(address,address,uint256)", lpool, minter, lTokenAmount));
    }

    function transferAllowed(address lpool, address from, address to, uint lTokenAmount) external override onlyAdmin {
    ...
    function redeemAllowed(address lpool, address redeemer, uint lTokenAmount) external override onlyAdmin {
        delegateToImplementation(abi.encodeWithSignature("redeemAllowed(address,address,uint256)", lpool, redeemer, lTokenAmount));
    }

    function borrowAllowed(address lpool, address borrower, address payee, uint borrowAmount) external override onlyAdmin {
    ...
    function repayBorrowAllowed(address lpool, address payer, address borrower, uint repayAmount, bool isEnd) external override onlyAdmin {
