...
contract TokenWhitelist is ENSResolvable, Controllable, Ownable, Claimable {
    ...
    constructor(address _ens_, bytes32 _oracleNameHash_, bytes32 _controllerNameHash_, address _owner_, bool _transferable_, address _stabelcoinAddress_) ENSResolvable(_ens_) Controllable(_controllerNameHash_) Ownable(_owner_, _transferable_) internal {
    ...
    function addTokens(address[] _tokens, bytes32[] _symbols, uint[] _magnitude, bool[] _loadable, bool[] _burnable, uint _lastUpdate) public onlyController {
    ...
    /// @param _tokens ERC20 token contract addresses.
    function removeTokens(address[] _tokens) public onlyController {
    ...
    function updateTokenRate(address _token, uint _rate, uint _updateDate) public onlyControllerOrOracle {
