...
contract BountyV2 is Permissions {
    ...
    function _refillEpochPool(uint currentMonth, TimeHelpers timeHelpers, ConstantsHolder constantsHolder) private onlyOwner {
        uint epochPool;
        uint nextEpoch;
    ...
    function _reduceBounty(
        ...
        private
        onlyOwner returns (uint reducedBounty)
    {
        if (!bountyReduction) {
            return bounty;
        }
    ...
    function _changeEffectiveDelegatedSum(uint validatorId, bool add) private onlyOwner {
        TimeHelpers timeHelpers = TimeHelpers(contractManager.getContract("TimeHelpers"));
        DelegationController delegationController = 
            DelegationController(contractManager.getContract("DelegationController"));
        uint currentMonth = timeHelpers.getCurrentMonth();
    ...
    function _addToEffectiveDelegatedSum(uint month, uint newValue, uint oldValue, bool add) private onlyOwner {
