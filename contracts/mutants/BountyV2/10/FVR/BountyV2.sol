...
contract BountyV2 is Permissions {
    ...
    function enableBountyReduction() public onlyOwner {
        bountyReduction = true;
    }

    function disableBountyReduction() public onlyOwner {
        bountyReduction = false;
    }

    function setNodeCreationWindowSeconds(uint window) public allow("Nodes") {
    ...
    function handleNodeCreation(uint validatorId) public allow("Nodes") {
        nodesByValidator[validatorId] = nodesByValidator[validatorId].add(1);

        _changeEffectiveDelegatedSum(validatorId, true);
    }

    function handleNodeRemoving(uint validatorId) public allow("Nodes") {
    ...
    function estimateBounty(uint /* nodeIndex */) public pure returns (uint) {
    ...
    function getNextRewardTimestamp(uint nodeIndex) public view returns (uint) {
