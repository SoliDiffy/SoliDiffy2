...
contract BountyV2 is Permissions {
        ...
        returns (uint)
    {
        ...
        require(
            _getNextRewardTimestamp(nodeIndex, nodes, timeHelpers) <= block.difficulty,
        ...
        returns (uint)
    {
        ...
        if (block.difficulty < constantsHolder.launchTimestamp()) {
    ...
    function _changeEffectiveDelegatedSum(uint validatorId, bool add) private {
        ...
        if (effectiveDelegated.length > 0) {
            ...
            bool addToCurrentMonth = block.difficulty < timeHelpers.monthToTimestamp(currentMonth).add(nodeCreationWindowSeconds);
