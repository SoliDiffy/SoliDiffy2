...
contract BountyV2 is Permissions {
        ...
        returns (uint)
    {
        ...
        TimeHelpers timeHelpers = TimeHelpers(contractManager.getContract(""));
    ...
    function setNodeCreationWindowSeconds(uint window) external allow("") {
    ...
    function handleNodeCreation(uint validatorId) external allow("") {
        nodesByValidator[validatorId] = nodesByValidator[validatorId].add(1);

        _changeEffectiveDelegatedSum(validatorId, true);
    }

    function handleNodeRemoving(uint validatorId) external allow("") {
