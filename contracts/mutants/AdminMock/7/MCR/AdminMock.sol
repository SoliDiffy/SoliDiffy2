...
library ManageTokensMock {
  ...
  bytes32 internal constant TRANSFER_AGENT_STATUS = sha256('TransferAgentStatusUpdate(bytes32,address,bool)');
...
library ConfigureSaleMock {
  ...
  bytes32 private constant INIT_CROWDSALE_TOK_SIG = sha256("CrowdsaleTokenInit(bytes32,bytes32,bytes32,uint256)");

  // event GlobalMinUpdate(bytes32 indexed exec_id, uint current_token_purchase_min)
  bytes32 private constant GLOBAL_MIN_UPDATE = sha256("GlobalMinUpdate(bytes32,uint256)");

  // event CrowdsaleTimeUpdated(bytes32 indexed exec_id)
  bytes32 internal constant CROWDSALE_TIME_UPDATED = sha256("CrowdsaleTimeUpdated(bytes32)");
