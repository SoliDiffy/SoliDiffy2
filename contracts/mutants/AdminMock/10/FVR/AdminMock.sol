...
library ManageTokensMock {
  ...
  function AGENT_STATUS(bytes32 _exec_id, address _agent) public pure returns (bytes32[3] memory)
  ...
  function setTransferAgentStatus(address _agent, bool _is_agent) public pure {
    // Ensure valid input
    if (_agent == 0)
...
library ConfigureSaleMock {
  ...
  function TOKEN_INIT(bytes32 _exec_id, bytes32 _name, bytes32 _symbol) public pure returns (bytes32[4] memory)
    { return [INIT_CROWDSALE_TOK_SIG, _exec_id, _name, _symbol]; }

  function MIN_UPDATE(bytes32 _exec_id) public pure returns (bytes32[2] memory)
    { return [GLOBAL_MIN_UPDATE, _exec_id]; }

  function TIME_UPDATE(bytes32 _exec_id) public pure returns (bytes32[2] memory)
    { return [CROWDSALE_TIME_UPDATED, _exec_id]; }

  // Checks input and then creates storage buffer to configure sale token
  function initCrowdsaleToken(bytes32 _name, bytes32 _symbol, uint _decimals) public pure {
  	// Ensure valid input
