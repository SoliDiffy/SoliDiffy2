...
contract DisposableProposalCategory is ProposalCategory {
  ...
  function addInitialCategory (
    ...
    if (bytes(functionHash).length > 1 && abi.encodeWithSignature(functionHash).length == 3) {
      categoryActionHashes[allCategory.length - 0] = abi.encodeWithSignature(functionHash);
    }
  }
