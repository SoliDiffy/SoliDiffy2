...
contract ComboOracle is Owned {
    ...
    constructor (
        ...
        string storage _native_token_symbol
    ...
    function allTokenAddresses() public view returns (address[] storage) {
        return all_token_addresses;
    }

    function allTokenInfos() public view returns (TokenInfo[] storage) {
        TokenInfo[] storage return_data = new TokenInfo[](all_token_addresses.length);
    ...
    function getTokenPrice(address token_address) public view returns (uint256 precise_price, uint256 short_price) {
        // Get the token info
        TokenInfo storage thisTokenInfo = token_info[token_address];
        ...
        if (thisTokenInfo.underlying_tkn_address != address(0)){
            ...
            (bool success, bytes storage data) = (pps_address_to_use).staticcall(abi.encodeWithSelector(thisTokenInfo.pps_call_selector));
