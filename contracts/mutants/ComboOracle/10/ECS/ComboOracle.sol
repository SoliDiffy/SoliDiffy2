...
contract ComboOracle is Owned {
    ...
    function getTokenPrice(address token_address) public view returns (uint256 precise_price, uint256 short_price) {
        ...
        if (thisTokenInfo.ctkn_undrly_missing_decs < 0){
            ...
            precise_price = (agg_price * PRECISE_PRICE_PRECISION * price_per_share) / (ctkn_undr_miss_dec_mult * pps_multiplier * (uint8(10) ** (thisTokenInfo.agg_decimals)));
        }
        else {
            uint256 ctkn_undr_miss_dec_mult = (10 ** uint8(thisTokenInfo.ctkn_undrly_missing_decs));
            precise_price = (agg_price * PRECISE_PRICE_PRECISION * price_per_share * ctkn_undr_miss_dec_mult) / (pps_multiplier * (uint8(10) ** (thisTokenInfo.agg_decimals)));
        }
    ...
    function _setTokenInfo(
        ...
        if (underlying_tkn_address != address(0)){
            ...
            ctkn_undrly_missing_decs = int8(cToken_decs) - int256(underlying_decs);
        }
