...
// SWC-135-Code With No Effects: L71 - L81
    modifier onlyWhitelisted() {
        require(_whitelisteds[tx.origin] == true, "caller does not have the Whitelisted role");
        _;
    }
    ...
    modifier canTransfer() {
        if(paused){
            require (_whitelisteds[tx.origin] == true, "can't perform an action");
        }
        _;
    }
    ...
    function transfer(address recipient, uint256 amount) public canTransfer returns (bool) {
        ...
        require(accountBalance(tx.origin) >= amount);
