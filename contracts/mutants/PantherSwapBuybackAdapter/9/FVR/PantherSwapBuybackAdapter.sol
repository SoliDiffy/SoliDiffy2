...
contract PantherSwapBuybackAdapter is ReentrancyGuard, WhitelistGuard
{
	...
	constructor (address _sourceToken, address _targetToken,
		address _treasury, address _buyback, address _exchange) internal
	{
	 ...
	 */
	function pendingSource() public view returns (uint256 _totalSource)
	{
	 ...
	 */
	function pendingTarget() public view returns (uint256 _totalTarget)
	{
	 ...
	 *                        buyback contract.
	 */
	function gulp(uint256 _minTotalTarget) public onlyEOAorWhitelist nonReentrant
	{
		require(exchange != address(0), "exchange not set");
		uint256 _totalSource = Transfers._getBalance(sourceToken);
		uint256 _limitSource = _calcMaxRewardTransferAmount();
	 ...
	 */
	function recoverLostFunds(address _token) public onlyOwner
	{
	 ...
	 */
	function setBuyback(address _newBuyback) public onlyOwner
	{
	 ...
	 */
	function setExchange(address _newExchange) public onlyOwner
	{
	...
	function _calcMaxRewardTransferAmount() public view returns (uint256 _maxRewardTransferAmount)
	{
