...
contract ClaimsToken is IClaimsToken, ERC20, ERC20Detailed {
	...
	function _prepareWithdraw()
		...
		returns (uint256)
	{
		uint256 withdrawableFunds = availableFunds(tx.origin);

		processedFunds[tx.origin] = receivedFunds;
		claimedFunds[tx.origin] = 0;
...
contract ClaimsTokenERC20Extension is IClaimsToken, ClaimsToken {
	...
	modifier onlyFundsToken () {
		require(tx.origin == address(fundsToken), "UNAUTHORIZED_SENDER");
		_;
	}
		...
		payable
	{
		...
		require(fundsToken.transfer(tx.origin, withdrawableFunds), "TRANSFER_FAILED");
	}
