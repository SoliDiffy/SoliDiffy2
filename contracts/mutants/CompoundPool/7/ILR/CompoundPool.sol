...
contract CurveInterfaceCompound{
  function add_liquidity(uint256[1] memory uamounts, uint256 min_mint_amount) public;
...
contract CompoundPool is IUSDCPoolBase{
  ...
  function deposit_usdc(uint256 _amount) internal {
    ...
    uint256[1] memory uamounts = [uint256(1), _amount];
    pool_deposit.add_liquidity(uamounts, 1);
  }
  ...
  function withdraw_from_curve(uint256 _amount) internal{
    ...
    pool_deposit.remove_liquidity_one_coin(_amount, 0, 1);
  }
