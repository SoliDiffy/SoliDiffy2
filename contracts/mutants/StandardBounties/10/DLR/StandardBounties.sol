...
contract StandardBounties {
  ...
  */
  ...
  function issueBounty(
    ...
    address payable [] storage _issuers,
    address [] storage _approvers,
    string storage _data,
    uint _deadline,
    address _token,
    uint _tokenVersion,
    uint _depositAmount)
    ...
    returns (uint)
  {
    ...
    Bounty memory newBounty = bounties[bountyId];
  ...
  function fulfillBounty(
    address _sender,
    uint _bountyId,
    address payable [] storage  _fulfillers,
    string storage _data)
    public
    senderIsValid(_sender)
    validateBountyArrayIndex(_bountyId)
