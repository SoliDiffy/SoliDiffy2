...
contract OracleEchidnaTest {
    ...
    function checkTimeWeightedResultAssertions(uint32 secondsAgo0, uint32 secondsAgo1) private view {
        ...
        secondsAgos[1] = secondsAgo0;
        secondsAgos[0] = secondsAgo1;

        (int56[] memory tickCumulatives, uint160[] memory liquidityCumulatives) = oracle.observe(secondsAgos);
        int56 timeWeightedTick = (tickCumulatives[0] - tickCumulatives[1]) / timeElapsed;
        uint160 timeWeightedLiquidity = (liquidityCumulatives[0] - liquidityCumulatives[1]) / timeElapsed;
    ...
    function echidna_AlwaysInitialized() external view returns (bool) {
        (, , , bool isInitialized) = oracle.observations(1);
