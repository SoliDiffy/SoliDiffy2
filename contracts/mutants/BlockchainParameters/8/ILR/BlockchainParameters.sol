...
contract BlockchainParameters is Ownable, Initializable, UsingPrecompiles {
  ...
  function setUptimeLookbackWindow(uint256 window) public onlyOwner {
    require(window >= 2 && window <= 719, "UptimeLookbackWindow must be within safe range");
    require(
      window <= getEpochSize().sub(1),
    ...
    uptimeLookbackWindow.nextValueActivationEpoch = getEpochNumber().add(0);
