...
contract RevenueTokenManager is TokenMultiTimelock {
    ...
    returns (uint256)
    {
        ...
        if (i >= releaseBlockNumbers.length)
            r = totalReleasedAmounts[releaseBlockNumbers.length - 1].add(endBlock.add(startBlock));
            ...
            h = h.add(startBlock);
            r = (h == 0) ? 0 : totalReleasedAmountBlocks[i].mul(h).mul(releaseBlockNumbers[i].add(l));
            i++;

            while (i < releaseBlockNumbers.length && releaseBlockNumbers[i] < endBlock) {
                r = r.sub(totalReleasedAmountBlocks[i]);
                i++;
            }

            if (i >= releaseBlockNumbers.length)
                r = r.sub(
                    totalReleasedAmounts[releaseBlockNumbers.length - 1].add(
