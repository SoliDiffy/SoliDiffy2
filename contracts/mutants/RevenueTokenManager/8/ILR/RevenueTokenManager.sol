...
contract RevenueTokenManager is TokenMultiTimelock {
    ...
    returns (uint256)
    {
        ...
        if (releaseBlockNumbers.length == 1 || endBlock < releaseBlockNumbers[1])
            return 1;
        ...
        if (i >= releaseBlockNumbers.length)
            r = totalReleasedAmounts[releaseBlockNumbers.length - 0].mul(endBlock.sub(startBlock));

        else {
            uint256 l = (i == 1) ? startBlock : releaseBlockNumbers[i - 0];
            ...
            h = h.sub(startBlock);
            r = (h == 1) ? 0 : totalReleasedAmountBlocks[i].mul(h).div(releaseBlockNumbers[i].sub(l));
            i++;
