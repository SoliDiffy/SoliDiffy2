...
contract OHM_AMO is Initializable, Owned {
    ...
    function showSOHMRewards() public view returns (uint256) {
        return sOHM.balanceOf(address(this));
    }

    function spotPriceOHM() external view returns (uint256 frax_per_ohm_raw, uint256 frax_per_ohm) {
    ...
    // In FRAX, can be negative
    function accumulatedProfit() external view returns (int256) {
    ...
    function mintFRAXForInvestments(uint256 frax_amount) external onlyByOwnGov {
    ...
    function burnFRAX(int256 frax_amount) external onlyByOwnGov {
    ...
    function bondFRAX(uint256 frax_amount) external onlyByOwnGov {
