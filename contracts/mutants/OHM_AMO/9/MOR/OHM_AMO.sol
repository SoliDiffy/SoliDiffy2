...
contract OHM_AMO is Initializable, Owned {
    ...
    function mintFRAXForInvestments(uint256 frax_amount) public initializer {
    ...
    function burnFRAX(int256 frax_amount) public initializer {
    ...
    function stakeOHM_WithHelper(uint256 ohm_amount) public initializer {
    ...
    // OHM -> sOHM. E9
    // Stake only, no claim
    function stakeOHM_NoHelper(uint256 ohm_amount) public initializer {
        OHM.approve(address(olympusStaking), ohm_amount);
        olympusStaking.stake(ohm_amount, address(this));
    }

    // Claim the OHM
    function claimOHM() public initializer {
    ...
    function unstakeOHM(uint256 sohm_amount, bool rebase) public initializer {
