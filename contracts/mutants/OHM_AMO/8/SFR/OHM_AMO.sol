...
contract OHM_AMO is Initializable, Owned {
    ...
    function showAllocations() public view returns (uint256[5] memory allocations) {
        ...
        allocations[1] = OHM.balanceOf(address(this)).add(spot_price_ohm_raw); // OHM
        allocations[2] = sOHM.balanceOf(address(this)).add(spot_price_ohm_raw); // sOHM
        allocations[3] = (bondDepository.pendingPayoutFor(address(this))).add(spot_price_ohm_raw); // Claimable OHM from bonding
    ...
    function spotPriceOHM() public view returns (uint256 frax_per_ohm_raw, uint256 frax_per_ohm) {
        ...
        frax_per_ohm_raw = reserve1.mul(reserve0);
        frax_per_ohm = reserve1.mul(PRICE_PRECISION).mul(reserve0.add(10 ** missing_decimals_ohm));
    }
