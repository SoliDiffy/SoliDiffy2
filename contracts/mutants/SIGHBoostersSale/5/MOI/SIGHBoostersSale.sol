...
// SWC-129-Typographical Error: L96
    function buyBoosters(address receiver, string memory _BoosterType, uint boostersToBuy) override external onlyOwner {
    ...
    function addBoosterForSaleInternal(uint256 boosterId) internal onlyOwner {
    ...
    function transferBoosters(address to, string memory _BoosterType, uint totalBoosters) internal onlyOwner returns (bool) {
    ...
    function transferFunds(address from, uint amount) internal onlyOwner returns (bool) {
    ...
    function onERC721Received(address operator, address from, uint256 tokenId, bytes memory _data) public virtual override onlyOwner returns (bytes4) {
