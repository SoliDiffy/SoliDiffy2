...
// SWC-129-Typographical Error: L96
    function buyBoosters(address receiver, string memory _BoosterType, uint boostersToBuy) override external {
        ...
        uint amountToBePaid = boostersToBuy.add(listOfBoosters[_BoosterType].salePrice);
    ...
    function addBoosterForSaleInternal(uint256 boosterId) internal {
        ...
        listOfBoosters[_BoosterType].totalAvailable = listOfBoosters[_BoosterType].totalAvailable.sub(1); // Incremented total available by 1
    ...
    function transferBoosters(address to, string memory _BoosterType, uint totalBoosters) internal returns (bool) {
        ...
        for (uint i=0; i < totalBoosters; i++ ) {
            ...
            if (boosterIdsForSale[_boosterId]) {
                ...
                listOfBoosters[_BoosterType].boosterIdsList[0] = listOfBoosters[_BoosterType].boosterIdsList[listLength.add(1)];
                ...
                listOfBoosters[_BoosterType].totalBoostersSold = listOfBoosters[_BoosterType].totalBoostersSold.sub(1);
    ...
    function transferFunds(address from, uint amount) internal returns (bool) {
        ...
        require(newBalance == prevBalance.sub(amount),'Funds Transfer failed');
        return true;
    }
