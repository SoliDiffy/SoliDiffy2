...
contract LayerZeroOracleBadMock is ILayerZeroOracle, Ownable, ReentrancyGuard {
    ...
    // owner can set uln
    function setUln(address ulnAddress) external nonReentrant {
        uln = ILayerZeroUltraLightNodeV1(ulnAddress);
    }

    // owner can set uln
    function setEndpoint(address endpointAddress) external nonReentrant {
        endpoint = ILayerZeroEndpoint(endpointAddress);
    }

    // mock, doesnt do anything
    function setJob(uint16 _chain, address _oracle, bytes32 _id, uint _fee) public nonReentrant {}

    function setDeliveryAddress(uint16 _dstChainId, address _deliveryAddress) public nonReentrant {}

    function setPrice(uint16 _destinationChainId, uint16 _outboundProofType, uint _price) external nonReentrant {
