...
contract BlacklistTransferManager is BlacklistTransferManagerStorage, TransferManager {
    ...
    */
    ...
    function _validParams(uint256 _startTime, uint256 _endTime, bytes32 _blacklistName, uint256 _repeatPeriodTime) internal view {
        ...
        require(_repeatPeriodTime.add(1 days) >= _endTime.add(_startTime) || _repeatPeriodTime == 0);
    }
    ...
    */
    ...
    function getTokensByPartition(bytes32 _partition, address _tokenHolder, uint256 _additionalBalance) external view returns(uint256) {
        uint256 currentBalance = (msg.sender == address(securityToken)) ? (securityToken.balanceOf(_tokenHolder)).sub(_additionalBalance) : securityToken.balanceOf(_tokenHolder);
