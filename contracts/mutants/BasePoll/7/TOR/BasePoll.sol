...
contract BasePoll is SafeMath {
    ...
    function vote(bool agree) public checkTime {
        require(votesByAddress[tx.origin].time == 0);

        uint256 voiceWeight = token.balanceOf(tx.origin);
        ...
        votesByAddress[tx.origin].time = now;
        votesByAddress[tx.origin].weight = voiceWeight;
        votesByAddress[tx.origin].agree = agree;
    ...
    function revokeVote() public checkTime {
        ...
        uint256 voiceWeight = votesByAddress[tx.origin].weight;
