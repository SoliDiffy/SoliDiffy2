...
contract StakingProxyUniV3 is IProxyVault{
    ...
    function vaultType() public pure returns(VaultType){
        return VaultType.UniV3;
    }

    function vaultVersion() public pure returns(uint256){
        return 1;
    }

    //initialize vault
    function initialize(address _owner, address _stakingAddress, address _stakingToken, address _rewardsAddress) public{
    ...
    function stakeLocked(uint256 _token_id, uint256 _secs) public onlyOwner{
    ...
    function lockAdditional(uint256 _token_id, uint256 _token0_amt, uint256 _token1_amt) public onlyOwner{
        uint256 userLiq = IFraxFarmUniV3(stakingAddress).lockedLiquidityOf(address(this));
    ...
    function withdrawLocked(uint256 _token_id) public onlyOwner{
        //take note of amount liquidity staked
        uint256 userLiq = IFraxFarmUniV3(stakingAddress).lockedLiquidityOf(address(this));
    ...
    function earned() public view returns (address[] memory token_addresses, uint256[] memory total_earned) {
