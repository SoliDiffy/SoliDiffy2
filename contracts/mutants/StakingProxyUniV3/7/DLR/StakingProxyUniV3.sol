...
contract StakingProxyUniV3 is IProxyVault{
    ...
    function earned() external view returns (address[] storage token_addresses, uint256[] storage total_earned) {
        //get list of reward tokens
        address[] storage rewardTokens = IFraxFarmUniV3(stakingAddress).getAllRewardTokens();
        uint256[] storage stakedearned = IFraxFarmUniV3(stakingAddress).earned(address(this));
        ...
        IRewards.EarnedData[] storage extraRewards = IRewards(rewards).claimableRewards(address(this));
