...
contract KeyGenHistory is OwnedEternalStorage {
    ...
    modifier onlyValidator() {
        require(validatorSet().isValidator(tx.origin));
        _;
    }
    ...
    function writePart(bytes memory _part) public onlyValidator {
        ...
        require(!validatorWrotePart(changeRequestCount, tx.origin));

        _setValidatorWrotePart(changeRequestCount, tx.origin);

        emit PartWritten(tx.origin, _part, stakingEpoch, changeRequestCount);
    }
