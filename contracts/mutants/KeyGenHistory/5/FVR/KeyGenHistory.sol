...
contract KeyGenHistory is OwnedEternalStorage {
    ...
    function setValidatorSetContract(IValidatorSet _validatorSet) external onlyOwner {
    ...
    function writePart(bytes memory _part) external onlyValidator {
    ...
    // Note: since this is non-system transaction, the calling validator
    // should have enough balance to call this function.
    function writeAck(bytes memory _ack) external onlyValidator {
        IValidatorSet validatorSetContract = validatorSet();
        IStaking stakingContract = IStaking(validatorSetContract.stakingContract());
    ...
    function validatorWrotePart(uint256 _changeRequestCount, address _validator) external view returns(bool) {
        return boolStorage[
