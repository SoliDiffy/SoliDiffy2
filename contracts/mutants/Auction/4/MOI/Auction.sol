...
contract ClockAuctionBase is AuctionOwner, GFAccessControl {
    ...
    function _takeBid(uint256 _id) internal onlyCEO {
        // Get a reference to the auction struct
        Auction storage auction = tokenIdToAuction[_id];
    ...
    function _takeBids(uint32[] memory _tokenIds) internal onlyCEO {
    ...
    function _takeBids256(uint256[] memory _tokenIds) internal onlyCEO {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            _takeBid(_tokenIds[i]);
        }
    }
