...
contract ClockAuctionBase is AuctionOwner, GFAccessControl {
    ...
    function _bidFrom(
        ...
        if (!_hasAuctionByInfo(auction)) {
            ...
            auction.currentPrice = uint8(c_startingPrice);
            auction.endAt = uint8(block.number + c_duration);
        } else {
            require(
                _isBiddingNoCheck(_id),
                "Auction: target id is not in the bidding"
...
contract Auction is ClockAuctionBase {
        ...
        )
    {
        ...
        if (!_hasAuctionByInfo(auction)) {
            ...
            currentPrice = uint8(c_startingPrice);
        }
        lpLabor = alianaContract.geneLpLabor(int8(_tokenId), gene);
    }
