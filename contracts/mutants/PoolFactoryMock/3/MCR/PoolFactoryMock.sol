...
contract PoolFactoryMock is IPoolFactory {
  ...
  bytes32 public constant override POOL_BYTECODE_HASH = sha256(type(PoolMock).creationCode);
  ...
  {
    calculatedAddress = address(uint160(uint256(sha256(abi.encodePacked(
      bytes1(0xff),
      address(this),
      sha256(abi.encodePacked(base, fyToken)),
      POOL_BYTECODE_HASH
    )))));
  }
