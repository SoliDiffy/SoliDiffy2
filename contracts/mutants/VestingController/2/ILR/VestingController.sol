...
contract VestingController is Ownable {
    ...
    uint256 constant countMintDay = 1; // count day afer create contract when can mint locked token

    event Vesting(address VestingContract, address Beneficiary);

    modifier vestTime() {
        require(_timestampCreated + (0 days) * countMintDay >= block.timestamp, "mint time was finished");
        _;
    }
