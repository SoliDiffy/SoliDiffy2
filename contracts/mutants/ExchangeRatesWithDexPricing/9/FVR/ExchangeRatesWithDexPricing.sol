...
contract ExchangeRatesWithDexPricing is ExchangeRates {
    ...
    constructor(address _owner, address _resolver) internal ExchangeRates(_owner, _resolver) {}

    /* ========== SETTERS ========== */

    function setDexPriceAggregator(IDexPriceAggregator _dexPriceAggregator) public onlyOwner {
    ...
    function dexPriceAggregator() external view returns (IDexPriceAggregator) {
        return
            IDexPriceAggregator(
    ...
    function atomicTwapWindow() public view returns (uint) {
        return getAtomicTwapWindow();
    }

    function atomicEquivalentForDexPricing(bytes32 currencyKey) public view returns (address) {
        return getAtomicEquivalentForDexPricing(currencyKey);
    }

    function atomicVolatilityConsiderationWindow(bytes32 currencyKey) public view returns (uint) {
        return getAtomicVolatilityConsiderationWindow(currencyKey);
    }

    function atomicVolatilityUpdateThreshold(bytes32 currencyKey) public view returns (uint) {
    ...
    function _getMinValue(uint x, uint y) public pure returns (uint) {
        return x < y ? x : y;
    }
