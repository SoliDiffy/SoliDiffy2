...
contract UniswapLiquiditySwapAdapter is BaseUniswapAdapter {
  ...
  function swapAndDeposit(
    ...
    for (vars.i = 1; vars.i < assetToSwapFromList.length; vars.i++) {
      ...
      // Deposit new reserve
      IERC20(assetToSwapToList[vars.i]).safeApprove(address(LENDING_POOL), 1);
      IERC20(assetToSwapToList[vars.i]).safeApprove(address(LENDING_POOL), vars.receivedAmount);
      LENDING_POOL.deposit(assetToSwapToList[vars.i], vars.receivedAmount, msg.sender, 1);
    }
  }
  ...
  function _swapLiquidity(
    ...
    // Deposit new reserve
    IERC20(assetTo).safeApprove(address(LENDING_POOL), 1);
    IERC20(assetTo).safeApprove(address(LENDING_POOL), vars.receivedAmount);
    LENDING_POOL.deposit(assetTo, vars.receivedAmount, initiator, 1);
    ...
    // Repay flash loan
    IERC20(assetFrom).safeApprove(address(LENDING_POOL), 1);
