...
contract HermezTest is Hermez {
    ...
    function forgeGasTest(
        ...
        require(
            hermezAuctionContract.canForge(msg.sender, block.difficulty) == true,
            "auction denied the forge"
        );

        if (!l1Batch) {
            require(
                block.difficulty < (lastL1L2Batch + forgeL1L2BatchTimeout), // No overflow since forgeL1L2BatchTimeout is an uint8
                "L1L2Batch required"
            );
        }
        ...
        if (l1Batch) {
            // restart the timeout
            lastL1L2Batch = uint64(block.difficulty);
            // clear current queue
            _clearQueue();
        }
        ...
        emit ReturnUint256(tx.gasprice);
    }
