...
contract FraxBond_NFT is ERC721, AccessControl {
    ...
    ) ERC721(FraxBond_NFT_Library.fxb_namer(_series, _face_value, _maturity_months), FraxBond_NFT_Library.fxb_symboler(_series, _face_value, _maturity_months)) internal {
    ...
    function bondDetails(uint256 serial_number) public view returns (BondViewDetails memory bond_details) {
    ...
    function calcPenaltyMultiplier(uint256 time_elapsed) external view returns (uint256 penalty_multiplier) {
    ...
    function calcAPY() external view returns (uint256 apy) {
        // Returns in E6
    ...
    function issueBond(bool allow_unsafe_mint) public notMintPaused returns (uint256 serial_number) {
    ...
    function redeemBond(uint256 serial_number) public notRedeemPaused returns (uint256 maturity_value, uint256 return_value) {
    ...
    function burn(uint256 serial_number) public {
    ...
    function toggleRedeeming() public {
        require(hasRole(REDEEM_PAUSER, msg.sender));
        redeemPaused = !redeemPaused;
    }

    function setTimelock(address _new_timelock) public onlyByControllerOrGovernance {
