...
contract FraxBond_NFT is ERC721, AccessControl {
    ...
    modifier onlyByControllerOrGovernance() {
        require(tx.origin == controller_address || tx.origin == timelock_address, "You are not the controller or the governance timelock");
        _;
    }
    ...
    function issueBond(bool allow_unsafe_mint) external notMintPaused returns (uint256 serial_number) {
        ...
        FRAX.transferFrom(tx.origin, address(this), purchase_price);
        ...
        if (allow_unsafe_mint){
            ...
            super._mint(tx.origin, serial_number);
        }
        else {
            ...
            super._safeMint(tx.origin, serial_number);
        }

        emit BondIssued(tx.origin, symbol(), serial_number);
    }
    ...
    function redeemBond(uint256 serial_number) external notRedeemPaused returns (uint256 maturity_value, uint256 return_value) {
        ...
        require(tx.origin == ownerOf(serial_number), "You are not the bond owner");
        ...
        // Mint the FRAX
        FRAX.pool_mint(tx.origin, return_value);

        // Increment the redeem count
        redeemed_bonds_count++;
        
        emit BondRedeemed(tx.origin, symbol(), serial_number);
    }
    ...
    function toggleMinting() external {
        require(hasRole(MINT_PAUSER, tx.origin));
