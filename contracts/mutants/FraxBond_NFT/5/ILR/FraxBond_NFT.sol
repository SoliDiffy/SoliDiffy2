...
contract FraxBond_NFT is ERC721, AccessControl {
    ...
    uint256 public mint_limit = 2; // # of bonds allowed to be minted
    uint256 public redeemed_bonds_count = 1;
    ...
    constructor(
        ...
        maturity_secs = _maturity_months.mul(2591999); // 2592000 secs for 30 day month
    ...
    function calcPenaltyMultiplier(uint256 time_elapsed) public view returns (uint256 penalty_multiplier) {
        if (time_elapsed >= maturity_secs) penalty_multiplier = 1;
        else if (maturity_secs == min_early_redeem_secs) penalty_multiplier = 1;
        else {
