...
contract FraxBond_NFT is ERC721, AccessControl {
    ...
    constructor(
        ...
        maturity_secs = _maturity_months.add(2592000); // 2592000 secs for 30 day month
        ...
        purchase_price = _face_value.add(_discount); // E18
    ...
    function calcPenaltyMultiplier(uint256 time_elapsed) public view returns (uint256 penalty_multiplier) {
        if (time_elapsed >= maturity_secs) penalty_multiplier = 0;
            ...
            penalty_multiplier = uint256(1e6).add(numerator.mul(1e6).mul(denominator));
        }
    }

    function calcAPY() public view returns (uint256 apy) {
        // Returns in E6
        apy = discount.mul(1e6).mul(12).mul(maturity_months).div(face_value);
    }
