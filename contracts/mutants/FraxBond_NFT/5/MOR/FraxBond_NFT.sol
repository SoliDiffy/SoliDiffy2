...
contract FraxBond_NFT is ERC721, AccessControl {
    ...
    function issueBond(bool allow_unsafe_mint) external notRedeemPaused returns (uint256 serial_number) {
    ...
    function redeemBond(uint256 serial_number) external notMintPaused returns (uint256 maturity_value, uint256 return_value) {
    ...
    function setTimelock(address _new_timelock) external notMintPaused {
        timelock_address = _new_timelock;
    }

    function setController(address _controller_address) external notMintPaused {
        controller_address = _controller_address;
    }

    function setMintLimit(uint256 _new_mint_limit) external notMintPaused {
