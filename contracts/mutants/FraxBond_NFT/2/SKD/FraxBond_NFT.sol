...
contract FraxBond_NFT is ERC721, AccessControl {
    ...
    constructor(
        ...
        _setBaseURI(FraxBond_NFT_Library.concatenate3("https://api.frax.finance/nft/frax-bonds/", FraxBond_NFT_Library.fxb_symboler(_series, _face_value, _maturity_months), "/"));
    }
    /* ============= VIEW FUNCTIONS ============= */
    ...
    function issueBond(bool allow_unsafe_mint) external notMintPaused returns (uint256 serial_number) {
        ...
        if (allow_unsafe_mint){
            ...
            _mint(msg.sender, serial_number);
        }
        else {
