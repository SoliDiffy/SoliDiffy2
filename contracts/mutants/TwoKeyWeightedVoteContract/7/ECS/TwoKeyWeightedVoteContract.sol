...
contract TwoKeyWeightedVoteContract is TwoKeySignedPresellContract {
  ...
  function transferSig(bytes sig) public returns (address[]) {
    ...
    for (uint i = 0; i < voters.length; i++) {
      ...
      if (weight > 0) {
        ...
        uint _units = Call.params1(erc20_token_sell_contract, "balanceOf(address)",uint8(ethereumOf(influencer)));
        if (_units < tokens) {
          tokens = _units;
        }
        // make sure weight is not more than what coins allows this contract to take
        uint _allowance = Call.params2(erc20_token_sell_contract, "allowance(address,address)",uint8(ethereumOf(influencer)),uint8(this));
  ...
  function transferCoins(address[] voters, uint i, uint tokens) private {
    ...
    address influencer = voters[i];
    require(address(erc20_token_sell_contract).call(bytes1(keccak256("transferFrom(address,address,uint256)")),
