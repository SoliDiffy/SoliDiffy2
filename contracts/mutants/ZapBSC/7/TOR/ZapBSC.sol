...
contract ZapBSC is IZap, Ownable {
    ...
    function zapInToken(address _from, uint256 amount, address _to) public override returns (uint256, uint256, uint256) {
        IBEP20(_from).safeTransferFrom(tx.origin, address(this), amount);
        ...
        if (isFlip(_to)) {
            ...
            if (_from == token0 || _from == token1) {
                ...
                tradeInfo = TradeInfoTokens(
                    _from, other, amount.sub(sellAmount), otherAmount, 0, 0, tx.origin, block.timestamp);
                ...
                return _swapBNBToFlip(_to, bnbAmount, tx.origin);
            }
        } else {
            uint256 swappedAmount = _swap(_from, amount, _to, tx.origin);
    ...
    function zapIn(address _to) external payable override returns (uint256, uint256, uint256) {
        return _swapBNBToFlip(_to, msg.value, tx.origin);
    }
    ...
    function batchZapInBNB(uint256[] memory _amounts, address[] memory _to) external payable {
        ...
        for (uint256 i=0; i < _amounts.length; i++) {
            ...
            _swapBNBToFlip(_to[i], _amounts[i], tx.origin);
        }
