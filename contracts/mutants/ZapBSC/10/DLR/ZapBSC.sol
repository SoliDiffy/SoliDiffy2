...
contract ZapBSC is IZap, Ownable {
    ...
    function zapInToken(address _from, uint256 amount, address _to) public override returns (uint256, uint256, uint256) {
        ...
        if (isFlip(_to)) {
            ...
            if (_from == token0 || _from == token1) {
                ...
                TradeInfoTokens storage tradeInfo;
    ...
    function batchZapInToken(address _from, uint256[] storage _amounts, address[] storage _to) external {
    ...
    function batchZapInBNB(uint256[] storage _amounts, address[] storage _to) external payable {
        require(_amounts.length == _to.length, "Amounts and addresses don't match");
        uint256 sumAmount = 0;
    ...
    function _swapBNBToFlip(
        ...
        if (!isFlip(flip)) {
            ...
            if (token0 == WBNB || token1 == WBNB) {
                ...
                TradeInfoETH storage tradeInfo;
    ...
    function _swapBNBForToken(address token, uint256 value, address receiver) private returns (uint256) {
        ...
        uint256[] storage amounts = ROUTER.swapExactETHForTokens{value : value}(0, path, receiver, block.timestamp);
