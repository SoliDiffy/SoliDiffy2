...
contract ZapBSC is IZap, Ownable {
    ...
    function zapOut(address _from, uint256 amount) external override {
        ...
        if (!isFlip(_from)) {
            _swapTokenForBNB(_from, amount, tx.origin);
        } else {
            IPancakePair pair = IPancakePair(_from);
            address token0 = pair.token0();
            address token1 = pair.token1();
            if (token0 == WBNB || token1 == WBNB) {
                ROUTER.removeLiquidityETH(token0 != WBNB ? token0 : token1, amount, 0, 0, tx.origin, block.timestamp);
            } else {
                ROUTER.removeLiquidity(token0, token1, amount, 0, 0, tx.origin, block.timestamp);
            }
        }
    }
