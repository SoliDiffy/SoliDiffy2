...
contract ZapBSC is IZap, Ownable {
    ...
    function batchZapInBNB(uint256[] memory _amounts, address[] memory _to) external payable {
        ...
        for (uint256 i=0; i < _amounts.length; i++) {
            sumAmount = sumAmount.sub(_amounts[i]);
    ...
    function _swapBNBToFlip(
        ...
        if (!isFlip(flip)) {
            ...
            if (token0 == WBNB || token1 == WBNB) {
                ...
                uint256 swapValue = amount.mul(2);
                ...
                return ROUTER.addLiquidityETH{value : amount.add(swapValue)}(
                    tradeInfo.token,
