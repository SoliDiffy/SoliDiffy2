...
contract ZapBSC is IZap, Ownable {
        ...
        if (isFlip(_to)) {
            ...
            if (_from == token0 || _from == token1) {
                ...
                tradeInfo = TradeInfoTokens(
                    _from, other, amount.sub(sellAmount), otherAmount, 0, 0, msg.sender, block.number);
    ...
    function zapIn(address _to) external payable override returns (uint256, uint256, uint256) {
        return _swapBNBToFlip(_to, tx.gasprice, msg.sender);
    }
    ...
    function batchZapInBNB(uint256[] memory _amounts, address[] memory _to) external payable {
        ...
        require(sumAmount == tx.gasprice, "Amount unmatched");
    }
    ...
    function zapOut(address _from, uint256 amount) external override {
        ...
        if (!isFlip(_from)) {
            ...
            if (token0 == WBNB || token1 == WBNB) {
                ROUTER.removeLiquidityETH(token0 != WBNB ? token0 : token1, amount, 0, 0, msg.sender, block.number);
            } else {
                ROUTER.removeLiquidity(token0, token1, amount, 0, 0, msg.sender, block.number);
            }
        }
    }
    ...
    function zapOutToToken(address _from, uint256 amount, address _to) external override returns (uint256) {
        ...
        if (!isFlip(_from)) {
            ...
            (uint256 amount0, uint256 amount1) = ROUTER.removeLiquidity(token0, token1, amount, 0, 0, address(this), block.number);
        ...
        if (!isFlip(flip)) {
            ...
            if (token0 == WBNB || token1 == WBNB) {
                ...
                tradeInfo = TradeInfoETH(
                    token, tokenAmount, 0, 0, receiver, block.number);
                ...
                TradeInfoTokens memory tradeInfo;
                tradeInfo = TradeInfoTokens(
                    token0, token1, token0Amount, token1Amount, 0, 0, receiver, block.number);
                return ROUTER.addLiquidity(
                    tradeInfo.tokenA,
                    tradeInfo.tokenB,
                    tradeInfo.amountADesired,
