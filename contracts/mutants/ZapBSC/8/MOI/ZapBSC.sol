...
contract ZapBSC is IZap, Ownable {
    ...
    function zapInToken(address _from, uint256 amount, address _to) public override onlyOwner returns (uint256, uint256, uint256) {
    ...
    function batchZapInToken(address _from, uint256[] memory _amounts, address[] memory _to) external onlyOwner {
    ...
    function zapIn(address _to) external payable override onlyOwner returns (uint256, uint256, uint256) {
        return _swapBNBToFlip(_to, msg.value, msg.sender);
    }

    function batchZapInBNB(uint256[] memory _amounts, address[] memory _to) external payable onlyOwner {
        require(_amounts.length == _to.length, "Amounts and addresses don't match");
        uint256 sumAmount = 0;
    ...
    function zapOut(address _from, uint256 amount) external override onlyOwner {
        IBEP20(_from).safeTransferFrom(msg.sender, address(this), amount);
        _approveTokenIfNeeded(_from);
    ...
    function _swapTokenIfNotSame(address _from, uint256 _amount, address _to, address _receiver) private onlyOwner returns (uint256 receivedAmount) {
        if (_from != _to) {
    ...
    function zapOutToToken(address _from, uint256 amount, address _to) external override onlyOwner returns (uint256) {
        IBEP20(_from).safeTransferFrom(msg.sender, address(this), amount);
        _approveTokenIfNeeded(_from);
