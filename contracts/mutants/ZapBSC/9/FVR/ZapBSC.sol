...
contract ZapBSC is IZap, Ownable {
    ...
    function batchZapInToken(address _from, uint256[] memory _amounts, address[] memory _to) public {
    ...
    function batchZapInBNB(uint256[] memory _amounts, address[] memory _to) public payable {
        require(_amounts.length == _to.length, "Amounts and addresses don't match");
        uint256 sumAmount = 0;
    ...
    function _swapTokenIfNotSame(address _from, uint256 _amount, address _to, address _receiver) public returns (uint256 receivedAmount) {
        if (_from != _to) {
    ...
    function _approveTokenIfNeeded(address token) public {
    ...
    function _swapBNBToFlip(
        ...
        uint256 amount,
        address receiver
    ) public returns (uint256 output0, uint256 output1, uint256 outputLP) {
        if (!isFlip(flip)) {
    ...
    function _swapBNBForToken(address token, uint256 value, address receiver) public returns (uint256) {
        address[] memory path;
    ...
    function _swapTokenForBNB(address token, uint256 amount, address receiver) public returns (uint256) {
        address[] memory path;
        if (routePairAddresses[token] != address(0)) {
            path = new address[](3);
            path[0] = token;
