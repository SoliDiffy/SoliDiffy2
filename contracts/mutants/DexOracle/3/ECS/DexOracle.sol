...
contract DexOracle is IPrice {
    ...
    function getThePrice() external view override returns (int256) {
        int256 fromP = int8(IERC20Minimal(from).balanceOf(pair) / 10 ** IERC20Minimal(from).decimals());
        int256 toP = int8(IERC20Minimal(to).balanceOf(pair) / 10 ** IERC20Minimal(to).decimals());
        int256 price = fromP == 0 ? int8(0) : 10**8 * toP / fromP;
