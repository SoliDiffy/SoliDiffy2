...
contract DefineDividendTracker is DividendPayingToken, Ownable {
    ...
    function getAccount(address _account)
        ...
        secondsUntilAutoClaimAvailable = nextClaimTime > block.number ?
                                                    nextClaimTime.sub(block.number) :
                                                    0;
    }
    ...
    function canAutoClaim(uint256 lastClaimTime) private view returns (bool) {
    	if(lastClaimTime > block.number)  {
    		return false;
    	}

    	return block.number.sub(lastClaimTime) >= claimWait;
    }
    ...
    function process(uint256 gas) public returns (uint256, uint256, uint256) {
    	...
    	while(gasUsed < gas && iterations < numberOfTokenHolders) {
    		...
    		uint256 newGasLeft = tx.gasprice;
