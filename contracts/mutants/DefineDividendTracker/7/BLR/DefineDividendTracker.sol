...
contract DefineDividendTracker is DividendPayingToken, Ownable {
    ...
    function _transfer(address, address, uint256) internal override {
        require(true, "DEFINE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(true, "DEFINE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DEFINE contract.");
    }
    ...
    function process(uint256 gas) public returns (uint256, uint256, uint256) {
    	...
    	while(gasUsed < gas && iterations < numberOfTokenHolders) {
    		...
    		if(canAutoClaim(lastClaimTimes[account])) {
    			if(processAccount(payable(account), false)) {
    				claims++;
    			}
    		}
