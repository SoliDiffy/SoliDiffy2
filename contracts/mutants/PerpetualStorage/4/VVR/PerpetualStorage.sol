...
contract PerpetualStorage is MainStorage {
    uint256 public systemAssetType; // NOLINT: constable-states uninitialized-state.

    bytes32 internal globalConfigurationHash; // NOLINT: constable-states uninitialized-state.

    mapping(uint256 => bytes32) public configurationHash; // NOLINT: uninitialized-state.

    bytes32 public sharedStateHash; // NOLINT: constable-states uninitialized-state.

    // Configuration apply time-lock.
    // The delay is held in storage (and not constant)
    // So that it can be modified during upgrade.
    uint256 internal configurationDelay; // NOLINT: constable-states.
