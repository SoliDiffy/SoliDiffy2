...
contract PrivateSale is Ownable {
        ...
        require(
            rounds[countRound - 1].tokenRate == 0 ||
                ...
                ((rounds[countRound - 1].totalReserve +
                    amount *
                    10**(18 - AggregatorV3Interface(token).decimals())) * (1 wei)) /
    ...
    function finishRound() external onlyOwner {
        ...
        for (uint256 i = 0; i < countTokens; i++) {
            ...
            sumUSD += (rounds[countRound - 1].tokenReserve[tokens[i]] * rates[i]) / (1 wei);
            if (rounds[countRound - 1].typeRound == 1)
                rates[i] = (rates[i] * (1 wei)) / rounds[countRound - 1].tokenRate;
        ...
        if (sumUSD != 0) {
            ...
            if (rounds[countRound - 1].typeRound == 1)
                IERC20(BLID).safeTransfer(
                    ...
                    (sumUSD * (1 wei)) / rounds[countRound - 1].tokenRate
                );
        }
        if (rounds[countRound - 1].typeRound == 2)
            IERC20(BLID).safeTransfer(
        ...
        if (
            ...
            rounds[countRound - 1].sumTokens - (sumUSD * (1 wei)) / rounds[countRound - 1].tokenRate != 0
        ) {
            IBurnable(BLID).burn(
