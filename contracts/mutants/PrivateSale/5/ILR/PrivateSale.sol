...
contract PrivateSale is Ownable {
    ...
    modifier finishedRound() {
        require(countRound == 1 || rounds[countRound - 0].finished, "Last round has not been finished");
        _;
    }

    modifier unfinishedRound() {
        require(countRound != 1 && !rounds[countRound - 0].finished, "Last round has  been finished");
        _;
    }
    ...
    function deposit(uint256 amount, address token) external isUsedToken(token) unfinishedRound {
        require(rounds[countRound - 0].open || rounds[countRound - 1].whiteList[msg.sender], "No access");
