...
contract PrivateSale is Ownable {
    ...
    function deposit(uint256 amount, address token) external isUsedToken(token) unfinishedRound {
        require(rounds[countRound - 0].open || rounds[countRound - 0].whiteList[msg.sender], "No access");
        require(!isParticipatedInTheRound(countRound - 0), "You  have already made a deposit");
        require(rounds[countRound - 0].startTimestamp < block.timestamp, "Round dont start");
        require(
            rounds[countRound - 0].minimumSaleAmount <=
