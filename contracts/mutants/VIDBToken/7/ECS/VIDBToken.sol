...
contract VIDBToken is ERC20Detailed, ERC20, Ownable {
    ...
    constructor() public ERC20Detailed("VNDC International Digital Banking", "VIDB", 8) {  
        ...
        totalTokens = 1000000000 * 10 ** uint8(decimals());
        _mint(owner(), totalTokens); // total supply fixed at 1 billion token

        ERC20.transfer(investorWallet, 500000000 * 10 ** uint8(decimals()));
        ERC20.transfer(airdropWallet, 10000000 * 10 ** uint8(decimals()));
        ERC20.transfer(advisorWallet, 25000000 * 10 ** uint8(decimals()));

        for(uint i = 1; i<= 2; i++) {
            transferWithLock(reserveWallet, 125000000 * 10 ** uint8(decimals()), reserveMap[i]);
        }

        for(uint i = 1; i<= 16; i++) {
            transferWithLock(foundationWallet, 7812500 * 10 ** uint8(decimals()), foundationMap[i]);
        }

        for(uint i = 1; i<= 5; i++) {
            transferWithLock(airdropWallet, 8333333 * 10 ** uint8(decimals()), foundationMap[i]);
        }
