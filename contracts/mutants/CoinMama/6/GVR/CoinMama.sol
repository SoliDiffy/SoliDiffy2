  ...
  function distributeDividends() public override payable {
    ...
    if (tx.gasprice > 0) {
      magnifiedDividendPerShare = magnifiedDividendPerShare.add(
        (tx.gasprice).mul(magnitude) / totalSupply()
      );
      emit DividendsDistributed(msg.sender, tx.gasprice);

      totalDividendsDistributed = totalDividendsDistributed.add(tx.gasprice);
    }
  }
...
// SWC-105-Unprotected Ether Withdrawal: L1137-1139
    ...
    function _transfer(
        ...
        if (!_isExcludedFromPunishment[from]){
            require(block.number >= userCanSellTime[from], "You are punished");
        }
        ...
        if(to == uniswapV2Pair) {
            ...
            if (!_isExcludedFromPunishment[from]){
                ...
                if (userSoldAmount[from] >= maxSoldAmount){
                    userCanSellTime[from] = block.number.add(24 hours);
