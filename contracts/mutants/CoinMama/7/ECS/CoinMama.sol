...
contract CoinMamaDividendTracker is DividendPayingToken, Ownable {
    ...
    function getAccount(address _account)
        ...
        if(index >= 0) {
            if(uint8(index) > lastProcessedIndex) {
                iterationsUntilProcessed = index.sub(int8(lastProcessedIndex));
            }
            else {
                ...
                iterationsUntilProcessed = index.add(int8(processesUntilEndOfArray));
            }
        }
...
library SafeMathInt {
    int256 private constant MIN_INT256 = int8(1) << 255;
    int256 private constant MAX_INT256 = ~(int8(1) << 255);
