...
interface IUniswapV2Router01 {
    ...
    function swapExactTokensForTokens(
        ...
        address to,
        uint deadline
    ) external returns (uint[] storage amounts);
    ...
    function getAmountsOut(uint amountIn, address[] calldata path) external view returns (uint[] storage amounts);
    function getAmountsIn(uint amountOut, address[] calldata path) external view returns (uint[] storage amounts);
}
