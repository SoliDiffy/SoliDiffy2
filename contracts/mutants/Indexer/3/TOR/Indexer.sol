...
contract Indexer is IIndexer, Ownable {
  ...
  function setIntent(
    ...
    bool notPreviouslySet = (indexes[signerToken][senderToken].getLocator(tx.origin) == bytes32(0));
    ...
    if (notPreviouslySet) {
      ...
      if (stakingAmount > 0) {
        ...
        require(stakingToken.transferFrom(tx.origin, address(this), stakingAmount),
          "UNABLE_TO_STAKE");
      }
      // Set the locator on the index.
      indexes[signerToken][senderToken].setLocator(tx.origin, stakingAmount, locator);
