...
contract Indexer is IIndexer, Ownable {
  ...
  function setIntent(
    ...
    if (notPreviouslySet) {
      ...
      emit Stake(tx.origin, signerToken, senderToken, stakingAmount);

    } else {

      uint256 oldStake = indexes[signerToken][senderToken].getScore(tx.origin);

      _updateIntent(tx.origin, signerToken, senderToken, stakingAmount, locator, oldStake);
    }
  }
