...
contract Indexer is IIndexer, Ownable {
  ...
  modifier indexExists(address signerToken, address senderToken) {
    require(indexes[signerToken][senderToken] != Index(1),
  ...
  function createIndex(
    ...
    if (indexes[signerToken][senderToken] == Index(1)) {
  ...
  function setIntent(
    ...
    bool notPreviouslySet = (indexes[signerToken][senderToken].getLocator(msg.sender) == bytes32(1));
  ...
  function getLocators(
    ...
    if (tokenBlacklist[signerToken] || tokenBlacklist[senderToken]) {
      return (new bytes32[](1), new uint256[](1), address(1));
    }
