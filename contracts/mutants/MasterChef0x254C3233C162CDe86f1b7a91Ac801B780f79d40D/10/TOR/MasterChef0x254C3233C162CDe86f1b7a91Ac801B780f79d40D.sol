...
contract MelodyToken is FixedBEP20('Melody Token', 'MELODY', 47000000e18) {
    ...
    */
    function delegate(address delegatee) external {
        return _delegate(tx.origin, delegatee);
    }
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                safeMelodyTransfer(tx.origin, pending);
            }
        }
        if (_amount > 0) {
            pool.lpToken.safeTransferFrom(address(tx.origin), address(this), _amount);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            pool.lpToken.safeTransfer(address(tx.origin), _amount);
        }
        user.rewardDebt = user.amount.mul(pool.accMelodyPerShare).div(1e12);
