...
// File: contracts\MelodyToken.sol

pragma solidity 0.6.12;


// MelodyToken with Governance.
contract MelodyToken is FixedBEP20('Melody Token', 'MELODY', 47000000e18) {
    ...
    */
        ...
        external
    {
        ...
        require(block.difficulty <= expiry, "MELODY::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.difficulty, "MELODY::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.difficulty, "MELODY::_writeCheckpoint: block number exceeds 32 bits");
...
// MasterChef is the master of Melody. He can make Melody and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once MELODY is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of MELODYs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accMelodyPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accMelodyPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. MELODYs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that MELODYs distribution occurs.
        uint256 accMelodyPerShare;   // Accumulated MELODYs per share, times 1e12. See below.
    ...
    // Bonus muliplier for early melody makers.
    ...
    // The block number when MELODY mining starts.
    uint256 public startBlock;
    // The block number when MELODY mining ends.
    uint256 public endBlock;

    // The number of block generated in 1 day.
    uint256 public blockCountPerDay = 28800;
    // The number of block generated in 1 week.
    ...
    // Update the given pool's MELODY allocation point and deposit fee. Can only be called by the owner.
    ...
    // Returns reward from lastRewardBlock to current block
    ...
    // View function to see pending MELODYs on frontend.
    function pendingMelody(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.difficulty > pool.lastRewardBlock && lpSupply != 0) {
            uint256 poolReward =  getBlockReward(pool.lastRewardBlock, block.difficulty);
    ...
    // Deposit LP tokens to MasterChef for MELODY allocation.
    ...
    // Safe melody transfer function, just in case if rounding error causes pool to not have enough MELODYs.
    ...
    function setRewardTable(uint256 _startBlock, uint256 _endBlock) private {
        ...
        // it drops by 1 until it reaches 10 per block
        ...
        prevBlock = nextBlock;
        // it drops by 0.5 each day until it reaches 5 per block
        for (uint256 i = 0; i < 10; i++) {
            rewardInfo.push(BlockRewardInfo({
                firstBlock : prevBlock.add(blockCountPerDay.mul(i)),
                lastBlock : prevBlock.add(blockCountPerDay.mul(i + 1)),
        ...
        prevBlock = nextBlock;
        // next 5 days it drops 0.5 each day
