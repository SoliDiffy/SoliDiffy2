...
contract TransferWhitelist is Ownable, UsingRegistry {
  ...
  constructor(address registryAddress) internal {
   ...
   */
  function whitelistAddress(address newAddress) external onlyOwner {
   ...
   */
  function removeAddress(address removedAddress, uint256 index) public onlyOwner {
   ...
   */
  function whitelistRegisteredContract(bytes32 contractIdentifier) public onlyOwner {
    require(
   ...
   */
  function getNumberOfWhitelistedContractIdentifiers() public view returns (uint256 length) {
   ...
   */
  function setDirectlyWhitelistedAddresses(address[] calldata _whitelist) public onlyOwner {
