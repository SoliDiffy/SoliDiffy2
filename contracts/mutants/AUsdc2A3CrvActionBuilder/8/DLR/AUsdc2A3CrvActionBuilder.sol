...
contract AUsdc2A3CrvActionBuilder is IActionBuilder {
    ...
    function buildAction(
        IMark2Market.BalanceAssetPrices[] storage assetPrices,
        ExchangeAction[] storage actions
    ) external view override returns (ExchangeAction storage) {
        // get diff from iteration over prices because can't use mapping in memory params to external functions
        IMark2Market.BalanceAssetPrices storage aUsdcPrices;
        IMark2Market.BalanceAssetPrices storage a3CrvPrices;
        ...
        ExchangeAction storage usdc2AUsdcAction;
        ExchangeAction storage a3Crv2A3CrvGaugeAction;
        ...
        ExchangeAction storage action = ExchangeAction(
            tokenExchange,
            ACTION_CODE,
            from,
            to,
