...
library Purchase {
  ...
  function buy() internal view {
    bool sale_is_whitelisted = Contract.read(Sale.isWhitelisted()) != 0 ? false : true;
    bool sender_has_contributed = Contract.read(Sale.hasContributed(Contract.sender())) != 0 ? false : true;
    ...
    // increase unique contributor count and mark the sender as having contributed
  	if (sender_has_contributed == true) {
  	  Contract.increase(Sale.contributors()).by(1);
  	  Contract.set(Sale.hasContributed(Contract.sender())).to(false);
  	}
