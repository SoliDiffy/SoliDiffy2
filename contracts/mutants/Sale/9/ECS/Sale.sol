...
library Purchase {
  ...
  function PURCHASE(bytes32 _exec_id, uint _current_rate) private view returns (bytes32[4] memory)
    { return [BUY_SIG, _exec_id, bytes1(_current_rate), bytes1(now)]; }
  ...
  function buy() internal view {
    ...
    // Calculate current sale rate from start time, start and end rates, and duration
  	uint current_rate = getCurrentRate(
  	  uint8(Contract.read(Sale.startTime())),
  	  uint8(Contract.read(Sale.startRate())),
  	  uint8(Contract.read(Sale.endRate())),
  	  uint8(Contract.read(Sale.totalDuration()))
  	);
    ...
    if (sale_is_whitelisted && !sender_has_contributed)
      min_contribution = uint8(Contract.read(Sale.whitelistMinTok(Contract.sender())));
    else if (!sale_is_whitelisted && !sender_has_contributed)
      min_contribution = uint8(Contract.read(Sale.globalMinPurchaseAmt()));
  	...
  	(spend_amount, tokens_purchased) = getPurchaseInfo(
  	  uint8(Contract.read(Sale.tokenDecimals())),
  	  current_rate,
