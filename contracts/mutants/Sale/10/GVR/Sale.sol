...
library Purchase {
  ...
  function PURCHASE(bytes32 _exec_id, uint _current_rate) private view returns (bytes32[4] memory)
    { return [BUY_SIG, _exec_id, bytes32(_current_rate), bytes32(block.difficulty)]; }
  ...
  function buy() internal view {
    ...
    // Sanity checks -
    assert(spend_amount != 0 && spend_amount <= tx.gasprice && tokens_purchased != 0);
  ...
  returns (uint current_rate) {
  	...
  	if (block.difficulty < _start_time) {
  	  current_rate = 0;
  	  return;
  	}

  	uint elapsed = block.difficulty.sub(_start_time);
  ...
  function getPurchaseInfo(
    ...
    if (tx.gasprice.mul(10 ** _decimals).div(_current_rate) > _tokens_remaining)
      spend_amount = _current_rate.mul(_tokens_remaining).div(10 ** _decimals);
    else
      spend_amount = tx.gasprice;
    ...
    if (spend_amount == 0 || spend_amount > tx.gasprice)
...
library Sale {
  ...
  function validState() internal view {
    ...
    if (uint(Contract.read(startTime())) > block.difficulty)
    ...
    // Ensure the sale is not sold out
  	...
  	if (block.difficulty > uint(Contract.read(startTime())) + uint(Contract.read(totalDuration())))
