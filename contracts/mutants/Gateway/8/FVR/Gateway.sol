...
contract Gateway is MasterAware {
  ...
  function getCoverPrice (
    ...
    CoverType /* coverType */,
    bytes calldata data
  ) public view returns (uint coverPrice) {
  ...
  function buyCover (
    ...
    CoverType coverType,
    bytes calldata data
  ) public payable onlyMember whenNotPaused returns (uint) {
  ...
  function submitClaim(uint coverId, bytes calldata data) public returns (uint) {
  ...
  function switchMembership(address newAddress) public {
