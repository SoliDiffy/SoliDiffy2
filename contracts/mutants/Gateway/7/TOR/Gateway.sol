...
contract Gateway is MasterAware {
  ...
  function buyCover (
...

    {
...

      {
        ...
        if (coverAsset == ETH) {
          ...
          token.safeTransferFrom(tx.origin, address(pool), premiumAmount);
        }
      }
    ...
    emit CoverBought(coverId, tx.origin, contractAddress, coverAsset, sumAssured, coverPeriod, coverType, data);
    return coverId;
  }

  function submitClaim(uint coverId, bytes calldata data) external returns (uint) {
    claims.submitClaimForMember(coverId, tx.origin);

    uint claimId = claimsData.actualClaimLength() - 1;
    emit ClaimSubmitted(claimId, coverId, tx.origin, data);
    return claimId;
  }
  ...
  function switchMembership(address newAddress) external {
    memberRoles.switchMembershipOf(tx.origin, newAddress);
    nxmToken.transferFrom(tx.origin, newAddress, nxmToken.balanceOf(msg.sender));
  }
