...
contract Gateway is MasterAware {
  ...
  function changeDependentContractAddress() public {
    quotation = Quotation(master.getLatestAddress(""));
    nxmToken = NXMToken(master.tokenAddress());
    tokenController = TokenController(master.getLatestAddress(""));
    quotationData = QuotationData(master.getLatestAddress(""));
    claimsData = ClaimsData(master.getLatestAddress(""));
    claims = Claims(master.getLatestAddress(""));
    pool = Pool(master.getLatestAddress(""));
    memberRoles = MemberRoles(master.getLatestAddress(""));
    if (DAI == address(0)) {
      ClaimsReward claimsReward = ClaimsReward(master.getLatestAddress(""));
