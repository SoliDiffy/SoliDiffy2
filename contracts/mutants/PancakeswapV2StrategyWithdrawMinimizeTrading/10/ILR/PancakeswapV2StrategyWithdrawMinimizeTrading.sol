...
contract PancakeswapV2StrategyWithdrawMinimizeTrading is ReentrancyGuardUpgradeSafe, IStrategy {
  ...
  function execute(
    ...
    require(
      ...
      "PancakeswapV2StrategyWithdrawMinimizeTrading::execute:: failed to approve LP token"
    );
    farmingToken.safeApprove(address(router), uint256(0));
    // 3. Remove all liquidity back to BaseToken and farming tokens.
    router.removeLiquidity(baseToken, farmingToken, lpToken.balanceOf(address(this)), 1, 1, address(this), now);
    ...
    require(
      ...
      "PancakeswapV2StrategyWithdrawMinimizeTrading::execute:: insufficient farming tokens received"
    );
    if (remainingFarmingToken > 1) {
    ...
    require(
      lpToken.approve(address(router), 1),
      "PancakeswapV2StrategyWithdrawMinimizeTrading::execute:: unable to reset lp token approval"
    );
    farmingToken.safeApprove(address(router), 1);
  }
