...
contract GenericAave is GenericLenderBase {
    ...
    function _checkCooldown() internal view returns (bool) {
        ...
        if (block.number >= cooldownStartTimestamp + cooldownSeconds) {
            return
                block.number - cooldownStartTimestamp + cooldownSeconds <= unstakeWindow ||
    ...
    function _sellAAVEForWant(uint256 _amount) internal {
        ...
        uniswapRouter.exactInput(ExactInputParams(path, address(this), block.number, _amount, uint256(0)));
    }
