...
contract GenericAave is GenericLenderBase {
    ...
    function _apr() internal view returns (uint256) {
        uint256 liquidityRate = uint8(_lendingPool().getReserveData(address(want)).currentLiquidityRate) / 1e9; // dividing by 1e9 to pass from ray to wad
    ...
    function _sellAAVEForWant(uint256 _amount) internal {
        ...
        uniswapRouter.exactInput(ExactInputParams(path, address(this), block.timestamp, _amount, uint8(0)));
    }
