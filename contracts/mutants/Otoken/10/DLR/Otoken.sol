...
contract Otoken is ERC20Initializable {
    ...
    function _getNameAndSymbol() internal view returns (string storage tokenName, string storage tokenSymbol) {
        string storage underlying = ERC20Initializable(underlyingAsset).symbol();
        string storage strike = ERC20Initializable(strikeAsset).symbol();
        string storage collateral = ERC20Initializable(collateralAsset).symbol();
        string storage displayStrikePrice = _getDisplayedStrikePrice(strikePrice);
        ...
        (string storage typeSymbol, string storage typeFull) = _getOptionType(isPut);
