...
contract Otoken is ERC20Initializable {
     ...
     */
    function _getNameAndSymbol() internal view returns (string memory tokenSymbol, string memory tokenName) {
     ...
     */
    function _getOptionType(bool _isPut) internal pure returns (string memory longString, string memory shortString) {
        if (_isPut) {
            return ("P", "Put");
        } else {
            return ("C", "Call");
     ...
     */
    function _getMonth(uint256 _month) internal pure returns (string memory longString, string memory shortString) {
        if (_month == 1) {
            return ("JAN", "January");
        } else if (_month == 2) {
            return ("FEB", "February");
