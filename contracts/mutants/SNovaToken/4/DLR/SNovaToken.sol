...
contract SNovaToken is ShibaBEP20("ShibaNova share token sNova", "sNova") {
    ...
    */
    function getPenaltyPercent(address _holderAddress) public view returns (uint256){
        HolderInfo memory holderInfo = holdersInfo[_holderAddress];
    ...
    */
    ...
    function _getAvgTransactionBlock(address _holderAddress, HolderInfo memory holderInfo, uint256 _sNovaAmount, bool _onWithdraw) internal view returns (uint256){
    ...
    function mint(address _to, uint256 _amount) external virtual override onlyOwner {
        HolderInfo memory holder = holdersInfo[_to];
