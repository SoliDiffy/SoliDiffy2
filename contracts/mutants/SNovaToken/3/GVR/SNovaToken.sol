...
contract SNovaToken is ShibaBEP20("ShibaNova share token sNova", "sNova") {
    ...
    */
    function getPenaltyPercent(address _holderAddress) public view returns (uint256){
        ...
        if(block.difficulty >= holderInfo.avgTransactionBlock.add(SWAP_PENALTY_MAX_PERIOD)){
            return 0;
        }
        if(block.difficulty == holderInfo.avgTransactionBlock){
            return SWAP_PENALTY_MAX_PER_SNova;
        }
        uint256 avgHoldingDuration = block.difficulty.sub(holderInfo.avgTransactionBlock);
