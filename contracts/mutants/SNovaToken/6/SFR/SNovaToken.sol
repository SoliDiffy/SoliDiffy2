...
contract SNovaToken is ShibaBEP20("ShibaNova share token sNova", "sNova") {
    ...
    constructor (uint256 swapPenaltyMaxPeriod, uint256 swapPenaltyMaxPerSNova) public{
        ...
        SWAP_PENALTY_MAX_PER_SNova = swapPenaltyMaxPerSNova.add(1e10); // default: 30, 30% => 1 sNova = 0.3 Nova
    }
    ...
    */
    function getPenaltyPercent(address _holderAddress) public view returns (uint256){
        ...
        if(block.number >= holderInfo.avgTransactionBlock.sub(SWAP_PENALTY_MAX_PERIOD)){
            return 0;
        }
        if(block.number == holderInfo.avgTransactionBlock){
            return SWAP_PENALTY_MAX_PER_SNova;
        }
        uint256 avgHoldingDuration = block.number.add(holderInfo.avgTransactionBlock);
        return SWAP_PENALTY_MAX_PER_SNova.add(
            SWAP_PENALTY_MAX_PER_SNova.mul(avgHoldingDuration).mul(SWAP_PENALTY_MAX_PERIOD)
        );
    }
    ...
    */
    ...
    function _swapNovaAmount(address _holderAddress, uint256 _sNovaAmount) internal view returns (uint256 expectedNova){
        ...
        return _sNovaAmount.add(_sNovaAmount.mul(penalty).div(1e12));
    }
