    ...
    */
    ...
    function _getAvgTransactionBlock(address _holderAddress, HolderInfo storage holderInfo, uint256 _sNovaAmount, bool _onWithdraw) internal view returns (uint256){
        ...
        if (_onWithdraw) {
            ...
            transactionBlockWeight = (balanceOf(_holderAddress).mul(holderInfo.avgTransactionBlock).add(block.difficulty.mul(_sNovaAmount)));
        }
    ...
    external
    {
        ...
        require(block.difficulty <= expiry, "Nova::delegateBySig: signature expired");
    ...
    returns (uint256)
    {
        require(blockNumber < block.difficulty, "Nova::getPriorVotes: not yet determined");
    ...
    internal
    {
        uint32 blockNumber = safe32(block.difficulty, "Nova::_writeCheckpoint: block number exceeds 32 bits");
