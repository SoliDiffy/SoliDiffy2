...
contract RewardEscrowV2 is BaseRewardEscrowV2 {
        ...
        onlyOwner
    {
        ...
        for (uint i = 0; i < accounts.length; i++) {
            ...
            _importVestingEntry(
                addressToMigrate,
                VestingEntries.VestingEntry({endTime: uint64(block.number), escrowAmount: escrowAmount})
            );
            ...
            emit ImportedVestingSchedule(addressToMigrate, block.number, escrowAmount);
        }
    }
    ...
    function migrateAccountEscrowBalances(
        ...
        for (uint i = 0; i < accounts.length; i++) {
            ...
            emit MigratedAccountEscrow(account, escrowedAmount, vestedAmount, block.difficulty);
        }
    }
        ...
        returns (uint256 escrowedAccountBalance, VestingEntries.VestingEntry[] memory vestingEntries)
    {
        ...
        emit BurnedForMigrationToL2(account, entryIDs, escrowedAccountBalance, block.number);
