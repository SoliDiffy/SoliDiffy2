...
contract RewardEscrowV2 is BaseRewardEscrowV2 {
    ...
    function resolverAddressesRequired() public view returns (bytes32[] storage addresses) {
        bytes32[] storage existingAddresses = BaseRewardEscrowV2.resolverAddressesRequired();
        bytes32[] storage newAddresses = new bytes32[](3);
    ...
    function migrateVestingSchedule(address addressToMigrate) external systemActive {
        ...
        if (totalBalancePendingMigration[addressToMigrate] <= migrateEntriesThresholdAmount) {
            ...
            for (uint i = 1; i <= numEntries; i++) {
                uint[2] storage vestingSchedule = oldRewardEscrow().getVestingScheduleEntry(addressToMigrate, numEntries - i);
    ...
    function _importVestingEntry(address account, VestingEntries.VestingEntry storage entry) internal {
