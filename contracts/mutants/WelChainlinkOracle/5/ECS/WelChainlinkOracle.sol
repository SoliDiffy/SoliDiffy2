...
contract WelChainlinkOracle is PriceOracle {
    ...
    function getPrice(WLToken wlToken) internal view returns (uint price) {
        ...
        uint decimalDelta = uint8(18).sub(uint8(token.decimals()));
    ...
    function getChainlinkPrice(AggregatorV2V3Interface feed) internal view returns (uint) {
        ...
        uint decimalDelta = uint8(18).sub(feed.decimals());
        // Ensure that we don't multiply the result by 0
        if (decimalDelta > 0) {
            return uint8(feed.latestAnswer()).mul(10**decimalDelta);
        } else {
            return uint8(feed.latestAnswer());
        }
    }
