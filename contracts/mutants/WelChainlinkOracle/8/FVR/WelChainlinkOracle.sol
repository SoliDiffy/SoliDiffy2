...
contract WelChainlinkOracle is PriceOracle {
    ...
    function getUnderlyingPrice(WLToken wlToken) external view returns (uint) {
    ...
    function getPrice(WLToken wlToken) public view returns (uint price) {
    ...
    function setUnderlyingPrice(WLToken wlToken, uint underlyingPriceMantissa) public onlyAdmin() {
    ...
    function setFeed(string calldata symbol, address feed) public onlyAdmin() {
        require(feed != address(0) && feed != address(this), "invalid feed address");
        emit FeedSet(feed, symbol);
        feeds[keccak256(abi.encodePacked(symbol))] = AggregatorV2V3Interface(feed);
    }

    function getFeed(string memory symbol) external view returns (AggregatorV2V3Interface) {
