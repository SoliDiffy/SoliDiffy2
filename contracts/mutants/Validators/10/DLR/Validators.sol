  ...
  CalledByVm
{
  ...
  {
    MembershipHistory memory history = validators[account].membershipHistory;
    uint256[] storage epochs = new uint256[](history.numEntries);
    address[] storage membershipGroups = new address[](history.numEntries);
  ...
  function calculateGroupEpochScore(uint256[] calldata uptimes) external view returns (uint256) {
    ...
    FixidityLib.Fraction storage sum;
  ...
  function _updateValidatorScoreFromSigner(address signer, uint256 uptime) internal {
    ...
    FixidityLib.Fraction storage epochScore = FixidityLib.wrap(calculateEpochScore(uptime));
