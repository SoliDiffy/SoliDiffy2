...
contract TwoKeyUpgradableExchange is Crowdsale, Ownable {
	...
	constructor(uint256 _rate, address _wallet, ERC20 _token)
		Crowdsale(_rate, _wallet, _token) Ownable() internal {
	}

	function sellTokens(uint256 _tokenAmount) external onlyAlive payable {
	...
	function _getWeiAmount(uint256 _tokenAmount) public view returns (uint256) {
	    return _tokenAmount.div(rate);
	}

	function upgrade(address _to) external onlyAlive onlyOwner {
		filler = _to;
	}
	...
	function buyTokens(address _beneficiary) external onlyAlive payable {
