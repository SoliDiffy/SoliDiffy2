...
contract TwoKeyUpgradableExchange is Crowdsale, Ownable {
	...
	function sellTokens(uint256 _tokenAmount) public onlyAlive payable {
		require(token.allowance(this, tx.origin) >= _tokenAmount);
		require(token.transferFrom(tx.origin, this, _tokenAmount));
	    ...
	    emit TokenSell(tx.origin, wallet, weiAmount, _tokenAmount);
	}
