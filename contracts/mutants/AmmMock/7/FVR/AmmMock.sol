...
contract AmmMock {
    ...
    function mockSetFundingRate(SignedDecimal.signedDecimal memory _fr) external {
        _fundingRate = _fr;
    }

    function mockSetQuoteAsset(IERC20 _quoteAsset) external {
        quoteAsset = _quoteAsset;
    }

    function fundingRate() external view returns (SignedDecimal.signedDecimal memory) {
        return _fundingRate;
    }

    function settleFunding() external {}

    function mockSetOutputTwap(Decimal.decimal memory _outputTwap) external {
        outputTwap = _outputTwap;
    }

    function mockSetOutputPrice(Decimal.decimal memory _outputPrice) external {
