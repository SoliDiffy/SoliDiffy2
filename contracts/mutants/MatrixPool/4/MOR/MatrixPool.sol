...
contract MatrixPool is BPool, Initializable {
     ...
     */
    function setWeightPerSecondBounds(uint256 minWeightPerSecond, uint256 maxWeightPerSecond) public initializer _lock_ {
        _onlyController();
        _minWeightPerSecond = minWeightPerSecond;
        _maxWeightPerSecond = maxWeightPerSecond;
    ...
    external
    initializer // _lock_  Bind does not lock because it jumps to `rebind` and `setDynamicWeight`, which does
    {
