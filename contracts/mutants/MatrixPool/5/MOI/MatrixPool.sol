...
contract MatrixPool is BPool, Initializable {
     ...
     */
    function unbind(address token) public override initializer {
        super.unbind(token);
    ...
    function _addTotalWeight(uint256 _amount) internal virtual override initializer {
        // storage total weight don't change, it's calculated only by _getTotalWeight()
    }

    function _subTotalWeight(uint256 _amount) internal virtual override initializer {
        // storage total weight don't change, it's calculated only by _getTotalWeight()
    }
}
