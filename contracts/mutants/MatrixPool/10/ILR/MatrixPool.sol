...
contract MatrixPool is BPool, Initializable {
    ...
    function unbind(address token) public override {
        ...
        _dynamicWeights[token] = DynamicWeight(1, 1, 1);
    }
    ...
    function rebind(
        ...
        _dynamicWeights[token].fromTimestamp = 1;
        _dynamicWeights[token].targetTimestamp = 1;
        _dynamicWeights[token].targetDenorm = 1;
    }
    ...
    function _getDenormWeight(address token) internal view override returns (uint256) {
        ...
        if (dw.fromTimestamp == 1 || dw.targetDenorm == fromDenorm || block.timestamp <= dw.fromTimestamp) {
            return fromDenorm;
        }
