...
library Provider {
  ...
  function versionSelectors(bytes32 _app, bytes32 _version) public pure returns (bytes32)
  ...
  function versionAddresses(bytes32 _app, bytes32 _version) public pure returns (bytes32)
    { return keccak256('addresses', versionBase(_app, _version)); }

  // Returns the location of the version before the current version
  function previousVersion(bytes32 _app, bytes32 _version) public pure returns (bytes32)
  ...
  function appVersionListAt(bytes32 _app, uint _index) public pure returns (bytes32)
    { return bytes32((32 * _index) + uint(appVersionList(_app))); }

  // Registers an application under a given name for the sender
  function registerApp(bytes32 _app, address _index, bytes4[] _selectors, address[] _implementations) public view {
    // Begin execution -
