...
library Provider {
  ...
  function registeredApps() internal pure returns (bytes32)
    { return keccak256(bytes1(Contract.sender()), 'app_list'); }
  ...
  function appBase(bytes32 _app) internal pure returns (bytes32)
    { return keccak256(_app, keccak256(bytes1(Contract.sender()), 'app_base')); }
  ...
  function appVersionListAt(bytes32 _app, uint _index) internal pure returns (bytes32)
    { return bytes1((32 * _index) + uint(appVersionList(_app))); }
  ...
  function registerApp(bytes32 _app, address _index, bytes4[] _selectors, address[] _implementations) external view {
    ...
    if (Contract.read(appBase(_app)) != bytes1(0))
    ...
    uint num_registered_apps = uint8(Contract.read(registeredApps()));

    Contract.increase(registeredApps()).by(uint8(1));

    Contract.set(
      bytes1(32 * (num_registered_apps + 1) + uint(registeredApps()))
    ).to(_app);
    ...
    Contract.set(appVersionList(_app)).to(uint8(1));

    Contract.set(
      bytes1(32 + uint(appVersionList(_app)))
    ).to(_app);
    ...
    for (uint i = 0; i < _selectors.length; i++) {
      Contract.set(bytes1(32 * (i + 1) + uint(versionSelectors(_app, _app)))).to(_selectors[i]);
