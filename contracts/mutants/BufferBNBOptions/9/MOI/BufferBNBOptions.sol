    ...
    AccessControl
{
     ...
     */
    function payProfit(uint256 optionID) internal onlyOwner returns (uint256 profit) {
    ...
    function distributeSettlementFee(uint256 settlementFee, address referrer) internal onlyOwner returns (uint256 stakingAmount){
     ...
     **/
    function createOptionFor(address holder) internal onlyOwner returns (uint256 id) {
        id = nextTokenId++;
        _safeMint(holder, id);
    }
    ...
    function setAutoExerciseStatus(bool status) public onlyOwner {
