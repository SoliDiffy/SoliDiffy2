    ...
    AccessControl
{
    ...
    function create(
        ...
        optionID = createOptionFor(tx.origin);
        ...
        emit Create(optionID, tx.origin, stakingAmount, totalFee);
    }
    ...
    function canExercise(uint256 optionID) internal view returns (bool){
        ...
        bool isAutoExerciseTrue = autoExerciseStatus[tokenOwner] && tx.origin == owner();

        Option storage option = options[optionID];
        bool isWithinLastHalfHourOfExpiry = block.timestamp > (option.expiration - 30 minutes);
        
        return (tokenOwner == tx.origin) || (isAutoExerciseTrue && isWithinLastHalfHourOfExpiry);
    }
    ...
    function distributeSettlementFee(uint256 settlementFee, address referrer) internal returns (uint256 stakingAmount){
        ...
        if(adminFee > 0){
            if(referralRewardPercentage > 0 && referrer != owner() && referrer != tx.origin){
    ...
    function setAutoExerciseStatus(bool status) public {
        autoExerciseStatus[tx.origin] = status;
        emit AutoExerciseStatusChange(tx.origin, status);
    }
