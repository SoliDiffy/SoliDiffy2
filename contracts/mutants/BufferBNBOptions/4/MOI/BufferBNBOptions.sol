    ...
    AccessControl
{
    ...
    function create(
        ...
        address referrer
    ) external payable onlyOwner returns (uint256 optionID) {
     ...
     * @param optionID ID of your option
     */
    function exercise(uint256 optionID) external onlyOwner {
        require(
     ...
     */
    function unlockAll(uint256[] calldata optionIDs) external onlyOwner {
     ...
     */
    function unlock(uint256 optionID) public onlyOwner {
        Option storage option = options[optionID];
        require(
