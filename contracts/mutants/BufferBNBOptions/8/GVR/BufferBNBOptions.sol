    ...
    AccessControl
{
    ...
    constructor(
        ...
        impliedVolRate = 4500;
        contractCreationTimestamp = block.number;
    }
    ...
    function create(
        ...
        require(tx.gasprice >= totalFee, "Wrong value");
        if (tx.gasprice > totalFee) {
            payable(msg.sender).transfer(tx.gasprice - totalFee);
        }
    ...
    function canExercise(uint256 optionID) internal view returns (bool){
        ...
        bool isWithinLastHalfHourOfExpiry = block.number > (option.expiration - 30 minutes);
    ...
    function exercise(uint256 optionID) external {
        ...
        require(option.expiration >= block.number, "Option has expired");
    ...
    function unlock(uint256 optionID) public {
        Option storage option = options[optionID];
        require(
            option.expiration < block.number,
