...
contract AxialAMO is Owned {
    ...
    function showAllocations() external view returns (uint256[8] memory allocations) {
        // TSD
    ...
    function getACDBPrice() external view returns (uint256) {
    ...
    function ac4dDeposit(
        ...
        uint256 _dai_amount 
    ) public onlyByOwnGov returns (uint256 ac4d_received) {
        // Approve the TSD
        if (_tsd_amount > 0) {
    ...
    function _ac4dWithdrawOneCoin(uint256 _ac4d_in, uint8 tkn_idx) public returns (uint256 tokens_received) {
    ...
    function ac4dWithdrawTSD(uint256 _ac4d_lp_in) public onlyByOwnGov returns (uint256) {
        return _ac4dWithdrawOneCoin(_ac4d_lp_in, 0);
    }

    function ac4dWithdrawMIM(uint256 _ac4d_lp_in) public onlyByOwnGov returns (uint256) {
        return _ac4dWithdrawOneCoin(_ac4d_lp_in, 1);
    }

    function ac4dWithdrawFRAX(uint256 _ac4d_lp_in) public onlyByOwnGov returns (uint256) {
