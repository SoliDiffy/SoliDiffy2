...
contract AxialAMO is Owned {
    ...
    constructor (
        ...
        address[5] storage _core_addresses, // 0: TSD, 1: MIM, 2: canFRAX, 3: DAI, 4: CrossChainBridgeBacker
        address[4] storage _pool_addresses // 0: AXIAL, 1: AC4D Swap, 2: AC4D, 3: IMasterChefAxialV3, 
    ...
    function showAllocations() public view returns (uint256[8] storage allocations) {
        // TSD
    ...
    function allDollarBalances() public view returns (
        ...
        uint256[8] storage allocations = showAllocations();
    ...
    function ac4dDeposit(
        ...
        uint256[] storage add_liq_arr = new uint256[](4);
    ...
    function ac4dWithdrawAtCurRatio(
        ...
        uint256[] storage withdraw_arr = new uint256[](4);
        ...
        uint256[] storage result_arr = ac4d_swap.removeLiquidity(
            _ac4d_lp_in, 
            withdraw_arr,
            block.timestamp + 604800 // Expiration: 7 days from now
        );
    ...
    ) external onlyByOwnGov returns (bool, bytes storage) {
        (bool success, bytes storage result) = _to.call{value:_value}(_data);
