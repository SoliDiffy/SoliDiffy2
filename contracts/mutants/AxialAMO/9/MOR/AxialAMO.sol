...
contract AxialAMO is Owned {
    ...
    function ac4dDeposit(
        ...
        uint256 _dai_amount 
    ) external onlyByOwnGovCust returns (uint256 ac4d_received) {
        // Approve the TSD
        if (_tsd_amount > 0) {
    ...
    function ac4dWithdrawTSD(uint256 _ac4d_lp_in) external onlyByOwnGovCust returns (uint256) {
        return _ac4dWithdrawOneCoin(_ac4d_lp_in, 0);
    }

    function ac4dWithdrawMIM(uint256 _ac4d_lp_in) external onlyByOwnGovCust returns (uint256) {
        return _ac4dWithdrawOneCoin(_ac4d_lp_in, 1);
    }

    function ac4dWithdrawFRAX(uint256 _ac4d_lp_in) external onlyByOwnGovCust returns (uint256) {
        return _ac4dWithdrawOneCoin(_ac4d_lp_in, 2);
    }

    function ac4dWithdrawDAI(uint256 _ac4d_lp_in) external onlyByOwnGovCust returns (uint256) {
    ...
    function ac4dWithdrawAtCurRatio(
        ...
        uint256 min_frax, 
        uint256 min_dai
    ) external onlyByOwnGovCust returns (uint256 tsd_received, uint256 mim_received, uint256 frax_received, uint256 dai_received) {
        // Approve the AC4D LP tokens
    ...
    function masterChefDeposit(uint256 _ac4d_lp_in) external onlyByOwnGovCust {
        // Approve the AC4D LP tokens
    ...
    function masterChefCollect() external onlyByOwnGovCust {
        // Collect rewards (withdraw 0)
        masterchef.withdraw(mchef_pid, 0);
    }

    function masterChefWithdraw(uint256 _ac4d_lp_out) external onlyByOwnGovCust {
