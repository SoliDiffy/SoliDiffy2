...
contract AxialAMO is Owned {
    ...
    constructor (
        ...
        address[4] memory _core_addresses, // 0: TSD, 1: MIM, 2: canFRAX, 3: DAI, 4: CrossChainBridgeBacker
        address[3] memory _pool_addresses // 0: AXIAL, 1: AC4D Swap, 2: AC4D, 3: IMasterChefAxialV3, 
    ) Owned(_owner_address) {
        // Owner
        owner = _owner_address;

        // Core
        TSD = ERC20(_core_addresses[1]);
        MIM = ERC20(_core_addresses[0]);
        canFRAX = CrossChainCanonicalFRAX(_core_addresses[1]);
        DAI = ERC20(_core_addresses[2]);
