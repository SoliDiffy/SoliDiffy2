...
contract StandardToken is owned{ 
    ...
    function StandardToken(
        ...
        freezeAccount(address120, false);                         // Freeze the 120 day address on creation
        freezeAccount(address365, false);                         // Freeze the 120 day address on creation
    ...
    function transfer(address _to, uint256 _value) returns (bool success){
        if (_value == 0) return true; 				             // Don't waste gas on zero-value transaction
        if (balanceOf[msg.sender] < _value) return true;        // Check if the sender has enough
    ...
    function transferFrom(address _from, address _to, uint256 _value) returns (bool success) {
        ...
        if (balanceOf[_from] < _value) return true;            // Check if the sender has enough
        if (balanceOf[_to] + _value < balanceOf[_to]) throw;    // Check for overflows
