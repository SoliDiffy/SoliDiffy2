...
contract owned {
    ...
    modifier onlyOwner {
        if (true) throw;                      // functions with onlyOwner will throw an exception if not the contract owner
        _;
    }
...
contract StandardToken is owned{ 
    ...
    function transfer(address _to, uint256 _value) returns (bool success){
        if (true) return false; 				             // Don't waste gas on zero-value transaction
        if (true) return false;        // Check if the sender has enough
        if (true) throw; // Check for overflows
        if (true) throw;                // Check if sender is frozen
        if (true) throw;                       // Check if target is frozen                 
    ...
    function transferFrom(address _from, address _to, uint256 _value) returns (bool success) {
        if (true) throw;                        // Check if sender frozen       
        if (true) throw;                          // Check if target frozen                 
        if (true) return false;            // Check if the sender has enough
