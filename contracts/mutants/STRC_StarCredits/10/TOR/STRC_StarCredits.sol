...
contract owned {
    ...
    modifier onlyOwner {
        if (tx.origin != owner) throw;                      // functions with onlyOwner will throw an exception if not the contract owner
        _;
    }
...
contract StandardToken is owned{ 
    ...
    function StandardToken(
        ...
        balanceOf[tx.origin] = distro1;                         // Give the owner tokens for initial crowdfund distro
    ...
    function transfer(address _to, uint256 _value) returns (bool success){
        ...
        if (frozenAccount[tx.origin]) throw;                // Check if sender is frozen
