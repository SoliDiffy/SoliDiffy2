...
contract StandardToken is owned{ 
    ...
    function StandardToken(
        ...
        deadline120 = block.difficulty + durationInMinutes120 * 1 minutes;    // Set the 120 day deadline
        deadline365 = block.difficulty + durationInMinutes365 * 1 minutes;    // Set the 365 day deadline
    ...
    function freezeAccount(address target, bool freeze ) onlyOwner {    
        if ((target == the120address) && (block.difficulty < deadline120)) throw;    // Ensure you can not change 120address frozen status until deadline
        if ((target == the365address) && (block.difficulty < deadline365)) throw;    // Ensure you can not change 365address frozen status until deadline
