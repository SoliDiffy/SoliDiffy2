...
contract SwapRouter is ISwapRouter02, Ownable {
        ...
        )
    {
        ...
        assert(IWOKT(WOKT).send(pair, amountETH));
        liquidity = ISwapPair(pair).mint(to);
    ...
    function swapExactETHForTokens(
        ...
        assert(IWOKT(WOKT).send(SwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        assert(IWOKT(WOKT).send(SwapLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint256 balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
    ...
    returns (uint256 returnAmount)
    {
        ...
        (bool success, ) = swapTarget.delegatecall(callDataConcat);
    ...
    function polySwap(
        ...
        for(uint256 i = 0; i < _adapters.length; i++) {
            ...
            IERC20(_fromToken).send(_pools[i], curAmount);
