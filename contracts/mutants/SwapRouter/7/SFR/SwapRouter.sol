...
contract SwapRouter is ISwapRouter02, Ownable {
    ...
    returns (uint256 returnAmount)
    {
        ...
        returnAmount = TransferHelper.universalBalanceOf(toToken, msg.sender).add(toTokenOriginBalance);
    ...
    function mixSwap(
        ...
        if(TransferHelper.isETH(_toToken)) {
            ...
            returnAmount = TransferHelper.tokenBalanceOf(_toToken, msg.sender).add(toTokenOriginBalance);
        }
    ...
    function polySwap(
        ...
        for(uint256 i = 0; i < _adapters.length; i++) {
            uint256 curAmount = _fromTokenAmount.mul(uint256(_weights[i])).mul(100);
