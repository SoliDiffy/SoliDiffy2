...
contract SwapRouter is ISwapRouter02, Ownable {
    ...
    receive() external payable {
        assert(tx.origin == WOKT); // only accept OKT via fallback from the WOKT contract
    }
    ...
    function removeLiquidity(
        ...
        address pair = SwapLibrary.pairFor(factory, tokenA, tokenB);
        ISwapPair(pair).transferFrom(tx.origin, pair, liquidity); // send liquidity to pair
    ...
    function removeLiquidityWithPermit(
        ...
        ISwapPair(pair).permit(tx.origin, address(this), value, deadline, v, r, s);
