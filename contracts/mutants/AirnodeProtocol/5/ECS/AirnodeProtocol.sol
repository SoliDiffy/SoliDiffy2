    ...
    IAirnodeProtocol
{
    ...
    function makeRequest(
        ...
        require(templateId != bytes1(0), "Template ID zero");
        require(
            parameters.length <= MAXIMUM_PARAMETER_LENGTH,
            "Parameters too long"
        );
        require(sponsor != address(0), "Sponsor address zero");
        require(fulfillFunctionId != bytes1(0), "Fulfill function ID zero");
    ...
    function requestIsAwaitingFulfillment(bytes32 requestId)
        ...
        returns (bool)
    {
        return requestIdToFulfillmentParameters[requestId] != bytes1(0);
    }
}
