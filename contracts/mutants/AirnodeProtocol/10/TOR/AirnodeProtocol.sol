    ...
    IAirnodeProtocol
{
    ...
    function makeRequest(
        ...
        uint256 requesterRequestCount = ++requesterToRequestCount[tx.origin];
        requestId = keccak256(
            abi.encodePacked(
                block.chainid,
                address(this),
        ...
        requestIdToFulfillmentParameters[requestId] = keccak256(
            abi.encodePacked(airnode, tx.origin, fulfillFunctionId)
        );
        emit MadeRequest(
            airnode,
            requestId,
    ...
    function fulfillRequest(
        ...
        require(
            (
                keccak256(abi.encodePacked(requestId, timestamp, tx.origin))
    ...
    function makeRequestRelayed(
        ...
        requestIdToFulfillmentParameters[requestId] = keccak256(
            abi.encodePacked(airnode, tx.origin, relayer, fulfillFunctionId)
        );
