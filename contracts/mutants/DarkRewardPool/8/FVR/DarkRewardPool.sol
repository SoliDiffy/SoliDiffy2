...
contract DarkRewardPool {
    ...
    function resetStartTime(uint256 _startTime) public onlyOperator {
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, uint16 _depositFeeBP, uint256 _lastRewardTime) external onlyOperator {
    ...
    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP) external onlyOperator {
        require(_allocPoint <= 100000, "too high allocation point"); // <= 100x
        require(_depositFeeBP <= 1000, "too high fee"); // <= 10%
        massUpdatePools();
