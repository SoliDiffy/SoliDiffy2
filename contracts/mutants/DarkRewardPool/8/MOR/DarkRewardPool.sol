...
contract DarkRewardPool {
    ...
    function emergencyWithdraw(uint256 _pid) external onlyOperator {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 _amount, address _to) external lock {
