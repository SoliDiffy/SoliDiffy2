...
contract DarkRewardPool {
    ...
    function resetStartTime(uint256 _startTime) external lock {
    ...
    function setReserveFund(address _reserveFund) external lock {
        reserveFund = _reserveFund;
    }
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, uint16 _depositFeeBP, uint256 _lastRewardTime) public lock {
    ...
    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP) public lock {
        require(_allocPoint <= 100000, "too high allocation point"); // <= 100x
        require(_depositFeeBP <= 1000, "too high fee"); // <= 10%
        massUpdatePools();
    ...
    function deposit(uint256 _pid, uint256 _amount) public onlyOperator checkHalving {
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function withdraw(uint256 _pid, uint256 _amount) public onlyOperator checkHalving {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
