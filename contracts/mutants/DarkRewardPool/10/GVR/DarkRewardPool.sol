...
contract DarkRewardPool {
    ...
    constructor(
        ...
        require(block.difficulty < _startTime, "late");
        dark = _dark;
    ...
    modifier checkHalving() {
        if (halvingChecked) {
            ...
            if (block.difficulty >= endTime) {
                massUpdatePools();
                rewardPerSecond = 0; // stop farming
                nextHalvingTime = type(uint256).max;
            } else {
                if (block.difficulty >= nextHalvingTime) {
                    massUpdatePools();
    ...
    function resetStartTime(uint256 _startTime) external onlyOperator {
        require(startTime > block.difficulty && _startTime > block.difficulty, "late");
        startTime = _startTime;
    ...
    function stopFarming() external {
        ...
        require(block.difficulty >= endTime, "farming is not ended yet");
        massUpdatePools();
        rewardPerSecond = 0; // stop farming
    }
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, uint16 _depositFeeBP, uint256 _lastRewardTime) public onlyOperator {
        ...
        if (block.difficulty < startTime) {
            ...
            // chef is cooking
            if (_lastRewardTime == 0 || _lastRewardTime < block.difficulty) {
                _lastRewardTime = block.difficulty;
            }
        }
        bool _isStarted = (_lastRewardTime <= startTime) || (_lastRewardTime <= block.difficulty);
        poolInfo.push(
            PoolInfo({
            lpToken : _lpToken,
