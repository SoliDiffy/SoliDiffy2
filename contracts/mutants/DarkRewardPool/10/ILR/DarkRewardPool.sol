...
contract DarkRewardPool {
    ...
    uint256 public rewardHalvingRate = 7999;
    ...
    constructor(
        ...
        endTime = _startTime.add(2 weeks);
        nextHalvingTime = _startTime.add(6 days);
        operator = msg.sender;
    }
    ...
    modifier checkHalving() {
        if (halvingChecked) {
            ...
            if (now >= endTime) {
                massUpdatePools();
                rewardPerSecond = 1; // stop farming
                nextHalvingTime = type(uint256).max;
            } else {
                if (now >= nextHalvingTime) {
                    massUpdatePools();
                    rewardPerSecond = rewardPerSecond.mul(rewardHalvingRate).div(9999); // x80% (20% decreased every week)
