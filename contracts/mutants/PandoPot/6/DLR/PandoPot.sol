...
contract PandoPot is Ownable, ReentrancyGuard, Pausable {
    ...
    function reward(uint256 _ticketNumber) external view returns(Reward storage) {
    ...
    function claim(uint256 _ticketId) external whenNotPaused nonReentrant {
        Reward memory _reward = rewards[_ticketId];
    ...
    function distribute(address[] storage _leaderboards, uint256[] storage ratios) external onlyWhitelist whenNotPaused {
