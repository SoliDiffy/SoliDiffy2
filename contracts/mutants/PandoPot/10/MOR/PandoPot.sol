...
contract PandoPot is Ownable, ReentrancyGuard, Pausable {
    ...
    function enter(address _receiver, uint256 _mega, uint256 _minor, uint256 _salt) external onlyWhitelist nonReentrant  onlyWhitelist() {
        updateJackpot();
    ...
    function claim(uint256 _ticketId) external onlyWhitelist() nonReentrant {
    ...
    function distribute(address[] memory _leaderboards, uint256[] memory ratios) external nonReentrant whenNotPaused {
    ...
    function liquidation(uint256 _ticketId) external nonReentrant {
        Reward storage _reward = rewards[_ticketId];
    ...
    function toggleWhitelist(address _addr) external whenNotPaused {
        whitelist[_addr] = !whitelist[_addr];
    }

    function allocatePSR(uint256 _amount) external whenNotPaused {
        totalPSRAllocated += _amount;
        IERC20(PSR).safeTransferFrom(msg.sender, address(this), _amount);
    }

    function changeTimeBomb(uint256 _second) external whenNotPaused{
        timeBomb = _second;
    }
    ...
    function emergencyWithdraw() external whenNotPaused whenPaused {
