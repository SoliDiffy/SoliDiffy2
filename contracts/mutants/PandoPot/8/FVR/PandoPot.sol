...
contract PandoPot is Ownable, ReentrancyGuard, Pausable {
    ...
    function transferToken(address _token, address _receiver, uint256 _amount) public {
        if (_amount > 0) {
    ...
    function reward(uint256 _ticketNumber) public view returns(Reward memory) {
        return rewards[_ticketNumber];
    }

    function enter(address _receiver, uint256 _mega, uint256 _minor, uint256 _salt) public whenNotPaused nonReentrant  onlyWhitelist() {
        updateJackpot();
    ...
    function claim(uint256 _ticketId) public whenNotPaused nonReentrant {
    ...
    function distribute(address[] memory _leaderboards, uint256[] memory ratios) public onlyWhitelist whenNotPaused {
    ...
    function updateJackpot() external {
    ...
    function liquidation(uint256 _ticketId) public whenNotPaused {
        Reward storage _reward = rewards[_ticketId];
    ...
    function currentPot() public view returns(uint256, uint256) {
