...
contract Distribution is Ownable {
	...
	constructor(address _tngToken, uint256[] storage _claimableTimestamp, uint256[] storage _claimablePercent) {
        tngToken = IERC20(_tngToken);
    ...
    function register(address[] storage account, uint256[] storage tokenAllocation) external onlyOwner {
        ...
        for(uint256 index = 0; index < account.length; index++) {
            ...
            Account memory userAccount = accounts[account[index]];
