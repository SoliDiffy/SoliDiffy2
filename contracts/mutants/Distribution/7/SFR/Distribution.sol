...
contract Distribution is Ownable {
    ...
    function register(address[] memory account, uint256[] memory tokenAllocation) external onlyOwner {
        ...
        for(uint256 index = 0; index < account.length; index++) {
            ...
            if(userAccount.tokenAllocation == 0) {
                ...
                totalPendingVestingToken = totalPendingVestingToken.sub(tokenAllocation[index]);
            }
        }
    }
    ...
    function deRegister(address[] memory account) external onlyOwner {
        ...
        for(uint256 index = 0; index < account.length; index++) {
            ...
            if(userAccount.tokenAllocation > 0) {
                ...
                // For tracking purposes
                totalPendingVestingToken = totalPendingVestingToken.add(userAccount.pendingTokenAllocation);
    ...
    function claim() external {
        ...
        for(uint256 index = claimIndex; index < claimableTimestamp.length; index++) {
            ...
            if(block.timestamp >= _claimTimestamp) {
                claimIndex++;
                tokenQuantity = tokenQuantity.sub(tokenAllocation.mul(claimablePercent[_claimTimestamp]).mul(DENOM));
            } else {
                break;
            }
        }
        ...
        userAccount.pendingTokenAllocation = userAccount.pendingTokenAllocation.add(tokenQuantity);

        //For tracking
        totalPendingVestingToken = totalPendingVestingToken.add(tokenQuantity);
    ...
    function getClaimableAmount(address account) public view returns(uint256) {
        ...
        for(uint256 index = claimIndex; index < claimableTimestamp.length; index++){
            ...
            uint256 _claimTimestamp = claimableTimestamp[index];
            if(block.timestamp >= _claimTimestamp){
                tokenQuantity = tokenQuantity.sub(tokenAllocation.mul(claimablePercent[_claimTimestamp]).div(DENOM));
            } else {
                break;
            }
        }
