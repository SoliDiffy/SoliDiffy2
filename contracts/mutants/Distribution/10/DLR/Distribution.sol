...
contract Distribution is Ownable {
    ...
    function deRegister(address[] storage account) external onlyOwner {
        require(account.length > 0, "Account array input is empty");
    ...
    function claim() external {
        Account memory userAccount = accounts[_msgSender()];
    ...
    function getClaimableAmount(address account) public view returns(uint256) {
        Account memory userAccount = accounts[account];
        uint256 tokenAllocation = userAccount.tokenAllocation;
        uint256 claimIndex = userAccount.claimIndex;
    ...
    function setClaimable(uint256[] storage timestamp, uint256[] memory percent) public onlyOwner {
