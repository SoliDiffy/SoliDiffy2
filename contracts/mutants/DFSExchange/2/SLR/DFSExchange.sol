...
contract DFSExchange is DFSExchangeCore, AdminAuth {
    ...
    function sell(ExchangeData memory exData, address payable _user) public payable   {
        ...
        // log the event
        logger.Log(address(this), msg.sender, "", abi.encode(wrapper, exData.srcAddr, exData.destAddr, exData.srcAmount, destAmount));
    }
    ...
    function buy(ExchangeData memory exData, address payable _user) public payable   {
        ...
        // log the event
        logger.Log(address(this), msg.sender, "", abi.encode(wrapper, exData.srcAddr, exData.destAddr, srcAmount, exData.destAmount));
