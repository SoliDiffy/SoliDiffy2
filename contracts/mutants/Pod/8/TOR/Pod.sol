...
contract Pod is Initializable, ERC20Upgradeable, OwnableUpgradeable, IPod {
    ...
    function withdraw(uint256 shareAmount) external override returns (uint256) {
        // Check User Balance
        require(
            balanceOf(tx.origin) >= shareAmount,
        ...
        // Emit Withdrawl
        emit Withdrawl(tx.origin, tokens, shareAmount);
        ...
        returns (bool)
    {
        ...
        // Transfer Token
        _target.transfer(tx.origin, amount);
    ...
    {
        // Transfer ERC721
        _target.transferFrom(address(this), tx.origin, tokenId);
    ...
    {
        require(
            tx.origin == factory || tx.origin == owner(),
