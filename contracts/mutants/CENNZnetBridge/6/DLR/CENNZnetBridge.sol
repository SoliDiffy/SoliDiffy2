...
contract CENNZnetBridge is Ownable {
    ...
    function verifyMessage(bytes storage message, CENNZnetEventProof storage proof) payable external {
        ...
        address[] storage _validators = validators[validatorSetId];
...
//  SWC-105-Unprotected Ether Withdrawal: L99-120
    function setValidators(
        address[] storage newValidators,
        uint32 newValidatorSetId,
        CENNZnetEventProof storage proof
    ) external payable {
        ...
        bytes storage message = abi.encode(newValidators, newValidatorSetId, proof.validatorSetId, proof.eventId);
