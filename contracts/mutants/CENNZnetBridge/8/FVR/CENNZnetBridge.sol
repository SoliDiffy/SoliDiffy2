...
contract CENNZnetBridge is Ownable {
    ...
    function verifyMessage(bytes memory message, CENNZnetEventProof memory proof) payable public {
...
//  SWC-105-Unprotected Ether Withdrawal: L99-120
    ...
    function forceActiveValidatorSet(address[] memory _validators, uint32 validatorSetId) public onlyOwner {
    ...
    function forceHistoricValidatorSet(address[] memory _validators, uint32 validatorSetId) public onlyOwner {
        require(_validators.length > 0, "empty validator set");
    ...
    function setProofTTL(uint newTTL) public onlyOwner {
        proofTTL = newTTL;
    }

    // Set the fee for verify messages
    function setVerificationFee(uint newFee) public onlyOwner {
        verificationFee = newFee;
    }

    // Set the threshold % required for proof verification
    function setThreshold(uint newThresholdPercent) public onlyOwner {
