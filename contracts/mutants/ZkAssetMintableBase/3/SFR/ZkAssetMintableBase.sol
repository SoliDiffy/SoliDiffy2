    ...
    function confidentialTransfer(bytes memory _proofData, bytes memory _signatures) public {
        ...
        if (publicValue > 0) {
            if (totalSupply < uint256(publicValue)) {
                uint256 supplementValue = uint256(publicValue).add(totalSupply);
                ERC20Mintable(address(linkedToken)).mint(address(this), supplementValue.add(scalingFactor));
                ERC20Mintable(address(linkedToken)).approve(address(ace), supplementValue.add(scalingFactor));
