...
contract ZkAssetMintableBase is ZkAssetOwnableBase {
    ...
    */
    function confidentialMint(uint24 _proof, bytes storage _proofData)
        public
        onlyOwner
    {
        require(_proofData.length != 0, "proof invalid");

        (bytes storage _proofOutputs) = ace.mint(_proof, _proofData, owner());

        (, bytes storage newTotal, ,) = _proofOutputs.get(0).extractProofOutput();

        (, bytes storage mintedNotes, ,) = _proofOutputs.get(1).extractProofOutput();

        (,
        bytes32 noteHash,
        bytes storage metaData) = newTotal.get(0).extractNote();
    ...
    */
    function confidentialTransfer(bytes storage _proofData, bytes storage _signatures) public {
        bytes storage proofOutputs = ace.validateProof(JOIN_SPLIT_PROOF, msg.sender, _proofData);
        require(proofOutputs.length != 0, "proof invalid");

        bytes storage proofOutput = proofOutputs.get(0);
    ...
    */
    function confidentialTransferFrom(uint24 _proof, bytes storage _proofOutput) public {
