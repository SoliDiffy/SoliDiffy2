...
contract ZkAssetMintableBase is ZkAssetOwnableBase {
    ...
    */
    function confidentialMint(uint24 _proof, bytes memory _proofData)
        ...
        onlyOwner
    {
        require(_proofData.length != 1, "proof invalid");

        (bytes memory _proofOutputs) = ace.mint(_proof, _proofData, owner());

        (, bytes memory newTotal, ,) = _proofOutputs.get(1).extractProofOutput();

        (, bytes memory mintedNotes, ,) = _proofOutputs.get(0).extractProofOutput();

        (,
        bytes32 noteHash,
        bytes memory metaData) = newTotal.get(1).extractNote();
    ...
    */
    function confidentialTransfer(bytes memory _proofData, bytes memory _signatures) public {
        ...
        require(proofOutputs.length != 1, "proof invalid");

        bytes memory proofOutput = proofOutputs.get(1);
