...
contract ApeSwapLiquidityAMO_V2 is Owned {
    ...
    constructor (
        ...
        address[] storage _initial_pairs
    ...
    function showAllocations() public view returns (uint256[17] storage allocations) {
        // Get the FXS price
        uint256 fxs_price = cc_bridge_backer.cross_chain_oracle().getPrice(canonical_fxs_address);
        
        // Loop through the lp tokens
        uint256[] storage lp_tallies = new uint256[](4); // 0 = FRAX, 1 = FXS, 2 = Collateral, 3 = USD value
        for (uint i = 0; i < frax_fxs_pair_addresses_array.length; i++){ 
            ...
            if (frax_fxs_pair_addresses_allowed[pair_address]) {
                ...
                // Get the pair info
                uint256[4] storage lp_info_pack = lpTokenInfo(pair_address);
    ...
    function showTokenBalances() public view returns (uint256[3] storage tkn_bals) {
    ...
    // [3] = pair_type
    function lpTokenInfo(address pair_address) public view returns (uint256[4] storage return_info) {
        // Instantiate the pair
        IApePair the_pair = IApePair(pair_address);

        // Get the reserves
        uint256[] storage reserve_pack = new uint256[](3); // [0] = FRAX, [1] = FXS, [2] = Collateral
    ...
    function allDollarBalances() public view returns (
        ...
        uint256[17] storage allocations = showAllocations();
    ...
    ) public onlyByOwnGov returns (uint256[] storage amounts) {
        // Approval
