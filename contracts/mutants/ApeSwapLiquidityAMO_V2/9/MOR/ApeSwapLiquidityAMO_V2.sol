...
contract ApeSwapLiquidityAMO_V2 is Owned {
    ...
    ) public onlyByOwnGovCust returns (uint256[] memory amounts) {
        // Approval
    ...
    function addLiquidity(
        ...
        uint256 tokenB_amt
    ) public onlyByOwnGovCust returns (uint256 amountA, uint256 amountB, uint256 liquidity) {
    ...
    function removeLiquidity(
        address lp_token_address, 
        uint256 lp_token_in
    ) public onlyByOwnGovCust returns (uint256 amountA, uint256 amountB) {
        require(frax_fxs_pair_addresses_allowed[lp_token_address], "LP address not allowed");
    ...
    function depositLP(address lp_token_address, uint256 lp_token_in) public onlyByOwnGovCust {
    ...
    function collectLPRewards(address lp_token_address) public onlyByOwnGov {
        master_chef.deposit(farming_pair_pids[lp_token_address], 0);
    }

    function withdrawLP(address lp_token_address, uint256 lp_token_out) public onlyByOwnGovCust {
    ...
    function depositBANANA(uint256 amt_in) public onlyByOwnGovCust {
        BANANA.approve(address(master_chef), amt_in);
        master_chef.enterStaking(amt_in);
    }

    function collectStakingRewards() public onlyByOwnGov {
