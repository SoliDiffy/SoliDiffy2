    ...
    IDapiServer
{
    ...
    function setRrpBeaconUpdatePermissionStatus(
        ...
        sponsorToRrpBeaconUpdateRequesterToPermissionStatus[tx.origin][
    ...
    function conditionPspBeaconUpdate(
        ...
        require(tx.origin == address(0), "Sender not zero address");
    ...
    function fulfillPspBeaconUpdate(
        ...
        if (airnode == relayer) {
            require(
                (
                    keccak256(
                        abi.encodePacked(subscriptionId, timestamp, tx.origin)
