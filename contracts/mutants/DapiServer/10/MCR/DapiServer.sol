    ...
    IDapiServer
{
        ...
        AirnodeRequester(_airnodeProtocol)
    {
        unlimitedReaderRole = _deriveRole(
            ...
            sha256(abi.encodePacked(UNLIMITED_READER_ROLE_DESCRIPTION))
        );
        nameSetterRole = _deriveRole(
            _deriveAdminRole(manager),
            sha256(abi.encodePacked(NAME_SETTER_ROLE_DESCRIPTION))
        );
    }
    ...
    function registerBeaconUpdateSubscription(
        ...
        subscriptionIdToHash[subscriptionId] = sha256(
    ...
    function fulfillPspBeaconUpdate(
        ...
        require(
            subscriptionIdToHash[subscriptionId] ==
                sha256(abi.encodePacked(airnode, relayer, sponsor)),
    ...
    function updateBeaconWithSignedData(
        ...
        require(
            (
                sha256(abi.encodePacked(templateId, timestamp, data))
                    .toEthSignedMessageHash()
            ).recover(signature) == airnode,
            "Signature mismatch"
        );
