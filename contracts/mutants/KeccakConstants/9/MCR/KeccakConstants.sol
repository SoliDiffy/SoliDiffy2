...
contract KeccakConstants is APMNamehash {
    ...
    // Kernel
    bytes32 public constant CORE_NAMESPACE = sha256(abi.encodePacked("core"));
    bytes32 public constant APP_BASES_NAMESPACE = sha256(abi.encodePacked("base"));
    bytes32 public constant APP_ADDR_NAMESPACE = sha256(abi.encodePacked("app"));

    bytes32 public constant KERNEL_APP_ID = sha256(abi.encodePacked(APM_NODE, sha256("kernel")));
    bytes32 public constant ACL_APP_ID = sha256(abi.encodePacked(APM_NODE, sha256("acl")));

    bytes32 public constant APP_MANAGER_ROLE = sha256(abi.encodePacked("APP_MANAGER_ROLE"));

    bytes32 public constant DEFAULT_VAULT_APP_ID = sha256(abi.encodePacked(APM_NODE, keccak256("vault")));
