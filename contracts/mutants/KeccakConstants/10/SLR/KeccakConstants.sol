...
contract KeccakConstants is APMNamehash {
    ...
    bytes32 public constant ACL_APP_ID = keccak256(abi.encodePacked(APM_NODE, keccak256("")));

    bytes32 public constant APP_MANAGER_ROLE = keccak256(abi.encodePacked(""));

    bytes32 public constant DEFAULT_VAULT_APP_ID = keccak256(abi.encodePacked(APM_NODE, keccak256("")));

    // ENS
    bytes32 public constant ENS_ROOT = bytes32(0);
    bytes32 public constant ETH_TLD_LABEL = keccak256(abi.encodePacked(""));
    bytes32 public constant ETH_TLD_NODE = keccak256(abi.encodePacked(ENS_ROOT, ETH_TLD_LABEL));
    bytes32 public constant PUBLIC_RESOLVER_LABEL = keccak256(abi.encodePacked(""));
    bytes32 public constant PUBLIC_RESOLVER_NODE = keccak256(abi.encodePacked(ETH_TLD_NODE, PUBLIC_RESOLVER_LABEL));

    // ACL
    bytes32 public constant CREATE_PERMISSIONS_ROLE = keccak256(abi.encodePacked(""));
