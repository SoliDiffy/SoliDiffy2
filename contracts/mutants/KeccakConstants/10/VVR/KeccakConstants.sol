...
contract KeccakConstants is APMNamehash {
    ...
    // Kernel
    bytes32 internal constant CORE_NAMESPACE = keccak256(abi.encodePacked("core"));
    bytes32 internal constant APP_BASES_NAMESPACE = keccak256(abi.encodePacked("base"));
    bytes32 internal constant APP_ADDR_NAMESPACE = keccak256(abi.encodePacked("app"));

    bytes32 internal constant KERNEL_APP_ID = keccak256(abi.encodePacked(APM_NODE, keccak256("kernel")));
    bytes32 internal constant ACL_APP_ID = keccak256(abi.encodePacked(APM_NODE, keccak256("acl")));

    bytes32 internal constant APP_MANAGER_ROLE = keccak256(abi.encodePacked("APP_MANAGER_ROLE"));

    bytes32 internal constant DEFAULT_VAULT_APP_ID = keccak256(abi.encodePacked(APM_NODE, keccak256("vault")));

    // ENS
    bytes32 internal constant ENS_ROOT = bytes32(0);
    bytes32 internal constant ETH_TLD_LABEL = keccak256(abi.encodePacked("eth"));
