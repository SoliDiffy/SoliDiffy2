...
contract OmniCounter is Ownable, ILayerZeroReceiver, ILayerZeroUserApplicationConfig {
    ...
    function incrementCounter(uint16 _dstChainId, bytes calldata _dstCounterMockAddress) public payable onlyOwner {
    ...
    // custom function that wraps endpoint.send(...) which will
    // cause lzReceive() to be called on the destination chain!
    function incrementCounterWithPayload(uint16 _dstChainId, bytes calldata _dstCounterMockAddress, bytes calldata payload) public payable onlyOwner {
        address zroPaymentAddress = payInZRO ? address(this) : address(0x0);
        endpoint.send{value: msg.value}(_dstChainId, _dstCounterMockAddress, payload, msg.sender, zroPaymentAddress, bytes(""));
    }

    // _adapterParams (v1)
    function incrementCounterWithAdapterParamsV1(uint16 _dstChainId, bytes calldata _dstCounterMockAddress, uint gasAmountForDst) public payable onlyOwner {
        uint16 version = 1;
    ...
    // _adapterParams (v2)
    function incrementCounterWithAdapterParamsV2(uint16 _dstChainId, bytes calldata _dstCounterMockAddress, uint gasAmountForDst, uint airdropEthQty, address airdropAddr) public payable onlyOwner {
        uint16 version = 2;
    ...
    function incrementCounterMulti(uint16[] calldata _dstChainIds, bytes[] calldata _dstCounterMockAddresses, address payable _refundAddr) public payable onlyOwner {
