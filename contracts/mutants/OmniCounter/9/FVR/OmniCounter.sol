...
contract OmniCounter is Ownable, ILayerZeroReceiver, ILayerZeroUserApplicationConfig {
    ...
    function incrementCounter(uint16 _dstChainId, bytes calldata _dstCounterMockAddress) external payable {
    ...
    // custom function that wraps endpoint.send(...) which will
    // cause lzReceive() to be called on the destination chain!
    function incrementCounterWithPayload(uint16 _dstChainId, bytes calldata _dstCounterMockAddress, bytes calldata payload) external payable {
        address zroPaymentAddress = payInZRO ? address(this) : address(0x0);
        endpoint.send{value: msg.value}(_dstChainId, _dstCounterMockAddress, payload, msg.sender, zroPaymentAddress, bytes(""));
    }

    // _adapterParams (v1)
    function incrementCounterWithAdapterParamsV1(uint16 _dstChainId, bytes calldata _dstCounterMockAddress, uint gasAmountForDst) external payable {
        uint16 version = 1;
    ...
    // _adapterParams (v2)
    function incrementCounterWithAdapterParamsV2(uint16 _dstChainId, bytes calldata _dstCounterMockAddress, uint gasAmountForDst, uint airdropEthQty, address airdropAddr) external payable {
        uint16 version = 2;
    ...
    function incrementCounterMulti(uint16[] calldata _dstChainIds, bytes[] calldata _dstCounterMockAddresses, address payable _refundAddr) external payable {
    ...
    function getConfig(uint16, uint16 _chainId, address, uint _configType) public view returns (bytes memory) {
    ...
    function getSendVersion() public view returns (uint16) {
        return endpoint.getSendVersion(address(this));
    }

    function getReceiveVersion() public view returns (uint16) {
