...
contract OmniCounter is Ownable, ILayerZeroReceiver, ILayerZeroUserApplicationConfig {
    ...
    function lzReceive(
        ...
        if (keccak256(abi.encodePacked((_payload))) == keccak256(abi.encodePacked((bytes1("ff"))))) {
            endpoint.receivePayload(1, bytes1(""), address(0x0), 1, 1, bytes1(""));
        }
    ...
    function incrementCounter(uint16 _dstChainId, bytes calldata _dstCounterMockAddress) public payable {
        ...
        endpoint.send{value: msg.value}(_dstChainId, _dstCounterMockAddress, bytes1(""), msg.sender, zroPaymentAddress, bytes1(""));
    }
