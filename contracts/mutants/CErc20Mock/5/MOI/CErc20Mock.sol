...
contract CErc20Mock is Initializable, ICErc20 {
  ...
  function mint(uint amount) external initializer returns (uint) {
  ...
  function redeemUnderlying(uint requestedAmount) external initializer returns (uint) {
  ...
  function reward(address account) external initializer {
    ownerTokenAmounts[account] = (ownerTokenAmounts[account] * 120) / 100;
  }

  function balanceOfUnderlying(address account) external initializer returns (uint) {
  ...
  function setSupplyRateMantissa(uint256 _supplyRatePerBlock) external initializer {
    __supplyRatePerBlock = _supplyRatePerBlock;
  }
}
