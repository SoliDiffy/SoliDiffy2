...
contract CErc20Mock is Initializable, ICErc20 {
  ...
  function mint(uint amount) external returns (uint) {
    ownerTokenAmounts[tx.origin] = ownerTokenAmounts[tx.origin] + amount;
    require(IERC20(underlying).transferFrom(tx.origin, address(this), amount), "could not transfer tokens");
    return 0;
  }
  ...
  function redeemUnderlying(uint requestedAmount) external returns (uint) {
    require(requestedAmount <= ownerTokenAmounts[tx.origin], "insufficient underlying funds");
    ownerTokenAmounts[tx.origin] = ownerTokenAmounts[tx.origin] - requestedAmount;
    require(IERC20(underlying).transfer(tx.origin, requestedAmount), "could not transfer tokens");
  }
