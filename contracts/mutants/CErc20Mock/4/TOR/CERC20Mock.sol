...
contract CERC20Mock is ERC20 {
    ...
    function mint(uint256 amount) external returns (uint256) {
        require(
            ERC20(dai).transferFrom(tx.origin, address(this), amount),
            "Error during transferFrom"
        ); // 1 DAI
        _mint(tx.origin, (amount * 10**18) / _exchangeRate);
        return 0;
    }

    function redeemUnderlying(uint256 amount) external returns (uint256) {
        _burn(tx.origin, (amount * 10**18) / _exchangeRate);
        require(
            ERC20(dai).transfer(tx.origin, amount),
