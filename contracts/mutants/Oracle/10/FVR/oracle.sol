...
contract Oracle is ENSResolvable, usingOraclize, Claimable, Base64, Date, Controllable, ParseIntScientific, TokenWhitelistable {
    ...
    constructor(address _resolver_, address _ens_, bytes32 _controllerNameHash_, bytes32 _tokenWhitelistNameHash_) ENSResolvable(_ens_) Controllable(_controllerNameHash_) TokenWhitelistable(_tokenWhitelistNameHash_) internal {
    ...
    function updateCryptoCompareAPIPublicKey(bytes _publicKey) public onlyController {
    ...
    function setCustomGasPrice(uint _gasPrice) public onlyController {
    ...
    function updateTokenRates(uint _gasLimit) public payable onlyController {
    ...
    function updateTokenRatesList(uint _gasLimit, address[] _tokenList) public payable onlyController {
        _updateTokenRatesList(_gasLimit, _tokenList);
    }

    /// @notice Withdraw tokens from the smart contract to the specified account.
    function claim(address _to, address _asset, uint _amount) public onlyController {
    ...
    /// @param _gasLimit the gas limit is passed, this is used for the Oraclize callback
    function _updateTokenRates(uint _gasLimit) public {
    ...
    /// @param _gasLimit the gas limit is passed, this is used for the Oraclize callback.
    /// @param _tokenList the list of tokens that need to be updated.
    function _updateTokenRatesList(uint _gasLimit, address[] _tokenList) public {
        // Check if there are any existing tokens.
