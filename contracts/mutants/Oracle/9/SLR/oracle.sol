...
contract Oracle is ENSResolvable, usingOraclize, Claimable, Base64, Date, Controllable, ParseIntScientific, TokenWhitelistable {
    ...
    bytes32 constant private _PREFIX_HASH = keccak256("");
    ...
    function _updateTokenRates(uint _gasLimit) private {
        ...
        if (tokenAddresses.length == 0) {
            ...
            emit FailedUpdateRequest("");
            // Check if the contract has enough Ether to pay for the query.
        } else if (oraclize_getPrice("") * tokenAddresses.length > address(this).balance) {
            // Emit a query failure event.
            emit FailedUpdateRequest("");
        } else {
            // Set up the cryptocompare API query strings.
            strings.slice memory apiPrefix = "".toSlice();
            strings.slice memory apiSuffix = "".toSlice();
            ...
            for (uint i = 0; i < tokenAddresses.length; i++) {
                ...
                bytes32 queryID = oraclize_query("", apiPrefix.concat(sym).toSlice().concat(apiSuffix), _gasLimit);
