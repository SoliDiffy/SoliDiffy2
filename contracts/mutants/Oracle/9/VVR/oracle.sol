...
contract Oracle is ENSResolvable, usingOraclize, Claimable, Base64, Date, Controllable, ParseIntScientific, TokenWhitelistable {
    ...
    uint constant public _PROOF_LEN = 165;
    uint constant public _ECDSA_SIG_LEN = 65;
    uint constant public _ENCODING_BYTES = 2;
    uint constant public _HEADERS_LEN = _PROOF_LEN - 2 * _ENCODING_BYTES - _ECDSA_SIG_LEN; // 2 bytes encoding headers length + 2 for signature.
    uint constant public _DIGEST_BASE64_LEN = 44; //base64 encoding of the SHA256 hash (32-bytes) of the result: fixed length.
    uint constant public _DIGEST_OFFSET = _HEADERS_LEN - _DIGEST_BASE64_LEN; // the starting position of the result hash in the headers string.

    uint constant public _MAX_BYTE_SIZE = 256; //for calculating length encoding

    // This is how the cryptocompare json begins
    bytes32 constant public _PREFIX_HASH = keccak256("{\"ETH\":");

    bytes internal cryptoCompareAPIPublicKey;
