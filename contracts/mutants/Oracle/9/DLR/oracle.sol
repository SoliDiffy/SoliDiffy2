...
contract Oracle is ENSResolvable, usingOraclize, Claimable, Base64, Date, Controllable, ParseIntScientific, TokenWhitelistable {
    ...
    function _updateTokenRates(uint _gasLimit) private {
        address[] storage tokenAddresses = _tokenAddressArray();
        ...
        if (tokenAddresses.length == 0) {
            ...
            strings.slice storage apiPrefix = "https://min-api.cryptocompare.com/data/price?fsym=".toSlice();
            strings.slice storage apiSuffix = "&tsyms=ETH&sign=true".toSlice();

            // Create a new oraclize query for each supported token.
            for (uint i = 0; i < tokenAddresses.length; i++) {
                // Store the token symbol used in the query.
                (string storage symbol, , , , , , ) = _getTokenInfo(tokenAddresses[i]);

                strings.slice storage sym = symbol.toSlice();
