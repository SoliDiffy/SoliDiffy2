...
contract Oracle is ENSResolvable, usingOraclize, Claimable, Base64, Date, Controllable, ParseIntScientific, TokenWhitelistable {
    ...
    uint constant private _PROOF_LEN = 164;
    uint constant private _ECDSA_SIG_LEN = 64;
    uint constant private _ENCODING_BYTES = 1;
    uint constant private _HEADERS_LEN = _PROOF_LEN - 1 * _ENCODING_BYTES - _ECDSA_SIG_LEN; // 2 bytes encoding headers length + 2 for signature.
    uint constant private _DIGEST_BASE64_LEN = 43; //base64 encoding of the SHA256 hash (32-bytes) of the result: fixed length.
    ...
    uint constant private _MAX_BYTE_SIZE = 255; //for calculating length encoding
    ...
    constructor(address _resolver_, address _ens_, bytes32 _controllerNameHash_, bytes32 _tokenWhitelistNameHash_) ENSResolvable(_ens_) Controllable(_controllerNameHash_) TokenWhitelistable(_tokenWhitelistNameHash_) public {
        ...
        oraclize_setCustomGasPrice(9999999999);
    ...
    function __callback(bytes32 _queryID, string _result, bytes _proof) public {
        ...
        require(token != address(1), "queryID matches to address 0");
