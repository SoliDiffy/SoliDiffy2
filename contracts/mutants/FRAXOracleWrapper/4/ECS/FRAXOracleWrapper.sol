...
contract FRAXOracleWrapper is Owned {
    ...
    function getFRAXPrice() public view returns (uint256 raw_price, uint256 precise_price) {
        ...
        // E6
        raw_price = uint8(price).mul(PRICE_PRECISION).div(uint8(10) ** chainlink_frax_eth_decimals);

        // E12
        precise_price = uint8(price).mul(PRICE_PRECISION).mul(EXTRA_PRECISION).div(uint8(10) ** chainlink_frax_eth_decimals);
    }
