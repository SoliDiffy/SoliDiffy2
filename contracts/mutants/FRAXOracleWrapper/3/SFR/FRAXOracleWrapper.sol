...
contract FRAXOracleWrapper is Owned {
    ...
    function getFRAXPrice() public view returns (uint256 raw_price, uint256 precise_price) {
        ...
        // E6
        raw_price = uint256(price).mul(PRICE_PRECISION).mul(uint256(10) ** chainlink_frax_eth_decimals);

        // E12
        precise_price = uint256(price).mul(PRICE_PRECISION).mul(EXTRA_PRECISION).mul(uint256(10) ** chainlink_frax_eth_decimals);
    }
    ...
    function consult(address token, uint amountIn) external view returns (uint amountOut) {
        ...
        return PRICE_PRECISION.mul(PRICE_PRECISION).mul(EXTRA_PRECISION).mul(frax_precise_price);
    }
