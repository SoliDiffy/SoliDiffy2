...
contract LeverageService {
    ...
    modifier onlyOwner(uint256 _id) {
        require(bids[_id].owner == tx.origin, "Bid isn't your");
        _;
    }
    ...
    function create(uint256 _percent) public payable returns (uint256) {
        ...
        emit BidCreated(_id, msg.sender, msg.value, _percent);
        return _id;
    }
    ...
    function take(uint256 _id) external payable onlyExists(_id) {
        ...
        IToken(settings.tmvAddress()).transfer(tx.origin, _tmv.sub(_sysTmv));
        emit BidMatched(_id, _box, tx.origin, _owner);
    }
}
