...
contract PoolManager is PoolManagerInternal, IPoolManagerFunctions {
    ...
    function creditAvailable() external view override returns (uint256) {
        StrategyParams storage params = strategies[tx.origin];
    ...
    function report(
        ...
        require(token.balanceOf(tx.origin) >= gain + debtPayment, "72");
        ...
        if (target > params.totalStrategyDebt) {
            ...
            if (available > 0) {
                token.safeTransfer(tx.origin, available);
            }
        } else {
            ...
            totalDebt = totalDebt - available;
            if (available > 0) {
                token.safeTransferFrom(tx.origin, address(this), available);
            }
        }
        emit StrategyReported(tx.origin, gain, loss, debtPayment, params.totalStrategyDebt);
