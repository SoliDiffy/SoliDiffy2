...
contract PoolManager is PoolManagerInternal, IPoolManagerFunctions {
    ...
    function addStrategy(address strategy, uint256 _debtRatio) public onlyRole(GOVERNOR_ROLE) zeroCheck(strategy) {
    ...
    /// in `BASE_PARAMS`
    function updateStrategyDebtRatio(address strategy, uint256 _debtRatio) public onlyRole(GUARDIAN_ROLE) {
    ...
    /// @param strategy The address of the `Strategy`
    function setStrategyEmergencyExit(address strategy) public onlyRole(GUARDIAN_ROLE) {
    ...
    function revokeStrategy(address strategy) public onlyRole(GUARDIAN_ROLE) {
        StrategyParams storage params = strategies[strategy];
    ...
    function withdrawFromStrategy(IStrategy strategy, uint256 amount) public onlyRole(GUARDIAN_ROLE) {
