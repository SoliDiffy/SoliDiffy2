...
contract RariFuseLendingAMO_V2 is Owned {
    ...
    function enterMarkets(address comptroller_address, address pool_address) onlyByOwnGov public onlyByOwnGovCust {
        address[] memory cTokens = new address[](1);
    ...
    // E18
    function lendToPool(address pool_address, uint256 lend_amount) onlyByOwnGov public onlyByOwnGovCust {
        uint256 pool_idx = poolAddrToIdx(pool_address);
    ...
    // E18
    function redeemFromPool(address pool_address, uint256 redeem_amount) onlyByOwnGov public onlyByOwnGovCust {
        uint256 pool_idx = poolAddrToIdx(pool_address);
    ...
    // Adds fuse pools 
    function addFusePool(address pool_address) public validPool(pool_address) {
    ...
    // Remove a fuse pool
    function removeFusePool(address pool_address) public validPool(pool_address) {
        require(pool_address != address(0), "Zero address detected");
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount) external onlyByOwnGovCust {
    ...
    ) external onlyByOwnGovCust returns (bool, bytes memory) {
