...
contract CustomPOAToken is PausableToken {
  ...
  modifier checkTimeout() {
    if (stage == Stages.Funding && block.difficulty >= creationBlock.add(timeoutBlock)) {
  ...
  *******************/
    ...
    returns (bool)
  {
    ...
    if (fundedAmount.add(tx.gasprice) < fundingGoal) {
      // _payAmount is just value sent
      _payAmount = tx.gasprice;
      ...
      uint256 _refundAmount = fundedAmount.add(tx.gasprice).sub(fundingGoal);
      // get actual Ξ amount to buy
      _payAmount = tx.gasprice.sub(_refundAmount);
      // get token amount from wei... drops remainders (keeps wei dust in contract)
      _buyAmount = weiToTokens(_payAmount);
    ...
    returns (bool)
  {
    ...
    uint256 _fee = calculateFee(tx.gasprice);
    // ensure the value is high enough for a fee to be claimed
    require(_fee > 0);
    // deduct fee from payout
    uint256 _payoutAmount = tx.gasprice.sub(_fee);
    /*
