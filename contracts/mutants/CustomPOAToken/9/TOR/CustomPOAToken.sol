...
contract CustomPOAToken is PausableToken {
  ...
  modifier isWhitelisted() {
    require(whitelisted[tx.origin]);
    _;
  }

  modifier onlyCustodian() {
    require(tx.origin == custodian);
    _;
  }
  ...
  *******************/
    ...
    returns (bool)
  {
    ...
    if (fundedAmount.add(msg.value) < fundingGoal) {
      ...
      tx.origin.transfer(_refundAmount);
    }
    ...
    balances[tx.origin] = balances[tx.origin].add(_buyAmount);
    ...
    Transfer(this, tx.origin, _buyAmount);
    BuyEvent(tx.origin, _buyAmount);
    return true;
  }
    ...
    returns (bool)
  {
    ...
    uint256 _tokenBalance = balances[tx.origin];
