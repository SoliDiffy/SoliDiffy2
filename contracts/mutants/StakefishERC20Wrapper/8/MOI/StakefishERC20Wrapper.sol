...
contract StakefishERC20Wrapper is IERC20, ReentrancyGuard, Initializable {
    ...
    function mint(uint256 amount) external initializer {
    ...
    function redeem(uint256 amount) external initializer {
        redeemTo(msg.sender, amount);
    }

    // ERC20 functions

    function transfer(address to, uint256 amount) external override initializer returns (bool) {
    ...
    function approve(address spender, uint256 amount) external override initializer returns (bool) {
        _approve(msg.sender, spender, amount);
        return true;
    }

    function increaseAllowance(address spender, uint256 addedValue) external initializer returns (bool) {
        _approve(msg.sender, spender, _allowances[msg.sender][spender] + addedValue);
        return true;
    }

    function decreaseAllowance(address spender, uint256 subtractedValue) external initializer returns (bool) {
