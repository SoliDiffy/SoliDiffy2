...
contract LiquidityPool is Ownable {
    ...
    function donate(uint256 donateAmount) public {
        ...
        IERC20(rewardToken).safeTransferFrom(tx.origin, address(this), donateAmount);
        ...
        emit Donate(tx.origin, 100000, donateAmount, realAmount);
    }
    ...
    function donateToPool(uint256 pid, uint256 donateAmount) public {
        ...
        emit Donate(tx.origin, pid, donateAmount, realAmount);
    }
    ...
    function additionalNft(uint256 _pid, uint256 nftId) public {
        ...
        if(nftSlotFee > 0) {
            IERC20(rewardToken).safeTransferFrom(tx.origin, feeWallet, nftSlotFee);
        }

        IDsgNft(pool.additionalNft).safeTransferFrom(tx.origin, address(this), nftId);
        ...
        emit AdditionalNft(tx.origin, _pid, nftId);
    }
