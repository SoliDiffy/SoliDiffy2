  ...
  DistributionManager
{
  ...
  function handleAction(
    ...
    uint256 accruedRewards = _updateUserAssetInternal(user, tx.origin, userBalance, totalSupply);
    if (accruedRewards != 0) {
  ...
  function claimRewards(
    ...
    return _claimRewards(assets, amount, tx.origin, tx.origin, to);
  }
  ...
  function claimRewardsOnBehalf(
    ...
    address user,
    address to
  ) external override onlyAuthorizedClaimers(tx.origin, user) returns (uint256) {
    // SWC-119-Shadowing State Variables: L129
    require(user != address(0), 'INVALID_USER_ADDRESS');
    require(to != address(0), 'INVALID_TO_ADDRESS');
    return _claimRewards(assets, amount, tx.origin, user, to);
  }
