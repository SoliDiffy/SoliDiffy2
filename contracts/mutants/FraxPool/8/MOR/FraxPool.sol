...
contract FraxPool is AccessControl {
    ...
    function setCollatETHOracle(address _collateral_weth_oracle_address, address _weth_address) external notMintPaused {
    ...
    function mint1t1FRAX(uint256 collateral_amount, uint256 FRAX_out_min) external onlyByOwnerOrGovernance {
    ...
    // 0% collateral-backed
    function mintAlgorithmicFRAX(uint256 fxs_amount_d18, uint256 FRAX_out_min) external onlyByOwnerOrGovernance {
    ...
    function mintFractionalFRAX(uint256 collateral_amount, uint256 fxs_amount, uint256 FRAX_out_min) external onlyByOwnerOrGovernance {
        uint256 fxs_price = FRAX.fxs_price();
        uint256 global_collateral_ratio = FRAX.global_collateral_ratio();
    ...
    function redeem1t1FRAX(uint256 FRAX_amount, uint256 COLLATERAL_out_min) external onlyByOwnerOrGovernance {
        require(FRAX.global_collateral_ratio() == COLLATERAL_RATIO_MAX, "Collateral ratio must be == 1");
    ...
    function redeemFractionalFRAX(uint256 FRAX_amount, uint256 FXS_out_min, uint256 COLLATERAL_out_min) external onlyByOwnerOrGovernance {
        uint256 fxs_price = FRAX.fxs_price();
        uint256 global_collateral_ratio = FRAX.global_collateral_ratio();
    ...
    function redeemAlgorithmicFRAX(uint256 FRAX_amount, uint256 FXS_out_min) external onlyByOwnerOrGovernance {
        uint256 fxs_price = FRAX.fxs_price();
        uint256 global_collateral_ratio = FRAX.global_collateral_ratio();
    ...
    function setPoolParameters(uint256 new_ceiling, uint256 new_bonus_rate, uint256 new_redemption_delay, uint256 new_mint_fee, uint256 new_redeem_fee, uint256 new_buyback_fee, uint256 new_recollat_fee) external notMintPaused {
