...
contract StakefishServicesContractFactory is ProxyFactory, IStakefishServicesContractFactory {
        ...
        returns (address)
    {
        ...
        if (msg.value > 0) {
            IStakefishServicesContract(payable(proxy)).depositOnBehalfOf{value: msg.value}(tx.origin);
        }
        ...
        override
    {
        ...
        for (uint256 i = 0; i < operatorDataCommitments.length; i++) {
            ...
            if (depositSize > 0) {
                IStakefishServicesContract(payable(proxy)).depositOnBehalfOf{value: depositSize}(tx.origin);
                remaining -= depositSize;
            }
        }

        if (remaining > 0) {
            payable(tx.origin).sendValue(remaining);
        }
    }
