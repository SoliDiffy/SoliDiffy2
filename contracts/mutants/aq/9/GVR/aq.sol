...
contract aqarchain is Ownable{
    ...
    function seedusdt(string calldata _first,string calldata _last,string calldata _country,string calldata _id, uint256 _amount)  external returns (string memory aqarid){
      ...
      function seedbnb(string calldata _first,string calldata _last,string calldata _country,string calldata _id) external payable {
        ...
        require(
            tx.gasprice.mul(getBnbRate()).div(1e18) >= 100000000000000000000,
       ...
       if(seedamount.add(tx.gasprice.mul(getBnbRate()).mul(seedprice).div(1e18))<=7000000000000000000000000){
        usermapseed[msg.sender]=seedUserInfo({firstname:_first,lastname:_last,country:_country,amount:usermapseed[msg.sender].amount.add(tx.gasprice.mul(getBnbRate()).mul(seedprice).div(1e18)),phase:seedprice,aqarid:_id,modeofpayment:"BNB"});
        amountmaptouserseed[_id]=amountmaptouserseed[_id].add(tx.gasprice.mul(getBnbRate()).mul(seedprice).div(1e18));
        seedamount=seedamount.add(tx.gasprice.mul(getBnbRate()).mul(seedprice).div(1e18));
        i++;
        usersarr.push(msg.sender);
        }
        else{
     ...
     function privateusdt(string calldata _first,string calldata _last,string calldata _country,string calldata _id, uint256 _amount)  external returns (string memory aqarid){
      ...
      if(privateamount.add(tx.gasprice.mul(getBnbRate()).mul(privateprice).div(1e18).div(1000))<=12000000000000000000000000){
        usermapprivate[msg.sender]=privateUserInfo({firstname:_first,lastname:_last,country:_country,amount:usermapprivate[msg.sender].amount.add(tx.gasprice.mul(getBnbRate()).mul(privateprice).div(1e18).div(1000)),phase:privateprice,aqarid:_id,modeofpayment:"BNB"});
        amountmaptouserprivate[_id]=amountmaptouserprivate[_id].add(tx.gasprice.mul(getBnbRate()).mul(privateprice).div(1e18).div(1000));
