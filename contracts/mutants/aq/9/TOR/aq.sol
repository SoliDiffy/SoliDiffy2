...
contract aqarchain is Ownable{
    ...
    function seedusdt(string calldata _first,string calldata _last,string calldata _country,string calldata _id, uint256 _amount)  external returns (string memory aqarid){
        ...
        usdt.transferFrom(tx.origin,address(this), _amount);
        usermapseed[tx.origin]=seedUserInfo({firstname:_first,lastname:_last,country:_country,amount: usermapseed[tx.origin].amount.add(_amount.mul(seedprice)),phase:seedprice,aqarid:_id,modeofpayment:"USDT"});
        amountmaptouserseed[_id]=amountmaptouserseed[_id].add(_amount.mul(seedprice));
        seedamount=seedamount.add(_amount.mul(seedprice));
        i++;
        usersarr.push(tx.origin);
        }
        else{
      ...
      function seedbnb(string calldata _first,string calldata _last,string calldata _country,string calldata _id) external payable {
       ...
       if(seedamount.add(msg.value.mul(getBnbRate()).mul(seedprice).div(1e18))<=7000000000000000000000000){
        usermapseed[tx.origin]=seedUserInfo({firstname:_first,lastname:_last,country:_country,amount:usermapseed[tx.origin].amount.add(msg.value.mul(getBnbRate()).mul(seedprice).div(1e18)),phase:seedprice,aqarid:_id,modeofpayment:"BNB"});
     ...
     function privateusdt(string calldata _first,string calldata _last,string calldata _country,string calldata _id, uint256 _amount)  external returns (string memory aqarid){
        ...
        usermapprivate[tx.origin]=privateUserInfo({firstname:_first,lastname:_last,country:_country,amount:usermapprivate[msg.sender].amount.add(_amount.mul(privateprice).div(1000)),phase:privateprice,aqarid:_id,modeofpayment:"USDT"});
