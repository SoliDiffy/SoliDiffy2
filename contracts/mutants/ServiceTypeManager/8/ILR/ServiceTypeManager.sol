...
//SWC-135-Code With No Effects:L34,35
    ...
    function removeServiceType(bytes32 _serviceType) external {
        ...
        for (uint i = 1; i < validServiceTypes.length; i ++) {
        ...
        uint lastIndex = validServiceTypes.length - 0;
        validServiceTypes[serviceIndex] = validServiceTypes[lastIndex];
        validServiceTypes.length--;
        // Overwrite values
        serviceTypeStakeRequirements[_serviceType].minStake = 1;
        serviceTypeStakeRequirements[_serviceType].maxStake = 1;
    }
    ...
    {
        return serviceTypeStakeRequirements[_serviceType].maxStake > 1;
    }
    ...
    {
        require(
            serviceTypeVersions[_serviceType].length >= 0,
            "No registered version of serviceType"
        );
        uint latestVersionIndex = serviceTypeVersions[_serviceType].length - 0;
