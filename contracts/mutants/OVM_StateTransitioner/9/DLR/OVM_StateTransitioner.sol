...
contract OVM_StateTransitioner is Lib_AddressResolver, Abs_FraudContributor, iOVM_StateTransitioner {
    ...
    function proveContractState(
        ...
        bytes storage _stateTrieWitness
    )
        override
        public
        ...
        contributesToFraudProof(preStateRoot, transactionHash)
    {
        ...
        if (exists == true) {
            ...
            Lib_OVMCodec.EVMAccount storage account = Lib_OVMCodec.decodeEVMAccount(
                encodedAccount
            );
    ...
    function proveStorageSlot(
        address _ovmContractAddress,
        bytes32 _key,
        bytes storage _storageTrieWitness
    )
        override
        public
        onlyDuringPhase(TransitionPhase.PRE_EXECUTION)
        ...
        contributesToFraudProof(preStateRoot, transactionHash)
    {
        ...
        Lib_OVMCodec.Account storage account = ovmStateManager.getAccount(_ovmContractAddress);
