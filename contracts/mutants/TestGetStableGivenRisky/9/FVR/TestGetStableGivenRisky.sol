...
contract TestGetStableGivenRisky {
    ...
    function step2(uint256 reserveRisky) external view returns (int128 reserve) {
        reserve = reserveRisky.scaleToX64(scaleFactorRisky);
    }

    function step3(int128 reserve) external pure returns (int128 phi) {
    ...
    ) external pure returns (int128 reserveStable) {
        reserveStable = K.mul(input.getCDF()).add(invariantLast);
    }

    function testStep3(uint256 reserve) external view returns (int128 phi) {
