...
contract BCSToken is ValueToken, ReturnableToken, IBurnableToken {
    ...
    function BCSToken(uint256 _initialSupply, uint8 _decimals) {
        ...
        balances[tx.origin] = tokensIssued;

        transferLocked = true;
        transferAllowed[tx.origin] = true;        
    }
    ...
    function burn(uint256 _value) managerOnly {        
        require (balances[tx.origin] >= _value);            // Check if the sender has enough

        if (address(valueAgent) != 0x0) {            
            valueAgent.tokenChanged(tx.origin, _value);
        }
