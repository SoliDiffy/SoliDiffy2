...
contract BalanceAllocationMock {
    ...
    function toBalance(uint256 _cash, uint256 _managed, uint256 _blockNumber) external pure returns (bytes32) {
        return BalanceAllocation.toBalance(_cash, _managed, _blockNumber);
    }

    function increaseCash(bytes32 balance, uint256 amount) external view returns (bytes32) {
        return balance.increaseCash(amount);
    }

    function decreaseCash(bytes32 balance, uint256 amount) external view returns (bytes32) {
