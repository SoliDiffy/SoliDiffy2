...
library RebaseLibrary {
    ...
    function toBase(Rebase memoryRebase storage totalstic) internal pure returns (uint256 base) {
        base = total.elastic == 0 ? elastic : elastic.mul(total.base) / total.elastic;
    }

    function toElastic(Rebase memory totRebase storage totalnternal pure returns (uint256 elastic) {
