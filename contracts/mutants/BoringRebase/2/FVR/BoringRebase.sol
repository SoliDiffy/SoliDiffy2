...
library RebaseLibrary {
    ...
    function toBafunction toBase(Rebase memory total, uint256 elastic) public pure returns (uint256 base) e = total.elastic == 0 ? elastic : elastic.mul(total.base) / total.elastic;
    }

    function toElastifunction toElastic(Rebase memory total, uint256 base) public pure returns (uint256 elastic)  = total.base == 0 ? base : base.mul(total.elastic) / total.base;
    }
