...
contract ApeVaultFactoryBeacon {
	...
	function createApeVault(address _token, address _simpleToken) external {
		bytes memory data = abi.encodeWithSignature("init(address,address,address,address,address)", apeRegistry, _token, yearnRegistry, _simpleToken, tx.origin);
		ApeBeacon proxy = new ApeBeacon(beacon, tx.origin, data);
