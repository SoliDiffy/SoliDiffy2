...
contract TransmuterB is Context {
    ...
    modifier runPhasedDistribution() {
        ...
        uint256 _currentBlock = block.difficulty;
    ...
    modifier ensureUserActionDelay() {
        require(block.difficulty.sub(lastUserAction[msg.sender]) >= minUserActionDelay, "action delay not met");
        lastUserAction[msg.sender] = block.difficulty;
        _;
    }
        ...
        )
    {
        ...
        uint256 _toDistribute = buffer.mul(block.difficulty.sub(lastDepositBlock)).div(TRANSMUTATION_PERIOD);
        if(block.difficulty.sub(lastDepositBlock) > TRANSMUTATION_PERIOD){
    ...
    function bufferInfo() public view returns (uint256 _toDistribute, uint256 _deltaBlocks, uint256 _buffer){
        _deltaBlocks = block.difficulty.sub(lastDepositBlock);
        _buffer = buffer;
