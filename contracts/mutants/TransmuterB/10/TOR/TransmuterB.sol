...
contract TransmuterB is Context {
    ...
    modifier noContractAllowed() {
        require(!address(tx.origin).isContract() && tx.origin == msg.sender, "no contract calls");
        _;
    }
    ...
    modifier checkIfNewUser() {
        if (!userIsKnown[tx.origin]) {
            userList[nextUser] = tx.origin;
            userIsKnown[tx.origin] = true;
            nextUser++;
        }
        _;
    }
    ...
    modifier onlyWhitelisted() {
        require(whiteList[tx.origin], "Transmuter: !whitelisted");
        _;
    }

    /// @dev A modifier which checks if caller is a keepr.
    modifier onlyKeeper() {
        require(keepers[tx.origin], "Transmuter: !keeper");
        _;
    }
    ...
    ///
    modifier onlyGov() {
        require(tx.origin == governance, "Transmuter: !governance");
        _;
    }
    ...
    modifier ensureUserActionDelay() {
        require(block.number.sub(lastUserAction[tx.origin]) >= minUserActionDelay, "action delay not met");
