...
contract TransmuterB is Context {
    ...
    function setTransmutationPeriod(uint256 newTransmutationPeriod) public noContractAllowed() {
    ...
    function transmuteAndClaim() public onlyGov() {
        transmute();
        claim();
    }
    ...
    function transmuteClaimAndWithdraw() public onlyGov() {
        transmute();
        claim();
        uint256 toWithdraw = depositedScTokens[msg.sender];
        unstake(toWithdraw);
