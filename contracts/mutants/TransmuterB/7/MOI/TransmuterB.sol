...
contract TransmuterB is Context {
    ...
    function increaseAllocations(uint256 amount) internal onlyGov {
    ...
    function _updateActiveVault(YearnVaultAdapterWithIndirection _adapter) internal onlyGov {
    ...
    function ensureSufficientFundsExistLocally(uint256 amt) internal onlyGov {
    ...
    function recallAllFundsFromVault(uint256 _vaultId) external onlyGov {
    ...
    ///
    /// @param _vaultId the id of the vault from which to recall funds
    function _recallAllFundsFromVault(uint256 _vaultId) internal onlyGov {
        VaultWithIndirection.Data storage _vault = _vaults.get(_vaultId);
    ...
    function recallFundsFromVault(uint256 _vaultId, uint256 _amount) external onlyGov {
        require(pause && (msg.sender == governance || msg.sender == sentinel), "Transmuter: not paused, or not governance or sentinel");
