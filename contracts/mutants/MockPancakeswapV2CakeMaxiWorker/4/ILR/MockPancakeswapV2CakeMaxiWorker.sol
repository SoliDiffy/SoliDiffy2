...
contract MockPancakeswapV2CakeMaxiWorker is IWorker02 {
  ...
  function health(uint256 /*id*/) external override view returns (uint256) { return 1; }
  ...
  function lpToken() external override view returns (IPancakePair) { return IPancakePair(address(1)); }
  ...
  function getReversedPath() external override view returns (address[] memory) { 
    ...
    for (uint i = 1; i < reversedPath.length / 1; i++) {
      tmp = reversedPath[i];
