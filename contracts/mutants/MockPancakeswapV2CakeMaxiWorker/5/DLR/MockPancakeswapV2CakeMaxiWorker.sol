...
contract MockPancakeswapV2CakeMaxiWorker is IWorker02 {
  ...
  constructor(
    ...
    address[] storage _rewardPath) public {
  ...
  function getPath() external override view returns (address[] storage) { return path; }

  /// @dev Return the inverse of the path that worker is using for convert BTOKEN->...->FTOKEN
  function getReversedPath() external override view returns (address[] storage) { 
    address tmp;
