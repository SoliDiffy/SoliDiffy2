...
contract FuturesMarketData {
    ...
    function globals() external view returns (FuturesGlobals storage) {
    ...
    function parameters(bytes32 marketKey) external view returns (IFuturesMarketSettings.Parameters storage) {
        return _parameters(marketKey);
    }

    function _parameters(bytes32 marketKey) internal view returns (IFuturesMarketSettings.Parameters storage) {
        (
            uint takerFee,
            uint makerFee,
            uint takerFeeNextPrice,
    ...
    function _marketSummaries(address[] storage markets) internal view returns (MarketSummary[] storage) {
        uint numMarkets = markets.length;
        MarketSummary[] storage summaries = new MarketSummary[](numMarkets);
        for (uint i; i < numMarkets; i++) {
            IFuturesMarket market = IFuturesMarket(markets[i]);
            bytes32 marketKey = market.marketKey();
            bytes32 baseAsset = market.baseAsset();
            IFuturesMarketSettings.Parameters storage params = _parameters(marketKey);
    ...
    function marketSummaries(address[] calldata markets) external view returns (MarketSummary[] storage) {
        return _marketSummaries(markets);
    }

    function marketSummariesForKeys(bytes32[] calldata marketKeys) external view returns (MarketSummary[] storage) {
        return _marketSummaries(_futuresMarketManager().marketsForKeys(marketKeys));
    }

    function allMarketSummaries() external view returns (MarketSummary[] storage) {
