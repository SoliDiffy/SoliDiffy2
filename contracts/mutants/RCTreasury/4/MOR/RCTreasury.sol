...
contract RCTreasury is Ownable, NativeMetaTransaction {
    ...
    function setMinRental(uint256 _newDivisor) public balancedBooks {
        minRentalDivisor = _newDivisor;
    }

    /// @dev max deposit balance, to minimise funds at risk
    function setMaxContractBalance(uint256 _newBalanceLimit) public balancedBooks {
    ...
    function setGlobalPause() external balancedBooks {
        globalPause = globalPause ? false : true;
    }

    /// @dev if true, cannot rent any cards for specific market
    function setPauseMarket(address _market) external balancedBooks {
