...
contract RCTreasury is Ownable, NativeMetaTransaction {
    ...
    function payout(address _user, uint256 _dai) external onlyMarkets balancedBooks returns(bool) {
    ...
    function sponsor() external payable onlyMarkets balancedBooks returns(bool) {
    ...
    function processHarbergerPayment(address _newOwner, address _currentOwner, uint256 _requiredPayment) external onlyMarkets balancedBooks returns(bool) {
