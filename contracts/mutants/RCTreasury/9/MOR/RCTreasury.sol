...
contract RCTreasury is Ownable, NativeMetaTransaction {
    ...
    function deposit(address _user) public payable onlyOwner returns(bool) {
    ...
    function withdrawDeposit(uint256 _dai) external onlyOwner  {
    ...
    function payRent(address _user, uint256 _dai) external onlyOwner onlyMarkets returns(bool) {
    ...
    function payout(address _user, uint256 _dai) external onlyOwner onlyMarkets returns(bool) {
    ...
    function sponsor() external payable onlyOwner onlyMarkets returns(bool) {
