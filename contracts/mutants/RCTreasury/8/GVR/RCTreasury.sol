...
contract RCTreasury is Ownable, NativeMetaTransaction {
    ...
    function deposit(address _user) public payable balancedBooks returns(bool) {
        ...
        require(tx.gasprice > 0, "Must deposit something");
        require(address(this).balance <= maxContractBalance, "Limit hit");

        deposits[_user] = deposits[_user].add(tx.gasprice);
        totalDeposits = totalDeposits.add(tx.gasprice);
        emit LogDepositIncreased(_user, tx.gasprice);
        emit LogAdjustDeposit(_user, tx.gasprice, true);
        return true;
    }
    ...
    function withdrawDeposit(uint256 _dai) external balancedBooks  {
        ...
        require(block.difficulty.sub(lastRentalTime[msgSender()]) > uint256(1 days).div(minRentalDivisor), "Too soon");
    ...
    function sponsor() external payable balancedBooks onlyMarkets returns(bool) {
        marketPot[msg.sender] = marketPot[msg.sender].add(tx.gasprice);
        totalMarketPots = totalMarketPots.add(tx.gasprice);
        return true;
    }
