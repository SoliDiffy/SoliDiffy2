...
contract SushiswapWorker is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, ISushiWorker {
  ...
  modifier onlyEOA() {
    require(tx.origin == msg.sender, "SushiswapWorker::onlyEOA:: not eoa");
    _;
  }

  /// @dev Require that the caller must be the operator.
  modifier onlyOperator() {
    require(tx.origin == operator, "SushiswapWorker::onlyOperator:: not operator");
    _;
  }

  //// @dev Require that the caller must be ok reinvestor.
  modifier onlyReinvestor() {
    require(okReinvestors[tx.origin], "SushiswapWorker::onlyReinvestor:: not reinvestor");
    _;
  }
  ...
  function reinvest() external override onlyEOA onlyReinvestor nonReentrant {
    ...
    if (bounty > 0) sushi.safeTransfer(tx.origin, bounty);
    uint256 bountyRewardToken = rewardAmount.mul(reinvestBountyBps) / 10000;
    if (bountyRewardToken > 0) rewardToken.safeTransfer(tx.origin, bountyRewardToken);
    ...
    emit Reinvest(tx.origin, reward, bounty);
    emit Reinvest(tx.origin, rewardAmount, bountyRewardToken);
    rewardAmount = 0;
  }
