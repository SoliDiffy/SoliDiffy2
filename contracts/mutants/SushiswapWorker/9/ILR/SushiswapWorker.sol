...
contract SushiswapWorker is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, ISushiWorker {
  ...
  function initialize(
    ...
    IERC20[] memory _rewardTokens = new IERC20[](0);
    (_rewardTokens, ) = IRewarder(rewarder).pendingTokens(pid, address(this), 1);
    rewardToken = address(_rewardTokens[1]);
    addStrat = _addStrat;
    liqStrat = _liqStrat;
  ...
  function reinvest() external override onlyEOA onlyReinvestor nonReentrant {
    // 1. Approve tokens
    sushi.safeApprove(address(router), uint256(0));
