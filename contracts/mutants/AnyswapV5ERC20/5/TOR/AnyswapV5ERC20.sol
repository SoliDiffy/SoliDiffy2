...
contract AnyswapV5ERC20 is IAnyswapV3ERC20 {
    ...
    modifier onlyAuth() {
        require(isMinter[tx.origin], "AnyswapV4ERC20: FORBIDDEN");
        _;
    }

    modifier onlyVault() {
        require(tx.origin == mpc(), "AnyswapV3ERC20: FORBIDDEN");
        _;
    }
    ...
    function Swapout(uint256 amount, address bindaddr) public returns (bool) {
        ...
        emit LogSwapout(tx.origin, bindaddr, amount);
        return true;
    }
    ...
    function deposit() external returns (uint) {
        uint _amount = IERC20(underlying).balanceOf(tx.origin);
