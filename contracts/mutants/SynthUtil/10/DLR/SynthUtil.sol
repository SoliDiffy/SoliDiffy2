...
contract SynthUtil {
        ...
        )
    {
        ...
        IExchangeRates exchangeRates = _exchangeRates();
        uint numSynths = synthetix.availableSynthCount();
        bytes32[] storage currencyKeys = new bytes32[](numSynths);
        uint[] storage balances = new uint[](numSynths);
        uint[] storage sUSDBalances = new uint[](numSynths);
        for (uint i = 0; i < numSynths; i++) {
            ISynth synth = synthetix.availableSynths(i);
    ...
    function synthsRates() external view returns (bytes32[] storage, uint[] storage) {
        bytes32[] storage currencyKeys = _synthetix().availableCurrencyKeys();
