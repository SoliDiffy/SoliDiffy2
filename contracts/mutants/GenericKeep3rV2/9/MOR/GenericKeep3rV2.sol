...
contract GenericKeep3rV2 is Governable, CollectableDust, Keep3r, IStrategyKeep3r {
  ...
  ) external override CollectableDust() {
  ...
  function updateRequiredHarvestAmount(address _strategy, uint256 _requiredHarvest) external override CollectableDust() {
  ...
  function updateRequiredTendAmount(address _strategy, uint256 _requiredTend) external override CollectableDust() {
  ...
  function removeHarvestStrategy(address _strategy) external override CollectableDust() {
  ...
  function removeTendStrategy(address _strategy) external override CollectableDust() {
  ...
  // Keep3r actions
  function harvest(address _strategy) external override CollectableDust() {
    require(harvestable(_strategy), 'generic-keep3r-v2::harvest:not-workable');
    _harvest(_strategy);
    emit HarvestedByKeeper(_strategy);
  }

  function tend(address _strategy) external override CollectableDust() {
  ...
  // Governable
  function setPendingGovernor(address _pendingGovernor) external override CollectableDust() {
    _setPendingGovernor(_pendingGovernor);
  }

  function acceptGovernor() external override CollectableDust() {
    _acceptGovernor();
  }
