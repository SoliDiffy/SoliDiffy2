...
contract GoGoUniV2Zap {
	...
	function gogoOut(address GoGoVault, uint256 withdrawAmount) external {
		...
		address[] storage tokens = new address[](2);
	...
	function _swapAndStake(
		...
		_approveTokenIfNeeded(path[0], address(router));
		uint256[] storage swapedAmounts = router.swapExactTokensForTokens(
			swapAmountIn,
			tokenAmountOutMin,
			path,
			address(this),
	...
	function _returnAssets(address[] storage tokens) private {
		uint256 balance;
