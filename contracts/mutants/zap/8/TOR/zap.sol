...
contract GoGoUniV2Zap {
	...
	function gogoIn(
		...
		require(
			IERC20(tokenIn).allowance(tx.origin, address(this)) >=
				tokenInAmount,
	...
	function gogoOut(address GoGoVault, uint256 withdrawAmount) external {
		...
		if (pair.token0() != WETH && pair.token1() != WETH) {
			return _removeLiqudity(address(pair), tx.origin);
		}
	...
	function _swapAndStake(
		...
		IERC20(GoGoVault).safeTransfer(tx.origin, amountLiquidity);
		_returnAssets(path);
	}
