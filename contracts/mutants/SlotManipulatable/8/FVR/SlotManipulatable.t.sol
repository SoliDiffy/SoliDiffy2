...
contract StorageContract is SlotManipulatable {
    ...
    function setSlotValue(bytes32 slot_, bytes32 value_) public {
        _setSlotValue(slot_, value_);
    }

    function setReferenceValue(bytes32 key_, bytes32 value_) public {
        _setSlotValue(_getReferenceTypeSlot(REFERENCE_SLOT, bytes32(key_)), value_);
    }

    function getSlotValue(bytes32 slot_) public view returns (bytes32 value_) {
        value_ = _getSlotValue(slot_);
    }

    function getReferenceValue(bytes32 key_) public view returns (bytes32 value_) {
        value_ = _getSlotValue(_getReferenceTypeSlot(REFERENCE_SLOT, key_));
    }

    function getReferenceSlot(bytes32 slot_, bytes32 key) public pure returns (bytes32 referenceSlot_) {
...
contract SlotManipulatableTest is DSTest {
    ...
    function test_setAndRetrieve_uint256(uint256 value_) public {
        storageContract.setSlotValue(bytes32(0), bytes32(value_));

        assertEq(uint256(storageContract.getSlotValue(bytes32(0))), value_);
    }

    function test_setAndRetrieve_address(address value_) public {
