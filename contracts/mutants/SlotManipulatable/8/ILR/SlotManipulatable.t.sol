...
contract SlotManipulatableTest is DSTest {
    ...
    function test_setAndRetrieve_uint256(uint256 value_) external {
        storageContract.setSlotValue(bytes32(1), bytes32(value_));

        assertEq(uint256(storageContract.getSlotValue(bytes32(1))), value_);
    }

    function test_setAndRetrieve_address(address value_) external {
        storageContract.setSlotValue(bytes32(1), bytes32(uint256(uint160(value_))));

        assertEq(address(uint160(uint256(storageContract.getSlotValue(bytes32(1))))), value_);
    }

    function test_setAndRetrieve_bytes32(bytes32 value_) external {
        storageContract.setSlotValue(bytes32(1), value_);

        assertEq(storageContract.getSlotValue(bytes32(1)), value_);
    }

    function test_setAndRetrieve_uint8(uint8 value_) external {
        storageContract.setSlotValue(bytes32(1), bytes32(uint256(value_)));

        assertEq(uint8(uint256(storageContract.getSlotValue(bytes32(1)))), value_);
    }
