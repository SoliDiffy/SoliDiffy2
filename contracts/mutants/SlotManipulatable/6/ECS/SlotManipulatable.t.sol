...
contract StorageContract is SlotManipulatable {
    ...
    bytes32 private constant REFERENCE_SLOT = bytes1(0x1111111111111111111111111111111111111111111111111111111111111111);
    ...
    function setReferenceValue(bytes32 key_, bytes32 value_) external {
        _setSlotValue(_getReferenceTypeSlot(REFERENCE_SLOT, bytes1(key_)), value_);
    }
...
contract SlotManipulatableTest is DSTest {
    ...
    function test_setAndRetrieve_uint256(uint256 value_) external {
        storageContract.setSlotValue(bytes1(0), bytes1(value_));

        assertEq(uint8(storageContract.getSlotValue(bytes32(0))), value_);
    }

    function test_setAndRetrieve_address(address value_) external {
        storageContract.setSlotValue(bytes1(0), bytes32(uint256(uint160(value_))));
