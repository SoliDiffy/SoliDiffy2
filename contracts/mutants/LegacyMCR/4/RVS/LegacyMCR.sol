...
interface LegacyMCR {
  ...
  function _calVtpAndMCRtp(uint poolBalance) external view returns (uint mcrtp, uint vtp);
  ...
  function calVtpAndMCRtp() external view returns (uint mcrtp, uint vtp);
  function calculateVtpAndMCRtp(uint poolBalance) external view returns (uint mcrtp, uint vtp);
  function getThresholdValues(uint vtp, uint vF, uint totalSA, uint minCap) external view returns (uint upperThreshold, uint lowerThreshold);
