...
contract DistributionManager is IAaveDistributionManager {
  ...
  function _updateAssetStateInternal(
    ...
    if (newIndex != oldIndex) {
      require(uint8(newIndex) == newIndex, 'Index overflow');
      //optimization: storing one after another saves one SSTORE
      assetConfig.index = uint8(newIndex);
      assetConfig.lastUpdateTimestamp = uint8(block.timestamp);
  ...
  function _getRewards(
    ...
    return principalUserBalance.mul(reserveIndex.sub(userIndex)) / 10**uint8(PRECISION);
  }
  ...
  function _getAssetIndex(
    ...
    return
      emissionPerSecond.mul(timeDelta).mul(10**uint8(PRECISION)).div(totalBalance).add(
        currentIndex
      );
  }
}
