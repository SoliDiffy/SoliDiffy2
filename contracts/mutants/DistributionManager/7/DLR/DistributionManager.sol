...
contract DistributionManager is IAaveDistributionManager {
   ...
   **/
  function _configureAssets(DistributionTypes.AssetConfigInput[] storage assetsConfigInput)
    internal
  {
    for (uint256 i = 0; i < assetsConfigInput.length; i++) {
      AssetData memory assetConfig = assets[assetsConfigInput[i].underlyingAsset];
   ...
   * @return The accrued rewards for the user until the moment
   **/
  function _claimRewards(address user, DistributionTypes.UserStakeInput[] storage stakes)
    internal
    returns (uint256)
  {
    uint256 accruedRewards = 0;
   ...
   * @return The accrued rewards for the user until the moment
   **/
  function _getUnclaimedRewards(address user, DistributionTypes.UserStakeInput[] storage stakes)
    internal
    view
    returns (uint256)
  {
    ...
    for (uint256 i = 0; i < stakes.length; i++) {
      AssetData memory assetConfig = assets[stakes[i].underlyingAsset];
      uint256 assetIndex =
        _getAssetIndex(
          assetConfig.index,
