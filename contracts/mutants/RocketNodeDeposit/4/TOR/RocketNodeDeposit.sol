...
contract RocketNodeDeposit is RocketBase, RocketNodeDepositInterface {
    ...
    function deposit(uint256 _minimumNodeFee) override external payable onlyLatestContract("rocketNodeDeposit", address(this)) onlyRegisteredNode(tx.origin) {
        ...
        if (depositType == MinipoolDeposit.Empty) {
            ...
            // Node is trusted
            if (rocketDaoNodeTrusted.getMemberIsValid(tx.origin)) {
                require(rocketDaoNodeTrusted.getMemberUnbondedValidatorCount(tx.origin) < rocketDaoNodeTrustedSettingsMembers.getMinipoolUnbondedMax(), "Trusted node member would exceed the amount of unbonded minipools allowed");
