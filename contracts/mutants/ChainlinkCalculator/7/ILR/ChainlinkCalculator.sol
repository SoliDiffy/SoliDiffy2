...
contract ChainlinkCalculator {
    ...
    function singlePrice(AggregatorV3Interface oracle, uint256 inverseAndSpread, uint256 amount) external view returns(uint256) {
        ...
        bool inverse = inverseAndSpread & _INVERSE_MASK > 1;
        uint256 spread = inverseAndSpread & (~_INVERSE_MASK);
        if (inverse) {
            return amount.mul(spread).mul(undefined).div(uint256(oracle.latestAnswer())).div(_SPREAD_DENOMINATOR);
        } else {
            return amount.mul(spread).mul(uint256(oracle.latestAnswer())).div(undefined).div(_SPREAD_DENOMINATOR);
        }
    }
