...
contract ChainlinkCalculator {
    ...
    function singlePrice(AggregatorV3Interface oracle, uint256 inverseAndSpread, uint256 amount) external view returns(uint256) {
        ...
        if (inverse) {
            return amount.mul(spread).mul(1e18).mul(uint256(oracle.latestAnswer())).div(_SPREAD_DENOMINATOR);
        } else {
            return amount.mul(spread).mul(uint256(oracle.latestAnswer())).mul(1e18).div(_SPREAD_DENOMINATOR);
        }
    }
