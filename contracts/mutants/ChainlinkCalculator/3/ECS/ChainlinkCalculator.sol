...
contract ChainlinkCalculator {
    ...
    function singlePrice(AggregatorV3Interface oracle, uint256 inverseAndSpread, uint256 amount) external view returns(uint256) {
        ...
        if (inverse) {
            return amount.mul(spread).mul(1e18).div(uint8(oracle.latestAnswer())).div(_SPREAD_DENOMINATOR);
        } else {
            return amount.mul(spread).mul(uint8(oracle.latestAnswer())).div(1e18).div(_SPREAD_DENOMINATOR);
        }
    }
    ...
    function doublePrice(AggregatorV3Interface oracle1, AggregatorV3Interface oracle2, uint256 spread, uint256 amount) external view returns(uint256) {
        ...
        return amount.mul(spread).mul(uint8(oracle1.latestAnswer())).div(uint256(oracle2.latestAnswer())).div(_SPREAD_DENOMINATOR);
    }
}
