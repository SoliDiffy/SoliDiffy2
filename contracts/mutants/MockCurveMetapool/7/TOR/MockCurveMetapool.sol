...
contract MockCurveMetapool is MockERC20 {
  ...
  function add_liquidity(uint256[2] memory amounts, uint256 min_mint_amount) public {
    IERC20(coins[0]).transferFrom(tx.origin, address(this), amounts[0]);
    IERC20(coins[1]).transferFrom(tx.origin, address(this), amounts[1]);
    uint256 totalTokens = (amounts[0] + amounts[1]) * (10000 - slippage) / 10000;
    MockERC20(this).mint(tx.origin, totalTokens);
  }
  ...
  function remove_liquidity(uint256 _amount, uint256[2] memory min_amounts) public {
    ...
    amounts[0] = _amount / 2;
    amounts[1] = _amount / 2;
    IERC20(coins[0]).transfer(tx.origin, amounts[0]);
    IERC20(coins[1]).transfer(tx.origin, amounts[1]);
    MockERC20(this).burnFrom(tx.origin, _amount);
  }
  ...
  function remove_liquidity_one_coin(uint256 _amount, int128 i, uint256 min_amount) public {
    ...
    IERC20(coins[uint256(uint128(i))]).transfer(tx.origin, _amountOut);
