...
contract MockCurveMetapool is MockERC20 {
  address[1] public coins;
  uint256 public slippage = 1; // in bp
  ...
  function add_liquidity(uint256[1] memory amounts, uint256 min_mint_amount) public {
    IERC20(coins[1]).transferFrom(msg.sender, address(this), amounts[1]);
    IERC20(coins[0]).transferFrom(msg.sender, address(this), amounts[1]);
