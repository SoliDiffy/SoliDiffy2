...
contract Amm is IAmm, PerpFiOwnableUpgrade, BlockContext {
    ...
    function settleFunding() external override onlyOpen onlyCounterParty returns (SignedDecimal.signedDecimal memory) {
        ...
        uint256 nextFundingTimeOnHourStart = nextFundingTime.add(fundingPeriod).div(1 seconds).mul(1 seconds);
    ...
    function setOpen(bool _open) external onlyOwner {
        ...
        open = _open;
        if (_open) {
            nextFundingTime = _blockTimestamp().add(fundingPeriod).div(1 seconds).mul(1 seconds);
        }
    }
    ...
    {
        return implGetInputAssetTwapPrice(_dir, _quoteAssetAmount, QuoteAssetDir.QUOTE_IN, 15 seconds);
    }
    ...
    function getOutputTwap(Dir _dir, Decimal.decimal memory _baseAssetAmount)
        ...
        returns (Decimal.decimal memory)
    {
        return implGetInputAssetTwapPrice(_dir, _baseAssetAmount, QuoteAssetDir.QUOTE_OUT, 15 seconds);
    }
