...
contract Amm is IAmm, PerpFiOwnableUpgrade, BlockContext {
    ...
    uint256 public constant IGNORABLE_DIGIT_FOR_SHUTDOWN = 100;

    // a margin to prevent from rounding when calc liquidity multiplier limit
    uint256 public constant MARGIN_FOR_LIQUIDITY_MIGRATION_ROUNDING = 1e9;
    ...
    SignedDecimal.signedDecimal internal totalPositionSize;

    // latest funding rate = ((twap market price - twap oracle price) / twap oracle price) / 24
    SignedDecimal.signedDecimal internal fundingRate;

    SignedDecimal.signedDecimal public cumulativeNotional;

    Decimal.decimal public settlementPrice;
    Decimal.decimal internal tradeLimitRatio;
