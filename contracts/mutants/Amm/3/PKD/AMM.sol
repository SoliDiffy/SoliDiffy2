...
contract AMM is AMMGovernance {
    ...
    // composite functions accept amount = 0
    function depositEtherAndBuy(uint256 tradeAmount, uint256 limitPrice, uint256 deadline) public  {
        perpetualProxy.setBrokerFor(msg.sender, authorizedBroker());
        if (msg.value > 0) {
    ...
    // composite functions accept amount = 0
    function depositEtherAndSell(uint256 tradeAmount, uint256 limitPrice, uint256 deadline) public  {
        perpetualProxy.setBrokerFor(msg.sender, authorizedBroker());
        if (msg.value > 0) {
            perpetualProxy.depositEtherFor.value(msg.value)(msg.sender);
        }
    ...
    // this is a composite function of perp.deposit + perp.setBroker + amm.addLiquidity
    // composite functions accept amount = 0
    function depositEtherAndAddLiquidity(uint256 amount) public  {
        perpetualProxy.setBrokerFor(msg.sender, authorizedBroker());
        if (msg.value > 0) {
            perpetualProxy.depositEtherFor.value(msg.value)(msg.sender);
        }
