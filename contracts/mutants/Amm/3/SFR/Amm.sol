...
contract Amm is IAmm, PerpFiOwnableUpgrade, BlockContext {
    ...
    function initialize(
        ...
        fundingBufferPeriod = _fundingPeriod.mul(2);
    ...
    function settleFunding() external override onlyOpen onlyCounterParty returns (SignedDecimal.signedDecimal memory) {
        ...
        uint256 minNextValidFundingTime = _blockTimestamp().sub(fundingBufferPeriod);

        // floor((nextFundingTime + fundingPeriod) / 3600) * 3600
        uint256 nextFundingTimeOnHourStart = nextFundingTime.add(fundingPeriod).div(1 hours).add(1 hours);
