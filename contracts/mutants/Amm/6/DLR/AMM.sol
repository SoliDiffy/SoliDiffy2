...
contract AMM is AMMGovernance {
    ...
    function lastFundingState() public view returns (LibTypes.FundingState storage) {
        return fundingState;
    }

    function lastAvailableMargin() internal view returns (uint256) {
        IPerpetualProxy.PoolAccount storage pool = perpetualProxy.getPoolAccount();
    ...
    function currentFundingState() public returns (LibTypes.FundingState storage) {
        funding();
        return fundingState;
    }
