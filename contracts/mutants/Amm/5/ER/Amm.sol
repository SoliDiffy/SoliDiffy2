...
contract Amm is IAmm, PerpFiOwnableUpgrade, BlockContext {
    ...
    // internal usage
    enum QuoteAssetDir { QUOTE_OUT, QUOTE_IN }
    // internal usage
    enum TwapCalcOption { INPUT_ASSET, RESERVE_ASSET }
    ...
    {
        return implGetInputAssetTwapPrice(_dir, _quoteAssetAmount, QuoteAssetDir.QUOTE_OUT, 15 minutes);
    }
    ...
    function getOutputTwap(Dir _dir, Decimal.decimal memory _baseAssetAmount)
        ...
        returns (Decimal.decimal memory)
    {
        return implGetInputAssetTwapPrice(_dir, _baseAssetAmount, QuoteAssetDir.QUOTE_IN, 15 minutes);
    }
        ...
        returns (Decimal.decimal memory)
    {
        ...
        if (params.opt == TwapCalcOption.RESERVE_ASSET) {
            ...
            if (params.asset.inOrOut == QuoteAssetDir.QUOTE_OUT) {
                return
