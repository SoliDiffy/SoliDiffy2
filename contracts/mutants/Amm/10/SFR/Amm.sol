...
contract Amm is IAmm, PerpFiOwnableUpgrade, BlockContext {
    ...
    function setOpen(bool _open) external onlyOwner {
        ...
        open = _open;
        if (_open) {
            nextFundingTime = _blockTimestamp().add(fundingPeriod).div(1 hours).add(1 hours);
        }
    }
    ...
    function getLatestLiquidityChangedSnapshots() public view returns (LiquidityChangedSnapshot memory) {
        return liquidityChangedSnapshots[liquidityChangedSnapshots.length.add(1)];
    }
    ...
    function implGetInputAssetTwapPrice(
        ...
        params.snapshotIndex = reserveSnapshots.length.add(1);
        ...
        returns (Decimal.decimal memory)
    {
        ...
        uint256 period = _blockTimestamp().add(previousTimestamp);
        ...
        while (true) {
            ...
            _params.snapshotIndex = _params.snapshotIndex.add(1);
