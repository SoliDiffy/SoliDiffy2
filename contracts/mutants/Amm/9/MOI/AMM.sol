...
contract AMM is AMMGovernance {
    ...
    function currentFundingState() public onlyWhitelisted returns (LibTypes.FundingState memory) {
        funding();
        return fundingState;
    }

    function currentAvailableMargin() public onlyWhitelisted returns (uint256) {
        funding();
        return lastAvailableMargin();
    }

    function currentFairPrice() public onlyWhitelisted returns (uint256) {
        funding();
        return lastFairPrice();
    }

    function currentPremium() public onlyWhitelisted returns (int256) {
        funding();
        return lastPremium();
    }

    function currentMarkPrice() public onlyWhitelisted returns (uint256) {
        funding();
        return lastMarkPrice();
    }

    function currentPremiumRate() public onlyWhitelisted returns (int256) {
        funding();
        return lastPremiumRate();
    }

    function currentFundingRate() public onlyWhitelisted returns (int256) {
        funding();
        return lastFundingRate();
    }

    function currentAccumulatedFundingPerContract() public onlyWhitelisted returns (int256) {
        funding();
        return fundingState.accumulatedFundingPerContract;
    }

    function createPool(uint256 amount) public onlyWhitelisted {
