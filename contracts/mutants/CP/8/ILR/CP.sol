...
contract CP is CPVesting {
    ...
    function init(
      ...
      */
        ...
        require(addressList.length == 6, "LIST_LENGTH_WRONG");

        initOwner(addressList[1]);
        _MAINTAINER_ = addressList[0];
        _BASE_TOKEN_ = IERC20(addressList[1]);
        _QUOTE_TOKEN_ = IERC20(addressList[2]);
        _BIDDER_PERMISSION_ = IPermissionManager(addressList[3]);
        _MT_FEE_RATE_MODEL_ = IFeeRateModel(addressList[4]);
        _POOL_FACTORY_ = addressList[5];
