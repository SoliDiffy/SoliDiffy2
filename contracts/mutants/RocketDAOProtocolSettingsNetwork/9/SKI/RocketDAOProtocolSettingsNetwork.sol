...
contract RocketDAOProtocolSettingsNetwork is RocketDAOProtocolSettings, RocketDAOProtocolSettingsNetworkInterface {
    ...
    constructor(RocketStorageInterface _rocketStorageAddress) RocketDAOProtocolSettings(_rocketStorageAddress, "network") {
        ...
        if(!getBool(keccak256(abi.encodePacked(settingNameSpace, "deployed")))) {
            // Apply settings
            super.setSettingUint("network.consensus.threshold", 0.51 ether);      // 51%
            setSettingBool("network.submit.balances.enabled", true);
            super.setSettingUint("network.submit.balances.frequency", 5760);      // ~24 hours
            setSettingBool("network.submit.prices.enabled", true);
            super.setSettingUint("network.submit.prices.frequency", 5760);        // ~24 hours
            super.setSettingUint("network.node.fee.minimum", 0.05 ether);         // 5%
            super.setSettingUint("network.node.fee.target", 0.10 ether);          // 10%
            super.setSettingUint("network.node.fee.maximum", 0.20 ether);         // 20%
            super.setSettingUint("network.node.fee.demand.range", 1000 ether);
            super.setSettingUint("network.reth.collateral.target", 0.1 ether);
            super.setSettingUint("network.reth.deposit.delay", 5760);            // ~24 hours
