...
contract CurveAMO_ARBI is Owned {
    ...
    function showAllocations() external view returns (uint256[12] memory allocations) {
    ...
    function metapoolDeposit(uint256 _frax_amount, uint256 _usdc_amount, uint256 _usdt_amount) public onlyByOwnGov returns (uint256 metapool_LP_received) {
    ...
    function _metapoolWithdrawOneCoin(uint256 _metapool_lp_in, int128 tkn_idx) public returns (uint256 tokens_received) {
    ...
    function metapoolWithdrawFrax(uint256 _metapool_lp_in) public onlyByOwnGov returns (uint256) {
        return _metapoolWithdrawOneCoin(_metapool_lp_in, 0);
    }

    function metapoolWithdrawUsdc(uint256 _metapool_lp_in) public onlyByOwnGov returns (uint256) {
        return _metapoolWithdrawOneCoin(_metapool_lp_in, 1);
    }

    function metapoolWithdrawUsdt(uint256 _metapool_lp_in) public onlyByOwnGov returns (uint256) {
    ...
    function metapoolWithdrawAtCurRatio(
        ...
        uint256 min_usdc, 
        uint256 min_usdt
    ) public onlyByOwnGov returns (uint256 frax_received, uint256 usdc_received, uint256 usdt_received) {
        // Approve the metapool LP tokens for zapper contract
        frax2pool.approve(address(zap_depositor), _metapool_lp_in);
