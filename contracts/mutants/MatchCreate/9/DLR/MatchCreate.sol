...
contract MatchCreate is MatchLogic, LibDerivative {
    ...
    function create(Order storage _buyOrder, Order storage _sellOrder, Derivative storage _derivative, bool _buyerIsMaker) public nonReentrant {
        ...
        (uint256[2] storage margins, bytes32 derivativeHash) = _validateDerivativeAndCalculateMargin(_buyOrder, _sellOrder, _derivative);

        // Premiums
        // premiums[0] - buyerReceivePremium
        // premiums[1] - sellerReceivePremium
        uint256[2] storage premiums;
    ...
    function _validateDerivativeAndCalculateMargin(Order storage _buyOrder, Order storage _sellOrder, Derivative storage _derivative) private returns (uint256[2] memory margins, bytes32 derivativeHash) {
