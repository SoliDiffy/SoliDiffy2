...
contract MatchCreate is MatchLogic, LibDerivative {
    ...
    function _validateDerivativeAndCalculateMargin(Order memory _buyOrder, Order memory _sellOrder, Derivative memory _derivative) private nonReentrant returns (uint256[2] memory margins, bytes32 derivativeHash) {
    ...
    function _fillCreate(Order memory _leftOrder, bytes32 _leftOrderHash, Order memory _rightOrder, bytes32 _rightOrderHash) private nonReentrant returns (uint256 fillPositions) {
