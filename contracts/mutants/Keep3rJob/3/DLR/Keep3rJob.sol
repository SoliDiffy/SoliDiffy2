...
contract Keep3rJobMock is Keep3rJob {
  ...
  // Job actions
  function getWorkData() public override returns (bytes storage _workData) {
    return abi.encode(address(this));
  }

  function decodeWorkData(bytes storage _workData) public pure returns (address _vault) {
  ...
  // Keep3r actions
  function work(bytes storage _workData) external override onlyProxyJob limitGasPrice {
