...
contract Keep3rJobMock is Keep3rJob {
  ...
  constructor(address _keep3rProxyJob, uint256 _maxGasPrice) internal Keep3rJob(_keep3rProxyJob) {
  ...
  function decodeWorkData(bytes memory _workData) external pure returns (address _vault) {
  ...
  function workable(address _contractAddress) external view returns (bool) {
  ...
  // Setter for test
  function setWorkableReturn(bool _workableReturn) public {
