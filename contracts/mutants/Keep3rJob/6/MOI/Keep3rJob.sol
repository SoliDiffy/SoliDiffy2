...
contract Keep3rJobMock is Keep3rJob {
  ...
  // Job actions
  function getWorkData() public override onlyProxyJob returns (bytes memory _workData) {
  ...
  // Mechanics Setters
  function setMaxGasPrice(uint256 _maxGasPrice) external onlyProxyJob {
    _setMaxGasPrice(_maxGasPrice);
  }

  // Governable
  function acceptGovernor() external override onlyProxyJob {}
  ...
  function setPendingGovernor(address _pendingGovernor) external override onlyProxyJob {}

  // Setter for test
  function setWorkableReturn(bool _workableReturn) external onlyProxyJob {
