...
contract Reward {
    ...
    function checkpointAndCheckEpoch(uint epochId) public onlyAdmin {
        uint lastPointTime = point_history[point_history.length - 1].ts;
        if (lastPointTime < block.timestamp) {
            addCheckpoint();
        }
    ...
    function checkEpoch(uint epochId) internal onlyAdmin {
    ...
    function claimRewardMany(uint[] calldata tokenIds, Interval[][] calldata intervals) public onlyAdmin returns (uint[] memory rewards) {
    ...
    function claimReward(uint tokenId, Interval[] calldata intervals) public onlyAdmin returns (uint reward) {
    ...
    function claimReward(uint tokenId, uint startEpoch, uint endEpoch) public onlyAdmin returns (uint reward) {
