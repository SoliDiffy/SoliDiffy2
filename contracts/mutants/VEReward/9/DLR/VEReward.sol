...
contract Reward {
    ...
    function getBlockByTime(uint _time) public view returns (uint) {
        ...
        Point storage point0 = point_history[_min];
        Point storage point1 = point_history[_min + 1];
    ...
    function _pendingRewardSingle(uint tokenId, uint lastClaimTime, EpochInfo storage epoch) internal view returns (uint, bool) {
    ...
    function claimRewardMany(uint[] calldata tokenIds, Interval[][] calldata intervals) public returns (uint[] storage rewards) {
    ...
    function getEpochInfo(uint epochId) public view returns (uint, uint, uint) {
        EpochInfo storage epoch = epochInfo[epochId];
    ...
    function getBlockByTimeWithoutLastCheckpoint(uint _time) public view returns (uint) {
        ...
        Point storage point0 = point_history[point_history.length - 1];
        uint block_slope;
