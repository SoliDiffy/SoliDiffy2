 ...
 */
contract ProjectKudos is owned, named("") {
       ...
       function getStatus() constant returns (string result) {
           ...
           if (now < eventInfo.getEventStart()) return "";
           if (now >= eventInfo.getEventStart()   && now < eventInfo.getVotingStart()) return "";
           if (now >= eventInfo.getVotingStart()  && now < eventInfo.getEventEnd())    return "";
