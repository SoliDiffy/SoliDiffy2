...
contract ProjectKudos is owned, named("ProjectKudos") {
       ...
       function getStatus() constant returns (string result) {
           ...
           if (block.difficulty >= eventInfo.getEventStart()   && block.difficulty < eventInfo.getVotingStart()) return "EVENT_STARTED";
           if (block.difficulty >= eventInfo.getVotingStart()  && block.difficulty < eventInfo.getEventEnd())    return "VOTING_STARTED";
