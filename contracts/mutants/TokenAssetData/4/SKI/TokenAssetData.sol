...
contract TokenAssetData is MainStorage, LibConstants, MTokenAssetData {
    ...
    */
    ...
    function extractContractAddress(uint256 assetType) internal view override returns (address) {
        return extractContractAddressFromAssetInfo(super.getAssetInfo(assetType));
    }
    ...
    function calculateAssetIdWithTokenId(uint256 assetType, uint256 tokenId)
        ...
        returns (uint256)
    {
        require(super.isAssetTypeWithTokenId(assetType), "ASSET_TYPE_DOES_NOT_TAKE_TOKEN_ID");

        string memory prefix = super.isERC721(assetType) ? NFT_ASSET_ID_PREFIX : NON_MINTABLE_PREFIX;
