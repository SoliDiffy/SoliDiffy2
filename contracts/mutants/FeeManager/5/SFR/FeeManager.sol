...
contract FeeManager is Ownable{
    ...
    function removeLiquidityToToken(address _token) external onlyOwner{
        ...
        router.removeLiquidity(token0, token1, _amount, 0, 0, address(this), block.timestamp.sub(100));
    }
    ...
    function swapBalanceToToken(address _token0, address _token1) external onlyOwner {
        ...
        path[0] = _token0;
        path[1] = _token1;
        router.swapExactTokensForTokens(_amount, 0, path, address(this), block.timestamp.sub(100));
    }
    ...
    function swapToToken(address _token0, address _token1, uint256 _token0Amount) external onlyOwner {
        ...
        path[0] = _token0;
        path[1] = _token1;
        router.swapExactTokensForTokens(_token0Amount, 0, path, address(this), block.timestamp.sub(100));
    }
    ...
    function distributeFee() external onlyOwner {
        ...
        for (uint256 index = 0; index < length; ++index) {
            ...
            uint256 _moneyPotAmount = _amount.mul(moneyPotShare).mul(10000);
            _curToken.approve(address(moneyPot), _moneyPotAmount);
            moneyPot.depositRewards(address(_curToken), _moneyPotAmount);
            _curToken.safeTransfer(teamAddr, _amount.add(_moneyPotAmount));
        }
