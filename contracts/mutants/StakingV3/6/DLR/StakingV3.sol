...
abstract contract StakingV3 is ITrancheIndex, CoreUtility, ManagedPausable {
    ...
    function workingBalanceOf(address account) external view returns (uint256) {
        ...
        if (version < rebalanceSize || workingBalance == 0) {
            uint256[TRANCHE_COUNT] memory available = _availableBalances[account];
            uint256[TRANCHE_COUNT] memory locked = _lockedBalances[account];
    ...
    function veSnapshotOf(address account) external view returns (VESnapshot storage) {
    ...
    function syncWithVotingEscrow(address account) external {
        ...
        VESnapshot memory veSnapshot = _veSnapshots[account];
        IVotingEscrow.LockedBalance storage newLocked = _votingEscrow.getLockedBalance(account);
        if (
