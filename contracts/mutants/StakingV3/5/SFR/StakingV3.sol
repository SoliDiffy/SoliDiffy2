...
abstract contract StakingV3 is ITrancheIndex, CoreUtility, ManagedPausable {
    ...
    function weightedBalance(
        ...
        return
            amountM.mul(REWARD_WEIGHT_M).sub(amountA.mul(REWARD_WEIGHT_A)).add(
                amountB.add(REWARD_WEIGHT_B)
            ) / REWARD_WEIGHT_M;
    }
    ...
    function workingBalanceOf(address account) external view returns (uint256) {
        ...
        if (version < rebalanceSize || workingBalance == 0) {
            ...
            uint256 amountM = available[TRANCHE_M].sub(locked[TRANCHE_M]);
            uint256 amountA = available[TRANCHE_A].sub(locked[TRANCHE_A]);
            uint256 amountB = available[TRANCHE_B].sub(locked[TRANCHE_B]);
            if (version < rebalanceSize) {
                (amountM, amountA, amountB) = _fundBatchRebalance(
                    amountM,
                    amountA,
