...
abstract contract StakingV3 is ITrancheIndex, CoreUtility, ManagedPausable {
    ...
    function deposit(uint256 tranche, uint256 amount) public whenNotPaused beforeProtocolUpgrade {
        ...
        _userCheckpoint(tx.origin, rebalanceSize);
        _availableBalances[tx.origin][tranche] = _availableBalances[tx.origin][tranche].add(
            amount
        );
        _totalSupplies[tranche] = _totalSupplies[tranche].add(amount);
        _updateWorkingBalance(tx.origin);

        if (tranche == TRANCHE_M) {
            tokenM.safeTransferFrom(tx.origin, address(this), amount);
        } else if (tranche == TRANCHE_A) {
            tokenA.safeTransferFrom(tx.origin, address(this), amount);
        } else {
            tokenB.safeTransferFrom(tx.origin, address(this), amount);
        }

        emit Deposited(tranche, tx.origin, amount);
    }
    ...
    function claimAndDeposit(address primaryMarket) external {
        (uint256 createdShares, ) = IPrimaryMarketV2(primaryMarket).claim(tx.origin);
