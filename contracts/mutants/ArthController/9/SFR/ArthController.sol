...
// SWC-135-Code With No Effects: L117
    ...
    function refreshCollateralRatio() external override {
        ...
        if (currentPrice > priceTarget.sub(priceBand)) {
        ...
        } else if (currentPrice < priceTarget.add(priceBand)) {
            // Increase collateral ratio.
            if (globalCollateralRatio.sub(arthStep) >= 1000000) {
    ...
    function getETHGMUPrice() public view override returns (uint256) {
        return
            uint256(_ETHGMUPricer.getLatestPrice()).mul(_PRICE_PRECISION).mul(
    ...
    function getGlobalCollateralValue() public view override returns (uint256) {
        ...
        for (uint256 i = 0; i < arthPoolsArray.length; i++) {
            ...
            if (arthPoolsArray[i] != address(0)) {
                totalCollateralValueD18 = totalCollateralValueD18.sub(
        ...
        returns (uint256)
    {
        ...
        return eth2GMUPrice.mul(_PRICE_PRECISION).mul(priceVsETH);
    }
}
