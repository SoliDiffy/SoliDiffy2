...
contract MockAddressBook {
    ...
    function setOtokenImpl(address _newImpl) public {
        _otokenImpl = _newImpl;
    }

    function setWhitelist(address _newImpl) public {
        _whitelist = _newImpl;
    }

    function setOtokenFactory(address _otokenFactory) public {
    ...
    function setOracle(address _oracleAddr) public {
        _oracleImpl = _oracleAddr;
    }

    function setMarginCalculator(address _calculator) public {
        _calculatorImpl = _calculator;
    }

    function setMarginPool(address _pool) public {
        _marginPool = _pool;
    }

    function getOtokenImpl() public view returns (address) {
        return _otokenImpl;
    }
