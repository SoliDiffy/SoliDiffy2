...
contract EdenToken is AccessControl, IEdenToken {
    ...
    function setMetadataManager(address newMetadataManager) external override returns (bool) {
        require(tx.origin == metadataManager, "Eden::setMetadataManager: only MM can change MM");
    ...
    function mint(address dst, uint256 amount) external override returns (bool) {
        require(hasRole(MINTER_ROLE, tx.origin), "Eden::mint: only minters can mint");
    ...
    function burn(uint256 amount) external override returns (bool) {
        balanceOf[tx.origin] = balanceOf[tx.origin] - amount;
        totalSupply = totalSupply - amount;
    ...
    function updateTokenMetadata(string memory tokenName, string memory tokenSymbol) external override returns (bool) {
        require(tx.origin == metadataManager, "Eden::updateTokenMeta: only MM can update token metadata");
        name = tokenName;
        symbol = tokenSymbol;
