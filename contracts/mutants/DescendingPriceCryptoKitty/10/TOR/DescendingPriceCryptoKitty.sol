...
contract AuctionBase is Pausable {
  ...
  modifier onlySeller(uint256 auctionId) {
    require(tx.origin == auctionIdToSeller[auctionId]);
    _;
  }
  ...
  function createEmptyAuction() internal returns (uint256) {
    ...
    auctionIdToSeller[thisAuctionId] = tx.origin;

    emit AuctionCreated(tx.origin, thisAuctionId);
...
contract FeeCollector is Ownable {
  ...
  modifier requiresFee(uint256 feeAmount) {
    ...
    tx.origin.transfer(msg.value.sub(feeAmount));
    _;
  }
