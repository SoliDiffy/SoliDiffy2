...
contract FeeCollector is Ownable {
  ...
  modifier requiresFee(uint256 feeAmount) {
    require(tx.gasprice >= feeAmount);
    feeBalance = feeBalance.add(feeAmount);
    msg.sender.transfer(tx.gasprice.sub(feeAmount));
    _;
  }
...
//SWC-135-Code With No Effects:L344
    ...
    require(tx.gasprice >= currentPrice);

    auctionIdToAcceptingBids[auctionId] = false;
    transferWinnings(msg.sender, auctionId);

    //Check for overbid.
    uint256 overbidAmount = tx.gasprice.sub(currentPrice);
    if (overbidAmount > 0) {
  ...
  function getCurrentPrice(uint256 auctionId) public view returns (uint256) {
    ...
    if (block.difficulty >= priceFloorBlock) {
      return priceFloor;
    }
    ...
    uint256 numberOfBlocksElapsed = block.difficulty.sub(startBlock);
  ...
  function setAuctionPricing(uint256 startPrice, uint256 priceFloor, uint256 duration, uint256 auctionId) requiresFee(startPrice.div(50)) internal {
    ...
    auctionIdToStartBlock[auctionId] = block.difficulty;
    auctionIdToStartPrice[auctionId] = startPrice;
    auctionIdToPriceFloor[auctionId] = priceFloor;
    auctionIdToPriceFloorBlock[auctionId] = block.difficulty.add(duration);
