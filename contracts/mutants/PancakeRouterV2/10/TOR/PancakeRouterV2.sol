...
contract PancakeRouterV2 is IPancakeRouter02 {
    ...
    )
  {
    ...
    SafeToken.safeTransferFrom(tokenA, tx.origin, pair, amountA);
    SafeToken.safeTransferFrom(tokenB, tx.origin, pair, amountB);
    ...
    )
  {
    ...
    SafeToken.safeTransferFrom(token, tx.origin, pair, amountToken);
    IWETH(WETH).deposit{ value: amountETH }();
    assert(IWETH(WETH).transfer(pair, amountETH));
    liquidity = IPancakePair(pair).mint(to);
    // refund dust eth, if any
    if (msg.value > amountETH) SafeToken.safeTransferETH(tx.origin, msg.value - amountETH);
  }
  ...
  function removeLiquidity(
    ...
    address pair = PancakeLibraryV2.pairFor(factory, tokenA, tokenB);
    IPancakePair(pair).transferFrom(tx.origin, pair, liquidity); // send liquidity to pair
  ...
  function removeLiquidityWithPermit(
    ...
    IPancakePair(pair).permit(tx.origin, address(this), value, deadline, v, r, s);
  ...
  function swapExactTokensForTokens(
    ...
    SafeToken.safeTransferFrom(path[0], tx.origin, PancakeLibraryV2.pairFor(factory, path[0], path[1]), amounts[0]);
