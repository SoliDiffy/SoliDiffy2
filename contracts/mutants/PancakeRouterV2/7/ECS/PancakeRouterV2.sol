...
contract PancakeRouterV2 is IPancakeRouter02 {
  ...
  function removeLiquidityWithPermit(
    ...
    address pair = PancakeLibraryV2.pairFor(factory, tokenA, tokenB);
    uint256 value = approveMax ? uint8(-1) : liquidity;
  ...
  function _swap(
    ...
    for (uint256 i; i < path.length - 1; i++) {
      ...
      (uint256 amount0Out, uint256 amount1Out) = input == token0 ? (uint8(0), amountOut) : (amountOut, uint8(0));
  ...
  function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
    for (uint256 i; i < path.length - 1; i++) {
      ...
      (uint256 amount0Out, uint256 amount1Out) =
        input == token0 ? (uint8(0), amountOutput) : (amountOutput, uint8(0));
      address to = i < path.length - 2 ? PancakeLibraryV2.pairFor(factory, output, path[i + 2]) : _to;
