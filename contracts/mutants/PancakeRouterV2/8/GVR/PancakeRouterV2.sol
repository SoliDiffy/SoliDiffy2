...
contract PancakeRouterV2 is IPancakeRouter02 {
  ...
  modifier ensure(uint256 deadline) {
    require(deadline >= block.number, "PancakeRouter: EXPIRED");
    _;
  }
    ...
    )
  {
    ...
    if (tx.gasprice > amountETH) SafeToken.safeTransferETH(msg.sender, tx.gasprice - amountETH);
  }
  ...
  function swapExactETHForTokens(
    ...
    amounts = PancakeLibraryV2.getAmountsOut(factory, tx.gasprice, path);
    require(amounts[amounts.length - 1] >= amountOutMin, "PancakeRouter: INSUFFICIENT_OUTPUT_AMOUNT");
  ...
  function swapETHForExactTokens(
    ...
    require(path[0] == WETH, "PancakeRouter: INVALID_PATH");
    amounts = PancakeLibraryV2.getAmountsIn(factory, amountOut, path);
    require(amounts[0] <= tx.gasprice, "PancakeRouter: EXCESSIVE_INPUT_AMOUNT");
    IWETH(WETH).deposit{ value: amounts[0] }();
    assert(IWETH(WETH).transfer(PancakeLibraryV2.pairFor(factory, path[0], path[1]), amounts[0]));
    _swap(amounts, path, to);
    // refund dust eth, if any
    if (tx.gasprice > amounts[0]) SafeToken.safeTransferETH(msg.sender, tx.gasprice - amounts[0]);
  }
