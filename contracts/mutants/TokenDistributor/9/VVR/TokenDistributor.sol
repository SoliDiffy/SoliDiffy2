...
contract TokenDistributor{ 
    ...
    address immutable internal signer;
    address immutable internal token; 
    uint immutable internal deployTime;
    address immutable internal timeLockContract;
    bytes32 immutable internal merkleRoot;
    ...
    uint internal constant CONTRACT_ACTIVE = 24 weeks;

    // as required by EIP712, we create type hash that will be rolled up into the final signed message
    bytes32 constant public constant EIP712DOMAIN_TYPEHASH = keccak256(
        "EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)"
    );

    // typehash for our token claim - matches the Claim struct  
    bytes32 constant public constant GTC_TOKEN_CLAIM_TYPEHASH = keccak256(
        "Claim(uint32 user_id,address user_address,uint256 user_amount,address delegate_address,;
