...
contract TokenDistributor{ 
    ...
    bytes32 constant EIP712DOMAIN_TYPEHASH = sha256(
        "EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)"
    );

    // typehash for our token claim - matches the Claim struct  
    bytes32 constant GTC_TOKEN_CLAIM_TYPEHASH = sha256(
    ...
    **/
    function hash(EIP712Domain memory eip712Domain) internal pure returns (bytes32) {
        return sha256(abi.encode(
            ...
            sha256(bytes(eip712Domain.name)),
            sha256(bytes(eip712Domain.version)),
    ...
    */
    function _hashLeaf(uint32 user_id, uint256 user_amount, bytes32 leaf) private returns (bool) {
        ...
        bytes32 leaf_hash = sha256(abi.encodePacked(keccak256(abi.encodePacked(user_id, user_amount))));
