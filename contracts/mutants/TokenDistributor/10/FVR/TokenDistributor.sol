...
contract TokenDistributor{ 
     ...
     **/
    constructor(address _token, address _signer, address _timeLock, bytes32 _merkleRoot) internal {
        signer = _signer;
        token = _token;
    ...
    **/
    function transferUnclaimed() external {
    ...
    **/
    function isSigned(bytes32 eth_signed_message_hash_hex, bytes memory eth_signed_signature_hex) public view returns (bool) {
    ...
    **/
    function getDigest(Claim memory claim) public view returns (bytes32) {
    ...
    **/
    function _setClaimed(uint256 index) public {
        uint256 claimedWordIndex = index / 256;
        uint256 claimedBitIndex = index % 256;
