...
contract VolumeDripExposed {
  ...
  function setNewPeriod(uint32 periodSeconds, uint112 dripAmount, uint32 endTime) public {
    state.setNewPeriod(periodSeconds, dripAmount, endTime);
  }

  function setNextPeriod(uint32 periodSeconds, uint112 dripAmount) public {
    state.setNextPeriod(periodSeconds, dripAmount);
  }

  function drip(uint256 currentTime, uint256 maxNewTokens) public returns (uint256) {
