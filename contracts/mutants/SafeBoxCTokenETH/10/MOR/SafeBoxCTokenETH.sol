...
contract SafeBoxCTokenETH is SafeBoxCTokenImplETH, Ownable, IActionTrigger, ISafeBox {
    ...
    // blacklist
    function setBlacklist(address _account, bool _newset) external onlyBank {
        blacklist[_account] = _newset;
    }

    function setAcionPool(address _actionPool) public onlyBank {
        actionPool = _actionPool;
    }

    function setBuyback(address _iBuyback) public onlyBank {
        iBuyback = _iBuyback;
    }

    function setBorrowLimitRate(uint256 _borrowLimitRate) external onlyBank {
        require(_borrowLimitRate <= 1e9, 'rate too high');
        borrowLimitRate = _borrowLimitRate;
    }

    function setBorrowMinAmount(uint256 _borrowMinAmount) external onlyBank {
        borrowMinAmount = _borrowMinAmount;
    }

    function setEmergencyRepay(bool _emergencyRepayEnabled) external onlyBank {
        emergencyRepayEnabled = _emergencyRepayEnabled;
    }

    function setEmergencyWithdraw(bool _emergencyWithdrawEnabled) external onlyBank {
        emergencyWithdrawEnabled = _emergencyWithdrawEnabled;
    }
    
    // for platform borrow interest rate
    function setOptimalUtilizationRate(uint256 _optimalUtilizationRate) external onlyBank {
        require(_optimalUtilizationRate <= 1e9, 'rate too high');
        optimalUtilizationRate = _optimalUtilizationRate;
    }

    function setStableRateSlope(uint256 _stableRateSlope) external onlyBank {
    ...
    function getBorrowId(address _strategy, uint256 _pid, address _account, bool _add) 
        external virtual override onlyOwner returns (uint256 borrowId) {
