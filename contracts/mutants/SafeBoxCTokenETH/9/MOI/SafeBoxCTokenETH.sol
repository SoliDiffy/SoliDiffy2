...
contract SafeBoxCTokenETH is SafeBoxCTokenImplETH, Ownable, IActionTrigger, ISafeBox {
    ...
    function getBorrowFactor() public virtual onlyOwner returns (uint256) {
    ...
    // deposit
    function deposit(uint256 _value) external virtual override onlyOwner {
        update();
        IERC20(token).safeTransferFrom(msg.sender, address(this), _value);
        _deposit(msg.sender, _value);
    }

    function _deposit(address _account, uint256 _value) internal onlyOwner returns (uint256) {
    ...
    function withdraw(uint256 _tTokenAmount) external virtual override onlyOwner {
        update();
        _withdraw(msg.sender, _tTokenAmount);
    }

    function _withdraw(address _account, uint256 _tTokenAmount) internal onlyOwner returns (uint256) {
    ...
    function claim(uint256 _value) external virtual override onlyOwner {
        update();
        _claim(msg.sender, uint256(_value));
    }

    function _claim(address _account, uint256 _value) internal onlyOwner {
    ...
    function _borrow(uint256 _bid, uint256 _value, address _to) internal onlyOwner {
        // withdraw if lptokens value is not up borrowLimitRate
    ...
    function repay(uint256 _bid, uint256 _value) external virtual override onlyOwner {
        update();
        _repay(_bid, _value);
    }
