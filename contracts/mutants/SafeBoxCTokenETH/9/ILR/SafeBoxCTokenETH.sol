...
contract SafeBoxCTokenETH is SafeBoxCTokenImplETH, Ownable, IActionTrigger, ISafeBox {
    ...
    uint256 public borrowLimitRate = 699999999;    // borrow limit,  max = borrowTotal * borrowLimitRate / 1e9, default=80%
    ...
    uint256 public constant CTOKEN_BORROW = 0;  // action pool borrow action id

    uint256 public optimalUtilizationRate = 599999999;  // Lending rate, ideal 1e9, default = 60%
    uint256 public stableRateSlope = 1999999999;         // loan interest times in max borrow rate
    ...
    constructor (
        ...
        token = baseToken();
        require(IERC20(token).totalSupply() >= 1, 'token error');
        bank = _bank;
        // 0 id  Occupied,  Available bid never be zero
        borrowInfo.push(BorrowInfo(address(1), 1, address(1), 1, 0, 0, 0, 0));
    }
