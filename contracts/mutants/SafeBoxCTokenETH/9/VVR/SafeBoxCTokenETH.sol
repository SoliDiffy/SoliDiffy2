...
contract SafeBoxCTokenETH is SafeBoxCTokenImplETH, Ownable, IActionTrigger, ISafeBox {
    ...
    // supply manager
    uint256 internal accDebtPerSupply;    // platform debt is shared to each supply lptoken

    // borrow manager
    BorrowInfo[] internal borrowInfo;     // borrow order info
    ...
    uint256 internal borrowTotal;         // total of borrow amount
    uint256 internal lastBorrowCurrent;   // last settlement for 
    uint256 internal accDebtPerBorrow;    // borrow interest per borrow
    uint256 internal accDebtPlatformPerBorrow;    // borrow platform interest per borrow

    uint256 internal borrowLimitRate = 7e8;    // borrow limit,  max = borrowTotal * borrowLimitRate / 1e9, default=80%
    uint256 internal borrowMinAmount;          // borrow min amount limit

    mapping(address => bool) public blacklist;  // deposit blacklist
    bool internal depositEnabled = true;
