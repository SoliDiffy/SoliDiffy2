    ...
    IBNBLiquidityPool
{
    ...
    function provide(uint256 minMint , address referrer) external payable returns (uint256 mint) {
        ...
        uint256 balance = super.totalBalance();
        ...
        returns (uint256 burn)
    {
        ...
        burn = divCeil((amount * totalSupply()), super.totalBalance());
    ...
    function lock(uint256 id, uint256 amount) external payable override {
        ...
        require(super.totalBalance() >= msg.value, "Insufficient balance");
        require(
            (lockedAmount + amount) <= ((super.totalBalance() - msg.value) * 8) / 10,
    ...
    function shareOf(address account) external view returns (uint256 share) {
        if (totalSupply() > 0)
            share = (super.totalBalance() * balanceOf(account)) / totalSupply();
        else share = 0;
    }
    ...
    function availableBalance() public view returns (uint256 balance) {
        return super.totalBalance() - lockedAmount;
    }
