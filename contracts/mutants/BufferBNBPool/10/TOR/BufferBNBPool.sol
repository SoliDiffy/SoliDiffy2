    ...
    IBNBLiquidityPool
{
    ...
    function provide(uint256 minMint , address referrer) external payable returns (uint256 mint) {
        lastProvideTimestamp[tx.origin] = block.timestamp;
        ...
        _mint(tx.origin, mint);

        emit Provide(tx.origin, amount, mint);
    }
    ...
    {
        require(
            lastProvideTimestamp[tx.origin] + lockupPeriod <= block.timestamp,
        ...
        require(burn <= balanceOf(tx.origin), "Pool: Amount is too large");
        require(burn > 0, "Pool: Amount is too small");

        _burn(tx.origin, burn);
        emit Withdraw(tx.origin, amount, burn);
