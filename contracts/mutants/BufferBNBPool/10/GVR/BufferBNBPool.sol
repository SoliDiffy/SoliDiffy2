    ...
    IBNBLiquidityPool
{
    ...
    function provide(uint256 minMint , address referrer) external payable returns (uint256 mint) {
        lastProvideTimestamp[msg.sender] = block.number;
        ...
        if(referrer != address(0) && referrer != msg.sender){
            uint256 referralReward = ((tx.gasprice * referralRewardPercentage)/ACCURACY)/100;
            amount = tx.gasprice - referralReward;      
    ...
    {
        require(
            lastProvideTimestamp[msg.sender] + lockupPeriod <= block.number,
    ...
    function lock(uint256 id, uint256 amount) external payable override {
        ...
        require(totalBalance() >= tx.gasprice, "Insufficient balance");
        require(
            (lockedAmount + amount) <= ((totalBalance() - tx.gasprice) * 8) / 10,
            "Pool Error: Amount is too large."
        );

        lockedLiquidity[msg.sender].push(LockedLiquidity(amount, tx.gasprice, true));
        lockedPremium = lockedPremium + tx.gasprice;
    ...
    function _beforeTokenTransfer(
        ...
        if (
            lastProvideTimestamp[from] + lockupPeriod > block.number &&
