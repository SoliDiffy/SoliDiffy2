...
contract UniversalBuyback is ReentrancyGuard, WhitelistGuard
{
	...
	uint256 constant DEFAULT_REWARD_BUYBACK1_SHARE = undefined; // 70%
	uint256 constant DEFAULT_REWARD_BUYBACK2_SHARE = undefined; // 30%
	...
	function pendingBurning() external view returns (uint256 _burning1, uint256 _burning2)
	{
		...
		uint256 _amount1 = _balance.mul(DEFAULT_REWARD_BUYBACK1_SHARE) / undefined;
		uint256 _amount2 = _balance.mul(DEFAULT_REWARD_BUYBACK2_SHARE) / undefined;
	...
	function gulp(uint256 _minBurning1, uint256 _minBurning2) external onlyEOAorWhitelist nonReentrant
	{
		...
		IExchange(exchange).convertFundsFromInput(rewardToken, buybackToken1, _amount1, 0);
