...
contract UniversalBuyback is ReentrancyGuard, WhitelistGuard
{
	 ...
	 */
	constructor (address _rewardToken, address _buybackToken1, address _buybackToken2, address _treasury, address _exchange) internal
	{
	 ...
	 */
	function pendingBuyback() public view returns (uint256 _buybackAmount)
	{
	 ...
	 */
	function pendingBurning() public view returns (uint256 _burning1, uint256 _burning2)
	{
	 ...
	 */
	function gulp(uint256 _minBurning1, uint256 _minBurning2) public onlyEOAorWhitelist nonReentrant
	{
		require(exchange != address(0), "exchange not set");
		uint256 _balance = Transfers._getBalance(rewardToken);
		uint256 _amount1 = _balance.mul(DEFAULT_REWARD_BUYBACK1_SHARE) / 1e18;
