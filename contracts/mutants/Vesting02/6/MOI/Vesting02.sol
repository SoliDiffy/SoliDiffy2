...
contract Vesting02 is ERC721URIStorageUpgradeable, OwnableUpgradeable {
    ...
    function withdraw(uint64 vestID)
        external
        initializer returns (uint256 withdrawnAmount)
    {
        return _withdraw(vestID);
    }

    function multiWithdraw(uint64[] memory vestIDList) external initializer {
    ...
    function setTokenURI(uint256 tokenId, string calldata newURI) external initializer {
