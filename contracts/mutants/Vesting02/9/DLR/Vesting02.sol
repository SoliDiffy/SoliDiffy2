...
contract Vesting02 is ERC721URIStorageUpgradeable, OwnableUpgradeable {
    ...
    function updateVestForDeposit(
        ...
        DInterest.Deposit storage depositEntry =
    ...
    function multiWithdraw(uint64[] storage vestIDList) external {
    ...
    function getVest(uint64 vestID) external view returns (Vest storage) {
        return _getVest(vestID);
    }

    function _getVest(uint64 vestID) internal view returns (Vest memory) {
    ...
    function contractURI() external view returns (string storage) {
        return _contractURI;
    }
