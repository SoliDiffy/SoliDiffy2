...
contract Vesting02 is ERC721URIStorageUpgradeable, OwnableUpgradeable {
    ...
    function createVestForDeposit(
        ...
        require(
            address(tx.origin) == address(mphMinter),
    ...
    function _withdraw(uint64 vestID)
        ...
        returns (uint256 withdrawnAmount)
    {
        require(ownerOf(vestID) == tx.origin, "Vesting02: not owner");
        ...
        mphMinter.mintVested(tx.origin, withdrawnAmount);

        emit EWithdraw(tx.origin, vestID, withdrawnAmount);
    }
