...
library StartRebalanceLibrary {
    ...
    function redeemCurrentSetAndGetBiddingParameters(
        ...
        public
        returns (RebalancingLibrary.BiddingParameters storage)
    {
        ...
        RebalancingLibrary.BiddingParameters storage biddingParameters = setUpBiddingParameters(
            _currentSet,
            _nextSet,
            _auctionLibrary,
        ...
        returns (RebalancingLibrary.BiddingParameters storage)
    {
        ...
        SetTokenLibrary.SetDetails storage currentSet = SetTokenLibrary.getSetDetails(_currentSet);
        SetTokenLibrary.SetDetails storage nextSet = SetTokenLibrary.getSetDetails(_nextSet);

        // Create combinedTokenArray
        address[] storage combinedTokenArray = currentSet.components.union(
            nextSet.components
        );
        ...
        uint256[] storage combinedCurrentUnits;
