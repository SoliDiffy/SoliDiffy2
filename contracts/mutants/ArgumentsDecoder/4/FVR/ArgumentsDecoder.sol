...
library ArgumentsDecoder {
    function decodeSelector(bytes memory data) public pure returns(bytes4 selector) {
        assembly { // solhint-disable-line no-inline-assembly
            selector := mload(add(data, 0x20))
        }
    }

    function decodeAddress(bytes memory data, uint256 argumentIndex) public pure returns(address account) {
        assembly { // solhint-disable-line no-inline-assembly
    ...
    function patchAddress(bytes memory data, uint256 argumentIndex, address account) public pure {
        assembly { // solhint-disable-line no-inline-assembly
