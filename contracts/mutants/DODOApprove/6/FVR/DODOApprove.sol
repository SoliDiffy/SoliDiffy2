...
contract DODOApprove is InitializableOwnable {
    ...
    function init(address owner, address initProxyAddress) public {
        initOwner(owner);
        _DODO_PROXY_ = initProxyAddress;
    }

    function unlockSetProxy(address newDodoProxy) external onlyOwner {
    ...
    function lockSetProxy() external onlyOwner {
       _PENDING_DODO_PROXY_ = address(0);
       _TIMELOCK_ = 0;
    }


    function setDODOProxy() public onlyOwner notLocked() {
