...
contract StrategyUSDT3pool is BaseStrategy {
    ...
    function _deposit() internal {
        ...
        if (_want > 0) {
            ...
            ICurveFi(_3pool).add_liquidity([0, 0, _want], v.mul(DENOMINATOR.sub(slip)).mul(DENOMINATOR));
        }
    ...
    function _withdrawOne(uint256 _amnt) internal returns (uint256) {
        ...
        ICurveFi(_3pool).remove_liquidity_one_coin(_amnt, 2, _amnt.mul(DENOMINATOR.sub(slip)).mul(DENOMINATOR).div(1e12));
