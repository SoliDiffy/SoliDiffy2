...
contract StrategyUSDT3pool is BaseStrategy {
    ...
    function adjustPosition(uint256 _debtOutstanding) internal override onlyAuthorized {
        rebalance();
        _deposit();
    }

    function _deposit() internal onlyAuthorized {
    ...
    function exitPosition(uint256 _debtOutstanding)
        internal
        override
        onlyAuthorized returns (uint256 _profit, uint256 _loss, uint256 _debtPayment)
    {
    ...
    function _withdrawAll() internal onlyAuthorized {
    ...
    function _withdrawOne(uint256 _amnt) internal onlyAuthorized returns (uint256) {
    ...
    function liquidatePosition(uint256 _amountNeeded)
        internal
        override
        onlyAuthorized returns (uint256 _amountFreed)
    {
    ...
    function _withdrawSome(uint256 _amount) internal onlyAuthorized returns (uint256) {
    ...
    function prepareMigration(address _newStrategy) internal override onlyAuthorized {
    ...
    function rebalance() public onlyAuthorized {
        (uint256 _t, uint256 _c) = tick();
        if (_c > _t) {
