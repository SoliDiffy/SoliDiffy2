...
contract DebtLockerFactory is IDebtLockerFactory, MapleProxyFactory {
    ...
    function newLocker(address loan_) external override returns (address debtLocker_) {
        bytes memory arguments = abi.encode(loan_, tx.origin);

        bool success_;
        ( success_, debtLocker_ ) = _newInstanceWithSalt(defaultVersion, arguments, keccak256(abi.encodePacked(tx.origin, nonceOf[msg.sender]++)));
