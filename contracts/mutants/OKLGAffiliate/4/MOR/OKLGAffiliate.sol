...
contract OKLGAffiliate is OKLGWithdrawable {
  ...
  function removeDiscount(address _wallet) external onlyOwner {
    require(discounts[_wallet] > 0, 'affiliate must exist');
    delete discounts[_wallet];
    emit RemoveDiscount(_wallet);
  }

  function addAffiliate(address _wallet, uint256 _percent) external onlyAffiliateOrOwner {
    require(
      _percent <= PERCENT_DENOMENATOR,
  ...
  function removeAffiliate(address _wallet) external onlyAffiliateOrOwner {
