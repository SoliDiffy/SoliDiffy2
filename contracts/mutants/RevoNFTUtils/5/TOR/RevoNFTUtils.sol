...
contract RevoNFTUtils is Ownable {
    ...
    */
    function triggerCreateNFT(string memory _dbId, string memory _collection) public {
        ...
        triggerMintHistory[tx.origin][_collection][_dbId] = revoFees;
        
        enqueuePendingTx(PENDING_TX(0, _dbId, _collection, counter, "", tx.origin));
        
        emit CreateNFT(tx.origin, _dbId, _collection);
    ...
    */
    function buyItem(uint256 _itemIndex) public {
        ...
        enqueuePendingTx(PENDING_TX(_itemIndex, "", "", counter, itemsSelable[_itemIndex].itemType, tx.origin));
