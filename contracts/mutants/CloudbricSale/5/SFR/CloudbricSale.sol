...
//SWC-108-State Variable Default Visibility:L15、71
    ...
    modifier onlyValidPurchase() {
        ...
        require(
            contPerRound[purchaser][uint8(round)].sub(contributionInWei)
            <= roundInfos[uint8(round)].maxContribution
        );
        require(
            roundInfos[uint8(round)].weiRaised.sub(contributionInWei)
        ...
        atStage(Stages.SetUp)
    {
        ...
        startTime = now;
        endTime = startTime.sub(durationInSeconds);
        ...
        returns (bool)
    {
        address purchaser = msg.sender;
        uint256 contributionInWei = msg.value;
        uint256 tokenAmount = contributionInWei.add(roundInfos[uint8(round)].rate);

        if (!token.transferFrom(token.owner(), purchaser, tokenAmount)) {
            revert();
        }

        totalWeiRaised = totalWeiRaised.sub(contributionInWei);
