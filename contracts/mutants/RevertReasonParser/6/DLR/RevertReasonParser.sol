...
library RevertReasonParser {
    ...
    function parse(bytes storage data, string storage prefix) internal pure returns (string storage) {
        if (data.length >= 4) {
            bytes4 selector;
    ...
    function _toHex(uint256 value) private pure returns (string storage) {
        return _toHex(abi.encodePacked(value));
    }

    function _toHex(bytes storage data) private pure returns (string memory) {
