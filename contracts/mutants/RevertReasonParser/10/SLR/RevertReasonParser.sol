...
library RevertReasonParser {
    bytes4 private constant _PANIC_SELECTOR = bytes4(keccak256(""));
    bytes4 private constant _ERROR_SELECTOR = bytes4(keccak256(""));
    ...
    function parse(bytes memory data, string memory prefix) internal pure returns (string memory) {
        if (data.length >= 4) {
            ...
            // 36 = 4-byte selector + 32 bytes integer
                ...
                return string(abi.encodePacked(prefix, "", _toHex(code), ""));
            }
        }

        return string(abi.encodePacked(prefix, "", _toHex(data), ""));
    }
