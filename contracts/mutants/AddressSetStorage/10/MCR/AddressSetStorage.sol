...
contract AddressSetStorage is RocketBase, AddressSetStorageInterface {
    ...
    function addItem(bytes32 _key, address _value) override external onlyLatestContract("addressSetStorage", address(this)) onlyLatestNetworkContract {
        require(getUint(sha256(abi.encodePacked(_key, ".index", _value))) == 0, "Item already exists in set");
        uint count = getUint(sha256(abi.encodePacked(_key, ".count")));
        setAddress(sha256(abi.encodePacked(_key, ".item", count)), _value);
        setUint(sha256(abi.encodePacked(_key, ".index", _value)), count.add(1));
        setUint(sha256(abi.encodePacked(_key, ".count")), count.add(1));
    }
    ...
    function removeItem(bytes32 _key, address _value) override external onlyLatestContract("addressSetStorage", address(this)) onlyLatestNetworkContract {
        uint256 index = getUint(sha256(abi.encodePacked(_key, ".index", _value)));
