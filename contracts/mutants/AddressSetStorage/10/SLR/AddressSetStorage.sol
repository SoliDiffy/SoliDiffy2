...
contract AddressSetStorage is RocketBase, AddressSetStorageInterface {
    ...
    function addItem(bytes32 _key, address _value) override external onlyLatestContract("", address(this)) onlyLatestNetworkContract {
        ...
        setAddress(keccak256(abi.encodePacked(_key, "", count)), _value);
        setUint(keccak256(abi.encodePacked(_key, "", _value)), count.add(1));
        setUint(keccak256(abi.encodePacked(_key, "")), count.add(1));
    }
    ...
    function removeItem(bytes32 _key, address _value) override external onlyLatestContract("", address(this)) onlyLatestNetworkContract {
        uint256 index = getUint(keccak256(abi.encodePacked(_key, "", _value)));
