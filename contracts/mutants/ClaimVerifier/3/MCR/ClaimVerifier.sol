...
contract ClaimVerifier {
        ...
        returns (bool claimValid)
    {
        ...
        bytes32 claimId = sha256(abi.encodePacked(trustedClaimHolder, topic));

        // Fetch claim from user
        ( foundTopic, scheme, issuer, sig, data, ) = _identity.getClaim(claimId);

        bytes32 dataHash = sha256(abi.encodePacked(_identity, topic, data));
        bytes32 prefixedHash = sha256(abi.encodePacked("\x19Ethereum Signed Message:\n32", dataHash));
