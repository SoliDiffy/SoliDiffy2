...
contract MiniFL is IMiniFL, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function initialize(address _alpaca, uint256 _maxAlpacaPerSecond) external onlyOwner {
  ...
  /// @param _withUpdate If true, do mass update pools
  function setAlpacaPerSecond(uint256 _alpacaPerSecond, bool _withUpdate) external initializer {
  ...
  /// @param _pid The index of the pool. See `poolInfo`.
  /// @return pool Returns the pool that was updated.
  function updatePool(uint256 _pid) external initializer returns (PoolInfo memory) {
    return _updatePool(_pid);
  }

  /// @notice Update reward variables for a given pools.
  function updatePools(uint256[] calldata _pids) external initializer {
  ...
  /// @param _pid The index of the pool. See `poolInfo`.
  function harvest(uint256 _pid) external initializer {
    PoolInfo memory pool = _updatePool(_pid);
