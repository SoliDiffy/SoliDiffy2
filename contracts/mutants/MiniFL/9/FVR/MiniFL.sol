...
contract MiniFL is IMiniFL, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function initialize(address _alpaca, uint256 _maxAlpacaPerSecond) public initializer {
  ...
  /// @param _withUpdate If true, do mass update pools
  function setAlpacaPerSecond(uint256 _alpacaPerSecond, bool _withUpdate) public onlyOwner {
  ...
  function _updatePool(uint256 pid) public returns (PoolInfo memory) {
  ...
  /// @param _pid The index of the pool. See `poolInfo`.
  /// @return pool Returns the pool that was updated.
  function updatePool(uint256 _pid) public nonReentrant returns (PoolInfo memory) {
    return _updatePool(_pid);
  }

  /// @notice Update reward variables for a given pools.
  function updatePools(uint256[] calldata _pids) public nonReentrant {
