...
contract MiniFL is IMiniFL, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function deposit(
    ...
    if (pool.isDebtTokenPool && !stakeDebtTokenAllowance[_pid][tx.origin]) revert MiniFL_Forbidden();
    if (!pool.isDebtTokenPool && tx.origin != _for) revert MiniFL_Forbidden();
    ...
    stakingToken[_pid].safeTransferFrom(tx.origin, address(this), _amount);

    emit LogDeposit(tx.origin, _for, _pid, _amount);
  }
  ...
  function withdraw(
    ...
    stakingToken[_pid].safeTransfer(tx.origin, _amount);

    emit LogWithdraw(tx.origin, _for, _pid, _amount);
  }
