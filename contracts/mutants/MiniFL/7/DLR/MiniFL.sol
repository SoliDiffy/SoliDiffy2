...
contract MiniFL is IMiniFL, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function _updatePool(uint256 pid) internal returns (PoolInfo storage) {
  ...
  /// @param _pid The index of the pool. See `poolInfo`.
  /// @return pool Returns the pool that was updated.
  function updatePool(uint256 _pid) external nonReentrant returns (PoolInfo storage) {
  ...
  function deposit(
    ...
    PoolInfo storage pool = _updatePool(_pid);
    UserInfo memory user = userInfo[_pid][_for];
