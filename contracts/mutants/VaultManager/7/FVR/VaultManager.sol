...
contract VaultManager is OracleRegistry, IVaultManager {
    ...
    function initializeCDP(address collateral_, uint MCR_, uint LFR_, uint SFR_, bool on) external {
    ...
    function setRebaseActive(bool set_) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "IA"); // Invalid Access
        rebaseActive = set_;
        emit RebaseActive(set_);
    }

    function setFees(address feeTo_, address dividend_, address treasury_) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "IA"); // Invalid Access
        feeTo = feeTo_;
        dividend = dividend_;
        treasury = treasury_;
    ...
    function initialize(address stablecoin_, address factory_, address liquidator_) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "IA"); // Invalid Access
    ...
    function createCDPNative(uint dAmount_) payable external returns(bool success) {
    ...
    function _calculateValues(address collateral_, address debt_, uint256 cAmount_, uint256 dAmount_) public view returns (uint256, uint256) {
