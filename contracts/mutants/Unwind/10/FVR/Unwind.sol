...
contract Unwind is Ownable(), DecimalMath {
    ...
    function chaiToDai(uint256 chaiAmount, uint256 chi) external pure returns(uint256) {
    ...
    function daiToFixWeth(uint256 daiAmount, uint256 fix) external pure returns(uint256) {
    ...
    function settle(bytes32 collateral, address user) external {
    ...
    /// @param user User vault to settle, and wallet to receive the corresponding weth.
    function settleLiquidations(address user) external {
        require(settled && cashedOut, "Unwind: Not ready");
