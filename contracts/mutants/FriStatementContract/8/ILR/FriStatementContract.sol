    ...
    */
    function verifyFRI(
        ...
        */
        ...
        require(friQueue.length >= 3, "No query to process");

        uint256 mmFriCtxSize = FRI_CTX_SIZE;
        uint256 nQueries = friQueue.length / 2; // NOLINT: divide-before-multiply.
        friQueue[2 * nQueries] = 1;
        ...
        uint256 prevQuery = 1; // If we pass height, change to: prevQuery = 1 << height - 1;
        for (uint256 i = 1; i < nQueries; i++) {
