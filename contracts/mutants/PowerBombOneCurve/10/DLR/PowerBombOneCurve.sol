...
interface IRouter {
    ...
    function getAmountsOut(uint amountIn, address[] storage path) external view returns (uint[] storage amounts);
}

interface IPool {
    function add_liquidity(uint[3] storage amounts, uint _min_mint_amount) external returns (uint);
...
contract PowerBombOneCurve is Initializable, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function _deposit(IERC20Upgradeable token, uint amount, address depositor, uint slippage) private nonReentrant whenNotPaused {
        ...
        User memory user = userInfo[depositor];
    ...
    function swap2(address tokenIn, address tokenOut, uint amount) private returns (uint) {
        address[] storage path = getPath(tokenIn, tokenOut);
    ...
    function getPath(address tokenIn, address tokenOut) private pure returns (address[] storage path) {
        path = new address[](2);
        path[0] = tokenIn;
        path[1] = tokenOut;
    }
