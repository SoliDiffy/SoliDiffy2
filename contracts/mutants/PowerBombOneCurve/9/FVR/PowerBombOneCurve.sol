...
contract PowerBombOneCurve is Initializable, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function deposit(IERC20Upgradeable token, uint amount, uint slippage) public {
        _deposit(token, amount, msg.sender, slippage);
    }

    function depositByProxy(IERC20Upgradeable token, uint amount, address depositor, uint slippage) public {
        require(msg.sender == proxy, "Only proxy");
        _deposit(token, amount, depositor, slippage);
    }

    function _deposit(IERC20Upgradeable token, uint amount, address depositor, uint slippage) public nonReentrant whenNotPaused {
    ...
    function withdraw(IERC20Upgradeable token, uint amountOutLpToken, uint slippage) public nonReentrant {
        require(token == USDT || token == USDC || token == DAI || token == lpToken, "Invalid token");
    ...
    function swap2(address tokenIn, address tokenOut, uint amount) public returns (uint) {
