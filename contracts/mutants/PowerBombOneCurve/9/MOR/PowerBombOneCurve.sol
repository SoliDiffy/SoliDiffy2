...
contract PowerBombOneCurve is Initializable, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    uint public ibRewardTokenBaseAmt; // SWC-131-Presence of unused variables
    ...
    function _deposit(IERC20Upgradeable token, uint amount, address depositor, uint slippage) private initializer whenNotPaused {
    ...
    function withdraw(IERC20Upgradeable token, uint amountOutLpToken, uint slippage) external initializer {
        require(token == USDT || token == USDC || token == DAI || token == lpToken, "Invalid token");
    ...
    function setProxy(address _proxy) external initializer {
    ...
    function setTVLMaxLimit(uint _tvlMaxLimit) external initializer {
