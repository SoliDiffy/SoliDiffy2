...
contract Reader is Constants {
  ...
  // 
  function depositVaultValues(address[] memory _vaults, bool _dp) external view returns (uint256[] memory values, uint256[] memory amounts) {
  ...
  function userVaultDepositAmounts(address _user, address[] memory _vaults) 
    external view returns (uint256[] memory underAmounts, uint256[] memory amounts) {
    uint len = _vaults.length;
    amounts = new uint[](len);
...
// 根据输入，预估实际可借和费用
  function pendingBorrow(uint amount) external view returns(uint fee, uint actualBorrow) {
...
// 根据输入，预估实际转换和费用
  function pendingRepay(address borrower, address vault, uint amount) external view returns(uint fee, uint actualRepay) {
  ...
  // 获取多个用户的价值
  function usersVaules(address[] memory users, bool dp) external view returns(uint[] memory totalBorrows, uint[] memory totalDeposits) {
