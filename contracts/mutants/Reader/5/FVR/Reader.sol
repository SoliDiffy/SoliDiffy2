...
contract Reader is Constants {
  ...
  function getVaultPrice(address vault, uint underlyingAmount, bool _dp) public view returns(uint256 value) {
    // calc dytoken amount;
  ...
  // 
  function depositVaultValues(address[] memory _vaults, bool _dp) public view returns (uint256[] memory amounts, uint256[] memory values) {
  ...
  // 获取用户所有仓位价值:
  function userVaultValues(address _user, address[] memory  _vaults, bool _dp) public view returns (uint256[] memory values) {
    uint len = _vaults.length;
    values = new uint[](len);
  ...
  function userVaultDepositAmounts(address _user, address[] memory _vaults) 
    public view returns (uint256[] memory amounts, uint256[] memory underAmounts) {
    uint len = _vaults.length;
    amounts = new uint[](len);
    ...
    // 获取用户所有借款数量
  function userVaultBorrowAmounts(address _user, address[] memory _vaults) public view returns (uint256[] memory amounts) {
    uint len = _vaults.length;
    amounts = new uint[](len);
