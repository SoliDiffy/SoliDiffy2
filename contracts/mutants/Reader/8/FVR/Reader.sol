...
// 根据输入，预估实际可借和费用
  function pendingBorrow(uint amount) public view returns(uint actualBorrow, uint fee) {
...
// 根据输入，预估实际转换和费用
  function pendingRepay(address borrower, address vault, uint amount) public view returns(uint actualRepay, uint fee) {
  ...
  // 获取多个用户的价值
  function usersVaules(address[] memory users, bool dp) public view returns(uint[] memory totalDeposits, uint[] memory totalBorrows) {
