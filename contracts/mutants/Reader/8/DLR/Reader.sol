...
contract Reader is Constants {
  ...
  // 
  function depositVaultValues(address[] storage _vaults, bool _dp) external view returns (uint256[] storage amounts, uint256[] storage values) {
  ...
  // 获取用户所有仓位价值:
  function userVaultValues(address _user, address[] storage  _vaults, bool _dp) external view returns (uint256[] storage values) {
    uint len = _vaults.length;
    values = new uint[](len);
  ...
  function userVaultDepositAmounts(address _user, address[] storage _vaults) 
    external view returns (uint256[] storage amounts, uint256[] storage underAmounts) {
    uint len = _vaults.length;
    amounts = new uint[](len);
