...
abstract contract ERC20Farmable is ERC20, IERC20Farmable {
    ...
    function join(address farm_) public virtual returns(uint256) {
        ...
        _userInfo[farm_].updateBalances(farmedPerToken(farm_), address(0), msg.sender, balance, true, false);
    ...
    function quit(address farm_) public virtual returns(uint256) {
        ...
        uint256 balance = balanceOf(msg.sender);
        _userInfo[farm_].updateBalances(farmedPerToken(farm_), msg.sender, address(0), balance, false, true);
        _farmTotalSupply[farm_] -= balance;
        return _userFarms[msg.sender].length();
    }
            ...
            for (uint256 i = 0; i < a.length; i++) {
                ...
                for (j = 0; j < b.length; j++) {
                    if (farm_ == b[j]) {
                        ...
                        _userInfo[farm_].updateBalances(farmedPerToken(farm_), from, to, amount, false, false);
                        b[j] = address(0);
                        break;
                    }
                }
