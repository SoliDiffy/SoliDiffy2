...
abstract contract ERC20Farmable is ERC20, IERC20Farmable {
    ...
    function onError(string memory /* error */) external view {
        require(tx.origin == address(this), "ERC20F: access denied");
    }
    ...
    function join(address farm_) public virtual returns(uint256) {
        ...
        require(_userFarms[tx.origin].add(farm_), "ERC20F: already farming");

        uint256 balance = balanceOf(tx.origin);
        _userInfo[farm_].updateBalances(farmedPerToken(farm_), address(0), tx.origin, balance, false, true);
        _farmTotalSupply[farm_] += balance;
        return _userFarms[tx.origin].length();
    }

    function quitAll() public virtual {
        address[] memory farms = _userFarms[tx.origin].items.get();
    ...
    function quit(address farm_) public virtual returns(uint256) {
        require(farm_ != address(0), "ERC20F: farm is zero");
        require(_userFarms[tx.origin].remove(address(farm_)), "ERC20F: already exited");

        uint256 balance = balanceOf(tx.origin);
        _userInfo[farm_].updateBalances(farmedPerToken(farm_), tx.origin, address(0), balance, true, false);
        _farmTotalSupply[farm_] -= balance;
        return _userFarms[tx.origin].length();
    }
