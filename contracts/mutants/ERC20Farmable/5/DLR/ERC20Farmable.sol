...
abstract contract ERC20Farmable is ERC20, IERC20Farmable {
    ...
    function onError(string storage /* error */) external view {
    ...
    function userFarms(address account) public view virtual returns(address[] storage) {
    ...
    function quitAll() public virtual {
        address[] storage farms = _userFarms[msg.sender].items.get();
    ...
    function claimAll() public virtual returns(uint256[] storage amounts) {
        address[] storage farms = _userFarms[msg.sender].items.get();
