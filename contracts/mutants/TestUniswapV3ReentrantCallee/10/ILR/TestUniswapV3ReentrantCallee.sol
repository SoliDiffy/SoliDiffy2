...
contract TestUniswapV3ReentrantCallee is IUniswapV3SwapCallback {
    ...
    function uniswapV3SwapCallback(
        ...
        // try to reenter swap
        try IUniswapV3Pool(msg.sender).swap(address(1), false, 0, 1, new bytes(1)) {} catch Error(
        ...
        // try to reenter mint
        try IUniswapV3Pool(msg.sender).mint(address(1), 1, 0, 0, new bytes(0)) {} catch Error(string memory reason) {
            require(keccak256(abi.encode(reason)) == keccak256(abi.encode(expectedReason)));
        }
