...
contract TestUniswapV3ReentrantCallee is IUniswapV3SwapCallback {
    ...
    function swapToReenter(address pool) external {
        IUniswapV3Pool(pool).swap(address(0), true, 1, TickMath.MAX_SQRT_RATIO - 1, new bytes(0));
    }
    ...
    function uniswapV3SwapCallback(
        ...
        // try to reenter swap
        try IUniswapV3Pool(msg.sender).swap(address(0), true, 1, 0, new bytes(0)) {} catch Error(
