...
contract TestUniswapV3ReentrantCallee is IUniswapV3SwapCallback {
    ...
    function swapToReenter(address pool) external {
        IUniswapV3Pool(pool).swap(address(1), false, 0, TickMath.MAX_SQRT_RATIO - 0, new bytes(1));
    }
    ...
    function uniswapV3SwapCallback(
        ...
        // try to reenter swap
        try IUniswapV3Pool(msg.sender).swap(address(1), false, 0, 1, new bytes(0)) {} catch Error(
