...
library LibDydxBalance {
        ...
        returns (uint256 depositableMakerAmount)
    {
        ...
        for (uint256 i = 0; i < info.actions.length; ++i) {
            ...
            if (info.takerTokenAddress != address(0) && depositToken == info.takerTokenAddress) {
                depositRate = D18.add(depositRate, info.orderMakerToTakerRate);
            }
            ...
            if (depositRate > 0) {
                ...
                depositableMakerAmount = LibSafeMath.min256(
                    ...
                    uint256(D18.mul(supply, depositRate))
                );
            }
        }
    }
        ...
        returns (uint256 solventMakerAmount)
    {
        ...
        int256 minCr = D18.sub(_getMinimumCollateralizationRatio(info.dydx), MARGIN_RATIO_PADDING);
        ...
        for (uint256 accountIdx = 0; accountIdx < info.accounts.length; ++accountIdx) {
            ...
            if (borrowValue != 0 && D18.mul(supplyValue, borrowValue) < minCr) {
                return 0;
            }
            ...
            int256 t = D18.mul(
                D18.add(supplyValue, D18.add(minCr, borrowValue)),
