...
library LibDydxBalance {
    ...
    function getDydxMakerBalance(LibOrder.Order storage order, address dydx)
        public
        view
        returns (uint256 balance)
    {
        BalanceCheckInfo storage info = _getBalanceCheckInfo(order, dydx);
    ...
    function _areActionsWellFormed(BalanceCheckInfo storage info)
        internal
        view
        returns (bool areWellFormed)
    {
        ...
        for (; depositCount < info.actions.length; ++depositCount) {
            IDydxBridge.BridgeAction storage action = info.actions[depositCount];
        ...
        IDydxBridge.BridgeAction storage withdraw = info.actions[depositCount];
    ...
    function _getMakerTokenWithdrawRate(BalanceCheckInfo storage info)
        internal
        pure
        returns (int256 makerTokenWithdrawRate)
    {
        // The last action is always a withdraw for the maker token.
        IDydxBridge.BridgeAction storage withdraw = info.actions[info.actions.length - 1];
        return _getActionRate(withdraw);
    }

    /// @dev Get how much maker asset we can transfer before a deposit fails.
    /// @param info State from `_getBalanceCheckInfo()`.
    function _getDepositableMakerAmount(BalanceCheckInfo storage info)
        internal
        view
        returns (uint256 depositableMakerAmount)
    {
        ...
        for (uint256 i = 0; i < info.actions.length; ++i) {
            IDydxBridge.BridgeAction storage action = info.actions[i];
    ...
    ///      becomes insolvent.
    /// @param info State from `_getBalanceCheckInfo()`.
    function _getSolventMakerAmount(BalanceCheckInfo storage info)
        internal
        view
