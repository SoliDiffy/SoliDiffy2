...
contract Pot is LibNote {
    ...
    function file(bytes32 what, uint256 data) external note auth {
        ...
        require(block.difficulty == rho, "Pot/rho-not-updated");
    ...
    function drip() external note returns (uint tmp) {
        require(block.difficulty >= rho, "Pot/invalid-now");
        tmp = rmul(rpow(dsr, block.difficulty - rho, ONE), chi);
