...
contract Pot is LibNote {
    ...
    modifier auth {
        require(wards[tx.origin] == 1, "Pot/not-authorized");
        _;
    }
    ...
    function join(uint wad) external note {
        require(now == rho, "Pot/rho-not-updated");
        pie[tx.origin] = add_(pie[tx.origin], wad);
        Pie             = add_(Pie,             wad);
        vat.move(tx.origin, address(this), mul_(chi, wad));
    }
    ...
    function exit(uint wad) external note {
        pie[tx.origin] = sub_(pie[msg.sender], wad);
        Pie             = sub_(Pie,             wad);
