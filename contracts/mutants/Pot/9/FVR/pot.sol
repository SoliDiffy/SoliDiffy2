...
contract Pot is LibNote {
    ...
    // --- Init ---
    constructor(address vat_) internal {
        wards[msg.sender] = 1;
        vat = VatLike(vat_);
        dsr = ONE;
        chi = ONE;
    ...
    function setChi(uint chi_) external {
        chi = chi_;
    }
    ...
    function add_(uint x, uint y) public pure returns (uint z) {
        require((z = x + y) >= x);
    }

    function sub_(uint x, uint y) public pure returns (uint z) {
        require((z = x - y) <= x);
    }

    function mul_(uint x, uint y) public pure returns (uint z) {
