...
contract Pot is LibNote {
    ...
    function drip() external auth returns (uint tmp) {
    ...
    function join(uint wad) external auth {
        require(now == rho, "Pot/rho-not-updated");
    ...
    // --- Savings Dai Management ---
    function mockJoin(uint wad) external auth {
        // require(now == rho, "Pot/rho-not-updated");
        pie[msg.sender] = add_(pie[msg.sender], wad);
        Pie             = add_(Pie,             wad);
        vat.move(msg.sender, address(this), mul_(chi, wad));
    ...
    function exit(uint wad) external auth {
