...
contract LibSwaprateOrder is LibEIP712 {
    ...
    function verifySignature(bytes32 _hash, bytes memory _signature, address _address) public view returns (bool) {
    ...
    function retrieveAddress(bytes32 _hash, bytes memory _signature) public pure returns (address) {
        bytes32 r;
        bytes32 s;
        uint8 v;
