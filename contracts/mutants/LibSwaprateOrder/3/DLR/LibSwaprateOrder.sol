...
contract LibSwaprateOrder is LibEIP712 {
    ...
    function verifySignature(bytes32 _hash, bytes storage _signature, address _address) internal view returns (bool) {
    ...
    function retrieveAddress(bytes32 _hash, bytes storage _signature) private pure returns (address) {
        bytes32 r;
        bytes32 s;
        uint8 v;
