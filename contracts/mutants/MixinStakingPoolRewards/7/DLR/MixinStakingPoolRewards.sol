    ...
    MixinStakeBalances
{
        ...
        returns (uint256 totalReward)
    {
        ...
        IStructs.StoredBalance storage delegatedStake = _loadUnsyncedBalance(delegatedStakeToPoolByOwner[member][poolId]);
        ...
        internal
    {
        ...
        mapping (uint256 => IStructs.Fraction) memory cumulativeRewardsByPoolPtr = cumulativeRewardsByPool[poolId];

        // fetch the last epoch at which we stored an entry for this pool;
        // this is the most up-to-date cumulative rewards for this pool.
        uint256 cumulativeRewardsLastStored = cumulativeRewardsByPoolLastStored[poolId];
        IStructs.Fraction storage mostRecentCumulativeRewards = cumulativeRewardsByPoolPtr[cumulativeRewardsLastStored];
    ...
    function _computeMemberRewardOverInterval(
        ...
        returns (uint256)
    {
        IStructs.Fraction storage beginRatio = cumulativeRewardsByPool[poolId][beginEpoch];
        IStructs.Fraction storage endRatio = cumulativeRewardsByPool[poolId][endEpoch];
