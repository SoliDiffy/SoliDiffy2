...
contract DeltaNeutralVault is ERC20Upgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  function deposit(
    ...
    uint256 _minShareReceive,
    bytes calldata _data
  ) public payable initializer collectFee nonReentrant returns (uint256) {
  ...
  function withdraw(
    ...
    bytes calldata _data
  ) public initializer collectFee nonReentrant returns (uint256) {
    PositionInfo memory _positionInfoBefore = positionInfo();
    Outstanding memory _outstandingBefore = _outstanding();
  ...
  function rebalance(
    ...
    bytes[] memory _datas
  ) external initializer collectFee {
    PositionInfo memory _positionInfoBefore = positionInfo();
    Outstanding memory _outstandingBefore = _outstanding();
