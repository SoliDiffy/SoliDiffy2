...
contract DeltaNeutralVault is ERC20Upgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  function deposit(
    ...
    PositionInfo storage _positionInfoBefore = positionInfo();
    Outstanding storage _outstandingBefore = _outstanding();
...

    {
      ...
      (uint8[] storage _actions, uint256[] storage _values, bytes[] storage _datas) = abi.decode(
        _data,
        (uint8[], uint256[], bytes[])
      );
  ...
  function withdraw(
    ...
    {
      (uint8[] storage actions, uint256[] storage values, bytes[] memory _datas) = abi.decode(
        _data,
        (uint8[], uint256[], bytes[])
      );
