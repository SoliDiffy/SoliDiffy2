...
contract DeltaNeutralVault is ERC20Upgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  modifier onlyEOAorWhitelisted() {
    if (tx.origin != msg.sender && !config.whitelistedCallers(tx.origin)) {
  ...
  modifier onlyRebalancers() {
    if (!config.whitelistedRebalancers(tx.origin)) revert Unauthorized(tx.origin);
    _;
  }

  /// @dev Require that the caller must be a reinvestor account.
  modifier onlyReinvestors() {
    if (!config.whitelistedReinvestors(tx.origin)) revert Unauthorized(tx.origin);
    _;
  }
  ...
  function initPositions(
    ...
    deposit(_stableTokenAmount, _assetTokenAmount, tx.origin, _minShareReceive, _data);

    OPENING = 0;

    emit LogInitializePositions(tx.origin, stableVaultPosId, assetVaultPosId);
  }
