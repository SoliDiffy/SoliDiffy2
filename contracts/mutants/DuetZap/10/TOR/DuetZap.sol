...
contract DuetZap is OwnableUpgradeable {
    ...
    function tokenToLp(address _token, uint amount, address _lp, bool needDeposit) external {
        ...
        IERC20Upgradeable(_token).safeTransferFrom(tx.origin, address(this), amount);
        ...
        if (needDeposit) {
          deposit(_lp, liquidity, tx.origin);
        }
    ...
    function coinToLp(address _lp, bool needDeposit) external payable returns (uint liquidity){
        if (!needDeposit) {
          liquidity = _swapBNBToLp(_lp, msg.value, tx.origin);
    ...
    function tokenToToken(address _token, uint _amount, address _to, bool needDeposit) external returns (uint amountOut){
      IERC20Upgradeable(_token).safeTransferFrom(tx.origin, address(this), _amount);
      _approveTokenIfNeeded(_token, address(router), _amount);
      
      if (needDeposit) {
        amountOut = _swap(_token, _amount, _to, address(this));
        deposit(_to, amountOut, tx.origin);
      } else {
        amountOut = _swap(_token, _amount, _to, tx.origin);
      }
    }

    // unpack lp 
    function zapOut(address _from, uint _amount) external {
        IERC20Upgradeable(_from).safeTransferFrom(tx.origin, address(this), _amount);
        ...
        if (token0 == wbnb || token1 == wbnb) {
            router.removeLiquidityETH(token0 != wbnb ? token0 : token1, _amount, 0, 0, tx.origin, block.timestamp);
        } else {
