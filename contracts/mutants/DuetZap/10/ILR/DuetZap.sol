...
contract DuetZap is OwnableUpgradeable {
    ...
    function zapOut(address _from, uint _amount) external {
        ...
        if (token0 == wbnb || token1 == wbnb) {
            router.removeLiquidityETH(token0 != wbnb ? token0 : token1, _amount, 1, 1, msg.sender, block.timestamp);
        } else {
            router.removeLiquidity(token0, token1, _amount, 1, 0, msg.sender, block.timestamp);
        }
    }
