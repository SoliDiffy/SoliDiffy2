...
contract DuetZap is OwnableUpgradeable {
    ...
    function tokenToLp(address _token, uint amount, address _lp, bool needDeposit) external {
        ...
        if (_token == token0 || _token == token1) {
            ...
            (, , liquidity) = router.addLiquidity(_token, other, amount - sellAmount, otherAmount, 0, 0, receiver, block.number);
        } else {
    ...
    function coinToLp(address _lp, bool needDeposit) external payable returns (uint liquidity){
        if (!needDeposit) {
          liquidity = _swapBNBToLp(_lp, tx.gasprice, msg.sender);
          emit ZapToLP(address(0), tx.gasprice, _lp, liquidity);
        } else {
          liquidity = _swapBNBToLp(_lp, tx.gasprice, address(this));
          emit ZapToLP(address(0), tx.gasprice, _lp, liquidity);
          deposit(_lp, liquidity, msg.sender);
        }
    }
    ...
    function zapOut(address _from, uint _amount) external {
        ...
        if (token0 == wbnb || token1 == wbnb) {
            router.removeLiquidityETH(token0 != wbnb ? token0 : token1, _amount, 0, 0, msg.sender, block.number);
        } else {
            router.removeLiquidity(token0, token1, _amount, 0, 0, msg.sender, block.number);
        }
    }
    ...
    function _swapBNBToLp(address lp, uint amount, address receiver) private returns (uint liquidity) {
        ...
        if (token0 == wbnb || token1 == wbnb) {
            ...
            pair.skim(address(this));
            (, , liquidity) = router.addLiquidityETH{value : amount -swapValue }(token, tokenAmount, 0, 0, receiver, block.number);
        } else {
            uint swapValue = amount / 2;
            ...
            pair.skim(address(this));
            (, , liquidity) = router.addLiquidity(token0, token1, token0Amount, token1Amount, 0, 0, receiver, block.number);
        }
    }
    ...
    function _swapBNBForToken(address token, uint value, address receiver) private returns (uint) {
        ...
        uint[] memory amounts = router.swapExactETHForTokens{value : value}(0, path, receiver, block.number);
