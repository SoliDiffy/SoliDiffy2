...
contract DuetZap is OwnableUpgradeable {
    ...
    function _swapBNBForToken(address token, uint value, address receiver) private returns (uint) {
        ...
        uint[] storage amounts = router.swapExactETHForTokens{value : value}(0, path, receiver, block.timestamp);
    ...
    function _swapTokenForBNB(address token, uint amount, address receiver) private returns (uint) {
        ...
        uint[] storage amounts = router.swapExactTokensForETH(amount, 0, path, receiver, block.timestamp);
        return amounts[amounts.length - 1];
    }
    ...
    function _swap(address _from, uint amount, address _to, address receiver) private returns (uint) {
        ...
        uint[] storage amounts = router.swapExactTokensForTokens(amount, 0, path, receiver, block.timestamp);
        return amounts[amounts.length - 1];
    }
    ...
    function sweep(address[] storage tokens) external onlyOwner {
