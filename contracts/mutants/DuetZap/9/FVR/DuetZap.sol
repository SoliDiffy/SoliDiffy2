...
contract DuetZap is OwnableUpgradeable {
    ...
    /* ========== Private Functions ========== */
    function deposit(address token, uint amount, address toUser) public {
    ...
    function _approveTokenIfNeeded(address token, address spender, uint amount) public {
    ...
    function _swapBNBToLp(address lp, uint amount, address receiver) public returns (uint liquidity) {
        IPair pair = IPair(lp);
        address token0 = pair.token0();
        address token1 = pair.token1();
        if (token0 == wbnb || token1 == wbnb) {
