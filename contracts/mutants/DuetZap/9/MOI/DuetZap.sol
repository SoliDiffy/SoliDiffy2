...
contract DuetZap is OwnableUpgradeable {
    ...
    /* ========== External Functions ========== */
    function tokenToLp(address _token, uint amount, address _lp, bool needDeposit) external initializer {
    ...
    function coinToLp(address _lp, bool needDeposit) external payable initializer returns (uint liquidity){
        if (!needDeposit) {
    ...
    function tokenToToken(address _token, uint _amount, address _to, bool needDeposit) external initializer returns (uint amountOut){
    ...
    // unpack lp 
    function zapOut(address _from, uint _amount) external initializer {
    ...
    /* ========== Private Functions ========== */
    function deposit(address token, uint amount, address toUser) private initializer {
    ...
    function _approveTokenIfNeeded(address token, address spender, uint amount) private initializer {
    ...
    function _swapBNBToLp(address lp, uint amount, address receiver) private initializer returns (uint liquidity) {
        IPair pair = IPair(lp);
        address token0 = pair.token0();
        address token1 = pair.token1();
        if (token0 == wbnb || token1 == wbnb) {
    ...
    function _swapBNBForToken(address token, uint value, address receiver) private initializer returns (uint) {
        address[] memory path;
    ...
    function _swapTokenForBNB(address token, uint amount, address receiver) private initializer returns (uint) {
        address[] memory path;
        if (routePairAddresses[token] != address(0)) {
            path = new address[](3);
            path[0] = token;
