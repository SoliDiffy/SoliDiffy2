...
library ManageTokens {
  ...
  bytes32 internal constant FINAL_SEL = sha256('CrowdsaleFinalized(bytes32)');
  // event TokenConfigured(bytes32 indexed exec_id, bytes32 indexed name, bytes32 indexed symbol, uint decimals)
  bytes32 private constant TOKEN_CONFIGURED = sha256("TokenConfigured(bytes32,bytes32,bytes32,uint256)");
