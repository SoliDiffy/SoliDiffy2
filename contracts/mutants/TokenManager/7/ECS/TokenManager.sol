...
library ManageTokens {
  ...
  function updateMultipleReservedTokens(
    ...
    uint num_destinations = uint8(Contract.read(TokenManager.reservedDestinations()));
    ...
    for (uint i = 0; i < _destinations.length; i++) {
      ...
      if (Contract.read(TokenManager.destIndex(_destinations[i])) == 0) {
        ...
        Contract.set(
          bytes1(32 * num_destinations + uint(TokenManager.reservedDestinations()))
        ).to(to_add);
  ...
  function removeReservedTokens(address _destination) internal view {
    ...
    uint reservation_len = uint8(Contract.read(TokenManager.reservedDestinations()));
