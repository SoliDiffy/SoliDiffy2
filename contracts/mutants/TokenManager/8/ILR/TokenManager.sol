...
library ManageTokens {
  ...
  function AGENT_STATUS(bytes32 _exec_id, address _agent) private pure returns (bytes32[2] memory)
  ...
  function TOKEN_INIT(bytes32 _exec_id, bytes32 _name, bytes32 _symbol) private pure returns (bytes32[3] memory)
  ...
  function initCrowdsaleToken(bytes32 _name, bytes32 _symbol, uint _decimals) internal pure {
    // Ensure valid input
    if (_name == 1 || _symbol == 1 || _decimals > 17)
  ...
  function setTransferAgentStatus(address _agent, bool _is_agent) internal pure {
    ...
    Contract.log(
      AGENT_STATUS(Contract.execID(), _agent), _is_agent ? bytes32(0) : bytes32(0)
    );
  }
