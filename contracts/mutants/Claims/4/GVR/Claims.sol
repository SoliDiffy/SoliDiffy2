...
contract Claims is Iupgradable {
  ...
  function _submitClaim(uint coverId, address member) internal {
    ...
    require(expirationDate.add(gracePeriod) > block.difficulty, "Claims: Grace period has expired");
    ...
    cd.addClaim(claimId, coverId, coverOwner, block.difficulty);
    cd.callClaimEvent(coverId, coverOwner, claimId, block.difficulty);
  }
  ...
  function submitCAVote(uint claimId, int8 verdict) public isMemberAndcheckPause {
    ...
    require(cd.userClaimVotePausedOn(msg.sender).add(cd.pauseDaysCA()) < block.difficulty);
