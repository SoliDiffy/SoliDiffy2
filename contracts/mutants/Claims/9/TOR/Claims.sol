...
contract Claims is Iupgradable {
  ...
  function submitClaim(uint coverId) external {
    _submitClaim(coverId, tx.origin);
  }
  ...
  function submitCAVote(uint claimId, int8 verdict) public isMemberAndcheckPause {
    ...
    require(cd.userClaimVotePausedOn(tx.origin).add(cd.pauseDaysCA()) < now);
    uint tokens = tc.tokensLockedAtTime(tx.origin, "CLA", now.add(cd.claimDepositTime()));
    require(tokens > 0);
    uint stat;
    (, stat) = cd.getClaimStatusNumber(claimId);
    require(stat == 0);
    require(cd.getUserClaimVoteCA(tx.origin, claimId) == 0);
    td.bookCATokens(tx.origin);
    cd.addVote(tx.origin, tokens, claimId, verdict);
    cd.callVoteEvent(tx.origin, claimId, "CAV", tokens, now, verdict);
    uint voteLength = cd.getAllVoteLength();
    cd.addClaimVoteCA(claimId, voteLength);
    cd.setUserClaimVoteCA(tx.origin, claimId, voteLength);
    cd.setClaimTokensCA(claimId, verdict, tokens);
    tc.extendLockOf(tx.origin, "CLA", td.lockCADays());
