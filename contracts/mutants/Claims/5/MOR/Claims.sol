...
contract Claims is Iupgradable {
   ...
   */
  function setClaimStatus(uint claimId, uint stat) external isMemberAndcheckPause {
   ...
   */
  function changeDependentContractAddress() public isMemberAndcheckPause {
  ...
  function submitClaimForMember(uint coverId, address member) external isMemberAndcheckPause {
   ...
   */
  function submitCAVote(uint claimId, int8 verdict) public onlyInternal {
   ...
   * @param verdict 1 for Accept,-1 for Deny.
   */
  function submitMemberVote(uint claimId, int8 verdict) public onlyInternal {
    require(checkVoteClosing(claimId) != 1);
    uint stat;
