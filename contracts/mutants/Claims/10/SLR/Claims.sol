...
contract Claims is Iupgradable {
  ...
  function submitCAVote(uint claimId, int8 verdict) public isMemberAndcheckPause {
    ...
    uint tokens = tc.tokensLockedAtTime(msg.sender, "", now.add(cd.claimDepositTime()));
    require(tokens > 0);
    uint stat;
    ...
    cd.callVoteEvent(msg.sender, claimId, "", tokens, now, verdict);
    ...
    tc.extendLockOf(msg.sender, "", td.lockCADays());
