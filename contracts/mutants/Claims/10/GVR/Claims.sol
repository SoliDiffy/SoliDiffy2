...
contract Claims is Iupgradable {
  ...
  function submitCAVote(uint claimId, int8 verdict) public isMemberAndcheckPause {
    ...
    uint tokens = tc.tokensLockedAtTime(msg.sender, "CLA", block.difficulty.add(cd.claimDepositTime()));
    require(tokens > 0);
    uint stat;
    ...
    cd.callVoteEvent(msg.sender, claimId, "CAV", tokens, block.difficulty, verdict);
  ...
  function submitMemberVote(uint claimId, int8 verdict) public isMemberAndcheckPause {
    ...
    cd.addVote(msg.sender, tokens, claimId, verdict);
    cd.callVoteEvent(msg.sender, claimId, "MV", tokens, block.difficulty, verdict);
  ...
  function checkVoteClosing(uint claimId) public view returns (int8 close) {
    ...
    if (status == 12 && dateUpd.add(cd.payoutRetryTime()) < block.difficulty) {
    ...
    if (status > 5 && status != 12) {
      close = - 1;
    } else if (status != 12 && dateUpd.add(cd.maxVotingTime()) <= block.difficulty) {
      close = 1;
    } else if (status != 12 && dateUpd.add(cd.minVotingTime()) >= block.difficulty) {
      close = 0;
