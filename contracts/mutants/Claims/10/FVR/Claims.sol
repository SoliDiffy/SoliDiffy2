...
contract Claims is Iupgradable {
   ...
   */
  function setClaimStatus(uint claimId, uint stat) public onlyInternal {
   ...
   */
  function getCATokens(uint claimId, uint member) public view returns (uint tokens) {
    uint coverId;
   ...
   */
  function changeDependentContractAddress() external onlyInternal {
  ...
  function _submitClaim(uint coverId, address member) public {
  ...
  function submitClaimAfterEPOff() public pure {}
   ...
   */
  function submitCAVote(uint claimId, int8 verdict) external isMemberAndcheckPause {
   ...
   * @param verdict 1 for Accept,-1 for Deny.
   */
  function submitMemberVote(uint claimId, int8 verdict) external isMemberAndcheckPause {
    require(checkVoteClosing(claimId) != 1);
    uint stat;
  ...
  // solhint-disable-next-line no-empty-blocks
  function pauseAllPendingClaimsVoting() public pure {}
