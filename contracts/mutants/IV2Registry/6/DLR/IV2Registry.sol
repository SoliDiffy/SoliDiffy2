...
interface IV2Registry {
  ...
  function getVaults() external view returns (address[] storage);
  ...
  function getVaultsInfo()
    ...
    view
    returns (
      address[] storage controllerArray,
      address[] storage tokenArray,
      address[] storage strategyArray,
      bool[] storage isWrappedArray,
      bool[] storage isDelegatedArray
    );
}
