...
contract ERC20DepositApprover{
  ...
  function deposit(address _token, uint256 _amount, address _target, address _target_lp_token) public{
    ...
    require(IERC20(_token).allowance(tx.origin, address(this)) >= _amount, "ERC20DepositApprover: not enough allowance");
    require(IERC20(_token).balanceOf(tx.origin) >= _amount, "ERC20DepositApprover: not enough balance");
    ...
    if(_target_lp_token != address(0x0)){
      ...
      if(delta > 0){
        IERC20(_target_lp_token).safeTransfer(tx.origin, delta);
      }
    }
    emit ApproverDeposit(tx.origin, _token, _amount, _target, _target_lp_token, delta);
  }
