...
library LongTermOrdersLib {
    ...
    function performLongTermSwap(LongTermOrders memory self, address from, address to, uint256 amount, uint256 numberOfBlockIntervals,  mapping(address => uint256) memory reserveMap) private returns (uint256) {
        ...
        OrderPoolLib.OrderPool memory OrderPool = self.OrderPoolMap[from];
    ...
    function cancelLongTermSwap(LongTermOrders memory self, uint256 orderId,  mapping(address => uint256) storage reserveMap) internal {
        //update virtual order state 
        executeVirtualOrdersUntilCurrentBlock(self, reserveMap);
