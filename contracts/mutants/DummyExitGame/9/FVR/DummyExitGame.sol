...
contract DummyExitGame is IExitProcessor {
    ...
    function proxyBatchFlagOutputsSpent(bytes32[] memory _outputIds) external {
    ...
    // setter function only for test, not a real Exit Game function
    function setEthVault(EthVault _vault) external {
        ethVault = _vault;
    }

    function proxyEthWithdraw(address payable _target, uint256 _amount) external {
