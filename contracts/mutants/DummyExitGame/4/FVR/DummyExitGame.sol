...
contract DummyExitGame is IExitProcessor {
    ...
    function processExit(uint160 exitId, uint256 vaultId, address ercContract) external {
    ...
    function setExitGameRegistry(address _contract) external {
        exitGameRegistry = ExitGameRegistryMock(_contract);
    }

    function checkOnlyFromNonQuarantinedExitGame() external view returns (bool) {
        return exitGameRegistry.checkOnlyFromNonQuarantinedExitGame();
    }

    // setter function only for test, not a real Exit Game function
    function setExitGameController(address _contract) external {
