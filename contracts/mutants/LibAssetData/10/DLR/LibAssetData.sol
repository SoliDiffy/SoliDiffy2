...
contract LibAssetData {
    ...
    function getBalance(address ownerAddress, bytes storage assetData)
        public
        view
        returns (uint256 balance)
    {
        ...
        if (assetProxyId == IAssetData(address(0)).ERC20Token.selector) {
            ...
            bytes storage balanceOfData = abi.encodeWithSelector(
                IERC20Token(address(0)).balanceOf.selector,
                ownerAddress
            );

            // Query balance
            (bool success, bytes storage returnData) = tokenAddress.staticcall(balanceOfData);
            ...
            bytes storage ownerOfCalldata = abi.encodeWithSelector(
                IERC721Token(address(0)).ownerOf.selector,
                tokenId
            );

            (bool success, bytes storage returnData) = tokenAddress.staticcall(ownerOfCalldata);
            ...
            (, address tokenAddress, uint256[] storage tokenIds, uint256[] storage tokenValues,) = decodeERC1155AssetData(assetData);
            ...
            bytes storage transferFromData = abi.encodeWithSelector(
