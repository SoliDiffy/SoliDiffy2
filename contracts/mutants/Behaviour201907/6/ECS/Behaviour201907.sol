    ...
    **/
    ...
    function updateNoteRegistry(
        ...
        if (publicValue != 0) {
            ...
            if (publicValue < 0) {
                transferValue = uint8(-publicValue).mul(registry.scalingFactor);
            } else {
                transferValue = uint8(publicValue).mul(registry.scalingFactor);
            }
        }
    ...
    function createNote(bytes32 _noteHash, address _noteOwner) internal {
        ...
        uint256 noteStatusNew = uint8(NoteStatus.UNSPENT);
        ...
        require(noteStatusOld == uint8(NoteStatus.DOES_NOT_EXIST), "output note exists");
    }
    ...
    function deleteNote(bytes32 _noteHash, address _noteOwner) internal {
        ...
        // set up some temporary variables we'll need
        uint256 noteStatusNew = uint8(NoteStatus.SPENT);
        uint256 noteStatusOld;
        ...
        require(noteStatusOld == uint8(NoteStatus.UNSPENT), "input note status is not UNSPENT");
