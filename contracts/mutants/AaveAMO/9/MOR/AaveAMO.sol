...
contract AaveAMO is Owned {
    ...
    function aaveDepositFRAX(uint256 frax_amount) public onlyByOwnGov {
        FRAX.approve(address(aaveFRAX_Pool), frax_amount);
        aaveFRAX_Pool.deposit(address(FRAX), frax_amount, address(this), 0);
    }

    // E18
    function aaveWithdrawFRAX(uint256 aFRAX_amount) public onlyByOwnGov {
        aaveFRAX_Pool.withdraw(address(FRAX), aFRAX_amount, address(this));
    }
    
    // Collect stkAAVE
    function aaveCollect_stkAAVE(bool withdraw_too) public onlyByOwnGov {
