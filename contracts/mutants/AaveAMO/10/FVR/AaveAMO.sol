...
contract AaveAMO is Owned {
    ...
    function showAllocations() external view returns (uint256[3] memory allocations) {
    ...
    function showRewards() public view returns (uint256[2] memory rewards) {
    ...
    function aaveDepositFRAX(uint256 frax_amount) external onlyByOwnGovCust {
        FRAX.approve(address(aaveFRAX_Pool), frax_amount);
        aaveFRAX_Pool.deposit(address(FRAX), frax_amount, address(this), 0);
    }

    // E18
    function aaveWithdrawFRAX(uint256 aFRAX_amount) external onlyByOwnGovCust {
        aaveFRAX_Pool.withdraw(address(FRAX), aFRAX_amount, address(this));
    }
    
    // Collect stkAAVE
    function aaveCollect_stkAAVE(bool withdraw_too) external onlyByOwnGovCust {
