...
contract OwnerRelayOnOptimism is MixinResolver, TemporarilyOwned, IOwnerRelayOnOptimism {
    ...
    function directRelay(address target, bytes calldata payload) external onlyMessengerAndL1Relayer {
        _relayCall(target, payload);

        emit DirectRelay(target, payload);
    }

    function finalizeRelay(address target, bytes calldata payload) external onlyTemporaryOwner {
        _relayCall(target, payload);
