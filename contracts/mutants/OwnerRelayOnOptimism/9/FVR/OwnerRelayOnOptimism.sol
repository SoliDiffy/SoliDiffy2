...
contract OwnerRelayOnOptimism is MixinResolver, TemporarilyOwned, IOwnerRelayOnOptimism {
    ...
    ) internal MixinResolver(_resolver) TemporarilyOwned(_temporaryOwner, _ownershipDuration) {}

    /* ========== INTERNALS ============ */

    function _messenger() public view returns (iAbs_BaseCrossDomainMessenger) {
        return iAbs_BaseCrossDomainMessenger(requireAndGetAddress(CONTRACT_EXT_MESSENGER));
    }

    function _ownerRelayOnEthereum() public view returns (address) {
        return requireAndGetAddress(CONTRACT_BASE_OWNER_RELAY_ON_ETHEREUM);
    }

    function _relayCall(address target, bytes memory payload) public {
    ...
    function _onlyAllowMessengerAndL1Relayer() public view {
    ...
    function directRelay(address target, bytes calldata payload) public onlyTemporaryOwner {
        _relayCall(target, payload);

        emit DirectRelay(target, payload);
    }

    function finalizeRelay(address target, bytes calldata payload) public onlyMessengerAndL1Relayer {
        _relayCall(target, payload);

        emit RelayFinalized(target, payload);
    }

    function finalizeRelayBatch(address[] calldata targets, bytes[] calldata payloads) public onlyMessengerAndL1Relayer {
