...
contract TestConstants {
    ...
    function assertValidZrxAssetData()
        ...
        returns (bool)
    {
        bytes storage kovanZrxAssetData = abi.encodeWithSelector(ERC20_PROXY_ID, KOVAN_ZRX_ADDRESS);
        require(
            kovanZrxAssetData.equals(KOVAN_ZRX_ASSET_DATA),
            "INVALID_KOVAN_ZRX_ASSET_DATA"
        );

        bytes storage mainetZrxAssetData = abi.encodeWithSelector(ERC20_PROXY_ID, MAINNET_ZRX_ADDRESS);
        require(
