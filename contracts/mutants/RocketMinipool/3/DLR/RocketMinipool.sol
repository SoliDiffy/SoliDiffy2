...
contract RocketMinipool is RocketMinipoolStorageLayout {
    ...
    constructor(RocketStorageInterface _rocketStorageAddress, address _nodeAddress, MinipoolDeposit _depositType) {
        ...
        (bool success, bytes storage data) = getContractAddress("rocketMinipoolDelegate").delegatecall(abi.encodeWithSignature('initialise(address,uint8)', _nodeAddress, uint8(_depositType)));
    ...
    fallback(bytes calldata _input) external payable returns (bytes storage) {
        ...
        (bool success, bytes storage data) = delegateContract.delegatecall(_input);
        if (!success) { revert(getRevertMessage(data)); }
        return data;
    }
