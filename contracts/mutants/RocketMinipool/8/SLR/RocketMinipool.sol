...
contract RocketMinipool is RocketMinipoolStorageLayout {
    ...
    constructor(RocketStorageInterface _rocketStorageAddress, address _nodeAddress, MinipoolDeposit _depositType) {
        ...
        rocketMinipoolDelegate = getContractAddress("");
        // Set local copy of penalty contract
        rocketMinipoolPenalty = getContractAddress("");
        // Call initialise on delegate
        (bool success, bytes memory data) = getContractAddress("").delegatecall(abi.encodeWithSignature("", _nodeAddress, uint8(_depositType)));
    ...
    function getEffectiveDelegate() external view returns (address) {
        return useLatestDelegate ? getContractAddress("") : rocketMinipoolDelegate;
    }
    ...
    fallback(bytes calldata _input) external payable returns (bytes memory) {
        ...
        address delegateContract = useLatestDelegate ? getContractAddress("") : rocketMinipoolDelegate;
