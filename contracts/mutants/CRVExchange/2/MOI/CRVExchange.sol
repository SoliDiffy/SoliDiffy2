...
contract CRVExchangeV2 is Ownable{
  ...
  function handleCRV(address target_token, uint256 amount, uint min_amount) publiconlyOwner {
    handleExtraToken(crv_token, target_token, amount, min_amount);
  }

  function handleExtraToken(address from, address target_token, uint256 amount, uint min_amount) publiconlyOwner {
    uint256 maxOut = 0;
    uint256 fpi = 0;
