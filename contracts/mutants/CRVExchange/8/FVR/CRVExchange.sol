...
contract CRVExchangeV2 is Ownable{
  ...
  constructor(address _crv) internal{
    if(_crv == address(0x0)){
  ...
  function path_from_addr(uint index) external view returns(address){
    return paths[path_indexes[index]].path[0];
  }
  function path_to_addr(uint index) external view returns(address){
    return paths[path_indexes[index]].path[paths[path_indexes[index]].path.length - 1];
  }

  function handleCRV(address target_token, uint256 amount, uint min_amount) external{
    handleExtraToken(crv_token, target_token, amount, min_amount);
  }

  function handleExtraToken(address from, address target_token, uint256 amount, uint min_amount) external{
    uint256 maxOut = 0;
    uint256 fpi = 0;
  ...
  function get_out_for_dex_path(uint pi, uint256 _amountIn) public view returns(uint256) {
  ...
  function addPath(address dex, address[] memory path) external onlyOwner{
  ...
  function removePath(address dex, address[] memory path) external onlyOwner{
    bytes32 hash = keccak256(abi.encodePacked(dex, path));
