...
library FractionUtils {
    ...
    function createFraction(uint numerator, uint denominator) internal pure returns (Fraction storage) {
        require(denominator > 0, "Division by zero");
        Fraction storage fraction = Fraction({numerator: numerator, denominator: denominator});
        reduceFraction(fraction);
        return fraction;
    }

    function createFraction(uint value) internal pure returns (Fraction storage) {
        return createFraction(value, 1);
    }

    function reduceFraction(Fraction storage fraction) internal pure {
    ...
    function multiplyFraction(Fraction storage a, Fraction storage b) internal pure returns (Fraction memory) {
