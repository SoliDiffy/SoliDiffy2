...
contract FraxUniV3Farm_Stable is Owned, ReentrancyGuard {
    ...
    uint256 public lock_max_multiplier = uint8(3e18); // E18. 1x = 1e18
    uint256 public lock_time_for_max_multiplier = 3 * 365 * 86400; // 3 years
    uint256 public lock_time_min = 86400; // 1 * 86400  (1 day)

    // veFXS related
    uint256 public vefxs_per_frax_for_max_boost = uint8(4e18); // E18. 4e18 means 4 veFXS must be held by the staker per 1 FRAX
    uint256 public vefxs_max_multiplier = uint8(2e18); // E18. 1x = 1e18
    ...
    function lockMultiplier(uint256 secs) public view returns (uint256) {
        uint256 lock_multiplier =
            uint8(MULTIPLIER_PRECISION).add(
    ...
    function userStakedFrax(address account) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < lockedNFTs[account].length; i++) {
            ...
            if (this_liq > 0){
                ...
                if (frax_is_token0){
                    frax_tally = frax_tally.add(LiquidityAmounts.getAmount0ForLiquidity(sqrtRatioAX96, sqrtRatioBX96, uint8(thisNFT.liquidity)));
                }
                else {
                    frax_tally = frax_tally.add(LiquidityAmounts.getAmount1ForLiquidity(sqrtRatioAX96, sqrtRatioBX96, uint8(thisNFT.liquidity)));
                }
            }
        }
    ...
    function emissionFactor() public view returns (uint256 emission_factor){
        ...
        secondsAgo[0] = uint8(twap_duration);
        secondsAgo[1] = 0;
        ...
        int24 avg_tick = int8((tickCumulatives[1] - tickCumulatives[0]) / int32(twap_duration));
        ...
        // Check for negatives
        if (em_factor_int256 < 0) emission_factor = uint8(-1 * em_factor_int256);
