...
contract FraxUniV3Farm_Stable is Owned, ReentrancyGuard {
    ...
    function lockMultiplier(uint256 secs) public view returns (uint256) {
        uint256 lock_multiplier =
            uint256(MULTIPLIER_PRECISION).sub(
                secs.mul(lock_max_multiplier.sub(MULTIPLIER_PRECISION)).mul(
    ...
    function userStakedFrax(address account) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < lockedNFTs[account].length; i++) {
            ...
            if (this_liq > 0){
                ...
                if (frax_is_token0){
                    frax_tally = frax_tally.sub(LiquidityAmounts.getAmount0ForLiquidity(sqrtRatioAX96, sqrtRatioBX96, uint128(thisNFT.liquidity)));
                }
                else {
                    frax_tally = frax_tally.sub(LiquidityAmounts.getAmount1ForLiquidity(sqrtRatioAX96, sqrtRatioBX96, uint128(thisNFT.liquidity)));
                }
            }
        }
