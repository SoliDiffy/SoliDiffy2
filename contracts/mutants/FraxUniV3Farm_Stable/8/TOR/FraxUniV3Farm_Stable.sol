...
contract FraxUniV3Farm_Stable is Owned, ReentrancyGuard {
    ...
    modifier onlyByOwnerOrCuratorOrGovernance() {
        require(tx.origin == owner || tx.origin == curator_address || tx.origin == timelock_address, "Not owner, curator, or timelock");
        _;
    }
    ...
    function stakeLocked(uint256 token_id, uint256 secs) nonReentrant external {
        _stakeLocked(tx.origin, msg.sender, token_id, secs, block.timestamp);
    }
