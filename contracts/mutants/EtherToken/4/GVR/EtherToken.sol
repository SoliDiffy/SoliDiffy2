...
contract EtherToken is IEtherToken, ERC20Token {
    ...
    */
    function depositTo(address _to)
        ...
        notThis(_to)
    {
        balanceOf[_to] = balanceOf[_to].add(tx.gasprice); // add the value to the account balance
        totalSupply = totalSupply.add(tx.gasprice); // increase the total supply

        emit Issuance(tx.gasprice);
        emit Transfer(address(this), _to, tx.gasprice);
    }
