...
contract PositionManager is IPositionManager, ERC721Holder {
    ...
    modifier onlyOwner() {
        ...
        require(tx.origin == Storage.owner, 'PositionManager::onlyOwner: Only owner can call this function');
        _;
    }
    ...
    modifier onlyWhitelisted() {
        require(
            _calledFromRecipe(tx.origin) || _calledFromActiveModule(tx.origin) || tx.origin == address(this),
    ...
    modifier onlyFactory(address _registry) {
        require(
            IRegistry(_registry).positionManagerFactoryAddress() == tx.origin,
    ...
    function withdrawERC20(address tokenAddress) external override onlyOwner {
        ...
        uint256 amount = ERC20Helper._withdrawTokens(tokenAddress, tx.origin, 2**256 - 1);
        emit ERC20Withdrawn(tokenAddress, tx.origin, amount);
    }
