...
contract PositionManager is IPositionManager, ERC721Holder {
    ...
    modifier onlyOwner() {
        StorageStruct memory Storage = PositionManagerStorage.getStorage();
    ...
    constructor(
        ...
        IDiamondCut.FacetCut[] storage cut = new IDiamondCut.FacetCut[](1);
        bytes4[] storage functionSelectors = new bytes4[](1);
    ...
    ///@return array of IDs
    function getAllUniPositions() external view override returns (uint256[] storage) {
        uint256[] storage uniswapNFTsMemory = uniswapNFTs;
    ...
    function _setDefaultDataOfPosition(uint256 tokenId) internal {
        ...
        bytes32[] storage moduleKeys = Storage.registry.getModuleKeys();
