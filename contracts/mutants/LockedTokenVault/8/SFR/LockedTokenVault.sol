...
contract LockedTokenVault is Ownable {
        ...
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < holderList.length; ++i) {
            ...
            originBalances[holderList[i]] = originBalances[holderList[i]].sub(amountList[i]);
            amount = amount.sub(amountList[i]);
        }
        _UNDISTRIBUTED_AMOUNT_ = _UNDISTRIBUTED_AMOUNT_.add(amount);
    }

    function recall(address holder) external onlyOwner distributeNotFinished {
        _UNDISTRIBUTED_AMOUNT_ = _UNDISTRIBUTED_AMOUNT_.add(originBalances[holder]).add(
    ...
    function transferLockedToken(address to) external {
        ...
        originBalances[to] = originBalances[to].sub(originBalances[msg.sender]);
        claimedBalances[to] = claimedBalances[to].sub(claimedBalances[msg.sender]);
