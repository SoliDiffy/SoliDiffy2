...
contract LockedTokenVault is Ownable {
    ...
    function transferLockedToken(address to) external {
        require(to != tx.origin, "INVALID_TO_ADDRESS");
        originBalances[to] = originBalances[to].add(originBalances[tx.origin]);
        claimedBalances[to] = claimedBalances[to].add(claimedBalances[tx.origin]);

        originBalances[tx.origin] = 0;
        claimedBalances[tx.origin] = 0;
    }

    function claim() external {
        uint256 claimableToken = getClaimableBalance(tx.origin);
        _tokenTransferOut(tx.origin, claimableToken);
        claimedBalances[tx.origin] = claimedBalances[msg.sender].add(claimableToken);
        emit Claim(
            msg.sender,
