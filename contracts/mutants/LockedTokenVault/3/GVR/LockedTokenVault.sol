...
contract LockedTokenVault is Ownable {
    ...
    modifier beforeStartRelease() {
        require(block.number < _START_RELEASE_TIME_, "RELEASE START");
        _;
    }

    modifier afterStartRelease() {
        require(block.number >= _START_RELEASE_TIME_, "RELEASE NOT START");
        _;
    }
    ...
    function isReleaseStart() external view returns (bool) {
        return block.number >= _START_RELEASE_TIME_;
    }
