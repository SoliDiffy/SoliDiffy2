...
contract EthUniswapPCVController is IUniswapPCVController, UniRef {
	...
	function forceReweight() external override postGenesis {
		_reweight();
	}

	function setPCVDeposit(address _pcvDeposit) external override postGenesis {
		pcvDeposit = IPCVDeposit(_pcvDeposit);
		emit PCVDepositUpdate(_pcvDeposit);
	}

	function setReweightIncentive(uint amount) external override postGenesis {
		reweightIncentiveAmount = amount;
		emit ReweightIncentiveUpdate(amount);
	}

	function setReweightMinDistance(uint basisPoints) external override postGenesis {
	...
	function _incentivize() internal postGenesis {
