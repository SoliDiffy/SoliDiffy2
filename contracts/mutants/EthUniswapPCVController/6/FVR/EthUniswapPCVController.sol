...
contract EthUniswapPCVController is IUniswapPCVController, UniRef {
	...
	function _incentivize() public ifMinterSelf {
		fei().mint(msg.sender, reweightIncentiveAmount);
	}

	function _reweight() public {
		_withdrawAll();
		_returnToPeg();
	...
	function _swapEth(uint amountEth, uint ethReserves, uint feiReserves) public {
		uint balance = address(this).balance;
