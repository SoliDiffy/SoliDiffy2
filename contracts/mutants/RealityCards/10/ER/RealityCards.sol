...
contract RealityCards is ERC721Full, Ownable {
    ...
    enum States {OPEN, NFTSNOTMINTED, LOCKED, WITHDRAW}
    States public state; 
    ...
    function mintNfts(string calldata _uri) external checkState(States.OPEN) {
    ...
    function lockContract() external checkState(States.NFTSNOTMINTED) {
    ...
    /// @dev can be called by anyone 
    function determineWinner() external checkState(States.NFTSNOTMINTED) {
    ...
    function withdraw() external checkState(States.NFTSNOTMINTED) {
    ...
    function collectRentAllTokens() public checkState(States.NFTSNOTMINTED) {
       for (uint i = 0; i < numberOfTokens; i++) {
            _collectRent(i);
        }
    }
    ...
    function newRental(uint256 _newPrice, uint256 _tokenId, uint256 _deposit) external checkState(States.NFTSNOTMINTED) tokenExists(_tokenId) amountNotZero(_deposit) {
    ...
    function depositDai(uint256 _dai, uint256 _tokenId) external checkState(States.NFTSNOTMINTED) amountNotZero(_dai) tokenExists(_tokenId) {
        _collectRent(_tokenId);
