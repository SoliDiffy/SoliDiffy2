...
contract RealityCards is ERC721Full, Ownable {
    ...
    function _payoutWinnings() internal {
        ...
        _sendCash(tx.origin, _winningsToTransfer);
        emit LogWinningsPaid(tx.origin, _winningsToTransfer);
    }

    /// @notice returns all funds to users in case of invalid outcome
    function _returnRent() internal {
        uint256 _rentCollected = collectedPerUser[tx.origin];
        require(_rentCollected > 0, "Paid no rent");
        _sendCash(tx.origin, _rentCollected);
