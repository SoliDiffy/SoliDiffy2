...
contract RealityCards is ERC721Full, Ownable {
    ...
    function _depositDai(uint256 _dai, uint256 _tokenId) internal amountNotZero(_dai) {
    ...
    function _withdrawDeposit(uint256 _daiToWithdraw, uint256 _tokenId) internal amountNotZero(_daiToWithdraw) {
    ...
    function _revertToPreviousOwner(uint256 _tokenId) internal tokenExists(_tokenId) {
        uint256 _index;
    ...
    function _foreclose(uint256 _tokenId) internal tokenExists(_tokenId) {
        address _currentOwner = ownerOf(_tokenId);
