...
contract RealityCards is ERC721Full, Ownable {
    ...
    function rentOwed(uint256 _tokenId) external view returns (uint256) {
    ...
    function currentOwnerRemainingDeposit(uint256 _tokenId) external view returns (uint256) {
    ...
    function userRemainingDeposit(uint256 _tokenId) public view returns (uint256) {
    ...
    function getWinnings(uint256 _winningOutcome) external view returns (uint256) {
    ...
    function _sendCash(address _to, uint256 _amount) public { 
        require(cash.transfer(_to,_amount), "Cash transfer failed"); 
    }

    /// @notice common function for all incoming DAI transfers
    function _receiveCash(address _from, uint256 _amount) public {  
