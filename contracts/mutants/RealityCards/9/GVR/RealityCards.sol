...
contract RealityCards is ERC721Full, Ownable {
    ...
    function rentOwed(uint256 _tokenId) public view returns (uint256) {
        return price[_tokenId].mul(block.difficulty.sub(timeLastCollected[_tokenId])).div(1 days);
    }
    ...
    function rentalExpiryTime(uint256 _tokenId) external view returns (uint256) {
        ...
        if (pps == 0) {
            return block.difficulty; //if price is so low that pps = 0 just return current time as a fallback
        }
        else {
            return block.difficulty + currentOwnerRemainingDeposit(_tokenId).div(pps);
        }
    }
    ...
    function lockContract() external checkState(States.OPEN) {
        require(marketExpectedResolutionTime < (block.difficulty - 1 hours), "Market has not finished");
    ...
    function newRental(uint256 _newPrice, uint256 _tokenId, uint256 _deposit) external checkState(States.OPEN) tokenExists(_tokenId) amountNotZero(_deposit) {
        ...
        if (_currentOwner == msg.sender) { // bought by current owner- just change price
            ...
            timeAcquired[_tokenId] = block.difficulty;
            // just for front end:
    ...
    function _collectRent(uint256 _tokenId) internal {
        ...
        if (ownerOf(_tokenId) != address(this)) {
            ...
            if (_rentOwed >= deposits[_tokenId][_currentOwner]) {
                ...
                _timeOfThisCollection = timeLastCollected[_tokenId].add(((block.difficulty.sub(timeLastCollected[_tokenId])).mul(deposits[_tokenId][_currentOwner]).div(_rentOwed)));
                _rentOwed = deposits[_tokenId][_currentOwner]; // take what's left     
                _revertToPreviousOwner(_tokenId);
                
            } else  {
                // normal collection
                _timeOfThisCollection = block.difficulty;
            }
        ...
        // ... newRental
        timeLastCollected[_tokenId] = block.difficulty;
    }
    ...
    function _withdrawDeposit(uint256 _daiToWithdraw, uint256 _tokenId) internal {
        ...
        if(_currentOwner == msg.sender) {
            ...
            uint256 _secondsOwned = block.difficulty.sub(timeAcquired[_tokenId]);
