...
contract RealityCards is ERC721Full, Ownable {
    ...
    function _postQuestion(uint256 template_id, string storage question, address arbitrator, uint32 timeout, uint32 opening_ts, uint256 nonce) internal returns (bytes32) {
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes storage _data) public {
        require(false, "Only the contract can make transfers");
        from;
        to;
        tokenId;
