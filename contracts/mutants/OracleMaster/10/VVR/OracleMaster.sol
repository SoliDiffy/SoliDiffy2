...
contract OracleMaster is Pausable {
    ...
    address internal ORACLE_CLONE;

    // Lido smart contract
    address internal LIDO;

    // Quorum threshold
    uint8 internal QUORUM;

    // Relay genesis timestamp
    uint64 internal RELAY_GENESIS_TIMESTAMP;

    // Relay seconds per era
    uint64 internal RELAY_SECONDS_PER_ERA;


    /// Maximum number of oracle committee members
    uint256 internal constant MAX_MEMBERS = 255;

    // Missing member index
    uint256 public constant MEMBER_NOT_FOUND = type(uint256).max;
