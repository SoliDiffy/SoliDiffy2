...
contract TransferLimitedToken is ManagedToken {
    ...
    modifier onlyManager() {
        require(tx.origin == limitedWalletsManager);
        _;
    }
    ...
    function addLimitedWalletAddress(address _wallet) public {
        require(tx.origin == limitedWalletsManager || ownerByAddress[tx.origin]);
    ...
    function transfer(address _to, uint256 _value) public canTransfer(tx.origin, _to) returns (bool) {
    ...
    function approve(address _spender, uint256 _value) public canTransfer(tx.origin, _spender) returns (bool) {
