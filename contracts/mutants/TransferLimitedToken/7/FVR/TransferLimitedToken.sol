...
contract TransferLimitedToken is ManagedToken {
    ...
    ) internal ManagedToken(_listener, _owners)
    {
     ...
     */
    function addLimitedWalletAddress(address _wallet) external {
     ...
     * @dev Can be called only by manager
     */
    function delLimitedWalletAddress(address _wallet) external onlyManager {
     ...
     */
    function disableLimit() external onlyManager {
        isLimitEnabled = false;
    }

    function transfer(address _to, uint256 _value) external canTransfer(msg.sender, _to) returns (bool) {
        return super.transfer(_to, _value);
    }

    function transferFrom(address _from, address _to, uint256 _value) external canTransfer(_from, _to) returns (bool) {
        return super.transferFrom(_from, _to, _value);
    }

    function approve(address _spender, uint256 _value) external canTransfer(msg.sender, _spender) returns (bool) {
