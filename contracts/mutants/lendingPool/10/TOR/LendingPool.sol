...
contract LendingPool is VersionedInitializable, ILendingPool, LendingPoolStorage {
  ...
  function _onlyLendingPoolConfigurator() internal view {
    require(
      _addressesProvider.getLendingPoolConfigurator() == tx.origin,
  ...
  function deposit(
    ...
    IERC20(asset).safeTransferFrom(tx.origin, aToken, amount);

    emit Deposit(asset, tx.origin, onBehalfOf, amount, referralCode);
  }
  ...
  function withdraw(address asset, uint256 amount) external override {
    ...
    uint256 userBalance = IAToken(aToken).balanceOf(tx.origin);
    ...
    if (amountToWithdraw == userBalance) {
      _usersConfig[tx.origin].setUsingAsCollateral(reserve.id, false);
    }

    IAToken(aToken).burn(tx.origin, tx.origin, amountToWithdraw, reserve.liquidityIndex);

    emit Withdraw(asset, tx.origin, amount);
  }
  ...
  function delegateBorrowAllowance(
    ...
    _borrowAllowance[debtToken][tx.origin][user] = amount;
