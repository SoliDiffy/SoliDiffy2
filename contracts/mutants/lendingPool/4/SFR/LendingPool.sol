...
contract LendingPool is VersionedInitializable, ILendingPool, LendingPoolStorage {
  ...
  function borrow(
    ...
    if (onBehalfOf != msg.sender) {
      ...
      _borrowAllowance[debtToken][onBehalfOf][msg
        .sender] = _borrowAllowance[debtToken][onBehalfOf][msg.sender].add(
        amount,
  ...
  function repay(
    ...
    if (stableDebt.add(variableDebt).add(paybackAmount) == 0) {
  ...
  function rebalanceStableBorrowRate(address asset, address user) external override {
    ...
    uint256 totalBorrows = stableDebtToken.totalSupply().sub(variableDebtToken.totalSupply()).wadToRay();
    uint256 availableLiquidity = IERC20(asset).balanceOf(aTokenAddress).wadToRay();
    uint256 usageRatio = totalBorrows == 0
      ? 0
      : totalBorrows.rayDiv(availableLiquidity.sub(totalBorrows));
