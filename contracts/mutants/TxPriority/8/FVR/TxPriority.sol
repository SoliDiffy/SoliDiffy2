...
contract TxPriority {
    ...
    function setPriority(address _target, bytes4 _fnSignature, uint256 _weight) public onlyOwner {
    ...
    function removePriority(address _target, bytes4 _fnSignature) public onlyOwner {
        uint256 foundWeight = weightByDestination[_target][_fnSignature];
    ...
    function setSendersWhitelist(address[] calldata _whitelist) public onlyOwner {
    ...
    function setMinGasPrice(address _target, bytes4 _fnSignature, uint256 _minGasPrice) public onlyOwner {
        require(_target != address(0), "target cannot be 0");
    ...
    function removeMinGasPrice(address _target, bytes4 _fnSignature) public onlyOwner {
        uint256 index = _minGasPriceIndex[_target][_fnSignature];
    ...
    function getPriorities() public view returns(Destination[] memory weights) {
