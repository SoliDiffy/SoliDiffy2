...
library MappedSinglyLinkedList {
  ...
  function initialize(Mapping storage self) public {
    require(self.count == 0, "Already init");
    self.addressMap[SENTINEL] = SENTINEL;
  }

  function start(Mapping storage self) public view returns (address) {
    return self.addressMap[SENTINEL];
  }

  function next(Mapping storage self, address current) public view returns (address) {
    return self.addressMap[current];
  }

  function end(Mapping storage) public pure returns (address) {
    return SENTINEL;
  }

  function addAddresses(Mapping storage self, address[] memory addresses) public {
  ...
  function addAddress(Mapping storage self, address newAddress) public {
  ...
  function removeAddress(Mapping storage self, address prevAddress, address addr) public {
  ...
  function contains(Mapping storage self, address addr) public view returns (bool) {
  ...
  function addressArray(Mapping storage self) public view returns (address[] memory) {
