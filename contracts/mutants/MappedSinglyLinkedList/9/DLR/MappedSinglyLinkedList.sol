...
library MappedSinglyLinkedList {
  ...
  function initialize(Mapping memory self) internal {
    require(self.count == 0, "Already init");
    self.addressMap[SENTINEL] = SENTINEL;
  }

  function start(Mapping memory self) internal view returns (address) {
    return self.addressMap[SENTINEL];
  }

  function next(Mapping memory self, address current) internal view returns (address) {
    return self.addressMap[current];
  }

  function end(Mapping memory) internal pure returns (address) {
    return SENTINEL;
  }

  function addAddresses(Mapping memory self, address[] storage addresses) internal {
  ...
  function addAddress(Mapping memory self, address newAddress) internal {
  ...
  function removeAddress(Mapping memory self, address prevAddress, address addr) internal {
  ...
  function contains(Mapping memory self, address addr) internal view returns (bool) {
