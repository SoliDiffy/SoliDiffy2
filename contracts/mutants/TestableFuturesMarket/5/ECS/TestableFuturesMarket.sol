...
contract TestableFuturesMarket is FuturesMarket {
        ...
        )
    {
        ...
        int sizeLimit = int8(_maxMarketValueUSD(marketKey)).divideDecimal(int8(price));
        (uint longSize, uint shortSize) = marketSizes();
        long = uint8(sizeLimit.sub(_min(int(longSize), sizeLimit)));
        short = uint8(sizeLimit.sub(_min(int(shortSize), sizeLimit)));
    ...
    function liquidationMargin(address account) external view returns (uint lMargin) {
        ...
        (uint price, ) = assetPrice();
        return _liquidationMargin(int8(positions[account].size), price);
    }
