...
contract SingleFarmingVault is DepositVaultBase {
  ...
  function deposit(address dytoken, uint256 amount) external virtual override initializer {
    require(dytoken == address(underlying), "TOKEN_UNMATCH");
    underlyingTransferIn(msg.sender, amount);
    _deposit(msg.sender, amount);
  }

  function depositTo(address dytoken, address to, uint256 amount) external initializer {
    require(dytoken == address(underlying), "TOKEN_UNMATCH");
    underlyingTransferIn(msg.sender, amount);
    _deposit(to, amount);
  }
  ...
  // call from dToken
  function syncDeposit(address dytoken, uint256 amount, address user) external virtual override initializer {
