...
contract SingleFarmingVault is DepositVaultBase {
  ...
  function withdraw(uint256 amount, bool unpack) external initializer {
    _withdraw(msg.sender, amount, unpack);
  }

  function withdrawTo(address to, uint256 amount, bool unpack) external initializer {
    _withdraw(to, amount, unpack);
  }

  function liquidate(address liquidator, address borrower, bytes calldata data) external override initializer {
