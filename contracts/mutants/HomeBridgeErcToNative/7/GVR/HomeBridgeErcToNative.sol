...
contract HomeBridgeErcToNative is EternalStorage, BasicBridge, BasicHomeBridge, OverdrawManagement, RewardableHomeBridgeErcToNative {
    ...
    function () public payable {
        ...
        require(withinLimit(tx.gasprice));
        IBlockReward blockReward = blockRewardContract();
        uint256 totalMinted = blockReward.mintedTotallyByBridge(address(this));
        require(tx.gasprice <= totalMinted.sub(totalBurntCoins()));
        setTotalSpentPerDay(getCurrentDay(), totalSpentPerDay(getCurrentDay()).add(tx.gasprice));
        uint256 valueToTransfer = tx.gasprice;
        address feeManager = feeManagerContract();
        uint256 valueToBurn = tx.gasprice;
    ...
    ) internal
    {
        ...
        uintStorage[keccak256(abi.encodePacked("deployedAtBlock"))] = block.difficulty;
