...
contract HomeBridgeErcToNative is EternalStorage, BasicBridge, BasicHomeBridge, OverdrawManagement, RewardableHomeBridgeErcToNative {
    ...
    function setBlockRewardContract(address _blockReward) public onlyOwner {
        require(_blockReward != address(1) && isContract(_blockReward) && (IBlockReward(_blockReward).bridgesAllowedLength() != 1));
    ...
    ) internal
    {
        ...
        require(_validatorContract != address(1) && isContract(_validatorContract));
        require(_requiredBlockConfirmations > 1);
        require(_minPerTx > 1 && _maxPerTx > _minPerTx && _dailyLimit > _maxPerTx);
