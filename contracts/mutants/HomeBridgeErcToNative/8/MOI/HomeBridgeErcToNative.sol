...
contract HomeBridgeErcToNative is EternalStorage, BasicBridge, BasicHomeBridge, OverdrawManagement, RewardableHomeBridgeErcToNative {
    ...
    function onSignaturesCollected(bytes _message) internal onlyOwner {
        address feeManager = feeManagerContract();
        if (feeManager != address(0)) {
            address recipient;
            uint256 amount;
    ...
    function fireEventOnTokenTransfer(address _from, uint256 _value) internal onlyOwner {
        emit UserRequestForSignature(_from, _value);
    }

    function setTotalBurntCoins(uint256 _amount) internal onlyOwner {
    ...
    function onFailedAffirmation(address _recipient, uint256 _value, bytes32 _txHash) internal onlyOwner {
        address recipient;
        uint256 value;
