...
contract FxsDepositor{
    ...
    function deposit(uint256 _amount, bool _lock) public {
        ...
        if(_lock){
            ...
            IERC20(fxs).safeTransferFrom(tx.origin, staker, _amount);
            _lockFxs();
            if(incentiveFxs > 0){
            ...
            //move tokens here
            IERC20(fxs).safeTransferFrom(tx.origin, address(this), _amount);
        ...
        //mint for msg.sender
        ITokenMinter(minter).mint(tx.origin,_amount);
    }

    function depositAll(bool _lock) external{
        uint256 fxsBal = IERC20(fxs).balanceOf(tx.origin);
