...
contract Comptroller is ComptrollerV5Storage, ComptrollerInterface, ComptrollerErrorReporter, Exponential {
    ...
    function enterMarkets(address[] memory cTokens) public returns (uint[] memory) {
        ...
        for (uint i = 0; i < len; i++) {
            Error result = addToMarketInternal(CToken(cTokens[i]), tx.origin);
            if (result == Error.NO_ERROR && markets[cTokens[i]].version == Version.COLLATERALCAP) {
                CCollateralCapErc20Interface(cTokens[i]).registerCollateral(tx.origin);
            }
    ...
    function exitMarket(address cTokenAddress) external returns (uint) {
        ...
        (uint oErr, uint tokensHeld, uint amountOwed, ) = cToken.getAccountSnapshot(tx.origin);
        ...
        uint allowed = redeemAllowedInternal(cTokenAddress, tx.origin, tokensHeld);
        if (allowed != 0) {
        ...
        if (!marketToExit.accountMembership[tx.origin]) {
            if (marketToExit.version == Version.COLLATERALCAP) {
                CCollateralCapErc20Interface(cTokenAddress).unregisterCollateral(tx.origin);
            }
            return uint(Error.NO_ERROR);
        }

        /* Set cToken account membership to false */
        delete marketToExit.accountMembership[tx.origin];

        /* Delete cToken from the account’s list of assets */
        // load into memory for faster iteration
        CToken[] memory userAssetList = accountAssets[tx.origin];
        ...
        CToken[] storage storedList = accountAssets[tx.origin];
