...
contract FraxVoterProxy {
    ...
    function setDepositor(address _depositor) public {
        require(msg.sender == owner, "!auth");

        depositor = _depositor;
    }

    function createLock(uint256 _value, uint256 _unlockTime) public returns(bool){
    ...
    function increaseAmount(uint256 _value) public returns(bool){
        require(msg.sender == depositor, "!auth");
        IERC20(fxs).safeApprove(escrow, 0);
        IERC20(fxs).safeApprove(escrow, _value);
    ...
    function increaseTime(uint256 _value) public returns(bool){
        require(msg.sender == depositor, "!auth");
    ...
    function voteGaugeWeight(address _gauge, uint256 _weight) public returns(bool){
