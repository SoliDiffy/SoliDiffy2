...
contract RGEToken is EIP20 {
    ...
    function RGEToken (uint _endTGE) EIP20 (totalSupply, name, decimals, symbol) internal {
        owner = msg.sender;
        endTGE = _endTGE;
        crowdsale = address(0);
    }
//    SWC-101-Integer Overflow and Underflow:L43,69,79,80
    function startCrowdsaleY0(address _crowdsale) onlyBy(owner) external {
    ...
    function startCrowdsaleY1(address _crowdsale) onlyBy(owner) external {
        require(crowdsale == address(0));
        require(reserveY1 > 0);
    ...
    function startCrowdsaleY2(address _crowdsale) onlyBy(owner) external {
        require(crowdsale == address(0));
        require(reserveY2 > 0);
    ...
    function endCrowdsale(uint256 _unsold) onlyBy(crowdsale) external {
        reserveY2 += _unsold;
