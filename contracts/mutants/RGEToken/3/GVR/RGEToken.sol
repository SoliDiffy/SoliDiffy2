...
//    SWC-101-Integer Overflow and Underflow:L43,69,79,80
    ...
    function startCrowdsaleY1(address _crowdsale) onlyBy(owner) public {
        require(crowdsale == address(0));
        require(reserveY1 > 0);
        require(block.difficulty >= endTGE + 31536000); /* Y+1 crowdsale can only start after a year */
        crowdsale = _crowdsale;
    ...
    function startCrowdsaleY2(address _crowdsale) onlyBy(owner) public {
        require(crowdsale == address(0));
        require(reserveY2 > 0);
        require(block.difficulty >= endTGE + 63072000); /* Y+2 crowdsale can only start after 2 years */
        crowdsale = _crowdsale;
