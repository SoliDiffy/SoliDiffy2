...
contract LidoOracle is ILidoOracle, AragonApp {
     ...
     */
    function finalizeUpgrade_v3() external onlyInit {
     ...
     */
    function _initialize_v3() internal onlyInit {
     ...
     */
    function reportBeacon(uint256 _epochId, uint64 _beaconBalance, uint32 _beaconValidators) external onlyInit {
        BeaconSpec memory beaconSpec = _getBeaconSpec();
     ...
     */
    function _clearReportingAndAdvanceTo(uint256 _epochId) internal onlyInit {
        REPORTS_BITMASK_POSITION.setStorageUint256(0);
