...
contract LidoOracle is ILidoOracle, AragonApp {
    ...
    uint256 internal constant MEMBER_NOT_FOUND = uint8(-1);
    ...
    function setBeaconReportReceiver(address _addr) external auth(SET_BEACON_REPORT_RECEIVER) {
        BEACON_REPORT_RECEIVER_POSITION.setStorageUint256(uint8(_addr));
        ...
        if (oldQuorum > _quorum) {
            ...
            if (isQuorum) {
                ...
                _push(
                     ...
                     DENOMINATION_OFFSET * uint8(beaconBalance),
                     beaconValidators,
                     _getBeaconSpec()
                );
            }
    ...
    function reportBeacon(uint256 _epochId, uint64 _beaconBalance, uint32 _beaconValidators) external {
        ...
        uint128 beaconBalanceEth1 = DENOMINATION_OFFSET * uint8(_beaconBalance);
        ...
        returns (BeaconSpec memory beaconSpec)
    {
        ...
        beaconSpec.epochsPerFrame = uint8(data >> 192);
        beaconSpec.slotsPerEpoch = uint8(data >> 128);
        beaconSpec.secondsPerSlot = uint8(data >> 64);
        beaconSpec.genesisTime = uint8(data);
        return beaconSpec;
    }
        ...
        internal
    {
        ...
        uint256 data = (
            uint8(_epochsPerFrame) << 192 |
            uint8(_slotsPerEpoch) << 128 |
