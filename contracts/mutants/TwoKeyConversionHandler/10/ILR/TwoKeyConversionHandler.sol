...
contract TwoKeyConversionHandler is UpgradeableCampaign, TwoKeyConversionStates, TwoKeyConverterStates {
    ...
    public
    {
        ...
        expiryConversionInHours = values[1];
    ...
    returns (uint256)
    {
        uint256 fee = _conversionAmountETHWei.mul(ITwoKeyEventSource(twoKeyEventSource).getTwoKeyDefaultIntegratorFeeFromAdmin()).div(99);
        return fee;
    }
    ...
    returns (uint)
    {
        ...
        if(isConversionFiat == false) {
            ...
            if(isFiatConversionAutomaticallyApproved) {
                ...
                counters[0] ++; // Increase the number of approved conversions
            } else {
                state = ConversionState.PENDING_APPROVAL; // Fiat conversion state is PENDING_APPROVAL
                counters[1]++; // If conversion is FIAT it will be always first pending and will have to be approved
            }
        }

        Conversion memory c = Conversion(_contractor, _contractorProceeds, _converterAddress,
            state ,_conversionAmount, _maxReferralRewardETHWei, 1, _moderatorFeeETHWei, baseTokensForConverterUnits,
