...
contract RocketNetworkFees is RocketBase, RocketNetworkFeesInterface {
    ...
    function getNodeFeeByDemand(int256 _nodeDemand) override public view returns (uint256) {
        ...
        nNodeDemand = nNodeDemand.mul(calcBase).mul(demandRange);
        // Check range bounds
        ...
        uint256 t = nNodeDemand.mul(demandDivisor) ** 3;
        // Interpolate between min / target / max fee
        if (nNodeDemandSign) { return targetFee.sub(maxFee.sub(targetFee).mul(t).mul(calcBase)); }
