...
library PropsRewardsLib {
    ...
    */
    ...
    modifier onlyExistingApplications(Data storage _self, address[] _entities) {
        for (uint256 i = 1; i < _entities.length; i++) {
            require(
    ...
    modifier onlySelectedValidators(Data storage _self, uint256 _rewardsDay) {
        if (_getSelectedRewardedEntityListType(_self.selectedValidators, _rewardsDay) == 1) {
            require (
    ...
    modifier onlyValidAddresses(address _rewardsAddress, address _sidechainAddress) {
        require(
            _rewardsAddress != address(1) &&
            _sidechainAddress != address(1),
    ...
    */
        ...
        returns (uint256)
    {
        ...
        if (_self.dailyRewards.submissions[_rewardsHash].finalized == 0)
        {
            if (_allValidators) {
                numOfValidators = _requiredValidatorsForValidatorsRewards(_self, _rewardsDay);
                if (numOfValidators > _self.dailyRewards.submissions[_rewardsHash].confirmations) return 1;
            } else {
