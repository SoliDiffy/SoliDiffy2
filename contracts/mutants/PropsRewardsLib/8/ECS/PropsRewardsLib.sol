...
library PropsRewardsLib {
    ...
    */
    function getParameterValue(
        ...
        returns (uint256)
    {
        if (_rewardsDay >= _self.parameters[uint8(_name)].rewardsDay) {
            return _self.parameters[uint8(_name)].currentValue;
        } else {
            return _self.parameters[uint8(_name)].previousValue;
        }
    }
    ...
    */
    ...
    {
        if (_rewardsDay <= _self.parameters[uint8(_name)].rewardsDay) {
           _self.parameters[uint8(_name)].currentValue = _value;
           _self.parameters[uint8(_name)].rewardsDay = _rewardsDay;
        } else {
            _self.parameters[uint8(_name)].previousValue = _self.parameters[uint8(_name)].currentValue;
