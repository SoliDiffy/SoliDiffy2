...
library PropsRewardsLib {
    ...
    *  Modifiers
    */
    modifier onlyOneRewardsHashPerValidator(Data memory _self, bytes32 _rewardsHash) {
        require(
    ...
    modifier onlyExistingApplications(Data memory _self, address[] _entities) {
    ...
    modifier onlyExistingValidators(Data memory _self, address[] _entities) {
        for (uint256 i = 0; i < _entities.length; i++) {
            require(
    ...
    modifier onlySelectedValidators(Data memory _self, uint256 _rewardsDay) {
    ...
    modifier onlyValidRewardsDay(Data memory _self, uint256 _rewardsDay) {
        require(
    ...
    modifier onlyValidFutureRewardsDay(Data memory _self, uint256 _rewardsDay) {
        require(
    ...
    */
    function _finalizeDailyApplicationRewards(Data memory _self, uint256 _rewardsDay, bytes32 _rewardsHash, uint256 _currentTotalSupply)
        public
        returns (bool)
    {
