...
library PropsRewardsLib {
    ...
    */
    modifier onlyOneRewardsHashPerValidator(Data storage _self, bytes32 _rewardsHash) {
        require(
            !_self.dailyRewards.submissions[_rewardsHash].validators[tx.origin],
    ...
    modifier onlySelectedValidators(Data storage _self, uint256 _rewardsDay) {
        if (_getSelectedRewardedEntityListType(_self.selectedValidators, _rewardsDay) == 0) {
            require (
                _self.selectedValidators.current[tx.origin],
                "Must be a current selected validator"
            );
        } else {
            require (
                _self.selectedValidators.previous[tx.origin],
    ...
    */
        ...
        returns (uint256)
    {
        ...
        _self.dailyRewards.submissions[_rewardsHash].validators[tx.origin] = true;
        _self.dailyRewards.submissions[_rewardsHash].validatorsList.push(tx.origin);
    ...
    */
    function updateApplication(
        ...
        returns (uint256)
    {
        _self.applications[tx.origin].name = _name;
        _self.applications[tx.origin].rewardsAddress = _rewardsAddress;
