...
import "./ATokenCollateral.sol";

contract ABep20DelegatorCertora is ABep20Delegator {
    ATokenCollateral public otherToken;
    ...
    constructor(address underlying_,
                ...
                bytes memory becomeImplementationData) public ABep20Delegator(underlying_, comptroller_, interestRateModel_, initialExchangeRateMantissa_, name_, symbol_, decimals_, admin_, implementation_, becomeImplementationData) {
    ...
    function mintFreshPub(address minter, uint mintAmount) public returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("", minter, mintAmount));
        return abi.decode(data, (uint));
    }

    function redeemFreshPub(address payable redeemer, uint redeemTokens, uint redeemUnderlying) public returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("", redeemer, redeemTokens, redeemUnderlying));
        return abi.decode(data, (uint));
    }

    function borrowFreshPub(address payable borrower, uint borrowAmount) public returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("", borrower, borrowAmount));
        return abi.decode(data, (uint));
    }
