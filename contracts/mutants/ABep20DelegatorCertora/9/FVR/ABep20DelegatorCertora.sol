...
contract ABep20DelegatorCertora is ABep20Delegator {
    ...
    constructor(address underlying_,
                ...
                bytes memory becomeImplementationData) internal ABep20Delegator(underlying_, comptroller_, interestRateModel_, initialExchangeRateMantissa_, name_, symbol_, decimals_, admin_, implementation_, becomeImplementationData) {
    ...
    function totalSupplyInOther() external view returns (uint) {
        return otherToken.totalSupply();
    }

    function totalBorrowsInOther() external view returns (uint) {
