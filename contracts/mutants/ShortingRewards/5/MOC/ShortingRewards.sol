...
contract ShortingRewards is IShortingRewards, RewardsDistributionRecipient, ReentrancyGuard, Pausable, MixinResolver {
    ...
    ) public MixinResolver(_resolver) Owned(_owner) {
    ...
    function enrol(address account, uint256 amount) external onlyShortContract nonReentrant updateReward(account) notPaused {
    ...
    function withdraw(address account, uint256 amount) external onlyShortContract updateReward(account) nonReentrant {
    ...
    function getReward(address account) external updateReward(account) nonReentrant {
    ...
    function notifyRewardAmount(uint256 reward) external updateReward(address(0)) onlyRewardsDistribution {
