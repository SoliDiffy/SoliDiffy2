...
contract Lock is Ownable, Pausable, ReentrancyGuard  {
    ...
    /* ========== PUBLIC FUNCTIONS ========== */
    function lock(address _account, uint256 _amount, uint256 _unlockAmount) public {
        if (startLock == 0) {
    ...
    function pending(address _account) external view returns(uint256 _pending) {
    ...
    function unlock(address _to) public whenNotPaused nonReentrant {
    ...
    function emergencyWithdraw(address _to) public whenPaused {
        LockedData storage _lockedData = data[msg.sender];
    ...
    function setStartLock(uint256 _value) public onlyOwner {
        startLock = _value;
        emit StartLockChanged(_value);
    }
    function setUnlockDuration(uint256 _newValue) public onlyOwner {
        unlockDuration = _newValue;
        emit UnlockDurationChanged(_newValue);
    }

    function setLockedTime(uint256 _newValue) public onlyOwner {
