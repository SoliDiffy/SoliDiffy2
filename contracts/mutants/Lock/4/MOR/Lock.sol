...
contract Lock is Ownable, Pausable, ReentrancyGuard  {
    ...
    function unlock(address _to) external whenPaused nonReentrant {
    ...
    function emergencyWithdraw(address _to) external whenNotPaused {
        LockedData storage _lockedData = data[msg.sender];
    ...
    function setStartLock(uint256 _value) external whenNotPaused {
        startLock = _value;
        emit StartLockChanged(_value);
    }
    function setUnlockDuration(uint256 _newValue) external whenNotPaused {
