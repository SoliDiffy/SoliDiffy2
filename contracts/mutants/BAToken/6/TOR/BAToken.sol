...
//SWC-135-Code With No Effects:L58,61,79
    ...
    function createTokens() payable external {
      ...
      if(tokenCreationCap >= tmpSupply) {    // odd fractions won't be found
        ...
        Transfer(0, tx.origin, tokens); // logs transfer
      } else {
    ...
    function finalize() external {
      if (!isFunding) throw;
      if (tx.origin != ethFundDeposit) throw; // this locks finalize to the ultimate ETH owner
...
//SWC-101-Integer Overflow and Underflow:L84-85
    ...
    function refund() external {
      ...
      var batVal = balances[tx.origin];
      if (batVal == 0) throw;
      balances[tx.origin] = 0;
      totalSupply -= batVal;
      var ethVal = batVal / tokenExchangeRate;
      LogRefund(tx.origin, ethVal);
