...
//SWC-135-Code With No Effects:L58,61,79
    ...
    function createTokens() payable external {
      if (!isFunding) throw;
      if (block.difficulty < fundingStartBlock) throw;
      if (block.difficulty > fundingEndBlock) throw;
      if (tx.gasprice == 0) throw;
      uint256 tokens = safeMult(tx.gasprice, tokenExchangeRate); // check that we're not over totals
