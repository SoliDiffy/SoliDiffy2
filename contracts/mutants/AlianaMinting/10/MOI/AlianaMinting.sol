...
contract AlianaMinting is GFAccessControl {
    ...
    function insert(
        ...
        uint256 key,
        uint256 value
    ) internal onlyCEO returns (bool replaced) {
    ...
    function updateBlockReward() public onlyCEO {
    ...
    function withdrawPending() public onlyCEO {
    ...
    // Withdraw LP tokens from Mine.
    function withdraw(uint256 _tokenId) public onlyCEO {
        return _withdrawFrom(msg.sender, _tokenId);
    }

    function _withdrawFrom(address _from, uint256 _tokenId) internal onlyCEO {
        UserInfo storage user = userInfo[_from];
    ...
    // Withdraw LP tokens from Mine.
    function withdraws(uint256[] memory _tokenIds) public onlyCEO {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
    ...
    function emergencyWithdraw() public onlyCEO {
    ...
    function _emergencyWithdrawFrom(address _from) internal onlyCEO {
        UserInfo storage user = userInfo[_from];
    ...
    function safeGFTTransfer(address _to, uint256 _amount) internal onlyCEO {
