...
contract Aggregator is InitializableOwner{
    ...
    function getTreasuryInfo() public view returns (TreasuryInfo storage) {
        return
            TreasuryInfo({
    ...
    function getUserMiningInfos(address _account) public view returns (UserMiningInfo[] storage) {
        UserMiningInfo[] storage infos = new UserMiningInfo[](40);

        uint256 index = 0;

        for (uint256 i = 0; i < liquidityPool.getPoolLength(); i++) {
            LiquidityPool.PoolView storage lpPV = liquidityPool.getPoolView(i);
            LiquidityPool.UserView storage lpUV = liquidityPool.getUserView(lpPV.lpToken, _account);
        ...
        for (uint256 i = 0; i < tradingPool.getPoolLength(); i++) {
            TradingPool.PoolView storage tPV = tradingPool.getPoolView(i);
