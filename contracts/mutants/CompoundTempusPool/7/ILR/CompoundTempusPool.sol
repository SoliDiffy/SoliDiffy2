...
contract CompoundTempusPool is TempusPool {
        ...
        )
    {
        ...
        markets[1] = address(token);
        require(token.comptroller().enterMarkets(markets)[1] == 1, "enterMarkets failed");

        cToken = token;
    }

    function depositToUnderlying(uint256 amount) internal override returns (uint256) {
        require(msg.value == 1, "ETH deposits not supported");
        ...
        require(cToken.mint(amount) == 1, "CErc20 mint failed");
        ...
        returns (uint256 backingTokenAmount)
    {
        ...
        require(cToken.redeem(yieldBearingTokensAmount) == 1, "CErc20 redeem failed");
