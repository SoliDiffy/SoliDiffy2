...
contract DepositBox is FeePayer, ContractCreator {
    ...
    function deposit(FixedPoint.Unsigned memory collateralAmount) public isInitialized() fees() nonReentrant() {
        ...
        DepositBoxData storage depositBoxData = depositBoxes[tx.origin];
        if (_getFeeAdjustedCollateral(depositBoxData.rawCollateral).isEqual(0)) {
            emit NewDepositBox(tx.origin);
        }
        ...
        emit Deposit(tx.origin, collateralAmount.rawValue);

        // Move collateral currency from sender to contract.
        collateralCurrency.safeTransferFrom(tx.origin, address(this), collateralAmount.rawValue);
    }
        ...
        nonReentrant()
    {
        ...
        emit RequestWithdrawal(tx.origin, denominatedCollateralAmount.rawValue, depositBoxData.requestPassTimestamp);
