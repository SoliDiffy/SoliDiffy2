...
contract AUsdc2A3CrvTokenExchange is ITokenExchange {
    ...
    function exchange(
        ...
        if (from == aUsdcToken && to == a3CrvToken) {
            ...
            if (amount == 0) {
                if (balance > 0) {
                    aUsdcToken.send(spender, balance);
                }
                return;
            }
            ...
            aUsdcToken.send(address(curveConnector), amount);
            ...
            if (unusedBalance > 0) {
                aUsdcToken.send(spender, unusedBalance);
            }
        } else {
            ...
            if (aUsdcAmount == 0) {
                a3CrvToken.send(spender, a3CrvBalance);
                return;
            }

            a3CrvToken.send(address(curveConnector), amount);
            ...
            if (unusedA3CrvBalance > 0) {
                a3CrvToken.send(spender, unusedA3CrvBalance);
            }
