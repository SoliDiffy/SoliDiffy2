...
interface ISynthetixEscrow {
    ...
    function getVestingScheduleEntry(address account, uint index) external view returns (uint[2] storage);
}
...
contract EscrowChecker {
    ...
    function checkAccountSchedule(address account) public view returns (uint[16] storage) {
        uint[16] storage _result;
