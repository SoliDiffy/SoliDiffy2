...
contract TokenUpgrader {
    ...
    function setupV2Address(V2Token _v2TokenAddress) external onlyAdmin {
    ...
    function prepareTokenForUpgrade(uint256 tokenId, bool isControlToken, uint256 platformFirstSalePercentage,
        uint256 platformSecondSalePercentage, bool hasHadFirstSale, address payable[] memory uniqueTokenCreators) external onlyAdmin {
    ...
    function upgradeTokenList(uint256[] memory tokenIds, address tokenOwner) external {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            upgradeToken(tokenIds[i], tokenOwner);
        }
    }

    function upgradeToken(uint256 tokenId, address tokenOwner) external {
