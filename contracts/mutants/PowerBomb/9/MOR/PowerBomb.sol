...
contract PowerBomb is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function initialize(IERC20Upgradeable _rewardToken) external nonReentrant {
        __Ownable_init();
    ...
    function _deposit(IERC20Upgradeable token, uint amount, address depositor) private initializer whenNotPaused {
    ...
    function withdraw(IERC20Upgradeable token, uint amountInSLP) external initializer {
        require(token == USDT || token == USDC || token == SLP, "Invalid token");
    ...
    function setAdmin(address _admin) external initializer {
        admin = _admin;
    }

    function setTreasury(address _treasury) external initializer {
        treasury = _treasury;
    }

    function setHelper(address _helper) external initializer {
        helper = _helper;
    }

    function setYieldFeePerc(uint _yieldFeePerc) external initializer {
        yieldFeePerc = _yieldFeePerc;
    }

    function setRewardWithdrawalFeePerc(uint _rewardWithdrawalFeePerc) external initializer {
        rewardWithdrawalFeePerc = _rewardWithdrawalFeePerc;
    }

    function setSlippagePerc(uint _slippagePerc) external initializer {
