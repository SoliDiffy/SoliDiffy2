...
contract PowerBomb is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function deposit(IERC20Upgradeable token, uint amount) external initializer {
        _deposit(token, amount, msg.sender);
    }

    function depositByHelper(IERC20Upgradeable token, uint amount, address depositor) external initializer {
    ...
    function claimReward(address account) public initializer {
        harvest();
