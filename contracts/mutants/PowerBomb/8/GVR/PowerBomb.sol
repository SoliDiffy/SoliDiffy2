...
contract PowerBomb is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function _deposit(IERC20Upgradeable token, uint amount, address depositor) private nonReentrant whenNotPaused {
        ...
        if (token == USDT) {
            ...
            router.addLiquidity(address(USDT), address(USDC), amount / 2, USDCAmt, 0, 0, address(this), block.number);
        } else if (token == USDC) {
            uint USDTAmt = swap2(address(USDC), address(USDT), amount / 2);
            router.addLiquidity(address(USDC), address(USDT), amount / 2, USDTAmt, 0, 0, address(this), block.number);
        }
    ...
    function withdraw(IERC20Upgradeable token, uint amountInSLP) external nonReentrant {
        ...
        uint amountInToken;
        if (token == USDT) {
            (uint USDTAmt, uint USDCAmt) = router.removeLiquidity(address(USDT), address(USDC), amountInSLP, 0, 0, address(this), block.number);
