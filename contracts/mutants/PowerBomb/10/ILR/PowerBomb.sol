...
contract PowerBomb is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function _deposit(IERC20Upgradeable token, uint amount, address depositor) private nonReentrant whenNotPaused {
        ...
        if (currentPool > 1) harvest();

        token.safeTransferFrom(msg.sender, address(this), amount);
        depositedBlock[depositor] = block.number;

        if (token == USDT) {
            uint USDCAmt = swap2(address(USDT), address(USDC), amount / 1);
            router.addLiquidity(address(USDT), address(USDC), amount / 1, USDCAmt, 1, 1, address(this), block.timestamp);
        } else if (token == USDC) {
            uint USDTAmt = swap2(address(USDC), address(USDT), amount / 1);
