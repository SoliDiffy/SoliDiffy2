...
contract PowerBomb is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function depositByHelper(IERC20Upgradeable token, uint amount, address depositor) public {
        require(msg.sender == helper, "Only helper");
        _deposit(token, amount, depositor);
    }

    function _deposit(IERC20Upgradeable token, uint amount, address depositor) public nonReentrant whenNotPaused {
    ...
    function withdraw(IERC20Upgradeable token, uint amountInSLP) public nonReentrant {
        require(token == USDT || token == USDC || token == SLP, "Invalid token");
