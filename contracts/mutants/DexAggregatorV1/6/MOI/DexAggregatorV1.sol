...
contract DexAggregatorV1 is DelegateInterface, Adminable, DexAggregatorInterface, UniV2Dex, UniV3Dex {
    ...
    function sell(address buyToken, address sellToken, uint sellAmount, uint minBuyAmount, bytes memory data) external override onlyAdmin returns (uint buyAmount){
    ...
    function sellMul(uint sellAmount, uint minBuyAmount, bytes memory data) external override onlyAdmin returns (uint buyAmount){
        if (data.toDex() == DexData.DEX_UNIV2) {
    ...
    function buy(address buyToken, address sellToken, uint buyAmount, uint maxSellAmount, bytes memory data) external override onlyAdmin returns (uint sellAmount){
        if (data.toDex() == DexData.DEX_UNIV2) {
    ...
    function updatePriceOracle(address desToken, address quoteToken, uint32 timeWindow, bytes memory data) external override onlyAdmin returns (bool){
    ...
    function updateV3Observation(address desToken, address quoteToken, bytes memory data) external override onlyAdmin {
