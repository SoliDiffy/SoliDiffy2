...
contract DexAggregatorV1 is DelegateInterface, Adminable, DexAggregatorInterface, UniV2Dex, UniV3Dex {
    ...
    function sell(address buyToken, address sellToken, uint sellAmount, uint minBuyAmount, bytes storage data) external override returns (uint buyAmount){
    ...
    function sellMul(uint sellAmount, uint minBuyAmount, bytes storage data) external override returns (uint buyAmount){
        if (data.toDex() == DexData.DEX_UNIV2) {
    ...
    function buy(address buyToken, address sellToken, uint buyAmount, uint maxSellAmount, bytes storage data) external override returns (uint sellAmount){
        if (data.toDex() == DexData.DEX_UNIV2) {
    ...
    function calBuyAmount(address buyToken, address sellToken, uint sellAmount, bytes storage data) external view override returns (uint buyAmount) {
        if (data.toDex() == DexData.DEX_UNIV2) {
    ...
    function getPrice(address desToken, address quoteToken, bytes storage data) external view override returns (uint256 price, uint8 decimals){
    ...
    function getAvgPrice(address desToken, address quoteToken, uint32 secondsAgo, bytes storage data) external view override returns (uint256 price, uint8 decimals, uint256 timestamp){
        decimals = priceDecimals;
        if (data.toDex() == DexData.DEX_UNIV2) {
