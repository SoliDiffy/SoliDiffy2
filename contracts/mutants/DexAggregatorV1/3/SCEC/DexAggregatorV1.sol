...
contract DexAggregatorV1 is DelegateInterface, Adminable, DexAggregatorInterface, UniV2Dex, UniV3Dex {
    ...
    function getPrice(address desToken, address quoteToken, bytes memory data) external view override returns (uint8 decimals, uint256 price){
    ...
    function getAvgPrice(address desToken, address quoteToken, uint32 secondsAgo, bytes memory data) external view override returns (uint256 timestamp, uint256 price, uint8 decimals){
        decimals = priceDecimals;
        if (data.toDex() == DexData.DEX_UNIV2) {
    ...
    ) external view override returns (uint256 timestamp, uint price, uint cAvgPrice, uint256 hAvgPrice, uint8 decimals){
        decimals = priceDecimals;
