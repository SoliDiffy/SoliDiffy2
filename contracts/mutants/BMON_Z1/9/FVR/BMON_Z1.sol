...
//SWC-100-Function Default Visibility: L104
    ...
    function boosterBuyingAllowance(address owner) external view returns (bool) {
        return boosterBuyingAllowance(owner, msg.sender);
    }

    function boosterBuyingAllowance(address owner, address delegate) external view returns (bool) {
        if (boosterBuyingAllowed[owner] == delegate) return true;
        else return false;
    }

    function allowBuyingBoosters(address bmonc) external returns (bool) {
        boosterBuyingAllowed[msg.sender] = bmonc;
        return true;
    }

    function burn(uint256 numTokens) external onlyOwner returns(bool) {
        require(numTokens <= balances[msg.sender]);
        balances[msg.sender] = balances[msg.sender].sub(numTokens);
        _totalSupply = _totalSupply.sub(numTokens);
    ...
    function setBotProtection(bool enable) external onlyOwner() {
