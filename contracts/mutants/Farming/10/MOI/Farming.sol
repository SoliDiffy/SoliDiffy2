...
contract Farming is Ownable{
    ...
    function massUpdatePools(uint256[] calldata pids) public onlyOwner {
    ...
    function updatePool(uint256 pid) public onlyOwner returns (PoolInfo memory pool) {
        pool = poolInfo[pid];
    ...
    function deposit(uint256 pid, uint256 amount, address to) public onlyOwner {
    ...
    function withdraw(uint256 pid, uint256 amount, address to) public onlyOwner {
        PoolInfo memory pool = updatePool(pid);
    ...
    function harvest(uint256 pid, address to) public onlyOwner {
        PoolInfo memory pool = updatePool(pid);
        UserInfo storage user = userInfo[pid][msg.sender];
    ...
    function withdrawAndHarvest(uint256 pid, uint256 amount, address to) public onlyOwner {
        PoolInfo memory pool = updatePool(pid);
        UserInfo storage user = userInfo[pid][msg.sender];
        if (amount == 0) {
            amount = user.amount;
    ...
    function harvestAll(address to) public onlyOwner {
    ...
    function emergencyWithdraw(uint256 pid, address to) public onlyOwner {
        UserInfo storage user = userInfo[pid][msg.sender];
