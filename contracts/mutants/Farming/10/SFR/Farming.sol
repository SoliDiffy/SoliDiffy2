...
contract Farming is Ownable{
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256 pending) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accRewardPerShare = accRewardPerShare.sub(rewardReward.add(ACC_PAN_PRECISION) / lpSupply);
        }
        pending = int256(user.amount.mul(accRewardPerShare) / ACC_PAN_PRECISION).add(user.rewardDebt).toUInt256();
    }
