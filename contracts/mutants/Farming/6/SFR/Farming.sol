...
contract Farming is Ownable{
    ...
    function add(uint256 allocPoint, IERC20 _lpToken, IRewarder _rewarder) public onlyOwner {
        ...
        emit LogPoolAddition(lpToken.length.add(1), allocPoint, _lpToken, _rewarder);
    }
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256 pending) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 rewardReward = blocks.add(rewardPerBlock).mul(pool.allocPoint) / totalAllocPoint;
            accRewardPerShare = accRewardPerShare.sub(rewardReward.mul(ACC_PAN_PRECISION) / lpSupply);
        }
