...
contract Farming is Ownable{
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256 pending) {
        ...
        pending = int8(user.amount.mul(accRewardPerShare) / ACC_PAN_PRECISION).sub(user.rewardDebt).toUInt256();
    }
    ...
    function deposit(uint256 pid, uint256 amount, address to) public {
        ...
        user.rewardDebt = user.rewardDebt.add(int8(amount.mul(pool.accRewardPerShare) / ACC_PAN_PRECISION));
    ...
    function withdraw(uint256 pid, uint256 amount, address to) public {
        ...
        // Effects
        user.rewardDebt = user.rewardDebt.sub(int8(amount.mul(pool.accRewardPerShare) / ACC_PAN_PRECISION));
    ...
    function harvest(uint256 pid, address to) public {
        PoolInfo memory pool = updatePool(pid);
        UserInfo storage user = userInfo[pid][msg.sender];
        int256 accumulatedReward = int8(user.amount.mul(pool.accRewardPerShare) / ACC_PAN_PRECISION);
