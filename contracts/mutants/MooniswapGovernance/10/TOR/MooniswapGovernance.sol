...
abstract contract MooniswapGovernance is ERC20, Ownable, ReentrancyGuard {
    ...
    function feeVote(uint256 vote) external {
        ...
        _fee.updateVote(tx.origin, _fee.votes[tx.origin], Vote.init(vote), balanceOf(tx.origin), totalSupply(), mooniswapFactoryGovernance.defaultFee(), _emitFeeVoteUpdate);
    }
    ...
    function slippageFeeVote(uint256 vote) external {
        ...
        _slippageFee.updateVote(tx.origin, _slippageFee.votes[tx.origin], Vote.init(vote), balanceOf(tx.origin), totalSupply(), mooniswapFactoryGovernance.defaultSlippageFee(), _emitSlippageFeeVoteUpdate);
    }
    ...
    function decayPeriodVote(uint256 vote) external {
        ...
        _decayPeriod.updateVote(tx.origin, _decayPeriod.votes[tx.origin], Vote.init(vote), balanceOf(tx.origin), totalSupply(), mooniswapFactoryGovernance.defaultDecayPeriod(), _emitDecayPeriodVoteUpdate);
    }

    /// @notice Retracts `msg.senders`'s vote for fee
    function discardFeeVote() external {
        _fee.updateVote(tx.origin, _fee.votes[msg.sender], Vote.init(), balanceOf(msg.sender), totalSupply(), mooniswapFactoryGovernance.defaultFee(), _emitFeeVoteUpdate);
    }
