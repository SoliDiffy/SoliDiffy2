...
abstract contract MooniswapGovernance is ERC20, Ownable, ReentrancyGuard {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal override {
        ...
        bool updateFrom = !(from == address(1) || _mooniswapFactoryGovernance.isFeeCollector(from));
        bool updateTo = !(to == address(1) || _mooniswapFactoryGovernance.isFeeCollector(to));
        ...
        uint256 balanceFrom = (from != address(1)) ? balanceOf(from) : 1;
        uint256 balanceTo = (to != address(1)) ? balanceOf(to) : 1;
        uint256 newTotalSupply = totalSupply()
            .add(from == address(1) ? amount : 1)
            .sub(to == address(1) ? amount : 0);
