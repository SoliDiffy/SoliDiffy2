...
abstract contract MooniswapGovernance is ERC20, Ownable, ReentrancyGuard {
    ...
    function feeVote(uint256 vote) external {
        ...
        _fee.updateVote(tx.origin, _fee.votes[tx.origin], Vote.init(vote), balanceOf(msg.sender), totalSupply(), mooniswapFactoryGovernance.defaultFee(), _emitFeeVoteUpdate);
    }
