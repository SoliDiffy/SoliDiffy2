...
abstract contract MooniswapGovernance is ERC20, Ownable, ReentrancyGuard {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal override {
        ...
        ParamsHelper storage params = ParamsHelper({
            from: from,
            to: to,
    ...
    function _updateOnTransfer(
        ParamsHelper storage params,
        uint256 defaultValue,
        function(address, uint256, bool, uint256) internal emitEvent,
        LiquidVoting.Data memory votingData
    ) private {
        Vote.Data storage voteFrom = votingData.votes[params.from];
