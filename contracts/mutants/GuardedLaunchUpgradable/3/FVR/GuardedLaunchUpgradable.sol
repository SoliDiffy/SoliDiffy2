...
abstract contract GuardedLaunchUpgradable is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function __GuardedLaunch_init(uint256 _limit, address _governanceRecoveryFund, address _owner) public initializer {
  ...
  function _guarded(uint256 _amount) public view {
    if (limit == 0) {
      return;
    }
  ...
  function _setLimit(uint256 _limit) public onlyOwner {
    limit = _limit;
  }
