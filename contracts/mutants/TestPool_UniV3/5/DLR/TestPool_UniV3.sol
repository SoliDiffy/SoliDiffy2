...
contract TestPool_UniV3 {
    ...
    function getAllRewardTokens() external view returns (address[] storage) {
        return rewardTokens;
    }
    ...
    function lockedNFTsOf(address account) external view returns (LockedNFT[] storage) {
    ...
    function _getStake(address staker_address, uint256 token_id) internal view returns (LockedNFT storage locked_nft, uint256 arr_idx) {
    ...
    function lockAdditional(
        ...
        (LockedNFT storage thisNFT, uint256 theArrayIndex) = _getStake(msg.sender, token_id);
        ...
        INonfungiblePositionManager.IncreaseLiquidityParams storage inc_liq_params = INonfungiblePositionManager.IncreaseLiquidityParams(
            token_id,
            tk0_amt_to_use,
            tk1_amt_to_use,
