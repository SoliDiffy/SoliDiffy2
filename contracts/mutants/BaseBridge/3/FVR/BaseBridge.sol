...
abstract contract BaseBridge is PerpFiOwnableUpgrade, IBaseBridge, DecimalERC20 {
    ...
    // PUBLIC
    //
    function __BaseBridge_init(IAMB _ambBridge, IMultiTokenMediator _multiTokenMediator) public initializer {
        __Ownable_init();
        setAMBBridge(_ambBridge);
        setMultiTokenMediator(_multiTokenMediator);
    }

    function setAMBBridge(IAMB _ambBridge) external onlyOwner {
        require(address(_ambBridge) != address(0), "address is empty");
        ambBridge = _ambBridge;
        emit BridgeChanged(address(_ambBridge));
    }

    function setMultiTokenMediator(IMultiTokenMediator _multiTokenMediator) external onlyOwner {
