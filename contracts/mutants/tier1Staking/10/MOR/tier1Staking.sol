...
function updateOracleAddress(address newOracleAddress ) public onlyAdmin returns (bool){
  ...
  function withdraw(string memory tier2ContractName, address tokenAddress, uint256 amount, address payable onBehalfOf) onlyOwner payable public returns(bool){
   ...
   function changeTier2Owner(address payable tier2Contract, address payable newOwner) onlyAdmin public returns (bool){
     Tier2StakingInterface tier2Con = Tier2StakingInterface(tier2Contract);
     tier2Con.changeOwner(newOwner);
     return true;
   }

   function changeOwner(address payable newOwner) onlyAdmin public returns (bool){
     owner = newOwner;
     return true;
   }
  ...
  function adminEmergencyWithdrawTokensTier2(address payable tier2Contract, address token, uint amount, address payable destination) public onlyAdmin returns(bool) {
    Tier2StakingInterface tier2Con = Tier2StakingInterface(tier2Contract);
