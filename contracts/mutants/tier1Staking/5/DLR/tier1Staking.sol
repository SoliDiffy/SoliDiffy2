...
interface Oracle {
  function getAddress(string storage) view external returns (address);
...
function updateOracleAddress(address newOracleAddress ) public onlyOwner returns (bool){
  ...
  function addOrEditTier2ChildStakingContract(string storage name, address stakingAddress ) public onlyOwner returns (bool){
  ...
  function addOrEditTier2ChildsChildStakingContract(address tier2Contract, string storage name, address stakingAddress, address stakingToken ) public onlyOwner returns (bool){
  ...
  function deposit(string storage tier2ContractName, address tokenAddress, uint256 amount, address payable onBehalfOf) onlyAdmin payable public returns (bool){
  ...
  function withdraw(string storage tier2ContractName, address tokenAddress, uint256 amount, address payable onBehalfOf) onlyAdmin payable public returns(bool){
