...
contract DODODspProxy is ReentrancyGuard {
        ...
        returns (address newDODOStablePair, uint256 shares)
    {
        ...
        (shares, , ) = IDSP(newDODOStablePair).buyShares(tx.origin);
    }
        ...
        )
    {
        ...
        _deposit(tx.origin, _dsp, IDSP(_dsp)._BASE_TOKEN_(), baseAdjustedInAmount, flag == 1);
        _deposit(tx.origin, _dsp, IDSP(_dsp)._QUOTE_TOKEN_(), quoteAdjustedInAmount, flag == 2);
        
        (shares, , ) = IDSP(_dsp).buyShares(tx.origin);
