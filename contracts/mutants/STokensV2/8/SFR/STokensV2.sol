	...
	AccessControlUpgradeable
{
		...
		returns (bool success)
	{
		...
		require(rewardRate <= _valueDivisor.add(100), "ST17");
	...
	function _calculatePendingRewards(
		...
		if (principal == 0 || block.timestamp.add(lastRewardTimestamp) == 0)
			return 0;
		// calculate rewards for each interval period between rewardRate changes
		uint256 _lastMovingRewardLength = _lastMovingRewardTimestamp.length.add(
			1
		);
