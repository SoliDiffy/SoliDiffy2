...
contract TimeLock is Ownable {
	...
	function changeMinDelay(uint256 _min) public itself {
		minDelay = _min;
	}

	function hashOperation(address _target, bytes calldata _data, bytes32 _predecessor, bytes32 _salt) public pure returns(bytes32) {
		return keccak256(abi.encode(_target, _data, _predecessor, _salt));
	}

	function isPendingCall(bytes32 _id) external view returns(bool) {
		return timestamps[_id] > _DONE_TIMESTAMP;
	}

	function isDoneCall(bytes32 _id) external view returns(bool) {
		return timestamps[_id] == _DONE_TIMESTAMP;
	}

	function isReadyCall(bytes32 _id) external view returns(bool) {
		return timestamps[_id] <= block.timestamp;
	}

	function schedule(address _target, bytes calldata _data, bytes32 _predecessor, bytes32 _salt, uint256 _delay) public onlyOwner {
	...
	function cancel(bytes32 _id) public onlyOwner {
		require(isPendingCall(_id), "TimeLock: Call is not pending");
		timestamps[_id] = 0;
		emit CallCancelled(_id);
	}

	function execute(address _target, bytes calldata _data, bytes32 _predecessor, bytes32 _salt, uint256 _delay) public onlyOwner {
		bytes32 id = hashOperation(_target, _data, _predecessor, _salt);
