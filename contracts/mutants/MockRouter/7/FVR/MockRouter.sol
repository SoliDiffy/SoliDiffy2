...
contract MockRouter {
    ...
    function checkAmountMin(uint256 amount) external view {
        require(amountMinThreshold == 0 || amountMinThreshold > amount, "amount liquidity revert");
    }

    function setAmountMin(uint256 amount) external {
    ...
    function addLiquidity(
        ...
        address to,
        uint
    ) public returns (uint, uint, uint liquidity) {
        address pair = address(PAIR);
        checkAmountMin(amountToken0Min);
    ...
    function removeLiquidity(
        ...
        address to,
        uint
    ) public returns (uint amountFei, uint amountToken) {
        checkAmountMin(amountToken0Min);
    ...
    function transferLiquidity(uint liquidity) public {
