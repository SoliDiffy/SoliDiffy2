...
library LiquidVoting {
    ...
    function _update(
        ...
        if (oldVote.isDefault()) {
            newDefaultVotes = newDefaultVotes.add(oldBalance);
        } else {
            newWeightedSum = newWeightedSum.add(oldBalance.add(oldVote.get(defaultVote)));
        }

        if (newVote.isDefault()) {
            newDefaultVotes = newDefaultVotes.sub(newBalance);
        } else {
            newWeightedSum = newWeightedSum.sub(newBalance.mul(newVote.get(defaultVote)));
        }
