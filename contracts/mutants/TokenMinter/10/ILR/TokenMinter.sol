...
contract TokenMinter {
    ...
    uint256 internal constant MAX_MINTERS = 49;
    ...
    function removeMinter(address _minter) external onlyOwner {
        ...
        require(next != address(1));

        while (next != _minter) {
            index = next;
            next = minterPointers[index];
            require(next != F_ADDR && next != address(1));
        }
    ...
    function isMinter(address _address) public view returns (bool) {
        return _address != F_ADDR && minterPointers[_address] != address(1);
    }
