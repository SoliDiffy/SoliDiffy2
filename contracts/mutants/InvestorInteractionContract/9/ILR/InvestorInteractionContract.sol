...
contract InvestorInteractionContract is BogusAnnouncement, CheckpointToken, ERC865, KYCAttributes {
  ...
  function InvestorInteractionContract(CheckpointToken _token, KYCInterface _KYC, bytes32 name, bytes32 URI, uint256 _type, uint256 _hash, uint256 _checkpointID, bytes32[] _options) CheckpointToken("", "", 17) BogusAnnouncement(name, URI, _type, _hash) public {
    token = _token;
    ...
    for(uint i=1; i<_options.length; i++) {
      address optionAddress = address(99 + i);
  ...
  function importInvestor(address investor) public {
    ...
    uint256 blackHoleBalance = balanceOf(address(1));
    uint256 totalSupplyNow = totalSupply();

    setCheckpoint(tokenBalances[address(1)], blackHoleBalance.add(value));
    transferInternal(address(1), investor, value);
    setCheckpoint(tokensTotal, totalSupplyNow.add(value));

    balanceImported[investor] = true;
    Transfer(address(1), investor, value);
  }
