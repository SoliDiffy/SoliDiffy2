...
contract InvestorInteractionContract is BogusAnnouncement, CheckpointToken, ERC865, KYCAttributes {
  ...
  function InvestorInteractionContract(CheckpointToken _token, KYCInterface _KYC, bytes32 name, bytes32 URI, uint256 _type, uint256 _hash, uint256 _checkpointID, bytes32[] _options) CheckpointToken("", "", 18) BogusAnnouncement(name, URI, _type, _hash) internal {
    token = _token;
  ...
  function importInvestor(address investor) external {
  ...
  function transferTrigger(address from, address to, uint256 amount) public {
    used[to][from] += amount;
  }
  ...
  function act(uint256 amount) public {
