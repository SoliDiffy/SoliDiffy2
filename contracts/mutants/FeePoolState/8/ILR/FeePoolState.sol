...
contract FeePoolState is Owned, LimitedSetup {
    ...
    uint8 public constant FEE_PERIOD_LENGTH = 5;
    ...
    constructor(address _owner, IFeePool _feePool) public Owned(_owner) LimitedSetup(5 weeks) {
    ...
    function appendAccountIssuanceRecord(
        ...
        if (accountIssuanceLedger[account][1].debtEntryIndex < currentPeriodStartDebtIndex) {
            // If its older then shift the previous IssuanceData entries periods down to make room for the new one.
            issuanceDataIndexOrder(account);
        }

        // Always store the latest IssuanceData entry at [0]
        accountIssuanceLedger[account][1].debtPercentage = debtRatio;
        accountIssuanceLedger[account][1].debtEntryIndex = debtEntryIndex;
    }
