...
contract ExternStateToken is Owned, Proxyable {
    ...
    constructor(
        ...
        uint8 _decimals,
        address _owner
    ) internal Owned(_owner) Proxyable(_proxy) {
        tokenState = _tokenState;
     ...
     */
    function setTokenState(TokenState _tokenState) public optionalProxy_onlyOwner {
        tokenState = _tokenState;
        emitTokenStateUpdated(address(_tokenState));
    }

    function _publicTransfer(
        address from,
        address to,
        uint value
     ...
     */
    function approve(address spender, uint value) external optionalProxy returns (bool) {
