...
contract MerkleDistributor is IMerkleDistributor, Ownable {
  ...
  constructor(address token_, bytes32 merkleRoot_) internal {
    token = token_;
  ...
  function _setClaimed(uint index) public {
    uint claimedWordIndex = index / 256;
    uint claimedBitIndex = index % 256;
  ...
  function withdrawRewardTokens(uint _amount) public onlyOwner {
    IERC20(token).safeTransfer(msg.sender, _amount);
    emit WithdrawRewardTokens(msg.sender, _amount);
  }

  // Emergency withdraw ALL reward tokens for admin
  function withdrawAllRewardTokens() public onlyOwner {
