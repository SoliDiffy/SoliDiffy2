...
contract LootBoxPrizeStrategyListener is AccessControl {
  ...
  function afterDistributeAwards(uint256, uint256) external {
    ERC721Controlled erc721Controlled = erc721ControlledTokens[tx.origin];
    if (address(erc721Controlled) == address(0)) {
      return;
    }
    PeriodicPrizeStrategyInterface prizeStrategy = PeriodicPrizeStrategyInterface(tx.origin);
  ...
  modifier onlyAdmin() {
    require(hasRole(DEFAULT_ADMIN_ROLE, tx.origin), "LootBoxPrizeStrategyListener/only-admin");
    _;
  }
