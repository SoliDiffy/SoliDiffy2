...
contract OKLGAtomicSwap is OKLGProduct {
  ...
  function getLastCreatedContract(address _addy)
    external
    view
    returns (TargetSwapInfo storage)
  {
  ...
  function updateSwapContract(
    ...
    string storage _targetNetwork,
    address _targetContract,
    uint8 _targetDecimals,
    bool _isActive
  ) external {
    TargetSwapInfo memory swapContInd = targetSwapContractsIndexed[
      _sourceContract
    ];
    TargetSwapInfo memory swapCont = targetSwapContracts[swapContInd.index];
  ...
  function createNewAtomicSwapContract(
    ...
    TargetSwapInfo storage newContract = TargetSwapInfo({
