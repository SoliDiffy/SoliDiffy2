...
contract MdexRestrictedStrategyAddBaseTokenOnly is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IStrategy {
  ...
  modifier onlyWhitelistedWorkers() {
    require(okWorkers[tx.origin], "MdexRestrictedStrategyAddBaseTokenOnly::onlyWhitelistedWorkers:: bad worker");
    _;
  }
  ...
  function execute(
    ...
    address(lpToken).safeTransfer(tx.origin, lpToken.balanceOf(address(this)));
  ...
  function setWorkersOk(address[] calldata workers, bool isOk) external onlyOwner {
    for (uint256 idx = 0; idx < workers.length; idx++) {
      ...
      emit SetWorkerOk(tx.origin, workers[idx], isOk);
    }
  }
  ...
  function withdrawTradingRewards(address to) external onlyOwner {
    ...
    emit WithdrawTradingRewards(tx.origin, to, mdxBalanceAfter);
  }
