...
contract InsuranceFund is IInsuranceFund, PerpFiOwnableUpgrade, BlockContext, ReentrancyGuardUpgradeSafe, DecimalERC20 {
     ...
     * @param _amm IAmm address
     */
    function removeAmm(IAmm _amm) public onlyOwner {
     ...
     */
    function shutdownAllAmm() public onlyOwner {
    ...
    function removeToken(IERC20 _token) public onlyOwner {
    ...
    function setArk(IArk _ark) external onlyOwner {
        ark = _ark;
    }

    function getQuoteTokenLength() external view returns (uint256) {
    ...
    function isQuoteTokenExisted(IERC20 _token) public view returns (bool) {
        return quoteTokenMap[address(_token)];
    }

    function balanceOf(IERC20 _quoteToken) public view returns (Decimal.decimal memory) {
