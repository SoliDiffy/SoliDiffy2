...
contract DTT {
    ...
    enum sigStandard { personal, typed, stringHex }
    enum sigDestination { approve, transfer, approveAndCall, transferFrom }
    ...
    function requireSignature (
        ...
        if (std == sigStandard.personal) { // Typed signature. This is the most likely scenario to be used and accepted
            require(
                signer == ecrecover(
                    keccak256(
        ...
        } else if (std == sigStandard.typed) { // Ethereum signed message signature (Geth and Trezor)
            require(
