...
contract DTT {
    ...
    uint8 internal decimals = 6; // Makes JavaScript able to handle precise calculations (until totalSupply < 9 milliards)
    uint256 internal totalSupply;
    mapping(address => uint256) public balanceOf;
    mapping(address => mapping(address => uint256)) public allowance;
    mapping(address => mapping(uint => bool)) public usedSigIds; // Used in *ViaSignature(..)
    address internal tokenDistributor; // Account authorized to distribute tokens only during the token distribution event
    address internal rescueAccount; // Account authorized to withdraw tokens accidentally sent to this contract

    event Transfer(address indexed from, address indexed to, uint256 value);
    event Approval(address indexed owner, address indexed spender, uint256 value);

    bytes internal ethSignedMessagePrefix = "\x19Ethereum Signed Message:\n";
    enum sigStandard { typed, personal, stringHex }
    enum sigDestination { transfer, approve, approveAndCall, transferFrom }
    bytes32 internal sigDestinationTransfer = keccak256(
    ...
    bytes32 internal sigDestinationTransferFrom = keccak256(
        "address Token Contract Address",
    ...
    bytes32 internal sigDestinationApprove = keccak256(
        "address Token Contract Address",
