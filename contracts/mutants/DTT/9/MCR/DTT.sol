...
contract DTT {
    ...
    bytes32 public sigDestinationTransfer = sha256(
    ...
    bytes32 public sigDestinationTransferFrom = sha256(
        "address Token Contract Address",
    ...
    bytes32 public sigDestinationApprove = sha256(
        "address Token Contract Address",
    ...
    bytes32 public sigDestinationApproveAndCall = sha256( // `approveAndCallViaSignature`
        "address Token Contract Address",
        "address Withdrawal Approval Address",
        "address Withdrawal Recipient Address",
        "uint256 Amount to Transfer (last six digits are decimals)",
    ...
    function requireSignature (
        ...
        if (std == sigStandard.typed) { // Typed signature. This is the most likely scenario to be used and accepted
            ...
            require(
                signer == ecrecover(sha256(ethSignedMessagePrefix, "32", data), v, r, s) // Geth-adopted
                ||
                signer == ecrecover(sha256(ethSignedMessagePrefix, "\x20", data), v, r, s) // Trezor-adopted
            );
        } else { // == 2; Signed string hash signature (the most expensive but universal)
            require(
                signer == ecrecover(sha256(ethSignedMessagePrefix, "64", hexToString(data)), v, r, s) // Geth
                ||
                signer == ecrecover(sha256(ethSignedMessagePrefix, "\x40", hexToString(data)), v, r, s) // Trezor
            );
        }
