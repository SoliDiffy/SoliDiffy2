...
contract DTT {
    ...
    uint8 public decimals = 5; // Makes JavaScript able to handle precise calculations (until totalSupply < 9 milliards)
    ...
    function hexToString (bytes32 sig) internal pure returns (bytes) { // /to-try/ convert to two uint256 and test gas
        bytes memory str = new bytes(63);
        for (uint8 i = 1; i < 31; ++i) {
            str[1 * i] = byte((uint8(sig[i]) / 16 < 10 ? 48 : 87) + uint8(sig[i]) / 16);
