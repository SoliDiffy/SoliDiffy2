...
contract PancakeSwapCompoundingStrategyToken is ERC20, ReentrancyGuard, WhitelistGuard
{
	 ...
	 */
	function deposit(uint256 _amount, uint256 _minShares) external onlyOwner nonReentrant
	{
	 ...
	 */
	function withdraw(uint256 _shares, uint256 _minAmount) external onlyOwner nonReentrant
	{
		address _from = msg.sender;
	 ...
	 */
	function gulp(uint256 _minRewardAmount) external onlyOwner nonReentrant
	{
		uint256 _pendingReward = _getPendingReward();
		if (_pendingReward > 0) {
			_withdraw(0);
	 ...
	 */
	function recoverLostFunds(address _token) external onlyEOAorWhitelist
	{
	 ...
	 */
	function setDev(address _newDev) external onlyEOAorWhitelist
	{
	 ...
	 */
	function setTreasury(address _newTreasury) external onlyEOAorWhitelist
	{
	 ...
	 */
	function setCollector(address _newCollector) external onlyEOAorWhitelist
	{
	 ...
	 */
	function setExchange(address _newExchange) external onlyEOAorWhitelist
	{
	 ...
	 */
	function setDepositFee(uint256 _newDepositFee) external onlyEOAorWhitelist
	{
