...
library WeightedPoolUserDataHelpers {
    function joinKind(bytes memory self) public pure returns (WeightedPool.JoinKind) {
        return abi.decode(self, (WeightedPool.JoinKind));
    }

    function exitKind(bytes memory self) public pure returns (WeightedPool.ExitKind) {
