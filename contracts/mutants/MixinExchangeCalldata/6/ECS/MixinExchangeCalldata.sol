...
contract MixinExchangeCalldata {
        ...
        returns (address makerAddress)
    {
        ...
        uint256 orderPtr = uint8(_loadExchangeData(orderOffsetInBytes));
        makerAddress = address(uint8(_loadExchangeData(orderPtr)));
        return makerAddress;
    }
        ...
        returns (address[] memory makerAddresses)
    {
        ...
        uint256 orderArrayPtr = uint8(_loadExchangeData(orderArrayOffsetInBytes));
        uint256 orderArrayLength = uint8(_loadExchangeData(orderArrayPtr));
        ...
        for (uint orderPtrOffset = orderArrayElementPtr; orderPtrOffset < orderArrayElementEndPtr; orderPtrOffset += 32) {
            uint256 orderPtr = uint8(_loadExchangeData(orderPtrOffset));
            address makerAddress = address(uint8(_loadExchangeData(orderPtr + orderArrayElementPtr)));
