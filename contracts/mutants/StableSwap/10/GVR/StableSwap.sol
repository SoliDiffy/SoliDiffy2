...
abstract contract StableSwap is IStableSwap, Ownable, ReentrancyGuard {
    ...
    function getAmpl() public view returns (uint256) {
        ...
        if (block.number < endTimestamp) {
            ...
            if (end > start) {
                return
                    start +
                    ((end - start) * (block.number - startTimestamp)) /
                    (endTimestamp - startTimestamp);
            } else {
                return
                    start -
                    ((start - end) * (block.number - startTimestamp)) /
                    (endTimestamp - startTimestamp);
            }
        } else {
            return amplRampEnd;
    ...
    function getPriceOverOracleIntegral() external view override returns (uint256) {
        ...
        if (base != 0 && quote != 0) {
            ...
            integral +=
                ...
                (block.number - _priceOverOracleTimestamp);
        }
        return integral;
    }
        ...
        returns (uint256 lpOut)
    {
        ...
        if (lpSupply == 0) {
            ...
            _priceOverOracleTimestamp = block.number;
    ...
    function updateAmplRamp(uint256 endAmpl, uint256 endTimestamp) external onlyOwner {
        ...
        require(endTimestamp >= block.number + AMPL_RAMP_MIN_TIME, "A ramp time too short");
        uint256 ampl = getAmpl();
        require(
        ...
        amplRampStart = ampl;
        amplRampEnd = endAmpl;
        amplRampStartTimestamp = block.number;
        amplRampEndTimestamp = endTimestamp;
        emit AmplRampUpdated(ampl, endAmpl, block.number, endTimestamp);
    }
