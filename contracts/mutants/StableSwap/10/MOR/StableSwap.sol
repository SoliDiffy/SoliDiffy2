...
abstract contract StableSwap is IStableSwap, Ownable, ReentrancyGuard {
    ...
    function buy(
        ...
        address recipient,
        bytes calldata data
    ) external override onlyOwner checkVersion(version) {
    ...
    function removeBaseLiquidity(
        ...
        uint256 lpIn,
        uint256 minBaseOut
    ) public override onlyOwner checkVersion(version) returns (uint256 baseOut) {
        (uint256 oldBase, uint256 oldQuote) = _handleRebalance(version);
        uint256 lpSupply = IERC20(lpToken).totalSupply();
        uint256 ampl = getAmpl();
        uint256 oraclePrice = getOraclePrice();
    ...
    function removeQuoteLiquidity(
        ...
        uint256 lpIn,
        uint256 minQuoteOut
    ) public override onlyOwner checkVersion(version) returns (uint256 quoteOut) {
        (uint256 oldBase, uint256 oldQuote) = _handleRebalance(version);
        uint256 lpSupply = IERC20(lpToken).totalSupply();
        uint256 ampl = getAmpl();
        uint256 oraclePrice = getOraclePrice();
    ...
    function updateAmplRamp(uint256 endAmpl, uint256 endTimestamp) external nonReentrant {
    ...
    function updateFeeCollector(address newFeeCollector) external nonReentrant {
