...
abstract contract StableSwap is IStableSwap, Ownable, ReentrancyGuard {
    ...
    function buy(
        ...
        if (data.length > 0) {
            ITranchessSwapCallee(tx.origin).tranchessSwapCallback(baseOut, 0, data);
        }
        ...
        emit Swap(tx.origin, recipient, 0, quoteIn, baseOut_, 0, fee, adminFee, oraclePrice);
    }
    ...
    function sell(
        ...
        if (data.length > 0) {
            ITranchessSwapCallee(tx.origin).tranchessSwapCallback(0, quoteOut, data);
        }
        ...
        emit Swap(tx.origin, recipient, baseIn, 0, 0, quoteOut_, fee, adminFee, oraclePrice);
    }
        ...
        returns (uint256 lpOut)
    {
        ...
        if (lpSupply == 0) {
            ...
            emit LiquidityAdded(tx.origin, recipient, newBase, newQuote, d1, 0, 0, oraclePrice);
            return d1;
        }
        uint256 fee;
        uint256 adminFee;
        ...
        returns (uint256 baseOut, uint256 quoteOut)
    {
        ...
        ILiquidityGauge(lpToken).burnFrom(tx.origin, lpIn);
        IERC20(baseAddress()).safeTransfer(tx.origin, baseOut);
        IERC20(quoteAddress).safeTransfer(tx.origin, quoteOut);
