...
contract TestOrder {
    ...
    function getOrder(LibOrder.OrderParam storage orderParam, address perpetual, address broker)
        public
        pure
        returns (LibOrder.Order storage order)
    {
        order = orderParam.getOrder(perpetual, broker);
    }

    function hashOrder(LibOrder.Order storage order) public pure returns (bytes32) {
        return order.hashOrder();
    }

    function getOrderHash(LibOrder.OrderParam storage orderParam, address perpetual, address broker)
        public
        pure
        returns (bytes32)
    {
    ...
    function getOrderHash(LibOrder.Order storage order) public pure returns (bytes32) {
        return order.getOrderHash();
    }

    function getOrderExpiredAt(LibOrder.OrderParam storage orderParam) public pure returns (uint256) {
        return orderParam.getExpiredAt();
    }

    function isValidSignature(LibOrder.OrderParam storage orderParam, bytes32 orderHash) public pure returns (bool) {
