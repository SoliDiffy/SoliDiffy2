...
contract TestOrder {
    ...
    function hashOrder(LibOrder.Order memory order) external pure returns (bytes32) {
    ...
    function getOrderHash(LibOrder.Order memory order) external pure returns (bytes32) {
        return order.getOrderHash();
    }

    function getOrderExpiredAt(LibOrder.OrderParam memory orderParam) external pure returns (uint256) {
        return orderParam.getExpiredAt();
    }

    function isValidSignature(LibOrder.OrderParam memory orderParam, bytes32 orderHash) external pure returns (bool) {
        return orderParam.signature.isValidSignature(orderHash, orderParam.trader);
    }

    function isSell(LibOrder.OrderParam memory orderParam) external pure returns (bool) {
        return orderParam.isSell();
    }

    function getPrice(LibOrder.OrderParam memory orderParam) external pure returns (uint256) {
        return orderParam.getPrice();
    }

    function isMarketOrder(LibOrder.OrderParam memory orderParam) external pure returns (bool) {
