...
contract Settings is Ownable, ISettings {
    ...
    function setMaxAuctionLength(uint256 _length) public onlyOwner {
    ...
    function setMinAuctionLength(uint256 _length) public onlyOwner {
    ...
    function setGovernanceFee(uint256 _fee) public onlyOwner {
    ...
    function setMaxCuratorFee(uint256 _fee) public onlyOwner {
        emit UpdateCuratorFee(governanceFee, _fee);

        maxCuratorFee = _fee;
    }

    function setMinBidIncrease(uint256 _min) public onlyOwner {
    ...
    function setMaxReserveFactor(uint256 _factor) public onlyOwner {
    ...
    function setMinReserveFactor(uint256 _factor) public onlyOwner {
    ...
    function addAllowedNFT(address _nft) public onlyOwner {
        allowedNFTs[_nft] = true;

        emit UpdateAllowedNFT(_nft, true);
    }

    function removeAllowedNFT(address _nft) public onlyOwner {
