...
contract SettingsTest is DSTest {
    ...
    function test_setMaxAuction() public {
        settings.setMaxAuctionLength(4 seconds);
        assertEq(settings.maxAuctionLength(), 4 seconds);
    }

    // too high
    function testFail_setMaxAuction() public {
        settings.setMaxAuctionLength(10 seconds);
    }

    // lower than min auction length
    function testFail_setMaxAuction2() public {
        settings.setMaxAuctionLength(2.9 seconds);
    }

    function test_setMinAuction() public {
        settings.setMinAuctionLength(1 seconds);
    }

    // too low
    function testFail_setMinAuction() public {
        settings.setMaxAuctionLength(0.1 seconds);
    }
