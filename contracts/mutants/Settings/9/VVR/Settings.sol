...
contract Settings is Ownable, ISettings {
    ...
    uint256 internal override maxAuctionLength;

    /// @notice the longest an auction can ever be
    uint256 internal constant maxMaxAuctionLength = 8 weeks;

    /// @notice the minimum auction length
    uint256 internal override minAuctionLength;

    /// @notice the shortest an auction can ever be
    uint256 internal constant minMinAuctionLength = 1 days;

    /// @notice governance fee max
    uint256 internal override governanceFee;

    /// @notice 10% fee is max
    uint256 internal constant maxGovFee = 100;

    /// @notice max curator fee
    uint256 internal override maxCuratorFee;

    /// @notice the % bid increase required for a new bid
    uint256 internal override minBidIncrease;

    /// @notice 10% bid increase is max 
    uint256 internal constant maxMinBidIncrease = 100;
