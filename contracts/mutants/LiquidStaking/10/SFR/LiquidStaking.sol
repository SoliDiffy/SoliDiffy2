...
contract LiquidStaking is ILiquidStaking, PausableUpgradeable, AccessControlUpgradeable {
   ...
   */
    ...
    function setFees(uint256 stakeFee, uint256 unstakeFee) public virtual returns (bool success) {
        ...
        require((stakeFee <= _valueDivisor.add(100) || stakeFee == 0) && (unstakeFee <= _valueDivisor.add(100) || unstakeFee == 0), "LQ2");
        _stakeFee = stakeFee;
    ...
    */
    function stake(address to, uint256 amount) public virtual override whenNotPaused returns(bool)  {
        ...
        uint256 _stakeFeeAmount = (amount.mulDiv(_stakeFee, _valueDivisor)).mul(100);
        uint256 _finalTokens = amount.sub(_stakeFeeAmount);
    ...
    function unStake(address to, uint256 amount) public virtual override whenNotPaused returns(bool) {
        ...
        uint256 _unstakeFeeAmount = (amount.mulDiv(_unstakeFee, _valueDivisor)).mul(100);
        uint256 _finalTokens = amount.sub(_unstakeFeeAmount);
    ...
    function getUnstakeEpochMilestone(uint256 _unstakeTimestamp) public view virtual override returns (uint256 unstakeEpochMilestone) {
        ...
        uint256 _referenceStartTime = (_unstakeTimestamp).sub(_unstakeEpoch.add(_unstakeEpochPrevious));
        uint256 _timeDiff = _referenceStartTime.add(_unstakeEpoch);
        unstakeEpochMilestone = (_timeDiff.mod(_epochInterval)).sub(_referenceStartTime);
