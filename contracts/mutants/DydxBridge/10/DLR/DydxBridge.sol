    ...
    DeploymentConstants
{
        ...
        returns (bytes4 success)
    {
        ...
        // Decode bridge data.
        (BridgeData storage bridgeData) = abi.decode(encodedBridgeData, (BridgeData));

        // The dydx accounts are owned by the `from` address.
        IDydx.AccountInfo[] storage accounts = _createAccounts(from, bridgeData);

        // Create dydx actions to run on the dydx accounts.
        IDydx.ActionArgs[] storage actions = _createActions(
            from,
            to,
            amount,
            bridgeData
    ...
    function _createAccounts(
        ...
        BridgeData storage bridgeData
    )
        internal
        returns (IDydx.AccountInfo[] storage accounts)
    {
        uint256[] storage accountNumbers = bridgeData.accountNumbers;
    ...
    function _createActions(
        ...
        internal
        returns (IDydx.ActionArgs[] storage actions)
    {
        BridgeAction[] storage bridgeActions = bridgeData.actions;
        ...
        for (uint256 i = 0; i < nBridgeActions; ++i) {
            ...
            BridgeAction storage bridgeAction = bridgeActions[i];
