...
library InteractiveCrowdsaleLib {
  ...
  function numDigits(uint256 _number) external pure returns (uint256) {
    uint256 _digits = 0;
    while (_number != 0) {
      _number /= 10;
      _digits++;
  ...
  function getCurrentBonus(InteractiveCrowdsaleStorage storage self) public view returns (uint256){
  ...
  function submitBid(InteractiveCrowdsaleStorage storage self,
                      ...
                      uint256 _valuePredict) external returns (bool)
  {
  ...
  function withdrawBid(InteractiveCrowdsaleStorage storage self) external returns (bool) {
