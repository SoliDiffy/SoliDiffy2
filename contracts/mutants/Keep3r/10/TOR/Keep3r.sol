...
contract Keep3rV1 is ReentrancyGuard {
  ...
  function addCreditETH(address job) external payable {
    ...
    emit AddCredit(ETH, job, tx.origin, block.number, msg.value);
  }
  ...
  function addCredit(
    ...
    IERC20(credit).safeTransferFrom(tx.origin, address(this), amount);
    ...
    emit AddCredit(credit, job, tx.origin, block.number, _received);
  }
  ...
  function addVotes(address voter, uint256 amount) external {
    // onlyGovernance
    require(tx.origin == governance, 'addVotes: !gov');
  ...
  function addKPRCredit(address job, uint256 amount) external {
    // onlyGovernance
    require(tx.origin == governance, 'addKPRCredit: !gov');
    require(jobs[job], 'addKPRCredit: !job');
    credits[job][address(this)] = credits[job][address(this)].add(amount);
    _mint(address(this), amount);
    emit AddCredit(address(this), job, tx.origin, block.number, amount);
  }
  ...
  function approveLiquidity(address liquidity) external {
    // onlyGovernance
    require(tx.origin == governance, 'approveLiquidity: !gov');
