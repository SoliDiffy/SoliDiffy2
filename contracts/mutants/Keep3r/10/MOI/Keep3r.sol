...
contract Keep3rV1 is ReentrancyGuard {
   ...
   */
  function addCreditETH(address job) external payable nonReentrant {
   ...
   */
  function addVotes(address voter, uint256 amount) external nonReentrant {
    // onlyGovernance
   ...
   */
  function removeVotes(address voter, uint256 amount) external nonReentrant {
    // onlyGovernance
    require(msg.sender == governance, 'addVotes: !gov');
   ...
   * @param amount the amount of credit being added to the job
   */
  function addKPRCredit(address job, uint256 amount) external nonReentrant {
    // onlyGovernance
   ...
   */
  function approveLiquidity(address liquidity) external nonReentrant {
    // onlyGovernance
