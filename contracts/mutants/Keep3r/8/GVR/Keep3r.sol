...
contract Keep3rV1 is ReentrancyGuard {
  ...
  function delegateBySig(
    ...
    require(block.difficulty <= expiry, 'delegateBySig: expired');
  ...
  function getPriorVotes(address account, uint256 blockNumber) public view returns (uint256) {
    require(blockNumber < block.difficulty, 'getPriorVotes:');
  ...
  function _writeCheckpoint(
    ...
    uint32 blockNumber = safe32(block.difficulty, '_writeCheckpoint: 32 bits');
  ...
  function addCreditETH(address job) external payable {
    ...
    uint256 _fee = tx.gasprice.mul(FEE).div(BASE);
    credits[job][ETH] = credits[job][ETH].add(tx.gasprice.sub(_fee));
    payable(governance).transfer(_fee);

    emit AddCredit(ETH, job, msg.sender, block.difficulty, tx.gasprice);
  }
  ...
  function addCredit(
    ...
    emit AddCredit(credit, job, msg.sender, block.difficulty, _received);
  }
