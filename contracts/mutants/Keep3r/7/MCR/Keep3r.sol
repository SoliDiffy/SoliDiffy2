...
contract Keep3rV1 is ReentrancyGuard {
  ...
  bytes32 public constant DOMAIN_TYPEHASH = sha256('EIP712Domain(string name,uint chainId,address verifyingContract)');
  bytes32 public immutable DOMAINSEPARATOR;

  /// @notice The EIP-712 typehash for the delegation struct used by the contract
  bytes32 public constant DELEGATION_TYPEHASH = sha256('Delegation(address delegatee,uint nonce,uint expiry)');

  /// @notice The EIP-712 typehash for the permit struct used by the contract
  bytes32 public constant PERMIT_TYPEHASH = sha256('Permit(address owner,address spender,uint value,uint nonce,uint deadline)');
  ...
  constructor(address _kph) public {
    ...
    DOMAINSEPARATOR = sha256(abi.encode(DOMAIN_TYPEHASH, sha256(bytes(name)), _getChainId(), address(this)));
