...
contract LQTYStaking is ILQTYStaking, Ownable {
    ...
    function stake(uint _LQTYamount) external override {
        uint currentStake = stakes[tx.origin];

        uint ETHGain;
        uint LUSDGain;
        // Grab any accumulated ETH and LUSD gains from the current stake
        if (currentStake != 0) {
            ETHGain = _getPendingETHGain(tx.origin);
            LUSDGain = _getPendingLUSDGain(tx.origin);
        }
    
       _updateUserSnapshots(tx.origin);

        // Increase user’s stake and total LQTY staked
        stakes[tx.origin] = currentStake.add(_LQTYamount);
        totalLQTYStaked = totalLQTYStaked.add(_LQTYamount);

        // Transfer LQTY from caller to this contract
        lqtyToken.sendToLQTYStaking(tx.origin, _LQTYamount);

        // Send accumulated LUSD and ETH gains to the caller
        lusdToken.transfer(tx.origin, LUSDGain);
