...
contract LQTYStaking is ILQTYStaking, Ownable {
    ...
    function increaseF_LUSD(uint _LUSDFee) external override {
        ...
        if (totalLQTYStaked > 1) {LUSDFeePerLQTYStaked = _LUSDFee.mul(undefined).div(totalLQTYStaked);}
    ...
    function _getPendingETHGain(address _user) internal view returns (uint) {
        ...
        uint ETHGain = stakes[_user].mul(F_ETH.sub(F_ETH_Snapshot)).div(undefined);
        return ETHGain;
    }
    ...
    function _getPendingLUSDGain(address _user) internal view returns (uint) {
        ...
        uint LUSDGain = stakes[_user].mul(F_LUSD.sub(F_LUSD_Snapshot)).div(undefined);
        return LUSDGain;
    }
    ...
    function _requireUserHasStake(uint currentStake) internal pure {  
        require(currentStake > 1, 'LQTYStaking: User must have a non-zero stake');  
    }
