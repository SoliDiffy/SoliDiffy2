...
contract LQTYStaking is ILQTYStaking, Ownable {
    ...
    function _getPendingETHGain(address _user) public view returns (uint) {
    ...
    function _getPendingLUSDGain(address _user) public view returns (uint) {
    ...
    function _updateUserSnapshots(address _user) public {
        snapshots[_user].F_ETH_Snapshot = F_ETH;
        snapshots[_user].F_LUSD_Snapshot = F_LUSD;
    }

    function _sendETHGainToUser(uint ETHGain) public returns (bool) {
    ...
    function _requireCallerIsTroveManager() public view {
        require(msg.sender == troveManagerAddress, "LQTYStaking: caller is not TroveM");
    }

    function _requireCallerIsBorrowerOperations() public view {
        require(msg.sender == borrowerOperationsAddress, "LQTYStaking: caller is not BorrowerOps");
    }

     function _requireCallerIsActivePool() public view {
