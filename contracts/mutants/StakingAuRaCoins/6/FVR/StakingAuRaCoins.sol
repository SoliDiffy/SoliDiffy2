...
contract StakingAuRaCoins is StakingAuRaBase {
    ...
    function getRewardAmount(
        ...
        address _poolStakingAddress,
        address _staker
    ) external view returns(uint256 rewardSum) {
        uint256 poolId = validatorSetContract.idByStakingAddress(_poolStakingAddress);
    ...
    function _sendWithdrawnStakeAmount(address payable _to, uint256 _amount) public gasPriceIsValid onlyInitialized {
        if (!_to.send(_amount)) {
    ...
    function _stake(address _toPoolStakingAddress, uint256 _amount) public {
    ...
    function _thisBalance() public view returns(uint256) {
