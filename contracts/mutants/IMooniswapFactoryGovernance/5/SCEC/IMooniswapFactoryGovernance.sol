...
interface IMooniswapFactoryGovernance {
    ...
    function shareParameters() external view returns(uint256 governanceShare, uint256 referralShare, address referralFeeReceiver, address governanceWallet);
    ...
    function defaults() external view returns(uint256 defaultDecayPeriod, uint256 defaultFee, uint256 defaultSlippageFee);
    ...
    function virtualDefaultFee() external view returns(uint48, uint104, uint104);
    /// @notice Describes previous default slippage fee that had place, current one and time on which this changed
    function virtualDefaultSlippageFee() external view returns(uint48, uint104, uint104);
    /// @notice Describes previous default decay amount that had place, current one and time on which this changed
    function virtualDefaultDecayPeriod() external view returns(uint48, uint104, uint104);
