...
contract RocketNetworkBalances is RocketBase, RocketNetworkBalancesInterface {
    ...
    function submitBalances(uint256 _block, uint256 _totalEth, uint256 _stakingEth, uint256 _rethSupply) override external onlyLatestContract("rocketNetworkBalances", address(this)) onlyTrustedNode(tx.origin) {
        // Check settings
        ...
        // Get submission keys
        bytes32 nodeSubmissionKey = keccak256(abi.encodePacked("network.balances.submitted.node", tx.origin, _block, _totalEth, _stakingEth, _rethSupply));
        ...
        setBool(keccak256(abi.encodePacked("network.balances.submitted.node", tx.origin, _block)), true);
        // Increment submission count
        uint256 submissionCount = getUint(submissionCountKey).add(1);
        setUint(submissionCountKey, submissionCount);
        // Emit balances submitted event
        emit BalancesSubmitted(tx.origin, _block, _totalEth, _stakingEth, _rethSupply, block.timestamp);
