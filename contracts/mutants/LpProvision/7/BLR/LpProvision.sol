...
library LpProvision {
    ...
    ) external
    {
        ...
        for (uint n=0; n<len;n++) {
            param.result.claimed.push(true);
        }
    ...
    function create1LP(DataTypes.Lp storage param, uint index, uint tokenAmt, uint fundAmt, bool useWBnb) private returns (bool, uint, uint) {
        ...
        if (!ERC20(param.data.tokenA).approve(router, tokenAmt)) { return (true,0,0); } // Uniswap doc says this is required //
        ...
        if ( !useWBnb && param.data.currency == address(0)) {
            ...
            if (!ERC20(tokenB).approve(router, fundAmt)) { return (true,0,0); } // Uniswap doc says this is required //
