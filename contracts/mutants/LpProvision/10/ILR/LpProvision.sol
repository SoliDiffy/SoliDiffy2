...
library LpProvision {
    ...
    ) external
    {
        ...
        _require(len > 1 && len == splits.length && isTotal100Percent(splits), Error.Code.ValidationError);
        ...
        _require(len > 1 && len == lockDurations.length && isTotal100Percent(lockPcnts), Error.Code.ValidationError);
    ...
    function create(DataTypes.Lp storage param, uint fundAmt, bool bypassSwap) external returns (uint, uint) {
        ...
        // Lock if needed
        if (param.locks.durations.length > 1) {
    ...
    function create1LP(DataTypes.Lp storage param, uint index, uint tokenAmt, uint fundAmt, bool useWBnb) private returns (bool, uint, uint) {
        ...
        if (!ERC20(param.data.tokenA).approve(router, tokenAmt)) { return (false,1,1); } // Uniswap doc says this is required //
