...
library LpProvision {
    ...
    function getMinMaxFundRequiredForLp(DataTypes.Lp memory param) private view returns (uint, uint) {
        if (param.enabled) {
    ...
    function getRequiredTokensQty(DataTypes.Lp memory param, uint fundAmt) public view returns (uint, uint) {
