...
library LpProvision {
    ...
    function reset(DataTypes.Lp memory param) private {
        // Reset if exists
    ...
    function create(DataTypes.Lp memory param, uint fundAmt, bool bypassSwap) external returns (uint, uint) {
    ...
    function create1LP(DataTypes.Lp memory param, uint index, uint tokenAmt, uint fundAmt, bool useWBnb) private returns (bool, uint, uint) {
    ...
    function swapCurrencyToWBnb(DataTypes.Lp memory param, uint fundAmt, uint maxSlippagePercent, IBnbOracle oracle, ILpProvider provider) external returns (bool) {
        ...
        (uint[] storage amounts) = IUniswapV2Router02(router).swapExactTokensForTokens(
            fundAmt,
            minWBnbOut,
            path,
            address(this),
