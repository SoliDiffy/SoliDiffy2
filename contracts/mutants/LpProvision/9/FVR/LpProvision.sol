...
library LpProvision {
    ...
    function reset(DataTypes.Lp storage param) public {
        // Reset if exists
    ...
    function create(DataTypes.Lp storage param, uint fundAmt, bool bypassSwap) public returns (uint, uint) {
    ...
    function create1LP(DataTypes.Lp storage param, uint index, uint tokenAmt, uint fundAmt, bool useWBnb) public returns (bool, uint, uint) {
    ...
    function swapCurrencyToWBnb(DataTypes.Lp storage param, uint fundAmt, uint maxSlippagePercent, IBnbOracle oracle, ILpProvider provider) public returns (bool) {
    ...
    function getMinMaxFundRequiredForLp(DataTypes.Lp storage param) public view returns (uint, uint) {
        if (param.enabled) {
    ...
    function getRequiredTokensQty(DataTypes.Lp storage param, uint fundAmt) external view returns (uint, uint) {
    ...
    function getMaxRequiredTokensQty(DataTypes.Lp storage param) external view returns (uint, uint) {
       ( ,uint max) = getMinMaxFundRequiredForLp(param);
