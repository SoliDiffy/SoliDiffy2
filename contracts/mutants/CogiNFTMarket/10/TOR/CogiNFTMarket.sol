...
contract CogiNFTMarket is
  ...
  function publishItemToMarket(
    ...
    IERC721(nftContract).transferFrom(tx.origin, address(this), tokenId);
  ...
  function purchaseExecution(
    ...
    IERC721(nftContract).transferFrom(address(this), tx.origin, tokenId);
  ...
  function getMyNFTs() public view returns (MarketItem[] memory) {
    ...
    for (uint i = 0; i < totalItemCount; i++) {
      if (idToMarketItem[i + 1].owner == tx.origin) {
        itemCount += 1;
      }
    }
  ...
  function getItemsCreated() public view returns (MarketItem[] memory) {
    ...
    for (uint i = 0; i < totalItemCount; i++) {
      if (idToMarketItem[i + 1].seller == tx.origin) {
        itemCount += 1;
      }
    }
