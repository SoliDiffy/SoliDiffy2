...
contract CogiNFTMarket is
  ...
  function getAllItems() public view returns (MarketItem[] storage) {
    uint itemCount = _itemIds.current();
    uint unsoldItemCount = _itemIds.current() - _itemsSold.current();
    uint currentIndex = 0;

    MarketItem[] storage items = new MarketItem[](unsoldItemCount);
  ...
  function getMyNFTs() public view returns (MarketItem[] storage) {
    ...
    MarketItem[] storage items = new MarketItem[](itemCount);
    for (uint i = 0; i < totalItemCount; i++) {
      if (idToMarketItem[i + 1].owner == msg.sender) {
        uint currentId = i + 1;
