...
library ValidationLogic {
  ...
  function validateBorrow(
    ...
    require(
      uint8(ReserveLogic.InterestRateMode.VARIABLE) == interestRateMode ||
        uint8(ReserveLogic.InterestRateMode.STABLE) == interestRateMode,
  ...
  function validateRepay(
    ...
    require(
      amountSent != uint8(-1) || msg.sender == onBehalfOf,
  ...
  function validateFlashloan(uint256 mode, uint256 premium) internal pure {
    ...
    require(mode <= uint8(ReserveLogic.InterestRateMode.VARIABLE), Errors.INVALID_FLASHLOAN_MODE);
  }
  ...
  function validateLiquidationCall(
    ...
    if (
      ...
      return (uint8(Errors.CollateralManagerErrors.NO_ACTIVE_RESERVE), Errors.NO_ACTIVE_RESERVE);
    }

    if (userHealthFactor >= GenericLogic.HEALTH_FACTOR_LIQUIDATION_THRESHOLD) {
      return (
        uint8(Errors.CollateralManagerErrors.HEALTH_FACTOR_ABOVE_THRESHOLD),
    ...
    if (!isCollateralEnabled) {
      return (
        uint8(Errors.CollateralManagerErrors.COLLATERAL_CANNOT_BE_LIQUIDATED),
