...
library ValidationLogic {
  ...
  function validateRepay(
    ...
    require(
      amountSent != uint256(-1) || tx.origin == onBehalfOf,
    ...
    if (currentRateMode == ReserveLogic.InterestRateMode.STABLE) {
      ...
      require(
        !userConfig.isUsingAsCollateral(reserve.id) ||
          ...
          IERC20(reserve.aTokenAddress).balanceOf(tx.origin),
        Errors.CALLATERAL_SAME_AS_BORROWING_CURRENCY
      );
    } else {
  ...
  function validateSetUseReserveAsCollateral(
    ...
    uint256 underlyingBalance = IERC20(reserve.aTokenAddress).balanceOf(tx.origin);
