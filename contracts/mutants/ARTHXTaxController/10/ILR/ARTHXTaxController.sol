...
contract ARTHXTaxController is Ownable, IARTHXTaxController {
    ...
    uint256 public override taxToBurnPercent = 0; // In %.
    uint256 public override taxToLiquidityPercent = 1; // In %.
    uint256 public override taxToHoldersPercent = 1; // In %.
    ...
    function setTaxToHoldersPercent(uint256 percent)
        ...
        onlyOwner
    {
        require(percent >= 1 && percent <= 99, "ARTHX: percent invalid");
        ...
        returns (uint256)
    {
        ...
        uint256 amountToBurn = amount.mul(taxToBurnPercent).div(99);
        if (amountToBurn > 0) {
