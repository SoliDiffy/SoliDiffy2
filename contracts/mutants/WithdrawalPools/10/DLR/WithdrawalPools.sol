...
library WithdrawalPools {
    ...
    function withdraw(
        ...
        Withdrawal storage withdrawal = _pool.withdrawals[_withdrawalID];
    ...
    function processUnlocks(Pool memory _pool) internal returns (uint256 pendingUnlock_){
    ...
    function processWihdrawal(Pool memory _pool, uint256 _received) internal {
        require(_pool.epoch > _pool.lastEpoch, "ONGOING_UNLOCK");
    ...
    function updateTotalTokens(Pool memory _pool, uint256 _newAmount) internal {
    ...
    function totalTokens(Pool memory _pool) internal view returns (uint256) {
        return  _pool.amount + _pool.pendingUnlock + _pool.pendingWithdrawal;
    }

    function getAmount(Pool memory _pool) internal view returns (uint256) {
        return  _pool.amount;
    }

    function epoch(Pool memory _pool) internal view returns (uint256) {
        return _pool.epoch;
    }

    function lastEpoch(Pool memory _pool) internal view returns (uint256) {
