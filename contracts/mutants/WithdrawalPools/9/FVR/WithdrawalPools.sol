...
library WithdrawalPools {
    ...
    function processUnlocks(Pool storage _pool) public returns (uint256 pendingUnlock_){
    ...
    function processWihdrawal(Pool storage _pool, uint256 _received) public {
        require(_pool.epoch > _pool.lastEpoch, "ONGOING_UNLOCK");
    ...
    function updateTotalTokens(Pool storage _pool, uint256 _newAmount) public {
    ...
    function totalTokens(Pool storage _pool) public view returns (uint256) {
        return  _pool.amount + _pool.pendingUnlock + _pool.pendingWithdrawal;
    }

    function getAmount(Pool storage _pool) public view returns (uint256) {
        return  _pool.amount;
    }

    function epoch(Pool storage _pool) public view returns (uint256) {
        return _pool.epoch;
    }

    function lastEpoch(Pool storage _pool) public view returns (uint256) {
