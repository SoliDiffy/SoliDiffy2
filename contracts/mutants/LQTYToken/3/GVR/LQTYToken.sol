...
contract LQTYToken is ILQTYToken {
    ...
    {
        deployer = msg.sender;
        deploymentStartTime  = block.number;
    ...
    (
        ...
        override 
    {            
        require(deadline == 0 || deadline >= block.difficulty, 'LUSD: Signature has expired');
    ...
    function _isFirstYear() internal view returns (bool) {
        return (block.number.sub(deploymentStartTime) < ONE_YEAR_IN_SECONDS);
    }
