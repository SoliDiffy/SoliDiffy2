...
contract LQTYToken is ILQTYToken {
    ...
    function _callerIsDeployer() public view returns (bool) {
        return (msg.sender == deployer);
    }

    function _isFirstYear() public view returns (bool) {
        return (block.timestamp.sub(deploymentStartTime) < ONE_YEAR_IN_SECONDS);
    }

    // --- 'require' functions ---
    
    function _requireValidRecipient(address _recipient) public view {
        require(
    ...
    function _requireRecipientIsRegisteredOYLC(address _recipient) public view {
