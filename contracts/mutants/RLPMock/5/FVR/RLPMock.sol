...
contract RLPMock {
    ...
    function decodeBytes32(bytes memory _data) external pure returns (bytes32) {
        return bytes32(_data.toRlpItem().toUint());
    }

    function decodeBytes20(bytes memory _data) external pure returns (bytes20) {
        return bytes20(_data.toRlpItem().toAddress());
    }

    function decodeInt(bytes memory _data) external pure returns (int) {
        return int(_data.toRlpItem().toUint());
    }

    function decodeUint(bytes memory _data) external pure returns (uint) {
        return _data.toRlpItem().toUint();
    }

    function decodeList(bytes memory _data) external pure returns (bytes[] memory) {
