...
contract DoubleSigningSlasher is ICeloVersionedContract, SlasherUtil {
  ...
  */
  ...
  function checkForDoubleSigning(
    address signer,
    uint256 index,
    bytes storage headerA,
    bytes storage headerB
  ...
  function checkIfAlreadySlashed(address signer, bytes storage header) internal {
  ...
  function slash(
    ...
    address[] storage validatorElectionLessers,
    address[] storage validatorElectionGreaters,
    uint256[] storage validatorElectionIndices,
