...
contract ServiceProviderFactory is InitializableV2 {
    ...
    ) external initializer returns (uint spID)
    {
    ...
    function deregister(
        bytes32 _serviceType,
        string calldata _endpoint
    ) external initializer returns (uint deregisteredSpID)
    {
        _requireIsInitialized();
    ...
    ) external initializer returns (uint newTotalStake)
    {
        _requireIsInitialized();
    ...
    external initializer returns (uint newStakeAmount)
    {
        _requireIsInitialized();
        require(
            !_claimPending(msg.sender),
     ...
     */
    function decreaseStake() external initializer returns (uint newTotalStake)
    {
        _requireIsInitialized();
