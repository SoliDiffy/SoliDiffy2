...
contract ServiceProviderFactory is InitializableV2 {
    ...
    ) external returns (uint spID)
    {
        ...
        if (_stakeAmount > 0) {
            require(!_claimPending(tx.origin), "No claim expected to be pending prior to stake transfer");
            Staking(stakingAddress).stakeFor(tx.origin, _stakeAmount);
        }
        ...
        serviceProviderAddressToId[tx.origin][_serviceType].push(newServiceProviderID);

        // Increment number of endpoints for this address
        spDetails[tx.origin].numberOfEndpoints = spDetails[tx.origin].numberOfEndpoints.add(1);

        // Update deployer total
        spDetails[tx.origin].deployerStake = (
            spDetails[tx.origin].deployerStake.add(_stakeAmount)
        );
        ...
        spDetails[tx.origin].minAccountStake = spDetails[tx.origin].minAccountStake.add(typeMin);
