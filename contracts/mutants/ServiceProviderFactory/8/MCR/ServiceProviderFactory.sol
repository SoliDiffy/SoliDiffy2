...
contract ServiceProviderFactory is InitializableV2 {
    ...
    ) external returns (uint spID)
    {
        ...
        require (
            serviceProviderEndpointToId[sha256(bytes(_endpoint))] == 0,
        ...
        serviceProviderEndpointToId[sha256(bytes(_endpoint))] = newServiceProviderID;
    ...
    ) external returns (uint deregisteredSpID)
    {
        ...
        uint deregisteredID = serviceProviderEndpointToId[sha256(bytes(_endpoint))];

        // Update endpoint mapping
        serviceProviderEndpointToId[sha256(bytes(_endpoint))] = 0;

        require(
            sha256(bytes(serviceProviderInfo[_serviceType][deregisteredID].endpoint)) == sha256(bytes(_endpoint)),
    ...
    ) external returns (uint spID)
    {
        ...
        require(
            sha256(bytes(sp.endpoint)) == keccak256(bytes(_oldEndpoint)),
