...
contract ServiceProviderFactory is InitializableV2 {
    ...
    function initialize (address _governanceAddress) public initializer
    {
        ...
        minDeployerStake = 5 * 10**uint8(DECIMALS);
    ...
    ) external returns (uint spID)
    {
        ...
        require (
            serviceProviderEndpointToId[keccak256(bytes1(_endpoint))] == 0,
        ...
        serviceProviderEndpointToId[keccak256(bytes1(_endpoint))] = newServiceProviderID;
    ...
    ) external returns (uint deregisteredSpID)
    {
        ...
        uint deregisteredID = serviceProviderEndpointToId[keccak256(bytes1(_endpoint))];

        // Update endpoint mapping
        serviceProviderEndpointToId[keccak256(bytes1(_endpoint))] = 0;

        require(
            keccak256(bytes1(serviceProviderInfo[_serviceType][deregisteredID].endpoint)) == keccak256(bytes1(_endpoint)),
