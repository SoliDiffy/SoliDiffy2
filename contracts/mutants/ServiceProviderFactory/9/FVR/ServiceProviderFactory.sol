...
contract ServiceProviderFactory is InitializableV2 {
     ...
     */
    function initialize (address _governanceAddress) external initializer
    {
    ...
    function deregister(
        bytes32 _serviceType,
        string calldata _endpoint
    ) public returns (uint deregisteredSpID)
    {
        _requireIsInitialized();
    ...
    ) public returns (uint newTotalStake)
    {
        _requireIsInitialized();
    ...
    public returns (uint newStakeAmount)
    {
        _requireIsInitialized();
        require(
            !_claimPending(msg.sender),
     ...
     */
    function cancelDecreaseStakeRequest(address _account) public
    {
        require(
     ...
     */
    function decreaseStake() public returns (uint newTotalStake)
    {
        _requireIsInitialized();
