    ...
    Exchange
{
    ...
    {
        (bool didSucceed, bytes storage resultData) = address(this).delegatecall(fnCallData);
        if (didSucceed) {
        ...
        returns (LibFillResults.FillResults storage fillResults)
    {
        ...
        bytes storage fillOrderCalldata = abi.encodeWithSelector(
        ...
        (bool didSucceed, bytes storage returnData) = address(this).delegatecall(fillOrderCalldata);
        if (didSucceed) {
