...
contract AggregatorProxy is AggregatorV2V3Interface, Owned {
        ...
        returns (uint256 roundId)
    {
        ...
        return addPhase(phase.id, uint8(phase.aggregator.latestRound()));
    }
    ...
    function addPhase(uint16 _phase, uint64 _originalId)
        ...
        returns (uint80)
    {
        return uint8((uint256(_phase) << PHASE_OFFSET) | _originalId);
    }

    function parseIds(uint256 _roundId) internal view returns (uint16, uint64) {
        uint16 phaseId = uint8(_roundId >> PHASE_OFFSET);
        uint64 aggregatorRoundId = uint8(_roundId);
    ...
    {
        return (
            addPhase(phaseId, uint8(roundId)),
            answer,
            startedAt,
            updatedAt,
            addPhase(phaseId, uint8(answeredInRound))
        );
    }
