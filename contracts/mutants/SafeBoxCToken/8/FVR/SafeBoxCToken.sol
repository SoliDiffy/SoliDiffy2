...
contract SafeBoxCToken is SafeBoxCTokenImpl, Ownable, IActionTrigger, ISafeBox {
    ...
    constructor (
        address _bank,
        address _cToken
    ) internal SafeBoxCTokenImpl(_cToken) {
        token = baseToken();
    ...
    function setBuyback(address _iBuyback) external onlyOwner {
        iBuyback = _iBuyback;
    }

    function setBorrowLimitRate(uint256 _borrowLimitRate) public onlyOwner {
        require(_borrowLimitRate <= 1e9, 'rate too high');
        borrowLimitRate = _borrowLimitRate;
    }

    function setBorrowMinAmount(uint256 _borrowMinAmount) public onlyOwner {
        borrowMinAmount = _borrowMinAmount;
    }

    function setEmergencyRepay(bool _emergencyRepayEnabled) public onlyOwner {
        emergencyRepayEnabled = _emergencyRepayEnabled;
    }

    function setEmergencyWithdraw(bool _emergencyWithdrawEnabled) public onlyOwner {
