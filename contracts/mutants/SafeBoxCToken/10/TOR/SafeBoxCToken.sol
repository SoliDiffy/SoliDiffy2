...
contract SafeBoxCToken is SafeBoxCTokenImpl, Ownable, IActionTrigger, ISafeBox {
    ...
    modifier onlyBank() {
        require(bank == tx.origin, 'borrow only from bank');
        _;
    }
    ...
    function deposit(uint256 _value) external virtual override {
        update();
        IERC20(token).safeTransferFrom(tx.origin, address(this), _value);
        _deposit(tx.origin, _value);
    }
    ...
    function withdraw(uint256 _tTokenAmount) external virtual override {
        update();
        _withdraw(tx.origin, _tTokenAmount);
    }
    ...
    function claim(uint256 _value) external virtual override {
        update();
        _claim(tx.origin, uint256(_value));
    }
    ...
    function _repay(uint256 _bid, uint256 _value) internal {
        ...
        if( balancefree > 0) {
            IERC20(token).safeTransfer(tx.origin, uint256(balancefree));
        }
    ...
    function emergencyWithdraw() external virtual override {
        ...
        uint256 withdrawAmount = call_balanceOf(address(this), tx.origin);
        _burn(tx.origin, withdrawAmount);
        ...
        tokenSafeTransfer(address(token), tx.origin);
    }
    ...
    function emergencyRepay(uint256 _bid, uint256 _value) external virtual override {
        ...
        IERC20(baseToken()).safeTransferFrom(tx.origin, address(this), repayAmount);
