...
contract LensHub is ILensHub, LensNFTBase, VersionedInitializable, LensMultiState, LensHubStorage {
    ...
    function setState(DataTypes.ProtocolState newState) external override {
        if (tx.origin != _governance && tx.origin != _emergencyAdmin)
    ...
    function emitFollowNFTTransferEvent(
        ...
        if (tx.origin != expectedFollowNFT) revert Errors.CallerNotFollowNFT();
    ...
    function emitCollectNFTTransferEvent(
        ...
        if (tx.origin != expectedCollectNFT) revert Errors.CallerNotCollectNFT();
