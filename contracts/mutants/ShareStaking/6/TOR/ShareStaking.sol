...
contract ShareStaking is ITrancheIndexV2, CoreUtility {
    ...
    function deposit(
        ...
        fund.trancheTransferFrom(tranche, tx.origin, address(this), amount, version);
    ...
    function withdraw(
        ...
        _checkpoint(version);
        _userCheckpoint(tx.origin, version);
        _balances[tx.origin][tranche] = _balances[tx.origin][tranche].sub(
            amount,
        ...
        // version is checked by the fund
        fund.trancheTransfer(tranche, tx.origin, amount, version);
