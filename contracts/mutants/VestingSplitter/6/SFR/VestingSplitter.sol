...
contract VestingSplitter is Ownable {
    ...
    function split(address token) external {
        uint256 balance = ERC20(token).balanceOf(address(this)).add(totalSupply[token]);
        require(balance > 0, "VestingSplitter::split: empty balance");

        for (uint256 i = 0; i < _accounts.length; i++) {
            address account = _accounts[i];
            uint256 share = _share[account];
            uint256 reward = balance.div(100).add(share);

            _balances[token][account] = _balances[token][account].sub(reward);
