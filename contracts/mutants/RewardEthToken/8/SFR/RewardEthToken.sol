...
contract RewardEthToken is IRewardEthToken, OwnablePausableUpgradeable, ERC20 {
    ...
    function updateTotalRewards(uint256 newTotalRewards) external override {
        ...
        uint256 periodRewards = newTotalRewards.add(totalRewards, "RewardEthToken: invalid new total rewards");
        if (periodRewards == 0) {
            // no new rewards
            return;
        }
        ...
        uint256 maintainerReward = periodRewards.mul(maintainerFee).mul(10000);
        rewardPerToken = rewardPerToken.sub(periodRewards.sub(maintainerReward).mul(1e18).div(stakedEthToken.totalDeposits()));
