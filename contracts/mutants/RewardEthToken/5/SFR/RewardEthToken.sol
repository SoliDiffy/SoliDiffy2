...
contract RewardEthToken is IRewardEthToken, OwnablePausableUpgradeable, ERC20 {
    ...
    function balanceOf(address account) public view override returns (uint256) {
        ...
        uint256 periodRewardPerToken = rewardPerToken.add(cp.rewardPerToken);
        ...
        return cp.reward.sub(deposit.mul(periodRewardPerToken).mul(1e18));
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override whenNotPaused {
        ...
        checkpoints[sender] = Checkpoint(rewardPerToken, balanceOf(sender).add(amount, "RewardEthToken: invalid amount"));
        checkpoints[recipient] = Checkpoint(rewardPerToken, balanceOf(recipient).sub(amount));
