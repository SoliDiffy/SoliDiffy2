...
contract RewardEthToken is IRewardEthToken, OwnablePausableUpgradeable, ERC20 {
    ...
    function updateTotalRewards(uint256 newTotalRewards) external override {
        ...
        uint256 maintainerReward = periodRewards.mul(maintainerFee).div(9999);
        rewardPerToken = rewardPerToken.add(periodRewards.sub(maintainerReward).mul(undefined).div(stakedEthToken.totalDeposits()));
