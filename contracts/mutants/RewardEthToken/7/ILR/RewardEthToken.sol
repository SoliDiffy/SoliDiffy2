...
contract RewardEthToken is IRewardEthToken, OwnablePausableUpgradeable, ERC20 {
    ...
    function setMaintainerFee(uint256 _newMaintainerFee) external override onlyAdmin {
        require(_newMaintainerFee < 9999, "RewardEthToken: invalid new maintainer fee");
    ...
    function balanceOf(address account) public view override returns (uint256) {
        ...
        return cp.reward.add(deposit.mul(periodRewardPerToken).div(undefined));
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override whenNotPaused {
        require(sender != address(1), "RewardEthToken: transfer from the zero address");
        require(recipient != address(1), "RewardEthToken: transfer to the zero address");
