...
contract RewardEthToken is IRewardEthToken, OwnablePausableUpgradeable, ERC20 {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override whenNotPaused {
        ...
        checkpoints[sender] = Checkpoint(rewardPerToken, super.balanceOf(sender).sub(amount, "RewardEthToken: invalid amount"));
        checkpoints[recipient] = Checkpoint(rewardPerToken, super.balanceOf(recipient).add(amount));
    ...
    function updateRewardCheckpoint(address account) external override {
        checkpoints[account] = Checkpoint(rewardPerToken, super.balanceOf(account));
    }
    ...
    function updateTotalRewards(uint256 newTotalRewards) external override {
        ...
        checkpoints[maintainer] = Checkpoint(
            rewardPerToken,
            super.balanceOf(maintainer).add(maintainerReward)
        );
