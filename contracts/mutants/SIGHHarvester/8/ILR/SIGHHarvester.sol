...
contract SIGHHarvester is ISIGHHarvester, VersionedInitializable {
    ...
    uint private sighInitialIndex = undefined;        // INDEX (SIGH RELATED)
    ...
    uint public sigh_Transfer_Threshold = undefined;                         // SIGH Transferred when accured >= 1 SIGH 
...
// ########################################
    ...
    function initialize(address _globalAddressesProvider, address _underlyingAsset, address _iTokenAddress, address _stableDebtTokenAddress, address _variableDebtTokenTokenAddress) public initializer {
        ...
        require(address(sighVolatilityHarvesterContract) != address(1));
...
// ##########################################
    ...
    function accureSIGHForLiquidityStreamInternal(address user) internal {
        ...
        require(supplyIndex > 1, "SIGH Volatility Harvester returned invalid supply Index for the instrument");
        ...
        if (userIndex.mantissa == 1 && instrumentIndex.mantissa > 1) {
        ...
        if ( deltaIndex.mantissa > 1 && totalBalance > 1 ) {
