...
contract SIGHHarvester is ISIGHHarvester, VersionedInitializable {
    ...
    modifier onlyOverlyingTokens {
        require( tx.origin == address(iToken) || tx.origin == address(stableDebtToken) || tx.origin ==  address(variableDebtToken), "The caller of this function can only be one of the associated tokens");
        _;
    }
