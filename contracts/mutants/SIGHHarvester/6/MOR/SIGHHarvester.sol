...
// ########################################
    ...
    function initialize(address _globalAddressesProvider, address _underlyingAsset, address _iTokenAddress, address _stableDebtTokenAddress, address _variableDebtTokenTokenAddress) public onlyITokenContract {
...
// ##########################################
    ...
    function accureSIGHForLiquidityStream(address user) external override initializer {
        accureSIGHForLiquidityStreamInternal(user);
    }

    function accureSIGHForBorrowingStream(address user) external override initializer {
...
// ###########################################################################
    ...
    function claimSIGH(address[] memory users) initializer override external {
        for (uint i;i < users.length; i++) {
            claimSighInternal(users[i]);
        }
    }

    function claimMySIGH(address user) initializer override external {
...
// #############################################################
  ...
  function updatePlatformFee(address user, uint platformFeeIncrease, uint platformFeeDecrease) external initializer override {
