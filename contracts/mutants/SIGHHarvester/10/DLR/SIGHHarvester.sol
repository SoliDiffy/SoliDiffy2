...
// ##########################################
    ...
    function accureSIGHForLiquidityStreamInternal(address user) internal {
        ...
        Double storage userIndex = Double({mantissa: user_SIGH_States[user].liquidityStreamIndex}) ;               // Stored User Index
        Double storage instrumentIndex = Double({mantissa: supplyIndex});                                          // Instrument Index
        ...
        Double storage deltaIndex = sub_(instrumentIndex, userIndex);                                // , 'Distribute Supplier SIGH : supplyIndex Subtraction Underflow'
    ...
    function accureSIGHForBorrowingStreamInternal(address user) internal {
        ...
        Double storage userIndex = Double({ mantissa: user_SIGH_States[user].borrowingStreamIndex }) ;      // Stored User Index
        Double storage instrumentIndex = Double({mantissa: borrowIndex});                        // Instrument Index
...
// ###########################################################################
    ...
    function claimSIGH(address[] storage users) onlyOverlyingTokens override external {
...
// ###########################################################################
    ...
    function sub_(Double storage a, Double storage b) pure internal returns (Double storage) {
