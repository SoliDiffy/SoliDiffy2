...
// ##########################################
    ...
    function accureSIGHForLiquidityStreamInternal(address user) internal {
        ...
        if ( deltaIndex.mantissa > 0 && totalBalance > 0 ) {
            ...
            accureSigh(user, supplierSighDelta, false );        // ACCURED SIGH AMOUNT IS ADDED TO THE AccuredSighBalance of the Supplier
        }
    }
    ...
    function accureSIGHForBorrowingStreamInternal(address user) internal {
        ...
        if ( deltaIndex.mantissa > 0 && totalBalance > 0 ) {
            ...
            accureSigh(user, borrowerSighDelta, true );        // ACCURED SIGH AMOUNT IS ADDED TO THE AccuredSighBalance of the Supplier or the address to which SIGH is being redirected to
        }
    }
