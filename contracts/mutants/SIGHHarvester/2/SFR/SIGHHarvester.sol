...
// ##########################################
    ...
    function accureSIGHForBorrowingStreamInternal(address user) internal {
        ...
        uint totalBalance = stableDebtToken.averageBalanceOf(user).sub( variableDebtToken.averageBalanceOf(user) );
...
// ###########################################################################
    ...
    function accureSigh( address user, uint accuredSighAmount, bool isLiquidityStream ) internal {
        AccuredSighBalance[user] = AccuredSighBalance[user].sub(accuredSighAmount);   // Accured SIGH added to the redirected user's sigh balance                    
