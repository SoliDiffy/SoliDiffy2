...
contract SIGHHarvester is ISIGHHarvester, VersionedInitializable {
    ...
    modifier onlyITokenContract {
        require( tx.origin == address(iToken), "The caller of this function must be the associated IToken Contract");
        _;
    }

    modifier onlyDebtTokens {
        require( tx.origin == address(stableDebtToken) || tx.origin ==  address(variableDebtToken), "The caller of this function can only be the associated Debt tokens");
        _;
    }
