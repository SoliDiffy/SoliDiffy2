...
contract PartialKeep3rV1OracleJob is UtilsReady, Keep3r, IPartialKeep3rV1OracleJob {
  ...
  // Keep3r Setters
  function setKeep3r(address _keep3r) external override UtilsReady() {
    _setKeep3r(_keep3r);
  }
  ...
  function setKeep3rRequirements(
    ...
    uint256 _age,
    bool _onlyEOA
  ) external override UtilsReady() {
    _setKeep3rRequirements(_bond, _minBond, _earned, _age, _onlyEOA);
  }

  function setRewardMultiplier(uint256 _rewardMultiplier) external override UtilsReady() {
  ...
  // Setters
  function addPairs(address[] calldata _pairs) external override UtilsReady() {
    for (uint256 i; i < _pairs.length; i++) {
      _addPair(_pairs[i]);
    }
  }

  function addPair(address _pair) external override UtilsReady() {
    _addPair(_pair);
  }
  ...
  function removePair(address _pair) external override UtilsReady() {
  ...
  // Keeper view actions
  function workable(address _pair) external view override UtilsReady() returns (bool) {
  ...
  function work(address _pair) public override UtilsReady() onlyKeeper returns (uint256 _credits) {
  ...
  function forceWork(address _pair) external override UtilsReady() {
