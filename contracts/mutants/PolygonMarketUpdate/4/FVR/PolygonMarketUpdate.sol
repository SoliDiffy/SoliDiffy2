...
contract PolygonMarketUpdate {
  ...
  function execute(uint256 testInt) public payable {
    _counter = _counter + 1;
    _testInt = testInt;
    emit UpdateExecuted(_counter, _testInt, UPDATED_ADDRESS, msg.value);
  }

  function executeWithDelegate(bytes32 testBytes) public payable {
  ...
  function getTestInt() external view returns (uint256) {
    return _testInt;
  }
}
