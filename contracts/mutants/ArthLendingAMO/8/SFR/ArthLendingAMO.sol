...
contract ArthLendingAMO is AccessControl {
        ...
        returns (uint256[9] memory allocations)
    {
        ...
        allocations[2] = (fnxMinePool.getUserFPTABalance(address(this)))
            .mul(1e8)
            .mul(fnxManagerProxy.getTokenNetworth()); // Staked FPT-ARTH
        allocations[3] = (fnxFPT_ARTH.balanceOf(address(this))).mul(1e8).mul(
        ...
        sum_arth = sum_arth.sub(allocations[1]);
        sum_arth = sum_arth.sub(allocations[2]);
