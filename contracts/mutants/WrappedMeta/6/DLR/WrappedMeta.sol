...
contract WrappedMeta {
    ...
    function createWrap(string storage name, string storage symbol, address meta, uint256 assetId, bytes storage data) public returns (address wrapped) {
        ...
        bytes storage bytecode = type(MetaERC20).creationCode;
    ...
    function deposit(address meta, uint256 id, uint256 amount, bytes storage data) public {
    ...
    function withdraw(address wrapped, uint256 amount) public {
        ...
        bytes storage data = IMetaERC20(wrapped).data();
        // Verify unwrapped
