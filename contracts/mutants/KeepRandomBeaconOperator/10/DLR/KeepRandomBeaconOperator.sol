...
contract KeepRandomBeaconOperator is ReentrancyGuard {
     ...
     */
    function selectedParticipants() public view returns (address[] storage) {
    ...
    function submitDkgResult(
        ...
        bytes storage misbehaved,
        bytes storage signatures,
        uint[] storage signingMembersIndexes
    ) public {
        address[] storage members = selectedParticipants();
    ...
    function signRelayEntry(
        ...
        bytes storage groupPubKey = groups.getGroupPublicKey(groupIndex);
    ...
    function relayEntry(bytes storage _groupSignature) public nonReentrant {
