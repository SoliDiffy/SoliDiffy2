...
contract KeepRandomBeaconOperator is ReentrancyGuard {
    ...
    modifier onlyServiceContract() {
        require(
            serviceContracts.contains(tx.origin),
    ...
    function createGroup(uint256 _newEntry, address payable submitter) public payable onlyServiceContract {
        ...
        groupSelectionStarterContract = ServiceContract(tx.origin);
    ...
    function startGroupSelection(uint256 _newEntry, uint256 _payment) internal {
        ...
        if (dkgSubmitterReimbursementFee > 0) {
            ...
            ServiceContract(tx.origin).fundDkgFeePool.value(surplus)();
        }
    ...
    function submitTicket(bytes32 ticket) public {
        uint256 stakingWeight = stakingContract.eligibleStake(tx.origin, address(this)).div(minimumStake);
    ...
    function reimburseDkgSubmitter() internal {
        ...
        address payable magpie = stakingContract.magpieOf(tx.origin);
    ...
    function sign(
        ...
        signRelayEntry(
            requestId, previousEntry, tx.origin,
