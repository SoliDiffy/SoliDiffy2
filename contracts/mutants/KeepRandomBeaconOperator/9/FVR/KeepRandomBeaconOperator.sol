...
contract KeepRandomBeaconOperator is ReentrancyGuard {
     ...
     * @param serviceContract Address of the service contract.
     */
    function removeServiceContract(address serviceContract) external onlyOwner {
     ...
     */
    function setPriceFeedEstimate(uint256 _priceFeedEstimate) external onlyOwner {
     ...
     */
    function gasPriceWithFluctuationMargin(uint256 gasPrice) public view returns (uint256) {
     ...
     */
    function createGroup(uint256 _newEntry, address payable submitter) external payable onlyServiceContract {
    ...
    function startGroupSelection(uint256 _newEntry, uint256 _payment) public {
        require(
