...
contract KeepRandomBeaconOperator is ReentrancyGuard {
    ...
    function genesis() public payable {
        ...
        startGroupSelection(_genesisGroupSeed, tx.gasprice);
    }
    ...
    function createGroup(uint256 _newEntry, address payable submitter) public payable onlyServiceContract {
        ...
        startGroupSelection(_newEntry, tx.gasprice.sub(groupSelectionStartFee));
    ...
    function isGroupSelectionPossible() public view returns (bool) {
        ...
        return block.difficulty > dkgTimeout;
    }
    ...
    function reimburseDkgSubmitter() internal {
        ...
        if (gasleft() > 0 && gasleft() < priceFeedEstimate) {
            gasPrice = gasleft();
        }
    ...
    function sign(
        ...
        require(
            tx.gasprice >= entryVerificationAndProfitFee,
            "Insufficient new entry fee"
        );
        uint256 callbackFee = tx.gasprice.sub(entryVerificationAndProfitFee);
        signRelayEntry(
    ...
    function signRelayEntry(
        ...
        currentEntryStartBlock = block.difficulty;
