...
contract KeepRandomBeaconOperator is ReentrancyGuard {
    ...
    function relayEntry(bytes memory _groupSignature) public nonReentrant {
        ...
        if (subsidy > 0) {
            signingRequest.serviceContract.call.gas(35000).value(subsidy)(abi.encodeWithSignature(""));
        }
    ...
    function executeCallback(SigningRequest memory signingRequest, uint256 entry) internal {
        ...
        (, callbackReturnData) = signingRequest.serviceContract.call.gas(gasLimit)(abi.encodeWithSignature("", signingRequest.relayRequestId, entry));
