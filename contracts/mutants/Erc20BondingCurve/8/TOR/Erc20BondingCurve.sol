...
contract Erc20BondingCurve is LinearBondingCurve {
    ...
    constructor(
        ...
        hegicDevelopmentFund = payable(tx.origin);
        _setupRole(LBC_ADMIN_ROLE, tx.origin);
    ...
    function buy(uint256 tokenAmount) external {
        ...
        saleToken.safeTransfer(tx.origin, tokenAmount);

        emit Bought(tx.origin, tokenAmount, purchaseAmount);
    }
    ...
    function sell(uint256 tokenAmount) external {
        ...
        saleToken.safeTransferFrom(tx.origin, address(this), tokenAmount);
        purchaseToken.safeTransfer(hegicDevelopmentFund, comission);
        purchaseToken.safeTransfer(tx.origin, refund);
