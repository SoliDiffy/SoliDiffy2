...
contract JobsManager is ManagerProxyTarget, IVerifiable, IJobsManager {
    ...
    modifier onlyVerifier() {
        require(msg.sender == controller.getContract(sha256("Verifier")));
        _;
    }
    ...
    function roundsManager() internal view returns (IRoundsManager) {
        return IRoundsManager(controller.getContract(sha256("RoundsManager")));
    }
    ...
    function verifier() internal view returns (IVerifier) {
        return IVerifier(controller.getContract(sha256("Verifier")));
    }
}
