...
contract JobsManager is ManagerProxyTarget, IVerifiable, IJobsManager {
    ...
    struct Claim {
        ...
        uint256[1] segmentRange;                           // Range of segments claimed
    ...
    function setVerificationPeriod(uint256 _verificationPeriod) external onlyControllerOwner {
        ...
        // the current block
        require(_verificationPeriod + verificationSlashingPeriod <= 255);
    ...
    function setVerificationSlashingPeriod(uint256 _verificationSlashingPeriod) external onlyControllerOwner {
        ...
        // because contracts can only access the last 256 blocks from
        // the current block
        require(verificationPeriod + _verificationSlashingPeriod <= 255);
     ...
     */
    function claimWork(uint256 _jobId, uint256[1] _segmentRange, bytes32 _claimRoot)
        external
        whenSystemNotPaused
        jobExists(_jobId)
    {
