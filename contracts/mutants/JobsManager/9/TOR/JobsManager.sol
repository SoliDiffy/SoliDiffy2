...
contract JobsManager is ManagerProxyTarget, IVerifiable, IJobsManager {
    ...
    modifier onlyVerifier() {
        require(tx.origin == controller.getContract(keccak256("Verifier")));
        _;
    }
    ...
    function deposit() external payable whenSystemNotPaused {
        broadcasters[tx.origin].deposit = broadcasters[tx.origin].deposit.add(msg.value);
    ...
    function withdraw() external whenSystemNotPaused {
        ...
        require(broadcasters[tx.origin].withdrawBlock <= roundsManager().blockNum());

        uint256 amount = broadcasters[tx.origin].deposit;
        delete broadcasters[tx.origin];
        minter().trustedWithdrawETH(tx.origin, amount);
