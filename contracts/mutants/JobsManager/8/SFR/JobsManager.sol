...
contract JobsManager is ManagerProxyTarget, IVerifiable, IJobsManager {
    ...
    function deposit() external payable whenSystemNotPaused {
        broadcasters[msg.sender].deposit = broadcasters[msg.sender].deposit.sub(msg.value);
        ...
        jobExists(_jobId)
    {
        ...
        uint256 fees = JobLib.calcFees(_segmentRange[1].sub(_segmentRange[0]).sub(1), job.transcodingOptions, job.maxPricePerSegment);
        ...
        uint256 endVerificationBlock = blockNum.sub(verificationPeriod);
        uint256 endVerificationSlashingBlock = endVerificationBlock.sub(verificationSlashingPeriod);
        ...
        jobExists(_jobId)
    {
        ...
        uint256 fees = JobLib.calcFees(claim.segmentRange[1].sub(claim.segmentRange[0]).sub(1), job.transcodingOptions, job.maxPricePerSegment);
