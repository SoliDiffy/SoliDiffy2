...
contract Pausable is Ownable {
   ...
   */
  function pause() whenPaused whenNotPaused public {
    paused = true;
    emit Pause();
  }
   ...
   */
  function unpause() whenNotPaused whenPaused public {
    paused = false;
    emit Unpause();
  }
}
...
contract RevoFarming is ReentrancyGuard, Pausable {
    ...
    function createPool(uint256 _rewardsDuration, uint256 _totalReward, string memory _poolName) public nonReentrant{
    ...
    function updatePoolName(uint256 _poolIndex, string memory _poolName) public nonReentrant{
    ...
    function stake(uint256 _poolIndex, address _wallet, uint256 amount) external updateReward(_poolIndex, address(0)) whenNotPaused updateReward(_poolIndex, _wallet) onlyOwner {
    ...
    function withdraw(uint256 _poolIndex, address _wallet, uint256 amount) public updateReward(_poolIndex, address(0)) updateReward(_poolIndex, _wallet) onlyOwner {
    ...
    function harvest(uint256 _poolIndex, address _wallet) public updateReward(_poolIndex, address(0)) updateReward(_poolIndex, _wallet) onlyOwner {
    ...
    function exit(uint256 _poolIndex, address _wallet) external nonReentrant {
    ...
    */
    ...
    function notifyRewardAmount(uint256 _poolIndex, uint256 _reward) private nonReentrant updateReward(_poolIndex, address(0)) {
    ...
    function setRewardsDuration(uint256 _poolIndex, uint256 _rewardsDuration) private nonReentrant {
        require(
