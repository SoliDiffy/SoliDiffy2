...
contract RevoFarming is ReentrancyGuard, Pausable {
    ...
    function getAllPools() external view returns(FarmingPool[] storage){
        FarmingPool[] storage poolsToReturn = new FarmingPool[](poolIndex);
    ...
    */
    function getUserStakes(address _user) public view returns(Stake[] storage){
        uint256 count;
        for(uint256 i = 0; i < poolIndex; i++){
            if(_balances[i][_user] > 0){ count++;}
        }
        
        Stake[] storage stakesToReturn = new Stake[](count);
        uint index;
        for(uint256 i = 0; i < poolIndex; i++){
