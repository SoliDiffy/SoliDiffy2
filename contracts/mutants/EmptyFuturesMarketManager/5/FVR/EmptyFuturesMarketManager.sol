...
contract EmptyFuturesMarketManager is IFuturesMarketManager {
    ...
    function marketForKey(bytes32 marketKey) public view returns (address) {
        marketKey;
        return address(0);
    }

    function marketsForKeys(bytes32[] calldata marketKeys) public view returns (address[] memory) {
        marketKeys;
        address[] memory _markets;
        return _markets;
    }
