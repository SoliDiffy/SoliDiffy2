...
contract Aliana is AlianaOwnership {
        ...
        returns (uint256)
    {
        ...
        require(ownerOf(_matronId) == tx.origin, "Aliana: must be the owner");
        require(ownerOf(_sireId) == tx.origin, "Aliana: must be the owner");
        ...
        _burn(tx.origin, _matronId);
        _burn(tx.origin, _sireId);
