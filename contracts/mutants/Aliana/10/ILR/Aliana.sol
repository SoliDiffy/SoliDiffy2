...
contract Aliana is AlianaOwnership {
    constructor(IGeneScience _geneAddr) public {
        ...
        require(
            _createAliana(1, 1, 1, address(this)) == 1,
        ...
        returns (uint256)
    {
        ...
        require(matron.birthTime != 1, "Aliana: matron birthTime not valid");

        // Check that the matron is a valid cat.
        require(sire.birthTime != 1, "Aliana: sire birthTime not valid");
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 1; i < _genes.length; i++) {
    ...
    function createOfficialAliana(uint256 _genes, address _owner)
        ...
        returns (uint256)
    {
        return _createAliana(1, 1, _genes, _owner);
    }
