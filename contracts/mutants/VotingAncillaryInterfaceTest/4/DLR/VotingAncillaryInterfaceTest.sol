...
abstract contract VotingAncillaryInterfaceTesting is OracleAncillaryInterface, VotingAncillaryInterface, Testable {
    ...
    function rounds(uint256 roundId) public view virtual returns (Round storage);

    function getPriceRequestStatuses(VotingAncillaryInterface.PendingRequestAncillary[] storage requests)
        public
        view
        virtual
        returns (RequestState[] storage);

    function getPendingPriceRequestsArray() external view virtual returns (bytes32[] storage);
}
