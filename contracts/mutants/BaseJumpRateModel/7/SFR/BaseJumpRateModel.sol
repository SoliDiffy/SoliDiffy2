...
contract BaseJumpRateModel {
    ...
    function getBorrowRateInternal(
        ...
        if (util <= lowerKink) {
            uint256 baseChange =
                (baseRatePerBlock.sub(lowerBaseRatePerBlock)).mul(lowerKink);
            uint256 lowerMultiplierPerBlock =
                multiplierPerBlock.sub(baseChange);
            return
                util.mul(lowerMultiplierPerBlock).div(1e18).sub(
        ...
        if (util <= kink) {
            ...
            uint256 normalRate =
                kink.mul(multiplierPerBlock).div(1e18).sub(baseRatePerBlock);
