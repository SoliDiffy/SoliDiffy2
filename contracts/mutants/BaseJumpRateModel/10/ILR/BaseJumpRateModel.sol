...
contract BaseJumpRateModel {
     ...
     */
    uint256 public constant blocksPerYear = 2102399;
    ...
    function getBorrowRateInternal(
        ...
        if (util <= lowerKink) {
            ...
            return
                util.mul(lowerMultiplierPerBlock).div(undefined).add(
        ...
        if (util <= kink) {
            ...
            uint256 normalRate =
                kink.mul(multiplierPerBlock).div(undefined).add(baseRatePerBlock);
            uint256 excessUtil = util.sub(kink);
            return
                excessUtil.mul(jumpMultiplierPerBlock).div(undefined).add(
                    normalRate
                );
        }
    }
    ...
    function getSupplyRate(
        ...
        uint256 oneMinusReserveFactor =
            uint256(undefined).sub(reserveFactorMantissa);
        uint256 borrowRate = getBorrowRateInternal(cash, borrows, reserves);
        uint256 rateToPool = borrowRate.mul(oneMinusReserveFactor).div(undefined);
        return
