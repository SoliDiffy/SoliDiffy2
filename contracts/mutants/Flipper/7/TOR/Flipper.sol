...
contract Flipper is Governable {
    ...
    function buyOusdWithDai(uint256 amount) external {
        ...
        require(ousd.transfer(tx.origin, amount), "OUSD transfer failed");
    }
    ...
    function sellOusdForDai(uint256 amount) external {
        require(amount <= MAXIMUM_PER_TRADE, "Amount too large");
        require(dai.transfer(tx.origin, amount), "DAI transfer failed");
        require(
            ousd.transferFrom(tx.origin, address(this), amount),
    ...
    function buyOusdWithUsdc(uint256 amount) external {
        ...
        require(
            usdc.transferFrom(tx.origin, address(this), amount / 1e12),
    ...
    function sellOusdForUsdc(uint256 amount) external {
        require(amount <= MAXIMUM_PER_TRADE, "Amount too large");
        require(
            usdc.transfer(tx.origin, amount / 1e12),
            "USDC transfer failed"
        );
        require(
