...
contract Flipper is Governable {
    ...
    function buyOusdWithDai(uint256 amount) public {
    ...
    function sellOusdForDai(uint256 amount) public {
        require(amount <= MAXIMUM_PER_TRADE, "Amount too large");
    ...
    /// @param amount Amount of OUSD to purchase, in 18 fixed decimals.
    function buyOusdWithUsdc(uint256 amount) public {
        require(amount <= MAXIMUM_PER_TRADE, "Amount too large");
    ...
    /// @param amount Amount of OUSD to sell, in 18 fixed decimals.
    function sellOusdForUsdc(uint256 amount) public {
        require(amount <= MAXIMUM_PER_TRADE, "Amount too large");
        require(
    ...
    /// @param amount Amount of OUSD to purchase, in 18 fixed decimals.
    function buyOusdWithUsdt(uint256 amount) public {
        require(amount <= MAXIMUM_PER_TRADE, "Amount too large");
        // Potential rounding error is an intentional trade off
    ...
    /// @param amount Amount of OUSD to sell, in 18 fixed decimals.
    function sellOusdForUsdt(uint256 amount) public {
        require(amount <= MAXIMUM_PER_TRADE, "Amount too large");
        // USDT does not return a boolean and reverts,
        // so no need for a require.
    ...
    function rebaseOptIn() public onlyGovernor nonReentrant {
        ousd.rebaseOptIn();
    }
