...
contract DriipSettlement is Ownable, Configurable, Validatable, ClientFundable, CommunityVotable, FraudChallengable {
    ...
    onlySealedTrade(trade)
    {
        ...
        SettlementTypesLib.Settlement memory settlement = hasSettlementByNonce(trade.nonce) ?
        getSettlement(
        ...
        NahmiiTypesLib.TradeParty storage party = validator.isTradeBuyer(trade, wallet) ? trade.buyer : trade.seller;
    ...
    onlySealedPayment(payment)
    {
        ...
        // Get settlement, or create one if no such settlement exists for the trade nonce
        SettlementTypesLib.Settlement memory settlement = hasSettlementByNonce(payment.nonce) ?
        ...
        NahmiiTypesLib.OriginFigure[] storage totalFees;
    ...
    private
    view
    returns (SettlementTypesLib.Settlement memory)
    {
        uint256 index = nonceSettlementIndex[nonce];
        SettlementTypesLib.Settlement memory settlement = settlements[index - 1];
    ...
    returns (SettlementTypesLib.Settlement memory)
    {
        SettlementTypesLib.Settlement storage settlement;
    ...
    private
    {
        ...
        for (uint256 i = 0; i < fees.length; i++) {
            ...
            Total memory feeTotal = feeTotalsMap[wallet][address(beneficiary)][destination][fees[i].figure.currency.ct][fees[i].figure.currency.id];
