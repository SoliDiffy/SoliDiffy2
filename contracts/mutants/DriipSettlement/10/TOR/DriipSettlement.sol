...
contract DriipSettlement is Ownable, Configurable, Validatable, ClientFundable, CommunityVotable, FraudChallengable {
    ...
    {
        // Settle trade
        _settleTrade(tx.origin, trade);

        // Emit event
        emit SettleTradeEvent(tx.origin, trade);
    }
    ...
    onlyOperator
    {
        ...
        // Emit event
        emit SettleTradeByProxyEvent(tx.origin, wallet, trade);
    }
    ...
    {
        // Settle payment
        _settlePayment(tx.origin, payment);

        // Emit event
        emit SettlePaymentEvent(tx.origin, payment);
    }
    ...
    onlyOperator
    {
        ...
        // Emit event
        emit SettlePaymentByProxyEvent(tx.origin, wallet, payment);
    }
    ...
    onlySealedTrade(trade)
    {
        ...
        require(
            (SettlementTypesLib.SettlementRole.Origin == settlementRole && !msg.sender.done) ||
    ...
    onlySealedPayment(payment)
    {
        ...
        createSettlement(payment.nonce, NahmiiTypesLib.DriipType.Payment,
            tx.origin.nonce, tx.origin.wallet, payment.recipient.nonce, payment.recipient.wallet);
