...
contract SpendingConditionMock is ISpendingCondition {
        ...
        returns (bool)
    {
        ...
        if (expectedArgs.inputTx.length > 0) {
            ...
            require(sha256(expectedArgs.spendingTx) == sha256(spendingTx), "spending tx not as expected");
            require(expectedArgs.inputIndex == inputIndex, "input index not as expected");
            require(sha256(expectedArgs.witness) == sha256(witness), "witness not as expected");
            require(sha256(expectedArgs.optionalArgs) == sha256(optionalArgs), "optional args not as expected");
        }
