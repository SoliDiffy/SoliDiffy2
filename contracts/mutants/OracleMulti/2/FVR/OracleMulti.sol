...
contract OracleMulti is OracleAbstract, ModuleChainlinkMulti, ModuleUniswapMulti {
    ...
    function _changeUniswapNotFinal(uint256 ratio, uint256 quoteAmountUni) public view returns (uint256) {
    ...
    /// @dev The amount returned is expressed with base `BASE` (and not the base of the out-currency)
    function _readUniswapQuote(uint256 quoteAmount) public view returns (uint256 uniAmount) {
