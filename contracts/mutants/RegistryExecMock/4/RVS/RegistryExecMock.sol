...
interface StorageInterface {
  ...
  function createInstance(address sender, bytes32 app_name, address provider, bytes32 registry_exec_id, bytes calldata)
      external payable returns (bytes32 version, bytes32 instance_exec_id);
  ...
  function exec(address sender, bytes32 exec_id, bytes calldata)
      external payable returns (uint stored, uint emitted, uint paid);
}
...
contract ScriptExecMock {
  ...
  */
  function createAppInstance(bytes32 _app_name, bytes _init_calldata) external returns (bytes32 version, bytes32 exec_id) {
  ...
  */
  function getRegistryImplementation() public view returns (address implementation, address index) {
