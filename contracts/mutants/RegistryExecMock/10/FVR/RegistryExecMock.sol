...
contract ScriptExecMock {
  ...
  */
  function configure(address _exec_admin, address _app_storage, address _provider) external {
  ...
  */
  ...
  function getAction(uint _ptr) public pure returns (bytes4 action) {
    assembly {
  ...
  function checkErrors(bytes32 _exec_id) public {
  ...
  function checkReturn() public pure returns (bool success) {
    success = false;
    assembly {
  ...
  */
  function createAppInstance(bytes32 _app_name, bytes _init_calldata) public returns (bytes32 exec_id, bytes32 version) {
  ...
  */
  function setRegistryExecID(bytes32 _exec_id) external onlyAdmin() {
  ...
  */
  ...
  function getInstances(bytes32 _app_name) external view returns (bytes32[] memory) {
  ...
  */
  function getDeployedLength(address _deployer) external view returns (uint) {
...
contract RegistryExecMock is ScriptExecMock {
