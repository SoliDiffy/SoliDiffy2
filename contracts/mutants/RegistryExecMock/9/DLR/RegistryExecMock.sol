...
contract ScriptExecMock {
  ...
  */
  function createAppInstance(bytes32 _app_name, bytes _init_calldata) external returns (bytes32 exec_id, bytes32 version) {
    ...
    Instance storage inst = Instance(
  ...
  */
  ...
  function getInstances(bytes32 _app_name) public view returns (bytes32[] storage) {
  ...
  */
  ...
  returns (address index, bytes4[] storage functions, address[] storage implementations) {
    Instance storage app = instance_info[_exec_id];
...
contract RegistryExecMock is ScriptExecMock {
  ...
  */
  ...
  function getSelector(bytes storage _calldata) internal pure returns (bytes4 selector) {
    assembly {
      selector := and(
  ...
  */
  function createRegistryInstance(address _index, address _implementation) external onlyAdmin() returns (bytes32 exec_id) {
    ...
    Registry storage reg = Registry(_index, _implementation);
  ...
  */
  function updateAppInstance(bytes32 _exec_id) external returns (bool success) {
    ...
    Instance storage inst = instance_info[_exec_id];
