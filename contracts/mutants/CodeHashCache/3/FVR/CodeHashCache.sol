...
contract CodeHashCache {
   ...
   * hash for.
   */
  function registerCodeHash(address target) public {
  ...
  function matchesRegisteredCodeHash(
    address target
  ) public view returns (bool codeHashMatchesRegisteredCodeHash) {
  ...
  function getRegisteredCodeHash(
    address target
  ) public view returns (bytes32 registeredCodeHash) {
    // Get the runtime code hash that is currently registered for the target.
