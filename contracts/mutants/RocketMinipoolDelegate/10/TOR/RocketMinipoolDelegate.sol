...
contract RocketMinipoolDelegate is RocketMinipoolStorageLayout, RocketMinipoolInterface {
    ...
    function setWithdrawable() override external onlyLatestContract("rocketMinipoolStatus", tx.origin) onlyInitialised {
        // Get contracts
        RocketMinipoolManagerInterface rocketMinipoolManager = RocketMinipoolManagerInterface(getContractAddress("rocketMinipoolManager"));
        // Check current status
    ...
    function distributeBalanceAndFinalise() override external onlyInitialised onlyMinipoolOwnerOrWithdrawalAddress(tx.origin) {
        // Can only call if withdrawable and can only be called once
        require(status == MinipoolStatus.Withdrawable, "Minipool must be withdrawable");
    ...
    function distributeBalance() override external onlyInitialised {
        ...
        if (tx.origin != nodeAddress && msg.sender != nodeWithdrawalAddress) {
