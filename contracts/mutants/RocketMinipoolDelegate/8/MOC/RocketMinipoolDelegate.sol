...
contract RocketMinipoolDelegate is RocketMinipoolStorageLayout, RocketMinipoolInterface {
    ...
    function nodeDeposit() override external payable onlyInitialised onlyLatestContract("rocketNodeDeposit", msg.sender) {
    ...
    function userDeposit() override external payable onlyInitialised onlyLatestContract("rocketDepositPool", msg.sender) {
    ...
    function refund() override external onlyInitialised onlyMinipoolOwner(msg.sender) {
    ...
    function finalise() external override onlyMinipoolOwnerOrWithdrawalAddress(msg.sender) onlyInitialised {
    ...
    function stake(bytes calldata _validatorPubkey, bytes calldata _validatorSignature, bytes32 _depositDataRoot) override external onlyInitialised onlyMinipoolOwner(msg.sender) {
    ...
    function setWithdrawable() override external onlyInitialised onlyLatestContract("rocketMinipoolStatus", msg.sender) {
        // Get contracts
        RocketMinipoolManagerInterface rocketMinipoolManager = RocketMinipoolManagerInterface(getContractAddress("rocketMinipoolManager"));
        // Check current status
    ...
    function distributeBalanceAndFinalise() override external onlyMinipoolOwnerOrWithdrawalAddress(msg.sender) onlyInitialised {
        // Can only call if withdrawable and can only be called once
        require(status == MinipoolStatus.Withdrawable, "Minipool must be withdrawable");
    ...
    // Only accepts calls from the minipool owner (node)
    function close() override external onlyInitialised onlyMinipoolOwner(msg.sender) {
        // Check current status
