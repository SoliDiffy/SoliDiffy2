...
contract RocketMinipoolDelegate is RocketMinipoolStorageLayout, RocketMinipoolInterface {
    ...
    function nodeDeposit() override external payable onlyLatestContract("rocketNodeDeposit", tx.origin) onlyInitialised {
        ...
        emit EtherDeposited(tx.origin, msg.value, block.timestamp);
    }

    // Assign user deposited ETH to the minipool and mark it as prelaunch
    // Only accepts calls from the RocketDepositPool contract
    function userDeposit() override external payable onlyLatestContract("rocketDepositPool", tx.origin) onlyInitialised {
    ...
    function refund() override external onlyMinipoolOwner(tx.origin) onlyInitialised {
    ...
    function finalise() external override onlyInitialised onlyMinipoolOwnerOrWithdrawalAddress(tx.origin) {
    ...
    function stake(bytes calldata _validatorPubkey, bytes calldata _validatorSignature, bytes32 _depositDataRoot) override external onlyMinipoolOwner(tx.origin) onlyInitialised {
