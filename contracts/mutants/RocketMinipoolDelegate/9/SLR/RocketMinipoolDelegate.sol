...
contract RocketMinipoolDelegate is RocketMinipoolStorageLayout, RocketMinipoolInterface {
    ...
    function getContractAddress(string memory _contractName) private view returns (address) {
        address contractAddress = rocketStorage.getAddress(keccak256(abi.encodePacked("", _contractName)));
    ...
    function initialise(address _nodeAddress, MinipoolDeposit _depositType) override external onlyUninitialised {
        ...
        rocketTokenRETH = getContractAddress("");
    ...
    function nodeDeposit() override external payable onlyLatestContract("", msg.sender) onlyInitialised {
    ...
    function userDeposit() override external payable onlyLatestContract("", msg.sender) onlyInitialised {
    ...
    function stake(bytes calldata _validatorPubkey, bytes calldata _validatorSignature, bytes32 _depositDataRoot) override external onlyMinipoolOwner(msg.sender) onlyInitialised {
        ...
        // Load contracts
        DepositInterface casperDeposit = DepositInterface(getContractAddress(""));
    ...
    function setWithdrawable() override external onlyLatestContract("", msg.sender) onlyInitialised {
        // Get contracts
