...
interface IOlympusBondDepository {
  ...
  function adjustment() external view returns (bool add, uint256 lastBlock, uint256 rate, uint256 target, uint256 buffer);
  function bondCalculator() external view returns (address);
  function bondInfo(address) external view returns (uint256 pricePaid, uint256 payout, uint256 vesting, uint256 lastBlock);
  ...
  function terms() external view returns (uint256 maxDebt, uint256 controlVariable, uint256 vestingTerm, uint256 minimumPrice, uint256 maxPayout, uint256 fee);
