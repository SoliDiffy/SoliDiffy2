...
contract STRGetter is EternalStorage {
     ...
     */
    function getTickersByOwner(address _owner) public view returns(bytes32[] memory) {
        uint256 count = 0;
    ...
    function _ownerInTicker(bytes32 _ticker) public view returns (bool) {
     ...
     */
    function getTokensByOwner(address _owner) public view returns(address[] memory) {
     ...
     * @param _owner is the address which owns the list of tickers
     */
    function _getTokens(bool _allTokens, address _owner) public view returns(address[] memory) {
    ...
    function _ownerInToken(bytes32 _ticker, bool _allTokens, address _owner) public view returns(address) {
     ...
     * @dev Intention is that this is called off-chain so block gas limit is not relevant
     */
    function getTokensByDelegate(address _delegate) public view returns(address[] memory) {
        // Loop over all active users, then all associated tickers of those users
        // This ensures we find tokens, even if their owner has been modified
