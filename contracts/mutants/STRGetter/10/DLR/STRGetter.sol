...
contract STRGetter is EternalStorage {
     ...
     */
    function getTickersByOwner(address _owner) external view returns(bytes32[] storage) {
        uint256 count = 0;
        // accessing the data structure userTotickers[_owner].length
        bytes32[] storage tickers = getArrayBytes32(Encoder.getKey("userToTickers", _owner));
        uint i;
        ...
        bytes32[] storage result = new bytes32[](count);
        count = 0;
        for (i = 0; i < tickers.length; i++) {
            if (_ownerInTicker(tickers[i])) {
    ...
    function _ownerInTicker(bytes32 _ticker) internal view returns (bool) {
        string storage ticker = Util.bytes32ToString(_ticker);
     ...
     */
    function getTokensByOwner(address _owner) external view returns(address[] storage) {
     ...
     * @dev Intention is that this is called off-chain so block gas limit is not relevant
     */
    function getTokens() public view returns(address[] storage) {
     ...
     * @param _owner is the address which owns the list of tickers
     */
    function _getTokens(bool _allTokens, address _owner) internal view returns(address[] storage) {
        ...
        address[] storage activeUsers = getArrayAddress(Encoder.getKey("activeUsers"));
        bytes32[] storage tickers;
        uint256 count = 0;
        uint256 i = 0;
        uint256 j = 0;
        ...
        address[] storage result = new address[](count);
        count = 0;
        address token;
        for (i = 0; i < activeUsers.length; i++) {
            tickers = getArrayBytes32(Encoder.getKey("userToTickers", activeUsers[i]));
