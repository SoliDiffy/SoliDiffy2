...
contract ElementBridge is IDefiBridge {
     ...
     */
    function getAssetExpiries(address asset) external view returns (uint64[] memory assetExpiries) {
    ...
    * @param trancheAddress the address of the tranche against which to store the current block number
     */
    function setTrancheDeploymentBlockNumber(address trancheAddress) public {
    ...
    * @return blockNumber the number of the block in which the tranche was first configured
     */
    function getTrancheDeploymentBlockNumber(uint256 interactionNonce) external view returns (uint256 blockNumber) {
    ...
    * @param pool address of a balancer pool contract
     */
    function validatePositionAndPoolAddresses(address wrappedPosition, address pool) public {
    ...
    function hashAssetAndExpiry(address asset, uint64 expiry) external pure returns (uint256 hashValue) {
     ...
     */
    function finaliseExpiredInteractions(uint256 gasFloor) public {
     ...
     */
    function addNonceAndExpiry(uint256 nonce, uint64 expiry) public returns (bool expiryAdded) {
