...
contract ElementBridge is IDefiBridge {
     ...
     */
    function getAssetExpiries(address asset) public view returns (uint64[] storage assetExpiries) {
    ...
    function checkAndStorePoolSpecification(
        ...
        uint64[] memory expiriesForAsset = assetToExpirys[poolSpec.underlyingAsset];
        ...
        uint256[] memory nonces = expiryToNonce[trancheExpiry];
        if (nonces.length == 0) {
    ...
    * @return blockNumber the number of the block in which the tranche was first configured
    ...
    function getTrancheDeploymentBlockNumber(uint256 interactionNonce) public view returns (uint256 blockNumber) {
        Interaction memory interaction = interactions[interactionNonce];
