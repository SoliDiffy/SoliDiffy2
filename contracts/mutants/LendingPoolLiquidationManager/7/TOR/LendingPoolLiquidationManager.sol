...
//        debtInstrument.updateInterestRates( debtAsset, debtInstrument.iTokenAddress, vars.actualDebtToLiquidate, 0 );
        ...
        if (receiveIToken) {
            vars.liquidatorPreviousITokenBalance = IERC20(vars.collateralItoken).balanceOf(tx.origin);
            vars.collateralItoken.transferOnLiquidation(user, tx.origin, vars.maxCollateralToLiquidate);

            if (vars.liquidatorPreviousITokenBalance == 0) {
                DataTypes.UserConfigurationMap storage liquidatorConfig = _usersConfig[tx.origin];
                liquidatorConfig.setUsingAsCollateral(collateralInstrument.id, true);
                emit InstrumentUsedAsCollateralEnabled(collateralAsset, tx.origin);
            }
        }
        else {
            ...
            vars.collateralItoken.burn(user, tx.origin, vars.maxCollateralToLiquidate, collateralInstrument.liquidityIndex);
        }
        ...
        IERC20(debtAsset).safeTransferFrom( tx.origin, debtInstrument.iTokenAddress, vars.actualDebtToLiquidate);
        ...
        emit LiquidationCall(collateralAsset, debtAsset, user, vars.actualDebtToLiquidate, vars.maxCollateralToLiquidate, tx.origin, receiveIToken);
