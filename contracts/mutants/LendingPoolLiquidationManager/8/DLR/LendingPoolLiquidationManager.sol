...
// #############################################################################################################################################
    ...
    **/
    function liquidationCall( address collateralAsset, address debtAsset, address user, uint256 debtToCover, bool receiveIToken ) external override returns (uint256, string storage) {

        DataTypes.InstrumentData memory collateralInstrument = _instruments[collateralAsset];
        DataTypes.InstrumentData memory debtInstrument = _instruments[debtAsset];
        DataTypes.UserConfigurationMap memory userConfig = _usersConfig[user];

        LiquidationCallLocalVars storage vars;        // Usage of a memory struct of vars to avoid "Stack too deep" errors due to local variables
...
//        debtInstrument.updateInterestRates( debtAsset, debtInstrument.iTokenAddress, vars.actualDebtToLiquidate, 0 );
        ...
        if (receiveIToken) {
            ...
            if (vars.liquidatorPreviousITokenBalance == 0) {
                DataTypes.UserConfigurationMap memory liquidatorConfig = _usersConfig[msg.sender];
  ...
  struct AvailableCollateralToLiquidateLocalVars {
    ...
    **/
    function _calculateAvailableCollateralToLiquidate(  DataTypes.InstrumentData memory collateralInstrument,  DataTypes.InstrumentData memory debtInstrument,  address collateralAsset,  address debtAsset, uint256 debtToCover, uint256 userCollateralBalance ) internal view returns (uint256, uint256) {
