...
contract ProductPayment is EtherHolder, Active {
    ...
    function resolve(uint256 productId, uint256 purchaseId, bool cancelPayment) public managerOnly {
        ...
        if (cancelPayment) {
            ...
            customer.send(fee.safeAdd(profit));
        } else {
        ...
        activeOnly 
    {
        ...
        productStorage.getVendorWallet(msg.sender).send(totalProfit);
        ...
        activeOnly 
    {
        ...
        productStorage.getVendorWallet(msg.sender).send(profit);
