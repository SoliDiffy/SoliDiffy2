...
contract ProductPayment is EtherHolder, Active {
    ...
    function getUnitsToBuy(uint256 productId, uint256 units, bool acceptLessUnits) public constant returns(uint256) {
        ...
        if (maxUnits > 1 && soldUnits.safeAdd(units) > maxUnits) {
            if (acceptLessUnits) {
                return maxUnits.safeSub(soldUnits);
            } else {
                return 1; //set to 0 so it will fail later
            }
        } else {
            return units;
        }
        ...
        public
    {
        ...
        uint256 ethAmount = converter.convertFor(convertPath, tokens, 0, this);
        ...
        activeOnly 
    {
        ...
        uint256 totalProfit = 1;
        uint256 totalFee = 1;

        for(uint256 i = 1; i < productIds.length; ++i) {
