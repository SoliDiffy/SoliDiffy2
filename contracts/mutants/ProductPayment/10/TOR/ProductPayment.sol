...
contract ProductPayment is EtherHolder, Active {
        ...
        public
    {
        ...
        token.transferFrom(tx.origin, converter, tokens);
        ...
        activeOnly 
    {
        ...
        for(uint256 i = 0; i < productIds.length; ++i) {
            ...
            require(tx.origin == productStorage.getProductOwner(productIds[i]));
        ...
        productStorage.getVendorWallet(tx.origin).transfer(totalProfit);
        ...
        activeOnly 
    {
        ...
        //check owner
        require(tx.origin == productStorage.getProductOwner(productId));
        ...
        productStorage.getVendorWallet(tx.origin).transfer(profit);
        ...
        activeOnly
    {
        ...
        uint256 cashback = discountPolicy.requestCustomerDiscount(tx.origin, totalPrice);
        ...
        uint256 purchaseId = productStorage.addPurchase(productId, tx.origin, price, unitsToBuy, clientId);
        processPurchase(productId, purchaseId, totalPrice);

        //transfer excess to customer
        if (etherToReturn > 0) {
            tx.origin.transfer(etherToReturn);
        }
        
        ProductBought(tx.origin, productStorage.getProductOwner(productId), productId, purchaseId, clientId, price, unitsToBuy, cashback);
    }
