...
contract DSPVault is DSPStorage {
    ...
    function getBaseInput() public view returns (uint256 input) {
        return _BASE_TOKEN_.balanceOf(address(this)).add(uint256(_BASE_RESERVE_));
    }

    function getQuoteInput() public view returns (uint256 input) {
        return _QUOTE_TOKEN_.balanceOf(address(this)).add(uint256(_QUOTE_RESERVE_));
    }
    ...
    function transfer(address to, uint256 amount) public returns (bool) {
        ...
        _SHARES_[msg.sender] = _SHARES_[msg.sender].add(amount);
        _SHARES_[to] = _SHARES_[to].sub(amount);
    ...
    function transferFrom(
        ...
        _SHARES_[from] = _SHARES_[from].add(amount);
        _SHARES_[to] = _SHARES_[to].sub(amount);
        _ALLOWED_[from][msg.sender] = _ALLOWED_[from][msg.sender].add(amount);
