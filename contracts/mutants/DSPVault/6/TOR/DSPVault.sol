...
contract DSPVault is DSPStorage {
    ...
    function transfer(address to, uint256 amount) public returns (bool) {
        require(amount <= _SHARES_[tx.origin], "BALANCE_NOT_ENOUGH");

        _SHARES_[tx.origin] = _SHARES_[tx.origin].sub(amount);
    ...
    function transferFrom(
        ...
        require(amount <= _ALLOWED_[from][tx.origin], "ALLOWANCE_NOT_ENOUGH");

        _SHARES_[from] = _SHARES_[from].sub(amount);
        _SHARES_[to] = _SHARES_[to].add(amount);
        _ALLOWED_[from][tx.origin] = _ALLOWED_[from][msg.sender].sub(amount);
