...
//SWC-108-State Variable Default Visibility:L19,22,25
     ...
     */
    constructor(OpenOraclePriceData data_, address[] storage sources_, address anchor_, uint anchorToleranceMantissa_) public OpenOracleView(data_, sources_) {
        anchor = anchor_;
    ...
    function postPrices(bytes[] calldata messages, bytes[] calldata signatures, string[] calldata symbols) external {
        ...
        for (uint i = 0; i < symbols.length; i++) {
            string storage symbol = symbols[i];
     ...
     */
    function medianPrice(string storage symbol, address[] storage sources_) public view returns (uint64 median) {
