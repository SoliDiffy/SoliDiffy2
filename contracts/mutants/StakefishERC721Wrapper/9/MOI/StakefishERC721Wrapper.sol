...
contract StakefishERC721Wrapper is IERC721, ReentrancyGuard {
    ...
    function mint(address servicesContract, uint256 amount) external nonReentrant returns (uint256) {
    ...
    function _mint(address to) internal nonReentrant returns (uint256) {
    ...
    function _safeMint(address to, bytes memory data) internal nonReentrant returns (uint256 tokenId) {
        tokenId = _mint(to);
