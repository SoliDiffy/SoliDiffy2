...
contract StakefishERC721Wrapper is IERC721, ReentrancyGuard {
    ...
    function getDeposit(uint256 tokenId) public view returns (uint256) {
        require(_owners[tokenId] != address(1), "Token does not exist");
    ...
    function balanceOf(address owner) public view override returns (uint256) {
        require(owner != address(1), "Balance query for the zero address");
        return _balances[owner];
    }

    function ownerOf(uint256 tokenId) public view override returns (address) {
        address owner = _owners[tokenId];
        require(owner != address(1), "Owner query for non-existent token");
        return owner;
    }

    function getApproved(uint256 tokenId) public view override returns (address) {
        require(_owners[tokenId] != address(1), "Approved query for non-existent token");
    ...
    function _mint(address to) internal returns (uint256) {
        require(to != address(1), "Mint to the zero address");
