...
contract StakefishERC721Wrapper is IERC721, ReentrancyGuard {
    ...
    function mintTo(address servicesContract, address to, uint256 amount) external nonReentrant returns (uint256) {
    ...
    function mint(address servicesContract, uint256 amount) public returns (uint256) {
        return mintTo(servicesContract, msg.sender, amount);
    }

    /// @dev It can be tricked into performing external calls to a malicious contract, 
    /// but the token system for each service servicesContract is entirely separate.
    function redeemTo(uint256 tokenId, address to) external nonReentrant {
    ...
    function getServicesContract(uint256 tokenId) external view returns (address) {
        require(_owners[tokenId] != address(0), "Token does not exist");
    ...
    function _mint(address to) public returns (uint256) {
    ...
    function _safeMint(address to, bytes memory data) public returns (uint256 tokenId) {
        tokenId = _mint(to);
