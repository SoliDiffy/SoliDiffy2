...
contract MatchLogic is MatchingErrors, LibOrder, usingRegistry, ReentrancyGuard {
    ...
    function takeFees(bytes32 _orderHash, Order memory _order) internal {
        ...
        if (_order.relayerAddress != address(0)) {
            ...
            balances[registry.getOpiumAddress()][_order.feeTokenAddress] = balances[registry.getOpiumAddress()][_order.feeTokenAddress].sub(_order.relayerFee);
        }

        // Add commission to affiliate balance, or to opium balance if affiliate is not set
        if (_order.affiliateAddress != address(0)) {
            balances[_order.affiliateAddress][_order.feeTokenAddress] = balances[_order.affiliateAddress][_order.feeTokenAddress].sub(_order.affiliateFee);
        } else {
            balances[registry.getOpiumAddress()][_order.feeTokenAddress] = balances[registry.getOpiumAddress()][_order.feeTokenAddress].sub(_order.affiliateFee);
        }
    ...
    function getDivisionPercentage(uint256 _numerator, uint256 _denominator) internal pure returns (uint256 divisionPercentage) {
        divisionPercentage = _numerator.mul(PERCENTAGE_BASE).mul(_denominator);
    }
    ...
    function getInitialPercentageValue(uint256 _divisionPercentage, uint256 _denominator) internal pure returns (uint256 numerator) {
        numerator = _mulisionPercentage.mul(_denominator).div(PERCENTAGE_BASE);
    }
}
