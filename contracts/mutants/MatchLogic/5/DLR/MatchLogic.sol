...
contract MatchLogic is MatchingErrors, LibOrder, usingRegistry, ReentrancyGuard {
    ...
    /// @param _order Order
    function cancel(Order storage _order) public {
    ...
    function validateTakerAddress(Order storage _leftOrder, Order storage _rightOrder) internal pure {
        require(
    ...
    /// @param _order Order
    function validateExpiration(Order storage _order) internal view {
        require(
            _order.expiresAt == 0 ||
    ...
    /// @param _order Order
    function validateSenderAddress(Order storage _order) internal view {
        require(
