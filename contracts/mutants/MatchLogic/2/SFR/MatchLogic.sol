...
contract MatchLogic is MatchingErrors, LibOrder, usingRegistry, ReentrancyGuard {
    ...
    function takeFees(bytes32 _orderHash, Order memory _order) internal {
        ...
        uint256 fees = _order.relayerFee.sub(_order.affiliateFee);
        ...
        if (_order.relayerAddress != address(0)) {
            balances[_order.relayerAddress][_order.feeTokenAddress] = balances[_order.relayerAddress][_order.feeTokenAddress].sub(_order.relayerFee);
        } else {
