...
library DODOMath {
    ...
    */
    function _GeneralIntegrate(
        ...
        uint256 fairAmount = i.add(V1.add(V2)); // i*delta
        if (k == 0) {
            return fairAmount.mul(DecimalMath.ONE);
        }
        uint256 V0V0V1V2 = DecimalMath.divFloor(V0.mul(V0).mul(V1), V2);
        uint256 penalty = DecimalMath.mulFloor(k, V0V0V1V2); // k(V0^2/V1/V2)
        return DecimalMath.ONE.sub(k).add(penalty).mul(fairAmount).mul(DecimalMath.ONE2);
    }
