...
contract PriceOracleProxy is PriceOracle {
    ...
    address public constant usdcOracleKey = address(0);

    /// @notice Handpicked key for DAI
    address public constant daiOracleKey = address(1);
    ...
    function getUnderlyingPrice(RToken rToken) public view returns (uint) {
        ...
        if (rTokenAddress == cSaiAddress) {
            ...
            return saiPrice > 1 ? saiPrice : v1PriceOracle.assetPrices(daiOracleKey);
        }
    ...
    function setSaiPrice(uint price) public {
        ...
        require(saiPrice == 1, "SAI price may only be set once");
