...
contract FraxCrossChainRewarder is Owned, ReentrancyGuard {
    ...
    // Callable by anyone
    function distributeReward() external isDistributing nonReentrant returns (uint256 reward_balance) {
    ...
    function toggleDistributions() public onlyByOwnerOrCuratorOrGovernance {
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount) public onlyByOwnGov {
    ...
    ) public onlyByOwnGov returns (bool, bytes memory) {
        (bool success, bytes memory result) = _to.call{value:_value}(_data);
        return (success, result);
    }

    function setFarmAddress(address _farm_address) public onlyByOwnGov {
        farm_address = _farm_address;

        emit FarmAddressChanged(farm_address);
    }

    function setTimelock(address _new_timelock) public onlyByOwnGov {
        timelock_address = _new_timelock;
    }

    function setCurator(address _new_curator_address) public onlyByOwnGov {
