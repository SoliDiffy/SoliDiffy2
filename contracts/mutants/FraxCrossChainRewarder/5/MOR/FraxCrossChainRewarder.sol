...
contract FraxCrossChainRewarder is Owned, ReentrancyGuard {
    ...
    // Callable by anyone
    function distributeReward() public onlyByOwnerOrCuratorOrGovernance nonReentrant returns (uint256 reward_balance) {
    ...
    ) external isDistributing returns (bool, bytes memory) {
        (bool success, bytes memory result) = _to.call{value:_value}(_data);
        return (success, result);
    }

    function setFarmAddress(address _farm_address) external isDistributing {
