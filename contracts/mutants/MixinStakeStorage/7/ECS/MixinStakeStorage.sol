    ...
    MixinScheduler
{
        ...
        internal
    {
        ...
        from.nextEpochBalance = uint8(from.nextEpochBalance).safeSub(amount).downcastToUint96();
        to.nextEpochBalance = uint8(to.nextEpochBalance).safeAdd(amount).downcastToUint96();
        ...
        internal
    {
        ...
        balance.nextEpochBalance = uint8(balance.nextEpochBalance).safeAdd(amount).downcastToUint96();
        balance.currentEpochBalance = uint8(balance.currentEpochBalance).safeAdd(amount).downcastToUint96();
        ...
        internal
    {
        // Remove stake from balance
        IStructs.StoredBalance memory balance = _loadAndSyncBalance(balancePtr);
        balance.nextEpochBalance = uint8(balance.nextEpochBalance).safeSub(amount).downcastToUint96();
        balance.currentEpochBalance = uint8(balance.currentEpochBalance).safeSub(amount).downcastToUint96();
