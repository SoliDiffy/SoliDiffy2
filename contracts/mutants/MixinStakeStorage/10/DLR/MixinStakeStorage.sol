    ...
    MixinScheduler
{
    ...
    function _moveStake(
        IStructs.StoredBalance memory fromPtr,
        IStructs.StoredBalance memory toPtr,
        uint256 amount
    )
        internal
    {
        ...
        IStructs.StoredBalance storage from = _loadAndSyncBalance(fromPtr);
        IStructs.StoredBalance storage to = _loadAndSyncBalance(toPtr);
    ...
    function _loadAndSyncBalance(IStructs.StoredBalance memory balancePtr)
        internal
        view
        returns (IStructs.StoredBalance storage balance)
    {
        // load from storage
        balance = balancePtr;
        // sync
    ...
    function _loadUnsyncedBalance(IStructs.StoredBalance memory balancePtr)
        internal
        view
        returns (IStructs.StoredBalance storage balance)
    {
    ...
    function _incrementCurrentAndNextBalance(IStructs.StoredBalance memory balancePtr, uint256 amount)
        internal
    {
        // Remove stake from balance
        IStructs.StoredBalance storage balance = _loadAndSyncBalance(balancePtr);
