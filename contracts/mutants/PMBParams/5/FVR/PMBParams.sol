...
contract PMBParams is Ownable{
  ...
  function changeWithdrawFeeRatio(uint256 _ratio) external onlyOwner{
    require(_ratio < ratio_base, "too large");
    withdraw_fee_ratio = _ratio;
  }

  function changeMortgageRatio(uint256 _ratio) external onlyOwner{
    require(_ratio > ratio_base, "too small");
    mortgage_ratio = _ratio;
  }

  function changeLiquidateFeeRatio(uint256 _ratio) external onlyOwner{
    require(_ratio < ratio_base, "too large");
    liquidate_fee_ratio = _ratio;
  }
  function changeMinimumDepositAmount(uint256 _amount) external onlyOwner{
