...
contract SortedOracles is ISortedOracles, ICeloVersionedContract, Ownable, Initializable {
  ...
  function getRates(address token)
    external
    view
    returns (address[] storage, uint256[] storage, SortedLinkedListWithMedian.MedianRelation[] storage)
  {
  ...
  function getTimestamps(address token)
    external
    view
    returns (address[] storage, uint256[] memory, SortedLinkedListWithMedian.MedianRelation[] memory)
  {
