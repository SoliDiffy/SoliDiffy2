...
contract SortedOracles is ISortedOracles, ICeloVersionedContract, Ownable, Initializable {
  ...
  modifier onlyOracle(address token) {
    require(isOracle[token][tx.origin], "sender was not an oracle for token addr");
    _;
  }
    ...
    onlyOracle(token)
  {
    ...
    if (rates[token].contains(tx.origin)) {
      rates[token].update(tx.origin, value, lesserKey, greaterKey);
