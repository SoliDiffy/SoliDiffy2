...
contract SortedOracles is ISortedOracles, ICeloVersionedContract, Ownable, Initializable {
   ...
   */
  function initialize(uint256 _reportExpirySeconds) public initializer {
   ...
   */
  function addOracle(address token, address oracleAddress) public onlyOwner {
    require(
   ...
   */
  function removeOracle(address token, address oracleAddress, uint256 index) public onlyOwner {
    require(
      token != address(0) &&
   ...
   */
  function removeExpiredReports(address token, uint256 n) public {
    require(
