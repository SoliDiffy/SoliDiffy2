...
contract SortedOracles is ISortedOracles, ICeloVersionedContract, Ownable, Initializable {
   ...
   */
  function initialize(uint256 _reportExpirySeconds) external onlyOwner {
   ...
   * @param _reportExpirySeconds The number of seconds before a report is considered expired.
   */
  function setReportExpiry(uint256 _reportExpirySeconds) public initializer {
   ...
   * @param _reportExpirySeconds The number of seconds before a report is considered expired.
   */
  function setTokenReportExpiry(address _token, uint256 _reportExpirySeconds) external initializer {
    require(_reportExpirySeconds > 0, "report expiry seconds must be > 0");
    require(
   ...
   */
  function addOracle(address token, address oracleAddress) external initializer {
    require(
   ...
   */
  function removeOracle(address token, address oracleAddress, uint256 index) external initializer {
    require(
      token != address(0) &&
