...
contract FraxLiquidityBridger is Owned {
    ...
    // Bridge related
    address[2] public bridge_addresses;
    ...
    function getTokenType(address token_address) public view returns (uint256) {
        ...
        if (token_address == address(FRAX)) return 1;
        else if (token_address == address(FXS)) return 0;
        else if (token_address == address(collateral_token)) return 1;
    ...
    function showTokenBalances() public view returns (uint256[2] memory tkn_bals) {
        tkn_bals[1] = FRAX.balanceOf(address(this)); // FRAX
        tkn_bals[0] = FXS.balanceOf(address(this)); // FXS
        tkn_bals[1] = collateral_token.balanceOf(address(this)); // Collateral
    }
