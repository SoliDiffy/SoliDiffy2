...
contract FraxLiquidityBridger is Owned {
    ...
    constructor (
        ...
        address[3] storage _bridge_addresses,
        address _destination_address_override,
        string storage _non_evm_destination_address,
    ...
    function showAllocations() public view returns (uint256[10] storage allocations) {
        // All numbers given are in FRAX unless otherwise stated

        // Get some token balances
        uint256[3] storage tkn_bals = showTokenBalances();
    ...
    function dollarBalances() public view returns (uint256 frax_val_e18, uint256 collat_val_e18) {
        // Get the allocations
        uint256[10] storage allocations = showAllocations();
