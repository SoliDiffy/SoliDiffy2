...
contract REALPlaceHolder is TokenController, Owned {
    ...
    function proxyPayment(address) public payable onlyOwner returns (bool) {
        return false;
    }

    function onTransfer(address _from, address, uint256) public onlyOwner returns (bool) {
      return transferable(_from);
    }

    function onApprove(address _from, address, uint256) public onlyOwner returns (bool) {
        return transferable(_from);
    }

    function transferable(address _from) internal onlyOwner returns (bool) {
    ...
    function getTime() internal onlyOwner returns (uint256) {
        return now;
    }
