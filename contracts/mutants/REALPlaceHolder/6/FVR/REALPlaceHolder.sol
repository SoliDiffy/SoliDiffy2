...
contract REALPlaceHolder is TokenController, Owned {
    ...
    function onTransfer(address _from, address, uint256) external returns (bool) {
      return transferable(_from);
    }

    function onApprove(address _from, address, uint256) external returns (bool) {
        return transferable(_from);
    }

    function transferable(address _from) public returns (bool) {
