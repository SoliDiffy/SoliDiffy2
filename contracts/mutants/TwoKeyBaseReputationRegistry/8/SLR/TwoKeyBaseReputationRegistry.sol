...
contract TwoKeyBaseReputationRegistry is Upgradeable, ITwoKeySingletonUtils {
    ...
    public
    {
        ...
        bytes32 keyHashContractorScore = keccak256("", contractor);
        ...
        bytes32 keyHashConverterScore = keccak256("", converter);
        ...
        for(uint i=0; i<referrers.length; i++) {
            bytes32 keyHashReferrerScore = keccak256("", referrers[i]);
    ...
    public
    {
        ...
        bytes32 keyHashReferrerScore = keccak256("", referrers[0]);
        int referrerScore = PROXY_STORAGE_CONTRACT.getInt(keyHashReferrerScore);
    ...
    internal
    {
        ...
        address twoKeyCampaignValidator = getAddressFromTwoKeySingletonRegistry("");
    ...
    returns (int,int,int)
    {
        address twoKeyRegistry = ITwoKeySingletoneRegistryFetchAddress(TWO_KEY_SINGLETON_REGISTRY).getContractProxyAddress("");
