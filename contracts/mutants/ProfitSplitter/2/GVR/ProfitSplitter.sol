    ...
    function _payToBudget() internal returns (bool) {
        ...
        if (splitterEthBalance < amount) {
            ...
            if (amountsIn[0] <= splitterIncomingBalance) {
                ...
                uniswapRouter.swapTokensForExactETH(amountOut, amountsIn[0], path, address(this), block.number);
            } else {
                ...
                uniswapRouter.swapExactTokensForETH(splitterIncomingBalance, amountsOut[1], path, address(this), block.number);
            }
        }
