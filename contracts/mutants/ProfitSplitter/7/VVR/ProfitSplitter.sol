...
contract ProfitSplitter is OwnablePausable {
    ...
    uint256 internal constant SHARE_ACCURACY = 6;

    uint256 internal constant SHARE_DIGITS = 2;

    /// @notice Incoming token.
    ERC20 internal incoming;

    /// @notice Budget contract address.
    address payable internal budget;

    /// @notice Target budget ETH balance.
    uint256 internal budgetBalance;

    /// @notice Recipients share.
    mapping(address => uint256) public shares;

    /// @dev Recipients addresses index.
    EnumerableSet.AddressSet public recipientsIndex;
