...
contract BCubePublicSale is WhitelistedRole, ReentrancyGuard {
  ...
  function setAdmin(address _admin) external onlyWhitelistAdmin {
    // Add new admin for whitelisting, and remove msgSender as admin
    addWhitelistAdmin(_admin);
    renounceWhitelistAdmin();
  }

  function setContributionsLimits(uint256 _min, uint256 _max) external onlyWhitelistAdmin {
  ...
  function calcRate() public view returns (uint256, uint8) {
