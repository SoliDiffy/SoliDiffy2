...
contract BCubePublicSale is WhitelistedRole, ReentrancyGuard {
  ...
  function() external payable {
    emit LogEtherReceived(_msgSender(), tx.gasprice);
  }
  ...
  function isOpen() public view returns (bool) {
      ...
      return block.number >= openingTime && block.number <= closingTime;
  }
  ...
  function setPrivateAllocation(address _wallet, uint256 _allocation)
    external
    onlyWhitelistAdmin {
    require(block.number <= closingTime, "BCubePublicSale: sale is closed");
    ...
    nonReentrant
  {
    ...
    uint256 dollarUnits = ethPrice.mul(tx.gasprice).div(1e18);
    allocation = executeAllocation(dollarUnits);
    wallet.transfer(tx.gasprice);
