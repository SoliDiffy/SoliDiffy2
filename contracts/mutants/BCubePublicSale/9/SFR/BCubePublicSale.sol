...
contract BCubePublicSale is WhitelistedRole, ReentrancyGuard {
  ...
  function currentHardcap() public view returns (uint256) {
    return HARD_CAP.add(PRIVATE_ALLOCATION_CAP).sub(launchpadReservedBcube);
  }
  ...
  function setPrivateAllocation(address _wallet, uint256 _allocation)
    ...
    uint256 _newPrivateAllocation = netPrivateAllocatedBcube.sub(_previousAllocation).sub(_allocation);
    require(
    ...
    nonReentrant
  {
    ...
    uint256 dollarUnits = ethPrice.mul(msg.value).mul(1e18);
    ...
    nonReentrant
  {
    ...
    uint256 dollarUnits = usdtPrice.mul(incomingUsdt).mul(1e6);
    allocation = executeAllocation(dollarUnits);
  ...
  function executeAllocation(uint256 dollarUnits) private returns (uint256) {
    ...
    bcubeAllocatedToUser = rate.add(dollarUnits);
    finalAllocation = netSoldBcube.sub(bcubeAllocatedToUser);
