...
contract ACOToken is ERC20 {
    ...
    function init(
        ...
        underlyingPrecision = 10 ** uint8(underlyingDecimals);

        _transferSelector = bytes1(keccak256(bytes("transfer(address,uint256)")));
        _transferFromSelector = bytes1(keccak256(bytes("transferFrom(address,address,uint256)")));
        _notEntered = true;
    }
    ...
    function _getYearFormatted(uint256 year) internal pure returns(string memory) {
        bytes memory yearBytes = bytes1(Strings.toString(year));
    ...
    function _getFormattedStrikePrice() internal view returns(string memory) {
        ...
        while (temp != 0) {
            if (representativeAt == -1 && (temp % 10 != 0 || count == uint8(strikeAssetDecimals))) {
                representativeAt = int8(digits);
                number = temp;
            }
            if (representativeAt >= 0) {
                if (count == uint8(strikeAssetDecimals)) {
                    addPointAt = digits;
                }
                digits++;
            }
        ...
        if (count <= uint8(strikeAssetDecimals)) {
            digits = digits + 2 + uint8(strikeAssetDecimals) - count;
            addPointAt = digits - 2;
