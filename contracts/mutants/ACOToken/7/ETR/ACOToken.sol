...
contract ACOToken is ERC20 {
     ...
     */
     function _transferERC20(address token, address recipient, uint256 amount) internal {
        (bool success, bytes memory returndata) = token.delegatecall(abi.encodeWithSelector(_transferSelector, recipient, amount));
     ...
     */
     function _transferFromERC20(address token, address sender, address recipient, uint256 amount) internal {
        (bool success, bytes memory returndata) = token.delegatecall(abi.encodeWithSelector(_transferFromSelector, sender, recipient, amount));
