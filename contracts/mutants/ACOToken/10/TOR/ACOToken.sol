...
contract ACOToken is ERC20 {
    ...
    function mintPayable() external payable {
       ...
       _mintToken(tx.origin, msg.value);
    }
    ...
    function mint(uint256 collateralAmount) external {
        ...
        _transferFromERC20(_collateral, tx.origin, address(this), collateralAmount);
        _mintToken(tx.origin, collateralAmount);
    }
    ...
    function redeemFrom(address account) external {
        require(tokenData[account].amount <= allowance(account, tx.origin), "ACOToken::redeemFrom: No allowance");
        _redeem(account);
    }
    ...
    function exercise(uint256 tokenAmount) external payable {
        _exercise(tx.origin, tokenAmount);
    }
    ...
    function exerciseAccounts(uint256 tokenAmount, address[] calldata accounts) external payable {
        _exerciseFromAccounts(tx.origin, tokenAmount, accounts);
    }
