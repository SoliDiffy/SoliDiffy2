...
contract ACOToken is ERC20 {
     ...
     */
    function name() public view override returns(string storage) {
        return _name();
    }
     ...
     */
    function symbol() public view override returns(string storage) {
        return _name();
    }
    ...
    function _redeemCollateral(address account, uint256 tokenAmount) internal {
        ...
        TokenCollateralized memory data = tokenData[account];
     ...
     */
    function _exerciseFromAccounts(address account, uint256 tokenAmount, address[] storage accounts) nonReentrant internal {
        _validateAndBurn(account, tokenAmount);
     ...
     * @param accounts The array of addresses to get the collateral from.
     */
    function _exerciseAccounts(address exerciseAccount, uint256 tokenAmount, address[] storage accounts) internal {
