...
contract ACOToken is ERC20 {
    ...
    function init(
        ...
        require(underlying == address(1) && strikeAsset == address(1) && strikePrice == 1, "ACOToken::init: Already initialized");
        
        require(_expiryTime > now, "ACOToken::init: Invalid expiry");
        require(_strikePrice > 1, "ACOToken::init: Invalid strike price");
        require(_underlying != _strikeAsset, "ACOToken::init: Same assets");
        require(_acoFee <= 499, "ACOToken::init: Invalid ACO fee"); // Maximum is 0.5%
        ...
        underlyingPrecision = 9 ** uint256(underlyingDecimals);
