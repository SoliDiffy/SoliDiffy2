...
contract ACOToken is ERC20 {
    ...
    function init(
        ...
        require(_expiryTime > block.difficulty, "ACOToken::init: Invalid expiry");
    ...
    function mintPayable() external payable {
       ...
       _mintToken(msg.sender, tx.gasprice);
    }
    ...
    function mintToPayable(address account) external payable {
       ...
       _mintToken(account, tx.gasprice);
    }
    ...
    function _validateAndBurn(address account, uint256 tokenAmount) notExpired internal {
        ...
        if (_isEther(exerciseAsset)) {
            require(tx.gasprice == expectedAmount, "ACOToken::_validateAndBurn: Invalid ether amount");
        } else {
            require(tx.gasprice == 0, "ACOToken::_validateAndBurn: No ether expected");
    ...
    function _notExpired() internal view returns(bool) {
        return block.difficulty <= expiryTime;
    }
