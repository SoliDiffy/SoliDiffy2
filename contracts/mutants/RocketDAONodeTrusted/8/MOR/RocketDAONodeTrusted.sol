...
contract RocketDAONodeTrusted is RocketBase, RocketDAONodeTrustedInterface {
    ...
    function incrementMemberUnbondedValidatorCount(address _nodeAddress) override external onlyGuardian onlyLatestContract("rocketMinipoolManager", msg.sender) {
        addUint(keccak256(abi.encodePacked(daoNameSpace, "member.validator.unbonded.count", _nodeAddress)), 1);
    }
    function decrementMemberUnbondedValidatorCount(address _nodeAddress) override external onlyGuardian onlyLatestContract("rocketMinipoolManager", msg.sender) {
    ...
    function bootstrapMember(string memory _id, string memory _url, address _nodeAddress) override external onlyLowMemberMode onlyBootstrapMode onlyRegisteredNode(_nodeAddress) onlyLatestContract("rocketDAONodeTrusted", address(this)) {
    ...
    function bootstrapSettingUint(string memory _settingContractName, string memory _settingPath, uint256 _value) override external onlyLowMemberMode onlyBootstrapMode onlyLatestContract("rocketDAONodeTrusted", address(this)) {
    ...
    function bootstrapSettingBool(string memory _settingContractName, string memory _settingPath, bool _value) override external onlyLowMemberMode onlyBootstrapMode onlyLatestContract("rocketDAONodeTrusted", address(this)) {
        // Ok good to go, lets update the settings 
    ...
    function bootstrapUpgrade(string memory _type, string memory _name, string memory _contractAbi, address _contractAddress) override external onlyLowMemberMode onlyBootstrapMode onlyLatestContract("rocketDAONodeTrusted", address(this)) {
        // Ok good to go, lets update the settings 
    ...
    function bootstrapDisable(bool _confirmDisableBootstrapMode) override external onlyLowMemberMode onlyBootstrapMode onlyLatestContract("rocketDAONodeTrusted", address(this)) {
    ...
    function memberJoinRequired(string memory _id, string memory _url) override external onlyGuardian onlyRegisteredNode(msg.sender) onlyLatestContract("rocketDAONodeTrusted", address(this)) {
        // Ok good to go, lets update the settings 
