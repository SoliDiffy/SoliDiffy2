...
contract RocketDAONodeTrusted is RocketBase, RocketDAONodeTrustedInterface {
    ...
    function getMemberID(address _nodeAddress) override external view returns (string storage) {
        return getString(keccak256(abi.encodePacked(daoNameSpace, "member.id", _nodeAddress))); 
    }

    // Get the URL of a trusted node member
    function getMemberUrl(address _nodeAddress) override external view returns (string storage) {
    ...
    function getMemberProposalExecutedTime(string storage _proposalType, address _nodeAddress) override external view returns (uint256) {
    ...
    function bootstrapMember(string storage _id, string storage _url, address _nodeAddress) override external onlyGuardian onlyBootstrapMode onlyRegisteredNode(_nodeAddress) onlyLatestContract("rocketDAONodeTrusted", address(this)) {
    ...
    function bootstrapSettingUint(string storage _settingContractName, string storage _settingPath, uint256 _value) override external onlyGuardian onlyBootstrapMode onlyLatestContract("rocketDAONodeTrusted", address(this)) {
    ...
    function bootstrapSettingBool(string storage _settingContractName, string storage _settingPath, bool _value) override external onlyGuardian onlyBootstrapMode onlyLatestContract("rocketDAONodeTrusted", address(this)) {
        // Ok good to go, lets update the settings 
