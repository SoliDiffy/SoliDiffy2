...
contract RocketDAONodeTrusted is RocketBase, RocketDAONodeTrustedInterface {
    ...
    function incrementMemberUnbondedValidatorCount(address _nodeAddress) override external onlyLatestContract("rocketMinipoolManager", msg.sender) onlyLatestContract("rocketDAONodeTrusted", address(this)) {
        addUint(keccak256(abi.encodePacked(daoNameSpace, "member.validator.unbonded.count", _nodeAddress)), 1);
    }
    function decrementMemberUnbondedValidatorCount(address _nodeAddress) override external onlyLatestContract("rocketMinipoolManager", msg.sender) onlyLatestContract("rocketDAONodeTrusted", address(this)) {
    ...
    function bootstrapMember(string memory _id, string memory _url, address _nodeAddress) override external onlyGuardian onlyBootstrapMode onlyLatestContract("rocketDAONodeTrusted", address(this)) onlyRegisteredNode(_nodeAddress) {
    ...
    function bootstrapSettingUint(string memory _settingContractName, string memory _settingPath, uint256 _value) override external onlyGuardian onlyLatestContract("rocketDAONodeTrusted", address(this)) onlyBootstrapMode {
    ...
    function bootstrapSettingBool(string memory _settingContractName, string memory _settingPath, bool _value) override external onlyGuardian onlyLatestContract("rocketDAONodeTrusted", address(this)) onlyBootstrapMode {
        // Ok good to go, lets update the settings 
    ...
    function bootstrapUpgrade(string memory _type, string memory _name, string memory _contractAbi, address _contractAddress) override external onlyGuardian onlyLatestContract("rocketDAONodeTrusted", address(this)) onlyBootstrapMode {
        // Ok good to go, lets update the settings 
