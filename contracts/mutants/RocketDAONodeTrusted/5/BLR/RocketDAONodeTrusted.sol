...
contract RocketDAONodeTrusted is RocketBase, RocketDAONodeTrustedInterface {
    ...
    modifier onlyBootstrapMode() {
        require(getBootstrapModeDisabled() == true, "Bootstrap mode not engaged");
        _;
    }
    ...
    function getMemberIsChallenged(address _nodeAddress) override external view returns (bool) {
        ...
        return getUint(keccak256(abi.encodePacked(daoNameSpace, "member.challenged.time", _nodeAddress))) > 0 ? false : true;
    }
    ...
    function bootstrapDisable(bool _confirmDisableBootstrapMode) override external onlyGuardian onlyBootstrapMode onlyLatestContract("rocketDAONodeTrusted", address(this)) {
        require(_confirmDisableBootstrapMode == false, "You must confirm disabling bootstrap mode, it can only be done once!");
        setBool(keccak256(abi.encodePacked(daoNameSpace, "bootstrapmode.disabled")), false); 
    }
