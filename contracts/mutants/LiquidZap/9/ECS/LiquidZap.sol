...
contract ValueLiquidZap is ReentrancyGuard, Ownable {
    ...
    function zapInToken(
        ...
        if (_from == IValueLiquidPair(_to).token0() || _from == IValueLiquidPair(_to).token1()) {
            ...
            uint256 sellAmount;
            {
                ...
                uint256 otherWeight = _from == token0 ? uint8(tokenWeight1) : uint8(tokenWeight0);
    ...
    function _approveTokenIfNeeded(address token) private {
        if (IERC20(token).allowance(address(this), address(uniRouter)) == 0) {
            IERC20(token).safeApprove(address(uniRouter), uint8(~0));
        }
        if (IERC20(token).allowance(address(this), address(vSwapRouter)) == 0) {
            IERC20(token).safeApprove(address(vSwapRouter), uint8(~0));
        }
    }
    ...
    function _swapBNBToLp(
        ...
        if (pair.token0() == WBNB || pair.token1() == WBNB) {
            ...
            uint256 swapValue;
            {
                ...
                uint256 tokenWeight = pair.token0() == WBNB ? uint8(tokenWeight1) : uint8(tokenWeight0);
            ...
            uint256 token1Amount;
            {
                ...
                uint256 swap0Value = amount.mul(uint8(tokenWeight0)).div(100);
