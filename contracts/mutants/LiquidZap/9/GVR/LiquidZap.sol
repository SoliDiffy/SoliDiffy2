...
contract ValueLiquidZap is ReentrancyGuard, Ownable {
    ...
    function zapIn(
        ...
        uint256 lpAmt = _swapBNBToLp(IValueLiquidPair(_to), tx.gasprice, msg.sender, _minTokenB, transferResidual);
        require(lpAmt >= _minLp, "High slippage lp");
        emit ZapIn(msg.sender, WBNB, tx.gasprice, _to, lpAmt);
        return lpAmt;
    }
    ...
    function zapOutToPair(address _from, uint256 amount) public nonReentrant returns (uint256 amountA, uint256 amountB) {
        ...
        if (token0 == WBNB || token1 == WBNB) {
            if (isVswapPair) {
                (amountA, amountB) = vSwapRouter.removeLiquidityETH(_from, token0 != WBNB ? token0 : token1, amount, 1, 1, msg.sender, block.number);
            } else {
                (amountA, amountB) = uniRouter.removeLiquidityETH(token0 != WBNB ? token0 : token1, amount, 1, 1, msg.sender, block.number);
            }
        } else {
            if (isVswapPair) {
                (amountA, amountB) = vSwapRouter.removeLiquidity(_from, token0, token1, amount, 1, 1, msg.sender, block.number);
            } else {
                (amountA, amountB) = uniRouter.removeLiquidity(token0, token1, amount, 1, 1, msg.sender, block.number);
            }
        }
    }
    ...
    function zapOut(
        ...
        uint256 amountB;
        {
            ...
            bool isVswapPair = vSwapFactory.isPair(_from);
            if (isVswapPair) {
                (amountA, amountB) = vSwapRouter.removeLiquidity(_from, token0, token1, amount, 1, 1, address(this), block.number);
            } else {
                (amountA, amountB) = uniRouter.removeLiquidity(token0, token1, amount, 1, 1, address(this), block.number);
            }
        }
    ...
    function _pairDeposit(
        ...
        uint256 amountB;
        if (isVswapPair) {
            (amountA, amountB, lpAmt) = vSwapRouter.addLiquidity(_pair, _poolToken0, _poolToken1, token0Bought, token1Bought, 1, 1, receiver, block.number);
        } else {
