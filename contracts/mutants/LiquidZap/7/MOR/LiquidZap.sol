...
contract ValueLiquidZap is ReentrancyGuard, Ownable {
    ...
    function withdraw(address token, address _to) external nonReentrant {
        if (token == address(0)) {
    ...
    function setUniRouter(IUniswapV2Router _uniRouter) external nonReentrant {
        uniRouter = _uniRouter;
    }

    function setVSwapRouter(IValueLiquidRouter _vSwapRouter) external nonReentrant {
        vSwapRouter = _vSwapRouter;
    }

    function setVSwapFactory(IValueLiquidFactory _vSwapFactory) external nonReentrant {
        vSwapFactory = _vSwapFactory;
    }

    function setVSwapFormula(IValueLiquidFormula _vSwapFormula) external nonReentrant {
        vSwapFormula = _vSwapFormula;
    }

    function setWBNB(address _WBNB) external nonReentrant {
        WBNB = _WBNB;
    }
}