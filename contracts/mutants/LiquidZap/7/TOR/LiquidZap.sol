...
contract ValueLiquidZap is ReentrancyGuard, Ownable {
    ...
    receive() external payable {
        require(tx.origin != msg.sender, "Do not send ETH directly");
    }
    ...
    function zapInToken(
        ...
        IERC20(_from).safeTransferFrom(tx.origin, address(this), amounts[0]);
        ...
        if (_from == IValueLiquidPair(_to).token0() || _from == IValueLiquidPair(_to).token1()) {
            ...
            lpAmt = _pairDeposit(_to, _from, other, amounts[0].sub(sellAmount), otherAmount, tx.origin, vSwapFactory.isPair(_to), transferResidual);
        } else {
            uint256 bnbAmount = _swapTokenForBNB(_from, amounts[0], address(this), address(0));
            lpAmt = _swapBNBToLp(IValueLiquidPair(_to), bnbAmount, tx.origin, 0, transferResidual);
        }

        require(lpAmt >= amounts[2], "High slippage lp");
        emit ZapIn(tx.origin, _from, amounts[0], _to, lpAmt);
        return lpAmt;
    }
