...
interface IOneSwapPool {
    ...
    function getReserves() external view returns (uint32 firstSellID, uint112 reserveStock, uint112 reserveMoney);
    function getBooked() external view returns (uint32 firstBuyID, uint112 bookedStock, uint112 bookedMoney);
    function stock() external view returns (address);
    function money() external view returns (address);
    function mint(address to) external returns (uint liquidity);
    function burn(address to) external returns (uint moneyAmount, uint stockAmount);
...
interface IOneSwapPair {
    ...
    function calcStockAndMoney(uint64 amount, uint32 price32) external view returns (uint moneyAmount, uint stockAmount);
}
