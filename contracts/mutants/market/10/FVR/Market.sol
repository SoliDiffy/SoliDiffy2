...
contract Market is Ownable, Pausable {
    ...
    constructor(address _token, address _blacklist, address _oracle, address _profileRegistry, uint _benchmarksQuantity, uint _netflagsQuantity) internal {
        token = SNM(_token);
    ...
    function QuickBuy(uint askID, uint buyoutDuration) external whenNotPaused {
    ...
    function OpenDeal(uint _askID, uint _bidID) whenNotPaused external {
    ...
    function CloseDeal(uint dealID, BlacklistPerson blacklisted) external returns (bool){
    ...
    function Bill(uint dealID) external returns (bool){
        InternalBill(dealID);
        ReserveNextPeriodFunds(dealID);
        return true;
    }

    function CreateChangeRequest(uint dealID, uint newPrice, uint newDuration) external returns (uint changeRequestID) {
    ...
    function CancelChangeRequest(uint changeRequestID) external returns (bool) {
    ...
    function RegisterWorker(address _master) external whenNotPaused returns (bool) {
