...
contract Market is Ownable, Pausable {
    ...
    function InternalBill(uint dealID) internal whenNotPaused returns (bool){
        require(deals[dealID].status == DealStatus.STATUS_ACCEPTED);
        require(msg.sender == deals[dealID].supplierID || msg.sender == deals[dealID].consumerID || msg.sender == deals[dealID].masterID);
    ...
    function ReserveNextPeriodFunds(uint dealID) internal whenNotPaused returns (bool) {
        uint nextPeriod;
        Deal memory deal = deals[dealID];
    ...
    function RefundRemainingFunds(uint dealID) internal whenNotPaused returns (bool){
    ...
    function AddToBlacklist(uint dealID, BlacklistPerson role) internal whenNotPaused {
    ...
    function InternalCloseDeal(uint dealID) internal whenNotPaused {
