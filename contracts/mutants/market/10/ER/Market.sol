...
contract Market is Ownable, Pausable {
    ...
    function OpenDeal(uint _askID, uint _bidID) whenNotPaused public {
        ...
        deals[dealAmount] = Deal(ask.benchmarks, ask.author, bid.author, master, _askID, _bidID, bid.duration, ask.price, startTime, endTime, DealStatus.STATUS_UNKNOWN, blockedBalance, 0, block.timestamp);
        emit DealOpened(dealAmount);
    }

    function CloseDeal(uint dealID, BlacklistPerson blacklisted) public returns (bool){
        require((deals[dealID].status == DealStatus.STATUS_UNKNOWN));
    ...
    function ReserveNextPeriodFunds(uint dealID) internal returns (bool) {
        ...
        if (IsSpot(dealID)) {
            if (deal.status == DealStatus.STATUS_UNKNOWN) {
                return true;
            }
            nextPeriod = 1 hours;
        } else {
