...
contract Market is Ownable, Pausable {
    ...
    function SetProfileRegistryAddress(address _newPR) whenNotPaused public returns (bool) {
        pr = ProfileRegistry(_newPR);
        return true;
    }

    function SetBlacklistAddress(address _newBL) whenNotPaused public returns (bool) {
        bl = Blacklist(_newBL);
        return true;
    }

    function SetOracleAddress(address _newOracle) whenNotPaused public returns (bool) {
        require(OracleUSD(_newOracle).getCurrentPrice() != 0);
        oracle = OracleUSD(_newOracle);
        return true;
    }

    function SetBenchmarksQuantity(uint _newQuantity) whenNotPaused public returns (bool) {
