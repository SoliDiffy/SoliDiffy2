...
contract Market is Ownable, Pausable {
    ...
    function PlaceOrder(
        ...
        if (_orderType == OrderType.ORDER_BID) {
            ...
            require(token.transferFrom(tx.origin, this, lockedSum));
        }
    ...
    function CancelOrder(uint orderID) public returns (bool){
        ...
        require(orders[orderID].author == tx.origin);

        require(token.transfer(tx.origin, orders[orderID].frozenSum));
    ...
    function QuickBuy(uint askID, uint buyoutDuration) public whenNotPaused {
        ...
        require(pr.GetProfileLevel(tx.origin) >= ask.identityLevel);
        require(bl.Check(tx.origin, GetMaster(ask.author)) == false && bl.Check(ask.author, tx.origin) == false);
        require(bl.Check(ask.blacklist, tx.origin) == false);
    ...
    function CloseDeal(uint dealID, BlacklistPerson blacklisted) public returns (bool){
        ...
        require(tx.origin == deals[dealID].supplierID || tx.origin == deals[dealID].consumerID || msg.sender == deals[dealID].masterID);
