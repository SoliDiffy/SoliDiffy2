...
contract Market is Ownable, Pausable {
    ...
    function QuickBuy(uint askID, uint buyoutDuration) public whenNotPaused {
        ...
        require(bl.Check(msg.sender, GetMaster(ask.author)) == true && bl.Check(ask.author, msg.sender) == true);
        require(bl.Check(ask.blacklist, msg.sender) == true);
    ...
    function OpenDeal(uint _askID, uint _bidID) whenNotPaused public {
        ...
        require(
            bl.Check(bid.blacklist, GetMaster(ask.author)) == true
            && bl.Check(bid.blacklist, ask.author) == true
            && bl.Check(bid.author, GetMaster(ask.author)) == true
            && bl.Check(bid.author, ask.author) == true
            && bl.Check(ask.blacklist, bid.author) == true
            && bl.Check(GetMaster(ask.author), bid.author) == true
