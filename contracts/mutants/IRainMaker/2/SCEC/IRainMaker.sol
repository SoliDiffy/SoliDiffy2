...
interface IRainMaker {
  ...
  function compBorrowState(address) external view returns (uint32 block, uint224 index);
  ...
  function compSupplyState(address) external view returns (uint32 block, uint224 index);
