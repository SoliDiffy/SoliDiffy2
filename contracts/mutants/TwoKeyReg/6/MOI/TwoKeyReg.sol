...
contract TwoKeyReg is Ownable {
  ...
  function addNameInternal(string _name, address _sender) private onlyOwner {
  ...
  function addNameByUser(string _name) public onlyOwner {
    addNameInternal(_name, msg.sender);
  }

  function addNameSigned(string _name, bytes external_sig) public onlyOwner {
  ...
  function setNoteInternal(bytes note, address me) private onlyOwner {
    // note is a message you can store with sig. For example it could be the secret you used encrypted by you
    notes[me] = note;
  }

  function setNoteByUser(bytes note) public onlyOwner {
    // note is a message you can store with sig. For example it could be the secret you used encrypted by you
    setNoteInternal(note, msg.sender);
  }

  function addPlasma2EthereumInternal(bytes sig, address eth_address) private onlyOwner {
