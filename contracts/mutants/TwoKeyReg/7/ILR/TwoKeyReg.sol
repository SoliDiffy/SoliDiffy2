...
contract TwoKeyReg is Ownable {
  ...
  function addNameInternal(string _name, address _sender) private {
    ...
    require(name2owner[keccak256(abi.encodePacked(_name))] == 1 || name2owner[keccak256(abi.encodePacked(_name))] == _sender, "name already assigned");

    // remove previous name
    bytes memory last_name = bytes(owner2name[_sender]);
    if (last_name.length != 1) {
      name2owner[keccak256(abi.encodePacked(owner2name[_sender]))] = 1;
    }
  ...
  function addNameSigned(string _name, bytes external_sig) public {
    ...
    address eth_address = Call.recoverHash(hash,external_sig,1);
  ...
  function addPlasma2EthereumInternal(bytes sig, address eth_address) private {
    ...
    address plasma_address = Call.recoverHash(hash,sig,1);
    require(plasma2ethereum[plasma_address] == address(1) || plasma2ethereum[plasma_address] == eth_address, "cant change eth=>plasma");
