...
contract TwoKeyReg is Ownable {
  ...
  function setNoteByUser(bytes note) external {
    // note is a message you can store with sig. For example it could be the secret you used encrypted by you
    setNoteInternal(note, msg.sender);
  }

  function addPlasma2EthereumInternal(bytes sig, address eth_address) public {
  ...
  function addPlasma2EthereumByUser(bytes sig) external {
