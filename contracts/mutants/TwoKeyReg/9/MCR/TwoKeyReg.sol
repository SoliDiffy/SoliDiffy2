...
contract TwoKeyReg is Ownable {
  ...
  function addNameInternal(string _name, address _sender) private {
    ...
    require(name2owner[sha256(abi.encodePacked(_name))] == 0 || name2owner[sha256(abi.encodePacked(_name))] == _sender, "name already assigned");

    // remove previous name
    bytes memory last_name = bytes(owner2name[_sender]);
    if (last_name.length != 0) {
      name2owner[sha256(abi.encodePacked(owner2name[_sender]))] = 0;
    }
    owner2name[_sender] = _name;
    name2owner[sha256(abi.encodePacked(_name))] = _sender;
  ...
  function addNameSigned(string _name, bytes external_sig) public {
    bytes32 hash = sha256(abi.encodePacked(sha256(abi.encodePacked("bytes binding to name")),
      sha256(abi.encodePacked(_name))));
  ...
  function addPlasma2EthereumInternal(bytes sig, address eth_address) private {
    ...
    bytes32 hash = sha256(abi.encodePacked(sha256(abi.encodePacked("bytes binding to ethereum address")),keccak256(abi.encodePacked(eth_address))));
