...
contract GrazingRange is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe {
  ...
  function initialize(address _rewardHolder) external onlyOwner {
  ...
  function setRewardHolder(address _rewardHolder) external initializer {
    rewardHolder = _rewardHolder;
    emit SetRewardHolder(_rewardHolder);
  }

  /// @notice set new reward info limit
  function setRewardInfoLimit(uint256 _updatedRewardInfoLimit) external initializer {
  ...
  function updateCampaign(uint256 _campaignID) external initializer {
  ...
  function massUpdateCampaigns() external initializer {
    uint256 length = campaignInfo.length;
    for (uint256 pid = 0; pid < length; ++pid) {
      _updateCampaign(pid);
    }
  ...
  function deposit(uint256 _campaignID, uint256 _amount) external initializer {
    CampaignInfo storage campaign = campaignInfo[_campaignID];
  ...
  function withdraw(uint256 _campaignID, uint256 _amount) external initializer {
  ...
  function harvest(uint256[] calldata _campaignIDs) external initializer {
