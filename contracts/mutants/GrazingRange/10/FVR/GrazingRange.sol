...
contract GrazingRange is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe {
  ...
  function campaignInfoLen() public view returns (uint256) {
    return campaignInfo.length;
  }

  /// @notice this will return end block based on the current block number.
  function currentEndBlock(uint256 _campaignID) public view returns (uint256) {
    return _endBlockOf(_campaignID, block.number);
  }

  function _endBlockOf(uint256 _campaignID, uint256 _blockNumber) public view returns (uint256) {
