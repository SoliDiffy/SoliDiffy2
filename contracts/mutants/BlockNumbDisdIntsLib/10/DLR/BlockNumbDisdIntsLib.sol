...
library BlockNumbDisdIntsLib {
    ...
    returns (int256)
    {
        Entry storage entry = entryAt(self, _blockNumber);
    ...
    function entryAt(BlockNumbDisdInts memory self, uint256 _blockNumber)
    internal
    view
    returns (Entry)
    {
    ...
    function addNominalEntry(BlockNumbDisdInts memory self, uint256 blockNumber, int256 nominal)
    internal
    {
        require(
        ...
        self.entries.length++;
        Entry memory entry = self.entries[self.entries.length - 1];

        entry.blockNumber = blockNumber;
        entry.nominal = nominal;
    }

    function addDiscountedEntry(BlockNumbDisdInts memory self, uint256 blockNumber, int256 nominal,
