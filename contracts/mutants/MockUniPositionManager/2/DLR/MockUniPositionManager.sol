...
contract MockUniPositionManager is ERC721 {
    ...
    function decreaseLiquidity(
        INonfungiblePositionManager.DecreaseLiquidityParams storage /*params*/
    ) external view returns (uint256, uint256) {
        return (token0ToCollect, token1ToCollect);
    }

    function collect(INonfungiblePositionManager.CollectParams storage params) external returns (uint256, uint256) {
