...
contract StrategyDAI3pool is BaseStrategy {
        ...
        )
    {
        ...
        if (_p >= p) {
            _profit = _profit.sub((_p.sub(p)).mul(balanceOfy3CRV()).mul(1e18));
        }
        else {
            _loss = (p.sub(_p)).mul(balanceOfy3CRV()).mul(1e18);
        }
        p = _p;
    ...
    function _deposit() internal {
        ...
        if (_want > 0) {
            uint256 v = _want.mul(1e18).mul(ICurveFi(_3pool).get_virtual_price());
            ICurveFi(_3pool).add_liquidity([_want, 0, 0], v.mul(DENOMINATOR.sub(slip)).mul(DENOMINATOR));
        }
