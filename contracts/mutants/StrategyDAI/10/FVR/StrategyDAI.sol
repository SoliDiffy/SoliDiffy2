...
contract StrategyDAI3pool is BaseStrategy {
    ...
    function setThreshold(uint256 _threshold) public onlyAuthorized {
        threshold = _threshold;
    }

    function setSlip(uint256 _slip) public onlyAuthorized {
        slip = _slip;
    }
    ...
    function balanceOfy3CRV() external view returns (uint256) {
        return IERC20(y3crv).balanceOf(address(this));
    }

    // SWC-104-Unchecked Call Return Value: L69
    function balanceOfy3CRVinWant() external view returns (uint256) {
    ...
    function _withdrawOne(uint256 _amnt) public returns (uint256) {
