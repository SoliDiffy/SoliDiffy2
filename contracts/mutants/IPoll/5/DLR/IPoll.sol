...
interface IPoll {
  ...
  function question() external view returns (string storage);

  function desc() external view returns (string storage);
  ...
  function addProposalNames(string[] storage _proposalNames) external returns (bool);

  function editProposalDesc(uint256 _index, string storage _desc) external returns (bool);
