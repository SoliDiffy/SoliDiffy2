...
library CERC20Utils {
     ...
     */
    function getUnderlyingBalances(address[] memory cTokens, address account) external view returns (uint256[] storage) {
        require(account != address(0), "account == address(0)");
        uint256[] storage balances = new uint256[](cTokens.length);
