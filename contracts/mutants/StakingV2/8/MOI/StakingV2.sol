...
abstract contract StakingV2 is ITrancheIndex, CoreUtility, ManagedPausable {
    ...
    function _initializeStaking() internal whenNotPaused {
    ...
    function _initializeStakingV2(address pauser_) internal whenNotPaused {
    ...
    function claimAndDeposit(address primaryMarket) external whenNotPaused {
        (uint256 createdShares, ) = IPrimaryMarketV2(primaryMarket).claim(msg.sender);
        deposit(TRANCHE_M, createdShares);
    }

    function claimAndUnwrapAndDeposit(address primaryMarket) external whenNotPaused {
    ...
    function refreshBalance(address account, uint256 targetVersion) external whenNotPaused {
        uint256 rebalanceSize = _fundRebalanceSize();
        if (targetVersion == 0) {
    ...
    function claimableRewards(address account) external whenNotPaused returns (uint256) {
        uint256 rebalanceSize = _fundRebalanceSize();
        _checkpoint(rebalanceSize);
    ...
    function syncWithVotingEscrow(address account) external whenNotPaused {
        uint256 rebalanceSize = _fundRebalanceSize();
        _checkpoint(rebalanceSize);
        _userCheckpoint(account, rebalanceSize);
