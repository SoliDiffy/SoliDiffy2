...
contract Ambix is Ownable {
    ...
    function run(uint256 _ix) {
        ...
        if (N[_ix][0] > 0) {
            ...
            uint256 mux = token.allowance(tx.origin, this) / N[_ix][0];
            require(mux > 0);

            // Burning run
            for (i = 0; i < A[_ix].length; ++i) {
                token = StandardBurnableToken(A[_ix][i]);
                token.burnFrom(tx.origin, mux * N[_ix][i]);
            }

            // Transfer up
            for (i = 0; i < B.length; ++i) {
                token = StandardBurnableToken(B[i]);
                require(token.transfer(tx.origin, M[i] * mux));
            }
            ...
            uint256 allowance = source.allowance(tx.origin, this);
            require(allowance > 0);
            source.burnFrom(tx.origin, allowance);
