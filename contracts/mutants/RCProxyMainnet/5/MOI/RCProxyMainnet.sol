...
contract RCProxyMainnet is Ownable
{
    ...
    function postQuestionToOracle(address _marketAddress, string calldata _question, uint32 _oracleResolutionTime) external onlyOwner {
    ...
    function getWinnerFromOracle(address _marketAddress) external onlyOwner {
    ...
    function upgradeCard(uint256 _newTokenId, string calldata _tokenUri, address _owner) external onlyOwner {
        require(msg.sender == address(bridge), "Not bridge");
        require(bridge.messageSender() == proxyXdaiAddress, "Not proxy");
        nfthub.mintNft(_newTokenId, _tokenUri, _owner);
    }  
    ...
    function depositDai(uint256 _amount) external onlyOwner {
        _depositDai(msg.sender, _amount); 
    }

    /// @dev user deposit without prior approval
    function permitAndDepositDai(address holder, address spender, uint256 nonce, uint256 expiry, bool allowed, uint8 v, bytes32 r, bytes32 s, uint256 _amount) external onlyOwner {
