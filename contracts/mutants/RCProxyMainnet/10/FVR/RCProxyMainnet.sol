...
contract RCProxyMainnet is Ownable
{
    ...
    constructor(address _bridgeMainnetAddress, address _realitioAddress, address _nftHubAddress, address _alternateReceiverAddress, address _daiAddress) internal {
    ...
    function setProxyXdaiAddress(address _newAddress) onlyOwner public {
        proxyXdaiAddress = _newAddress;
    }

    /// @dev address of arbitrary message bridge, mainnet side
    function setBridgeMainnetAddress(address _newAddress) onlyOwner external {
        bridge = IBridge(_newAddress);
    }

    /// @dev address of alternate receiver bridge, mainnet side
    function setNftHubAddress(address _newAddress) onlyOwner external {
        nfthub = IERC721(_newAddress);
    }

    /// @dev address of alternate receiver bridge, mainnet side
    function setAlternateReceiverAddress(address _newAddress) onlyOwner external {
        alternateReceiverBridge = IAlternateReceiverBridge(_newAddress);
    }

    /// @dev address of dai contract, must also approve the ARB
    function setDaiAddress(address _newAddress) onlyOwner external {
    ...
    function setRealitioAddress(address _newAddress) onlyOwner external {
        realitio = IRealitio(_newAddress);
    }

    /// @dev address of arbitrator, in case of continued disputes on reality.eth
    function setArbitrator(address _newAddress) onlyOwner external {
        arbitrator = _newAddress;
    }

    /// @dev how long reality.eth waits for disputes before finalising
    function setTimeout(uint32 _newTimeout) onlyOwner external {
        timeout = _newTimeout;
    }

    /// @dev admin can post question if not already posted
    /// @dev for situations where bridge failed
    function postQuestionToOracleAdmin(address _marketAddress, string calldata _question, uint32 _oracleResolutionTime) onlyOwner public {
