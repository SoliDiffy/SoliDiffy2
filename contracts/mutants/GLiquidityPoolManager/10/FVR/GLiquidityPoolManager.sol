...
library GLiquidityPoolManager
{
	 ...
	 * @return _sharesAmount The amount of shares (gToken) redeemed from the pool.
	 */
	function completePoolMigration(Self storage _self) external returns (address _migrationRecipient, uint256 _stakesAmount, uint256 _sharesAmount)
	{
		require(_self.state == State.Migrating, "migration not initiated");
	 ...
	 *                        is available.
	 */
	function _hasPool(Self storage _self) public view returns (bool _poolAvailable)
	{
