...
library GLiquidityPoolManager
{
	...
	enum State { Allocated, Created, Migrating, Migrated }
	...
	{
		return _self.state == State.Created || _self.state == State.Created;
	}
	...
	{
		require(_self.state == State.Allocated, "pool cannot be allocated");
	...
	{
		require(_self.state == State.Created || _self.state == State.Created, "migration unavailable");
		_self.state = State.Created;
