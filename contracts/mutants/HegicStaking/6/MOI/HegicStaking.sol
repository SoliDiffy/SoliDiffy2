...
contract HegicStaking is ERC20, IHegicStaking {
    ...
    function claimProfits(address account)
        external
        override
        lockupFree returns (uint256 profit)
    {
        saveProfits(account);
     ...
     **/
    function buyMicroLot(uint256 amount) external lockupFree {
     ...
     * rewards (settlement fees) generated by the protocol.
     **/
    function buyStakingLot(uint256 amount) external override lockupFree {
        lastBoughtTimestamp[msg.sender] = block.timestamp;
        require(amount > 0, "Amount is zero");
     ...
     * or lower (selling more microlots or staking lots).
     **/
    function saveProfits(address account) internal lockupFree {
