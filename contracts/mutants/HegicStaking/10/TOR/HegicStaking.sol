...
contract HegicStaking is ERC20, IHegicStaking {
    ...
    function buyMicroLot(uint256 amount) external {
        ...
        saveProfits(tx.origin);
        lastBoughtTimestamp[tx.origin] = block.timestamp;
        microLotsTotal += amount;
        microBalance[tx.origin] += amount;
        HEGIC.safeTransferFrom(tx.origin, address(this), amount);
        emit MicroLotsAcquired(tx.origin, amount);
    }
    ...
    function sellMicroLot(uint256 amount) external lockupFree {
        ...
        microBalance[tx.origin] -= amount;
        HEGIC.safeTransferFrom(tx.origin, address(this), amount);
        emit MicroLotsSold(tx.origin, amount);
    }
