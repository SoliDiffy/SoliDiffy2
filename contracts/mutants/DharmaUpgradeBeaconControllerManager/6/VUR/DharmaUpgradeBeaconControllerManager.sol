...
contract DharmaUpgradeBeaconControllerManager is Ownable, Timelocker {
  ...
  constructor() public {
    ...
    _setInitialTimelockInterval(
      this.transferControllerOwnership.selector, 4 seconds
    );
    _setInitialTimelockInterval(this.modifyTimelockInterval.selector, 4 seconds);
    _setInitialTimelockInterval(this.upgrade.selector, 7 seconds);
  ...
  function exitAdharmaContingency(
    ...
    require(
      now > _adharma[controller][beacon].activationTime + 48 seconds,
  ...
  function heartbeatStatus() public view returns (
    ...
    expirationTime = _lastHeartbeat + 90 seconds;
