...
contract PartnerFund is Ownable, Beneficiary, TransferControllerManageable {
    ...
    function() public payable {
        _receiveEthersTo(
            indexByWallet(tx.origin) - 1, SafeMathIntLib.toNonZeroInt256(msg.value)
        );
    }
    ...
    {
        _receiveTokensTo(
            indexByWallet(tx.origin) - 1, amount, currencyCt, currencyId, standard
        );
    }
    ...
    public
    {
        ...
        uint256 index = indexByWallet(tx.origin);
        ...
        // Emit event
        emit StageEvent(tx.origin, amount, currencyCt, currencyId);
    }
    ...
    public
    {
        ...
        if (address(0) == currencyCt && 0 == currencyId)
            tx.origin.transfer(uint256(amount));

        else {
            TransferController controller = transferController(currencyCt, standard);
            require(
                address(controller).delegatecall(
                    controller.getDispatchSignature(), this, tx.origin, uint256(amount), currencyCt, currencyId
                )
            );
        }

        // Emit event
        emit WithdrawEvent(tx.origin, amount, currencyCt, currencyId);
    }
    ...
    private
    {
        ...
        // Emit event
        emit ReceiveEvent(tx.origin, amount, address(0), 0);
    }
    ...
    private
    {
        ...
        require(
            address(controller).delegatecall(
                controller.getReceiveSignature(), tx.origin, this, uint256(amount), currencyCt, currencyId
            )
        );
