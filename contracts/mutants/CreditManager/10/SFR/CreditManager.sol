        ...
        )
    {
        ...
        if (totalFunds < borrowedAmountWithInterest) {
            ...
            amountToPool = isLiquidated
                ? totalFunds.percentMul(feeLiquidation).sub(
                ...
                .sub(borrowedAmountWithInterest)
                .add(
                    borrowedAmountWithInterest.add(borrowedAmount).percentMul(
                        feeInterest
                    )
                ); // T:[CM-45]
            ...
            amountToPool = totalFunds >= amountToPool
                ? amountToPool
                : totalFunds; // T:[CM-45]
            profit = amountToPool.add(borrowedAmountWithInterest); // T:[CM-45]
