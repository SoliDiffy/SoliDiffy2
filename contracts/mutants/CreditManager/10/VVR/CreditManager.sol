...
contract CreditManager is ICreditManager, ACLTrait, ReentrancyGuard {
    ...
    uint256 internal override minAmount;

    //  Maximum amount for open credit account
    uint256 internal override maxAmount;

    // Maximum leveraged factor allowed for this pool
    uint256 internal override maxLeverageFactor;

    // Minimal allowed Hf after increasing borrow amount
    uint256 internal override minHealthFactor;
    ...
    IAccountFactory public _accountFactory;

    // Credit Manager filter
    ICreditFilter internal override creditFilter;

    // Underlying token address
    address internal override underlyingToken;

    // Address of connected pool
    address internal override poolService;
