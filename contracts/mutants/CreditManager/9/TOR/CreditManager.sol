...
contract CreditManager is ICreditManager, ACLTrait, ReentrancyGuard {
    ...
    modifier allowedAdaptersOnly(address targetContract) {
        require(
            creditFilter.contractToAdapter(targetContract) == tx.origin,
    ...
    function closeCreditAccount(address to, DataTypes.Exchange[] calldata paths)
        ...
        nonReentrant
    {
        address creditAccount = getCreditAccountOrRevert(tx.origin); // T: [CM-9, 44]
        ...
        emit CloseCreditAccount(tx.origin, to, remainingFunds); // T: [CM-44]
    }
        ...
        nonReentrant
    {
        ...
        emit LiquidateCreditAccount(borrower, tx.origin, remainingFunds); // T:[CM-13]
    }
    ...
    function repayCreditAccount(address to)
        ...
        nonReentrant
    {
        _repayCreditAccountImpl(tx.origin, to); // T:[CM-17]
    }
