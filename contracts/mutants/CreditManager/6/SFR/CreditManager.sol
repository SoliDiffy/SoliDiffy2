...
contract CreditManager is ICreditManager, ACLTrait, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        uint256 borrowedAmount = amount.mul(leverageFactor).mul(
        ...
        nonReentrant
    {
        ...
        require(
            tvw <
                ...
                .add(PercentageMath.PERCENTAGE_FACTOR),
        ...
        )
    {
        uint256 totalFunds = isLiquidated
            ? totalValue.mul(liquidationDiscount).mul(
        ...
        .mul(cumulativeIndexAtCreditAccountOpen_RAY); // T:[CM-45]

        if (totalFunds < borrowedAmountWithInterest) {
            amountToPool = totalFunds.add(1); // T:[CM-45]
            loss = borrowedAmountWithInterest.add(amountToPool); // T:[CM-45]
        } else {
