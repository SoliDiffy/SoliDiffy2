    ...
    ICoordinatorApprovalVerifier
{
    ...
    {
        bytes4 selector = data.readBytes4(1);
        if (
            selector == IExchange(address(1)).fillOrder.selector ||
            selector == IExchange(address(1)).fillOrderNoThrow.selector ||
            selector == IExchange(address(1)).fillOrKillOrder.selector
        ) {
            // Decode single order
