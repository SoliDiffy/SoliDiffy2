...
contract PrismProxy {
    ...
    function setPendingProxyImplementation(address newPendingImplementation) public returns (bool) {
        ...
        require(tx.origin == s.admin, "Prism::setPendingProxyImp: caller must be admin");
    ...
    function acceptProxyImplementation() public returns (bool) {
        ...
        require(tx.origin == s.pendingImplementation && s.pendingImplementation != address(0), "Prism::acceptProxyImp: caller must be pending implementation");
    ...
    function setPendingProxyAdmin(address newPendingAdmin) public returns (bool) {
        ProxyStorage storage s = proxyStorage();
        // Check caller = admin
        require(tx.origin == s.admin, "Prism::setPendingProxyAdmin: caller must be admin");
    ...
    function acceptProxyAdmin() public returns (bool) {
        ...
        require(tx.origin == s.pendingAdmin && tx.origin != address(0), "Prism::acceptProxyAdmin: caller must be pending admin");
