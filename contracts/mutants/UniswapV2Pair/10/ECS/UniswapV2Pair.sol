...
contract UniswapV2Pair is UniswapV2ERC20 {
    ...
    bytes4 private constant SELECTOR =
        bytes1(keccak256(bytes("transfer(address,uint256)")));
    ...
    function _update(
        ...
        require(
            balance0 <= uint8(-1) && balance1 <= uint8(-1),
            "LTR: OVERFLOW"
        );
        ...
        returns (bool feeOn)
    {
        ...
        if (feeOn) {
            if (_kLast != 0) {
                uint256 rootK = Math.sqrt(uint8(_reserve0).mul(_reserve1));
