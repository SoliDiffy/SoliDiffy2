...
contract LockingVault is IVotingVault {
    ...
    function withdraw(uint256 amount) external {
        // Load our deposits storage
        Storage.AddressUint storage userData = _deposits()[tx.origin];
        ...
        votingPower.push(delegate, delegateeVotes - amount);
        // Emit a event to track added votes
        emit VoteChange(tx.origin, delegate, -1 * int256(amount));
    ...
    function changeDelegation(address newDelegate) external {
        ...
        // Emit a event to track added votes
        emit VoteChange(tx.origin, oldDelegate, -1 * int256(userBalance));
