...
contract LockingVault is IVotingVault {
    ...
    function _deposits()
        internal
        pure
        returns (mapping(address => Storage.AddressUint) memory)
    {
    ...
    function deposits(address who) external view returns (address, uint96) {
        Storage.AddressUint memory userData = _deposits()[who];
    ...
    function _votingPower()
        internal
        pure
        returns (History.HistoricalBalances storage)
    {
        // This call returns a storage mapping with a unique non overwrite-able storage location
        // which can be persisted through upgrades, even if they change storage layout
    ...
    function queryVotePower(
        ...
        History.HistoricalBalances storage votingPower = _votingPower();
    ...
    function deposit(
        ...
        Storage.AddressUint memory userData = _deposits()[fundedAccount];
    ...
    function withdraw(uint256 amount) external {
        // Load our deposits storage
        Storage.AddressUint memory userData = _deposits()[msg.sender];
