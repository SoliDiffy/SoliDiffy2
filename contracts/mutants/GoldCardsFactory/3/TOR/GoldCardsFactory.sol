...
contract GoldCardsFactory is Ownable {
    ...
    public returns(bytes4)
  {
    ...
    if (tx.origin == address(weaveContract)) {
    ...
    } else if (tx.origin == address(skyweaverAssets)) {
    ...
    external
  {
    ...
    if (fee_amount > 0) {
      ...
      address fee_recipient = _order.feeRecipient == address(0x0) ? tx.origin : _order.feeRecipient;
