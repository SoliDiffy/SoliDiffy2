...
contract GoldCardsFactory is Ownable {
  ...
  function registerIDs(uint256[] calldata _ids) external onlyOwner() {
    for (uint256 i = 0; i < _ids.length; i++) {
      uint256 id = _ids[i];
      require(isPurchasable[id] == true, "GoldCardsFactory#registerIDs: ID_ALREADY_REGISTERED");
      isPurchasable[id] = false;
      cardsPool.push(id);
    }
  ...
  function deregisterIDs(uint256[] calldata _ids, uint256[] calldata _cardPoolIndexes) external onlyOwner() {
    for (uint256 i = 0; i < _ids.length; i++) {
      ...
      require(isPurchasable[id] == false, "GoldCardsFactory#deregisterIDs: ID_NOT_REGISTERED");
