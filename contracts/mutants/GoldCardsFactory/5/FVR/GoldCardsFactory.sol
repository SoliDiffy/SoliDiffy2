...
contract GoldCardsFactory is Ownable {
   ...
   */
  function registerIDs(uint256[] calldata _ids) public onlyOwner() {
   ...
   */
  function deregisterIDs(uint256[] calldata _ids, uint256[] calldata _cardPoolIndexes) public onlyOwner() {
    for (uint256 i = 0; i < _ids.length; i++) {
      uint256 id = _ids[i];
   ...
   */
  function updateGoldPrice(uint256 _newPrice) public onlyOwner() {
   ...
   */
  function updateGoldRefund(uint256 _newRefund) public onlyOwner() {
