...
library VaultWithIndirection {
  ...
  function totalValue(Data memory _self) internal view returns (uint256) {
  ...
  function token(Data memory _self) internal view returns (IDetailedERC20) {
  ...
  function deposit(Data memory _self, uint256 _amount) internal returns (uint256) {
  ...
  function depositAll(Data memory _self) internal returns (uint256) {
    IDetailedERC20 _token = _self.token();
