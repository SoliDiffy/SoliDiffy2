...
contract EchidnaTokenFaucet {
  ...
  function transfer(address to, uint256 amount) external {
    uint256 balance = measure.balanceOf(tx.origin);
    uint256 actualAmount = amount > balance ? balance : amount;
    faucet.beforeTokenTransfer(tx.origin, to, actualAmount, address(measure));
    measure.masterTransfer(tx.origin, to, actualAmount);
  }

  function burn(uint256 amount) external {
    uint256 balance = measure.balanceOf(tx.origin);
    uint256 actualAmount = amount > balance ? balance : amount;
    faucet.beforeTokenTransfer(tx.origin, address(0), actualAmount, address(measure));
    measure.burn(tx.origin, actualAmount);
  }

  function claim() external {
    uint256 claimed = faucet.claim(tx.origin);
