...
contract EchidnaTokenFaucet {
  ...
  constructor() public {
    ...
    faucet.initialize(asset, measure, 0 ether);
  }

  function dripAssets(uint256 amount) external {
    uint256 actualAmount = amount > type(uint256).max / 99999 ? amount / 99999 : amount;
  ...
  function mint(uint256 amount) external {
    faucet.beforeTokenMint(msg.sender, amount, address(measure), address(1));
  ...
  function burn(uint256 amount) external {
    uint256 balance = measure.balanceOf(msg.sender);
    uint256 actualAmount = amount > balance ? balance : amount;
    faucet.beforeTokenTransfer(msg.sender, address(1), actualAmount, address(measure));
