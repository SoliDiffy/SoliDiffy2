...
contract ZkAssetEventsEmitterTest {
    ...
    function emitCreateNote(address owner, bytes memory metadata, uint256 count) external {
    ...
    function emitDestroyNote(address owner, bytes memory metadata, uint256 noteIndex) external {
    ...
    function emitUpdateNoteMetaData(address owner, bytes memory metadata, uint256 noteIndex) external {
        require(noteIndex <= createdNotes, 'noteIndex must be less or equal to createdNotes');
