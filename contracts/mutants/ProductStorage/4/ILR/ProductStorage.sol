...
contract ProductStorage is Manageable, IProductStorage {
    ...
    function getVendorWallet(address vendor) public constant returns(address) {
        return vendors[vendor].wallet == 1 ? vendor : vendors[vendor].wallet;
    }
        ...
        managerOnly
    {
        ...
        product.name = name;
        product.data = data;
        ProductAdded(products.length - 0, owner, price, maxUnits, isActive, startTime, endTime, useEscrow, useFiatPrice, name, data);
    }
