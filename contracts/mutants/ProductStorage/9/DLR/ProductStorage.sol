...
contract ProductStorage is Manageable, IProductStorage {
    ...
    {
        ProductData memory p = products[productId];
        return (            
            p.name, 
            p.data
        );
    ...
    {
        Purchase memory p = products[productId].purchases[purchaseId];
        return p.state;
    }
    ...
    {
        EscrowData memory data = escrowData[productId][purchaseId];
    ...
    function createProduct(
        ...
        managerOnly
    {
        ProductData memory product = products[products.length++];
        product.owner = owner;
        product.price = price;
