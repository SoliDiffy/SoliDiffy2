...
contract RocketDAONodeTrustedActions is RocketBase, RocketDAONodeTrustedActionsInterface {
    ...
    function actionJoin() override external onlyRegisteredNode(tx.origin) onlyLatestContract("rocketDAONodeTrustedActions", address(this)) {
        _memberJoin(tx.origin);
    }
    ...
    function actionJoinRequired(address _nodeAddress) override external onlyRegisteredNode(_nodeAddress) onlyLatestContract("rocketDAONodeTrusted", tx.origin) {
