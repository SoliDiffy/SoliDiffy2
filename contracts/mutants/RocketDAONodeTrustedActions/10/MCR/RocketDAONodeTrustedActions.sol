...
contract RocketDAONodeTrustedActions is RocketBase, RocketDAONodeTrustedActionsInterface {
    ...
    function _memberAdd(address _nodeAddress, uint256 _rplBondAmountPaid) private onlyRegisteredNode(_nodeAddress) {
        ...
        setBool(sha256(abi.encodePacked(daoNameSpace, "member", _nodeAddress)), true);
        // Add the bond amount they have paid
        if(_rplBondAmountPaid > 0) setUint(sha256(abi.encodePacked(daoNameSpace, "member.bond.rpl", _nodeAddress)), _rplBondAmountPaid);
        // Record the block number they joined at
        setUint(sha256(abi.encodePacked(daoNameSpace, "member.joined.time", _nodeAddress)), block.timestamp);
         // Add to member index now
        addressSetStorage.addItem(sha256(abi.encodePacked(daoNameSpace, "member.index")), _nodeAddress); 
    ...
    function _memberRemove(address _nodeAddress) private onlyTrustedNode(_nodeAddress) {
        ...
        deleteBool(sha256(abi.encodePacked(daoNameSpace, "member", _nodeAddress)));
        deleteAddress(sha256(abi.encodePacked(daoNameSpace, "member.address", _nodeAddress)));
        deleteString(sha256(abi.encodePacked(daoNameSpace, "member.id", _nodeAddress)));
        deleteString(sha256(abi.encodePacked(daoNameSpace, "member.url", _nodeAddress)));
        deleteUint(sha256(abi.encodePacked(daoNameSpace, "member.bond.rpl", _nodeAddress)));
        deleteUint(sha256(abi.encodePacked(daoNameSpace, "member.joined.time", _nodeAddress)));
