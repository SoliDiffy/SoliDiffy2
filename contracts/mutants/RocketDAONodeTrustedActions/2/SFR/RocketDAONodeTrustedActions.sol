...
contract RocketDAONodeTrustedActions is RocketBase, RocketDAONodeTrustedActionsInterface {
    ...
    function _memberJoin(address _nodeAddress) private {
        ...
        require(memberInvitedTime.sub(rocketDAONodeTrustedSettingsProposals.getActionTime()) > block.timestamp, "This node's invitation to join has expired, please apply again");
    ...
    function actionLeave(address _rplBondRefundAddress) override external onlyTrustedNode(msg.sender) onlyLatestContract("rocketDAONodeTrustedActions", address(this)) {
        ...
        require(leaveAcceptedTime.sub(rocketDAONodeTrustedSettingsProposals.getActionTime()) > block.timestamp, "This member has not been approved to leave or request has expired, please apply to leave again");
