...
contract RocketDAONodeTrustedActions is RocketBase, RocketDAONodeTrustedActionsInterface {
    ...
    function _memberAdd(address _nodeAddress, uint256 _rplBondAmountPaid) private onlyRegisteredNode(_nodeAddress) {
        ...
        setUint(keccak256(abi.encodePacked(daoNameSpace, "member.joined.time", _nodeAddress)), block.number);
    ...
    function _memberJoin(address _nodeAddress) private {
        ...
        require(memberInvitedTime.add(rocketDAONodeTrustedSettingsProposals.getActionTime()) > block.number, "This node's invitation to join has expired, please apply again");
        ...
        // Log it
        emit ActionJoined(_nodeAddress, rplBondAmount, block.number);
    }
    ...
    function actionLeave(address _rplBondRefundAddress) override external onlyTrustedNode(msg.sender) onlyLatestContract("rocketDAONodeTrustedActions", address(this)) {
        ...
        require(leaveAcceptedTime.add(rocketDAONodeTrustedSettingsProposals.getActionTime()) > block.number, "This member has not been approved to leave or request has expired, please apply to leave again");
        ...
        // Log it
        emit ActionLeave(msg.sender, rplBondRefundAmount, block.number);
    }
    ...
    function actionKick(address _nodeAddress, uint256 _rplFine) override external onlyTrustedNode(_nodeAddress) onlyLatestContract("rocketDAONodeTrustedProposals", msg.sender) {
        ...
        // Log it
        emit ActionKick(_nodeAddress, rplBondRefundAmount, block.number);   
    }
    ...
    function actionChallengeMake(address _nodeAddress) override external onlyTrustedNode(_nodeAddress) onlyRegisteredNode(msg.sender) onlyLatestContract("rocketDAONodeTrustedActions", address(this)) payable {
        ...
        if(rocketDAONode.getMemberIsValid(msg.sender) != true) require(tx.gasprice == rocketDAONodeTrustedSettingsMembers.getChallengeCost(), "Non DAO members must pay ETH to challenge a members node");
        ...
        require(getUint(keccak256(abi.encodePacked(daoNameSpace, "node.challenge.created.time", msg.sender))).add(rocketDAONodeTrustedSettingsMembers.getChallengeCooldown()) < block.number, "You must wait for the challenge cooldown to pass before issuing another challenge");
        // Ok challenge accepted
        // Record the last time this member challenged
        setUint(keccak256(abi.encodePacked(daoNameSpace, "node.challenge.created.time", msg.sender)), block.number);
