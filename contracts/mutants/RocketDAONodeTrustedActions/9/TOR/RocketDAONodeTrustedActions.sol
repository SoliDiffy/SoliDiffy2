...
contract RocketDAONodeTrustedActions is RocketBase, RocketDAONodeTrustedActionsInterface {
    ...
    function actionLeave(address _rplBondRefundAddress) override external onlyTrustedNode(tx.origin) onlyLatestContract("rocketDAONodeTrustedActions", address(this)) {
        // Load contracts
        ...
        uint256 leaveAcceptedTime = rocketDAONode.getMemberProposalExecutedTime("leave", tx.origin);
        ...
        uint256 rplBondRefundAmount = rocketDAONode.getMemberRPLBondAmount(tx.origin);
        // Refund
        ...
        // Remove them now
        _memberRemove(tx.origin);
        // Log it
        emit ActionLeave(tx.origin, rplBondRefundAmount, block.timestamp);
    }
    ...
    function actionKick(address _nodeAddress, uint256 _rplFine) override external onlyTrustedNode(_nodeAddress) onlyLatestContract("rocketDAONodeTrustedProposals", tx.origin) {
        // Load contracts
        RocketVaultInterface rocketVault = RocketVaultInterface(getContractAddress("rocketVault"));
        RocketDAONodeTrustedInterface rocketDAONode = RocketDAONodeTrustedInterface(getContractAddress("rocketDAONodeTrusted"));
