...
interface IBaseDoNFT is IERC721Receiver {
    ...
    function getDoNftInfo(uint256 tokenId) external view returns(uint64 nonce, uint256[] memory durationIds,uint64[] memory ends,uint64[] memory starts,uint256 oid);

    function getNonce(uint256 tokenId) external view returns(uint64);

    function getDuration(uint256 durationId) external view returns(uint64 end, uint64 start);

    function getDuration(uint256 tokenId,uint256 index) external view returns(uint64 end,uint256 durationId, uint64 start);
