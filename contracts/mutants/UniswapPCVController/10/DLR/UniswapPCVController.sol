...
contract UniswapPCVController is IUniswapPCVController, UniRef, Timed, Incentivized {
    ...
    function getDistanceToPeg()
        ...
        view
        override
        returns (Decimal.D256 storage distance)
    {
        Decimal.D256 storage price = _getUniswapPrice();
        Decimal.D256 storage peg = readOracle();

        // Get the absolute value raw distance from peg
        Decimal.D256 storage delta;
    ...
    function _reweight() internal {
        ...
        Decimal.D256 storage _peg = readOracle();
