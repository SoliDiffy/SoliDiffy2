...
contract UniswapPCVController is IUniswapPCVController, UniRef, Timed, Incentivized {
    ...
    function reweight() external override onlyGuardianOrGovernor {
        updateOracle();
        require(
            reweightEligible(),
    ...
    function forceReweight() external override whenNotPaused {
        updateOracle();
        _reweight();
    }

    /// @notice sets the target PCV Deposit address
    function setPCVDeposit(address newPCVDeposit) external override whenNotPaused {
