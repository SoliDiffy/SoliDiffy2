...
library PaymentChallengeStandardExit {
    ...
    function run(
        ...
        PaymentExitDataModel.StandardExitMap memory exitMap,
        PaymentStandardExitRouterArgs.ChallengeStandardExitArgs storage args
    )
        public
    {
        ChallengeStandardExitData storage data = ChallengeStandardExitData({
            controller: self,
            args: args,
    ...
    function verifyChallengeExitExists(ChallengeStandardExitData storage data) private pure {
        require(data.exitData.exitable == true, "The exit does not exist");
    }

    function verifyChallengeTxProtocolFinalized(ChallengeStandardExitData storage data) private view {
        UtxoPosLib.UtxoPos storage utxoPos = UtxoPosLib.UtxoPos(data.exitData.utxoPos);
