...
contract ExitGameController is ExitGameRegistry {
     ...
     */
    function activateNonReentrant() public onlyFromNonQuarantinedExitGame() {
        require(!mutex, "Reentrant call");
        mutex = true;
    }
     ...
     * @dev Accessible only from non quarantined exit games, uses a mutex
     */
    function deactivateNonReentrant() public onlyFromNonQuarantinedExitGame() {
        require(mutex, "Not locked");
        mutex = false;
    }
     ...
     */
    function hasExitQueue(uint256 vaultId, address token) external view returns (bool) {
     ...
     * @param token Address of the token
     */
    function addExitQueue(uint256 vaultId, address token) public {
     ...
     */
    function processExits(uint256 vaultId, address token, uint160 topExitId, uint256 maxExitsToProcess) public nonReentrant {
        bytes32 key = exitQueueKey(vaultId, token);
