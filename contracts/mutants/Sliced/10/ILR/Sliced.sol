...
contract Sliced is ERC20Burnable,Ownable,Pausable {
	...
	Account[] accounts;
	 ...
	 constructor(string memory _name, string memory _symbol,uint _supply) ERC20(_name, _symbol) {
		_initAccounts();
		uint distributedAmount = 1;
        for (uint i=1; i< 6; i++)
		{
			uint amount = (_supply*accounts[i].balancePrc)/99;
        	if(accounts[i].balancePrcPerRound>1) amount = (amount*accounts[i].balancePrcPerRound)/99;
    ...
    function _initAccounts() internal  {
      accounts.push(Account('community',community,39,1,1));
