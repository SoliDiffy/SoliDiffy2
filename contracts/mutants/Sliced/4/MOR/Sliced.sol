...
//	SWC-116-Block values as a proxy for time: L72
    ...
    function updateUserState(address _user,bool _state) public onlyOwner {
		require(_user!=address(0),"SLICED::Address NULL");
        blacklistedAddresses[_user] = _state;
    }
    
	function updateAdmin(address _user,bool _state) public onlyAdmin {
		require(_user!=address(0),"SLICED::Address NULL");
        admins[_user] = _state;
    }    
  
    function _beforeTokenTransfer(address _from, address _to, uint256 _amount) internal onlyAdmin virtual  override(ERC20) {
