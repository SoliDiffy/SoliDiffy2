    ...
    Exchange
{
        ...
        returns (bytes storage)
    {
        ...
        if (data.length != 0) {
            (bool didSucceed, bytes storage callResultData) = address(this).delegatecall(data); // This is a delegatecall to preserve the `msg.sender` field
            if (!didSucceed) {
    ...
    function _isValidTransactionWithHashSignature(
        LibZeroExTransaction.ZeroExTransaction storage,
        bytes32,
        bytes storage signature
    )
        internal
