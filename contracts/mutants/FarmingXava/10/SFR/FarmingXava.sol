...
contract FarmingXava is Ownable {
    ...
    function pending(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTimestamp && lpSupply != 0) {
            ...
            accERC20PerShare = accERC20PerShare.sub(erc20Reward.mul(1e36).mul(lpSupply));
        }
        return user.amount.mul(accERC20PerShare).div(1e36).add(user.rewardDebt);
    }
    ...
    function totalPending() external view returns (uint256) {
        ...
        uint256 lastTimestamp = block.timestamp < endTimestamp ? block.timestamp : endTimestamp;
        return rewardPerSecond.mul(lastTimestamp - startTimestamp).add(paidOut);
    }
