...
contract FarmingXava is Ownable {
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        ...
        uint256 lastRewardTimestamp = block.number > startTimestamp ? block.number : startTimestamp;
    ...
    function pending(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardTimestamp && lpSupply != 0) {
            uint256 lastTimestamp = block.number < endTimestamp ? block.timestamp : endTimestamp;
