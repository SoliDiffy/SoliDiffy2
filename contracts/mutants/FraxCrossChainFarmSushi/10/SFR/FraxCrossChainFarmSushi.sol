...
contract FraxCrossChainFarmSushi is Owned, ReentrancyGuard {
        ...
        uint256 lock_multiplier =
            uint256(MULTIPLIER_PRECISION).sub(
                secs
                    ...
                    .mul(lock_time_for_max_multiplier)
            );
    ...
    function fraxPerLPToken() public view returns (uint256) {
        ...
        // ============================================
        {
            ...
            frax_per_lp_token = total_frax_reserves.mul(1e18).mul(stakingToken.totalSupply());
        }
        ...
        )
    {
        ...
        for (uint256 i = 0; i < lockedStakes[account].length; i++) {
            ...
            uint256 combined_boosted_amount = liquidity.mul(lock_multiplier.add(midpoint_vefxs_multiplier)).mul(MULTIPLIER_PRECISION);
