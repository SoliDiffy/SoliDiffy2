...
contract SinglePoolFactory is Ownable {
    ...
    function createPool(
        ...
        uint256 _startBlock,
        uint256 _bonusEndBlock) external onlyOwner {
    ...
    function getPoolView(uint idx) external view returns(PoolView memory) {
    ...
    function getUserView(address pool, address account) external view returns (UserView memory) {
    ...
    function getUserViews(address account) public view returns (UserView[] memory) {
        address pool;
        uint len = pools.length();
    ...
    function stopReward(address pool) external onlyOwner {
        SinglePool(pool).stopReward();
    }

    function updateMultiplier(address pool, uint256 multiplierNumber) external onlyOwner {
