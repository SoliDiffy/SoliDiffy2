...
contract SinglePoolFactory is Ownable {
    ...
    function getAllPoolViews() public view returns(PoolView[] storage){
        uint len = pools.length();
        PoolView[] storage views = new PoolView[](len);
    ...
    function getPoolView(uint idx) public view returns(PoolView storage) {
        ...
        string storage depositSymbol = IERC20Metadata(address(depositToken)).symbol();
        string storage depositName = IERC20Metadata(address(depositToken)).name();
        uint8 depositDecimals = IERC20Metadata(address(depositToken)).decimals();

        string storage rewardSymbol = IERC20Metadata(address(rewardToken)).symbol();
        string storage rewardName = IERC20Metadata(address(rewardToken)).name();
    ...
    function getUserView(address pool, address account) public view returns (UserView storage) {
    ...
    function getUserViews(address account) external view returns (UserView[] storage) {
        address pool;
        uint len = pools.length();

        UserView[] storage views = new UserView[](len);
        for (uint256 i = 0; i < len; i++) {
            pool = pools.at(i);
