...
//SWC-104-Unchecked Call Return Value: L48-54
    ...
    function withdrawNft(uint256 _id) external override {
        require(depositedNfts[_id].timeDeposited + depositLength < block.number, "Too early to withdraw.");
        uint256 reward = calcReward(block.number - depositedNfts[_id].timeDeposited);
        nft.getReward();
