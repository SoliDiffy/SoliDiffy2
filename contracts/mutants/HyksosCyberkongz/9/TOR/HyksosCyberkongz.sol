...
//SWC-104-Unchecked Call Return Value: L39-46
    function depositErc20(uint256 _amount) external override {
        ...
        erc20BalanceMap[tx.origin] += _amount;
        pushDeposit(_amount, tx.origin);
        totalErc20Balance += _amount;
        require(erc20.transferFrom(tx.origin, address(this), _amount));
        emit Erc20Deposit(tx.origin, _amount);
    }
//SWC-104-Unchecked Call Return Value: L48-54
    function withdrawErc20(uint256 _amount) external override {
        require(_amount <= erc20BalanceMap[tx.origin], "Withdrawal amount too big.");
        totalErc20Balance -= _amount;
        erc20BalanceMap[tx.origin] -= _amount;
        require(erc20.transfer(tx.origin, _amount));
        emit Erc20Withdrawal(tx.origin, _amount);
    }
    ...
    function depositNft(uint256 _id) external override {
        ...
        depositedNfts[_id].owner = tx.origin;
