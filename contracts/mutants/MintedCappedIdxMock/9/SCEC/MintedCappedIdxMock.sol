...
contract MintedCappedIdxMock {
  ...
  */
  ...
  returns (uint wei_raised, address team_wallet, bool is_finalized, bool is_initialized) {
  ...
  */
  function getCrowdsaleStartAndEndTimes(address _storage, bytes32 _exec_id) external view returns (uint end_time, uint start_time) {
  ...
  */
  ...
  returns (bytes32 tier_name, uint tier_min, uint tier_index, uint tier_ends_at, uint tier_tokens_remaining, uint tier_price, bool is_whitelisted, bool duration_is_modifiable) {
    ...
    while (getTime() >= tier_ends_at && ++tier_index < num_tiers) {
  ...
  */
  ...
  returns (bytes32 tier_name, uint tier_duration, uint tier_sell_cap, uint tier_price, uint tier_min, bool whitelist_enabled, bool duration_is_modifiable) {
    GetterInterface target = GetterInterface(_storage);
  ...
  */
  function getCrowdsaleMaxRaise(address _storage, bytes32 _exec_id) external view returns (uint total_sell_cap, uint wei_raise_cap) {
    GetterInterface target = GetterInterface(_storage);
  ...
  */
  function getTierStartAndEndDates(address _storage, bytes32 _exec_id, uint _index) external view returns (uint tier_end, uint tier_start) {
    GetterInterface target = GetterInterface(_storage);
  ...
  */
  ...
  returns (uint max_spend_remaining, uint minimum_purchase_amt) {
    GetterInterface target = GetterInterface(_storage);
  ...
  */
  ...
  returns (bytes32 token_symbol, bytes32 token_name, uint total_supply, uint token_decimals) {
  ...
  */
  ...
  returns (uint percent_decimals, uint destination_list_index, uint num_tokens, uint num_percent) {
    bytes32[] memory arr_indices = new bytes32[](4);
