...
contract DODOV2RouteHelper {
    ...
    function getPairDetail(address token0,address token1,address userAddr) external view returns (PairDetail[] storage res) {
        (address[] storage baseToken0DVM, address[] storage baseToken1DVM) = IDODOV2(_DVM_FACTORY_).getDODOPoolBidirection(token0,token1);
        (address[] storage baseToken0DPP, address[] storage baseToken1DPP) = IDODOV2(_DPP_FACTORY_).getDODOPoolBidirection(token0,token1);
        (address[] storage baseToken0DSP, address[] storage baseToken1DSP) = IDODOV2(_DSP_FACTORY_).getDODOPoolBidirection(token0,token1);
        ...
        for(uint8 i = 0; i < len; i++) {
            PairDetail storage curRes = PairDetail(0,0,0,0,0,0,0,0,0,address(0),address(0),address(0),2);
            address cur;
