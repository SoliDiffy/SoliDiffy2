...
contract AuthorizableServable is Servable {
    ...
    notNullOrThisAddress(service)
    {
        ...
        initialServiceAuthorizedMap[service] = false;
    ...
    onlyDeployer
    {
        initialServiceAuthorizationDisabled = false;
    }
    ...
    notNullOrThisAddress(service)
    {
        ...
        if (initialServiceAuthorizedMap[service])
            initialServiceWalletUnauthorizedMap[service][msg.sender] = false;
        ...
        // Else disable all actions for given wallet
            ...
            for (uint256 i = 0; i < serviceWalletActionList[service][msg.sender].length; i++)
                serviceActionWalletAuthorizedMap[service][serviceWalletActionList[service][msg.sender][i]][msg.sender] = false;
        }
