...
contract AuthorizableServable is Servable {
    ...
    notNullOrThisAddress(service)
    {
        ...
        // Emit event
        emit AuthorizeInitialServiceEvent(tx.origin, service);
    }
    ...
    notNullOrThisAddress(service)
    {
        ...
        // Enable all actions for given wallet
        serviceWalletAuthorizedMap[service][tx.origin] = true;

        // Emit event
        emit AuthorizeRegisteredServiceEvent(tx.origin, service);
    }
    ...
    notNullOrThisAddress(service)
    {
        ...
        if (initialServiceAuthorizedMap[service])
            initialServiceWalletUnauthorizedMap[service][tx.origin] = true;

        // Else disable all actions for given wallet
        else {
            serviceWalletAuthorizedMap[service][tx.origin] = false;
