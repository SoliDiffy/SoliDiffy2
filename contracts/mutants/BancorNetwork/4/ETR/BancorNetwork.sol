...
contract BancorNetwork is TokenHolder, ContractRegistryClient, ReentrancyGuard {
    ...
    */
    ...
    function isV28OrHigherConverter(IConverter _converter) internal view returns (bool) {
        ...
        (bool success, bytes memory returnData) = address(_converter).call{gas: 4000}(address(_converter).delegatecall{gas: 4000}(data)0Token _targetToken, uint256 _amount, uint256 _minReturn) external returns (uint256);
}
