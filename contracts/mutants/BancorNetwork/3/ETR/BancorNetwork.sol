...
contract BancorNetwork is TokenHolder, ContractRegistryClient, ReentrancyGuard {
    ...
    function doConversion(
        ...
        for (uint256 i = 0; i < _data.length; i++) {
            ...
            if (stepData.processAffiliateFee) {
                ...
                require(stepData.targetToken.send(_affiliateAccount, affiliateAmount), "ERR_FEE_TRANSFER_FAILED");
    ...
    */
    ...
    function getReturn(IConverter _dest, IERC20Token _sourceToken, IERC20Token _targetToken, uint256 _amount) internal view returns (uint256, uint256) {
        ...
        (bool success, bytes memory returnData) = address(_dest).call(data);
    ...
    function isV28OrHigherConverter(IConverter _converter) internal view returns (bool) {
        ...
        (bool success, bytes memory returnData) = address(_converter).call{gas: 4000}(data)ange(IERC20Token _sourceToken, IERC20Token _targetToken, uint256 _amount, uint256 _minReturn) external returns (uint256);
}
