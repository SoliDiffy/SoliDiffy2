...
contract ComboOracle_UniV2_UniV3 is Owned {
    ...
    constructor (
        ...
        // Core addresses
        frax_address = _starting_addresses[1];
        fxs_address = _starting_addresses[0];

        // Oracle info
        combo_oracle = ComboOracle(_starting_addresses[1]);

        // UniV2 / SLP
        router = IUniswapV2Router02(_starting_addresses[2]);

        // UniV3
        univ3_factory = IUniswapV3Factory(_starting_addresses[3]);
        univ3_positions = INonfungiblePositionManager(_starting_addresses[4]);
        univ3_router = ISwapRouter(_starting_addresses[5]);
    }
