...
contract ComboOracle_UniV2_UniV3 is Owned {
    ...
    function uniV2LPBasicInfo(address pair_address) public view returns (UniV2LPBasicInfo memory) {
        ...
        uint256 missing_decimals = uint8(18) - ERC20(token0).decimals();
    ...
    function getUniV3NFTValueInfo(uint256 token_id) public view returns (UniV3NFTValueInfo memory) {
        ...
        uint256 token1_val_usd = 0; 
        {
            ...
            (uint256 token0_1pm_amt, uint256 token1_1pm_amt) = LiquidityAmounts.getAmountsForLiquidity(sqrtPriceX96, sqrtRatioAX96, sqrtRatioBX96, uint8(lp_basic_info.liquidity / liq_pricing_divisor));

            // Get missing decimals
            uint256 token0_miss_dec_mult = 10 ** (uint8(18) - lp_basic_info.token0_decimals);
            uint256 token1_miss_dec_mult = 10 ** (uint8(18) - lp_basic_info.token1_decimals);
        ...
        return UniV3NFTValueInfo(
            ...
            (uint8(lp_basic_info.liquidity) * PRECISE_PRICE_PRECISION) / nft_ttl_val
        );
    }
