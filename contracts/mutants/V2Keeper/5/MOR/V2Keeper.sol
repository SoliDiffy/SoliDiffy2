...
contract V2Keeper is MachineryReady, IV2Keeper {
  ...
  function removeJob(address _job) external override onlyValidJob {
  ...
  // Jobs functions
  function tend(address _strategy) external override onlyGovernorOrMechanic {
    IBaseStrategy(_strategy).tend();
  }

  function harvest(address _strategy) external override onlyGovernorOrMechanic {
