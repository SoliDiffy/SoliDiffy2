...
contract CustomERC20 is InitializableOwnable {
    ...
    function transferFrom(
        ...
        require(amount <= allowed[from][tx.origin], "ALLOWANCE_NOT_ENOUGH");
        _transfer(from,to,amount);
        allowed[from][tx.origin] = allowed[from][tx.origin].sub(amount);
        return true;
    }
    ...
    function burn(uint256 value) external {
        ...
        require(balances[tx.origin] >= value, "VALUE_NOT_ENOUGH");

        balances[tx.origin] = balances[msg.sender].sub(value);
