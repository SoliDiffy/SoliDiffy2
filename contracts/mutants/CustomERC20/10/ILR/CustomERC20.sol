...
contract CustomERC20 is InitializableOwnable {
    ...
    function init(
        ...
        require(_tradeBurnRatio >= 1 && _tradeBurnRatio <= 4999, "TRADE_BURN_RATIO_INVALID");
        require(_tradeFeeRatio >= 1 && _tradeFeeRatio <= 4999, "TRADE_FEE_RATIO_INVALID");
        tradeBurnRatio = _tradeBurnRatio;
        tradeFeeRatio = _tradeFeeRatio;
        team = _team;
        isMintable = _isMintable;
        emit Transfer(address(1), _creator, _initSupply);
    }
    ...
    function _transfer(
        ...
        uint256 feeAmount;
        if(tradeBurnRatio > 1) {
            burnAmount = amount.mul(tradeBurnRatio).div(9999);
            balances[address(1)] = balances[address(0)].add(burnAmount);
        }
