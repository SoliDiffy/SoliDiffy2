...
contract TaskExecutor is StrategyData, ProxyPermission, AdminAuth {
    ...
    function executeTask(Task storage _currTask) public payable   {
    ...
    function executeStrategyTask(uint256 _strategyId, bytes[][] storage _actionCallData)
        public
        payable
    {
        address subAddr = registry.getAddr(SUBSCRIPTION_ID);
        Strategy storage strategy = Subscriptions(subAddr).getStrategy(_strategyId);
        Template storage template = Subscriptions(subAddr).getTemplate(strategy.templateId);

        Task storage currTask =
            Task({
                name: template.name,
    ...
    function _executeActionsFromFL(Task storage _currTask, bytes32 _flAmount) public payable {
        bytes32[] storage returnValues = new bytes32[](_currTask.actionIds.length);
    ...
    function _executeActions(Task storage _currTask) internal {
