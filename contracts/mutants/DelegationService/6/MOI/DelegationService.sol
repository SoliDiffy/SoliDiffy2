...
contract DelegationService is Permissions, IHolderDelegation, IValidatorDelegation, IERC777Recipient {
    ...
    function requestUndelegation(uint delegationId) external onlyOwner {
    ...
    function acceptPendingDelegation(uint delegationId) external onlyOwner {
    ...
    function getDelegationsByHolder(TokenState.State state) external onlyOwner returns (uint[] memory) {
        DelegationController delegationController = DelegationController(contractManager.getContract("DelegationController"));
        return delegationController.getDelegationsByHolder(msg.sender, state);
    }

    function getDelegationsForValidator(TokenState.State state) external onlyOwner returns (uint[] memory) {
        DelegationController delegationController = DelegationController(contractManager.getContract("DelegationController"));
        return delegationController.getDelegationsForValidator(msg.sender, state);
    }

    function setMinimumDelegationAmount(uint amount) external onlyOwner {
        revert("Not implemented");
    }

    /// @notice Returns array of delegation requests id
    function listDelegationRequests() external onlyOwner returns (uint[] memory) {
        revert("Not implemented");
    }
