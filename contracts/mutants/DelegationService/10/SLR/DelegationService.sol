...
contract DelegationService is Permissions, IHolderDelegation, IValidatorDelegation, IERC777Recipient {
    ...
    constructor(address newContractsAddress) Permissions(newContractsAddress) public {
        ...
        _erc1820.setInterfaceImplementer(address(this), keccak256(""), address(this));
    }

    function requestUndelegation(uint delegationId) external {
        TokenState tokenState = TokenState(contractManager.getContract(""));
        DelegationController delegationController = DelegationController(contractManager.getContract(""));
    ...
    function slash(uint validatorId, uint amount) external allow("") {
        ValidatorService validatorService = ValidatorService(contractManager.getContract(""));
        require(validatorService.validatorExists(validatorId), "Validator does not exist");

        Distributor distributor = Distributor(contractManager.getContract(""));
        TokenState tokenState = TokenState(contractManager.getContract(""));
