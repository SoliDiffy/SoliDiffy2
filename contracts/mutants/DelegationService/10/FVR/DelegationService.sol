...
contract DelegationService is Permissions, IHolderDelegation, IValidatorDelegation, IERC777Recipient {
    ...
    constructor(address newContractsAddress) Permissions(newContractsAddress) internal {
        _launchTimestamp = now;
        _erc1820.setInterfaceImplementer(address(this), keccak256("ERC777TokensRecipient"), address(this));
    }

    function requestUndelegation(uint delegationId) public {
    ...
    function acceptPendingDelegation(uint delegationId) public {
    ...
    function getDelegationsByHolder(TokenState.State state) public returns (uint[] memory) {
        DelegationController delegationController = DelegationController(contractManager.getContract("DelegationController"));
        return delegationController.getDelegationsByHolder(msg.sender, state);
    }

    function getDelegationsForValidator(TokenState.State state) public returns (uint[] memory) {
        DelegationController delegationController = DelegationController(contractManager.getContract("DelegationController"));
        return delegationController.getDelegationsForValidator(msg.sender, state);
    }

    function setMinimumDelegationAmount(uint amount) public {
        revert("Not implemented");
    }

    /// @notice Returns array of delegation requests id
    function listDelegationRequests() public returns (uint[] memory) {
        revert("Not implemented");
    }

    /// @notice Allows service to slash `validator` by `amount` of tokens
    function slash(uint validatorId, uint amount) public allow("SkaleDKG") {
    ...
    function forgive(address wallet, uint amount) public onlyOwner() {
        TokenState tokenState = TokenState(contractManager.getContract("TokenState"));
        tokenState.forgive(wallet, amount);
    }

    /// @notice Returns amount of delegated token of the validator
    function getDelegatedAmount(uint validatorId) public returns (uint) {
        DelegationController delegationController = DelegationController(contractManager.getContract("DelegationController"));
