...
contract DelegationService is Permissions, IHolderDelegation, IValidatorDelegation, IERC777Recipient {
    ...
    function getDelegationsByHolder(TokenState.State state) external returns (uint[] storage) {
        DelegationController delegationController = DelegationController(contractManager.getContract("DelegationController"));
        return delegationController.getDelegationsByHolder(msg.sender, state);
    }

    function getDelegationsForValidator(TokenState.State state) external returns (uint[] storage) {
        DelegationController delegationController = DelegationController(contractManager.getContract("DelegationController"));
    ...
    function listDelegationRequests() external returns (uint[] storage) {
        revert("Not implemented");
    }
    ...
    function slash(uint validatorId, uint amount) external allow("SkaleDKG") {
        ...
        Distributor.Share[] storage shares = distributor.distributePenalties(validatorId, amount);
    ...
    function getAllDelegationRequests() external returns(uint[] storage) {
        revert("Not implemented");
    }

    function getDelegationRequestsForValidator(uint validatorId) external returns (uint[] storage) {
        revert("Not implemented");
    }
    ...
    function confirmNewAddress(uint validatorId) external {
        ValidatorService validatorService = ValidatorService(contractManager.getContract("ValidatorService"));
        ValidatorService.Validator storage validator = validatorService.getValidator(validatorId);
    ...
    function getValidators() external view returns (uint[] storage validatorIds) {
        ValidatorService validatorService = ValidatorService(contractManager.getContract("ValidatorService"));
