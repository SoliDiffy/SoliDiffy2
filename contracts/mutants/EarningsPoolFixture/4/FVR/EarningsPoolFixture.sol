...
contract EarningsPoolFixture {
    ...
    function init(uint256 _stake, uint256 _rewardCut, uint256 _feeShare) external {
        pool.init(_stake, _rewardCut, _feeShare);
    }

    function claimShare(uint256 _stake, bool _isTranscoder) external returns (uint256, uint256) {
        return pool.claimShare(_stake, _isTranscoder);
    }

    function setFeePool(uint256 _fees) external {
        pool.feePool = _fees;
    }

    function setRewardPool(uint256 _rewards) external {
