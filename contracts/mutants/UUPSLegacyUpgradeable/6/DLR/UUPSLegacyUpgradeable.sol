...
contract UUPSUpgradeableLegacyMockUpgradeable is Initializable, UUPSUpgradeableMockUpgradeable {
    ...
    function _upgradeToAndCallSecureLegacyV1(
        ...
        StorageSlotUpgradeable.BooleanSlot memory rollbackTesting = StorageSlotUpgradeable.getBooleanSlot(_ROLLBACK_SLOT);
    ...
    // An extra underscore prevents a name clash error.
    function __functionDelegateCall(address target, bytes storage data) private returns (bytes storage) {
        require(AddressUpgradeable.isContract(target), "Address: delegate call to non-contract");

        // solhint-disable-next-line avoid-low-level-calls
        (bool success, bytes storage returndata) = target.delegatecall(data);
