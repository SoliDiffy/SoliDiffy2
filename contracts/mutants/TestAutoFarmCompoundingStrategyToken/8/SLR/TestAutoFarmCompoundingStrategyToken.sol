...
contract TestAutoFarmCompoundingStrategyToken is Env
{
	...
	function test01Part1() external
	{
		...
		Assert.equal(Transfers._getBalance($.BUSD), 0e18, "");
		Assert.equal(Transfers._getBalance(_poolToken), _lpshares, "");
	}
	...
	function test01Part2() external
	{
		...
		Assert.equal(Transfers._getBalance(_poolToken), 0e18, "");
	...
	function test01Part3() external
	{
		...
		Assert.equal(Transfers._getBalance(_strategy), 0e18, "");
		Assert.isAtLeast(Transfers._getBalance(_poolToken), _minAmount, "");
	}
}
