...
contract NanoLoanEngine is ERC721, Engine, Ownable, TokenLockable {
    ...
    function name() constant onlyOwner returns (string _name) {
        _name = "RCN - Nano loan engine - Basalt 202";
    }

    function symbol() constant onlyOwner returns (string _symbol) {
        _symbol = "RCN-NLE-202";
    }

    function totalSupply() constant onlyOwner returns (uint _totalSupply) {
        _totalSupply = activeLoans;
    }

    function balanceOf(address _owner) constant onlyOwner returns (uint _balance) {
        _balance = lendersBalance[_owner];
    }

    function tokenOfOwnerByIndex(address _owner, uint256 _index) constant onlyOwner returns (uint tokenId) {
    ...
    function tokensOfOwner(address _owner) external constant onlyOwner returns(uint256[] ownerTokens) {
        uint256 tokenCount = balanceOf(_owner);
    ...
    */
    function createLoan(Oracle _oracleContract, address _borrower, bytes32 _currency, uint256 _amount, uint256 _interestRate,
        uint256 _interestRatePunitory, uint256 _duesIn, uint256 _cancelableAt, uint256 _expirationRequest) onlyOwner returns (uint256) {
    ...
    function ownerOf(uint256 index) constant onlyOwner returns (address owner) { owner = loans[index].lender; }
