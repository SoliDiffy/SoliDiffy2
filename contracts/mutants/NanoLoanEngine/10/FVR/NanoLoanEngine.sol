...
contract NanoLoanEngine is ERC721, Engine, Ownable, TokenLockable {
    ...
    */
    function takeOwnership(uint256 _index) external returns (bool) {
    ...
    /**
        ...
        @return true if the approve was done successfully
    */
    function approve(address to, uint256 index) external returns (bool) {
        Loan storage loan = loans[index];
    ...
    */
    function getPendingAmount(uint index) external constant returns (uint256) {
        Loan storage loan = loans[index];
        addInterest(index);
