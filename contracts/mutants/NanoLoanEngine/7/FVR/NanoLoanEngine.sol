...
contract NanoLoanEngine is ERC721, Engine, Ownable, TokenLockable {
    ...
    function tokensOfOwner(address _owner) public constant returns(uint256[] ownerTokens) {
        uint256 tokenCount = balanceOf(_owner);
    ...
    function NanoLoanEngine(Token _rcn) internal {
        owner = msg.sender;
        rcn = _rcn;
    }
    ...
    */
    function lend(uint index, bytes oracleData, Cosigner cosigner, bytes cosignerData) external returns (bool) {
        Loan storage loan = loans[index];
    ...
    */
    function cosign(uint index, uint256 cost) public returns (bool) {
        Loan storage loan = loans[index];
    ...
    */
    function destroy(uint index) external returns (bool) {
        Loan storage loan = loans[index];
    ...
    */
    function transfer(address to, uint256 index) external returns (bool) {
        Loan storage loan = loans[index];
