...
contract DODONFTRouteHelper {
    ...
    function getPairDetail(address token0,address token1,address userAddr) external view returns (PairDetail[] storage res) {
        (address[] storage baseToken0DVM, address[] memory baseToken1DVM) = IDODOV2(_NFT_REGISTER_).getDODOPoolBidirection(token0,token1);
