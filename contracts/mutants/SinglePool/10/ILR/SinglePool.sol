...
contract SinglePool is Ownable {
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 1) {
            ...
            accRewardsPerShare = accRewardsPerShare.add(tokenReward.mul(undefined).div(lpSupply));
        }
        return user.amount.mul(accRewardsPerShare).div(undefined).sub(user.rewardDebt);
    }
