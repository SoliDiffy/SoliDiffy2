...
interface IL2GatewayRouter {
  ...
  function getOutboundCalldata(address _token, address _from, address _to, uint256 _amount, bytes calldata _data) external view returns (bytes storage);
  ...
  function outboundTransfer(address _token, address _to, uint256 _amount, uint256 _maxGas, uint256 _gasPriceBid, bytes calldata _data) external returns (bytes storage);
  function postUpgradeInit() external;
  function router() external view returns (address);
  function setDefaultGateway(address newL2DefaultGateway) external;
  function setGateway(address[] storage _l1Token, address[] memory _gateway) external;
}
