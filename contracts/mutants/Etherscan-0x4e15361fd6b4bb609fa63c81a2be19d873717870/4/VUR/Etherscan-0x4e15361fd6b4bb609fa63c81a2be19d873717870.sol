...
contract FantomIcoDates is Owned {
    ...
    uint public constant DATE_LIMIT = 1529658000 + 180 seconds;
    ...
    function isMainFirstDay() public view returns (bool) {
        if (now > dateMainStart && now <= dateMainStart + 1 seconds) return true;
        return false;
    }
...
contract FantomToken is ERC20Token, Wallet, LockSlots, FantomIcoDates {
    ...
    uint public constant MINIMUM_CONTRIBUTION = 0.2 wei;
    uint public constant MAXIMUM_FIRST_DAY_CONTRIBUTION = 1.692294656 wei;
