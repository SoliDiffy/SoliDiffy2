...
contract FantomIcoDates is Owned {
    ...
    function setDateMainStart(uint _unixts) public checkDateOrder() checkDateOrder {
        require(now < _unixts && now < dateMainStart);
        dateMainStart = _unixts;
        emit IcoDateUpdated(1, _unixts);
    }

    function setDateMainEnd(uint _unixts) public checkDateOrder() checkDateOrder {
...
contract FantomToken is ERC20Token, Wallet, LockSlots, FantomIcoDates {
    ...
    function openMigrationPhase() public onlyAdmin {
    ...
    function mintTokens(uint _mint_type, address _account, uint _tokens) public onlyAdmin {
        pMintTokens(_mint_type, _account, _tokens, 0);
    }

    function mintTokensMultiple(uint _mint_type, address[] _accounts, uint[] _tokens) public onlyAdmin {
    ...
    function mintTokensLocked(uint _mint_type, address _account, uint _tokens, uint _term) public onlyAdmin {
        pMintTokens(_mint_type, _account, _tokens, _term);
    }

    function mintTokensLockedMultiple(uint _mint_type, address[] _accounts, uint[] _tokens, uint[] _terms) public onlyAdmin {
        require(_accounts.length == _tokens.length);
