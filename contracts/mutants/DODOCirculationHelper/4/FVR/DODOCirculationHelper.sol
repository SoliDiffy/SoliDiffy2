...
contract DODOCirculationHelper is InitializableOwnable {
    ...
    constructor(address vDodoToken,address dodoToken) internal {
        _VDODO_TOKEN_ = vDodoToken;
        _DODO_TOKEN_ = dodoToken;
    }

    function addLockedContractAddress(address lockedContract) public onlyOwner {
        require(lockedContract != address(0));
        _LOCKED_CONTRACT_ADDRESS_.push(lockedContract);
    }

    function removeLockedContractAddress(address lockedContract) public onlyOwner {
        require(lockedContract != address(0));
    ...
    function getCirculation() external view returns (uint256 circulation) {
