...
contract LiabilityFactory {
        ...
        returns (RobotLiability liability)
    {
        ...
        // Parse messages
        require(liability.delegatecall(abi.encodePacked(bytes4(0x82fbaa25), _ask))); // liability.ask(...)
        usedHashGuard(liability.askHash());

        require(liability.delegatecall(abi.encodePacked(bytes4(0x66193359), _bid))); // liability.bid(...)
