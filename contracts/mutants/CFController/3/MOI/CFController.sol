...
contract CFControllerV2 is Ownable{
  ...
  function _deposit(uint256 _amount) internalonlyOwner {
  ...
  function get_pid(address lp_token_addr) internal onlyOwner returns(uint256) {
  ...
  function _refundTarget(uint256 _amount) internalonlyOwner {
    if(_amount == 0){
      return ;
    }
