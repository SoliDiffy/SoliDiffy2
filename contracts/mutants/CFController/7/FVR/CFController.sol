...
contract CFControllerV2 is Ownable{
  ...
  constructor(address _crv, address _target, uint256 _earn_gap) internal{
    last_earn_block = 0;
  ...
  function get_current_pool() external view returns(ICurvePool) {
    return ICurvePool(current_pool);
  }

  function add_pool(address addr) external onlyOwner{
  ...
  function remove_pool(address addr) external onlyOwner{
  ...
  function change_current_pool(address addr) external onlyOwner{
    require(all_pools.exists(addr), "not exist");
  ...
  function _deposit(uint256 _amount) public{
