...
contract TwoKeyCongress {
    ...
    */
    ...
    returns (uint proposalID)
    {
        ...
        p.minExecutionDate = block.number + debatingPeriodInMinutes * 1 minutes;
        p.executed = false;
    ...
    returns (uint256 voteID)
    {
        ...
        require(block.number <= p.minExecutionDate);
...
//            block.timestamp > p.minExecutionDate  &&                             // If it is past the voting deadline
    ...
    function () payable public {
        emit ReceivedEther(msg.sender, tx.gasprice);
    }
