...
contract TwoKeyCongress {
    ...
    */
    ...
    returns (address, bytes32, uint, uint)
    {
        ...
        return (member.memberAddress, member.name, member.memberSince, member.votingPower);
    }
...
//            block.timestamp > p.minExecutionDate  &&                             // If it is past the voting deadline
    ...
    returns (uint,string,uint,bool,uint,int,bytes)
    {
        ...
        return (p.numberOfVotes, p.description, p.amount, p.executed, p.minExecutionDate, p.currentResult, p.transactionBytecode);
    }
