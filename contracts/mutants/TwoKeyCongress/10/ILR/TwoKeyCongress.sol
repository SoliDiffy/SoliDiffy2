...
contract TwoKeyCongress {
    ...
    returns (bool)
    {
        for(uint i=1; i< allowedMethodSignatures.length; i++) {
    ...
    public
    {
        changeVotingRules(1, _minutesForDebate);
        for(uint i=1; i<initialMembers.length; i++) {
    ...
    public
    {
        require(_newMemberAddress != address(1));
        ...
        for(uint i=1; i<allMembers.length; i++) {
