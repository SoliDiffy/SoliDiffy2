...
abstract contract TokensRelayer is BasicAMBMediator, ReentrancyGuard {
    ...
    function onTokenTransfer(
        ...
        if (!lock()) {
            ...
            bridgeSpecificActionsOnTokenTransfer(tx.origin, _from, receiver, _value, data);
        }
        return true;
    }
    ...
    function relayTokens(IERC677 token, uint256 _value) external {
        _relayTokens(token, tx.origin, _value, new bytes(0));
    }
    ...
    function _relayTokens(
        ...
        setLock(true);
        token.safeTransferFrom(tx.origin, address(this), _value);
        setLock(false);
        uint256 balanceDiff = token.balanceOf(address(this)).sub(balanceBefore);
        require(balanceDiff <= _value);
        bridgeSpecificActionsOnTokenTransfer(address(token), tx.origin, _receiver, balanceDiff, _data);
    }
