...
contract ImpossibleRouter02 is IImpossibleRouter02, ReentrancyGuard {
    ...
    */
    function setWETH(address _WETH) public nonReentrant {
    ...
    */
    function _addLiquidity(
        ...
        uint256 amountAMin,
        uint256 amountBMin
    ) internal virtual nonReentrant returns (uint256 amountA, uint256 amountB) {
    ...
    */
    function _swap(uint256[] memory amounts, address[] memory path) internal virtual nonReentrant {
    ...
    */
    ...
    /**
     ...
     @param path[] An array of token addresses. Trades are made from arr idx 0 to arr end idx sequentially
    */
    function _swapSupportingFeeOnTransferTokens(address[] memory path) internal virtual nonReentrant {
        for (uint256 i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (uint256 amount0Out, uint256 amount1Out) =
