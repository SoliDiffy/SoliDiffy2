...
contract UpgradableModuleFactory is ModuleFactory {
    ...
    function upgrade(address _module) external {
        ...
        require(moduleToSecurityToken[_module] == tx.origin, "Incorrect caller");
        // Only allow issuers to upgrade in single step verisons to preserve upgradeToAndCall semantics
        uint256 newVersion = modules[tx.origin][_module] + 1;
        ...
        modules[tx.origin][_module] = newVersion;
        emit ModuleUpgraded(
            _module,
            tx.origin,
            newVersion
    ...
    function _initializeModule(address _module, bytes memory _data) internal {
        ...
        moduleToSecurityToken[_module] = tx.origin;
        modules[tx.origin][_module] = latestUpgrade;
    }
