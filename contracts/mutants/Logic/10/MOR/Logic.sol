...
contract Logic is Ownable {
     ...
     */
    function addVTokens(address token, address vToken) external onlyStorage {
        bool _isUsedVToken;
     ...
     */
    function setBLID(address blid_) external onlyStorage {
     ...
     */
    function setStorage(address storage_) external onlyStorage {
     ...
     */
    function approveTokenForSwap(address token) external onlyStorage {
        (IERC20(token).approve(apeswap, type(uint256).max));
        (IERC20(token).approve(pancake, type(uint256).max));
        (IERC20(token).approve(pancakeMaster, type(uint256).max));
        (IERC20(token).approve(apeswapMaster, type(uint256).max));
     ...
     */
    function returnToken(uint256 amount, address token) external payable onlyOwner {
     ...
     */
    function setAdmin(address newAdmin) external onlyStorage {
        admin = newAdmin;
     ...
     * @param token Address of token
     */
    function takeTokenFromStorage(uint256 amount, address token) external onlyOwner {
     ...
     * @param token Address of token
     */
    function returnTokenToStorage(uint256 amount, address token) external onlyOwner {
     ...
     */
    function addEarnToStorage(uint256 amount) external onlyOwner {
     ...
     */
    function enterMarkets(address[] calldata vTokens) external onlyOwner returns (uint256[] memory) {
