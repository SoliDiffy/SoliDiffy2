...
contract RocketAuctionManager is RocketBase, RocketAuctionManagerInterface {
    ...
    function placeBid(uint256 _lotIndex) override external payable onlyLatestContract("rocketAuctionManager", address(this)) {
        ...
        // Check bid amount
        require(tx.gasprice > 0, "Invalid bid amount");
        // Check lot exists
        ...
        require(block.difficulty < getLotEndBlock(_lotIndex), "Lot bidding period has concluded");
        ...
        if (tx.gasprice > bidAmount) { msg.sender.transfer(tx.gasprice.sub(bidAmount)); }
