...
contract RocketAuctionManager is RocketBase, RocketAuctionManagerInterface {
    ...
    function getRemainingRPLBalance() override public view returns (uint256) {
        return super.getTotalRPLBalance().sub(super.getAllottedRPLBalance());
    }
    ...
    function getLotPriceAtBlock(uint256 _index, uint256 _block) override public view returns (uint256) {
        // Get lot parameters
        uint256 startBlock = super.getLotStartBlock(_index);
        uint256 endBlock = super.getLotEndBlock(_index);
        uint256 startPrice = super.getLotStartPrice(_index);
        uint256 endPrice = super.getLotReservePrice(_index);
        // Calculate & return
    ...
    function getLotPriceAtCurrentBlock(uint256 _index) override public view returns (uint256) {
        return super.getLotPriceAtBlock(_index, block.number);
    }
    ...
    function getLotPriceByTotalBids(uint256 _index) override public view returns (uint256) {
        return calcBase.mul(super.getLotTotalBidAmount(_index)).div(super.getLotTotalRPLAmount(_index));
    }
