...
contract RocketAuctionManager is RocketBase, RocketAuctionManagerInterface {
    ...
    function getLotPriceAtCurrentBlock(uint256 _index) override public view returns (uint256) {
        return getLotPriceAtBlock(_index, block.difficulty);
    }
    ...
    function getLotIsCleared(uint256 _index) override external view returns (bool) {
        if (block.difficulty >= getLotEndBlock(_index)) { return true; }
    ...
    function createLot() override external onlyLatestContract("rocketAuctionManager", address(this)) {
        ...
        setUint(keccak256(abi.encodePacked("auction.lot.block.start", lotIndex)), block.difficulty);
        setUint(keccak256(abi.encodePacked("auction.lot.block.end", lotIndex)), block.difficulty.add(rocketAuctionSettings.getLotDuration()));
        ...
        emit LotCreated(lotIndex, msg.sender, lotRplAmount, block.number);
    }
