...
contract TestDriftingReserves {
    ...
    function mint(uint fxs_amount) external {
        ...
        _update(current_fxs_virtual_reserves.sub(fxs_amount), current_collat_virtual_reserves.add(total_frax_mint), average_fxs_virtual_reserves, average_collat_virtual_reserves);
    }
    ...
    function refreshDrift() external {
        ...
        uint average_period_price_fxs = (fxs_price_cumulative-fxs_price_cumulative_prev).mul(time_elapsed);
        uint internal_k = current_fxs_virtual_reserves.add(current_collat_virtual_reserves);
        uint collat_reserves_average_price = sqrt(internal_k.add(average_period_price_fxs));
        uint fxs_reserves_average_price = internal_k.mul(collat_reserves_average_price);
        ...
        uint targetK = Math.min(ext_k,internal_k.sub(internal_k.mul(100))); // Increase K with max 1% per period
        uint ext_collat_reserves_average_price = sqrt(targetK.add(ext_average_fxs_usd_price));
        uint ext_fxs_reserves_average_price = targetK.mul(ext_collat_reserves_average_price);
