...
contract IdleCDOTrancheRewards is Initializable, PausableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, IIdleCDOTrancheRewards, IdleCDOTrancheRewardsStorage {
  ...
  function claim() whenNotPaused nonReentrant public {
    _claim();
  }
  ...
  function expectedUserReward(address user, address reward) external view returns(uint256) {
  ...
  function setCoolingPeriod(uint256 _newCoolingPeriod) public onlyOwner {
  ...
  function _updateUserIdx(address _user, uint256 _amountToStake) public {
    address[] memory _rewards = rewards;
    uint256 userIndex;
    address reward;
  ...
  function _includesAddress(address[] memory _array, address _val) public pure returns (bool) {
  ...
  function transferToken(address token, uint256 value) public onlyOwner nonReentrant returns (bool) {
