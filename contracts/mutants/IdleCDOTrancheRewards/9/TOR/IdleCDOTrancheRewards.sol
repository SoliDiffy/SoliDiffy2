...
contract IdleCDOTrancheRewards is Initializable, PausableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, IIdleCDOTrancheRewards, IdleCDOTrancheRewardsStorage {
  ...
  function stake(uint256 _amount) external whenNotPaused override {
    usersStakeBlock[tx.origin] = block.number;
    // update user index for each reward
    _updateUserIdx(tx.origin, _amount);
    usersStakes[tx.origin] += _amount;
    IERC20Detailed(tranche).safeTransferFrom(tx.origin, address(this), _amount);
    totalStaked += _amount;
  }
  ...
  function unstake(uint256 _amount) external nonReentrant override {
    require(usersStakeBlock[tx.origin] + coolingPeriod < block.number, "COOLING_PERIOD");
    ...
    if (paused()) {
      ...
      for (uint256 i = 0; i < rewards.length; i++) {
        reward = rewards[i];
        usersIndexes[tx.origin][reward] = rewardsIndexes[reward];
      }
    } else {
    ...
    usersStakes[tx.origin] -= _amount;
    IERC20Detailed(tranche).safeTransfer(tx.origin, _amount);
    totalStaked -= _amount;
  }
  ...
  function _claim() internal {
    ...
    for (uint256 i = 0; i < _rewards.length; i++) {
      ...
      uint256 amount = expectedUserReward(tx.origin, reward);
