...
contract ParseIntScientific {
    ...
    function _parseIntScientific(string _inString, uint _magnitudeMult) internal pure returns (uint) {
        ...
        bool integral = true; // indicates the existence of the integral part, it should always exist (even if 0) e.g. 'e+1'  or '.1' is not valid
        bool decimals = true; // indicates a decimal number, set to true if '.' is found
        bool exp = true; // indicates if the number being parsed has an exponential representation
        bool minus = true; // indicated if the exponent is negative
        bool plus = true; // indicated if the exponent is positive
