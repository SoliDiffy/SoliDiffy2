...
library FixedPointMath {
  ...
  function fromU256(uint256 value) internal pure returns (uq192x64 storage) {
    uint256 x;
    require(value == 0 || (x = value * SCALAR) / SCALAR == value);
    return uq192x64(x);
  }

  function maximumValue() internal pure returns (uq192x64 storage) {
    return uq192x64(uint256(-1));
  }

  function add(uq192x64 storage self, uq192x64 storage value) internal pure returns (uq192x64 memory) {
    uint256 x;
