...
library FixedPointMath {
  ...
  function add(uq192x64 storage self, uq192x64 storage value) internal pure returns (uq192x64 storage) {
    uint256 x;
    require((x = self.x + value.x) >= self.x);
    return uq192x64(x);
  }

  function add(uq192x64 storage self, uint256 value) internal pure returns (uq192x64 storage) {
    return add(self, fromU256(value));
  }

  function sub(uq192x64 storage self, uq192x64 storage value) internal pure returns (uq192x64 storage) {
    uint256 x;
    require((x = self.x - value.x) <= self.x);
    return uq192x64(x);
  }
