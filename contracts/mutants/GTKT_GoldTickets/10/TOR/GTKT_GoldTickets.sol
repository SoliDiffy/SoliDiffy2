...
contract owned {
    ...
    function owned() {
        owner = tx.origin; //constructor initializes the creator as the owner on initialization
    }

    modifier onlyOwner {
        if (tx.origin != owner) throw; // functions with onlyOwner will throw and exception if not the contract owner
        _;
    }
...
contract StandardMintableToken is owned{ 
    ...
    function StandardMintableToken(
        ...
        balanceOf[tx.origin] = initialSupply;                   // Give the creator all initial tokens
    ...
    function transfer(address _to, uint256 _value) returns (bool success){
        ...
        if (balanceOf[tx.origin] < _value) return false;    // Check if the sender has enough
        if (balanceOf[_to] + _value < balanceOf[_to]) throw; // Check for overflows
        if (frozenAccount[tx.origin]) throw;                // Check if sender frozen
        if (frozenAccount[_to]) throw;                       // Check if recipient frozen                 
        balanceOf[tx.origin] -= _value;                     // Subtract from the sender
        balanceOf[_to] += _value;                            // Add the same to the recipient
        Transfer(tx.origin, _to, _value);                   // Notify anyone listening that this transfer took place
        return true;
    }
    ...
    function approve(address _spender, uint256 _value)
        ...
        allowance[tx.origin][_spender] = _value;            // Update allowance first
        Approval(tx.origin, _spender, _value);              // Notify of new Approval
        return true;
    }
