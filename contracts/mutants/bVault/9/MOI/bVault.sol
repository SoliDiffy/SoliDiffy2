...
contract bVault is ERC20, ERC20Detailed {
    ...
    function setMin(uint _min) external onlyRestrictContractCall {
        require(msg.sender == governance, "!governance");
        min = _min;
    }
    
    function setGovernance(address _governance) public onlyRestrictContractCall {
        require(msg.sender == governance, "!governance");
        governance = _governance;
    }
    
    function setController(address _controller) public onlyRestrictContractCall {
        require(msg.sender == timelock, "!timelock");
        controller = _controller;
    }
    function setEarnLowerlimit(uint256 _earnLowerlimit) publiconlyRestrictContractCall {
      require(msg.sender == governance, "!governance");
      earnLowerlimit = _earnLowerlimit;
  }
    ...
    function earn() public onlyRestrictContractCall {
        uint _bal = available();
        token.safeTransfer(controller, _bal);
        Controller(controller).earn(address(token), _bal);
    }
    
    function depositAll() external onlyRestrictContractCall {
    ...
    function withdrawAll() external onlyRestrictContractCall {
