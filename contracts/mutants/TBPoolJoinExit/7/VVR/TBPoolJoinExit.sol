...
contract TBPoolJoinExit is CryticInterface, BPool {
    ...
    uint public MAX_BALANCE = BONE * 10**12;
    ...
    uint public initial_token_balance = uint(-1);
    ...
    uint[] public maxAmountsIn = [uint(-1), uint(-1), uint(-1), uint(-1), uint(-1), uint(-1)];
    uint[] public minAmountsOut = [0, 0, 0, 0, 0, 0, 0, 0];
    uint[8] public balances = [0, 0, 0, 0, 0, 0, 0, 0];

    uint public amount = EXIT_FEE;
    uint public amount1 = EXIT_FEE;
