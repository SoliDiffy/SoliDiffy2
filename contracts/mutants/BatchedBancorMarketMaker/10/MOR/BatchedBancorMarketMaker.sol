...
contract BatchedBancorMarketMaker is EtherTokenConstant, IsContract, AragonApp {
    ...
    */
    function open() external onlyInit {
    ...
    */
    function updateFormula(IBancorFormula _formula) external onlyInit {
        require(isContract(_formula), ERROR_CONTRACT_IS_EOA);
    ...
    */
    function updateFees(uint256 _buyFeePct, uint256 _sellFeePct) external onlyInit {
        require(_feeIsValid(_buyFeePct) && _feeIsValid(_sellFeePct), ERROR_INVALID_PERCENTAGE);
    ...
    */
    function removeCollateralToken(address _collateral) external onlyInit {
    ...
    */
    function openBuyOrder(address _buyer, address _collateral, uint256 _value) external payable onlyInit {
    ...
    */
    function openSellOrder(address _seller, address _collateral, uint256 _amount) external onlyInit {
        require(isOpen,                                             ERROR_NOT_OPEN);
        require(_collateralIsWhitelisted(_collateral),              ERROR_COLLATERAL_NOT_WHITELISTED);
        require(!_batchIsCancelled(_currentBatchId(), _collateral), ERROR_BATCH_CANCELLED);
