...
contract BatchedBancorMarketMaker is EtherTokenConstant, IsContract, AragonApp {
    ...
    */
    function open() public auth(OPEN_ROLE) {
    ...
    */
    function updateFormula(IBancorFormula _formula) public auth(UPDATE_FORMULA_ROLE) {
        require(isContract(_formula), ERROR_CONTRACT_IS_EOA);
    ...
    */
    function updateFees(uint256 _buyFeePct, uint256 _sellFeePct) public auth(UPDATE_FEES_ROLE) {
        require(_feeIsValid(_buyFeePct) && _feeIsValid(_sellFeePct), ERROR_INVALID_PERCENTAGE);
    ...
    */
    function removeCollateralToken(address _collateral) public auth(REMOVE_COLLATERAL_TOKEN_ROLE) {
    ...
    */
    function openBuyOrder(address _buyer, address _collateral, uint256 _value) public payable auth(OPEN_BUY_ORDER_ROLE) {
    ...
    */
    function openSellOrder(address _seller, address _collateral, uint256 _amount) public auth(OPEN_SELL_ORDER_ROLE) {
        require(isOpen,                                             ERROR_NOT_OPEN);
        require(_collateralIsWhitelisted(_collateral),              ERROR_COLLATERAL_NOT_WHITELISTED);
        require(!_batchIsCancelled(_currentBatchId(), _collateral), ERROR_BATCH_CANCELLED);
