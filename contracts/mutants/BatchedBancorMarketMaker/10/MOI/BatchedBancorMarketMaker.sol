...
contract BatchedBancorMarketMaker is EtherTokenConstant, IsContract, AragonApp {
    ...
    */
    ...
    function _currentBatch(address _collateral) internal onlyInit returns (uint256, Batch storage) {
    ...
    function _open() internal onlyInit {
        isOpen = true;
    ...
    function _updateFormula(IBancorFormula _formula) internal onlyInit {
        formula = _formula;

        emit UpdateFormula(address(_formula));
    }

    function _updateFees(uint256 _buyFeePct, uint256 _sellFeePct) internal onlyInit {
        buyFeePct = _buyFeePct;
        sellFeePct = _sellFeePct;
    ...
    function _cancelCurrentBatch(address _collateral) internal onlyInit {
    ...
    function _removeCollateralToken(address _collateral) internal onlyInit {
    ...
    function _openBuyOrder(address _buyer, address _collateral, uint256 _value) internal onlyInit {
        (uint256 batchId, Batch storage batch) = _currentBatch(_collateral);
