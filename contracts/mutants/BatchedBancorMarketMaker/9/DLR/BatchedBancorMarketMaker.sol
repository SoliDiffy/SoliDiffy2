...
contract BatchedBancorMarketMaker is EtherTokenConstant, IsContract, AragonApp {
    ...
    */
    ...
    {
        Batch memory batch = metaBatches[_batchId].batches[_collateral];
    ...
    function _slippageIsValid(Batch memory _batch, address _collateral) internal view returns (bool) {
    ...
    function _buySlippageIsValid(Batch memory _batch, uint256 _startingPricePPM, uint256 _maximumSlippage) internal view returns (bool) {
        /**
         * NOTE
    ...
    function _sellSlippageIsValid(Batch memory _batch, uint256 _startingPricePPM, uint256 _maximumSlippage) internal view returns (bool) {
        /**
         * NOTE
         * the case where starting price is zero is handled
         * in the meta function _slippageIsValid()
    ...
    function _currentBatch(address _collateral) internal returns (uint256, Batch memory) {
        uint256 batchId = _currentBatchId();
        MetaBatch memory metaBatch = metaBatches[batchId];
