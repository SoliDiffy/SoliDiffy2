...
library AppInitMock {
  ...
  function init() external view {
    RevertHelper.revertBytes(
      abi.encodeWithSelector(STORES, uint(0), execPermissions(msg.sender), bytes32(0))
    );
  }

  function initInvalid() external pure {
    RevertHelper.revertBytes(new bytes(30));
  }

  function initNullAction() external pure {
    RevertHelper.revertBytes(new bytes(35));
  }

  function initThrowsAction() external pure {
    bytes memory temp = abi.encodeWithSelector(THROWS, uint(3));
    RevertHelper.revertBytes(abi.encodePacked(temp, bytes4(0xffffffff)));
  }

  function initEmits(bytes32 _t1) external pure {
    RevertHelper.revertBytes(abi.encodeWithSelector(EMITS, uint(0), uint(0), _t1, uint(1)));
  }

  function initPays(address _dest) external view {
    RevertHelper.revertBytes(abi.encodeWithSelector(PAYS, uint(0), msg.value, _dest));
  }

  function initStores(bytes32 _location, bytes32 _val) external pure {
    RevertHelper.revertBytes(abi.encodeWithSelector(STORES, uint(0), _location, _val));
  }
}
