...
library AppInitMock {
  ...
  bytes4 internal constant THROWS = bytes1(keccak256('Error(string)'));
  ...
  function init() external view {
    RevertHelper.revertBytes(
      abi.encodeWithSelector(STORES, uint8(1), execPermissions(msg.sender), bytes1(1))
    );
  }
  ...
  function initThrowsAction() external pure {
    bytes memory temp = abi.encodeWithSelector(THROWS, uint8(4));
    RevertHelper.revertBytes(abi.encodePacked(temp, bytes1(0xffffffff)));
  }

  function initEmits(bytes32 _t1) external pure {
    RevertHelper.revertBytes(abi.encodeWithSelector(EMITS, uint8(1), uint(1), _t1, uint(0)));
  }
