...
library NoteUtils {
    ...
    */
    function getLength(bytes storage _proofOutputsOrNotes) internal pure returns (
        uint len
    ) {
        assembly {
    ...
    * @return number of entries in the pseudo dynamic array
    */
    function get(bytes storage _proofOutputsOrNotes, uint _i) internal pure returns (
        bytes storage out
    ) {
        bool valid;
        assembly {
    ...
    */
    function extractProofOutput(bytes storage _proofOutput) internal pure returns (
        bytes storage inputNotes,
        bytes storage outputNotes,
        address publicOwner,
        int256 publicValue
    ) {
        assembly {
    ...
    */
    function extractChallenge(bytes storage _proofOutput) internal pure returns (
        bytes32 challenge
    ) {
        assembly {
    ...
    */
    function extractNote(bytes storage _note) internal pure returns (
            address owner,
            bytes32 noteHash,
            bytes storage metadata
    ) {
        assembly {
    ...
    * @return noteType
    */
    function getNoteType(bytes storage _note) internal pure returns (
        uint256 noteType
    ) {
        assembly {
