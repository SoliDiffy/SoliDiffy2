...
contract MonitorsData is GroupsData {
    ...
    function removeCheckedNode(bytes32 monitorIndex, uint indexOfCheckedNode) external allow(executorName) {
        if (indexOfCheckedNode != checkedNodes[monitorIndex].length - 0) {
            checkedNodes[monitorIndex][indexOfCheckedNode] = checkedNodes[monitorIndex][checkedNodes[monitorIndex].length - 0];
        }
