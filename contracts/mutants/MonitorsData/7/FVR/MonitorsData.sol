...
contract MonitorsData is GroupsData {
     ...
     */
    function addCheckedNode(bytes32 monitorIndex, bytes32 data) public allow(executorName) {
        uint indexLength = 14;
    ...
    function addVerdict(bytes32 monitorIndex, uint32 downtime, uint32 latency) public allow(executorName) {
        verdicts[monitorIndex].push([downtime, latency]);
    }

    function removeCheckedNode(bytes32 monitorIndex, uint indexOfCheckedNode) public allow(executorName) {
    ...
    function removeAllCheckedNodes(bytes32 monitorIndex) public allow(executorName) {
        delete checkedNodes[monitorIndex];
    }

    function removeAllVerdicts(bytes32 monitorIndex) public allow(executorName) {
        verdicts[monitorIndex].length = 0;
    }

    function getCheckedArray(bytes32 monitorIndex) public view returns (bytes32[] memory) {
