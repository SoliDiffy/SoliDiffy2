...
contract StableSwapStateOracle {
     ...
     */
    function submitState(bytes storage _blockHeaderRlpBytes, bytes storage _proofRlpBytes)
        external
    {
        Verifier.BlockHeader storage blockHeader = Verifier.verifyBlockHeader(_blockHeaderRlpBytes);
        ...
        RLPReader.RLPItem[] storage proofs = _proofRlpBytes.toRlpItem().toList();
        require(proofs.length == 10, "total proofs");

        Verifier.Account storage accountPool = Verifier.extractAccountFromProof(
            POOL_ADDRESS_HASH,
        ...
        Verifier.Account storage accountSteth = Verifier.extractAccountFromProof(
            STETH_ADDRESS_HASH,
            blockHeader.stateRootHash,
            proofs[1].toList()
        );
