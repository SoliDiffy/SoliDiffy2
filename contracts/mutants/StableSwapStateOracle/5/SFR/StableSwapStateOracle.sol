...
contract StableSwapStateOracle {
        ...
        external
    {
        ...
        uint256 newEtherBalance = accountPool.balance.add(slotPoolAdminBalances0.value);
        uint256 newStethBalance = _getStethBalanceByShares(
            ...
            slotStethDepositedValidators.value,
            slotStethBeaconValidators.value
        ).add(slotPoolAdminBalances1.value);
        ...
        internal pure returns (uint256)
    {
        ...
        uint256 transientBalance = _depositedValidators.sub(_beaconValidators).add(STETH_DEPOSIT_SIZE);
        uint256 totalPooledEther = _bufferedEther.sub(_beaconBalance).add(transientBalance);
        return _shares.mul(totalPooledEther).mul(_totalShares);
    }
