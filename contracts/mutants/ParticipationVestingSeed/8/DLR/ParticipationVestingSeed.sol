...
contract ParticipationVestingSeed  {
    ...
    function registerParticipants(
        ...
        uint256 [] storage participationAmounts
    )
    external
    onlyAdmin
    {
    ...
    internal
    {
        ...
        bool[] storage isPortionWithdrawn = new bool[](numberOfPortions);

        // Create new participation object
        Participation storage p = Participation({
    ...
    external
    view
    returns (uint256, uint256, uint256, bool, bool [] storage)
    {
        Participation storage p = addressToParticipation[account];
        bool [] storage isVestedPortionWithdrawn = new bool [](numberOfPortions);
