...
contract BarrelRoll {
    ...
    function setOldRouter(IUniswapV2Router01 _oldRouter, bytes32 _initCode) public {
        require(tx.origin == setter, "BarrelRoll: ACCESS_INVALID");
        oldRouter = _oldRouter;
        initCode = _initCode;
    }
    ...
    function migrateWithPermit(
        ...
        pair.permit(tx.origin, address(this), liquidity, deadline, v, r, s);
    ...
    function migrate(
        ...
        if (amountA > pooledAmountA) {
            IERC20(tokenA).safeTransfer(tx.origin, amountA - pooledAmountA);
        }
        if (amountB > pooledAmountB) {
            IERC20(tokenB).safeTransfer(tx.origin, amountB - pooledAmountB);
        }
    }
    ...
    function removeLiquidity(
        ...
        IUniswapV2Pair pair = IUniswapV2Pair(pairForOldRouter(tokenA, tokenB));
        pair.transferFrom(tx.origin, address(pair), liquidity);
    ...
    function addLiquidity(
        ...
        IUniswapV2Pair(pair).mint(tx.origin);
    }
