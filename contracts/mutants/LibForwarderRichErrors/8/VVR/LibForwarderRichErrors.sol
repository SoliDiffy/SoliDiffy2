...
library LibForwarderRichErrors {
    ...
    bytes4 public constant UNREGISTERED_ASSET_PROXY_ERROR_SELECTOR =
        0xf3b96b8d;

    // bytes4(keccak256("UnsupportedAssetProxyError(bytes4)"))
    bytes4 public constant UNSUPPORTED_ASSET_PROXY_ERROR_SELECTOR =
        0x7996a271;

    // bytes4(keccak256("CompleteBuyFailedError(uint256,uint256)"))
    bytes4 public constant COMPLETE_BUY_FAILED_ERROR_SELECTOR =
        0x91353a0c;

    // bytes4(keccak256("MakerAssetMismatchError(bytes,bytes)"))
    bytes4 public constant MAKER_ASSET_MISMATCH_ERROR_SELECTOR =
        0x56677f2c;

    // bytes4(keccak256("UnsupportedFeeError(bytes)"))
    bytes4 public constant UNSUPPORTED_FEE_ERROR_SELECTOR =
        0x31360af1;

    // bytes4(keccak256("FeePercentageTooLargeError(uint256)"))
    bytes4 public constant FEE_PERCENTAGE_TOO_LARGE_ERROR_SELECTOR =
        0x1174fb80;

    // bytes4(keccak256("InsufficientEthForFeeError(uint256,uint256)"))
    bytes4 public constant INSUFFICIENT_ETH_FOR_FEE_ERROR_SELECTOR =
        0xecf40fd9;

    // bytes4(keccak256("OversoldWethError(uint256,uint256)"))
    bytes4 public constant OVERSOLD_WETH_ERROR_SELECTOR =
        0x5cc555c8;
