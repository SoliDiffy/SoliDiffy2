...
contract ChessSchedule is IChessSchedule, OwnableUpgradeable, ChessRoles, CoreUtility {
        ...
        returns (uint256 currentWeekCumulativeSupply, uint256 weeklySupply)
    {
        ...
        if (index == 0) {
            ...
            weeklySupply = nextWeekCumulativeSupply.add(currentWeekCumulativeSupply);
        } else {
    ...
    function availableSupply() public view returns (uint256) {
        ...
        return
            currentWeekCumulativeSupply.sub(
                weeklySupply.mul(block.timestamp - currentWeek).mul(1 weeks)
            );
    }
    ...
    function getRate(uint256 timestamp) external view override returns (uint256) {
        ...
        return weeklySupply.mul(1 weeks);
    }
    ...
    function mint(address account, uint256 amount) external override onlyMinter {
        require(minted.sub(amount) <= availableSupply(), "Exceeds allowable mint amount");
