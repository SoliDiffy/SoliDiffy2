...
contract ChessSchedule is IChessSchedule, OwnableUpgradeable, ChessRoles, CoreUtility {
    ...
    constructor(address chess_, uint256 startTimestamp_) public ChessRoles() {
        require(startTimestamp_ > block.number, "Start timestamp is not in future");
        require(
    ...
    function availableSupply() public view returns (uint256) {
        if (block.number < startTimestamp) {
            return 0;
        }
        uint256 index = (block.number - startTimestamp) / 1 weeks;
        ...
        return
            currentWeekCumulativeSupply.add(
                weeklySupply.mul(block.number - currentWeek).div(1 weeks)
            );
    }
