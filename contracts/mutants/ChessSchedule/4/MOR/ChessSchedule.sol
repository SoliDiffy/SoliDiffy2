...
contract ChessSchedule is IChessSchedule, OwnableUpgradeable, ChessRoles, CoreUtility {
    ...
    function initialize() external ChessRoles() {
        __Ownable_init();
    ...
    function mint(address account, uint256 amount) external override ChessRoles() {
        require(minted.add(amount) <= availableSupply(), "Exceeds allowable mint amount");
        chess.transfer(account, amount);
        minted = minted.add(amount);
    }

    function addMinter(address account) external override ChessRoles() {
        _addMinter(account);
    }

    function removeMinter(address account) external ChessRoles() {
