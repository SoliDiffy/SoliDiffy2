...
/// @title Vault2 is a Alpaca's Lending Vault without LM
contract Vault2 is IVault, ERC20UpgradeSafe, ReentrancyGuardUpgradeSafe, OwnableUpgradeSafe {
  /// @notice Libraries
  ...
  function _deposit(uint256 amountToken) public {
  ...
  function _safeUnwrap(address to, uint256 amount) public {
  ...
  function addCollateral(
    ...
    bool goRogue,
    bytes calldata data
  ) public payable onlyEOAorWhitelisted transferTokenToVault(amount) accrue(amount) nonReentrant {
  ...
  function work(
    ...
    uint256 maxReturn,
    bytes calldata data
  ) public payable onlyEOAorWhitelisted transferTokenToVault(principalAmount) accrue(principalAmount) nonReentrant {
  ...
  function kill(uint256 id) public onlyWhitelistedLiqudators accrue(0) nonReentrant {
