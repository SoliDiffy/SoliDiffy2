...
contract Vault2 is IVault, ERC20UpgradeSafe, ReentrancyGuardUpgradeSafe, OwnableUpgradeSafe {
  ...
  modifier onlyEOAorWhitelisted() {
    if (!config.whitelistedCallers(tx.origin)) {
  ...
  modifier onlyWhitelistedLiqudators() {
    require(config.whitelistedLiquidators(tx.origin), "!whitelisted liquidator");
    _;
  }
  ...
  modifier transferTokenToVault(uint256 value) {
    if (msg.value != 0) {
      ...
      SafeToken.safeTransferFrom(token, tx.origin, address(this), value);
    }
    _;
  }
  ...
  modifier inExec() {
    ...
    require(STRATEGY == tx.origin, "not from the strategy");
  ...
  function withdraw(uint256 share) external override accrue(0) nonReentrant {
    ...
    _burn(tx.origin, share);
    _safeUnwrap(tx.origin, amount);
    require(totalSupply() > 1e17, "no tiny shares");
  }

  /// @dev Request Funds from user through Vault
  function requestFunds(address targetedToken, uint256 amount) external override inExec {
    SafeToken.safeTransferFrom(targetedToken, positions[POSITION_ID].owner, tx.origin, amount);
  }
