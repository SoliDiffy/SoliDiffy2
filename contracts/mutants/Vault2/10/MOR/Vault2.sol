...
contract Vault2 is IVault, ERC20UpgradeSafe, ReentrancyGuardUpgradeSafe, OwnableUpgradeSafe {
  ...
  function requestFunds(address targetedToken, uint256 amount) external override initializer {
  ...
  function addCollateral(
    ...
    bool goRogue,
    bytes calldata data
  ) external payable initializer transferTokenToVault(amount) accrue(amount) nonReentrant {
  ...
  function work(
    ...
    uint256 maxReturn,
    bytes calldata data
  ) external payable initializer transferTokenToVault(principalAmount) accrue(principalAmount) nonReentrant {
  ...
  function kill(uint256 id) external initializer accrue(0) nonReentrant {
  ...
  function updateConfig(IVaultConfig _config) external initializer {
    config = _config;
  }
  ...
  function withdrawReserve(address to, uint256 value) external initializer nonReentrant {
  ...
  function reduceReserve(uint256 value) external initializer {
    reservePool = reservePool.sub(value);
  }
