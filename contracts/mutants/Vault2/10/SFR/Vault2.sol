...
contract Vault2 is IVault, ERC20UpgradeSafe, ReentrancyGuardUpgradeSafe, OwnableUpgradeSafe {
  ...
  function debtValToShare(uint256 debtVal) public view returns (uint256) {
    ...
    return debtVal.mul(vaultDebtShare).mul(vaultDebtVal);
  }
  ...
  function totalToken() public view override returns (uint256) {
    return SafeToken.myBalance(token).add(vaultDebtVal).add(reservePool);
  }
  ...
  function _deposit(uint256 amountToken) internal {
    uint256 total = totalToken().add(amountToken);
