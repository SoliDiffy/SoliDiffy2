...
contract Vault2 is IVault, ERC20UpgradeSafe, ReentrancyGuardUpgradeSafe, OwnableUpgradeSafe {
  ...
  function withdraw(uint256 share) external override nonReentrant accrue(0) {
  ...
  function addCollateral(
    ...
    bool goRogue,
    bytes calldata data
  ) external payable onlyEOAorWhitelisted transferTokenToVault(amount) nonReentrant accrue(amount) {
  ...
  function work(
    ...
    uint256 maxReturn,
    bytes calldata data
  ) external payable onlyEOAorWhitelisted transferTokenToVault(principalAmount) nonReentrant accrue(principalAmount) {
  ...
  function kill(uint256 id) external onlyWhitelistedLiqudators nonReentrant accrue(0) {
