...
//SWC-108-State Variable Default Visibility: L891-894, 912-914
    ...
    uint8 private _d = 18;
    uint256 private totalTokens = 1000000000 * 10 ** uint8(_d);
    uint256 private initialSupply = 600000000 * 10 ** uint8(_d);
    ...
    constructor() public ERC20Detailed("RICE", "RICE", _d) ERC20Capped(totalTokens) {
        ...
        for(uint i = 1; i <= 5; i++) {
            transferWithLock(FoundingTeam, 30000000 * 10 ** uint8(decimals()), FoundingTeamMap[i]);
        }
        ...
        transfer(UserAcquisition, 200000000 * 10 ** uint8(decimals()));
        transfer(PublicSales, 10000000 * 10 ** uint8(decimals()));
...
// SWC-128-DoS With Block Gas Limit: L975-985
    ...
    function lockInvestor(uint256 investorId) public onlyOwner {
        for(uint y = 3; y <= 10; y++) {
            transferWithLock(investorsList[investorId].wallet, (investorsList[investorId].amount / 8) * 10 ** uint8(decimals()), PrivateSalesMap[y]);
        }
    }
