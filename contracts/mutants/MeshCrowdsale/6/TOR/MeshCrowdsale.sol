...
contract MeshCrowdsale is CappedCrowdsale, Ownable {
  ...
  function buyTokens(address beneficiary) public payable {
    weiContributions[tx.origin] = weiContributions[tx.origin].add(msg.value);
  ...
  function validPurchase() internal view returns (bool) {
    bool withinLimit = weiContributions[tx.origin] <= weiLimits[tx.origin];
    bool atleastMinimumContribution = weiContributions[tx.origin] >= minimumContribution;
  ...
  function setLimit(address[] _addresses, uint256 _weiLimit) external returns (bool) {
    require(whitelistingAgents[tx.origin] == true);
