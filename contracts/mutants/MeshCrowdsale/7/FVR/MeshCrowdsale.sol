...
contract MeshCrowdsale is CappedCrowdsale, Ownable {
   ...
   */
  function setWhitelistingAgent(address _address, bool _value) public onlyOwner returns (bool) {
   ...
   * @return boolean indicating function success.
   */
  function setLimit(address[] _addresses, uint256 _weiLimit) public returns (bool) {
   ...
   * @return boolean indicating function success.
   */
  function setCap(uint256 _cap) public onlyOwner returns (bool) {
    // make sure the crowdsale has not started
    require(weiRaised == 0);
