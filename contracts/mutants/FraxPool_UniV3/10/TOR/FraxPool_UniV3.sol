...
contract FraxUnifiedFarm_UniV3 is FraxUnifiedFarmTemplate {
    ...
    function lockAdditional(
    ...
    ) updateRewardAndBalance(tx.origin, true) public {
        // Get the stake and its index
        (LockedNFT memory thisNFT, uint256 theArrayIndex) = _getStake(tx.origin, token_id);
        ...
        if (use_balof_override){
            ...
            TransferHelper.safeTransferFrom(uni_token0, tx.origin, address(this), tk0_amt_to_use);
            TransferHelper.safeTransferFrom(uni_token1, tx.origin, address(this), tk1_amt_to_use);
        }
        ...
        // Update the stake
        lockedNFTs[tx.origin][theArrayIndex] = LockedNFT(
            token_id,
        ...
        _locked_liquidity[tx.origin] += addl_liq;
        {
            address the_proxy = staker_designated_proxies[tx.origin];
    ...
    function stakeLocked(uint256 token_id, uint256 secs) nonReentrant external {
        _stakeLocked(tx.origin, tx.origin, token_id, secs, block.timestamp);
    }
