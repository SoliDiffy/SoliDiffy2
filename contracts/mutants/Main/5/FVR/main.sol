...
contract BUSDVYNCSTAKE is ReentrancyGuard, Ownable {
    ...
    function set_compoundStart(uint256 _blocktime) external onlyOwner {
    ...
    function set_data(address _data) external onlyOwner {
        dataAddress = _data;
        data = GetDataInterface(_data);
    }

    function set_treasuryAddress(address _treasury) external onlyOwner{
        TreasuryAddress= _treasury;
        treasury = TreasuryInterface(_treasury);
    }

    function nextCompound() external view returns (uint256 _nextCompound) {
