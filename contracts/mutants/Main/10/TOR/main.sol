...
// SWC-104-Unchecked Call Return Value: L117
    function stake(uint256 amount) external nonReentrant {
        busd.transferFrom(tx.origin, address(this), amount);
        userInfo[tx.origin]
            .lastCompoundedRewardWithStakeUnstakeClaim = lastCompoundedReward(
            tx.origin
        );

        if (userInfo[tx.origin].isStaker == true) {
            uint256 _pendingReward = compoundedReward(tx.origin);
            uint256 cpending = cPendingReward(tx.origin);
            userInfo[tx.origin].stakeBalanceWithReward =
                userInfo[tx.origin].stakeBalanceWithReward +
                _pendingReward;
            userInfo[tx.origin].autoClaimWithStakeUnstake =
                userInfo[tx.origin].autoClaimWithStakeUnstake +
                _pendingReward;
