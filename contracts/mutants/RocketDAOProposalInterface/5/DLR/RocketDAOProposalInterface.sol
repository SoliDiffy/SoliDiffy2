...
interface RocketDAOProposalInterface {
    ...
    function getDAO(uint256 _proposalID) external view returns (string storage);
    function getProposer(uint256 _proposalID) external view returns (address);
    function getMessage(uint256 _proposalID) external view returns (string storage);
    ...
    function getPayload(uint256 _proposalID) external view returns (bytes storage);
    ...
    function add(address _member, string storage _dao, string storage _message, uint256 _startBlock, uint256 _durationBlocks, uint256 _expiresBlocks, uint256 _votesRequired, bytes memory _payload) external returns (uint256);
