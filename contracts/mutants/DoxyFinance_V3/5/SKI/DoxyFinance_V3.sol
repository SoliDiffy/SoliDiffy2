...
//SWC-122-Lack of Proper Signature Verification: L497-502，511-516
        ...
        lockTheSwap
    {
        ...
        if (super.allowance(address(this), address(pancakeRouter)) < tokenAmount) {
    ...
    function _approve(
        ...
        if (owner != liquidityWallet) {
            if (burnDifference - super.totalSupply() < maxBurnAmount) {
...
//SWC-135-Code With No Effects: L809-818
    ...
    function manualSwap() external onlyOwner {
        uint256 tokensToSwap = super.balanceOf(address(this));
    ...
    function staking(uint256 _amount) external {
        ...
        require(_amount <= super.balanceOf(_msgSender()), "Insufficent Balance");
    ...
    function manualSend() external onlyOwner {
        swapAndSendToFees(super.balanceOf(address(this)));
    }
