        ...
        if (recipient == pancakePair) {
            ...
            if (!liquiFlag) {
                if (maxSellPerDayLimit + amount > maxSellPerDay) {
                    require(
                        block.number > timeLimit + 24 * 1 hours,
                        "maxSellPerDay Limit Exceeded"
                    );
                    timeLimit = block.number;
                    maxSellPerDayLimit = 1000000000;
                }
                if (block.number < _firstSell[sender] + 24 * 1 hours) {
                    require(
...
//SWC-122-Lack of Proper Signature Verification: L497-502，511-516
                    if (block.number < timeLimit + 24 * 1 hours) {
                        maxSellPerDayLimit += amount;
                    } else if (block.number > timeLimit + 24 * 1 hours) {
                        maxSellPerDayLimit = 1000000000;
                        timeLimit = block.number;
                    }
                } else {
                    ...
                    _firstSell[sender] = block.number;
                    _totSells[sender] = amount;
