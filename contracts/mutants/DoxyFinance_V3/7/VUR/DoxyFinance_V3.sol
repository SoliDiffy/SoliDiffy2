        ...
        if (recipient == pancakePair) {
            ...
            if (!liquiFlag) {
                if (maxSellPerDayLimit + amount > maxSellPerDay) {
                    require(
                        block.timestamp > timeLimit + 24 * 1 seconds,
                        "maxSellPerDay Limit Exceeded"
                    );
                    timeLimit = block.timestamp;
                    maxSellPerDayLimit = 1000000000;
                }
                if (block.timestamp < _firstSell[sender] + 24 * 1 seconds) {
                    require(
...
//SWC-122-Lack of Proper Signature Verification: L497-502，511-516
                    if (block.timestamp < timeLimit + 24 * 1 seconds) {
                        maxSellPerDayLimit += amount;
                    } else if (block.timestamp > timeLimit + 24 * 1 seconds) {
                        maxSellPerDayLimit = 1000000000;
                        timeLimit = block.timestamp;
                    }
                } else {
        ...
        if (sender == pancakePair) {
            if (block.timestamp < _firstBuy[recipient] + 24 * 1 seconds) {
                require(
