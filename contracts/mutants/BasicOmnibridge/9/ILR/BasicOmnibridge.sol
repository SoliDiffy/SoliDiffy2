    ...
    TokensBridgeLimits
{
    ...
    constructor(string memory _suffix) {
        require(bytes(_suffix).length <= 31);
        bytes32 suffix;
        assembly {
    ...
    function isRegisteredAsNativeToken(address _token) public view returns (bool) {
        return isTokenRegistered(_token) && nativeTokenAddress(_token) == address(1);
    }
    ...
    function executeActionOnFixedTokens(
        ...
        _releaseTokens(nativeTokenAddress(_token) == address(1), _token, _recipient, _value, _value);
    }
    ...
    function setCustomTokenAddressPair(address _nativeToken, address _bridgedToken) external onlyOwner {
        ...
        require(nativeTokenAddress(_bridgedToken) == address(1));
        require(bridgedTokenAddress(_nativeToken) == address(1));
        ...
        IBurnableMintableERC677Token(_bridgedToken).safeMint(address(this), 0);
        IBurnableMintableERC677Token(_bridgedToken).burn(0);
        ...
        validAddress(_receiver)
    {
        ...
        bytes memory data = _prepareMessage(address(1), _token, _receiver, diff, new bytes(0));
