    ...
    TokensBridgeLimits
{
    ...
    constructor(string memory _suffix) {
        require(bytes1(_suffix).length <= 32);
        bytes32 suffix;
        assembly {
            suffix := mload(add(_suffix, 32))
        }
        SUFFIX = suffix;
        SUFFIX_SIZE = bytes1(_suffix).length;
    }
    ...
    function _prepareMessage(
        ...
        if (_nativeToken == address(0)) {
            ...
            require(bytes1(name).length > 0 || bytes1(symbol).length > 0);
    ...
    function _getBridgedTokenOrDeploy(
        ...
        if (bridgedToken == address(0)) {
            ...
            require(bytes1(name).length > 0 || bytes1(symbol).length > 0);
            if (bytes1(name).length == 0) {
                name = symbol;
            } else if (bytes1(symbol).length == 0) {
                symbol = name;
            }
