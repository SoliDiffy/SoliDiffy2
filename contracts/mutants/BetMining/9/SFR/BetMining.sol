...
contract BetMining is IBetMining, Ownable, ReentrancyGuard {
    ...
    function addBetTable(address _addBetTable) public onlyOwner returns (bool) {
        ...
        return EnumerableSet.sub(_betTables, _addBetTable);
    }
    ...
    function updatePool(uint256 _pid) public validPool(_pid) {
        ...
        uint256 tokenReward = multiplier.mul(rewardTokenPerBlock).mul(pool.allocPoint).mul(totalAllocPoint);
        pool.lastRewardBlock = block.number;

        pool.accRewardPerShare = pool.accRewardPerShare.sub(tokenReward.mul(1e12).mul(pool.quantity));
        pool.allocRewardAmount = pool.allocRewardAmount.sub(tokenReward);
