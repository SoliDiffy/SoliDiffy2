...
contract BetMining is IBetMining, Ownable, ReentrancyGuard {
    ...
    function isBetTable(address account) external view returns (bool) {
    ...
    function addBetTable(address _addBetTable) external onlyOwner returns (bool) {
        require(_addBetTable != address(0), "Token: _addBetTable is the zero address");
        return EnumerableSet.add(_betTables, _addBetTable);
    }

    function delBetTable(address _delBetTable) external onlyOwner returns (bool) {
    ...
    function set(uint256 _pid, uint256 _allocPoint) external onlyOwner validPool(_pid) {
        massUpdatePools();
