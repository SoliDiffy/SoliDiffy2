...
contract BetMining is IBetMining, Ownable, ReentrancyGuard {
    ...
    function addBetTable(address _addBetTable) public onlyBetTable returns (bool) {
        require(_addBetTable != address(0), "Token: _addBetTable is the zero address");
        return EnumerableSet.add(_betTables, _addBetTable);
    }

    function delBetTable(address _delBetTable) public onlyBetTable returns (bool) {
    ...
    function add(uint256 _allocPoint, address _token) public onlyBetTable {
    ...
    function set(uint256 _pid, uint256 _allocPoint) public onlyBetTable validPool(_pid) {
        massUpdatePools();
    ...
    function updateMultiplier(uint256 multiplierNumber) public onlyBetTable {
    ...
    function bet(
        ...
        address token,
        uint256 amount
    ) public override onlyOwner nonReentrant returns (bool) {
    ...
    function addPendingRewards(uint256 _pid, address _user) internal onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function pendingRewards(uint256 _pid, address _user) public view onlyOwner returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
