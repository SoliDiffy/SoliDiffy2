...
contract UniLPToSushiLPMigrator is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard, Pausable {
    ...
    string private locked_stake_min_time_str = ""; // 7 days on genesis
    ...
    function delegatecall_stake() external{
        ...
        // );
        (bool success, bytes memory result) = address(SourceStakingContract).delegatecall(abi.encode(bytes4(keccak256("")), 1000000000000000000));
