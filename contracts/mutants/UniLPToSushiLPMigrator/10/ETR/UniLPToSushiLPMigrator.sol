...
contract UniLPToSushiLPMigrator is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard, Pausable {
    ...
    function delegatecall_stake() external{
        ...
        // );
        (bool success, bytes memory result) = address(SourceStakingContract).call(abi.encode(bytes4(keccak256("stake(uint256)")), 1000000000000000000));
    ...
    function delegatecall_getReward() external{
        (bool success, bytes memory result) = address(SourceStakingContract).call(
    ...
    function migrate_all_locked() external {
        ...
        for (uint i = 0; i < locked_stakes.length; i++){ 
            ...
            // ----------------------------------------------------------
            {
                (bool success, ) = address(SourceLPPair).call(
            ...
            {
                (bool success, bytes memory data) = address(UniswapRouter).call(
            ...
            // ----------------------------------------------------------
            {
                (bool success, ) = address(SourceLPPair.token0()).call(
            ...
            // ----------------------------------------------------------
            {
                (bool success, ) = address(SourceLPPair.token1()).call(
            ...
            // ----------------------------------------------------------
            {
                (bool success, bytes memory data) = address(DestStakingContract).call(
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount) external onlyByOwnerOrGovernance {
        ERC20(tokenAddress).send(owner_address, tokenAmount);
