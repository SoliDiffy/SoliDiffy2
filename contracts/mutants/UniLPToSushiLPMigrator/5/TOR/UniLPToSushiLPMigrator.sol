...
contract UniLPToSushiLPMigrator is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard, Pausable {
    ...
    function migrate_all_locked() external {
        ...
        IStakingRewardsDualForMigrator.ILockedStake[] memory locked_stakes = SourceStakingContract.lockedStakesOf(tx.origin);
        ...
        emit Migrated(tx.origin, source_staking_contract_addr, dest_staking_contract_addr);
    }
    ...
    modifier onlyByOwnerOrGovernance() {
        require(tx.origin == owner_address || msg.sender == timelock_address, "You are not the owner or the governance timelock");
        _;
    }
