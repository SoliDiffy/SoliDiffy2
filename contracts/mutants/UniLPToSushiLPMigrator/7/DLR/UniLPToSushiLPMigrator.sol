...
contract UniLPToSushiLPMigrator is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard, Pausable {
    ...
    function delegatecall_stake() external{
        ...
        // );
        (bool success, bytes storage result) = address(SourceStakingContract).delegatecall(abi.encode(bytes4(keccak256("stake(uint256)")), 1000000000000000000));
    ...
    function delegatecall_getReward() external{
        (bool success, bytes storage result) = address(SourceStakingContract).delegatecall(
    ...
    function migrate_all_locked() external {
        ...
        IStakingRewardsDualForMigrator.ILockedStake[] storage locked_stakes = SourceStakingContract.lockedStakesOf(msg.sender);
        for (uint i = 0; i < locked_stakes.length; i++){ 
            ...
            {
                (bool success, bytes storage data) = address(UniswapRouter).delegatecall(
            ...
            {
                (bool success, bytes storage data) = address(SushiSwapRouter).delegatecall(
            ...
            // ----------------------------------------------------------
            {
                (bool success, bytes storage data) = address(DestStakingContract).delegatecall(
