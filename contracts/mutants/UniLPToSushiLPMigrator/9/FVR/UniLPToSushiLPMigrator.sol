...
contract UniLPToSushiLPMigrator is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard, Pausable {
    ...
    function delegatecall_stake() public{
    ...
    function delegatecall_getReward() public{
        (bool success, bytes memory result) = address(SourceStakingContract).delegatecall(
    ...
    function normal_getReward() public {
        SourceStakingContract.getReward();
    }

    function migrate_unlocked() public {

    }

    function migrate_one_locked(bytes32 kek_id) public {

    }

    function migrate_all_locked() public {
        // Unlock the stakes
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount) public onlyByOwnerOrGovernance {
        ERC20(tokenAddress).transfer(owner_address, tokenAmount);
        emit Recovered(tokenAddress, tokenAmount);
    }

    function setOwnerAndTimelock(address _new_owner, address _new_timelock) public onlyByOwnerOrGovernance {
