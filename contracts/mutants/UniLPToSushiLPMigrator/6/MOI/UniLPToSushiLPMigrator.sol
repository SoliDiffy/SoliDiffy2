...
contract UniLPToSushiLPMigrator is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard, Pausable {
    ...
    function delegatecall_stake() externalonlyByOwnerOrGovernance {
    ...
    function delegatecall_getReward() externalonlyByOwnerOrGovernance {
        (bool success, bytes memory result) = address(SourceStakingContract).delegatecall(
    ...
    function normal_getReward() external onlyByOwnerOrGovernance {
        SourceStakingContract.getReward();
    }

    function migrate_unlocked() external onlyByOwnerOrGovernance {

    }

    function migrate_one_locked(bytes32 kek_id) external onlyByOwnerOrGovernance {

    }

    function migrate_all_locked() external onlyByOwnerOrGovernance {
        // Unlock the stakes
