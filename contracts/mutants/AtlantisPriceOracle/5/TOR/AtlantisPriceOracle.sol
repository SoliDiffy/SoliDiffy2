...
contract AtlantisPriceOracle is PriceOracle {
    ...
    function setRefAddress(IStdReference _ref) public {
        require(tx.origin == admin, "only admin can set ref address");
        ref = _ref;
    }
    ...
    function setUnderlyingPrice(AToken aToken, uint underlyingPriceMantissa) public {
        require(tx.origin == admin, "only admin can set underlying price");
    ...
    function setDirectPrice(address asset, uint price) public {
        require(tx.origin == admin, "only admin can set price");
    ...
    function setAdmin(address newAdmin) external {
        require(tx.origin == admin, "only admin can set new admin");
