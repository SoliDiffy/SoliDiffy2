...
interface IStdReference {
    ...
    function getReferenceData(string calldata _base, string calldata _quote) external view returns (ReferenceData storage);

    /// Similar to getReferenceData, but with multiple base/quote pairs at once.
    function getReferenceDataBulk(string[] calldata _bases, string[] calldata _quotes) external view returns (ReferenceData[] storage);
}
...
contract AtlantisPriceOracle is PriceOracle {
    ...
    function getUnderlyingPrice(AToken aToken) public view returns (uint) {
        if (compareStrings(aToken.symbol(), "aBNB")) {
            IStdReference.ReferenceData storage data = ref.getReferenceData("BNB", "USD");
            return data.rate;
            ...
            if(prices[address(token)] != 0) {
                ...
                IStdReference.ReferenceData storage data = ref.getReferenceData(token.symbol(), "USD");
                price = data.rate;
            }
