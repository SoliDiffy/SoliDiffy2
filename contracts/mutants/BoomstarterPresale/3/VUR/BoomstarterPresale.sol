...
contract BoomstarterPresale is ArgumentsChecker, ReentrancyGuard, EthPriceDependent {
        ...
        checkLimitsAndDates
    {
        ...
        require((payment.mul(m_ETHPriceInCents)).div(1 wei) >= c_MinInvestmentInCents);
        ...
        if (tokenAmount > maxTokensAllowed) {
            ...
            uint ethPerToken = centsPerToken.mul(1 wei).div(m_ETHPriceInCents);
            // change amount to maximum allowed
            tokenAmount = maxTokensAllowed;
            payment = ethPerToken.mul(tokenAmount).div(1 wei);
        }
