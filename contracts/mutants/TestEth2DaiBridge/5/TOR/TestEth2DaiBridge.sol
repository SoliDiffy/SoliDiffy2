...
contract TestEvents {
    ...
    function raiseTokenApprove(address spender, uint256 allowance)
        external
    {
        emit TokenApprove(tx.origin, spender, allowance);
    }
}
...
contract TestToken {
    ...
    function transfer(address to, uint256 amount)
        ...
        returns (bool)
    {
        TestEvents(tx.origin).raiseTokenTransfer(tx.origin, to, amount);
    ...
    function approve(address spender, uint256 allowance)
        ...
        returns (bool)
    {
        TestEvents(tx.origin).raiseTokenApprove(spender, allowance);
        return true;
    }
