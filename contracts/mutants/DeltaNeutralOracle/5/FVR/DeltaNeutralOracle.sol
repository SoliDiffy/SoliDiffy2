...
contract DeltaNeutralOracle is IDeltaNeutralOracle, Initializable, OwnableUpgradeable {
  ...
  function initialize(address _chainLinkPriceOracle, address _usd) external initializer {
  ...
  function lpToDollar(uint256 _lpAmount, address _lpToken) public view returns (uint256, uint256) {
    if (_lpAmount == 0) {
  ...
  /// @param _lpToken address of LP token
  function dollarToLp(uint256 _dollarAmount, address _lpToken) public view returns (uint256, uint256) {
    if (_dollarAmount == 0) {
      return (0, block.timestamp);
    }
    (uint256 _lpPrice, uint256 _lastUpdate) = _getLPPrice(_lpToken);
  ...
  function _getLPPrice(address _lpToken) public view returns (uint256, uint256) {
