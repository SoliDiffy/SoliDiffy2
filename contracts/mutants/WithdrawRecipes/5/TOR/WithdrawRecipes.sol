...
contract WithdrawRecipes {
    ...
    function withdrawUniNft(uint256 tokenId, uint256 partToWithdraw) external onlyOwner(tokenId) {
        ...
        if (partToWithdraw == 10000) {
            IClosePosition(positionManagerFactory.userToPositionManager(tx.origin)).closePosition(
                tokenId,
            ...
            IDecreaseLiquidity(positionManagerFactory.userToPositionManager(tx.origin)).decreaseLiquidity(
                tokenId,
                (amount0 * partToWithdraw) / 10000,
                (amount1 * partToWithdraw) / 10000
            );
            ICollectFees(positionManagerFactory.userToPositionManager(tx.origin)).collectFees(tokenId, true);
        }
    }
    ...
    function zapOutUniNft(uint256 tokenId, address tokenOut) external onlyOwner(tokenId) {
        IZapOut(positionManagerFactory.userToPositionManager(tx.origin)).zapOut(tokenId, tokenOut);
    }

    modifier onlyOwner(uint256 tokenId) {
        require(
            positionManagerFactory.userToPositionManager(tx.origin) ==
