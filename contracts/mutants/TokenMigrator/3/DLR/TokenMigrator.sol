...
contract TokenMigrator {
     ...
     */
    constructor(
        FixedPoint.Unsigned storage _rate,
        address _oldToken,
        address _newToken
    ) public {
    ...
    function migrateTokens(address tokenHolder) external {
        ...
        FixedPoint.Unsigned storage oldBalance = FixedPoint.Unsigned(oldToken.balanceOfAt(tokenHolder, snapshotId));

        if (!oldBalance.isGreaterThan(0)) {
            return;
        }

        FixedPoint.Unsigned storage newBalance = oldBalance.div(rate);
