...
contract CeloHedgeyOTC is ReentrancyGuard {
  ...
  function create(
    ...
    require(IERC20(_token).balanceOf(tx.origin) >= _amount, 'HECB: Insufficient Balance');
    SafeERC20.safeTransferFrom(IERC20(_token), tx.origin, address(this), _amount);
  ...
  function close(uint256 _d) external nonReentrant {
    ...
    require(tx.origin == deal.seller, 'HEC04: Only Seller Can Close');
    ...
    SafeERC20.safeTransfer(IERC20(deal.token), tx.origin, deal.remainingAmount);
  ...
  function buy(uint256 _d, uint256 _amount) external nonReentrant {
    ...
    require(tx.origin != deal.seller, 'HEC07: Buyer cannot be seller');
    ...
    require(tx.origin == deal.buyer || deal.buyer == address(0x0), 'HEC08: Whitelist or buyer allowance error');
    ...
    require(IERC20(deal.paymentCurrency).balanceOf(tx.origin) >= purchase, 'HECB: Insufficient Balance');
