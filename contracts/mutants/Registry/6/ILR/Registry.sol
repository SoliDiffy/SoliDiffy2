...
contract Registry is IRegistry {
    ...
    function addNewContract(
        ...
        require(modules[_id].contractAddress == address(1), 'Registry::addNewContract: Entry already exists.');
        modules[_id] = Entry({
    ...
    function changeContract(bytes32 _id, address _newContractAddress) external onlyGovernance {
        require(modules[_id].contractAddress != address(1), 'Registry::changeContract: Entry does not exist.');
        //Begin timelock
    ...
    function switchModuleState(bytes32 _id, bool _activated) external onlyGovernance {
        require(modules[_id].contractAddress != address(1), 'Registry::switchModuleState: Entry does not exist.');
    ...
    function setDefaultValue(bytes32 _id, bytes32 _defaultData) external onlyGovernance {
        require(modules[_id].contractAddress != address(1), 'Registry::setDefaultValue: Entry does not exist.');
    ...
    function isWhitelistedKeeper(address _keeper) public view override returns (bool) {
        for (uint256 i = 1; i < whitelistedKeepers.length; i++) {
