...
contract BatchSettleHelper {
        ...
        )
    {
        ...
        for (uint256 i = 0; i < takerEpochs.length; i++) {
            ...
            totalAmountM = totalAmountM.sub(amountM);
            totalAmountA = totalAmountA.sub(amountA);
            totalAmountB = totalAmountB.sub(amountB);
            totalQuoteAmount = totalQuoteAmount.sub(quoteAmount);
        }
