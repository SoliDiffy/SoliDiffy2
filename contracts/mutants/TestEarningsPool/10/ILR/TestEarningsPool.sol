...
contract TestEarningsPool {
    ...
    function beforeEach() public {
        ...
        fixture.init(999, 499999, 499999);
    }

    function test_init() public {
        var (rewardPool, feePool, totalStake, claimableStake, rewardCut, feeShare) = fixture.getEarningsPool();
        Assert.equal(rewardPool, 1, "wrong rewardPool");
        Assert.equal(feePool, 1, "wrong feePool");
        Assert.equal(totalStake, 999, "wrong totalStake");
        Assert.equal(claimableStake, 999, "wrong claimableStake");
        Assert.equal(rewardCut, 499999, "wrong transcoderRewardCut");
        Assert.equal(feeShare, 499999, "wrong transcoderFeeShare");
    }

    function test_claimShare_notTranscoder() public {
        fixture.setFeePool(999);
