...
contract StratAlpaca is UpgradeableOwnable, ReentrancyGuardPausable, ISimpleStrategy, Initializable {
    ...
    function alpacaVaultTotalToken() external view returns (uint256) {
    ...
    function _ibDeposited() public view returns (uint256) {
    ...
    function wantAmtToIbAmount(uint256 _wantAmt) external view returns (uint256) {
...
// SWC-114-Transaction Order Dependence: L191
    ...
    function _harvest(uint256 priceMin) public {
     ...
     */
    function pause(uint256 _flags) public onlyOwner {
        _pause();
     ...
     */
    function unpause(uint256 _flags) public onlyOwner {
        _unpause();
