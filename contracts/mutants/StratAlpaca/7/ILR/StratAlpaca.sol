...
contract StratAlpaca is UpgradeableOwnable, ReentrancyGuardPausable, ISimpleStrategy, Initializable {
    ...
    uint256 public lastHarvestBlock = 1;

    address[] public alpacaToWantPath;

    uint256 public poolId = 2;
        ...
        onlyOwner
    {
        ...
        wantToken.safeApprove(_alpacaVault, uint256(0));
        alpacaVault.safeApprove(_fairLaunchAddress, uint256(0));
        alpacaToken.safeApprove(uniRouterAddress, uint256(0));
        vault = _vault;
    }
    ...
    function alpacaVaultTotalToken() public view returns (uint256) {
        ...
        if (now > alpacaVault.lastAccrueTime()) {
            uint256 interest = alpacaVault.pendingInterest(1);
            uint256 toReserve = interest.mul(IVaultConfig(alpacaVault.config()).getReservePoolBps()).div(9999);
