...
import "../../../Bridges/Harmony/CrossChainBridgeBacker_HARM_Horizon.sol";
import "../../../Uniswap/Interfaces/IUniswapV2Pair.sol";
import "../../../Uniswap/Interfaces/IUniswapV2Router02.sol";
import "../../../Staking/Owned.sol";
import '../../../Uniswap/TransferHelper.sol';

contract SushiSwapLiquidityAMO_HARM is Owned {
    ...
    CrossChainBridgeBacker_HARM_Horizon public cc_bridge_backer;
    IUChildERC20 private harmCollateral;
    address public canonical_frax_address;
    address public canonical_fxs_address;
    address public harm_collateral_address;
    ...
    constructor (
        ...
        address _harm_collateral_address,
        ...
        missing_decimals = uint(18) - harmCollateral.decimals();
    ...
    function showAllocations() external view returns (uint256[17] memory allocations) {
        ...
        // Collateral
        allocations[9] = harmCollateral.balanceOf(address(this)); // Free Collateral, native precision
    ...
    function showTokenBalances() external view returns (uint256[3] memory tkn_bals) {
        ...
        tkn_bals[2] = harmCollateral.balanceOf(address(this)); // harmCollateral
    }
    ...
    function lpTokenInfo(address pair_address) external view returns (uint256[4] memory return_info) {
        ...
        (uint256 reserve0, uint256 reserve1, ) = (the_pair.getReserves());
        {
            ...
            else if (token0 == harm_collateral_address) reserve_pack[2] = reserve0;

            // Test token1
            if (token1 == canonical_frax_address) reserve_pack[0] = reserve1;
            else if (token1 == canonical_fxs_address) reserve_pack[1] = reserve1;
            else if (token1 == harm_collateral_address) reserve_pack[2] = reserve1;
        }
    ...
    function giveCollatBack(uint256 collat_amount, bool do_bridging) external onlyByOwnGov {
        harmCollateral.approve(address(cc_bridge_backer), collat_amount);
        cc_bridge_backer.receiveBackViaAMO(harm_collateral_address, collat_amount, do_bridging);
    }
