...
library VolumeDripManager {
  ...
  function getActiveVolumeDrips(State memory self, address measure) internal view returns (address[] storage) {
  ...
  function set(State memory self, address measure, address dripToken, uint32 periodSeconds, uint112 dripAmount) internal {
  ...
  /// @param measure The measure token
  /// @param dripToken The drip token
  function isActive(State memory self, address measure, address dripToken) internal view returns (bool) {
