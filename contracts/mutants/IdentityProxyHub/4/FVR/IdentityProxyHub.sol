...
contract IdentityProxyHub is UsingRegistry {
   ...
   */
  function getIdentityProxy(bytes32 identifier) external view returns (IdentityProxy) {
   ...
   * @return The identifier's IdentityProxy address.
   */
  function getOrDeployIdentityProxy(bytes32 identifier) external returns (IdentityProxy) {
   ...
   */
  function passesIdentityHeuristic(address addr, bytes32 identifier) external view returns (bool) {
   ...
   */
  function makeCall(bytes32 identifier, address destination, bytes calldata data) public payable {
