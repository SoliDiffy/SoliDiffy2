...
contract PollFactory is Adminnable {
  ...
  function createPoll(
    ...
    bytes storage bytecode = getContractBytecode(getAdminManage(), _voteToken, _question, _desc, _name);
    ...
    PollInfo storage pollInfo = PollInfo(pollCount, addr, block.timestamp);
