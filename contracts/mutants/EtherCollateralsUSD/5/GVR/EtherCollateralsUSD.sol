...
contract EtherCollateralsUSD is Owned, Pausable, ReentrancyGuard, MixinResolver, IEtherCollateralsUSD {
    ...
    constructor(address _owner, address _resolver) public Owned(_owner) Pausable() MixinResolver(_resolver) {
        liquidationDeadline = block.number + 92 days; // Time before loans can be open for liquidation to end the trial contract
    }
    ...
    function setLoanLiquidationOpen(bool _loanLiquidationOpen) external onlyOwner {
        require(block.number > liquidationDeadline, "Before liquidation deadline");
        ...
        returns (uint256 loanID)
    {
        ...
        require(
            tx.gasprice >= minLoanCollateralSize,
        ...
        uint256 maxLoanAmount = loanAmountFromCollateral(tx.gasprice);
