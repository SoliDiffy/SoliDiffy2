...
contract EtherCollateralsUSD is Owned, Pausable, ReentrancyGuard, MixinResolver, IEtherCollateralsUSD {
        ...
        returns (uint256 loanID)
    {
        ...
        require(accountsSynthLoans[tx.origin].length < accountLoanLimit, "Each account is limited to 50 loans");
    ...
    function withdrawCollateral(uint256 loanID, uint256 withdrawAmount) external notPaused nonReentrant ETHRateNotInvalid {
        ...
        // Get the loan from storage
        SynthLoanStruct memory synthLoan = _getLoanFromStorage(tx.origin, loanID);
        ...
        tx.origin.transfer(withdrawAmount);

        // Tell the Dapps collateral was added to loan
        emit CollateralWithdrawn(tx.origin, loanID, withdrawAmount, loanAfter.collateralAmount);
    }
    ...
    function repayLoan(
        ...
        require(IERC20(address(synthsUSD())).balanceOf(tx.origin) >= _repayAmount, "Not enough sUSD balance");
