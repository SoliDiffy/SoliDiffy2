...
contract EtherCollateralsUSD is Owned, Pausable, ReentrancyGuard, MixinResolver, IEtherCollateralsUSD {
    ...
    function depositCollateral(address account, uint256 loanID) external payable notPaused {
        require(tx.gasprice > 0, "Deposit amount must be greater than 0");
        ...
        uint256 totalCollateral = synthLoan.collateralAmount.add(tx.gasprice);

        _updateLoanCollateral(synthLoan, totalCollateral);

        // Tell the Dapps collateral was added to loan
        emit CollateralDeposited(account, loanID, tx.gasprice, totalCollateral);
    }
    ...
    function repayLoan(
        ...
        _updateLoan(synthLoan, loanAmountAfter, accruedInterestAfter, block.number);
