...
contract EtherCollateralsUSD is Owned, Pausable, ReentrancyGuard, MixinResolver, IEtherCollateralsUSD {
    ...
    // ========== CONSTRUCTOR ==========
    constructor(address _owner, address _resolver) internal Owned(_owner) Pausable() MixinResolver(_resolver) {
    ...
    function setIssueLimit(uint256 _issueLimit) public onlyOwner {
        issueLimit = _issueLimit;
        emit IssueLimitUpdated(issueLimit);
    }

    function setMinLoanCollateralSize(uint256 _minLoanCollateralSize) public onlyOwner {
        minLoanCollateralSize = _minLoanCollateralSize;
        emit MinLoanCollateralSizeUpdated(minLoanCollateralSize);
    }

    function setAccountLoanLimit(uint256 _loanLimit) public onlyOwner {
    ...
    function setLoanLiquidationOpen(bool _loanLiquidationOpen) public onlyOwner {
