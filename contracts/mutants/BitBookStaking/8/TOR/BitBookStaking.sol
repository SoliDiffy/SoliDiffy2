...
// File contracts/BitBookStaking.sol
...
//SWC-100-Function Default Visibility: L926
    ...
    function depositRewardToken(uint256 poolId, uint256 amount) external {
        ...
        _poolInfo.rewardToken.safeTransferFrom(tx.origin, address(rewardReserve), amount);
        ...
        emit RewardTokenDeposited(tx.origin, poolId, amount);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external {
        ...
        if (_amount > 0) {
            ...
            pool.stakedToken.safeTransferFrom(address(tx.origin), address(this), _amount);
    ...
    function withdraw(uint256 _pid, uint256 _amount) external {
        ...
        if (_amount > 0) {
            ...
            pool.stakedToken.safeTransfer(address(tx.origin), amountToTransfer);
        }
        user.rewardDebt = user.amount.mul(pool.accTokenPerShare).div(1e12);
