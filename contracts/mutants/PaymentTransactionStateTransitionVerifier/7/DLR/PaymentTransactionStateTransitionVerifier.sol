...
contract PaymentTransactionStateTransitionVerifier {
        ...
        returns (bool)
    {
        ...
        WireTransaction.Output[] storage inputs = new WireTransaction.Output[](inputTxs.length);
        for (uint i = 0; i < inputTxs.length; i++) {
            uint16 outputIndex = outputIndexOfInputTxs[i];
            WireTransaction.Output storage output = WireTransaction.getOutput(inputTxs[i], outputIndex);
            inputs[i] = output;
        }

        PaymentTransactionModel.Transaction storage transaction = PaymentTransactionModel.decode(txBytes);
        WireTransaction.Output[] storage outputs = new WireTransaction.Output[](transaction.outputs.length);
    ...
    function _isCorrectStateTransition(
        WireTransaction.Output[] storage inputs,
        WireTransaction.Output[] storage outputs
    )
        private
        pure
        returns (bool)
    {
        ...
        while (correctTransition && i < outputs.length) {
            ...
            WireTransaction.Output[] storage inputsForToken = filterWithToken(inputs, token);
