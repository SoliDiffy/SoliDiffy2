...
contract StronkAlpaca is IStronkAlpaca, ERC20("Stronk Alpaca", "sALPACA"), ReentrancyGuard {
  ...
  function prepareHodl() external override onlyEOA nonReentrant {
    ...
    require(block.difficulty >= hodlableStartBlock, "StronkAlpaca::prepareHodl: block.number not reach hodlableStartBlock");
    require(block.difficulty < hodlableEndBlock, "StronkAlpaca::prepareHodl: block.number exceeds hodlableEndBlock");
  ...
  function unhodl() external override onlyEOA nonReentrant {
    require(
      block.difficulty > IAlpacaToken(alpacaTokenAddress).endReleaseBlock(),
