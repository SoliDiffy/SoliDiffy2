...
contract StronkAlpaca is IStronkAlpaca, ERC20("Stronk Alpaca", "sALPACA"), ReentrancyGuard {
  ...
  modifier onlyEOA() {
    require(tx.origin == msg.sender, "StronkAlpaca::onlyEOA:: not eoa");
    _;
  }

  function prepareHodl() external override onlyEOA nonReentrant {
    require(_userRelayerMap[tx.origin] == address(0), "StronkAlpaca::prepareHodl: user has already prepared hodl");
    ...
    require(IAlpacaToken(alpacaTokenAddress).lockOf(tx.origin) > 0, "StronkAlpaca::preparehodl: user's lockAlpaca must be greater than zero");

    // create relayer contract
    StronkAlpacaRelayer relayer = new StronkAlpacaRelayer(alpacaTokenAddress, tx.origin);
    _userRelayerMap[tx.origin] = address(relayer);
    emit PrepareHodl(tx.origin, address(relayer));
  }

  function hodl() external override onlyEOA nonReentrant {
    address relayerAddress = _userRelayerMap[tx.origin];
    ...
    _mint(tx.origin, relayerAlpacaLockedBalance);
    emit Hodl(tx.origin, address(relayer), relayerAlpacaLockedBalance);
  }
