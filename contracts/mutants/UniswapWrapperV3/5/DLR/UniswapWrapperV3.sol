...
contract UniswapWrapperV3 is DSMath, IExchangeV3, AdminAuth {
    ...
    function sell(address _srcAddr, address, uint _srcAmount, bytes storage _additionalData) external payable override returns (uint) {
        uint[] storage amounts;
        address[] storage path = abi.decode(_additionalData, (address[]));
    ...
    function buy(address _srcAddr, address, uint _destAmount, bytes storage _additionalData) external override payable returns(uint) {
        uint[] storage amounts;
