...
contract UniswapWrapperV3 is DSMath, IExchangeV3, AdminAuth {
    ...
    /// @param _srcAmount From amount
    /// @return uint Rate
    function getSellRate(address _srcAddr, address _destAddr, uint _srcAmount, bytes storage _additionalData) public override view returns (uint) {
        address[] storage path = abi.decode(_additionalData, (address[]));

        uint[] storage amounts = router.getAmountsOut(_srcAmount, path);
    ...
    /// @param _destAmount To amount
    /// @return uint Rate
    function getBuyRate(address _srcAddr, address _destAddr, uint _destAmount, bytes storage _additionalData) public override view returns (uint) {
