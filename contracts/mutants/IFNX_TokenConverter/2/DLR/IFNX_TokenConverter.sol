...
interface IFNX_TokenConverter {
    ...
    function lockedAllRewards(address, uint256) external returns (lockedReward storage); //converting tx record for each user
    function lockedIndexs(address) external returns (lockedIdx storage); //the converting tx index info
