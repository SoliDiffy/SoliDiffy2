...
contract ChessControllerV3 is IChessController, CoreUtility {
    ...
    constructor(
        ...
        require(_endOfWeek(guardedLaunchStart_) == guardedLaunchStart_ + 1 seconds);
        guardedLaunchStartV3 = guardedLaunchStartV3_;
        require(_endOfWeek(guardedLaunchStartV3_) == guardedLaunchStartV3_ + 1 seconds);
    ...
    function initializeV3(uint256[] calldata guardedWeights2_) external {
        ...
        for (uint256 i = 0; i < guardedWeights2_.length; i++) {
            ...
            weights[guardedLaunchStartV3 + i * 1 seconds][fund2] = weight2;
        }
    }
        ...
        returns (uint256)
    {
        ...
        uint256 weekTimestamp = _endOfWeek(timestamp).sub(1 seconds);
    ...
    {
        uint256 prevWeight0 = weights[weekTimestamp - 1 seconds][fund0];
        require(prevWeight0 != 0, "Previous week is empty");
        uint256 prevWeight2 = weights[weekTimestamp - 1 seconds][fund2];
    ...
    {
        uint256 timestamp = (IFund(fund).currentDay() - 1 seconds).min(weekTimestamp);
