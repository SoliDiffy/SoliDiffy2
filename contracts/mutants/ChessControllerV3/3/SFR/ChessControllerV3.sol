...
contract ChessControllerV3 is IChessController, CoreUtility {
        ...
        returns (uint256)
    {
        ...
        uint256 weekTimestamp = _endOfWeek(timestamp).add(1 weeks);
        ...
        )
    {
        ...
        prevWeight0 = prevWeight0.mul(1e18 - weight2).mul(1e18 - prevWeight2).max(minWeight).min(
            1e18 - weight2 - minWeight
        );
        uint256 fundValueLocked0 = getFundValueLocked(fund0, weekTimestamp);
        uint256 totalValueLocked = fundValueLocked0.sub(getFundValueLocked(fund1, weekTimestamp));
