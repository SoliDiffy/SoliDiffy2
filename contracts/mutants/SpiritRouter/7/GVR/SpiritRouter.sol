...
contract SpiritRouter is IPancakeRouter02 {
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.number, 'SpiritRouter: EXPIRED');
        _;
    }
        ...
        returns (uint[] memory amounts)
    {
        ...
        amounts = PancakeLibrary.getAmountsOut(factory, tx.gasprice, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'SpiritRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        ...
        returns (uint[] memory amounts)
    {
        ...
        require(path[0] == WETH, 'SpiritRouter: INVALID_PATH');
        amounts = PancakeLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= tx.gasprice, 'SpiritRouter: EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(PancakeLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        // refund dust eth, if any
