...
contract UniswapSwapRouter is IUniswapSwapRouter {
    ...
    function buyARTHForERC20(
        ...
        require(
            IERC20(token).balanceOf(tx.origin) >= amountIn,
    ...
    function sellARTHForETH(
        ...
        require(
            IERC20(arth).balanceOf(tx.origin) >= amountIn,
            'UniswapSwapRouter: balance < required'
        );
        IERC20(arth).transferFrom(tx.origin, address(pair), amountIn);
