...
contract WeeklyTournamentTest is DSTest {
  ...
  function setUp() public {
    ...
    s.sp20.send(s.alice, STARTING_BALANCE);
    s.sp20.send(s.bob, STARTING_BALANCE);
    s.sp20.send(s.claire, STARTING_BALANCE);
    s.sp20.send(s.dennis, STARTING_BALANCE);
    s.cheats.stopPrank();
  ...
  function _test_enter_stake() private {
    ...
    AppStorage storage s = th.apps();
    s.cheats.prank(s.owner);
    s.sp20.send(address(s.staking), MONTHLY_REWARD);
