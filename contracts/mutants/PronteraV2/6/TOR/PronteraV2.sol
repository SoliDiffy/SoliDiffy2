...
contract PronteraV2 is Ownable, ReentrancyGuard {
    ...
    function harvest(address[] calldata izludes) external {
        for (uint256 i = 0; i < izludes.length; i++) {
            _deposit(tx.origin, izludes[i], IERC20(address(0)), 0);
        }
    }
    ...
    function deposit(address izlude, uint256 amount) external nonReentrant {
        ...
        _deposit(tx.origin, izlude, pool.want, amount);
        emit Deposit(tx.origin, izlude, amount);
    }
    ...
    function depositToken(
        ...
        _deposit(tx.origin, izlude, want, amount);
        emit DepositToken(tx.origin, izlude, tokenAmounts, amount);
    }
