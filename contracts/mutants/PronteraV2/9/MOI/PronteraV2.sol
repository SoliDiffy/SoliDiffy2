...
abstract contract Ownable is Context {
    ...
    function _setOwner(address newOwner) private onlyOwner {
...
contract PronteraV2 is Ownable, ReentrancyGuard {
    ...
    function _addTraversal(address izlude) private nonReentrant {
    ...
    function removeTraversal(uint256 index) external nonReentrant {
    ...
    function updatePool(address izlude) public nonReentrant {
        PoolInfo storage pool = poolInfo[izlude];
    ...
    ) private nonReentrant returns (uint256 amount) {
        PoolInfo storage pool = poolInfo[izlude];
        UserInfo storage user = userInfo[izlude][_user];
    ...
    function emergencyWithdraw(address izlude) external nonReentrant {
        PoolInfo storage pool = poolInfo[izlude];
