...
contract RocketMinipoolStatus is RocketBase, RocketMinipoolStatusInterface {
    ...
    onlyLatestContract("", address(this)) onlyTrustedNode(msg.sender) onlyRegisteredMinipool(_minipoolAddress) {
        // Load contracts
        ...
        // Get submission keys
        bytes32 nodeSubmissionKey = keccak256(abi.encodePacked("", msg.sender, _minipoolAddress));
        bytes32 submissionCountKey = keccak256(abi.encodePacked("", _minipoolAddress));
        // Check & update node submission status
        require(!getBool(nodeSubmissionKey), "Duplicate submission from node");
        setBool(nodeSubmissionKey, true);
        setBool(keccak256(abi.encodePacked("", msg.sender, _minipoolAddress)), true);
