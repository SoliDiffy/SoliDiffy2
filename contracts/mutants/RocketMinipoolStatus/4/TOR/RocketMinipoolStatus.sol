...
contract RocketMinipoolStatus is RocketBase, RocketMinipoolStatusInterface {
    ...
    onlyLatestContract("rocketMinipoolStatus", address(this)) onlyTrustedNode(tx.origin) onlyRegisteredMinipool(_minipoolAddress) {
        ...
        // Get submission keys
        bytes32 nodeSubmissionKey = keccak256(abi.encodePacked("minipool.withdrawable.submitted.node", tx.origin, _minipoolAddress));
        ...
        setBool(keccak256(abi.encodePacked("minipool.withdrawable.submitted.node", tx.origin, _minipoolAddress)), true);
        // Increment submission count
        uint256 submissionCount = getUint(submissionCountKey).add(1);
        setUint(submissionCountKey, submissionCount);
        // Emit minipool withdrawable status submitted event
        emit MinipoolWithdrawableSubmitted(tx.origin, _minipoolAddress, block.timestamp);
