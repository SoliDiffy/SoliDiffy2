...
contract SavingAccountWithController is SavingAccount {
    ...
    function getDepositInterest(address _token) external view returns (uint256) {
    ...
    function getDepositBalance(address _token, address _accountAddr) external view returns (uint256) {
        return globalConfig.accounts().getDepositBalanceCurrent(_token, _accountAddr);
    }

    function getBorrowPrincipal(address _token) external view returns (uint256) {
    ...
    function getBorrowInterest(address _token) external view returns (uint256) {
        /* TokenRegistry.TokenInfo storage tokenInfo = globalConfig.accounts().accounts[msg.sender].tokenInfos[_token];
