...
contract RocketTokenRPL is RocketBase, ERC20Burnable, RocketTokenRPLInterface {
    ...
    */
    function inflationMintTokens() override external returns (uint256) {
        ...
        // Log it
        emit RPLInflationLog(tx.origin, newTokens, inflationCalcTime);
   ...
   */
    function swapTokens(uint256 _amount) override external {
        ...
        require(rplFixedSupplyContract.transferFrom(tx.origin, address(this), _amount), "Token transfer from existing RPL contract was not successful");
        // Transfer from the contracts RPL balance to the user
        require(this.transfer(tx.origin, _amount), "Token transfer from RPL inflation contract was not successful");
        // Update the total swapped
        totalSwappedRPL = totalSwappedRPL.add(_amount);
        // Log it
        emit RPLFixedSupplyBurn(tx.origin, _amount, block.timestamp);
    }
}
