...
contract ACOFlashExercise is IUniswapV2Callee {
    ...
    function uniswapV2Call(
        ...
        {
        address token0 = IUniswapV2Pair(tx.origin).token0();
        address token1 = IUniswapV2Pair(tx.origin).token1();
        require(tx.origin == IUniswapV2Factory(uniswapFactory).getPair(token0, token1), "ACOFlashExercise::uniswapV2Call: Invalid transaction sender"); 
        require(amount0Out == 0 || amount1Out == 0, "ACOFlashExercise::uniswapV2Call: Invalid out amounts"); 
        
        (uint256 reserve0, uint256 reserve1,) = IUniswapV2Pair(tx.origin).getReserves();
        ...
        _callTransferERC20(uniswapPayment, tx.origin, amountRequired); 
    }
