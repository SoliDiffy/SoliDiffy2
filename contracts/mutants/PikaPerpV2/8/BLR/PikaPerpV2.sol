...
contract PikaPerpV2 is ReentrancyGuard {
    ...
    bool canUserStake = true;
    bool allowPublicLiquidator = true;
    Vault private vault;
    ...
    function openPosition(
        ...
        _updateOpenInterest(productId, amount, isLong, false);
    ...
    function closePositionWithId(
        ...
        _updateOpenInterest(uint256(position.productId), margin.mul(uint256(position.leverage)).div(BASE), position.isLong, true);
    ...
    function releaseMargin(uint256 positionId) external onlyOwner {
        ...
        _updateOpenInterest(uint256(position.productId), amount, position.isLong, true);
