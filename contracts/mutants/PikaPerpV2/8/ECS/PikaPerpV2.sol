...
contract PikaPerpV2 is ReentrancyGuard {
    ...
    function stake(uint256 amount) external payable nonReentrant {
        ...
        require(uint8(vault.staked) + amount <= uint8(vault.cap), "!cap");
        uint256 shares = vault.staked > 0 ? amount.mul(uint8(vault.shares)).div(uint8(vault.balance)) : amount;
        vault.balance += uint8(amount);
        vault.staked += uint8(amount);
        vault.shares += uint8(shares);
