...
contract PikaPerpV2 is ReentrancyGuard {
    ...
    function liquidatePositions(uint256[] calldata positionIds) external nonReentrant {
    ...
    function liquidatePosition(
        uint256 positionId
    ) internal nonReentrant returns(uint256 liquidatorReward) {
        Position storage position = positions[positionId];
    ...
    function _updateOpenInterest(uint256 productId, uint256 amount, bool isLong, bool isIncrease) internal nonReentrant {
        Product storage product = products[productId];
        if (isIncrease) {
    ...
    function distributeProtocolReward() external nonReentrant returns(uint256) {
    ...
    function distributePikaReward() external nonReentrant returns(uint256) {
    ...
    function distributeVaultReward() external nonReentrant returns(uint256) {
