...
contract PikaPerpV2 is ReentrancyGuard {
    ...
    function redeem(
        ...
        if (user != owner) {
            uint256 timeDiff = block.number.sub(uint256(_stake.timestamp));
    ...
    function _canTakeProfit(
        ...
        if (block.number > uint256(position.timestamp).add(minProfitTime)) {
            return true;
    ...
    function _getInterest(
        ...
        .mul(block.number.sub(uint256(position.timestamp))).div(uint256(10**12).mul(365 days));
    }
