...
contract VestingVault is IVotingVault {
    ...
    function claim() public {
        // load the grant
        VestingVaultStorage.Grant storage grant = _grants()[tx.origin];
        // get the withdrawable amount
        uint256 withdrawable = _getWithdrawableAmount(grant);

        // transfer the available amount
        token.transfer(tx.origin, withdrawable);
        grant.withdrawn += uint128(withdrawable);

        // update the user's voting power
        _syncVotingPower(tx.origin, grant);
    }
    ...
    function delegate(address _to) public {
        ...
        emit VoteChange(_to, tx.origin, int256(newVotingPower));
