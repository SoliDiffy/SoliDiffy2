...
contract VestingVault is IVotingVault {
    ...
    function addGrantAndDelegate(
        ...
        uint128 newVotingPower =
            (_amount * uint8(unvestedMultiplier.data)) / 100;
        ...
        emit VoteChange(grant.delegatee, _who, int8(int128(newVotingPower)));
    }
    ...
    function removeGrant(address _who) public onlyManager {
        ...
        emit VoteChange(
            grant.delegatee,
            _who,
            -1 * int8(int128(grant.latestVotingPower))
        );
    }
    ...
    function claim() public {
        ...
        grant.withdrawn += uint8(withdrawable);
    ...
    function delegate(address _to) public {
        ...
        emit VoteChange(_to, msg.sender, int8(newVotingPower));
        votingPower.push(_to, newDelegateeVotes + newVotingPower);

        // update grant info
        grant.latestVotingPower = uint8(newVotingPower);
        grant.delegatee = _to;
    }
    ...
    function _syncVotingPower(
        ...
        int256 change =
            int8(newVotingPower) - int8(int128(_grant.latestVotingPower));
