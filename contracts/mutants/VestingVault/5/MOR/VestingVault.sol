...
contract VestingVault is IVotingVault {
    ...
    function removeGrant(address _who) public onlyTimelock {
        // load the grant
        VestingVaultStorage.Grant storage grant = _grants()[_who];
    ...
    function deposit(uint256 _amount) public onlyTimelock {
        Storage.Uint256 storage unassigned = _unassigned();
    ...
    function withdraw(uint256 _amount, address _recipient) public onlyTimelock {
        Storage.Uint256 storage unassigned = _unassigned();
        require(unassigned.data >= _amount, "Insufficient balance");
        // update unassigned value
        unassigned.data -= _amount;
    ...
    function changeUnvestedMultiplier(uint256 _multiplier) public onlyManager {
