...
library BokkyPooBahsRedBlackTreeLibrary {
    ...
    function first(Tree memory self) internal view returns (uint _key) {
        _key = self.root;
        if (_key != EMPTY) {
    ...
    function last(Tree memory self) internal view returns (uint _key) {
        _key = self.root;
        if (_key != EMPTY) {
    ...
    function next(Tree memory self, uint target) internal view returns (uint cursor) {
    ...
    function prev(Tree memory self, uint target) internal view returns (uint cursor) {
        require(target != EMPTY);
    ...
    function exists(Tree memory self, uint key) internal view returns (bool) {
    ...
    function getNode(Tree memory self, uint key) internal view returns (uint _returnKey, uint _parent, uint _left, uint _right, bool _red) {
        require(exists(self, key));
        return(key, self.nodes[key].parent, self.nodes[key].left, self.nodes[key].right, self.nodes[key].red);
    }

    function insert(Tree memory self, uint key) internal {
        require(key != EMPTY);
        require(!exists(self, key));
        uint cursor = EMPTY;
    ...
    function remove(Tree memory self, uint key) internal {
        require(key != EMPTY);
        require(exists(self, key));
        uint probe;
        uint cursor;
