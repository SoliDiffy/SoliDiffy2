...
contract XVS is Tokenlock {
     ...
     */
    function approve(address spender, uint rawAmount) public validLock returns (bool) {
        uint96 amount;
        if (rawAmount == uint(-1)) {
            amount = uint96(-1);
        } else {
     ...
     */
    function transfer(address dst, uint rawAmount) public validLock returns (bool) {
     ...
     * @return Whether or not the transfer succeeded
     */
    function transferFrom(address src, address dst, uint rawAmount) public validLock returns (bool) {
