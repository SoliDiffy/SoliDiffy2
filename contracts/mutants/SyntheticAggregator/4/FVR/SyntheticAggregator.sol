...
contract SyntheticAggregator is SyntheticAggregatorErrors, LibDerivative, LibCommission, ReentrancyGuard {
    ...
    function getAuthorCommission(bytes32 _derivativeHash, Derivative memory _derivative) external nonReentrant returns (uint256 commission) {
    ...
    function getAuthorAddress(bytes32 _derivativeHash, Derivative memory _derivative) external nonReentrant returns (address authorAddress) {
        // Initialize derivative if wasn't initialized before
        _initDerivative(_derivativeHash, _derivative);
    ...
    function getMargin(bytes32 _derivativeHash, Derivative memory _derivative) external nonReentrant returns (uint256 buyerMargin, uint256 sellerMargin) {
    ...
    function isPool(bytes32 _derivativeHash, Derivative memory _derivative) external nonReentrant returns (bool result) {
