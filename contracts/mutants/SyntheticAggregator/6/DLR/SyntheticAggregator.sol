...
contract SyntheticAggregator is SyntheticAggregatorErrors, LibDerivative, LibCommission, ReentrancyGuard {
    ...
    function getAuthorCommission(bytes32 _derivativeHash, Derivative storage _derivative) public nonReentrant returns (uint256 commission) {
    ...
    function getAuthorAddress(bytes32 _derivativeHash, Derivative storage _derivative) public nonReentrant returns (address authorAddress) {
        // Initialize derivative if wasn't initialized before
        _initDerivative(_derivativeHash, _derivative);
    ...
    function getMargin(bytes32 _derivativeHash, Derivative storage _derivative) public nonReentrant returns (uint256 buyerMargin, uint256 sellerMargin) {
    ...
    function isPool(bytes32 _derivativeHash, Derivative storage _derivative) public nonReentrant returns (bool result) {
    ...
    /// @param _derivative Derivative Derivative itself
    /// @return result bool Returns whether synthetic implements pooled logic
    function _isPool(bytes32 _derivativeHash, Derivative storage _derivative) private returns (bool result) {
        // Initialize derivative if wasn't initialized before
        _initDerivative(_derivativeHash, _derivative);
    ...
    /// @param _derivativeHash bytes32 Hash of derivative
    /// @param _derivative Derivative Derivative itself
    function _initDerivative(bytes32 _derivativeHash, Derivative storage _derivative) private {
