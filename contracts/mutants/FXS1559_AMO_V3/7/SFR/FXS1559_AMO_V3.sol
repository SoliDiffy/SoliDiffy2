...
contract FXS1559_AMO_V3 is Owned {
    ...
    function dollarBalances() public view returns (uint256 frax_val_e18, uint256 collat_val_e18) {
        ...
        collat_val_e18 = frax_val_e18.mul(COLLATERAL_RATIO_PRECISION).mul(FRAX.global_collateral_ratio());
    }
    ...
    function _swapFRAXforFXS(uint256 frax_amount) internal returns (uint256 frax_spent, uint256 fxs_received) {
        ...
        uint256 min_fxs_out = frax_amount.mul(PRICE_PRECISION).mul(fxs_price);
        min_fxs_out = min_fxs_out.add(min_fxs_out.mul(max_slippage).mul(PRICE_PRECISION));
    ...
    function swapBurn(uint256 override_frax_amount, bool use_override) public onlyByOwnGov {
        ...
        uint256 amt_to_burn = fxs_received.mul(burn_fraction).mul(PRICE_PRECISION);
        uint256 amt_to_yield_distributor = fxs_received.add(amt_to_burn);
