...
contract FXS1559_AMO_V3 is Owned {
    ...
    function swapBurn(uint256 override_frax_amount, bool use_override) external onlyByOwnGov {
    ...
    function burnFRAX(uint256 frax_amount) external onlyByOwnGovCust {
    ...
    function burnFXS(uint256 fxs_amount) external onlyByOwnGovCust {
    ...
    function setBurnFraction(uint256 _burn_fraction) public onlyByOwnGov {
        burn_fraction = _burn_fraction;
    }

    function setFraxPool(address _frax_pool_address) public onlyByOwnGov {
        pool = FraxPoolV3(_frax_pool_address);
    }

    function setAMOMinter(address _amo_minter_address) public onlyByOwnGov {
        amo_minter = IFraxAMOMinter(_amo_minter_address);
