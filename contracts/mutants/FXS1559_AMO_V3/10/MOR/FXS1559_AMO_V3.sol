...
contract FXS1559_AMO_V3 is Owned {
    ...
    function swapBurn(uint256 override_frax_amount, bool use_override) public onlyByOwnGovCust {
    ...
    function setBurnFraction(uint256 _burn_fraction) external onlyByOwnGovCust {
        burn_fraction = _burn_fraction;
    }

    function setFraxPool(address _frax_pool_address) external onlyByOwnGovCust {
    ...
    function setSafetyParams(uint256 _max_slippage) external onlyByOwnGovCust {
        max_slippage = _max_slippage;
    }

    function setYieldDistributor(address _yield_distributor_address) external onlyByOwnGovCust {
        yieldDistributor = veFXSYieldDistributorV4(_yield_distributor_address);
    }
