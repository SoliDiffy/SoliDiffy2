...
contract CakeMaxiWorker is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IWorker02 {
  ...
  function initialize(
    ...
    require(path.length >= 1, "CakeMaxiWorker::initialize:: path length must be >= 2");
    require(
    ...
    require(rewardPath.length >= 1, "CakeMaxiWorker::initialize:: rewardPath length must be >= 2");
    require(
