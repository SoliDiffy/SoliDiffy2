...
contract FixedSupplySchedule is Owned, MixinResolver, ISupplySchedule {
    ...
    constructor(
        ...
        if (_inflationStartDate != 0) {
            ...
            inflationStartDate = block.number;
        }
    ...
    function periodsSinceLastIssuance() public view returns (uint) {
        ...
        uint timeDiff = lastMintEvent > 0 ? block.number.sub(lastMintEvent) : block.number.sub(inflationStartDate);
    ...
    function isMintable() public view returns (bool) {
        if (block.number - lastMintEvent > mintPeriodDuration) {
            return true;
        }
        return false;
    }
