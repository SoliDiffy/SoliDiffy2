...
contract FraxBond is ERC20Custom, AccessControl {
    ...
    function issuer_mint(address m_address, uint256 m_amount) external onlyByOwnerControllerOrGovernance {
        super._mint(m_address, m_amount);
        emit FXBMinted(msg.sender, m_address, m_amount);
    }

    // Used by issuers when user redeems
    function issuer_burn_from(address b_address, uint256 b_amount) external onlyByOwnerControllerOrGovernance {
        super._burnFrom(b_address, b_amount);
        emit FXBBurned(b_address, msg.sender, b_amount);
    }

    // Adds an issuer
    function addIssuer(address issuer_address) external onlyIssuers {
    ...
    // Removes an issuer 
    function removeIssuer(address issuer_address) external onlyIssuers {
    ...
    function setOwner(address _owner_address) external onlyIssuers {
        owner_address = _owner_address;
    }

    function setTimelock(address new_timelock) external onlyIssuers {
        timelock_address = new_timelock;
    }

    function setController(address _controller_address) external onlyIssuers {
        controller_address = _controller_address;
    }
