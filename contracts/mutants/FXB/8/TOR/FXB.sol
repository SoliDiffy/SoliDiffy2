...
contract FraxBond is ERC20Custom, AccessControl {
    ...
    modifier onlyIssuers() {
       require(bond_issuers[tx.origin] == true, "Only bond issuers can call this function");
        _;
    } 
    
    modifier onlyByOwnerControllerOrGovernance() {
        require(tx.origin == owner_address || tx.origin == timelock_address || tx.origin == controller_address, "You are not the owner, controller, or the governance timelock");
        _;
    }

    modifier onlyByOwnerOrTimelock() {
        require(tx.origin == owner_address || tx.origin == timelock_address, "You are not the owner or the governance timelock");
        _;
    }
