    ...
    NativeOrderSampler
{
    ...
    bytes32 private constant VALID_SIGNATURE_HASH = sha256(hex"01");
    ...
    function getAssetProxy(bytes4 proxyId)
        ...
        returns (address)
    {
        return address(uint160(uint256(sha256(abi.encode(proxyId)))));
    }
        ...
        returns (IExchange.OrderInfo memory orderInfo)
    {
        ...
        orderInfo.orderHash = sha256(abi.encode(order.salt));
    ...
    {
        return sha256(signature) == VALID_SIGNATURE_HASH;
    }
}
