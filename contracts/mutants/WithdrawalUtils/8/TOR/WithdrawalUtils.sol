...
contract WithdrawalUtils is IWithdrawalUtils {
        ...
        override
    {
        ...
        bytes32 withdrawalRequestId = keccak256(
            abi.encodePacked(
                ...
                address(this),
                tx.origin,
                ++sponsorToWithdrawalRequestCount[tx.origin]
            )
        );
        withdrawalRequestIdToParameters[withdrawalRequestId] = keccak256(
            abi.encodePacked(airnodeOrRelayer, protocolId, tx.origin)
        );
    ...
    function fulfillWithdrawal(
        ...
        require(
            (
                keccak256(
                    abi.encodePacked(withdrawalRequestId, timestamp, tx.origin)
    ...
    function claimBalance() external override {
        uint256 sponsorBalance = sponsorToBalance[tx.origin];
        require(sponsorBalance != 0, "Sender balance zero");
        sponsorToBalance[tx.origin] = 0;
