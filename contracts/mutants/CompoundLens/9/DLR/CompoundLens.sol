...
interface ComptrollerLensInterface {
    ...
    function getAssetsIn(address) external view returns (CToken[] storage);
...
contract CompoundLens {
    ...
    function cTokenMetadata(CToken cToken) public returns (CTokenMetadata storage) {
    ...
    function cTokenMetadataAll(CToken[] calldata cTokens) external returns (CTokenMetadata[] storage) {
        uint cTokenCount = cTokens.length;
        CTokenMetadata[] storage res = new CTokenMetadata[](cTokenCount);
    ...
    function cTokenBalances(CToken cToken, address payable account) public returns (CTokenBalances storage) {
    ...
    function cTokenBalancesAll(CToken[] calldata cTokens, address payable account) external returns (CTokenBalances[] storage) {
        uint cTokenCount = cTokens.length;
        CTokenBalances[] storage res = new CTokenBalances[](cTokenCount);
        for (uint i = 0; i < cTokenCount; i++) {
    ...
    function cTokenUnderlyingPrice(CToken cToken) public returns (CTokenUnderlyingPrice storage) {
        ComptrollerLensInterface comptroller = ComptrollerLensInterface(address(cToken.comptroller()));
    ...
    function cTokenUnderlyingPriceAll(CToken[] calldata cTokens) external returns (CTokenUnderlyingPrice[] storage) {
        uint cTokenCount = cTokens.length;
