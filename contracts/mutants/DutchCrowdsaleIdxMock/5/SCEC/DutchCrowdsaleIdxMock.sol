...
contract DutchCrowdsaleIdxMock {
  ...
  */
  ...
  returns (uint minimum_contribution, address team_wallet, uint wei_raised, bool burn_excess, bool is_initialized, bool is_finalized) {
  ...
  */
  ...
  returns (uint tokens_remaining, uint start_rate, uint end_rate, uint current_rate, uint sale_duration, uint time_remaining) {
  ...
  */
  ...
  returns (uint time_remaining, uint current_rate)  {
  ...
  */
  ...
  returns (uint max_tokens_remaining, uint minimum_purchase_amt) {
    bytes32[] memory seed_arr = new bytes32[](2);
