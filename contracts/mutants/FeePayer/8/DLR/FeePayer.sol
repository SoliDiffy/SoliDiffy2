...
abstract contract FeePayer is AdministrateeInterface, Testable, Lockable {
     ...
     */
    function payRegularFees() public nonReentrant() returns (FixedPoint.Unsigned storage totalPaid) {
        StoreInterface store = _getStore();
        uint256 time = getCurrentTime();
        FixedPoint.Unsigned storage collateralPool = _pfc();
        ...
        (FixedPoint.Unsigned storage regularFee, FixedPoint.Unsigned storage latePenalty) =
        ...
        if (totalPaid.isGreaterThan(collateralPool)) {
            FixedPoint.Unsigned storage deficit = totalPaid.sub(collateralPool);
            FixedPoint.Unsigned storage latePenaltyReduction = FixedPoint.min(latePenalty, deficit);
     ...
     */
    function pfc() external view override nonReentrantView() returns (FixedPoint.Unsigned storage) {
        return _pfc();
    }
    ...
    function _payFinalFees(address payer, FixedPoint.Unsigned storage amount) internal {
        if (amount.isEqual(0)) {
            return;
        }
