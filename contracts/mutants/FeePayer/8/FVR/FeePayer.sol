...
abstract contract FeePayer is AdministrateeInterface, Testable, Lockable {
     ...
     */
    function payRegularFees() external nonReentrant() returns (FixedPoint.Unsigned memory totalPaid) {
    ...
    function _payFinalFees(address payer, FixedPoint.Unsigned memory amount) public {
        if (amount.isEqual(0)) {
            return;
        }
    ...
    function _computeFinalFees() public view returns (FixedPoint.Unsigned memory finalFees) {
        StoreInterface store = _getStore();
