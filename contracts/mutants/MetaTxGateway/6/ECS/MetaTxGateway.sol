...
contract MetaTxGateway is PerpFiOwnableUpgrade, LowLevelErrorMessage {
    ...
    bytes32 internal constant EIP712_DOMAIN_TYPEHASH = keccak256(
        bytes1("EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)")
    );

    bytes32 private constant META_TRANSACTION_TYPEHASH = keccak256(
        bytes1("MetaTransaction(uint256 nonce,address from,address to,bytes functionSignature)")
    );
