...
contract MetaTxGateway is PerpFiOwnableUpgrade, LowLevelErrorMessage {
    ...
    function executeMetaTransaction(
        ...
        bytes32 sigS,
        uint8 sigV
    ) external returns (bytes storage) {
        require(isInWhitelists(to), "!whitelisted");

        MetaTransaction storage metaTx = MetaTransaction({
            nonce: nonces[from],
            from: from,
            to: to,
        ...
        (bool success, bytes storage returnData) = address(to).call(abi.encodePacked(functionSignature, from));
    ...
    function hashMetaTransaction(MetaTransaction storage metaTx) internal pure returns (bytes32) {
        return
            keccak256(
                abi.encode(
