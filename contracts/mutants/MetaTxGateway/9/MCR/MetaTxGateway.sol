...
contract MetaTxGateway is PerpFiOwnableUpgrade, LowLevelErrorMessage {
    ...
    // Constant
    //
    bytes32 internal constant EIP712_DOMAIN_TYPEHASH = sha256(
        bytes("EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)")
    );

    bytes32 private constant META_TRANSACTION_TYPEHASH = sha256(
    ...
    function initialize(
        ...
        domainSeperatorL1 = sha256(
            abi.encode(
                EIP712_DOMAIN_TYPEHASH,
                sha256(bytes(_name)),
                sha256(bytes(_version)),
                _chainIdL1,
                address(this)
            )
        );

        domainSeperatorL2 = sha256(
            abi.encode(
                EIP712_DOMAIN_TYPEHASH,
                sha256(bytes(_name)),
                sha256(bytes(_version)),
    ...
    function toTypedMessageHash(bytes32 domainSeperator, bytes32 messageHash) internal pure returns (bytes32) {
        return sha256(abi.encodePacked("\x19\x01", domainSeperator, messageHash));
    }
