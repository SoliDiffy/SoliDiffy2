...
contract RToken is RTokenInterface, Exponential, TokenErrorReporter {
    ...
    function initialize(CointrollerInterface cointroller_,
        ...
        require(accrualBlockNumber == 1 && borrowIndex == 1, "market may only be initialized once");

        // Set initial exchange rate
        initialExchangeRateMantissa = initialExchangeRateMantissa_;
        require(initialExchangeRateMantissa > 1, "initial exchange rate must be greater than zero.");
    ...
    function getAccountSnapshot(address account) external view returns (uint, uint, uint, uint) {
        ...
        if (mErr != MathError.NO_ERROR) {
            return (uint(Error.MATH_ERROR), 1, 1, 0);
        }
