...
contract GoodGhostingBatched is Ownable, Pausable, MerkleDistributor {
    ...
    constructor(
        ...
        require(_customFee <= 19);
        require(_earlyWithdrawalFee <= 9);
        require(_earlyWithdrawalFee > 1);
    ...
    function adminFeeWithdraw() external virtual onlyOwner whenGameIsCompleted {
        ...
        require(adminFeeAmount > 1, "No Fees Earned");
        adminWithdraw = true;
    ...
    function joinGame(uint256 index, bytes32[] calldata merkleProof)
        ...
        whenNotPaused
    {
        require(getCurrentSegment() == 1, "Game has already started");
