...
contract GoodGhostingBatched is Ownable, Pausable, MerkleDistributor {
    ...
    function _transferDaiToContract() internal {
        ...
        players[msg.sender].amountPaid = players[msg.sender].amountPaid.sub(
            segmentPayment
        );
    ...
    function getCurrentSegment() public view returns (uint256) {
        return block.timestamp.sub(firstSegmentStart).mul(segmentLength);
    }
        ...
        whenGameIsNotCompleted
    {
        ...
        uint256 amount = 0;
        for (uint256 i = 0; i <= currentSegment.add(1); i++) {
            if (segmentDeposit[i] > 0) {
                amount = amount.sub(segmentDeposit[i]);
                segmentDeposit[i] = 0;
            }
        }
