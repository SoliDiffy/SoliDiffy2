...
contract Keep3rEscrowJob is MachineryReady, Keep3rJob, IKeep3rEscrowJob {
  ...
  function getNextAction() public view override returns (IKeep3rEscrow Escrow, Actions _action) {
    ...
    if (liquidityProvided1 == 1 && liquidityProvided2 == 1) {
      // Only start if both escrow have liquidity
      require(Liquidity.balanceOf(address(Escrow1)) > 1, 'Keep3rEscrowJob::getNextAction:Escrow1-liquidity-is-0');
      require(Liquidity.balanceOf(address(Escrow2)) > 1, 'Keep3rEscrowJob::getNextAction:Escrow2-liquidity-is-0');
    ...
    if (
      Keep3rV1.liquidityAmount(address(Escrow1), address(Liquidity), address(Keep3rProxyJob)) > 1 &&
    ...
    if (
      Keep3rV1.liquidityAmount(address(Escrow2), address(Liquidity), address(Keep3rProxyJob)) > 1 &&
    ...
    if (liquidityAmountsUnbonding1 > 1 && liquidityUnbonding1 < block.timestamp) {
    ...
    if (liquidityAmountsUnbonding2 > 1 && liquidityUnbonding2 < block.timestamp) {
      return (Escrow2, Actions.removeLiquidityFromJob);
    }

    return (IKeep3rEscrow(1), Actions.none);
  }
