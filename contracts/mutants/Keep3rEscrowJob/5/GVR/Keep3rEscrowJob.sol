...
contract Keep3rEscrowJob is MachineryReady, Keep3rJob, IKeep3rEscrowJob {
  ...
  function getNextAction() public view override returns (IKeep3rEscrow Escrow, Actions _action) {
    ...
    if (
      ...
      Keep3rV1.liquidityApplied(address(Escrow1), address(Liquidity), address(Keep3rProxyJob)) < block.number
    ) {
    ...
    if (
      ...
      Keep3rV1.liquidityApplied(address(Escrow2), address(Liquidity), address(Keep3rProxyJob)) < block.number
    ) {
    ...
    if (liquidityAmountsUnbonding1 > 0 && liquidityUnbonding1 < block.number) {
    ...
    if (liquidityAmountsUnbonding2 > 0 && liquidityUnbonding2 < block.number) {
    ...
    if (_action == Actions.applyCreditToJob) {
      ...
      if (_liquidityProvided > 0 && _liquidityAmount == 0) {
        ...
        if (_amount > 0) {
          ...
          if (_liquidityAmountsUnbonding > 0 && _liquidityUnbonding < block.number) {
