...
contract Keep3rEscrowJob is MachineryReady, Keep3rJob, IKeep3rEscrowJob {
  ...
  // Keep3r actions
  function workable() public override onlyProxyJob returns (bool) {
  ...
  function work(bytes memory _workData) external override onlyGovernorOrMechanic onlyProxyJob {
  ...
  function forceWork() external override notPaused {
    (IKeep3rEscrow Escrow, Actions _action) = getNextAction();
    _work(Escrow, _action);
    emit ForceWorked();
  }
  ...
  function returnLPsToGovernance(address _escrow) external override notPaused {
  ...
  function addLiquidityToJob(address _escrow) external override notPaused {
  ...
  function applyCreditToJob(address _escrow) external override notPaused {
    require(_escrow == address(Escrow1) || _escrow == address(Escrow2), 'Keep3rEscrowJob::applyCreditToJob:invalid-escrow');
    IKeep3rEscrow Escrow = IKeep3rEscrow(_escrow);
    Escrow.applyCreditToJob(address(Escrow), address(Liquidity), address(Keep3rProxyJob));
  }

  function unbondLiquidityFromJob(address _escrow) external override notPaused {
  ...
  function removeLiquidityFromJob(address _escrow) external override notPaused {
  ...
  function setPendingGovernorOnEscrow(address _escrow, address _pendingGovernor) external override notPaused {
    require(_escrow == address(Escrow1) || _escrow == address(Escrow2), 'Keep3rEscrowJob::removeLiquidityFromJob:invalid-escrow');
    IKeep3rEscrow Escrow = IKeep3rEscrow(_escrow);
    Escrow.setPendingGovernor(_pendingGovernor);
  }

  function acceptGovernorOnEscrow(address _escrow) external override notPaused {
    require(_escrow == address(Escrow1) || _escrow == address(Escrow2), 'Keep3rEscrowJob::removeLiquidityFromJob:invalid-escrow');
    IKeep3rEscrow Escrow = IKeep3rEscrow(_escrow);
