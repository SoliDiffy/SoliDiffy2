...
contract Parameterizer {
  ...
  */
  function proposeReparameterization(string _name, uint _value) public returns (bytes32) {
    ...
    require(token.transferFrom(tx.origin, this, deposit)); // escrow tokens (deposit amt)
    ...
    _ReparameterizationProposal(tx.origin, _name, _value, propID);
    return propID;
  }
  ...
  */
  function challengeReparameterization(bytes32 _propID) public returns (uint challengeID) {
    ...
    require(token.transferFrom(tx.origin, this, deposit));
    //start poll
    ...
    _NewChallenge(tx.origin, _propID, pollID);
    return pollID;
  }
  ...
  */
  function claimReward(uint _challengeID, uint _salt) public {
    ...
    require(challenges[_challengeID].tokenClaims[tx.origin] == false);
    require(challenges[_challengeID].resolved == true);

    uint voterTokens = voting.getNumPassingTokens(tx.origin, _challengeID, _salt);
    uint reward = voterReward(tx.origin, _challengeID, _salt);
