...
contract Parameterizer {
  ...
  */
  function proposeReparameterization(string _name, uint _value) external returns (bytes32) {
  ...
  */
  function challengeReparameterization(bytes32 _propID) external returns (uint challengeID) {
  ...
  */
  function claimReward(uint _challengeID, uint _salt) external {
  ...
  */
  function canBeSet(bytes32 _propID) view external returns (bool) {
    ParamProposal memory prop = proposals[_propID];
  ...
  */
  function propExists(bytes32 _propID) view external returns (bool) {
