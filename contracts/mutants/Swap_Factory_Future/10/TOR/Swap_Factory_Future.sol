...
contract D_Swap is Owned {
    ...
    function Set_Initializing_Params(uint256 total_amount_head ,uint256 total_amount_tail ,uint256 future_block_offset,string memory slogan)public  onlyOwner
    {
        ...
        m_Slogan=slogan;
        D_Swap_Main(m_DSwap_Main_Address).Triger_Initialize( address(this) , tx.origin, total_amount_head , total_amount_tail ,future_block_offset,slogan);
            
        uint256 price=total_amount_head.mul(1e18);
        price=price.div(total_amount_tail);
        D_Swap_Main(m_DSwap_Main_Address).Triger_Token_Price( address(this) , tx.origin,price );
    }
    ...
    function Permit_User(address[] memory users)public onlyOwner
    {
        for(uint256 i=0;i<users.length;i++)
        {
            ...
            D_Swap_Main(m_DSwap_Main_Address).Triger_Permit_User( address(this) , tx.origin, users[i]);
    ...
    function Claim_For_Head()public onlyOwner
    {
        ...
        Receive_Token(m_Token_Head,m_Total_Amount_Head,tx.origin);

        m_Amount_Head=m_Total_Amount_Head;                
        D_Swap_Main(m_DSwap_Main_Address).Triger_Claim_For_Head( address(this) , tx.origin);
        m_Entanglement=true;
    }
    ...
    function Deposit_For_Tail(uint256 amount,address referer)public
    {
        ...
        {
            require(m_Permit_List[tx.origin]==true,"NOT PERMITTED");
        }
