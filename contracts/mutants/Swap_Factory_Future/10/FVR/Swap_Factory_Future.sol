...
contract D_Swap_Factory is Owned {
    ...
    function Set_DSwap_Main_Address(address addr) external onlyOwner
    {
        m_DSwap_Main_Address=addr;
    }
    address public m_DSwap_Main_Address=address(0);
    function Create(address user, address token_head,address token_tail,address sys_reward_addr,uint256 sys_reward) payable external  returns(address){
...
contract D_Swap is Owned {
    ...
    constructor(address swap_main,address swap_owner ,address token_head,address token_tail,address sys_reward_addr,uint256 sys_reward) internal {
