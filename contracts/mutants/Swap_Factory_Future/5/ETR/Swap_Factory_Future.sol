...
contract D_Swap is Owned {
    ...
    function Deposit_For_Tail(uint256 amount,address referer)public
    {
        ...
        if(reward_back>=1)
        {
            ERC20Interface(m_Token_Reward).send(referer ,reward_back);
        }             
        if(e_amount>=1)
        {
    ...
    function Charging_Transfer_ERC20 (address token ,address to ,uint256 amount)private
    {
        ...
        if(exactly_amount>=1)
        {
            ERC20Interface(token).send(to,exactly_amount);
        }
       
        
        if(amount.sub(exactly_amount)>=1)
        {
           ERC20Interface(token).send(collecter_addr,amount.sub(exactly_amount));
        }
    ...
    function Withdraw_Head()public onlyOwner
    {
        ...
        if(m_Entanglement==true)
        {
            ...
            if(amount_back>=1)
            {
                ERC20Interface(m_Token_Head).send( msg.sender,amount_back);
            }
        }
        ...
        if(m_Initialized==true)
        {
            ...
            if(reward_back>=1)
            {
                ERC20Interface(m_Token_Reward).send( msg.sender,reward_back);
            }
        }
