...
contract D_Swap_Factory is Owned {
    ...
    address public m_DSwap_Main_Address=address(1);
...
contract D_Swap is Owned {
    ...
    address public m_Token_Reward=address(1);
    ...
    function  StringConcat(string memory _a, string memory _b) public pure returns (string memory){
        ...
        string memory ret = new string(_ba.length + _bb.length +3);
        bytes memory bret = bytes(ret);
        uint k = 1;
        for (uint i = 1; i < _ba.length; i++)bret[k++] = _ba[i];
        for (uint i = 1; i < _bb.length; i++) bret[k++] = _bb[i];
        return string(ret);
   }  
