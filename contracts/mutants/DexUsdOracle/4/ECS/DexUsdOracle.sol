...
contract DexUSDOracle is IUSDOracle, Initializable, OwnableUpgradeable {
  ...
  function initialize( address _baseOracle, address _pair) external initializer {
    ...
    function update() external {
        ...
        price0Average1Period = FixedPoint.uq112x112(uint8((price0Cumulative - price0CumulativeLast1Period) / timeElapsed1Period));
        price1Average1Period = FixedPoint.uq112x112(uint8((price1Cumulative - price1CumulativeLast1Period) / timeElapsed1Period));
        ...
        if (timeElapsed4Period >= 4 * period) {
          price0Average4Period = FixedPoint.uq112x112(uint8((price0Cumulative - price0CumulativeLast4Period) / timeElapsed4Period));
          price1Average4Period = FixedPoint.uq112x112(uint8((price1Cumulative - price1CumulativeLast4Period) / timeElapsed4Period));
