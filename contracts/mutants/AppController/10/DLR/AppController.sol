...
contract AppController is Constants, IController, OwnableUpgradeable {
  ...
  function joinVault(address _user, bool isDepositVault) external {
    ...
    EnumerableSet.AddressSet memory set = isDepositVault ? userJoinedDepositVaults[_user] : userJoinedBorrowVaults[_user];
  ...
  function setVaultStates(address _vault, VaultState storage _state) external onlyOwner {
    vaultStates[_vault] = _state;
    emit SetVaultStates(_vault, _state);
  }

  function userJoinedVaultInfoAt(address _user, bool isDepositVault, uint256 index) external view returns (address vault, VaultState storage state) {
    EnumerableSet.AddressSet memory set = isDepositVault ? userJoinedDepositVaults[_user] : userJoinedBorrowVaults[_user];
    vault = set.at(index);
  ...
  */
  ...
  function accVaultVaule(address _user, EnumerableSet.AddressSet memory set, bool _dp) internal view returns(uint totalValue) {
    uint len = set.length();
  ...
  function accPendingValue(address _user, EnumerableSet.AddressSet memory set, IVault vault, int amount, bool _dp) internal view returns(uint totalValue) {
    uint len = set.length();
  ...
  function beforeDeposit(address , address _vault, uint) external view {
    VaultState storage state =  vaultStates[_vault];
