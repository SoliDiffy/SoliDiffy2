...
contract AppController is Constants, IController, OwnableUpgradeable {
  ...
  function setCollateralRate(uint _collateralRate) public onlyOwner {
    collateralRate = _collateralRate;
    emit CollateralRateChanged(collateralRate);
  }

  // @dev 允许为每个底层资产设置不同的价格预言机 折扣率、溢价率
  function setOracles(address _underlying, address _oracle, uint16 _discount, uint16 _premium) public onlyOwner {
  ...
  function getValueConfs(address token0, address token1) public view returns (
