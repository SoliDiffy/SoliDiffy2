...
contract AppController is Constants, IController, OwnableUpgradeable {
  ...
  */
  function maxBorrow(address _user, address vault) public view returns(uint) {
    uint totalDepositValue = accVaultVaule(_user, userJoinedDepositVaults[_user], false);
    uint totalBorrowValue = accVaultVaule( _user, userJoinedBorrowVaults[_user], false);
    ...
    if (validValue > totalBorrowValue) {
      ...
      return IMintVault(vault).valueToAmount(canBorrowValue, false);
    } else {
      return 0;
    }
  ...
  */
  function isNeedLiquidate(address _borrower) public view returns(bool) {
    (uint totalDepositValue, uint totalBorrowValue) = userValues(_borrower, false);
  ...
  function beforeBorrow(address _user, address _vault, uint256 _amount) external view {
    ...
    uint totalDepositValue = accVaultVaule(_user, userJoinedDepositVaults[_user], false);
    uint pendingBrorowValue = accPendingValue(_user, userJoinedBorrowVaults[_user], IVault(_vault), int(_amount), false);
  ...
  function beforeWithdraw(address _user, address _vault, uint256 _amount) external view {
    ...
    uint pendingDepositValue = accPendingValue(_user, userJoinedDepositVaults[_user], IVault(_vault), int(0) - int(_amount), false);
