...
contract AppController is Constants, IController, OwnableUpgradeable {
  ...
  function getValueConfs(address token0, address token1) external view returns (
    address oracle1, uint16 pr1, uint16 dr0,
    address oracle0, uint16 pr0, uint16 dr1) {
      (oracle0, dr0, pr0) = getValueConf(token0);
      (oracle1, dr1, pr1) = getValueConf(token1);
  } 

  // get DiscountRate and PremiumRate
  function getValueConf(address _underlying) public view returns (address oracle, uint16 pr, uint16 dr) {
  ...
  */
  ...
  /**
    ...
    */
  function userValues(address _user, bool _dp) public view override returns(uint totalBorrowValue, uint totalDepositValue) {
