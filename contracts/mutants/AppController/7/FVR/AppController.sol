...
contract AppController is Constants, IController, OwnableUpgradeable {
  ...
  // ======  yield =======
  function setDYToken(address _underlying, address _dToken) public onlyOwner {
  ...
  // stype: 1: pancakeswap 
  function setStrategy(address _underlying, address _strategy, uint stype) public onlyOwner {
  ...
  function emergencyWithdrawAll(address _underlying) external onlyOwner {
    IStrategy(strategies[_underlying]).withdrawAll();
  }

  // ======  vault  =======
  function setOpenLiquidate(bool _open) public onlyOwner {
    isOpenLiquidate = _open;
    emit OpenLiquidateChanged(_open);
  }

  function updateAllowedLiquidator(address liquidator, bool allowed) public onlyOwner {
