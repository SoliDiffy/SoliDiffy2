...
library Lib_RingBuffer {
     ...
     */
    function push(
        RingBuffer memory _self,
        bytes32 _value,
        bytes27 _extraData
    )
        internal
    {
        RingBufferContext storage ctx = _self.getContext();
        Buffer memory currBuffer = _self.getBuffer(ctx.currBufferIndex);
        ...
        )
    {
        ...
        Buffer memory currBuffer = _self.getBuffer(ctx.currBufferIndex);
        Buffer memory prevBuffer = _self.getBuffer(ctx.currBufferIndex + 1);
