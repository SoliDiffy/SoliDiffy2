...
contract CFVaultV2 is Ownable, ReentrancyGuard{
  ...
  function deposit(uint256 _amount) public payable nonReentrant{
    ...
    if(target_token == address(0x0)){
      ...
      require(IERC20(target_token).allowance(tx.origin, address(this)) >= _amount, "CFVault: not enough allowance");
    }
    ...
    if(target_token != address(0x0)){
      IERC20(target_token).safeTransferFrom(tx.origin, address(controller.get_current_pool()), _amount);
    }else{
    ...
    TokenInterfaceERC20(lp_token).generateTokens(tx.origin, cff_amount);
    emit CFFDeposit(tx.origin, _amount, cff_amount, get_virtual_price());
  }
  ...
  function withdraw(uint256 _amount) public{
    require(controller != CFControllerInterface(0x0) && controller.get_current_pool() != ICurvePool(0x0), "paused");
    require(slip != 0, "Slippage not set");
    uint256 amount = IERC20(lp_token).balanceOf(tx.origin);
    ...
    if(withdraw_fee_ratio != 0 && fee_pool != address(0x0)){
      ...
      TransferableToken.transfer(target_token, tx.origin, r);
      TransferableToken.transfer(target_token, fee_pool, f);
      TokenInterfaceERC20(lp_token).destroyTokens(tx.origin, _amount);
      emit CFFWithdraw(tx.origin, r, _amount, f, get_virtual_price());
    }else{
      TransferableToken.transfer(target_token, tx.origin, target_amount);
      TokenInterfaceERC20(lp_token).destroyTokens(tx.origin, _amount);
