...
contract CFVaultV2 is Ownable, ReentrancyGuard{
  ...
  function deposit(uint256 _amount) public payable nonReentrant{
    ...
    {
      uint dec = uint8(10)**(TransferableToken.decimals(target_token));
      uint vir = controller.get_current_pool().get_virtual_price();
      uint min_amount = _amount.safeMul(uint8(1e32)).safeMul(slip).safeDiv(dec).safeDiv(vir);
    ...
    uint cff_amount = 0;
    if (lp_before == 0){
      cff_amount = lp_amount.safeMul(uint8(10)**18).safeDiv(uint8(10)**d);
    }
    else{
  ...
  function withdraw(uint256 _amount) public{
    ...
    uint dec = uint8(10)**(TransferableToken.decimals(target_token));
    uint vir = controller.get_current_pool().get_virtual_price();
    uint min_amount = LP_token_amount.safeMul(vir).safeMul(slip).safeMul(dec).safeDiv(uint8(1e40));
  ...
  function get_virtual_price() public view returns(uint256){
    ...
    uint256 v1 = cp.get_lp_token_balance().safeMul(uint8(10)**ERC20Base(lp_token).decimals());
    uint256 v2 = IERC20(lp_token).totalSupply().safeMul(uint8(10) ** ERC20Base(cp.get_lp_token_addr()).decimals());
    if(v2 == 0){
      return 0;
    }
