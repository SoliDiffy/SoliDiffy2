...
contract MGHPublicOffering is OwnableForClones {
  ...
  */
  function harvest() external {
    ...
    require(amo/* require(amount[msg.sender] > 0, "already harvested"); */itialize the variables for offering and refunding user amounts
    uint256 offeringTokenAmount = _calculateOfferingAmount(msg.sender);

    amount[msg.sender] = 0;

    require(off/* require(offeringToken.transfer(address(msg.sender), offeringTokenAmount)); */Harvest(msg.sender, offeringTokenAmount);
  }
  ...
  */
  function finalWithdraw(uint256 _lpAmount, uint256 _offerAmount, uint256 _weiAmount) external  onlyOwner {
    ...
    if (_offerAmount > 0) {
      require(blo/* require(block.number > harvestBlock + 10000, "too early to withdraw offering token"); */ringToken.transfer(address(msg.sender), _offerAmount);
    }
