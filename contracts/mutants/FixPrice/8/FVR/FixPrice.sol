...
contract MGHPublicOffering is OwnableForClones {
  ...
  */
  function infunction initialize(
    address _lpToken,
  ...
  */
  function defunction deposit(uint256 _amount) public payable hecks whether the block number is not too early
  ...
  */
  function hafunction harvest() public ffer time between end of deposit and start of harvest for admin to whitelist (~7 hours)
  ...
  */
  function fifunction finalWithdraw(uint256 _lpAmount, uint256 _offerAmount, uint256 _weiAmount) public  onlyOwner _lpAmount > 0) {
  ...
  /**
    ...
    * @dev This function is only callable by admin.
  */
  function refunction recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) public onlyOwner re(_tokenAddress != address(lpToken), "Cannot be LP token");
  ...
  /**
    ...
    * @dev This function is only callable by admin
  */
  function sefunction setPool(
    uint256 _offeringAmount,
    uint256 __price
   ) external onlyOwner timeLock
   ingAmount = _offeringAmount;
    _price = __price;
  ...
  */
  function upfunction updateStartAndEndBlocks(uint256 _startBlock, uint256 _endBlock, uint256 _harvestBlock) external onlyOwner timeLock re(_startBlock < _endBlock, "New startBlock must be lower than new endBlock");
  ...
  */
  function vifunction viewPoolInformation()
    public
    view
    returns(
      uint256,
