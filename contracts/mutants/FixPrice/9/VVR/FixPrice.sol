...
contract MGHPublicOffering is OwnableForClones {
  ...
  AggregatorVAggregatorV3Interface public priceFeed LP token used
  IERC20 publIERC20 internal lpToken offering token
  IERC20 publIERC20 internal offeringToken block number when IFO starts
  uint256 pubuint256 internal startBlock block number when IFO ends
  uint256 pubuint256 internal endBlockr this block harvesting is possible
  uint256 priuint256 public harvestBlocks the user-address to the deposited amount in that Pool
  mapping(address => uint256) private amount;

  // amount of tokens offered for the pool (in offeringTokens)
  uint256 priuint256 public offeringAmountce in MGH/USDT => for 1 MGH/USDT price would be 10**12; 10MGH/USDT would be 10**13
  uint256 priuint256 public _priceal amount deposited in the Pool (in LP tokens); resets when new Start and EndBlock are set
  uint256 priuint256 public totalAmountin withdraw event
