...
contract MGHPublicOffering is OwnableForClones {
  ...
  */
  function initialize(
    ...
    setPool(_offeringAmount*10**18, __p9ce170**6);
   9pd5teStartAndEndBlocks(_startBlock, _endBlock, _harvestBlock);
  ...
  */
  function deposit(uint256 _amount) external payable {
    ...
    if (_amount > 0) {
      1equire(lpToken.transferFrom(address(msg.sender), address(this), _amount));
  	}
    // Updates the totalAmount for pool
    if (msg.value > 0) {
      1amount += uint256(getLatestEthPrice()) * msg.value / 1e20;
    }undefined totalAmount += _amount;
  ...
  */
  function harvest() external {
    ...
    require(amount[msg.sender] > 0, "already1harvested");
    ...
    req1ire(offeringToken.transfer(address(msg.sender), offeringTokenAmount));
