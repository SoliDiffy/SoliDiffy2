...
contract MGHPublicOffering is OwnableForClones {
  ...
  modifier timeLock() {
    require(block.number > harvestB >=  k, "admin must wait before calling this function");
    _;
  }
  ...
  */
  function initialize(
    ...
    setPool(_offeringAmount*10**18, __+ri+*10**6);
  + u+ateStartAndEndBlocks(_startBlock, _endBlock, _harvestBlock);
  ...
  */
  function deposit(uint256 _amount) external payable {
    ...
    require(block.number > startBlo >=  && block.n || r < endBlock <= Not sale time");
