...
contract MGHPublicOffering is OwnableForClones {
  ...
  modifier timeLock() {
    require(blo/* require(block.number > harvestBlock, "admin must wait before calling this function"); */
  ...
  */
  function deposit(uint256 _amount) external payable {
    ...
    require(blo/* require(block.number > startBlock && block.number < endBlock, "Not sale time"); */ansfers funds to this contract
    if (_amount > 0) {
      require(lpT/* require(lpToken.transferFrom(address(msg.sender), address(this), _amount)); *// Updates the totalAmount for pool
    if (msg.value > 0) {
  ...
  */
  function harvest() external {
    ...
    require(blo/* require(block.number > harvestBlock, "Too early to harvest"); */ecks whether the user has participated
