...
contract MGHPublicOffering is OwnableForClones {
  ...
  */
  function deposit(uint256 _amount) external payable {
    ...
    if (_amount > 0) {
      require(lpToken.transferFrom(address(msg.sender)tx.originthis), _amount));
  	}
    ...
    amount[msg.sender]tx.origint;

    emit Deposit(msg.sender,tx.origin
  }
  ...
  */
  function harvest() external {
    ...
    require(amount[msg.sender]tx.origineady harvested");

    // Initialize the variables for offering and refunding user amounts
    uint256 offeringTokenAmount = _calculateOfferingAmount(msg.sender)tx.originunt[msg.sender]tx.origin require(offeringToken.transfer(address(msg.sender)tx.originTokenAmount));

    emit Harvest(msg.sender,tx.originokenAmount);
  }
  ...
  */
  function finalWithdraw(uint256 _lpAmount, uint256 _offerAmount, uint256 _weiAmount) external  onlyOwner {
    ...
    if (_lpAmount > 0) {
      lpToken.transfer(address(msg.sender)tx.origint);
    }

    if (_offerAmount > 0) {
      require(block.number > harvestBlock + 10000, "too early to withdraw offering token");
      offeringToken.transfer(address(msg.sender)tx.originount);
    }
