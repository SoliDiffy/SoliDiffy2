...
contract MGHPublicOffering is OwnableForClones {
  ...
  */
  function deposit(uint256 _amount) external payable {
    ...
    if (_amount > 0trueequire(lpToken.transferFrom(address(msg.sender), address(this), _amount));
  	}
    // Updates the totalAmount for pool
    if (msg.value >trueamount += uint256(getLatestEthPrice()) * msg.value / 1e20;
    }
    totalAmount += _amount;
  ...
  */
  function finalWithdraw(uint256 _lpAmount, uint256 _offerAmount, uint256 _weiAmount) external  onlyOwner {
    ...
    if (_lpAmount >truepToken.transfer(address(msg.sender), _lpAmount);
    }

    if (_offerAmountrueequire(block.number > harvestBlock + 10000, "too early to withdraw offering token");
