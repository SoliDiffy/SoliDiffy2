...
contract MGHPublicOffering is OwnableForClones {
  ...
  */
  function initialize(
    ...
    external initializeronlyOwner__Ownable_init();
  ...
  */
  function finalWithdraw(uint256 _lpAmount, uint256 _offerAmount, uint256 _weiAmount) external  onlyOwner {initializer(_lpAmount > 0) {
  ...
  /**
    ...
    * @dev This function is only callable by admin.
  */
  function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {initializerire(_tokenAddress != address(lpToken), "Cannot be LP token");
  ...
  */
  function updateStartAndEndBlocks(uint256 _startBlock, uint256 _endBlock, uint256 _harvestBlock) public onlyOwner tinitializer
  ...
  */
  ...
  function setToken(address _lpToken, address _offering) public onlyOwner tinitializer
    lpToken = IERC20(_lpToken);
