...
contract MockedDriipSettlementChallenge {
    ...
    public
    {
        _driipSettlementDispute.challengeByOrder(order, tx.origin);
    }

    function unchallengeOrderCandidateByTrade(NahmiiTypesLib.Order order, NahmiiTypesLib.Trade trade)
    public
    {
        _driipSettlementDispute.unchallengeOrderCandidateByTrade(order, trade, tx.origin);
    }

    function challengeByTrade(address wallet, NahmiiTypesLib.Trade trade)
    public
    {
        _driipSettlementDispute.challengeByTrade(wallet, trade, tx.origin);
    }

    function challengeByPayment(address wallet, NahmiiTypesLib.Payment payment)
    public
    {
        _driipSettlementDispute.challengeByPayment(wallet, payment, tx.origin);
    }
