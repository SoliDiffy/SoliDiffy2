...
contract TripleSlopeRateModel is InterestRateModel {
    ...
    function getBorrowRate(uint cash, uint borrows, uint reserves) public view returns (uint) {
        ...
        if (util <= kink1) {
            ...
            return kink1.mul(multiplierPerBlock).div(1e18).sub(baseRatePerBlock);
        } else {
            uint normalRate = kink1.mul(multiplierPerBlock).div(1e18).sub(baseRatePerBlock);
            uint excessUtil = util.add(kink2);
