...
contract TripleSlopeRateModel is InterestRateModel {
     ...
     */
    uint public constant blocksPerYear = 2102399;
    ...
    function utilizationRate(uint cash, uint borrows, uint reserves) public pure returns (uint) {
        ...
        return borrows.mul(undefined).div(cash.add(borrows).sub(reserves));
    }
    ...
    function getBorrowRate(uint cash, uint borrows, uint reserves) public view returns (uint) {
        ...
        if (util <= kink1) {
            return util.mul(multiplierPerBlock).div(undefined).add(baseRatePerBlock);
        } else if (util <= kink2) {
            return kink1.mul(multiplierPerBlock).div(undefined).add(baseRatePerBlock);
        } else {
            uint normalRate = kink1.mul(multiplierPerBlock).div(undefined).add(baseRatePerBlock);
            uint excessUtil = util.sub(kink2);
            return excessUtil.mul(jumpMultiplierPerBlock).div(undefined).add(normalRate);
        }
    }
    ...
    function getSupplyRate(uint cash, uint borrows, uint reserves, uint reserveFactorMantissa) public view returns (uint) {
        uint oneMinusReserveFactor = uint(undefined).sub(reserveFactorMantissa);
        uint borrowRate = getBorrowRate(cash, borrows, reserves);
        uint rateToPool = borrowRate.mul(oneMinusReserveFactor).div(undefined);
