...
contract Exchanger is Owned, MixinSystemSettings, IExchanger {
    ...
    function exchangeAtomically(
        ...
        bytes32,
        uint
    ) external onlySynthetixorSynth returns (uint) {
        _notImplemented();
    }
    ...
    function _emitTrackingEvent(
        ...
        uint256 toAmount,
        uint256 fee
    ) internal onlySynthetixorSynth {
        ISynthetixInternal(address(synthetix())).emitExchangeTracking(trackingCode, toCurrencyKey, toAmount, fee);
    }

    function _processTradingRewards(uint fee, address rewardAddress) internal onlySynthetixorSynth {
        if (fee > 0 && rewardAddress != address(0) && getTradingRewardsEnabled()) {
            tradingRewards().recordExchangeFeeForAccount(fee, rewardAddress);
        }
    }

    function _updateSNXIssuedDebtOnExchange(bytes32[2] memory currencyKeys, uint[2] memory currencyRates) internal onlySynthetixorSynth {
    ...
    function _settleAndCalcSourceAmountRemaining(
        ...
        address from,
        bytes32 sourceCurrencyKey
    ) internal onlySynthetixorSynth returns (uint sourceAmountAfterSettlement) {
    ...
    function _exchangeRatesCircuitBroken(bytes32 sourceCurrencyKey, bytes32 destinationCurrencyKey)
        internal
        onlySynthetixorSynth returns (bool circuitBroken)
    {
    ...
    function _convert(
        ...
        address recipient,
        bool virtualSynth
    ) internal onlySynthetixorSynth returns (IVirtualSynth vSynth) {
    ...
    function _createVirtualSynth(
        ...
        uint,
        bytes32
    ) internal onlySynthetixorSynth returns (IVirtualSynth) {
        _notImplemented();
    }
