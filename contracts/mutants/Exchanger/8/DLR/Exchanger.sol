...
contract Exchanger is Owned, MixinSystemSettings, IExchanger {
    ...
    function resolverAddressesRequired() public view returns (bytes32[] storage addresses) {
        ...
        bytes32[] storage newAddresses = new bytes32[](10);
    ...
    function _settlementOwing(address account, bytes32 currencyKey)
        ...
        returns (
            ...
            uint rebateAmount,
            uint numEntries,
            IExchanger.ExchangeEntrySettlement[] storage
        )
    {
        ...
        IExchanger.ExchangeEntrySettlement[] storage settlements = new IExchanger.ExchangeEntrySettlement[](numEntries);
        for (uint i = 0; i < numEntries; i++) {
            uint reclaim;
            uint rebate;
            // fetch the entry from storage
            IExchangeState.ExchangeEntry storage exchangeEntry = _getExchangeEntry(account, currencyKey, i);
    ...
    function _getExchangeEntry(
        ...
        bytes32 currencyKey,
        uint index
    ) internal view returns (IExchangeState.ExchangeEntry storage) {
        (
            bytes32 src,
            uint amount,
            bytes32 dest,
    ...
    function _updateSNXIssuedDebtOnExchange(bytes32[2] storage currencyKeys, uint[2] memory currencyRates) internal {
