...
contract MasterChef is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    function setRewardDistribution(RewardDistribution _rewardDistribution) external {
    ...
    function add(uint32 _allocPoint, IBEP20 _lpToken, uint16 _depositFeeBP, uint32 _harvestInterval, bool _withRewards) public  nonDuplicated(_lpToken) {
    ...
    function set(uint256 _pid, uint32 _allocPoint, uint16 _depositFeeBP, uint32 _harvestInterval) public  {
    ...
    function deposit(uint16 _pid, uint256 _amount, address _referrer) public  {
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function emergencyWithdraw(uint16 _pid) public  {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    function updateEmissionRate(uint128 _bullPerBlock) public  {
        _massUpdatePools();
        bullPerBlock = _bullPerBlock;
    ...
    function setBullReferral(IBullReferral _bullReferral) public  {
        bullReferral = _bullReferral;
    }

    /// @dev Update referral commission rate by the owner. Should be in basis points.
    function setReferralCommissionRate(uint16 _referralCommissionRate) public  {
