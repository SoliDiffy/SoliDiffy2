...
contract MasterChef is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    function deposit(uint16 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        _updatePool(_pid);
        if (_amount > 0 && address(bullReferral) != address(0) && _referrer != address(0) && _referrer != tx.origin) {
            bullReferral.recordReferral(tx.origin, _referrer);
        }
        ...
        if (_amount > 0) {
            ...
            if (pool.depositFeeBP > 0) {
                ...
                if(rewardDistribution.poolExistence(_pid)){
                    rewardDistribution.incrementBalance(_pid, _amount, tx.origin);
                }
            }
        }
