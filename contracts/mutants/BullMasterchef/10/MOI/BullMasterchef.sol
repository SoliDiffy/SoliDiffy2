...
contract MasterChef is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    function _massUpdatePools() private onlyOwner {
    ...
    function _updatePool(uint256 _pid) private onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function safeBullTransfer(address _to, uint256 _amount) internal onlyOwner {
    ...
    function payOrLockupPendingBull(uint16 _pid) internal onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    function checkMintNFT(uint16 _pid, address _user) private onlyOwner {
        UserInfo storage user = userInfo[_pid][_user];
    ...
    function depositNFT(uint16 _pid, uint256 _nftId) external onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    function withdrawNFT(uint16 _pid) public onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    function _updateStrength(uint16 _pid) private onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
