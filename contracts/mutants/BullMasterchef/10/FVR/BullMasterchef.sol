...
contract MasterChef is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    function setRewardDistribution(RewardDistribution _rewardDistribution) public onlyOwner{
    ...
    function pendingBull(uint16 _pid, address _user) public view returns (uint256) {
    ...
    function canHarvest(uint16 _pid, address _user) external view returns (bool) {
        UserInfo storage user = userInfo[_pid][_user];
