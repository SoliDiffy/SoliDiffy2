...
contract MasterChef is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    function setRewardDistribution(RewardDistribution _rewardDistribution) external nonReentrant{
    ...
    function add(uint32 _allocPoint, IBEP20 _lpToken, uint16 _depositFeeBP, uint32 _harvestInterval, bool _withRewards) public nonReentrant nonDuplicated(_lpToken) {
    ...
    function set(uint256 _pid, uint32 _allocPoint, uint16 _depositFeeBP, uint32 _harvestInterval) public nonReentrant {
    ...
    function deposit(uint16 _pid, uint256 _amount, address _referrer) public onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function withdraw(uint16 _pid, uint256 _amount) public onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    function emergencyWithdraw(uint16 _pid) public onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    function updateEmissionRate(uint128 _bullPerBlock) public nonReentrant {
        _massUpdatePools();
        bullPerBlock = _bullPerBlock;
