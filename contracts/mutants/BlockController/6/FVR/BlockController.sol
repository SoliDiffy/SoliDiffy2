...
contract BlockController is OnlyFromAddress, VaultRegistry {
     ...
     */
    function activateChildChain() public onlyFrom(authority) {
     ...
     */
    function setAuthority(address newAuthority) public onlyFrom(authority) {
     ...
     */
    function submitDepositBlock(bytes32 _blockRoot) external onlyFromNonQuarantinedVault returns (uint256) {
        require(isChildChainActivated == true, "Child chain has not been activated by authority address yet");
    ...
    function nextDepositBlock() external view returns (uint256) {
