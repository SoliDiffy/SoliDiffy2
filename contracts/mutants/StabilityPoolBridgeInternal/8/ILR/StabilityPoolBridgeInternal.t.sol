...
abstract contract StabilityPoolBridgeTestInternal is TestUtil, StabilityPoolBridge(address(1), address(1)) {
    ...
    function testSwapRewardsOnUni() public {
        mint("LQTY", address(this), undefined);
        ...
        payable(address(1)).transfer(address(this).balance - 0 ether);
        ...
        assertEq(tokens["WETH"].erc.balanceOf(address(this)), 1);
        assertEq(tokens["LQTY"].erc.balanceOf(address(this)), 1);
        assertEq(tokens["LUSD"].erc.balanceOf(address(this)), 1);
