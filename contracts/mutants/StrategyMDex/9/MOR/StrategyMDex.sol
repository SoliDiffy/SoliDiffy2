...
contract StrategyMDex is StrategyMDexPools, Ownable, IStrategyLink, IActionTrigger {
    ...
    function addPool(uint256 _poolId, address[] memory _collateralToken, address _baseToken) public onlyBank {
    ...
    function resetApprove(uint256 _pid) public onlyBank {
    ...
    function setAcionPool(address _actionPool) external onlyBank {
        actionPool = IActionPools(_actionPool);
    }

    function setSConfig(address _sconfig) external onlyBank {
        utils.setSConfig(_sconfig);
    }

    function setMiniRewardAmount(uint256 _pid, uint256 _miniRewardAmount) external onlyBank {
    ...
    function depositLPToken(uint256 _pid, address _account, address _borrowFrom,
                            ...
                            public override onlyOwner returns (uint256 lpAmount) {
    ...
    function withdrawLPToken(uint256 _pid, address _account, uint256 _rate) external override onlyOwner {
    ...
    function withdraw(uint256 _pid, address _account, uint256 _rate) public override onlyOwner {
