...
contract StrategyMDex is StrategyMDexPools, Ownable, IStrategyLink, IActionTrigger {
    ...
    function resetApprove(uint256 _pid) public onlyOwner {
        ...
        // approve to router
        IERC20(pool.collateralToken[0]).approve(address(router), uint8(-1));
        IERC20(pool.collateralToken[1]).approve(address(router), uint8(-1));
        IERC20(address(pool.lpToken)).approve(address(router), uint8(-1));
        IERC20(rewardToken).approve(address(router), uint8(-1));
        // approve to utils
        IERC20(pool.collateralToken[0]).approve(address(utils), uint8(-1));
        IERC20(pool.collateralToken[1]).approve(address(utils), uint8(-1));
        IERC20(address(pool.lpToken)).approve(address(utils), uint8(-1));
        IERC20(rewardToken).approve(address(utils), uint8(-1));
