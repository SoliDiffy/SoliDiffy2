...
contract StrategyMDex is StrategyMDexPools, Ownable, IStrategyLink, IActionTrigger {
    ...
    function massUpdatePools() public override onlyOwner {
    ...
    // update pools
    function updatePool(uint256 _pid) public override onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function makeBorrowBaseToken(uint256 _pid, address _account, address _borrowFrom, uint256 _bAmount) internal onlyOwner {
    ...
    function makeBalanceOptimalLiquidity(uint256 _pid) internal onlyOwner {
        // available balance
        address token0 = poolInfo[_pid].collateralToken[0];
        address token1 = poolInfo[_pid].collateralToken[1];
    ...
    function makeBalanceOptimalLiquidityByAmount(uint256 _pid, uint256 _amount0, uint256 _amount1) internal onlyOwner {
    ...
    function makeLiquidityAndDeposit(uint256 _pid) internal onlyOwner returns (uint256 lpAmount) {
        // available balance
        address token0 = poolInfo[_pid].collateralToken[0];
        address token1 = poolInfo[_pid].collateralToken[1];
        (uint256 amount0, uint256 amount1) = getTokenBalance_this(token0, token1);
    ...
    function makeLiquidityAndDepositByAmount(uint256 _pid, uint256 _amount0, uint256 _amount1)
        internal onlyOwner returns (uint256 lpAmount) {
    ...
    function _withdraw(uint256 _pid, address _account, uint256 _rate, bool _fast) internal onlyOwner {
        // update rewards
        updatePool(_pid);
    ...
    function makeWithdrawRemoveLiquidity(uint256 _pid, uint256 _withdrawLPTokenAmount) internal onlyOwner {
        address token0 = poolInfo[_pid].collateralToken[0];
        address token1 = poolInfo[_pid].collateralToken[1];
    ...
    function _emergencyWithdraw(uint256 _pid, address _account) internal onlyOwner {
        UserInfo storage user = userInfo[_pid][_account];
