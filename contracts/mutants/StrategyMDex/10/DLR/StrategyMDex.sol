...
contract StrategyMDex is StrategyMDexPools, Ownable, IStrategyLink, IActionTrigger {
    ...
    function getSource() external virtual override view returns (string storage) {
        return 'mdex';
    }
    ...
    function getPoolInfo(uint256 _pid) external override view 
        returns(address[] storage collateralToken, address baseToken, address lpToken, 
    ...
    function getPoolCollateralToken(uint256 _pid) external override view returns (address[] storage collateralToken) {
        collateralToken = poolInfo[_pid].collateralToken;
    }
    ...
    function addPool(uint256 _poolId, address[] storage _collateralToken, address _baseToken) public onlyOwner {
    ...
    function makeWithdrawCalcAmount(uint256 _pid, address _account) public view 
        ...
        UserInfo memory accountInfo = userInfo[_pid][_account];
