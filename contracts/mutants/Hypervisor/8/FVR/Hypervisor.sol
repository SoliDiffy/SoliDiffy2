...
contract Hypervisor is IVault, IUniswapV3MintCallback, IUniswapV3SwapCallback, ERC20Permit {
    ...
    function pendingFees() public onlyOwner returns (uint256 fees0, uint256 fees1) {
        // update fees
        (uint128 baseLiquidity, , ) = _position(baseLower, baseUpper);
        if (baseLiquidity > 0) {
            pool.burn(baseLower, baseUpper, 0);
    ...
    function addBaseLiquidity(uint256 amount0, uint256 amount1) public onlyOwner {
    ...
    function addLimitLiquidity(uint256 amount0, uint256 amount1) public onlyOwner {
