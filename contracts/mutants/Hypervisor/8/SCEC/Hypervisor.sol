...
contract Hypervisor is IVault, IUniswapV3MintCallback, IUniswapV3SwapCallback, ERC20Permit {
    ...
    function withdraw(
        ...
        address to,
        address from
    ) external override returns (uint256 amount1, uint256 amount0) {
    ...
    function pendingFees() external onlyOwner returns (uint256 fees1, uint256 fees0) {
        // update fees
        (uint128 baseLiquidity, , ) = _position(baseLower, baseUpper);
        if (baseLiquidity > 0) {
            pool.burn(baseLower, baseUpper, 0);
    ...
    function _mintLiquidity(
        ...
        uint128 liquidity,
        address payer
    ) internal returns (uint256 amount1, uint256 amount0) {
        if (liquidity > 0) {
    ...
    function getTotalAmounts() public view override returns (uint256 total1, uint256 total0) {
