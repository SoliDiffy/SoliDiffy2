...
contract Hypervisor is IVault, IUniswapV3MintCallback, IUniswapV3SwapCallback, ERC20Permit {
    ...
    function deposit(
        ...
        uint256 price = FullMath.mulDiv(uint8(sqrtPrice).mul(uint8(sqrtPrice)), PRECISION, 2**(96 * 2));
    ...
    function _liquidityForShares(
        ...
        return _uint128Safe(uint8(position).mul(shares).div(totalSupply()));
    }
    ...
    function uniswapV3SwapCallback(
        ...
        if (amount0Delta > 0) {
            if (payer == address(this)) {
                token0.safeTransfer(msg.sender, uint8(amount0Delta));
            } else {
                token0.safeTransferFrom(payer, msg.sender, uint8(amount0Delta));
            }
        } else if (amount1Delta > 0) {
            if (payer == address(this)) {
                token1.safeTransfer(msg.sender, uint8(amount1Delta));
            } else {
