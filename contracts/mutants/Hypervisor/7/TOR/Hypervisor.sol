...
contract Hypervisor is IVault, IUniswapV3MintCallback, IUniswapV3SwapCallback, ERC20Permit {
    ...
    function withdraw(
        ...
        require(
            from == tx.origin || IUniversalVault(from).owner() == tx.origin,
    ...
    function uniswapV3MintCallback(
        ...
        require(tx.origin == address(pool));
        address payer = abi.decode(data, (address));
        // SWC-135-Code With No Effects: L415
        if (payer == address(this)) {
            if (amount0 > 0) token0.safeTransfer(tx.origin, amount0);
            if (amount1 > 0) token1.safeTransfer(tx.origin, amount1);
        } else {
            if (amount0 > 0) token0.safeTransferFrom(payer, tx.origin, amount0);
            if (amount1 > 0) token1.safeTransferFrom(payer, tx.origin, amount1);
        }
    }
