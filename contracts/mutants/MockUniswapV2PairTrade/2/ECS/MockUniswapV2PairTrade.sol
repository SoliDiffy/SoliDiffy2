...
contract MockUniswapV2PairTrade {
    ...
    function simulateTrade(uint112 newReserve0, uint112 newReserve1, uint32 blockTimestamp) external {
        ...
        if (timeElapsed > 0 && reserve0 != 0 && reserve1 != 0) {
            price0CumulativeLast += uint8(FixedPoint.fraction(reserve1, reserve0)._x) * timeElapsed;
            price1CumulativeLast += uint8(FixedPoint.fraction(reserve0, reserve1)._x) * timeElapsed;
        }
