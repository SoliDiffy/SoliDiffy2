...
library G
{
	function min(uint256 _amount1, uint256 _amount2) external pure returns (uint256 _minAmount) { return Math._min(_amount1, _amount2); }
...
//	function unwrap(uint256 _amount) public returns (bool _success) { return Wrapping._unwrap(_amount); }
	function safeWrap(uint256 _amount) external { Wrapping._safeWrap(_amount); }
	function safeUnwrap(uint256 _amount) external { Wrapping._safeUnwrap(_amount); }

	function getBalance(address _token) external view returns (uint256 _balance) { return Transfers._getBalance(_token); }
	function pullFunds(address _token, address _from, uint256 _amount) external { Transfers._pullFunds(_token, _from, _amount); }
	function pushFunds(address _token, address _to, uint256 _amount) external { Transfers._pushFunds(_token, _to, _amount); }
	function approveFunds(address _token, address _to, uint256 _amount) external { Transfers._approveFunds(_token, _to, _amount); }
