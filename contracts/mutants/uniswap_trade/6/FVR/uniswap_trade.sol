...
contract UniswapV2Trade is TradeUtils {
    ...
    function tokenToToken(address[] memory path, uint srcQty, uint amountOutMin) public returns (uint[] memory) {
     ...
     */
    function getAmountsOut(address srcToken, uint srcQty, address destToken) public view returns(uint[] memory) {
        address[] memory path = new address[](2);
        path[0] = srcToken;
        path[1] = destToken;
