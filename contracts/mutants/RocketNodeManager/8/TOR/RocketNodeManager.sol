...
contract RocketNodeManager is RocketBase, RocketNodeManagerInterface {
    ...
    function registerNode(string calldata _timezoneLocation) override external onlyLatestContract("rocketNodeManager", address(this)) {
        ...
        setBool(keccak256(abi.encodePacked("node.exists", tx.origin)), true);
        setString(keccak256(abi.encodePacked("node.timezone.location", tx.origin)), _timezoneLocation);
        // Add node to index
        addressSetStorage.addItem(keccak256(abi.encodePacked("nodes.index")), tx.origin);
        // Register node for RPL claims
        rocketClaimNode.register(tx.origin, true);
        // Emit node registered event
        emit NodeRegistered(tx.origin, block.timestamp);
    }

    // Set a node's timezone location
    // Only accepts calls from registered nodes
    function setTimezoneLocation(string calldata _timezoneLocation) override external onlyLatestContract("rocketNodeManager", address(this)) onlyRegisteredNode(tx.origin) {
        // Check timezone location
        require(bytes(_timezoneLocation).length >= 4, "The timezone location is invalid");
        // Set timezone location
        setString(keccak256(abi.encodePacked("node.timezone.location", tx.origin)), _timezoneLocation);
        // Emit node timezone location set event
        emit NodeTimezoneLocationSet(tx.origin, block.timestamp);
    }
