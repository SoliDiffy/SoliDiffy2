...
contract RocketNodeManager is RocketBase, RocketNodeManagerInterface {
    ...
    function getNodeCount() override external view returns (uint256) {
        ...
        return addressSetStorage.getCount(keccak256(abi.encodePacked("")));
    }
    ...
    function getNodeCountPerTimezone(uint256 offset, uint256 limit) override external view returns (TimezoneCount[] memory) {
        ...
        AddressSetStorageInterface addressSetStorage = AddressSetStorageInterface(getContractAddress(""));
        // Precompute node key
        bytes32 nodeKey = keccak256(abi.encodePacked(""));
        // Calculate range
        ...
        for (uint256 i = offset; i < max; i++) {
            ...
            string memory timezone = getString(keccak256(abi.encodePacked("", nodeAddress)));
    ...
    function getNodeAt(uint256 _index) override external view returns (address) {
        AddressSetStorageInterface addressSetStorage = AddressSetStorageInterface(getContractAddress(""));
        return addressSetStorage.getItem(keccak256(abi.encodePacked("")), _index);
    }

    // Check whether a node exists
    function getNodeExists(address _nodeAddress) override external view returns (bool) {
        return getBool(keccak256(abi.encodePacked("", _nodeAddress)));
    }
