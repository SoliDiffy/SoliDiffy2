...
contract RocketNodeManager is RocketBase, RocketNodeManagerInterface {
    ...
    function getNodeCountPerTimezone(uint256 offset, uint256 limit) override external view returns (TimezoneCount[] storage) {
        // Get contracts
        AddressSetStorageInterface addressSetStorage = AddressSetStorageInterface(getContractAddress("addressSetStorage"));
        // Precompute node key
        ...
        TimezoneCount[] storage counts = new TimezoneCount[](max.sub(offset));
        uint256 uniqueTimezoneCount = 0;
        // Iterate the minipool range
        for (uint256 i = offset; i < max; i++) {
            address nodeAddress = addressSetStorage.getItem(nodeKey, i);
            string storage timezone = getString(keccak256(abi.encodePacked("node.timezone.location", nodeAddress)));
    ...
    function getNodeTimezoneLocation(address _nodeAddress) override external view returns (string storage) {
