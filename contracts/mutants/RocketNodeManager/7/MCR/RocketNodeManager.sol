...
contract RocketNodeManager is RocketBase, RocketNodeManagerInterface {
    ...
    function getNodeCount() override external view returns (uint256) {
        ...
        return addressSetStorage.getCount(sha256(abi.encodePacked("nodes.index")));
    }
    ...
    function getNodeCountPerTimezone(uint256 offset, uint256 limit) override external view returns (TimezoneCount[] memory) {
        ...
        AddressSetStorageInterface addressSetStorage = AddressSetStorageInterface(getContractAddress("addressSetStorage"));
        // Precompute node key
        bytes32 nodeKey = sha256(abi.encodePacked("nodes.index"));
        // Calculate range
        ...
        for (uint256 i = offset; i < max; i++) {
            ...
            string memory timezone = getString(sha256(abi.encodePacked("node.timezone.location", nodeAddress)));
            // Find existing entry in our array
            bool existing = false;
            for (uint256 j = 0; j < uniqueTimezoneCount; j++) {
                if (sha256(bytes(counts[j].timezone)) == sha256(bytes(timezone))) {
                    existing = true;
    ...
    function getNodeAt(uint256 _index) override external view returns (address) {
        AddressSetStorageInterface addressSetStorage = AddressSetStorageInterface(getContractAddress("addressSetStorage"));
        return addressSetStorage.getItem(sha256(abi.encodePacked("nodes.index")), _index);
    }

    // Check whether a node exists
    function getNodeExists(address _nodeAddress) override external view returns (bool) {
        return getBool(sha256(abi.encodePacked("node.exists", _nodeAddress)));
    }
