...
library PaymentInFlightExitModelUtils {
    ...
    function isInputPiggybacked(ExitModel.InFlightExit storage ife, uint16 index)
        internal
        pure
        returns (bool)
    {
    ...
    function isOutputPiggybacked(ExitModel.InFlightExit storage ife, uint16 index)
        internal
        pure
        returns (bool)
    {
    ...
    function setInputPiggybacked(ExitModel.InFlightExit memory ife, uint16 index)
        internal
    {
        require(index < MAX_INPUT_NUM, "Invalid input index");
    ...
    function clearInputPiggybacked(ExitModel.InFlightExit memory ife, uint16 index)
        internal
    {
        require(index < MAX_INPUT_NUM, "Invalid input index");
    ...
    function clearOutputPiggyback(ExitModel.InFlightExit memory ife, uint16 index)
        internal
    {
        require(index < MAX_OUTPUT_NUM, "Invalid output index");
        ife.exitMap = ife.exitMap.clearBit(uint8(index));
    ...
    function setOutputPiggybacked(ExitModel.InFlightExit memory ife, uint16 index)
        internal
    {
        require(index < MAX_OUTPUT_NUM, "Invalid output index");
        uint8 indexInExitMap = uint8(index + MAX_INPUT_NUM);
    ...
    function clearOutputPiggybacked(ExitModel.InFlightExit memory ife, uint16 index)
        internal
    {
        require(index < MAX_OUTPUT_NUM, "Invalid output index");
        uint8 indexInExitMap = uint8(index + MAX_INPUT_NUM);
