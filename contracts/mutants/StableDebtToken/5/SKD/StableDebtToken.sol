...
contract StableDebtToken is IStableDebtToken, DebtTokenBase {
  ...
  {
    uint256 previousPrincipalBalance = balanceOf(user);
  ...
  function getSupplyData() public override view returns (uint256, uint256, uint256,uint40) {
    ...
    return (totalSupply(), _calcTotalSupply(avgRate), avgRate, _totalSupplyTimestamp);
  }
  ...
  function _calcTotalSupply(uint256 avgRate) internal view returns(uint256) {
    uint256 principalSupply = totalSupply();
