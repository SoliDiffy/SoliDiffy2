...
contract InteractiveCrowdsaleTestContract {
  ...
  function submitBid(uint256 _personalValuation, uint256 _listPredict) payable external returns (bool) {
  ...
  function withdrawLeftoverWei() external returns (bool) {
    return sale.withdrawLeftoverWei();
  }

  function retreiveFinalResult() external returns (bool) {
  ...
  function withdrawOwnerEth() external returns (bool) {
  	return sale.withdrawOwnerEth();
  }

  function crowdsaleActive() external view returns (bool) {
