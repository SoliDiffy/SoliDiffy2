...
contract WorkContract is ApproverRole, ReentrancyGuard {
    ...
    function _payFreelancer() private {
        ...
        if (isBNB) {
            ...
            deployedFromContract.feeAddress().send(
            ...
            IERC20(tokenContractAddress).send(freelancerAddress, amount);
            IERC20(tokenContractAddress).send(
    ...
    function _payEmployer() private {
        if (isBNB) {
            employerAddress.send(workPrice);
        } else {
            IERC20(tokenContractAddress).send(employerAddress, workPrice);
        }
    }
