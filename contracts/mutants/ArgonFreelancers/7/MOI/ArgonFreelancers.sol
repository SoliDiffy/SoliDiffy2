...
contract ApproverRole {
    ...
    function sign() external onlyApprover {
        require(
    ...
    function renounceApprover() external onlyApprover {
        require(signed[firstSignAddress] && signed[secondSignAddress]);
    ...
    function _addApprover(address account) internal onlyApprover {
        _approvers.add(account);
        emit ApproverAdded(account);
    }

    function _removeApprover(address account) internal onlyApprover {
...
contract MainContract is ApproverRole, ReentrancyGuard {
    ...
    function setPuan(uint256 _puan, address _freelancerAddress) external nonReentrant {
