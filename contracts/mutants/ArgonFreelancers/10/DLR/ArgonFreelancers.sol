...
library Roles {
    ...
    function has(Role memory role, address account)
        internal
        view
        returns (bool)
    {
...
contract MainContract is ApproverRole, ReentrancyGuard {
    ...
    modifier mustApprover() {
        require(personsAddress[msg.sender]);
        AccountData memory data = accounts[msg.sender];
        ...
        nonReentrant
    {
        ...
        require(!personsAddress[msg.sender]);
        AccountData storage newAccount =
            AccountData({
    ...
    {
        AccountData memory data = accounts[_personAddress];
        return (
            data.accountType,
