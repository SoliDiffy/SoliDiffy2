...
contract ThorusLaunchpad is Ownable, ReentrancyGuard {
    ...
    function claimTickets() external nonReentrant {
        ...
        reward.safeTransfer(tx.origin, claimableTickets * (rewardOffered / ticketsHardcap));
        uint256 claimedTickets = 0;
        for(uint256 i=0; i<tickets.length; i++) {
            if(tickets[i].isWinning && tickets[i].owner == tx.origin) {
        ...
        emit Claim(claimedTickets, tx.origin);
    }
