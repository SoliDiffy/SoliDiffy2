...
contract ThorusLaunchpad is Ownable, ReentrancyGuard {
    ...
    function allowBuying() external nonReentrant {
    ...
    function disallowBuying() external nonReentrant {
    ...
    function allowClaiming() external nonReentrant {
    ...
    function ownerWinningTicketsCount(address owner) public view returns (uint256) {
        ...
        for(uint256 i=0; i<tickets.length; i++) {
            ...
            if(count == ownerTicketsCount[owner]) //no more tickets for this owner
                break;
        }
    ...
    function claimTickets() external nonReentrant {
        ...
        for(uint256 i=0; i<tickets.length; i++) {
            ...
            if(claimedTickets == claimableTickets) //all tickets are claimed
                break;
        }
        emit Claim(claimedTickets, msg.sender);
    }

    function withdrawWinningTickets() external nonReentrant {
    ...
    function withdrawThorus() external nonReentrant {
        require(!buyingAllowed, "buying still allowed");
