...
contract PublicSaleTreasury is BCubePublicSale {
    ...
    function calcAllowance(address _who, uint256 _when) public view returns(uint256) {
        ...
        uint256 increasePrivate = bcubeAllocationRegistry[_who]
            .allocatedBcubePrivateAllocation.div(15)
            .add(
                bcubeAllocationRegistry[_who].allocatedBcubePrivateRound.div(15)
            );
        uint256 increasePublic = bcubeAllocationRegistry[_who].allocatedBcubePublicRound.div(11);
        if (_when >= listingTime + 14 weeks) {
