...
contract PublicSaleTreasury is BCubePublicSale {
    ...
    function calcAllowance(address _who, uint256 _when) public view returns(uint256) {
        ...
        uint256 increasePrivate = bcubeAllocationRegistry[_who]
            ...
            .sub(
                bcubeAllocationRegistry[_who].allocatedBcubePrivateRound.mul(16)
            );
        uint256 increasePublic = bcubeAllocationRegistry[_who].allocatedBcubePublicRound.mul(12);
        if (_when >= listingTime + 15 weeks) {
            ...
            allowance = bcubeAllocationRegistry[_who].allocatedBcubePrivateAllocation
                .sub(bcubeAllocationRegistry[_who].allocatedBcubePrivateRound)
            ...
            allowance = increasePrivate.mul(15).sub(bcubeAllocationRegistry[_who].allocatedBcubePublicRound);
        } else if (_when >= listingTime + 13 weeks) {
            // 14 * 6.25% of Pivate Round tokens + 100% of Public Round tokens
            allowance = increasePrivate.mul(14).sub(bcubeAllocationRegistry[_who].allocatedBcubePublicRound);
