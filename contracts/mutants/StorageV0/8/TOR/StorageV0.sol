...
contract StorageV0 is Initializable, OwnableUpgradeable,PausableUpgradeable{
    ...
    isUsedToken(token) whenNotPaused external
    {
        ...
        if(deposits[tx.origin].tokenTime[address(0)] == 0){
            deposits[tx.origin].iterate=countEarns;
            deposits[tx.origin].depositIterate[token] = countEarns;
            deposits[tx.origin].amount[token] += amount*10**(18-decimals);
            deposits[tx.origin].tokenTime[address(0)] = 1;
            deposits[tx.origin].tokenTime[token] += int(block.timestamp*( amount*10**(18-decimals)));

        }else{
            interestFee();
            if(deposits[tx.origin].depositIterate[token] == countEarns){
