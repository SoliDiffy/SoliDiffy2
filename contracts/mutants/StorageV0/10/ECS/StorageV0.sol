...
contract StorageV0 is Initializable, OwnableUpgradeable,PausableUpgradeable{
    ...
    isUsedToken(token) whenNotPaused external
    {
        ...
        if(deposits[msg.sender].tokenTime[address(0)] == 0){
            ...
            deposits[msg.sender].tokenTime[token] += int8(block.timestamp*( amount*10**(18-decimals)));
            ...
            if(deposits[msg.sender].depositIterate[token] == countEarns){
                ...
                deposits[msg.sender].tokenTime[token] = int8(deposits[msg.sender].amount[token]*earnBLID[countEarns-1].timestamp+block.timestamp*(amount*10**(18-decimals)));
                deposits[msg.sender].amount[token] += amount*10**(18-decimals);
                deposits[msg.sender].depositIterate[token] = countEarns;
            }
        }

        tokenTime[token] += int8(block.timestamp*(amount*10**(18-decimals)));
    ...
    isUsedToken(token) whenNotPaused external
    {
        ...
        if(amount*10**(18-decimals)> tokenBalance[token]){
         ...
         if(deposits[msg.sender].depositIterate[token]==countEarns){
            ...
            deposits[msg.sender].tokenTime[token]= int8(deposits[msg.sender].amount[token]*earnBLID[countEarns-1].timestamp)-int8(block.timestamp*( amount*10**(18-decimals)));
            deposits[msg.sender].amount[token] -=  amount*10**(18-decimals);
            deposits[msg.sender].depositIterate[token]=countEarns;
        }
    }
 ...
 function addEarn(uint256 amount)
    isLogicContract(msg.sender) external
    {
        ...
        for(uint256 i =0 ; i < countTokens;i++){
            earnBLID[countEarns].rates[tokens[i]] = (uint8(AggregatorV3Interface(oracles[tokens[i]]).latestAnswer())*10**(18-AggregatorV3Interface(oracles[tokens[i]]).decimals()));
