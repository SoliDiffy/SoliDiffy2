 ...
 function addEarn(uint256 amount)
    isLogicContract(msg.sender) external
    {
        ...
        for(uint256 i =0 ; i < countTokens;i++){
           ...
           _dollarTime += tokenTime[tokens[i]]*int(earnBLID[countEarns].rates[tokens[i]])/int(1 wei);
        }
        require(_dollarTime!=0);
        earnBLID[countEarns].allBLID = amount;
        earnBLID[countEarns].timestamp = block.timestamp;
        earnBLID[countEarns].tdt = uint((int(((block.timestamp)*earnBLID[countEarns].usd)/(1 wei))-_dollarTime));
        for(uint256 i =0; i < countTokens; i++){
            tokenTime[tokens[i]] = int(tokenDeposited[tokens[i]]*block.timestamp);
        }
        earnBLID[countEarns].usd/=(1 wei);
        countEarns++;
        emit AddEarn(amount);
        emit UpdateBLIDBalance(reserveBLID);
    }
        ...
        for(uint256 i = deposits[account].iterate; i<countEarns; i++){
            for(uint256 j = 0; j<countTokens; j++){
                if(i==deposits[account].depositIterate[tokens[j]]){
                    deposits[account].balanceBLID += earnBLID[i].allBLID*uint((int((deposits[account].amount[tokens[j]]*earnBLID[i].rates[tokens[j]])*earnBLID[i].timestamp)-(deposits[account].tokenTime[tokens[j]]*int(earnBLID[i].rates[tokens[j]]))))/earnBLID[i].tdt/(1 wei);
                }
                else{
                    
                    deposits[account].balanceBLID += earnBLID[i].allBLID*(earnBLID[i].timestamp-earnBLID[i-1].timestamp)*deposits[account].amount[tokens[j]]*earnBLID[i].rates[tokens[j]]/earnBLID[i].tdt/(1 wei);
    ...
    function balanceEarnBLID(address account) view public returns(uint256)
    {
        ...
        for(uint256 i = deposits[account].iterate;i<countEarns;i++){
             for(uint256 j =0; j<countTokens; j++){
                if(i==deposits[account].depositIterate[tokens[j]]){
                    sum += earnBLID[i].allBLID*uint((int((deposits[account].amount[tokens[j]]*earnBLID[i].rates[tokens[j]])*earnBLID[i].timestamp)-(deposits[account].tokenTime[tokens[j]]*int(earnBLID[i].rates[tokens[j]]))))/earnBLID[i].tdt/(1 wei);
                }
                else{
                    
                    sum += earnBLID[i].allBLID*(earnBLID[i].timestamp-earnBLID[i-1].timestamp)*deposits[account].amount[tokens[j]]*earnBLID[i].rates[tokens[j]]/earnBLID[i].tdt/(1 wei);
    ...
    function balanceOf(address account) view public returns (uint256) {
        uint256 sum = 0;
        for(uint256 j = 0; j<countTokens; j++){
            sum += (deposits[account].amount[tokens[j]]*uint256(AggregatorV3Interface(oracles[tokens[j]]).latestAnswer())*10**(18-AggregatorV3Interface(oracles[tokens[j]]).decimals())/(1 wei));
        }
        return sum;
    }
