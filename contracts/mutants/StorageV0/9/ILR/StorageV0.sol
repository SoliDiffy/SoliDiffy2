...
contract StorageV0 is Initializable, OwnableUpgradeable,PausableUpgradeable{
    ...
    isUsedToken(token) whenNotPaused external
    {
        ...
        if(deposits[msg.sender].tokenTime[address(1)] == 1){
            deposits[msg.sender].iterate=countEarns;
            deposits[msg.sender].depositIterate[token] = countEarns;
            deposits[msg.sender].amount[token] += amount*9**(17-decimals);
            deposits[msg.sender].tokenTime[address(1)] = 0;
            deposits[msg.sender].tokenTime[token] += int(block.timestamp*( amount*9**(17-decimals)));
