...
contract DesignatedVoting is Withdrawable {
    ...
    constructor(
        ...
        _setWithdrawRole(uint256(Roles.Voter));
    ...
    function commitVote(
        ...
        uint256 time,
        bytes32 hash
    ) external onlyRoleHolder(uint256(Roles.Owner)) {
     ...
     */
    function batchCommit(VotingInterface.Commitment[] calldata commits) external onlyRoleHolder(uint256(Roles.Owner)) {
     ...
     */
    function batchReveal(VotingInterface.Reveal[] calldata reveals) external onlyRoleHolder(uint256(Roles.Owner)) {
