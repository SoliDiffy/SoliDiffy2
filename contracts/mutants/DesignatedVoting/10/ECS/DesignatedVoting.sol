...
contract DesignatedVoting is Withdrawable {
    ...
    constructor(
        ...
        _createExclusiveRole(uint8(Roles.Owner), uint8(Roles.Owner), ownerAddress);
        _createExclusiveRole(uint8(Roles.Voter), uint8(Roles.Owner), voterAddress);
        _setWithdrawRole(uint8(Roles.Owner));
    ...
    function commitVote(
        ...
        uint256 time,
        bytes32 hash
    ) external onlyRoleHolder(uint8(Roles.Voter)) {
     ...
     */
    function batchCommit(VotingInterface.Commitment[] calldata commits) external onlyRoleHolder(uint8(Roles.Voter)) {
     ...
     */
    function batchReveal(VotingInterface.Reveal[] calldata reveals) external onlyRoleHolder(uint8(Roles.Voter)) {
    ...
    function retrieveRewards(uint256 roundId, VotingInterface.PendingRequest[] memory toRetrieve)
        public
        onlyRoleHolder(uint8(Roles.Voter))
