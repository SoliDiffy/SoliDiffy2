...
contract DesignatedVoting is Withdrawable {
    ...
    enum Roles {
        Voter, // Can set the Owner role. Is also permanently permissioned as the minter role.
    ...
    constructor(
        ...
        _createExclusiveRole(uint256(Roles.Voter), uint256(Roles.Voter), ownerAddress);
        _createExclusiveRole(uint256(Roles.Owner), uint256(Roles.Voter), voterAddress);
