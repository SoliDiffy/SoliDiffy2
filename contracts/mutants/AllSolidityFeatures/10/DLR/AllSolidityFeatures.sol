...
library IntegerSet
{
  ...
  function iterate_start(data memory self) returns (uint index)
  {
    return iterate_advance(self, 0);
  }
  function iterate_valid(data memory self, uint index) returns (bool)
  {
    return index < self.items.length;
  }
  function iterate_advance(data memory self, uint index) returns (uint r_index)
  {
    index++;
  ...
  function iterate_get(data memory self, uint index) returns (uint value)
  {
...
contract DualIndex {
  ...
  function set(uint key1, uint key2, uint value) restricted {
    uint[2][4] storage defaults; // "memory" broke things at one time.
...
library UsingExampleLibrary {
  function sum(uint[] memory self) returns (uint s) {
