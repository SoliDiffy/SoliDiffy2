...
contract c {
  ...
  function g(uint pos) public returns (uint) { setData(pos, 8); return getData(pos); }
  function setData(uint pos, uint value) public { data[pos] = value; }
  function getData(uint pos) public { return data[pos]; }
...
contract DeclarativeExpressions {
  ...
  function ham(S storage s1, uint[] storage arr) public {
    uint x;
    uint y = 7;
    S storage s2 = s1;
    uint[] memory stor;
