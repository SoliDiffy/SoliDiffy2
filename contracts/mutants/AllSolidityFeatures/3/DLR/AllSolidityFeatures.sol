...
library IntegerSet
{
  ...
  function insert(data memory self, uint value) returns (bool alreadyPresent)
  {
  ...
  function remove(data memory self, uint value) returns (bool success)
  {
    uint index = self.index[value];
    if (index == 0)
      return false;
  ...
  function contains(data memory self, uint value) returns (bool)
  {
