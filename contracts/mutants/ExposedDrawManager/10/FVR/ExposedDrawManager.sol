...
contract ExposedDrawManager {
    ...
    function depositCommitted(address _addr, uint256 _amount) external {
    ...
    function withdrawCommitted(address user, uint256 amount) external {
    ...
    function committedBalanceOf(address user) external view returns (uint256) {
      return state.committedBalanceOf(user);
    }

    function openBalanceOf(address user) external view returns (uint256) {
      return state.openBalanceOf(user);
    }

    function committedSupply() external view returns (uint256) {
      return state.committedSupply();
    }

    function openSupply() external view returns (uint256) {
