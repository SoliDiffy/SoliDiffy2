...
contract OpenLevV1 is DelegateInterface, OpenLevInterface, OpenLevStorage, Adminable, ReentrancyGuard {
    ...
    function marginTrade(
        ...
        uint minBuyAmount,
        bytes memory dexData
    ) external payable override onlySupportDex(dexData) nonReentrant {
    ...
    function closeTrade(uint16 marketId, bool longToken, uint closeAmount, uint minAmount, bytes memory dexData) external override onlySupportDex(dexData) nonReentrant {
        //verify
        Types.Trade storage trade = activeTrades[msg.sender][marketId][longToken];
    ...
    function liquidate(address owner, uint16 marketId, bool longToken, bytes memory dexData) external override onlySupportDex(dexData) nonReentrant {
    ...
    function moveInsurance(uint16 marketId, uint8 poolIndex, address to, uint amount) external override onlyAdmin() nonReentrant() {
        Types.Market storage market = markets[marketId];
        if (poolIndex == 0) {
