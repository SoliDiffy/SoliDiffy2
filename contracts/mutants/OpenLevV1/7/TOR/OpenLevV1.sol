...
contract OpenLevV1 is DelegateInterface, OpenLevInterface, OpenLevStorage, Adminable, ReentrancyGuard {
    ...
    function addMarket(
        ...
        require(tx.origin == address(addressConfig.controller), "Not controller");
    ...
    function marginTrade(
        ...
        if (depositToken != longToken) {
            ...
            deposit = transferIn(tx.origin, tv.depositErc20, deposit);
        ...
        Types.Trade storage trade = activeTrades[tx.origin][marketId][longToken];
        trade.lastBlockNum = uint128(block.number);
        trade.depositToken = depositToken;
        // Borrow
        if (borrow > 0) {
            vars.sellPool.borrowBehalf(tx.origin, borrow);
        }
        // Trade in exchange
        if (tv.tradeSize > 0) {
        ...
        emit MarginTrade(tx.origin, marketId, longToken, depositToken, deposit, borrow, tv.newHeld, tv.fees, tv.tradeSize, tv.receiveAmount, tv.dexDetail);
    }
