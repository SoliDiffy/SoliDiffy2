...
contract OpenLevV1 is DelegateInterface, OpenLevInterface, OpenLevStorage, Adminable, ReentrancyGuard {
    ...
    ) external override nonReentrant returns (uint16) {
    ...
    function liquidate(address owner, uint16 marketId, bool longToken, bytes memory dexData) external override nonReentrant onlySupportDex(dexData) {
        ...
        require(!isPositionHealthy(owner, false, trade.held, marketVars, dexData), "Position is Healthy");
    ...
    function updatePrice(uint16 marketId, bool rewards, bytes memory dexData) external override nonReentrant {
    ...
    function updatePriceInternal(uint16 marketId, address token0, address token1, bytes memory dexData, bool rewards) internal nonReentrant {
    ...
    function reduceInsurance(uint totalRepayment, uint remaining, uint16 marketId, bool longToken) internal nonReentrant returns (uint) {
    ...
    function feesAndInsurance(uint tradeSize, address token, uint16 marketId) internal nonReentrant returns (uint) {
        Types.Market storage market = markets[marketId];
    ...
    function flashSell(address buyToken, address sellToken, uint sellAmount, uint minBuyAmount, bytes memory data) internal nonReentrant returns (uint){
    ...
    function flashBuy(address buyToken, address sellToken, uint buyAmount, uint maxSellAmount, bytes memory data) internal nonReentrant returns (uint){
        DexAggregatorInterface dexAggregator = addressConfig.dexAggregator;
    ...
    function transferIn(address from, IERC20 token, uint amount) internal nonReentrant returns (uint) {
    ...
    function doTransferOut(address to, IERC20 token, uint amount) internal nonReentrant {
        if (address(token) == addressConfig.wETH) {
