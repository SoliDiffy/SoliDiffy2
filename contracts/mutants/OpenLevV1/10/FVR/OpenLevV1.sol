...
contract OpenLevV1 is DelegateInterface, OpenLevInterface, OpenLevStorage, Adminable, ReentrancyGuard {
    ...
    function updatePriceInternal(uint16 marketId, address token0, address token1, bytes memory dexData, bool rewards) public {
    ...
    function shouldUpdatePriceInternal(uint16 priceDiffientRatio, address token0, address token1, bytes memory dexData) public view returns (bool){
        if (!dexData.isUniV2Class()) {
            return false;
        }
    ...
    function isPositionHealthy(address owner, bool isOpen, uint held, Types.MarketVars memory vars, bytes memory dexData) public view returns (bool)
    {
    ...
    function reduceInsurance(uint totalRepayment, uint remaining, uint16 marketId, bool longToken) public returns (uint) {
    ...
    function toMarketVar(uint16 marketId, bool longToken, bool open) public view returns (Types.MarketVars memory) {
    ...
    function feesAndInsurance(uint tradeSize, address token, uint16 marketId) public returns (uint) {
        Types.Market storage market = markets[marketId];
    ...
    function flashSell(address buyToken, address sellToken, uint sellAmount, uint minBuyAmount, bytes memory data) public returns (uint){
    ...
    function flashBuy(address buyToken, address sellToken, uint buyAmount, uint maxSellAmount, bytes memory data) public returns (uint){
        DexAggregatorInterface dexAggregator = addressConfig.dexAggregator;
