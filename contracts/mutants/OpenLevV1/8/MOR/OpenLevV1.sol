...
contract OpenLevV1 is DelegateInterface, OpenLevInterface, OpenLevStorage, Adminable, ReentrancyGuard {
    ...
    function marginTrade(
        ...
        uint minBuyAmount,
        bytes memory dexData
    ) external payable override onlyAdmin() onlySupportDex(dexData) {
    ...
    function closeTrade(uint16 marketId, bool longToken, uint closeAmount, uint minAmount, bytes memory dexData) external override onlyAdmin() onlySupportDex(dexData) {
        //verify
        Types.Trade storage trade = activeTrades[msg.sender][marketId][longToken];
    ...
    function liquidate(address owner, uint16 marketId, bool longToken, bytes memory dexData) external override onlyAdmin() onlySupportDex(dexData) {
    ...
    function marginRatio(address owner, uint16 marketId, bool longToken, bytes memory dexData) external override nonReentrant view returns (uint current, uint cAvg, uint hAvg, uint32 limit) {
    ...
    function setCalculateConfig(uint16 defaultFeesRate,
        ...
        uint16 feesDiscount,
        uint128 feesDiscountThreshold) external override nonReentrant {
    ...
    function setMarketConfig(uint16 marketId, uint16 feesRate, uint16 marginLimit, uint16 priceDiffientRatio, uint32[] memory dexs) external override nonReentrant {
        Types.Market storage market = markets[marketId];
    ...
    function setAllowedDepositTokens(address[] memory tokens, bool allowed) external override nonReentrant {
