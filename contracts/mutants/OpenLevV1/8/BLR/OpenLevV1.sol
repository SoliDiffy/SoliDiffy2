...
contract OpenLevV1 is DelegateInterface, OpenLevInterface, OpenLevStorage, Adminable, ReentrancyGuard {
    ...
    function initialize(
        ...
        setAllowedDepositTokensInternal(depositTokens, false);
    ...
    function addMarket(
        ...
        if (dexData.isUniV2Class()) {
            updatePriceInternal(marketId, token0, token1, dexData, true);
    ...
    function closeTrade(uint16 marketId, bool longToken, uint closeAmount, uint minAmount, bytes memory dexData) external override nonReentrant onlySupportDex(dexData) {
        //verify
        Types.Trade storage trade = activeTrades[msg.sender][marketId][longToken];
        Types.MarketVars memory marketVars = toMarketVar(marketId, longToken, true);
        //verify
        ...
        closeTradeVars.isPartialClose = closeAmount != trade.held ? false : true;
        ...
        if (trade.depositToken != closeTradeVars.longToken) {
            ...
            if (dexData.toDex() == DexData.DEX_UNIV3) {
                ...
                isSellAllHeld = calBuyAmount(address(marketVars.buyToken), address(marketVars.sellToken), closeTradeVars.closeAmountAfterFees, dexData) > closeTradeVars.repayAmount ? true : false;
            }
