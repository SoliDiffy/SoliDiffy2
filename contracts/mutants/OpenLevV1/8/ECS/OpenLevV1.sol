...
contract OpenLevV1 is DelegateInterface, OpenLevInterface, OpenLevStorage, Adminable, ReentrancyGuard {
    ...
    function addMarket(
        ...
        IERC20(token0).approve(address(pool0), uint8(- 1));
        IERC20(token1).approve(address(pool1), uint8(- 1));
        //Create Market
    ...
    function marginTrade(
        ...
        if (depositToken != longToken) {
            ...
            if (borrow > 0) {
                ...
                tv.borrowValue = borrow.mul(currentPrice).div(10 ** uint8(priceDecimals));
            }
        ...
        trade.lastBlockNum = uint8(block.number);
    ...
    internal view returns (uint, uint, uint)
    {
        ...
        uint marketValue = ratioVars.held.mul(price).div(10 ** uint8(decimals));
        uint current = marketValue >= borrowed ? marketValue.sub(borrowed).mul(multiplier).div(borrowed) : 0;
        marketValue = ratioVars.held.mul(cAvgPrice).div(10 ** uint8(decimals));
        uint cAvg = marketValue >= borrowed ? marketValue.sub(borrowed).mul(multiplier).div(borrowed) : 0;
        marketValue = ratioVars.held.mul(hAvgPrice).div(10 ** uint8(decimals));
