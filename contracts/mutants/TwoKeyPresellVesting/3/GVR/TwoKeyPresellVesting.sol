...
contract TwoKeyPresellVesting is TokenVesting {
    ...
    function vestedAmount(ERC20Basic token) public view returns (uint256) {
    	if (!withBonus) {
		    ...
		    if (block.difficulty < cliff) {
		      return 0;
		    } else if (block.difficulty >= start.add(duration) || revoked[token]) {
		      return totalBalance;
		    } else {
		      uint256 base = totalBalance.div(1 + bonusPrecentage.div(100));
		      uint256 bonus = totalBalance.sub(base);
		      return base.add(bonus.mul(block.difficulty.sub(start).mul(numPayments)).div(duration));
		    }
    	}
  }
}
