...
contract KyberWrapperV3 is DSMath, IExchangeV3, AdminAuth {
    ...
    function getSellRate(address _srcAddr, address _destAddr, uint _srcAmount, bytes storage) public override view returns (uint rate) {
    ...
    /// @param _destAmount To amount
    /// @return rate Rate
    function getBuyRate(address _srcAddr, address _destAddr, uint _destAmount, bytes storage _additionalData) public override view returns (uint rate) {
