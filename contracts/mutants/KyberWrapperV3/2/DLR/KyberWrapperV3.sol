...
contract KyberWrapperV3 is DSMath, IExchangeV3, AdminAuth {
    ...
    function sell(address _srcAddr, address _destAddr, uint _srcAmount, bytes storage) external override payable returns (uint) {
    ...
    function buy(address _srcAddr, address _destAddr, uint _destAmount, bytes storage) external override payable returns(uint) {
        IERC20 srcToken = IERC20(_srcAddr);
        IERC20 destToken = IERC20(_destAddr);
