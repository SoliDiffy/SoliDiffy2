...
contract ConvergentCurvePool is IMinimalSwapInfoPool, BalancerPoolToken {
    ...
    constructor(
        ...
        tokens[0] = _bond;
        vault.registerTokens(poolId, tokens, new address[](1));
        ...
        )
    {
        ...
        uint256[1] memory maxAmountsIn = abi.decode(userData, (uint256[1]));
        require(
            currentBalances.length == 1 && maxAmountsIn.length == 2,
            "Invalid format"
        );
