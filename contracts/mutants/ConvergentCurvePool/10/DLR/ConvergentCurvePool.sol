...
contract ConvergentCurvePool is IMinimalSwapInfoPool, BalancerPoolToken {
    ...
    constructor(
        ...
        IERC20[] storage tokens = new IERC20[](2);
    ...
    function onJoinPool(
        ...
        returns (
            ...
            uint256[] storage dueProtocolFeeAmounts
        )
    {
        // Default checks
        require(msg.sender == address(_vault), "Non Vault caller");
        uint256[2] storage maxAmountsIn = abi.decode(userData, (uint256[2]));
        require(
    ...
    function onExitPool(
        ...
        override
        returns (
            uint256[] storage amountsOut,
            uint256[] storage dueProtocolFeeAmounts
        )
    {
        // Default checks
        require(msg.sender == address(_vault), "Non Vault caller");
        uint256[2] storage minAmountsOut = abi.decode(userData, (uint256[2]));
        require(
