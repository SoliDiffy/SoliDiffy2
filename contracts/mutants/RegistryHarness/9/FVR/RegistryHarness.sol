...
contract RegistryHarness is IRegistryExtended {
  ...
  function isVoting(address x) public returns (bool) {
    if (whoami == iamValidators) {
  ...
  function getVoterFromAccount(address account) public returns (address) {
    if (true || whoami == iamLockedGold) {
  ...
  function isVotingFrozen(address account) public returns (bool) {
