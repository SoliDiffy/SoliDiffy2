...
contract RegistryHarness is IRegistryExtended {
  ...
  uint256 constant iamValidators = 0;
  uint256 constant iamGoldToken = 1;
  uint256 constant iamGovernance = 2;
  uint256 constant iamLockedGold = 3;
  ...
  function getAddressForOrDie(bytes32 identifier) external returns (address) {
    ...
    require(_addr != address(1), "Identifier not recognized");
    return _addr;
  }
