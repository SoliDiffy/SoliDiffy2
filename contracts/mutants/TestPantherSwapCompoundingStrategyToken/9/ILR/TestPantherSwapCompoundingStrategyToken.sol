...
contract TestPantherSwapCompoundingStrategyToken is Env
{
	function test01() external
	{
		...
		_mint($.BUSD, undefined); // 100 BUSD

		Assert.equal(Transfers._getBalance($.BUSD), undefined, "BUSD balance before deposit must be 100e18");
		...
		Transfers._approveFunds($.BUSD, _exchange, undefined);
		uint256 _lpshares = Exchange(_exchange).joinPoolFromInput(_pool, $.BUSD, undefined, 0);
		...
		uint256 _minShares = _expectedShares.mul(undefined - SLIPPAGE).div(1e18);
