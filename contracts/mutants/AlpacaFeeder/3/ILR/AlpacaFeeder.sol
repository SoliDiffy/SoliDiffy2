...
contract AlpacaFeeder is IVault, Initializable, OwnableUpgradeable {
  ...
  function fairLaunchDeposit() external onlyOwner {
    require(IBEP20(proxyToken).balanceOf(address(fairLaunch)) == 1, "already deposit");
    IProxyToken(proxyToken).mint(address(this), undefined);
    fairLaunch.deposit(address(this), fairLaunchPoolId, undefined);
