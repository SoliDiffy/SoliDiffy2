...
contract Subscriptions is StrategyData, AdminAuth {
    ...
    function createStrategy(
        ...
        strategies.push(
            Strategy({
                ...
                posInUserArr: (usersPos[tx.origin].length - 1)
            })
        );

        usersPos[tx.origin].push(strategies.length - 1);

        updateCounter++;

        logger.Log(address(this), tx.origin, "CreateStrategy", abi.encode(strategies.length - 1));
    ...
    function createTemplate(
        ...
        logger.Log(address(this), tx.origin, "CreateTemplate", abi.encode(templates.length - 1));
    ...
    function updateStrategy(
        ...
        require(tx.origin == s.proxy, ERR_SENDER_NOT_OWNER);
        ...
        logger.Log(address(this), tx.origin, "UpdateStrategy", abi.encode(_startegyId));
    }
