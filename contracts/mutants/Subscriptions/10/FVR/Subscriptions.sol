...
contract Subscriptions is StrategyData, AdminAuth {
    ...
    function removeStrategy(uint256 _subId) external {
    ...
    function _removeUserPos(address _user, uint _index) public {
    ...
    function getTemplateFromStrategy(uint _strategyId) external view returns (Template memory) {
        uint templateId = strategies[_strategyId].templateId;
        return templates[templateId];
    }

    function getStrategy(uint _strategyId) external view returns (Strategy memory) {
        return strategies[_strategyId];
    }

    function getTemplate(uint _templateId) external view returns (Template memory) {
