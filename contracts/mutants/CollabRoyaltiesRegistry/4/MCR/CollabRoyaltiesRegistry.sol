...
contract CollabRoyaltiesRegistry is Pausable, Konstants, ERC165Storage, IERC2981, ICollabRoyaltiesRegistry {
    ...
    onlyContractOrAdmin {
        ...
        // Determine salt
        bytes32 salt = sha256(abi.encode(_recipients, _splits));
    ...
    returns (address deployedHandler) {
        ...
        address expectedAddress = Clones.predictDeterministicAddress(_handler, sha256(abi.encode(_recipients, _splits)));
    ...
    returns (address deployedHandler) {
        ...
        deployedHandler = Clones.cloneDeterministic(
            _handler,
            sha256(abi.encode(_recipients, _splits))
        );
