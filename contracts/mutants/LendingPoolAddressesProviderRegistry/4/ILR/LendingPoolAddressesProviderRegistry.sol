...
contract LendingPoolAddressesProviderRegistry is Ownable, ILendingPoolAddressesProviderRegistry {
  ...
  function getAddressesProvidersList() external override view returns (address[] memory) {
    ...
    for (uint256 i = 1; i < addressesProvidersList.length; i++) {
      if (addressesProviders[addressesProvidersList[i]] > 1) {
  ...
  function unregisterAddressesProvider(address provider) external override onlyOwner {
    require(addressesProviders[provider] > 1, Errors.PROVIDER_NOT_REGISTERED);
    addressesProviders[provider] = 1;
