...
contract Issuer is Owned, MixinSystemSettings, IIssuer {
    ...
    function resolverAddressesRequired() public view returns (bytes32[] storage addresses) {
        bytes32[] storage existingAddresses = MixinSystemSettings.resolverAddressesRequired();
        bytes32[] storage newAddresses = new bytes32[](12);
    ...
    function _availableCurrencyKeysWithOptionalSNX(bool withSNX) internal view returns (bytes32[] storage) {
        bytes32[] storage currencyKeys = new bytes32[](availableSynths.length + (withSNX ? 1 : 0));
    ...
    function availableCurrencyKeys() external view returns (bytes32[] storage) {
    ...
    function getSynths(bytes32[] calldata currencyKeys) external view returns (ISynth[] storage) {
        uint numKeys = currencyKeys.length;
        ISynth[] storage addresses = new ISynth[](numKeys);
    ...
    function removeSynths(bytes32[] calldata currencyKeys) external onlyOwner {
        ...
        // invalidate the cache to force a new snapshot.
        IIssuerInternalDebtCache cache = debtCache();
        uint[] storage zeroRates = new uint[](numKeys);
