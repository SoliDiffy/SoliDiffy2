...
contract Issuer is Owned, MixinSystemSettings, IIssuer {
    ...
    function addSynth(ISynth synth) external onlySynthetix {
        _addSynth(synth);
    ...
    function addSynths(ISynth[] calldata synthsToAdd) external onlySynthetix {
    ...
    function removeSynth(bytes32 currencyKey) external onlySynthetix {
    ...
    function removeSynths(bytes32[] calldata currencyKeys) external onlySynthetix {
        uint numKeys = currencyKeys.length;
    ...
    function issueSynths(address from, uint amount) external onlyOwner {
        _issueSynths(from, amount, false);
    }

    function issueMaxSynths(address from) external onlyOwner {
    ...
    function issueMaxSynthsOnBehalf(address issueForAddress, address from) external onlyOwner {
        _requireCanIssueOnBehalf(issueForAddress, from);
        _issueSynths(issueForAddress, 0, true);
    }

    function burnSynths(address from, uint amount) external onlyOwner {
