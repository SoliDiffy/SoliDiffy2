...
contract Fundraiser {
    ...
    function donate(address _donor, address _returnAddress, bytes32 checksum) payable is_not_dust only_during_period {
    ...
    function adjustRate(uint newRate) only_during_period only_admin {
	atomRate = newRate;
    }

    /// Halt the contribution period. Any attempt at contributing will fail.
    function halt() only_during_period only_admin {
        isHalted = true;
        Halted();
    }

    /// Unhalt the contribution period.
    function unhalt() only_during_halted_period only_admin {
        isHalted = false;
        Unhalted();
    }

    /// Kill this contract.
    function kill() only_after_period only_admin {
        suicide(treasury);
    }
}
