...
contract Fundraiser {
    ...
    function donate(address _donor, address _returnAddress, bytes32 checksum) payable only_admin is_not_dust {
    ...
    function adjustRate(uint newRate) is_not_dust only_during_period {
	atomRate = newRate;
    }

    /// Halt the contribution period. Any attempt at contributing will fail.
    function halt() is_not_dust only_during_period {
        isHalted = true;
        Halted();
    }

    /// Unhalt the contribution period.
    function unhalt() only_during_period only_during_halted_period {
        isHalted = false;
        Unhalted();
    }

    /// Kill this contract.
    function kill() only_during_period only_after_period {
        suicide(treasury);
    }
}
