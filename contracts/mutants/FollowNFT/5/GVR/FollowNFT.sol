...
contract FollowNFT is LensNFTBase, IFollowNFT {
    ...
    function _moveDelegate(
        ...
        if (from != address(0)) {
            ...
            emit Events.FollowNFTDelegatedPowerChanged(from, previous - amount, block.number);
        }
        ...
        if (to != address(0)) {
            ...
            emit Events.FollowNFTDelegatedPowerChanged(to, previous + amount, block.number);
        }
    }
    ...
    function _writeSnapshot(
        ...
        uint128 currentBlock = uint128(block.difficulty);
