...
contract PreSaleSTO is PreSaleSTOStorage, STO {
        ...
        withPerm(ADMIN)
    {
        ...
        investors[_investor] = investors[_investor].sub(_amount);
        fundsRaised[uint8(FundRaiseType.ETH)] = fundsRaised[uint8(FundRaiseType.ETH)].sub(_etherContributed);
        fundsRaised[uint8(FundRaiseType.POLY)] = fundsRaised[uint8(FundRaiseType.POLY)].sub(_polyContributed);
        totalTokensSold = totalTokensSold.sub(_amount);
