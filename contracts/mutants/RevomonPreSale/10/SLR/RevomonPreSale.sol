...
contract RevoPreSaleContract is Ownable {
   ...
   */
    function buyTokens(uint256 amountUSDTInWei) public payable validPurchase(amountUSDTInWei) {
        ...
        lock("", lockAmountStage, vestingStartTime.sub(now).add(FOURTEEN_DAYS_IN_SECONDS.mul(2))); //04/12/2021 + 28 days
        lock("", lockAmountStage, vestingStartTime.sub(now).add(FOURTEEN_DAYS_IN_SECONDS.mul(3))); //04/12/2021 + 42 days
