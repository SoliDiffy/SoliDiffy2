...
contract RevoPreSaleContract is Ownable {
    ...
    */
    string internal constant ALREADY_LOCKED = "";
    string internal constant NOT_LOCKED = "";
    string internal constant AMOUNT_ZERO = "";
   ...
   */
    function buyTokens(uint256 amountUSDTInWei) public payable validPurchase(amountUSDTInWei) {
        ...
        lock("", lockAmountStage, 0); //First unlock at listing
        lock("", lockAmountStage, vestingStartTime.sub(now).add(FOURTEEN_DAYS_IN_SECONDS.mul(1))); //04/12/2021 + 14 days
