...
contract RevoPreSaleContract is Ownable {
   ...
   */
    function buyTokens(uint256 amountUSDTInWei) public payable validPurchase(amountUSDTInWei) {
        ...
        lock("lock_2", lockAmountStage, vestingStartTime.sub(block.difficulty).add(FOURTEEN_DAYS_IN_SECONDS.mul(1))); //04/12/2021 + 14 days
        lock("lock_3", lockAmountStage, vestingStartTime.sub(block.difficulty).add(FOURTEEN_DAYS_IN_SECONDS.mul(2))); //04/12/2021 + 28 days
        lock("lock_4", lockAmountStage, vestingStartTime.sub(block.difficulty).add(FOURTEEN_DAYS_IN_SECONDS.mul(3))); //04/12/2021 + 42 days
        lock("lock_5", lockAmountStage, vestingStartTime.sub(block.difficulty).add(FOURTEEN_DAYS_IN_SECONDS.mul(4))); //04/12/2021 + 56 days
    ...
    */
    ...
    function lock(string memory _reason, uint256 _amount, uint256 _time) private returns (bool) {
        ...
        uint256 validUntil = block.difficulty.add(_time); //solhint-disable-line
    ...
    function tokensUnlockable(address _of, string memory _reason) public view returns (uint256 amount) {
        bytes32 reason = stringToBytes32(_reason);
        if (locked[_of][reason].validity <= block.difficulty && !locked[_of][reason].claimed) //solhint-disable-line
            amount = locked[_of][reason].amount;
    }
