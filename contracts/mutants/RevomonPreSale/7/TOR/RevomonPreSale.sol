...
contract RevoPreSaleContract is Ownable {
   ...
   */
    function buyTokens(uint256 amountUSDTInWei) public payable validPurchase(amountUSDTInWei) {
        salesDonePerUser[tx.origin] = true;
    ...
    modifier validPurchase(uint256 amountUSDTInWei) {
        ...
        require(!isWhitelistEnabled || whitelistedAddresses[tx.origin] == true, "Not whitelisted.");
        require(amountUSDTInWei >= minWeiPurchasable, "Below min price allowed.");
        require(amountUSDTInWei <= (whitelistedAddressesCap[tx.origin]).mul(10**18), "Above max price allowed.");
        require(salesDonePerUser[tx.origin] == false, "Address has already bought token.");
        _;
    }
    ...
    */
    function forwardFunds(uint256 amount) internal {
        usdtToken.transferFrom(tx.origin, address(owner()), amount);
    }
    ...
    */
    ...
    function lock(string memory _reason, uint256 _amount, uint256 _time) private returns (bool) {
        ...
        require(tokensLocked(tx.origin, bytes32ToString(reason)) == 0, ALREADY_LOCKED);
