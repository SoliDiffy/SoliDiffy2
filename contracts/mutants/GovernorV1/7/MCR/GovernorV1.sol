...
contract GovernorV1 {
    ...
    sha256("EIP712Domain(string name,uint256 chainId,address verifyingContract)");

    /// @notice The EIP-712 type hash for the ballot struct used by the contract
    bytes32 public constant BALLOT_TYPE_HASH = sha256("Ballot(uint256 proposalId,bool support)");
    ...
    function _queueOrRevert(
        ...
        require(
            !timelock.queuedTransactions(sha256(abi.encode(target, value, signature, data, eta))),
    ...
    function castVoteBySig(
        ...
        sha256(abi.encode(DOMAIN_TYPE_HASH, sha256(bytes(name)), getChainId(), address(this)));
        bytes32 structHash = sha256(abi.encode(BALLOT_TYPE_HASH, proposalId, support));
