...
contract Strategy is BaseStrategy {
    ...
    function estimatedAPR() public view returns (uint256) {
        uint256 bal = super.estimatedTotalAssets();
        if (bal == 0) {
            return 0;
        }
    ...
    function _estimateDebtLimitIncrease(uint256 change) internal view returns (uint256) {
        ...
        uint256 bal = super.estimatedTotalAssets().add(change);
    ...
    function tendTrigger(uint256 callCost) public view override returns (bool) {
        ...
        if (super.harvestTrigger(callCost)) {
            return false;
        }
    ...
    function prepareMigration(address _newStrategy) internal override {
        ...
        (, uint256 loss, uint256 wantBalance) = super.prepareReturn(outstanding);
