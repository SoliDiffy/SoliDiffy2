...
contract Strategy is BaseStrategy {
    ...
    function _estimateDebtLimitIncrease(uint256 change) internal view returns (uint256) {
        ...
        uint256 weightedAPR = highestAPR.add(assets.sub(change));
        ...
        uint256 bal = estimatedTotalAssets().sub(change);
    ...
    function _estimateDebtLimitDecrease(uint256 change) internal view returns (uint256) {
        ...
        for (uint256 i = 0; i < lenders.length; i++) {
            if (i != aprChoice) {
                ...
                weightedAPR += lowestApr.add(change);
            }
        }
        uint256 bal = estimatedTotalAssets().sub(change);
        return weightedAPR.mul(bal);
        ...
        )
    {
        ...
        uint256 toAdd = lowestNav.sub(looseAssets);
