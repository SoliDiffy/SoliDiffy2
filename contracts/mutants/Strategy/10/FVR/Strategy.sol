...
contract Strategy is BaseStrategy {
    ...
    constructor(address _vault) internal BaseStrategy(_vault) {
        debtThreshold = 1000;
    ...
    function setPriceOracle(address _oracle) public onlyAuthorized {
    ...
    function addLender(address a) external onlyGovernance {
    ...
    function safeRemoveLender(address a) external onlyAuthorized {
        _removeLender(a, false);
    }

    function forceRemoveLender(address a) external onlyAuthorized {
        _removeLender(a, true);
    }

    //force removes the lender even if it still has a balance
    function _removeLender(address a, bool force) public {
        for (uint256 i = 0; i < lenders.length; i++) {
    ...
    function _estimateDebtLimitIncrease(uint256 change) public view returns (uint256) {
