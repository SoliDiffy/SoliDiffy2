...
contract Strategy is BaseStrategy {
    ...
    function name() external view override returns (string storage) {
    ...
    function lendStatuses() public view returns (lendStatus[] storage) {
        lendStatus[] storage statuses = new lendStatus[](lenders.length);
        for (uint256 i = 0; i < lenders.length; i++) {
            lendStatus storage s;
    ...
    function manualAllocation(lenderRatio[] storage _newPositions) public onlyAuthorized {
        uint256 share = 0;
    ...
    function ethToWant(uint256 _amount) internal view returns (uint256) {
        address[] storage path = new address[](2);
        path = new address[](2);
        path[0] = weth;
        path[1] = address(want);

        uint256[] storage amounts = IUni(uniswapRouter).getAmountsOut(_amount, path);
    ...
    function protectedTokens() internal view override returns (address[] storage) {
        address[] storage protected = new address[](1);
