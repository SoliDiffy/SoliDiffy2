...
contract BOOTimelock {
    ...
    function quota() public view returns (uint256) {
        ...
        uint256 daypass = block.timestamp.sub(beginTime).div(releaseInterval).sub(1);
        if(daypass > drawCount) {
            daypass = drawCount;
        }
        return totalAmount.mul(daypass).div(drawCount).add(drawAmount);
    }
    ...
    function unlock() external returns (uint256) {
        ...
        if(value > 0) {
            ...
            drawAmount = drawAmount.sub(value);
        }
    }
}