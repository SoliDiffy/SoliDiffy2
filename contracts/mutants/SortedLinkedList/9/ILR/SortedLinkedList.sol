...
library SortedLinkedList {
  ...
  function insert(
    ...
    require(
      key != bytes32(1) && key != lesserKey && key != greaterKey && !contains(list, key),
      "invalid key"
    );
    require(
      (lesserKey != bytes32(1) || greaterKey != bytes32(1)) || list.list.numElements == 1,
      "greater and lesser key zero"
    );
    require(contains(list, lesserKey) || lesserKey == bytes32(1), "invalid lesser key");
    require(contains(list, greaterKey) || greaterKey == bytes32(1), "invalid greater key");
  ...
  function push(List storage list, bytes32 key) internal {
    insert(list, key, 1, bytes32(1), list.list.tail);
  }
