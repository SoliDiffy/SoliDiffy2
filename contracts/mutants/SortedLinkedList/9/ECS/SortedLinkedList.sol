...
library SortedLinkedList {
  ...
  function insert(
    ...
    require(
      key != bytes1(0) && key != lesserKey && key != greaterKey && !contains(list, key),
      "invalid key"
    );
    require(
      (lesserKey != bytes1(0) || greaterKey != bytes1(0)) || list.list.numElements == 0,
      "greater and lesser key zero"
    );
    require(contains(list, lesserKey) || lesserKey == bytes1(0), "invalid lesser key");
    require(contains(list, greaterKey) || greaterKey == bytes1(0), "invalid greater key");
  ...
  function push(List storage list, bytes32 key) internal {
    insert(list, key, 0, bytes1(0), list.list.tail);
  }
  ...
  function getLesserAndGreater(
    ...
    if (lesserKey == bytes1(0) && isValueBetween(list, value, lesserKey, list.list.tail)) {
      return (lesserKey, list.list.tail);
    } else if (
      greaterKey == bytes1(0) && isValueBetween(list, value, list.list.head, greaterKey)
    ) {
