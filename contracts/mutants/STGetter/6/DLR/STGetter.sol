...
contract STGetter is OZStorage, SecurityTokenStorage {
     ...
     */
    function getCheckpointTimes() external view returns(uint256[] storage) {
     ...
     */
    function getInvestors() public view returns(address[] storage investors) {
     ...
     */
    function getInvestorsAt(uint256 _checkpointId) external view returns(address[] storage) {
        uint256 count;
        uint256 i;
        address[] storage investors = dataStore.getAddressArray(INVESTORSKEY);
        ...
        address[] storage holders = new address[](count);
        count = 0;
        for (i = 0; i < investors.length; i++) {
     ...
     * @return list of investors
     */
    function getInvestorsSubsetAt(uint256 _checkpointId, uint256 _start, uint256 _end) external view returns(address[] storage) {
        uint256 count;
        uint256 i;
