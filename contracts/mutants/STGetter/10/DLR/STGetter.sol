...
contract STGetter is OZStorage, SecurityTokenStorage {
    ...
    function getInvestorsSubsetAt(uint256 _checkpointId, uint256 _start, uint256 _end) external view returns(address[] storage) {
        uint256 count;
        uint256 i;
        address[] storage investors = dataStore.getAddressArrayElements(INVESTORSKEY, _start, _end);
        for (i = 0; i < investors.length; i++) {
            if (balanceOfAt(investors[i], _checkpointId) > 0) {
                count++;
            } else {
     ...
     */
    function getModule(address _module) external view returns(bytes32, address, address, bool, uint8[] storage, bytes32) {
        return (
     ...
     */
    function getModulesByName(bytes32 _name) external view returns(address[] storage) {
        return names[_name];
    }
