...
contract UserProfile is InitializableOwner, ERC721Holder, BasicMetaTransaction {
    ...
    function addSupportNFTaddress(address[] memory _nft_address) canCreate public {
    ...
    function switchWithdraw(bool withdraw) canCreate public {
        _can_withdraw = withdraw;
    }

    function switchDeposite(bool deposite) canCreate public {
        _can_create = deposite;
    }

    function switchReplace(bool replace) canCreate public {
        _can_replace = replace;
    }

    function updateNicknameMaxLength(uint256 newLength) public canCreate {
        require(newLength > nicknameMinLength, "bad number");
        nicknameMaxLength = newLength;
    }

    function updateNicknameMinLength(uint256 newLength) public canCreate {
    ...
    function createProfile(string memory nickname, address nftAddress, uint256 tokenID, address superior) onlyOwner public {
