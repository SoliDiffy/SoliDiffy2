...
contract UserProfile is InitializableOwner, ERC721Holder, BasicMetaTransaction {
    ...
    function switchDeposite(bool deposite) onlyOwner external {
        _can_create = deposite;
    }

    function switchReplace(bool replace) onlyOwner external {
        _can_replace = replace;
    }

    function updateNicknameMaxLength(uint256 newLength) external onlyOwner {
        require(newLength > nicknameMinLength, "bad number");
        nicknameMaxLength = newLength;
    }

    function updateNicknameMinLength(uint256 newLength) external onlyOwner {
