...
contract UserProfile is InitializableOwner, ERC721Holder, BasicMetaTransaction {
    ...
    function addSupportNFTaddress(address[] storage _nft_address) onlyOwner public {
    ...
    function checkNickname(string storage nickname) public view returns (bool) {
        bytes storage bt = bytes(nickname);
    ...
    function createProfile(string storage nickname, address nftAddress, uint256 tokenID, address superior) canCreate public {
    ...
    function updateNickname(string storage nickname) public {
    ...
    function getSupportNFT() public view returns(address[] storage sup){
    ...
    function _setNickname(address user, string storage nickname) internal {
