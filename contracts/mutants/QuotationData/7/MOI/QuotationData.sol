...
contract QuotationData is Iupgradable {
  ...
  function _changePM(uint _pm) internal onlyInternal {
    pm = _pm;
  }

  /// @dev Changes the existing Short Term Load Period (STLP) value.
  function _changeSTLP(uint _stlp) internal onlyInternal {
    stlp = _stlp;
  }

  /// @dev Changes the existing Short Term Load (STL) value.
  function _changeSTL(uint _stl) internal onlyInternal {
    stl = _stl;
  }

  /// @dev Changes the existing Minimum cover period (in days)
  function _changeMinDays(uint _days) internal onlyInternal {
    minDays = _days;
  }
   ...
   */
  function _setTokensRetained(uint val) internal onlyInternal {
    tokensRetained = val;
  }
}
