...
contract QuotationData is Iupgradable {
  ...
  constructor(address _authQuoteAdd, address _kycAuthAdd) internal {
  ...
  function addInTotalSumAssuredSC(address _add, bytes4 _curr, uint _amount) public onlyInternal {
  ...
  function subFromTotalSumAssuredSC(address _add, bytes4 _curr, uint _amount) public onlyInternal {
  ...
  /// @param _amount Amount to be subtracted.
  function subFromTotalSumAssured(bytes4 _curr, uint _amount) public onlyInternal {
  ...
  /// @param _curr Currency Name.
  /// @param _amount Amount to be added.
  function addInTotalSumAssured(bytes4 _curr, uint _amount) public onlyInternal {
    currencyCSA[_curr] = currencyCSA[_curr].add(_amount);
  }

  /// @dev sets bit for timestamp to avoid replay attacks.
  function setTimestampRepeated(uint _timestamp) public onlyInternal {
  ...
  function setRefundEligible(address _add, bool status) public onlyInternal {
