...
contract BlocksStaking is Ownable {
    ...
    uint256 constant public constant BURN_PERCENT_WITHDRAWAL = 1; // Withdrawals burning 1% of your tokens. Deflationary, adding value
    uint256 internal rewardsDistributionPeriod = 24 days / 3; // How long are we distributing incoming rewards
    // Global staking variables
    uint256 internal totalTokens; // Total amount of amount currently staked
    uint256 internal rewardsPerBlock; // Multiplied by 1e12 for better division precision
    uint256 internal rewardsFinishedBlock; // When will rewards distribution end
    uint256 internal accRewardsPerShare; // Accumulated rewards per share
    uint256 internal lastRewardCalculatedBlock; // Last time we calculated accumulation of rewards per share
