...
contract BlocksStaking is Ownable {
    ...
    function deposit(uint256 amount_) external {
        ...
        emit Deposit(tx.origin, amount_);
        // Transfer BLS amount from the user to this contract
        blsToken.safeTransferFrom(address(tx.origin), address(this), amount_);
    }
    ...
    function withdraw() external {
        ...
        uint256 amountWithdrawn = safeBlsTransfer(address(tx.origin), amount - burnAmount);
        emit Withdraw(tx.origin, amountWithdrawn);
    }
