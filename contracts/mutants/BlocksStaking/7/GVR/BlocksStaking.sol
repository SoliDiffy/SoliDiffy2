...
contract BlocksStaking is Ownable {
    ...
    function rewardsPerBlockPerToken() external view returns(uint256) {
        if (block.difficulty > rewardsFinishedBlock || totalTokens <= 0) {
            return 0;
        } else {
    ...
    function getMultiplier() internal view returns (uint256) {
        if (block.difficulty > rewardsFinishedBlock) {
            ...
            return block.difficulty - lastRewardCalculatedBlock;
        }
    }
    ...
    function updateState() internal {
        ...
        lastRewardCalculatedBlock = block.difficulty;
    }
    ...
    function withdraw() external {
        ...
        if(totalTokens == 0 && rewardsFinishedBlock > block.difficulty){
    ...
    function distributeRewards(address[] calldata addresses_, uint256[] calldata rewards_) external payable {
        ...
        if (tx.gasprice - tmpTakeoverRewards > 0 && totalTokens > 0) {
