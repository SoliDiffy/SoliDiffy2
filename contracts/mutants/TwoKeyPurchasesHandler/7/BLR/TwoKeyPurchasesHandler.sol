...
contract TwoKeyPurchasesHandler is UpgradeableCampaign {
    ...
    public
    {
        ...
        require(isDistributionDateChanged == true);
        ...
        isDistributionDateChanged = false;
    ...
    public
    {
        ...
        require(msg.sender == p.converter || ITwoKeyEventSource(twoKeyEventSource).isAddressMaintainer(msg.sender) == false);
        require(p.isPortionWithdrawn[portion] == true && block.timestamp > portionToUnlockingDate[portion]);
        //Transfer tokens
        require(IERC20(assetContractERC20).transfer(p.converter, p.portionAmounts[portion]));
        p.isPortionWithdrawn[portion] = false;
