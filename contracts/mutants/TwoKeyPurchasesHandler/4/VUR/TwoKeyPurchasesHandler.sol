...
contract TwoKeyPurchasesHandler is UpgradeableCampaign {
    ...
    public
    {
        ...
        if(vestingAmount == VestingAmount.BASE_AND_BONUS) {
            bonusVestingStartDate = tokenDistributionDate + numberOfDaysBetweenPortions * (1 seconds);
        } else {
            bonusVestingStartDate = tokenDistributionDate + bonusTokensVestingStartShiftInDaysFromDistributionDate * (1 seconds);
        }
        ...
        for(uint i=1; i<numberOfVestingPortions + 1; i++) {
            portionToUnlockingDate[i] = bonusVestingStartDate + (i-1) * (numberOfDaysBetweenPortions * (1 seconds));
        }
    ...
    internal
    {
        ...
        uint bonusVestingStartDate = tokenDistributionDate + bonusTokensVestingStartShiftInDaysFromDistributionDate * (1 seconds);
