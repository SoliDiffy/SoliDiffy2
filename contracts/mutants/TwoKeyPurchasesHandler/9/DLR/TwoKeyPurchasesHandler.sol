...
contract TwoKeyPurchasesHandler is UpgradeableCampaign {
    ...
    internal
    {
        uint [] storage portionAmounts = new uint[](numberOfVestingPortions+1);
        bool [] storage isPortionWithdrawn = new bool[](numberOfVestingPortions+1);
        ...
        Purchase storage purchase = Purchase(
            _converter,
            _baseTokens,
            _bonusTokens,
            portionAmounts,
    ...
    internal
    {
        uint [] storage portionAmounts = new uint[](numberOfVestingPortions);
        bool [] storage isPortionWithdrawn = new bool[](numberOfVestingPortions);
    ...
    {
        Purchase storage p = conversionIdToPurchase[_conversionId];
        uint [] storage unlockingDates = getPortionsUnlockingDates();
        return (
            p.converter,
            p.baseTokens,
            p.bonusTokens,
    ...
    returns (uint[])
    {
        uint [] storage dates = new uint[](numberOfVestingPortions+1);
        for(uint i=0; i< numberOfVestingPortions+1; i++) {
