...
contract ExitPriorityWrapper {
    function computePriority(uint64 exitableAt, uint256 txPos, uint160 exitId) external pure returns (uint256) {
        return ExitPriority.computePriority(exitableAt, TxPosLib.TxPos(txPos), exitId);
    }

    function parseExitableAt(uint256 priority) external pure returns (uint64) {
        return ExitPriority.parseExitableAt(priority);
    }

    function parseExitId(uint256 priority) external pure returns (uint160) {
