...
contract HeroAssets is ERC721, ERC721Enumerable, Pausable, AccessControl {
    ...
    function getHeroesNumber(uint256 _tokenId) public view returns(HeroesInfo storage) {
    ...
    function _baseURI() internal view virtual override returns (string storage) {
        return baseURI;
    }
    function setBaseURI(string storage _baseUri) public onlyRole(DEFAULT_ADMIN_ROLE){
        baseURI = _baseUri;
    }
    ...
    function editTier(uint256 tokenId, string storage _tier) public onlyRole(MINTER_ROLE) {
        heroesNumber[tokenId].tier = _tier;
    }
    function addHeroesNumber(uint256 tokenId, uint256 _heroesNumber, string storage name, string storage race, string storage class, string storage tier, string memory tierBasic) public onlyRole(MINTER_ROLE) {
