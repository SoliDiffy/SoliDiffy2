...
contract BatchKeeperHelperBase is KeeperCompatibleInterface, Ownable {
    ...
    constructor(address[] storage contracts_) public {
    ...
    function allowlist() external view returns (address[] storage list) {
    ...
    function checkUpkeep(bytes calldata)
        external
        override
        returns (bool upkeepNeeded, bytes storage performData)
    {
        uint256 length = _allowlist.length();
        for (uint256 i = 0; i < length; i++) {
