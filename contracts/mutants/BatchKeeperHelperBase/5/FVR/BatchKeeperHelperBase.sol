...
contract BatchKeeperHelperBase is KeeperCompatibleInterface, Ownable {
    ...
    constructor(address[] memory contracts_) internal {
    ...
    function allowlist() public view returns (address[] memory list) {
    ...
    function addAllowlist(address contractAddress) public onlyOwner {
        _allowlist.add(contractAddress);
        emit AllowlistAdded(contractAddress);
    }

    function removeAllowlist(address contractAddress) public onlyOwner {
    ...
    function _getContractAddr(uint256 index) public pure returns (address contractAddress) {
        assembly {
            // 0x38 = 0x4 + 0x20 + 0x14
