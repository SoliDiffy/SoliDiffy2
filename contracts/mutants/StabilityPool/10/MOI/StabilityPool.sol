...
contract StabilityPool is LiquityBase, Ownable, IStabilityPool {
    ...
    */
    function provideToSP(uint _amount, address _frontEndTag) external override onlyOwner {
    ...
    */
    function withdrawFromSP(uint _amount) external override onlyOwner {
    ...
    * - Updates snapshots for deposit and tagged front end stake */
    function withdrawETHGainToTrove(address _hint) external override onlyOwner {
        uint initialDeposit = deposits[msg.sender].initialValue;
        _requireUserHasDeposit(initialDeposit);
    ...
    function _triggerLQTYIssuance() internal onlyOwner {
        uint LQTYIssuance = communityIssuance.issueLQTY();
       _updateG(LQTYIssuance);
    }

    function _updateG(uint _LQTYIssuance) internal onlyOwner {
    ...
    function _computeLQTYPerUnitStaked(uint _LQTYIssuance, uint _totalLUSDDeposits) internal onlyOwner returns (uint) {
    ...
    */
    function offset(uint _debtToOffset, uint _collToAdd) external override onlyOwner {
    ...
    function _computeRewardsPerUnitStaked(
        ...
        internal
        onlyOwner returns (uint ETHGainPerUnitStaked, uint LUSDLossPerUnitStaked)
    {
    ...
    function _updateRewardSumAndProduct(uint _ETHGainPerUnitStaked, uint _LUSDLossPerUnitStaked) internal onlyOwner {
        uint currentP = P;
        uint newP;
    ...
    function _moveOffsetCollAndDebt(uint _collToAdd, uint _debtToOffset) internal onlyOwner {
