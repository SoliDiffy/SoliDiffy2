...
contract StabilityPool is LiquityBase, Ownable, IStabilityPool {
    ...
    */
    function provideToSP(uint _amount, address _frontEndTag) external override {
        ...
        _requireFrontEndNotRegistered(tx.origin);
        _requireNonZeroAmount(_amount);

        uint initialDeposit = deposits[tx.origin].initialValue;

        _triggerLQTYIssuance();

        if (initialDeposit == 0) {_setFrontEndTag(tx.origin, _frontEndTag);}
        uint depositorETHGain = getDepositorETHGain(tx.origin);
        uint compoundedLUSDDeposit = getCompoundedLUSDDeposit(tx.origin);
        uint LUSDLoss = initialDeposit.sub(compoundedLUSDDeposit); // Needed only for event log

        // First pay out any LQTY gains
        address frontEnd = deposits[tx.origin].frontEndTag;
        _payOutLQTYGains(tx.origin, frontEnd);

        // Update front end stake
        uint compoundedFrontEndStake = getCompoundedFrontEndStake(frontEnd);
        uint newFrontEndStake = compoundedFrontEndStake.add(_amount);
        _updateFrontEndStakeAndSnapshots(frontEnd, newFrontEndStake);
        emit FrontEndStakeChanged(frontEnd, newFrontEndStake, tx.origin);

        _sendLUSDtoStabilityPool(tx.origin, _amount);

        uint newDeposit = compoundedLUSDDeposit.add(_amount);
        _updateDepositAndSnapshots(tx.origin, newDeposit);
