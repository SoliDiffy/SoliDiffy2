...
contract StabilityPool is LiquityBase, Ownable, IStabilityPool {
    ...
    * - Updates snapshots for deposit and tagged front end stake */
    ...
    function _triggerLQTYIssuance() public {
        uint LQTYIssuance = communityIssuance.issueLQTY();
       _updateG(LQTYIssuance);
    }

    function _updateG(uint _LQTYIssuance) public {
    ...
    function _computeLQTYPerUnitStaked(uint _LQTYIssuance, uint _totalLUSDDeposits) public returns (uint) {
    ...
    */
    ...
    function _updateRewardSumAndProduct(uint _ETHGainPerUnitStaked, uint _LUSDLossPerUnitStaked) public {
        uint currentP = P;
        uint newP;
