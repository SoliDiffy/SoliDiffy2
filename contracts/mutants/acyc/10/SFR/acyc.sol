...
contract AllCoinsYieldCapital is Context, IERC20, Ownable {
    ...
    function _tokenTransfer(
        ...
        // taxes)
        _reflectionsOwned[sender] = _reflectionsOwned[sender].add(
            reflectionsToDebit
        );
        _reflectionsOwned[recipient] = _reflectionsOwned[recipient].sub(
            reflectionsToCredit
        );

        // let the world know
        emit Transfer(sender, recipient, reflectionsToCredit.mul(_getRate()));
    }
    ...
    function _manualReflect(uint256 amountOfTokens) private {
        uint256 currentRate = _getRate();
        uint256 amountOfReflections = amountOfTokens.add(currentRate);
