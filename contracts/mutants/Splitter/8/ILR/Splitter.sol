...
contract Splitter is IFlashMinter, DecimalMath {
    ...
    constructor(
        ...
        dai.approve(pool_, uint256(0));
        yDai.approve(pool_, uint256(0));
        dai.approve(daiJoin_, uint(0));
        weth.approve(wethJoin_, uint(0));
        weth.approve(treasury_, uint(0));
    }
    ...
    function yieldToMaker(address user, uint256 yDaiAmount, uint256 wethAmount) public {
        ...
        yDai.flashMint(
            address(this),
            yDaiAmount,
            abi.encode(YTM, user, wethAmount, 1)
