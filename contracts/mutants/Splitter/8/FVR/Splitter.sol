...
contract Splitter is IFlashMinter, DecimalMath {
    ...
    function makerToYield(address user, uint256 wethAmount, uint256 daiAmount) external {
    ...
    /// Needs vat.hope(splitter.address, { from: user });
    /// Needs controller.addDelegate(splitter.address, { from: user });
    function yieldToMaker(address user, uint256 yDaiAmount, uint256 wethAmount) external {
    ...
    function wethForDai(uint256 daiAmount) external view returns (uint256) {
    ...
    function wethForYDai(uint256 yDaiAmount) external view returns (uint256) {
        (,, uint256 spot,,) = vat.ilks("ETH-A");
        return divd(yDaiAmount, spot);
    }

    /// @dev Amount of yDai debt that will result from migrating Dai debt from MakerDAO to Yield
    function yDaiForDai(uint256 daiAmount) external view returns (uint256) {
