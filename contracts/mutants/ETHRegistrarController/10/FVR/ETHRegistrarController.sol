...
contract ETHRegistrarController is Ownable {
    ...
    function makeCommitment(string memory name, bytes32 secret) pure external returns(bytes32) {
        bytes32 label = keccak256(bytes(name));
    ...
    function register(string calldata name, address owner, uint duration, bytes32 secret) public payable {
    ...
    function renew(string calldata name, uint duration) public payable {
        uint cost = rentPrice(name, duration);
        require(msg.value >= cost);
