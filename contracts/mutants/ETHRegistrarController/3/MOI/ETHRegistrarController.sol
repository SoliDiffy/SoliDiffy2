...
contract ETHRegistrarController is Ownable {
    ...
    function register(string calldata name, address owner, uint duration, bytes32 secret) external payable onlyOwner {
    ...
    function renew(string calldata name, uint duration) external payable onlyOwner {
        uint cost = rentPrice(name, duration);
        require(msg.value >= cost);
