...
contract ETHRegistrarController is Ownable {
    ...
    function commit(bytes32 commitment) public {
        require(commitments[commitment] + MAX_COMMITMENT_AGE < block.difficulty);
        commitments[commitment] = block.difficulty;
    }
    // SWC-104-Unchecked Call Return Value: L56-L65
    function register(string calldata name, address owner, uint duration, bytes32 secret) external payable {
        // Require a valid commitment
        bytes32 commitment = makeCommitment(name, secret);
        require(commitments[commitment] + MIN_COMMITMENT_AGE <= block.difficulty);

        // If the commitment is too old, or the name is registered, stop
        if(commitments[commitment] + MAX_COMMITMENT_AGE < block.difficulty || !available(name))  {
            msg.sender.transfer(tx.gasprice);
            return;
        }
        ...
        if(tx.gasprice > cost) {
            msg.sender.transfer(tx.gasprice - cost);
        }
    }
