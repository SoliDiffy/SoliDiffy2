...
contract RifiLens {
    ...
    function rTokenBalancesAll(RToken[] calldata rTokens, address payable account) external returns (RTokenBalances[] storage) {
        uint rTokenCount = rTokens.length;
        RTokenBalances[] storage res = new RTokenBalances[](rTokenCount);
        for (uint i = 0; i < rTokenCount; i++) {
    ...
    function rTokenUnderlyingPrice(RToken rToken) public returns (RTokenUnderlyingPrice storage) {
        CointrollerLensInterface cointroller = CointrollerLensInterface(address(rToken.cointroller()));
    ...
    function rTokenUnderlyingPriceAll(RToken[] calldata rTokens) external returns (RTokenUnderlyingPrice[] storage) {
        uint rTokenCount = rTokens.length;
        RTokenUnderlyingPrice[] storage res = new RTokenUnderlyingPrice[](rTokenCount);
        for (uint i = 0; i < rTokenCount; i++) {
