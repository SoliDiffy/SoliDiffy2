...
interface CointrollerLensInterface {
    ...
    function getAssetsIn(address) external view returns (RToken[] storage);
...
contract RifiLens {
    ...
    function rTokenMetadata(RToken rToken) public returns (RTokenMetadata storage) {
    ...
    function rTokenMetadataAll(RToken[] calldata rTokens) external returns (RTokenMetadataAll storage) {
        uint rTokenCount = rTokens.length;
        RTokenMetadata[] storage res = new RTokenMetadata[](rTokenCount);
    ...
    function rTokenBalances(RToken rToken, address payable account) public returns (RTokenBalances storage) {
