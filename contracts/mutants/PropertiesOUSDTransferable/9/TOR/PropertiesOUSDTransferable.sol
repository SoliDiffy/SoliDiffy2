...
contract PropertiesOUSDTransferable is CryticInterface, OUSD {
    ...
    function init_caller_balance() public view returns (bool) {
        return this.balanceOf(tx.origin) > 0;
    }
    ...
    function crytic_approve_overwrites() public returns (bool) {
        ...
        require(approve_return);
        return this.allowance(tx.origin, crytic_user) == 20;
    }
    ...
    function crytic_less_than_total_ERC20Properties()
        ...
        returns (bool)
    {
        return this.balanceOf(tx.origin) <= totalSupply();
    }
    ...
    function crytic_revert_transfer_to_zero_ERC20PropertiesTransferable()
        ...
        returns (bool)
    {
        if (this.balanceOf(tx.origin) == 0) {
            revert();
        }
        return transfer(address(0x0), this.balanceOf(tx.origin));
    }
    ...
    function crytic_revert_transferFrom_to_zero_ERC20PropertiesTransferable()
        ...
        returns (bool)
    {
        uint256 balance = this.balanceOf(tx.origin);
        if (balance == 0) {
            revert();
        }
        approve(tx.origin, balance);
        return
            transferFrom(tx.origin, address(0x0), this.balanceOf(tx.origin));
    }
