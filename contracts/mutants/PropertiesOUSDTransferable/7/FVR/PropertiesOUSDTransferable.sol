...
contract PropertiesOUSDTransferable is CryticInterface, OUSD {
    function init_total_supply() external view returns (bool) {
        return
            this.totalSupply() >= 0 && this.totalSupply() == initialTotalSupply;
    }

    function init_owner_balance() external view returns (bool) {
        return initialBalance_owner == this.balanceOf(crytic_owner);
    }

    function init_user_balance() external view returns (bool) {
        return initialBalance_user == this.balanceOf(crytic_user);
    }

    function init_attacker_balance() external view returns (bool) {
        return initialBalance_attacker == this.balanceOf(crytic_attacker);
    }

    function init_caller_balance() external view returns (bool) {
        return this.balanceOf(msg.sender) > 0;
    }

    function init_total_supply_is_balances() external view returns (bool) {
        return
