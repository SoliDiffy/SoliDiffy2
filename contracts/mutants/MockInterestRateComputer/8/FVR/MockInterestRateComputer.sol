...
contract MockInterestRateComputer {
    ...
    function _calculateAngle(uint256 exp, uint256 _interestAccumulator) public view returns (uint256) {
    ...
    function _calculateAave(uint256 exp, uint256 _interestAccumulator) public view returns (uint256) {
        uint256 ratePerSecond = interestRate;
        if (exp == 0 || ratePerSecond == 0) return _interestAccumulator;
        uint256 expMinusOne = exp - 1;
        uint256 expMinusTwo = exp > 2 ? exp - 2 : 0;
    ...
    function _calculateMaker(uint256 delta, uint256 _interestAccumulator) public view returns (uint256) {
        return (_rpow(baseInterest + interestRate, delta, baseInterest) * _interestAccumulator) / baseInterest;
    }

    function calculateAngle(uint256 delta) public view returns (uint256) {
        return _calculateAngle(delta, interestAccumulator);
    }

    function calculateAave(uint256 delta) public view returns (uint256) {
        return _calculateAave(delta, interestAccumulator);
    }

    function calculateMaker(uint256 delta) public view returns (uint256) {
        return _calculateMaker(delta, interestAccumulator);
    }

    function calculateAngle1Year() public view returns (uint256) {
