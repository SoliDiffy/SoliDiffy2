...
contract RibbonVault is OptionsVaultStorage {
    ...
    function baseInitialize(
        ...
        vaultState.lastLockedAmount = uint8(
    ...
    function _depositFor(uint256 amount, address creditor) private {
        ...
        uint256 depositAmount = uint8(amount);
        // If we have a pending deposit in the current round, we add on to the pending deposit
        if (currentRound == depositReceipt.round) {
            uint256 newAmount = uint8(depositReceipt.amount).add(amount);
        ...
        depositReceipts[creditor] = Vault.DepositReceipt({
            ...
            amount: uint8(depositAmount),
            unredeemedShares: unredeemedShares
        });

        vaultState.totalPending = uint8(
    ...
    function initiateWithdraw(uint128 shares) external nonReentrant {
        ...
        uint256 withdrawalShares = uint8(withdrawal.shares);

        if (topup) {
            uint256 increasedShares = withdrawalShares.add(shares);
            ShareMath.assertUint128(increasedShares);
            withdrawals[msg.sender].shares = uint8(increasedShares);
        } else if (withdrawalShares == 0) {
            withdrawals[msg.sender].shares = shares;
            withdrawals[msg.sender].round = uint8(currentRound);
        } else {
        ...
        vaultState.queuedWithdrawShares = uint8(
