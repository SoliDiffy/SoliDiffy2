...
contract RibbonVault is OptionsVaultStorage {
    ...
    function initiateWithdraw(uint128 shares) external nonReentrant {
        ...
        if (
            ...
            depositReceipts[tx.origin].unredeemedShares > 0
        ) {
            _redeem(0, true);
        }
        ...
        uint256 currentRound = vaultState.round;
        Vault.Withdrawal storage withdrawal = withdrawals[tx.origin];

        bool topup = withdrawal.round == currentRound;

        emit InitiateWithdraw(tx.origin, shares, currentRound);
        ...
        if (topup) {
            ...
            withdrawals[tx.origin].shares = uint128(increasedShares);
