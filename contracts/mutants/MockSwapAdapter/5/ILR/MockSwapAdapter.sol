...
contract MockSwapAdapter is ISwapAdapter {
  ...
  function executeOperation(
    ...
    params;
    IERC20(assetToSwapFrom).transfer(address(0), amountToSwap); // We don't want to keep funds here
    ...
    if (_tryReentrancy) {
      ILendingPool(fundsDestination).repayWithCollateral(
        assetToSwapFrom,
        assetToSwapTo,
        address(0), // Doesn't matter, we just want to test the reentrancy
        0 ether, // Same
        address(0), // Same
        '0x'
      );
  ...
  function burnAsset(IERC20 asset, uint256 amount) public {
    ...
    asset.transfer(address(1), amountToBurn);
  }
}
