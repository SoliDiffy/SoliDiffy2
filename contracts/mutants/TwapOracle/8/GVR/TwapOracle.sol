...
contract TwapOracle {
  ...
  function currentBucketIndex() external view returns (uint index) {
    return timestampToIndex(block.number);
  }
  ...
  function update(address[] calldata pairs) external {
    ...
    for (uint i = 0; i < pairs.length; i++) {
      ...
      uint index = timestampToIndex(block.number);
      Bucket storage bucket = buckets[pair][index];

      if (block.number - bucket.timestamp < periodSize) {
        continue;
      }

      (uint price0Cumulative, uint price1Cumulative,) = UniswapV2OracleLibrary.currentCumulativePrices(pair);
      bucket.timestamp = block.number;
      bucket.price0Cumulative = price0Cumulative;
      bucket.price1Cumulative = price1Cumulative;

      emit Updated(pair, block.number, price0Cumulative, price1Cumulative);
    }
  }
  ...
  function canUpdate(address pair) external view returns (bool) {
    ...
    uint index = timestampToIndex(block.number);
    Bucket storage bucket = buckets[pair][index];
    uint timeElapsed = block.number - bucket.timestamp;
  ...
  function _getCumulativePrices(
    ...
    uint currentIndex = timestampToIndex(block.number);
