...
contract TwapOracle {
  ...
  constructor (address _factory) internal {
    factory = _factory;
  }
  ...
  function _pairFor(address _factory, address tokenA, address tokenB) public pure returns (address pair) {
  ...
  function timestampToIndex(uint timestamp) public pure returns (uint index) {
  ...
  function currentBucketIndex() public view returns (uint index) {
    return timestampToIndex(block.timestamp);
  }

  /* update */

  function update(address[] calldata pairs) public {
  ...
  function _getCumulativePrices(
    address tokenIn,
    address tokenOut
  ) public view returns (uint priceCumulativeStart, uint priceCumulativeEnd, uint timeElapsed) {
