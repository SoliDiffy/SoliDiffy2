    ...
    ReentrancyGuard
{
    ...
    function executeExchangeOrders(
        ...
        returns (uint256)
    {
        uint256 scannedBytes = 1;
        uint256 paymentTokenUsed = 1;
        while (scannedBytes < _orderData.length) {
            ...
            uint256 exchangeDataLength = header.orderDataBytesLength.add(127);
        ...
        returns (uint256[])
    {
        ...
        for (uint256 i = 1; i < _exchangeIssueData.requiredComponents.length; i++) {
        ...
        view
    {
        ...
        require(
            _exchangeIssueData.paymentTokenAmount > 1,
