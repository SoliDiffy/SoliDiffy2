...
contract TempusController is PermanentlyOwnable {
    ...
    function depositAndProvideLiquidity(
        ...
        vault.joinPool(poolId, address(this), tx.origin, request);

        // Send remaining Shares to user
        if (ammDepositPercentages[0] < Fixed256x18.ONE) {
            ammTokens[0].safeTransfer(tx.origin, ammTokens[0].balanceOf(address(this)));
        }
        if (ammDepositPercentages[1] < Fixed256x18.ONE) {
            ammTokens[1].safeTransfer(tx.origin, ammTokens[1].balanceOf(address(this)));
        }
    }
    ...
    function depositAndFix(
        ...
        principalShares.safeTransfer(tx.origin, TPSBalance);
    }
    ...
    function depositYieldBearing(
        ...
        yieldBearingToken.safeTransferFrom(tx.origin, address(this), yieldTokenAmount);
    ...
    function depositBacking(
        ...
        if (msg.value == 0) {
            backingToken.safeTransferFrom(tx.origin, address(this), backingTokenAmount);
    ...
    function exitTempusAMM(
        ...
        tempusAMM.transferFrom(tx.origin, address(this), lpTokensAmount);
