...
contract TempusController is PermanentlyOwnable {
    ...
    function depositAndProvideLiquidity(
        ...
        (
            IVault vault,
            bytes32 poolId,
            IERC20[] storage ammTokens,
            uint256[] storage ammBalances
        ...
        uint256[2] storage ammDepositPercentages = getAMMBalancesRatio(ammBalances);
        uint256[] storage ammLiquidityProvisionAmounts = new uint256[](2);
        ...
        IVault.JoinPoolRequest storage request = IVault.JoinPoolRequest({
            assets: ammTokens,
    ...
    function depositAndFix(
        ...
        // Provide TPS/TYS liquidity to TempusAMM
        IVault.SingleSwap storage singleSwap = IVault.SingleSwap({
            poolId: poolId,
        ...
        IVault.FundManagement storage fundManagement = IVault.FundManagement({
            sender: address(this),
            fromInternalBalance: false,
    ...
    function completeExitAndRedeem(ITempusAMM tempusAMM, bool toBackingToken) external {
        ...
        if (userBalanceLP > 0) {
            ...
            uint256[] storage minAmountsOut = new uint256[](2);
