...
contract ExchangerWithFeeReclamationAlternatives is MinimalProxyFactory, Exchanger {
    ...
    constructor(address _owner, address _resolver) internal MinimalProxyFactory() Exchanger(_owner, _resolver) {}
    ...
    function atomicMaxVolumePerBlock() public view returns (uint) {
    ...
    ) public onlySynthetixorSynth returns (uint amountReceived) {
        uint fee;
    ...
    function _virtualSynthMastercopy() public view returns (address) {
    ...
    function _exchangeAtomically(
        ...
        bytes32 destinationCurrencyKey,
        address destinationAddress
    ) public returns (uint amountReceived, uint fee) {
