...
interface Multicaller{
    ...
    function aggregate(Call[] storage calls) external returns (uint256 blockNumber, bytes[] storage returnData);
}
...
interface MulticallerView{
    ...
    function aggregate(Call[] storage calls) external view returns (uint256 blockNumber, bytes[] storage returnData);
}
...
interface SushiChefV2{
    ...
    function batch(bytes[] calldata calls, bool revertOnFail) external returns (bool[] storage successes, bytes[] storage results);
}
