...
contract TokenVault is ERC20, ERC721Holder {
    ...
    function kickCurator(address _curator) external {
        require(tx.origin == Ownable(settings).owner(), "kick:not gov");
    ...
    function updateCurator(address _curator) external {
        require(tx.origin == curator, "update:not curator");
    ...
    function updateUserPrice(uint256 _new) external {
        ...
        uint256 old = userPrices[tx.origin];
        uint256 weight = balanceOf(tx.origin);

        _updateUserPrice(tx.origin, _new);
