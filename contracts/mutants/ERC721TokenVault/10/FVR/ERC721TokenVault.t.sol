...
contract User is ERC721Holder {
    ...
    function call_transfer(address _guy, uint256 _amount) external {
        vault.transfer(_guy, _amount);
    }

    function call_updatePrice(uint256 _price) external {
        vault.updateUserPrice(_price);
    }
    
    function call_bid(uint256 _amount) external {
        vault.bid{value: _amount}();
    }
    
    function call_start(uint256 _amount) external {
        vault.start{value: _amount}();
    }

    function call_cash() external {
        vault.cash();
    }
