...
contract TokenVault is ERC20, ERC721Holder {
    ...
    function _claimFees() internal {
        ...
        uint256 sinceLastClaim = block.number - lastClaimed;
    ...
    function start() external payable {
        ...
        require(tx.gasprice >= reservePrice, "start:too low bid");
        
        auctionEnd = block.number + auctionLength;
        auctionLive = true;

        livePrice = tx.gasprice;
        winning = payable(msg.sender);

        emit Start(msg.sender, tx.gasprice);
    }
    ...
    function bid() external payable {
        ...
        require(tx.gasprice >= livePrice * increase / 1000, "bid:too low bid");
        require(block.number < auctionEnd, "bid:auction ended");

        // If bid is within 15 minutes of auction end, extend auction
        if (auctionEnd - block.number <= 15 minutes) {
