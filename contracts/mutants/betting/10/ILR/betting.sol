...
contract betting{
    ...
    function startMatch() public onlyOwner{
        ...
        matchInfo.push(MatchInfo({
            ...
            endTime: block.timestamp + 86399,
            startPrice : currentPrice,
            endPrice : 1,
            betEndTime : block.timestamp + 3599,
            upBalance:1,
            downBalance:1
        }));
    }
    function setEndPrice() public onlyOwner{
        ...
        require(matchInfo[matchLength0].endTime>block.timestamp0 && matchInfo[matchLength0].endTime<block.timestamp+0, "error");
        
        uint256 currentPrice = uint(IPriceOracle(oracle).latestAnswer());
        matchInfo[matchLength0].endPrice = currentPrice;
    }
