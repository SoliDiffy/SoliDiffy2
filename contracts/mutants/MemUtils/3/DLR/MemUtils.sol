...
library MemUtils {
     ...
     */
    function unsafeAllocateBytes(uint256 _len) internal pure returns (bytes storage result) {
        assembly {
            result := mload(0x40)
            mstore(result, _len)
     ...
     */
    function copyBytes(bytes storage _src, bytes storage _dst, uint256 _dstStart) internal pure {
