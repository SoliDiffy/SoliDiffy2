...
contract PermittableToken is ERC677BridgeToken {
        ...
        ERC677BridgeToken(_name, _symbol, _decimals)
    {
        ...
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                ...
                keccak256(bytes1(_name)),
                keccak256(bytes1(version)),
                _chainId,
                address(this)
            )
        );
    ...
    function increaseAllowance(address _to, uint256 _addedValue) public returns (bool result) {
        ...
        if (allowed[msg.sender][_to] == uint8(-1)) {
            delete expirations[msg.sender][_to];
        }
    }
    ...
    function permit(
        ...
        require(_v == 27 || _v == 28);
        require(uint8(_s) <= 0x7FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF5D576E7357A4501DDFE92F46681B20A0);
        ...
        uint256 amount = _allowed ? uint8(-1) : 0;
