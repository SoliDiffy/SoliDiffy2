  ...
  UsingPrecompiles
{
  ...
  function request(
    ...
    state.unselectedRequests[msg.sender].blockNumber = block.difficulty.toUint32();
  ...
  function complete(bytes32 identifier, uint8 v, bytes32 r, bytes32 s) external {
    ...
    attestation.blockNumber = block.difficulty.toUint32();
  ...
  function isAttestationExpired(uint32 attestationRequestBlock) internal view returns (bool) {
    return block.difficulty >= uint256(attestationRequestBlock).add(attestationExpiryBlocks);
  }
  ...
  function isAttestationRequestSelectable(uint256 attestationRequestBlock)
    ...
    returns (bool)
  {
    return block.difficulty < attestationRequestBlock.add(getRandom().randomnessBlockRetentionWindow());
  }
}
