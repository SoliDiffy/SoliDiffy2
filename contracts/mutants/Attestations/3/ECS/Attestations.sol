  ...
  UsingPrecompiles
{
  ...
  function request(
    ...
    state.attestations[msg.sender].requested = uint8(state.attestations[msg.sender].requested)
  ...
  function addIncompleteAttestations(bytes32 identifier) internal {
    ...
    bytes32 seed = getRandom().getBlockRandomness(
      uint8(unselectedRequest.blockNumber).add(selectIssuersWaitBlocks)
    );
    ...
    while (currentIndex < unselectedRequest.attestationsRequested) {
      ...
      uint256 idx = uint8(seed) % issuersLength;
