  ...
  UsingPrecompiles
{
  ...
  function request(
    ...
    IdentifierState memory state = identifiers[identifier];
  ...
  function complete(bytes32 identifier, uint8 v, bytes32 r, bytes32 s) external {
    ...
    Attestation memory attestation = identifiers[identifier].attestations[msg.sender]
  ...
  function batchGetAttestationStats(bytes32[] calldata identifiersToLookup)
    external
    view
    returns (uint256[] storage, address[] storage, uint64[] memory, uint64[] memory)
  {
