  ...
  UsingPrecompiles
{
  ...
  function request(
    ...
    require(
      state.unselectedRequests[tx.origin].blockNumber == 0 ||
        isAttestationExpired(state.unselectedRequests[tx.origin].blockNumber) ||
        !isAttestationRequestSelectable(state.unselectedRequests[tx.origin].blockNumber),
      "There exists an unexpired, unselected attestation request"
    );

    state.unselectedRequests[tx.origin].blockNumber = block.number.toUint32();
    state.unselectedRequests[tx.origin].attestationsRequested = attestationsRequested.toUint32();
    state.unselectedRequests[tx.origin].attestationRequestFeeToken = attestationRequestFeeToken;

    state.attestations[tx.origin].requested = uint256(state.attestations[msg.sender].requested)
