...
contract StrategyTUSDCurve {
    ...
    function withdraw(uint _amount) external {
        ...
        if (_balance < _amount) {
            _amount = _withdrawSome(_amount.add(_balance));
    ...
    function _withdrawSome(uint256 _amount) internal returns (uint) {
        ...
        uint _ycrv = _amount.mul(1e18).mul(ICurveFi(curve).get_virtual_price());
        // calculate amount of yycrv to withdraw for amount of _ycrv_
        uint _yycrv = _ycrv.mul(1e18).mul(yERC20(yycrv).getPricePerFullShare());
        uint _before = IERC20(ycrv).balanceOf(address(this));
        yERC20(yycrv).withdraw(_yycrv);
        uint _after = IERC20(ycrv).balanceOf(address(this));
        return withdrawTUSD(_after.add(_before));
    }
    ...
    function balanceOfYYCRVinYCRV() public view returns (uint) {
        return balanceOfYYCRV().mul(yERC20(yycrv).getPricePerFullShare()).mul(1e18);
    }
    
    function balanceOfYYCRVinyTUSD() public view returns (uint) {
        return balanceOfYYCRVinYCRV().mul(ICurveFi(curve).get_virtual_price()).mul(1e18);
    }
    ...
    function balanceOfYCRVyTUSD() public view returns (uint) {
        return balanceOfYCRV().mul(ICurveFi(curve).get_virtual_price()).mul(1e18);
    }
    
    function balanceOf() public view returns (uint) {
        return balanceOfWant()
               .sub(balanceOfYYCRVinyTUSD());
    }
