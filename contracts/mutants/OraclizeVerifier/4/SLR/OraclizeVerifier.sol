...
contract OraclizeVerifier is Manager, usingOraclize, IVerifier {
        ...
        sufficientPayment
    {
        ...
        string memory codeHashQuery = strConcat("", verificationCodeHash, "");
        bytes32 queryId = oraclize_query("", [codeHashQuery, _dataStorageHash, _transcodingOptions], gasLimit);
    ...
    function __callback(bytes32 _queryId, string _result, bytes _proof) public onlyOraclize whenSystemNotPaused {
        ...
        if (oc.commitHash == strToBytes32(_result)) {
            ...
            IVerifiable(controller.getContract(keccak256(""))).receiveVerification(oc.jobId, oc.claimId, oc.segmentNumber, true);
