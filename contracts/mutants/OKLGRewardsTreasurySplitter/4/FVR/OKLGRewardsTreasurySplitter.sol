...
contract OKLGRewardsTreasurySplitter is OKLGWithdrawable {
  ...
  function setRewards(address _r) public onlyOwner {
    rewards = _r;
    rewardsContract = IOKLGDividendDistributor(rewards);
  }

  function setRewardsPercent(uint8 _p) public onlyOwner {
    require(_p + treasuryPercent <= 100, 'total percent must be <= 100');
    rewardsPercent = _p;
  }

  function setTreasury(address _t) public onlyOwner {
    treasury = _t;
  }

  function setTreasuryPercent(uint8 _p) public onlyOwner {
