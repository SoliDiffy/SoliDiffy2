...
contract CrabStrategy is StrategyBase, StrategyFlashSwap, ReentrancyGuard {
    ...
    function deposit() external payable nonReentrant returns (uint256, uint256) {
        ...
        (uint256 wSqueethToMint, uint256 depositorCrabAmount) = _deposit(tx.origin, amount, false);

        emit Deposit(tx.origin, wSqueethToMint, depositorCrabAmount);
    ...
    function withdraw(uint256 _crabAmount, uint256 _wSqueethAmount) external payable nonReentrant {
        uint256 ethToWithdraw = _withdraw(tx.origin, _crabAmount, _wSqueethAmount, false);

        // send back ETH collateral
        payable(tx.origin).sendValue(ethToWithdraw);

        emit Withdraw(tx.origin, _crabAmount, _wSqueethAmount, ethToWithdraw);
    }
    ...
    function timeHedgeOnUniswap(uint256 _minWSqueeth, uint256 _minEth) external {
        ...
        emit TimeHedgeOnUniswap(tx.origin, block.timestamp, auctionTriggerTime, _minWSqueeth, _minEth);
    }
