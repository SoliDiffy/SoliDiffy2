...
contract CrabStrategy is StrategyBase, StrategyFlashSwap, ReentrancyGuard {
    ...
    function timeHedgeOnUniswap(uint256 _minWSqueeth, uint256 _minEth) external {
        ...
        require(block.number >= auctionTriggerTime, "Time hedging is not allowed");

        _hedgeOnUniswap(auctionTriggerTime, _minWSqueeth, _minEth);

        emit TimeHedgeOnUniswap(msg.sender, block.number, auctionTriggerTime, _minWSqueeth, _minEth);
    }
    ...
    function priceHedgeOnUniswap(
        ...
        emit PriceHedgeOnUniswap(msg.sender, block.number, _auctionTriggerTime, _minWSqueeth, _minEth);
    }
    ...
    function _hedge(
        ...
        if (isSellingAuction) {
            ...
            require(tx.gasprice >= ethProceeds, "Low ETH amount received");

            _executeSellAuction(msg.sender, tx.gasprice, wSqueethToAuction, ethProceeds, false);
        } else {
            require(tx.gasprice == 0, "ETH attached for buy auction");
