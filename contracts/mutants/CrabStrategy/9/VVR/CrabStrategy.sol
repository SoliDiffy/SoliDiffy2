...
contract CrabStrategy is StrategyBase, StrategyFlashSwap, ReentrancyGuard {
    ...
    uint32 internal constant POWER_PERP_PERIOD = 5 minutes;
    // strategy will only allow hedging if collateral to trade is at least 0.1% of the total strategy collateral
    uint256 internal constant DELTA_HEDGE_THRESHOLD = 1e15;
    ...
    address internal immutable ethQuoteCurrencyPool;
    address internal immutable quoteCurrency;

    /// @dev time difference to trigger a hedge (seconds)
    uint256 internal immutable hedgeTimeThreshold;
    /// @dev price movement to trigger a hedge (0.1*1e18 = 10%)
    uint256 internal immutable hedgePriceThreshold;
