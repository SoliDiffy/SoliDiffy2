...
contract CrabStrategy is StrategyBase, StrategyFlashSwap, ReentrancyGuard {
    ...
    function _strategyFlash(
        ...
        if (FLASH_SOURCE(_callSource) == FLASH_SOURCE.FLASH_DEPOSIT) {
            ...
            IWPowerPerp(wPowerPerp).send(ethWSqueethPool, _amountToPay);
            ...
            IWETH9(weth).send(ethWSqueethPool, _amountToPay);
            ...
            IWPowerPerp(wPowerPerp).send(ethWSqueethPool, _amountToPay);
            IWPowerPerp(wPowerPerp).send(_caller, wSqueethProfit);
