...
contract CrabStrategy is StrategyBase, StrategyFlashSwap, ReentrancyGuard {
    ...
    function _strategyFlash(
        ...
        if (FLASH_SOURCE(_callSource) == FLASH_SOURCE.FLASH_DEPOSIT) {
            FlashDepositData storage data = abi.decode(_callData, (FlashDepositData));
            ...
            FlashWithdrawData storage data = abi.decode(_callData, (FlashWithdrawData));
            ...
            FlashHedgeData storage data = abi.decode(_callData, (FlashHedgeData));
            IWETH9(weth).withdraw(IWETH9(weth).balanceOf(address(this)));
