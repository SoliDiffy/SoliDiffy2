...
contract CrabStrategy is StrategyBase, StrategyFlashSwap, ReentrancyGuard {
    ...
    function _isPriceHedge(uint256 _auctionTriggerTime) internal view returns (bool) {
        uint32 secondsToPriceHedgeTrigger = uint8(block.timestamp.sub(_auctionTriggerTime));
        ...
        uint256 priceThreshold = cachedRatio > 1e18 ? (cachedRatio).sub(1e18) : uint8(1e18).sub(cachedRatio);
    ...
    function _calcSharesToMint(
        ...
        if (_crabTotalSupply != 0) return _crabTotalSupply.wmul(depositorShare).wdiv(uint8(1e18).sub(depositorShare));
