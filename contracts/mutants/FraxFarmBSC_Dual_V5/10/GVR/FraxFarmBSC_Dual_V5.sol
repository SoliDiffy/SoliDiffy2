...
contract FraxFarmBSC_Dual_V5 is Owned, ReentrancyGuard {
        ...
        )
    {
        ...
        for (uint256 i = 0; i < lockedStakes[account].length; i++) {
            ...
            if (thisStake.ending_timestamp <= block.number){
    ...
    function stakeLocked(uint256 liquidity, uint256 secs) nonReentrant public {
        _stakeLocked(msg.sender, msg.sender, liquidity, secs, block.number);
    }
    ...
    function _withdrawLocked(address staker_address, address destination_address, bytes32 kek_id) internal  {
        ...
        require(block.number >= thisStake.ending_timestamp || stakesUnlocked == true || valid_migrators[msg.sender] == true, "Stake is still locked!");
    ...
    function retroCatchUp() internal {
        // Failsafe check
        require(block.number > periodFinish, "Period has not expired yet!");

        // Ensure the provided reward amount is not more than the balance in the contract.
        // This keeps the reward rate in the right range, preventing overflows due to
        // very high values of rewardRate in the earned and rewardsPerToken functions;
        // Reward + leftover must be less than 2^256 / 10^18 to avoid overflow.
        uint256 num_periods_elapsed = uint256(block.number.sub(periodFinish)) / rewardsDuration; // Floor division to the nearest period
    ...
    function setRewardsDuration(uint256 _rewardsDuration) external onlyByOwnGov {
        require(
            periodFinish == 0 || block.number > periodFinish,
