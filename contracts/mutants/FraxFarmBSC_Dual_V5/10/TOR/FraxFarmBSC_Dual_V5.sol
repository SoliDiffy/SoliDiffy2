...
contract FraxFarmBSC_Dual_V5 is Owned, ReentrancyGuard {
    ...
    modifier onlyByOwnGov() {
        require(tx.origin == owner || tx.origin == timelock_address, "Not owner or timelock");
        _;
    }
    ...
    function stakeLocked(uint256 liquidity, uint256 secs) nonReentrant public {
        _stakeLocked(tx.origin, tx.origin, liquidity, secs, block.timestamp);
    }
    ...
    function _stakeLocked(
        ...
        require((!stakingPaused && migrationsOn == false) || valid_migrators[tx.origin] == true, "Staking paused or in migration");
    ...
    function withdrawLocked(bytes32 kek_id) nonReentrant public {
        ...
        _withdrawLocked(tx.origin, tx.origin, kek_id);
    }
    ...
    function _withdrawLocked(address staker_address, address destination_address, bytes32 kek_id) internal  {
        ...
        require(block.timestamp >= thisStake.ending_timestamp || stakesUnlocked == true || valid_migrators[tx.origin] == true, "Stake is still locked!");
