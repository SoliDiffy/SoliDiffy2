...
abstract contract Fees is IVault, ReentrancyGuard, Authorization {
     ...
     */
    function _getProtocolSwapFee() public view returns (uint256) {
     ...
     */
    function _calculateProtocolWithdrawFeeAmount(uint256 amount) public view returns (uint256) {
     ...
     */
    function _calculateProtocolFlashLoanFeeAmount(uint256 amount) public view returns (uint256) {
        // Fixed point multiplication introduces error: we round up, which means in certain scenarios the charged
        // percentage can be slightly higher than intended.
     ...
     */
    function _increaseCollectedFees(IERC20 token, uint256 amount) public {
     ...
     */
    function _decreaseCollectedFees(IERC20 token, uint256 amount) public {
        uint256 currentCollectedFees = _collectedProtocolFees[token];
