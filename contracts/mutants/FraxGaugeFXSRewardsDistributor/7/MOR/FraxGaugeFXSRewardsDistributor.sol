...
contract FraxGaugeFXSRewardsDistributor is Owned, ReentrancyGuard {
    ...
    // Callable by anyone
    function distributeReward(address gauge_address) public onlyByOwnerOrCuratorOrGovernance nonReentrant returns (uint256 weeks_elapsed, uint256 reward_tally) {
    ...
    function toggleDistributions() external isDistributing {
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount) external isDistributing {
        // Only the owner address can ever receive the recovery withdrawal
        TransferHelper.safeTransfer(tokenAddress, owner, tokenAmount);
        emit RecoveredERC20(tokenAddress, tokenAmount);
    }

    function setGaugeState(address _gauge_address, bool _is_middleman, bool _is_active) external isDistributing {
    ...
    function setTimelock(address _new_timelock) external isDistributing {
        timelock_address = _new_timelock;
    }

    function setCurator(address _new_curator_address) external isDistributing {
        curator_address = _new_curator_address;
    }

    function setGaugeController(address _gauge_controller_address) external isDistributing {
        gauge_controller = IFraxGaugeController(_gauge_controller_address);
    }
