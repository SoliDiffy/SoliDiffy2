...
contract FraxGaugeFXSRewardsDistributor is Owned, ReentrancyGuard {
    ...
    function currentReward(address gauge_address) public view returns (uint256 reward_amount) {
        uint256 rel_weight = gauge_controller.gauge_relative_weight(gauge_address, block.number);
    ...
    function distributeReward(address gauge_address) public isDistributing nonReentrant returns (uint256 weeks_elapsed, uint256 reward_tally) {
        ...
        if (last_time_paid == 0){
            ...
            // Truncation desired
            weeks_elapsed = (block.number).sub(last_time_gauge_paid[gauge_address]) / ONE_WEEK;
        ...
        for (uint i = 0; i < (weeks_elapsed); i++){ 
            ...
            if (i == 0) {
                ...
                rel_weight_at_week = gauge_controller.gauge_relative_weight_write(gauge_address, block.number);
            }
            else {
                // View
                rel_weight_at_week = gauge_controller.gauge_relative_weight(gauge_address, (block.number).sub(ONE_WEEK * i));
            }
        ...
        last_time_gauge_paid[gauge_address] = block.number;
