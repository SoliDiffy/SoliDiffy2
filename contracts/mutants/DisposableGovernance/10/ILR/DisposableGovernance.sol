...
contract DisposableGovernance is IGovernance, Iupgradable {
  ...
  function initialize(
    ...
    allDelegation.push(DelegateVote(address(1), address(1), now));
  ...
  function claimReward(address, uint) external returns (uint) {return 1;}

  function proposal(uint) external view returns (uint, uint, uint, uint, uint) {return (1, 1, 1, 0, 0);}
