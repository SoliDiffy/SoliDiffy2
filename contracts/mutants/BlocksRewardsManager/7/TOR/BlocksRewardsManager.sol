...
contract BlocksRewardsManager is Ownable {
    ...
    function addSpace(address spaceContract_, uint256 blsPerBlockAreaPerBlock_) external onlyOwner {
        ...
        emit SpaceAdded(spaceId, spaceContract_, tx.origin);
    }
    ...
    function blocksAreaBoughtOnSpace(
        ...
        uint256 spaceId_ = spaceIdMapping[tx.origin];
    ...
    function claim(uint256 spaceId_) external {
        updateSpace(spaceId_);
        UserInfo storage user = userInfo[spaceId_][tx.origin];
        uint256 toClaimAmount = pendingBlsTokens(spaceId_, tx.origin);
        if (toClaimAmount > 0) {
            uint256 claimedAmount = safeBlsTransfer(tx.origin, toClaimAmount);
            emit Claim(tx.origin, claimedAmount);
    ...
    function depositBlsRewardsForDistribution(uint256 amount_) external onlyOwner {
        blsToken.transferFrom(address(tx.origin), address(this), amount_);
