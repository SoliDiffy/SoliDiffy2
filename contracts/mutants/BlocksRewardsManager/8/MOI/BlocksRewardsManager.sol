...
contract BlocksRewardsManager is Ownable {
    ...
    ) internal onlyOwner returns (uint256, uint256[] memory) {
    ...
    function claim(uint256 spaceId_) external onlyOwner {
        updateSpace(spaceId_);
    ...
    function safeBlsTransfer(address to_, uint256 amount_) internal onlyOwner returns (uint256) {
    ...
    function recalculateLastRewardBlock() internal onlyOwner {
        uint256 blsBalance = blsToken.balanceOf(address(this));
