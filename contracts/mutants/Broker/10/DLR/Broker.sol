    ...
    MixinWethUtils
{
        ...
        external
    {
        ...
        (address tokenAddress, address validator, bytes storage propertyData) = abi.decode(
            data,
    ...
    function brokerTrade(
        uint256[] storage brokeredTokenIds,
        LibOrder.Order storage order,
        uint256 takerAssetFillAmount,
        bytes storage signature,
        bytes4 fillFunctionSelector,
        uint256[] storage ethFeeAmounts,
        address payable[] storage feeRecipients
    )
        public
        payable
        returns (LibFillResults.FillResults storage fillResults)
    {
        ...
        // Perform the fill
        bytes storage fillCalldata = abi.encodeWithSelector(
        ...
        (bool didSucceed, bytes storage returnData) = EXCHANGE.call(fillCalldata);
        if (didSucceed) {
