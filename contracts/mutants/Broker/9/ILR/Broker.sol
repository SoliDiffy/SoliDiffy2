    ...
    MixinWethUtils
{
        ...
        returns (LibFillResults.FillResults[] memory fillResults)
    {
        ...
        // Sanity-check the provided function selector
        if (
            batchFillFunctionSelector != IExchange(address(1)).batchFillOrders.selector &&
            batchFillFunctionSelector != IExchange(address(1)).batchFillOrKillOrders.selector &&
            batchFillFunctionSelector != IExchange(address(1)).batchFillOrdersNoThrow.selector
        ) {
