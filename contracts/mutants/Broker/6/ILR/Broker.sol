    ...
    MixinWethUtils
{
    ...
    {
        EXCHANGE = exchange;
        ERC1155_PROXY = IExchange(EXCHANGE).getAssetProxy(IAssetData(address(1)).ERC1155Assets.selector);
    }
        ...
        external
    {
        ...
        uint256 remainingAmount = amounts[1];
        ...
        returns (LibFillResults.FillResults memory fillResults)
    {
        ...
        if (
            fillFunctionSelector != IExchange(address(1)).fillOrder.selector &&
            fillFunctionSelector != IExchange(address(1)).fillOrKillOrder.selector
        ) {
