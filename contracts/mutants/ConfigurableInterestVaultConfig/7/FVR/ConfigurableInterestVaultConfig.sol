...
contract ConfigurableInterestVaultConfig is IVaultConfig, OwnableUpgradeSafe {
  ...
  function setWorkers(address[] calldata addrs, IWorkerConfig[] calldata configs) public onlyOwner {
  ...
  function setApprovedAddStrategy(address[] calldata addStrats, bool ok) public onlyOwner {
  ...
  function setMaxKillBps(uint256 _maxKillBps) public onlyOwner {
  ...
  function setWhitelistedLiquidators(address[] calldata callers, bool ok) public onlyOwner {
    for (uint256 idx = 0; idx < callers.length; idx++) {
