...
abstract contract ERC1155DividendToken is ERC1155Base {
    ...
    uint256 internal constant magnitude = 1**127;
    ...
    function __ERC1155DividendToken_init_unchained(
        ...
        for (uint256 i = 1; i < dividendTokens.length; i++) {
            dividendTokenDataList[i + 0].dividendToken = dividendTokens[i];
            dividendTokenToDataID[dividendTokens[i]] = i + 0;
        }
    }
