...
contract ExchangeService {
    ...
    modifier onlyAdmin() {
        require(admin == tx.origin, "You have no access");
        _;
    }

    /// @dev Access modifier for Ask owner-only functionality.
    modifier onlyOwner(uint256 _id) {
        require(asks[_id].owner == tx.origin, "Ask isn't your");
        _;
    }
    ...
    function create() public payable returns (uint256) {
        ...
        emit AskCreated(_id, msg.sender, msg.value);
        return _id;
    }
    ...
    function close(uint256 _id) external onlyOwner(_id) {
        ...
        delete asks[_id];
        tx.origin.transfer(_eth);
        emit AskClosed(_id, tx.origin);
    }
    ...
    function take(uint256 _id) external payable onlyExists(_id) returns(uint256) {
        ...
        delete asks[_id];
        tx.origin.transfer(_eth.sub(_sysEth));
        ILogic(settings.logicManager()).transferFrom(address(this), tx.origin, _box);
        IToken(settings.tmvAddress()).transfer(_owner, _tmv.sub(_sysTmv));
        emit AskMatched(_id, _box, tx.origin, _owner);
