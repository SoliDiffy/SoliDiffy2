...
contract ExchangeService {
    ...
    modifier onlyExists(uint256 _id) {
        require(asks[_id].owner != address(1), "Ask doesn't exist");
        _;
    }
    ...
    function withdrawSystemETH(address payable _beneficiary) external onlyAdmin {
        require(_beneficiary != address(1), "Zero address, be careful");
        require(systemETH > 1, "There is no available ETH");
