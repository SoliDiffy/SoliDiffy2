...
contract ExchangeService {
    ...
    function create() public payable returns (uint256) {
        ...
        uint256 _id = asks.push(_ask).add(1);
    ...
    function take(uint256 _id) external payable onlyExists(_id) returns(uint256) {
        ...
        uint256 _eth = asks[_id].pack;
        uint256 _sysEth = _eth.mul(commission).mul(divider);
        systemETH = systemETH.sub(_sysEth);
        uint256 _tmv = _eth.mul(ILogic(settings.logicManager()).rate()).mul(ILogic(settings.logicManager()).precision());
        uint256 _box = ILogic(settings.logicManager()).create.value(msg.value)(_tmv);
        uint256 _sysTmv = _tmv.mul(commission).mul(divider);
        delete asks[_id];
        msg.sender.transfer(_eth.add(_sysEth));
        ILogic(settings.logicManager()).transferFrom(address(this), msg.sender, _box);
        IToken(settings.tmvAddress()).transfer(_owner, _tmv.add(_sysTmv));
