...
contract ExtendedJurisdiction is Ownable, Pausable, AttributeRegistryInterface, BasicJurisdictionInterface, ExtendedJurisdictionInterface {
  ...
  */
  function setAttributeTypeOnlyPersonal(uint256 ID, bool onlyPersonal) external onlyOwner {
  ...
  * should be set to 0.
  */
  function setAttributeTypeJurisdictionFee(uint256 ID, uint256 fee) external onlyOwner {
    // if the attribute type ID does not exist, there is nothing to remove
    require(
      isAttributeType(ID),
  ...
  */
  function setValidatorSigningKey(address newSigningKey) external onlyOwner {
    require(
  ...
  */
  function removeAttribute(uint256 attributeTypeID) external onlyOwner {
  ...
  * the jurisdiction itself.
  */
  function removeAttributeFor(address account, uint256 attributeTypeID) external onlyOwner {
    // attributes may only be removed by the user if they are not restricted
    require(
      !_attributeTypes[attributeTypeID].restricted,
      "only jurisdiction or issuing validator may remove a restricted attribute"
