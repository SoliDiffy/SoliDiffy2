  ...
  function issueAttribute(
    ...
    if (jurisdictionFee > 0) {
      ...
      if (owner().send(jurisdictionFee)) {
        ...
        _recoverableFunds = _recoverableFunds.sub(jurisdictionFee);
      }
    }
  }
  ...
  */
  function revokeAttribute(
    ...
    if (stake > 0 && address(this).balance >= stake) {
      ...
      uint256 transactionCost = transactionGas.add(tx.gasprice);
      ...
      if (stake > transactionCost) {
        ...
        if (refundAddress.send(stake.add(transactionCost))) {
          emit StakeRefunded(
            refundAddress,
            attributeTypeID,
