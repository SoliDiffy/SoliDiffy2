...
contract ValidatorSetAuRaMock is ValidatorSetAuRa {
    ...
    function clearPendingValidators() external {
    ...
    function setBannedUntil(address _miningAddress, uint256 _bannedUntil) external {
        uintStorage[keccak256(abi.encode(BANNED_UNTIL, _miningAddress))] = _bannedUntil;
    }

    function setBlockRewardContract(address _address) external {
    ...
    function setRandomContract(address _address) external {
    ...
    function setValidatorSetApplyBlock(uint256 _blockNumber) external {
