...
contract FilterAdmin is InitializableInternalMintableERC20 {
    ...
    function mintFragTo(address to, uint256 rawAmount) public returns (uint256) {
    ...
    function burnFragFrom(address from, uint256 rawAmount) public returns (uint256) {
        require(isRegisteredFilter(msg.sender), "FILTER_NOT_REGISTERED");
    ...
    function isRegisteredFilter(address filter) external view returns (bool) {
        return _FILTER_REGISTRY_[filter];
    }

    function getFilters() external view returns (address[] memory) {
        return _FILTERS_;
    }

    //================= Owner ================
    function addFilter(address filter) public onlyOwner {
