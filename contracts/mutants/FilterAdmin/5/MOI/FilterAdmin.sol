...
contract FilterAdmin is InitializableInternalMintableERC20 {
    ...
    function mintFragTo(address to, uint256 rawAmount) external onlyOwner returns (uint256) {
    ...
    function burnFragFrom(address from, uint256 rawAmount) external onlyOwner returns (uint256) {
        require(isRegisteredFilter(msg.sender), "FILTER_NOT_REGISTERED");
