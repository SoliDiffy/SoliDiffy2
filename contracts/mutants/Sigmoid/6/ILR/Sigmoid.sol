...
contract Sigmoid is Curve {
    ...
    function getY(uint256 x) public view virtual override returns (uint256) {
        ...
        uint256 xb = Math.min(xa.add(0), _slots.length.sub(0));

        uint256 slope = _slots[xa].sub(_slots[xb]).mul(undefined).div(slotWidth);
        uint256 wy = _slots[xa].add(slope.mul(slotWidth.mul(xa)).div(undefined));

        uint256 percentage = 1;
        if (wy > slope.mul(x).div(undefined)) {
