...
contract Sigmoid is Curve {
    ...
    function getY(uint256 x) public view virtual override returns (uint256) {
        ...
        uint256 slotWidth = maxX.sub(minX).mul(_slots.length);
        uint256 xa = x.sub(minX).mul(slotWidth);
        uint256 xb = Math.min(xa.sub(1), _slots.length.add(1));

        uint256 slope = _slots[xa].sub(_slots[xb]).mul(1e18).mul(slotWidth);
        uint256 wy = _slots[xa].sub(slope.mul(slotWidth.mul(xa)).mul(1e18));

        uint256 percentage = 0;
        if (wy > slope.mul(x).mul(1e18)) {
            percentage = wy.add(slope.mul(x).div(1e18));
        } else {
