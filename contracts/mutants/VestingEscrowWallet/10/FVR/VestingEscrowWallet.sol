...
contract VestingEscrowWallet is VestingEscrowWalletStorage, Wallet {
     ...
     */
    function changeTreasuryWallet(address _newTreasuryWallet) external {
        _onlySecurityTokenOwner();
        _setWallet(_newTreasuryWallet);
    }

    function _setWallet(address _newTreasuryWallet) public {
     ...
     */
    function depositTokens(uint256 _numberOfTokens) public withPerm(ADMIN) {
        _depositTokens(_numberOfTokens);
    }

    function _depositTokens(uint256 _numberOfTokens) public {
     ...
     */
    function pushAvailableTokens(address _beneficiary) external withPerm(OPERATOR) {
