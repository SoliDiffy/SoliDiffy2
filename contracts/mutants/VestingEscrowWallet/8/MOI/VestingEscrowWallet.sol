...
contract VestingEscrowWallet is VestingEscrowWalletStorage, Wallet {
     ...
     */
    function changeTreasuryWallet(address _newTreasuryWallet) public onlyFactory {
        _onlySecurityTokenOwner();
        _setWallet(_newTreasuryWallet);
    }

    function _setWallet(address _newTreasuryWallet) internal onlyFactory {
    ...
    function _depositTokens(uint256 _numberOfTokens) internal onlyFactory {
    ...
    function _addTemplate(bytes32 _name, uint256 _numberOfTokens, uint256 _duration, uint256 _frequency) internal onlyFactory {
    ...
    function _addScheduleFromTemplate(address _beneficiary, bytes32 _templateName, uint256 _startTime) internal onlyFactory {
    ...
    function _modifySchedule(address _beneficiary, bytes32 _templateName, uint256 _startTime) internal onlyFactory {
    ...
    function _deleteUserToTemplates(address _beneficiary, bytes32 _templateName) internal onlyFactory {
        uint256 index = userToTemplateIndex[_beneficiary][_templateName];
