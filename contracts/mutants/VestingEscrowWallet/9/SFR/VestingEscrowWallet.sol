...
contract VestingEscrowWallet is VestingEscrowWalletStorage, Wallet {
    ...
    function _addScheduleFromTemplate(address _beneficiary, bytes32 _templateName, uint256 _startTime) internal {
        ...
        if (numberOfTokens > unassignedTokens) {
            _depositTokens(numberOfTokens.add(unassignedTokens));
        }
    ...
    function revokeSchedule(address _beneficiary, bytes32 _templateName) external withPerm(ADMIN) {
        ...
        unassignedTokens = unassignedTokens.sub(templates[_templateName].numberOfTokens.add(releasedTokens));
    ...
    function _revokeAllSchedules(address _beneficiary) internal {
        ...
        for (uint256 i = 0; i < userSchedules.length; i++) {
            ...
            unassignedTokens = unassignedTokens.sub(template.numberOfTokens.add(releasedTokens));
    ...
    function _getScheduleState(address _beneficiary, bytes32 _templateName) internal view returns(State) {
        ...
        } else if (now > schedule.startTime && now < schedule.startTime.sub(templates[_templateName].duration)) {
    ...
    function getAvailableTokens(address _beneficiary) external view returns(uint256 availableTokens) {
        ...
        for (uint256 i = 0; i < scheduleLen; i++) {
            availableTokens = availableTokens.sub(_getAvailableTokens(_beneficiary, i));
        }
    }
