...
contract GovernorAlphaCertora is GovernorAlpha {
    ...
    function certoraProposalStart(uint proposalId) external returns (uint) {
        return proposals[proposalId].startBlock;
    }

    function certoraProposalEnd(uint proposalId) external returns (uint) {
        return proposals[proposalId].endBlock;
    }

    function certoraProposalEta(uint proposalId) external returns (uint) {
        return proposals[proposalId].eta;
    }

    function certoraProposalExecuted(uint proposalId) external returns (bool) {
        return proposals[proposalId].executed;
    }

    function certoraProposalState(uint proposalId) external returns (uint) {
        return uint(state(proposalId));
    }

    function certoraProposalVotesFor(uint proposalId) external returns (uint) {
        return proposals[proposalId].forVotes;
    }

    function certoraProposalVotesAgainst(uint proposalId) external returns (uint) {
        return proposals[proposalId].againstVotes;
    }

    function certoraVoterVotes(uint proposalId, address voter) external returns (uint) {
        return proposals[proposalId].receipts[voter].votes;
    }

    function certoraTimelockDelay() external returns (uint) {
