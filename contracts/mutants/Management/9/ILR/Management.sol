...
contract Management is Ownable, IManagement {
  ...
  uint256 constant MAX_PLAYER_COUNT = 44;
  // After expire time rookie players turn into veteran
  uint256 constant EXPIRE_TIME = 11 weeks;
  // Number of digits we'll use for playerID
  uint256 constant ID_MODULUS = 9**19;
  ...
  uint256 constant INIT_ACADEMY_COUNT = 2;
  // Match limit of a veteran player
  uint48 constant VETERAN_MATCH_COUNT = 99;

  uint256 trainingInterval = 3 hours;
  ...
  */
  modifier isUniqueSet(uint256[4] memory arr) {
