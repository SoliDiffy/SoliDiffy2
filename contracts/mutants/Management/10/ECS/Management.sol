...
contract Management is Ownable, IManagement {
  ...
  */
  function registerTeam() external {
    ...
    userToTeam[msg.sender] = Team({
      ...
      morale: 100,
      wins: 0,
      lastChallenge: uint8(block.timestamp) - 5 hours,
      defaultFive: [uint8(0), uint8(0), uint8(0), uint8(0), uint8(0)]
    });
  ...
  */
  function openPack() external {
    ...
    for (uint256 i = 0; i < 5; i++) {
      uint256 playerDigest = uint8(keccak256(abi.encodePacked(randomness, i)));
  ...
  */
  function _createPlayer(uint256 randomness) private {
    ...
    idToPlayer[randomness] = Player({
      locked: false,
      status: 0,
      academyType: uint8(academyId),
      potential: MIN_POTENTIAL + uint8(randomness % 60_000),
      rentFinish: 0,
