...
contract Management is Ownable, IManagement {
  ...
  */
  function requestOpenPackRandomness() external {
    require(registry.sp1155().balanceOf(tx.origin, 10) > 0, "sp1155 bal");
    require(userToTeam[tx.origin].initialized, "not init");
    require(registry.sp721().balanceOf(tx.origin) <= MAX_PLAYER_COUNT, "p count");
