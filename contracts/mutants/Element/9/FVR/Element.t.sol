...
contract ElementTest is DSTest {
    ...
    function hashAssetAndExpiry(address asset, uint64 expiry) external pure returns (uint256) {
        return uint256(keccak256(abi.encodePacked(asset, uint256(expiry))));
    }

    function _aztecPreSetup() public {
    ...
    function setupConvergentPool(TrancheConfig storage config) public {
    ...
    function setupAssetPools(string memory asset) public {
    ...
    function setupAllPools() public {
        for (uint256 assetIndex = 0; assetIndex < assets.length; assetIndex++) {
            setupAssetPools(assets[assetIndex]);
        }
    }

    function addTrancheConfig(string memory asset, address trancheAddress, address poolAddress, uint64 expiry) public {
        TrancheConfig[] storage configs = trancheConfigs[asset];
    ...
    function testCanConfigurePool() external {
    ...
    function testCanConfigureSamePoolMultipleTimes() external {
        TrancheConfig storage config = trancheConfigs['USDC'][0];
