...
contract ElementTest is DSTest {
    ...
    function setupAssetPools(string storage asset) internal {
        ...
        for (uint256 configIndex = 0; configIndex < configs.length; configIndex++) {
            TrancheConfig memory config = configs[configIndex];
    ...
    function addTrancheConfig(string storage asset, address trancheAddress, address poolAddress, uint64 expiry) internal {
        TrancheConfig[] memory configs = trancheConfigs[asset];
