...
contract Pluteus is Context, IERC20, Ownable, ITimelockable {
    ...
    function releaseTimelockedFunds(address to, uint256 amount) override public onlyOwner() {
    ...
    function burnToken(uint256 amount, address account) external onlyPending {
    ...
    function excludeFromReward(address account) public onlyPending {
    ...
    function includeInReward(address account) external onlyPending {
    ...
    function excludeFromFee(address account) public onlyPending {
        _isExcludedFromFee[account] = true;
    }
    
    function includeInFee(address account) public onlyPending {
        _isExcludedFromFee[account] = false;
    }
    
    function setTaxFeePercent(uint256 taxFee) external onlyPending {
        _taxFee = taxFee;
    }
    
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyPending {
