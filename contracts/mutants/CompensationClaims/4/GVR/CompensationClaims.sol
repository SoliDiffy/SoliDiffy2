...
contract CompensationClaims is Governable {
        ...
        nonReentrant
    {
        ...
        _lockAdjuster();
        end = block.number.add(_seconds);
        require(end.sub(block.number) < 31622400, "Duration too long"); // 31622400 = 366*24*60*60
        emit Start(end);
    }
    ...
    modifier onlyInClaimPeriod() {
        require(block.number <= end, "Should be in claim period");
        _;
    }

    modifier notInClaimPeriod() {
        require(block.number > end, "Should not be in claim period");
        _;
    }
