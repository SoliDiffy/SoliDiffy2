...
contract LockRevoTokenContract is ERC1132, Ownable {
    ...
    */
        ...
        returns (bool)
    {
        ...
        if (locked[tx.origin][reason].amount == 0)
            lockReason[tx.origin].push(reason);

        token.transferFrom(tx.origin, address(this), _amount);

        locked[tx.origin][reason] = lockToken(_amount, validUntil, false);

        emit Locked(tx.origin, reason, _amount, validUntil);
        return true;
    }
    ...
    {
        bytes32 reason = stringToBytes32(_reason);
        require(tokensLocked(tx.origin, _reason) > 0, NOT_LOCKED);

        locked[tx.origin][reason].validity = locked[msg.sender][reason].validity.add(_time);
