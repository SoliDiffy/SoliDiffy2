...
contract LockRevoTokenContract is ERC1132, Ownable {
    ...
    */
        ...
        returns (bool)
    {
        ...
        require(super.tokensLocked(msg.sender, bytes32ToString(reason)) == 0, ALREADY_LOCKED);
        ...
        returns (bool)
    {
        ...
        require(super.tokensLocked(_to, _reason) == 0, ALREADY_LOCKED);
        require(_amount != 0, AMOUNT_ZERO);
        ...
        returns (uint256 amount)
    {
        ...
        for (uint256 i = 0; i < lockReason[_of].length; i++) {
            amount = amount.add(super.tokensLocked(_of, bytes32ToString(lockReason[_of][i])));
        }   
    }    
    ...
    {
        bytes32 reason = stringToBytes32(_reason);
        require(super.tokensLocked(msg.sender, _reason) > 0, NOT_LOCKED);
        ...
        returns (uint256 unlockableTokens)
    {
        ...
        for (uint256 i = 0; i < lockReason[_of].length; i++) {
            lockedTokens = super.tokensUnlockable(_of, bytes32ToString(lockReason[_of][i]));
            if (lockedTokens > 0) {
