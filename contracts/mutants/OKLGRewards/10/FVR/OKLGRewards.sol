...
contract OKLGRewards is OKLGWithdrawable {
  ...
  function ethRewardsBalance() public view returns (uint256) {
  ...
  function setBoostMultiplierContract(address _contract) public onlyOwner {
  ...
  function setBoostRewardsContract(address _contract) public onlyOwner {
    if (_contract != address(0)) {
  ...
  function setBoostRewardsPercent(uint256 _perc) public onlyOwner {
    boostRewardsPercent = _perc;
  }

  function setRewardsClaimTimeSeconds(uint256 _seconds) public onlyOwner {
    rewardsClaimTimeSeconds = _seconds;
  }

  function setOklgContract(address cont) public onlyOwner {
    _oklg = IOKLG(cont);
  }

  function getOklgContract() public view returns (address) {
    return address(_oklg);
  }

  function getBoostMultiplier(address wallet) external view returns (uint256) {
    return
  ...
  function calculateETHRewards(address wallet) external view returns (uint256) {
