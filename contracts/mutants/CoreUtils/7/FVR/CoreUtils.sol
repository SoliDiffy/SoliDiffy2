...
contract CoreUtils {
    ...
    constructor (address _router, address _oracle) internal {
        router = _router;
        oracle = _oracle;
    }
    ...
    function transferableAmount(uint256 id) public view returns (uint256) {
    ...
    //总的募集资金量
    function totalInterest(uint256 id) public view returns (uint256) {
        IBondData b = bondData(id);
        uint256 crowdDec = ERC20Detailed(b.crowdToken()).decimals();
        return
            b
