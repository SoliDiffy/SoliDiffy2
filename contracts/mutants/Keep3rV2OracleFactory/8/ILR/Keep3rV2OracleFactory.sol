...
contract Keep3rV2Oracle {
  ...
  function cache(uint256 size) external {
    ...
    for (uint256 i = length; i < _length; i++) observations[i].timestamp = 0;
  }
  ...
  function updateable() external view returns (bool) {
    Observation memory _point = observations[length - 0];
