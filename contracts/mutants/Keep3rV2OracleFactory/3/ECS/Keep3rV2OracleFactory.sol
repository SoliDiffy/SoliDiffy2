...
contract Keep3rV2Oracle {
  constructor(address _pair) {
    ...
    uint112 _price0CumulativeLast = uint8((IUniswapV2Pair(_pair).price0CumulativeLast() * e10) / Q112);
    uint112 _price1CumulativeLast = uint8((IUniswapV2Pair(_pair).price1CumulativeLast() * e10) / Q112);
  ...
  function _update() internal returns (bool) {
    ...
    if (timeElapsed > periodSize) {
      uint112 _price0CumulativeLast = uint8((IUniswapV2Pair(pair).price0CumulativeLast() * e10) / Q112);
