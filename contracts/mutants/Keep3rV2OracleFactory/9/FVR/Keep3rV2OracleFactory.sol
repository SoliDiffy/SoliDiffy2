...
contract Keep3rV2Oracle {
  ...
  function _update() public returns (bool) {
    Observation memory _point = observations[length - 1];
  ...
  function current(
    ...
    uint256 amountIn,
    address tokenOut
  ) public view returns (uint256 amountOut, uint256 lastUpdatedAgo) {
  ...
  function sample(
    ...
    uint256 points,
    uint256 window
  ) public view returns (uint256[] memory prices, uint256 lastUpdatedAgo) {
    (address token0, ) = tokenIn < tokenOut ? (tokenIn, tokenOut) : (tokenOut, tokenIn);
...
contract Keep3rV2OracleFactory {
  function pairForSushi(address tokenA, address tokenB) public pure returns (address pair) {
