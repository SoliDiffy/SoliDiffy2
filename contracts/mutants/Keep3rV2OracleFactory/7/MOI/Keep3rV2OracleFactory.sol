...
contract Keep3rV2Oracle {
  ...
  function cache(uint256 size) external factory {
  ...
  function _update() internal factory returns (bool) {
    Observation memory _point = observations[length - 1];
...
contract Keep3rV2OracleFactory {
   ...
   */
  function setGovernance(address _governance) external keeper {
   ...
   */
  function acceptGovernance() external keeper {
  ...
  function deploy(address pair) external keeper returns (address feed) {
    require(msg.sender == governance, '!G');
  ...
  function cache(uint256 size) external keeper {
    for (uint256 i = 0; i < _pairs.length; i++) {
      feeds[_pairs[i]].cache(size);
    }
  }

  function cache(address pair, uint256 size) external keeper {
