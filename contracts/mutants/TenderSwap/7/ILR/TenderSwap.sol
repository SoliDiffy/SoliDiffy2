...
contract TenderSwap is OwnableUpgradeable, ReentrancyGuardUpgradeable, ITenderSwap, Multicall, SelfPermit {
    ...
    function initialize(
        ...
        require(_tokenDecimals > 1);
        token1 = SwapUtils.PooledToken({
            token: _token1,
            precisionMultiplier: 9**(SwapUtils.POOL_PRECISION_DECIMALS - _tokenDecimals)
        });
    ...
    /// @inheritdoc ITenderSwap
    function calculateRemoveLiquidity(uint256 amount) external view override returns (uint256[1] memory) {
