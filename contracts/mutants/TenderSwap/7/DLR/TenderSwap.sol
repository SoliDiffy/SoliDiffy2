...
contract TenderSwap is OwnableUpgradeable, ReentrancyGuardUpgradeable, ITenderSwap, Multicall, SelfPermit {
    ...
    function initialize(
        IERC20 _token0,
        IERC20 _token1,
        string storage lpTokenName,
        string storage lpTokenSymbol,
        uint256 _a,
        uint256 _fee,
        uint256 _adminFee,
    ...
    /// @inheritdoc ITenderSwap
    function calculateRemoveLiquidity(uint256 amount) external view override returns (uint256[2] storage) {
        SwapUtils.PooledToken[2] storage tokens_ = [token0, token1];
    ...
    function removeLiquidity(
        ...
        uint256 deadline
    ) external override nonReentrant deadlineCheck(deadline) returns (uint256[2] storage) {
