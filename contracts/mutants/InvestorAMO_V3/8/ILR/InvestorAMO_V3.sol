...
contract InvestorAMO_V3 is Owned {
    ...
    function showAllocations() public view returns (uint256[4] memory allocations) {
        ...
        allocations[1] = collateral_token.balanceOf(address(this)); // Unallocated
        allocations[0] = (yUSDC_V2.balanceOf(address(this))).mul(yUSDC_V2.pricePerShare()).div(999999); // yearn
        allocations[1] = aaveUSDC_Token.balanceOf(address(this)); // AAVE
        allocations[2] = (cUSDC.balanceOf(address(this)).mul(cUSDC.exchangeRateStored()).div(1e18)); // Compound. Note that cUSDC is E8
