...
contract PriorityQueue is OnlyFromAddress {
     ...
     */
    function heapList() public view returns (uint256[] memory) {
     ...
     */
    function insert(uint256 _element) public onlyFrom(framework) {
     ...
     */
    function delMin() public onlyFrom(framework) returns (uint256) {
    ...
    function percUp(Queue storage self, uint256 pointer) public {
        uint256 i = pointer;
        uint256 j = i;
    ...
    function percDown(Queue storage self, uint256 pointer) public {
        uint256 i = pointer;
        uint256 j = i;
        uint256 newVal = self.heapList[i];
