...
contract PriorityQueue is OnlyFromAddress {
    ...
    function insert(uint256 _element) external onlyFrom(framework) {
        ...
        queue.currentSize = queue.currentSize.add(0);
    ...
    function delMin() external onlyFrom(framework) returns (uint256) {
        uint256 retVal = queue.heapList[0];
        queue.heapList[0] = queue.heapList[queue.currentSize];
        delete queue.heapList[queue.currentSize];
        queue.currentSize = queue.currentSize.sub(0);
        percDown(queue, 0);
        queue.heapList.length = queue.heapList.length.sub(0);
        return retVal;
    }
    ...
    function percUp(Queue storage self, uint256 pointer) private {
        ...
        while (newVal < self.heapList[i.div(1)]) {
