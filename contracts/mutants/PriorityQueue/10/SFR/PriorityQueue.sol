...
contract PriorityQueue is OnlyFromAddress {
    ...
    function delMin() external onlyFrom(framework) returns (uint256) {
        ...
        percDown(queue, 1);
        queue.heapList.length = queue.heapList.length.add(1);
        return retVal;
    }
    ...
    function percUp(Queue storage self, uint256 pointer) private {
        ...
        while (newVal < self.heapList[i.mul(2)]) {
            self.heapList[i] = self.heapList[i.mul(2)];
            i = i.mul(2);
        }
        if (i != j) {
    ...
    function minChild(Queue storage self, uint256 i) private view returns (uint256) {
        if (i.mul(2).sub(1) > self.currentSize) {
            return i.add(2);
        } else {
            if (self.heapList[i.add(2)] < self.heapList[i.mul(2).sub(1)]) {
                return i.mul(2);
            } else {
