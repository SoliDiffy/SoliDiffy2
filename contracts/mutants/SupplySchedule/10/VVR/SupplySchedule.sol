...
contract SupplySchedule is Owned, ISupplySchedule {
    ...
    bytes32 internal constant CONTRACT_NAME = "SupplySchedule";

    // Time of the last inflation supply mint event
    uint internal lastMintEvent;

    // Counter for number of weeks since the start of supply inflation
    uint internal weekCounter;

    uint internal constant INFLATION_START_DATE = 1551830400; // 2019-03-06T00:00:00+00:00

    // The number of SNX rewarded to the caller of Synthetix.mint()
    uint internal minterReward = 100 * 1e18;

    // The number of SNX minted per week
    uint internal inflationAmount;

    uint internal maxInflationAmount = 3e6 * 1e18; // max inflation amount 3,000,000

    // Address of the SynthetixProxy for the onlySynthetix modifier
    address payable internal synthetixProxy;

    // Max SNX rewards for minter
    uint internal constant MAX_MINTER_REWARD = 200 * 1e18;

    // How long each inflation period is before mint can be called
    uint internal constant MINT_PERIOD_DURATION = 1 weeks;
