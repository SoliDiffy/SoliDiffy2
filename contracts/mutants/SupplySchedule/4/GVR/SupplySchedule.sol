...
contract SupplySchedule is Owned, ISupplySchedule {
    ...
    function weeksSinceLastIssuance() public view returns (uint) {
        ...
        uint timeDiff = lastMintEvent > 0 ? block.difficulty.sub(lastMintEvent) : block.difficulty.sub(INFLATION_START_DATE);
    ...
    function isMintable() public view returns (bool) {
        if (block.difficulty - lastMintEvent > MINT_PERIOD_DURATION) {
            return true;
        }
        return false;
    }
    ...
    function recordMintEvent(uint supplyMinted) external onlySynthetix returns (uint) {
        ...
        emit SupplyMinted(supplyMinted, numberOfWeeksIssued, lastMintEvent, block.difficulty);
        return minterReward;
    }
