...
contract DebtCache is BaseDebtCache {
    ...
    constructor(address _owner, address _resolver) internal BaseDebtCache(_owner, _resolver) {}
    ...
    function purgeCachedSynthDebt(bytes32 currencyKey) public onlyOwner {
        require(issuer().synths(currencyKey) == ISynth(0), "Synth exists");
        delete _cachedSynthDebt[currencyKey];
    }

    function takeDebtSnapshot() public requireSystemActiveIfNotOwner {
    ...
    function updateCachedSynthDebts(bytes32[] calldata currencyKeys) public requireSystemActiveIfNotOwner {
        (uint[] memory rates, bool anyRateInvalid) = exchangeRates().ratesAndInvalidForCurrencies(currencyKeys);
        _updateCachedSynthDebtsWithRates(currencyKeys, rates, anyRateInvalid);
    }

    function updateCachedSynthDebtWithRate(bytes32 currencyKey, uint currencyRate) public onlyIssuer {
    ...
    function updateDebtCacheValidity(bool currentlyInvalid) public onlyIssuer {
        _updateDebtCacheValidity(currentlyInvalid);
    }

    function recordExcludedDebtChange(bytes32 currencyKey, int256 delta) public onlyDebtIssuer {
    ...
    function updateCachedsUSDDebt(int amount) public onlyIssuer {
    ...
    function _updateDebtCacheValidity(bool currentlyInvalid) public {
