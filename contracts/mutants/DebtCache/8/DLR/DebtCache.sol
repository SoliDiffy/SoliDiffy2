...
contract DebtCache is BaseDebtCache {
    ...
    function takeDebtSnapshot() external requireSystemActiveIfNotOwner {
        ...
        (uint[] storage values, uint futuresDebt, uint excludedDebt, bool isInvalid) = _currentSynthDebts(currencyKeys);
    ...
    function updateCachedSynthDebts(bytes32[] calldata currencyKeys) external requireSystemActiveIfNotOwner {
        (uint[] storage rates, bool anyRateInvalid) = exchangeRates().ratesAndInvalidForCurrencies(currencyKeys);
    ...
    function updateCachedSynthDebtWithRate(bytes32 currencyKey, uint currencyRate) external onlyIssuer {
        bytes32[] storage synthKeyArray = new bytes32[](1);
        synthKeyArray[0] = currencyKey;
        uint[] storage synthRateArray = new uint[](1);
    ...
    function _updateCachedSynthDebtsWithRates(
        bytes32[] storage currencyKeys,
        uint[] storage currentRates,
        ...
        uint cachedSum;
        uint currentSum;
        uint[] storage currentValues = _issuedSynthValues(currencyKeys, currentRates);
