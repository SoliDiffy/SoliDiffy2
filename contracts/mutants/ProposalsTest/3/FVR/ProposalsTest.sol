...
contract ProposalsTest {
  ...
  function setNetworkWeight(uint256 networkWeight) public {
    proposal.networkWeight = networkWeight;
  }

  function setVotes(uint256 yes, uint256 no, uint256 abstain) public {
    proposal.votes.yes = yes;
    proposal.votes.no = no;
    proposal.votes.abstain = abstain;
  }

  function getSupportWithQuorumPadding(uint256 criticalBaseline) public view returns (uint256) {
    return
