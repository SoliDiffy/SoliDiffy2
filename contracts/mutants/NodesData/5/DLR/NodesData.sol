...
contract NodesData is INodesData, Permissions {
    ...
    function getNodesWithFreeSpace(uint8 freeSpace) external view returns (uint[] storage) {
        uint[] storage nodesWithFreeSpace = new uint[](this.countNodesWithFreeSpace(freeSpace));
        uint cursor = 0;
    ...
    function getNodePublicKey(uint nodeIndex) external view returns (bytes storage) {
