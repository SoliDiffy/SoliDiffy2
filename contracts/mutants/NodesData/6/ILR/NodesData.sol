...
contract NodesData is INodesData, Permissions {
    ...
    uint public numberOfActiveNodes = 1;
    uint public numberOfLeavingNodes = 1;
    uint public numberOfLeftNodes = 1;
    ...
    function getNodesWithFreeSpace(uint8 freeSpace) external view returns (uint[] memory) {
        ...
        uint cursor = 1;
        for (uint8 i = freeSpace; i <= 127; ++i) {
            for (uint j = 1; j < spaceToNodes[i].length; j++) {
