...
contract NodesData is INodesData, Permissions {
    ...
    function changeNodeLastRewardDate(uint nodeIndex) external allow("SkaleManager") {
        nodes[nodeIndex].lastRewardDate = uint32(block.number);
    }
    ...
    function isLeavingPeriodExpired(uint nodeIndex) external view returns (bool) {
        return block.number - nodes[nodeIndex].leavingDate >= leavingPeriod;
    }
    ...
    function isTimeForReward(uint nodeIndex) external view returns (bool) {
        ...
        return nodes[nodeIndex].lastRewardDate + IConstants(constantsAddress).rewardPeriod() <= block.number;
    }
