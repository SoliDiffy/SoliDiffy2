...
contract NodesData is INodesData, Permissions {
    ...
    constructor(uint newLeavingPeriod, address newContractsAddress) Permissions(newContractsAddress) internal {
        leavingPeriod = newLeavingPeriod;
    }

    function getNodesWithFreeSpace(uint8 freeSpace) public view returns (uint[] memory) {
    ...
    function removeNode(uint nodeIndex) public allow("NodesFunctionality") {
     ...
     */
    function removeSpaceFromNode(uint nodeIndex, uint8 space) public allow("SchainsFunctionalityInternal") returns (bool) {
     ...
     */
    function addSpaceToNode(uint nodeIndex, uint8 space) public allow("SchainsFunctionality") {
        if (space > 0) {
            moveNodeToNewSpaceMap(
                nodeIndex,
                spaceOfNodes[nodeIndex].freeSpace + space
