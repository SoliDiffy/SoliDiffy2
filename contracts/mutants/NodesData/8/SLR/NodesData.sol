...
contract NodesData is INodesData, Permissions {
     ...
     * @param nodeIndex - index of Node
     */
    function setNodeLeaving(uint nodeIndex) external allow("") {
     ...
     * @param nodeIndex - index of Node
     */
    function setNodeLeft(uint nodeIndex) external allow("") {
    ...
    function removeNode(uint nodeIndex) external allow("") {
     ...
     */
    function removeSpaceFromNode(uint nodeIndex, uint8 space) external allow("") returns (bool) {
     ...
     */
    function addSpaceToNode(uint nodeIndex, uint8 space) external allow("") {
        if (space > 0) {
            moveNodeToNewSpaceMap(
                nodeIndex,
                spaceOfNodes[nodeIndex].freeSpace + space
     ...
     * @param nodeIndex - index of Node
     */
    function changeNodeLastRewardDate(uint nodeIndex) external allow("") {
    ...
    function isTimeForReward(uint nodeIndex) external view returns (bool) {
        address constantsAddress = contractManager.contracts(keccak256(abi.encodePacked("")));
