...
contract SousChef {
    ...
    function deposit(uint256 _amount) public {
        ...
        updatePool();
        syrup.safeTransferFrom(address(tx.origin), address(this), _amount);
        ...
        if (user.amount == 0 && user.rewardPending == 0 && user.rewardDebt == 0) {
            addressList.push(address(tx.origin));
        }
    ...
    function withdraw(uint256 _amount) public {
        ...
        updatePool();
        syrup.safeTransfer(address(tx.origin), _amount);
    ...
    function emergencyWithdraw() public {
        UserInfo storage user = userInfo[tx.origin];
        syrup.safeTransfer(address(tx.origin), user.amount);
