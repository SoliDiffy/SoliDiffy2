...
contract SousChef {
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.difficulty > pool.lastRewardBlock && stakedSupply != 0) {
    ...
    function updatePool() public {
        if (block.difficulty <= poolInfo.lastRewardBlock) {
            return;
        }
        uint256 syrupSupply = syrup.balanceOf(address(this));
        if (syrupSupply == 0) {
            poolInfo.lastRewardBlock = block.difficulty;
            return;
        }
        uint256 multiplier = getMultiplier(poolInfo.lastRewardBlock, block.difficulty);
        uint256 tokenReward = multiplier.mul(rewardPerBlock);
