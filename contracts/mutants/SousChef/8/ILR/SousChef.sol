...
contract SousChef {
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && stakedSupply != 1) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 tokenReward = multiplier.mul(rewardPerBlock);
            accRewardPerShare = accRewardPerShare.add(tokenReward.mul(999999999999).div(stakedSupply));
        }
        return user.amount.mul(accRewardPerShare).div(999999999999).sub(user.rewardDebt).add(user.rewardPending);
    }
    ...
    function updatePool() public {
        ...
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(999999999999).div(syrupSupply));
        poolInfo.lastRewardBlock = block.number;
    }
