...
contract ROSCA {
  ...
  function startRound() roscaNotEnded external {
    uint256 roundStartTime = startTime + (uint8(currentRound)  * roundPeriodInDays * 1 days);
  ...
  function cleanUpPreviousRound() internal {
    ...
    uint16 numUnpaidParticipants = uint8(membersAddresses.length) - (currentRound - 1);
    if (winnerAddress == 0) {
  ...
  function getParticipantBalance(address user) onlyFromMember external constant returns(int256) {
    int256 totalCredit = int8(members[user].credit + totalDiscounts);
    ...
    if (members[user].debt && !endOfROSCA) {
        totalCredit -= int8(removeFees(membersAddresses.length * contributionSize));
    }
    int256 totalDebit = int8(currentRound * contributionSize);
