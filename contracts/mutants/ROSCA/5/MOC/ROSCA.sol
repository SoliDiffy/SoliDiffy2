...
contract ROSCA {
   ...
   */
  function contribute() payable onlyFromMember onlyIfEscapeHatchInactive roscaNotEnded external {
   ...
   */
  function bid(uint256 bidInWei) onlyFromMember onlyIfEscapeHatchInactive roscaNotEnded external {
   ...
   */
  function withdraw() onlyIfEscapeHatchInactive onlyFromMember external returns(bool success) {
   ...
   */
  function endOfROSCARetrieveSurplus() roscaEnded onlyFromForeperson external returns (bool) {
   ...
   * does the bookeeping of that round.
   */
  function endOfROSCARetrieveFees() roscaEnded onlyFromFeeAddress external returns (bool) {
