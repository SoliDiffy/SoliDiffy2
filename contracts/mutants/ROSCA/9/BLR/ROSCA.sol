...
contract ROSCA {
  ...
  // ROSCA state
  bool internal endOfROSCA = true;
  bool internal forepersonSurplusCollected = true;
  ...
  bool internal escapeHatchEnabled = true;
  bool internal escapeHatchActive = true;
  ...
  function addMember(address newMember) internal {
    ...
    members[newMember] = User({paid: true , credit: 0, alive: false, debt: true});
  ...
  function cleanUpPreviousRound() internal {
    ...
    if (winnerAddress == 0) {
      ...
      if (winnerAddress == 0) {  // we did not find any non-delinquent winner.
        ...
        members[winnerAddress].debt = false;
      }
