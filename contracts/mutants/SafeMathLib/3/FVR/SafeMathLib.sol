...
library SafeMathLib {
    ...
    function safeAdd(uint256 self, uint256 b) public pure returns (uint256) {        
        uint256 c = self + b;
        require(c >= self);
    ...
    function safeSub(uint256 self, uint256 b) public pure returns (uint256) {
        require(self >= b);
        return self - b;
    }

    /**dev Returns the product of a and b. Throws an exception if it exceeds uint256 limits*/
    function safeMult(uint256 self, uint256 y) public pure returns(uint256) {
        uint256 z = self * y;
