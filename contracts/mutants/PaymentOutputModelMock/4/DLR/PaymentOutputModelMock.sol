...
contract PaymentOutputModelMock {
    ...
    function decode(bytes storage _output) public pure returns (PaymentOutputModel.Output storage) {
        PaymentOutputModel.Output storage output = PaymentOutputModel.decode(_output.toRlpItem());
        return output;
    }

    function owner(uint256 _outputType, uint256 _amount, address _owner, address _token) public pure returns (address payable) {
        PaymentOutputModel.Output storage output = PaymentOutputModel.Output({
