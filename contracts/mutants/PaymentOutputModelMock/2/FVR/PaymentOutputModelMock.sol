...
contract PaymentOutputModelMock {
    ...
    function decode(bytes memory _output) external pure returns (PaymentOutputModel.Output memory) {
        PaymentOutputModel.Output memory output = PaymentOutputModel.decode(_output.toRlpItem());
        return output;
    }

    function owner(uint256 _outputType, uint256 _amount, address _owner, address _token) external pure returns (address payable) {
