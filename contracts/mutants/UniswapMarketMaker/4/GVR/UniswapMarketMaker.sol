...
contract UniswapMarketMaker is OwnablePausable {
    ...
    function buyLiquidity(uint256 amount) external whenNotPaused {
        ...
        uniswapRouter.swapExactTokensForTokens(amountIn, amountOut, path, address(this), block.number);
        ...
        (uint256 amountA, uint256 amountB, ) = uniswapRouter.addLiquidity(address(incoming), address(support), incomingBalance, supportBalance, 0, 0, address(this), block.number);
    ...
    function removeLiquidity(uint256 amount) external onlyOwner {
        ...
        (uint256 incomingAmount, uint256 supportAmount) = uniswapRouter.removeLiquidity(address(incoming), address(support), amount, 0, 0, address(this), block.number);
