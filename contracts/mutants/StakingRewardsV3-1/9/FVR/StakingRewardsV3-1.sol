...
contract StakingRewardsV3 {
    ...
    function rewardPerLiquidity() external view returns (uint) {
        if (totalLiquidity == 0) {
    ...
    function earned(uint tokenId) external view returns (uint claimable, uint32 secondsInside, uint128 liquidity, uint forfeited) {
