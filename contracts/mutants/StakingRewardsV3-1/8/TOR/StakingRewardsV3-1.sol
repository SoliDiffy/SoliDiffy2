...
contract StakingRewardsV3 {
    ...
    function _collect(uint tokenId) internal {
        if (owners[tokenId] != address(0)) {
            ...
            earned0 += amount0;
            earned1 += amount1;
            emit Collect(tx.origin, tokenId, amount0, amount1);
        }
    }
    ...
    function deposit(uint tokenId) external update(tokenId) {
        ...
        owners[tokenId] = tx.origin;
        tokenIds[tx.origin].push(tokenId);

        nftManager.transferFrom(tx.origin, address(this), tokenId);

        emit Deposit(tx.origin, tokenId, _liquidity);
    }
    ...
    function _withdraw(uint tokenId) internal {
        require(owners[tokenId] == tx.origin);
        uint _liquidity = liquidityOf[tokenId];
        liquidityOf[tokenId] = 0;
        totalLiquidity -= _liquidity;
        owners[tokenId] = address(0);
        _remove(tokenIds[tx.origin], tokenId);
