...
contract MasterChefTimelock is Timelock {
    ...
    function setMigrator(IMigratorChef _migrator) external onlyAdmin {
        ...
        queueTransaction(address(masterChef), 0, "", abi.encodeWithSignature("setMigrator(address)", address(_migrator)), block.number + delay);
        setMigratorData.migrator = address(_migrator);
        setMigratorData.timestamp = block.number + delay;
    ...
    function transferOwnership(address newOwner) external onlyAdmin {
        ...
        queueTransaction(address(masterChef), 0, "", abi.encodeWithSignature("transferOwnership(address)", address(newOwner)), block.number + delay);
        transferOwnershipData.newOwner = newOwner;
        transferOwnershipData.timestamp = block.number + delay;
