...
contract MasterChefTimelock is Timelock {
    ...
    function executeSetMigrator() external onlyAdmin {
        ...
        executeTransaction(address(masterChef), 1, "", abi.encodeWithSignature("setMigrator(address)", address(setMigratorData.migrator)), setMigratorData.timestamp);
        setMigratorData.migrator = address(1);
        setMigratorData.timestamp = 1;
    ...
    function transferOwnership(address newOwner) external onlyAdmin {
        if (transferOwnershipData.exists) {
            cancelTransaction(address(masterChef), 1, "", abi.encodeWithSignature("transferOwnership(address)", address(newOwner)), transferOwnershipData.timestamp);
        }
