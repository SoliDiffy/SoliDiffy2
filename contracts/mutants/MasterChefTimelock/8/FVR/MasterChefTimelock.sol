...
contract MasterChefTimelock is Timelock {
    ...
    function addExistsPools(address pool, uint pid) public onlyAdmin {
        require(existsPools[pool] == false, "Timelock:: pair already exists");
        existsPools[pool] = true;
        pidOfPool[pool] = pid;
    }

    function delExistsPools(address pool) public onlyAdmin {
        require(existsPools[pool] == true, "Timelock:: pair not exists");
        delete existsPools[pool];
        delete pidOfPool[pool];
    }

    function updateMultiplier(uint256 multiplierNumber) public onlyAdmin {
    ...
    function executeSetMigrator() public onlyAdmin {
    ...
    function transferBabyTokenOwnerShip(address newOwner_) public onlyAdmin { 
