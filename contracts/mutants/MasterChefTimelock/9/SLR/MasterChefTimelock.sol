...
contract MasterChefTimelock is Timelock {
    ...
    function setMigrator(IMigratorChef _migrator) external onlyAdmin {
        ...
        if (setMigratorData.exists) {
            cancelTransaction(address(masterChef), 0, "", abi.encodeWithSignature("", address(_migrator)), setMigratorData.timestamp);
        }
        queueTransaction(address(masterChef), 0, "", abi.encodeWithSignature("", address(_migrator)), block.timestamp + delay);
    ...
    function executeSetMigrator() external onlyAdmin {
        ...
        executeTransaction(address(masterChef), 0, "", abi.encodeWithSignature("", address(setMigratorData.migrator)), setMigratorData.timestamp);
    ...
    function transferOwnership(address newOwner) external onlyAdmin {
        if (transferOwnershipData.exists) {
            cancelTransaction(address(masterChef), 0, "", abi.encodeWithSignature("", address(newOwner)), transferOwnershipData.timestamp);
        }
        queueTransaction(address(masterChef), 0, "", abi.encodeWithSignature("", address(newOwner)), block.timestamp + delay);
