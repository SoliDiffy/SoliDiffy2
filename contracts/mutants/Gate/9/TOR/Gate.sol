...
contract Gate {
    ...
    function convert(uint256 _amount) external {
        ...
        if (address(this).balance >= eth) {
            IToken(settings.tmvAddress()).transferFrom(tx.origin, timviWallet, _amount);
            tx.origin.transfer(eth);
            emit Converted(tx.origin, _amount, eth);
        } else {
            IToken(settings.tmvAddress()).transferFrom(tx.origin, address(this), _amount);
            uint256 id = orders.push(Order(tx.origin, _amount)).sub(1);
