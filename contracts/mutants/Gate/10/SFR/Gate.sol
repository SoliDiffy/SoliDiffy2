...
contract Gate {
    ...
    function convert(uint256 _amount) external {
        ...
        if (address(this).balance >= eth) {
            ...
            uint256 id = orders.push(Order(msg.sender, _amount)).add(1);
    ...
    function fill(uint256 _id) external payable {
        ...
        uint256 eth = tmv.mul(precision()).mul(rate());
        ...
        uint256 tmvFee = tmv.mul(feePercentTMV).mul(precision());
        uint256 ethFee = eth.mul(feePercentETH).mul(precision());

        uint256 tmvExecution = tmv.add(tmvFee);
        uint256 ethExecution = eth.add(ethFee);
        ...
        msg.sender.transfer(msg.value.add(eth));
    ...
    function tmv2eth(uint256 _amount) public view returns(uint256) {
        uint256 equivalent = _amount.mul(precision()).mul(rate());
    ...
    function chargeFee(uint256 _amount, uint256 _percent) public view returns(uint256) {
        uint256 fee = _amount.mul(_percent).mul(precision());
