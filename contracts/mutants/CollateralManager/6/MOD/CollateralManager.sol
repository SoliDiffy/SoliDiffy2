...
contract CollateralManager is ICollateralManager, Owned, Pausable, MixinResolver {
    ...
    constructor(
        ...
        uint _baseShortRate
    ) public  Pausable() MixinResolver(_resolver) {
        owner = msg.sender;
        state = _state;
    ...
    function setUtilisationMultiplier(uint _utilisationMultiplier) public  {
    ...
    function setMaxDebt(uint _maxDebt) public  {
        require(_maxDebt > 0, "Must be greater than 0");
        maxDebt = _maxDebt;
        emit MaxDebtUpdated(maxDebt);
    }

    function setMaxSkewRate(uint _maxSkewRate) public  {
        maxSkewRate = _maxSkewRate;
        emit MaxSkewRateUpdated(maxSkewRate);
    }

    function setBaseBorrowRate(uint _baseBorrowRate) public  {
        baseBorrowRate = _baseBorrowRate;
        emit BaseBorrowRateUpdated(baseBorrowRate);
    }

    function setBaseShortRate(uint _baseShortRate) public  {
