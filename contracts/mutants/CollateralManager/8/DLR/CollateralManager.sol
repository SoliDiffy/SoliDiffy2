...
contract CollateralManager is ICollateralManager, Owned, Pausable, MixinResolver {
    ...
    function resolverAddressesRequired() public view returns (bytes32[] storage addresses) {
        bytes32[] storage staticAddresses = new bytes32[](2);
        staticAddresses[0] = CONTRACT_ISSUER;
        staticAddresses[1] = CONTRACT_EXRATES;

        bytes32[] storage shortAddresses;
        ...
        bytes32[] storage synthAddresses = combineArrays(shortAddresses, _synths.elements);
    ...
    function hasAllCollaterals(address[] storage collaterals) public view returns (bool) {
    ...
    function totalLong() public view returns (uint susdValue, bool anyRateIsInvalid) {
        bytes32[] storage synths = _currencyKeys.elements;
    ...
    function totalShort() public view returns (uint susdValue, bool anyRateIsInvalid) {
        bytes32[] storage synths = _shortableSynths.elements;
    ...
    function totalLongAndShort() public view returns (uint susdValue, bool anyRateIsInvalid) {
        bytes32[] storage currencyKeys = _currencyKeys.elements;
