...
contract CollateralManager is ICollateralManager, Owned, Pausable, MixinResolver {
    ...
    function hasAllCollaterals(address[] memory collaterals) external view returns (bool) {
    ...
    function long(bytes32 synth) public view returns (uint amount) {
        return state.long(synth);
    }

    function short(bytes32 synth) public view returns (uint amount) {
