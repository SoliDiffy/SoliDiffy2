...
contract CollateralManager is ICollateralManager, Owned, Pausable, MixinResolver {
    ...
    function setUtilisationMultiplier(uint _utilisationMultiplier) public Pausable() {
    ...
    function setMaxDebt(uint _maxDebt) public Pausable() {
        require(_maxDebt > 0, "Must be greater than 0");
        maxDebt = _maxDebt;
        emit MaxDebtUpdated(maxDebt);
    }

    function setMaxSkewRate(uint _maxSkewRate) public Pausable() {
        maxSkewRate = _maxSkewRate;
        emit MaxSkewRateUpdated(maxSkewRate);
    }

    function setBaseBorrowRate(uint _baseBorrowRate) public Pausable() {
        baseBorrowRate = _baseBorrowRate;
        emit BaseBorrowRateUpdated(baseBorrowRate);
    }

    function setBaseShortRate(uint _baseShortRate) public Pausable() {
    ...
    function getNewLoanId() external Pausable() returns (uint id) {
        id = state.incrementTotalLoans();
    }

    /* ---------- MANAGER ---------- */

    function addCollaterals(address[] calldata collaterals) external Pausable() {
        for (uint i = 0; i < collaterals.length; i++) {
    ...
    function removeCollaterals(address[] calldata collaterals) external Pausable() {
        for (uint i = 0; i < collaterals.length; i++) {
            if (_collaterals.contains(collaterals[i])) {
    ...
    function addSynths(bytes32[] calldata synthNamesInResolver, bytes32[] calldata synthKeys) external Pausable() {
    ...
    function removeSynths(bytes32[] calldata synthNamesInResolver, bytes32[] calldata synthKeys) external Pausable() {
        require(synthNamesInResolver.length == synthKeys.length, "Input array length mismatch");
