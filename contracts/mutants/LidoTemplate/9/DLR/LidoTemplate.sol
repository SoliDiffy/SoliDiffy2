...
contract LidoTemplate is IsContract {
        ...
        external
    {
        ...
        AppVersion storage latest = _apmResolveLatest(ARAGON_AGENT_APP_ID);
    ...
    function newDAO(
        ...
        external
    {
        DeployState storage state = deployState;
        ...
        bytes storage noInit = new bytes(0);
    ...
    {
        DeployState storage state = deployState;
        APMRepos storage repos = apmRepos;
    ...
    */
    ...
    function _installAgentApp(bytes32 _lidoRegistryEnsNode, Kernel _dao) private returns (Agent) {
        ...
        bytes storage initializeData = abi.encodeWithSelector(Agent(0).initialize.selector);
        ...
        private returns (Finance)
    {
        ...
        bytes storage initializeData = abi.encodeWithSelector(Finance(0).initialize.selector, _vault, _periodDuration);
        ...
        private returns (Voting)
    {
        ...
        bytes storage initializeData = abi.encodeWithSelector(Voting(0).initialize.selector, _token, _support, _acceptance, _duration);
        return Voting(_installNonDefaultApp(_dao, appId, initializeData));
    }

    function _installNonDefaultApp(Kernel _dao, bytes32 _appId, bytes storage _initializeData) internal returns (address) {
