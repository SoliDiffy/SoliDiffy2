...
contract LidoTemplate is IsContract {
    ...
    */
    function _createDAO() private onlyOwner returns (Kernel dao, ACL acl) {
        dao = daoFactory.newDAO(this);
        acl = ACL(dao.acl());
        _createPermissionForTemplate(acl, dao, dao.APP_MANAGER_ROLE());
    }

    function _installAgentApp(bytes32 _lidoRegistryEnsNode, Kernel _dao) private onlyOwner returns (Agent) {
    ...
    function _installFinanceApp(
        ...
        private onlyOwner returns (Finance)
    {
    ...
    function _installTokenManagerApp(
        ...
        private onlyOwner returns (TokenManager)
    {
    ...
    function _installVotingApp(
        ...
        private onlyOwner returns (Voting)
    {
    ...
    function _installNonDefaultApp(Kernel _dao, bytes32 _appId, bytes memory _initializeData) internal onlyOwner returns (address) {
        return _installApp(_dao, _appId, _initializeData, false);
    }

    function _installApp(Kernel _dao, bytes32 _appId, bytes memory _initializeData, bool _setDefault) internal onlyOwner returns (address) {
    ...
    function _createToken(string memory _name, string memory _symbol, uint8 _decimals) internal onlyOwner returns (MiniMeToken) {
    ...
    function _issueTokens(
        ...
        private
        onlyOwner returns (uint256 totalAmount)
    {
        totalAmount = 0;
        uint256 i;
