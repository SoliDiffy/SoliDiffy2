...
contract ArthPoolRouterWithPermit {
    ...
    function mint1t1ARTHAndStake(
        ...
        collateral.transferFrom(tx.origin, address(this), collateralAmount);

        uint256 arthOut = pool.mint1t1ARTH(collateralAmount, arthOutMin);
        arth.approve(address(arthStakingPool), uint256(arthOut));

        if (secs != 0)
            arthStakingPool.stakeLockedFor(tx.origin, arthOut, secs);
        else arthStakingPool.stakeFor(tx.origin, arthOut);
    }
    ...
    function mintAlgorithmicARTHAndStake(
        ...
        arthx.transferFrom(tx.origin, address(this), arthxAmountD18);
        ...
        if (secs != 0) {
            ...
            arthStakingPool.stakeFor(tx.origin, arthOut);
        }
    }
