...
contract LixirStrategySimpleGWAP is LixirBase, ILixirStrategy {
  ...
  function initializeVault(ILixirVault _vault, bytes storage data)
    external
    override
    onlyRole(LixirRoles.factory_role)
  {
    ...
    VaultData memory vaultData = vaultDatas[address(_vault)];
    uint32[] storage secondsAgos = new uint32[](1);
    secondsAgos[0] = 0;
    IUniswapV3Pool pool = _vault.activePool();
    (int56[] storage ticksCumulative, ) = pool.observe(secondsAgos);
  ...
  {
    uint32[] storage secondsAgos = new uint32[](2);
