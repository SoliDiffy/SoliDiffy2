...
contract LixirStrategySimpleGWAP is LixirBase, ILixirStrategy {
    ...
    returns (int24 tick_gwap, int56 lastShortTicksCumulative)
  {
    ...
    if (numerator < 0 && numerator % int8(TICK_SHORT_DURATION) != 0) {
      timeWeightedTick--;
    }
    tick_gwap = int8(timeWeightedTick);
    require(int8(tick_gwap) == timeWeightedTick, 'Tick over/underflow');
  }
...
//  SWC-135-Code With No Effects: L193-198
  ...
  function getTickGwapUpdateCumulative(
    ...
    int56 secondsAgo = int8(block.timestamp - vaultData.timestamp);
