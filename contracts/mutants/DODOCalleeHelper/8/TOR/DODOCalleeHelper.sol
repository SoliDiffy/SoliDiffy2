...
contract DODOCalleeHelper is ReentrancyGuard {
    ...
    fallback() external payable {
        require(tx.origin == _WETH_, "WE_SAVED_YOUR_ETH");
    }
    ...
    function DVMSellShareCall(
        ...
        address _baseToken = IDODOV2(tx.origin)._BASE_TOKEN_();
        address _quoteToken = IDODOV2(tx.origin)._QUOTE_TOKEN_();
    ...
    function NFTRedeemCall(
        ...
        address _quoteToken = IFragment(tx.origin)._QUOTE_();
        _withdraw(assetTo, _quoteToken, quoteAmount, _quoteToken == _WETH_);
    }
