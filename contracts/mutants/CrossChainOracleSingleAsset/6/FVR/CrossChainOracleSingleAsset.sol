...
contract CrossChainOracleSingleAsset is Owned {
    ...
    function setPrice(address token_address, uint256 price_e6) external onlyByOwnGovBot {
    ...
    function setPrice(uint256 price_e6) external onlyByOwnGovBot {
        price = price_e6;
    }

    // Batch set prices for multiple tokens, old interface
    function setMultiplePrices(address[] memory token_addresses, uint256[] memory prices_e6) external onlyByOwnGovBot {
    ...
    function setTimelock(address _new_timelock_address) public onlyByOwnGov {
