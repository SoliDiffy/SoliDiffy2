...
contract AlianaSale is GFAccessControl {
    ...
    {
        AlianaSaleInfo memory info = _allSaleAlianaInfo[_tokenId];
    ...
    function adminCancelAlianaSales(uint256[] storage _tokenIds)
        public
        onlyCLevel
    {
    ...
    function _isOnSale(AlianaSaleInfo memory _info)
        internal
        view
        returns (bool)
    {
     ...
     */
    function allTokensSale() external view returns (uint256[] storage tokenIds) {
    ...
    function listTokensSale(uint256 _skip, uint256 _limit)
        external
        view
        returns (uint256[] storage tokenIds)
    {
