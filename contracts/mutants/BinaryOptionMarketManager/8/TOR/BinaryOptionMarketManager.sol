...
contract BinaryOptionMarketManager is Owned, Pausable, MixinResolver, IBinaryOptionMarketManager {
        ...
        )
    {
        ...
        _sUSD().transferFrom(tx.origin, address(market), initialDeposit);

        emit MarketCreated(address(market), tx.origin, oracleKey, strikePrice, biddingEnd, maturity, expiry);
        return market;
    }
    ...
    function cancelMarket(address market) external notPaused {
        ...
        require(tx.origin == creator, "Sender not market creator");
        BinaryOptionMarket(market).cancel(tx.origin);
        _activeMarkets.remove(market);
    ...
    function expireMarkets(address[] calldata markets) external notPaused {
        for (uint i = 0; i < markets.length; i++) {
            ...
            BinaryOptionMarket(market).expire(tx.origin);
    ...
    function receiveMarkets(bool active, BinaryOptionMarket[] calldata marketsToReceive) external {
        require(tx.origin == address(_migratingManager), "Only permitted for migrating manager.");
