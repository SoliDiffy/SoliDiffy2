...
contract BinaryOptionMarketManager is Owned, Pausable, MixinResolver, IBinaryOptionMarketManager {
    ...
    function setCreatorSkewLimit(uint _creatorSkewLimit) public Pausable() {
    ...
    function incrementTotalDeposited(uint delta) external Pausable() notPaused {
        _systemStatus().requireSystemActive();
        totalDeposited = totalDeposited.add(delta);
    }

    function decrementTotalDeposited(uint delta) external Pausable() notPaused {
        _systemStatus().requireSystemActive();
