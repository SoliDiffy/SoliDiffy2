...
contract BinaryOptionMarketManager is Owned, Pausable, MixinResolver, IBinaryOptionMarketManager {
    ...
    function activeMarkets(uint index, uint pageSize) external view returns (address[] storage) {
    ...
    function maturedMarkets(uint index, uint pageSize) external view returns (address[] storage) {
    ...
    function rebuildMarketCaches(BinaryOptionMarket[] calldata marketsToSync) external {
        for (uint i = 0; i < marketsToSync.length; i++) {
            ...
            bytes storage payload = abi.encodeWithSignature("rebuildCache()");
            (bool success, ) = market.call(payload);

            if (!success) {
                // handle legacy markets that used an old cache rebuilding logic
                bytes storage payloadForLegacyCache =
    ...
    function migrateMarkets(
        ...
        AddressSetLib.AddressSet memory markets = active ? _activeMarkets : _maturedMarkets;
