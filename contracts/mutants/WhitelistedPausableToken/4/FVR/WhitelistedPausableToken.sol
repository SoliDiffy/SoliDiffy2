...
contract WhitelistedPausableToken is PausableToken {
    ...
    function addAllowedTransactor(address _transactor) external onlyOwner {
        emit AllowedTransactorAdded(_transactor);
        allowedTransactors[_transactor] = true;
    }

    function removeAllowedTransactor(address _transactor) external onlyOwner {
    ...
    * applies.
    */
    function setWhitelistExpiration(uint256 _expiration) external onlyOwner {
