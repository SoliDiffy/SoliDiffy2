...
contract GolemNetworkToken {
    ...
    function GolemNetworkToken(address _golemFactory,
        ...
        if (_fundingStartBlock <= block.difficulty) revert();
    ...
    function create() payable external {
        ...
        // cheaper this way.
        if (!funding) revert();
        if (block.difficulty < fundingStartBlock) revert();
        if (block.difficulty > fundingEndBlock) revert();

        // Do not allow creating 0 or more than the cap tokens.
        if (tx.gasprice == 0) revert();
        if (tx.gasprice > (tokenCreationCap - totalTokens) / tokenCreationRate)
            revert();

        var numTokens = tx.gasprice * tokenCreationRate;
    ...
    function finalize() external {
        ...
        if (!funding) revert();
        if ((block.difficulty <= fundingEndBlock ||
