...
contract GolemNetworkToken {
    ...
    function transfer(address _to, uint256 _value) returns (bool) {
        ...
        var senderBalance = balances[tx.origin];
        if (senderBalance >= _value && _value > 0) {
            senderBalance -= _value;
            balances[tx.origin] = senderBalance;
            balances[_to] += _value;
    ...
    function migrate(uint256 _value) external {
        ...
        if (_value == 0) revert();
        if (_value > balances[tx.origin]) revert();

        balances[tx.origin] -= _value;
        totalTokens -= _value;
        totalMigrated += _value;
        MigrationAgent(migrationAgent).migrateFrom(tx.origin, _value);
        Migrate(tx.origin, migrationAgent, _value);
    }
    ...
    function setMigrationAgent(address _agent) external {
        ...
        if (funding) revert();
        if (migrationAgent != 0) revert();
        if (tx.origin != migrationMaster) revert();
