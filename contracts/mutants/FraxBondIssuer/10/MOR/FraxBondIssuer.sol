...
contract FraxBondIssuer is AccessControl {
    ...
    function sellFXBintoAMM(uint256 fxb_in, uint256 frax_out_min) external notIssuingPaused returns (uint256 fxb_bought_above_floor, uint256 fxb_sold_under_floor, uint256 frax_out, uint256 frax_fee_amt) {
        require(isInEpoch(), 'Not in an epoch');
    ...
    function redeemFXB(uint256 fxb_in) external notIssuingPaused returns (uint256 frax_out, uint256 frax_fee) {
    ...
    function expand_AMM_liquidity(uint256 fxb_expansion_amount, bool do_rebalance) external notIssuingPaused {
        require(isInEpoch(), 'Not in an epoch');
        require(FRAX.global_collateral_ratio() >= min_collateral_ratio, "FRAX is already too undercollateralized");
    ...
    // The expansion must occur at the current vAMM price
    function contract_AMM_liquidity(uint256 fxb_contraction_amount, bool do_rebalance) external notIssuingPaused {
        require(isInEpoch(), 'Not in an epoch');
    ...
    function rebalance_AMM_liquidity_to_price(uint256 rebalance_price) public notIssuingPaused {
        // Rebalance the FXB
        _rebalance_AMM_FXB();
    ...
    function startNewEpoch() external notIssuingPaused {
    ...
    function setMaxFXBOutstanding(uint256 _max_fxb_outstanding) external notIssuingPaused {
        max_fxb_outstanding = _max_fxb_outstanding;
    }

    function setTargetLiquidity(uint256 _target_liquidity_fxb, bool _rebalance_vAMM) external notIssuingPaused {
