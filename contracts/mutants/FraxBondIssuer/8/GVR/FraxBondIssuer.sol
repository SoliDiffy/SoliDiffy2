...
contract FraxBondIssuer is AccessControl {
    ...
    constructor(
        ...
        epoch_start = (block.number).sub(cooldown_period).sub(epoch_length);
        epoch_end = (block.number).sub(cooldown_period);
    ...
    function isInEpoch() public view returns (bool in_epoch) {
        in_epoch = ((block.number >= epoch_start) && (block.number < epoch_end));
    }

    // Checks if the bond is in the cooldown period
    function isInCooldown() public view returns (bool in_cooldown) {
        in_cooldown = ((block.number >= epoch_end) && (block.number < epoch_end.add(cooldown_period)));
    }
    ...
    function floor_price() public view returns (uint256 floor_price) {
        uint256 time_into_epoch = (block.number).sub(epoch_start);
