...
contract FraxBondIssuer is AccessControl {
    ...
    function buyUnissuedFXB(uint256 frax_in, uint256 fxb_out_min) public  returns (uint256 fxb_out, uint256 fxb_fee_amt) {
    ...
    function buyFXBfromAMM(uint256 frax_in, uint256 fxb_out_min) external  returns (uint256 fxb_out, uint256 fxb_fee_amt) {
        require(isInEpoch(), 'Not in an epoch');
    ...
    function sellFXBintoAMM(uint256 fxb_in, uint256 frax_out_min) external  returns (uint256 fxb_bought_above_floor, uint256 fxb_sold_under_floor, uint256 frax_out, uint256 frax_fee_amt) {
        require(isInEpoch(), 'Not in an epoch');
    ...
    function redeemFXB(uint256 fxb_in) external  returns (uint256 frax_out, uint256 frax_fee) {
    ...
    function expand_AMM_liquidity(uint256 fxb_expansion_amount, bool do_rebalance) external  {
        require(isInEpoch(), 'Not in an epoch');
        require(FRAX.global_collateral_ratio() >= min_collateral_ratio, "FRAX is already too undercollateralized");
    ...
    // The expansion must occur at the current vAMM price
    function contract_AMM_liquidity(uint256 fxb_contraction_amount, bool do_rebalance) external  {
        require(isInEpoch(), 'Not in an epoch');
    ...
    function rebalance_AMM_liquidity_to_price(uint256 rebalance_price) public  {
        // Rebalance the FXB
        _rebalance_AMM_FXB();
    ...
    function setMaxFXBOutstanding(uint256 _max_fxb_outstanding) external  {
