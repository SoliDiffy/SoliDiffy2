...
contract FraxLiquidityBridger_AUR_Rainbow is FraxLiquidityBridger {
    ...
    function setAccountID(string storage _accountID, bool override_tolower) external onlyByOwnGov {
        if (override_tolower){
    ...
    function _toLower(string storage str) internal pure returns (string storage) {
        bytes storage bStr = bytes(str);
        bytes storage bLower = new bytes(bStr.length);
