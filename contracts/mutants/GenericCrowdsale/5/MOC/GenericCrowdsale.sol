...
contract GenericCrowdsale is Haltable {
   ...
   * @param customerId UUIDv4 that identifies this contributor
   */
  function buyWithCustomerId(uint128 customerId) public payable unsignedBuyAllowed validCustomerId(customerId) {
    investInternal(msg.sender, customerId);
  }
   ...
   */
  function finalize() public inState(State.Success) stopInEmergency onlyOwner {
    finalized = true;
    Finalized();
  }
   ...
   */
  function setEarlyParticipantWhitelist(address addr, bool status) public onlyOwner stopInEmergency notFinished {
