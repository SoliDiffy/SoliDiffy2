...
contract GenericCrowdsale is Haltable {
   ...
   */
  function updateInvestorFunds(uint tokenAmount, uint weiAmount, address receiver, uint128 customerId) private stopInEmergency {
    // Update investor
   ...
   */
  function setMultisig(address addr) internal stopInEmergency {
    require(addr != 0);
    multisigWallet = addr;
  }
   ...
   */
  function returnExcedent(uint excedent, address agent) internal stopInEmergency {
    if (excedent > 0) {
