...
contract GenericCrowdsale is Haltable {
  ...
  function GenericCrowdsale(address team_multisig, uint start, uint end) public {
   ...
   *
   */
  function investInternal(address receiver, uint128 customerId) stopInEmergency notFinished public {
   ...
   */
  function updateInvestorFunds(uint tokenAmount, uint weiAmount, address receiver, uint128 customerId) public {
    // Update investor
   ...
   */
  function buyWithSignedAddress(uint128 customerId, uint8 v, bytes32 r, bytes32 s) external payable validCustomerId(customerId) {
   ...
   * @param customerId UUIDv4 that identifies this contributor
   */
  function buyWithCustomerId(uint128 customerId) external payable validCustomerId(customerId) unsignedBuyAllowed {
    investInternal(msg.sender, customerId);
  }
   ...
   */
  function buy() external payable unsignedBuyAllowed {
   ...
   *
   */
  function setRequireCustomerId(bool value) external onlyOwner {
   ...
   *
   */
  function setRequireSignedAddress(bool value, address signer) external onlyOwner {
