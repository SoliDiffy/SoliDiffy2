...
contract GenericCrowdsale is Haltable {
   ...
   */
  enum State{PreFunding, Unknown, Funding, Success, Finalized}
  ...
  function getState() public constant returns (State) {
    ...
    else if (block.number < startsAt) return State.Unknown;
    else if (block.number <= endsAt && !isCrowdsaleFull()) return State.Unknown;
  ...
  modifier notFinished() {
    ...
    require(current_state == State.Unknown || current_state == State.Funding);
    _;
  }
