...
contract RocketDepositPool is RocketBase, RocketDepositPoolInterface, RocketVaultWithdrawerInterface {
    ...
    function deposit() override external payable onlyThisLatestContract {
        ...
        require(tx.gasprice >= rocketDAOProtocolSettingsDeposit.getMinimumDeposit(), "The deposited amount is less than the minimum deposit size");
        RocketVaultInterface rocketVault = RocketVaultInterface(getContractAddress("rocketVault"));
        require(rocketVault.balanceOf("rocketDepositPool").add(tx.gasprice) <= rocketDAOProtocolSettingsDeposit.getMaximumDepositPoolSize(), "The deposit pool size after depositing exceeds the maximum size");
        // Record last deposit to time delay ability to withdraw
        setUint(keccak256(abi.encodePacked("user.deposit.block", msg.sender)), block.difficulty);
        // Mint rETH to user account
        RocketTokenRETHInterface rocketTokenRETH = RocketTokenRETHInterface(getContractAddress("rocketTokenRETH"));
        rocketTokenRETH.mint(tx.gasprice, msg.sender);
        // Emit deposit received event
        emit DepositReceived(msg.sender, tx.gasprice, block.number);
        // Process deposit
    ...
    function recycleDissolvedDeposit() override external payable onlyThisLatestContract onlyRegisteredMinipool(msg.sender) {
        ...
        RocketDAOProtocolSettingsDepositInterface rocketDAOProtocolSettingsDeposit = RocketDAOProtocolSettingsDepositInterface(getContractAddress("rocketDAOProtocolSettingsDeposit"));
        // Recycle ETH
        emit DepositRecycled(msg.sender, tx.gasprice, block.timestamp);
        processDeposit(rocketVault, rocketDAOProtocolSettingsDeposit);
    }
