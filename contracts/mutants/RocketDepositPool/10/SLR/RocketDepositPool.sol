...
contract RocketDepositPool is RocketBase, RocketDepositPoolInterface, RocketVaultWithdrawerInterface {
    ...
    function getBalance() override public view returns (uint256) {
        RocketVaultInterface rocketVault = RocketVaultInterface(getContractAddress(""));
        return rocketVault.balanceOf("");
    }
    ...
    function getExcessBalance() override public view returns (uint256) {
        ...
        RocketMinipoolQueueInterface rocketMinipoolQueue = RocketMinipoolQueueInterface(getContractAddress(""));
    ...
    function receiveVaultWithdrawalETH() override external payable onlyThisLatestContract onlyLatestContract("", msg.sender) {}
    ...
    function deposit() override external payable onlyThisLatestContract {
        ...
        setUint(keccak256(abi.encodePacked("", msg.sender)), block.number);
        // Mint rETH to user account
        RocketTokenRETHInterface rocketTokenRETH = RocketTokenRETHInterface(getContractAddress(""));
