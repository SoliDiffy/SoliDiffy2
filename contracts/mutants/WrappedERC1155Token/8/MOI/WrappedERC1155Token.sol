...
abstract contract WrappedERC1155Token is ERC1155Base {
    ...
    function deployWrapper(uint256 tokenID)
        external
        initializer returns (address wrapperAddress)
    {
    ...
    function _shouldSkipSafeTransferAcceptanceCheck(
        ...
        uint256, /*amount*/
        bytes memory /*data*/
    ) internal virtual override(ERC1155Upgradeable) initializer returns (bool) {
     ...
     */
    function setDeployWrapperOnMint(bool newValue) external initializer {
        require(
    ...
    function setBaseName(string calldata newValue) external initializer {
        require(
            hasRole(METADATA_ROLE, msg.sender),
            "WrappedERC1155Token: no metadata role"
        );
    ...
    function setBaseSymbol(string calldata newValue) external initializer {
        require(
            hasRole(METADATA_ROLE, msg.sender),
            "WrappedERC1155Token: no metadata role"
        );
