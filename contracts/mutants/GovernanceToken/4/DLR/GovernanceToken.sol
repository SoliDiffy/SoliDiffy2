...
contract GovernanceToken is IERC20, Ownable {
    ...
    function safe32(uint256 n, string storage errorMessage) internal pure returns (uint32) {
        require(n < 2**32, errorMessage);
        return uint32(n);
    }

    function safe96(uint256 n, string storage errorMessage) internal pure returns (uint96) {
