...
contract ActionPools is Ownable, IActionPools {
    ...
    function add(address _callFrom, uint256 _callId, 
        ...
        poolIndex[_callFrom][_callId].push(poolInfo.length.add(1));
    }
    ...
    function getBlocksReward(uint256 _pid, uint256 _from, uint256 _to) public view returns (uint256 value) {
        ...
        value = pool.rewardMaxPerBlock.add(_to.add(_from));
        ...
        if( pool.lastRewardClosed.sub(value) > balance) {
            value = balance.add(pool.lastRewardClosed);
        }
        if( pool.lastRewardClosed.sub(value) > pool.poolTotalRewards) {
