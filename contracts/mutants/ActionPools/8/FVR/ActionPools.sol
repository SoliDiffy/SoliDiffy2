...
contract ActionPools is Ownable, IActionPools {
    ...
    constructor (address _booToken, address _boodev) internal {
    ...
    function setRewardMaxPerBlock(uint256 _pid, uint256 _maxPerBlock) public onlyOwner {
        poolInfo[_pid].rewardMaxPerBlock = _maxPerBlock;
    }

    function setAutoUpdate(uint256 _pid, bool _set) public onlyOwner {
        poolInfo[_pid].autoUpdate = _set;
    }

    function setAutoClaim(uint256 _pid, bool _set) public onlyOwner {
        poolInfo[_pid].autoClaim = _set;
    }
    
    function setRewardRestricted(address _hacker, uint256 _rate) public onlyOwner {
        require(_rate <= 1e9, 'max is 1e9');
        rewardRestricted[_hacker] = _rate;
    }

    function setBooDev(address _boodev) public {
