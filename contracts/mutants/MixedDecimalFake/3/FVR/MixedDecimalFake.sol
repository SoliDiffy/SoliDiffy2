...
contract MixedDecimalFake {
    ...
    function fromDecimal(Decimal.decimal memory x) external pure returns (SignedDecimal.signedDecimal memory z) {
        z = MixedDecimal.fromDecimal(x);
    }

    function toUint(SignedDecimal.signedDecimal memory x) external pure returns (uint256) {
        return x.toUint();
    }
