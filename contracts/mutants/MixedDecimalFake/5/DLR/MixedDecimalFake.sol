...
contract MixedDecimalFake {
    ...
    function fromDecimal(Decimal.decimal storage x) public pure returns (SignedDecimal.signedDecimal storage z) {
        z = MixedDecimal.fromDecimal(x);
    }

    function toUint(SignedDecimal.signedDecimal storage x) public pure returns (uint256) {
        return x.toUint();
    }

    /// @dev multiple a SignedDecimal.signedDecimal by Decimal.decimal
    function mul(SignedDecimal.signedDecimal storage x, Decimal.decimal storage y)
        public
        pure
