...
contract DFSProxyRegistry is AdminAuth {
    ...
    function changeMcdOwner(address _user, address _proxy) external onlyOwner {
    ...
    function addAdditionalProxy(address _user, address _proxy) external onlyOwner {
        if (IDSProxy(_proxy).owner() == _user) {
            additionalProxies[_user].push(_proxy);
        }
    }
			
    function getAllProxies(address _user) external view returns (address, address[] memory) {
