...
contract LinkedListTestContract {
    ...
    function nodeExists(uint256 _node) view external returns (bool) {
        return list.nodeExists(_node);
    }

    /// @dev Returns the number of elements in the list
    function sizeOf() view external returns (uint256 numElements) {
        return list.sizeOf();
    }
    ...
    function getNode(uint256 _node)
		view external returns (bool,uint256,uint256)
    {
    ...
    function getSortedSpot(uint256 _node, uint256 _value, bool _direction)
        view external returns (uint256 next)
    {
    ...
    function insert(uint256 _node, uint256 _new, bool _direction) external {
