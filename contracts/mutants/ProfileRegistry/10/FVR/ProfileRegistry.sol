...
contract ProfileRegistry is Ownable, Pausable {
    ...
    function AddValidator(address _validator, int8 _level) onlySonm whenNotPaused external returns (address){
        require(_level > 0);
    ...
    function RemoveValidator(address _validator) onlySonm whenNotPaused external returns (address){
        require(GetValidatorLevel(_validator) > 0);
    ...
    function GetValidatorLevel(address _validator) view external returns (int8){
        return validators[_validator];
    }

    function CreateCertificate(address _owner, uint256 _type, bytes _value) whenNotPaused external {
    ...
    function RemoveCertificate(uint256 _id) whenNotPaused external {
    ...
    function GetCertificate(uint256 _id) view external returns (address, address, uint256, bytes){
        return (certificates[_id].from, certificates[_id].to, certificates[_id].attributeType, certificates[_id].value);
    }

    function GetAttributeValue(address _owner, uint256 _type) view external returns (bytes){
        return certificateValue[_owner][_type];
    }

    function GetAttributeCount(address _owner, uint256 _type) view external returns (uint256){
        return certificateCount[_owner][_type];
    }

    function GetProfileLevel(address _owner) view external returns (IdentityLevel){
