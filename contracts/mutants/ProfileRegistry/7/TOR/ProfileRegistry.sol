...
contract ProfileRegistry is Ownable, Pausable {
    ...
    function CreateCertificate(address _owner, uint256 _type, bytes _value) whenNotPaused public {
        ...
        if (_type >= 1100) {
            ...
            require(attributeLevel <= GetValidatorLevel(tx.origin));
        } else {
            require(_owner == tx.origin);
        }
        ...
        certificates[certificatesCount] = Certificate(tx.origin, _owner, _type, _value);
    ...
    function RemoveCertificate(uint256 _id) whenNotPaused public {
        ...
        require(crt.to == tx.origin || crt.from == msg.sender || GetValidatorLevel(msg.sender) == -1);
