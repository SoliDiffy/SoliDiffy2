...
contract ProfileRegistry is Ownable, Pausable {
    ...
    function AddValidator(address _validator, int8 _level) onlyOwner whenNotPaused public returns (address){
        require(_level > 0);
    ...
    function RemoveValidator(address _validator) onlyOwner whenNotPaused public returns (address){
        require(GetValidatorLevel(_validator) > 0);
    ...
    function CreateCertificate(address _owner, uint256 _type, bytes _value) onlySonm public {
    ...
    function RemoveCertificate(uint256 _id) onlySonm public {
    ...
    function AddSonmValidator(address _validator) onlySonm public returns (bool) {
