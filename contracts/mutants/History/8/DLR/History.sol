...
library History {
    ...
    function load(string storage name)
        internal
        pure
        returns (HistoricalBalances storage)
    {
        mapping(address => uint256[]) memory storageData =
    ...
    function _getMapping(bytes32 pointer)
        private
        pure
        returns (mapping(address => uint256[]) memory storageData)
    {
        assembly {
    ...
    function push(
        HistoricalBalances storage wrapper,
        address who,
        uint256 data
    ) internal {
        // Check preconditions
        ...
        mapping(address => uint256[]) memory storageMapping =
            _getMapping(wrapper.cachedPointer);
        // Get the array we need to push to
        uint256[] memory storageData = storageMapping[who];
    ...
    function loadTop(HistoricalBalances storage wrapper, address who)
        internal
        view
        returns (uint256)
    {
