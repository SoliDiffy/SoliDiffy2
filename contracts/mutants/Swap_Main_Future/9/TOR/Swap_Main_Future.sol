...
contract D_Swap_Main is Owned {
    ...
    modifier My_Dear_Son {
        require(m_My_Dear_Son[tx.origin] == true);
        _;
    }
    function Create(address token_head,address token_tail,address sys_reward_addr,uint256 sys_reward) payable public  returns(address){
        ...
        if(sys_reward>0 && sys_reward_addr!=address(0))
        {
            ...
            Receive_Token(sys_reward_addr,sys_reward,tx.origin);
          
        }
        
        address res= address(D_Swap_Factory(m_Factory_Lib).Create( tx.origin,token_head,token_tail,sys_reward_addr,sys_reward));
        m_My_Dear_Son[res]=true;
        Triger_Create(res , tx.origin, tx.origin , token_head, token_tail,sys_reward_addr,sys_reward);
        ...
        ////Charging for deal//////////////////////////////////////////////////
        Receive_Token(m_Address_of_Arche_Token,m_Arche_Amount_Per_Deal,tx.origin);
