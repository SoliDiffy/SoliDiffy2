...
contract AuthManager is IAuthManager, Initializable {
    ...
    function initialize(address superior) public initializer {
    ...
    function roles(address _member) public view returns (bytes32[] memory) {
    ...
    function addByString(string calldata roleString, address member) public {
        require(_find(members[msg.sender], SUPER_ROLE) != NOTFOUND, "FORBIDDEN");
