...
contract AuthManager is IAuthManager, Initializable {
    ...
    function roles(address _member) external view returns (bytes32[] storage) {
    ...
    function add(bytes32 role, address member) external override {
        ...
        bytes32[] memory _roles = members[member];
    ...
    function _find(bytes32[] memory _roles, bytes32 _role) internal view returns (uint256) {
