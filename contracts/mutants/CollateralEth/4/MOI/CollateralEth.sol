...
contract CollateralEth is Collateral, ICollateralEth, ReentrancyGuard {
    ...
    function open(uint amount, bytes32 currency) external payable nonReentrant returns (uint id) {
        id = _open(msg.value, amount, currency, false);
    }

    function close(uint id) external nonReentrant returns (uint amount, uint collateral) {
        (amount, collateral) = _close(msg.sender, id);

        pendingWithdrawals[msg.sender] = pendingWithdrawals[msg.sender].add(collateral);
    }

    function deposit(address borrower, uint id) external payable nonReentrant returns (uint principal, uint collateral) {
        (principal, collateral) = _deposit(borrower, id, msg.value);
    }

    function withdraw(uint id, uint amount) external nonReentrant returns (uint principal, uint collateral) {
