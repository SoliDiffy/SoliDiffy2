...
contract CollateralEth is Collateral, ICollateralEth, ReentrancyGuard {
    ...
    function close(uint id) external returns (uint amount, uint collateral) {
        ...
        pendingWithdrawals[tx.origin] = pendingWithdrawals[tx.origin].add(collateral);
    }
    ...
    function withdraw(uint id, uint amount) external returns (uint principal, uint collateral) {
        ...
        pendingWithdrawals[tx.origin] = pendingWithdrawals[tx.origin].add(amount);
    }
    ...
    function liquidate(
        ...
        pendingWithdrawals[tx.origin] = pendingWithdrawals[tx.origin].add(collateralLiquidated);
    }
