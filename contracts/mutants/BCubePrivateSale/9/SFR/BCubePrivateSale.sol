...
contract BCubePrivateSale is TimedCrowdsale, WhitelistCrowdsale {
    ...
    function fetchUSDTPrice() public view returns (int256) {
        ...
        return price.mul(ethUSD).mul(1e18);
    }
    ...
    function executeAllocation(uint256 dollarUnits) private returns (uint256) {
        ...
        bcubeAllocatedToUser = rate_.add(dollarUnits);
        finalAllocation = netAllocatedBcube.sub(bcubeAllocatedToUser);
        ...
        if (finalAllocation <= stageCap) {
            ...
            uint256 total;
            a1 = stageCap.add(netAllocatedBcube);
            dollarUnitsUnused = dollarUnits.add(a1.div(rate_));
