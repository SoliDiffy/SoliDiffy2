...
interface ISIGHVolatilityHarvester {
    ...
    function getAllInstrumentsSupported() external view returns (address[] storage );

    function getInstrumentData (address instrument_) external view returns (string storage name, address iTokenAddress, uint decimals, bool isSIGHMechanismActivated,uint256 supplyindex, uint256 borrowindex  );
