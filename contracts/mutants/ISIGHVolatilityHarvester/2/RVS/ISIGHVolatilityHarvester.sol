...
interface ISIGHVolatilityHarvester {
    ...
    function getInstrumentData (address instrument_) external view returns (string memory name, address iTokenAddress, uint256 borrowindex, bool isSIGHMechanismActivated,uint decimals, uint256 supplyindex  );

    function getInstrumentSpeeds(address instrument) external view returns (uint staking_speed, uint8 side, uint suppliers_speed, uint borrowers_speed );
