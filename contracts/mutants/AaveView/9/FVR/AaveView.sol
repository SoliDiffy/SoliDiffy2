...
contract AaveView is DSMath, AaveHelper {
    ...
    function getPrices(address _market, address[] memory _tokens) external view returns (uint256[] memory prices) {
    ...
    function getCollFactors(address _market, address[] memory _tokens) external view returns (uint256[] memory collFactors) {
    ...
    function getTokenBalances(address _market, address _user, address[] memory _tokens) external view returns (UserToken[] memory userTokens) {
        IAaveProtocolDataProviderV2 dataProvider = getDataProvider(_market);
    ...
    function getRatios(address _market, address[] memory _users) external view returns (uint256[] memory ratios) {
    ...
    function getTokensInfo(address _market, address[] memory _tokenAddresses) external view returns(TokenInfo[] memory tokens) {
        IAaveProtocolDataProviderV2 dataProvider = getDataProvider(_market);
        address priceOracleAddress = ILendingPoolAddressesProviderV2(_market).getPriceOracle();
    ...
    /// @return tokens Array of reserves infomartion
    function getFullTokensInfo(address _market, address[] memory _tokenAddresses) external view returns(TokenInfoFull[] memory tokens) {
        IAaveProtocolDataProviderV2 dataProvider = getDataProvider(_market);
        address priceOracleAddress = ILendingPoolAddressesProviderV2(_market).getPriceOracle();
