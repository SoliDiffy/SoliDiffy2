...
contract AaveView is DSMath, AaveHelper {
    ...
    function getRatios(address _market, address[] storage _users) public view returns (uint256[] storage ratios) {
    ...
    function getTokensInfo(address _market, address[] storage _tokenAddresses) public view returns(TokenInfo[] storage tokens) {
        IAaveProtocolDataProviderV2 dataProvider = getDataProvider(_market);
        address priceOracleAddress = ILendingPoolAddressesProviderV2(_market).getPriceOracle();
