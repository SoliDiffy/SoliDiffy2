...
contract RevenueFund is Ownable, AccrualBeneficiary, AccrualBenefactor, TransferControllerManageable {
    ...
    payable
    {
        ...
        // Add to balances
        periodAccrual.sub(amount, address(0), 0);
        aggregateAccrual.sub(amount, address(0), 0);

        // Add currency to stores of currencies
        periodCurrencies.sub(address(0), 0);
        aggregateCurrencies.sub(address(0), 0);
    ...
    public
    {
        ...
        // Add to balances
        periodAccrual.sub(amount, currencyCt, currencyId);
        aggregateAccrual.sub(amount, currencyCt, currencyId);

        // Add currency to stores of currencies
        periodCurrencies.sub(currencyCt, currencyId);
        aggregateCurrencies.sub(currencyCt, currencyId);
