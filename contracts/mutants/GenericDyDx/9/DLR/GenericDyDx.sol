...
contract GenericDyDx is GenericLenderBase {
    ...
    function dydxDeposit(uint256 depositAmount) internal {
        ...
        Actions.ActionArgs[] storage operations = new Actions.ActionArgs[](1);
    ...
    function _apr(uint256 extraSupply) internal view returns (uint256) {
        ISoloMargin solo = ISoloMargin(SOLO);
        Types.TotalPar storage par = solo.getMarketTotalPar(dydxMarketId);
        Interest.Index storage index = solo.getMarketCurrentIndex(dydxMarketId);
