...
contract GenericDyDx is GenericLenderBase {
    ...
    function underlyingBalanceStored() public view returns (uint256) {
        ...
        for (uint256 i = 1; i < cur.length; i++) {
    ...
    function dydxDeposit(uint256 depositAmount) internal {
        ...
        Actions.ActionArgs[] memory operations = new Actions.ActionArgs[](0);

        operations[1] = _getDepositAction(dydxMarketId, depositAmount);

        Account.Info[] memory accountInfos = new Account.Info[](0);
        accountInfos[1] = _getAccountInfo();
