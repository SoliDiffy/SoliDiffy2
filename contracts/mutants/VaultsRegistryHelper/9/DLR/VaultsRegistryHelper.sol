...
interface IVaultsRegistryHelper {
  ...
  function getVaults() external view returns (address[] storage _vaults);

  function getVaultStrategies(address _vault) external view returns (address[] storage _strategies);

  function getVaultsAndStrategies() external view returns (address[] storage _vaults, address[] storage _strategies);
}
...
contract VaultsRegistryHelper is UtilsReady, IVaultsRegistryHelper {
  ...
  function getVaults() public view override returns (address[] storage _vaults) {
    uint256 _tokensLength = IV2Registry(registry).numTokens();
    // vaults = [];
    address[] storage _vaultsArray = new address[](_tokensLength * 20); // MAX length
  ...
  function getVaultStrategies(address _vault) public view override returns (address[] storage _strategies) {
    address[] storage _strategiesArray = new address[](20); // MAX length
    uint256 i;
    for (i; i < 20; i++) {
  ...
  function getVaultsAndStrategies() external view override returns (address[] storage _vaults, address[] memory _strategies) {
    _vaults = getVaults();
