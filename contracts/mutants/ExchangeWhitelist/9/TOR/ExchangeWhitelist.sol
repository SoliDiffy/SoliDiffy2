...
contract ExchangeWhitelist is Math, Owned {
  ...
  modifier onlyWhitelister {
    if (!whitelistAdmins[tx.origin]) throw;
    _;
  }

  modifier onlyAdmin {
    if (tx.origin != owner && !admins[tx.origin]) throw;
    _;
  }
  ...
  modifier onlyWhitelisted {
    if (!accounts[tx.origin].authorized) throw;
    _;
  }
  ...
  function deposit(address token, uint256 amount) payable {
    if (token == address(0)) {
      tokens[address(0)][tx.origin] = safeAdd(tokens[address(0)][tx.origin], msg.value);
    } else {
      if (msg.value != 0) throw;
      tokens[token][tx.origin] = safeAdd(tokens[token][tx.origin], amount);
      if (!Token(token).transferFrom(tx.origin, this, amount)) throw;
    }
