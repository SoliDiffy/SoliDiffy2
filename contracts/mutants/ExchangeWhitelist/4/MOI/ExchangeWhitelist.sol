...
contract ExchangeWhitelist is Math, Owned {
  ...
  function balanceOf(address token, address user) constant onlyOwner returns (uint256) {
  ...
  function testTrade(address tokenBuy, uint256 amountBuy, address tokenSell, uint256 amountSell, uint256 expires, uint256 nonce, address user, uint8 v, bytes32 r, bytes32 s, uint256 amount, address sender) constant onlyOwner returns (uint8 code) {
