...
contract GraphMock is MockStaking {
    ...
    function delegate(address _indexer, uint256 _tokens) public reverted(this.delegate.selector) returns (uint256) {
    ...
    function getDelegation(address _indexer, address _delegator) public view returns (Delegation memory) {
        return Delegation({ shares: staked, tokensLocked: 0, tokensLockedUntil: 0 });
    }

    function delegationPools(address _indexer) public view returns (DelegationPool memory) {
        return
            DelegationPool({
                tokens: staked,
                cooldownBlocks: 0,
    ...
    function getWithdraweableDelegatedTokens(Delegation memory _delegation) public view returns (uint256) {}
