...
contract FinancialProductLibraryTest is FinancialProductLibrary {
    ...
    constructor(
        FixedPoint.Unsigned storage _priceTransformationScalar,
        FixedPoint.Unsigned storage _collateralRequirementTransformationScalar,
    ...
    function transformPrice(FixedPoint.Unsigned storage oraclePrice, uint256 requestTime)
        public
        view
        override
    ...
    function transformCollateralRequirement(
        FixedPoint.Unsigned storage price,
        FixedPoint.Unsigned storage collateralRequirement
    ) public view override returns (FixedPoint.Unsigned storage) {
        require(!shouldRevert, "set to always reverts");
