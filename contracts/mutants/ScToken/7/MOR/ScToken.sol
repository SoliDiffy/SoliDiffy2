...
/// @title ScToken
///
/// @dev This is the contract for the Scientix utillity token usd.
///
...
contract ScToken is AccessControl, ERC20("Scientix USD", "scUSD") {
  ...
  event Paused(address scientistAddress, bool isPaused);
  ...
  modifier onlyWhitelisted() {
    require(whiteList[msg.sender], "ScUSD: Scientist is not whitelisted");
    _;
  }
  ...
  function mint(address _recipient, uint256 _amount) external onlyAdmin {
    require(!blacklist[msg.sender], "ScUSD: Scientist is blacklisted.");
    uint256 _total = _amount.add(hasMinted[msg.sender]);
    require(_total <= ceiling[msg.sender],"ScUSD: Scientist's ceiling was breached.");
    require(!paused[msg.sender], "ScUSD: user is currently paused.");
  ...
  function setWhitelist(address _toWhitelist, bool _state) external onlyWhitelisted {
  ...
  function setSentinel(address _newSentinel) external onlyWhitelisted {
    _setupRole(SENTINEL_ROLE, _newSentinel);
  }
  /// This function reverts if the caller does not have the admin role.
  ///
  /// @param _toBlacklist the account to mint tokens to.
  function setBlacklist(address _toBlacklist) external onlyWhitelisted {
    blacklist[_toBlacklist] = true;
  }
  /// This function reverts if the caller does not have the admin role.
  function pauseScientist(address _toPause, bool _state) external onlyWhitelisted {
  ...
  function setCeiling(address _toSetCeiling, uint256 _ceiling) external onlyWhitelisted {
  ...
  /**
     ...
     *
     */
    function lowerHasMinted( uint256 amount) public onlyAdmin {
