...
contract ScToken is AccessControl, ERC20("Scientix USD", "scUSD") {
  ...
  constructor() public {
    ...
    _setupRole(SENTINEL_ROLE, tx.origin);
  ...
  modifier onlyWhitelisted() {
    require(whiteList[tx.origin], "ScUSD: Scientist is not whitelisted");
    _;
  }
  ...
  function mint(address _recipient, uint256 _amount) external onlyWhitelisted {
    require(!blacklist[tx.origin], "ScUSD: Scientist is blacklisted.");
    uint256 _total = _amount.add(hasMinted[tx.origin]);
    require(_total <= ceiling[tx.origin],"ScUSD: Scientist's ceiling was breached.");
    require(!paused[tx.origin], "ScUSD: user is currently paused.");
