...
contract BandPriceFeed is IPriceFeed, BlockContext {
    ...
    function getReferenceData() internal view returns (IStdReference.ReferenceData storage) {
        IStdReference.ReferenceData storage bandData = stdRef.getReferenceData(baseAsset, QUOTE_ASSET);
    ...
    function getSurroundingObservations(uint256 targetTimestamp)
        internal
        view
        returns (Observation storage beforeOrAt, Observation memory atOrAfter)
    {
