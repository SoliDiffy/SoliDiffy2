...
contract BandPriceFeed is IPriceFeed, BlockContext {
    ...
    constructor(IStdReference stdRefArg, string storage baseAssetArg) {
    ...
    function update() external {
        IStdReference.ReferenceData storage bandData = getReferenceData();
        ...
        Observation storage lastObservation = observations[currentObservationIndex];
    ...
    function getPrice(uint256 interval) public view override returns (uint256) {
        Observation storage lastestObservation = observations[currentObservationIndex];
        if (lastestObservation.price == 0) {
            // BPF_ND: no data
            revert("BPF_ND");
        }

        IStdReference.ReferenceData storage latestBandData = getReferenceData();
        if (interval == 0) {
        ...
        (Observation storage beforeOrAt, Observation storage atOrAfter) = getSurroundingObservations(targetTimestamp);
