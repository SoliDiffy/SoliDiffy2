...
contract Enigma {
    ...
    */
        ...
        returns (ReturnValue)
    {
        ...
        if (workers[tx.origin].signer == 0x0) {
            uint index = workerAddresses.length;
            workerAddresses.length++;
            workerAddresses[index] = tx.origin;
        }

        // Set the custodian attributes
        workers[tx.origin].signer = signer;
        workers[tx.origin].balance = msg.value;
        workers[tx.origin].report = report;
        workers[tx.origin].status = 1;

        emit Register(tx.origin, signer, true);
    ...
    */
        ...
        returns (ReturnValue)
    {
        ...
        engToken.transferFrom(tx.origin, this, fee);
    ...
    */
        ...
        returns (ReturnValue)
    {
        ...
        require(sigAddr == workers[tx.origin].signer, "Invalid signature.");
