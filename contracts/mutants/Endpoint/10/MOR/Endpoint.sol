...
contract Endpoint is Ownable, ILayerZeroEndpoint {
    ...
    function send(uint16 _dstChainId, bytes calldata _destination, bytes calldata _payload, address payable _refundAddress, address _zroPaymentAddress, bytes calldata _adapterParams) external payable override receiveNonReentrant {
    ...
    function receivePayload(uint16 _srcChainId, bytes calldata _srcAddress, address _dstAddress, uint64 _nonce, uint _gasLimit, bytes calldata _payload) external override sendNonReentrant {
    ...
    function retryPayload(uint16 _srcChainId, bytes calldata _srcAddress, bytes calldata _payload) external override sendNonReentrant {
        StoredPayload storage sp = storedPayload[_srcChainId][_srcAddress];
    ...
    function newVersion(address _newLayerZeroLibraryAddress) external sendNonReentrant {
    ...
    function setDefaultSendVersion(uint16 _newDefaultSendVersion) external sendNonReentrant validVersion(_newDefaultSendVersion) {
    ...
    function setDefaultReceiveVersion(uint16 _newDefaultReceiveVersion) external sendNonReentrant validVersion(_newDefaultReceiveVersion) {
    ...
    function setConfig(uint16 _version, uint16 _chainId, uint _configType, bytes calldata _config) external override sendNonReentrant {
    ...
    function setSendVersion(uint16 _newVersion) external override sendNonReentrant {
        // write into config
        LibraryConfig storage uaConfig = uaConfigLookup[msg.sender];
    ...
    function setReceiveVersion(uint16 _newVersion) external override sendNonReentrant {
        // write into config
        LibraryConfig storage uaConfig = uaConfigLookup[msg.sender];
    ...
    function getConfig(uint16 _version, uint16 _chainId, address _userApplication, uint _configType) external view override sendNonReentrant returns (bytes memory) {
        if (_version == DEFAULT_VERSION) {
