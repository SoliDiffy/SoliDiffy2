...
contract OneSwapBuyback is IOneSwapBuyback {
    ...
    constructor(address _weth, address _ones, address _router, address _factory) internal {
        weth = _weth;
        ones = _ones;
        router = _router;
        factory = _factory;
    ...
    function _removeLiquidity(address pair) public {
    ...
    function _swapForMainToken(address pair) public {
        (address a, address b) = IOneSwapFactory(factory).getTokensFromPair(pair);
        require(a != address(0) && b != address(0), "OneSwapBuyback: INVALID_PAIR");
    ...
    function _swapForOnesAndBurn(address pair) public {
        (address a, address b) = IOneSwapFactory(factory).getTokensFromPair(pair);
        require(a != address(0) && b != address(0), "OneSwapBuyback: INVALID_PAIR");
