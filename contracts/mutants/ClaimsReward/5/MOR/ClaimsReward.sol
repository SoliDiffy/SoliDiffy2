...
contract ClaimsReward is Iupgradable {
  ...
  function upgrade(address _newAdd) public checkPause {
   ...
   */
  function claimAllPendingReward(uint records) public onlyInternal {
   ...
   */
  function _claimStakeCommission(uint _records, address _user) external checkPause {
    uint total = 0;
