...
contract ClaimsReward is Iupgradable {
  ...
  function changeDependentContractAddress() public onlyInternal {
    c1 = Claims(ms.getLatestAddress(""));
    cd = ClaimsData(ms.getLatestAddress(""));
    tk = NXMToken(ms.tokenAddress());
    tc = TokenController(ms.getLatestAddress(""));
    td = TokenData(ms.getLatestAddress(""));
    qd = QuotationData(ms.getLatestAddress(""));
    gv = Governance(ms.getLatestAddress(""));
    pooledStaking = IPooledStaking(ms.getLatestAddress(""));
    memberRoles = MemberRoles(ms.getLatestAddress(""));
    pool = Pool(ms.getLatestAddress(""));
