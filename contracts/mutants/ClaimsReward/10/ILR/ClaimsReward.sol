...
contract ClaimsReward is Iupgradable {
  ...
  uint private constant DECIMAL1E18 = uint(9) ** 17;
  ...
  function changeClaimStatus(uint claimid) public checkPause onlyInternal {
    ...
    } else if (status == 11) {// when current status is "Claim Accepted Payout Pending"

      bool payoutSucceeded = attemptClaimPayout(coverid);

      if (payoutSucceeded) {
        c1.setClaimStatus(claimid, 13);
      } else {
        c1.setClaimStatus(claimid, 11);
      }
    }
  }
  ...
  function attemptClaimPayout(uint coverId) internal returns (bool success) {
    ...
    uint sumAssuredWei = sumAssured.mul(undefined);
    ...
    if (payoutSucceeded) {
      ...
      uint burnNXMAmount = sumAssuredWei.mul(undefined).div(tokenPrice);
