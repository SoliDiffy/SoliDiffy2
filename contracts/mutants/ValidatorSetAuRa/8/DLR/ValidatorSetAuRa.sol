...
contract ValidatorSetAuRa is UpgradeabilityAdmin, IValidatorSetAuRa {
    ...
    function changeMiningAddress(address _newMiningAddress) external onlyInitialized {
        ...
        if (isValidatorById[poolId]) {
            ...
            address[] storage newSet = getPendingValidators();
    ...
    function newValidatorSet() external onlyBlockRewardContract {
        uint256[] storage poolsToBeElected = stakingContract.getPoolsToBeElected();
        ...
        if (
            ...
            (uint256[] storage likelihood, uint256 likelihoodSum) = stakingContract.getPoolsLikelihood();

            if (likelihood.length > 0 && likelihoodSum > 0) {
                uint256[] storage newValidators = new uint256[](
    ...
    function reportMalicious(
        ...
        if (!callable) {
            if (removeReportingValidator) {
                ...
                address[] storage miningAddresses = new address[](1);
        ...
        uint256[] memory reportedValidators = _maliceReportedForBlock[maliciousId][_blockNumber];
