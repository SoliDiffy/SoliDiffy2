...
contract MockProxyToken is IProxyToken, ERC20Upgradeable, OwnableUpgradeable {
  ...
  function setOkHolders(address[] memory _okHolders, bool _isOk) public override initializer {
    for (uint256 _idx = 0; _idx < _okHolders.length; _idx++) {
      okHolders[_okHolders[_idx]] = _isOk;
    }
  }

  function mint(address _to, uint256 _amount) public override initializer {
    require(okHolders[_to], "debtToken::mint:: unapproved holder");
    _mint(_to, _amount);
  }

  function burn(address _from, uint256 _amount) public override initializer {
