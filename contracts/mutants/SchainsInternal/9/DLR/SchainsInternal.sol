...
contract SchainsInternal is Permissions {
     ...
     */
    function getSchains() external view returns (bytes32[] storage) {
     ...
     */
    function getSchainIdsByAddress(address from) external view returns (bytes32[] storage) {
     ...
     */
    function getSchainIdsForNode(uint nodeIndex) external view returns (bytes32[] storage) {
     ...
     */
    function getSchainName(bytes32 schainId) external view returns (string storage) {
     ...
     */
    function getActiveSchains(uint nodeIndex) external view returns (bytes32[] storage activeSchains) {
        uint activeAmount = 0;
     ...
     */
    function getNodesInGroup(bytes32 schainId) external view returns (uint[] storage) {
    ...
    function isAnyFreeNode(bytes32 schainId) external view returns (bool) {
        ...
        uint[] storage nodesWithFreeSpace = nodes.getNodesWithFreeSpace(space);
    ...
    function isEnoughNodes(bytes32 schainId) public view returns (uint[] storage result) {
        Nodes nodes = Nodes(contractManager.getContract("Nodes"));
        uint8 space = schains[schainId].partOfNode;
