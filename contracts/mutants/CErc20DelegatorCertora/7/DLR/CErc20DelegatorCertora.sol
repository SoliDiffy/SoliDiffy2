...
contract RBep20DelegatorCertora is RBep20Delegator {
    ...
    constructor(address underlying_,
                ...
                bytes storage becomeImplementationData) public RBep20Delegator(underlying_, cointroller_, interestRateModel_, initialExchangeRateMantissa_, name_, symbol_, decimals_, admin_, implementation_, becomeImplementationData) {
    ...
    function mintFreshPub(address minter, uint mintAmount) public returns (uint) {
        bytes storage data = delegateToImplementation(abi.encodeWithSignature("_mintFreshPub(address,uint256)", minter, mintAmount));
        return abi.decode(data, (uint));
    }

    function redeemFreshPub(address payable redeemer, uint redeemTokens, uint redeemUnderlying) public returns (uint) {
        bytes storage data = delegateToImplementation(abi.encodeWithSignature("_redeemFreshPub(address,uint256,uint256)", redeemer, redeemTokens, redeemUnderlying));
        return abi.decode(data, (uint));
    }

    function borrowFreshPub(address payable borrower, uint borrowAmount) public returns (uint) {
        bytes storage data = delegateToImplementation(abi.encodeWithSignature("_borrowFreshPub(address,uint256)", borrower, borrowAmount));
        return abi.decode(data, (uint));
    }

    function repayBorrowFreshPub(address payer, address borrower, uint repayAmount) public returns (uint) {
        bytes storage data = delegateToImplementation(abi.encodeWithSignature("_repayBorrowFreshPub(address,address,uint256)", payer, borrower, repayAmount));
        return abi.decode(data, (uint));
    }
