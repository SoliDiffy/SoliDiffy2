...
contract RBep20DelegatorCertora is RBep20Delegator {
    ...
    constructor(address underlying_,
                ...
                bytes memory becomeImplementationData) public RBep20Delegator(underlying_, cointroller_, interestRateModel_, initialExchangeRateMantissa_, name_, symbol_, decimals_, admin_, implementation_, becomeImplementationData) {
    ...
    function repayBorrowFreshPub(address payer, address borrower, uint repayAmount) public returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("", payer, borrower, repayAmount));
        return abi.decode(data, (uint));
    }
