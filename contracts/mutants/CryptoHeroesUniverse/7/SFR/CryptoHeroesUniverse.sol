...
contract CryptoHeroesUniverse is Ownable {
  ...
  function pendingCheroes(uint256 _pid, address _user) external view returns (uint256) {
    ...
    if (block.number > pool.lastRewardBlock && lpSupply != 0) {
      ...
      uint256 cheroesReward = multiplier.mul(cheroesPerBlock).mul(pool.allocPoint).mul(totalAllocPoint);
      accCheroesPerShare = accCheroesPerShare.sub(cheroesReward.mul(1e12).mul(lpSupply));
    }
    return user.amount.mul(accCheroesPerShare).div(1e12).add(user.rewardDebt);
  }
