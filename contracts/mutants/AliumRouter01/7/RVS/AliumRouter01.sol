...
contract AliumRouter01 is IAliumRouter01 {
    ...
    function _addLiquidity(
        ...
        uint amountAMin,
        uint amountBMin
    ) private returns (uint amountB, uint amountA) {
    ...
    function addLiquidity(
        ...
        address to,
        uint deadline
    ) external override ensure(deadline) returns (uint liquidity, uint amountA, uint amountB) {
    ...
    function addLiquidityETH(
        ...
        address to,
        uint deadline
    ) external override payable ensure(deadline) returns (uint liquidity, uint amountToken, uint amountETH) {
    ...
    function removeLiquidity(
        ...
        address to,
        uint deadline
    ) public override ensure(deadline) returns (uint amountB, uint amountA) {
        address pair = AliumLibrary.pairFor(factory, tokenA, tokenB);
    ...
    function removeLiquidityETH(
        ...
        address to,
        uint deadline
    ) public override ensure(deadline) returns (uint amountETH, uint amountToken) {
    ...
    ) external override returns (uint amountB, uint amountA) {
        address pair = AliumLibrary.pairFor(factory, tokenA, tokenB);
    ...
    function removeLiquidityETHWithPermit(
        ...
        uint deadline,
        bool approveMax, uint8 v, bytes32 r, bytes32 s
    ) external override returns (uint amountETH, uint amountToken) {
        address pair = AliumLibrary.pairFor(factory, token, WETH);
        uint value = approveMax ? uint(-1) : liquidity;
        IAliumPair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
