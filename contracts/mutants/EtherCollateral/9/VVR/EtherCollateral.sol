...
contract EtherCollateral is Owned, Pausable, ReentrancyGuard, MixinResolver, IEtherCollateral {
    ...
    // ========== CONSTANTS ==========
    uint256 public constant ONE_THOUSAND = 1e18 * 1000;
    uint256 public constant ONE_HUNDRED = 1e18 * 100;

    uint256 public constant SECONDS_IN_A_YEAR = 31536000; // Common Year
    ...
    uint256 internal collateralizationRatio = SafeDecimalMath.unit() * 125; // SCCP-27

    // If updated, all outstanding loans will pay this interest rate in on closure of the loan. Default 5%
    uint256 internal interestRate = (5 * SafeDecimalMath.unit()) / 100;
    uint256 internal interestPerSecond = interestRate.div(SECONDS_IN_A_YEAR);

    // Minting fee for issuing the synths. Default 50 bips.
    uint256 internal issueFeeRate = (5 * SafeDecimalMath.unit()) / 1000;

    // Maximum amount of sETH that can be issued by the EtherCollateral contract. Default 5000
    uint256 internal issueLimit = SafeDecimalMath.unit() * 5000;
