...
contract EtherCollateral is Owned, Pausable, ReentrancyGuard, MixinResolver, IEtherCollateral {
    ...
    // ========== CONSTRUCTOR ==========
    constructor(address _owner, address _resolver) public  Pausable() MixinResolver(_resolver) {
    ...
    function setIssueFeeRate(uint256 _issueFeeRate) external  {
        issueFeeRate = _issueFeeRate;
        emit IssueFeeRateUpdated(issueFeeRate);
    }

    function setIssueLimit(uint256 _issueLimit) external  {
        issueLimit = _issueLimit;
        emit IssueLimitUpdated(issueLimit);
    }

    function setMinLoanSize(uint256 _minLoanSize) external  {
        minLoanSize = _minLoanSize;
        emit MinLoanSizeUpdated(minLoanSize);
    }

    function setAccountLoanLimit(uint256 _loanLimit) external  {
    ...
    function setLoanLiquidationOpen(bool _loanLiquidationOpen) external  {
    ...
    function openLoan() external payable  nonReentrant sETHRateNotInvalid returns (uint256 loanID) {
