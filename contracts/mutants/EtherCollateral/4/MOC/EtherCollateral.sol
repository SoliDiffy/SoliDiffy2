...
contract EtherCollateral is Owned, Pausable, ReentrancyGuard, MixinResolver, IEtherCollateral {
    ...
    // ========== CONSTRUCTOR ==========
    constructor(address _owner, address _resolver) public Owned(_owner) MixinResolver(_resolver) Pausable() {
    ...
    function openLoan() external payable notPaused sETHRateNotInvalid nonReentrant returns (uint256 loanID) {
    ...
    function closeLoan(uint256 loanID) external sETHRateNotInvalid nonReentrant {
        _closeLoan(msg.sender, loanID);
    }

    // Liquidation of an open loan available for anyone
    function liquidateUnclosedLoan(address _loanCreatorsAddress, uint256 _loanID) external sETHRateNotInvalid nonReentrant {
