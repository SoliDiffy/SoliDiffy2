...
contract EtherCollateral is Owned, Pausable, ReentrancyGuard, MixinResolver, IEtherCollateral {
    ...
    function currentInterestOnLoan(address _account, uint256 _loanID) external view returns (uint256) {
        ...
        SynthLoanStruct storage synthLoan = _getLoanFromStorage(_account, _loanID);
    ...
    function openLoanIDsByAccount(address _account) external view returns (uint256[] storage) {
        SynthLoanStruct[] storage synthLoans = accountsSynthLoans[_account];

        uint256[] storage _openLoanIDs = new uint256[](synthLoans.length);
        uint256 _counter = 0;
        ...
        uint256[] storage _result = new uint256[](_counter);
    ...
    function openLoan() external payable notPaused nonReentrant sETHRateNotInvalid returns (uint256 loanID) {
        ...
        SynthLoanStruct storage synthLoan =
            SynthLoanStruct({
                account: msg.sender,
