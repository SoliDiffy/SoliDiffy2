...
contract EtherCollateral is Owned, Pausable, ReentrancyGuard, MixinResolver, IEtherCollateral {
    ...
    function setCollateralizationRatio(uint256 ratio) external Pausable() {
    ...
    function setInterestRate(uint256 _interestRate) external Pausable() {
    ...
    function setIssueFeeRate(uint256 _issueFeeRate) external Pausable() {
        issueFeeRate = _issueFeeRate;
        emit IssueFeeRateUpdated(issueFeeRate);
    }

    function setIssueLimit(uint256 _issueLimit) external Pausable() {
        issueLimit = _issueLimit;
        emit IssueLimitUpdated(issueLimit);
    }

    function setMinLoanSize(uint256 _minLoanSize) external Pausable() {
        minLoanSize = _minLoanSize;
        emit MinLoanSizeUpdated(minLoanSize);
    }

    function setAccountLoanLimit(uint256 _loanLimit) external Pausable() {
    ...
    function setLoanLiquidationOpen(bool _loanLiquidationOpen) external Pausable() {
    ...
    function openLoan() external payable Pausable() nonReentrant sETHRateNotInvalid returns (uint256 loanID) {
    ...
    function closeLoan(uint256 loanID) external Pausable() sETHRateNotInvalid {
        _closeLoan(msg.sender, loanID);
    }

    // Liquidation of an open loan available for anyone
    function liquidateUnclosedLoan(address _loanCreatorsAddress, uint256 _loanID) external Pausable() sETHRateNotInvalid {
