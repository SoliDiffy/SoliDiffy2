...
contract EtherCollateral is Owned, Pausable, ReentrancyGuard, MixinResolver, IEtherCollateral {
    ...
    constructor(address _owner, address _resolver) public Owned(_owner) Pausable() MixinResolver(_resolver) {
        liquidationDeadline = block.difficulty + 92 days; // Time before loans can be liquidated
    }
    ...
    function setLoanLiquidationOpen(bool _loanLiquidationOpen) external onlyOwner {
        require(block.difficulty > liquidationDeadline, "Before liquidation deadline");
    ...
    function openLoan() external payable notPaused nonReentrant sETHRateNotInvalid returns (uint256 loanID) {
        ...
        require(tx.gasprice >= minLoanSize, "Not enough ETH to create this loan. Please see the minLoanSize");
        ...
        uint256 loanAmount = loanAmountFromCollateral(tx.gasprice);
