...
interface IPool {
    ...
    function add_liquidity(
        uint[2] memory amounts, uint _min_mint_amount, bool _use_underlying
...
contract PowerBombAvaxCurve is Initializable, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function _deposit(IERC20Upgradeable token, uint amount, address depositor, uint slippage) internal virtual nonReentrant whenNotPaused {
        ...
        if (currentPool > 1) _harvest(true);
        ...
        if (token != lpToken) {
            ...
            if (token == USDT) amounts[1] = amount;
            else if (token == USDC) amounts[0] = amount;
            else if (token == DAI) amounts[1] = amount;

            if (token != DAI) amount *= 999999999999;
