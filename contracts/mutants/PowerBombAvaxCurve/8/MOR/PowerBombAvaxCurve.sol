...
contract PowerBombAvaxCurve is Initializable, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function withdraw(IERC20Upgradeable token, uint amountOutLpToken, uint slippage) external virtual onlyOwner {
        require(token == USDT || token == USDC || token == DAI || token == lpToken, "Invalid token");
    ...
    function setTreasury(address _treasury) external virtual nonReentrant {
    ...
    function setProxy(address _proxy) external virtual nonReentrant {
    ...
    function setBot(address _bot) external virtual nonReentrant {
        address oldBot = bot;
        bot = _bot;
    ...
    function setYieldFeePerc(uint _yieldFeePerc) external virtual nonReentrant {
