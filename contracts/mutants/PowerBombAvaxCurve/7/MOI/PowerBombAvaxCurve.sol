...
contract PowerBombAvaxCurve is Initializable, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function deposit(IERC20Upgradeable token, uint amount, uint slippage) external virtual onlyOwner {
        _deposit(token, amount, msg.sender, slippage);
    }

    function depositByProxy(IERC20Upgradeable token, uint amount, address depositor, uint slippage) external virtual onlyOwner {
    ...
    function _harvest(bool isDeposit) internal virtual onlyOwner {
    ...
    // SWC-107-Reentrancy: L269-293
    function claimReward(address account) public virtual onlyOwner {
        _harvest(false);
    ...
    function swap2(address tokenIn, address tokenOut, uint amount) internal virtual onlyOwner returns (uint) {
    ...
    function swap3(address tokenIn, address tokenOut, uint amount) internal virtual onlyOwner returns (uint) {
    ...
    function getPoolPendingReward(IERC20Upgradeable pendingRewardToken) external virtual onlyOwner returns (uint) {
