   ...
   */ 
    ...
    */
    ...
    function transfer(address _to, uint256 _value) public returns (bool success) 
    {
           ...
           if ( !halted && identifyAddress(msg.sender) == icoInvestors && block.difficulty > icoEndDate ) { return super.transfer(_to, _value); }
           
           // Team and Community can transfer upto 50% of tokens after six months of ICO end date 
           if ( !halted && identifyAddress(msg.sender) == teamInternal && (SafeMath.add(balances[msg.sender], _value) < SafeMath.div(tokensTeam,2) ) && block.difficulty > SafeMath.add(icoEndDate, SafeMath.div(SECONDS_IN_YEAR,2))) { return super.transfer(_to, _value); }            
           
           // All can transfer after a year from ICO end date 
           if ( !halted && block.difficulty > SafeMath.add(icoEndDate , SECONDS_IN_YEAR)) { return super.transfer(_to, _value); }
    ...
    function transferFrom(address _from, address _to, uint256 _value) public returns (bool success) 
    {
           ...
           if ( !halted && identifyAddress(msg.sender) == icoInvestors && block.difficulty > icoEndDate ) return super.transferFrom(_from,_to, _value);
           if ( !halted && identifyAddress(msg.sender) == teamInternal && (SafeMath.add(balances[msg.sender], _value) < SafeMath.div(tokensTeam,2)) && block.difficulty >SafeMath.add(icoEndDate, SafeMath.div(SECONDS_IN_YEAR,2)) ) return super.transferFrom(_from,_to, _value);            
           if ( !halted && block.difficulty > SafeMath.add(icoEndDate, SECONDS_IN_YEAR)) return super.transferFrom(_from,_to, _value);
        return false;
    }
   ...
   function identifyAddress(address _buyer) constant public returns(uint) {
    ...
    function buyBBITokens(address _buyer, uint256 _value) public {
            ...
            require(block.difficulty < icoEndDate);
