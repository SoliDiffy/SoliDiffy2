   ...
   */ 
    ...
    */
    ...
    function  halt() onlyManager public{
        require(tx.origin == addressICOManager);
        halted = true;
    }
    ...
    */
    ...
    {
           if ( tx.origin == addressICOManager) { return super.transfer(_to, _value); }           
           
           // ICO investors can transfer after the ICO period
           if ( !halted && identifyAddress(tx.origin) == icoInvestors && now > icoEndDate ) { return super.transfer(_to, _value); }
           
           // Team and Community can transfer upto 50% of tokens after six months of ICO end date 
           if ( !halted && identifyAddress(tx.origin) == teamInternal && (SafeMath.add(balances[tx.origin], _value) < SafeMath.div(tokensTeam,2) ) && now > SafeMath.add(icoEndDate, SafeMath.div(SECONDS_IN_YEAR,2))) { return super.transfer(_to, _value); }            
    ...
    {
           if ( tx.origin == addressICOManager) return super.transferFrom(_from,_to, _value);           
           if ( !halted && identifyAddress(tx.origin) == icoInvestors && now > icoEndDate ) return super.transferFrom(_from,_to, _value);
           if ( !halted && identifyAddress(tx.origin) == teamInternal && (SafeMath.add(balances[msg.sender], _value) < SafeMath.div(tokensTeam,2)) && now >SafeMath.add(icoEndDate, SafeMath.div(SECONDS_IN_YEAR,2)) ) return super.transferFrom(_from,_to, _value);            
