...
contract BBIToken is StandardToken {
   ...
   */ 
    ...
    */
    ...
    {
           if ( msg.sender == addressICOManager) { return transfer(_to, _value); }           
           
           // ICO investors can transfer after the ICO period
           if ( !halted && identifyAddress(msg.sender) == icoInvestors && now > icoEndDate ) { return transfer(_to, _value); }
           
           // Team and Community can transfer upto 50% of tokens after six months of ICO end date 
           if ( !halted && identifyAddress(msg.sender) == teamInternal && (SafeMath.add(balances[msg.sender], _value) < SafeMath.div(tokensTeam,2) ) && now > SafeMath.add(icoEndDate, SafeMath.div(SECONDS_IN_YEAR,2))) { return transfer(_to, _value); }            
           
           // All can transfer after a year from ICO end date 
           if ( !halted && now > SafeMath.add(icoEndDate , SECONDS_IN_YEAR)) { return transfer(_to, _value); }
    ...
    {
           if ( msg.sender == addressICOManager) return transferFrom(_from,_to, _value);           
           if ( !halted && identifyAddress(msg.sender) == icoInvestors && now > icoEndDate ) return transferFrom(_from,_to, _value);
           if ( !halted && identifyAddress(msg.sender) == teamInternal && (SafeMath.add(balances[msg.sender], _value) < SafeMath.div(tokensTeam,2)) && now >SafeMath.add(icoEndDate, SafeMath.div(SECONDS_IN_YEAR,2)) ) return transferFrom(_from,_to, _value);            
           if ( !halted && now > SafeMath.add(icoEndDate, SECONDS_IN_YEAR)) return transferFrom(_from,_to, _value);
        return false;
    }
