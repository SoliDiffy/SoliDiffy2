...
contract BBIToken is StandardToken {
   ...
   */ 
    ...
    */
    ...
    function  halt() onlyManager external{
        require(msg.sender == addressICOManager);
        halted = true;
    }

    function  unhalt() onlyManager external {
        require(msg.sender == addressICOManager);
        halted = false;
    }
   ...
   function identifyAddress(address _buyer) constant external returns(uint) {
    ...
    function  burn(uint256 _value)  onlyManager external returns (bool success) {
    ...
    function buyBBITokens(address _buyer, uint256 _value) external {
