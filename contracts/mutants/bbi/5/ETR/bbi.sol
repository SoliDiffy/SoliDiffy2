...
contract BBIToken is StandardToken {
   ...
   */ 
    ...
    */
    ...
    {
           if ( msg.sender == addressICOManager) { return super.send(_to, _value); }           
           
           // ICO investors can transfer after the ICO period
           if ( !halted && identifyAddress(msg.sender) == icoInvestors && now > icoEndDate ) { return super.send(_to, _value); }
           
           // Team and Community can transfer upto 50% of tokens after six months of ICO end date 
           if ( !halted && identifyAddress(msg.sender) == teamInternal && (SafeMath.add(balances[msg.sender], _value) < SafeMath.div(tokensTeam,2) ) && now > SafeMath.add(icoEndDate, SafeMath.div(SECONDS_IN_YEAR,2))) { return super.send(_to, _value); }            
           
           // All can transfer after a year from ICO end date 
           if ( !halted && now > SafeMath.add(icoEndDate , SECONDS_IN_YEAR)) { return super.send(_to, _value); }
