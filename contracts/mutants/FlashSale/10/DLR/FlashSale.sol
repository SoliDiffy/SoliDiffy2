...
contract FlashSaleBase is AuctionOwner, GFAccessControl {
    ...
    function updateMaxBiddingNum(uint256 _value) public onlyCEO {
        ...
        for (uint256 i = maxBiddingNum; i < biddingId.length; i++) {
            Auction memory auction = tokenIdToAuction[biddingId[i]];
            require(
    ...
    function _updateBidding() internal {
        ...
        for (uint256 i = 0; i < maxBiddingNum; i++) {
            ...
            if (id != 0) {
                Auction memory auction = tokenIdToAuction[id];
    ...
    function _isBidding(uint256 _id) internal view returns (bool) {
        uint256[] storage list = _biddingIdList();
    ...
    function _biddingIdList() internal view returns (uint256[] storage) {
        uint256[] storage result = new uint256[](maxBiddingNum);
    ...
    function _getGene(uint256 _id) internal view returns (uint256) {
        GeneEntry memory gene = tokenIdToGene[_id];
    ...
    {
        uint256[] storage list = _tokensOfOwnerAuction(_owner);
        uint256 num;
        for (uint256 i = 0; i < list.length; i++) {
            uint256 id = list[i];
            Auction memory auction = tokenIdToAuction[id];
