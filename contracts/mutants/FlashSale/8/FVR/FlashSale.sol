...
contract FlashSaleBase is AuctionOwner, GFAccessControl {
    ...
    constructor(IERC20 _gaeToken, uint256 _cycleBlock) internal {
        gaeToken = _gaeToken;
    ...
    function getCycleBlock() external view returns (uint256) {
        return c_cycleBlock;
    }

    function setCycleBlock(uint256 _cycleBlock) external onlyCEO {
    ...
    function getStartBlockOffset() external view returns (uint256) {
        return c_startBlockOffset;
    }

    function setStartBlockOffset(uint256 _startBlockOffset) external onlyCEO {
        require(
