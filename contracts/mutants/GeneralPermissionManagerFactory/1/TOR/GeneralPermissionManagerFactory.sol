...
contract GeneralPermissionManagerFactory is UpgradableModuleFactory {
    ...
    function deploy(
        ...
        returns(address)
    {
        address permissionManager = address(new GeneralPermissionManagerProxy(logicContracts[latestUpgrade].version, tx.origin, polymathRegistry.getAddress("PolyToken"), logicContracts[latestUpgrade].logicContract));
