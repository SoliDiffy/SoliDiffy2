...
// ############################################################################################################################
    ...
    function calculateDepositFee(address _user,address instrument, uint256 _amount, uint boosterId) external override onlyLendingPool returns (uint256 ,uint256 ,uint256 ) {
        ...
        if ( !boosterFuelInfo[boosterId].initiated ) {
            string storage category = SIGH_Boosters.getBoosterCategory(boosterId);
...
// #################################
    ...
    function fuelTopUp(uint optionNo, uint boosterID) override external {
        ...
        optionType storage selectedOption = fuelTopUpOptions[category][optionNo];
...
// #################################
    ...
    function UpdateABoosterCategoryFuelAmount(string storage categoryName, uint initialFuel ) external override onlySighFinanceConfigurator returns (bool) {
