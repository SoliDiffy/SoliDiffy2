...
// ###############################
    ...
    */
    ...
    function refreshConfiguration() external override  returns (bool) {
...
// ############################################################################################################################
    ...
    function calculateDepositFee(address _user,address instrument, uint256 _amount, uint boosterId) external override  returns (uint256 ,uint256 ,uint256 ) {
    ...
    function calculateBorrowFee(address _user, address instrument, uint256 _amount, uint boosterId) external override  returns (uint256, uint256) {
...
// #################################
    ...
    function updateTotalDepositFeePercent(uint _depositFeePercent) external override  returns (bool) {
        totalDepositFeePercent = _depositFeePercent;
        emit depositFeePercentUpdated(_depositFeePercent);
        return true;
    }

    function updateTotalBorrowFeePercent(uint totalBorrowFeePercent_) external override  returns (bool) {
        totalBorrowFeePercent = totalBorrowFeePercent_;
        emit borrowFeePercentUpdated(totalBorrowFeePercent_);
        return true;
    }

    function updateTotalFlashLoanFeePercent(uint totalFlashLoanFeePercent_ ) external override  returns (bool) {
        totalFlashLoanFeePercent = totalFlashLoanFeePercent_;
        emit flashLoanFeePercentUpdated(totalFlashLoanFeePercent_);
        return true;
    }

    function updatePlatformFeePercent(uint _platformFeePercent) external override  returns (bool) {
        platformFeePercent = _platformFeePercent;
        emit platformFeePercentUpdated(_platformFeePercent);
        return true;
    }

    function UpdateABoosterCategoryFuelAmount(string memory categoryName, uint initialFuel ) external override  returns (bool) {
    ...
    function updateATopUpOption(string memory category, uint optionNo, uint _fee, uint _multiplier) external override  returns (bool) {
