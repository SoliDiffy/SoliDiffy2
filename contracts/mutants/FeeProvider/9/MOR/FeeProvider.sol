...
// #################################
    ...
    function updateTotalFlashLoanFeePercent(uint totalFlashLoanFeePercent_ ) external override initializer returns (bool) {
        totalFlashLoanFeePercent = totalFlashLoanFeePercent_;
        emit flashLoanFeePercentUpdated(totalFlashLoanFeePercent_);
        return true;
    }

    function updatePlatformFeePercent(uint _platformFeePercent) external override initializer returns (bool) {
        platformFeePercent = _platformFeePercent;
        emit platformFeePercentUpdated(_platformFeePercent);
        return true;
    }

    function UpdateABoosterCategoryFuelAmount(string memory categoryName, uint initialFuel ) external override initializer returns (bool) {
