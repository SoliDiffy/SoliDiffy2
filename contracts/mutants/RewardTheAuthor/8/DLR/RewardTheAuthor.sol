...
contract RewardTheAuthor is InitializableOwner, BasicMetaTransaction {
    ...
    function getSupportTokenViews() public view returns (TokenView[] storage ls) {
        uint256 len = _supportTokens.length();
        if (len == 0) {
            return ls;
        }
    ...
    function claimAll() public {
        address[] storage supportTokens = getSupportTokens();
    ...
    function getUserView(address user) public view returns (UserView storage) {
        address[] storage supportTokens = getSupportTokens();
        uint256[] storage pendingAmounts = new uint256[](supportTokens.length);
        uint256[] storage claimedAmounts = new uint256[](supportTokens.length);
