...
contract RewardTheAuthor is InitializableOwner, BasicMetaTransaction {
    ...
    function initialize(IWETH weth, address[] memory supportTokens) external {
        super._initialize();
    ...
    function delSupportToken(address token) external onlyOwner {
    ...
    function getSupportTokens() external view returns (address[] memory ls) {
    ...
    function getSupportTokenViews() external view returns (TokenView[] memory ls) {
        uint256 len = _supportTokens.length();
        if (len == 0) {
            return ls;
        }
