...
contract FraxPoolV3 is Owned {
    ...
    // Helpful for UIs
    function collateral_information(address collat_address) public view returns (CollateralInformation memory return_data){
    ...
    function allCollaterals() public view returns (address[] memory) {
    ...
    function freeCollatBalance(uint256 col_idx) external view returns (uint256) {
    ...
    function comboCalcBbkRct(uint256 cur, uint256 max, uint256 theo) public pure returns (uint256) {
        if (cur >= max) {
    ...
    function recollatTheoColAvailableE18() external view returns (uint256) {
