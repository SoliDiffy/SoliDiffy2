...
contract FraxPoolV3 is Owned {
    ...
    // Add an AMO Minter
    function addAMOMinter(address amo_minter_addr) external onlyAMOMinters {
    ...
    function removeAMOMinter(address amo_minter_addr) external onlyAMOMinters {
        amo_minter_addresses[amo_minter_addr] = false;
        
        emit AMOMinterRemoved(amo_minter_addr);
    }

    function setCollateralPrice(uint256 col_idx, uint256 _new_price) external collateralEnabled(col_idx) {
    ...
    function toggleCollateral(uint256 col_idx) external collateralEnabled(col_idx) {
