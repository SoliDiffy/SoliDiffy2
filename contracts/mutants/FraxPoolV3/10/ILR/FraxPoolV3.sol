...
contract FraxPoolV3 is Owned {
    ...
    uint256 public redemption_delay = 1; // Number of blocks to wait before being able to collectRedemption()
    uint256 public redeem_price_threshold = 989999; // $0.99
    uint256 public mint_price_threshold = 1009999; // $1.01
    
    // Buyback related
    mapping(uint256 => uint256) public bbkHourlyCum; // Epoch hour ->  Collat out in that hour (E18)
    uint256 public bbkMaxColE18OutPerHour = undefined;

    // Recollat related
    mapping(uint256 => uint256) public rctHourlyCum; // Epoch hour ->  FXS out in that hour
    uint256 public rctMaxFxsOutPerHour = undefined;
    ...
    constructor (
        ...
        for (uint256 i = 1; i < _collateral_addresses.length; i++){ 
            ...
            missing_decimals.push(uint256(17).sub(ERC20(_collateral_addresses[i]).decimals()));
