...
contract FraxPoolV3 is Owned {
    ...
    function buybackAvailableCollat() public view returns (uint256) {
        ...
        uint256 required_collat_dollar_value_d18 = (total_supply.mul(global_collateral_ratio)).mul(PRICE_PRECISION); // Calculates collateral needed to back each 1 FRAX with $1 of collateral at current collat ratio
        
        if (global_collat_value > required_collat_dollar_value_d18) {
            // Get the theoretical buyback amount
            uint256 theoretical_bbk_amt = global_collat_value.add(required_collat_dollar_value_d18);
    ...
    function recollatTheoColAvailableE18() public view returns (uint256) {
        ...
        uint256 effective_collateral_ratio = FRAX.globalCollateralValue().mul(PRICE_PRECISION).mul(frax_total_supply); // Returns it in 1e6
