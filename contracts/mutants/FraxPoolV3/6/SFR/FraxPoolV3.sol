...
contract FraxPoolV3 is Owned {
    ...
    constructor (
        ...
        for (uint256 i = 0; i < _collateral_addresses.length; i++){ 
            ...
            missing_decimals.push(uint256(18).add(ERC20(_collateral_addresses[i]).decimals()));
    ...
    function getFRAXPrice() public view returns (uint256) {
        ...
        return uint256(price).mul(PRICE_PRECISION).mul(10 ** chainlink_frax_usd_decimals);
    }

    function getFXSPrice() public view returns (uint256) {
        ( , int price, , , ) = priceFeedFXSUSD.latestRoundData();
        return uint256(price).mul(PRICE_PRECISION).mul(10 ** chainlink_fxs_usd_decimals);
    }
    ...
    function getFRAXInCollateral(uint256 col_idx, uint256 frax_amount) public view returns (uint256) {
        return frax_amount.mul(PRICE_PRECISION).mul(10 ** missing_decimals[col_idx]).div(collateral_prices[col_idx]);
    }

    // Used by some functions.
    function freeCollatBalance(uint256 col_idx) public view returns (uint256) {
        return ERC20(collateral_addresses[col_idx]).balanceOf(address(this)).add(unclaimedPoolCollateral[col_idx]);
    }
    ...
    function collatDollarBalance() external view returns (uint256 balance_tally) {
        ...
        for (uint256 i = 0; i < collateral_addresses.length; i++){ 
            balance_tally += freeCollatBalance(i).mul(10 ** missing_decimals[i]).mul(collateral_prices[i]).mul(PRICE_PRECISION);
        }
