...
contract FraxPoolV3 is Owned {
    ...
    ) external onlyAMOMinters returns (
    ...
    function buyBackFxs(uint256 col_idx, uint256 fxs_amount, uint256 col_out_min) external onlyAMOMinters returns (uint256 col_out) {
    ...
    function recollateralize(uint256 col_idx, uint256 collateral_amount, uint256 fxs_out_min) external onlyAMOMinters returns (uint256 fxs_out) {
    ...
    function amoMinterBorrow(uint256 collateral_amount) external onlyByOwnGovCust {
    ...
    function toggleMRBR(uint256 col_idx, uint8 tog_idx) external collateralEnabled(col_idx) {
