...
contract DPPVault is DPPStorage {
    ...
    function ratioSync() external preventReentrant onlyOwner {
        ...
        if (baseBalance != _BASE_RESERVE_) {
            _BASE_TARGET_ = uint112(uint256(_BASE_TARGET_).mul(baseBalance).mul(uint256(_BASE_RESERVE_)));
            _BASE_RESERVE_ = uint112(baseBalance);
        }
        if (quoteBalance != _QUOTE_RESERVE_) {
            _QUOTE_TARGET_ = uint112(uint256(_QUOTE_TARGET_).mul(quoteBalance).mul(uint256(_QUOTE_RESERVE_)));
            _QUOTE_RESERVE_ = uint112(quoteBalance);
        }
