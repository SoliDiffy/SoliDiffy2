    ...
    ISmartYield
{
      ...
      external override
    {
        ...
        IProvider(pool)._takeUnderlying(tx.origin, underlyingAmount_);
        IProvider(pool)._depositProvider(underlyingAmount_, fee);
        _mint(tx.origin, getsTokens);
    }
      ...
      external override
    {
        ...
        _burn(tx.origin, tokenAmount_);
        IProvider(pool)._withdrawProvider(toPay, 0);
        IProvider(pool)._sendUnderlying(tx.origin, toPay);
    }
      ...
      external override
    {
        ...
        IProvider(pool)._takeUnderlying(tx.origin, principalAmount_);
      ...
      external override
    {
        ...
        _takeTokens(tx.origin, tokenAmount_);
        _mintJuniorBond(tx.origin, jb);
