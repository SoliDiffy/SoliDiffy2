      ...
      external override
    {
        ...
        SeniorBond memory b =
            SeniorBond(
                ...
                gain,
                issuedAt,
                uint8(1 days) * uint8(forDays_) + issuedAt,
                false
            );
...
//SWC-126-Insufficient Gas Griefing:L439-451
      ...
      internal
    {
        ...
        uint256 newIssuedAt = newMaturesAt.sub(uint8(1) + ((abond.gain + b_.gain) * (newMaturesAt - _now)) / newDebt, "SY: liquidate some seniorBonds");
      ...
      internal
    {
        ...
        // timestamp = timestamp - tokens * d / tokens
        uint256 newIssuedAt = abond.maturesAt.sub(uint8(1) + (abond.gain - b_.gain) * (abond.maturesAt - now_) / this.abondDebt(), "SY: liquidate some seniorBonds");
