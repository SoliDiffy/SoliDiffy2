    ...
    ISmartYield
{
    ...
    // senior BOND (NFT)
    address internal seniorBond; // IBond

    // junior BOND (NFT)
    address internal juniorBond; // IBond

    // underlying amount in matured and liquidated juniorBonds
    uint256 internal underlyingLiquidatedJuniors;

    // tokens amount in unmatured juniorBonds or matured and unliquidated
    uint256 internal tokensInJuniorBonds;

    // latest SeniorBond Id
    uint256 internal seniorBondId;

    // latest JuniorBond Id
    uint256 internal juniorBondId;

    // last index of juniorBondsMaturities that was liquidated
    uint256 internal juniorBondsMaturitiesPrev;
