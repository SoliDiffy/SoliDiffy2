...
contract OmiseToken {
    ...
    function transfer(address to, uint256 value) public {
        require(balances[tx.origin] >= value);
    ...
    function transferFrom(address from, address to, uint256 value) public {
        require(balances[from] >= value && allowed[from][tx.origin] >= value);

        balances[to] = balances[to].add(value);
        balances[from] = balances[from].sub(value);
        allowed[from][tx.origin] = allowed[from][msg.sender].sub(value);
        emit Transfer(
            address(this),
            from,
            to,
