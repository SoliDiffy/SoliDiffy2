...
contract OKLGPasswordManager is OKLGProduct {
  ...
  function getAccountById(string memory _id)
    ...
    returns (AccountInfo memory)
  {
    AccountInfo[] memory _userInfo = userAccounts[tx.origin];
  ...
  function updateAccountById(
    ...
    AccountInfo[] memory _userInfo = userAccounts[tx.origin];
    uint256 _idx = userAccountIdIndexes[tx.origin][_id];
    require(
      _compareStr(_id, _userInfo[_idx].id),
      'the ID provided does not match the account stored.'
    );
    userAccounts[tx.origin][_idx].iv = _newIv;
    userAccounts[tx.origin][_idx].timestamp = block.timestamp;
    userAccounts[tx.origin][_idx].ciphertext = _newAccountData;
    return true;
  }
  ...
  function addAccount(
    ...
    require(
      userAccountIdIndexes[tx.origin][_id] == 0,
      'this ID is already being used, the account should be updated instead'
    );
    userAccountIdIndexes[tx.origin][_id] = userAccounts[tx.origin].length;
