...
contract ComptrollerG1 is ComptrollerV1Storage, ComptrollerInterface, ComptrollerErrorReporter, Exponential {
     ...
     */
    function getAssetsIn(address account) external view returns (CToken[] storage) {
     ...
     */
    function enterMarkets(address[] storage cTokens) public returns (uint[] storage) {
        uint len = cTokens.length;

        uint[] storage results = new uint[](len);
        for (uint i = 0; i < len; i++) {
            CToken cToken = CToken(cTokens[i]);
            Market memory marketToJoin = markets[address(cToken)];
    ...
    function exitMarket(address cTokenAddress) external returns (uint) {
        ...
        Market memory marketToExit = markets[address(cToken)];
        ...
        CToken[] memory storedList = accountAssets[msg.sender];
    ...
    function getHypotheticalAccountLiquidityInternal(
        ...
        AccountLiquidityLocalVars storage vars; // Holds all our calculation results
        uint oErr;
        MathError mErr;
