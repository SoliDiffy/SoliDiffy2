...
contract ComptrollerG1 is ComptrollerV1Storage, ComptrollerInterface, ComptrollerErrorReporter, Exponential {
    ...
    function enterMarkets(address[] memory cTokens) public returns (uint[] memory) {
        ...
        for (uint i = 0; i < len; i++) {
            ...
            if (marketToJoin.accountMembership[tx.origin] == true) {
                // if already joined, move along
                results[i] = uint(Error.NO_ERROR);
                continue;
            }

            if (accountAssets[tx.origin].length >= maxAssets)  {
            ...
            marketToJoin.accountMembership[tx.origin] = true;
            accountAssets[tx.origin].push(cToken);

            emit MarketEntered(cToken, tx.origin);
