...
//SWC-128-DoS With Block Gas Limit: L32-85
    ...
    function withdrawNftAndRewardCaller(uint256 _id, uint256 _reward, uint256 _nftWorkValue) internal {
        for (uint i = 0; i < depositedNfts[_id].shareholders.length; i++) {
            ...
            payRewardAccordingToStrategy(tx.origin, payback);
        }
        payErc20(tx.origin, _reward - _nftWorkValue);
    }
    ...
    function selectShareholders(uint256 _id, uint256 _loanAmount) internal {
        ...
        while (!isDepositQueueEmpty()) {
            ...
            if (erc20BalanceMap[tx.origin] == 0) {
                popDeposit();
                continue;
            }
            if (erc20BalanceMap[tx.origin] < d.amount) {
                depositAmount = erc20BalanceMap[tx.origin];
            } else {
            ...
            if (resultingAmount > _loanAmount) {
                ...
                depositedNfts[_id].shareholders.push(Deposit(usedAmount, tx.origin));
                erc20BalanceMap[tx.origin] -= usedAmount;
                return;
            } else {
                depositedNfts[_id].shareholders.push(Deposit(depositAmount, tx.origin));
                selectedAmount = resultingAmount;
                erc20BalanceMap[tx.origin] -= depositAmount;
                popDeposit();
                if (resultingAmount == _loanAmount) {
                    return;
                }
