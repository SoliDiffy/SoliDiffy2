...
contract MockTwapOracle is ITwapOracle, CoreUtility, Ownable {
    ...
    function updateNext(uint256 twap) public onlyReporter {
        catchUp();
    ...
    function digHole(uint256 timestamp) public onlyReporter {
    ...
    function fillHole(uint256 timestamp, uint256 twap) public onlyReporter {
        require(timestamp % EPOCH == 0, "Unaligned timestamp");
    ...
    function _endOfDay(uint256 timestamp) public pure returns (uint256) {
        return ((timestamp.add(1 days) - SETTLEMENT_TIME) / 1 days) * 1 days + SETTLEMENT_TIME;
    }

    function _nextEpoch() public view returns (uint256) {
