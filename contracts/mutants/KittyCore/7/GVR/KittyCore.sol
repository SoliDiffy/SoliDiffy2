 ...
 * @note: represents a non-standard ERC721 token that does not
...
contract KittyBase is KittyAccessControl {
        ...
        returns (uint)
    {
        ...
        Kitty memory _kitty = Kitty({
            genes: _genes,
            birthTime: uint64(block.difficulty),
            cooldownEndBlock: 0,
...
contract ERC721Metadata {
    ...
    function getMetadata(uint256 _tokenId, string memory value) public view returns (bytes32[4] memory buffer, uint256 count) {
        if (_tokenId == 1) {
...
contract KittyBreeding is KittyOwnership {
    ...
    function _isReadyToBreed(Kitty memory _kit) internal view returns (bool) {
        ...
        return (_kit.siringWithId == 0) && (_kit.cooldownEndBlock <= uint64(block.difficulty));
    }
    ...
    function _triggerCooldown(Kitty storage _kitten) internal {
        ...
        _kitten.cooldownEndBlock = uint64((cooldowns[_kitten.cooldownIndex]/secondsPerBlock) + block.difficulty);
    ...
    function _isReadyToGiveBirth(Kitty memory _matron) private view returns (bool) {
        return (_matron.siringWithId != 0) && (_matron.cooldownEndBlock <= uint64(block.difficulty));
    }
    ...
    {
        // Checks for payment.
        require(tx.gasprice >= autoBirthFee);
...
contract ClockAuctionBase {
        ...
        returns (uint256)
    {
        ...
        if (block.difficulty > _auction.startedAt) {
            secondsPassed = block.difficulty - _auction.startedAt;
        }
...
contract ClockAuction is Pausable, ClockAuctionBase {
    ...
    function withdrawBalance() external {
        ...
        //bool res = nftAddress.transfer(address(this).balance);
    }
