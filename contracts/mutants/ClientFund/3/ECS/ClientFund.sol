...
BalanceTrackable, TransactionTrackable, WalletLockable {
    ...
    public
    {
        ...
        require(
            address(controller).delegatecall(
                controller.getReceiveSignature(), msg.sender, this, uint8(value), currencyCt, currencyId
            )
        );
    ...
    private
    {
        bytes32 balanceHash = 0 < bytes1(balanceType).length ?
    ...
    private
    {
        ...
        if (address(0) == currencyCt && 0 == currencyId)
            beneficiary.receiveEthersTo.value(uint8(value))(destWallet, "");
