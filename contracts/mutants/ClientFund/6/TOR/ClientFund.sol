...
BalanceTrackable, TransactionTrackable, WalletLockable {
    ...
    public
    {
        ...
        value = _subtractFromStaged(tx.origin, value, currencyCt, currencyId, fungible);

        balanceTracker.add(
            tx.origin, balanceTracker.depositedBalanceType(), value, currencyCt, currencyId, fungible
        );

        // Emit event
        emit UnstageEvent(tx.origin, value, currencyCt, currencyId);
    }
