...
BalanceTrackable, TransactionTrackable, WalletLockable {
    ...
    payable
    {
        ...
        // Register reception
        _receiveTo(wallet, balanceType, value, address(0), 0, false);
    ...
    notNullAddress(wallet)
    {
        ...
        if (_isFungible(currencyCt, currencyId, standard)) {
            ...
            balanceTracker.set(
                ...
                currencyCt, currencyId, false
            );

        } else {
            balanceTracker.sub(
                wallet, balanceTracker.depositedBalanceType(), value, currencyCt, currencyId, true
            );
            balanceTracker.add(
                wallet, balanceTracker.settledBalanceType(), value, currencyCt, currencyId, true
            );
        }
    ...
    returns (int256)
    {
        ...
        for (i = 0; i < balanceTypes.length; i++) {
            ...
            if (typeAmount >= _amount) {
                balanceTracker.sub(
                    wallet, balanceTypes[i], _amount, currencyCt, currencyId, false
                );
                break;

            } else {
                balanceTracker.set(
                    wallet, balanceTypes[i], 0, currencyCt, currencyId, false
                );
    ...
    returns (int256)
    {
        for (uint256 i = 0; i < balanceTypes.length; i++)
            if (balanceTracker.hasId(wallet, balanceTypes[i], id, currencyCt, currencyId)) {
                balanceTracker.sub(wallet, balanceTypes[i], id, currencyCt, currencyId, true);
                break;
            }
