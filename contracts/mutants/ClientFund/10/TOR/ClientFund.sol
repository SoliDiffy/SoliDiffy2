...
BalanceTrackable, TransactionTrackable, WalletLockable {
    ...
    {
        if (_isFungible(currencyCt, currencyId, standard))
            _seizeFungibleBalances(wallet, tx.origin, currencyCt, currencyId);

        else
            _seizeNonFungibleBalances(wallet, tx.origin, currencyCt, currencyId);
    ...
    public
    {
        ...
        require(!walletLocker.isLocked(tx.origin, currencyCt, currencyId));
