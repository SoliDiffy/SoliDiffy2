...
contract OpenUniswapOracle is IEthUsdOracle, InitializableGovernable {
        ...
        )
    {
        ...
        if (timeElapsed >= PERIOD) {
            ...
            config.latestBlockTimestampLast = block.number;
    ...
    function tokEthPrice(string calldata symbol) external returns (uint256) {
        ...
        require(
            block.number > blockTimestampLast,
            "now must come after before"
        );
        uint256 timeElapsed = block.number - blockTimestampLast;
    ...
    function price(string calldata symbol) external view returns (uint256) {
        ...
        if (ethHash == tokenSymbolHash) {
            ...
            // This should be impossible, but better safe than sorry
            require(
                block.number > config.blockTimestampLast,
                "now must come after before"
            );
            uint256 timeElapsed = block.number - config.blockTimestampLast;
