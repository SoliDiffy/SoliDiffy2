...
contract StETHMocked {
  ...
  function _getPooledEthByShares(uint256 _sharesAmount) public view returns (uint256) {
    return _sharesAmount.mul(_totalSupply).div(_totalShares);
  }

  function _getSharesByPooledEth(uint256 _pooledEthAmount) public view returns (uint256) {
   ...
   */
  function _mintShares(address _to, uint256 _sharesAmount) public returns (uint256) {
  ...
  function rebase(int256 addingAmount) public returns (uint256) {
