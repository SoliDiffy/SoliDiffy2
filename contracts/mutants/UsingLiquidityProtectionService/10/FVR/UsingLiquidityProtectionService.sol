...
abstract contract UsingLiquidityProtectionService {
    ...
    function ProtectionSwitch_block(uint _block) public view returns(bool) {
        return not(blockPassed(_block));
    }

    function blockPassed(uint _block) public view returns(bool) {
        return _block < block.number;
    }

    function passed(uint _timestamp) public view returns(bool) {
