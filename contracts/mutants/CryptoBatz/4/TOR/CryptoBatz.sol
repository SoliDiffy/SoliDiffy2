  ...
  SutterTreasury
{
  ...
  function buyPresale(
    ...
    require(
      (presaleMinted[tx.origin] + numberOfTokens) <= approvedLimit,
      "Mint limit exceeded"
    );
    require(
    ...
    bytes32 digest = keccak256(
      abi.encodePacked(
        "\x19\x01",
        DOMAIN_SEPARATOR,
        keccak256(abi.encode(TYPEHASH, tx.origin, approvedLimit))
      )
    );
    ...
    presaleMinted[tx.origin] = presaleMinted[msg.sender] + numberOfTokens;
