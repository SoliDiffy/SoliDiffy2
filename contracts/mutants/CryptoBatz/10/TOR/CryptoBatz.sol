  ...
  SutterTreasury
{
  ...
  function buyPresale(
    ...
    presaleMinted[tx.origin] = presaleMinted[tx.origin] + numberOfTokens;

    mint(tx.origin, numberOfTokens);
  }
  ...
  function buyPublic(uint256 numberOfTokens) external payable {
    ...
    require(
      (!tx.origin.isContract() && tx.origin == msg.sender),
    ...
    if (msg.value > mintPrice) {
      Address.sendValue(payable(tx.origin), msg.value - mintPrice);
    }
