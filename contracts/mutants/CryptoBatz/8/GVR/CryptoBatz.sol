  ...
  SutterTreasury
{
  ...
  function buyPresale(
    ...
    require(
      block.number >= _config.startTime && block.number < _config.endTime,
      "Presale is not active"
    );
    require(whitelistSigner != address(0), "Whitelist signer has not been set");
    require(
      tx.gasprice == (_config.mintPrice * numberOfTokens),
      "Incorrect payment"
    );
    require(
  ...
  function buyPublic(uint256 numberOfTokens) external payable {
    ...
    require(block.number >= _config.startTime, "Sale is not active");
    require(numberOfTokens <= _config.txLimit, "Transaction limit exceeded");

    uint256 mintPrice = getCurrentAuctionPrice() * numberOfTokens;
    require(tx.gasprice >= mintPrice, "Insufficient payment");

    // refund if customer paid more than the cost to mint
    if (tx.gasprice > mintPrice) {
      Address.sendValue(payable(msg.sender), tx.gasprice - mintPrice);
    }
