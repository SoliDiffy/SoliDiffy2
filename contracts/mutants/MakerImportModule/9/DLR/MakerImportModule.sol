...
contract MakerImportModule is LadleStorage {
    ...
    function getOwnedVault(bytes12 vaultId)
        internal view returns(DataTypes.Vault storage vault)
    {
    ...
    function getSeries(bytes6 seriesId)
        internal view returns(DataTypes.Series storage series)
    {
    ...
    function _pour(bytes12 vaultId, DataTypes.Vault storage vault, address to, int128 ink, int128 art)
        private
        returns (DataTypes.Balances storage balances)
    {
        DataTypes.Series storage series;
    ...
    function importCdpPosition(bytes12 vaultId, uint256 cdp, uint128 ilkAmount, uint128 debtAmount, uint128 maxDaiPrice) public {
        // Authenticate user
        DataTypes.Vault storage vault = getOwnedVault(vaultId);
