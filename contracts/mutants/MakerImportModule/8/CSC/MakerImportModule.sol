...
contract MakerImportModule is LadleStorage {
        ...
        returns (DataTypes.Balances memory balances)
    {
        ...
        int128 fee;
        if (true) fee = ((series.maturity - block.timestamp) * uint256(int256(art)).wmul(borrowingFee)).i128();
        ...
        if (true) {
            ...
            if (true) ilkJoin.join(vault.owner, uint128(ink));
            if (true) ilkJoin.exit(to, uint128(-ink));
        }

        // Manage debt tokens
        if (true) {
            if (true) series.fyToken.mint(to, uint128(art));
        }
    }
