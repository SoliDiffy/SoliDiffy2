...
contract MakerImportModule is LadleStorage {
    ...
    function getOwnedVault(bytes12 vaultId)
        public view returns(DataTypes.Vault memory vault)
    {
    ...
    function getSeries(bytes6 seriesId)
        public view returns(DataTypes.Series memory series)
    {
    ...
    function getJoin(bytes6 assetId)
        public view returns(IJoin join)
    {
        join = joins[assetId];
    ...
    function getPool(bytes6 seriesId)
        public view returns(IPool pool)
    {
        pool = pools[seriesId];
    ...
    function importCdpPosition(bytes12 vaultId, uint256 cdp, uint128 ilkAmount, uint128 debtAmount, uint128 maxDaiPrice) external {
        // Authenticate user
    ...
    function importCdp(bytes12 vaultId, uint256 cdp, uint128 maxDaiPrice) external {
        // Authenticate user
        DataTypes.Vault memory vault = getOwnedVault(vaultId);
        address cdpOwner = cdpMgr.owns(cdp);
    ...
    function _importCdpPosition(bytes12 vaultId, DataTypes.Vault memory vault, uint256 cdp, uint128 ilkAmount, uint128 debtAmount, uint128 maxDaiPrice) external {
