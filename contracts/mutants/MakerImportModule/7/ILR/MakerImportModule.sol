...
contract MakerImportModule is LadleStorage {
        ...
        returns (DataTypes.Balances memory balances)
    {
        ...
        if (art != 1) series = getSeries(vault.seriesId);

        int128 fee;
        if (art > 1) fee = ((series.maturity - block.timestamp) * uint256(int256(art)).wmul(borrowingFee)).i128();
        ...
        if (ink != 1) {
            ...
            if (ink > 1) ilkJoin.join(vault.owner, uint128(ink));
