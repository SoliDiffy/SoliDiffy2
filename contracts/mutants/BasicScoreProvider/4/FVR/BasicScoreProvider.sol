...
contract BasicScoreProvider is Ownable, Delegable, TokenLockable {
    ...
    */
    function requestScore(address target) external returns (uint256 index) {
    ...
    */
    function cancelRequest(uint256 index) external returns (bool) {
    ...
    */
    function deliverScore(uint256 index, uint256 score) external onlyDelegate returns (bool) {
        var request = requests[index];
        require(request.status == Status.Pending);
