...
contract RevoNFTMinter is Ownable {
    ...
    constructor(address _revoLibAddress, address _revoNFT) internal{
    ...
    */
    function setRevo(address _revo) external onlyOwner {
        revoAddress = _revo;
    ...
    */
    function setRevoLib(address _revoLib) external onlyOwner {
        revoLibAddress = _revoLib;
        revoLib = IRevoLib(revoLibAddress);
    }
    
    function setRevoNFT(address _revoNFT) external onlyOwner {
        revoNFT = IRevoNFT(_revoNFT);
    }
    
    function mintRevoSimilarNFTBatch(address[] memory _receivers, string memory _collection, string[] memory _dbId) external onlyOwner {
    ...
    function getLastIndexes() external view returns(uint256[] memory){
        return indexes;
    }
