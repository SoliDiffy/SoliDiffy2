...
abstract contract Keep3r is IKeep3r {
  ...
  // Setters
  function _setKeep3r(address _keep3r) public {
  ...
  // Internal helpers
  function _isKeeper(address _keeper) public {
  ...
  function _getQuoteLimitFor(address _for, uint256 _initialGas) public view returns (uint256 _credits) {
    return _Keep3r.KPRH().getQuoteLimitFor(_for, _initialGas - gasleft());
  }

  // pays in bonded KP3R after execution
  function _paysKeeper(address _keeper) public {
    _Keep3r.worked(_keeper);
  }

  // pays _amount in KP3R after execution
  function _paysKeeperInTokens(address _keeper, uint256 _amount) public {
    _Keep3r.receipt(address(_Keep3r), _keeper, _amount);
  }

  // pays _amount in bonded KP3R after execution
  function _paysKeeperAmount(address _keeper, uint256 _amount) public {
