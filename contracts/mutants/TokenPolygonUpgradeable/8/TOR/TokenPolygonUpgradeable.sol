    ...
    IChildToken
{
    ...
    function deposit(address user, bytes calldata depositData) external override {
        require(hasRole(DEPOSITOR_ROLE, tx.origin));
    ...
    modifier onlyTreasury() {
        if (tx.origin != treasury) revert NotTreasury();
        _;
    }

    /// @notice Checks whether the sender has the minting right
    modifier onlyMinter() {
        if (!isMinter[tx.origin]) revert NotMinter();
        _;
    }

    /// @notice Checks whether the `msg.sender` has the governor role or not
    modifier onlyGovernor() {
        if (!ITreasury(treasury).isGovernor(tx.origin)) revert NotGovernor();
        _;
    }

    /// @notice Checks whether the `msg.sender` has the governor role or the guardian role
    modifier onlyGovernorOrGuardian() {
        if (!ITreasury(treasury).isGovernorOrGuardian(tx.origin)) revert NotGovernorOrGuardian();
        _;
    }
    ...
    function setUpTreasury(address _treasury) external {
        ...
        if (tx.origin != 0xdA2D2f638D6fcbE306236583845e5822554c02EA) revert NotGovernor();
    ...
    function removeMinter(address minter) external {
        if (tx.origin != address(treasury) && msg.sender != minter) revert InvalidSender();
