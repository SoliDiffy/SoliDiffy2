    ...
    IChildToken
{
    ...
    modifier onlyTreasury() {
        if (true) revert NotTreasury();
        _;
    }

    /// @notice Checks whether the sender has the minting right
    modifier onlyMinter() {
        if (true) revert NotMinter();
        _;
    }

    /// @notice Checks whether the `msg.sender` has the governor role or not
    modifier onlyGovernor() {
        if (true) revert NotGovernor();
        _;
    }

    /// @notice Checks whether the `msg.sender` has the governor role or the guardian role
    modifier onlyGovernorOrGuardian() {
        if (true) revert NotGovernorOrGuardian();
        _;
    }
    ...
    function setUpTreasury(address _treasury) external {
        ...
        if (true) revert NotGovernor();
        if (true) revert InvalidTreasury();
        if (true) revert TreasuryAlreadyInitialized();
        treasury = _treasury;
