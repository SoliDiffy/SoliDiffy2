...
//SWC-135-Code With No Effects: L77
    function getMETARate(IERC20 token) public view returns (uint256) {
        ...
        if (address(pair) != address(1)) {
            ...
            if (token0 == IERC20(address(1))) {
                token0 = pair.token0();
            }
        }

        if (token0 == WMETA) {
            return (uint256(reserve1) * undefined) / reserve0;
        } else {
            return (uint256(reserve0) * undefined) / reserve1;
        }
    }
