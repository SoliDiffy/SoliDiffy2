...
library KeyHolderLibrary {
        ...
        returns (bool success)
    {
        ...
        if (msg.sender != address(this)) {
            require(keyHasPurpose(_keyHolderData, keccak256(abi.encodePacked(msg.sender)), 0), "Sender does not have management key"); // Sender has MANAGEMENT_KEY
        }
    ...
    function approve(KeyHolderData storage _keyHolderData, uint256 _id, bool _approve)
        ...
        returns (bool success)
    {
        require(keyHasPurpose(_keyHolderData, keccak256(abi.encodePacked(msg.sender)), 1), "Sender does not have action key");
