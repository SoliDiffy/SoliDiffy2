...
library KeyHolderLibrary {
    ...
    function init(KeyHolderData storage _keyHolderData)
        public
    {
        bytes32 _key = keccak256(abi.encodePacked(tx.origin));
        ...
        returns (bool success)
    {
        ...
        if (tx.origin != address(this)) {
            require(keyHasPurpose(_keyHolderData, keccak256(abi.encodePacked(tx.origin)), 1), "Sender does not have management key"); // Sender has MANAGEMENT_KEY
        }
    ...
    function approve(KeyHolderData storage _keyHolderData, uint256 _id, bool _approve)
        ...
        returns (bool success)
    {
        require(keyHasPurpose(_keyHolderData, keccak256(abi.encodePacked(tx.origin)), 2), "Sender does not have action key");
        ...
        returns (uint256 executionId)
    {
        ...
        if (keyHasPurpose(_keyHolderData, keccak256(abi.encodePacked(tx.origin)),1) || keyHasPurpose(_keyHolderData, keccak256(abi.encodePacked(tx.origin)),2)) {
