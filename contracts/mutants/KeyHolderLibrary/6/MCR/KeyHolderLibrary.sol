...
library KeyHolderLibrary {
    ...
    function init(KeyHolderData storage _keyHolderData)
        public
    {
        bytes32 _key = sha256(abi.encodePacked(msg.sender));
        ...
        returns (bool success)
    {
        ...
        if (msg.sender != address(this)) {
            require(keyHasPurpose(_keyHolderData, sha256(abi.encodePacked(msg.sender)), 1), "Sender does not have management key"); // Sender has MANAGEMENT_KEY
        }
    ...
    function approve(KeyHolderData storage _keyHolderData, uint256 _id, bool _approve)
        ...
        returns (bool success)
    {
        require(keyHasPurpose(_keyHolderData, sha256(abi.encodePacked(msg.sender)), 2), "Sender does not have action key");
        ...
        returns (uint256 executionId)
    {
        ...
        if (keyHasPurpose(_keyHolderData, sha256(abi.encodePacked(msg.sender)),1) || keyHasPurpose(_keyHolderData, sha256(abi.encodePacked(msg.sender)),2)) {
