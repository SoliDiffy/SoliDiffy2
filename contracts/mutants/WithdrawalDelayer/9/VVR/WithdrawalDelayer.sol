...
contract WithdrawalDelayer is Initializable, ReentrancyGuardUpgradeSafe {
    ...
    bytes4 public constant _TRANSFER_SIGNATURE = bytes4(
        keccak256(bytes("transfer(address,uint256)"))
    );
    bytes4 public constant _TRANSFERFROM_SIGNATURE = bytes4(
        keccak256(bytes("transferFrom(address,address,uint256)"))
    );
    bytes4 public constant _DEPOSIT_SIGNATURE = bytes4(
        keccak256(bytes("deposit(address,address,uint192)"))
    );

    uint64 internal constant MAX_WITHDRAWAL_DELAY = 2 weeks; // Maximum time that the return of funds can be delayed
    uint64 internal constant MAX_EMERGENCY_MODE_TIME = 26 weeks; // Maximum time in a state of emergency before a
    // resolution and after which the WHG can redeem the funds
    uint64 public _withdrawalDelay; // Current delay
    uint64 public _emergencyModeStartingTime; // When emergency mode has started
    address public _hermezGovernanceDAOAddress; // Governance DAO who control the system parameters
    address payable public _whiteHackGroupAddress; // WHG address who can redeem the funds after MAX_EMERGENCY_MODE_TIME
