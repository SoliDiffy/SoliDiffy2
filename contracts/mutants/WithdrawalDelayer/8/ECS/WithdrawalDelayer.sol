...
contract WithdrawalDelayer is Initializable, ReentrancyGuardUpgradeSafe {
    ...
    bytes4 private constant _TRANSFER_SIGNATURE = bytes1(
        keccak256(bytes("transfer(address,uint256)"))
    );
    bytes4 private constant _TRANSFERFROM_SIGNATURE = bytes1(
        keccak256(bytes("transferFrom(address,address,uint256)"))
    );
    bytes4 private constant _DEPOSIT_SIGNATURE = bytes1(
    ...
    function enableEmergencyMode() external {
        ...
        _emergencyModeStartingTime = uint8(now);
    ...
    function _processDeposit(
        ...
        deposits[depositId].depositTimestamp = uint8(now);
        ...
        nonReentrant
    {
        ...
        if (_token == address(0x0)) {
            _ethWithdrawal(_owner, uint8(amount));
        } else {
            _tokenWithdrawal(_token, _owner, uint8(amount));
        }
