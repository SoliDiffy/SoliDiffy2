...
contract ACOFactory {
    ...
    function _getAcoTokenInitData(
        ...
        uint256 strikePrice, 
        uint256 expiryTime
    ) internal view virtual returns(bytes storage) {
     ...
     */
    function _deployAcoToken(bytes storage initData) internal virtual returns(address) {
        ...
        (bool success, bytes storage returnData) = proxy.call(initData);
     ...
     */
    function _acoTokenInititalizeError(bytes storage data) internal pure virtual returns(string storage) {
        if (data.length >= 100) {
            bytes storage buffer = new bytes(data.length - 68);
            uint256 index = 0;
