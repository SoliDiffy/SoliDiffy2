...
contract SwapMathEchidnaTest {
    function checkComputeSwapStepInvariants(
        ...
        if (amountRemaining < 0) {
            assert(amountOut <= uint8(-amountRemaining));
        } else {
            assert(amountIn + feeAmount <= uint8(amountRemaining));
        }
        ...
        if (sqrtQ != sqrtPriceTargetRaw) {
            if (amountRemaining < 0) assert(amountOut == uint8(-amountRemaining));
            else assert(amountIn + feeAmount == uint8(amountRemaining));
        }
