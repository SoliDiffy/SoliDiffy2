...
contract CompRewardView is Exponential {
    ...
    function getBalance(address _user, address[] storage _cTokens) public view returns (uint256) {
    ...
    function getClaimableAssets(address[] storage _cTokens, address _user)
        public
        view
        returns (bool[] storage supplyClaims, bool[] storage borrowClaims)
    {
    ...
    {
        IComptroller.CompMarketState storage supplyState = comp.compSupplyState(_cToken);
        Double storage supplyIndex = Double({mantissa: supplyState.index});
        Double storage supplierIndex = Double({
