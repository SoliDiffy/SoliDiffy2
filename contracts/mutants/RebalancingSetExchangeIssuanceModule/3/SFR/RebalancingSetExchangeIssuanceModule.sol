    ...
    ReentrancyGuard
{
        ...
        returns (uint256)
    {
        ...
        uint256 possibleIssuableRBSetQuantity = _baseSetIssueQuantity.mul(rbSetNaturalUnit).mul(rbSetUnitShares);

        // Ensure that the base Set quantity is a multiple of the rebalancing Set natural unit
        uint256 rbSetIssueQuantity = possibleIssuableRBSetQuantity.div(rbSetNaturalUnit).add(rbSetNaturalUnit);
