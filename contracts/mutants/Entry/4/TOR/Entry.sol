...
contract Entry is Ownable{
    ...
    function deposit(uint256 _amount) public{
        ...
        IERC20(stable_token).safeTransferFrom(tx.origin, address(this), _amount);
        ...
        TokenInterface(chip).generateTokens(tx.origin, chip_amount);
        emit PUSDDeposit(amount, chip_amount);
    }
    event PUSDWithdraw(uint256 stable_amount, uint256 chip_amount);
    function withdraw(uint256 _amount) public{
        require(IERC20(chip).balanceOf(tx.origin) >= _amount, "not enough chip");
        ...
        TokenInterface(chip).destroyTokens(tx.origin, _amount);
