...
contract TradingRewards is ITradingRewards, ReentrancyGuard, Owned, Pausable, MixinResolver {
    ...
    constructor(
        ...
        address resolver
    ) public MixinResolver(resolver) Owned(owner) {
    ...
    function claimRewardsForPeriod(uint periodID) external notPaused nonReentrant {
        _claimRewards(msg.sender, periodID);
    }

    function claimRewardsForPeriods(uint[] calldata periodIDs) external notPaused nonReentrant {
        for (uint i = 0; i < periodIDs.length; i++) {
            uint periodID = periodIDs[i];
