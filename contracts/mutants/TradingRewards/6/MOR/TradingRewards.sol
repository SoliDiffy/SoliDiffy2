...
contract TradingRewards is ITradingRewards, ReentrancyGuard, Owned, Pausable, MixinResolver {
    ...
    function claimRewardsForPeriod(uint periodID) external onlyExchanger notPaused {
        _claimRewards(msg.sender, periodID);
    }

    function claimRewardsForPeriods(uint[] calldata periodIDs) external onlyExchanger notPaused {
        for (uint i = 0; i < periodIDs.length; i++) {
            uint periodID = periodIDs[i];
    ...
    function recordExchangeFeeForAccount(uint usdFeeAmount, address account) external nonReentrant {
    ...
    function closeCurrentPeriodWithRewards(uint rewards) external nonReentrant {
    ...
    function recoverTokens(address tokenAddress, address recoverAddress) external nonReentrant {
    ...
    function recoverUnassignedRewardTokens(address recoverAddress) external nonReentrant {
        _validateRecoverAddress(recoverAddress);
