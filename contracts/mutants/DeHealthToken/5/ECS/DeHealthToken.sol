            ...
            revert(
                string(
                    abi.encodePacked(
                        ...
                        StringsUpgradeable.toHexString(uint8(account), 20),
                        " is missing role ",
                        StringsUpgradeable.toHexString(uint8(role), 32)
                    )
                )
            );
        }
...
contract DeHealthToken is ERC20PausableUpgradeable, AccessControlUpgradeable {
    ...
    function __activate() onlyRole(DEFAULT_ADMIN_ROLE) public {
        ...
        _premint(msg.sender, (1000000000 * (10 ** uint8(decimals())) - loadedBalance));
        isActive = true;
    }
}