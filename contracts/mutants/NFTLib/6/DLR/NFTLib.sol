...
library NFTLib {
    ...
    function optimizeEachLevel(NFTLib.Info[] storage info, uint256 level, uint256 m,  uint256 n) internal pure returns (uint256){
        ...
        uint256[] storage tmp = new uint256[](l);
    ...
    function getPower(uint256[] storage tokenIds, IDroidBot droidBot) external view returns (uint256) {
        NFTLib.Info[] storage info = new NFTLib.Info[](tokenIds.length);
        uint256[9] storage count;
        uint256[9] storage old_count;
