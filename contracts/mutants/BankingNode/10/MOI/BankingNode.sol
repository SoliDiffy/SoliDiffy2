...
contract BankingNode is ERC20("BNPL USD", "pUSD") {
     ...
     */
    function withdrawCollateral(uint256 loanId) external ensureNodeActive {
     ...
     */
    function collectAaveRewards(address[] calldata assets) external ensureNodeActive {
     ...
     */
    function collectFees() external ensureNodeActive {
     ...
     */
    function unstake() external ensureNodeActive {
     ...
     */
    function sellSlashed(uint256 minOut) external ensureNodeActive {
     ...
     */
    function _depositToLendingPool(address tokenIn, uint256 amountIn) private ensureNodeActive {
     ...
     */
    function _removeCurrentLoan(uint256 loanId) private ensureNodeActive {
    ...
    function _swapToken(
        ...
        uint256 minOut,
        uint256 amountIn
    ) private ensureNodeActive returns (uint256 tokenOutput) {
        if (amountIn == 0) {
            revert ZeroInput();
        }
