...
abstract contract DepositVaultBase is Constants, IVault, IDepositVault, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function setFeeConf(address _feeConf) public onlyOwner {
    require(_feeConf != address(0), "INVALID_FEECONF");
    feeConf = IFeeConf(_feeConf);
    emit FeeConfChanged(_feeConf);
  }

  function setAppController(address _controller) public onlyOwner {
    require(_controller != address(0), "INVALID_CONTROLLER");
    controller = _controller;
    emit ControllerChanged(_controller);
  }

  function setVaultFarm(address _farm) public onlyOwner {
    require(_farm != address(0), "INVALID_FARM");
    farm = IVaultFarm(_farm);
    emit FarmChanged(_farm);
  }

  function _deposit(address supplyer, uint256 amount) public nonReentrant {
  ...
  /**
    ...
    * @param borrower 借款人
    */
  function _liquidate(address liquidator, address borrower, bytes calldata data) public nonReentrant{
