...
contract NodesFunctionality is Permissions, INodesFunctionality {
    ...
    */
     ...
     */
    function completeWithdrawDeposit(address from, uint nodeIndex) public allow("SkaleManager") {
        INodesData nodesData = INodesData(contractManager.getContract("NodesData"));
        ValidatorService validatorService = ValidatorService(contractManager.getContract("ValidatorService"));
     ...
     * @return name of Node
     */
    function fallbackDataConverterPublicKeyAndName(bytes memory data) public pure returns (bytes memory, string memory) {
        require(data.length > 77, "Incorrect bytes data config");
