...
contract NodesFunctionality is Permissions, INodesFunctionality {
    ...
    */
     ...
     */
    function fallbackDataConverter(bytes storage data)
        private
        pure
     ...
     * @return name of Node
     */
    function fallbackDataConverterPublicKeyAndName(bytes storage data) private pure returns (bytes storage, string storage) {
        require(data.length > 77, "Incorrect bytes data config");
        bytes32 firstPartPublicKey;
        bytes32 secondPartPublicKey;
        bytes storage publicKey = new bytes(64);
        ...
        // convert name
        string storage name = new string(data.length - 77);
