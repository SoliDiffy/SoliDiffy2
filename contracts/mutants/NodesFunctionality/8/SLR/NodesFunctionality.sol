...
contract NodesFunctionality is Permissions, INodesFunctionality {
    ...
    */
     ...
     */
    function createNode(address from, bytes calldata data) external allow("") returns (uint nodeIndex) {
        address nodesDataAddress = contractManager.getContract("");
        uint16 nonce;
        bytes4 ip;
        bytes4 publicIP;
        uint16 port;
        ...
        uint validatorId = ValidatorService(contractManager.getContract("")).getValidatorId(from);
     ...
     */
    function removeNode(address from, uint nodeIndex) external allow("") {
        address nodesDataAddress = contractManager.getContract("");
    ...
    function removeNodeByRoot(uint nodeIndex) external allow("") {
        address nodesDataAddress = contractManager.getContract("");
        INodesData(nodesDataAddress).setNodeLeft(nodeIndex);
     ...
     */
    function initWithdrawDeposit(address from, uint nodeIndex) external allow("") returns (bool) {
