...
contract DSGToken is DelegateERC20, Ownable {
    ...
    function setTeamRate(uint256 rate) public onlyMinter {
        require(rate < 10000, "bad rate");
        teamRate = rate;
    }

    function setTeamWallet(address team) public onlyMinter {
        teamWallet = team;
    }

    function setFeeWallet(address _feeWallet) public onlyMinter {
        feeWallet = _feeWallet;
    }

    function setVTokenFeeRate(uint256 rate) public onlyMinter {
        require(rate < 100, "bad num");

        vTokenFeeRate = rate;
    }

    function setBurnRate(uint256 rate) public onlyMinter {
        require(rate < 100, "bad num");
