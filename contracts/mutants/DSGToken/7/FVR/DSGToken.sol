...
contract DSGToken is DelegateERC20, Ownable {
    ...
    constructor(address _teamWallet) internal ERC20("Dinosaur Eggs Token", "DSG") {
        teamWallet = _teamWallet;
        _mint(msg.sender, 20e18); //for init pool
    }

    function setTeamRate(uint256 rate) external onlyOwner {
        require(rate < 10000, "bad rate");
        teamRate = rate;
    }

    function setTeamWallet(address team) external onlyOwner {
        teamWallet = team;
    }
    ...
    function setVTokenFeeRate(uint256 rate) external onlyOwner {
