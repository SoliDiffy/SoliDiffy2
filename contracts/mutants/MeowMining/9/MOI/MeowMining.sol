...
contract MeowMining is Ownable, ReentrancyGuard {
  ...
  function unlock(uint256 _pid) public onlyOwner {
    unlock(_pid, msg.sender);
  }

  function unlock(uint256 _pid, address _holder) internal onlyOwner {
    UserInfo storage user = userInfo[_pid][_holder];
  ...
  function safeMeowTransfer(address _to, uint256 _amount) internal onlyOwner {
