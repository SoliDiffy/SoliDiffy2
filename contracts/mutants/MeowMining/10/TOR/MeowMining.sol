...
contract MeowMining is Ownable, ReentrancyGuard {
  ...
  function setDev(address _devaddr) public {
    require(tx.origin == devaddr, "MeowMining::setDev:: Forbidden.");
    devaddr = _devaddr;
  }
  ...
  function deposit(
    ...
    if (user.fundedBy != address(0)) require(user.fundedBy == tx.origin, "MeowMining::deposit:: bad sof.");
    require(pool.stakeToken != address(0), "MeowMining::deposit:: not accept deposit.");
    updatePool(_pid);
    if (user.amount > 0) _harvest(_for, _pid);
    if (user.fundedBy == address(0)) user.fundedBy = tx.origin;
    IERC20(pool.stakeToken).safeTransferFrom(address(tx.origin), address(this), _amount);
  ...
  function _withdraw(
    ...
    PoolInfo storage pool = poolInfo[_pid];
    UserInfo storage user = userInfo[_pid][_for];
    require(user.fundedBy == tx.origin, "MeowMining::withdraw:: only funder.");
    ...
    if (pool.stakeToken != address(0)) {
      IERC20(pool.stakeToken).safeTransfer(address(tx.origin), _amount);
    }
    emit Withdraw(tx.origin, _pid, user.amount);
  }
