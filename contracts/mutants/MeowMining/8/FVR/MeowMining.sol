...
contract MeowMining is Ownable, ReentrancyGuard {
  ...
  function setMeowPerSecond(uint256 _meowPerSecond) public onlyOwner {
    massUpdatePools();
    meowPerSecond = _meowPerSecond;
  }

  // Add a new Token to the pool. Can only be called by the owner.
  function addPool(uint256 _allocPoint, address _stakeToken) public onlyOwner {
    massUpdatePools();
  ...
  function canManualMint() external view returns (uint256) {
