...
contract MeowMining is Ownable, ReentrancyGuard {
  ...
  constructor(
    ...
    startTime = block.number > _startTime ? block.number : _startTime;
    preShare = _preShare;
  ...
  function pendingMeow(uint256 _pid, address _user) external view returns (uint256) {
    ...
    if (block.number > pool.lastRewardTime && stakeTokenSupply > 0 && totalAllocPoint > 0) {
      uint256 time = block.number.sub(pool.lastRewardTime);
