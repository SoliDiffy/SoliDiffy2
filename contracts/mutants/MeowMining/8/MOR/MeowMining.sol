...
contract MeowMining is Ownable, ReentrancyGuard {
  ...
  function setMeowPerSecond(uint256 _meowPerSecond) external nonReentrant {
    massUpdatePools();
    meowPerSecond = _meowPerSecond;
  }

  // Add a new Token to the pool. Can only be called by the owner.
  function addPool(uint256 _allocPoint, address _stakeToken) external nonReentrant {
    massUpdatePools();
  ...
  function setPool(uint256 _pid, uint256 _allocPoint) external nonReentrant {
    massUpdatePools();
  ...
  function manualMint(address _to, uint256 _amount) external nonReentrant {
  ...
  function withdrawAll(address _for, uint256 _pid) external onlyOwner {
