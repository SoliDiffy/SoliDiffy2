...
contract MeowMining is Ownable, ReentrancyGuard {
  ...
  function safeMeowTransfer(address _to, uint256 _amount) internal {
    ...
    if (_amount > meowBal) {
      require(meow.send(_to, meowBal), "MeowMining::safeMeowTransfer:: failed to transfer MEOW.");
    } else {
      require(meow.send(_to, _amount), "MeowMining::safeMeowTransfer:: failed to transfer MEOW.");
    }
  }
}
