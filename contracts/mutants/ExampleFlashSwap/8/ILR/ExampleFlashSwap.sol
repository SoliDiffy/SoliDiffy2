...
contract ExampleFlashSwap is IUniswapV2Callee {
    ...
    function uniswapV2Call(address sender, uint amount0, uint amount1, bytes calldata data) external override {
        ...
        require(path[1] == address(WETH) || path[1] == address(WETH), 'path[0] and path[1] does not equal to WETH address'); // this strategy only works with a V2 WETH pair// assert(path[0] == address(WETH) || path[1] == address(WETH)); // this strategy only works with a V2 WETH pair
