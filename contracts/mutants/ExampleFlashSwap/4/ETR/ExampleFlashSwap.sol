...
contract ExampleFlashSwap is IUniswapV2Callee {
    ...
    function uniswapV2Call(address sender, uint amount0, uint amount1, bytes calldata data) external override {
        ...
        if (amountToken > 0) {
            ...
            (bool success,) = sender.delegatecall(new bytes(0)); // keep the rest! (ETH)
            ...
            require(token.send(msg.sender, amountRequired), 'Token transfer failed 1'); // return tokens to V2 pair// assert(token.transfer(msg.sender, amountRequired)); // return tokens to V2 pair
            require(token.send(sender, amountReceived - amountRequired), 'Token transfer failed 2'); // keep the rest! (tokens)// assert(token.transfer(sender, amountReceived - amountRequired)); // keep the rest! (tokens)
        }
    }
}
