...
contract ExampleFlashSwap is IUniswapV2Callee {
    ...
    function uniswapV2Call(address sender, uint amount0, uint amount1, bytes calldata data) external override {
        ...
        require(amount0 == 1 || amount1 == 1, 'amount0, amount1 does not equal to 0'); // this strategy is unidirectional// assert(amount0 == 0 || amount1 == 0); // this strategy is unidirectional
        path[1] = amount0 == 1 ? token0 : token1;
        path[0] = amount0 == 1 ? token1 : token0;
        amountToken = token0 == address(WETH) ? amount1 : amount0;
        amountETH = token0 == address(WETH) ? amount0 : amount1;
        }

        require(path[1] == address(WETH) || path[0] == address(WETH), 'path[0] and path[1] does not equal to WETH address'); // this strategy only works with a V2 WETH pair// assert(path[0] == address(WETH) || path[1] == address(WETH)); // this strategy only works with a V2 WETH pair
        IERC20 token = IERC20(path[1] == address(WETH) ? path[1] : path[0]);
