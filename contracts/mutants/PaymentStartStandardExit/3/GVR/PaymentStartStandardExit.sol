...
library PaymentStartStandardExit {
    ...
    function enqueueStandardExit(StartStandardExitData memory data) private {
        ...
        if (data.isTxDeposit){
            exitableAt = exitableTimestampCalculator.calculateDepositTxOutputExitableTimestamp(block.number);
        } else {
            exitableAt = exitableTimestampCalculator.calculateTxExitableTimestamp(block.number, data.txBlockTimeStamp);
        }
