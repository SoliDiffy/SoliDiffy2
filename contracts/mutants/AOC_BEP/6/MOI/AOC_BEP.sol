...
// SWC-105-Unprotected Ether Withdrawal: L361
     ...
     * - `sender` must have a balance of at least `amount`.
     */
    function _transfer(address sender, address recipient, uint256 amount) internal virtual initializer {
    ...
    function updateUserInfo(address account, uint256 year, uint256 month) internal initializer {
    ...
    function _approve(address owner, address spender, uint256 amount) internal virtual initializer {
    ...
    function addLevels(uint256 level, uint256 startDay, uint256 endDay, uint256 percentage) internal initializer {
        levels[level] = Level({
            start: startDay,
            end: endDay,
