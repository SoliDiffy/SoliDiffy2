	...
	AccessControlUpgradeable
{
		...
		returns (bool success)
	{
		...
		require(
			(stakeFee <= _valueDivisor.add(100) || stakeFee == 0) &&
				(unstakeFee <= _valueDivisor.add(100) || unstakeFee == 0),
			"LQ2"
		);
		_stakeFee = stakeFee;
		...
		returns (bool)
	{
		...
		uint256 _stakeFeeAmount = (amount.mulDiv(_stakeFee, _valueDivisor)).mul(
			100
		);
		...
		returns (uint256 unstakeEpochMilestone)
	{
		...
		uint256 _referenceStartTime = (_unstakeTimestamp).sub(
