	...
	AccessControlUpgradeable
{
	...
	function initialize(
		...
		setUnstakeEpoch(block.number, block.number, epochInterval);
	}
		...
		returns (bool)
	{
		...
		emit StakeTokens(to, amount, _finalTokens, block.number);
		...
		returns (bool)
	{
		...
		_unstakingExpiration[to].push(block.number);
		...
		emit UnstakeTokens(to, amount, _finalTokens, block.number);
		...
		whenNotPaused
	{
		...
		for (
			...
			if (block.number >= _getUnstakeTime) {
		...
		emit WithdrawUnstakeTokens(staker, _withdrawBalance, block.number);
