        ...
        returns(uint32 typesMask, bool cache)
    {
        ...
        if (_to == validatorSetContract.randomContract()) {
            if (signature == COMMIT_HASH_SIGNATURE && _data.length > 4+32) {
                ...
                return (IRandomAuRa(_to).commitHashCallable(_sender, numberHash) ? CALL : NONE, true);
            } else if (
                ...
                return (IRandomAuRa(_to).revealNumberCallable(_sender, num) ? CALL : NONE, true);
            } else {
                return (NONE, true);
            }
        }
        ...
        if (_to == address(validatorSetContract)) {
            ...
            if (signature == EMIT_INITIATE_CHANGE_SIGNATURE) {
                ...
                return (validatorSetContract.emitInitiateChangeCallable() ? CALL : NONE, true);
                ...
                return (callable ? CALL : NONE, true);
            } else if (_gasPrice > 0) {
                ...
                return (validatorSetContract.isValidator(_sender) ? NONE : CALL, true);
            }
        }
