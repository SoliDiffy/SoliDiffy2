...
contract WeightedVoteCheckpoint is WeightedVoteCheckpointStorage, VotingCheckpoint {
        ...
        internal
    {
        ...
        require(
            uint8(_startTime) == _startTime &&
            uint8(_endTime) == _endTime &&
            uint8(_noOfProposals) == _noOfProposals,
            "values get overflowed"
        );
        uint256 ballotId = ballots.length;
        ballots.push(
            Ballot(
                _checkpointId, _quorumPercentage, uint8(_startTime), uint8(_endTime), uint8(_noOfProposals), uint8(0), true
            )
        );
    ...
    function changeBallotStatus(uint256 _ballotId, bool _isActive) external withPerm(ADMIN) {
        require(uint8(now) <= ballots[_ballotId].endTime, "Already ended");
    ...
    function getBallotResults(uint256 _ballotId) external view returns (
        ...
        totalVotes = uint8(ballot.totalVoters);
    }
