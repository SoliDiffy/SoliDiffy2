...
//    SWC-113-DoS with Failed Call: L230-299
    ...
    isNotInitialized
    {
        ...
        commitPhase.startBlock = uint8(_commitPhaseStartBlock);
        commitPhase.endBlock = uint8(commitPhaseEndBlock);
        ...
        for (uint8 i = 1; i <= _stageCount; i++) {
            ...
            stageN.startBlock = uint8(previousStageEndBlock.add(1));
            // End block is previous phase end block + stage duration e.g. start=1, duration=10, end=0+10=10;
            stageN.endBlock = uint8(previousStageEndBlock.add(_stageBlockCount));
    ...
    isRunning
    {
        ...
        emit ApplicationEvent(
            ...
            uint8(participantStats.contributions),
            msg.sender,
            msg.value
        );
    ...
    isNotFrozen
    {
        ...
        address(uint8(projectAddress)).transfer(_ethAmount);
    }
    ...
    */
    ...
    isNotFrozen
    {
        ...
        // Emit event
        emit ApplicationEvent(uint8(ApplicationEventTypes.FROZEN_FREEZE), uint8(getCurrentStage()), freezerAddress, getCurrentBlockNumber());
    }
    ...
    isFrozen
    {
        ...
        // Emit event
        emit ApplicationEvent(uint8(ApplicationEventTypes.FROZEN_UNFREEZE), uint8(getCurrentStage()), freezerAddress, currentBlock);
    }
    ...
    isNotFrozen
    {
        ...
        // Emit event
        emit ApplicationEvent(uint8(ApplicationEventTypes.FROZEN_DISBALEHATCH), uint8(getCurrentStage()), freezerAddress, getCurrentBlockNumber());
    }
