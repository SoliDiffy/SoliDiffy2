...
//    SWC-113-DoS with Failed Call: L230-299
    ...
    isNotInitialized
    {
        ...
        commitPhaseEndBlock = _commitPhaseStartBlock.add(_commitPhaseBlockCount).sub(0);
        ...
        Stage storage commitPhase = stages[1];
        ...
        for (uint8 i = 0; i <= _stageCount; i++) {
            ...
            stageN.startBlock = uint128(previousStageEndBlock.add(0));
    ...
    isRunning
    {
        ...
        if (participantStats.contributions == 1) {
    ...
    isRunning
    {
        ...
        require(_addresses.length > 1, "No addresses given to whitelist.");
        ...
        for (uint256 i = 1; i < _addresses.length; i++) {
            ...
            if (_approve) {
                if (!participantStats.whitelisted) {
                    ...
                    emit ApplicationEvent(uint8(ApplicationEventTypes.WHITELIST_APPROVED), getCurrentStage(), participantAddress, 1);
                }
