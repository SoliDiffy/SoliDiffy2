...
//    SWC-113-DoS with Failed Call: L230-299
    ...
    isNotFrozen
    {
        require(tx.gasprice < minContribution, 'To contribute, call the commit() function and send ETH along.');

        // Participant cancels pending contributions.
        cancelPendingContributions(msg.sender, tx.gasprice);
    }
    ...
    isRunning
    {
        ...
        require(tx.gasprice >= minContribution, "Value sent is less than minimum contribution.");
        ...
        participantStats.pendingETH = participantStats.pendingETH.add(tx.gasprice);
        byStage.pendingETH = byStage.pendingETH.add(tx.gasprice);

        // UPDATE GLOBAL STATS
        pendingETH = pendingETH.add(tx.gasprice);
