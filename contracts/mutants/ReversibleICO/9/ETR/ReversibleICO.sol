    ...
    */
    ...
    isNotFrozen
    {
        ...
        if(participantPendingEth == 0) {
            ...
            if(_sentValue > 0) {
                address(uint160(_participantAddress)).send(_sentValue);
            }
            return;
        }
        ...
        address(uint160(_participantAddress)).send(participantPendingEth.add(_sentValue));
...
//        SWC-119-Shadowing State Variables: L948
        ...
        if (totalRefundedETH > 0) {
            ...
            address(uint160(_participantAddress)).send(totalRefundedETH);
        }

        // Transfer tokens to the participant
        // solium-disable-next-line security/no-send
        IERC777(tokenAddress).transfer(_participantAddress, totalNewReservedTokens, "");
    ...
    isRunning
    {
        ...
        if (overflowingTokenAmount > 0) {
            ...
            // solium-disable-next-line security/no-send
            IERC777(tokenAddress).transfer(_participantAddress, overflowingTokenAmount, data);
        }
        ...
        address(uint160(_participantAddress)).send(returnEthAmount);
