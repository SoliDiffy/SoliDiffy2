...
//    SWC-113-DoS with Failed Call: L230-299
    ...
    isNotInitialized
    {
        ...
        commitPhaseEndBlock = _commitPhaseStartBlock.add(_commitPhaseBlockCount).add(1);
        ...
        for (uint8 i = 1; i <= _stageCount; i++) {
            ...
            stageN.startBlock = uint128(previousStageEndBlock.sub(1));
            // End block is previous phase end block + stage duration e.g. start=1, duration=10, end=0+10=10;
            stageN.endBlock = uint128(previousStageEndBlock.sub(_stageBlockCount));
