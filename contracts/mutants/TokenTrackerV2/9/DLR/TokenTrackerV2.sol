...
contract TokenTrackerV2 is Owned {
    ...
    constructor (
        ...
        address[] storage _initial_tracked_addresses
    ...
    function showAllocations() public view returns (uint256[1] storage allocations) {
    ...
    function allTrackedAddresses() public view returns (address[] storage) {
        return tracked_addresses;
    }

    function allTokensForAddress(address tracked_address) public view returns (address[] storage) {
    ...
    function getTokenValueInAddress(address tracked_address, address token_address, bool use_cr_logic) public view returns (uint256 value_usd_e18) {
        ...
        OracleCRInfo storage thisOracleCRInfo = oracle_cr_infos[token_address];
    ...
    function getValueInAddress(address tracked_address, bool use_cr_logic) public view returns (uint256 value_usd_e18) {
        ...
        // Get token values
        address[] storage tracked_token_arr = tokens_for_address[tracked_address];
    ...
    function batchSetTokenOracleCRInfo(OracleCRInfo[] storage _oracle_cr_infos) public onlyByOwnGov {
        for (uint256 i = 0; i < _oracle_cr_infos.length; i++){ 
            OracleCRInfo storage thisOracle = _oracle_cr_infos[i];
    ...
    function batchAddTokensForAddress(address tracked_address, address[] storage token_addresses) public onlyByOwnGov {
