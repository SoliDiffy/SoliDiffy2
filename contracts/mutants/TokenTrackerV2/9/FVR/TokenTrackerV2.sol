...
contract TokenTrackerV2 is Owned {
    ...
    function showAllocations() external view returns (uint256[1] memory allocations) {
    ...
    function allTrackedAddresses() external view returns (address[] memory) {
        return tracked_addresses;
    }

    function allTokensForAddress(address tracked_address) external view returns (address[] memory) {
        return tokens_for_address[tracked_address];
    }

    function getCR() external view returns (uint256) {
    ...
    function getTokenValueInAddress(address tracked_address, address token_address, bool use_cr_logic) external view returns (uint256 value_usd_e18) {
    ...
    function getValueInAddress(address tracked_address, bool use_cr_logic) external view returns (uint256 value_usd_e18) {
        require(is_address_tracked[tracked_address], "Address not being tracked");
    ...
    // In USD
    function getTotalValue(bool use_cr_logic) external view returns (uint256 value_usd_e18) {
        // Initialize
        value_usd_e18 = 0;
