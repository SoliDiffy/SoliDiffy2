...
contract TokenTrackerV2 is Owned {
    ...
    function setAMOMinter(address _amo_minter_address) external onlyByOwnGovCust {
        amo_minter = IFraxAMOMinter(_amo_minter_address);
    ...
    function toggleUseStoredCR() external onlyByOwnGov {
        use_stored_cr = !use_stored_cr;
    }

    // ---------------- Oracle Related ----------------

    function setTokenOracleCRInfo(address token_address, address oracle_address, bool use_cr) public onlyByOwnGovCust {
    ...
    function batchSetTokenOracleCRInfo(OracleCRInfo[] memory _oracle_cr_infos) public onlyByOwnGovCust {
    ...
    function toggleTrackedAddress(address tracked_address) public onlyByOwnGovCust {
        is_address_tracked[tracked_address] = !is_address_tracked[tracked_address];
    }

    function addTrackedAddress(address tracked_address) public onlyByOwnGovCust {
        for (uint i = 0; i < tracked_addresses.length; i++){ 
    ...
    function removeTrackedAddress(address tracked_address) public onlyByOwnGovCust {
    ...
    function batchAddTokensForAddress(address tracked_address, address[] memory token_addresses) public onlyByOwnGovCust {
        for (uint i = 0; i < token_addresses.length; i++){ 
            addTokenForAddress(tracked_address, token_addresses[i]);
        }
    }

    function addTokenForAddress(address tracked_address, address token_address) public onlyByOwnGovCust {
    ...
    function removeTokenForAddress(address tracked_address, address token_address) public onlyByOwnGovCust {
        address[] memory tracked_token_arr = tokens_for_address[tracked_address];
