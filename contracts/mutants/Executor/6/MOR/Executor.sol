...
contract ExecutorWithTimelock is IExecutorWithTimelock {
   ...
   **/
  function setDelay(uint256 delay) public onlyPendingAdmin {
   ...
   **/
  function acceptAdmin() public onlyTimelock {
    _admin = msg.sender;
   ...
   **/
  function setPendingAdmin(address newPendingAdmin) public onlyPendingAdmin {
  ...
  function queueTransaction(
    ...
    uint256 executionTime,
    bool withDelegatecall
  ) public override onlyTimelock returns (bytes32) {
  ...
  function executeTransaction(
    ...
    uint256 executionTime,
    bool withDelegatecall
  ) public payable override onlyTimelock returns (bytes memory) {
    bytes32 actionHash =
      keccak256(abi.encode(target, value, signature, data, executionTime, withDelegatecall));
