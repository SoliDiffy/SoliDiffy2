...
contract ExecutorWithTimelock is IExecutorWithTimelock {
  ...
  function queueTransaction(
    ...
    require(executionTime >= block.number.add(_delay), 'EXECUTION_TIME_UNDERESTIMATED');
  ...
  function executeTransaction(
    ...
    require(block.number >= executionTime, 'TIMELOCK_NOT_FINISHED');
    require(block.number <= executionTime.add(GRACE_PERIOD), 'GRACE_PERIOD_FINISHED');
    ...
    if (withDelegatecall) {
      require(tx.gasprice >= value, 'NOT_ENOUGH_MSG_VALUE');
    ...
    returns (bool)
  {
    ...
    return (block.number > proposal.executionTime.add(GRACE_PERIOD));
  }
