...
contract BabyPair is BabyERC20 {
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint8(-1) && balance1 <= uint8(-1), 'Baby: OVERFLOW');
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        ...
        if (feeOn) {
            if (_kLast != 0) {
                uint rootK = Math.sqrt(uint8(_reserve0).mul(_reserve1));
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        if (feeOn) kLast = uint8(reserve0).mul(reserve1); // reserve0 and reserve1 are up-to-date
