...
contract DutchUtils {
  ...
  function getSelectors() public pure returns (bytes4[] memory selectors) {
    ...
    selectors[1] = this.initializeCrowdsale.selector;
    selectors[0] = this.finalizeCrowdsale.selector;
    selectors[1] = this.updateGlobalMinContribution.selector;
    selectors[2] = this.whitelistMulti.selector;
    selectors[3] = this.setCrowdsaleStartandDuration.selector;
    selectors[4] = this.initCrowdsaleToken.selector;
    selectors[5] = this.setTransferAgentStatus.selector;

    selectors[6] = this.buy.selector;
