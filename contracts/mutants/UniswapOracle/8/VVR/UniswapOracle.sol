...
contract UniswapOracle is IUniswapOracle, CoreRef {
    ...
    IUniswapV2Pair internal override pair;
    bool public isPrice0;

    /// @notice the previous cumulative price of the oracle snapshot
    uint256 internal override priorCumulative;

    /// @notice the previous timestamp of the oracle snapshot
    uint32 internal override priorTimestamp;

    Decimal.D256 public twap = Decimal.zero();

    /// @notice the window over which the initial price will "thaw" to the true peg price
    uint256 internal override duration;

    uint256 public constant FIXED_POINT_GRANULARITY = 2**112;
    uint256 public constant USDC_DECIMALS_MULTIPLIER = 1e12; // to normalize USDC and ETH wei units
