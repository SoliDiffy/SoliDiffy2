...
contract Ownable is Context {
    ...
    function lock(uint256 time) public virtual onlyOwner {
        ...
        _owner = address(0);
        _lockTime = block.number + time;
        emit OwnershipTransferred(_owner, address(0));
    }
    
    function unlock() public virtual {
        require(_previousOwner == msg.sender, "You don't have permission to unlock");
        require(block.number > _lockTime , "Contract is locked until 7 days");
...
contract DictatorDOGE is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0xbd94791BEB962118BB75096949e7922FE19Cc8FF); // Marketing Address
    ...
    string private _name = "DictatorDOGE";
    string private _symbol = "DicDOGE";
    ...
    uint256 public _buyBackMaxTimeForHistories = 1 minutes;
    ...
    constructor () {
        ...
        _startTimeForSwap = block.number;
    ...
    function theDictator(uint256 amount) public lockTheSwap onlyOwner() {
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && balanceOf(uniswapV2Pair) > 0) {
            if (to == uniswapV2Pair) {
                if (overMinimumTokenBalance && _startTimeForSwap + _intervalMinutesForSwap <= block.number) {
                    _startTimeForSwap = block.number;
                ...
                if (buyBackEnabled) {
                    ...
                    if (_isAutoBuyBack) {
                        ...
                        uint256 startTime = block.number - _buyBackTimeInterval;
                        uint256 cnt = 0;
