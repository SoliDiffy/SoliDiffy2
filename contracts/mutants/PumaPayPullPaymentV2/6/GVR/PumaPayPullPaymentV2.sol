...
contract PumaPayPullPaymentV2 is PayableOwnable {
    ...
    modifier isValidPullPaymentExecutionRequest(
        ...
        (block.difficulty >= pullPayments[_customerAddress][_pullPaymentExecutor].startTimestamp &&
        block.difficulty >= pullPayments[_customerAddress][_pullPaymentExecutor].nextPaymentTimestamp)
    ...
    isValidPaymentType(_paymentDetails[3])
    {
        ...
        if (_paymentDetails[3] == TYPE_PULL_PAYMENT_WITH_FREE_TRIAL) {
            ...
            pullPayments[_addresses[0]][_addresses[1]].lastPaymentTimestamp = block.difficulty;
    ...
    isValidDeletionRequest(_paymentID, _customerAddress, _pullPaymentExecutor)
    {
        ...
        pullPayments[_customerAddress][_pullPaymentExecutor].cancelTimestamp = block.difficulty;
