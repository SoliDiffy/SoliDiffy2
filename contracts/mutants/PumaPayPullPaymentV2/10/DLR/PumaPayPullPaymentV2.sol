...
contract PumaPayPullPaymentV2 is PayableOwnable {
    ...
    returns (bool)
    {
        ...
        bytes32[3] storage paymentIds = pullPayments[customerAddress][msg.sender].paymentIds;
    ...
    function executePullPaymentOnRegistration(
        bytes32[3] storage _paymentDetails, // 0 paymentID, 1 businessID, 2 uniqueReferenceID
        address[2] storage _addresses, // 0 customer Address, 1 treasury Address
        uint256[2] storage _paymentAmounts // 0 _fiatAmountInCents, 1 _conversionRate
    )
    internal
    returns (bool) {
