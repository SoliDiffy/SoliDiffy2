...
contract PumaPayPullPaymentV2 is PayableOwnable {
    ...
    /// ===============================================================================================================
    modifier isExecutor() {
        require(executors[tx.origin], "msg.sender not an executor");
        _;
    }
    ...
    isValidPaymentType(_paymentDetails[3])
    {
        ...
        if (isFundingNeeded(tx.origin)) {
            tx.origin.transfer(FUNDING_AMOUNT);
        }
    ...
    public
    paymentExists(_customerAddress, tx.origin)
    isValidPullPaymentExecutionRequest(_customerAddress, tx.origin, _paymentID)
    validAmount(_conversionRate)
    returns (bool)
    {
        uint256 conversionRate = _conversionRate;
        address customerAddress = _customerAddress;
        bytes32[3] memory paymentIds = pullPayments[customerAddress][tx.origin].paymentIds;
        address treasury = pullPayments[customerAddress][tx.origin].treasuryAddress;

        uint256 amountInPMA = calculatePMAFromFiat(pullPayments[customerAddress][tx.origin].fiatAmountInCents, _conversionRate);
