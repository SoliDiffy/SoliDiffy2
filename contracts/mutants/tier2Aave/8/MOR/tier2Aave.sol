...
function updateATokens(address tokenAddress, address aTokenAddress) public onlyOwner returns (bool){
    tokenToAToken[tokenAddress] = aTokenAddress;
    aTokenToToken[aTokenAddress] = tokenAddress;
    return true;
}

  function addOrEditStakingContract(string memory name, address stakingAddress, address stakingToken ) public onlyAdmin returns (bool){
  ...
  function updateCommission(uint amount) public onlyAdmin returns(bool){
      commission = amount;
      return true;
  }

  function deposit(address tokenAddress, uint256 amount, address onBehalfOf) payable onlyAdmin public returns (bool){
  ...
  function withdraw(address tokenAddress, uint256 amount, address payable onBehalfOf) onlyAdmin payable public returns(bool){
   ...
   function changeOwner(address payable newAdmin) onlyAdmin public returns (bool){
     owner = newAdmin;
     return true;
   }

   function changeAdmin(address payable newOwner) onlyOwner public returns (bool){
     admin = newOwner;
     return true;
   }
  ...
  function adminEmergencyWithdrawTokens(address token, uint amount, address payable destination) public onlyAdmin returns(bool) {
