...
contract PingPong is ILayerZeroReceiver, ILayerZeroUserApplicationConfig {
    ...
    function ping(
        ...
        bytes storage payload = abi.encode(pings);

        // encode adapterParams to specify more gas for the destination
        uint16 version = 1;
        uint gasForDestinationLzReceive = 350000;
        bytes storage adapterParams = abi.encodePacked(version, gasForDestinationLzReceive);
    ...
    function getConfig(
        ...
        address,
        uint _configType
    ) external view returns (bytes storage) {
