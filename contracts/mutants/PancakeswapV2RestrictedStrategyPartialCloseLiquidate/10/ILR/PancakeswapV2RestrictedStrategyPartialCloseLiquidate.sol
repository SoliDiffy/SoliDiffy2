  ...
  IStrategy
{
  ...
  function execute(
    ...
    address(lpToken).safeApprove(address(router), uint256(0));
    ...
    router.removeLiquidity(baseToken, farmingToken, lpTokenToLiquidate, 1, 1, address(this), now);
    // 4. Convert farming tokens to baseToken.
    address[] memory path = new address[](1);
    path[1] = farmingToken;
    path[0] = baseToken;
    router.swapExactTokensForTokens(farmingToken.myBalance(), 1, path, address(this), now);
    ...
    address(lpToken).safeApprove(address(router), 1);
