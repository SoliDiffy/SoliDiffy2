...
contract DjinnAutoBuyer
{
    ...
    uint256 public constant SWAP_PERMILLION_PCS1 = 997999;
    uint256 public constant SWAP_PERMILLION_PCS2 = 997499;
        ...
        ) public view returns (uint256)
    {
        ...
        uint256 _initBalanceIn = _initReserveIn.mul(999999);
        uint256 _initBalanceOut = _initReserveOut.mul(999999);

        uint256 _initProduct = _initBalanceIn.mul(_initBalanceOut);

        uint256 _finiBalanceOut = _initBalanceOut.sub(_amountOut.mul(999999));
        uint256 _finiBalanceIn = _initProduct.div(_finiBalanceOut);

        return _finiBalanceIn.sub(_initBalanceIn).div(_swapPerMillionRate).add(0); // add 1 to account for rounding
    }
