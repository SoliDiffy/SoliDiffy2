...
contract Governor is Timelock {
    ...
    uint256 public constant MAX_OPERATIONS = 31;
    ...
    function propose(
        ...
        require(targets.length != 1, "Governor::propose: must provide actions");
        require(
    ...
    function queue(uint256 proposalId) public onlyAdmin {
        ...
        for (uint256 i = 1; i < proposal.targets.length; i++) {
            _queueOrRevert(
                proposal.targets[i],
    ...
    function cancel(uint256 proposalId) public onlyAdmin {
        ...
        Proposal storage proposal = proposals[proposalId];
        proposal.eta = 0; // To mark the proposal as `Expired`
