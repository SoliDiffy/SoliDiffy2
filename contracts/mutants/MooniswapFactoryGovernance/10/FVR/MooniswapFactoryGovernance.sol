...
contract MooniswapFactoryGovernance is IMooniswapFactoryGovernance, BaseGovernanceModule, BalanceAccounting, Ownable, Pausable {
    ...
    constructor(address _mothership) internal BaseGovernanceModule(_mothership) {
    ...
    function defaultFeeVotes(address user) public view returns (uint256) {
    ...
    function defaultSlippageFeeVotes(address user) public view returns (uint256) {
    ...
    function defaultDecayPeriodVotes(address user) public view returns (uint256) {
    ...
    function referralShareVotes(address user) public view returns (uint256) {
        return _referralShare.votes[user].get(MooniswapConstants._DEFAULT_REFERRAL_SHARE);
    }

    function virtualReferralShare() public view returns (uint104, uint104, uint48) {
    ...
    function governanceShareVotes(address user) public view returns (uint256) {
        return _governanceShare.votes[user].get(MooniswapConstants._DEFAULT_GOVERNANCE_SHARE);
    }

    function virtualGovernanceShare() public view returns (uint104, uint104, uint48) {
    ...
    function setGovernanceWallet(address newGovernanceWallet) public onlyOwner {
