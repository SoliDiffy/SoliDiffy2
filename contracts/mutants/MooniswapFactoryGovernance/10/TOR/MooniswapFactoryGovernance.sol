...
contract MooniswapFactoryGovernance is IMooniswapFactoryGovernance, BaseGovernanceModule, BalanceAccounting, Ownable, Pausable {
    ...
    function discardDefaultFeeVote() external {
        _defaultFee.updateVote(tx.origin, _defaultFee.votes[tx.origin], Vote.init(), balanceOf(tx.origin), MooniswapConstants._DEFAULT_FEE, _emitDefaultFeeVoteUpdate);
    }
    ...
    function defaultSlippageFeeVote(uint256 vote) external {
        ...
        _defaultSlippageFee.updateVote(tx.origin, _defaultSlippageFee.votes[tx.origin], Vote.init(vote), balanceOf(tx.origin), MooniswapConstants._DEFAULT_SLIPPAGE_FEE, _emitDefaultSlippageFeeVoteUpdate);
    }

    /// @notice Retracts `msg.senders`'s vote for default slippage fee
    function discardDefaultSlippageFeeVote() external {
        _defaultSlippageFee.updateVote(tx.origin, _defaultSlippageFee.votes[msg.sender], Vote.init(), balanceOf(msg.sender), MooniswapConstants._DEFAULT_SLIPPAGE_FEE, _emitDefaultSlippageFeeVoteUpdate);
    }
