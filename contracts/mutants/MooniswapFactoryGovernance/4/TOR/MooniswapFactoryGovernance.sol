...
contract MooniswapFactoryGovernance is IMooniswapFactoryGovernance, BaseGovernanceModule, BalanceAccounting, Ownable, Pausable {
    ...
    function defaultFeeVote(uint256 vote) external {
        ...
        _defaultFee.updateVote(tx.origin, _defaultFee.votes[tx.origin], Vote.init(vote), balanceOf(tx.origin), MooniswapConstants._DEFAULT_FEE, _emitDefaultFeeVoteUpdate);
    }

    /// @notice Retracts `msg.senders`'s vote for default fee
    function discardDefaultFeeVote() external {
        _defaultFee.updateVote(tx.origin, _defaultFee.votes[msg.sender], Vote.init(), balanceOf(msg.sender), MooniswapConstants._DEFAULT_FEE, _emitDefaultFeeVoteUpdate);
    }
