        ...
        if (pandoBoxCreated[_ndays] < pandoBoxPerDay) {
            if (createPandoBoxFee > 0) {
                if (_option == 0) { // only PAN
                    PAN.safeTransferFrom(tx.origin, address(this), createPandoBoxFee);
                    ...
                    ERC20Burnable(address(PAN)).burnFrom(tx.origin, createPandoBoxFee * PSRRatio / 100);
                    ERC20Burnable(address(PSR)).burnFrom(tx.origin, _amount_PSR);
                }
            }
            pandoBoxCreated[_ndays]++;
            uint256 _lv = getPandoBoxLv();
            uint256 _boxId = pandoBox.create(tx.origin, _lv);
            emit BoxCreated(tx.origin, _lv, _option, _boxId);
        }
    }

    function createDroidBot(uint256 _pandoBoxId) external {
        if (pandoBox.ownerOf(_pandoBoxId) == tx.origin) {
            ...
            uint256 _newBotId = droidBot.create(tx.origin, _lv, _power);

            (uint256 _megaNum, uint256 _minorNum) = dataStorage.getJDroidBotCreating(_info.level);
            pandoPot.enter(tx.origin, _megaNum, _minorNum, PANOracle.consult());
            emit BotCreated(tx.origin, _pandoBoxId, _newBotId);
        }
    }

    function upgradeDroidBot(uint256 _droidBot0Id, uint256 _droidBot1Id) external{
        require(droidBot.ownerOf(_droidBot0Id) == tx.origin && droidBot.ownerOf(_droidBot1Id) == msg.sender, 'NFTRouter : not owner of bot');
