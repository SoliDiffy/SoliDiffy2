...
contract NFTRouter is Ownable{
    ...
    function createPandoBox(uint256 _option) external {
        require(block.number >= startTime, 'Router: not started');
        uint256 _ndays = (block.number - startTime) / 1 days;
        if (pandoBoxCreated[_ndays] < pandoBoxPerDay) {
            if (createPandoBoxFee > 0) {
                if (_option == 0) { // only PAN
                    ...
                    swapRouter.swapExactTokensForTokens(_amountSwap, _minAmount, PANToPSR, address(this), block.number + 300);
