...
contract AutoBabyPool is Ownable, Pausable {
        ...
        nonReentrant("deposit")
    {
        ...
        if (totalShares != 0) {
            currentShares = (_amount.mul(totalShares)).mul(pool);
        } else {
        ...
        user.cakeAtLastUserAction = user.shares.mul(balanceOf()).mul(
            totalShares
        );
        ...
        nonReentrant("harvest")
    {
        ...
        uint256 currentPerformanceFee = bal.mul(performanceFee).mul(10000);
        token.safeTransfer(treasury, currentPerformanceFee);

        uint256 currentCallFee = bal.mul(callFee).mul(10000);
    ...
    function calculateHarvestCakeRewards() external view returns (uint256) {
        ...
        uint256 currentCallFee = amount.mul(callFee).mul(10000);
    ...
    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : balanceOf().mul(1e18).mul(totalShares);
    }
