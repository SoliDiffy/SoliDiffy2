...
contract BalancerLBPSwapper is IPCVSwapper, OracleRef, Timed, WeightedBalancerPoolManager {
    ...
    */
    ...
    function init(IWeightedPool _pool) public {
    ...
    */
    ...
    function swapEndTime() external view returns(uint256 endTime) { 
        (,endTime,) = pool.getGradualWeightUpdateParams();
    }

    /// @notice returns the token reserves of `pool` and the last block they updated
    function getReserves() external view returns(uint256 spentReserves, uint256 receivedReserves, uint256 lastChangeBlock) {
    ...
    function getPoolBalances(uint256 spentReserves, uint256 receivedReserves) external view returns (
        uint256 bptTotal, 
        uint256 bptBalance, 
        uint256 spentBalance, 
        uint256 receivedBalance
    ...
    function setSwapFrequency(uint256 _frequency) public onlyGovernorOrAdmin {
    ...
    function setMinTokenSpent(uint256 newMinTokenSpentBalance) public onlyGovernorOrAdmin {
    ...
    function _setReceivingAddress(address newTokenReceivingAddress) public {
