...
contract BalancerLBPSwapper is IPCVSwapper, OracleRef, Timed, WeightedBalancerPoolManager {
    ...
    */
    ...
    function init(IWeightedPool _pool) external {
        ...
        pid = _pid;
        (IERC20[] storage tokens,,) = _vault.getPoolTokens(_pid);
    ...
    */
    ...
    function swap() external override afterTime whenNotPaused {
        ...
        if (bptBalance != 0) {
            IVault.ExitPoolRequest storage exitRequest;

            // Uses encoding for exact BPT IN withdrawal using all held BPT
            bytes storage userData = abi.encode(IWeightedPool.ExitKind.EXACT_BPT_IN_FOR_TOKENS_OUT, bptBalance);
        ...
        if (spentTokenBalance > minTokenSpentBalance) {
            ...
            uint256[] storage amountsIn = _getTokensIn(spentTokenBalance);
            bytes storage userData = abi.encode(IWeightedPool.JoinKind.EXACT_TOKENS_IN_FOR_BPT_OUT, amountsIn, 0);

            IVault.JoinPoolRequest storage joinRequest;
    ...
    function getReserves() public view returns(uint256 spentReserves, uint256 receivedReserves, uint256 lastChangeBlock) {
        (IERC20[] storage tokens, uint256[] storage balances, uint256 _lastChangeBlock ) = vault.getPoolTokens(pid);
