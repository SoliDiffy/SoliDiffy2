...
contract Cloudbric is StandardToken, BurnableToken, Ownable {
    ...
    string internal constant symbol = "CLB";
    string internal constant name = "Cloudbric";
    uint8 internal constant decimals = 18;
    uint256 internal constant INITIAL_SUPPLY = 1000000000 * (10 ** uint256(decimals));
    uint256 internal constant TOKEN_SALE_ALLOWANCE = 540000000 * (10 ** uint256(decimals));
    uint256 internal constant ADMIN_ALLOWANCE = INITIAL_SUPPLY - TOKEN_SALE_ALLOWANCE;

    // Address of token administrator
    address internal adminAddr;

    // Address of token sale contract
    address internal tokenSaleAddr;

    // Enable transfer after token sale is completed
    bool internal transferEnabled = false;
