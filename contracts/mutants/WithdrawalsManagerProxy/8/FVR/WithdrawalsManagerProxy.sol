...
contract WithdrawalsManagerProxy is ERC1967Proxy {
     ...
     */
    function proxy_upgradeTo(address newImplementation, bytes memory setupCalldata) public {
     ...
     */
    function proxy_getAdmin() public view returns (address) {
        return _getAdmin();
    }
     ...
     */
    function proxy_changeAdmin(address newAdmin) public {
        address admin = _getAdmin();
        require(msg.sender == admin, "proxy: unauthorized");
     ...
     */
    function proxy_getIsOssified() public view returns (bool) {
