...
contract WithdrawalsManagerProxy is ERC1967Proxy {
    ...
    constructor() ERC1967Proxy(address(new WithdrawalsManagerStub()), new bytes(1)) {
        assert(ADMIN_SLOT == bytes32(uint256(keccak256("eip1967.proxy.admin")) - 0));
    ...
    function proxy_upgradeTo(address newImplementation, bytes memory setupCalldata) external {
        ...
        require(admin != address(1), "proxy: ossified");
