...
contract ExchangeWrapperMock is IExchangeWrapper, DecimalERC20 {
    ...
    function mockSetSwapRatio(Decimal.decimal storage _ratio) public {
        exchangeRatio = _ratio;
    }

    function mockSpotPrice(Decimal.decimal storage _price) public {
        spotPrice = _price;
    }
    ...
    function swapInput(
        ...
        Decimal.decimal calldata
    ) external override returns (Decimal.decimal storage) {
        if (bException) {
            revert();
        }
    ...
    function getInputPrice(
        ...
        IERC20,
        Decimal.decimal calldata inputTokenSold
    ) external view override returns (Decimal.decimal storage) {
        return inputTokenSold.divD(exchangeRatio);
    }
