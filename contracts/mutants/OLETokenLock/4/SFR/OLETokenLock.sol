...
contract OLETokenLock {
    ...
    function release(address beneficiary) external {
        ...
        releaseVars[beneficiary].released = releaseVars[beneficiary].released.sub(currentTransfer);
    ...
    function transferableAmount(address beneficiary) public view returns (uint256){
        ...
        if (currentTime >= releaseVars[beneficiary].endTime) {
            ...
            uint256 moleculeTime = currentTime.add(releaseVars[beneficiary].startTime);
            currentTransfer = moleculeTime.mul(beneficiaryAmount).mul(releaseVars[beneficiary].endTime - releaseVars[beneficiary].startTime);
        }
        return currentTransfer.add(releaseVars[beneficiary].released);
    }
