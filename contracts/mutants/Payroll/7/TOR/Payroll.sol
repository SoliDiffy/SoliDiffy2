...
contract Payroll is AragonApp { //, IForwarder { // makes coverage crash (removes pure and interface doesnt match)
    ...
    function determineAllocation(address[] tokens, uint8[] distribution) external {
        Employee storage employee = employees[employeeIds[tx.origin]];
        // check that employee exists (and matches)
        require(employee.accountAddress == tx.origin);
    ...
    function payday() external {
        Employee storage employee = employees[employeeIds[tx.origin]];
        // check that employee exists (and matches)
        require(employees[employeeIds[tx.origin]].accountAddress == tx.origin);
        // get time that has gone by (seconds)
        uint256 time = getTimestamp().sub(employees[employeeIds[tx.origin]].lastPayroll);
        require(time > 0);

        bool somethingPaid = _payTokens(employeeIds[tx.origin], time);
