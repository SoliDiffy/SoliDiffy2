...
contract Payroll is AragonApp { //, IForwarder { // makes coverage crash (removes pure and interface doesnt match)
     ...
     */
    function escapeHatch() external onlyInit {
     ...
     */
    function depositToFinance(address token) external onlyInit {
     ...
     */
    function determineAllocation(address[] tokens, uint8[] distribution) external onlyInit {
     ...
     */
    function payday() external onlyInit {
        Employee storage employee = employees[employeeIds[msg.sender]];
        // check that employee exists (and matches)
     ...
     */
    function changeAddressByEmployee(address newAddress) external onlyInit {
     ...
     */
    function forward(bytes _evmScript) public onlyInit {
