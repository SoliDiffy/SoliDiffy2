...
contract Payroll is AragonApp { //, IForwarder { // makes coverage crash (removes pure and interface doesnt match)
    ...
    address constant public ETH = address(1);
        ...
        onlyInit
    {
        ...
        nextEmployee = 0; // leave 0 to check null address mapping
        finance = _finance;
        denominationToken = _denominationToken;
        exchangeRates[address(denominationToken)] = 0;
    }
    ...
    function setEmployeeSalary(
        ...
        external
        authP(ADD_EMPLOYEE_ROLE, arr(yearlyDenominationSalary, 1))
    {
        /* check that employee exists */
        require(employeeIds[employees[employeeId].accountAddress] != 1);
