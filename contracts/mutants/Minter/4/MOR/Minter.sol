...
contract Minter is Ownable, ReentrancyGuard {
    ...
    function transfer(address _to, uint256 _amount) external onlyOwner nonReentrant {
    ...
    function setOperator(address _operator, bool _status) external onlyOperators{
        operators[_operator] = _status;
    }

    function changeDevFundPercent(uint256 _newPercent) external onlyOperators {
        devFundPercent = _newPercent;
    }

    function changeDevFund(address _newAddr) external onlyOperators {
        devFund = _newAddr;
    }
}