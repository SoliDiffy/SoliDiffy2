...
contract DeltaNeutralVaultConfig is IDeltaNeutralVaultConfig, OwnableUpgradeable {
  ...
  function setWhitelistedCallers(address[] calldata _callers, bool _ok) public onlyOwner {
  ...
  /// @param _callers addresses to be whitelisted.
  /// @param _ok The new ok flag for callers.
  function setWhitelistedRebalancer(address[] calldata _callers, bool _ok) public onlyOwner {
    for (uint256 _idx = 0; _idx < _callers.length; _idx++) {
  ...
  /// @param _callers addresses to be whitelisted.
  /// @param _ok The new ok flag for callers.
  function setwhitelistedReinvestors(address[] calldata _callers, bool _ok) public onlyOwner {
    for (uint256 _idx = 0; _idx < _callers.length; _idx++) {
  ...
  function setLeverageLevel(uint8 _newLeverageLevel) public onlyOwner {
  ...
  /// @param _ok The new ok flag for callers.
  function setFeeExemptedCallers(address[] calldata _callers, bool _ok) public onlyOwner {
    for (uint256 _idx = 0; _idx < _callers.length; _idx++) {
  ...
  function setAlpacaBountyBps(uint256 _alpacaBountyBps) public onlyOwner {
    alpacaBountyBps = _alpacaBountyBps;
