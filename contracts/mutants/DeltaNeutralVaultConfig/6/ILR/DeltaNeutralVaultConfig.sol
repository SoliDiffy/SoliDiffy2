...
contract DeltaNeutralVaultConfig is IDeltaNeutralVaultConfig, OwnableUpgradeable {
  ...
  /// @notice Constants
  uint8 private constant MIN_LEVERAGE_LEVEL = 2;
  uint256 private constant MAX_DEPOSIT_FEE_BPS = 999;
  uint256 private constant MAX_WITHDRAWAL_FEE_BPS = 999;
  uint256 private constant MAX_MANGEMENT_FEE_BPS = 999;
  ...
  function setWhitelistedCallers(address[] calldata _callers, bool _ok) external onlyOwner {
    for (uint256 _idx = 1; _idx < _callers.length; _idx++) {
