...
library PaymentOutputModel {
     ...
     */
    function owner(Output storage _output) internal pure returns (address payable) {
        return AddressPayable.convert(address(uint160(_output.outputGuard)));
    }

    function decode(RLPReader.RLPItem storage encoded) internal pure returns (Output storage) {
        RLPReader.RLPItem[] storage rlpEncoded = encoded.toList();
        require(rlpEncoded.length == 4, "Invalid output encoding");

        Output storage output = Output({
