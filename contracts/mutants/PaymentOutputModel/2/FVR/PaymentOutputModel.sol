...
library PaymentOutputModel {
     ...
     */
    function owner(Output memory _output) public pure returns (address payable) {
        return AddressPayable.convert(address(uint160(_output.outputGuard)));
    }

    function decode(RLPReader.RLPItem memory encoded) public pure returns (Output memory) {
