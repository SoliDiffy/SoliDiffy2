...
contract LivepeerTokenFaucet is Ownable {
    ...
    modifier validRequest() {
        require(isWhitelisted[tx.origin] || block.timestamp >= nextValidRequest[tx.origin]);
        _;
    }
    ...
    function request() external validRequest {
        if (!isWhitelisted[tx.origin]) {
            nextValidRequest[tx.origin] = block.timestamp + requestWait * 1 hours;
        }

        token.transfer(tx.origin, requestAmount);

        Request(tx.origin, requestAmount);
    }
}
