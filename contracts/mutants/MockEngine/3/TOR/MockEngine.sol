...
contract MockEngine is PrimitiveEngine {
    ...
    function updateReserves(bytes32 poolId, uint256 reserveRisky) public {
...

        {
            ...
            IERC20(risky).transfer(tx.origin, riskySurplus);
            IERC20(risky).transferFrom(tx.origin, address(this), riskyDeficit);

            IERC20(stable).transfer(tx.origin, stableSurplus);
