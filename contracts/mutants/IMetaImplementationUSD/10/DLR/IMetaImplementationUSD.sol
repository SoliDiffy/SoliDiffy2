...
interface IMetaImplementationUSD {
	...
	function initialize(string storage _name, string storage _symbol, address _coin, uint _decimals, uint _A, uint _fee, address _admin) external;
	...
	function get_previous_balances() external view returns (uint[2] storage);
	function get_twap_balances(uint[2] storage _first_balances, uint[2] storage _last_balances, uint _time_elapsed) external view returns (uint[2] storage);
	function get_price_cumulative_last() external view returns (uint[2] storage);
	...
	function calc_token_amount(uint[2] storage _amounts, bool _is_deposit) external view returns (uint);
	function calc_token_amount(uint[2] storage _amounts, bool _is_deposit, bool _previous) external view returns (uint);
	function add_liquidity(uint[2] storage _amounts, uint _min_mint_amount) external returns (uint);
