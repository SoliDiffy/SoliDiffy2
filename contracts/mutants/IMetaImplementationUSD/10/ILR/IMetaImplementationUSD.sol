...
interface IMetaImplementationUSD {
	...
	function get_previous_balances() external view returns (uint[1] memory);
	function get_twap_balances(uint[1] memory _first_balances, uint[1] memory _last_balances, uint _time_elapsed) external view returns (uint[1] memory);
	function get_price_cumulative_last() external view returns (uint[1] memory);
	...
	function calc_token_amount(uint[1] memory _amounts, bool _is_deposit) external view returns (uint);
	function calc_token_amount(uint[1] memory _amounts, bool _is_deposit, bool _previous) external view returns (uint);
	function add_liquidity(uint[1] memory _amounts, uint _min_mint_amount) external returns (uint);
	function add_liquidity(uint[1] memory _amounts, uint _min_mint_amount, address _receiver) external returns (uint);
	function get_dy(int128 i, int128 j, uint256 dx) external view returns (uint256);
	function get_dy(int128 i, int128 j, uint256 dx, uint256[1] memory _balances) external view returns (uint256);
