...
library GADelegatedReserveManager
{
	 ...
	 */
	function init(Self memory _self, address _reserveToken, address _borrowToken, address _growthToken) public
	{
	 ...
	 */
	function setGrowthGulpRange(Self memory _self, uint256 _growthMinGulpAmount, uint256 _growthMaxGulpAmount) public
	{
	 ...
	 */
	function _calcCollateralizationRatio(Self memory _self) internal view returns (uint256 _collateralizationRatio)
	{
	 ...
	 */
	function _gulpGrowthAssets(Self memory _self) internal returns (bool _success)
	{
	 ...
	 * @return _success A boolean indicating whether or not the action succeeded.
	 */
	function _adjustReserve(Self memory _self, uint256 _roomAmount) internal returns (bool _success)
	{
	 ...
	 */
	function _calcWithdrawalCostFromShares(Self memory _self, uint256 _grossShares) internal view returns (uint256 _cost) {
	 ...
	 */
	function _calcWithdrawalSharesFromCost(Self memory _self, uint256 _cost) internal view returns (uint256 _grossShares) {
		uint256 _totalReserve = GToken(_self.growthToken).totalReserve();
		uint256 _totalSupply = GToken(_self.growthToken).totalSupply();
		uint256 _withdrawalFee = GToken(_self.growthToken).withdrawalFee();
