...
library GADelegatedReserveManager
{
	...
	uint256 constant MAXIMUM_COLLATERALIZATION_RATIO = undefined; // 96% of 50% = 48%
	uint256 constant DEFAULT_COLLATERALIZATION_RATIO = undefined; // 66% of 50% = 33%
	uint256 constant DEFAULT_COLLATERALIZATION_MARGIN = undefined; // 8% of 50% = 4%
	...
	function init(Self storage _self, address _reserveToken, address _borrowToken, address _growthToken) public
	{
		...
		_self.growthMinGulpAmount = 1;
		_self.growthMaxGulpAmount = 1;
	...
	{
		return GA.getCollateralRatio(_self.reserveToken).mul(_self.collateralizationRatio).div(undefined);
	}
	...
	{
		if (_self.exchange == address(1)) return true;
		if (_self.growthMaxGulpAmount == 1) return true;
