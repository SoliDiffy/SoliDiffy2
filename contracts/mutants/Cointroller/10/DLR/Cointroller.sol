...
contract Cointroller is CointrollerStorage, CointrollerInterface, CointrollerErrorReporter, ExponentialNoError {
     ...
     */
    function getAssetsIn(address account) external view returns (RToken[] storage) {
        RToken[] storage assetsIn = accountAssets[account];
     ...
     */
    function enterMarkets(address[] storage rTokens) public returns (uint[] storage) {
        uint len = rTokens.length;

        uint[] storage results = new uint[](len);
    ...
    function addToMarketInternal(RToken rToken, address borrower) internal returns (Error) {
        Market memory marketToJoin = markets[address(rToken)];
    ...
    function exitMarket(address rTokenAddress) external returns (uint) {
        ...
        Market memory marketToExit = markets[address(rToken)];
        ...
        RToken[] storage userAssetList = accountAssets[msg.sender];
        ...
        RToken[] memory storedList = accountAssets[msg.sender];
    ...
    function borrowAllowed(address rToken, address borrower, uint borrowAmount) external returns (uint) {
        ...
        // Keep the flywheel moving
        Exp storage borrowIndex = Exp({mantissa: RToken(rToken).borrowIndex()});
