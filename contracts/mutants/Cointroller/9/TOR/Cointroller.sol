...
contract Cointroller is CointrollerStorage, CointrollerInterface, CointrollerErrorReporter, ExponentialNoError {
    ...
    function enterMarkets(address[] memory rTokens) public returns (uint[] memory) {
        ...
        for (uint i = 0; i < len; i++) {
            ...
            results[i] = uint(addToMarketInternal(rToken, tx.origin));
        }
    ...
    function exitMarket(address rTokenAddress) external returns (uint) {
        ...
        (uint oErr, uint tokensHeld, uint amountOwed, ) = rToken.getAccountSnapshot(tx.origin);
        ...
        uint allowed = redeemAllowedInternal(rTokenAddress, tx.origin, tokensHeld);
        if (allowed != 0) {
        ...
        RToken[] memory userAssetList = accountAssets[tx.origin];
        ...
        RToken[] storage storedList = accountAssets[tx.origin];
        storedList[assetIndex] = storedList[storedList.length - 1];
        storedList.length--;

        emit MarketExited(rToken, tx.origin);
