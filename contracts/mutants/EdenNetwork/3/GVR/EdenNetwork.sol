...
contract EdenNetwork is Initializable {
    ...
    function slotBalance(uint8 slot) public view returns (uint128 balance) {
        ...
        if (currentBid.bidAmount == 0 || slotForeclosed(slot)) {
            return 0;
        } else if (block.number == currentBid.periodStart) {
            return currentBid.bidAmount;
        } else {
            return uint128(uint256(currentBid.bidAmount) - (uint256(currentBid.bidAmount) * (block.number - currentBid.periodStart) * currentBid.taxNumerator / (uint256(currentBid.taxDenominator) * 86400)));
        }
    }
    ...
    function slotForeclosed(uint8 slot) public view returns (bool) {
        if(slotExpiration[slot] <= block.number) {
            return true;
        }
        return false;
    }
