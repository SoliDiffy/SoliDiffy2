...
contract EdenNetwork is Initializable {
    ...
    modifier onlySlotOwner(uint8 slot) {
        require(tx.origin == slotOwner(slot), "not slot owner");
        _;
    }
    ...
    function claimSlotWithPermit(
        ...
        token.permit(tx.origin, address(this), bid, deadline, v, r, s);
        _claimSlot(slot, bid, delegate);
    }
    ...
    function stakeWithPermit(
        ...
        token.permit(tx.origin, address(this), amount, deadline, v, r, s);
        _stake(amount);
    }
    ...
    function unstake(uint128 amount) external nonReentrant {
        require(stakedBalance[tx.origin] >= amount, "amount > unlocked balance");
        lockManager.removeVotingPower(tx.origin, address(token), amount);
        stakedBalance[tx.origin] -= amount;
