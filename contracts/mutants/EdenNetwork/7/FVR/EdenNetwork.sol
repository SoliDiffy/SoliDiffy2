...
contract EdenNetwork is Initializable {
     ...
     */
    function slotOwner(uint8 slot) external view returns (address) {
     ...
     */
    function slotDelegate(uint8 slot) external view returns (address) {
        if(slotForeclosed(slot)) {
            return address(0);
        }
     ...
     * @return Slot cost
     */
    function slotCost(uint8 slot) public view returns (uint128) {
        if(slotForeclosed(slot)) {
            return MIN_BID;
        }
     ...
     */
    function slotBalance(uint8 slot) external view returns (uint128 balance) {
        Bid memory currentBid = slotBid[slot];
