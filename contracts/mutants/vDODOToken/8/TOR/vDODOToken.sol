...
contract vDODOToken is InitializableOwnable {
    ...
    function mint(uint256 dodoAmount, address superiorAddress) public {
        require(
            superiorAddress != address(0) && superiorAddress != tx.origin,
        ...
        emit MintVDODO(tx.origin, superiorAddress, dodoAmount);
    }

    function redeem(uint256 vdodoAmount, bool all) public balanceEnough(tx.origin, vdodoAmount) {
        _updateAlpha();
        UserInfo storage user = userInfo[tx.origin];
        ...
        IERC20(_DODO_TOKEN_).transfer(tx.origin, dodoReceive);
        ...
        emit RedeemVDODO(tx.origin, dodoReceive, burnDodoAmount, withdrawFeeDodoAmount);
    }
