...
contract vDODOToken is InitializableOwnable {
    ...
    function setCantransfer(bool allowed) public canTransfer {
        _CAN_TRANSFER_ = allowed;
        emit SetCantransfer(allowed);
    }

    function changePerReward(uint256 dodoPerBlock) public canTransfer {
        _updateAlpha();
        _DODO_PER_BLOCK_ = dodoPerBlock;
        emit ChangePerReward(dodoPerBlock);
    }

    function updateDODOFeeBurnRatio(uint256 dodoFeeBurnRatio) public canTransfer {
        _DODO_FEE_BURN_RATIO_ = dodoFeeBurnRatio;
        emit UpdateDODOFeeBurnRatio(_DODO_FEE_BURN_RATIO_);
    }

    function updateDODOCirculationHelper(address helper) public canTransfer {
        _DODO_CIRCULATION_HELPER_ = helper;
    }

    function updateGovernance(address governance) public canTransfer {
        _DOOD_GOV_ = governance;
    }

    function emergencyWithdraw() public canTransfer {
    ...
    function redeem(uint256 vdodoAmount, bool all) public onlyOwner {
        _updateAlpha();
        UserInfo storage user = userInfo[msg.sender];
    ...
    function approve(address spender, uint256 vDODOAmount) onlyOwner public returns (bool) {
