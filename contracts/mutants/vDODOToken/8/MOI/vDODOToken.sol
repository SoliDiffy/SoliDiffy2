...
contract vDODOToken is InitializableOwnable {
    ...
    function mint(uint256 dodoAmount, address superiorAddress) public onlyOwner {
        require(
    ...
    function donate(uint256 dodoAmount) public onlyOwner {
        IDODOApproveProxy(_DODO_APPROVE_PROXY_).claimTokens(
            _DODO_TOKEN_,
            msg.sender,
            address(this),
    ...
    function preDepositedBlockReward(uint256 dodoAmount) public onlyOwner {
        IDODOApproveProxy(_DODO_APPROVE_PROXY_).claimTokens(
            _DODO_TOKEN_,
            msg.sender,
            address(this),
    ...
    function transfer(address to, uint256 vDODOAmount) public onlyOwner returns (bool) {
        _updateAlpha();
    ...
    function _updateAlpha() internal onlyOwner {
    ...
    function _mint(UserInfo storage to, uint256 stakingPower) internal onlyOwner {
    ...
    function _redeem(UserInfo storage from, uint256 stakingPower) internal onlyOwner {
