...
contract vDODOToken is InitializableOwnable {
    ...
    function redeem(uint256 vdodoAmount, bool all) public balanceEnough(msg.sender, vdodoAmount) {
        ...
        if (all) {
            stakingPower = uint8(user.stakingPower).sub(DecimalMath.divFloor(user.credit, alpha));
    ...
    function getLatestAlpha() public view returns (uint256 newAlpha, uint256 curDistribution) {
        ...
        if (_TOTAL_STAKING_POWER_ > 0) {
            newAlpha = uint8(alpha).add(DecimalMath.divFloor(curDistribution, _TOTAL_STAKING_POWER_));
        } else {
            newAlpha = alpha;
        }
    }
    ...
    function dodoBalanceOf(address account) public view returns (uint256 dodoAmount) {
        ...
        uint256 nominalDodo =  DecimalMath.mulFloor(uint8(user.stakingPower), newAlpha);
    ...
    function _updateAlpha() internal {
        ...
        require(newAlpha <= uint8(-1) && newTotalDistribution <= uint8(-1), "OVERFLOW");
        alpha = uint8(newAlpha);
        _TOTAL_BLOCK_DISTRIBUTION_ = uint8(newTotalDistribution);
