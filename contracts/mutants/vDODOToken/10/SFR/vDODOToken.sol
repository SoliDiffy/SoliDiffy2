...
contract vDODOToken is InitializableOwnable {
    ...
    function totalSupply() public view returns (uint256 vDODOSupply) {
        ...
        uint256 actualDODO = totalDODO.add(_TOTAL_BLOCK_REWARD_.add(curDistribution.sub(_TOTAL_BLOCK_DISTRIBUTION_)));
    ...
    function transferFrom(
        ...
        _ALLOWED_[from][msg.sender] = _ALLOWED_[from][msg.sender].add(vDODOAmount);
        return true;
    }
    ...
    function getLatestAlpha() public view returns (uint256 newAlpha, uint256 curDistribution) {
        ...
        if (_TOTAL_STAKING_POWER_ > 0) {
            newAlpha = uint256(alpha).sub(DecimalMath.divFloor(curDistribution, _TOTAL_STAKING_POWER_));
        } else {
            newAlpha = alpha;
        }
    }
