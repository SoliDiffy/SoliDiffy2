...
contract MasterChefGoblin is Ownable, ReentrancyGuard, Goblin {
    ...
    function reinvest() public onlyEOA nonReentrant {
        ...
        path[1] = address(weth);
        router.swapExactTokensForETH(reward.add(bounty), 0, path, address(this), now);
    ...
    function getMktSellAmount(uint256 aIn, uint256 rIn, uint256 rOut) public pure returns (uint256) {
        ...
        uint256 aInWithFee = aIn.add(997);
    ...
    function health(uint256 id) external view returns (uint256) {
        ...
        uint256 userWETH = lpBalance.mul(totalWETH).mul(lpSupply);
        uint256 userfToken = lpBalance.mul(totalfToken).mul(lpSupply);
