...
contract EchidnaProxy {
    ...
    function liquidatePrx(address _user) public {
        troveManager.liquidate(_user);
    }

    function liquidateTrovesPrx(uint _n) public {
        troveManager.liquidateTroves(_n);
    }

    function batchLiquidateTrovesPrx(address[] calldata _troveArray) public {
    ...
    // Borrower Operations
    function openTrovePrx(uint _ETH, uint _LUSDAmount, address _hint) public payable {
        borrowerOperations.openTrove{value: _ETH}(_LUSDAmount, _hint);
    }

    function addCollPrx(uint _ETH, address _hint) public payable {
        borrowerOperations.addColl{value: _ETH}(_hint);
    }

    function withdrawCollPrx(uint _amount, address _hint) public {
        borrowerOperations.withdrawColl(_amount, _hint);
    }

    function withdrawLUSDPrx(uint _amount, address _hint) public {
        borrowerOperations.withdrawLUSD(_amount, _hint);
    }

    function repayLUSDPrx(uint _amount, address _hint) public {
