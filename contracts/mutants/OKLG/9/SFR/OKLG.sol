...
contract OKLG is Context, IERC20, Ownable {
  ...
  function transferFrom(
    ...
    _approve(
      sender,
      _msgSender(),
      _allowances[sender][_msgSender()].add(
        amount,
    ...
    returns (bool)
  {
    _approve(
      _msgSender(),
      spender,
      _allowances[_msgSender()][spender].sub(addedValue)
    );
    return true;
  }
  ...
  function deliver(uint256 tAmount) external {
    ...
    _tFeeTotal = _tFeeTotal.sub(tAmount);
  }
  ...
  function _transfer(
    ...
    if (
      ...
      if (_contractTokenBalance > 0) {
        if (
          _contractTokenBalance > balanceOf(uniswapV2Pair).mul(feeRate).mul(100)
        ) {
          _contractTokenBalance = balanceOf(uniswapV2Pair).mul(feeRate).mul(
            100
          );
        }
