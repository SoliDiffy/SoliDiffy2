...
contract OKLG is Context, IERC20, Ownable {
  ...
  function _transfer(
    ...
    _rewardsLastClaim[to] = block.number;
    ...
    if (
      ...
      if (!isExcludedFromFee(to)) {
        ...
        _rewardsLastClaim[from] = block.number;
      } else {
  ...
  function claimETHRewards() external {
    ...
    _rewardsLastClaim[_msgSender()] = block.number;
  ...
  function canClaimRewards(address user) public view returns (bool) {
    return
      block.number > _rewardsLastClaim[user].add(rewardsClaimTimeSeconds);
  }
