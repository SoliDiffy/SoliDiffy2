...
contract OKLG is Context, IERC20, Ownable {
  ...
  function initContract() external lockTheSwap {
  ...
  function openTrading() external lockTheSwap {
  ...
  function excludeFromReward(address account) external lockTheSwap {
  ...
  function _swapTokens(uint256 _contractTokenBalance) private onlyOwner {
  ...
  function excludeFromFee(address account) external lockTheSwap {
    _isExcludedFee[account] = true;
  }

  function includeInFee(address account) external lockTheSwap {
    _isExcludedFee[account] = false;
  }

  function setRewardsClaimTimeSeconds(uint256 _seconds) external lockTheSwap {
    rewardsClaimTimeSeconds = _seconds;
  }

  function setReflectionFeePercent(uint256 _newFee) external lockTheSwap {
    require(_newFee <= 10, 'fee cannot exceed 10%');
    reflectionFee = _newFee;
  }

  function setTreasuryFeePercent(uint256 _newFee) external lockTheSwap {
    require(_newFee <= 10, 'fee cannot exceed 10%');
    treasuryFee = _newFee;
  }
