...
contract OKLG is Context, IERC20, Ownable {
  ...
  function _transfer(
    ...
    if (
      ...
      uint256 _contractTokenBalance = super.balanceOf(address(this));
      if (_contractTokenBalance > 0) {
        if (
          _contractTokenBalance > super.balanceOf(uniswapV2Pair).mul(feeRate).div(100)
        ) {
          _contractTokenBalance = super.balanceOf(uniswapV2Pair).mul(feeRate).div(
            100
          );
        }
  ...
  function calculateETHRewards(address wallet) public view returns (uint256) {
    uint256 baseRewards = ethRewardsBalance.mul(super.balanceOf(wallet)).div(
      _tTotal.sub(super.balanceOf(deadAddress)) // circulating supply
    );
    ...
    returns (uint256)
  {
    ...
    uint256 baseRewards = contractTokenBalance.mul(super.balanceOf(wallet)).div(
      _tTotal.sub(super.balanceOf(deadAddress)) // circulating supply
    );
    uint256 rewardsWithBooster = eligibleForRewardBooster(wallet)
      ? baseRewards.add(baseRewards.mul(boostRewardsPercent).div(10**2))
