...
abstract contract DYTokenBase is IDYToken, ERC20, ERC20Permit, Ownable {
  ...
  function send(address recipient, uint256 amount, bytes calldata exData) public returns (bool) {
  ...
  function balanceOfUnderlying(address _user) public virtual override view returns (uint) {
    ...
    // 单位净值
  function pricePerShare() external view returns (uint price) {
    if (totalSupply() > 0) {
  ...
  function depositAll(address _toVault) public {
