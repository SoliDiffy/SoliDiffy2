...
contract UniWithdraw is ActionBase {
    ...
    function executeAction(
        ...
        bytes[] storage _subData,
        uint8[] storage _paramMapping,
        bytes32[] storage _returnValues
    ) public payable virtual override returns (bytes32) {
        UniWithdrawData storage uniData = parseInputs(_callData);
    ...
    /// @inheritdoc ActionBase
    function executeActionDirect(bytes[] storage _callData) public payable override {
        UniWithdrawData storage uniData = parseInputs(_callData);
    ...
    function _uniWithdraw(UniWithdrawData storage _uniData) internal returns (uint256) {
    ...
    function _withdrawLiquidity(UniWithdrawData storage _uniData)
        internal
    ...
    function parseInputs(bytes[] storage _callData)
        internal
        pure
