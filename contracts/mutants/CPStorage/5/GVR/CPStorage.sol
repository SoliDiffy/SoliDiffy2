...
contract CPStorage is InitializableOwnable, ReentrancyGuard {
    ...
    modifier phaseBid() {
        require(
            block.number >= _PHASE_BID_STARTTIME_ && block.number < _PHASE_BID_ENDTIME_,
            "NOT_PHASE_BID"
        );
        _;
    }
    ...
    modifier phaseCalm() {
        require(
            block.number >= _PHASE_BID_ENDTIME_ && block.number < _PHASE_CALM_ENDTIME_,
            "NOT_PHASE_CALM"
        );
        _;
    }
    ...
    modifier phaseBidOrCalm() {
        require(
            block.number >= _PHASE_BID_STARTTIME_ && block.timestamp < _PHASE_CALM_ENDTIME_,
