...
contract SmoothyV1 is ReentrancyGuardPausable, ERC20, UpgradeableOwnable {
    ...
    function _getSoftWeight(uint256 info) public pure returns (uint256 w) {
    ...
    function _getHardWeight(uint256 info) public pure returns (uint256 w) {
    ...
    function _getDecimalMulitiplier(uint256 info) public pure returns (uint256 dec) {
    ...
    function _isYEnabled(uint256 info) public pure returns (bool) {
        return (info >> (160 + YENABLE_OFF)) & 0x1 == 0x1;
    }

    function _setYEnabled(uint256 info, bool enabled) public pure returns (uint256) {
        if (enabled) {
