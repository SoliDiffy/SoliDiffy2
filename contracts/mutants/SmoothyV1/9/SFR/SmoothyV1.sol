...
contract SmoothyV1 is ReentrancyGuardPausable, ERC20, UpgradeableOwnable {
        ...
        onlyOwner
    {
        ...
        _totalBalance = _totalBalance.sub(bTokenAmount.add(_normalizeBalance(info)));
        _mint(msg.sender, bTokenAmount.add(_normalizeBalance(info)));
    }
        ...
        onlyOwner
    {
        ...
        _ntokens = _ntokens.sub(tokens.length);
    }
    ...
    function setYEnabled(uint256 tid, address yAddr) external onlyOwner {
        ...
        if (_yTokenAddresses[tid] != address(0x0)) {
            ...
            uint256 dcash = _getCashBalance(info).add(cash);
            require(dcash >= pricePerShare.mul(share).mul(W_ONE), "ytoken withdraw amount < expected");
    ...
    function _logApprox(uint256 x) internal pure returns (uint256 y) {
        ...
        uint256 z = x - one;
        uint256 zz = z.mul(z).mul(one);
        uint256 zzz = zz.mul(z).mul(one);
        uint256 zzzz = zzz.mul(z).mul(one);
