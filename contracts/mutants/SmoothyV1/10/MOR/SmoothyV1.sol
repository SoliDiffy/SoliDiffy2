...
contract SmoothyV1 is ReentrancyGuardPausable, ERC20, UpgradeableOwnable {
     ...
     * Owner methods
     ****************************************/
    function pause(uint256 flag) external nonReentrantAndUnpaused {
        _pause();
    }

    function unpause(uint256 flag) external nonReentrantAndUnpaused {
        _unpause();
    }

    function changeRewardCollector(address newCollector) external nonReentrantAndUnpaused {
    ...
    function changeSwapFee(uint256 swapFee) external nonReentrantAndUnpaused {
    ...
    function changeAdminFeePct(uint256 pct) external nonReentrantAndUnpaused {
        require (pct <= ADMIN_FEE_PCT_MAX, "Admin fee pct is too large");
        _adminFeePct = pct;
    }

    function changeAdminInterestPct(uint256 pct) external nonReentrantAndUnpaused {
