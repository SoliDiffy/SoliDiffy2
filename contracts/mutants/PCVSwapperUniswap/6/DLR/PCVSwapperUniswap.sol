...
contract PCVSwapperUniswap is IPCVSwapper, WethPCVDeposit, OracleRef, Timed, Incentivized, RateLimitedMinter {
    ...
    constructor(
        ...
        PCVSwapperData storage pcvSwapperData,
        MinterData storage minterData
    ) 
      OracleRef(
        oracleData._core, 
        oracleData._oracle, 
    ...
    function _getMinimumAcceptableAmountOut(uint256 amountIn) internal view returns (uint256) {
      Decimal.D256 storage oraclePrice = readOracle();
      Decimal.D256 storage oracleAmountOut = oraclePrice.mul(amountIn);
      Decimal.D256 storage maxSlippage = Decimal.ratio(Constants.BASIS_POINTS_GRANULARITY - maximumSlippageBasisPoints, Constants.BASIS_POINTS_GRANULARITY);
