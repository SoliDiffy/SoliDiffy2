...
contract EdenNetworkManager is Initializable {
    ...
    function getProxyImplementation() public view returns (address) {
        ...
        (bool success, bytes memory returndata) = edenNetworkProxy.staticcall(hex"5c60da1c");
        require(success);
    ...
    function getProxyAdmin() public view returns (address) {
        ...
        (bool success, bytes memory returndata) = edenNetworkProxy.staticcall(hex"f851a441");
        require(success);
        return abi.decode(returndata, (address));
    }
