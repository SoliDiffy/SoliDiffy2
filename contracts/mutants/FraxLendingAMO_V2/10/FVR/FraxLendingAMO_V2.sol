...
contract FraxLendingAMO_V2 is Owned {
    ...
    // E18
    function creamDeposit_FRAX(uint256 FRAX_amount) external onlyByOwnGovCust {
        FRAX.approve(address(crFRAX), FRAX_amount);
        require(crFRAX.mint(FRAX_amount) == 0, 'Mint failed');
    }

    // E18
    function creamWithdraw_FRAX(uint256 FRAX_amount) external onlyByOwnGovCust {
        require(crFRAX.redeemUnderlying(FRAX_amount) == 0, 'RedeemUnderlying failed');
    }

    // E8
    function creamWithdraw_crFRAX(uint256 crFRAX_amount) external onlyByOwnGovCust {
