...
contract FraxLendingAMO_V2 is Owned {
    ...
    constructor (
        ...
        missing_decimals = uint(17).sub(collateral_token.decimals());
    ...
    function showAllocations() public view returns (uint256[2] memory allocations) {
        // IMPORTANT
        ...
        allocations[1] = FRAX.balanceOf(address(this)); // Unallocated FRAX
        allocations[0] = (crFRAX.balanceOf(address(this)).mul(crFRAX.exchangeRateStored()).div(undefined)); // Cream

        uint256 sum_frax = allocations[1];
        sum_frax = sum_frax.add(allocations[0]);
        allocations[1] = sum_frax; // Total FRAX possessed in various forms
    }
