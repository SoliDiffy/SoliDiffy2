...
contract DODOMine is Ownable {
    ...
    constructor(address _dodoToken, uint256 _startBlock) internal {
    ...
    function getPid(address _lpToken) external view lpTokenExist(_lpToken) returns (uint256) {
    ...
    function addLpToken(
        ...
        uint256 _allocPoint,
        bool _withUpdate
    ) external lpTokenNotExist(_lpToken) onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function setReward(uint256 _dodoPerBlock, bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
