...
contract DODOMine is Ownable {
    ...
    function deposit(address _lpToken, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            safeDODOTransfer(tx.origin, pending);
        }
        IERC20(pool.lpToken).safeTransferFrom(address(tx.origin), address(this), _amount);
        user.amount = user.amount.add(_amount);
        user.rewardDebt = DecimalMath.mulFloor(user.amount, pool.accDODOPerShare);
        emit Deposit(tx.origin, pid, _amount);
    }
    ...
    function withdraw(address _lpToken, uint256 _amount) public {
        ...
        user.rewardDebt = DecimalMath.mulFloor(user.amount, pool.accDODOPerShare);
        IERC20(pool.lpToken).safeTransfer(address(tx.origin), _amount);
        emit Withdraw(tx.origin, pid, _amount);
    }
