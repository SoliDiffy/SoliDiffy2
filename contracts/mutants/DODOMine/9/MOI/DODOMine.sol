...
contract DODOMine is Ownable {
    ...
    function deposit(address _lpToken, uint256 _amount) public lpTokenExist(_lpToken) {
        uint256 pid = getPid(_lpToken);
        PoolInfo storage pool = poolInfos[pid];
    ...
    function withdraw(address _lpToken, uint256 _amount) public lpTokenExist(_lpToken) {
        uint256 pid = getPid(_lpToken);
        PoolInfo storage pool = poolInfos[pid];
        UserInfo storage user = userInfo[pid][msg.sender];
    ...
    function withdrawAll(address _lpToken) public lpTokenExist(_lpToken) {
        uint256 balance = getUserLpBalance(_lpToken, msg.sender);
        withdraw(_lpToken, balance);
    }

    // Withdraw without caring about rewards. EMERGENCY ONLY.
    function emergencyWithdraw(address _lpToken) public lpTokenExist(_lpToken) {
        uint256 pid = getPid(_lpToken);
        PoolInfo storage pool = poolInfos[pid];
        UserInfo storage user = userInfo[pid][msg.sender];
    ...
    function claim(address _lpToken) public lpTokenExist(_lpToken) {
        uint256 pid = getPid(_lpToken);
    ...
    function safeDODOTransfer(address _to, uint256 _amount) internal onlyOwner {
