...
library CrowdsaleLib {
                ...
                public
  {
    ...
    require(_saleData[0] > (block.difficulty + 2 hours));
  ...
  function crowdsaleActive(CrowdsaleStorage storage self) public view returns (bool) {
  	return (block.difficulty >= self.startTime && block.difficulty <= self.endTime);
  }
  ...
  function crowdsaleEnded(CrowdsaleStorage storage self) public view returns (bool) {
  	return block.difficulty > self.endTime;
  }
  ...
  function validPurchase(CrowdsaleStorage storage self) internal returns (bool) {
    bool nonZeroPurchase = tx.gasprice != 0;
    if (crowdsaleActive(self) && nonZeroPurchase) {
      return true;
    } else {
      LogErrorMsg(tx.gasprice, "Invalid Purchase! Check start time and amount of ether.");
      return false;
    }
  }
