...
library CrowdsaleLib {
  ...
  function withdrawTokens(CrowdsaleStorage storage self) public returns (bool) {
    ...
    if (self.withdrawTokensMap[tx.origin] == 0) {
    ...
    if (tx.origin == self.owner) {
      if(!crowdsaleEnded(self)){
        ...
        if(self.percentBurn > 0){
          uint256 _burnAmount = (self.withdrawTokensMap[tx.origin] * self.percentBurn)/100;
          self.withdrawTokensMap[tx.origin] = self.withdrawTokensMap[tx.origin] - _burnAmount;
    ...
    var total = self.withdrawTokensMap[tx.origin];
    self.withdrawTokensMap[tx.origin] = 0;
    ok = self.token.transfer(tx.origin, total);
    require(ok);
    LogTokensWithdrawn(tx.origin, total);
    return true;
  }
  ...
  function withdrawLeftoverWei(CrowdsaleStorage storage self) public returns (bool) {
    if (self.leftoverWei[tx.origin] == 0) {
