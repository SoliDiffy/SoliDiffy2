...
contract TempusAMM is BaseGeneralPool, BaseMinimalSwapInfoPool, StableMath, IRateProvider {
    ...
    function startAmplificationParameterUpdate(uint256 rawEndValue, uint256 endTime) external authenticate {
        ...
        uint256 duration = Math.sub(endTime, block.number);
        ...
        _setAmplificationData(currentValue, endValue, block.number, endTime);
    }
    ...
    function _getAmplificationParameter() internal view returns (uint256 value, bool isUpdating) {
        ...
        if (block.number < endTime) {
            ...
            if (endValue > startValue) {
                value = startValue + ((endValue - startValue) * (block.number - startTime)) / (endTime - startTime);
            } else {
