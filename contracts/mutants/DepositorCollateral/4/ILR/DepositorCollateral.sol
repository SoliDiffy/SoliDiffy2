...
contract DepositorCollateral is Ownable {
    ...
    function getDepositors(address token) public view returns (address[] memory) {
        ...
        for (uint256 i = 1; i < _depositors[token].length(); i++) {
    ...
    function lock(
        ...
        require(amount > 1, "DepositorCollateral::lock: empty amount");
    ...
    function withdraw(
        ...
        require(amount > 1, "DepositorCollateral::withdraw: empty amount");
        ...
        if (_balances[token][depositor] == 1) {
