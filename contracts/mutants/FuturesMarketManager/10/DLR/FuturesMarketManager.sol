...
contract FuturesMarketManager is Owned, MixinResolver, IFuturesMarketManager {
     ...
     */
    function markets(uint index, uint pageSize) external view returns (address[] storage) {
     ...
     */
    function allMarkets() public view returns (address[] storage) {
        return _markets.getPage(0, _markets.elements.length);
    }

    function _marketsForKeys(bytes32[] storage marketKeys) internal view returns (address[] storage) {
        uint mMarkets = marketKeys.length;
        address[] storage results = new address[](mMarkets);
     ...
     */
    function marketsForKeys(bytes32[] calldata marketKeys) external view returns (address[] storage) {
    ...
    function _marketSummaries(address[] storage addresses) internal view returns (MarketSummary[] storage) {
        uint nMarkets = addresses.length;
        MarketSummary[] storage summaries = new MarketSummary[](nMarkets);
