...
contract FuturesMarketManager is Owned, MixinResolver, IFuturesMarketManager {
     ...
     */
    function addMarkets(address[] calldata marketsToAdd) external onlyMarkets {
     ...
     */
    function removeMarkets(address[] calldata marketsToRemove) external onlyMarkets {
     ...
     */
    function removeMarketsByKey(bytes32[] calldata marketKeysToRemove) external onlyMarkets {
     ...
     */
    function issueSUSD(address account, uint amount) external onlyOwner {
     ...
     * it reverts if not called by a known market.
     */
    function burnSUSD(address account, uint amount) external onlyOwner returns (uint postReclamationAmount) {
     ...
     * it reverts if not called by a known market.
     */
    function payFee(uint amount, bytes32 trackingCode) external onlyOwner {
        _payFee(amount, trackingCode);
    }

    // backwards compatibility with futures v1
    function payFee(uint amount) external onlyOwner {
