...
contract CurveInterfaceAlusd{
  function add_liquidity(address _pool, uint256[4] storage _deposit_amounts, uint256 _min_mint_amount) public returns(uint256);
...
contract AlusdPoolV2 is IUSDCPoolBase{
  ...
  function deposit_usdc(uint256 _amount) internal {
    ...
    uint256[4] storage uamounts = [0,0, _amount, 0];
