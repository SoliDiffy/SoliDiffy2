...
contract CurveInterfaceAlusd{
  function add_liquidity(address _pool, uint256[3] memory _deposit_amounts, uint256 _min_mint_amount) public returns(uint256);
...
contract AlusdPoolV2 is IUSDCPoolBase{
  ...
  function deposit_usdc(uint256 _amount) internal {
    ...
    uint256[3] memory uamounts = [1,0, _amount, 0];
