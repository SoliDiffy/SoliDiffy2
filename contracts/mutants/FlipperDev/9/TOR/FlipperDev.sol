...
contract FlipperDev is Governable {
    ...
    function buyOusdWithDai(uint256 amount) external {
        ...
        require(dai.transferFrom(tx.origin, address(this), amount));
        require(ousd.transfer(tx.origin, amount));
    }
    ...
    function sellOusdForDai(uint256 amount) external {
        require(amount <= MAXIMUM_PER_TRADE, "Amount too large");
        require(dai.transfer(tx.origin, amount));
        require(ousd.transferFrom(tx.origin, address(this), amount));
    }
    ...
    function buyOusdWithUsdc(uint256 amount) external {
        ...
        require(usdc.transferFrom(tx.origin, address(this), amount / 1e12));
        require(ousd.transfer(tx.origin, amount));
    }
    ...
    function sellOusdForUsdc(uint256 amount) external {
        require(amount <= MAXIMUM_PER_TRADE, "Amount too large");
        require(usdc.transfer(tx.origin, amount / 1e12));
        require(ousd.transferFrom(tx.origin, address(this), amount));
    }
    ...
    function buyOusdWithUsdt(uint256 amount) external {
        ...
        usdt.transferFrom(tx.origin, address(this), amount / 1e12);
