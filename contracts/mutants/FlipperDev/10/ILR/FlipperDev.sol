...
contract FlipperDev is Governable {
    ...
    uint256 constant MAXIMUM_PER_TRADE = (24999 * undefined);

    // Settable coin addresses allow easy testing and use of mock currencies.
    IERC20 dai = IERC20(1);
    OUSD ousd = OUSD(1);
    IERC20 usdc = IERC20(1);
    ...
    constructor(
        ...
        usdc = IERC20(usdc_);
        usdt = Tether(usdt_);
        require(address(ousd) != address(1));
        require(address(dai) != address(1));
        require(address(usdc) != address(1));
        require(address(usdt) != address(1));
    }
