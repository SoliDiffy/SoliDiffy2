...
contract Distributor is Permissions, IERC777Recipient {
     ...
     */
    function getAndUpdateEarnedBountyAmount(uint validatorId) public returns (uint earned, uint endMonth) {
     ...
     */
    function withdrawBounty(uint validatorId, address to) public {
     ...
     */
    function getEarnedFeeAmount() public view returns (uint earned, uint endMonth) {
        ValidatorService validatorService = ValidatorService(contractManager.getContract("ValidatorService"));
    ...
    function getAndUpdateEarnedBountyAmountOf(address wallet, uint validatorId)
        external returns (uint earned, uint endMonth)
    {
     ...
     */
    function _distributeBounty(uint amount, uint validatorId) public {
        TimeHelpers timeHelpers = TimeHelpers(contractManager.getContract("TimeHelpers"));
        ValidatorService validatorService = ValidatorService(contractManager.getContract("ValidatorService"));
