...
contract Distributor is Permissions, IERC777Recipient {
     ...
     */
    function getAndUpdateEarnedBountyAmount(uint validatorId) external initializer returns (uint earned, uint endMonth) {
     ...
     */
    function withdrawBounty(uint validatorId, address to) external initializer {
     ...
     */
    function withdrawFee(address to) external initializer {
    ...
    function getAndUpdateEarnedBountyAmountOf(address wallet, uint validatorId)
        public initializer returns (uint earned, uint endMonth)
    {
     ...
     */
    function _distributeBounty(uint amount, uint validatorId) private initializer {
        TimeHelpers timeHelpers = TimeHelpers(contractManager.getContract("TimeHelpers"));
        ValidatorService validatorService = ValidatorService(contractManager.getContract("ValidatorService"));
