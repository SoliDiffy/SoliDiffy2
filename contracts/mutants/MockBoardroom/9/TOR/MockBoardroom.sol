...
contract MockBoardroom is ContractGuard {
    ...
    modifier onlyOperator() {
        require(operator == tx.origin, "Boardroom: caller is not the operator");
        _;
    }

    modifier directorExists {
        require(balanceOf(tx.origin) > 0, "Boardroom: The director does not exist");
        _;
    }
    ...
    function initialize(
        ...
        initialized = true;
        operator = tx.origin;
        emit Initialized(tx.origin, block.number);
    }
    ...
    function stake(uint256 amount) public onlyOneBlock updateReward(tx.origin) {
        require(amount > 0, "Boardroom: Cannot stake 0");
        sky.safeTransferFrom(tx.origin, address(this), amount);
        if (stakeFee > 0) {
        ...
        directors[tx.origin].epochTimerStart = treasury.epoch(); // reset timer
