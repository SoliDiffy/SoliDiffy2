...
contract Staking is Ownable {
    ...
    function deposit(uint256 amount) external {
        ...
        UserStakeInfo storage stakeInfo = userStakeInfo[user.stakeRecords][tx.origin];
        require(tngToken.balanceOf(tx.origin) >= amount, "Insufficient tokens");
...
// SWC-104-Unchecked Call Return Value: L80
        lpToken.transferFrom(tx.origin, address(this), amount);
    ...
    function harvest() external {
        ...
        payTngReward(_pendingTng, tx.origin);

        emit Harvest(tx.origin, _pendingTng);
    }
