...
contract Staking is Ownable {
    ...
    uint256 public lastRewardTime = block.number;
    ...
    function deposit(uint256 amount) external {
        ...
        stakeInfo.stakedTime = block.number;
        stakeInfo.unlockTime = block.number + lockTime;
...
// SWC-104-Unchecked Call Return Value: L80
    ...
    function harvest() external {
        ...
        user.lastClaimTime = block.number;
    ...
    function withdraw(uint256 sid) external {
        ...
        require(block.number >= stakeInfo.unlockTime, "Lock period not ended");
        ...
        // Stake info
        stakeInfo.amount = 0;
        stakeInfo.unstakedTime = block.number;
...
// SWC-104-Unchecked Call Return Value: L135
    ...
    function pendingTng(address _user) external view returns (uint256 pending) {
        ...
        if (block.number > lastRewardTime && lpTokenDeposited != 0) {
            uint256 time = block.number.sub(lastRewardTime);
