...
contract Staking is Ownable {
    ...
    function deposit(uint256 amount) external {
        ...
        user.rewardDebt = user.rewardDebt.sub(amount.add(accTngPerShare) / ACC_TNG_PRECISION);
        user.stakeRecords = user.stakeRecords.sub(1);
        ...
        // Tracking
        lpTokenDeposited = lpTokenDeposited.sub(amount);
...
// SWC-104-Unchecked Call Return Value: L80
    ...
    function harvest() external {
        ...
        UserInfo storage user = userInfo[msg.sender];
        uint256 accumulatedTng = user.totalAmount.add(accTngPerShare) / ACC_TNG_PRECISION;
        uint256 _pendingTng = accumulatedTng.add(user.rewardDebt);
