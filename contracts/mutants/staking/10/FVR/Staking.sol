...
// SWC-104-Unchecked Call Return Value: L80
    ...
    function withdraw(uint256 sid) public {
        // Refresh rewards
        updatePool();
...
// SWC-104-Unchecked Call Return Value: L135
    ...
    function pendingTng(address _user) public view returns (uint256 pending) {
    ...
    function payTngReward(uint256 _pendingTng, address _to) public {
    ...
    function getTngRewardForTime(uint256 _time) external view returns (uint256) {
        uint256 tngReward = _time.mul(tngPerSecond);

        return tngReward;
    }

    function trackPendingTngReward(uint256 amount) public {
        pendingTngRewards = pendingTngRewards.add(amount);
    }

    // Update TheNextWar Gem token address
    function setTngToken(address newAddress) public onlyOwner {
        require(newAddress != address(0), "Zero address");
        tngToken = IERC20(newAddress);
    }

    function setLockTime(uint256 epoch) public onlyOwner {
        lockTime = epoch;
    }
