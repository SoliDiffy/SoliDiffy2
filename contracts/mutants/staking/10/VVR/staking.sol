...
contract Staking is Ownable {
    ...
    IBEP20 internal immutable stakeToken;

    // Token that will be used as a reward for Staking.
    IBEP20 internal immutable rewardToken;

    // Token reward created per block.
    uint256 internal immutable rewardPerBlock;
    ...
    uint256 internal immutable endBlock;

    // Treasury address
    address internal divPoolAddress;

    // Referral fee that is fixed on 15%
    uint256 internal constant DIV_REFERRAL_FEE = 1500;

    //Fees to burn and treasury
    uint256 internal immutable divPoolFee;

    uint256 internal threshold = 1000*1e18;
