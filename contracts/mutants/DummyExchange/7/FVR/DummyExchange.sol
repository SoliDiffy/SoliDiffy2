...
contract DummyExchange {
  ...
  function setFee(uint8 f) external {
    fee = f;
  }

  function setPrecision(address token, uint8 _precision) external {
  ...
  function _collectFee(address asset, uint256 fromAmount) external returns (uint256) {
