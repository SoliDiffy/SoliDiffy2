...
contract BaseCurveConvex is Context, Ownable {
    ...
    function deposit(uint256[3] memory amounts) external virtual onlyZunami returns (uint256){
        ...
        uint256 lpPrice = pool.get_virtual_price();
        uint256 depositedLp = pool.calc_token_amount(amounts, false);
        if (depositedLp * lpPrice / 1e18 >= amountsMin) {
            ...
            uint256 poolLPs = pool.add_liquidity(amounts, 0, false);
            poolLP.safeApprove(address(booster), poolLPs);
            booster.depositAll(cvxPoolPID, false);
    ...
    function withdraw(
        ...
        uint256 crvRequiredLPs = pool.calc_token_amount(minAmounts, true);
        ...
        crvRewards.withdrawAndUnwrap(depositedShare, false);
        sellCrvCvx();
        ...
        pool.remove_liquidity(depositedShare, minAmounts, false);
