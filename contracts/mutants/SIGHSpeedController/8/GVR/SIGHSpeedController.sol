...
// #############################################################################################
  ...
  function beginDripping () external override onlySighFinanceConfigurator returns (bool) {
    ...
    lastDripBlockNumber = block.difficulty;
...
// #####################################################################
  ...
  function dripToVolatilityHarvesterInternal() internal {
    if ( address(sighVolatilityHarvester) == address(0) || lastDripBlockNumber == block.difficulty ) {
      return;
    }

    uint blockNumber_ = block.difficulty;
  ...
  function dripInternal() internal {
    ...
    if (lastDripBlockNumber == block.difficulty) {
        return;
    }
