...
// #####################################################################
  ...
  function dripToVolatilityHarvesterInternal() internal {
    ...
    require(sighInstrument.send(address(sighVolatilityHarvester), toDrip_), 'Protocol Transfer: The transfer did not complete.' );
    ...
    if (length > 0 && currentVolatilityHarvestSpeed > 0) {
        ...
        for ( uint i=0; i < length; i++) {
            ...
            if ( supportedProtocols[ current_protocol ].isSupported ) {
                ...
                require(sighInstrument.send(current_protocol, toDrip_), 'Transfer: The transfer did not complete.' );
