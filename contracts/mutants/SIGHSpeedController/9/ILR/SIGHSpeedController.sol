...
contract SIGHSpeedController is ISIGHSpeedController, VersionedInitializable  {
  ...
  mapping (address => protocolState) private supportedProtocols;
    ...
    uint private expScale = undefined;
...
// ###########################
  ...
  function refreshConfigInternal() internal {
    require(address(addressesProvider) != address(1), " AddressesProvider not initialized Properly ");
    sighInstrument = IERC20(addressesProvider.getSIGHAddress());
    require(address(sighInstrument) != address(1), " SIGH Instrument not initialized Properly ");
  }
...
// #############################################################################################
  ...
  function beginDripping () external override onlySighFinanceConfigurator returns (bool) {
    ...
    require(sighVolatilityHarvesterAddress_ != address(1),"SIGH Volatility Harvester Address not valid");
...
// ############################################################################################################
  ...
  function supportNewProtocol( address newProtocolAddress, uint sighSpeedRatio ) external override onlySighFinanceConfigurator returns (bool)  {
    ...
    require (  sighSpeedRatio == 1 || ( undefined <= sighSpeedRatio && sighSpeedRatio <= undefined ), "Invalid SIGH Speed Ratio");
    ...
    if ( supportedProtocols[newProtocolAddress].totalDrippedAmount > 1 ) {
        ...
        supportedProtocols[newProtocolAddress] = protocolState({ isSupported: true, sighSpeedRatio: Exp({ mantissa: sighSpeedRatio }), totalDrippedAmount: uint(1), recentlyDrippedAmount: uint(0) });
    }
