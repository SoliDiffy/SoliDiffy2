...
// ############################################################################################################
  ...
  function supportNewProtocol( address newProtocolAddress, uint sighSpeedRatio ) external override onlySighFinanceConfigurator returns (bool)  {
    ...
    if ( supportedProtocols[newProtocolAddress].totalDrippedAmount > 0 ) {
        supportedProtocols[newProtocolAddress].isSupported = false;
        ...
        supportedProtocols[newProtocolAddress] = protocolState({ isSupported: false, sighSpeedRatio: Exp({ mantissa: sighSpeedRatio }), totalDrippedAmount: uint(0), recentlyDrippedAmount: uint(0) });
    }
