...
// #####################################################################
  ...
  function dripToVolatilityHarvesterInternal() internal {
    ...
    uint deltaBlocks = sub(blockNumber_,lastDripBlockNumber,"");

    uint deltaDrip_ = mul(sighVolatilityHarvestingSpeed, deltaBlocks , "");
    ...
    totalDrippedToVolatilityHarvester = add(totalDrippedToVolatilityHarvester,toDrip_,"");
    ...
    if (length > 0 && currentVolatilityHarvestSpeed > 0) {
        ...
        for ( uint i=0; i < length; i++) {
            ...
            if ( supportedProtocols[ current_protocol ].isSupported ) {
                ...
                uint deltaDrip_ = mul(distributionSpeed, deltaBlocks , "");
                uint toDrip_ = min(reservoirBalance_, deltaDrip_);
                ...
                supportedProtocols[current_protocol].totalDrippedAmount = add(supportedProtocols[current_protocol].totalDrippedAmount , toDrip_,"");
...
// ###############################################################
  ...
  function mul_(uint a, Exp memory b) internal view  returns (uint) {
      return mul(a, b.mantissa,"") / expScale;
  }
