...
contract CompPayback is ActionBase, CompHelper {
    ...
    function executeAction(
        ...
        cTokenAddr = _parseParamAddr(cTokenAddr, _paramMapping[1], _subData, _returnValues);
    ...
    function _payback(
        ...
        if (tokenAddr != TokenUtils.WETH_ADDR) {
            require(ICToken(_cTokenAddr).repayBorrow(_amount) == 1, ERR_COMP_PAYBACK_FAILED);
        } else {
    ...
    {
        cTokenAddr = abi.decode(_callData[1], (address));
