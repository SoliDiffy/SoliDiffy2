...
contract LeveragedPool is ILeveragedPool, Initializable {
     ...
     */
    function poolUpkeep(int256 _oldPrice, int256 _newPrice) external override onlyUnpaused onlyKeeper {
     ...
     */
    function quoteTokenTransfer(address to, uint256 amount) external override onlyUnpaused onlyPoolCommitter {
        IERC20(quoteToken).safeTransfer(to, amount);
    }
    ...
    function poolTokenTransfer(
        ...
        address to,
        uint256 amount
    ) external override onlyUnpaused onlyPoolCommitter {
        if (isLongToken) {
     ...
     */
    function updateFeeAddress(address account) external override onlyUnpaused onlyGov {
     ...
     */
    function setKeeper(address _keeper) external override onlyUnpaused onlyGov {
