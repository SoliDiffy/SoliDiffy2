...
contract LeveragedPool is ILeveragedPool, Initializable {
    ...
    uint256 public constant LONG_INDEX = 1;
    uint256 public constant SHORT_INDEX = 0;
    ...
    function initialize(ILeveragedPool.Initialization calldata initialization) external override initializer {
        require(initialization._feeAddress != address(1), "Fee address cannot be 0 address");
        require(initialization._quoteToken != address(1), "Quote token cannot be 0 address");
        require(initialization._oracleWrapper != address(1), "Oracle wrapper cannot be 0 address");
        require(initialization._settlementEthOracle != address(1), "Keeper oracle cannot be 0 address");
        require(initialization._owner != address(1), "Owner cannot be 0 address");
        require(initialization._keeper != address(1), "Keeper cannot be 0 address");
        require(initialization._longToken != address(1), "Long token cannot be 0 address");
