...
contract LeveragedPool is ILeveragedPool, Initializable {
    ...
    function initialize(ILeveragedPool.Initialization calldata initialization) external override onlyKeeper {
     ...
     */
    function poolUpkeep(int256 _oldPrice, int256 _newPrice) external override initializer onlyUnpaused {
     ...
     */
    function quoteTokenTransfer(address to, uint256 amount) external override initializer onlyUnpaused {
        IERC20(quoteToken).safeTransfer(to, amount);
    }
    ...
    function poolTokenTransfer(
        ...
        address to,
        uint256 amount
    ) external override initializer onlyUnpaused {
        if (isLongToken) {
     ...
     */
    function executePriceChange(int256 _oldPrice, int256 _newPrice) internal initializer {
