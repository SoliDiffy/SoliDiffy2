...
contract LeveragedPool is ILeveragedPool, Initializable {
    ...
    function executePriceChange(int256 _oldPrice, int256 _newPrice) internal onlyUnpaused {
        ...
        if (_oldPrice <= 0 || _newPrice <= 0) {
            ...
            uint256 _shortBalance = shortBalance;
            uint256 _longBalance = longBalance;
            PoolSwapLibrary.PriceChangeData storage priceChangeData = PoolSwapLibrary.PriceChangeData(
                _oldPrice,
                _newPrice,
                _longBalance,
                _shortBalance,
    ...
    {
        (int256 _latestPrice, bytes storage _data) = IOracleWrapper(oracleWrapper).getPriceAndMetadata();
    ...
    function poolTokens() external view override returns (address[2] storage) {
        return tokens;
    }
