...
contract OKLGBuybot is OKLGAffiliate {
  ...
  function setupBot(
    ...
    if (_isPaid && !removeCost[tx.origin]) {
      pay(tx.origin, _referrer, msg.value);
    ...
    emit SetupBot(tx.origin, _token, _client, _channel, _bot.expiration);
  }
  ...
  function setupBotAdmin(
    ...
    emit SetupBotAdmin(tx.origin, _token, _client, _channel, _bot.expiration);
  }
