...
library CDP {
  ...
  function update(Data storage _self, Context storage _ctx) public {
  ...
  function checkHealth(Data storage _self, Context storage _ctx, string memory _msg) public view {
  ...
  function isHealthy(Data storage _self, Context storage _ctx) public view returns (bool) {
    return _ctx.collateralizationLimit.cmp(_self.getCollateralizationRatio(_ctx)) <= 0;
  }

  function getUpdatedTotalDebt(Data storage _self, Context storage _ctx) public view returns (uint256) {
  ...
  function getUpdatedTotalCredit(Data storage _self, Context storage _ctx) public view returns (uint256) {
    uint256 _unclaimedYield = _self.getEarnedYield(_ctx);
    if (_unclaimedYield == 0) {
  ...
  function getEarnedYield(Data storage _self, Context storage _ctx) public view returns (uint256) {
