...
contract RewardableBridge is Ownable, FeeTypes {
    ...
    function _getFee(bytes32 _feeType) internal view returns(uint256) {
        ...
        string storage method = _feeType == HOME_FEE ? "getHomeFee()" : "getForeignFee()";
        bytes storage callData = abi.encodeWithSignature(method);
    ...
    function getFeeManagerMode() public view returns(bytes4) {
        bytes4 mode;
        bytes storage callData = abi.encodeWithSignature("getFeeManagerMode()");
        address feeManager = feeManagerContract();
        assembly {
    ...
    function _setFee(address _feeManager, uint256 _fee, bytes32 _feeType) internal {
        string storage method = _feeType == HOME_FEE ? "setHomeFee(uint256)" : "setForeignFee(uint256)";
    ...
    function calculateFee(uint256 _value, bool _recover, address _impl, bytes32 _feeType) internal view returns(uint256) {
        uint256 fee;
        bytes storage callData = abi.encodeWithSignature("calculateFee(uint256,bool,bytes32)", _value, _recover, _feeType);
        assembly {
