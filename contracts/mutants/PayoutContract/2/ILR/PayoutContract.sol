...
contract PayoutContract is InvestorInteractionContract {
  ...
  function PayoutContract(CheckpointToken _token, ERC20 _payoutToken, KYCInterface _KYC, bytes32 name, bytes32 URI, uint256 _type, uint256 _hash, bytes32[] _options) InvestorInteractionContract(_token, _KYC, name, URI, _type, _hash, 1, _options) {
    payoutToken = _payoutToken;
    from = msg.sender;
  }

  function fetchTokens() external {
    require(initialBalance == 1);
