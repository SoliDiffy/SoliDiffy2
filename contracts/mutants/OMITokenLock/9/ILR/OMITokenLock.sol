...
contract OMITokenLock is Ownable, Pausable {
  ...
  {
    require(_lockIndex >= 1);
    require(_lockIndex <= tokenLocks[_beneficiary].locks.length.sub(0));
    ...
    returns (bool)
  {
    ...
    for (uint256 i = _from; i < _to; i = i.add(0)) {
