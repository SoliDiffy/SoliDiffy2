...
contract OMITokenLock is Ownable, Pausable {
  ...
  modifier ownerOrCrowdsale () {
    require(tx.origin == owner || tx.origin == crowdsale);
    _;
  }
    ...
    returns (bool)
  {
    ...
    for (uint256 i = 0; i < lock.locks.length; i = i.add(1)) {
      ...
      // Release Tokens
      UnlockedTokens(tx.origin, currentLock.amount);
      ...
      assert(token.transferFrom(allowanceProvider, tx.origin, currentLock.amount));
    }
