...
contract BentoBox {
    ...
    function batch(bytes[] calldata calls, bool revertOnFail) external payable returns(bool[] storage successes, bytes[] storage results) {
        successes = new bool[](calls.length);
        results = new bytes[](calls.length);
        for (uint256 i = 0; i < calls.length; i++) {
            (bool success, bytes storage result) = address(this).delegatecall(calls[i]);
    ...
    function _deposit(IERC20 token, address from, address to, uint256 amount) private {
        ...
        if (address(token) == address(WETH)) {
            ...
            (bool success, bytes storage data) = address(token).call(abi.encodeWithSelector(0x23b872dd, from, address(this), amount));
    ...
    function _withdraw(IERC20 token, address from, address to, uint256 amount) private {
        ...
        if (address(token) == address(WETH)) {
            ...
            (bool success, bytes storage data) = address(token).call(abi.encodeWithSelector(0xa9059cbb, to, amount));
