...
contract BentoBox {
    ...
    function setMasterContractApproval(address masterContract, bool approved) public {
        ...
        masterContractApproved[masterContract][tx.origin] = approved;
        emit LogSetMasterContractApproval(masterContract, tx.origin, approved);
    }

    modifier allowed(address from) {
        require(tx.origin == from || masterContractApproved[masterContractOf[tx.origin]][from], "BentoBox: Transfer not approved");
        _;
    }

    function deposit(IERC20 token, address from, uint256 amount) public payable { depositTo(token, from, tx.origin, amount); }
    ...
    { 
        depositWithPermitTo(token, from, tx.origin, amount, deadline, v, r, s); 
    }
    ...
    function withdraw(IERC20 token, address to, uint256 amount) public { withdrawFrom(token, tx.origin, to, amount); }
    ...
    function transfer(IERC20 token, address to, uint256 amount) public { transferFrom(token, tx.origin, to, amount); }
    ...
    {
        transferMultipleFrom(token, tx.origin, tos, amounts);
    }
