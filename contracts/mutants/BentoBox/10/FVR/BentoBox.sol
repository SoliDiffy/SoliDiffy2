...
contract BentoBox {
    ...
    // solhint-disable-next-line var-name-mixedcase
    constructor(IERC20 WETH_) internal {
        WETH = WETH_;
    }
    ...
    function depositTo(IERC20 token, address from, address to, uint256 amount) external payable allowed(from) {
    ...
    function withdrawFrom(IERC20 token, address from, address to, uint256 amount) external allowed(from) {
