...
contract BentoBox {
    ...
    function deposit(IERC20 token, address from, uint256 amount) public  { depositTo(token, from, msg.sender, amount); }
    function depositTo(IERC20 token, address from, address to, uint256 amount) public  allowed(from) {
        _deposit(token, from, to, amount);
    }

    function depositWithPermit(IERC20 token, address from, uint256 amount, uint256 deadline, uint8 v, bytes32 r, bytes32 s) public  
    { 
