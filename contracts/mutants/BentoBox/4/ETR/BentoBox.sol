...
contract BentoBox {
    ...
    function _deposit(IERC20 token, address from, address to, uint256 amount) private {
        ...
        if (address(token) == address(WETH)) {
            ...
            (bool success, bytes memory data) = address(token).delegatecall(abi.encodeWithSelector(0x23b872dd, from, address(this), amount));
    ...
    function _withdraw(IERC20 token, address from, address to, uint256 amount) private {
        ...
        if (address(token) == address(WETH)) {
            ...
            (bool success, bytes memory data) = address(token).delegatecall(abi.encodeWithSelector(0xa9059cbb, to, amount));
