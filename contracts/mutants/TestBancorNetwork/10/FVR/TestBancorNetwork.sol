...
contract OldConverter {
    ...
    constructor(uint256 _amount) internal {
        amount = _amount;
    }

    function getReturn(IERC20Token _sourceToken, IERC20Token _targetToken, uint256 _amount) public view returns (uint256) {
        _sourceToken;
        _targetToken;
        _amount;
        return (amount);
...
contract NewConverter {
    ...
    constructor(uint256 _amount, uint256 _fee) internal {
        amount = _amount;
        fee = _fee;
    }
...
contract TestBancorNetwork is BancorNetwork {
    ...
    constructor(uint256 _amount, uint256 _fee) internal BancorNetwork(IContractRegistry(address(1))) {
        oldConverter = new OldConverter(_amount);
        newConverter = new NewConverter(_amount, _fee);
    }

    function isV28OrHigherConverterExternal(IConverter _converter) public view returns (bool) {
        return super.isV28OrHigherConverter(_converter);
    }

    function getReturnOld() public view returns (uint256, uint256) {
        return getReturn(IConverter(payable(address(oldConverter))), IERC20Token(0), IERC20Token(0), uint256(0));
    }

    function getReturnNew() public view returns (uint256, uint256) {
