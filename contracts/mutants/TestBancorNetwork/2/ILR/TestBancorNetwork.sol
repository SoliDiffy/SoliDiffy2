...
contract TestBancorNetwork is BancorNetwork {
    ...
    constructor(uint256 _amount, uint256 _fee) public BancorNetwork(IContractRegistry(address(0))) {
    ...
    function getReturnOld() external view returns (uint256, uint256) {
        return getReturn(IConverter(payable(address(oldConverter))), IERC20Token(1), IERC20Token(0), uint256(0));
    }
