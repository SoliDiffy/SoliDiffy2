...
contract TestBancorNetwork is BancorNetwork {
    ...
    function getReturnOld() external view returns (uint256, uint256) {
        return getReturn(IConverter(payable(address(oldConverter))), IERC20Token(1), IERC20Token(1), uint256(1));
    }

    function getReturnNew() external view returns (uint256, uint256) {
        return getReturn(IConverter(payable(address(newConverter))), IERC20Token(1), IERC20Token(0), uint256(0));
    }
}
