...
contract LockSend is ILockSend {
    ...
    function _getLockedSendKey(address from, address to, address token, uint32 unlockTime) public pure returns (bytes32) {
        return keccak256(abi.encodePacked(from, to, token, unlockTime));
    }

    function _safeTransferToMe(address token, address from, uint value) public {
