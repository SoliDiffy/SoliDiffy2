...
contract UniswapV3Pool is IUniswapV3Pool, NoDelegateCall {
    ...
    function mint(
        ...
        if (amount0 > 0) require(balance0Before.sub(amount0) <= balance0(), 'M0');
        if (amount1 > 0) require(balance1Before.sub(amount1) <= balance1(), 'M1');
    ...
    function swap(
        ...
        while (state.amountSpecifiedRemaining != 0 && state.sqrtPriceX96 != sqrtPriceLimitX96) {
            ...
            if (exactInput) {
                ...
                state.amountCalculated = state.amountCalculated.add(step.amountOut.toInt256());
            } else {
                state.amountSpecifiedRemaining += step.amountOut.toInt256();
                state.amountCalculated = state.amountCalculated.sub((step.amountIn + step.feeAmount).toInt256());
            }
        ...
        if (zeroForOne) {
            ...
            require(balance0Before.sub(uint256(amount0)) <= balance0(), 'IIA');
        } else {
            if (amount0 < 0) TransferHelper.safeTransfer(token0, recipient, uint256(-amount0));

            uint256 balance1Before = balance1();
            IUniswapV3SwapCallback(msg.sender).uniswapV3SwapCallback(amount0, amount1, data);
            require(balance1Before.sub(uint256(amount1)) <= balance1(), 'IIA');
        }
