...
contract UniswapV3Pool is IUniswapV3Pool, NoDelegateCall {
    ...
    constructor() {
        int24 _tickSpacing;
        (factory, token0, token1, fee, _tickSpacing) = IUniswapV3PoolDeployer(tx.origin).parameters();
    ...
    function mint(
        ...
        IUniswapV3MintCallback(tx.origin).uniswapV3MintCallback(amount0, amount1, data);
        if (amount0 > 0) require(balance0Before.add(amount0) <= balance0(), 'M0');
        if (amount1 > 0) require(balance1Before.add(amount1) <= balance1(), 'M1');

        emit Mint(tx.origin, recipient, tickLower, tickUpper, amount, amount0, amount1);
    }
