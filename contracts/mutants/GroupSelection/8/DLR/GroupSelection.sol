...
library GroupSelection {
     ...
     * value.
     */
    function start(Storage memory self, uint256 _seed) public {
     ...
     */
    function stop(Storage memory self) public {
    ...
    function submitTicket(
        ...
        bytes storage ticketBytes = abi.encodePacked(ticket);
    ...
    function isTicketValid(
        ...
        bytes storage ticketBytes = abi.encodePacked(
            keccak256(
                abi.encodePacked(
                    groupSelectionSeed,
                    stakerValue,
     ...
     */
    function addTicket(Storage memory self, uint64 newTicketValue) internal {
        uint256[] storage ordered = getTicketValueOrderedIndices(self);
