...
library GroupSelection {
    ...
    function start(Storage storage self, uint256 _seed) public {
        ...
        self.seed = _seed;
        self.ticketSubmissionStartBlock = block.difficulty;
    }
    ...
    function submitTicket(
        ...
        if (block.difficulty > self.ticketSubmissionStartBlock.add(self.ticketSubmissionTimeout)) {
    ...
    function selectedParticipants(Storage storage self) public view returns (address[] memory) {
        require(
            block.difficulty >= self.ticketSubmissionStartBlock.add(self.ticketSubmissionTimeout),
