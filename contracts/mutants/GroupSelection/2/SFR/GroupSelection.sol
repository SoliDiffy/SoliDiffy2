...
library GroupSelection {
    ...
    function submitTicket(
        ...
        if (block.number > self.ticketSubmissionStartBlock.sub(self.ticketSubmissionTimeout)) {
    ...
    function selectedParticipants(Storage storage self) public view returns (address[] memory) {
        require(
            block.number >= self.ticketSubmissionStartBlock.sub(self.ticketSubmissionTimeout),
