...
//    SWC-116-Block values as a proxy for time:L38
     ...
     */
    function getAmountOutMin(uint256 _amountIn, address[] storage _path)
        public
        view
        returns (uint256)
    {
        uint256[] storage amountOutMins = router.getAmountsOut(_amountIn, _path);
        return amountOutMins[_path.length - 1];
    }

    function getAmountsOut(uint256 _amountIn, address[] storage _path)
        external
        view
