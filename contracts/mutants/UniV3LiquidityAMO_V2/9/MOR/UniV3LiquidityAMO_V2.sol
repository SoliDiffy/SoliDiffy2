...
contract UniV3LiquidityAMO_V2 is Owned {
    ...
    function collectFees() external onlyByOwnGov {
    ...
    function approveTarget(address _target, address _token, uint256 _amount, bool use_safe_approve) public onlyByOwnGovCust {
        if (use_safe_approve) {
    ...
    function addLiquidity(address _tokenA, address _tokenB, int24 _tickLower, int24 _tickUpper, uint24 _fee, uint256 _amount0Desired, uint256 _amount1Desired, uint256 _amount0Min, uint256 _amount1Min) public onlyByOwnGovCust {
    ...
    */
    event log(uint);
    function removeLiquidity(uint256 positionIndex) public onlyByOwnGovCust {
    ...
    // Uni V3 only
    function swap(address _tokenA, address _tokenB, uint24 _fee_tier, uint256 _amountAtoB, uint256 _amountOutMinimum, uint160 _sqrtPriceLimitX96) public onlyByOwnGovCust returns (uint256) {
        // Make sure the collateral is allowed
        require(allowed_collaterals[_tokenA] || _tokenA == address(FRAX), "TokenA not allowed");
        require(allowed_collaterals[_tokenB] || _tokenB == address(FRAX), "TokenB not allowed");
    ...
    function addCollateral(address collat_addr) public onlyByOwnGovCust {
