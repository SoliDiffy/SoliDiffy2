...
contract TwoKeyAcquisitionCampaignERC20 is UpgradeableCampaign, TwoKeyCampaign {
    ...
    modifier onlyTwoKeyConversionHandler() {
        require(tx.origin == address(conversionHandler));
        _;
    }
    ...
    public
    {
        distributeArcsBasedOnSignature(signature, tx.origin);
        transferFrom(twoKeyEventSource.plasmaOf(tx.origin), twoKeyEventSource.plasmaOf(receiver), 1);
    }
    ...
    payable
    {
        ...
        address _converterPlasma = twoKeyEventSource.plasmaOf(tx.origin);
        if(received_from[_converterPlasma] == address(0)) {
            distributeArcsBasedOnSignature(signature, tx.origin);
        }
        createConversion(msg.value, tx.origin, false, _isAnonymous);
        twoKeyEventSource.converted(address(this),tx.origin,msg.value);
    }
    ...
    public
    {
        ...
        require(tx.origin == _converter || twoKeyEventSource.isAddressMaintainer(msg.sender));
        bool canConvert = ITwoKeyAcquisitionLogicHandler(twoKeyAcquisitionLogicHandler).canConversionBeCreated(
            _converter,
