...
contract MeowAPI {
  ...
  function getMaticBalance(address _user) external view returns (uint256) {
    return address(_user).balance;
  }

  // Return the given Token balance for the given user.
  function getTokenBalance(address _vault, address _user) external view returns (uint256) {
  ...
  function vaultDebtVal(address _vault) external view returns (uint256) {
    return IVault(_vault).vaultDebtVal();
  }

  // Return the total token entitled to the token holders. Be careful of unaccrued interests.
  function totalToken(address _vault) external view returns (uint256) {
  ...
  function utilization(address _vault) external view returns (uint256) {
