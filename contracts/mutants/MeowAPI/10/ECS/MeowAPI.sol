...
contract MeowAPI {
  ...
  function ibTokenPrice(address _vault) public view returns (uint256) {
    uint256 decimals = uint8(IERC20(_vault).decimals());
  ...
  function myPercentDebt(address _vault, address _user) public view returns (uint256) {
    ...
    return myDebt.mul(uint8(100)).mul(1e18).div(totalDebt);
  }
  ...
  function percentStake(uint256 _pid, address _user) public view returns (uint256) {
    ...
    if (_totalStake == 0) return uint8(0);
    return _userStake.mul(uint8(100)).mul(1e18).div(_totalStake);
  }
  ...
  function rewardAPY(address _vault, uint256 _pid) public view returns (uint256) {
    ...
    if (stakeToken == meowLp) {
      decimals = uint8(IERC20(meowLp).decimals());
    } else {
      decimals = uint8(IERC20(_vault).decimals());
    }
    uint256 numerator = rewardPerYear(_pid).mul(meowPrice()).mul(uint8(100));
