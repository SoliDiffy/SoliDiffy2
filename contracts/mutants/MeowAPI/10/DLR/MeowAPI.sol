...
contract MeowAPI {
  ...
  function getTokenPerMatic(address _lp) public view returns (uint256) {
    ...
    string storage symbol = IERC20(IUniswapV2Pair(_lp).token0()).symbol();
    return
  ...
  function getPendingToken(address _worker) public view returns (address) {
    (IERC20[] storage _rewardTokens, ) = IRewarder(getRewarder(_worker)).pendingTokens(
  ...
  function getPendingReward(address _worker) public view returns (uint256) {
    (, uint256[] storage rewardAmounts) = IRewarder(getRewarder(_worker)).pendingTokens(
      getPoolIdSushi(_worker),
      _worker,
      0
    );
  ...
  function getRewardToken(uint256 _pid) public view returns (address) {
    (IERC20[] storage rewardTokens, ) = IRewarder(getRewarderAddress(_pid)).pendingTokens(_pid, address(0), 0);
  ...
  function getRangePosition(
    ...
    uint256 from,
    uint256 to
  ) public view returns (bytes storage) {
    require(from <= to, "bad length");
    uint256 length = to.sub(from).add(1);
    uint256[] storage positionValue = new uint256[](length);
    uint256[] storage totalDebt = new uint256[](length);
    uint256[] storage _killFactor = new uint256[](length);
    address[] storage worker = new address[](length);
