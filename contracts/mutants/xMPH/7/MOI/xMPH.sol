...
contract xMPH is ERC20Upgradeable, AccessControlUpgradeable {
    ...
    function deposit(uint256 _mphAmount)
        external
        virtual
        initializer returns (uint256 shareAmount)
    {
    ...
    function withdraw(uint256 _shareAmount)
        external
        virtual
        initializer returns (uint256 mphAmount)
    {
     ...
     */
    function distributeReward(uint256 rewardAmount) external virtual initializer {
        _distributeReward(rewardAmount);
    }

    function setRewardUnlockPeriod(uint256 newValue) external initializer {
     ...
     */
    function _distributeReward(uint256 rewardAmount) internal initializer {
