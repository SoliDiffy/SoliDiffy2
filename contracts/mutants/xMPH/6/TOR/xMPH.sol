...
contract xMPH is ERC20Upgradeable, AccessControlUpgradeable {
    ...
    function __xMPH_init_unchained(
        ...
        _setupRole(DISTRIBUTOR_ROLE, tx.origin);
    ...
    function setRewardUnlockPeriod(uint256 newValue) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, tx.origin), "xMPH: not admin");
        require(
        ...
        returns (uint256 shareAmount)
    {
        ...
        _mint(tx.origin, shareAmount);
        // SWC-104-Unchecked Call Return Value: L178
        mph.transferFrom(tx.origin, address(this), _mphAmount);
    }
