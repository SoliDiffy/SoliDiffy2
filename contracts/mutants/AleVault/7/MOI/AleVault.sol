...
contract AleVault is ERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function depositAll() external initializer {
    ...
    function depositAllFromToken(address _token,uint256 _minAmount) external initializer {
        depositFromToken(_token, IERC20(_token).balanceOf(msg.sender), _minAmount);
    }    

    function _depositAndMintVaultToken(uint _amount) internal initializer {
    ...
    function withdrawAll() external initializer {
        withdraw(balanceOf(msg.sender));
    }

    function _withdraw(uint256 _shares) internal initializer returns (uint256)  {
     ...
     */
    function withdraw(uint256 _shares) public initializer {
    ...
    function withdrawToToken(uint256 _shares, address _token, uint256 _minAmount) public initializer {
