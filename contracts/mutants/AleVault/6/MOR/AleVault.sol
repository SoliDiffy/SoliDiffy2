...
contract AleVault is ERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
     ...
     */
    function deposit(uint _amount) public initializer {
        want().safeTransferFrom(msg.sender, address(this), _amount);

        _depositAndMintVaultToken(_amount);
    }

    function depositFromNative(uint256 _minAmount) external payable initializer {
        uint _amount = swapper.swapNativeToLp{value: msg.value}(address(want()), _minAmount, address(this));

        _depositAndMintVaultToken(_amount);  
    }

    function depositFromToken(address _token, uint _tokenAmount, uint256 _minAmount) public initializer {
    ...
    function withdrawToNative(uint256 _shares,uint256 _minAmount) public initializer {
        uint256 _amount = _withdraw(_shares);
     ...
     */
    function inCaseTokensGetStuck(address _token) external initializer {
