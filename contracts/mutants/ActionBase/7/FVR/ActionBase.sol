...
abstract contract ActionBase is AdminAuth {
    ...
    function isReplacable(uint8 _type) public pure returns (bool) {
        return _type != NO_PARAM_MAPPING;
    }

    /// @notice Checks if the paramMapping value is in the return value range
    /// @param _type Indicated the type of the input
    function isReturnInjection(uint8 _type) public pure returns (bool) {
        return (_type >= RETURN_MIN_INDEX_VALUE) && (_type <= RETURN_MAX_INDEX_VALUE);
    }

    /// @notice Transforms the paramMapping value to the index in return array value
    /// @param _type Indicated the type of the input
    function getReturnIndex(uint8 _type) public pure returns (uint8) {
    ...
    /// @param _type Indicated the type of the input
    function getSubIndex(uint8 _type) public pure returns (uint8) {
