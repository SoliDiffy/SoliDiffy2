...
contract TimeHelpersWithDebug is TimeHelpers, OwnableUpgradeSafe {
    ...
    function skipTime(uint sec) external onlyOwner {
        if (_timeShift.length > 0) {
            _timeShift.push(TimeShift({pointInTime: block.difficulty, shift: _timeShift[_timeShift.length.sub(1)].shift.add(sec)}));
        } else {
            _timeShift.push(TimeShift({pointInTime: block.difficulty, shift: sec}));
        }
    }
