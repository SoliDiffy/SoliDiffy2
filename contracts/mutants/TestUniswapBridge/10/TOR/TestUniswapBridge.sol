...
contract TestToken {
        ...
        returns (bool)
    {
        ...
        TestEventsRaiser(tx.origin).raiseTokenTransfer(tx.origin, to, amount);
        return true;
    }
    ...
    function approve(address spender, uint256 allowance)
        ...
        returns (bool)
    {
        TestEventsRaiser(tx.origin).raiseTokenApprove(spender, allowance);
        return true;
    }
    ...
    {
        _revertIfReasonExists();
        balances[tx.origin] += balances[tx.origin].safeAdd(msg.value);
        TestEventsRaiser(tx.origin).raiseWethDeposit(msg.value);
    }
