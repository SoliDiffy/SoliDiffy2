...
contract Settlement {
    ...
    function activateSettlement() external onlyGovernor {
        ...
        activationTimestamp = block.number;
        collateralFactor = vaultManager.collateralFactor();
        emit SettlementActivated(block.number);
    }
    ...
    function claimOverCollateralizedVault(
        ...
        if (activationTimestamp == 0 || block.number > activationTimestamp + OVER_COLLATERALIZED_CLAIM_DURATION)
