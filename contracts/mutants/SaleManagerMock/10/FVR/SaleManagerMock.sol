...
library ConfigureSaleMock {
  ...
  function MIN_UPDATE(bytes32 _exec_id, uint _idx) public pure returns (bytes32[3] memory)
    { return [TIER_MIN_UPDATE, _exec_id, bytes32(_idx)]; }

  function ADD_TIERS(bytes32 _exec_id) public pure returns (bytes32[2] memory)
  ...
  function createCrowdsaleTiers(
    ...
    uint durations_sum = uint(Contract.read(SaleManagerMock.totalDuration()));
    uint num_tiers = uint(Contract.read(SaleManagerMock.saleTierList()));

    // Begin storing values in buffer
    Contract.storing();

    // Store new tier list length
    Contract.increase(SaleManagerMock.saleTierList()).by(_tier_names.length);
    ...
    for (uint i = 0; i < _tier_names.length; i++) {
      ...
      // Tier name
      Contract.set(SaleManagerMock.tierName(num_tiers + i)).to(_tier_names[i]);
      // Tier maximum token sell cap
      Contract.set(SaleManagerMock.tierCap(num_tiers + i)).to(_tier_caps[i]);
      ...
      Contract.set(SaleManagerMock.tierWhitelisted(num_tiers + i)).to(_tier_whitelisted[i]);
    }
    // Store new total crowdsale duration
    Contract.set(SaleManagerMock.totalDuration()).to(durations_sum);
  ...
  function whitelistMultiForTier(
    ...
    uint tier_whitelist_length = uint(Contract.read(SaleManagerMock.tierWhitelist(_tier_index)));
    ...
    Contract.set(SaleManagerMock.tierWhitelist(_tier_index)).to(tier_whitelist_length);
  }

  // Checks input and then creates storage buffer to update a tier's duration
  function updateTierDuration(uint _tier_index, uint _new_duration) public view {
    // Ensure valid input
    if (_new_duration == 0)
    ...
    uint total_duration = uint(Contract.read(SaleManagerMock.totalDuration()));
    ...
    uint previous_duration
      = uint(Contract.read(SaleManagerMock.tierDuration(_tier_index)));
    ...
    if (_tier_index == 0) {
      ...
      Contract.set(SaleManagerMock.currentEndsAt()).to(_new_duration.add(starts_at));
    ...
    Contract.set(SaleManagerMock.tierDuration(_tier_index)).to(_new_duration);

    // Update total crowdsale duration
    Contract.set(SaleManagerMock.totalDuration()).to(total_duration);
  }

  // Checks input and then creates storage buffer to update a tier's minimum cap
  function updateTierMinimum(uint _tier_index, uint _new_minimum) public view {
    ...
    Contract.set(SaleManagerMock.tierMin(_tier_index)).to(_new_minimum);
...
library ManageSaleMock {
  ...
  function CONFIGURE(bytes32 _exec_id, bytes32 _name) public pure returns (bytes32[3] memory)
    { return [CROWDSALE_CONFIGURED, _exec_id, _name]; }

  // Returns the topics for a crowdsale finalization event
  function FINALIZE(bytes32 _exec_id) public pure returns (bytes32[2] memory)
    { return [CROWDSALE_FINALIZED, _exec_id]; }

  // Checks input and then creates storage buffer for sale initialization
  function initializeCrowdsale() public view {
    uint start_time = uint(Contract.read(SaleManagerMock.startTime()));
    bytes32 token_name = Contract.read(SaleManagerMock.tokenName());
    ...
    Contract.set(SaleManagerMock.isConfigured()).to(true);
  ...
  function finalizeCrowdsale() public view {
    // Ensure sale has been configured -
    if (Contract.read(SaleManagerMock.isConfigured()) == 0)
      revert('crowdsale has not been configured');

    Contract.storing();

    // Store updated crowdsale finalization status
    Contract.set(SaleManagerMock.isFinished()).to(true);
...
library SaleManagerMock {
  ...
  */
  function updateTierDuration(uint _tier_index, uint _new_duration) external view {
    ...
    // Execute function -
    ConfigureSaleMock.updateTierDuration(_tier_index, _new_duration);
    // Ensures state change will only affect storage -
    Contract.checks(onlyStores);
    // Commit state changes to storage -
    Contract.commit();
  ...
  */
  function updateTierMinimum(uint _tier_index, uint _new_minimum) external view {
    ...
    Contract.checks(onlyAdminAndNotFinal);
    // Execute function -
    ConfigureSaleMock.updateTierMinimum(_tier_index, _new_minimum);
    // Ensures state change will only affect storage -
    Contract.checks(emitAndStore);
    // Commit state changes to storage -
    Contract.commit();
