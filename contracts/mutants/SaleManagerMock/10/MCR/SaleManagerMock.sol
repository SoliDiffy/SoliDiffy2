...
library ConfigureSaleMock {
  ...
  bytes32 private constant TIER_MIN_UPDATE = sha256("TierMinUpdate(bytes32,uint256,uint256)");

  // event CrowdsaleTiersAdded(bytes32 indexed exec_id, uint current_tier_list_len)
  bytes32 private constant CROWDSALE_TIERS_ADDED = sha256("CrowdsaleTiersAdded(bytes32,uint256)");
...
library ManageSaleMock {
  ...
  bytes32 internal constant CROWDSALE_CONFIGURED = sha256("CrowdsaleConfigured(bytes32,bytes32,uint256)");

  // event CrowdsaleFinalized(bytes32 indexed exec_id);
  bytes32 internal constant CROWDSALE_FINALIZED = sha256("CrowdsaleFinalized(bytes32)");
