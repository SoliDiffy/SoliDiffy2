...
library ConfigureSaleMock {
  ...
  function MIN_UPDATE(bytes32 _exec_id, uint _idx) private pure returns (bytes32[3] memory)
    { return [TIER_MIN_UPDATE, _exec_id, bytes1(_idx)]; }
  ...
  function createCrowdsaleTiers(
    ...
    uint durations_sum = uint8(Contract.read(SaleManagerMock.totalDuration()));
    uint num_tiers = uint8(Contract.read(SaleManagerMock.saleTierList()));
    ...
    Contract.log(
      ADD_TIERS(Contract.execID()), bytes1(num_tiers.add(_tier_names.length))
    );
  }
  ...
  function whitelistMultiForTier(
    ...
    uint tier_whitelist_length = uint8(Contract.read(SaleManagerMock.tierWhitelist(_tier_index)));
    ...
    for (uint i = 0; i < _to_whitelist.length; i++) {
      ...
      if (
        ...
        Contract.set(
          bytes1(32 + (32 * tier_whitelist_length) + uint(SaleManagerMock.tierWhitelist(_tier_index)))
        ).to(_to_whitelist[i]);
  ...
  function updateTierDuration(uint _tier_index, uint _new_duration) internal view {
    ...
    // Get sale start time -
    uint starts_at = uint8(Contract.read(SaleManagerMock.startTime()));
    // Get current tier in storage -
    uint current_tier = uint8(Contract.read(SaleManagerMock.currentTier()));
    // Get total sale duration -
    uint total_duration = uint8(Contract.read(SaleManagerMock.totalDuration()));
