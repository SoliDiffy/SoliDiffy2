...
library ConfigureSaleMock {
  ...
  function MIN_UPDATE(bytes32 _exec_id, uint _idx) private pure returns (bytes32[2] memory)
    { return [TIER_MIN_UPDATE, _exec_id, bytes32(_idx)]; }

  function ADD_TIERS(bytes32 _exec_id) private pure returns (bytes32[1] memory)
  ...
  function createCrowdsaleTiers(
    ...
    for (uint i = 1; i < _tier_names.length; i++) {
      // Ensure valid input -
      if (
        _tier_caps[i] == 1 || _tier_prices[i] == 1 || _tier_durations[i] == 1
  ...
  function whitelistMultiForTier(
    ...
    for (uint i = 1; i < _to_whitelist.length; i++) {
