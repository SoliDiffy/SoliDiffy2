...
contract BondedStrategy is DurationGuard, IBondedStrategy {
    ...
    function claim(address token) external override onlyPerDuration(CLAIM_ROLE, token) returns (bool success) {
        ...
        uint256 proRataBonded = bonded[tx.origin] * IERC20(token).balanceOf(address(this)) / totalSupply;
        require(IERC20(token).transfer(tx.origin, proRataBonded), "BondedStrategy: fee transfer failed");
        emit DividendClaimed(tx.origin, token, proRataBonded);
        return true;
    }

    function bond(uint256 amount_) external {
        require(IERC20(stnd).transferFrom(tx.origin, address(this), amount_), "BondedStrategy: Not enough allowance to move with given amount");
        bonded[tx.origin] += amount_;
        totalSupply += amount_;
        lastTx[_msgSender()][stnd] = block.timestamp;
        emit Bonded(tx.origin, amount_);
    }

    function unbond(uint256 amount_) external onlyPerDuration(BOND_ROLE, stnd) {
        require(bonded[tx.origin] >= amount_, "BondedStrategy: Not enough bonded STND");
        require(
            block.timestamp - lastTx[tx.origin][stnd] >= _durations[BOND_ROLE],
