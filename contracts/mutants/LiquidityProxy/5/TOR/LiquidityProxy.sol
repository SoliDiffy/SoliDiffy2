...
contract LiquidityProxy
{
    ...
    {
        require(operator == tx.origin, "LiquidityProxy: Caller is not the operator");
        _;
    }
        ...
        ) external payable returns (uint amountA_, uint amountB_, uint liquidity_)
    {
        ...
        IERC20(_tokenA).safeTransferFrom(tx.origin, _tokenLp, amountA_);
        IERC20(_tokenB).safeTransferFrom(tx.origin, _tokenLp, amountB_);
        ...
        ) external payable returns (uint amountToken_, uint amountBnb_, uint liquidity_)
    {
        ...
        IERC20(_token).safeTransferFrom(tx.origin, _tokenLp, amountToken_);
