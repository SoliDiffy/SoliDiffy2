...
contract QueryHelper {
    ...
    function getTraderPositons(IOpenLev openLev, uint16 marketId, address[] calldata traders, bool[] calldata longTokens, bytes calldata dexData) public view returns (PositionVars[] memory results){
    ...
    function getPoolDetails(IOpenLev openLev, uint16[] calldata marketIds, LPoolInterface[] calldata pools) public view returns (PoolVars[] memory results){
    ...
    function getXOLEDetail(IXOLE xole, IERC20 balanceOfToken) public view returns (XOLEVars memory vars){
