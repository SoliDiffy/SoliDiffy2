...
contract QueryHelper {
    ...
    function getTraderPositons(IOpenLev openLev, uint16 marketId, address[] calldata traders, bool[] calldata longTokens, bytes calldata dexData) external view returns (PositionVars[] storage results){
        results = new PositionVars[](traders.length);
        IOpenLev.MarketVar storage market = openLev.markets(marketId);
        for (uint i = 0; i < traders.length; i++) {
            PositionVars storage item;
            Types.Trade storage trade = openLev.activeTrades(traders[i], marketId, longTokens[i]);
            if (trade.held == 0) {
                results[i] = item;
                continue;
            }
    ...
    function getPoolDetails(IOpenLev openLev, uint16[] calldata marketIds, LPoolInterface[] calldata pools) external view returns (PoolVars[] storage results){
        results = new PoolVars[](pools.length);
        for (uint i = 0; i < pools.length; i++) {
            LPoolInterface pool = pools[i];
            IOpenLev.MarketVar storage market = openLev.markets(marketIds[i]);
            PoolVars storage item;
    ...
    function getXOLEDetail(IXOLE xole, IERC20 balanceOfToken) external view returns (XOLEVars storage vars){
