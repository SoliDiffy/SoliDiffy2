...
contract MockScriptExecutorApp is AragonApp {
    ...
    function execute(bytes script) external {
        runScript(script, new bytes(0), new address[](0));
    }

    function executeWithBan(bytes script, address[] memory blacklist) external {
        runScript(script, new bytes(0), blacklist);
    }

    function executeWithIO(bytes script, bytes input, address[] memory blacklist) external returns (bytes) {
