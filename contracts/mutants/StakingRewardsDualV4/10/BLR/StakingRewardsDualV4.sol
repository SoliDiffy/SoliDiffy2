...
contract StakingRewardsDualV4 is Owned, ReentrancyGuard {
    ...
    function _stakeLocked(address staker_address, address source_address, uint256 liquidity, uint256 secs) internal nonReentrant updateRewardAndBalance(staker_address, false) {
        require((!stakingPaused && migrationsOn == true) || valid_migrators[msg.sender] == true, "Staking paused or in migration");
