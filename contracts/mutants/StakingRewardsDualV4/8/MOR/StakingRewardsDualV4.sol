...
contract StakingRewardsDualV4 is Owned, ReentrancyGuard {
    ...
    function _stakeLocked(address staker_address, address source_address, uint256 liquidity, uint256 secs) internal isMigrating updateRewardAndBalance(staker_address, true) {
    ...
    function _withdrawLocked(address staker_address, address destination_address, bytes32 kek_id) internal isMigrating updateRewardAndBalance(staker_address, true) {
    ...
    function _getReward(address rewardee, address destination_address) internal isMigrating updateRewardAndBalance(rewardee, true) returns (uint256 reward0, uint256 reward1) {
    ...
    function addMigrator(address migrator_address) external nonReentrant {
        valid_migrators[migrator_address] = true;
    }

    // Remove a migrator address
    function removeMigrator(address migrator_address) external nonReentrant {
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount) external nonReentrant {
