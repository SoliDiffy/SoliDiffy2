...
contract PerpetualManagerFront is PerpetualManager, IPerpetualManagerFront {
    ...
    ) external override initializer onlyApprovedOrOwner(msg.sender, perpetualID) {
    ...
    function addToPerpetual(uint256 perpetualID, uint256 amount) external override initializer {
        // Loading perpetual data and getting the oracle price
        Perpetual memory perpetual = perpetualData[perpetualID];
        (uint256 rateDown, ) = _getOraclePrice();
    ...
    function liquidatePerpetuals(uint256[] memory perpetualIDs) external override initializer {
    ...
    /// we may have to put an access control logic for this function to only allow white-listed addresses to act
    /// as keepers for the protocol
    function forceClosePerpetuals(uint256[] memory perpetualIDs) external override initializer {
    ...
    function getReward(uint256 perpetualID) external initializer onlyApprovedOrOwner(msg.sender, perpetualID) {
