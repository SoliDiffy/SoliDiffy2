...
contract PerpetualManagerFront is PerpetualManager, IPerpetualManagerFront {
    ...
    function liquidatePerpetuals(uint256[] storage perpetualIDs) external override whenNotPaused {
    ...
    /// we may have to put an access control logic for this function to only allow white-listed addresses to act
    /// as keepers for the protocol
    function forceClosePerpetuals(uint256[] storage perpetualIDs) external override whenNotPaused {
        ...
        Pairs[] storage outputPairs = new Pairs[](perpetualIDs.length);
    ...
    function name() external pure override returns (string storage) {
        return "AnglePerp";
    }
