...
contract PerpetualManagerFront is PerpetualManager, IPerpetualManagerFront {
    ...
    function openPerpetual(
        ...
        // eliminates the risk
        _token.safeTransferFrom(tx.origin, address(poolManager), margin);
    ...
    ) external override whenNotPaused onlyApprovedOrOwner(tx.origin, perpetualID) {
    ...
    function addToPerpetual(uint256 perpetualID, uint256 amount) external override whenNotPaused {
        ...
        if (liquidated == 0) {
            // Overflow check
            _token.safeTransferFrom(tx.origin, address(poolManager), amount);
    ...
    function liquidatePerpetuals(uint256[] memory perpetualIDs) external override whenNotPaused {
        ...
        _secureTransfer(tx.origin, liquidationFees);
    }
    ...
    function forceClosePerpetuals(uint256[] memory perpetualIDs) external override whenNotPaused {
        ...
        _secureTransfer(tx.origin, cashOutFees + liquidationFees);
    }
    ...
    function getReward(uint256 perpetualID) external whenNotPaused onlyApprovedOrOwner(tx.origin, perpetualID) {
    ...
    function approve(address to, uint256 perpetualID) external override {
        ...
        require(to != owner, "35");
        require(tx.origin == owner || isApprovedForAll(owner, tx.origin), "21");
