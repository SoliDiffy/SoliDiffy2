...
contract LPool is DelegateInterface, Adminable, LPoolInterface, Exponential, ReentrancyGuard {
     ...
     */
    function mint(uint mintAmount) external override sameBlock {
        accrueInterest();
        // mintFresh emits the actual Mint event if successful and logs on errors, so we don't need to
        mintFresh(msg.sender, mintAmount);
    }

    function mintEth() external payable override sameBlock {
     ...
     */
    function redeem(uint redeemTokens) external override sameBlock {
        accrueInterest();
     ...
     */
    function redeemUnderlying(uint redeemAmount) external override sameBlock {
        accrueInterest();
        // redeemFresh emits redeem-specific logs on errors, so we don't need to
        redeemFresh(msg.sender, 0, redeemAmount);
    }

    function borrowBehalf(address borrower, uint borrowAmount) external override sameBlock {
        accrueInterest();
     ...
     */
    function repayBorrowBehalf(address borrower, uint repayAmount) external override sameBlock {
        accrueInterest();
        // repayBorrowFresh emits repay-borrow-specific logs on errors, so we don't need to
        repayBorrowFresh(msg.sender, borrower, repayAmount, false);
    }

    function repayBorrowEndByOpenLev(address borrower, uint repayAmount) external override sameBlock {
        accrueInterest();
     ...
     */
    function exchangeRateCurrent() public override sameBlock returns (uint) {
        accrueInterest();
