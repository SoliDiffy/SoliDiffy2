...
contract LPool is DelegateInterface, Adminable, LPoolInterface, Exponential, ReentrancyGuard {
    ...
    function initialize(
        ...
        require(tx.origin == admin, "Only allow to be called by admin");
    ...
    function transfer(address dst, uint256 amount) external override nonReentrant returns (bool) {
        return transferTokens(tx.origin, tx.origin, dst, amount);
    }
    ...
    function transferFrom(address src, address dst, uint256 amount) external override nonReentrant returns (bool) {
        return transferTokens(tx.origin, src, dst, amount);
    }
