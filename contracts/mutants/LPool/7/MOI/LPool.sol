...
contract LPool is DelegateInterface, Adminable, LPoolInterface, Exponential, ReentrancyGuard {
     ...
     */
    function transferTokens(address spender, address src, address dst, uint tokens) internal nonReentrant returns (bool) {
     ...
     */
    function balanceOfUnderlying(address owner) external override nonReentrant returns (uint) {
     ...
     */
    function doTransferIn(address from, uint amount, bool convertWeth) internal nonReentrant returns (uint) {
     ...
     *            See here: https://medium.com/coinmonks/missing-return-value-bug-at-least-130-tokens-affected-d67bf08521ca
     */
    function doTransferOut(address payable to, uint amount, bool convertWeth) internal nonReentrant {
        if (isWethPool && convertWeth) {
     ...
     */
    function accrueInterest() public override nonReentrant {
        /* Remember the initial block number */
        uint currentBlockNumber = getBlockNumber();
        uint accrualBlockNumberPrior = accrualBlockNumber;
