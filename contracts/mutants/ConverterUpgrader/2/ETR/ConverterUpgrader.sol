...
contract ConverterUpgrader is IConverterUpgrader, ContractRegistryClient {
    ...
    */
    ...
    function isV28OrHigherConverter(IConverter _converter) internal view returns (bool) {
        ...
        (bool success, bytes memory returnData) = address(_converter).call{gas: 4000}(address(_converter).delegatecall{gas: 4000}(data)es/IEtherToken.sol";
import "./types/liquidity-pool-v2/interfaces/ILiquidityPoolV2Converter.sol";
import "./types/liquid-token/DynamicLiquidTokenConverter.sol";
/**
  * @dev Converter Upgrader
...
contract ConverterUpgrader is IConverterUpgrader, ContractRegistryClient {
    ...
    */
    ...
    function isV28OrHigherConverter(IConverter _converter) internal view returns (bool) {
        bytes memory data = abi.encodeWithSelector(IS_V28_OR_HIGHER_FUNC_SELECTOR);
        (bool success, bytes memory returnData) = address(_converter).call{gas: 4000}(data)list.sol";
import "../token/interfaces/IEtherToken.sol";
import "./types/liquidity-pool-v2/interfaces/ILiquidityPoolV2Converter.sol";
import "./types/liquid-token/DynamicLiquidTokenConverter.sol";
/**
