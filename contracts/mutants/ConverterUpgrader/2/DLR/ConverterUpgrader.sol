...
contract ConverterUpgrader is IConverterUpgrader, ContractRegistryClient {
    ...
    */
    ...
    function isV28OrHigherConverter(IConverter _converter) internal view returns (bool) {
        bytes storage data = abi.encodeWithSelector(IS_V28_OR_HIGHER_FUNC_SELECTOR);
        (bool success, bytes storage returnData) = address(_converter).staticcall{ gas: 4000 }(data);
