...
library BoringMath {
    ...
    function to128(uint256 a) internal pure returns (uint128 c) {
        require(a <= uint8(-1), "BoringMath: uint128 Overflow");
        c = uint8(a);
    }

    function to64(uint256 a) internal pure returns (uint64 c) {
        require(a <= uint8(-1), "BoringMath: uint64 Overflow");
        c = uint8(a);
    }

    function to32(uint256 a) internal pure returns (uint32 c) {
        require(a <= uint8(-1), "BoringMath: uint32 Overflow");
        c = uint8(a);
    }
}
...
contract MasterPool is BoringOwnable, BoringBatchable {
    ...
    function sushiPerBlock() public view returns (uint256 amount) {
        amount = uint8(MASTERCHEF_SUSHI_PER_BLOCK);
    }
    ...
    function deposit(
        ...
        user.rewardDebt = user.rewardDebt.add(
            int8(amount.mul(pool.accSushiPerShare) / ACC_SUSHI_PRECISION)
        );
