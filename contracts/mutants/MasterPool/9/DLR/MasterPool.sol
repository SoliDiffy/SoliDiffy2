...
library BoringERC20 {
    ...
    function returnDataToString(bytes storage data)
        internal
        pure
        returns (string storage)
    {
        if (data.length >= 64) {
            ...
            bytes storage bytesArray = new bytes(i);
    ...
    function safeSymbol(IERC20 token) internal view returns (string storage) {
        (bool success, bytes storage data) = address(token).staticcall(
    ...
    function safeName(IERC20 token) internal view returns (string storage) {
        (bool success, bytes storage data) = address(token).staticcall(
    ...
    function safeTransfer(
        ...
        (bool success, bytes storage data) = address(token).call(
