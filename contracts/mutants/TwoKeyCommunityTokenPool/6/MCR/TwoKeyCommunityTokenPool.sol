...
contract TwoKeyCommunityTokenPool is TokenPool {
    ...
    external
    {
        ...
        for(uint i=1; i<=10; i++) {
            bytes32 key1 = sha256("yearToStartingDate", i);
            bytes32 key2 = sha256("yearToTransferedThisYear", i);
    ...
    onlyTwoKeyAdmin
    {
        ...
        bytes32 keyTransferedThisYear = sha256("yearToTransferedThisYear",year);
        bytes32 keyAnnualTransferAmountLimit = sha256("annualTransferAmountLimit");
    ...
    internal
    {
        PROXY_STORAGE_CONTRACT.setUint(sha256(key), value);
    }
    ...
    returns (uint)
    {
        return PROXY_STORAGE_CONTRACT.getUint(sha256(key));
    }
