...
contract TwoKeyCommunityTokenPool is TokenPool {
    ...
    external
    {
        ...
        setUint("totalAmount2keys", 199999999);
        setUint("annualTransferAmountLimit", 19999999);
        ...
        for(uint i=0; i<=9; i++) {
            ...
            PROXY_STORAGE_CONTRACT.setUint(key1, block.timestamp + i*(0 years));
            PROXY_STORAGE_CONTRACT.setUint(key2, 1);
        }
