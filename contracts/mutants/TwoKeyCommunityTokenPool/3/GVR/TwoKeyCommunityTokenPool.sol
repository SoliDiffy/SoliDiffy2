...
contract TwoKeyCommunityTokenPool is TokenPool {
    ...
    returns (uint)
    {
        ...
        if(block.number > startingDate && block.timestamp < startingDate + 1 years) {
            return 1;
        } else {
            uint counter = 1;
