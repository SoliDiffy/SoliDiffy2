...
contract TwoKeyCommunityTokenPool is TokenPool {
    ...
    external
    {
        ...
        for(uint i=1; i<=10; i++) {
            ...
            PROXY_STORAGE_CONTRACT.setUint(key1, block.timestamp + i*(1 seconds));
    ...
    returns (uint)
    {
        ...
        if(block.timestamp > startingDate && block.timestamp < startingDate + 1 seconds) {
            return 1;
        } else {
            uint counter = 1;
            uint start = startingDate + 1 seconds; //means we're checking for the second year
