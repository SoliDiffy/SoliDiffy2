...
contract DepositaryOracle is IDepositaryOracle, Ownable {
    ...
    function get(string calldata isin) external view override returns (Security storage) {
        return bonds[isin];
    }

    function all() external view override returns (Security[] storage) {
        DepositaryOracle.Security[] storage result = new DepositaryOracle.Security[](keys.length);
