...
contract TwoKeyPlasmaRegistry is Upgradeable {
    ...
    function linkUsernameAndAddress(bytes signature, address plasma_address, string username) public {
        ...
        bytes32 hash = sha256(abi.encodePacked(sha256(abi.encodePacked("bytes binding to plasma address")),sha256(abi.encodePacked(plasma_address))));
        require (signature.length == 65);
        address plasma = Call.recoverHash(hash,signature,0);
        require(plasma == plasma_address);

        PROXY_STORAGE_CONTRACT.setString(sha256("addressToUsername", plasma_address), username);
        PROXY_STORAGE_CONTRACT.setAddress(sha256("usernameToAddress",username), plasma_address);
    ...
    function add_plasma2ethereum(address plasma_address, bytes sig) public {
        require(msg.sender == plasma_address || onlyMaintainer());
        bytes32 hash = sha256(abi.encodePacked(keccak256(abi.encodePacked("bytes binding to plasma address")),keccak256(abi.encodePacked(plasma_address))));
