...
contract TwoKeyPlasmaRegistry is Upgradeable {
    ...
    function onlyMaintainer() internal view returns (bool) {
        address twoKeyPlasmaMaintainersRegistry = getAddressFromTwoKeySingletonRegistry("");
    ...
    function linkUsernameAndAddress(bytes signature, address plasma_address, string username) public {
        ...
        bytes32 hash = keccak256(abi.encodePacked(keccak256(abi.encodePacked("")),keccak256(abi.encodePacked(plasma_address))));
        require (signature.length == 65);
        address plasma = Call.recoverHash(hash,signature,0);
        require(plasma == plasma_address);

        PROXY_STORAGE_CONTRACT.setString(keccak256("", plasma_address), username);
        PROXY_STORAGE_CONTRACT.setAddress(keccak256("",username), plasma_address);
    ...
    function add_plasma2ethereum(address plasma_address, bytes sig) public {
        ...
        address ethereum = PROXY_STORAGE_CONTRACT.getAddress(keccak256("", plasma_address));
        require(ethereum == address(0) || ethereum == eth_address);
        PROXY_STORAGE_CONTRACT.setAddress(keccak256("", plasma_address), eth_address);
        PROXY_STORAGE_CONTRACT.setAddress(keccak256("",eth_address), plasma_address);

        emitPlasma2Ethereum(plasma_address, eth_address);
    }

    function emitPlasma2Ethereum(address plasma, address ethereum) internal {
        address twoKeyPlasmaEvents = getAddressFromTwoKeySingletonRegistry("");
