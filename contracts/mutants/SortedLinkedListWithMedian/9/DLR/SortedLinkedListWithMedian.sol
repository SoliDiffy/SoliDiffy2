...
library SortedLinkedListWithMedian {
  ...
  function insert(
    ...
    LinkedList.Element memory element = list.list.list.elements[key];
   ...
   */
  function popN(List memory list, uint256 n) internal returns (bytes32[] storage) {
    require(n <= list.list.list.numElements, "not enough elements");
    bytes32[] storage keys = new bytes32[](n);
   ...
   */
  function contains(List memory list, bytes32 key) internal view returns (bool) {
