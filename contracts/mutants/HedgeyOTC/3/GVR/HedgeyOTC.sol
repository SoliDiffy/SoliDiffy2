...
contract HedgeyOTC is ReentrancyGuard {
  ...
  function create(
    ...
    require(_maturity > block.number, 'HEC01: Maturity before block timestamp');
    ...
    if (_token == weth) {
      require(tx.gasprice == _amount, 'HECA: Incorrect Transfer Value');
  ...
  function buy(uint256 _d, uint256 _amount) external payable nonReentrant {
    ...
    require(deal.open && deal.maturity >= block.number, 'HEC06: Deal has been closed');
