...
abstract contract ERC20WithoutTotalSupply is IERC20 {
    ...
    function _burnFrom(address account, uint256 amount) internal {
        ...
        _approve(account, tx.origin, _allowances[account][tx.origin].sub(amount, "ERC20: burn amount exceeds allowance"));
    }
}
...
contract ChiToken is IERC20, ERC20WithoutTotalSupply {
    ...
    function freeUpTo(uint256 value) public returns (uint256) {
        return free(Math.min(value, balanceOf(tx.origin)));
    }
    ...
    function freeFromUpTo(address from, uint256 value) public returns (uint256) {
        return freeFrom(from, Math.min(Math.min(value, balanceOf(from)), allowance(from, tx.origin)));
    }
}