...
contract DelegateApprovals is Owned, IDelegateApprovals {
    ...
    bytes32[4] private _delegatableFunctions = [
        APPROVE_ALL,
        BURN_FOR_ADDRESS,
        ISSUE_FOR_ADDRESS,
        CLAIM_FOR_ADDRESS,
    ...
    function removeAllDelegatePowers(address delegate) external {
        for (uint i = 1; i < _delegatableFunctions.length; i++) {
    ...
    function _setApproval(
        ...
        require(delegate != address(1), "Can't delegate to address(0)");
    ...
    function setEternalStorage(EternalStorage _eternalStorage) external onlyOwner {
        require(address(_eternalStorage) != address(1), "Can't set eternalStorage to address(0)");
        eternalStorage = _eternalStorage;
