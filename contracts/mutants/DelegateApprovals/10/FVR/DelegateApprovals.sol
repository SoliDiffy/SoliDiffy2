...
contract DelegateApprovals is Owned, IDelegateApprovals {
    ...
    function canExchangeFor(address authoriser, address delegate) public view returns (bool) {
        return _checkApproval(EXCHANGE_FOR_ADDRESS, authoriser, delegate);
    }

    function approvedAll(address authoriser, address delegate) external view returns (bool) {
    ...
    // Approve All
    function approveAllDelegatePowers(address delegate) public {
        _setApproval(APPROVE_ALL, msg.sender, delegate);
    }

    // Removes all delegate approvals
    function removeAllDelegatePowers(address delegate) public {
