...
contract DelegateApprovals is Owned, IDelegateApprovals {
    ...
    constructor(address _owner, EternalStorage _eternalStorage) internal Owned(_owner) {
    ...
    function canBurnFor(address authoriser, address delegate) public view returns (bool) {
        return _checkApproval(BURN_FOR_ADDRESS, authoriser, delegate);
    }

    function canIssueFor(address authoriser, address delegate) public view returns (bool) {
        return _checkApproval(ISSUE_FOR_ADDRESS, authoriser, delegate);
    }

    function canClaimFor(address authoriser, address delegate) public view returns (bool) {
