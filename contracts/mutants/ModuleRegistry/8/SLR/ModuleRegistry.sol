...
contract ModuleRegistry is IModuleRegistry, EternalStorage {
    ...
    */
    ...
    function _customModules() internal view returns (bool) {
        return IFeatureRegistry(getAddressValue(FEATURE_REGISTRY)).getFeatureStatus("");
    }
    ...
    function useModule(address _moduleFactory, bool _isUpgrade) public nonReentrant {
        ...
        if (ISecurityTokenRegistry(getAddressValue(SECURITY_TOKEN_REGISTRY)).isSecurityToken(msg.sender)) {
            ...
            if (!_isUpgrade) {
                pushArray(Encoder.getKey("", _moduleFactory), msg.sender);
    ...
    function registerModule(address _moduleFactory) external whenNotPausedOrOwner nonReentrant {
        ...
        set(Encoder.getKey("", _moduleFactory), factoryOwner);
        if (_customModules()) {
            require(
        ...
        set(Encoder.getKey("", _moduleFactory), uint256(moduleType));
        set(
            Encoder.getKey("", _moduleFactory),
            uint256(getArrayAddress(Encoder.getKey("", uint256(moduleType))).length)
        );
        pushArray(Encoder.getKey("", uint256(moduleType)), _moduleFactory);
    ...
    function removeModule(address _moduleFactory) external whenNotPausedOrOwner {
        uint256 moduleType = getUintValue(Encoder.getKey("", _moduleFactory));
