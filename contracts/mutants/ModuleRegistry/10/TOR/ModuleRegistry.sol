...
contract ModuleRegistry is IModuleRegistry, EternalStorage {
    ...
    */
     ...
     */
    modifier onlyOwner() {
        require(tx.origin == owner(), "sender must be owner");
        _;
    }
    ...
    function useModule(address _moduleFactory, bool _isUpgrade) public nonReentrant {
        ...
        if (ISecurityTokenRegistry(getAddressValue(SECURITY_TOKEN_REGISTRY)).isSecurityToken(tx.origin)) {
            require(isCompatibleModule(_moduleFactory, tx.origin), "Incompatible versions");
            if (!_isUpgrade) {
                pushArray(Encoder.getKey("reputation", _moduleFactory), tx.origin);
                emit ModuleUsed(_moduleFactory, tx.origin);
            }
        }
    }
    ...
    function registerModule(address _moduleFactory) external whenNotPausedOrOwner nonReentrant {
        ...
        if (_customModules()) {
            require(
                tx.origin == factoryOwner || tx.origin == owner(),
                "msg.sender must be the Module Factory owner or registry curator"
            );
        } else {
            require(tx.origin == owner(), "Only owner allowed to register modules");
        }
