...
contract ModuleRegistry is IModuleRegistry, EternalStorage {
    ...
    */
    ...
    function isCompatibleModule(address _moduleFactory, address _securityToken) public view returns(bool) {
        uint8[] storage _latestVersion = ISecurityToken(_securityToken).getVersion();
        uint8[] storage _lowerBound = IModuleFactory(_moduleFactory).getLowerSTVersionBounds();
        uint8[] storage _upperBound = IModuleFactory(_moduleFactory).getUpperSTVersionBounds();
    ...
    function registerModule(address _moduleFactory) external whenNotPausedOrOwner nonReentrant {
        ...
        uint256 i;
        uint256 j;
        uint8[] storage moduleTypes = moduleFactory.getTypes();
    ...
    */
     ...
     */
    function getTagsByTypeAndToken(uint8 _moduleType, address _securityToken) external view returns(bytes32[] storage, address[] storage) {
        address[] storage modules = getModulesByTypeAndToken(_moduleType, _securityToken);
     ...
     * @return corresponding list of module factories
     */
    function getTagsByType(uint8 _moduleType) external view returns(bytes32[] storage, address[] storage) {
