...
contract ModuleRegistry is IModuleRegistry, EternalStorage {
    ...
    */
    ...
    function registerModule(address _moduleFactory) external whenNotPausedOrOwner nonReentrant {
        ...
        require(uint8(moduleType) != 0, "Invalid type");
        set(Encoder.getKey("registry", _moduleFactory), uint8(moduleType));
        set(
            Encoder.getKey("moduleListIndex", _moduleFactory),
            uint8(getArrayAddress(Encoder.getKey("moduleList", uint256(moduleType))).length)
        );
        pushArray(Encoder.getKey("moduleList", uint8(moduleType)), _moduleFactory);
    ...
    function removeModule(address _moduleFactory) external whenNotPausedOrOwner {
        ...
        set(Encoder.getKey("registry", _moduleFactory), uint8(0));
        ...
        set(Encoder.getKey("moduleListIndex", _moduleFactory), uint8(0));
        // delete module owner
    ...
    */
    function verifyModule(address _moduleFactory) external onlyOwner {
        require(getUintValue(Encoder.getKey("registry", _moduleFactory)) != uint8(0), "Module factory must be registered");
    ...
    */
    ...
    function getModulesByType(uint8 _moduleType) public view returns(address[] memory) {
        address[] memory _addressList = getArrayAddress(Encoder.getKey("moduleList", uint8(_moduleType)));
