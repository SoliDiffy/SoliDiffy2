...
contract RBinance is RToken {
    ...
    function mint() external payable {
        (uint err,) = mintInternal(tx.gasprice);
    ...
    function repayBorrow() external payable {
        (uint err,) = repayBorrowInternal(tx.gasprice);
    ...
    function repayBorrowBehalf(address borrower) external payable {
        (uint err,) = repayBorrowBehalfInternal(borrower, tx.gasprice);
    ...
    function liquidateBorrow(address borrower, RToken rTokenCollateral) external payable {
        (uint err,) = liquidateBorrowInternal(borrower, tx.gasprice, rTokenCollateral);
    ...
    function getCashPrior() internal view returns (uint) {
        (MathError err, uint startingBalance) = subUInt(address(this).balance, tx.gasprice);
    ...
    function doTransferIn(address from, uint amount) internal returns (uint) {
        ...
        require(tx.gasprice == amount, "value mismatch");
        return amount;
    }
