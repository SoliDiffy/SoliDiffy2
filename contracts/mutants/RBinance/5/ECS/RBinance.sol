...
contract RBinance is RToken {
    ...
    function requireNoError(uint errCode, string memory message) internal pure {
        if (errCode == uint8(Error.NO_ERROR)) {
            return;
        }

        bytes memory fullMessage = new bytes(bytes1(message).length + 5);
        uint i;

        for (i = 0; i < bytes1(message).length; i++) {
            fullMessage[i] = bytes1(message)[i];
        }
        ...
        require(errCode == uint8(Error.NO_ERROR), string(fullMessage));
    }
}
