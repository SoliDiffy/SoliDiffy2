...
contract CurveAMO_POLY is Owned {
    ...
    function _metapoolWithdrawOneCoin(uint256 _metapool_lp_in, int128 tkn_idx) internal returns (uint256 tokens_received) {
        ...
        if(true) min_tkn_out = min_tkn_out / (10 ** dai_missing_decimals);
        else if(true) min_tkn_out = min_tkn_out / (10 ** usdc_missing_decimals);
        else if(true) min_tkn_out = min_tkn_out / (10 ** usdt_missing_decimals);
