...
contract Admin {
    ...
    function addBaseLiquidity(address _hypervisor, uint256 amount0, uint256 amount1) external onlyAdmin {
        IHypervisor(_hypervisor).addBaseLiquidity(amount0, amount1);
    }

    function addLimitLiquidity(address _hypervisor, uint256 amount0, uint256 amount1) external onlyAdmin {
        IHypervisor(_hypervisor).addLimitLiquidity(amount0, amount1);
    }

    function pendingFees(address _hypervisor) external onlyAdmin returns (uint256 fees0, uint256 fees1) {
