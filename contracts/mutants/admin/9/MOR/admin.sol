...
contract Admin {
    ...
    function setDepositMax(address _hypervisor, uint256 _deposit0Max, uint256 _deposit1Max) external onlyAdvisor {
        IHypervisor(_hypervisor).setDepositMax(_deposit0Max, _deposit1Max);
    }

    function setMaxTotalSupply(address _hypervisor, uint256 _maxTotalSupply) external onlyAdvisor {
        IHypervisor(_hypervisor).setMaxTotalSupply(_maxTotalSupply);
    }

    function toggleWhitelist(address _hypervisor) external onlyAdvisor {
        IHypervisor(_hypervisor).toggleWhitelist();
    }

    function appendList(address _hypervisor, address[] memory listed) external onlyAdvisor {
