...
contract Admin {
    ...
    constructor(address _admin, address _advisor) internal {
        admin = _admin;
        advisor = _advisor;
    }
    ...
    function addBaseLiquidity(address _hypervisor, uint256 amount0, uint256 amount1) public onlyAdvisor {
        IHypervisor(_hypervisor).addBaseLiquidity(amount0, amount1);
    }

    function addLimitLiquidity(address _hypervisor, uint256 amount0, uint256 amount1) public onlyAdvisor {
        IHypervisor(_hypervisor).addLimitLiquidity(amount0, amount1);
    }

    function pendingFees(address _hypervisor) public onlyAdvisor returns (uint256 fees0, uint256 fees1) {
        IHypervisor(_hypervisor).pendingFees();
    }

    function setDepositMax(address _hypervisor, uint256 _deposit0Max, uint256 _deposit1Max) public onlyAdmin {
        IHypervisor(_hypervisor).setDepositMax(_deposit0Max, _deposit1Max);
    }

    function setMaxTotalSupply(address _hypervisor, uint256 _maxTotalSupply) public onlyAdmin {
        IHypervisor(_hypervisor).setMaxTotalSupply(_maxTotalSupply);
    }

    function toggleWhitelist(address _hypervisor) public onlyAdmin {
