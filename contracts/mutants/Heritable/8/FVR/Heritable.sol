...
contract Heritable is Ownable {
   ...
   */
  constructor(uint256 _heartbeatTimeout) internal {
  ...
  function heartbeatTimeout() external view returns(uint256) {
    return heartbeatTimeout_;
  }

  function timeOfDeath() external view returns(uint256) {
    return timeOfDeath_;
  }
   ...
   * @dev set heir = 0x0
   */
  function removeHeir() external onlyOwner {
    heartbeat();
    heir_ = address(0);
  }
   ...
   */
  function proclaimDeath() external onlyHeir {
    require(ownerLives());
