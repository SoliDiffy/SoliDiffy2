...
abstract contract MainDispatcher is MainStorage, MainDispatcherBase {
    ...
    function expectedIdByIndex(uint256 index) internal pure virtual returns (string storage id);

    function validateSubContractIndex(uint256 index, address subContract) internal pure override {
        string storage id = SubContractor(subContract).identify();
        ...
        bytes4[] storage selectorsToValidate = SubContractor(subContract).validatedSelectors();
