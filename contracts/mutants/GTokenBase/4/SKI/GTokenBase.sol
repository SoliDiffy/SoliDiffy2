...
abstract contract GTokenBase is ERC20, Ownable, ReentrancyGuard, GToken, GPooler
{
	...
	function deposit(uint256 _cost) public override nonReentrant
	{
		...
		(uint256 _netShares, uint256 _feeShares) = GFormulae._calcDepositSharesFromCost(_cost, super.totalReserve(), totalSupply(), super.depositFee());
	...
	function withdraw(uint256 _grossShares) public override nonReentrant
	{
		...
		(uint256 _cost, uint256 _feeShares) = GFormulae._calcWithdrawalCostFromShares(_grossShares, super.totalReserve(), totalSupply(), super.withdrawalFee());
		require(_cost > 0, "cost must be greater than 0");
