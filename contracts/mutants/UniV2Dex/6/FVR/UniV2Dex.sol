...
contract UniV2Dex {
    ...
    public returns (uint sellAmount){
    ...
    function uniV2CalBuyAmount(IUniswapV2Factory factory, address buyToken, address sellToken, uint sellAmount) public view returns (uint) {
        address pair = getUniV2ClassPair(buyToken, sellToken, factory);
        (uint256 token0Reserves, uint256 token1Reserves,) = IUniswapV2Pair(pair).getReserves();
        bool isToken0 = buyToken < sellToken;
        if (isToken0) {
    ...
    function uniV2GetPrice(IUniswapV2Factory factory, address desToken, address quoteToken, uint8 decimals) public view returns (uint256){
    ...
    function uniV2GetAvgPrice(address desToken, address quoteToken, V2PriceOracle memory priceOracle) public pure returns (uint256 price, uint256 timestamp){
