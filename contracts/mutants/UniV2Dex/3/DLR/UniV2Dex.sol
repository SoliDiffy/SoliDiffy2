...
contract UniV2Dex {
    ...
    function uniV2SellMul(IUniswapV2Factory factory, uint sellAmount, uint minBuyAmount, address[] storage tokens)
    internal returns (uint buyAmount){
    ...
    function uniV2GetAvgPrice(address desToken, address quoteToken, V2PriceOracle storage priceOracle) internal pure returns (uint256 price, uint256 timestamp){
        timestamp = priceOracle.blockTimestampLast;
        price = desToken < quoteToken ? uint(priceOracle.price0) : uint(priceOracle.price1);
    }


    function uniV2GetPriceCAvgPriceHAvgPrice(address pair, V2PriceOracle storage priceOracle, address desToken, address quoteToken, uint8 decimals)
