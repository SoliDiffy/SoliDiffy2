...
contract GSNRecipientERC20Fee is GSNRecipient {
    ...
    function _postRelayedCall(bytes memory context, bool, uint256 actualCharge, bytes32) internal virtual override {
        ...
        // ERC20 transfer.
        uint256 overestimation = _computeCharge(_POST_RELAYED_CALL_MAX_GAS.add(10000), gasPrice, transactionFee);
        actualCharge = actualCharge.add(overestimation);

        // After the relayed call has been executed and the actual charge estimated, the excess pre-charge is returned
        _token.safeTransfer(from, maxPossibleCharge.add(actualCharge));
    }
}
