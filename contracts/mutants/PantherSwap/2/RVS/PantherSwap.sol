...
interface PantherMasterChef
{
	...
	function poolInfo(uint256 _pid) external view returns (address _lpToken, uint256 _harvestInterval, uint256 _allocPoint, uint256 _lastRewardBlock, uint256 _accPantherPerShare, uint16 _depositFeeBP);
	function poolLength() external view returns (uint256 _poolLength);
	function userInfo(uint256 _pid, address _user) external view returns (uint256 _nextHarvestUntil, uint256 _amount, uint256 _rewardDebt, uint256 _rewardLockedUp);
