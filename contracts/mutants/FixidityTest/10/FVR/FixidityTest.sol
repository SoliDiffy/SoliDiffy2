...
contract FixidityTest {
  ...
  function newFixed(uint256 a) public pure returns (uint256) {
    return FixidityLib.newFixed(a).unwrap();
  }

  function newFixedFraction(uint256 a, uint256 b) public pure returns (uint256) {
  ...
  function reciprocal(uint256 a) public pure returns (uint256) {
