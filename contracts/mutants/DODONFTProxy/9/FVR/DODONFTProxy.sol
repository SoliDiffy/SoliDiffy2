...
contract DODONFTProxy is ReentrancyGuard, InitializableOwnable {
    ...
    function createNFTCollateralVault(string memory name, string memory baseURI) public returns (address newVault) {
    ...
    ) public returns (address newFragment, address newDvm) {
    ...
    function buyout(
        ...
        uint256 deadLine
    ) public payable preventReentrant judgeExpired(deadLine) {
        if(flag == 0)
    ...
    //============= Owner ===================
    function updateVaultTemplate(address newVaultTemplate) public onlyOwner {
        _VAULT_TEMPLATE_ = newVaultTemplate;
        emit ChangeVaultTemplate(newVaultTemplate);
    }

    function updateFragTemplate(address newFragTemplate) public onlyOwner {
        _FRAG_TEMPLATE_ = newFragTemplate;
        emit ChangeFragTemplate(newFragTemplate);
    }

    function updateMtFeeRateTemplate(address newMtFeeRateTemplate) public onlyOwner {
        _MT_FEE_RATE_MODEL_ = newMtFeeRateTemplate;
        emit ChangeMtFeeRateTemplate(newMtFeeRateTemplate);
    }

    function updateDvmTemplate(address newDvmTemplate) public onlyOwner {
        _DVM_TEMPLATE_ = newDvmTemplate;
        emit ChangeDvmTemplate(newDvmTemplate);
    }

    function updateBuyoutModel(address newBuyoutModel) public onlyOwner {
