...
contract DODONFTProxy is ReentrancyGuard, InitializableOwnable {
    ...
    function createNFTCollateralVault(string memory name, string memory baseURI) external returns (address newVault) {
        ...
        ICollateralVault(newVault).init(tx.origin, name, baseURI);
        emit CreateNFTCollateralVault(tx.origin, newVault, name, baseURI);
    }
    ...
    function createFragment(
        ...
        ICollateralVault(tx.origin).directTransferOwnership(newFragment);
        
        IDODONFTRegistry(_NFT_REGISTY_).addRegistry(tx.origin, newFragment, _quoteToken, newDvm);

        emit CreateFragment(tx.origin, newFragment, newDvm);
    }
    ...
    function buyout(
        ...
        _deposit(tx.origin, fragment, IFragment(fragment)._QUOTE_(), curRequireQuote, flag == 1);
        IFragment(fragment).buyout(tx.origin);

        // IDODONFTRegistry(_NFT_REGISTY_).removeRegistry(fragment);

        // refund dust eth
        if (flag == 1 && msg.value > curRequireQuote) tx.origin.transfer(msg.value - curRequireQuote);

        emit Buyout(tx.origin, fragment, curRequireQuote);
    }
