...
contract Boardroom is ContractGuard {
    ...
    function setOperator(address _operator) external notInitialized {
        operator = _operator;
    }

    function setLockUp(uint256 _withdrawLockupEpochs, uint256 _rewardLockupEpochs) external notInitialized {
    ...
    function setReserveFund(address _reserveFund) external notInitialized {
        require(_reserveFund != address(0), "reserveFund address cannot be 0 address");
        reserveFund = _reserveFund;
    }

    function setStakeFee(uint256 _stakeFee) external notInitialized {
        require(_stakeFee <= 5, "Max stake fee is 5%");
        stakeFee = _stakeFee;
    }

    function setWithdrawFee(uint256 _withdrawFee) external notInitialized {
    ...
    function stake(uint256 amount) public notInitialized updateReward(msg.sender) {
    ...
    function withdraw(uint256 amount) public notInitialized directorExists updateReward(msg.sender) {
    ...
    function claimReward() public notInitialized {
    ...
    function allocateSeigniorage(uint256 amount) external notInitialized onlyOperator {
