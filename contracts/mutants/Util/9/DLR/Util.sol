...
library Util {
    ...
    */
    function upper(string storage _base) internal pure returns(string storage) {
        bytes storage _baseBytes = bytes(_base);
    ...
    function stringToBytes32(string storage _source) internal pure returns(bytes32) {
     ...
     */
    /// Notice - Maximum length for _source will be 32 chars otherwise returned bytes32 value will have lossy value.
    function bytesToBytes32(bytes storage _b, uint _offset) internal pure returns(bytes32) {
        bytes32 result;
     ...
     */
    function bytes32ToString(bytes32 _source) internal pure returns(string storage) {
        bytes storage bytesString = new bytes(32);
        uint charCount = 0;
        uint j = 0;
        for (j = 0; j < 32; j++) {
        ...
        bytes storage bytesStringTrimmed = new bytes(charCount);
     ...
     * @return bytes4 sig
     */
    function getSig(bytes storage _data) internal pure returns(bytes4 sig) {
