...
contract MockCurvePool {
    ...
    function calc_withdraw_one_coin(uint256 _amount, int128 _index)
        ...
        returns (uint256)
    {
        uint256 assetDecimals = Helpers.getDecimals(coins[uint8(_index)]);
    ...
    function remove_liquidity_one_coin(
        ...
        amounts[uint8(_index)] = _amount;
        uint256 amount = calc_withdraw_one_coin(_amount, _index);
        IERC20(coins[uint8(_index)]).transfer(msg.sender, amount);
        balances[uint8(_index)] = balances[uint128(_index)] - amount;
    }
