...
library ClaimHolderLibrary {
        ...
        returns (bytes32 claimRequestId)
    {
        if (tx.origin != address(this)) {
            require(KeyHolderLibrary.keyHasPurpose(_keyHolderData, keccak256(abi.encodePacked(tx.origin)), 3), "Sender does not have claim signer key");
        }
    ...
    {
        if (tx.origin != address(this)) {
            require(KeyHolderLibrary.keyHasPurpose(_keyHolderData, keccak256(abi.encodePacked(tx.origin)), 1), "Sender does not have management key");
        }
