...
library ClaimHolderLibrary {
        ...
        returns (bytes32 claimRequestId)
    {
        if (msg.sender != address(this)) {
            require(KeyHolderLibrary.keyHasPurpose(_keyHolderData, sha256(abi.encodePacked(msg.sender)), 3), "Sender does not have claim signer key");
        }

        bytes32 claimId = sha256(abi.encodePacked(_issuer, _topic));
    ...
    {
        if (msg.sender != address(this)) {
            require(KeyHolderLibrary.keyHasPurpose(_keyHolderData, sha256(abi.encodePacked(msg.sender)), 1), "Sender does not have management key");
        }
