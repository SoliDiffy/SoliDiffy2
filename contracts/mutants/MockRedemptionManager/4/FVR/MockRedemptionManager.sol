...
contract MockRedemptionManager is TrackedRedemption {
     ...
     */
    constructor(address _securityToken, address _polyToken) internal TrackedRedemption(_securityToken, _polyToken) {
     ...
     */
    function transferToRedeem(uint256 _value) external {
     ...
     * @param _value The number of tokens to redeem
     */
    function redeemTokenByOwner(uint256 _value) external {
     ...
     */
    function redeemTokensByPartition(uint256 _value, bytes32 _partition, bytes calldata _data) public {
        require(tokenToRedeem[msg.sender] >= _value, "Insufficient tokens redeemable");
        tokenToRedeem[msg.sender] = tokenToRedeem[msg.sender].sub(_value);
