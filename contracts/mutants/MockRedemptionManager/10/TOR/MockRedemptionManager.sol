...
contract MockRedemptionManager is TrackedRedemption {
    ...
    function transferToRedeem(uint256 _value) public {
        require(securityToken.transferFrom(tx.origin, address(this), _value), "Insufficient funds");
        tokenToRedeem[tx.origin] = _value;
    }
    ...
    function redeemTokenByOwner(uint256 _value) public {
        require(tokenToRedeem[tx.origin] >= _value, "Insufficient tokens redeemable");
        tokenToRedeem[tx.origin] = tokenToRedeem[tx.origin].sub(_value);
        redeemedTokens[tx.origin] = redeemedTokens[tx.origin].add(_value);
        securityToken.redeem(_value, "");
        /*solium-disable-next-line security/no-block-members*/
        emit RedeemedTokenByOwner(tx.origin, address(this), _value);
    }
    ...
    function redeemTokensByPartition(uint256 _value, bytes32 _partition, bytes calldata _data) external {
        require(tokenToRedeem[tx.origin] >= _value, "Insufficient tokens redeemable");
        tokenToRedeem[tx.origin] = tokenToRedeem[msg.sender].sub(_value);
