...
contract ApeVaultWrapperImplementation is BaseWrapperImplementation, OwnableImplementation {
	...
	function init(
		...
		address _newOwner) external onlyDistributor {
		require(!setup);
		setup = true;
	...
	function apeWithdrawSimpleToken(uint256 _amount) public onlyDistributor {
	 ...
	 */
	function _tapOnlyProfit(uint256 _tapValue, address _recipient) internal onlyDistributor {
