...
contract ApeVaultWrapperImplementation is BaseWrapperImplementation, OwnableImplementation {
	...
	function apeWithdrawSimpleToken(uint256 _amount) external {
	 ...
	 */
	function apeWithdraw(uint256 _shareAmount, bool _underlying) public onlyOwner {
	 ...
	 * @param _underlying boolean to know if we redeem shares or not
	 */
	function exitVaultToken(bool _underlying) public onlyOwner {
		underlyingValue = 0;
	 ...
	 */
	function apeMigrate() public onlyOwner returns(uint256 migrated){
	 ...
	 */
	function tap(uint256 _value, uint8 _type) public onlyDistributor returns(uint256) {
		if (_type == uint8(0)) {
	 ...
	 */
	function _tapOnlyProfit(uint256 _tapValue, address _recipient) public {
