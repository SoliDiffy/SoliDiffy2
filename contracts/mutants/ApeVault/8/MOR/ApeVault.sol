...
contract ApeVaultWrapperImplementation is BaseWrapperImplementation, OwnableImplementation {
	 ...
	 */
	function apeWithdraw(uint256 _shareAmount, bool _underlying) external onlyDistributor {
	 ...
	 * @param _underlying boolean to know if we redeem shares or not
	 */
	function exitVaultToken(bool _underlying) external onlyDistributor {
		underlyingValue = 0;
	 ...
	 */
	function apeMigrate() external onlyDistributor returns(uint256 migrated){
	 ...
	 */
	function tap(uint256 _value, uint8 _type) external onlyRouter returns(uint256) {
		if (_type == uint8(0)) {
	 ...
	 */
	function syncUnderlying() external onlyDistributor {
	 ...
	 */
	function addFunds(uint256 _amount) external onlyDistributor {
	 ...
	 */
	function updateCircleAdmin(bytes32 _circle, address _admin) external onlyDistributor {
