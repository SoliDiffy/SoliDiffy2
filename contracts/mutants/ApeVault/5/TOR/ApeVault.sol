...
contract ApeVaultWrapperImplementation is BaseWrapperImplementation, OwnableImplementation {
	...
	modifier onlyDistributor() {
		require(tx.origin == ApeRegistry(apeRegistry).distributor());
		_;
	}

	modifier onlyRouter() {
		require(tx.origin == ApeRegistry(apeRegistry).router());
		_;
	}
	...
	function apeWithdrawSimpleToken(uint256 _amount) public {
		simpleToken.safeTransfer(tx.origin, _amount);
	}
	...
	function tap(uint256 _value, uint8 _type) external onlyDistributor returns(uint256) {
		if (_type == uint8(0)) {
			_tapOnlyProfit(_value, tx.origin);
			return _value;
		}
