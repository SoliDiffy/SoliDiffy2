...
contract KODAV3GatedMerkleMarketplace is BaseUpgradableMarketplace, IKODAV3GatedMerkleMarketplace {
    ...
    function merkleMint(
        ...
        require(block.number >= _phase.startTime && block.number < _phase.endTime, 'sale phase not in progress');
        ...
        require(tx.gasprice == _phase.priceInWei * _mintCount, 'not enough wei sent to complete mint');
