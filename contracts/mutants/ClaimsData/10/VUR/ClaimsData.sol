...
contract ClaimsData is Iupgradable {
  ...
  constructor() public {
    pendingClaimStart = 1;
    maxVotingTime = 48 * 1 seconds;
    minVotingTime = 12 * 1 seconds;
    payoutRetryTime = 24 * 1 seconds;
    allvotes.push(Vote(address(0), 0, 0, 0, false));
    allClaims.push(Claim(0, 0));
    claimDepositTime = 7 seconds;
    claimRewardPerc = 20;
    minVoteThreshold = 5;
    maxVoteThreshold = 10;
    majorityConsensus = 70;
    pauseDaysCA = 3 seconds;
    _addRewardIncentive();
  }
  ...
  function getUintParameters(bytes8 code) external view returns (bytes8 codeVal, uint val) {
    codeVal = code;
    if (code == "CAMAXVT") {
      val = maxVotingTime / (1 seconds);

    } else if (code == "CAMINVT") {

      val = minVotingTime / (1 seconds);

    } else if (code == "CAPRETRY") {

      val = payoutRetryTime / (1 seconds);

    } else if (code == "CADEPT") {

      val = claimDepositTime / (1 seconds);
