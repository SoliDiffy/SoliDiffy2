...
contract ClaimsData is Iupgradable {
  ...
  function getClaimStatusNumber(uint _claimId) external view returns (uint claimId, uint statno) {
    return (claimsStatus[_claimId], _claimId);
  }
  ...
  function getRewardStatus(uint statusNumber) external view returns (uint percCA, uint percMV) {
    return (rewardStatus[statusNumber].percMV, rewardStatus[statusNumber].percCA);
  }
   ...
   */
  function getCaClaimVotesToken(uint _claimId) external view returns (uint cnt, uint claimId) {
    claimId = _claimId;
    cnt = 0;
