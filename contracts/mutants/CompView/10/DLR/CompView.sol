...
contract CompView is Exponential, DSMath {
    ...
    function getSafetyRatio(address _user) public view returns (uint) {
        ...
        address[] storage assets = comp.getAssetsIn(_user);
        ...
        for (uint i = 0; i < assets.length; i++) {
            ...
            if (cTokenBalance != 0) {
                ...
                Exp storage collateralFactor = Exp({mantissa: collFactorMantissa});
                Exp storage exchangeRate = Exp({mantissa: exchangeRateMantissa});

                (, Exp storage tokensToUsd) = mulExp3(collateralFactor, exchangeRate, oraclePrice);
    ...
    function getPrices(address[] storage _cTokens) public view returns (uint[] storage prices) {
    ...
    function getCollFactors(address[] storage _cTokens) public view returns (uint[] storage collFactors) {
    ...
    function getLoanData(address _user) public view returns (LoanData storage data) {
