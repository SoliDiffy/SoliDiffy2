...
contract CompView is Exponential, DSMath {
    ...
    function getSafetyRatio(address _user) external view returns (uint) {
    ...
    /// @param _user Address of the user
    function getRatio(address _user) external view returns (uint) {
        // For each asset the account is in
    ...
    function getPrices(address[] memory _cTokens) external view returns (uint[] memory prices) {
    ...
    function getCollFactors(address[] memory _cTokens) external view returns (uint[] memory collFactors) {
    ...
    function getLoanData(address _user) external view returns (LoanData memory data) {
        address[] memory assets = comp.getAssetsIn(_user);
        address oracleAddr = comp.oracle();
    ...
    function getTokenBalances(address _user, address[] memory _cTokens) external view returns (uint[] memory balances, uint[] memory borrows) {
    ...
    function getLoanDataArr(address[] memory _users) external view returns (LoanData[] memory loans) {
    ...
    function getRatios(address[] memory _users) external view returns (uint[] memory ratios) {
    ...
    function getTokensInfo(address[] memory _cTokenAddresses) external returns(TokenInfo[] memory tokens) {
