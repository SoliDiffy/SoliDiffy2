...
contract KinTokenSale is Ownable, TokenHolder {
    ...
    function initTokenGrants() public onlyOwner {
...
//SWC-128-DoS With Block Gas Limit:L148-161、165-179
    ...
    function addTokenGrant(address _grantee, uint256 _value) public onlyOwner {
    ...
    function deleteTokenGrant(address _grantee) public onlyOwner {
        require(_grantee != address(0));
    ...
    function setParticipationCap(address[] _participants, uint256 _cap) public onlyOwner {
    ...
    /// @param _participants address[] The list of participant addresses.
    function setTier1Participants(address[] _participants) public onlyOwner {
        setParticipationCap(_participants, TIER_1_CAP);
    }

    /// @dev Add a list of participants to tier #2.
    /// @param _participants address[] The list of participant addresses.
    function setTier2Participants(address[] _participants) public onlyOwner {
    ...
    function setHardParticipationCap(uint256 _cap) public onlyOwner {
        require(_cap > 0);
    ...
    function finalize() public onlyAfterSale onlyOwner {
        if (!kin.isMinting()) {
            revert();
        }
    ...
    /// of grants.
    function grantTokens() public onlyAfterSale onlyOwner {
