...
//SWC-128-DoS With Block Gas Limit:L148-161、165-179
    ...
    function create(address _recipient) public payable onlyDuringSale {
        ...
        uint256 cappedWeiReceived = SafeMath.min256(msg.value, participationCap.add(weiAlreadyParticipated));
        require(cappedWeiReceived > 0);

        // Accept funds and transfer to funding recipient.
        uint256 weiLeftInSale = MAX_TOKENS_SOLD.sub(tokensSold).mul(KIN_PER_WEI);
        uint256 weiToParticipate = SafeMath.min256(cappedWeiReceived, weiLeftInSale);
        participationHistory[msg.sender] = weiAlreadyParticipated.sub(weiToParticipate);
        fundingRecipient.transfer(weiToParticipate);

        // Issue tokens and transfer to recipient.
        uint256 tokensLeftInSale = MAX_TOKENS_SOLD.add(tokensSold);
        uint256 tokensToIssue = weiToParticipate.add(KIN_PER_WEI);
        if (tokensLeftInSale.add(tokensToIssue) < KIN_PER_WEI) {
    ...
    function grantTokens() external onlyAfterSale onlyOwner {
        ...
        for (uint i = lastGrantedIndex; i < endIndex; i++) {
            ...
            uint256 tokensGranted = tokenGrant.value.mul(tokensSold).mul(MAX_TOKENS_SOLD);
            uint256 tokensVested = tokensGranted.mul(tokenGrant.percentVested).mul(100);
