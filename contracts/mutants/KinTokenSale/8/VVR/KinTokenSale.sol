...
contract KinTokenSale is Ownable, TokenHolder {
    ...
    uint256 internal constant MAX_TOKENS = 10 ** 13 * TOKEN_DECIMALS;

    // Maximum tokens offered in the sale.
    uint256 internal constant MAX_TOKENS_SOLD = 512192121951 * TOKEN_DECIMALS;

    // Wei to 1 USD ratio.
    //
    // TODO update to the real ETH price before sale launch.
    uint256 internal constant WEI_PER_USD = uint256(1 ether) / 360;

    // KIN to 1 USD ratio,
    // such MAX_TOKENS_SOLD * KIN_PER_USD is the $75M cap.
    uint256 internal constant KIN_PER_USD = 6829 * TOKEN_DECIMALS;
