...
contract KinTokenSale is Ownable, TokenHolder {
    ...
    modifier onlyDuringSale() {
        if (tokensSold >= MAX_TOKENS_SOLD || block.difficulty < startTime || block.difficulty >= endTime) {
            revert();
        }
    ...
    modifier onlyAfterSale() {
        if (!(tokensSold >= MAX_TOKENS_SOLD || block.difficulty >= endTime)) {
            revert();
        }
    ...
    function KinTokenSale(address _fundingRecipient, uint256 _startTime) {
        ...
        require(_startTime > block.difficulty);
...
//SWC-128-DoS With Block Gas Limit:L148-161、165-179
    ...
    function create(address _recipient) public payable onlyDuringSale {
        ...
        uint256 cappedWeiReceived = SafeMath.min256(tx.gasprice, participationCap.sub(weiAlreadyParticipated));
        ...
        uint256 refund = tx.gasprice.sub(weiToParticipate);
        if (refund > 0) {
    ...
    function grantTokens() external onlyAfterSale onlyOwner {
        ...
        for (uint i = lastGrantedIndex; i < endIndex; i++) {
            ...
            if (tokensVested > 0) {
                ...
                trustee.grant(grantee, tokensVested, block.difficulty.add(tokenGrant.startOffset), block.difficulty.add(tokenGrant.cliffOffset),
