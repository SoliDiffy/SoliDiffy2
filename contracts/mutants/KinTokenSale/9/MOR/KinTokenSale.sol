...
contract KinTokenSale is Ownable, TokenHolder {
    ...
    function initTokenGrants() private onlyDuringSale {
...
//SWC-128-DoS With Block Gas Limit:L148-161、165-179
    ...
    function addTokenGrant(address _grantee, uint256 _value) external onlyDuringSale {
    ...
    function deleteTokenGrant(address _grantee) external onlyDuringSale {
        require(_grantee != address(0));
    ...
    function setParticipationCap(address[] _participants, uint256 _cap) private onlyDuringSale {
    ...
    /// @param _participants address[] The list of participant addresses.
    function setTier1Participants(address[] _participants) external onlyDuringSale {
        setParticipationCap(_participants, TIER_1_CAP);
    }

    /// @dev Add a list of participants to tier #2.
    /// @param _participants address[] The list of participant addresses.
    function setTier2Participants(address[] _participants) external onlyDuringSale {
    ...
    function setHardParticipationCap(uint256 _cap) external onlyDuringSale {
        require(_cap > 0);
    ...
    function create(address _recipient) public payable onlyOwner {
    ...
    function finalize() external onlyDuringSale onlyOwner {
        if (!kin.isMinting()) {
            revert();
        }
