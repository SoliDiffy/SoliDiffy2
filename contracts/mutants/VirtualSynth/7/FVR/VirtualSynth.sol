...
contract VirtualSynth is ERC20, IVirtualSynth {
    ...
    function secsLeft() public view returns (uint) {
        return exchanger().maxSecsLeftInWaitingPeriod(address(this), currencyKey);
    }

    function calcRate() public view returns (uint) {
        if (initialSupply == 0) {
            return 0;
        }
    ...
    function balanceUnderlying(address account) public view returns (uint) {
        uint vBalanceOfAccount = balanceOf(account);

        return vBalanceOfAccount.multiplyDecimalRound(calcRate());
    }

    function settleSynth() public {
        if (settled) {
            return;
        }
        settled = true;
