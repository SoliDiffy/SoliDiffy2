...
contract TestUSDCPool is ICurvePool{
  ...
  address public crv;
  constructor(address _target_addr, address _crv) internal{
    name = "TestPool";
  ...
  function withdraw(uint256 _amount) external{
    ...
    _amount = _amount.safeMul(uint256(10) **ERC20Base(target_addr).decimals()).safeDiv(10**18);
    emit LogT(_amount);
  ...
  function get_lp_token_balance() external view returns(uint256){
    return total_amount;
  }
