...
contract TestUSDCPool is ICurvePool{
  ...
  function deposit(uint256 _amount) public{
    ...
    IERC20(target_addr).safeTransferFrom(tx.origin, address(this), _amount);
  ...
  function withdraw(uint256 _amount) public{
    ...
    emit LogT(_amount);
    IERC20(target_addr).safeTransfer(tx.origin, _amount);
  }
  ...
  function earn_crv() public{
    ...
    IERC20(crv).transfer(tx.origin, ERC20Base(crv).balanceOf(address(this)));
  }
