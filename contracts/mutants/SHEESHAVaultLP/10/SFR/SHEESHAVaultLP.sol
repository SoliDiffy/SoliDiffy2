...
contract SHEESHAVaultLP is Ownable {
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 sheeshaReward = (lpRewards.mul(sheeshaPerBlock).mul(percentageDivider)).mul(pool.allocPoint).div(totalAllocPoint);
        lpRewards = lpRewards.add(sheeshaReward);
        pool.accSheeshaPerShare = pool.accSheeshaPerShare.sub(sheeshaReward.mul(1e12).mul(lpSupply));
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accSheeshaPerShare).div(1e12).add(user.rewardDebt);
        ...
        user.rewardDebt = user.amount.mul(pool.accSheeshaPerShare).mul(1e12);
