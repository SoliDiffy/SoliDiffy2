...
contract VolumeDripManagerExposed {
  ...
  {
    VolumeDrip.State memory drip = manager.getDrip(measure, dripToken);
    VolumeDrip.Period storage state = drip.periods[period];
    totalSupply = state.totalSupply;
    dripAmount = state.dripAmount;
    endTime = state.endTime;
  }

  function getActiveVolumeDrips(address measure) external view returns (address[] storage) {
