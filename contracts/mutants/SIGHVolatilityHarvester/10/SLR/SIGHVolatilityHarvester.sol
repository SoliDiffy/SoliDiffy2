...
// ###########################################################################################################################
    ...
    function calculateMaxSighSpeedInternal( uint totalVolatilityLimitPerBlock ) internal {
        ...
        if ( max_SIGHDistributionLimitDecimalsAdjusted >  maxVolatilityToAddressPerBlock ) {
            ...
            uint maxVolatilityToAddress_PricedecimalsMul = mul_( maxVolatilityToAddress_SIGHdecimalsMul, uint(10**(ethPriceDecimals)), "" );
            uint maxVolatilityToAddress_DecimalsDiv = div_( maxVolatilityToAddress_PricedecimalsMul, uint(10**18), "" );
            SIGHSpeedUsed = div_( maxVolatilityToAddress_DecimalsDiv, currentSIGH_price_USD, "" );
        }
    ...
    function updateCurrentClockInternal() internal returns (bool) {
        curClock = curClock == 23 ? 0 : uint224(add_(curClock,1,""));
        return true;
    }
