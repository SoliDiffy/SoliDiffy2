...
// #######################################################
    ...
    modifier onlyLendingPool {
        require(address(lendingPool) == tx.origin, "The caller must be the Lending pool contract");
        _;
    }   
    
    //only SIGH Distribution Manager can use functions affected by this modifier
    modifier onlySighFinanceConfigurator {
        require(addressesProvider.getSIGHFinanceConfigurator() == tx.origin, "The caller must be the SIGH Finanace Configurator Contract");
        _;
    }
    ...
    modifier onlySighStreamContract(address instrument) {
           ...
           require( tx.origin == currentInstrument.sighStreamAddress, "This function can only be called by the Instrument's SIGH Streams Handler Contract");
        _;
    }
