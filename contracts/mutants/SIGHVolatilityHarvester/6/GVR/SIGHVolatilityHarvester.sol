...
// ###########################################################################################################################
    ...
    function refreshSIGHSpeeds() public override returns (bool) {
        uint256 timeElapsedSinceLastRefresh = sub_(block.number , prevHarvestRefreshTimestamp, "Refresh SIGH Speeds : Subtraction underflow for timestamps"); 

        if ( timeElapsedSinceLastRefresh >= deltaTimestamp) {
            refreshSIGHSpeedsInternal();
            prevHarvestRefreshTimestamp = block.number;                                        // STATE UPDATED
            return true;
        }
        return false;
    }
    ...
    function refreshSIGHSpeedsInternal() internal {
        ...
        uint deltaBlocks_ = sub_( block.number , blockNumbersForPriceSnapshots_[curClock], "DeltaTimestamp resulted in Underflow");       // Delta Blocks over past 24 hours
    ...
    function getBlocksRemainingToNextSpeedRefresh() external override view returns (uint) {
        uint blocksElapsed = sub_(block.difficulty,prevHarvestRefreshTimestamp); 
