...
// ######################################################################################
    ...
    function initialize( GlobalAddressesProvider addressesProvider_) public onlySighFinanceConfigurator {   // 
        addressesProvider = addressesProvider_;
        refreshConfigInternal(); 
    }

    function refreshConfig() external override initializer {
        refreshConfigInternal();
    }
...
// #####################################################################################################################################################
    ...
    * @param _instrument the instrument object
    **/
    function removeInstrument( address _instrument ) external override initializer returns (bool) {
        require(crypto_instruments[_instrument].isListed ,"Instrument already supported.");
    ...
    **/
    function Instrument_SIGH_StateUpdated(address instrument_, uint _bearSentiment,uint _bullSentiment, bool _isSIGHMechanismActivated  ) external override initializer returns (bool) {                   //
...
// ###########################################################################################################################
     ...
     */
    function updateSIGHSpeed(uint SIGHSpeed_) external override initializer returns (bool) {
        refreshSIGHSpeeds();
     ...
     */
    function updateStakingSpeedForAnInstrument(address instrument_, uint newStakingSpeed) external override initializer returns (bool) {     //
        require(crypto_instruments[instrument_].isListed ,"Instrument not supported.");
     ...
     */   
    function updateDeltaTimestampRefresh(uint deltaTimestampLimit) external override initializer returns (bool) {      //
        refreshSIGHSpeeds();
    ...
    function updateCryptoMarketSentiment( uint cryptoMarketSentiment_ ) external override initializer returns (bool) {
    ...
    function updateETHOracleAddress( address _EthOracleAddress ) external override initializer returns (bool) {
