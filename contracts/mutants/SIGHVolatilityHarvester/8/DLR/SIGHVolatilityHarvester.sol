...
// #######################################################
    ...
    modifier onlySighStreamContract(address instrument) {
           SupportedInstrument storage currentInstrument = crypto_instruments[instrument];
    ...
    **/
    function addInstrument( address _instrument, address _iTokenAddress,address _stableDebtToken,address _variableDebtToken, address _sighStreamAddress, uint8 _decimals ) external override returns (bool) {
        ...
        if ( instrumentPriceCycles[_instrument].initializationCounter == 0 ) {
            uint256[24] storage emptyPrices;
...
// ###########################################################################################################################
    ...
    function refreshSIGHSpeedsInternal() internal {
        address[] storage all_Instruments_ = all_Instruments;
        ...
        Exp storage totalProtocolVolatility = Exp({mantissa: 0});                            // TOTAL LOSSES (Over past 24 hours)
        Exp storage totalProtocolVolatilityLimit = Exp({mantissa: 0});                            // TOTAL LOSSES (Over past 24 hours)
        ...
        for (uint i = 0; i < all_Instruments_.length; i++) {
            ...
            Exp storage previousPriceUSD = Exp({ mantissa: instrumentPriceCycles[_currentInstrument].recordedPriceSnapshot[curClock] });            // 24hr old price snapshot
            Exp storage currentPriceUSD = Exp({ mantissa: oracle.getAssetPrice( _currentInstrument ) });                                            // current price from the oracle
            ...
            if ( !crypto_instruments[_currentInstrument].isSIGHMechanismActivated || instrumentPriceCycles[_currentInstrument].initializationCounter != uint32(24) ) {     // if LOSS MINIMIZNG MECHANISM IS NOT ACTIVATED FOR THIS INSTRUMENT
                ...
                MathError error;
                Exp storage volatility = Exp({mantissa: 0});
