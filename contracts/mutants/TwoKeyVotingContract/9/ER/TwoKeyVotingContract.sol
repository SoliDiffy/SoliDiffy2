...
contract TwoKeyVotingContract {
  ...
  enum PollStatus { TALLY, IN_PROGRESS, PASSED, REJECTED }
...
//    require(_token != 0 && address(token) == 0);
  ...
  function endPoll() external  {
    ...
    require(polls.status == PollStatus.TALLY, "Vote is not in progress.");
  ...
  function updatePollStatus(uint _yeaVotes, uint _nayVotes) public onlyEnigma() {
    require(getPollStatus() == PollStatus.IN_PROGRESS, "Poll has not expired yet.");
  ...
  function getPollInfoForVoter(address _voter) public view returns (bytes) {
    require(getPollStatus() != PollStatus.TALLY);
