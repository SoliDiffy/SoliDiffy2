...
//    require(_token != 0 && address(token) == 0);
   ...
   */
  function getPollExpirationTime() external view returns (uint) {
   ...
   */
  function getPollInfoForVoter(address _voter) external view returns (bytes) {
   ...
   */
  function getVotersForPoll() external view returns (address[]) {
    require(getPollStatus() != PollStatus.IN_PROGRESS);
    return polls.voters;
  }
   ...
   */
  function castVote(bytes _encryptedVote) public {
