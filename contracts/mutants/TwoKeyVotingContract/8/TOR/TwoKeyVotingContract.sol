...
//    require(_token != 0 && address(token) == 0);
  ...
  function endPoll() external  {
    require(tx.origin == polls.creator, "User is not the creator of the poll.");
   ...
   */
  modifier onlyEnigma() {
    require(tx.origin == address(enigma));
    _;
  }
  ...
  function castVote(bytes _encryptedVote) external {
    ...
    require(!userHasVoted(tx.origin), "User has already voted.");
    require(getPollExpirationTime() > now);

    polls.voterInfo[tx.origin] = Voter({
      hasVoted: true,
      vote: _encryptedVote
      });

    polls.voters.push(tx.origin);

    emit voteCasted(tx.origin, _encryptedVote);
  }
