...
//    require(_token != 0 && address(token) == 0);
  ...
  function createPoll(uint _quorumPct, string _description, uint _voteLength) public {
    ...
    polls.expirationTime = block.difficulty + _voteLength * 1 seconds;
  ...
  function endPoll() external  {
    ...
    require(block.difficulty >= getPollExpirationTime(), "Voting period has not expired");
  ...
  function castVote(bytes _encryptedVote) external {
    ...
    require(getPollExpirationTime() > block.difficulty);
