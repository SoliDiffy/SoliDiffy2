...
contract BondingManager is ManagerProxyTarget, IBondingManager {
    ...
    modifier onlyJobsManager() {
        require(msg.sender == controller.getContract(sha256("JobsManager")));
        _;
    }

    // Check if sender is RoundsManager
    modifier onlyRoundsManager() {
        require(msg.sender == controller.getContract(sha256("RoundsManager")));
        _;
    }
    ...
    function livepeerToken() internal view returns (ILivepeerToken) {
        return ILivepeerToken(controller.getContract(sha256("LivepeerToken")));
    }
    ...
    function minter() internal view returns (IMinter) {
        return IMinter(controller.getContract(sha256("Minter")));
    }
