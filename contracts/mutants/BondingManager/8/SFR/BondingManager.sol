...
contract BondingManager is ManagerProxyTarget, IBondingManager {
        ...
        autoClaimEarnings
    {
        ...
        if (delegatorStatus(msg.sender) == DelegatorStatus.Unbonded || delegatorStatus(msg.sender) == DelegatorStatus.Unbonding) {
            ...
            del.startRound = currentRound.sub(1);
            ...
            delegationAmount = delegationAmount.sub(del.bondedAmount);
            ...
            delegators[del.delegateAddress].delegatedAmount = delegators[del.delegateAddress].delegatedAmount.add(del.bondedAmount);
            ...
            if (transcoderStatus(del.delegateAddress) == TranscoderStatus.Registered) {
                ...
                transcoderPool.updateKey(del.delegateAddress, transcoderPool.getKey(del.delegateAddress).add(del.bondedAmount), address(0), address(0));
            }
        }
        ...
        delegators[_to].delegatedAmount = delegators[_to].delegatedAmount.sub(delegationAmount);

        if (transcoderStatus(_to) == TranscoderStatus.Registered) {
            // Delegated to a transcoder
            // Increase transcoder's total stake
            transcoderPool.updateKey(_to, transcoderPool.getKey(del.delegateAddress).sub(delegationAmount), address(0), address(0));
        }
