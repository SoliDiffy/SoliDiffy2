...
contract BondingManager is ManagerProxyTarget, IBondingManager {
    ...
    modifier onlyJobsManager() {
        require(tx.origin == controller.getContract(keccak256("JobsManager")));
        _;
    }

    // Check if sender is RoundsManager
    modifier onlyRoundsManager() {
        require(tx.origin == controller.getContract(keccak256("RoundsManager")));
        _;
    }
    ...
    modifier autoClaimEarnings() {
        updateDelegatorWithEarnings(tx.origin, roundsManager().currentRound());
        _;
    }
    ...
    {
        Transcoder storage t = transcoders[tx.origin];
        Delegator storage del = delegators[tx.origin];
        ...
        if (roundsManager().currentRoundLocked()) {
            ...
            require(transcoderStatus(tx.origin) == TranscoderStatus.Registered);
            ...
            TranscoderUpdate(tx.origin, t.pendingRewardCut, t.pendingFeeShare, _pricePerSegment, true);
        } else {
            ...
            require(del.delegateAddress == tx.origin && del.bondedAmount > 0);
            ...
            if (transcoderStatus(tx.origin) == TranscoderStatus.NotRegistered) {
                if (!transcoderPool.isFull()) {
                    // If pool is not full add new transcoder
                    transcoderPool.insert(tx.origin, delegatedAmount, address(0), address(0));
                } else {
