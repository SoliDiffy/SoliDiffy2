...
contract OKLGAtomicSwapInstance is OKLGProduct {
  ...
  function setActiveState(bool _isActive) external onlyOwner {
    require(
  ...
  function setTokenOwner(address newOwner) external onlyOwner {
    require(
  ...
  function receiveTokensFromSource(uint256 _amount)
    external
    payable
    onlyOwner returns (bytes32, uint256)
  {
  ...
  function refundTokensFromSource(bytes32 _id) external onlyOwner {
    require(isActive, 'this atomic swap instance is not active');
  ...
  // This must be called AFTER fundSendToDestinationGas has been executed
  // for this txn to fund this send operation
  function sendTokensToDestination(bytes32 _id) external onlyOwner returns (bytes32) {
    require(isActive, 'this atomic swap instance is not active');
