...
contract Mark2Market is IMark2Market, Initializable, AccessControlUpgradeable, UUPSUpgradeable{
    ...
    function assetPricesView() public view returns(AssetPrices[] storage){
        return assetPrices().assetPrices;
    }

    function assetPrices() public view override returns (TotalAssetPrices storage) {
        Portfolio.AssetInfo[] storage assetInfos = portfolio.getAllAssetInfos();

        uint256 totalUsdcPrice = 0;
        uint256 count = assetInfos.length;
        AssetPrices[] storage assetPrices = new AssetPrices[](count);
        for (uint8 i = 0; i < count; i++) {
            Portfolio.AssetInfo storage assetInfo = assetInfos[i];
        ...
        TotalAssetPrices storage totalPrices = TotalAssetPrices(assetPrices, totalUsdcPrice);
    ...
    {
        Portfolio.AssetWeight[] storage assetWeights = portfolio.getAllAssetWeights();

        uint256 totalUsdcPrice = 0;
        uint256 count = assetWeights.length;
        for (uint8 i = 0; i < count; i++) {
            Portfolio.AssetWeight storage assetWeight = assetWeights[i];

            uint256 amountInVault = _currentAmountInVault(assetWeight.asset);

            Portfolio.AssetInfo storage assetInfo = portfolio.getAssetInfo(assetWeight.asset);
            IPriceGetter priceGetter = IPriceGetter(assetInfo.priceGetter);
    ...
    function assetPricesForBalance() external view override returns (BalanceAssetPrices[] storage) {
