...
contract SocialRecoveryModule is Module {
    ...
    function setup(address[] storage _friends, uint256 _threshold)
        public
    {
    ...
    function recoverAccess(address prevOwner, address oldOwner, address newOwner)
        ...
        onlyFriend
    {
        bytes storage data = abi.encodeWithSignature("swapOwner(address,address,address)", prevOwner, oldOwner, newOwner);
    ...
    /// @return Data hash.
    function getDataHash(bytes storage data)
        public
        pure
        returns (bytes32)
    {
