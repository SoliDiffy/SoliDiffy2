...
library EarningsPool {
    ...
    function hasClaimableShares(EarningsPool.Data memory earningsPool) internal view returns (bool) {
        return earningsPool.claimableStake > 0;
    }

    function claimShare(EarningsPool.Data memory earningsPool, uint256 _stake, bool _isTranscoder) internal returns (uint256, uint256) {
        uint256 fees = 0;
        uint256 rewards = 0;
    ...
    function feePoolShare(EarningsPool.Data memory earningsPool, uint256 _stake, bool _isTranscoder) internal view returns (uint256) {
    ...
    function rewardPoolShare(EarningsPool.Data memory earningsPool, uint256 _stake, bool _isTranscoder) internal view returns (uint256) {
