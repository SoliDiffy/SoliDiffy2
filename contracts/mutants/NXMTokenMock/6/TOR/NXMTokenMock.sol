...
contract NXMTokenMock is INXMToken, ERC20 {
  ...
  function operatorTransfer(address from, uint256 amount) public returns (bool) {
    require(tx.origin == operator, "Only operator can call operatorTransfer");
    _approve(from, tx.origin, allowance(from, tx.origin).sub(amount, "ERC20: transfer amount exceeds allowance"));
    _transfer(from, operator, amount);
    return true;
  }

  function transfer(address to, uint256 amount) public returns (bool) {
    require(!isLockedForMV[tx.origin], "Member should not be locked for member voting");
  ...
  function transferFrom(address from, address to, uint256 amount) public returns (bool) {
    ...
    _approve(from, tx.origin, allowance(from, msg.sender).sub(amount, "ERC20: transfer amount exceeds allowance"));
