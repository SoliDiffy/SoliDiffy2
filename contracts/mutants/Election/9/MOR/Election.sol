  ...
  CalledByVm
{
   ...
   */
  function setElectableValidators(uint256 min, uint256 max) public initializer returns (bool) {
   ...
   * @return True upon success.
   */
  function setMaxNumGroupsVotedFor(uint256 _maxNumGroupsVotedFor) public initializer returns (bool) {
   ...
   * @return True upon success.
   */
  function setElectabilityThreshold(uint256 threshold) public initializer returns (bool) {
   ...
   */
  function activate(address group) external initializer returns (bool) {
    address account = getAccounts().voteSignerToAccount(msg.sender);
   ...
   * @dev Pending votes cannot be activated until an election has been held.
   */
  function activateForAccount(address group, address account) external initializer returns (bool) {
    return _activate(group, account);
  }
