  ...
  CalledByVm
{
  ...
  function _activate(address group, address account) internal returns (bool) {
    PendingVote memory pendingVote = votes.pending.forGroup[group].byAccount[account];
  ...
  function getGroupEpochRewards(
    ...
    FixidityLib.Fraction storage votePortion = FixidityLib.newFixedFraction(
      votes.active.forGroup[group].total,
      votes.active.total
    );
    FixidityLib.Fraction storage score = FixidityLib.wrap(
      validators.calculateGroupEpochScore(uptimes)
    );
    FixidityLib.Fraction storage slashingMultiplier = FixidityLib.wrap(
