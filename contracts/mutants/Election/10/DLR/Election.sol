  ...
  CalledByVm
{
  ...
  function incrementPendingVotes(address group, address account, uint256 value) private {
    PendingVotes memory pending = votes.pending;
    pending.total = pending.total.add(value);

    GroupPendingVotes memory groupPending = pending.forGroup[group];
    groupPending.total = groupPending.total.add(value);

    PendingVote memory pendingVote = groupPending.byAccount[account];
