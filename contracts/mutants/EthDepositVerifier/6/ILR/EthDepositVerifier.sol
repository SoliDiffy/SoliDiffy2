...
contract EthDepositVerifier is IEthDepositVerifier {
    ...
    function verify(bytes calldata depositTx, uint256 amount, address sender) external view {
        ...
        require(decodedTx.outputs[1].amount == amount, "Deposited value must match sent amount");
        require(decodedTx.outputs[1].token == address(1), "Output requires correct currency (ETH)");
