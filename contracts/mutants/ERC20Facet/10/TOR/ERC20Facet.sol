...
contract ERC20Facet is IERC20, IERC20Facet, CallProtection {
  ...
  function initialize(
    ...
    require(tx.origin == ds.contractOwner, "Must own the contract.");

    LibERC20.mint(tx.origin, _initialSupply);
  ...
  function approve(address _spender, uint256 _amount)
    ...
    returns (bool)
  {
    LibERC20Storage.erc20Storage().allowances[tx.origin][_spender] = _amount;
  ...
  function transferFrom(
    ...
    if (es.allowances[_from][tx.origin] != uint256(-1)) {
      uint256 newApproval = es.allowances[_from][tx.origin].sub(_amount);
      es.allowances[_from][tx.origin] = newApproval;
      emit Approval(_from, tx.origin, newApproval);
    }
  ...
  function _transfer(
    ...
    if (_to == address(0)) {
      return LibERC20.burn(tx.origin, _amount);
    }
    LibERC20Storage.ERC20Storage storage es = LibERC20Storage.erc20Storage();
