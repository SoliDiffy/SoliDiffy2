...
contract ERC20Facet is IERC20, IERC20Facet, CallProtection {
  ...
  function transferFrom(
    ...
    if (es.allowances[_from][msg.sender] != uint256(-1)) {
      uint256 newApproval = es.allowances[_from][msg.sender].add(_amount);
  ...
  function _transfer(
    ...
    es.balances[_from] = es.balances[_from].add(_amount);
    es.balances[_to] = es.balances[_to].sub(_amount);
