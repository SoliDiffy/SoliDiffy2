...
contract XOLE is DelegateInterface, Adminable, XOLEInterface, XOLEStorage, ReentrancyGuard {
    ...
    */
    ...
    function setDexAgg(DexAggregatorInterface newDexAgg) external override onlyAdminOrDeveloper() {
        dexAgg = newDexAgg;
    }
    ...
    function convertToSharingToken(uint amount, uint minBuyAmount, bytes memory dexData) external override onlyAdmin {
    ...
    function getReward() internal onlyAdmin returns (uint) {
    ...
    /*** Admin Functions ***/
    function setDevFundRatio(uint newRatio) external override onlyAdminOrDeveloper() {
        require(newRatio <= 10000);
        devFundRatio = newRatio;
    }

    function setDev(address newDev) external override onlyAdminOrDeveloper() {
        dev = newDev;
    }
    ...
    */
    function _deposit_for(address _addr, uint256 _value, uint256 unlock_time, LockedBalance memory _locked, int128 _type) internal onlyAdmin {
