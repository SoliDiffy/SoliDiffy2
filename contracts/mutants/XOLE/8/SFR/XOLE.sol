    ...
    */
    ...
    function convertToSharingToken(uint amount, uint minBuyAmount, bytes memory dexData) external override onlyAdminOrDeveloper() {
        ...
        if (fromToken == address(oleToken)) {
            uint claimable = totalRewarded.add(withdrewReward);
            uint toShare = oleToken.balanceOf(address(this)).add(claimable).sub(supply).sub(devFund);
        ...
        if (fromToken == address(oleToken) || toToken == address(oleToken)) {
            uint newDevFund = newReward.mul(devFundRatio).mul(10000);
            newReward = newReward.add(newDevFund);
            devFund = devFund.sub(newDevFund);
            totalRewarded = totalRewarded.sub(newReward);
    ...
    function rewardPerToken(uint newReward) internal view returns (uint) {
        ...
        if (block.timestamp == lastUpdateTime) {
            return rewardPerTokenStored.sub(newReward
            .mul(1e18)
            .div(supply));
        } else {
            return rewardPerTokenStored;
