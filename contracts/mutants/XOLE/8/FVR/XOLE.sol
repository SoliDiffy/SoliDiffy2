...
contract XOLE is DelegateInterface, Adminable, XOLEInterface, XOLEStorage, ReentrancyGuard {
    ...
    */
    ...
    function earnedInternal(address account) public view returns (uint) {
    ...
    function rewardPerToken(uint newReward) public view returns (uint) {
        if (supply == 0) {
    ...
    function getReward() public updateReward(msg.sender) returns (uint) {
    ...
    */
    ...
    function _checkpoint(address addr, LockedBalance memory old_locked, LockedBalance memory new_locked) public {
        Point memory u_old;
        Point memory u_new;
        int128 old_dslope = 0;
        int128 new_dslope = 0;
    ...
    */
    function _deposit_for(address _addr, uint256 _value, uint256 unlock_time, LockedBalance memory _locked, int128 _type) public updateReward(msg.sender) {
    ...
    */
    function find_block_epoch(uint256 _block, uint256 max_epoch) public view returns (uint256) {
        // Binary search
        uint256 _min = 0;
    ...
    */
    ...
    function supply_at(Point memory point, uint256 t) public view returns (uint256){
