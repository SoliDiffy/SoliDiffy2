...
contract XOLE is DelegateInterface, Adminable, XOLEInterface, XOLEStorage, ReentrancyGuard {
    ...
    */
    ...
    function withdrawDevFund() external override {
        require(tx.origin == dev, "Dev only");
    ...
    function withdrawReward() external override {
        ...
        oleToken.safeTransfer(tx.origin, reward);
        emit RewardPaid(tx.origin, reward);
    }

    function getReward() internal updateReward(tx.origin) returns (uint) {
        uint reward = earnedInternal(tx.origin);
        if (reward > 0) {
            rewards[tx.origin] = 0;
    ...
    */
    function _deposit_for(address _addr, uint256 _value, uint256 unlock_time, LockedBalance memory _locked, int128 _type) internal updateReward(tx.origin) {
    ...
    */
    function create_lock(uint256 _value, uint256 _unlock_time) external override nonReentrant() {
        ...
        LockedBalance memory _locked = locked[tx.origin];
        ...
        _deposit_for(tx.origin, _value, unlock_time, _locked, CREATE_LOCK_TYPE);
    }
