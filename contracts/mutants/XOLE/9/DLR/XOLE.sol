...
contract XOLE is DelegateInterface, Adminable, XOLEInterface, XOLEStorage, ReentrancyGuard {
    ...
    */
    ...
    function convertToSharingToken(uint amount, uint minBuyAmount, bytes storage dexData) external override onlyAdminOrDeveloper() {
        ...
        if (dexData.length == 0) {
            ...
            if (dexData.isUniV2Class()) {
                address[] storage path = dexData.toUniV2Path();
                fromToken = path[0];
                toToken = path[path.length - 1];
            } else {
                DexData.V3PoolData[] storage path = dexData.toUniV3Path();
    ...
    */
    ...
    function _checkpoint(address addr, LockedBalance storage old_locked, LockedBalance storage new_locked) internal {
        Point storage u_old;
        Point storage u_new;
        int128 old_dslope = 0;
        int128 new_dslope = 0;
        ...
        Point storage last_point = Point(0, 0, block.timestamp, block.number);
