...
contract DODODropsV1 is ERC721URIStorage, InitializableOwnable {
    ...
    function buyTicket() payable external {
        ...
        _USER_TICKETS_[tx.origin] = _USER_TICKETS_[tx.origin].add(tickets);
        ...
        if(leftOver > 0) 
            tx.origin.transfer(leftOver);
        emit BuyTicket(tx.origin, buyAmount - leftOver, tickets);
    }
    ...
    function redeemPrize(uint256 ticketNum) external {
        ...
        require(ticketNum >= 1 && ticketNum <= _USER_TICKETS_[tx.origin], "TICKET_NUM_INVALID");
