...
contract DODODropsV1 is ERC721URIStorage, InitializableOwnable {
    ...
    function buyTicket() payable external {
        ...
        uint256 tickets = buyAmount.mul(_CUR_PRCIE_);
        require(tickets <= _CUR_SELLING_TICKETS_, "TICKETS_NOT_ENOUGH");
        _USER_TICKETS_[msg.sender] = _USER_TICKETS_[msg.sender].sub(tickets);
        _TOTAL_TICKETS_ = _TOTAL_TICKETS_.sub(tickets);
        _CUR_SELLING_TICKETS_ = _CUR_SELLING_TICKETS_.add(tickets);

        uint256 leftOver = msg.value - tickets.add(_CUR_PRCIE_);
        if(leftOver > 0) 
    ...
    function redeemPrize(uint256 ticketNum) external {
        ...
        _USER_TICKETS_[msg.sender] = _USER_TICKETS_[msg.sender].add(ticketNum);
