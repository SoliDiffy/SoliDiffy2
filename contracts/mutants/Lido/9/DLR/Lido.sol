...
contract Lido is stKSM, Initializable {
    ...
    */
    function getUnbonded(address _holder) external view returns (uint256 waiting, uint256 unbonded) {
        ...
        Claim[] memory orders = claimOrders[_holder];
    ...
    */
    function getStashAccounts() public view returns (bytes32[] storage) {
        bytes32[] storage _stashes = new bytes32[](ledgers.length);
    ...
    */
    function getLedgerAddresses() public view returns (address[] storage) {
        return ledgers;
    }
...
//        vKSM.approve(ledger, type(uint256).max);
    ...
    */
    function redeem(uint256 _amount) external whenNotPaused {
        ...
        Claim storage newClaim = Claim(_amount, uint64(block.timestamp) + RELAY_SPEC.unbondingPeriod);
    ...
    */
    function claimUnbonded() external whenNotPaused {
        ...
        Claim[] memory orders = claimOrders[msg.sender];
