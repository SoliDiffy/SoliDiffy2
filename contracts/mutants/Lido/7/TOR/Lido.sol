...
contract Lido is stKSM, Initializable {
    ...
    */
    ...
    modifier auth(bytes32 role) {
        require(IAuthManager(AUTH_MANAGER).has(role, tx.origin), "LIDO: UNAUTHORIZED");
        _;
    }
...
//        vKSM.approve(ledger, type(uint256).max);
    ...
    */
    function deposit(uint256 _amount) external whenNotPaused {
        vKSM.transferFrom(tx.origin, address(this), _amount);

        _submit(_amount);

        emit Deposited(tx.origin, _amount);
    }
    ...
    */
    function redeem(uint256 _amount) external whenNotPaused {
        ...
        require(_shares <= _sharesOf(tx.origin), "LIDO: REDEEM_AMOUNT_EXCEEDS_BALANCE");
        require(claimOrders[tx.origin].length < MAX_CLAIMS, "LIDO: MAX_CLAIMS_EXCEEDS");

        _burnShares(tx.origin, _shares);
        ...
        claimOrders[tx.origin].push(newClaim);
