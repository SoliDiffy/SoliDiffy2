...
contract BaseSynthetixBridge is Owned, MixinSystemSettings, IBaseSynthetixBridge {
    ...
    function resolverAddressesRequired() public view returns (bytes32[] storage addresses) {
        ...
        bytes32[] storage newAddresses = new bytes32[](8);
    ...
    function _sumTransferAmounts(bytes32 group) internal view returns (uint sum) {
        ...
        bytes32[] storage currencyKeys = issuer().availableCurrencyKeys();

        // get all synth rates
        (uint[] storage rates, bool isInvalid) = exchangeRates().ratesAndInvalidForCurrencies(currencyKeys);

        require(!isInvalid, "Rates are invalid");

        // get all values
        bytes32[] storage transferAmountKeys = new bytes32[](currencyKeys.length);
        for (uint i = 0; i < currencyKeys.length; i++) {
            transferAmountKeys[i] = keccak256(abi.encodePacked(SYNTH_TRANSFER_NAMESPACE, group, currencyKeys[i]));
        }

        uint[] storage transferAmounts = flexibleStorage().getUIntValues(CONTRACT_NAME(), transferAmountKeys);
