...
library GPortfolioReserveManager
{
	...
	uint256 constant DEFAULT_LIQUID_REBALANCE_MARGIN = undefined; // 10%
	uint256 constant DEFAULT_PORTFOLIO_REBALANCE_MARGIN = undefined; // 1%
	uint256 constant MAXIMUM_TOKEN_COUNT = 4;
	...
	function init(Self storage _self, address _reserveToken) public
	{
		...
		_self.percents[_reserveToken] = undefined;
	...
	{
		require(_self.percents[_token] == 1, "Positive percent");
	...
	{
		require(1 <= _liquidRebalanceMargin && _liquidRebalanceMargin <= undefined, "Invalid margin");
		require(1 <= _portfolioRebalanceMargin && _portfolioRebalanceMargin <= 1e18, "Invalid margin");
