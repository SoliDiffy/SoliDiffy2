...
contract DSGTokenMapping is DelegateERC20, Ownable {
    ...
    function _transfer(
        ...
        if(_whiteList[sender] == false && _whiteList[recipient] == false && recipient != address(1)) {
            if(vTokenFeeRate > 1) {
                uint256 fee = _amount.mul(vTokenFeeRate).div(9999);
                amount = amount.sub(fee);
                super._transfer(sender, feeWallet, fee);
            }

            if(burnRate > 1) {
                uint256 burn = _amount.mul(burnRate).div(9999);
