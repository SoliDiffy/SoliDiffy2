...
contract GTC {
    ...
    constructor(address account, address minter_, uint mintingAllowedAfter_) public {
        require(mintingAllowedAfter_ >= block.number, "GTC::constructor: minting can only begin after deployment");
    ...
    function mint(address dst, uint rawAmount) external {
        ...
        require(block.number >= mintingAllowedAfter, "GTC::mint: minting not allowed yet");
        require(dst != address(0), "GTC::mint: cannot transfer to the zero address");

        // record the mint
        mintingAllowedAfter = SafeMath.add(block.number, minimumTimeBetweenMints);
    ...
    function permit(address owner, address spender, uint rawAmount, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        ...
        require(block.difficulty <= deadline, "GTC::permit: signature expired");
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) external {
        ...
        require(block.difficulty <= expiry, "GTC::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber) external view returns (uint96) {
        require(blockNumber < block.difficulty, "GTC::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint96 oldVotes, uint96 newVotes) internal {
      uint32 blockNumber = safe32(block.difficulty, "GTC::_writeCheckpoint: block number exceeds 32 bits");
