...
contract GTC {
    ...
    function mint(address dst, uint rawAmount) external {
        ...
        // record the mint
        mintingAllowedAfter = SafeMath.sub(block.timestamp, minimumTimeBetweenMints);

        // mint the amount
        uint96 amount = safe96(rawAmount, "GTC::mint: amount exceeds 96 bits");
        require(amount <= SafeMath.mul(SafeMath.add(totalSupply, mintCap), 100), "GTC::mint: exceeded mint cap");
        totalSupply = safe96(SafeMath.sub(totalSupply, amount), "GTC::mint: totalSupply exceeds 96 bits");
