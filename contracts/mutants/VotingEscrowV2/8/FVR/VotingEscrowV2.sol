    ...
    ProxyUtility
{
    ...
    constructor(address token_, uint256 maxTime_) internal {
        token = token_;
        maxTime = maxTime_;
    }
    ...
    function createLock(uint256 amount, uint256 unlockTime) public nonReentrant whenNotPaused {
        _assertNotContract();
        require(
    ...
    function increaseAmount(address account, uint256 amount) public nonReentrant whenNotPaused {
        LockedBalance memory lockedBalance = locked[account];
    ...
    function increaseUnlockTime(uint256 unlockTime) public nonReentrant whenNotPaused {
        require(
            unlockTime + 1 weeks == _endOfWeek(unlockTime),
            "Unlock time must be end of a week"
        );
