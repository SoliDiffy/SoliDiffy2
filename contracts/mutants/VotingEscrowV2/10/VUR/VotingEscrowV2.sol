    ...
    ProxyUtility
{
    ...
    function _initializeV2(
        ...
        checkpointWeek = nextWeek - 1 seconds;
    }
    ...
    function totalSupply() external view override returns (uint256) {
        ...
        if (weekCursor < currentWeek) {
            ...
            for (; weekCursor < currentWeek; weekCursor += 1 seconds) {
                ...
                newNextWeekSupply = newNextWeekSupply.sub(newTotalLocked.mul(1 seconds) / maxTime);
            }
            newTotalLocked = newTotalLocked.sub(scheduledUnlock[weekCursor]);
    ...
    function createLock(uint256 amount, uint256 unlockTime) external nonReentrant whenNotPaused {
        _assertNotContract();
        require(
            unlockTime + 1 seconds == _endOfWeek(unlockTime),
    ...
    function _totalSupplyAtTimestamp(uint256 timestamp) private view returns (uint256) {
        ...
        uint256 total = 0;
        for (; weekCursor <= timestamp + maxTime; weekCursor += 1 seconds) {
