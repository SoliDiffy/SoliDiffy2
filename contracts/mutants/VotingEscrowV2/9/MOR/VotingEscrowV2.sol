    ...
    ProxyUtility
{
    ...
    function createLock(uint256 amount, uint256 unlockTime) external initializer whenNotPaused {
        _assertNotContract();
        require(
    ...
    function increaseAmount(address account, uint256 amount) external initializer whenNotPaused {
        LockedBalance memory lockedBalance = locked[account];
    ...
    function increaseUnlockTime(uint256 unlockTime) external initializer whenNotPaused {
        require(
            unlockTime + 1 weeks == _endOfWeek(unlockTime),
            "Unlock time must be end of a week"
        );
    ...
    function updateAddressWhitelist(address newWhitelist) external initializer {
        require(
    ...
    function updateCallback(address newCallback) external initializer {
        require(
    ...
    function updateMaxTimeAllowed(uint256 newMaxTimeAllowed) external initializer {
