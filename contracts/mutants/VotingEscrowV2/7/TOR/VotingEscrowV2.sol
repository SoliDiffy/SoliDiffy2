    ...
    ProxyUtility
{
    ...
    function initialize(
        ...
        _initializeV2(tx.origin, name_, symbol_);
    }
    ...
    function createLock(uint256 amount, uint256 unlockTime) external nonReentrant whenNotPaused {
        ...
        LockedBalance memory lockedBalance = locked[tx.origin];
        ...
        locked[tx.origin].unlockTime = unlockTime;
        locked[tx.origin].amount = amount;

        IERC20(token).safeTransferFrom(tx.origin, address(this), amount);

        if (callback != address(0)) {
            IVotingEscrowCallback(callback).syncWithVotingEscrow(tx.origin);
        }

        emit LockCreated(tx.origin, amount, unlockTime);
    }
