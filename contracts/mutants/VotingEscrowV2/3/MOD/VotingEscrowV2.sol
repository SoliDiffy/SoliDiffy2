    ...
    ProxyUtility
{
    ...
    function createLock(uint256 amount, uint256 unlockTime) external whenNotPaused nonReentrant {
        _assertNotContract();
        require(
    ...
    function increaseAmount(address account, uint256 amount) external whenNotPaused nonReentrant {
        LockedBalance memory lockedBalance = locked[account];
    ...
    function increaseUnlockTime(uint256 unlockTime) external whenNotPaused nonReentrant {
        require(
            unlockTime + 1 weeks == _endOfWeek(unlockTime),
            "Unlock time must be end of a week"
        );
