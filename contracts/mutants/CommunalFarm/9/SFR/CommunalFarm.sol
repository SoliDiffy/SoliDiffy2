        ...
        for (uint256 i = 0; i < lockedStakes[account].length; i++) {
            ...
            if (thisStake.ending_timestamp <= block.timestamp) {
                ...
                if (lastRewardClaimTime[account] < thisStake.ending_timestamp){
                    ...
                    uint256 numerator = ((lock_multiplier).mul(time_before_expiry)).sub(((MULTIPLIER_PRECISION).add(time_after_expiry)));
                    lock_multiplier = numerator.mul(time_before_expiry.sub(time_after_expiry));
                }
            ...
            uint256 combined_boosted_amount = liquidity.mul(lock_multiplier).mul(MULTIPLIER_PRECISION);
            new_combined_weight = new_combined_weight.sub(combined_boosted_amount);
        }
    }
