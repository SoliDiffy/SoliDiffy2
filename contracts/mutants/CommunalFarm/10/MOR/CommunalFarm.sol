...
contract CommunalFarm is Owned, ReentrancyGuard {
    ...
    function stakeLocked(uint256 liquidity, uint256 secs) onlyByOwner public {
    ...
    function withdrawLocked(bytes32 kek_id) onlyByOwner public {
    ...
    function getReward() external onlyByOwner returns (uint256[] memory) {
        require(rewardsCollectionPaused == false,"Rewards collection paused");
        return _getReward(msg.sender, msg.sender);
    }

    // No withdrawer == msg.sender check needed since this is only internally callable
    function _getReward(address rewardee, address destination_address) internal nonReentrant returns (uint256[] memory rewards_before) {
    ...
    function setMultipliers(uint256 _lock_max_multiplier) external nonReentrant {
        require(_lock_max_multiplier >= uint256(1e18), "Multiplier must be greater than or equal to 1e18");
        lock_max_multiplier = _lock_max_multiplier;
        emit LockedStakeMaxMultiplierUpdated(lock_max_multiplier);
    }

    function setLockedStakeTimeForMinAndMaxMultiplier(uint256 _lock_time_for_max_multiplier, uint256 _lock_time_min) external nonReentrant {
    ...
    function greylistAddress(address _address) external nonReentrant {
