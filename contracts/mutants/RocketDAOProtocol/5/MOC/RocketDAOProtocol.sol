...
contract RocketDAOProtocol is RocketBase, RocketDAOProtocolInterface {
    ...
    function bootstrapSettingMulti(string[] memory _settingContractNames, string[] memory _settingPaths, SettingType[] memory _types, bytes[] memory _values) override external onlyGuardian onlyLatestContract("rocketDAOProtocol", address(this)) onlyBootstrapMode {
    ...
    function bootstrapSettingUint(string memory _settingContractName, string memory _settingPath, uint256 _value) override external onlyGuardian onlyLatestContract("rocketDAOProtocol", address(this)) onlyBootstrapMode {
        // Ok good to go, lets update the settings
    ...
    function bootstrapSettingBool(string memory _settingContractName, string memory _settingPath, bool _value) override external onlyGuardian onlyLatestContract("rocketDAOProtocol", address(this)) onlyBootstrapMode {
        // Ok good to go, lets update the settings 
    ...
    function bootstrapSettingAddress(string memory _settingContractName, string memory _settingPath, address _value) override external onlyGuardian onlyLatestContract("rocketDAOProtocol", address(this)) onlyBootstrapMode {
        // Ok good to go, lets update the settings 
    ...
    function bootstrapSettingClaimer(string memory _contractName, uint256 _perc) override external onlyGuardian onlyLatestContract("rocketDAOProtocol", address(this)) onlyBootstrapMode {
