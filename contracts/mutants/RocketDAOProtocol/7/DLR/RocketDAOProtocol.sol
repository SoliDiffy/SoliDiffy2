...
contract RocketDAOProtocol is RocketBase, RocketDAOProtocolInterface {
    ...
    function bootstrapSettingMulti(string[] storage _settingContractNames, string[] storage _settingPaths, SettingType[] storage _types, bytes[] storage _values) override external onlyGuardian onlyBootstrapMode onlyLatestContract("rocketDAOProtocol", address(this)) {
    ...
    function bootstrapSettingUint(string storage _settingContractName, string storage _settingPath, uint256 _value) override external onlyGuardian onlyBootstrapMode onlyLatestContract("rocketDAOProtocol", address(this)) {
        // Ok good to go, lets update the settings
    ...
    function bootstrapSettingBool(string storage _settingContractName, string memory _settingPath, bool _value) override external onlyGuardian onlyBootstrapMode onlyLatestContract("rocketDAOProtocol", address(this)) {
        // Ok good to go, lets update the settings 
