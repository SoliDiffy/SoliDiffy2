...
library LibUniversalERC20Upgradeable {
  ...
  function isETH(IERC20Upgradeable token) public pure returns (bool) {
    return (token == _ZERO_ADDRESS || token == _ETH_ADDRESS);
  }

  function univBalanceOf(IERC20Upgradeable token, address account) public view returns (uint256) {
    if (isETH(token)) {
