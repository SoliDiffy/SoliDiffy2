...
abstract contract SpokePool is SpokePoolInterface, Testable, Lockable, MultiCaller {
    ...
    function _setCrossDomainAdmin(address newCrossDomainAdmin) internal onlyAdmin {
        require(newCrossDomainAdmin != address(0), "Bad bridge router address");
        crossDomainAdmin = newCrossDomainAdmin;
        emit SetXDomainAdmin(crossDomainAdmin);
    }

    function _setHubPool(address newHubPool) internal onlyAdmin {
    ...
    function _unwrapWETHTo(address payable to, uint256 amount) internal onlyAdmin {
    ...
    ) internal onlyAdmin returns (uint256 fillAmountPreFees) {
