...
library GCDelegatedReserveManager
{
	...
	{
		if (_self.exchange == address(0)) return false;
		if (_self.miningMaxGulpAmount == 0) return false;
		uint256 _miningAmount = G.getBalance(_self.miningToken);
		if (_miningAmount == 0) return false;
		if (_miningAmount < _self.miningMinGulpAmount) return false;
	...
	{
		if (_self.exchange == address(0)) return false;
		if (_self.growthMaxGulpAmount == 0) return false;
		...
		if (_redeemableAmount <= _borrowAmount) return false;
		uint256 _growthAmount = _redeemableAmount.sub(_borrowAmount);
		if (_growthAmount < _self.growthMinGulpAmount) return false;
