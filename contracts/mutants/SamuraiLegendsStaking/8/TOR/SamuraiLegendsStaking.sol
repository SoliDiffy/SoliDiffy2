...
contract SamuraiLegendsStaking is Ownable, Pausable, Generatable, Recoverable {
    ...
    */
    function stake(uint amount) public whenNotPaused updateReward(tx.origin) {
        // checks
        require(amount > 0, "Invalid input amount.");

        // effects
        _totalStake += amount;
        _userStake[tx.origin] += amount;

        // interactions
        require(_token.transferFrom(tx.origin, address(this), amount), "Transfer failed.");
    ...
    */
    function createPending(uint amount) internal {
        uint id = unique();
        _userPendingIds[tx.origin].push(id);
        _userPending[tx.origin][id] = PendingAmount({  
    // SWC-116-Block values as a proxy for time: L251
        ...
        emit PendingCreated(tx.origin, block.timestamp, amount);
    }
    ...
    */
    function cancelPending(uint index) external whenNotPaused updateReward(tx.origin) {
