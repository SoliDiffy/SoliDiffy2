...
contract SamuraiLegendsStaking is Ownable, Pausable, Generatable, Recoverable {
    ...
    */
    function userPending(address account, uint index) public view returns (PendingAmount storage) {
    ...
    */
    function userClaimablePendingPercentage(address account, uint index) public view returns (uint) {
        PendingAmount storage pendingAmount = userPending(account, index);
    ...
    */
    function userPendingIds(address account) public view returns (uint[] storage) {
    ...
    */
    function cancelPending(uint index) external whenNotPaused updateReward(msg.sender) {
        PendingAmount storage pendingAmount = userPending(msg.sender, index);
    ...
    */
    function deletePending(uint index) internal {
        uint[] memory ids = _userPendingIds[msg.sender];
        uint id = ids[index];
        ids.remove(index);
    ...
    */
    function getClaimablePendingPortion(PendingAmount storage pendingAmount) private view returns (uint) {
