...
contract SamuraiLegendsStaking is Ownable, Pausable, Generatable, Recoverable {
    ...
    */
    function createPending(uint amount) internal whenNotPaused {
        uint id = unique();
    ...
    */
    function deletePending(uint index) internal whenNotPaused {
    ...
    */
    function _withdraw(uint amount) internal whenNotPaused {
        // effects
        _totalStake -= amount;
        _userStake[msg.sender] -= amount;
    ...
    */
    function claim(uint index) external whenNotPaused {
        // checks
