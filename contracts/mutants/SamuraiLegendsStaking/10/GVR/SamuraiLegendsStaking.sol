...
contract SamuraiLegendsStaking is Ownable, Pausable, Generatable, Recoverable {
    ...
    uint32 private _rewardUpdatedAt = uint32(block.number);
    ...
    */
    ...
    // SWC-116-Block values as a proxy for time: L156
        return rewardFinishedAt > block.number ? block.number : rewardFinishedAt;
    }
    ...
    */
    ...
    // SWC-116-Block values as a proxy for time: L251
        ...
        emit PendingCreated(msg.sender, block.number, amount);
    }
    ...
    */
    function getClaimablePendingPortion(PendingAmount memory pendingAmount) private view returns (uint) {
        ...
        return (block.number - pendingAmount.createdAt) / pendingAmount.pendingPeriod.period; // 0 1 2 3 4
    }
    ...
    */
    function addReward(uint _reward) external onlyOwner updateReward(address(0)) {
        ...
        if (block.number > rewardFinishedAt) { // Reward duration finished
            rewardRate = uint160(_reward / rewardDuration);
        } else {
            uint remainingReward = rewardRate * (rewardFinishedAt - block.number);
            rewardRate = uint160((remainingReward + _reward) / rewardDuration);
        }

        // effects
        _rewardUpdatedAt = uint32(block.number);
        rewardFinishedAt = uint32(block.number + rewardDuration);
