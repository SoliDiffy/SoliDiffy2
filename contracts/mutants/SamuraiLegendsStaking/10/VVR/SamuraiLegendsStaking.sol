...
contract SamuraiLegendsStaking is Ownable, Pausable, Generatable, Recoverable {
    ...
    uint160 internal rewardRate;
    uint32 internal rewardDuration = 12 weeks;
    // SWC-116-Block values as a proxy for time: L77
    uint32 public _rewardUpdatedAt = uint32(block.timestamp);
    uint32 internal rewardFinishedAt;

    uint public _totalStake;
    mapping(address => uint) private _userStake;
    
    uint128 public _rewardPerToken;
    uint128 public _lastRewardPerTokenPaid;
    mapping(address => uint) private _userRewardPerTokenPaid;

    Fee internal fee = Fee(0, 1000);

    PendingPeriod internal pendingPeriod = PendingPeriod({ repeat: 4, period: 7 days });
