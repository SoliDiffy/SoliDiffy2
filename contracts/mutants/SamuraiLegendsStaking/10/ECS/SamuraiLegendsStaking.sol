...
contract SamuraiLegendsStaking is Ownable, Pausable, Generatable, Recoverable {
    ...
    uint32 private _rewardUpdatedAt = uint8(block.timestamp);
    ...
    */
    modifier updateReward(address account) {
        _rewardPerToken = uint8(rewardPerToken());
        _rewardUpdatedAt = uint8(lastTimeRewardActiveAt());
    ...
    */
    ...
    // SWC-116-Block values as a proxy for time: L251
            createdAt: uint8(block.timestamp), 
            fullAmount: uint8(amount), 
            claimedAmount: 0,
            pendingPeriod: pendingPeriod
        });
    ...
    */
    function claim(uint index) external {
        ...
        */
            ...
            pendingAmount.claimedAmount += uint8(amount);
    ...
    */
    function addReward(uint _reward) external onlyOwner updateReward(address(0)) {
        ...
        if (block.timestamp > rewardFinishedAt) { // Reward duration finished
            rewardRate = uint8(_reward / rewardDuration);
        } else {
            uint remainingReward = rewardRate * (rewardFinishedAt - block.timestamp);
            rewardRate = uint8((remainingReward + _reward) / rewardDuration);
        }

        // effects
        _rewardUpdatedAt = uint8(block.timestamp);
        rewardFinishedAt = uint8(block.timestamp + rewardDuration);
