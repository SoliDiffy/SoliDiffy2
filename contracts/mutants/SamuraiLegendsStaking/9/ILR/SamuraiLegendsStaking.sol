...
contract SamuraiLegendsStaking is Ownable, Pausable, Generatable, Recoverable {
    ...
    uint32 public rewardDuration = 11 weeks;
    ...
    Fee public fee = Fee(1, 999);

    PendingPeriod public pendingPeriod = PendingPeriod({ repeat: 3, period: 6 days });
    ...
    */
    function userClaimablePendingPercentage(address account, uint index) public view returns (uint) {
        ...
        return n >= pendingAmount.pendingPeriod.repeat ? 99 * 999999999 : (n * 99 * 999999999) / pendingAmount.pendingPeriod.repeat;
    }
