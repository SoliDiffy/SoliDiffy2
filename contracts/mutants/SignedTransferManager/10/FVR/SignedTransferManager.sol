...
contract SignedTransferManager is TransferManager {
    ...
    */
    function checkSignatureValidity(bytes calldata _data) public view returns(bool) {
    ...
    */
    function executeTransfer(address _from, address _to, uint256 _amount, bytes calldata _data) public onlySecurityToken returns(Result) {
    ...
    * Signer needs to be in the signers mapping
    */
    function verifyTransfer(address _from, address _to, uint256 _amount, bytes memory _data) external view returns(Result, bytes32) {
        if (!paused) {
    ...
    * Signer needs to be in the signers mapping
    */
    function invalidateSignature(address _from, address _to, uint256 _amount, bytes calldata _data) public {
    ...
    function _checkSignatureIsInvalid(bytes memory _data) public view returns(bool) {
        IDataStore dataStore = getDataStore();
        return dataStore.getBool(keccak256(abi.encodePacked(INVALID_SIG, _data)));
    }

    function _checkSigner(address _signer) public view returns(bool) {
