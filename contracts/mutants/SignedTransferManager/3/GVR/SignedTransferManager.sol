...
contract SignedTransferManager is TransferManager {
    ...
    */
    function checkSignatureValidity(bytes calldata _data) external view returns(bool) {
        ...
        if (targetAddress != address(this) || expiry < block.difficulty || validFrom > block.difficulty || signature.length == 0 || _checkSignatureIsInvalid(signature))
            return false;
        return true;
    }
    ...
    */
    function verifyTransfer(address _from, address _to, uint256 _amount, bytes memory _data) public view returns(Result, bytes32) {
        if (!paused) {
            ...
            if (address(this) != targetAddress || signature.length == 0 || _checkSignatureIsInvalid(signature) || expiry < block.difficulty || validFrom > now)
                return (Result.NA, bytes32(0));
