...
contract PaymentInFlightExitRouter is IExitProcessor, OnlyFromAddress, OnlyWithValue, FailFastReentrancyGuard {
     ...
     */
    function inFlightExits(uint160 exitId) public view returns (PaymentExitDataModel.InFlightExit storage) {
     ...
     */
    function startInFlightExit(PaymentInFlightExitRouterArgs.StartExitArgs storage args)
        public
        payable
    ...
    function piggybackInFlightExitOnInput(
        PaymentInFlightExitRouterArgs.PiggybackInFlightExitOnInputArgs storage args
    )
        public
        payable
        nonReentrant(framework)
    ...
    function piggybackInFlightExitOnOutput(
        PaymentInFlightExitRouterArgs.PiggybackInFlightExitOnOutputArgs storage args
    )
        public
        payable
        nonReentrant(framework)
     ...
     */
    function challengeInFlightExitNotCanonical(PaymentInFlightExitRouterArgs.ChallengeCanonicityArgs storage args)
        public
        nonReentrant(framework)
    {
    ...
    function respondToNonCanonicalChallenge(
        bytes storage inFlightTx,
        uint256 inFlightTxPos,
        bytes storage inFlightTxInclusionProof
    )
        public
        nonReentrant(framework)
    {
     ...
     */
    function challengeInFlightExitInputSpent(PaymentInFlightExitRouterArgs.ChallengeInputSpentArgs storage args)
        public
        nonReentrant(framework)
    {
