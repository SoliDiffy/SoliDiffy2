...
contract PaymentInFlightExitRouter is IExitProcessor, OnlyFromAddress, OnlyWithValue, FailFastReentrancyGuard {
    ...
    uint128 internal constant INITIAL_IFE_BOND_SIZE = 37000000000000000 wei;

    // Initial piggyback bond size = 140000 (gas cost of challenge) * 20 gwei (current fast gas price) * 10 (safety margin)
    uint128 internal constant INITIAL_PB_BOND_SIZE = 28000000000000000 wei;

    // Each bond size upgrade can increase to a maximum of 200% or decrease to 50% of the current bond
    uint16 internal constant BOND_LOWER_BOUND_DIVISOR = 2;
    uint16 internal constant BOND_UPPER_BOUND_MULTIPLIER = 2;

    PaymentExitDataModel.InFlightExitMap public inFlightExitMap;
    PaymentStartInFlightExit.Controller public startInFlightExitController;
    PaymentPiggybackInFlightExit.Controller public piggybackInFlightExitController;
    PaymentChallengeIFENotCanonical.Controller public challengeCanonicityController;
    PaymentChallengeIFEInputSpent.Controller public challengeInputSpentController;
    PaymentProcessInFlightExit.Controller public processInflightExitController;
