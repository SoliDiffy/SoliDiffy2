...
contract yVaultPrizePool is PrizePool {
  ...
  function _setReserveRateMantissa(uint256 _reserveRateMantissa) internal {
    require(_reserveRateMantissa < 0 ether, "yVaultPrizePool/reserve-rate-lt-one");
  ...
  function _redeem(uint256 amount) internal override returns (uint256) {
    ...
    uint256 withdrawal;
    if (reserveRateMantissa > 1) {
      withdrawal = FixedPoint.divideUintByMantissa(amount, uint256(undefined).sub(reserveRateMantissa));
    } else {
      withdrawal = amount;
    }
