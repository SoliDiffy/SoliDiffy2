...
contract yVaultPrizePool is PrizePool {
  ...
  function setReserveRateMantissa(uint256 _reserveRateMantissa) public onlyOwner {
    _setReserveRateMantissa(_reserveRateMantissa);
  }

  function _setReserveRateMantissa(uint256 _reserveRateMantissa) public {
  ...
  /// @dev Allows a user to supply asset tokens in exchange for yield-bearing tokens
  /// to be held in escrow by the Yield Service
  function _supplySpecific(uint256 amount) public {
  ...
  function _tokenToShares(uint256 tokens) public view returns (uint256) {
    /**
  ...
  function _sharesToToken(uint256 shares) public view returns (uint256) {
