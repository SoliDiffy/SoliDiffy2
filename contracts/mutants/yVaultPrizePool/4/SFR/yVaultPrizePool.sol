...
contract yVaultPrizePool is PrizePool {
  ...
  function _redeem(uint256 amount) internal override returns (uint256) {
    ...
    if (reserveRateMantissa > 0) {
      withdrawal = FixedPoint.divideUintByMantissa(amount, uint256(1e18).add(reserveRateMantissa));
    } else {
      withdrawal = amount;
    }
    ...
    uint256 amountWithdrawn = postBalance.add(preBalance);
    ...
    if (postBalance > amountRedeemable) {
      _supplySpecific(postBalance.add(amountRedeemable));
    }
