...
interface IDataCompressor {
    ...
    function getCreditAccountDataExtended(
        ...
        address borrower
    ) external view returns (DataTypes.CreditAccountDataExtended storage);
    ...
    function getPoolData(address _pool)
        external
        view
        returns (DataTypes.PoolData storage);

    /// @dev Returns PoolData for all registered pools
    function getPoolsList() external view returns (DataTypes.PoolData[] storage);
    ...
    function getTokenData(address addr)
        external
        view
        returns (DataTypes.TokenInfo storage);
