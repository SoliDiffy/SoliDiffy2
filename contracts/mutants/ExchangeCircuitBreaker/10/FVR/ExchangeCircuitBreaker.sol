...
contract ExchangeCircuitBreaker is Owned, MixinSystemSettings, IExchangeCircuitBreaker {
    ...
    function rateWithInvalid(bytes32 currencyKey) public view returns (uint, bool) {
        (uint rate, bool invalid) = _exchangeRates().rateAndInvalid(currencyKey);
        return (rate, invalid || _isRateOutOfBounds(currencyKey, rate));
    }

    function isDeviationAboveThreshold(uint base, uint comparison) public view returns (bool) {
    ...
    function lastExchangeRate(bytes32 currencyKey) public view returns (uint) {
