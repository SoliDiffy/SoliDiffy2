...
contract PLCRVoting {
    ...
    */
    function getNumPassingTokens(address _voter, uint _pollID, uint _salt) external constant returns (uint correctVotes) {
        require(pollEnded(_pollID));
...
// SWC-101-Integer Overflow and Underflow: L211
    ...
    @param _pollID Integer identifier associated with target poll
    */
    function isPassed(uint _pollID) constant external returns (bool passed) {
        require(pollEnded(_pollID));
