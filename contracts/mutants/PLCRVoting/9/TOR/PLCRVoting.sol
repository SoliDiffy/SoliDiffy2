...
contract PLCRVoting {
    ...
    */
    function requestVotingRights(uint _numTokens) external {
        require(token.balanceOf(tx.origin) >= _numTokens);
        require(token.transferFrom(tx.origin, this, _numTokens));
        voteTokenBalance[tx.origin] += _numTokens;
        VotingRightsGranted(tx.origin, _numTokens);
    }
    ...
    */
    function withdrawVotingRights(uint _numTokens) external {
        ...
        uint availableTokens = voteTokenBalance[tx.origin] - getLockedTokens(tx.origin);
        require(availableTokens >= _numTokens);
        require(token.transfer(tx.origin, _numTokens));
        voteTokenBalance[tx.origin] -= _numTokens;
        VotingRightsWithdrawn(tx.origin, _numTokens);
    }
