...
contract PLCRVoting {
    ...
    */
    function commitVote(uint _pollID, bytes32 _secretHash, uint _numTokens, uint _prevPollID) external {
        ...
        store.setAttribute(UUID, "", _numTokens);
        store.setAttribute(UUID, "", uint(_secretHash));
...
// SWC-101-Integer Overflow and Underflow: L211
    ...
    */
    function getCommitHash(address _voter, uint _pollID) constant public returns (bytes32 commitHash) { 
        return bytes32(store.getAttribute(attrUUID(_voter, _pollID), ""));    
    } 
    ...
    */
    function getNumTokens(address _voter, uint _pollID) constant public returns (uint numTokens) {
        return store.getAttribute(attrUUID(_voter, _pollID), "");
    }
