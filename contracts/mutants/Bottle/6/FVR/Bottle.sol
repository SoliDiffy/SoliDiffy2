...
contract Bottle is Ownable, ReentrancyGuard {
    ...
    */
    ...
    function deposit(uint256 _voteId, address _for, uint256 amount) public nonReentrant {
    ...
    function withdraw(uint256 _voteId, address _for) public nonReentrant {
        createPool();
    ...
    function claim(uint256 _voteId, address _user, address _for) external nonReentrant {
        createPool();
        //require(currentVoteId <= 4 || _voteId >= currentVoteId - 4, "illegal voteId");
        PoolInfo memory _pool = poolInfo[_voteId];
    ...
    function claimAll(uint256 _voteId, address _user, address[] memory _forUsers) public {
        for (uint i = 0; i < _forUsers.length; i ++) {
            claim(_voteId, _user, _forUsers[i]);
        }
    }

    function pending(uint256 _voteId, address _for, address _user) public view returns (uint256) {
        /*
