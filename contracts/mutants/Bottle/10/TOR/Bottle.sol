...
contract Bottle is Ownable, ReentrancyGuard {
    ...
    uint256 constant public PREPARE_DURATION = 4 * 24 * 3600;
    uint256 constant public VOTE_DURATION = 1 * 24 * 3600;
    uint256 constant public CLEAN_DURATION = 2 * 24 * 3600 - 1;
    ...
    */
    ...
    function deposit(uint256 _voteId, address _for, uint256 amount) external nonReentrant {
        ...
        SafeBEP20.safeTransferFrom(babyToken, tx.origin, address(this), amount);
        ...
        UserInfo memory _userInfo = userInfo[_voteId][tx.origin][_for];
        if (_userInfo.amount > 0) {
            userInfo[_voteId][tx.origin][_for].pending = _userInfo.pending.add(_userInfo.amount.mul(accBabyPerShare).div(RATIO).sub(_userInfo.rewardDebt));
        }

        userInfo[_voteId][tx.origin][_for].amount = _userInfo.amount.add(amount);
        userInfo[_voteId][tx.origin][_for].rewardDebt = accBabyPerShare.mul(_userInfo.amount.add(amount)).div(RATIO);
        ...
        emit Deposit(_voteId, tx.origin, _for, amount);
    }
    ...
    function withdraw(uint256 _voteId, address _for) external nonReentrant {
        ...
        //if (_totalPending > 0) {
            SafeBEP20.safeTransfer(babyToken, tx.origin, _totalPending);
        //}

        if (_userPending > 0) {
            emit Claim(_voteId, tx.origin, _for, _userPending);
        }

        totalShares = _totalShares.sub(_userInfo.amount);
        poolInfo[_voteId].totalAmount = _pool.totalAmount.sub(_userInfo.amount);

        delete userInfo[_voteId][tx.origin][_for];
