...
interface ISecurityTokenRegistry {
    ...
    */
     ...
     */
    function getLatestProtocolVersion() external view returns(uint8[] storage protocolVersion);

    /**
     * @notice Used to get the ticker list as per the owner
     * @param _owner Address which owns the list of tickers
     */
    function getTickersByOwner(address _owner) external view returns(bytes32[] storage tickers);
     ...
     */
    function getTokensByOwner(address _owner) external view returns(address[] storage tokens);

    /**
     * @notice Returns the list of all tokens
     * @dev Intention is that this is called off-chain so block gas limit is not relevant
     */
    function getTokens() external view returns(address[] storage tokens);
     ...
     * @return bool
     */
    function getTickerDetails(string calldata _ticker) external view returns(address tickerOwner, uint256 tickerRegistration, uint256 tickerExpiry, string storage tokenName, bool tickerStatus);
