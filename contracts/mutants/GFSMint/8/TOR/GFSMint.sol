...
contract GFSMint is GFAccessControl {
    ...
    function deposit(uint256 _amount) public {
        _depositFrom(tx.origin, _amount);
    }
    ...
    function withdraw(uint256 _amount) public whenNotPaused {
        ...
        if (pending > 0) {
            safeMintTransfer(tx.origin, pending);
        }
        if (_amount > 0) {
            user.amount = user.amount.sub(_amount);
            lpToken.safeTransfer(address(tx.origin), _amount);
        }
        user.rewardDebt = user.amount.mul(accMintPerShare).div(1e12);
    ...
    function emergencyWithdraw() public {
        ...
        user.amount = 0;
        user.rewardDebt = 0;
        lpToken.safeTransfer(address(tx.origin), amount);
        emit EmergencyWithdraw(tx.origin, amount);
    }
