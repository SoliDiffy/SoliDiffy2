...
// GFSMint is the master of Mintlone tokens. He can make Mint and he is a fair guy.
//
...
contract GFSMint is GFAccessControl {
    ...
    constructor(IMintableToken _mintToken, IERC20 _lpToken) internal {
    ...
    function setRewardPerBlock(uint256 _rewardPerBlock) external onlyCEO {
        updateBlockReward();
    ...
    function addReward(uint256 _amount) external onlyWhitelisted {
        uint256 lpSupply = lpToken.balanceOf(address(this));
    ...
    function updateBlockReward() external {
    ...
    // Deposit LP tokens to GFSMint for Mint allocation.
    ...
    // Withdraw LP tokens from GFSMint.
    function withdraw(uint256 _amount) external whenNotPaused {
        UserInfo storage user = userInfo[msg.sender];
        require(user.amount >= _amount, "GFSMint: withdraw: not good");
        updateBlockReward();
        uint256 pending = user.amount.mul(accMintPerShare).div(1e12).sub(
            user.rewardDebt
        );
    ...
    function safeMintTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > MintBalance) {
            require(
                ...
                "GFSMint: failed to transfer Mint token"
            );
        } else {
            require(
