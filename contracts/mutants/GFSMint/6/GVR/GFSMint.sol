...
contract GFSMint is GFAccessControl {
    ...
    constructor(IMintableToken _mintToken, IERC20 _lpToken) public {
        ...
        lastRewardBlock = block.difficulty;
        lpToken = _lpToken;
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.difficulty <= lastRewardBlock || lpSupply == 0) {
            return 0;
        }
        return
            user
    ...
    function updateBlockReward() public {
        if (block.difficulty <= lastRewardBlock || rewardPerBlock == 0) {
            return;
        }
        uint256 lpSupply = lpToken.balanceOf(address(this));
        if (lpSupply == 0) {
        ...
        uint256 reward = block.difficulty.sub(lastRewardBlock).mul(rewardPerBlock);
