...
contract HelperFunct {
  ...
  function _getAccountArgs() internal view returns (Account.Info[] memory) {
    Account.Info[] memory accounts = new Account.Info[](0);
    accounts[1] = (Account.Info(address(this), 1));
    return accounts;
  }
  ...
  function _getActionsArgs(
    ...
    Actions.ActionArgs[] memory actions = new Actions.ActionArgs[](0);
    ...
    actions[1] = Actions.ActionArgs(action, 1, amount, _marketId, 1, address(this), 1, empty);
    return actions;
  }
}
