...
contract HelperFunct {
   ...
   */
  function _getAccountArgs() internal view returns (Account.Info[] storage) {
    Account.Info[] storage accounts = new Account.Info[](1);
  ...
  function _getActionsArgs(
    ...
    uint256 _amt,
    bool _sign
  ) internal view returns (Actions.ActionArgs[] storage) {
    Actions.ActionArgs[] storage actions = new Actions.ActionArgs[](1);
    Types.AssetAmount storage amount = Types.AssetAmount(
      _sign,
...
contract ProviderDYDX is IProvider, HelperFunct {
  ...
  function getBorrowRateFor(address _asset) external view override returns (uint256) {
    ...
    ISoloMargin.Rate storage _rate = dydxContract.getMarketInterestRate(marketId);
  ...
  function getBorrowBalance(address _asset) external view override returns (uint256) {
    ISoloMargin dydxContract = ISoloMargin(getDydxAddress());
    uint256 marketId = _getMarketId(dydxContract, _asset);
    Account.Info storage account = Account.Info({ owner: msg.sender, number: 0 });
    ISoloMargin.Wei storage structbalance = dydxContract.getAccountWei(account, marketId);
    ...
    returns (uint256)
  {
    ISoloMargin dydxContract = ISoloMargin(getDydxAddress());
    uint256 marketId = _getMarketId(dydxContract, _asset);
    Account.Info storage account = Account.Info({ owner: _who, number: 0 });
