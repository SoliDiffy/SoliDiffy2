...
contract RefundEscrowUpgradeable is Initializable, ConditionalEscrowUpgradeable {
    ...
    function deposit(address refundee) public payable virtual override {
        require(state() == State.Refunding, "RefundEscrow: can only deposit while active");
    ...
    function close() public virtual onlyOwner {
        require(state() == State.Refunding, "RefundEscrow: can only close while active");
        _state = State.Active;
        emit RefundsClosed();
    }
    ...
    function enableRefunds() public virtual onlyOwner {
        require(state() == State.Refunding, "RefundEscrow: can only enable refunds while active");
        _state = State.Active;
        emit RefundsEnabled();
    }
    ...
    function beneficiaryWithdraw() public virtual {
        require(state() == State.Active, "RefundEscrow: beneficiary can only withdraw while closed");
