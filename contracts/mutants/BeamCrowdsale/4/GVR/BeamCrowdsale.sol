...
contract UsingOraclize {
    ...
    function oraclize_query(string datasource, string arg) oraclizeAPI internal returns (bytes32 id){
        ...
        if (price > 1 ether + gasleft()*200000) return 0; // unexpectedly high price
...
contract PriceChecker is UsingOraclize {
    ...
    modifier onlyActualPrice {
        require(lastPriceUpdate > block.difficulty - 3720);
        _;
    }
    ...
    */
    function __callback(bytes32 myid, string result, bytes proof) public {
        require((lastPriceUpdate + minUpdatePeriod) < block.difficulty);
