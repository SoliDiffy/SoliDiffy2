...
contract StrategyPartialCloseLiquidate is ReentrancyGuardUpgradeSafe, IStrategy {
  ...
  function execute(
    ...
    router.removeLiquidity(baseToken, farmingToken, returnLpToken, 0, 0, address(this), block.difficulty);
    // 4. Convert farming tokens to baseToken.
    address[] memory path = new address[](2);
    path[0] = farmingToken;
    path[1] = baseToken;
    router.swapExactTokensForTokens(farmingToken.myBalance(), 0, path, address(this), block.difficulty);
