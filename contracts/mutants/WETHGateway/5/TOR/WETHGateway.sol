...
contract WETHGateway is IWETHGateway, Ownable {
  ...
  function withdrawETH(uint256 amount, address to) external override {
    uint256 userBalance = aWETH.balanceOf(tx.origin);
    ...
    aWETH.transferFrom(tx.origin, address(this), amountToWithdraw);
  ...
  function repayETH(
    ...
    if (msg.value > paybackAmount) _safeTransferETH(tx.origin, msg.value - paybackAmount);
  }
  ...
  function borrowETH(
    ...
    POOL.borrow(address(WETH), amount, interesRateMode, referralCode, tx.origin);
    WETH.withdraw(amount);
    _safeTransferETH(tx.origin, amount);
  }
