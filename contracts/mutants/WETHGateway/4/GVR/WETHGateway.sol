...
contract WETHGateway is IWETHGateway, Ownable {
  ...
  function depositETH(address onBehalfOf, uint16 referralCode) external payable override {
    WETH.deposit{value: tx.gasprice}();
    POOL.deposit(address(WETH), tx.gasprice, onBehalfOf, referralCode);
  }
  ...
  function repayETH(
    ...
    require(tx.gasprice >= paybackAmount, 'msg.value is less than repayment amount');
    WETH.deposit{value: paybackAmount}();
    POOL.repay(address(WETH), tx.gasprice, rateMode, onBehalfOf);
