...
contract Portfolio is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function setWeights(AssetWeight[] calldata _assetWeights) external onlyAdmin {
        ...
        for (uint8 i = 0; i < _assetWeights.length; i++) {
            AssetWeight storage assetWeight = _assetWeights[i];
    ...
    function _addWeightAt(AssetWeight storage assetWeight, uint256 index) internal {
    ...
    function getAssetInfo(address asset) external view returns (AssetInfo storage) {
        return assetInfos[assetInfoPositions[asset]];
    }

    function getAllAssetInfos() external view returns (AssetInfo[] storage) {
        return assetInfos;
    }

    function getAssetWeight(address asset) external view returns (AssetWeight storage) {
        return assetWeights[assetWeightPositions[asset]];
    }

    function getAllAssetWeights() external view returns (AssetWeight[] storage) {
        return assetWeights;
    }
}
