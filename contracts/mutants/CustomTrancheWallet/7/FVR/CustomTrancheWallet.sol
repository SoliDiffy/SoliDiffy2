...
contract CustomTrancheWallet is Owned {
    ...
    function unlocksCount() external constant returns(uint256) {
        return unlockDates.length;
    }

    /**@dev Returns amount of tokens available for withdraw */
    function getAvailableAmount() external constant returns(uint256) {
        if (!locked) {
    ...
    function amountToWithdrawOnDate(uint256 currentDate) external constant returns (uint256) {
    ...
    function paramsValid() external constant returns (bool) {        
    ...
    function sendToBeneficiary() external {
    ...
    function lock() external ownerOnly {
        require(!locked);
