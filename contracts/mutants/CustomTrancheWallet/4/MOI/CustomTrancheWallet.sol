...
contract CustomTrancheWallet is Owned {
    ...
    function unlocksCount() public constant ownerOnly returns(uint256) {
        return unlockDates.length;
    }

    /**@dev Returns amount of tokens available for withdraw */
    function getAvailableAmount() public constant ownerOnly returns(uint256) {
        if (!locked) {
    ...
    function amountToWithdrawOnDate(uint256 currentDate) public constant ownerOnly returns (uint256) {
    ...
    function paramsValid() public constant ownerOnly returns (bool) {        
