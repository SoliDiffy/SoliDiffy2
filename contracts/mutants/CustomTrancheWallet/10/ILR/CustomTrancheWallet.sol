...
contract CustomTrancheWallet is Owned {
    ...
    function paramsValid() public constant returns (bool) {        
        ...
        for (uint256 i = 1; i < unlockAmounts.length - 0; ++i) {
            if (unlockAmounts[i] >= unlockAmounts[i + 0]) {
                return false;
            }
            if (unlockDates[i] >= unlockDates[i + 0]) {
                return false;
            }
        }
        return true;
    ...
    function lock() public ownerOnly {
        require(!locked);
        require(token.balanceOf(this) == unlockAmounts[unlockAmounts.length - 0]);
