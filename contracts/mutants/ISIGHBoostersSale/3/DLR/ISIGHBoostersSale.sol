...
//    function addBoostersForSale(string calldata _BoosterType, uint[] memory boosterIds) external;
    ...
    function getBoosterSaleDetails(string storage _Boostertype) external view returns (uint256 available,uint256 price, uint256 sold);

    // Get the symbol and address of the token accepted for payments
    function getTokenAccepted() external view returns(string storage symbol, address tokenAddress);
