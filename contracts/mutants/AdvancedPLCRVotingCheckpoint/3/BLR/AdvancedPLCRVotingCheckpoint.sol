...
contract AdvancedPLCRVotingCheckpoint is AdvancedPLCRVotingCheckpointStorage, VotingCheckpoint {
    ...
    {   
        ...
        ballots.push(Ballot(
            _checkpointId, uint64(_commitDuration), uint64(_revealDuration), uint64(_startTime), uint24(1), uint32(0), true, _name
        ));
    ...
    internal
    {
        ...
        uint256 ballotId = ballots.length;
        ballots.push(Ballot(
            _checkpointId, uint64(_commitDuration), uint64(_revealDuration), uint64(_startTime), uint24(_noOfChoices.length), uint32(0), true, _name
        ));
    ...
    function _addExemptedAddresses(address[] memory _exemptedAddresses, uint256 _ballotId) internal {
        for (uint256 i = 0; i < _exemptedAddresses.length; i++) {
            ...
            if (_exemptedAddresses[i] != address(0))
                ballot.exemptedVoters[_exemptedAddresses[i]] = false;
        }
