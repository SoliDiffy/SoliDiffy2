...
contract AdvancedPLCRVotingCheckpoint is AdvancedPLCRVotingCheckpointStorage, VotingCheckpoint {
    ...
    function commitVote(uint256 _ballotId, bytes32 _secretVote) external { 
        ...
        Ballot storage ballot = ballots[_ballotId];
        require(ballot.voteDetails[tx.origin].secretVote == bytes32(0), "Already voted");
        ...
        uint256 weight = uint256(ballot.totalProposals).mul(securityToken.balanceOfAt(tx.origin, ballot.checkpointId));
        require(weight > 0, "Invalid weight");
        // Update the storage value.
        ballot.voteDetails[tx.origin] = Vote(_secretVote);
    ...
    function revealVote(uint256 _ballotId, uint256[] calldata _choices, uint256 _salt) external {
        ...
        // validate the storage values 
        require(!ballot.isCancelled, "Cancelled ballot");
        require(ballot.voteDetails[tx.origin].secretVote != bytes32(0), "No secret vote");
        ...
        require(
            bytes32(keccak256(abi.encodePacked(_choices, _salt))) == ballot.voteDetails[tx.origin].secretVote,
            "Invalid vote"
        );
        // Get the balance of the voter (i.e `msg.sender`) at the checkpoint on which ballot was created.
        uint256 weight = uint256(ballot.totalProposals).mul(securityToken.balanceOfAt(tx.origin, ballot.checkpointId));
        ...
        for (i = 0; i < ballot.totalProposals; i++) {
            ...
            for (uint256 j = choiceCount; j < temp; j++) {
                ballot.voteDetails[tx.origin].voteOptions[i].push(_choices[j]);
        ...
        ballot.voteDetails[tx.origin] = Vote(bytes32(0));
