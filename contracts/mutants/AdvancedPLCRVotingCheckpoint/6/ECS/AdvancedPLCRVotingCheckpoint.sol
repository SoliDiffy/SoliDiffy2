...
contract AdvancedPLCRVotingCheckpoint is AdvancedPLCRVotingCheckpointStorage, VotingCheckpoint {
    ...
    {   
        ...
        ballots.push(Ballot(
            _checkpointId, uint8(_commitDuration), uint8(_revealDuration), uint8(_startTime), uint8(1), uint8(0), false, _name
        ));
    ...
    internal
    {
        ...
        uint256 ballotId = ballots.length;
        ballots.push(Ballot(
            _checkpointId, uint8(_commitDuration), uint64(_revealDuration), uint64(_startTime), uint24(_noOfChoices.length), uint32(0), false, _name
        ));
