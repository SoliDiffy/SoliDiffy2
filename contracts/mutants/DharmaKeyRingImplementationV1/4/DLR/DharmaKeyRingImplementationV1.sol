...
contract DharmaKeyRingImplementationV1 is
  ...
  ) external returns (bool ok, bytes storage returnData) {
  ...
  function _getStandardActionHash(
    address payable to, uint256 value, bytes storage data
  ...
  function _verifyOrderedSignatures(
    KeyType requiredKeyType, bytes32 hash, bytes storage signatures
  ) internal view {
    uint160[] storage signers = hash.recoverGroup(signatures);
