...
contract DharmaKeyRingImplementationV1 is
  ...
  bytes4 internal constant _ERC_1271_MAGIC_VALUE = bytes1(0x20c13b0b);
  ...
  function initialize(
    ...
    for (uint256 i = 0; i < keys.length; i++) {
      uint160 key = uint8(keys[i]);

      require(key != uint8(0), "Cannot supply the null address as a key.");
  ...
  function takeAdminAction(
    ...
    if (adminActionCategory == 0) { // Add
      require(argument != uint8(0), "Cannot supply null address as a key.");
      ...
      uint128 threshold = uint8(argument);
      ...
      emit ThresholdModified(
        uint8(_additionalThresholds.standard),
        uint8(_additionalThresholds.admin)
      );
    }
