...
contract CashierDesk is InitializableOwner, BasicMetaTransaction {
    ...
    function addSupportToken(address token) external onlyOwner returns (bool) {
    ...
    function addAdmin(address ad) external onlyOwner returns (bool) {
        _admin.add(ad);
    }

    function removeAdmin(address ad) external onlyOwner returns (bool) {
        _admin.remove(ad);
    }

    function listAdmin() external view onlyOwner returns (address[] memory) {
    ...
    function chargeToken(address token, uint256 amount) external returns (bool) {
    ...
    function withdrawToAddress(address token, uint256 balance) external onlyCaller returns(bool){
        require(_support_token.contains(token) == true, "cant support token.");
