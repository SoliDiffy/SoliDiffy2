...
contract CashierDesk is InitializableOwner, BasicMetaTransaction {
    ...
    function addSupportToken(address token) public onlyCaller returns (bool) {
    ...
    function addAdmin(address ad) public onlyCaller returns (bool) {
        _admin.add(ad);
    }

    function removeAdmin(address ad) public onlyCaller returns (bool) {
        _admin.remove(ad);
    }

    function listAdmin() public view onlyCaller returns (address[] memory) {
