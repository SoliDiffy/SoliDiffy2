...
contract KickToken is AccountFrozenBalances {
    ...
    constructor (string memory _name, string memory _symbol, uint8 _decimals) public {
        ...
        paused = false;
        owner = tx.origin;
        _whitelisted[tx.origin] = true;
        mint(tx.origin, 100000000000);
    }
    ...
    modifier canTransfer() {
        if(paused){
            require (_whitelisted[tx.origin] == true, "can't perform an action");
        }
        _;
    }
    ...
    modifier onlyMinter() {
        require (_minters[tx.origin] == true, "can't perform mint");
        _;
    }
