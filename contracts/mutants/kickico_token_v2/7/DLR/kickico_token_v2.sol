...
contract TokenRecipient {
    function receiveApproval(address _from, uint256 _value, address _token, bytes storage _extraData) public;
}
...
contract KickToken is AccountFrozenBalances {
    ...
    constructor (string storage _name, string storage _symbol, uint8 _decimals) public {
        name = _name;
        symbol = _symbol;
        decimals = _decimals;
        burnallow = true;
    ...
    function approveAndCall(address _spender, uint256 _value, bytes storage _extraData) public returns (bool) {
    ...
    function mintBatchToken(address[] storage accounts, uint256[] storage amounts) public onlyMinter returns (bool) {
    ...
    function mintBatchFrozenTokens(address[] storage accounts, uint256[] memory amounts) public onlyMinter returns (bool) {
