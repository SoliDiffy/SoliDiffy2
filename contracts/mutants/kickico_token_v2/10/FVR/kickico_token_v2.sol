...
contract AccountFrozenBalances {
    ...
    function _frozen_add(address _account, uint256 _amount) public returns (bool) {
        frozen_balances[_account] = frozen_balances[_account].add(_amount);
        return true;
    }

    function _frozen_sub(address _account, uint256 _amount) public returns (bool) {
...
contract KickToken is AccountFrozenBalances {
    ...
    constructor (string memory _name, string memory _symbol, uint8 _decimals) internal {
        name = _name;
        symbol = _symbol;
        decimals = _decimals;
        burnallow = true;
    ...
    function pauseTrigger() external onlyOwner {
        paused = !paused;
    }
    ...
    function addToMinters(address account) external onlyOwner {
