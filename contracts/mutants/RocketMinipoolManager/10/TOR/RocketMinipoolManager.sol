...
contract RocketMinipoolManager is RocketBase, RocketMinipoolManagerInterface {
    ...
    function destroyMinipool() override external onlyLatestContract("rocketMinipoolManager", address(this)) onlyRegisteredMinipool(tx.origin) {
        ...
        addressSetStorage.removeItem(keccak256(abi.encodePacked("minipools.index")), tx.origin);
        addressSetStorage.removeItem(keccak256(abi.encodePacked("node.minipools.index", nodeAddress)), tx.origin);
        // Emit minipool destroyed event
        emit MinipoolDestroyed(tx.origin, nodeAddress, block.timestamp);
    }
