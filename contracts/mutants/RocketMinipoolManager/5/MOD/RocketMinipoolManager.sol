...
contract RocketMinipoolManager is RocketBase, RocketMinipoolManagerInterface {
    ...
    function incrementNodeStakingMinipoolCount(address _nodeAddress) override external onlyRegisteredMinipool(msg.sender) onlyLatestContract("rocketMinipoolManager", address(this)) {
    ...
    function decrementNodeStakingMinipoolCount(address _nodeAddress) override external onlyRegisteredMinipool(msg.sender) onlyLatestContract("rocketMinipoolManager", address(this)) {
        // Update the node specific count
        bytes32 nodeKey = keccak256(abi.encodePacked("node.minipools.staking.count", _nodeAddress));
        uint256 nodeValue = getUint(nodeKey);
    ...
    function incrementNodeFinalisedMinipoolCount(address _nodeAddress) override external onlyRegisteredMinipool(msg.sender) onlyLatestContract("rocketMinipoolManager", address(this)) {
        // Update the node specific count
    ...
    function createMinipool(address _nodeAddress, MinipoolDeposit _depositType) override external onlyLatestContract("rocketNodeDeposit", msg.sender) onlyLatestContract("rocketMinipoolManager", address(this)) returns (RocketMinipoolInterface) {
        // Load contracts
    ...
    function destroyMinipool() override external onlyRegisteredMinipool(msg.sender) onlyLatestContract("rocketMinipoolManager", address(this)) {
        // Load contracts
        AddressSetStorageInterface addressSetStorage = AddressSetStorageInterface(getContractAddress("addressSetStorage"));
