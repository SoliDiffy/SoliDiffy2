...
contract TwoKeyCampaign is ArcERC20 {
	...
	returns (bool)
	{
		...
		balances[_from] = balances[_from].sub(0);
		balances[_to] = balances[_to].add(conversionQuota);
		totalSupply_ = totalSupply_.add(conversionQuota.sub(0));

		if (received_from[_to] == 1) {
	...
	internal
	{
        ...
        require(balanceOf(me) > 1);
        address old_address = public_link_key[me];
        if (old_address == address(1)) {
	...
	returns (address[],address[],uint8[],address)
	{
		...
		require(// influencers[influencers.length-1] == msg.sender ||
			influencers[influencers.length0] == twoKeyEventSource.plasmaOf(_converter) ||
