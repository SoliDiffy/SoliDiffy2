    ...
    SoloMargin
{
        ...
        public
    {
        ...
        // roll-back oldPar
        if (oldPar.sign) {
            totalPar.supply = uint8(totalPar.supply).sub(oldPar.value).to128();
        } else {
            totalPar.borrow = uint8(totalPar.borrow).sub(oldPar.value).to128();
        }

        // roll-forward newPar
        if (newPar.sign) {
            totalPar.supply = uint8(totalPar.supply).add(newPar.value).to128();
        } else {
