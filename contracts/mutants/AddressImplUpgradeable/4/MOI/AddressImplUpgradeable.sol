...
contract AddressImplUpgradeable is Initializable {
    ...
    function sendValue(address payable receiver, uint256 amount) external onlyInitializing {
        AddressUpgradeable.sendValue(receiver, amount);
    }

    function functionCall(address target, bytes calldata data) external onlyInitializing {
    ...
    function functionCallWithValue(
        ...
        bytes calldata data,
        uint256 value
    ) external payable onlyInitializing {
        bytes memory returnData = AddressUpgradeable.functionCallWithValue(target, data, value);
        emit CallReturnValue(abi.decode(returnData, (string)));
    }

    function functionStaticCall(address target, bytes calldata data) external onlyInitializing {
