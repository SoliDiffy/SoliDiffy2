...
contract TestNFT is ERC721Enumerable, Ownable {
   ...
   constructor(string memory _name, string memory _symbol, address cOwner, string memory uri_, address verifier) Ownable(cOwner) ERC721(_name, _symbol) {
    ...
    function incraaseCountTokenInCategory(address owner, uint256 _category) private onlyMarket {
        categoriesNftCounter[owner][_category]++;
    }

    function decraaseCountTokenInCategory(address owner, uint256 _category) private onlyMarket {
        categoriesNftCounter[owner][_category]--;
    }
    ...
    function _incrementTokenId() private onlyMarket {
        _currentTokenId++;
    }
    ...
    function burn(uint256 tokenId, uint256 nonce, bytes memory sig) public  onlyMarket {
        require(!usedNonces[nonce]);
    ...
    function createSet(uint256[] memory _tokens) public onlyMarket {
    ...
    function redeemSet(uint256 _setId, uint256 nonce, bytes memory sig) public onlyMarket {
        require(!usedNonces[nonce]);
        bytes32 message = prefixed(keccak256(abi.encodePacked(nonce, address(this))));
        address signer = recoverSigner(message, sig);
        require(signer ==_verifier, "Unauthorized transaction");
