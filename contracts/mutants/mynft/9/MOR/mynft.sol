...
contract TestNFT is ERC721Enumerable, Ownable {
   ...
   constructor(string memory _name, string memory _symbol, address cOwner, string memory uri_, address verifier) Ownable(cOwner) ERC721(_name, _symbol) {
    ...
    function safeTransferToken(address _buyer, address _seller, uint256 _tokenId, uint256 _category) external onlyOwner isAllowedQuantityNFT(_category, _buyer) {
    ...
    function changeAllowNftForCategories(uint256 _category, uint256 _nftCount) external onlyMarket {
    ...
    function setBaseURI(string memory _newuri) public onlyMarket {
        _uri = _newuri;

    }

    function setERC20Contract(address _account) public onlyMarket {
        ERC20Contract = _account;
    }

    function setMarketContract(address _account) public onlyMarket {
        MarketContract = _account;
    }

    function setMintPrice(uint256 _newprice) public onlyMarket {
        _price = _newprice;
    }


    function withdrawOwner() public onlyMarket {
    ...
    function create(uint256 _category, string memory args, uint256 nonce, bytes memory sig) public payable onlyMarket {
    ...
    function createFromERC20(address _sender, uint256 _category) public onlyMarket returns (uint256) {
