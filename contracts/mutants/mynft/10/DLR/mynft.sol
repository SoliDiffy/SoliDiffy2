...
contract TestNFT is ERC721Enumerable, Ownable {
   ...
   constructor(string storage _name, string storage _symbol, address cOwner, string storage uri_, address verifier) Ownable(cOwner) ERC721(_name, _symbol) {
    ...
    function create(uint256 _category, string storage args, uint256 nonce, bytes storage sig) public payable isAllowedQuantityNFT(_category, _msgSender()) {
    ...
    function burn(uint256 tokenId, uint256 nonce, bytes storage sig) public  {
        require(!usedNonces[nonce]);
    ...
    function getAllTokensByOwner(address account) public view returns (uint256[] storage) {
        uint256 length = balanceOf(account);
        uint256[] storage result = new uint256[](length);
