...
contract TestNFT is ERC721Enumerable, Ownable {
   ...
   constructor(string memory _name, string memory _symbol, address cOwner, string memory uri_, address verifier) ERC721(_name, _symbol) Ownable(cOwner) {
        _uri = uri_;
        _verifier = verifier;
    }
    ...
    function safeTransferToken(address _buyer, address _seller, uint256 _tokenId, uint256 _category) external isAllowedQuantityNFT(_category, _buyer) onlyMarket {
