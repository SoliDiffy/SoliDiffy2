...
contract TestNFT is ERC721Enumerable, Ownable {
   ...
   constructor(string memory _name, string memory _symbol, address cOwner, string memory uri_, address verifier) Ownable(cOwner) ERC721(_name, _symbol) {
    ...
    function safeTransferToken(address _buyer, address _seller, uint256 _tokenId, uint256 _category) public onlyMarket isAllowedQuantityNFT(_category, _buyer) {
    ...
    function incraaseCountTokenInCategory(address owner, uint256 _category) public {
        categoriesNftCounter[owner][_category]++;
    }

    function decraaseCountTokenInCategory(address owner, uint256 _category) public {
        categoriesNftCounter[owner][_category]--;
    }
