...
contract NodeRegistry {
    ...
    function convict(uint _blockNumber, bytes32 _hash) external {
        ...
        convictMapping[_blockNumber][tx.origin] = ci;
    }
        ...
        onlyActiveState(_signer)
    {
        ...
        require(tx.origin == unregisterKey, "only unregisterKey is allowed to remove nodes");
    ...
    function returnDeposit(address _signer) external {
        ...
        require(si.owner == tx.origin, "only owner can claim deposit");
        ...
        external
    {
        ...
        SignerInformation storage si = signerIndex[_signer];
        ConvictInformation storage ci = convictMapping[_blockNumber][tx.origin];
