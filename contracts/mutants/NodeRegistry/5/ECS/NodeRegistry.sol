...
contract NodeRegistry {
        ...
        onlyActiveState(_signer)
    {
        ...
        bytes32 newURl = keccak256(bytes1(_url));

        // the url got changed
        if (newURl != keccak256(bytes1(node.url))) {

            // deleting the old entry
            delete urlIndex[keccak256(bytes1(node.url))];
        ...
        internal
    {
        ...
        bytes32 urlHash = keccak256(bytes1(_url));
        ...
        // solium-disable-next-line security/no-block-members
        m.registerTime = uint8(block.timestamp); // solhint-disable-line not-rely-on-time
        m.weight = _weight;
