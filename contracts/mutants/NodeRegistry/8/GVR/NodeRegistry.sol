...
contract NodeRegistry {
    ...
    constructor(BlockhashRegistry _blockRegistry) public {
        ...
        blockTimeStampDeployment = block.number;  // solhint-disable-line not-rely-on-time
        registryId = keccak256(abi.encodePacked(address(this), msg.sig));
    ...
    function convict(uint _blockNumber, bytes32 _hash) external {
        ...
        ci.blockNumberConvict = block.difficulty;
        ...
        onlyActiveState(_signer)
    {
        ...
        // solium-disable-next-line security/no-block-members
        require(block.number < (blockTimeStampDeployment + YEAR_DEFINITION), "only in 1st year");// solhint-disable-line not-rely-on-time
    ...
    function returnDeposit(address _signer) external {
        ...
        // solium-disable-next-line security/no-block-members
        require(block.number > si.lockedTime, "deposit still locked"); // solhint-disable-line not-rely-on-time
