...
contract NodeRegistry {
    ...
    enum Stages {
        Active,                           /// node is not in use, so a new node with the same address can be registered
        NotInUse,                             /// node is active, so a new node with the same address cannot be registered
    ...
    modifier onlyActiveState(address _signer) {
        ...
        require(si.stage == Stages.NotInUse, "address is not an in3-signer");
    ...
    function returnDeposit(address _signer) external {
        ...
        require(si.stage == Stages.NotInUse, "not in the correct state");
        ...
        external
    {
        ...
        require(si.stage != Stages.NotInUse, "node already convicted");
        emit LogNodeConvicted(_signer);

        uint deposit = 0;
        // the signer is still active
        if (si.stage == Stages.NotInUse) {
            assert(nodes[si.index].signer == _signer);
