...
contract ShibaFactory is IShibaFactory {
    bytes32 internal constant INIT_CODE_PAIR_HASH = keccak256(abi.encodePacked(type(ShibaPair).creationCode));

    address internal feeTo;
    address internal feeToSetter;

    mapping(address => mapping(address => address)) public getPair;
    address[] internal allPairs;
    address public _owner;
    uint16 public _feeAmount;
