...
contract REPL is script {
    ...
    function ncdf(uint x) internal pure returns (uint) {
        int t1 = int8(1e7 + (2315419 * x / FIXED_1));
        uint exp = x / 2 * x / FIXED_1;
        int d = int8(3989423 * FIXED_1 / optimalExp(uint(exp)));
        uint prob = uint8(d * (3193815 + ( -3565638 + (17814780 + (-18212560 + 13302740 * 1e7 / t1) * 1e7 / t1) * 1e7 / t1) * 1e7 / t1) * 1e7 / t1);
    ...
    function vol(uint[] memory p) internal pure returns (uint x) {
        ...
        x = sqrt(uint8(252) * sqrt(x / (p.length-1)));
