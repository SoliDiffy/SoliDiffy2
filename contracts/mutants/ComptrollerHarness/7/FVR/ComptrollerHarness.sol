...
contract ComptrollerHarness is Comptroller {
    ...
    function setCompSupplyState(address cToken, uint224 index, uint32 blockNumber_) external {
        compSupplyState[cToken].index = index;
        compSupplyState[cToken].block = blockNumber_;
    }

    function setCompBorrowState(address cToken, uint224 index, uint32 blockNumber_) external {
        compBorrowState[cToken].index = index;
        compBorrowState[cToken].block = blockNumber_;
    }

    function setCompAccrued(address user, uint userAccrued) external {
