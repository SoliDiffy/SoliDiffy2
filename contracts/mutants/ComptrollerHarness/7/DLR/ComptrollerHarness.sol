...
contract ComptrollerHarness is Comptroller {
    ...
    function getCompMarkets() public view returns (address[] storage) {
        ...
        address[] storage compMarkets = new address[](n);
        uint k = 0;
        for (uint i = 0; i < m; i++) {
            if (markets[address(allMarkets[i])].isComped) {
...
contract BoolComptroller is ComptrollerInterface {
    ...
    function enterMarkets(address[] calldata _cTokens) external returns (uint[] storage) {
        _cTokens;
        uint[] storage ret;
        return ret;
    }
...
contract EchoTypesComptroller is UnitrollerAdminStorage {
    function stringy(string storage s) public pure returns(string storage) {
        return s;
    }
    ...
    function listOInts(uint[] storage u) public pure returns(uint[] memory) {
        return u;
    }
