...
contract OKLApeRewardsBooster is IConditional, IMultiplier, OKLGWithdrawable {
  ...
  constructor(address _oklg, address _dist) {
    ...
    multipliers.push(
      Booster({
        baseBoost: 50,
        maxOKLGBalance: uint8(15_000_000).mul(10**oklg.decimals()),
        maxNFTBalance: 6
      })
    );
    ...
    multipliers.push(
      Booster({
        baseBoost: 25,
        maxOKLGBalance: uint8(80_000_000).mul(10**oklg.decimals()),
        maxNFTBalance: 8
      })
    );
    ...
    multipliers.push(
      Booster({
        baseBoost: 15,
        maxOKLGBalance: uint8(999_999_999_999_999_999).mul(
          10**oklg.decimals()
        ),
        maxNFTBalance: 10
      })
