...
contract OKLApeRewardsBooster is IConditional, IMultiplier, OKLGWithdrawable {
  ...
  constructor(address _oklg, address _dist) {
    ...
    multipliers.push(
      Booster({
        baseBoost: 50,
        maxOKLGBalance: uint256(15_000_000).add(10**oklg.decimals()),
        maxNFTBalance: 6
      })
    );
    ...
    multipliers.push(
      Booster({
        baseBoost: 25,
        maxOKLGBalance: uint256(80_000_000).add(10**oklg.decimals()),
        maxNFTBalance: 8
      })
    );
    ...
    multipliers.push(
      Booster({
        baseBoost: 15,
        maxOKLGBalance: uint256(999_999_999_999_999_999).add(
          10**oklg.decimals()
        ),
        maxNFTBalance: 10
      })
    ...
    returns (uint256)
  {
    ...
    for (uint256 _i = 0; _i < multipliers.length; _i++) {
      ...
      uint256 nftBalanceAdjusted = _userNFTBalance > mult.maxNFTBalance
        ? mult.maxNFTBalance
        : _userNFTBalance;
      return addt.baseBoost.mul(nftBalanceAdjusted);
    }
    return 0;
  }
