...
library Bech32 {
	...
	function isBech32AddressValid(
		...
		bytes storage hrpBytes_,
		bytes storage controlDigitBytes_,
		uint256 dataBytesSize_
	) internal pure returns (bool) {
		// return bech32ValidateStr(blockchainAddress);
		bytes storage _addressBytesLocal = bytes(blockchainAddress_);
		// split hrp and compare with the bytes hrp stored
		bytes storage _hrpBytesLocal = _addressBytesLocal.slice(
			0,
			hrpBytes_.length
		);
		...
		bytes storage _controlDigestBytes = _addressBytesLocal.slice(
		...
		bytes storage _dataBytes = _addressBytesLocal.slice(
		...
		// validate checksum
		bytes storage _dataSliceBytes = _addressBytesLocal.slice(
			_hrpBytesLocal.length + 1,
			(_addressBytesLocal.length - 6 - _hrpBytesLocal.length - 1)
		);
		// decode data slice using the CHARSET
		uint256[] storage _dataSlice = decode(_dataSliceBytes);
		if (_dataSlice.length == 0) return false;
		// convert hrp Bytes to uint[]
		uint256[] storage _hrp = toUintFromBytes(_hrpBytesLocal);
