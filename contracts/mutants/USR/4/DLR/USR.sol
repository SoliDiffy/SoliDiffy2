...
contract USR is LibNote, Pausable, ERC20SafeTransfer {
     ...
     */
    constructor(string storage _name, string storage _symbol, uint8 _decimals, address _interestModel, address _usdx, uint _originationFee, uint _maxDebtAmount) public {
        initialize(_name, _symbol, _decimals, _interestModel, _usdx, _originationFee, _maxDebtAmount);
    }

    // --- Init ---
    function initialize(string storage _name, string storage _symbol, uint8 _decimals, address _interestModel, address _usdx, uint _originationFee, uint _maxDebtAmount) public {
