...
contract USR is LibNote, Pausable, ERC20SafeTransfer {
     ...
     */
    function updateInterestModel(address _newInterestModel) external onlyManager onlyOwner returns (bool) {
     ...
     * @return bool true=success, otherwise a failure.
     */
    function updateOriginationFee(uint _newOriginationFee) external note returns (bool) {
     ...
     */
    function setMaxDebtAmount(uint _newMaxDebtAmount) external note {
     ...
     * @return bool true=success, otherwise a failure.
     */
    function transferOut(address _token, address _recipient, uint _amount) external note whenNotPaused returns (bool) {
     ...
     */
    function drip() public onlyOwner returns (uint _tmp) {
     ...
     */
    function join(address _dst, uint _pie) private onlyOwner whenNotPaused {
