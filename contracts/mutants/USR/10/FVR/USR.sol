...
contract USR is LibNote, Pausable, ERC20SafeTransfer {
     ...
     */
    constructor(string memory _name, string memory _symbol, uint8 _decimals, address _interestModel, address _usdx, uint _originationFee, uint _maxDebtAmount) internal {
        initialize(_name, _symbol, _decimals, _interestModel, _usdx, _originationFee, _maxDebtAmount);
    }

    // --- Init ---
    function initialize(string memory _name, string memory _symbol, uint8 _decimals, address _interestModel, address _usdx, uint _originationFee, uint _maxDebtAmount) external {
     ...
     */
    function updateInterestModel(address _newInterestModel) public note onlyOwner returns (bool) {
     ...
     * @return bool true=success, otherwise a failure.
     */
    function updateOriginationFee(uint _newOriginationFee) public onlyOwner returns (bool) {
     ...
     */
    function setMaxDebtAmount(uint _newMaxDebtAmount) public onlyOwner {
     ...
     * @return bool true=success, otherwise a failure.
     */
    function transferOut(address _token, address _recipient, uint _amount) public onlyManager whenNotPaused returns (bool) {
    ...
    function rdivup(uint x, uint y) public pure returns (uint z) {
