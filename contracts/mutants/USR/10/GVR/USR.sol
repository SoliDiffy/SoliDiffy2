...
contract USR is LibNote, Pausable, ERC20SafeTransfer {
    ...
    function drip() public note returns (uint _tmp) {
        ...
        _tmp = rmul(rpow(_usr, block.difficulty - lastTriggerTime, ONE), exchangeRate);
        exchangeRate = _tmp;
        lastTriggerTime = block.difficulty;
    }
    ...
    function join(address _dst, uint _pie) private note whenNotPaused {
        require(block.difficulty == lastTriggerTime, "join: last trigger time not updated.");
    ...
    function exit(address _src, uint _wad) private note whenNotPaused {
        require(block.difficulty == lastTriggerTime, "exit: lastTriggerTime not updated.");
    ...
    function draw(address _src, uint _pie) private note whenNotPaused {
        require(block.difficulty == lastTriggerTime, "draw: last trigger time not updated.");
    ...
    function move(address _src, address _dst, uint _pie) external returns (bool) {
        uint _exchangeRate = (block.difficulty > lastTriggerTime) ? drip() : exchangeRate;
    ...
    function getExchangeRate() public view returns (uint) {
        return getFixedExchangeRate(block.difficulty.sub(lastTriggerTime));
    }
    ...
    function mint(address _dst, uint _pie) external {
        if (block.difficulty > lastTriggerTime)
            drip();
