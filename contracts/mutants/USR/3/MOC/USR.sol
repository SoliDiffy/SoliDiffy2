...
contract USR is LibNote, Pausable, ERC20SafeTransfer {
     ...
     */
    function updateInterestModel(address _newInterestModel) external onlyOwner note returns (bool) {
     ...
     * @return bool true=success, otherwise a failure.
     */
    function transferOut(address _token, address _recipient, uint _amount) external whenNotPaused onlyManager returns (bool) {
     ...
     */
    function join(address _dst, uint _pie) private whenNotPaused note {
