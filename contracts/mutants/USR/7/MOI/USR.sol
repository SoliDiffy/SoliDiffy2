...
contract USR is LibNote, Pausable, ERC20SafeTransfer {
    ...
    // --- Init ---
    function initialize(string memory _name, string memory _symbol, uint8 _decimals, address _interestModel, address _usdx, uint _originationFee, uint _maxDebtAmount) public note {
    ...
    // --- Token ---
    function transfer(address _dst, uint _wad) external note returns (bool) {
        return transferFrom(msg.sender, _dst, _wad);
    }

    // like transferFrom but Token-denominated
    function move(address _src, address _dst, uint _pie) external note returns (bool) {
    ...
    function transferFrom(address _src, address _dst, uint _wad) public note returns (bool)
    {
    ...
    function approve(address _spender, uint _wad) external note returns (bool) {
    ...
    function mint(address _dst, uint _pie) external note {
    ...
    function burn(address _src, uint _wad) external note {
        if (now > lastTriggerTime)
            drip();
        exit(_src, _wad);
    }
