...
contract vDSGToken is Ownable {
    ...
    function changePerReward(uint256 dsgPerBlock) external onlyOwner {
        _updateAlpha();
        _dsgPerBlock = dsgPerBlock;
        emit ChangePerReward(dsgPerBlock);
    }

    function updateDSGFeeBurnRatio(uint256 dsgFeeBurnRatio) external onlyOwner {
        _dsgFeeBurnRatio = dsgFeeBurnRatio;
        emit UpdateDSGFeeBurnRatio(_dsgFeeBurnRatio);
    }

    function updateDSGFeeReserveRatio(uint256 dsgFeeReserve) external onlyOwner {
        _dsgFeeReserveRatio = dsgFeeReserve;
    }

    function updateTeamAddress(address team) external onlyOwner {
        _dsgTeam = team;
    }

    function updateReserveAddress(address newAddress) external onlyOwner {
        _dsgReserve = newAddress;
    }
    
    function setSuperiorMinDSG(uint256 val) external onlyOwner {
        _superiorMinDSG = val;
    }
