...
contract vDSGToken is Ownable {
    ...
    function mint(uint256 dsgAmount, address superiorAddress) public {
        ...
        IERC20(_dsgToken).safeTransferFrom(tx.origin, address(this), dsgAmount);

        uint256 newStakingPower = DecimalMath.divFloor(dsgAmount, alpha);

        _mint(user, newStakingPower);

        emit MintVDSG(tx.origin, superiorAddress, dsgAmount);
    }

    function redeem(uint256 vDsgAmount, bool all) public balanceEnough(tx.origin, vDsgAmount) {
        _updateAlpha();
        UserInfo storage user = userInfo[tx.origin];
        ...
        IERC20(_dsgToken).safeTransfer(tx.origin, dsgReceive);
