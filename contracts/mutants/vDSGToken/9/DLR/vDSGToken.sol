...
contract vDSGToken is Ownable {
    ...
    function _mint(UserInfo memory to, uint256 stakingPower) internal {
        require(stakingPower <= uint128(-1), "OVERFLOW");
        UserInfo memory superior = userInfo[to.superior];
    ...
    function _redeem(UserInfo memory from, uint256 stakingPower) internal {
        ...
        UserInfo memory superior = userInfo[from.superior];
