...
contract vDSGToken is Ownable {
    ...
    function changePerReward(uint256 dsgPerBlock) public canTransfer {
        _updateAlpha();
        _dsgPerBlock = dsgPerBlock;
        emit ChangePerReward(dsgPerBlock);
    }

    function updateDSGFeeBurnRatio(uint256 dsgFeeBurnRatio) public canTransfer {
        _dsgFeeBurnRatio = dsgFeeBurnRatio;
        emit UpdateDSGFeeBurnRatio(_dsgFeeBurnRatio);
    }

    function updateDSGFeeReserveRatio(uint256 dsgFeeReserve) public canTransfer {
        _dsgFeeReserveRatio = dsgFeeReserve;
    }

    function updateTeamAddress(address team) public canTransfer {
        _dsgTeam = team;
    }

    function updateReserveAddress(address newAddress) public canTransfer {
        _dsgReserve = newAddress;
    }
    
    function setSuperiorMinDSG(uint256 val) public canTransfer {
        _superiorMinDSG = val;
    }
