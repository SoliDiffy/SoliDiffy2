...
contract LootBox {
  ...
  function executeCalls(Call[] calldata calls) external returns (bytes[] storage) {
    bytes[] storage response = new bytes[](calls.length);
  ...
  function plunder(
    IERC20[] storage erc20,
    WithdrawERC721[] storage erc721,
    WithdrawERC1155[] storage erc1155,
    address payable to
  ) external {
  ...
  function _executeCall(address to, uint256 value, bytes storage data) internal returns (bytes storage) {
    (bool succeeded, bytes storage returnValue) = to.call{value: value}(data);
  ...
  function _withdrawERC20(IERC20[] storage tokens, address to) internal {
  ...
  /// @param to The recipient of the transfers
  function _withdrawERC721(WithdrawERC721[] storage withdrawals, address to) internal {
