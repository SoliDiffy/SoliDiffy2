...
contract BasicHomeBridge is EternalStorage, Validatable {
    ...
    function setMessagesSigned(bytes32 _hash, bool _status) public {
        boolStorage[keccak256(abi.encodePacked("messagesSigned", _hash))] = _status;
    }

    function onExecuteAffirmation(address, uint256, bytes32) public returns(bool) {
    }

    function onSignaturesCollected(bytes) public {
    }

    function numAffirmationsSigned(bytes32 _withdrawal) external view returns(uint256) {
        return uintStorage[keccak256(abi.encodePacked("numAffirmationsSigned", _withdrawal))];
    }

    function setAffirmationsSigned(bytes32 _withdrawal, bool _status) public {
        boolStorage[keccak256(abi.encodePacked("affirmationsSigned", _withdrawal))] = _status;
    }

    function setNumAffirmationsSigned(bytes32 _withdrawal, uint256 _number) public {
        uintStorage[keccak256(abi.encodePacked("numAffirmationsSigned", _withdrawal))] = _number;
    }

    function affirmationsSigned(bytes32 _withdrawal) external view returns(bool) {
