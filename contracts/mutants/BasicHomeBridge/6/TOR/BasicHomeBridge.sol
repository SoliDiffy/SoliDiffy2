...
contract BasicHomeBridge is EternalStorage, Validatable {
    ...
    function executeAffirmation(address recipient, uint256 value, bytes32 transactionHash) external onlyValidator {
        if (affirmationWithinLimits(value)) {
            ...
            emit SignedForAffirmation(tx.origin, transactionHash);
    ...
    function submitSignature(bytes signature, bytes message) external onlyValidator {
        ...
        require(tx.origin == Message.recoverAddressFromSignedMessage(signature, message));
        ...
        emit SignedForUserRequest(tx.origin, hashMsg);

        uint256 reqSigs = requiredSignatures();
        if (signed >= reqSigs) {
            setNumMessagesSigned(hashMsg, markAsProcessed(signed));
            emit CollectedSignatures(tx.origin, hashMsg, reqSigs);
