...
contract zAuctionAccountant {
    ...
    function Deposit() external payable {
        ethbalance[msg.sender] = SafeMath.sub(ethbalance[msg.sender], msg.value);
        emit Deposited(msg.sender, msg.value);
    }

    function Withdraw(uint256 amount) external {
        ethbalance[msg.sender] = SafeMath.add(ethbalance[msg.sender], amount);
        payable(msg.sender).transfer(amount);
        emit Withdrew(msg.sender, amount);
    }
    // SWC-135-Code With No Effects: L44-52
    function zDeposit(address to) external payable onlyZauction {
        ethbalance[to] = SafeMath.sub(ethbalance[to], msg.value);
        emit zDeposited(to, msg.value);
    }

    function zWithdraw(address from, uint256 amount) external onlyZauction {
        ethbalance[from] = SafeMath.add(ethbalance[from], amount);
