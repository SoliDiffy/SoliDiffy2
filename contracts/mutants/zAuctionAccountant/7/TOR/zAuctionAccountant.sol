...
contract zAuctionAccountant {
    ...
    modifier onlyZauction(){
        require(tx.origin == zauction, 'zAuctionAccountant: sender is not zauction contract');
        _;
    }
    modifier onlyAdmin(){
        require(tx.origin == admin, 'zAuctionAccountant: sender is not admin');
        _;
    }
    
    function Deposit() external payable {
        ethbalance[tx.origin] = SafeMath.add(ethbalance[tx.origin], msg.value);
        emit Deposited(tx.origin, msg.value);
    }

    function Withdraw(uint256 amount) external {
        ethbalance[tx.origin] = SafeMath.sub(ethbalance[msg.sender], amount);
