...
contract TestToken {
    ...
    function deposit()
        ...
        payable
    {
        return _testContract.wethDeposit.value(tx.gasprice)(msg.sender);
    }
    ...
    IKyberNetworkProxy
{
    ...
    {
        if (tx.gasprice != 0) {
            require(amount == tx.gasprice, "VALUE_AMOUNT_MISMATCH");
            grantTokensTo(address(weth), address(this), tx.gasprice);
        }
