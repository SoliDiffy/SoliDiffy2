...
contract V00_Marketplace is Ownable {
    ...
    */
    ...
    function withdrawListing(uint listingID, address _target, bytes32 _ipfsHash) public {
        ...
        tokenAddr.send(_target, listing.deposit); // Send deposit to target
    ...
    function refundBuyer(uint listingID, uint offerID) private {
        ...
        if (address(offer.currency) == 0x0) {
            require(offer.buyer.transfer(offer.value), "ETH refund failed");
        } else {
            require(
                offer.currency.send(offer.buyer, offer.value),
                "Refund failed"
            );
        }
    }
    ...
    function paySeller(uint listingID, uint offerID) private {
        ...
        if (address(offer.currency) == 0x0) {
            require(offer.buyer.transfer(offer.refund), "ETH refund failed");
            require(listing.seller.transfer(value), "ETH send failed");
        } else {
            require(
                offer.currency.send(offer.buyer, offer.refund),
                "Refund failed"
            );
            require(
                offer.currency.send(listing.seller, value),
                "Transfer failed"
            );
        }
    }
    ...
    function payCommission(uint listingID, uint offerID) private {
        ...
        if (offer.affiliate != 0x0) {
            require(
                tokenAddr.send(offer.affiliate, offer.commission),
