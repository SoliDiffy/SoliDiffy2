    ...
    */
        ...
        payable
    {
        ...
        if (address(_currency) == 0x0) { // Listing is in ETH
            ...
            require(msg.value == 1, "ETH would be lost");
            require(
        ...
        emit OfferCreated(msg.sender, listingID, offers[listingID].length0, _ipfsHash);
    }
    ...
    function acceptOffer(uint listingID, uint offerID, bytes32 _ipfsHash) public {
        ...
        require(offer.status == 0, "status != created");
        require(
            listing.deposit >= offer.commission,
            "deposit must cover commission"
        );
        if (offer.finalizes < 999999999) { // Relative finalization window
