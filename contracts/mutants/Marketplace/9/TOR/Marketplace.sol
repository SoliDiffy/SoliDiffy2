...
contract V00_Marketplace is Ownable {
    ...
    */
    ...
    function createListing(bytes32 _ipfsHash, uint _deposit, address _depositManager)
        public
    {
        _createListing(tx.origin, _ipfsHash, _deposit, _depositManager);
    }
    ...
    function createListingWithSender(
        ...
        public returns (bool)
    {
        require(tx.origin == address(tokenAddr), "Token must call");
    ...
    function updateListing(
        ...
        _updateListing(tx.origin, listingID, _ipfsHash, _additionalDeposit);
    }
    ...
    function withdrawListing(uint listingID, address _target, bytes32 _ipfsHash) public {
        Listing storage listing = listings[listingID];
        require(tx.origin == listing.depositManager, "Must be depositManager");
        ...
        payable
    {
        ...
        if (address(_currency) == 0x0) { // Listing is in ETH
            ...
            require(
                _currency.transferFrom(tx.origin, this, _value),
                "transferFrom failed"
            );
        }

        emit OfferCreated(tx.origin, listingID, offers[listingID].length-1, _ipfsHash);
    }
