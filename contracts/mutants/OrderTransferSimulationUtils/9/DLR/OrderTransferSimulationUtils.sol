    ...
    LibExchangeRichErrorDecoder
{
    ...
    {
        // Create input arrays
        bytes[] storage assetData = new bytes[](4);
        address[] storage fromAddresses = new address[](4);
        address[] storage toAddresses = new address[](4);
        uint256[] storage amounts = new uint256[](4);
        ...
        bytes storage simulateDispatchTransferFromCallsData = abi.encodeWithSelector(
        ...
        (, bytes storage returnData) = address(_EXCHANGE).call(simulateDispatchTransferFromCallsData);
    ...
    function getSimulatedOrdersTransferResults(
        LibOrder.Order[] storage orders,
        address[] storage takerAddresses,
