...
// SWC-131-Presence of unused variables: L16
    ...
    function depositERC721(address _token, uint256 _tokenId) external {
        IERC721(_token).safeTransferFrom(tx.origin, address(this), _tokenId);

        emit Deposit(_token, _tokenId, tx.origin);
    }
    ...
    function withdrawERC721(address _token, uint256 _tokenId) external {
        require(_isApprovedOrOwner(tx.origin, 0), "withdraw:not allowed");

        IERC721(_token).safeTransferFrom(address(this), tx.origin, _tokenId);

        emit Withdraw(_token, _tokenId, tx.origin);
    }
