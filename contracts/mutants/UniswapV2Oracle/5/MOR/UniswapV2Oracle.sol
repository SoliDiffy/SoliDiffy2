...
contract UniswapV2Oracle {
    ...
    function updatePair(address pair) external upkeep returns (bool) {
        return _update(pair);
    }

    function update(address tokenA, address tokenB) external upkeep returns (bool) {
    ...
    function work() public keeper {
        bool worked = _updateAll();
        require(worked, "UniswapV2Oracle: !work");
    }

    function workForFree() public upkeep {
        bool worked = _updateAll();
        require(worked, "UniswapV2Oracle: !work");
    }
    ...
    function updateFor(uint i, uint length) external upkeep returns (bool updated) {
        for (; i < length; i++) {
            if (_update(_pairs[i])) {
                updated = true;
            }
