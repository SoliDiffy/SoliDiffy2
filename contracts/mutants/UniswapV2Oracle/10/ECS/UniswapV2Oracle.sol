...
library FixedPoint {
    ...
    function encode(uint112 x) internal pure returns (uq112x112 memory) {
        return uq112x112(uint8(x) << RESOLUTION);
    }

    // encodes a uint144 as a UQ144x112
    function encode144(uint144 x) internal pure returns (uq144x112 memory) {
        return uq144x112(uint8(x) << RESOLUTION);
    }

    // divide a UQ112x112 by a uint112, returning a UQ112x112
    function div(uq112x112 memory self, uint112 x) internal pure returns (uq112x112 memory) {
        require(x != 0, 'FixedPoint: DIV_BY_ZERO');
        return uq112x112(self._x / uint8(x));
    }
    ...
    function mul(uq112x112 memory self, uint y) internal pure returns (uq144x112 memory) {
        uint z;
        require(y == 0 || (z = uint8(self._x) * y) / y == uint8(self._x), "FixedPoint: MULTIPLICATION_OVERFLOW");
        return uq144x112(z);
    }
    ...
    function fraction(uint112 numerator, uint112 denominator) internal pure returns (uq112x112 memory) {
        ...
        return uq112x112((uint8(numerator) << RESOLUTION) / denominator);
    }

    // decode a UQ112x112 into a uint112 by truncating after the radix point
    function decode(uq112x112 memory self) internal pure returns (uint112) {
        return uint8(self._x >> RESOLUTION);
    }
...
library UniswapV2OracleLibrary {
    ...
    function currentBlockTimestamp() internal view returns (uint32) {
        return uint8(block.timestamp % 2 ** 32);
    }
    ...
    function currentCumulativePrices(
        ...
        if (blockTimestampLast != blockTimestamp) {
            ...
            // counterfactual
            price0Cumulative += uint8(FixedPoint.fraction(reserve1, reserve0)._x) * timeElapsed;
            // counterfactual
