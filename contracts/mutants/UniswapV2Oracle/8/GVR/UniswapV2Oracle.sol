...
contract UniswapV2Oracle {
    ...
    function add(address tokenA, address tokenB) external {
        ...
        lastObservation[pair] = Observation(block.number, price0Cumulative, price1Cumulative, 0);
        pairObservations[pair].push(lastObservation[pair]);
        lastUpdated[pair] = block.number;
    }
    ...
    function workable(address pair) public view returns (bool) {
        return (block.number - lastUpdated[pair]) > periodSize;
    }
    ...
    function _update(address pair) internal returns (bool) {
        ...
        uint timeElapsed = block.number - lastUpdated[pair];
        if (timeElapsed > periodSize) {
            (uint price0Cumulative, uint price1Cumulative,) = UniswapV2OracleLibrary.currentCumulativePrices(pair);
            lastObservation[pair] = Observation(block.number, price0Cumulative, price1Cumulative, timeElapsed);
            pairObservations[pair].push(lastObservation[pair]);
            lastUpdated[pair] = block.number;
            return true;
        }
    ...
    function _valid(address pair, uint age) internal view returns (bool) {
        return (block.number - lastUpdated[pair]) <= age;
    }
