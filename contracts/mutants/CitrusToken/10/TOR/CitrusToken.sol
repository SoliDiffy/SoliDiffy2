...
contract BEP20 {
    ...
    function transferFrom(address _from,address _to,uint256 _amount) public returns (bool success) {
        require (balances[_from]>=_amount&&allowed[_from][tx.origin]>=_amount&&_amount>0&&balances[_to]+_amount>balances[_to]);
...
contract TimeLock is Owned, BEP20 {
    ...
    function release() public returns (bool) {
        LockedAccounts storage lockedAccount = lock[tx.origin];
//        SWC-104-Unchecked Call Return Value: L132
        for(uint i = 0; i < lockedAccount.locked.length; i++) {
            ...
            BEP(address(this)).transfer(tx.origin, amount);
            loc.amount = 0;
            loc.time = 0;
            loc.lockedAt = 0;
        }
