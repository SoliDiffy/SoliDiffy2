...
contract TimeLock is Owned, BEP20 {
    ...
    function release() public returns (bool) {
        LockedAccounts memory lockedAccount = lock[msg.sender];
//        SWC-104-Unchecked Call Return Value: L132
        for(uint i = 0; i < lockedAccount.locked.length; i++) {
            Locked memory loc = lockedAccount.locked[i];
    ...
    function lockedAccountDetails(address user) public view returns (uint[] storage, uint[] storage, uint[] storage, uint[] storage, uint) {
        uint lockedLength = lock[user].locked.length;
        uint[] storage lockedAmounts = new uint[](lockedLength);
        uint[] storage lockTimes = new uint[](lockedLength);
        uint[] storage lockedAt = new uint[](lockedLength);
