...
contract MarketXYZLendingAMO is Owned {
    ...
    // Helpful for UIs
    function allPoolAddresses() public view returns (address[] memory) {
    ...
    function poolAddrToIdx(address pool_address) external view returns (uint256) {
        for (uint i = 0; i < fuse_pools_array.length; i++){ 
    ...
    function fraxInPoolByPoolIdx(uint256 pool_idx) external view returns (uint256) {
    ...
    function fraxInPoolByPoolAddr(address pool_address) external view returns (uint256) {
