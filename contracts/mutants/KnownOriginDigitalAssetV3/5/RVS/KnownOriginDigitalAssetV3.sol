...
IHasSecondarySaleFees // Rariable / Foundation royalties
{
    ...
    function facilitateNextPrimarySale(uint256 _editionId) external returns (address _creator, address _receiver, uint256 _tokenId);

    // Utility method to get all data needed for the next primary sale, high token ID to low
    function facilitateReversePrimarySale(uint256 _editionId) external returns (address _creator, address _receiver, uint256 _tokenId);

    // Expanded royalty method for the edition, not token
    function royaltyAndCreatorInfo(uint256 _editionId, uint256 _value) external returns (address _creator, address _receiver, uint256 _amount);
    ...
    function getEditionDetails(uint256 _tokenId) external view returns (address _owner, address _originalCreator, uint256 _editionId, uint16 _size, string memory _uri);
...
IKODAV3Minter {
    ...
    returns (address _originalCreator, address _owner, uint16 _size, uint256 _editionId, string memory _uri) {
        ...
        return (
        _ownerOf(_tokenId,
        edition.creator, edition.editionSize,
        editionId),
        editionId,
        tokenURI(_tokenId)
        );
