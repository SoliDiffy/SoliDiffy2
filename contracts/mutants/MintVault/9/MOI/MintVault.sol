...
contract MintVault is TokenRecipient, MintVaultBase {
  ...
  function underlyingTransferIn(address sender, uint256 amount) internal virtual override initializer {
    IERC20Upgradeable(underlying).safeTransferFrom(sender, address(this), amount);
  }

  function underlyingTransferOut(address receipt, uint256 amount, bool) internal virtual override initializer {
  ...
  function underlyingMint(address to, uint amount) internal virtual override initializer {
    IDUSD(underlying).mint(to, amount);
  }

// TODO: 不同的dAsset 可以可能有不同的实现mint。
  function underlyingBurn(uint amount) internal virtual override initializer {
    IDUSD(underlying).burn(amount);
  }

  function borrow(uint256 amount) external override initializer {
    _borrow(msg.sender, amount);
  }


  function tokensReceived(address from, uint amount, bytes calldata exData) external override initializer returns (bool) {
  ...
  // 
  function repay(uint256 amount) external override initializer {
    _repayFor(msg.sender, msg.sender, amount, false);
  }

  function repayTo(address to, uint256 amount) external override initializer {
    _repayFor(msg.sender, to, amount, false);
  }

  function liquidate(address liquidator, address borrower, bytes calldata data) external  initializer {
