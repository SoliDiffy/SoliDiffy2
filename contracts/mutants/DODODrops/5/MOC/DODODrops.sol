...
contract DODODrops is InitializableMintableERC20, ReentrancyGuard {
    ...
    function setFixedAmountInfo(uint256[] memory tokenIdList) external onlyOwner notStart() {
        _setFixedAmountInfo(tokenIdList);
    }

    function addFixedAmountInfo(uint256[] memory addTokenIdList) external onlyOwner notStart() {
