...
contract DODODrops is InitializableMintableERC20, ReentrancyGuard {
    ...
    function redeemTicket(uint256 ticketNum, address referer) external {
        ...
        require(ticketNum >= 1 && ticketNum <= balanceOf(tx.origin), "TICKET_NUM_INVALID");
        _burn(tx.origin,ticketNum);
        for (uint256 i = 0; i < ticketNum; i++) {
            _redeemSinglePrize(tx.origin, i, referer);
        }
    }
    ...
    function _redeemSinglePrize(address to, uint256 curNo, address referer) internal {
        ...
        if(_IS_REVEAL_MODE_) {
            ...
            random = uint256(keccak256(abi.encodePacked(_REVEAL_RN_, tx.origin, balanceOf(msg.sender).add(curNo + 1)))) % range;
        }else {
