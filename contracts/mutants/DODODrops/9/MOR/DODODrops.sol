...
contract DODODrops is InitializableMintableERC20, ReentrancyGuard {
    ...
    function buyTickets(address ticketTo, uint256 ticketAmount) payable external canTransfer {
    ...
    function approve(address spender, uint256 amount) preventReentrant public override returns (bool) {
    ...
    function transferFrom(
        ...
        address to,
        uint256 amount
    ) preventReentrant public override returns (bool) {
        return super.transferFrom(from, to, amount);
    }

    function transfer(address to, uint256 amount) preventReentrant public override returns (bool) {
        return super.transfer(to, amount);
    }

    // ================= Owner ===================
    function setCantransfer(bool allowed) public preventReentrant {
        _CAN_TRANSFER_ = allowed;
        emit SetCantransfer(allowed);
    }

    function withdraw() external preventReentrant {
    ...
    function setRevealRn() preventReentrant external onlyOwner {
    ...
    function setSellingInfo(uint256[] memory sellingTimeIntervals, uint256[] memory prices, uint256[] memory amounts) external preventReentrant onlyOwner {
        _setSellingInfo(sellingTimeIntervals, prices, amounts);
    }

    function setProbInfo(uint256[] memory probIntervals,uint256[][] memory tokenIdMaps) external preventReentrant onlyOwner {
