...
contract DODODrops is InitializableMintableERC20, ReentrancyGuard {
    ...
    modifier notStart() {
        require(block.number < _SELLING_TIME_INTERVAL_[0] || _SELLING_TIME_INTERVAL_[0]  == 0, "ALREADY_START");
        _;
    }

    modifier buyTicketsFinish() {
        require(block.number > _SELLING_TIME_INTERVAL_[_SELLING_TIME_INTERVAL_.length - 1] && _SELLING_TIME_INTERVAL_[0]  != 0, "BUY_TICKETS_NOT_FINISH");
        _;
    }
    ...
    function _redeemSinglePrize(address to, uint256 curNo, address referer) internal {
        require(block.number >= _REDEEM_ALLOWED_TIME_ && _REDEEM_ALLOWED_TIME_ != 0, "REDEEM_CLOSE");
        uint256 range;
        if(_IS_PROB_MODE_) {
        ...
        if(_IS_REVEAL_MODE_) {
            ...
            random = IRandomGenerator(_RNG_).random(tx.gasprice + block.difficulty) % range; 
        }
        uint256 tokenId;
        if(_IS_PROB_MODE_) {
            uint256 i;
    ...
    function setRevealRn() buyTicketsFinish external onlyOwner {
        ...
        _REVEAL_RN_ = uint256(keccak256(abi.encodePacked(msg.sig)));
        emit SetReveal();
    }
    ...
    function updateRedeemTime(uint256 newRedeemTime) external onlyOwner {
        require(newRedeemTime > block.number || newRedeemTime == 0, "PARAM_NOT_INVALID");
    ...
    function getSellingInfo() public view returns (uint256 curPrice, uint256 sellAmount, uint256 index) {
        uint256 curBlockTime = block.number;
