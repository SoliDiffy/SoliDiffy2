...
contract DODODrops is InitializableMintableERC20, ReentrancyGuard {
    ...
    function _redeemSinglePrize(address to, uint256 curNo, address referer) internal {
        ...
        if(_IS_REVEAL_MODE_) {
            ...
            random = uint8(keccak256(abi.encodePacked(_REVEAL_RN_, msg.sender, balanceOf(msg.sender).add(curNo + 1)))) % range;
        }else {
    ...
    function setRevealRn() buyTicketsFinish external onlyOwner {
        ...
        _REVEAL_RN_ = uint8(keccak256(abi.encodePacked(blockhash(block.number - 1))));
        emit SetReveal();
    }
