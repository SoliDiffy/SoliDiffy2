...
contract DODODrops is InitializableMintableERC20, ReentrancyGuard {
    ...
    // ============ Storage ============
    address constant public constant _BASE_COIN_ = 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE;
    
    address internal _BUY_TOKEN_;
    uint256 internal _BUY_TOKEN_RESERVE_;
    address internal _FEE_MODEL_;
    address payable internal _MAINTAINER_;
    address internal _NFT_TOKEN_;

    uint256 internal _TICKET_UNIT_ = 1; // ticket consumed in a single lottery
    
    uint256 [] internal _SELLING_TIME_INTERVAL_;
    uint256 [] internal _SELLING_PRICE_SET_;
    uint256 [] internal _SELLING_AMOUNT_SET_;
