...
contract DODODrops is InitializableMintableERC20, ReentrancyGuard {
    ...
    // ================= Owner ===================
    function setCantransfer(bool allowed) public  {
    ...
    function setSellingInfo(uint256[] memory sellingTimeIntervals, uint256[] memory prices, uint256[] memory amounts) external  onlyOwner {
        _setSellingInfo(sellingTimeIntervals, prices, amounts);
    }

    function setProbInfo(uint256[] memory probIntervals,uint256[][] memory tokenIdMaps) external  onlyOwner {
        _setProbInfo(probIntervals, tokenIdMaps);
    }

    function setFixedAmountInfo(uint256[] memory tokenIdList) external  onlyOwner {
