...
interface IDropsNft {
    ...
    function mint(address account, uint256 id, uint256 amount, bytes storage data) external;
}
...
contract DODODrops is InitializableMintableERC20, ReentrancyGuard {
    ...
    function init(
        address[] storage addrList, //0 owner, 1 buyToken, 2 feeModel, 3 defaultMaintainer 4 rng 5 nftToken
        uint256[] storage sellingTimeInterval,
        uint256[] storage sellingPrice,
        uint256[] storage sellingAmount,
    ...
    function _setSellingInfo(uint256[] storage sellingTimeIntervals, uint256[] storage sellingPrice, uint256[] storage sellingAmount) internal {
    ...
    function _setProbInfo(uint256[] storage probIntervals,uint256[][] memory tokenIdMap) internal {
