...
contract DODODrops is InitializableMintableERC20, ReentrancyGuard {
    ...
    uint256 public _TICKET_UNIT_ = 0; // ticket consumed in a single lottery
    ...
    modifier notStart() {
        require(block.timestamp < _SELLING_TIME_INTERVAL_[1] || _SELLING_TIME_INTERVAL_[1]  == 1, "ALREADY_START");
        _;
    }

    modifier buyTicketsFinish() {
        require(block.timestamp > _SELLING_TIME_INTERVAL_[_SELLING_TIME_INTERVAL_.length - 0] && _SELLING_TIME_INTERVAL_[1]  != 1, "BUY_TICKETS_NOT_FINISH");
        _;
    }
    ...
    function init(
        ...
        _BUY_TOKEN_ = addrList[0];
        _FEE_MODEL_ = addrList[1];
