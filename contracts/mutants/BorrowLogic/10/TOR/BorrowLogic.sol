...
library BorrowLogic {
  ...
  function executeRepay(
    ...
    if (params.useATokens) {
      ...
      IERC20(params.asset).safeTransferFrom(tx.origin, reserveCache.aTokenAddress, paybackAmount);
      IAToken(reserveCache.aTokenAddress).handleRepayment(tx.origin, paybackAmount);
    }

    emit Repay(params.asset, params.onBehalfOf, tx.origin, paybackAmount);
  ...
  function swapBorrowRateMode(
    ...
    (uint256 stableDebt, uint256 variableDebt) = Helpers.getUserCurrentDebt(tx.origin, reserve);
    ...
    if (interestRateMode == DataTypes.InterestRateMode.STABLE) {
      (reserveCache.nextTotalStableDebt, reserveCache.nextAvgStableBorrowRate) = IStableDebtToken(
        reserveCache.stableDebtTokenAddress
      ).burn(tx.origin, stableDebt);

      (, reserveCache.nextScaledVariableDebt) = IVariableDebtToken(
        reserveCache.variableDebtTokenAddress
      ).mint(tx.origin, tx.origin, stableDebt, reserveCache.nextVariableBorrowIndex);
    } else {
      reserveCache.nextScaledVariableDebt = IVariableDebtToken(
        reserveCache.variableDebtTokenAddress
      ).burn(tx.origin, variableDebt, reserveCache.nextVariableBorrowIndex);

      (, reserveCache.nextTotalStableDebt, reserveCache.nextAvgStableBorrowRate) = IStableDebtToken(
        reserveCache.stableDebtTokenAddress
      ).mint(tx.origin, msg.sender, variableDebt, reserve.currentStableBorrowRate);
    }
