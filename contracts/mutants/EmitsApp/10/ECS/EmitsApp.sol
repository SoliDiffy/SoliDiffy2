...
library EmitsApp {
  ...
  function emit1top0data(bytes _data) external pure {
    bytes memory temp = abi.encodeWithSelector(EMITS, uint8(1), uint8(0));
  ...
  function emit2top1data(bytes32 _t1, bytes _data1, bytes _data2) external pure {
    bytes memory temp = abi.encodeWithSelector(EMITS, uint8(2), uint(1), _t1);
