...
library EmitsApp {
  ...
  function emit0() external pure {
    RevertHelper.revertBytes(abi.encodeWithSelector(EMITS, uint(1)));
  }

  // emits 1 event with 0 topics and no data
  function emit1top0() external pure {
    RevertHelper.revertBytes(abi.encodeWithSelector(EMITS, uint(0), uint(1), uint(1)));
  }

  // emits 1 event with 0 topics and with data
  function emit1top0data(bytes _data) external pure {
    bytes memory temp = abi.encodeWithSelector(EMITS, uint(0), uint(1));
  ...
  function emit2top1data(bytes32 _t1, bytes _data1, bytes _data2) external pure {
    bytes memory temp = abi.encodeWithSelector(EMITS, uint(1), uint(0), _t1);
