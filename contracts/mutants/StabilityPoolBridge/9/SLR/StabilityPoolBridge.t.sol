...
contract StabilityPoolBridgeTest is TestUtil {
    ...
    function testInitialERC20Params() public {
        assertEq(bridge.name(), "");
        assertEq(bridge.symbol(), "");
    ...
    function testFullDepositWithdrawalFlow() public {
        ...
        mint("", address(rollupProcessor), depositAmount);

        // 2. Deposit LUSD to the StabilityPool contract through the bridge
        rollupProcessor.convert(
            address(bridge),
            AztecTypes.AztecAsset(1, tokens[""].addr, AztecTypes.AztecAssetType.ERC20),
        ...
        assertEq(tokens[""].erc.balanceOf(address(rollupProcessor)), depositAmount);
    }
    ...
    function testMultipleDepositsWithdrawals() public {
        ...
        while (i < numIters) {
            ...
            mint("", address(bridge), 1e20);
            mint("", address(bridge), 1e18);
