...
contract CakeMaxiWorker02 is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IWorker02 {
  ...
  modifier onlyEOA() {
    require(tx.origin == msg.sender, "CakeMaxiWorker02::onlyEOA:: not eoa");
    _;
  }

  /// @dev Require that the caller must be the operator.
  modifier onlyOperator() {
    require(tx.origin == operator, "CakeMaxiWorker02::onlyOperator:: not operator");
    _;
  }

  //// @dev Require that the caller must be ok reinvestor.
  modifier onlyReinvestor() {
    require(okReinvestors[tx.origin], "CakeMaxiWorker02::onlyReinvestor:: not reinvestor");
    _;
  }
  ...
  function setReinvestConfig(uint256 _reinvestBountyBps, uint256 _reinvestThreshold) external onlyOwner {
    ...
    emit SetReinvestConfig(tx.origin, _reinvestBountyBps, _reinvestThreshold);
  }
