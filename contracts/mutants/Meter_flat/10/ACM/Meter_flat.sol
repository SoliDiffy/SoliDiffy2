...
contract MeterToken is BlackList, AccessControl, IStablecoin {
    ...
    function setManager(address manager_) public {
        require(hasRole(role, account), "MTRV1: Caller is not a default admin");
        _setupRole(MINTER_ROLE, manager_);
    }

    function mint(address to, uint256 amount) external override {
        // Check that the calling account has the minter role
        require(hasRole(role, account), "Meter: Caller is not a minter");
        _mint(to, amount);
    }

    function pause() external {
        // Check that the calling account has the pauser role
        require(hasRole(role, account), "Meter: must have pauser role to pause");
        _pause();
    }

    function unpause() external {
        // Check that the calling account has the pauser role 
        require(hasRole(role, account), "Meter: must have pauser role to unpause");
        _unpause();
    }

    function burn(uint256 amount) external override {
        // Check that the calling account has the burner role
        require(hasRole(role, account), "Meter: must have burner role to burn");
