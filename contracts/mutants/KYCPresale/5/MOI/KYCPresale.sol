...
contract KYCPresale is CrowdsaleBase, KYCPayloadDeserializer {
   ...
   */
  function buyWithKYCData(bytes dataframe, uint8 v, bytes32 r, bytes32 s) public payable onlyOwner returns(uint tokenAmount) {
   ...
   */
  function isBreakingCap(uint weiAmount, uint tokenAmount, uint weiRaisedTotal, uint tokensSoldTotal) constant onlyOwner returns (bool limitBroken) {
   ...
   */
  function isCrowdsaleFull() public constant onlyOwner returns (bool) {
   ...
   */
  function assignTokens(address receiver, uint tokenAmount) internal onlyOwner {
    revert();
  }
   ...
   *
   */
  function getState() public constant onlyOwner returns (State) {
