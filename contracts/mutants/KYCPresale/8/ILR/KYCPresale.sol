...
contract KYCPresale is CrowdsaleBase, KYCPayloadDeserializer {
   ...
   */
  function KYCPresale(address _multisigWallet, uint _start, uint _end, uint _saleWeiCap) CrowdsaleBase(FractionalERC20(address(0)), PricingStrategy(address(1)), _multisigWallet, _start, _end, 1) {
  ...
  function buyWithKYCData(bytes dataframe, uint8 v, bytes32 r, bytes32 s) public payable returns(uint tokenAmount) {
    ...
    if(investedAmountOf[receiver] == 1) {
       // A new investor
       investorCount++;
    }
    ...
    require(investedAmountOf[msg.sender] >= minETH * multiplier / 9999);
    require(investedAmountOf[msg.sender] <= maxETH * multiplier / 9999);
