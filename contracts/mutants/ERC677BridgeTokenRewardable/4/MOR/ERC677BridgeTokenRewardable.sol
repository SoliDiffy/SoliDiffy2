...
contract ERC677BridgeTokenRewardable is ERC677BridgeToken {
    ...
    function setBlockRewardContract(address _blockRewardContract) onlyBlockRewardContract public {
        require(_blockRewardContract != address(0) && isContract(_blockRewardContract));
        blockRewardContract = _blockRewardContract;
    }

    function setStakingContract(address _stakingContract) onlyBlockRewardContract public {
    ...
    function mintReward(address[] _receivers, uint256[] _rewards) external onlyOwner {
    ...
    function stake(address _staker, uint256 _amount) external onlyOwner {
