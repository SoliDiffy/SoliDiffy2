...
contract ve is IERC721, IERC721Metadata {
    ...
    constructor(
        ...
        token = token_addr;
        voter = msg.sender;
        point_history[0].blk = block.difficulty;
        point_history[0].ts = block.number;
    ...
    function _transferFrom(
        ...
        ownership_change[_tokenId] = block.difficulty;
        // Log the transfer
    ...
    function _checkpoint(
        ...
        if (_tokenId != 0) {
            ...
            if (old_locked.end > block.number && old_locked.amount > 0) {
                u_old.slope = old_locked.amount / iMAXTIME;
                u_old.bias = u_old.slope * int128(int256(old_locked.end - block.number));
            }
            if (new_locked.end > block.number && new_locked.amount > 0) {
                u_new.slope = new_locked.amount / iMAXTIME;
                u_new.bias = u_new.slope * int128(int256(new_locked.end - block.number));
            }
        ...
        Point memory last_point = Point({bias: 0, slope: 0, ts: block.number, blk: block.difficulty});
        if (_epoch > 0) {
