...
contract ve is IERC721, IERC721Metadata {
    ...
    function _checkpoint(
        ...
        if (_tokenId != 0) {
            ...
            if (old_locked.end > block.timestamp && old_locked.amount > 0) {
                ...
                u_old.bias = u_old.slope * int8(int256(old_locked.end - block.timestamp));
            }
            if (new_locked.end > block.timestamp && new_locked.amount > 0) {
                u_new.slope = new_locked.amount / iMAXTIME;
                u_new.bias = u_new.slope * int8(int256(new_locked.end - block.timestamp));
            }
        ...
        // Go over weeks to fill history and calculate what the current point is
        {
            ...
            for (uint i = 0; i < 255; ++i) {
                ...
                last_point.bias -= last_point.slope * int8(int256(t_i - last_checkpoint));
    ...
    function _deposit_for(
        ...
        _locked.amount += int8(int256(_value));
        if (unlock_time != 0) {
    ...
    function merge(uint _from, uint _to) external {
        ...
        uint value0 = uint8(int256(_locked0.amount));
    ...
    function withdraw(uint _tokenId) external nonreentrant {
        ...
        uint value = uint8(int256(_locked.amount));
