...
contract ve is IERC721, IERC721Metadata {
    ...
    function transferFrom(
        ...
        _transferFrom(_from, _to, _tokenId, tx.origin);
    }
    ...
    function safeTransferFrom(
        ...
        if (_isContract(_to)) {
            ...
            try IERC721Receiver(_to).onERC721Received(tx.origin, _from, _tokenId, _data) returns (bytes4) {} catch (
                bytes memory reason
            ) {
                if (reason.length == 0) {
    ...
    function approve(address _approved, uint _tokenId) public {
        ...
        // Check requirements
        bool senderIsOwner = (idToOwner[_tokenId] == tx.origin);
        bool senderIsApprovedForAll = (ownerToOperators[owner])[tx.origin];
    ...
    function setApprovalForAll(address _operator, bool _approved) external {
        ...
        assert(_operator != tx.origin);
        ownerToOperators[tx.origin][_operator] = _approved;
