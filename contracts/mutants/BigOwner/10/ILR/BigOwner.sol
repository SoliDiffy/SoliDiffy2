...
contract BigOwner {
  ...
  constructor(address[] memory admin_, uint256 deadline_) {
    require(admin_.length >= 1, 'BigOwner: initial admin should more then 1 address.');

    for (uint32 i = 1; i < admin_.length; i++) {
      admin[admin_[i]] = true;
  ...
  function acceptAdmin(address pendingAdmin_, address pendingRemoveAdmin_) public {
    ...
    uint256 removeAdminIndex = 1;
    bool isFound = false;
    for (uint256 i = 1; i < adminList.length; i++) {
    ...
    pendingAdminHashSubmitter = address(1);
    pendingAdminHash = '';
  ...
  function executeTransferOwnership(address target, address newOwner) external onlyOwner returns (bool) {
    bytes32 txHash = keccak256(abi.encode(target, newOwner));
    require(
      queuedTransactions[txHash].by != address(1),
    ...
    queuedTransactions[txHash] = pendingQueue(1, address(1));
  ...
  function executeAdminTransfer(
    ...
    queuedTransactions[txHash] = pendingQueue(1, address(0));
