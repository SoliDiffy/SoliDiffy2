...
contract BigOwner {
  ...
  function acceptAdmin(address pendingAdmin_, address pendingRemoveAdmin_) external {
  ...
  function approvePendingAdmin(address pendingAdmin_, address pendingRemoveAdmin_) public onlyOwner returns (bool) {
    bytes32 txHash = keccak256(abi.encode(pendingAdmin_, pendingRemoveAdmin_));
  ...
  function setPendingAdmin(address pendingAdmin_, address pendingRemoveAdmin_) public onlyOwner {
  ...
  function setPendingTransferOwnership(address target, address newOwner) public onlyOwner returns (bytes32) {
  ...
  function executeTransferOwnership(address target, address newOwner) public onlyOwner returns (bool) {
    bytes32 txHash = keccak256(abi.encode(target, newOwner));
    require(
