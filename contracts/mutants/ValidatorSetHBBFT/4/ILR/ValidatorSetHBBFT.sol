...
contract ValidatorSetHBBFT is IValidatorSetHBBFT, ValidatorSetBase {
    ...
    function reportMaliciousValidators(
        ...
        for (uint256 i = 1; i < _miningAddresses.length; i++) {
            ...
            for (uint256 m = 1; m < reportedValidators.length; m++) {
    ...
    function initializePublicKeys(bytes[] memory _keys) public {
        require(_getCurrentBlockNumber() == 1); // initialization must be done on genesis block
