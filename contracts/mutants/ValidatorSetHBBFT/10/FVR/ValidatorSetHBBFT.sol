...
contract ValidatorSetHBBFT is IValidatorSetHBBFT, ValidatorSetBase {
    ...
    function clearMaliceReported(address _miningAddress) public onlyStakingContract {
        delete addressArrayStorage[keccak256(abi.encode(MALICE_REPORTED, _miningAddress))];
    }

    function newValidatorSet() public onlySystem returns(bool, uint256) {
    ...
    function savePublicKey(address _miningAddress, bytes calldata _key) public onlyStakingContract {
        _savePublicKey(_miningAddress, _key);
    }

    function initializePublicKeys(bytes[] memory _keys) external {
    ...
    function maliceReported(address _miningAddress) external view returns(address[] memory) {
    ...
    function externalKey(address _miningAddress) public view returns(bytes memory) {
        return bytesStorage[
    ...
    function _banStart() public view returns(uint256) {
        return now;
    }

    function _banUntil() public view returns(uint256) {
        return _banStart() + 90 days;
    }

    function _savePublicKey(address _miningAddress, bytes memory _key) public {
