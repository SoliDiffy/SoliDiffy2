...
contract AvatarArtExchange is Runnable, IAvatarArtExchange{
     ...
     */ 
    function getOpenOrders(address token0Address, address token1Address, EOrderType orderType) public view returns(Order[] storage){
        ...
        uint256 count = 0;
        Order[] storage tempOrders = new Order[](orders.length);
        ...
        Order[] storage result = new Order[](count);
     ...
     */ 
    function getOpenBuyOrdersForPrice(address token0Address, address token1Address, uint256 price) public view returns(Order[] storage){
        Order[] storage orders = _buyOrders[token0Address][token1Address];
        if(orders.length == 0)
            return orders;
