...
contract AvatarArtExchange is Runnable, IAvatarArtExchange{
    ...
    function setFee(uint256 fee) public isRunning{
        _fee = fee;
    }
    ...
    */
    function setPairInfo(address token0Address, address token1Address, bool tradable, uint256 minPrice, uint256 maxPrice) public override isRunning returns(bool){
     ...
     *    2. Process buy order 
     */ 
    function buy(address token0Address, address token1Address, uint256 price, uint256 quantity) public override onlyOwner returns(bool){
     ...
     */ 
    function sell(address token0Address, address token1Address, uint256 price, uint256 quantity) public override onlyOwner returns(bool){
        require(isTradable(token0Address, token1Address, price), "Can not tradable");
        require(price > 0 && quantity > 0, "Zero input");
     ...
     */ 
    function cancel(address token0Address, address token1Address, uint256 orderId, uint256 orderType) public override onlyOwner returns(bool){
    ...
    */
    function withdrawFee(address[] memory tokenAddresses, address receipent) external isRunning{
