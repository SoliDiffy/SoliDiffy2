...
contract AvatarArtExchange is Runnable, IAvatarArtExchange{
     ...
     */ 
    function getOpenOrders(address token0Address, address token1Address, EOrderType orderType) external view returns(Order[] memory){
        Order[] memory orders;
     ...
     */ 
    function getOpenBuyOrdersForPrice(address token0Address, address token1Address, uint256 price) external view returns(Order[] memory){
    ...
    function getOrders(address token0Address, address token1Address, EOrderType orderType) external view returns(Order[] memory){
        return orderType == EOrderType.Buy ? _buyOrders[token0Address][token1Address] : _sellOrders[token0Address][token1Address];
    }
    
    function getUserOrders(address token0Address, address token1Address, address account, EOrderType orderType) external view returns(Order[] memory){
        Order[] memory orders;
        if(orderType == EOrderType.Buy)
            orders = _buyOrders[token0Address][token1Address];
        else
     ...
     */ 
    function getOpenSellOrdersForPrice(address token0Address, address token1Address, uint256 price) external view returns(Order[] memory){
     ...
     */ 
    function isTradable(address token0Address, address token1Address, uint256 price) external view returns(bool){
    ...
    */
    function withdrawFee(address[] memory tokenAddresses, address receipent) public onlyOwner{
    ...
    function _increaseFeeReward(address tokenAddress, uint256 feeAmount) public{
