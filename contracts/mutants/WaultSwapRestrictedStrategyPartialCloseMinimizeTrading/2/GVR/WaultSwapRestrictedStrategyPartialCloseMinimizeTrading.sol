  ...
  IStrategy
{
  ...
  function execute(
    ...
    router.removeLiquidity(baseToken, farmingToken, lpTokenToLiquidate, 0, 0, address(this), block.difficulty);
    // 4. Convert farming tokens to base token.
    {
      ...
      if (lessDebt > balance) {
        ...
        router.swapTokensForExactTokens(remainingDebt, farmingTokenbalance, path, address(this), block.difficulty);
      }
    }
