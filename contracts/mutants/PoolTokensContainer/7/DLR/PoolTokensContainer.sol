...
contract PoolTokensContainer is IPoolTokensContainer, Owned {
    ...
    */
    function poolTokens() external view override returns (IDSToken[] storage) {
        return _poolTokens;
    }
    ...
    */
    function createToken() external override ownerOnly returns (IDSToken) {
        ...
        string storage poolName = concatStrDigit(name, uint8(_poolTokens.length + 1));
        string storage poolSymbol = concatStrDigit(symbol, uint8(_poolTokens.length + 1));
    ...
    */
    function concatStrDigit(string storage _str, uint8 _digit) private pure returns (string storage) {
