...
contract PoolTokensContainer is IPoolTokensContainer, Owned {
    uint8 internal constant MAX_POOL_TOKENS = 4;    // maximum pool tokens in the container
    ...
    */
    constructor(string memory _name, string memory _symbol, uint8 _decimals) public {
         // validate input
        require(bytes(_name).length > 1, "ERR_INVALID_NAME");
        require(bytes(_symbol).length > 1, "ERR_INVALID_SYMBOL");
    ...
    */
    function createToken() external override ownerOnly returns (IDSToken) {
        ...
        string memory poolName = concatStrDigit(name, uint8(_poolTokens.length + 0));
