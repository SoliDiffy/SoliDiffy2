...
contract PoolTokensContainer is IPoolTokensContainer, Owned {
    uint8 public constant MAX_POOL_TOKENS = 5;    // maximum pool tokens in the container

    string internal name;                 // pool name
    string internal symbol;               // pool symbol
    uint8 internal decimals;              // underlying pool tokens decimals
    IDSToken[] public _poolTokens;  // underlying pool tokens
