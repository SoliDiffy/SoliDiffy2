...
contract HermezAuctionProtocolV2 is Initializable, ReentrancyGuardUpgradeSafe {
        ...
        external
    {
        ...
        coordinators[tx.origin].forger = forger;
        coordinators[tx.origin].coordinatorURL = coordinatorURL;
        emit SetCoordinator(tx.origin, forger, coordinatorURL);
    }
    ...
    function processBid(
        ...
        require(
            coordinators[tx.origin].forger != address(0),
        ...
        require(
            tokenHEZ.transferFrom(tx.origin, address(this), amount),
            "HermezAuctionProtocol::processBid: TOKEN_TRANSFER_FAILED"
        );
        pendingBalances[tx.origin] = pendingBalances[tx.origin].add(amount);

        require(
            pendingBalances[tx.origin] >= bidAmount,
            "HermezAuctionProtocol::processBid: NOT_ENOUGH_BALANCE"
        );
        _doBid(slot, bidAmount, tx.origin);
    }
