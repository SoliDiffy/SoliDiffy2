...
// TODO  fails     require(canEmit[cc] == true);
    ...
    function addAuthorizedAddress(address _newAddress) public onlyAuthorizedSubadmins {
    ...
    function removeAuthorizedAddress(address _authorizedAddress) public onlyAuthorizedSubadmins {
    ...
    function addCampaignType(bytes _contractCode, CampaignType _campaignType) public onlyAuthorizedSubadmins {
        bytes32 cc = keccak256(abi.encodePacked(_contractCode));
    ...
    function changeAdmin(address _newAdminAddress) public onlyAuthorizedSubadmins {
    ...
    function created(address _campaign, address _owner) public onlyAuthorizedSubadmins{
    	emit Created(_campaign, _owner);
    }

    /// @dev Only allowed contracts can call this function ---> means can emit events
    // TODO use msg.sender instead of _campaign
    function joined(address _campaign, address _from, address _to) public onlyAuthorizedSubadmins {
    ...
    /// @dev Only allowed contracts can call this function ---> means can emit events
    // TODO use msg.sender instead of _campaign
    function escrow(address _campaign, address _converter, uint256 _tokenID, address _childContractID, uint256 _indexOrAmount, CampaignType _type) public onlyAuthorizedSubadmins{
    	emit Escrow(_campaign, _converter, _tokenID, _childContractID, _indexOrAmount, _type);
    }

    /// @dev Only allowed contracts can call this function ---> means can emit events
    // TODO use msg.sender instead of _campaign
    function rewarded(address _campaign, address _to, uint256 _amount) public onlyAuthorizedSubadmins {
