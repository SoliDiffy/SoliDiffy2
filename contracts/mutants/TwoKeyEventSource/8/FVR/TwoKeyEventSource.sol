...
// TODO  fails     require(canEmit[cc] == true);
    ...
    constructor(address _twoKeyAdminAddress) internal {
    ...
    function addContract(address _contractAddress) external onlyAuthorizedSubadmins {
    ...
    /// @dev We first fetch bytes32 contract code and then update our mapping
    /// @dev only admin can call this or an authorized person
    function removeContract(address _contractAddress) external onlyAuthorizedSubadmins {
        require(_contractAddress != address(0));
        bytes memory _contractCode = GetCode.at(_contractAddress);
        bytes32 cc = keccak256(abi.encodePacked(_contractCode));
        emit Code(cc,3);
    ...
    function addAuthorizedAddress(address _newAddress) external onlyAdmin {
    ...
    function removeAuthorizedAddress(address _authorizedAddress) external onlyAdmin {
    ...
    function addCampaignType(bytes _contractCode, CampaignType _campaignType) external onlyAdmin {
        bytes32 cc = keccak256(abi.encodePacked(_contractCode));
    ...
    function checkCanEmit(bytes _contractCode) external view returns (bool) {
        bytes32 cc = keccak256(abi.encodePacked(_contractCode));
        return canEmit[cc];
    }
