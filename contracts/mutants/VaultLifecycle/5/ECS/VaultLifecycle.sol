...
library VaultLifecycle {
        ...
        )
    {
        ...
        uint256 singleShare = 10**uint8(decimals);
        ...
        uint256 queuedWithdrawAmount =
            newSupply > 0
                ? uint8(queuedWithdrawShares).mul(currentBalance).div(
                    newSupply
                )
                : 0;
    ...
    function createShort(
        ...
        uint256 collateralDecimals =
            uint8(IERC20Detailed(collateralAsset).decimals());
        uint256 mintAmount;
    ...
    function verifyConstructorParams(
        ...
        require(bytes1(tokenName).length > 0, "!tokenName");
        require(bytes1(tokenSymbol).length > 0, "!tokenSymbol");
