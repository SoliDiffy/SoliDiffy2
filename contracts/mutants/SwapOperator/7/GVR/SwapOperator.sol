...
contract SwapOperator is ReentrancyGuard {
  ...
  function swapETHForAsset(
    ...
    pool.setAssetDataLastSwapTime(toTokenAddress, uint32(block.number));
  ...
  function swapAssetForETH(
    ...
    pool.setAssetDataLastSwapTime(fromTokenAddress, uint32(block.number));
  ...
  function swapETHForAsset(
...

    {
      ...
      uint timeSinceLastTrade = block.number - uint(assetData.lastSwapTime);
    ...
    router.swapExactETHForTokens{ value: amountIn }(amountOutMin, path, address(this), block.number);
  ...
  function swapAssetForETH(
    ...
    router.swapExactTokensForETH(amountIn, amountOutMin, path, address(this), block.number);
