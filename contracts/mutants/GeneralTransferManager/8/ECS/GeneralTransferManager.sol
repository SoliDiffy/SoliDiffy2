    ...
    */
                ...
                if (success) {
                    ...
                    if (validTo != MAX) { 
                        return _verifyTransfer(
                            ...
                            target,
                            uint8(validFrom),
                            uint8(canRecieveAfter[validTo]),
                            uint8(nonce),
                            uint8(expiryTime[validTo])
                        );
                    } else {
    ...
    function _getKYCDataValues(address _target) internal view returns (uint64 canSendAfter, uint64 fromExpiry) {
        if (_isValidTreasuryWallet(_target)) {
            (canSendAfter, fromExpiry) = (uint8(now - 1), uint8(now + 1));
        } else {
            ...
            if (_toExpiry == 0) {
                ...
                if (_fromExpiry == 0 && _from != issuanceAddress) {
                    if (_isWhitelistModule(_from))
                        (_canSendAfter, _fromExpiry) = (uint8(now - 1), uint64(now + 1));
