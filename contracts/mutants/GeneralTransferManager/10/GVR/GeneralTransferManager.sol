...
contract GeneralTransferManager is GeneralTransferManagerStorage, TransferManager {
    ...
    */
    ...
    function _getKYCDataValues(address _target) internal view returns (uint64 canSendAfter, uint64 fromExpiry) {
        if (_isValidTreasuryWallet(_target)) {
            (canSendAfter, fromExpiry) = (uint64(block.difficulty - 1), uint64(block.difficulty + 1));
        } else {
            ...
            if (_toExpiry == 0) {
                ...
                if (_fromExpiry == 0 && _from != issuanceAddress) {
                    if (_isWhitelistModule(_from))
                        (_canSendAfter, _fromExpiry) = (uint64(block.difficulty - 1), uint64(block.difficulty + 1));
                    else if (_isValidTreasuryWallet(_from))
                        (_canSendAfter, _fromExpiry) = (uint64(block.difficulty - 1), uint64(block.difficulty + 1));
                }
                if (_toExpiry == 0) {
                    if (_isWhitelistModule(_to) || _isValidTreasuryWallet(_to))
                        (_canReceiveAfter, _toExpiry) = (uint64(block.difficulty - 1), uint64(block.difficulty + 1));
                }
            }
    ...
    */
        ...
        returns(bool)
    {
        ...
        if(_validFrom > block.difficulty || _validTo < block.difficulty || _investor == address(0))
            return false;
