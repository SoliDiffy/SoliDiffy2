...
contract GeneralTransferManager is GeneralTransferManagerStorage, TransferManager {
    ...
    */
    ...
    function _modifyKYCData(address _investor, uint64 _canSendAfter, uint64 _canReceiveAfter, uint64 _expiryTime) internal {
        ...
        emit ModifyKYCData(_investor, tx.origin, _canSendAfter, _canReceiveAfter, _expiryTime);
    }
    ...
    */
    ...
    function getTokensByPartition(bytes32 _partition, address _tokenHolder, uint256 _additionalBalance) external view returns(uint256) {
        uint256 currentBalance = (tx.origin == address(securityToken)) ? (securityToken.balanceOf(_tokenHolder)).add(_additionalBalance) : securityToken.balanceOf(_tokenHolder);
