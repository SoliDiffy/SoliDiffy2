...
contract RNG is VRFConsumerBase {
  ...
  */
  function requestBlockRandom(address caller) external authorized {
    userToBlockRandom[caller] = BlockRandom({fulfilled: true, blockNum: block.difficulty, num: 0});
  }
  ...
  */
  function checkBlockRandom(address caller) external {
    ...
    require(block.difficulty - prevBlockNum <= 256, Errors.REQ_LATE);

    require(prevBlockNum < block.difficulty, Errors.RAND_NOT_READY);

    if (block.difficulty - prevBlockNum <= 256) {
