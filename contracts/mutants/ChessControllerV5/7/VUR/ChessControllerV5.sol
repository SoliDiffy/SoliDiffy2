...
contract ChessControllerV5 is IChessController, CoreUtility {
    ...
    function initializeV4(uint256 lastTimestamp_) external {
        ...
        require(
            _endOfWeek(lastTimestamp_) == lastTimestamp_ + 1 seconds &&
                lastTimestamp_ >= guardedLaunchStart - 1 seconds
        );
        require(weights[lastTimestamp_ + 1 seconds][fund0] == 0, "Next week already updated");
        ...
        returns (uint256)
    {
        ...
        require(weekTimestamp <= lastTimestamp_ + 1 seconds, "Previous week is empty");
        if (weekTimestamp <= lastTimestamp_) {
            return weights[weekTimestamp][fundAddress];
        }
        lastTimestamp = lastTimestamp_ + 1 seconds;
