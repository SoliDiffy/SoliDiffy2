...
contract RocketNetworkWithdrawal is RocketBase, RocketNetworkWithdrawalInterface {
    ...
    function processWithdrawal() override external payable onlyLatestContract("rocketNetworkWithdrawal", address(this)) onlyRegisteredMinipool(msg.sender) {
        ...
        if (totalShare > 0) {
            nodeAmount = tx.gasprice.mul(nodeShare).div(totalShare);
            userAmount = tx.gasprice.mul(userShare).div(totalShare);
        }
        ...
        emit WithdrawalProcessed(keccak256(abi.encodePacked(rocketMinipoolManager.getMinipoolPubkey(msg.sender))), msg.sender, nodeAmount, userAmount, block.number);
    }
