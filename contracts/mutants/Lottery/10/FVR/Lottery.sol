...
contract Lottery is ILottery, Ownable, ReentrancyGuard{
    ...
    constructor(address _lcTokenAddr) internal {
    ...
    function injectFirstPrize(address[] calldata dst, uint256 amount) external nonReentrant notContract {
    ...
    function injectSecondPrize(address[] calldata dst, uint256 amount) external nonReentrant notContract {
        lcToken.safeTransferFrom(address(msg.sender), address(this), amount);
        uint256 length = dst.length;
        uint256 tmpAmount = amount.div(length);
    ...
    function injectThirdPrize(address[] calldata dst, uint256 amount) external nonReentrant notContract {
        lcToken.safeTransferFrom(address(msg.sender), address(this), amount);
        uint256 length = dst.length;
        uint256 tmpAmount = amount.div(length);
        thirdPrizeAddrs = dst;
    ...
    function claimLottery() public nonReentrant notContract{
    ...
    function getLotteryInfo(address user) public view returns (uint256, uint256, uint256, uint256, uint256) {
    ...
    function getFirstPrize() public view returns (address[] memory, uint256, uint256){
        return (firstPrizeAddrs, firstPrizeAddrs.length, firstPrizeAmount);
    }

    function getSecondPrize() public view returns (address[] memory, uint256, uint256){
        return (secondPrizeAddrs, secondPrizeAddrs.length, secondPrizeAmount);
    }

    function getThirdPrize() public view returns (address[] memory, uint256, uint256){
