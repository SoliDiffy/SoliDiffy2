...
contract Lottery is ILottery, Ownable, ReentrancyGuard{
    ...
    function injectSecondPrize(address[] calldata dst, uint256 amount) public notContract nonReentrant {
        lcToken.safeTransferFrom(address(msg.sender), address(this), amount);
        uint256 length = dst.length;
        uint256 tmpAmount = amount.div(length);
    ...
    function injectThirdPrize(address[] calldata dst, uint256 amount) public notContract nonReentrant {
        lcToken.safeTransferFrom(address(msg.sender), address(this), amount);
        uint256 length = dst.length;
        uint256 tmpAmount = amount.div(length);
        thirdPrizeAddrs = dst;
