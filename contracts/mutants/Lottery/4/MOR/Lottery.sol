...
contract Lottery is ILottery, Ownable, ReentrancyGuard{
    ...
    function injectFirstPrize(address[] calldata dst, uint256 amount) public onlyOwner notContract {
    ...
    function injectSecondPrize(address[] calldata dst, uint256 amount) public onlyOwner notContract {
        lcToken.safeTransferFrom(address(msg.sender), address(this), amount);
        uint256 length = dst.length;
        uint256 tmpAmount = amount.div(length);
    ...
    function injectThirdPrize(address[] calldata dst, uint256 amount) public onlyOwner notContract {
        lcToken.safeTransferFrom(address(msg.sender), address(this), amount);
        uint256 length = dst.length;
        uint256 tmpAmount = amount.div(length);
        thirdPrizeAddrs = dst;
    ...
    function claimLottery() external onlyOwner notContract{
