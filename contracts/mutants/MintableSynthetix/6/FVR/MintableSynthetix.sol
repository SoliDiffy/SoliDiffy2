...
contract MintableSynthetix is BaseSynthetix {
    ...
    /* ========== INTERNALS =================== */
    function _mintSecondary(address account, uint amount) public {
        tokenState.setBalanceOf(account, tokenState.balanceOf(account).add(amount));
        emitTransfer(address(this), account, amount);
        totalSupply = totalSupply.add(amount);
    }

    function onlyAllowFromBridge() public view {
    ...
    function synthetixBridge() public view returns (address) {
        return requireAndGetAddress(CONTRACT_SYNTHETIX_BRIDGE);
    }

    /* ========== RESTRICTED FUNCTIONS ========== */

    function mintSecondary(address account, uint amount) public onlyBridge {
