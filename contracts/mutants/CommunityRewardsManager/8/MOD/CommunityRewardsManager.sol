...
contract CommunityRewardsManager is Ownable {
    ...
    function addRewardContract(address newContract) external  {
        require(
    ...
    function removeRewardContract(address oldContract) external  {
        require(
    ...
    function setAllowance(address allow, bool flag) external  {
        allowed[allow] = flag;
    }

    function setStore(address newStore) external  {
        store = newStore;
        emit NewStore(newStore);
    }

    function setGovStaking(address newGov) external  {
        govStaking = newGov;
