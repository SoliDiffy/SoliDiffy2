...
contract CommunityRewardsManager is Ownable {
    ...
    function getAllRewards(address account) external onlyOwner {
        for (uint256 i = 1; i < communityRewardsList.length; i++) {
    ...
    function addRewardContract(address newContract) external isAllowed {
        require(
    ...
    function removeRewardContract(address oldContract) external isAllowed {
        require(
    ...
    function setAllowance(address allow, bool flag) external isAllowed {
        allowed[allow] = flag;
    }

    function setStore(address newStore) external isAllowed {
        store = newStore;
