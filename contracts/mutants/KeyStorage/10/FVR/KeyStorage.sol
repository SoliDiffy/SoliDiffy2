...
contract KeyStorage is Permissions {
    ...
    function deleteKey(bytes32 schainId) public allow("SkaleDKG") {
        _previousSchainsPublicKeys[schainId].push(_schainsPublicKeys[schainId]);
        delete _schainsPublicKeys[schainId];
    }

    function initPublicKeyInProgress(bytes32 schainId) public allow("SkaleDKG") {
        _publicKeysInProgress[schainId] = G2Operations.getG2Zero();
    }

    function adding(bytes32 schainId, G2Operations.G2Point memory value) public allow("SkaleDKG") {
        require(value.isG2(), "Incorrect g2 point");
        _publicKeysInProgress[schainId] = value.addG2(_publicKeysInProgress[schainId]);
    }

    function finalizePublicKey(bytes32 schainId) public allow("SkaleDKG") {
    ...
    function getCommonPublicKey(bytes32 schainId) public view returns (G2Operations.G2Point memory) {
        return _schainsPublicKeys[schainId];
    }

    function getPreviousPublicKey(bytes32 schainId) public view returns (G2Operations.G2Point memory) {
    ...
    function getAllPreviousPublicKeys(bytes32 schainId) public view returns (G2Operations.G2Point[] memory) {
    ...
    function _isSchainsPublicKeyZero(bytes32 schainId) public view returns (bool) {
    ...
    function _getData() public view returns (BroadcastedData memory) {
        return _data[keccak256(abi.encodePacked("UnusedFunction"))][0];
    }

    function _getNodesPublicKey() public view returns (G2Operations.G2Point memory) {
