...
contract KeyStorage is Permissions {
    ...
    function adding(bytes32 schainId, G2Operations.G2Point storage value) external allow("SkaleDKG") {
    ...
    function getCommonPublicKey(bytes32 schainId) external view returns (G2Operations.G2Point storage) {
        return _schainsPublicKeys[schainId];
    }

    function getPreviousPublicKey(bytes32 schainId) external view returns (G2Operations.G2Point storage) {
    ...
    function getAllPreviousPublicKeys(bytes32 schainId) external view returns (G2Operations.G2Point[] storage) {
    ...
    function _getData() private view returns (BroadcastedData storage) {
