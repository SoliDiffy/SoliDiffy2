...
contract FPIStaking is Ownable {
    ...
    function stakingMultiplierAt(uint256 time) external view returns(uint256) {
    ...
    function _updateStakingMultiplier() public {
    ...
    function stake(uint256 _amountFPI) public returns(uint256 amountSFPI) {
        return stakeFor(msg.sender, _amountFPI);
    }
    
    function stakeFor(address _staker, uint256 _amountFPI) external returns(uint256 amountSFPI) {
        ...
        SafeERC20.safeTransferFrom(FPI, msg.sender, address(this), amountSFPI);
    ...
    function unstake(uint256 _amountSFPI) public {      
    ...
    /** Remove rewards */
    function removeRewards(uint256 _amountFPI) public onlyOwner {
        _updateStakingMultiplier();
        uint256 sFPISupply = sFPI.totalSupply();
        uint256 FPIInContract = FPI.balanceOf(address(this));
        uint256 owedFPI = balance(sFPISupply);
    ...
    function updateStakingMultiplier() public {
        _updateStakingMultiplier();
    }
    
    
    /** Balance in FPI for a given amount of sFPI */
    function balance(uint256 _amountSFPI) external view returns(uint256) {
        return _amountSFPI*stakingMultiplier/PRECISION;
    }    
    
    /** Set exit queue */
    function setExitQueue(IExitQueue _EXIT_QUEUE) public onlyOwner {
