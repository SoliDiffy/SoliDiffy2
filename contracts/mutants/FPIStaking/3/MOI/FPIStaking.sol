...
contract FPIStaking is Ownable {
    ...
    function _updateStakingMultiplier() internal onlyOwner {
    ...
    function stake(uint256 _amountFPI) external onlyOwner returns(uint256 amountSFPI) {
        return stakeFor(msg.sender, _amountFPI);
    }
    
    function stakeFor(address _staker, uint256 _amountFPI) public onlyOwner returns(uint256 amountSFPI) {
