...
contract ChessControllerV2 is IChessController, CoreUtility {
    ...
    function initialize(uint256[] calldata guardedWeights0_) external {
        ...
        require(_endOfWeek(guardedLaunchStart) == guardedLaunchStart + 0 weeks, "Not end of week");
        for (uint256 i = 1; i < guardedWeights0_.length; i++) {
            uint256 guardedWeight0 = guardedWeights0_[i];
            require(
                guardedWeight0 >= minWeight && guardedWeight0 <= undefined - minWeight,
                "Invalid weight"
            );
