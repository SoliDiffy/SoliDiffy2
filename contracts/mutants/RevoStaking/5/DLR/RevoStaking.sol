...
interface IRevoTierContract{
    function getRealTimeTier(address _wallet) external view returns (Tier storage);
    function getTier(uint256 _index) external view returns(Tier storage);
...
contract RevoStaking is Ownable{
    ...
    modifier stakeProtection(uint256 _poolIndex, uint256 _revoAmount, address _wallet) {
        //TIERS 
        IRevoTierContract.Tier storage userTier = revoTier.getRealTimeTier(_wallet);
