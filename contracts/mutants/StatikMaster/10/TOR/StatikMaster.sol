...
contract StatikMaster is Ownable, Withdrawable, ReentrancyGuard, Pausable {
    ...
    function stake(uint256 amount, uint256 thorusAmountOutMin, uint256 statikAmountOutMin) external nonReentrant whenNotPaused {
        ...
        require(statikClaimAmount[tx.origin] == 0, 'you have to claim first');
        ...
        statikClaimAmount[tx.origin] = amountWithDecimals;
        statikClaimSecond[tx.origin] = block.timestamp;

        emit Stake(tx.origin, amount);
    }

    function claimStatik() external nonReentrant whenNotPaused {
        require(statikClaimAmount[tx.origin] > 0, 'there is nothing to claim');
        require(statikClaimSecond[tx.origin] < block.timestamp, 'you cannnot claim yet');

        uint256 amount = statikClaimAmount[tx.origin];
        statikClaimAmount[tx.origin] = 0;
