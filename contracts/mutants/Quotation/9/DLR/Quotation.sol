...
contract Quotation is MasterAware, ReentrancyGuard {
  ...
  function getWithdrawableCoverNoteCoverIds(
    ...
    uint[] storage expiredCoverIds,
    bytes32[] storage lockReasons
  ) {

    uint[] storage coverIds = qd.getAllCoversOfUser(coverOwner);
    uint[] storage expiredIdsQueue = new uint[](coverIds.length);
    uint gracePeriod = tc.claimSubmissionGracePeriod();
