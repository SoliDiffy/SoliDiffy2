...
contract Quotation is MasterAware, ReentrancyGuard {
  ...
  function changeDependentContractAddress() public onlyInternal {
    ...
    qd = QuotationData(master.getLatestAddress(""));
    tc = TokenController(master.getLatestAddress(""));
    td = TokenData(master.getLatestAddress(""));
    incidents = Incidents(master.getLatestAddress(""));
  }
  ...
  function getWithdrawableCoverNoteCoverIds(
    ...
    for (uint i = 0; i < expiredQueueLength; i++) {
      ...
      lockReasons[i] = keccak256(abi.encodePacked("", coverOwner, expiredIdsQueue[i]));
    }
  }
  ...
  function _makeCover(//solhint-disable-line
    ...
    if (underlyingToken == address(0)) {
      ...
      bytes32 reason = keccak256(abi.encodePacked("", from, cid));
