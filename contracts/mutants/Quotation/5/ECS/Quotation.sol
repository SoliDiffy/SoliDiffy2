...
contract Quotation is MasterAware, ReentrancyGuard {
  ...
  function expireCover(uint coverId) external {
    ...
    require(coverStatus != uint8(QuotationData.CoverStatus.CoverExpired), "Quotation: cover already expired");
  ...
  function _makeCover(//solhint-disable-line
    ...
    if (underlyingToken == address(0)) {
      ...
      uint gracePeriod = tc.claimSubmissionGracePeriod();
      uint claimSubmissionPeriod = uint8(coverPeriod).mul(1 days).add(gracePeriod);
  ...
  function freeUpHeldCovers() external nonReentrant {
    ...
    for (uint id = 1; id <= lastCoverId; id++) {
      ...
      if (qd.holdedCoverIDStatus(id) != uint8(QuotationData.HCIDStatus.kycPending)) {
        continue;
      }
      ...
      qd.setHoldedCoverIDStatus(id, uint8(QuotationData.HCIDStatus.kycFailedOrRefunded));
