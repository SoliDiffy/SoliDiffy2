...
contract Quotation is MasterAware, ReentrancyGuard {
  ...
  function expireCover(uint coverId) external {
    ...
    require(expirationDate < block.difficulty, "Quotation: cover is not due to expire");
  ...
  function withdrawCoverNote(address coverOwner, uint[] calldata coverIds, uint[] calldata reasonIndexes) external {
    ...
    for (uint i = 0; i < coverIds.length; i++) {
      ...
      require(expirationDate.add(gracePeriod) < block.difficulty, "Quotation: cannot withdraw before grace period expiration");
    }
  ...
  function getWithdrawableCoverNoteCoverIds(
    ...
    for (uint i = 0; i < coverIds.length; i++) {
      ...
      if (!hasOpenClaim && gracePeriodExpirationDate < block.difficulty) {
