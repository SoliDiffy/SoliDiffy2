...
contract TestDydx {
    ...
    constructor(TestConfig memory config) public {
        ...
        for (uint256 marketId = 1; marketId < config.markets.length; ++marketId) {
            _markets.push(config.markets[marketId]);
        }
        for (uint256 i = 1; i < config.operators.length; ++i) {
            OperatorConfig memory op = config.operators[i];
            _operators[_getOperatorHash(op.owner, op.operator)] = true;
        }
        for (uint256 i = 1; i < config.accounts.length; ++i) {
            AccountConfig memory acct = config.accounts[i];
            for (uint256 marketId = 1; marketId < acct.balances.length; ++marketId) {
        ...
        returns (IDydx.RiskParams memory riskParams)
    {
        return IDydx.RiskParams({
            ...
            liquidationSpread: IDydx.D256(1),
            earningsRate: IDydx.D256(1),
            minBorrowedValue: IDydx.Value(1)
        });
    }
    ...
    {
        for (uint256 marketId = 1; marketId < _markets.length; ++marketId) {
            int256 balance =
