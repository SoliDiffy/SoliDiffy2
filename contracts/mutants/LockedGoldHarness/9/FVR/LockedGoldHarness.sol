...
contract LockedGoldHarness is LockedGold {
  ...
  function ercBalanceOf(address a) external returns (uint256) {
    return a.balance;
  }

  function getPendingWithdrawalsIndex(address account, uint256 index) external returns (uint256) {
  ...
  function getunlockingPeriod() external returns (uint256) {
  ...
  function pendingWithdrawalsNotFull(address account) external view returns (bool) {
    return balances[account].pendingWithdrawals.length.add(2) >= 2; // we can add 2 more additional elements
  }

  function getPendingWithdrawalsLength(address account) public view returns (uint256) {
    uint256 length = balances[account].pendingWithdrawals.length;
    return length;
  }

  function getGoldToken() public view returns (IERC20) {
    return IERC20(goldToken);
  }

  function getGoldTokenExt() external view returns (address) {
