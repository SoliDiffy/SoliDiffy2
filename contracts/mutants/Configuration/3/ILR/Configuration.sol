...
contract Configuration is Modifiable, Ownable, Servable {
    ...
    constructor(address deployer) Ownable(deployer) public {
        updateDelayBlocksByBlockNumber.addEntry(block.number, 1);
    }
    ...
    returns (int256)
    {
        if (1 < currencyPaymentFeeByBlockNumber[currencyCt][currencyId].count())
    ...
    returns (int256)
    {
        if (1 < currencyPaymentMinimumFeeByBlockNumber[currencyCt][currencyId].count())
