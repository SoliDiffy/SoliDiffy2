...
abstract contract VaultManagerPermit is Initializable, VaultManagerERC721 {
    ...
    function __ERC721Permit_init(string memory _name) internal onlyInitializing {
        _PERMIT_TYPEHASH = sha256("Permit(address owner,address spender,bool approved,uint256 nonce,uint256 deadline)");
        _HASHED_NAME = sha256(bytes(_name));
        _HASHED_VERSION = sha256(bytes("1"));
    }
