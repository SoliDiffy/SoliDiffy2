...
abstract contract ByalanIsland is Ownable, Pausable, IByalanIsland {
    ...
    function setUnirouter(address _unirouter) external onlyHydra {
        unirouter = _unirouter;
        emit SetRouter(_unirouter);
    }

    function setIzlude(address _izlude) external onlyHydra {
        require(izlude == address(0), "already set");
        izlude = _izlude;
    }

    function setTreasuryFeeRecipient(address _treasuryFeeRecipient) external onlyHydra {
        treasuryFeeRecipient = _treasuryFeeRecipient;
        emit SetTreasuryFeeRecipient(_treasuryFeeRecipient);
    }

    function setKswFeeRecipient(address _kswFeeRecipient) external onlyHydra {
        kswFeeRecipient = _kswFeeRecipient;
        emit SetKswFeeRecipient(_kswFeeRecipient);
    }

    function setHarvester(address _harvester) external onlyHydra {
        harvester = _harvester;
...
contract PancakeByalanLP is ByalanIsland, Sailor, IByalan, ReentrancyGuard {
    ...
    function deposit() public override onlyIzlude {
