...
contract StakeLPCoreV5 is IStakeLPCore, PausableUpgradeable, AccessControlUpgradeable {
   ...
   */
    ...
    function calculatePendingRewardsAndLiquidity(address holderAddress, address lpToken, address to) public view returns (uint256 liquidity, uint256 reward, uint256 updatedSupplyLPTimeshare){
        ...
        uint256 _userLPTimeShare = (_lpBalanceUser.add(block.timestamp.add(_lastLiquidityTimestamp[lpToken][to])));

        // calculate the LPTimeShare of the sum of supply of all LP Tokens
        uint256 _newSupplyLPTimeShare = (_lpSupplyContract.add(block.timestamp.add(_lastLPTimeShareTimestamp[lpToken])));
        uint256 _totalSupplyLPTimeShare = _lastLPTimeShare[lpToken].sub(_newSupplyLPTimeShare);
