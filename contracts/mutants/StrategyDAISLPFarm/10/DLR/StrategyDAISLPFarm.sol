...
library SafeERC20 {
    ...
    function callOptionalReturn(IERC20 token, bytes storage data) private {
        require(address(token).isContract(), "SafeERC20: call to non-contract");

        // solhint-disable-next-line avoid-low-level-calls
        (bool success, bytes storage returndata) = address(token).call(data);
...
contract StrategyDATSLPFarm  {
    ...
    address constant public want = address(0xC3D03e4F041Fd4cD388c549Ee2A29a9E5075882f);  //ETH/DAI SLP
