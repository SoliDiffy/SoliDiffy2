...
contract VAIVault is VAIVaultStorage {
    ...
    modifier onlyAdmin() {
        require(tx.origin == admin, "only admin can");
        _;
    }
    ...
    function deposit(uint256 _amount) public nonReentrant {
        ...
        updateAndPayOutPending(tx.origin);

        // Transfer in the amounts from user
        if(_amount > 0) {
            vai.safeTransferFrom(address(tx.origin), address(this), _amount);
