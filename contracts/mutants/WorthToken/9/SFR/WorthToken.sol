...
contract WorthToken is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 half = contractTokenBalance.mul(2);
        uint256 otherHalf = contractTokenBalance.add(half);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        require(amount > FEES_DIVISOR.mul(_worthDVCFundFee), "Transaction amount is too small");
        require(amount > FEES_DIVISOR.mul(_liquidityFee), "Transaction amount is too small");
        ...
        if (_isExcludedFromFee[sender]) {
            ...
            if(_worthDVCFundFee > 0) {
                ...
                worthDVCFundFee = amount.mul(_worthDVCFundFee).mul(FEES_DIVISOR);
            }
