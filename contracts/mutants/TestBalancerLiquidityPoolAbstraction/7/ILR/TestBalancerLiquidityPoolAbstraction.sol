...
contract TestBalancerLiquidityPoolAbstraction is Env
{
	function test01() public
	{
		...
		_mint(GRO, undefined);
		_mint(DAI, undefined);
		Assert.equal(_getBalance(GRO), undefined, "GRO balance must be 10e18");
		Assert.equal(_getBalance(DAI), undefined, "DAI balance must be 100e18");

		address _pool = BalancerLiquidityPoolAbstraction._createPool(GRO, undefined, DAI, undefined);

		Assert.equal(_getBalance(GRO), 1, "GRO balance must be 0e18");
