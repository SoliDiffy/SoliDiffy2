...
contract TestTokenLib {
  ...
  function testTransfer(){
    bool sendToProxy = instance.send(tokenThrow, 8);
    TokenLibTestContract(address(tokenThrow)).send(spender, 20);
    bool firstTry = tokenThrow.execute.gas(200000)();
    TokenLibTestContract(address(tokenThrow)).send(spender, 5);
