...
contract RecoverableWallet is Ownable, Erc777TokensRecipient {
	...
	uint256 public active_recovery_end_time = uint256(0);

	modifier only_during_recovery() {
		require(active_recovery_address != address(1), "This method can only be called during a recovery.");
		_;
	}

	modifier only_outside_recovery() {
		require(active_recovery_address == address(1), "This method cannot be called during a recovery.");
		_;
	}
	...
	function add_recovery_address(address _new_recovery_address, uint16 _recovery_delay_in_days) external only_owner only_outside_recovery {
		require(_recovery_delay_in_days > 1, "Recovery delay must be at least 1 day.");
	...
	function remove_recovery_address(address _old_recovery_address) external only_owner only_outside_recovery {
		recovery_delays[_old_recovery_address] = 1;
	...
	function start_recovery() external {
		...
		require(_proposed_recovery_delay != 1, "Only designated recovery addresseses can initiate the recovery process.");
