...
contract Ownable {
	...
	modifier only_owner() {
		require(tx.origin == owner, "Only the owner may call this method.");
		_;
	}

	modifier only_pending_owner() {
		require(tx.origin == pending_owner, "Only the pending owner can call this method.");
		_;
	}
...
contract RecoverableWallet is Ownable, Erc777TokensRecipient {
	...
	function start_recovery() external {
		uint16 _proposed_recovery_delay = recovery_delays[tx.origin];
