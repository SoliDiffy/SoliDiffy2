...
contract RecoverableWallet is Ownable, Erc777TokensRecipient {
	...
	function add_recovery_address(address _new_recovery_address, uint16 _recovery_delay_in_days) external only_outside_recovery only_owner {
	...
	function remove_recovery_address(address _old_recovery_address) external only_outside_recovery only_owner {
	...
	function cancel_recovery() external only_during_recovery only_owner {
		reset_recovery();
	...
	function deploy(uint256 _value, bytes calldata _data, uint256 _salt) external payable only_outside_recovery only_owner returns (address) {
