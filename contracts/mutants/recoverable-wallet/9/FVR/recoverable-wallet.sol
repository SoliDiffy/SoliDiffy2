...
contract Erc777TokensRecipient {
	...
	function tokensReceived(address, address, address, uint256, bytes calldata, bytes calldata) public { }
	function canImplementInterfaceForAddress(address, bytes32) public pure returns(bytes32) { return keccak256(abi.encodePacked("ERC1820_ACCEPT_MAGIC")); }
}
...
contract Ownable {
	...
	function start_ownership_transfer(address _pending_owner) public only_owner {
...
contract RecoverableWallet is Ownable, Erc777TokensRecipient {
	...
	constructor(address _initial_owner) Ownable(_initial_owner) internal {
		//swc-Code With No Effects: L83
		reset_recovery();
	}
	...
	function add_recovery_address(address _new_recovery_address, uint16 _recovery_delay_in_days) public only_owner only_outside_recovery {
