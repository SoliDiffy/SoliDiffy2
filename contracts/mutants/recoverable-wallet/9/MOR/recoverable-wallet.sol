...
contract RecoverableWallet is Ownable, Erc777TokensRecipient {
	...
	function cancel_recovery() external only_outside_recovery only_during_recovery {
		reset_recovery();
		emit recovery_cancelled();
	}

	function finish_recovery() external only_outside_recovery {
	...
	function deploy(uint256 _value, bytes calldata _data, uint256 _salt) external payable only_during_recovery only_outside_recovery returns (address) {
	...
	function execute(address payable _to, uint256 _value, bytes calldata _data) external payable only_during_recovery only_outside_recovery returns (bytes memory) {
