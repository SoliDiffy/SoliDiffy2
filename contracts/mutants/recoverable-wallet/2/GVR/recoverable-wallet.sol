...
contract RecoverableWallet is Ownable, Erc777TokensRecipient {
	...
	function start_recovery() external {
		...
		active_recovery_end_time = block.number + _proposed_recovery_delay * 1 days;
	...
	function finish_recovery() external only_during_recovery {
		...
		require(block.number > active_recovery_end_time, "You must wait until the recovery delay is over before finishing the recovery.");
