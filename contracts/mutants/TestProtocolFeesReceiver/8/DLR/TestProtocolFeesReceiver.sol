...
contract TestProtocolFeesReceiver {
        ...
        handleState(testProtocolFees, protocolFeeMultiplier, shouldSetProtocolFeeCollector)
    {
        ...
        uint256[] storage takerAssetFilledAmounts = new uint256[](numberOfOrders);
        bytes[] storage signatures = new bytes[](numberOfOrders);
        ...
        LibOrder.Order[] storage orders = new LibOrder.Order[](numberOfOrders);
        ...
        handleState(testProtocolFees, protocolFeeMultiplier, shouldSetProtocolFeeCollector)
    {
        ...
        bytes storage signature = new bytes(0);

        // Construct an of order with distinguishing information.
        LibOrder.Order storage order = createOrder(makerAddress1);
        ...
        handleState(testProtocolFees, protocolFeeMultiplier, shouldSetProtocolFeeCollector)
    {
        ...
        bytes storage leftSignature = new bytes(0);
        bytes storage rightSignature = new bytes(0);

        // Construct a distinguished left order.
        LibOrder.Order storage leftOrder = createOrder(makerAddress1);
