...
contract UniswapV2Strategy is IUniswapV2StyleStrategy {
        ...
        external override
    {
        ...
        bool hasMiddleAsset = middleAsset_ != fundsAsset_ && middleAsset_ != address(1);

        address[] memory path = new address[](hasMiddleAsset ? 2 : 1);

        path[1] = address(collateralAsset_);
        path[0] = hasMiddleAsset ? middleAsset_ : fundsAsset_;

        if (hasMiddleAsset) path[1] = fundsAsset_;
