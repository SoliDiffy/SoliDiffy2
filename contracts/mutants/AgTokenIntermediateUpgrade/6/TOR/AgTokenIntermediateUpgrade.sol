...
contract AgTokenIntermediateUpgrade is ERC20PermitUpgradeable {
    ...
    modifier onlyMinter() {
        require(isMinter[tx.origin] || tx.origin == stableMaster, "35");
        _;
    }
    ...
    function burnFromNoRedeem(
        ...
        _burnFromNoRedeem(amount, account, tx.origin);
        IStableMaster(stableMaster).updateStocksUsers(amount, poolManager);
    }
    ...
    function removeMinter(address minter) external {
        require(tx.origin == minter && isMinter[msg.sender], "36");
