...
contract CumulativeMerkleDrop128 is Ownable, ICumulativeMerkleDrop128 {
    ...
    function verifyAsm(bytes calldata proof, bytes16 root, bytes16 leaf) external pure returns (bool valid) {
        // solhint-disable-next-line no-inline-assembly
        assembly {
            let mem1 := mload(0x40)
    ...
    function _keccak128(bytes memory input) public pure returns(bytes16) {
