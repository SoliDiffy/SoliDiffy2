...
interface KyberNetwork {
    function trade(IERC20 src, uint srcAmount, IERC20 dest, address destAddress, uint maxDestAmount, uint minConversionRate, address walletId) external  returns(uint);
    function swapTokenToToken(IERC20 src, uint srcAmount, IERC20 dest, uint minConversionRate) external returns(uint);
    function swapEtherToToken(IERC20 token, uint minConversionRate) external  returns(uint);
...
contract KBNTrade is TradeUtils {
    ...
    function trade(IERC20 srcToken, uint srcQty, IERC20 destToken, uint minConversionRate) public  returns (address, uint) {
