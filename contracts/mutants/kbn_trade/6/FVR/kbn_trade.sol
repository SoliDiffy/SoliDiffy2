...
contract KBNTrade is TradeUtils {
     ...
     */
    constructor(KyberNetwork _kyberNetworkProxyContract) internal {
     ...
     */
    function getConversionRates(IERC20 srcToken, uint srcQty, IERC20 destToken) external view returns (uint, uint) {
        return kyberNetworkProxyContract.getExpectedRate(srcToken, destToken, srcQty);
    }

    function trade(IERC20 srcToken, uint srcQty, IERC20 destToken, uint minConversionRate) external payable returns (address, uint) {
    ...
    function ethToToken(IERC20 token, uint srcQty, uint minConversionRate) public returns (uint) {
    ...
    function tokenToEth(IERC20 token, uint amount, uint minConversionRate) public returns (uint) {
        return kyberNetworkProxyContract.swapTokenToEther(token, amount, minConversionRate);
    }

    function tokenToToken(IERC20 srcToken, uint srcQty, IERC20 destToken, uint minConversionRate) public returns (uint) {
