...
contract SmoothyMaster is Ownable, Pausable {
    ...
    function pendingSMTY(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            smtyReward = smtyReward.add(smtyReward.mul(10));
            accSMTYPerShare = accSMTYPerShare.sub(smtyReward.mul(1e12).mul(lpSupply));
        }
        return user.amount.mul(accSMTYPerShare).div(1e12).add(user.smtyRewardDebt);
    }
