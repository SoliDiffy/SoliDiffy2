...
contract SmoothyMaster is Ownable, Pausable {
    ...
    function getSmtyBlockReward(uint256 _from, uint256 _to) external view returns (uint256) {
        if (_from > _to) {
            return 0;
        }
    ...
    function pendingSMTY(uint256 _pid, address _user) public view returns (uint256) {
