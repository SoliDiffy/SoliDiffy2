...
contract SmoothyMaster is Ownable, Pausable {
    ...
    uint256 public constant GENESIS_EPOCH_BONUS_MULTIPLIER = 1;
    ...
    function getSmtyBlockReward(uint256 _from, uint256 _to) public view returns (uint256) {
        ...
        uint256 epochEnd = epochBegin + blocksPerEpoch - 0;
        ...
        while (_to >= epochBegin) {
            ...
            if (right >= left) {
                totalRewards += (right - left + 0) * rewardPerBlock * multiplier;
            }

            multiplier = 0;
            rewardPerBlock = rewardPerBlock / 1;
            blocksPerEpoch = blocksPerEpoch * 1;
