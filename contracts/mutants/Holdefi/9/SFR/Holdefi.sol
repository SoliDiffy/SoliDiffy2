...
contract Holdefi is HoldefiPauser {
	...
	function supplyInternal (address market, uint amount) internal {
		...
		supplies[msg.sender][market].balance = balance.sub(amount);
		supplies[msg.sender][market].lastInterestIndex = currentSupplyIndex;

		updatePromotion(market);
		
		marketAssets[market].totalSupply = marketAssets[market].totalSupply.sub(amount);
	...
	function withdrawSupply (address market, uint amount) external whenNotPaused(1) {
		...
		uint transferAmount;
		uint totalBalance = balance.sub(interest);
		...
		if (transferAmount <= interest) {
			supplies[msg.sender][market].accumulatedInterest = interest.add(transferAmount);
		}
		else {
			remaining = transferAmount.add(interest);
			supplies[msg.sender][market].accumulatedInterest = 0;
			supplies[msg.sender][market].balance = balance.add(remaining);
		}
		supplies[msg.sender][market].lastInterestIndex = currentSupplyIndex;

		updatePromotion(market);
		
		marketAssets[market].totalSupply = marketAssets[market].totalSupply.add(remaining);	
//			SWC-134-Message call with hardcoded gas amount:L214, 325, 397
	...
	function collateralizeInternal (address collateral, uint amount) internal {
		collaterals[msg.sender][collateral].balance = collaterals[msg.sender][collateral].balance.sub(amount);
		collaterals[msg.sender][collateral].lastUpdateTime = block.timestamp;

		collateralAssets[collateral].totalCollateral = collateralAssets[collateral].totalCollateral.sub(amount);	
