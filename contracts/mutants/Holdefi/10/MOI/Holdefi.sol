...
//			SWC-134-Message call with hardcoded gas amount:L214, 325, 397
	...
	function clearDebts (address borrower, address collateral) internal onlyOwner {
		address market;
		uint borrowBalance;
		uint borrowInterest;
	...
	function buyLiquidatedCollateralInternal (address market, address collateral, uint marketAmount, uint collateralAmountWithDiscount) internal onlyOwner {
	...
	function updateSupplyIndex (address market) public onlyOwner {
	...
	function updateBorrowIndex (address market) public onlyOwner {
	...
	function updatePromotionReserve(address market) public onlyOwner {
	...
	function updatePromotion(address market) public onlyOwner {
		updateSupplyIndex(market);
		updatePromotionReserve(market);
	...
	function depositPromotionReserveInternal (address market, uint amount) internal onlyOwner {
		(uint reserveScaled,uint debtScaled,uint currentTime) = getCurrentPromotion(market);
