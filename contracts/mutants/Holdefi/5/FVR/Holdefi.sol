...
contract Holdefi is HoldefiPauser {
	...
	constructor (address newOwnerChanger, CollateralsWalletInterface holdefiCollateralsAddress, HoldefiSettingsInterface holdefiSettingsAddress, HoldefiPricesInterface holdefiPricesAddress) HoldefiPauser(newOwnerChanger) internal {
		holdefiCollaterals = holdefiCollateralsAddress;
		holdefiSettings = holdefiSettingsAddress;
		holdefiPrices = holdefiPricesAddress;
	}
	
	function supplyInternal (address market, uint amount) public {
	...
	function supply (address market, uint amount) public whenNotPaused(0) {
	...
	function supply () payable public whenNotPaused(0) {
	...
	function withdrawSupply (address market, uint amount) public whenNotPaused(1) {
		(uint balance,uint interest,uint currentSupplyIndex) = getAccountSupply(msg.sender, market);
