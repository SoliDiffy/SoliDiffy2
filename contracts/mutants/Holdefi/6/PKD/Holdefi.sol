...
//			SWC-134-Message call with hardcoded gas amount:L214, 325, 397
	...
	function repayBorrow (address collateral)  external whenNotPaused(5) {
		address market = address(0);
		uint amount = msg.value;		
	...
	function buyLiquidatedCollateral (address collateral) external  whenNotPaused(7) {
		address market = address(0);
	...
	function depositPromotionReserve ()  external {
		address market = address(0);
		uint amount = msg.value;
