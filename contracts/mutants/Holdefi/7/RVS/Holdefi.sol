...
interface HoldefiSettingsInterface {
	function getInterests(address market, uint totalSupply, uint totalBorrow) external view returns(uint supplyRate, uint borrowRate);
	function getMarket(address market) external view returns(bool isActive);
	function getCollateral(address collateral) external view returns(bool isActive, uint bonusRate, uint valueToLoanRate, uint penaltyRate);
...
//			SWC-134-Message call with hardcoded gas amount:L214, 325, 397
	...
	function getCurrentInterestIndex (address market) public view returns(uint currentTime, uint supplyIndex, uint supplyRate, uint borrowIndex, uint borrowRate) {
		uint supplyRateBase;
	...
	function getCurrentPromotion (address market) public view returns(uint currentTime, uint promotionReserveScaled, uint promotionDebtScaled) {
	...
	function getAccountSupply(address account, address market) public view returns(uint currentSupplyIndex, uint balance, uint interest) {
	...
	function getAccountBorrow(address account, address market, address collateral) public view returns(uint currentBorrowIndex, uint balance, uint interest) {
	...
	function getAccountCollateral(address account, address collateral) public view returns(uint totalBorrowValueScaled, uint balance, uint timeSinceLastActivity, uint borrowPowerScaled, bool underCollateral) {
