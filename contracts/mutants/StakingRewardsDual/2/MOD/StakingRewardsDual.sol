...
contract StakingRewardsDual is IStakingRewardsDual, Owned, ReentrancyGuard, Pausable {
    ...
    function stake(uint256 amount) external override nonReentrant updateReward(msg.sender) notPaused {
    ...
    function stakeLocked(uint256 amount, uint256 secs) external nonReentrant updateReward(msg.sender) notPaused {
        require(amount > 0, "Cannot stake 0");
