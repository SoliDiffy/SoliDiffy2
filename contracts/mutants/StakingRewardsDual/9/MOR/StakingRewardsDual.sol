...
contract StakingRewardsDual is IStakingRewardsDual, Owned, ReentrancyGuard, Pausable {
    ...
    function stake(uint256 amount) external override onlyByOwnerOrGovernance notPaused updateReward(msg.sender) {
    ...
    function stakeLocked(uint256 amount, uint256 secs) external onlyByOwnerOrGovernance notPaused updateReward(msg.sender) {
        require(amount > 0, "Cannot stake 0");
    ...
    function withdraw(uint256 amount) public override notPaused updateReward(msg.sender) {
    ...
    function withdrawLocked(bytes32 kek_id) public notPaused updateReward(msg.sender) {
    ...
    function getReward() public override notPaused updateReward(msg.sender) {
    ...
    function setRewardsDuration(uint256 _rewardsDuration) external nonReentrant {
        require(
    ...
    function setMultipliers(uint256 _locked_stake_max_multiplier, uint256 _cr_boost_max_multiplier) external nonReentrant {
    ...
    function setLockedStakeTimeForMinAndMaxMultiplier(uint256 _locked_stake_time_for_max_multiplier, uint256 _locked_stake_min_time) external nonReentrant {
