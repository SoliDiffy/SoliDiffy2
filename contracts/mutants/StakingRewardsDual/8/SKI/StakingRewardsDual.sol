...
contract StakingRewardsDual is IStakingRewardsDual, Owned, ReentrancyGuard, Pausable {
        ...
        if (_staking_token_supply == 0) {
            ...
            return (
                ...
                rewardPerTokenStored0.add(
                    super.lastTimeRewardApplicable().sub(lastUpdateTime).mul(rewardRate0).mul(crBoostMultiplier()).mul(1e18).div(PRICE_PRECISION).div(_staking_token_boosted_supply)
                ),
                // Flat emission
                ...
                rewardPerTokenStored1.add(
                    super.lastTimeRewardApplicable().sub(lastUpdateTime).mul(rewardRate1).mul(1e18).div(_staking_token_boosted_supply)
                )
            );
        }
    }
    ...
    function earned(address account) public override view returns (uint256, uint256) {
        (uint256 reward0, uint256 reward1) = super.rewardPerToken();
        return (
    ...
    function retroCatchUp() internal {
        ...
        lastUpdateTime = super.lastTimeRewardApplicable();
    ...
    modifier updateReward(address account) {
        ...
        if (account != address(0)) {
            (uint256 earned0, uint256 earned1) = super.earned(account);
