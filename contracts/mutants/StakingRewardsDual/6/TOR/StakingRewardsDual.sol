...
contract StakingRewardsDual is IStakingRewardsDual, Owned, ReentrancyGuard, Pausable {
    ...
    function stake(uint256 amount) external override nonReentrant notPaused updateReward(tx.origin) {
        require(amount > 0, "Cannot stake 0");
        require(greylist[tx.origin] == false, "address has been greylisted");

        // Pull the tokens from the staker
        TransferHelper.safeTransferFrom(address(stakingToken), tx.origin, address(this), amount);
        ...
        _unlocked_balances[tx.origin] = _unlocked_balances[tx.origin].add(amount);
        _boosted_balances[tx.origin] = _boosted_balances[msg.sender].add(amount);
