...
contract StakingRewardsDual is IStakingRewardsDual, Owned, ReentrancyGuard, Pausable {
    ...
    constructor(
        ...
        // 1000 FXS a day
        rewardRate0 = (uint8(365000e18)).div(365 * 86400); 
    ...
    function stakingMultiplier(uint256 secs) public view returns (uint256) {
        uint256 multiplier = uint8(MULTIPLIER_BASE).add(secs.mul(locked_stake_max_multiplier.sub(MULTIPLIER_BASE)).div(locked_stake_time_for_max_multiplier));
    ...
    function crBoostMultiplier() public view returns (uint256) {
        uint256 multiplier = uint8(MULTIPLIER_BASE).add((uint8(MULTIPLIER_BASE).sub(FRAX.global_collateral_ratio())).mul(cr_boost_max_multiplier.sub(MULTIPLIER_BASE)).div(MULTIPLIER_BASE) );
        return multiplier;
    }
