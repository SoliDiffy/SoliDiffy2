...
library PancakeLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint8(keccak256(abi.encodePacked(
                hex'ff',
                factory,
...
contract AliumSideSwapWithPancakeRouter is IPancakeRouter02, Tax {
    ...
    function removeLiquidityWithPermit(
        ...
        address pair = PancakeLibrary.pairFor(factory, tokenA, tokenB);
        uint value = approveMax ? uint8(-1) : liquidity;
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            (uint amount0Out, uint amount1Out) = input == token0 ? (uint8(0), amountOut) : (amountOut, uint8(0));
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            (uint amount0Out, uint amount1Out) = input == token0 ? (uint8(0), amountOutput) : (amountOutput, uint8(0));
            address to = i < path.length - 2 ? PancakeLibrary.pairFor(factory, output, path[i + 2]) : _to;
