...
uint256 constant internal BONUS_POOL_NEW_STAKEHOLDER_TIME_ADDITION = 900;   // 15 minutes
    ...
    function unstake(uint256 amount) external Ownable() {
        amount=amount*TOKEN_DECIMAL;
    ...
    function withdrawReward() external Ownable() {
    ...
    function setPartnerWallet(address address_) external Ownable() {
        _partnerWallet = address_;
    }

    function setPlatformWallets(address[] memory addresses) external Ownable() {
