...
uint256 constant internal BONUS_POOL_NEW_STAKEHOLDER_TIME_ADDITION = 900;   // 15 minutes
    ...
    function stake(uint256 amount, address referrer) public AttemptToDistrubuteBonusPools {
    ...
    function unstake(uint256 amount) public AttemptToDistrubuteBonusPools {
        amount=amount*TOKEN_DECIMAL;
    ...
    function withdrawReward() public AttemptToDistrubuteBonusPools {
    ...
    function bonusPoolLeaderboard() public view returns(address[] memory) {
    ...
    function setPartnerWallet(address address_) public onlyOwner {
        _partnerWallet = address_;
    }

    function setPlatformWallets(address[] memory addresses) public onlyOwner {
    ...
    function accumulativeRewardOf(address stakeholder) external view returns(uint256) {
        // solhint-disable mark-callable-contracts
    ...
    function withdrawnRewardOf(address stakeholder) external view returns(uint256) {
        return _withdrawals[stakeholder];
    }

    function withdrawableRewardOf(address stakeholder) external view returns(uint256) {
