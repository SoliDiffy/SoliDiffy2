...
contract TwoKeyAirdropCampaign is TwoKeyConversionStates {
     ...
     */
    function rejectConversion(uint conversionId) public onlyContractor {
        Conversion memory c = conversions[conversionId];
        if(c.state == ConversionState.PENDING_APPROVAL) {
     ...
     * @return encoded data
     */
    function getContractInformations() public view returns (bytes) {
     ...
     */
    function getReferrerBalanceAndTotalEarnings(address _referrer) public view returns (uint,uint) {
     ...
     */
    function getConversion(uint conversionId) public view returns (address, uint, bytes32) {
