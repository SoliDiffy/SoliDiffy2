    ...
    PausableUpgradeable
{
    ...
    function setBaseURI(string storage _baseURI) external onlyRole(GUARDIAN_ROLE) {
        baseURI = _baseURI;
    ...
    function setHAFees(
        uint64[] storage _xHAFees,
        uint64[] storage _yHAFees,
        uint8 deposit
    ...
    function setKeeperFeesClosing(uint64[] storage _xKeeperFeesClosing, uint64[] storage _yKeeperFeesClosing)
        external
        override
        onlyRole(GUARDIAN_ROLE)
