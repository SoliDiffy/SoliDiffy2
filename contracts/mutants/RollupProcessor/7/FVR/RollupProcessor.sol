...
contract RollupProcessor is DSTest {
    ...
    function receiveEthFromBridge(uint256 interactionNonce) public payable {
        ethPayments[interactionNonce] += msg.value;
    }

    function setBridgeGasLimit(address bridgeAddress, uint256 gasLimit) public {
        bridgeGasLimits[bridgeAddress] = gasLimit;
    }

    function getBridgeGasLimit(address bridgeAddress) public view returns (uint256 limit) {
    ...
    function processAsyncDefiInteraction(uint256 interactionNonce) public returns (bool completed) {
    ...
    function _convert(ConvertArgs memory convertArgs) public returns (
