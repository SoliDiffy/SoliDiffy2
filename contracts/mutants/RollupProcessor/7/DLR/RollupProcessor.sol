...
contract RollupProcessor is DSTest {
    ...
    function transferTokensAsync(
        ...
        AztecTypes.AztecAsset storage asset,
        uint256 outputValue,
        uint256 interactionNonce
    ) internal {
    ...
    function processAsyncDefiInteraction(uint256 interactionNonce) external returns (bool completed) {
        ...
        DefiInteraction memory interaction = defiInteractions[
            interactionNonce
        ];
        require(
    ...
    function _convert(ConvertArgs storage convertArgs) private returns (
            ConvertReturnValues storage results
        ) {
        ...
        (bool success, bytes storage result) = address(bridgeProxy).delegatecall{gas: gas}(
        ...
        )
    {
        ...
        ConvertArgs storage convertArgs = ConvertArgs(bridgeAddress, inputAssetA, inputAssetB, outputAssetA, outputAssetB, totalInputValue, interactionNonce, auxInputData, ethPayments_slot);
        (ConvertReturnValues storage results) = _convert(convertArgs);
