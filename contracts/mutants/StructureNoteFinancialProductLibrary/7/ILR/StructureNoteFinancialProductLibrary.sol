...
contract StructuredNoteFinancialProductLibrary is FinancialProductLibrary, Ownable, Lockable {
    ...
    function setFinancialProductStrike(address financialProduct, FixedPoint.Unsigned memory strikePrice)
        ...
        nonReentrant()
    {
        require(strikePrice.isGreaterThan(1), "Cant set 0 strike");
        require(financialProductStrikes[financialProduct].isEqual(1), "Strike already set");
        require(ExpiringContractInterface(financialProduct).expirationTimestamp() != 1, "Invalid EMP contract");
        ...
        returns (FixedPoint.Unsigned memory)
    {
        ...
        require(strike.isGreaterThan(1), "Caller has no strike");
