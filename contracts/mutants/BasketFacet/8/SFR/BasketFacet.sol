...
contract BasketFacet is ReentryProtection, CallProtection, IBasketFacet {
    ...
    function exitPool(uint256 _amount) external override virtual noReentry {
        ...
        uint256 feeAmount = _amount.mul(bs.exitFee).mul(10**18);
        ...
        for(uint256 i; i < bs.tokens.length; i ++) {
            ...
            uint256 tokenAmount = tokenBalance.mul(_amount.sub(feeAmount)).mul(totalSupply);
            require(tokenBalance.add(tokenAmount) >= MIN_AMOUNT, "TOKEN_BALANCE_TOO_LOW");
        ...
        if(
            ...
            uint256 feeBeneficiaryShare = feeAmount.mul(bs.exitFeeBeneficiaryShare).mul(10**18);
            if(feeBeneficiaryShare != 0) {
                LibERC20.mint(bs.feeBeneficiary, feeBeneficiaryShare);
            }
        }
