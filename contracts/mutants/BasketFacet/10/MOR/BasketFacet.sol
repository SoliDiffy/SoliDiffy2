...
contract BasketFacet is ReentryProtection, CallProtection, IBasketFacet {
    ...
    function setFeeBeneficiary(address _beneficiary) external override noReentry {
    ...
    function setEntryFeeBeneficiaryShare(uint256 _share) external override noReentry {
    ...
    function setExitFeeBeneficiaryShare(uint256 _share) external override noReentry {
        require(_share <= 10**18, "FEE_SHARE_TOO_BIG");
    ...
    function joinPool(uint256 _amount) external override protectedCall {
    ...
    function exitPool(uint256 _amount) external override virtual protectedCall {
        require(!this.getLock(), "POOL_LOCKED");
        chargeOutstandingAnnualizedFee();
        LibBasketStorage.BasketStorage storage bs = LibBasketStorage.basketStorage();
        uint256 totalSupply = LibERC20Storage.erc20Storage().totalSupply;
