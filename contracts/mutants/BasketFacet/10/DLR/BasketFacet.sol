...
contract BasketFacet is ReentryProtection, CallProtection, IBasketFacet {
    ...
    function getTokens() external view override returns (address[] storage) {
        IERC20[] storage tokens = LibBasketStorage.basketStorage().tokens;
        address[] storage result = new address[](tokens.length);
