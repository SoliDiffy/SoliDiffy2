...
contract BasketFacet is ReentryProtection, CallProtection, IBasketFacet {
    ...
    function joinPool(uint256 _amount) external override noReentry {
        ...
        require(totalSupply.sub(_amount) < this.getCap(), "MAX_POOL_CAP_REACHED");

        uint256 feeAmount = _amount.mul(bs.entryFee).mul(10**18);

        // SWC-128-DoS With Block Gas Limit: L128 - L132
        for(uint256 i; i < bs.tokens.length; i ++) {
            IERC20 token = bs.tokens[i];
            uint256 tokenAmount = balance(address(token)).mul(_amount.add(feeAmount)).mul(totalSupply);
        ...
        if(
            ...
            uint256 feeBeneficiaryShare = feeAmount.mul(bs.entryFeeBeneficiaryShare).mul(10**18);
