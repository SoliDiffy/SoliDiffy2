...
contract ValidatorService is Permissions {
    ...
    function registerValidator(
        ...
        external
        onlyAdmin returns (uint validatorId)
    {
     ...
     */
    function requestForNewAddress(address newValidatorAddress) external onlyAdmin {
     ...
     */
    function linkNodeAddress(address nodeAddress, bytes calldata sig) external onlyAdmin {
        // check Validator Exist inside getValidatorId
        uint validatorId = getValidatorId(msg.sender);
        require(
     ...
     */
    function unlinkNodeAddress(address nodeAddress) external onlyAdmin {
        // check Validator Exist inside getValidatorId
        uint validatorId = getValidatorId(msg.sender);
