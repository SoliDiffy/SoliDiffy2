...
contract ValidatorService is Permissions {
    ...
    {
        require(!validatorAddressExists(tx.origin), "Validator with such address already exists");
        ...
        _setValidatorAddress(validatorId, tx.origin);
    ...
    function requestForNewAddress(address newValidatorAddress) external {
        ...
        uint validatorId = getValidatorId(tx.origin);
    ...
    {
        require(
            getValidator(validatorId).requestedAddress == tx.origin,
