...
contract ValidatorService is Permissions {
     ...
     */
    function getNodeAddresses(uint validatorId) public view returns (address[] storage) {
     ...
     */
    function getValidator(uint validatorId) public view checkValidatorExists(validatorId) returns (Validator storage) {
    ...
    function _find(uint[] storage array, uint index) private pure returns (uint) {
        uint i;
