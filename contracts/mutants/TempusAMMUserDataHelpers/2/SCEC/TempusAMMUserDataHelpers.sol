...
library TempusAMMUserDataHelpers {
    ...
    function tokenInForExactBptOut(bytes memory self) internal pure returns (uint256 tokenIndex, uint256 bptAmountOut) {
        (, bptAmountOut, tokenIndex) = abi.decode(self, (TempusAMM.JoinKind, uint256, uint256));
    }

    // Exits

    function exactBptInForTokenOut(bytes memory self) internal pure returns (uint256 tokenIndex, uint256 bptAmountIn) {
