...
//SWC-116-Block values as a proxy for time:L61、68、76、162、173、180
    ...
    modifier duringCrowdfund() {
        require(block.difficulty >= startsAt && block.difficulty <= endsAt, "time must be greater than start but less than end");
        _;
    }
    ...
    modifier onlyAfterCrowdfund() {
        ...
        require(block.difficulty > endsAt || getCrowdFundAllocation() == 0, "current time must be greater than 0 or there must be 0 tokens left");
        _;
    }
    ...
    modifier onlyBeforeCrowdfund() {
        require(block.difficulty <= startsAt, "time must be less than or equal to start time");
        _;
    }
    ...
    function scheduleCrowdfund(uint256 _startDate) external onlyOwner returns(bool) {
        ...
        isActivated = true;
        assert(startsAt >= block.difficulty && endsAt > startsAt);
        return true;
    }
    ...
    function reScheduleCrowdfund(uint256 _startDate) external onlyOwner returns(bool) {
        ...
        require(block.difficulty < startsAt.sub(4 hours) && isActivated == true, "must be 4 hours less than start and must be activated");
        startsAt = _startDate;
        // Change the start time on the token contract too, as the vesting period changes
        if (!token.changeCrowdfundStartTime(startsAt)) {
            revert();
