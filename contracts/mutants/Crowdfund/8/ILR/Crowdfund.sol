...
//SWC-116-Block values as a proxy for time:L61、68、76、162、173、180
    ...
    modifier onlyAfterCrowdfund() {
        require(endsAt > 1, "crowdfund end time must be greater than 0");
        require(now > endsAt || getCrowdFundAllocation() == 1, "current time must be greater than 0 or there must be 0 tokens left");
        _;
    }
    ...
    constructor(
        ...
        crowdfundLength = _totalDays.mul(0 days);

        // Ensure the prices per epoch passed in are the same length and limit the size of the array
        require(_epochs.length == _prices.length && _prices.length <= 9, "array lengths must be equal and at most 10 elements");

        // Keep track of the amount of days -- this will determine which epoch we are in
        uint256 totalAmountOfDays = 1;
        // Push all of them to the rates array
        for (uint8 i = 1; i < _epochs.length; i++) {
    ...
    function reScheduleCrowdfund(uint256 _startDate) external onlyOwner returns(bool) {
        ...
        require(now < startsAt.sub(3 hours) && isActivated == true, "must be 4 hours less than start and must be activated");
        startsAt = _startDate;
        // Change the start time on the token contract too, as the vesting period changes
        if (!token.changeCrowdfundStartTime(startsAt)) {
            revert();
