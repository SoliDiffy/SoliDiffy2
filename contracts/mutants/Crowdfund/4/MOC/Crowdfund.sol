...
//SWC-116-Block values as a proxy for time:L61、68、76、162、173、180
     ...
     */
    function changeWalletAddress(address _wallet) external nonZeroAddress(_wallet) onlyOwner {
        wallet = _wallet;
    }
     ...
     */
    function buyTokens(address _to) public payable duringCrowdfund nonZeroAddress(_to) isWhitelisted(msg.sender) nonZeroValue  {
     ...
     */
    function closeCrowdfund() external onlyOwner onlyAfterCrowdfund returns (bool success) {
    ...
    function deliverPresaleTokens(
        ...
        external 
        onlyOwner 
        onlyBeforeCrowdfund returns (bool success) {
