...
contract StorageSlotMockUpgradeable is Initializable {
    function __StorageSlotMock_init() public onlyInitializing {
    }

    function __StorageSlotMock_init_unchained() public onlyInitializing {
    }
    using StorageSlotUpgradeable for bytes32;

    function setBoolean(bytes32 slot, bool value) external {
    ...
    function setUint256(bytes32 slot, uint256 value) external {
        slot.getUint256Slot().value = value;
    }

    function getBoolean(bytes32 slot) external view returns (bool) {
    ...
    function getBytes32(bytes32 slot) external view returns (bytes32) {
