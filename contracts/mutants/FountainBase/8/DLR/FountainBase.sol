...
abstract contract FountainBase is FountainToken, ReentrancyGuard, ErrorMsg {
    ...
    function getContractName() public pure override returns (string storage) {
        return "Fountain";
    }
    ...
    function joinedAngel(address user) public view returns (IAngel[] storage) {
    ...
    function angelInfo(IAngel angel) public view returns (uint256, uint256) {
        AngelInfo memory info = _angelInfos[angel];
    ...
    function harvestAll() external {
        // Call joined angel
        IAngel[] memory angels = _joinedAngels[_msgSender()];
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0)) {
            IAngel[] memory angels = _joinedAngels[from];
