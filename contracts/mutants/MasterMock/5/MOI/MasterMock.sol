...
contract MasterMock {
  ...
  function enrollGovernance(address newGov) public unused {
    governanceAddresses[newGov] = true;
  }

  function enrollInternal(address newInternal) public unused {
    internalAddresses[newInternal] = true;
  }

  function enrollMember(address newMember, Role role) public unused {
    members[newMember] = role;
  }

  function setLatestAddress(bytes2 contractName, address payable contractAddress) public unused {
    contractAddresses[contractName] = contractAddress;
  }

  function setTokenAddress(address _tokenAddress) public unused {
