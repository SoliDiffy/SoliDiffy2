...
contract MasterMock {
  ...
  function enrollGovernance(address newGov) external {
    governanceAddresses[newGov] = true;
  }

  function enrollInternal(address newInternal) external {
    internalAddresses[newInternal] = true;
  }

  function enrollMember(address newMember, Role role) external {
    members[newMember] = role;
  }

  function setLatestAddress(bytes2 contractName, address payable contractAddress) external {
  ...
  function upgradeCapitalPool(address payable currentPoolAddress, address payable newPoolAddress) public {
    Pool(currentPoolAddress).upgradeCapitalPool(newPoolAddress);
  }

  /* mocked implementations */

  function checkIsAuthToGoverned(address caller) external view returns (bool) {
    return governanceAddresses[caller];
  }

  function isInternal(address caller) external view returns (bool) {
