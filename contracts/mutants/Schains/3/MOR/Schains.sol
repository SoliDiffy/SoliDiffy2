...
contract Schains is Permissions {
     ...
     */
    function addSchain(address from, uint deposit, bytes calldata data) external initializer {
     ...
     */
    function deleteSchain(address from, string calldata name) external initializer {
     ...
     * - Schain exists.
     */
    function deleteSchainByRoot(string calldata name) external initializer {
        NodeRotation nodeRotation = NodeRotation(contractManager.getContract("NodeRotation"));
        bytes32 schainId = keccak256(abi.encodePacked(name));
