...
contract Schains is Permissions {
     ...
     */
    function addSchain(address from, uint deposit, bytes calldata data) public allow("SkaleManager") {
     ...
     */
    function deleteSchain(address from, string calldata name) public allow("SkaleManager") {
     ...
     * - Schain exists.
     */
    function deleteSchainByRoot(string calldata name) public allow("SkaleManager") {
        NodeRotation nodeRotation = NodeRotation(contractManager.getContract("NodeRotation"));
        bytes32 schainId = keccak256(abi.encodePacked(name));
     ...
     */
    function restartSchainCreation(string calldata name) public allow("SkaleManager") {
        NodeRotation nodeRotation = NodeRotation(contractManager.getContract("NodeRotation"));
        bytes32 schainId = keccak256(abi.encodePacked(name));
     ...
     */
    function addSpace(uint nodeIndex, uint8 partOfNode) public allowTwo("Schains", "NodeRotation") {
