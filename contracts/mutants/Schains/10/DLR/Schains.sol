...
contract Schains is Permissions {
    ...
    function _fallbackSchainParametersDataConverter(bytes storage data)
        private
        pure
        returns (SchainParameters storage schainParameters)
    {
    ...
    {
        SchainsInternal schainsInternal = SchainsInternal(contractManager.getContract("SchainsInternal"));
        uint[] storage nodesInGroup = schainsInternal.createGroupForSchain(schainId, numberOfNodes, partOfNode);
