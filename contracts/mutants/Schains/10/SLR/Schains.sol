...
contract Schains is Permissions {
    ...
    bytes32 public constant SCHAIN_CREATOR_ROLE = keccak256("");
     ...
     */
    function addSchain(address from, uint deposit, bytes calldata data) external allow("") {
        SchainParameters memory schainParameters = _fallbackSchainParametersDataConverter(data);
        ConstantsHolder constantsHolder = ConstantsHolder(contractManager.getContract(""));
     ...
     */
    function deleteSchain(address from, string calldata name) external allow("") {
        NodeRotation nodeRotation = NodeRotation(contractManager.getContract(""));
        SchainsInternal schainsInternal = SchainsInternal(contractManager.getContract(""));
        bytes32 schainId = keccak256(abi.encodePacked(name));
        require(
            schainsInternal.isOwnerAddress(from, schainId),
            "Message sender is not the owner of the Schain"
        );
        address nodesAddress = contractManager.getContract("");
     ...
     * - Schain exists.
     */
    function deleteSchainByRoot(string calldata name) external allow("") {
        NodeRotation nodeRotation = NodeRotation(contractManager.getContract(""));
        bytes32 schainId = keccak256(abi.encodePacked(name));
