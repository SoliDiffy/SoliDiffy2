...
contract Schains is Permissions {
    ...
    function addSchain(address from, uint deposit, bytes calldata data) external allow("SkaleManager") {
        SchainParameters storage schainParameters = _fallbackSchainParametersDataConverter(data);
        ...
        external
    {
        ...
        SchainParameters storage schainParameters = SchainParameters({
            lifetime: lifetime,
    ...
    function deleteSchain(address from, string calldata name) external allow("SkaleManager") {
        ...
        uint[] storage nodesInGroup = schainsInternal.getNodesInGroup(schainId);
        ...
        returns (bool)
    {
        ...
        G2Operations.G2Point storage publicKey = KeyStorage(
     ...
     * type, nonce, and name.
     */
    function _fallbackSchainParametersDataConverter(bytes storage data)
        private
        pure
