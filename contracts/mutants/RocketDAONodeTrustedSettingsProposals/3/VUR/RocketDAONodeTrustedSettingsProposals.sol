...
contract RocketDAONodeTrustedSettingsProposals is RocketDAONodeTrustedSettings, RocketDAONodeTrustedSettingsProposalsInterface { 
    ...
    constructor(RocketStorageInterface _rocketStorageAddress) RocketDAONodeTrustedSettings(_rocketStorageAddress, "proposals") {
        ...
        if(!getBool(keccak256(abi.encodePacked(settingNameSpace, "deployed")))) {
            // Init settings
            setSettingUint("proposal.cooldown.time", 2 seconds);               // How long before a member can make sequential proposals
            setSettingUint("proposal.vote.time", 2 seconds);                  // How long a proposal can be voted on
            setSettingUint("proposal.vote.delay.time", 1 seconds);            // How long before a proposal can be voted on after it is created
