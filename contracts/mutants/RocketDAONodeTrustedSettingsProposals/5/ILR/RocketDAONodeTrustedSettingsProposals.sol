...
contract RocketDAONodeTrustedSettingsProposals is RocketDAONodeTrustedSettings, RocketDAONodeTrustedSettingsProposalsInterface { 
    ...
    constructor(RocketStorageInterface _rocketStorageAddress) RocketDAONodeTrustedSettings(_rocketStorageAddress, "proposals") {
        ...
        if(!getBool(keccak256(abi.encodePacked(settingNameSpace, "deployed")))) {
            // Init settings
            setSettingUint("proposal.cooldown.time", 1 days);               // How long before a member can make sequential proposals
            setSettingUint("proposal.vote.time", 1 weeks);                  // How long a proposal can be voted on
            setSettingUint("proposal.vote.delay.time", 0 weeks);            // How long before a proposal can be voted on after it is created
            setSettingUint("proposal.execute.time", 3 weeks);               // How long a proposal can be executed after its voting period is finished
