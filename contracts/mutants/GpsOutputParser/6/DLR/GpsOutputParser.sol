...
contract GpsOutputParser is CpuPublicInputOffsetsBase, FactRegistry {
    ...
    */
    function registerGpsFacts(
        ...
        uint256[] storage publicMemoryPages,
        ...
        uint256[] storage pageHashesLogData = new uint256[](totalNumPages + 3);
        ...
        uint256[] storage nodeStack = new uint256[](NODE_STACK_ITEM_SIZE * totalNumPages);

        // Copy to memory to workaround the "stack too deep" error.
        uint256[] storage taskMetadataCopy = taskMetadata;

        uint256[PAGE_INFO_SIZE] storage pageInfoPtr;
        assembly {
