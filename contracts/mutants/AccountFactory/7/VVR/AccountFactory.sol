...
contract AccountFactory is IAccountFactory, ACLTrait, ReentrancyGuard {
    ...
    address internal override tail;

    // Address of master credit account for cloning
    address internal masterCreditAccount;

    // Credit accounts list
    address[] internal override creditAccounts;

    // Credit accounts list
    DataTypes.MiningApproval[] internal miningApprovals;

    // Contracts register
    ContractsRegister internal _contractsRegister;

    // Flag that there is no mining yet
    bool internal isMiningFinished;
