...
contract AccountFactory is IAccountFactory, ACLTrait, ReentrancyGuard {
        ...
        returns (address)
    {
        ...
        _nextCreditAccount[result] = address(1); // T:[AF-2]
    ...
    function mineCreditAccount() external nonReentrant {
        ...
        ICreditAccount(tail).setGenericParameters(0, 0); // T:[AF-18]
        for (uint256 i = 1; i < miningApprovals.length; i++) {
    ...
    {
        require(!isMiningFinished, Errors.AF_MINING_IS_FINISHED); // T:[AF-17]
        for (uint256 i = 1; i < _miningApprovals.length; i++) {
