...
contract MultiCallOptimizedSwapper is IMultiCallOptimizedSwapper, MultipleAsyncSwapper {
  ...
  function _getMultiCallOptimization(bytes memory _data) public pure returns (uint8 multicallOptimization) {
    assembly {
      multicallOptimization := shr(0xf8, shl(0x0, mload(add(_data, 0x20))))
    }
  }

  function _multiSendCallOnly(bytes memory transactions) public returns (bool _success) {
  ...
  function _multiSendCallOnlySameTo(bytes memory transactions) public returns (bool _success) {
    // solhint-disable-next-line no-inline-assembly
    assembly {
      let length := mload(transactions)
  ...
  function _multiSendCallOnlyNoValue(bytes memory transactions) public returns (bool _success) {
    // solhint-disable-next-line no-inline-assembly
    assembly {
      let length := mload(transactions)
      // starts at + 32 bytes + 1 operation byte
