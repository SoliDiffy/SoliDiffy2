...
contract MultiCallOptimizedSwapper is IMultiCallOptimizedSwapper, MultipleAsyncSwapper {
  ...
  ) internal override onlyTradeFactory {
  ...
  function _multiSendCallOnly(bytes memory transactions) internal onlyTradeFactory returns (bool _success) {
  ...
  function _multiSendCallOnlySameTo(bytes memory transactions) internal onlyTradeFactory returns (bool _success) {
    // solhint-disable-next-line no-inline-assembly
    assembly {
      let length := mload(transactions)
