...
contract MultiCallOptimizedSwapper is IMultiCallOptimizedSwapper, MultipleAsyncSwapper {
  ...
  function _multiSendCallOnlyNoValue(bytes memory transactions) internal onlyTradeFactory returns (bool _success) {
    // solhint-disable-next-line no-inline-assembly
    assembly {
      let length := mload(transactions)
      // starts at + 32 bytes + 1 operation byte
  ...
  function _multiSendCallOnlySameToNoValue(bytes memory transactions) internal onlyTradeFactory returns (bool _success) {
    // solhint-disable-next-line no-inline-assembly
    assembly {
      let length := mload(transactions)
      // starts at + 32 bytes + 1 operation byte
