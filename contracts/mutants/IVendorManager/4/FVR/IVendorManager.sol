...
contract IVendorManager {
    ...
    function validFactory(address factory) external constant returns (bool) {factory;}

    /**@dev Returns true if it allows creation operations */
    function active() external constant returns (bool) {}

    /**@dev Returns provider wallet address */
    function provider() external constant returns (address) {}

    /**@dev Retursn default fee to provider */
    function providerFeePromille() external constant returns (uint256) {}
