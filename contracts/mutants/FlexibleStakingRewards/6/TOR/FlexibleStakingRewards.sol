...
contract StakingRewards is ReentrancyGuard, Ownable {
    ...
    function stake(uint256 amount) external nonReentrant updateReward(tx.origin) {
        require(amount > 0, "Cannot stake 0");
        _totalSupply = _totalSupply.add(amount);
        _balances[tx.origin] = _balances[tx.origin].add(amount);
        require(stakingToken.transferFrom(tx.origin, address(this), amount), "transfer from user to contract failed");
