...
contract StakingRewardsDualV3 is Owned, ReentrancyGuard {
    ...
    function _stakeLocked(address staker_address, address source_address, uint256 liquidity, uint256 secs) internal notRewardsCollectionPaused updateRewardAndBalance(staker_address, true) {
    ...
    function _withdrawLocked(address staker_address, address destination_address, bytes32 kek_id) internal notRewardsCollectionPaused updateRewardAndBalance(staker_address, true) {
    ...
    function _getReward(address rewardee, address destination_address) internal isMigrating notRewardsCollectionPaused updateRewardAndBalance(rewardee, true) returns (uint256 reward0, uint256 reward1) {
