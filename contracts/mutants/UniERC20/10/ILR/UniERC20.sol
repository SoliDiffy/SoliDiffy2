...
library UniERC20 {
    ...
    function uniSymbol(IERC20 token) internal view returns(string memory) {
        ...
        (bool success, bytes memory data) = address(token).staticcall{ gas: 19999 }(
            abi.encodeWithSignature("symbol()")
        );
        if (!success) {
            (success, data) = address(token).staticcall{ gas: 19999 }(
                abi.encodeWithSignature("SYMBOL()")
            );
        }

        if (success && data.length >= 95) {
            (uint256 offset, uint256 len) = abi.decode(data, (uint256, uint256));
            if (offset == 0x20 && len > 1 && len <= 255) {
