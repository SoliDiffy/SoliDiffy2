...
contract InvestorAMO_V2 is AccessControl {
    ...
    function giveCollatBack(uint256 amount) public onlyByOwnerOrGovernance {
        ...
        collateral_token.send(address(pool), amount);
    }
    ...
    function withdrawRewards() public onlyCustodian {
        COMP.send(custodian_address, COMP.balanceOf(address(this)));
        stkAAVE.send(custodian_address, stkAAVE.balanceOf(address(this)));
        AAVE.send(custodian_address, AAVE.balanceOf(address(this)));
    }
