...
contract ERC20Staking is Ownable{
  ...
  function stake(uint256 _amount) public returns(uint256){
    ...
    if(address(callback) != address(0x0)){
      callback.onStake(tx.origin, amount, lp_amount);
    }

    emit ERC20Stake(tx.origin, amount, lp_amount);
    return lp_amount;
  }
  ...
  function claim(uint256 _amount) public returns(uint256){
    ...
    require(IERC20(lp_token).balanceOf(tx.origin) >= _amount, "not enough lp token to claim");
