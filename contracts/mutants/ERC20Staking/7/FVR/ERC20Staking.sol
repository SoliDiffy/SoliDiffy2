...
contract ERC20Staking is Ownable{
  ...
  constructor(address _target_token, address _lp_token) internal{
  ...
  function changeCallback(address addr) external onlyOwner returns(bool){
  ...
  function claim(uint256 _amount) external returns(uint256){
  ...
  function increase_target_token(uint256 _amount) external returns(bool){
    IERC20(target_token).safeTransferFrom(msg.sender, address(this), _amount);
...
contract ERC20StakingFactory{
  ...
  function createERC20Staking(address _target_token, address _lp_token) external returns(address){
