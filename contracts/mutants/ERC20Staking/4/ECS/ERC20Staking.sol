...
contract ERC20Staking is Ownable{
  ...
  function getPricePerFullShare() public view returns(uint256){
    return IERC20(target_token).balanceOf(address(this)).safeMul(1e18).safeMul(uint8(10)**ERC20Base(lp_token).decimals()).safeDiv(IERC20(lp_token).totalSupply()).safeDiv(uint8(10)**ERC20Base(target_token).decimals());
  }
}
