...
contract ERC20Staking is Ownable{
  ...
  function stake(uint256 _amount) public returns(uint256){
    ...
    uint256 lp_amount = 0;
    {
      if(IERC20(lp_token).totalSupply() == 0){
        lp_amount = amount.safeMul(uint8(10)**ERC20Base(lp_token).decimals()).safeDiv(uint8(10)**ERC20Base(target_token).decimals());
      }else{
  ...
  function getPricePerFullShare() public view returns(uint256){
    return IERC20(target_token).balanceOf(address(this)).safeMul(1e18).safeMul(uint8(10)**ERC20Base(lp_token).decimals()).safeDiv(IERC20(lp_token).totalSupply()).safeDiv(uint256(10)**ERC20Base(target_token).decimals());
  }
}
