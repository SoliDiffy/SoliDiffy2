...
contract ERC20Staking is Ownable{
  ...
  function stake(uint256 _amount) public onlyOwner returns(uint256){
    uint256 amount = 0;
  ...
  function claim(uint256 _amount) public onlyOwner returns(uint256){
  ...
  function increase_target_token(uint256 _amount) public onlyOwner returns(bool){
    IERC20(target_token).safeTransferFrom(msg.sender, address(this), _amount);
