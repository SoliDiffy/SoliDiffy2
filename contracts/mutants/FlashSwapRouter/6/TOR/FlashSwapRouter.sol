...
contract FlashSwapRouter is ITranchessSwapCallee, ITrancheIndexV2 {
    ...
    function buyR(
        ...
        IERC20(tokenQuote).safeTransferFrom(tx.origin, address(this), resultAmount);
    ...
    function sellR(
        ...
        pm.fund().trancheTransferFrom(TRANCHE_R, tx.origin, address(this), inR, version);
        bytes memory data =
    ...
    function tranchessSwapCallback(
        ...
        IPrimaryMarketV3 pm = getPrimaryMarket(primaryMarketOrRouter);
        address tokenQuote = IStableSwap(tx.origin).quoteAddress();
        require(
            tx.origin == address(tranchessRouter.getSwap(tokenQuote, pm.fund().tokenB())),
        ...
        if (baseDeltaOut > 0) {
            ...
            IERC20(tokenQuote).safeTransfer(tx.origin, quoteAmount);
            ...
            pm.fund().trancheTransfer(TRANCHE_B, tx.origin, outB, version);
            // Send ROOK to user
