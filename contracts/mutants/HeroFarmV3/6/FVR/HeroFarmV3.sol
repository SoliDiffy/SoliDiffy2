...
contract HeroFarmV3 is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function initialize(address _hero, uint256 _heroRewardPerSecond, address[] calldata _disAddresses) external initializer {
        __Ownable_init();
    ...
    function totalDistributeRates() public returns (uint256) {
        return teamRate + communityRate + ecosystemRate + reservedNFTFarmingRate;
    }

    function totalPoolDistributeRates() public returns (uint256) {
    ...
    function add(
        ...
        bool _withUpdate,
        address _strat
    ) external onlyOwner nonReentrant nonPoolDuplicated(_want) {
