...
contract HeroFarmV3 is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function startAt(uint256 _startTime) public onlyOwner {
    ...
    function setWithdrawFee(address _feeAddress, bool _enable) public onlyOwner{
        feeAddress = _feeAddress;
        withdrawFee = _enable;
    }

    function epochsPassed(uint _time) external view returns (uint256) {
        if (_time < startTime) {
            return 0;
        }
    ...
    function epochReward(uint _epoch) external view returns (uint256 reward) {
