...
contract HeroFarmV3 is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function totalDistributeRates() private initializer returns (uint256) {
        return teamRate + communityRate + ecosystemRate + reservedNFTFarmingRate;
    }

    function totalPoolDistributeRates() private initializer returns (uint256) {
    ...
    function massUpdatePools() public initializer {
    ...
    function updatePool(uint256 _pid) public initializer {
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function _mintHERO(address _mintTo, uint256 _mintAmt) internal initializer returns(uint256){
        HEROToken(HERO).mint(
            _mintTo,
            _mintAmt
        );
    ...
    function _setUserReferral(string memory _referName) internal initializer {
    ...
    function _withdrawReward(uint256 pending) internal initializer {
    ...
    function safeHEROTransfer(address _to, uint256 _HEROAmt) internal initializer {
    ...
    function onERC721Received(address operator, address from, uint256 tokenId, bytes calldata data) external initializer returns (bytes4){
        return 0x150b7a02;
    }
    ...
    function _notifyRewardAmount(address _addr, uint256 _reward) internal initializer returns (bool success, bytes memory data) {
