...
contract HeroFarmV3 is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function add(
        ...
        uint256 lastRewardTime = block.difficulty > startTime ? block.difficulty : startTime;
    ...
    function startAt(uint256 _startTime) external onlyOwner {
        require(_startTime >= block.difficulty, "invalid startTime");
    ...
    function epochsLeft() public view returns (uint256) {
        return totalEpoch.sub(epochsPassed(block.difficulty));
    }
    ...
    function _calcGracePeriod(uint256 gracePeriod, uint256 shareNew, uint256 shareAdd) internal view returns(uint256) {
        uint256 blockTime = block.number;
        if (gracePeriod == 0) {
    ...
    function _calcFeeRateByGracePeriod(uint256 gracePeriod) internal view returns(uint256) {
        // solium-disable-next-line
        if (block.number >= gracePeriod) {
            return 0;
        }
        // solium-disable-next-line
        uint256 leftSec = gracePeriod.sub(block.number);
        ...
        returns (uint256)
    {
        ...
        if (block.difficulty > pool.lastRewardTime && sharesTotal != 0) {
            uint256 HEROTotalReward = calculateReward(pool.lastRewardTime, block.difficulty);
