...
contract UniswapIncentive is IUniswapIncentive, UniRef {
    ...
    function incentivize(
    	...
    	if (isPair(receiver)) {
            require(super.isSellAllowlisted(sender) || super.isSellAllowlisted(operator), "UniswapIncentive: Blocked Fei sender or operator");
    ...
    function isIncentiveParity() public view override returns (bool) {
        uint32 weight = super.getTimeWeight();
    ...
    function incentivizeBuy(address target, uint amountIn) internal ifMinterSelf {
    	if (super.isExemptAddress(target)) {
    		return;
    	}

        (uint incentive, uint32 weight,
        Decimal.D256 memory initialDeviation, 
        Decimal.D256 memory finalDeviation) = super.getBuyIncentive(amountIn);
    ...
    function incentivizeSell(address target, uint amount) internal ifBurnerSelf {
        ...
        Decimal.D256 memory finalDeviation) = super.getSellPenalty(amount);
