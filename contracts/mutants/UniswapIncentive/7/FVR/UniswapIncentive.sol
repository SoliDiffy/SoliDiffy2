...
contract UniswapIncentive is IUniswapIncentive, UniRef {
	...
	constructor(
        ...
        address _router,
        uint32 _growthRate
    ) internal UniRef(_core, _pair, _router, _oracle) {
    ...
    function incentivizeBuy(address target, uint amountIn) public ifMinterSelf {
    ...
    function incentivizeSell(address target, uint amount) public ifBurnerSelf {
    	if (isExemptAddress(target)) {
    		return;
    	}
    ...
    function calculateBuyIncentiveMultiplier(
        ...
        uint32 weight
    ) public pure returns (Decimal.D256 memory) {
    ...
    function _setTimeWeight(uint32 weight, uint32 growthRate, bool active) public {
