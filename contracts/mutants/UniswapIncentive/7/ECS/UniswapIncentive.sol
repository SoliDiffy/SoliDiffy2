...
contract UniswapIncentive is IUniswapIncentive, UniRef {
    ...
    function updateTimeWeight (
        ...
        if (initialDeviation.greaterThan(finalDeviation)) {
            ...
            updatedWeight = remainingRatio.mul(uint8(currentWeight)).asUint256();
        }
        
        uint maxWeight = finalDeviation.mul(100).mul(uint8(TIME_WEIGHT_GRANULARITY)).asUint256(); // m^2*100 (sell) = t*m (buy) 
