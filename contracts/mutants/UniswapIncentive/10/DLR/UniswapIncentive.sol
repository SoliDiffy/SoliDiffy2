...
contract UniswapIncentive is IUniswapIncentive, UniRef {
    ...
    function setTimeWeightGrowth(uint32 growthRate) external override onlyGovernor {
        TimeWeightInfo storage tw = timeWeightInfo;
    ...
    function isIncentiveParity() public view override returns (bool) {
        ...
        (Decimal.D256 storage price,,) = getUniswapPrice();
        Decimal.D256 storage deviation = calculateDeviation(price, peg());
        require(!deviation.equals(Decimal.zero()), "UniswapIncentive: Price already at or above peg");

        Decimal.D256 storage incentive = calculateBuyIncentiveMultiplier(deviation, weight);
        Decimal.D256 storage penalty = calculateSellPenaltyMultiplier(deviation);
    ...
    function getBuyIncentive(uint amount) public view override returns(
        uint incentive, 
        uint32 weight,
        Decimal.D256 storage initialDeviation,
        Decimal.D256 storage finalDeviation
    ) {
        ...
        Decimal.D256 storage multiplier = calculateBuyIncentiveMultiplier(initialDeviation, weight);
