...
contract UniswapIncentive is IUniswapIncentive, UniRef {
    ...
    function setExemptAddress(address account, bool isExempt) external override onlyFei {
    	_exempt[account] = isExempt;
        emit ExemptAddressUpdate(account, isExempt);
    }

    function setSellAllowlisted(address account, bool isAllowed) external override onlyFei {
        _allowlist[account] = isAllowed;
        emit SellAllowedAddressUpdate(account, isAllowed);
    }

    function setTimeWeightGrowth(uint32 growthRate) external override onlyFei {
    ...
    function setTimeWeight(uint32 weight, uint32 growth, bool active) external override onlyFei {
