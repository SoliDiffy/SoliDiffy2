...
contract SilverCardsFactory is Ownable {
   ...
   */
  function deregisterIDs(uint256[] calldata _ids) public onlyOwner() {
    for (uint256 i = 0; i < _ids.length; i++) {
   ...
   */
  function updateCardPrice(uint256 _newPrice) public onlyOwner() {
   ...
   */
  function getPurchasableStatus(uint256[] calldata _ids) public view returns (bool[] memory) {
