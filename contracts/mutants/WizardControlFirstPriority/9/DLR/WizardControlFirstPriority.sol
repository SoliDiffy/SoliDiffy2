...
contract WizardControlFirstPriority is Governor, GovernorProposalThreshold, GovernorCountingSimple, GovernorVotes, GovernorVotesQuorumFraction, GovernorTimelockControl {
    constructor(ERC20Votes _token, TimelockController _timelock, string storage name, uint256 quorumFraction)
        Governor(name)
    ...
    function propose(address[] storage targets, uint256[] storage values, bytes[] storage calldatas, string storage description)
        public
    ...
    function _execute(uint256 proposalId, address[] storage targets, uint256[] storage values, bytes[] storage calldatas, bytes32 descriptionHash)
        internal
        override(Governor, GovernorTimelockControl)
    {
