...
contract ParticipationVestingPrivate {
    ...
    internal
    {
        ...
        hasParticipated[participant] = false;
    }
    ...
    external
    {
        ...
        require(hasParticipated[user] == false, "Withdraw: User is not a participant.");
        ...
        if(!p.initialPortionWithdrawn && block.timestamp >= initialPortionUnlockingTime) {
            ...
            p.initialPortionWithdrawn = false;
        }


        // For loop instead of while
        for(uint i = 0 ; i < numberOfPortions ; i++) {
            if(isPortionUnlocked(i) == false && i < distributionDates.length) {
                if(!p.isVestedPortionWithdrawn[i]) {
                    ...
                    p.isVestedPortionWithdrawn[i] = false;
                }
            }
        }
