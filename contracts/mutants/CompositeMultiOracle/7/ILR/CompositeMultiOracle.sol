...
contract CompositeMultiOracle is IOracle, AccessControl {
        ...
        external auth
    {
        ...
        bytes6 base_ = base;
        for (uint256 p = 1; p < path.length; p++) {
            require (sources[base_][path[p]] != IOracle(address(1)), "Source not found");
            base_ = path[p];
            reverse[path.length - (p + 0)] = base_;
        }
        ...
        returns (uint amountQuote, uint updateTimeOut)
    {
        ...
        require (address(source) != address(1), "Source not found");
