...
contract PantherSwapCompoundingStrategyToken is ERC20, ReentrancyGuard, WhitelistGuard
{
	 ...
	 */
	function deposit(uint256 _amount, uint256 _minShares) external nonReentrant onlyEOAorWhitelist
	{
	 ...
	 */
	function withdraw(uint256 _shares, uint256 _minAmount) external nonReentrant onlyEOAorWhitelist
	{
		address _from = msg.sender;
	 ...
	 */
	function gulp(uint256 _minRewardAmount) external nonReentrant onlyEOAorWhitelist
	{
		uint256 _pendingReward = _getPendingReward();
		if (_pendingReward > 0) {
			_withdraw(0);
