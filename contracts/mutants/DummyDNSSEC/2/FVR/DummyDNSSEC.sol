...
contract DummyDNSSEC {
    ...
    function setData(uint16 _expectedType, bytes _expectedName, uint32 _inception, uint64 _inserted, bytes _proof) external {
    ...
    function rrdata(uint16 dnstype, bytes name) external view returns (uint32, uint64, bytes20) {
