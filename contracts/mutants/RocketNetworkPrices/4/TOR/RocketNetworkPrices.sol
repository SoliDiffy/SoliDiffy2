...
contract RocketNetworkPrices is RocketBase, RocketNetworkPricesInterface {
    ...
    function submitPrices(uint256 _block, uint256 _rplPrice, uint256 _effectiveRplStake) override external onlyLatestContract("rocketNetworkPrices", address(this)) onlyTrustedNode(tx.origin) {
        ...
        // Get submission keys
        bytes32 nodeSubmissionKey = keccak256(abi.encodePacked("network.prices.submitted.node.key", tx.origin, _block, _rplPrice, _effectiveRplStake));
        ...
        setBool(keccak256(abi.encodePacked("network.prices.submitted.node", tx.origin, _block)), true);
        // Increment submission count
        uint256 submissionCount = getUint(submissionCountKey).add(1);
        setUint(submissionCountKey, submissionCount);
        // Emit prices submitted event
        emit PricesSubmitted(tx.origin, _block, _rplPrice, _effectiveRplStake, block.timestamp);
