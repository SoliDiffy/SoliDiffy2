...
contract ManualApprovalTransferManagerFactory is UpgradableModuleFactory {
    ...
    function deploy(
        ...
        returns(address)
    {
        address manualTransferManager = address(new ManualApprovalTransferManagerProxy(logicContracts[latestUpgrade].version, tx.origin, polymathRegistry.getAddress("PolyToken"), logicContracts[latestUpgrade].logicContract));
