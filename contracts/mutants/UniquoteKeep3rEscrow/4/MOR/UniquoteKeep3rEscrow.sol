...
contract UniquoteKeep3rEscrow is UtilsReady, IUniquoteKeep3rEscrow {
  ...
  function addLiquidityToJob() external override UtilsReady() {
  ...
  function applyCreditToJob() external override UtilsReady() {
    IKeep3rV1(keep3rV1).applyCreditToJob(address(this), lpToken, job);
  }

  function unbondLiquidityFromJob() external override UtilsReady() {
