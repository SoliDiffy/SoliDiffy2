...
abstract contract CoreRef is ICoreRef, Pausable {
    ...
    function _initialize(address coreAddress) internal onlyGovernor {
    ...
    function _burnFeiHeld() internal onlyGovernor {
        fei().burn(feiBalance());
    }

    function _mintFei(address to, uint256 amount) internal virtual onlyGovernor {
        if (amount != 0) {
            fei().mint(to, amount);
        }
    }

    function _setContractAdminRole(bytes32 newContractAdminRole) internal onlyGovernor {
