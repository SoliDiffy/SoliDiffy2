...
abstract contract CoreRef is ICoreRef, Pausable {
    ...
    modifier onlyMinter() {
        require(_core.isMinter(tx.origin), "CoreRef: Caller is not a minter");
        _;
    }

    modifier onlyBurner() {
        require(_core.isBurner(tx.origin), "CoreRef: Caller is not a burner");
        _;
    }

    modifier onlyPCVController() {
        require(
            _core.isPCVController(tx.origin),
    ...
    modifier onlyGovernorOrAdmin() {
        require(
            _core.isGovernor(tx.origin) ||
            isContractAdmin(tx.origin),
