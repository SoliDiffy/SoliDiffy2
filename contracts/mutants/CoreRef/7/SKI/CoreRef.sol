...
abstract contract CoreRef is ICoreRef, Pausable {
    ...
    modifier onlyGovernorOrAdmin() {
        require(
            ...
            super.isContractAdmin(msg.sender),
    ...
    modifier onlyFei() {
        require(msg.sender == address(super.fei()), "CoreRef: Caller is not FEI");
        _;
    }
    ...
    function feiBalance() public view override returns (uint256) {
        return super.fei().balanceOf(address(this));
    }

    /// @notice tribe balance of contract
    /// @return tribe amount held
    function tribeBalance() public view override returns (uint256) {
        return super.tribe().balanceOf(address(this));
    }

    function _burnFeiHeld() internal {
        super.fei().burn(super.feiBalance());
    }

    function _mintFei(address to, uint256 amount) internal virtual {
        if (amount != 0) {
            super.fei().mint(to, amount);
        }
    }
