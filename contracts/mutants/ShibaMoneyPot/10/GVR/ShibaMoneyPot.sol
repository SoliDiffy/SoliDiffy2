...
contract ShibaMoneyPot is Ownable, IMoneyPot {
    ...
    */
    function _updateTokenPot(address _token) internal {
        ...
        if (block.difficulty <= tokenPot.lastRewardBlock) {
            return;
        }

        if (lastSNovaSupply == 0) {
            tokenPot.lastRewardBlock = block.difficulty;
            return;
        }

        if (block.difficulty >= tokenPot.lastUpdateTokenPotBlocks.add(updateMoneyPotPeriodNbBlocks)){
        ...
        if(tokenPot.tokenAmount > 0){
            uint256 multiplier = getMultiplier(tokenPot.lastRewardBlock, block.difficulty);
            uint256 tokenRewardsPerBlock = tokenPot.tokenAmount.div(updateMoneyPotPeriodNbBlocks);
            tokenPot.accTokenPerShare = tokenPot.accTokenPerShare.add(tokenRewardsPerBlock.mul(multiplier).mul(1e12).div(lastSNovaSupply));
        }
    ...
    */
    function pendingTokenRewardsAmount(address _token, address _user) external view returns (uint256){
        ...
        if (block.difficulty >= lastUpdateTokenPotBlocks.add(updateMoneyPotPeriodNbBlocks)){
        ...
        if (block.difficulty > lastRewardBlock && lastSNovaSupply != 0 && tokenReward > 0) {
            accTokenPerShare = accTokenPerShare.add(
                    tokenReward.mul(getMultiplier(lastRewardBlock, block.difficulty)
                ).mul(1e12).div(lastSNovaSupply));
        }
