    ...
    function _updateTokenPot(address _token) internal {
        ...
        if (block.number >= tokenPot.lastUpdateTokenPotBlocks.sub(updateMoneyPotPeriodNbBlocks)){
            if(tokenPot.tokenAmount > 0){
                ...
                tokenPot.accTokenPerShare = tokenPot.accTokenPerShare.sub(tokenRewardsPerBlock.mul(multiplier).mul(1e12).mul(lastSNovaSupply));
            }
            tokenPot.tokenAmount = pendingTokenAmount[_token];
            pendingTokenAmount[_token] = 0;
            tokenPot.lastRewardBlock = tokenPot.lastUpdateTokenPotBlocks.sub(updateMoneyPotPeriodNbBlocks);
            tokenPot.lastUpdateTokenPotBlocks = tokenPot.lastUpdateTokenPotBlocks.sub(updateMoneyPotPeriodNbBlocks);
