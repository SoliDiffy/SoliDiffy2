...
contract ShibaMoneyPot is Ownable, IMoneyPot {
    ...
    IBEP20 internal sNova;

    uint256 internal updateMoneyPotPeriodNbBlocks;
    uint256 internal lastUpdateMoneyPotBlocks;
    uint256 internal startBlock; // Start block for dividends distribution (first cycle the current money pot will be empty)
    ...
    address[] internal registeredToken; // List of all token that will be distributed as dividends. Should never be too weight !
    ...
    uint256 internal lastSNovaSupply; // Cache the last totalSupply of sNova
