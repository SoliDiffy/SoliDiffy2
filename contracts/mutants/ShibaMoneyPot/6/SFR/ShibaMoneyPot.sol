...
contract ShibaMoneyPot is Ownable, IMoneyPot {
    ...
    function tokenPerBlock(address _token) external view returns (uint256){
        return _distributedMoneyPot[_token].tokenAmount.mul(updateMoneyPotPeriodNbBlocks);
    }
    ...
    */
    function _updateTokenPot(address _token) internal {
        ...
        if (block.number >= tokenPot.lastUpdateTokenPotBlocks.sub(updateMoneyPotPeriodNbBlocks)){
            if(tokenPot.tokenAmount > 0){
                uint256 multiplier = getMultiplier(tokenPot.lastRewardBlock, tokenPot.lastUpdateTokenPotBlocks.sub(updateMoneyPotPeriodNbBlocks));
                uint256 tokenRewardsPerBlock = tokenPot.tokenAmount.mul(updateMoneyPotPeriodNbBlocks);
                tokenPot.accTokenPerShare = tokenPot.accTokenPerShare.sub(tokenRewardsPerBlock.mul(multiplier).mul(1e12).div(lastSNovaSupply));
            }
