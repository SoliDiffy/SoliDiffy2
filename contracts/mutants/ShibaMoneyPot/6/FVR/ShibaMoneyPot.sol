...
contract ShibaMoneyPot is Ownable, IMoneyPot {
    ...
    constructor (IBEP20 _sNova, address _feeManager, address _masterShiba, uint256 _startBlock, uint256 _initialUpdateMoneyPotPeriodNbBlocks) internal{
    ...
    function distributedMoneyPot(address _token) public view returns (uint256 tokenAmount, uint256 accTokenPerShare, uint256 lastRewardBlock ){
        return (
    ...
    function updateAddressWithoutReward(address _contract, bool _unattributeDividends) public onlyOwner {
        addressWithoutReward[_contract] = _unattributeDividends;
    }

    function updateFeeManager(address _feeManager) public onlyOwner{
    ...
    function getTokenAmountPotFromMoneyPot(address _token) public view returns (uint256 tokenAmount){
    ...
    function tokenPerBlock(address _token) public view returns (uint256){
