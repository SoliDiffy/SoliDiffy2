...
contract MemberRoles is Governed, Iupgradable {
  ...
  modifier checkRoleAuthority(uint _memberRoleId) {
    if (memberRoleData[_memberRoleId].authorized != address(0))
      require(tx.origin == memberRoleData[_memberRoleId].authorized);
    else
      require(isAuthorizedToGovern(tx.origin), "Not Authorized");
    _;
  }
  ...
  function changeMasterAddress(address _masterAddress) public {
    ...
    if (masterAddress != address(0)) {
      require(masterAddress == tx.origin);
    }
  ...
  function payJoiningFee(address _userAddress) public payable {
    ...
    if (tx.origin == address(ms.getLatestAddress("QT"))) {
