...
contract MemberRoles is Governed, Iupgradable {
  ...
  external
  checkRoleAuthority(uint8(Role.AdvisoryBoard)) {

    _updateRole(_newABAddress, uint8(Role.AdvisoryBoard), true);
    _updateRole(_removeAB, uint8(Role.AdvisoryBoard), false);
  ...
  external {
    ...
    _updateRole(ms.owner(), uint8(Role.Owner), false);
    _updateRole(_newOwnerAddress, uint8(Role.Owner), true);
  }
  ...
  function addInitialABMembers(address[] calldata abArray) external onlyOwner {
    ...
    require(maxABCount >=
      SafeMath.add(numberOfMembers(uint8(Role.AdvisoryBoard)), abArray.length)
    );
