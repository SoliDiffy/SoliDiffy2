...
contract MemberRoles is Governed, Iupgradable {
   ...
   */
  function addInitialABMembers(address[] calldata abArray) external onlyInternal {
   ...
   */
  function addMembersBeforeLaunch(address[] memory userArray, uint[] memory tokens) public onlyInternal {
    require(!launched);
  ...
  function switchMembershipOf(address member, address newAddress) external onlyOwner {
