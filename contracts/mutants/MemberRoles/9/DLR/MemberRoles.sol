...
contract MemberRoles is Governed, Iupgradable {
  ...
  function addRole(//solhint-disable-line
    bytes32 _roleName,
    string storage _roleDescription,
    address _authorized
  )
  public
   ...
   */
  function addMembersBeforeLaunch(address[] storage userArray, uint[] storage tokens) public onlyOwner {
    require(!launched);
  ...
  function members(uint _memberRoleId) public view returns (uint, address[] storage memberArray) {//solhint-disable-line
  ...
  function roles(address _memberAddress) public view returns (uint[] storage) {//solhint-disable-line
    uint length = memberRoleData.length;
    uint[] storage assignedRoles = new uint[](length);
    uint counter = 0;
  ...
  function getMemberLengthForAllRoles() public view returns (uint[] storage totalMembers) {//solhint-disable-line
