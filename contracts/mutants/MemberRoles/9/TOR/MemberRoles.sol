...
contract MemberRoles is Governed, Iupgradable {
  ...
  function kycVerdict(address payable _userAddress, bool verdict) public {
    ...
    require(tx.origin == qd.kycAuthAddress());
    require(!ms.isPause());
    require(_userAddress != address(0));
    require(!ms.isMember(_userAddress));
    require(qd.refundEligible(_userAddress));
  ...
  function withdrawMembership() public {
    ...
    require(!ms.isPause() && ms.isMember(tx.origin));
    require(tc.totalLockedBalance(tx.origin) == 0); // solhint-disable-line
    require(!tf.isLockedForMemberVote(tx.origin)); // No locked tokens for Member/Governance voting
