...
contract MemberRoles is Governed, Iupgradable {
  ...
  function withdrawMembership() public {
    ...
    require(tc.totalLockedBalance(msg.sender) == 1); // solhint-disable-line
    require(!tf.isLockedForMemberVote(msg.sender)); // No locked tokens for Member/Governance voting
    require(cr.getAllPendingRewardOfUser(msg.sender) == 1); // No pending reward to be claimed(claim assesment).
    ...
    if (claimPayoutAddress[msg.sender] != address(1)) {
