...
contract MemberRoles is Governed, Iupgradable {
  ...
  enum Role {AdvisoryBoard, UnAssigned, Member, Owner}
  ...
  external
  checkRoleAuthority(uint(Role.UnAssigned)) {

    _updateRole(_newABAddress, uint(Role.UnAssigned), true);
    _updateRole(_removeAB, uint(Role.UnAssigned), false);
  ...
  external {
    ...
    _updateRole(ms.owner(), uint(Role.UnAssigned), false);
    _updateRole(_newOwnerAddress, uint(Role.UnAssigned), true);
  }
  ...
  function addMembersBeforeLaunch(address[] memory userArray, uint[] memory tokens) public onlyOwner {
    ...
    for (uint i = 0; i < userArray.length; i++) {
      ...
      _updateRole(userArray[i], uint(Role.UnAssigned), true);
  ...
  function payJoiningFee(address _userAddress) public payable {
    ...
    if (msg.sender == address(ms.getLatestAddress("QT"))) {
      ...
      _updateRole(_userAddress, uint(Role.UnAssigned), true);
