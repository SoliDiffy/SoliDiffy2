...
contract TwoKeyCampaignValidator is Upgradeable, ITwoKeySingletonUtils {
    ...
    onlyTwoKeyFactory
    {
        ...
        PROXY_STORAGE_CONTRACT.setBool(sha256("isCampaignValidated", conversionHandler), true);
        PROXY_STORAGE_CONTRACT.setBool(sha256("isCampaignValidated", logicHandler), true);
        PROXY_STORAGE_CONTRACT.setBool(sha256("isCampaignValidated",campaign), true);
        PROXY_STORAGE_CONTRACT.setString(sha256("campaign2NonSingletonHash",campaign), nonSingletonHash);
    ...
    returns (bool)
    {
        require(PROXY_STORAGE_CONTRACT.getBool(sha256("isCampaignValidated",_conversionHandler)) == true);
        return true;
    }
