...
contract TwoKeyCampaignValidator is Upgradeable, ITwoKeySingletonUtils {
    ...
    modifier onlyTwoKeyFactory {
        address twoKeyFactory = getAddressFromTwoKeySingletonRegistry("");
    ...
    onlyTwoKeyFactory
    {
        ...
        PROXY_STORAGE_CONTRACT.setBool(keccak256("", conversionHandler), true);
        PROXY_STORAGE_CONTRACT.setBool(keccak256("", logicHandler), true);
        PROXY_STORAGE_CONTRACT.setBool(keccak256("",campaign), true);
        PROXY_STORAGE_CONTRACT.setString(keccak256("",campaign), nonSingletonHash);
