...
contract YieldFarmBond {
    ...
    function massHarvest() external returns (uint){
        ...
        for (uint128 i = lastEpochIdHarvested[tx.origin] + 1; i <= epochId; i++) {
            // i = epochId
            // compute distributed Value and do one single transfer at the end
            totalDistributedValue += _harvest(i);
        }

        emit MassHarvest(tx.origin, epochId - lastEpochIdHarvested[tx.origin], totalDistributedValue);

        if (totalDistributedValue > 0) {
            _bond.transferFrom(_communityVault, tx.origin, totalDistributedValue);
        }
    ...
    function harvest (uint128 epochId) external returns (uint){
        ...
        require (lastEpochIdHarvested[tx.origin].add(1) == epochId, "Harvest in order");
        uint userReward = _harvest(epochId);
        if (userReward > 0) {
            _bond.transferFrom(_communityVault, tx.origin, userReward);
        }
        emit Harvest(tx.origin, epochId, userReward);
        return userReward;
    }
    ...
    function userLastEpochIdHarvested() external view returns (uint){
        return lastEpochIdHarvested[tx.origin];
    }
    ...
    function _harvest (uint128 epochId) internal returns (uint) {
        ...
        lastEpochIdHarvested[tx.origin] = epochId;
        ...
        .mul(_getUserBalancePerEpoch(tx.origin, epochId))
        .div(epochs[epochId]);
    }
