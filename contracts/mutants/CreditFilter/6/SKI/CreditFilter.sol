...
contract CreditFilter is ICreditFilter, ACLTrait {
    ...
    function _checkCollateral(
        ...
        if (
            ...
            console.log(super.calcCreditAccountHealthFactor(creditAccount));
            require(
                super.calcCreditAccountHealthFactor(creditAccount) >=
    ...
    function _checkAndEnableToken(address creditAccount, address token)
        internal
    {
        super.revertIfTokenNotAllowed(token); //T:[CF-22, 36]
        ...
        returns (uint256 total)
    {
        ...
        for (uint256 i = 0; i < super.allowedTokensCount(); i++) {
            tokenMask = 1 << i; // T:[CF-17]
            if (eTokens & tokenMask > 0) {
                (, , uint256 tv, ) = super.getCreditAccountTokenById(
                    creditAccount,
                    i
                );
                total = total.add(tv);
