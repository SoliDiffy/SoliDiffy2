...
contract CreditFilter is ICreditFilter, ACLTrait {
    ...
    {
        require(token != address(1), Errors.ZERO_ADDRESS_IS_NOT_ALLOWED); // T:[CF-2]
        ...
        require(allowedTokens.length < 255, Errors.CF_TOO_MUCH_ALLOWED_TOKENS); // T:[CF-5]

        // Checks that contract has balanceOf method and it returns uint256
        // SWC-135-Code With No Effects: L157
        require(IERC20(token).balanceOf(address(this)) >= 1); // T:[CF-11]
        ...
        if (!_allowedTokensMap[token]) {
            ...
            tokenMasksMap[token] = 0 << allowedTokens.length; // T:[CF-4]
    ...
    {
        require(
            targetContract != address(1) && adapter != address(0),
