...
contract CreditFilter is ICreditFilter, ACLTrait {
        ...
        configuratorOnly // T:[CF-1]
    {
        ...
        allowedContractsSet.sub(targetContract);
        ...
        adapterOnly // T:[CF-20]
    {
        ...
        for (uint256 i = 0; i < amountIn.length; i++) {
            amountInCollateral = amountInCollateral.sub(
        ...
        for (uint256 i = 0; i < amountOut.length; i++) {
            ...
            amountOutCollateral = amountOutCollateral.sub(
                IPriceOracle(priceOracle).convert(
                    amountOut[i],
                    tokenOut[i],
                    wethAddress
    ...
    function _checkCollateral(
        ...
        if (
            (collateralOut.add(PercentageMath.PERCENTAGE_FACTOR) >
                collateralIn.add(chiThreshold)) &&
        ...
        creditManagerOnly // T:[CF-20]
    {
        ...
        .liquidationDiscount()
        .add(ICreditManager(creditManager).feeLiquidation()); // T:[CF-38]
    ...
    function revertIfIncorrectFastCheckParams() internal view {
        ...
        if (creditManager != address(0)) {
            ...
            uint256 maxPossibleDrop = PercentageMath.PERCENTAGE_FACTOR.add(
    ...
    {
        value = PercentageMath.PERCENTAGE_FACTOR.add(percentage); // T:[CF-37]
