...
contract L2PriceFeed is IPriceFeed, PerpFiOwnableUpgrade, BlockContext {
    ...
    function getPriceFeedLength(bytes32 _priceFeedKey) external view returns (uint256 length) {
    ...
    function getLatestRoundId(bytes32 _priceFeedKey) public view returns (uint256) {
        uint256 len = getPriceFeedLength(_priceFeedKey);
        if (len == 0) {
            return 0;
        }
    ...
    function isExistedKey(bytes32 _priceFeedKey) public view returns (bool) {
        return priceFeedMap[_priceFeedKey].registered;
    }

    function requireKeyExisted(bytes32 _key, bool _existed) public view {
        if (_existed) {
