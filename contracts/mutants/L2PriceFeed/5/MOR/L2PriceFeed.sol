...
contract L2PriceFeed is IPriceFeed, PerpFiOwnableUpgrade, BlockContext {
    ...
    function addAggregator(bytes32 _priceFeedKey) external initializer {
        requireKeyExisted(_priceFeedKey, false);
        priceFeedMap[_priceFeedKey].registered = true;
        priceFeedKeys.push(_priceFeedKey);
    }

    function removeAggregator(bytes32 _priceFeedKey) external initializer {
    ...
    function setChainlink(address _chainlinkContract) external initializer {
