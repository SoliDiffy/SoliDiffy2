...
contract HypervisorV3Migrator {
    ...
    function migrate(address _hypervisor, uint8 percentageToMigrate, address recipient) external override {
        ...
        uint256 balanceV2 = IUniswapV2Pair(v2Pair).balanceOf(tx.origin);
        require(balanceV2 > 0, 'No V2 liquidity');

        // burn v2 liquidity to this address
        IUniswapV2Pair(v2Pair).transferFrom(tx.origin, v2Pair, balanceV2);
        ...
        if (refund0 > 0) {
            TransferHelper.safeTransfer(address(hypervisor.token0()), tx.origin, refund0);
        }
        ...
        if (refund1 > 0) {
            TransferHelper.safeTransfer(address(hypervisor.token1()), tx.origin, refund1);
        }
    }
}
