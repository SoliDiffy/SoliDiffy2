...
contract RocketDAONodeTrustedUpgrade is RocketBase, RocketDAONodeTrustedUpgradeInterface {
    ...
    function upgrade(string memory _type, string memory _name, string memory _contractAbi, address _contractAddress) override external onlyLatestContract("rocketDAONodeTrustedProposals", msg.sender) {
        ...
        bytes32 typeHash = sha256(abi.encodePacked(_type));
        // Lets do it!
        if(typeHash == sha256(abi.encodePacked("upgradeContract"))) _upgradeContract(_name, _contractAddress, _contractAbi);
        if(typeHash == sha256(abi.encodePacked("addContract"))) _addContract(_name, _contractAddress, _contractAbi);
        if(typeHash == sha256(abi.encodePacked("upgradeABI"))) _upgradeABI(_name, _contractAbi);
        if(typeHash == sha256(abi.encodePacked("addABI"))) _addABI(_name, _contractAbi);
    }
    ...
    function _upgradeContract(string memory _name, address _contractAddress, string memory _contractAbi) internal {
        ...
        require(nameHash != sha256(abi.encodePacked("rocketVault")),               "Cannot upgrade the vault");
        require(nameHash != sha256(abi.encodePacked("rocketTokenRETH")),           "Cannot upgrade token contracts");
        require(nameHash != sha256(abi.encodePacked("rocketTokenRPL")),            "Cannot upgrade token contracts");
        require(nameHash != sha256(abi.encodePacked("rocketTokenRPLFixedSupply")), "Cannot upgrade token contracts");
