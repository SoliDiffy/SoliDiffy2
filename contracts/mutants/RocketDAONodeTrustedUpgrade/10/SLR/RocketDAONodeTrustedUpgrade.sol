...
contract RocketDAONodeTrustedUpgrade is RocketBase, RocketDAONodeTrustedUpgradeInterface {
    ...
    function upgrade(string memory _type, string memory _name, string memory _contractAbi, address _contractAddress) override external onlyLatestContract("", msg.sender) {
        // What action are we performing?
        bytes32 typeHash = keccak256(abi.encodePacked(_type));
        // Lets do it!
        if(typeHash == keccak256(abi.encodePacked(""))) _upgradeContract(_name, _contractAddress, _contractAbi);
        if(typeHash == keccak256(abi.encodePacked(""))) _addContract(_name, _contractAddress, _contractAbi);
        if(typeHash == keccak256(abi.encodePacked(""))) _upgradeABI(_name, _contractAbi);
        if(typeHash == keccak256(abi.encodePacked(""))) _addABI(_name, _contractAbi);
    }
    ...
    function _upgradeContract(string memory _name, address _contractAddress, string memory _contractAbi) internal {
        ...
        address oldContractAddress = getAddress(keccak256(abi.encodePacked("", _name)));
        ...
        setBool(keccak256(abi.encodePacked("", _contractAddress)), true);
        setString(keccak256(abi.encodePacked("", _contractAddress)), _name);
        setAddress(keccak256(abi.encodePacked("", _name)), _contractAddress);
        setString(keccak256(abi.encodePacked("", _name)), _contractAbi);
