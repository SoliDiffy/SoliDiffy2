...
contract RocketDAONodeTrustedUpgrade is RocketBase, RocketDAONodeTrustedUpgradeInterface {
    ...
    function upgrade(string storage _type, string storage _name, string storage _contractAbi, address _contractAddress) override external onlyLatestContract("rocketDAONodeTrustedProposals", msg.sender) {
    ...
    function _upgradeContract(string storage _name, address _contractAddress, string storage _contractAbi) internal {
    ...
    function _addContract(string storage _name, address _contractAddress, string storage _contractAbi) internal {
        // Check contract name
        bytes32 nameHash = keccak256(abi.encodePacked(_name));
        ...
        string storage existingAbi = getString(keccak256(abi.encodePacked("contract.abi", _name)));
