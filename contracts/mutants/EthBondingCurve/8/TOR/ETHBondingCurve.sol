...
contract ETHBondingCurve is LinearBondingCurve, IETHBondingCurve {
    ...
    function buy(uint256 tokenAmount) external payable override {
        ...
        token.safeTransfer(tx.origin, tokenAmount);
        if (msg.value > ethAmount)
            payable(tx.origin).transfer(msg.value - ethAmount);
        emit Bought(tx.origin, tokenAmount, ethAmount);
    }
    ...
    function sell(uint256 tokenAmount) external override {
        ...
        soldAmount = nextSold;
        token.safeTransferFrom(tx.origin, address(this), tokenAmount);
