...
contract BorrowerOperations is LiquityBase, Ownable, IBorrowerOperations {
    ...
    function openTrove(uint _LUSDAmount, address _hint) external payable override onlyOwner {
    ...
    function addColl(address _hint) external payable override onlyOwner {
        _adjustTrove(msg.sender, 0, 0, false, _hint);
    }

    // Send ETH as collateral to a trove. Called by only the Stability Pool.
    function moveETHGainToTrove(address _borrower, address _hint) external payable override onlyOwner {
        _requireCallerIsStabilityPool();
        _adjustTrove(_borrower, 0, 0, false, _hint);
    }

    // Withdraw ETH collateral from a trove
    function withdrawColl(uint _collWithdrawal, address _hint) external override onlyOwner {
    ...
    function withdrawLUSD(uint _LUSDAmount, address _hint) external override onlyOwner {
    ...
    function repayLUSD(uint _LUSDAmount, address _hint) external override onlyOwner {
    ...
    */
    function adjustTrove(uint _collWithdrawal, uint _debtChange, bool _isDebtIncrease, address _hint) external payable override onlyOwner {
        _adjustTrove(msg.sender, _collWithdrawal, _debtChange, _isDebtIncrease, _hint);
    }

    function _adjustTrove(address _borrower, uint _collWithdrawal, uint _debtChange, bool _isDebtIncrease, address _hint) internal onlyOwner {
    ...
    function closeTrove() external override onlyOwner {
    ...
    function claimRedeemedCollateral(address _user) external override onlyOwner {
