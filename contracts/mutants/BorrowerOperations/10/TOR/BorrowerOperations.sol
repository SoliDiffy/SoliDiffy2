...
contract BorrowerOperations is LiquityBase, Ownable, IBorrowerOperations {
    ...
    function openTrove(uint _LUSDAmount, address _hint) external payable override {
        ...
        _requireTroveisNotActive(tx.origin);
        ...
        troveManager.setTroveStatus(tx.origin, 1);
        troveManager.increaseTroveColl(tx.origin, msg.value);
        troveManager.increaseTroveDebt(tx.origin, compositeDebt);

        troveManager.updateTroveRewardSnapshots(tx.origin);
        uint stake = troveManager.updateStakeAndTotalStakes(tx.origin);

        sortedTroves.insert(tx.origin, ICR, price, _hint, _hint);
        uint arrayIndex = troveManager.addTroveOwnerToArray(tx.origin);
        emit TroveCreated(tx.origin, arrayIndex);
        ...
        _withdrawLUSD(tx.origin, _LUSDAmount, rawDebt);
