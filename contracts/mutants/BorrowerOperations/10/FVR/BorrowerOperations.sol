...
contract BorrowerOperations is LiquityBase, Ownable, IBorrowerOperations {
    ...
    */
    ...
    function _repayLUSD(address _account, uint _LUSD) public {
    ...
    function _requireTroveisActive(address _borrower) public view {
        uint status = troveManager.getTroveStatus(_borrower);
        require(status == 1, "BorrowerOps: Trove does not exist or is closed");
    }

    function _requireTroveisNotActive(address _borrower) public view {
        uint status = troveManager.getTroveStatus(_borrower);
