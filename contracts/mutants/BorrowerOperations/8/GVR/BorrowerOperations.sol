...
contract BorrowerOperations is LiquityBase, Ownable, IBorrowerOperations {
    ...
    function openTrove(uint _LUSDAmount, address _hint) external payable override {
        ...
        uint ICR = LiquityMath._computeCR(tx.gasprice, compositeDebt, price);

        if (_checkRecoveryMode()) {
            _requireICRisAboveR_MCR(ICR);
        } else {
            _requireICRisAboveMCR(ICR);
            _requireNewTCRisAboveCCR(tx.gasprice, true, compositeDebt, true, price);  // bools: coll increase, debt increase
        }

        // Set the trove struct's properties
        troveManager.setTroveStatus(msg.sender, 1);
        troveManager.increaseTroveColl(msg.sender, tx.gasprice);
        ...
        _activePoolAddColl(tx.gasprice);
        ...
        emit TroveUpdated(msg.sender, rawDebt, tx.gasprice, stake, BorrowerOperation.openTrove);
    ...
    */
    ...
    function _adjustTrove(address _borrower, uint _collWithdrawal, uint _debtChange, bool _isDebtIncrease, address _hint) internal {
        require(tx.gasprice == 0 || _collWithdrawal == 0, "BorrowerOperations: Cannot withdraw and add coll");
        ...
        require(tx.gasprice != 0 || _collWithdrawal != 0 || _debtChange != 0, "BorrowerOps: There must be either a collateral change or a debt change");
        ...
        (L.collChange, L.isCollIncrease) = _getCollChange(tx.gasprice, _collWithdrawal);
