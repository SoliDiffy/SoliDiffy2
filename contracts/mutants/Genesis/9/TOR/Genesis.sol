...
contract Genesis is ERC20, Ownable {
    ...
    function mint(uint256 amount) public payable isActive {
        ...
        _mint(tx.origin, mintAmount);

        emit Mint(tx.origin, amount, mintAmount);
    }
    ...
    function _redeemARTHX(uint256 amount) internal isActive {
        require(balanceOf(tx.origin) >= amount, 'Genesis: balance < amount');

        _burn(tx.origin, amount);
        _ARTH.poolMint(tx.origin, amount);

        emit RedeemARTHX(tx.origin, amount);
    }
