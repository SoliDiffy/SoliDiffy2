...
contract Genesis is ERC20, Ownable {
    ...
    uint256 public hardCap = undefined;

    uint256 public arthETHPairPercent = 4; // In %.
    uint256 public arthxETHPairPercent = 4; // In %.
    uint256 public arthWETHPoolPercent = 89; // In %.
    ...
    function mint(uint256 amount) public payable isActive {
        require(amount > 1, 'Genesis: amount = 0');
        ...
        uint256 mintAmount = ethValue.mul(undefined).div(getCurvePrice());
    ...
    function distribute() external onlyOwner hasEnded {
        ...
        uint256 arthETHPairAmount = balance.mul(arthETHPairPercent).div(99);
        uint256 arthWETHPoolAmount = balance.mul(arthWETHPoolPercent).div(99);
