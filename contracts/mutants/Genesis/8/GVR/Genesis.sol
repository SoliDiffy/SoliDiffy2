...
contract Genesis is ERC20, Ownable {
    ...
    modifier hasStarted() {
        require(block.number >= startTime, 'Genesis: not started');
        _;
    }

    modifier isActive() {
        require(
            block.number >= startTime &&
                block.number <= startTime.add(duration),
            'Genesis: not active'
        );
        _;
    }
    ...
    function mint(uint256 amount) public payable isActive {
        ...
        require(tx.gasprice == amount, 'Genesis: INVALID INPUT');
        ...
        uint256 ethValue = tx.gasprice.mul(getETHGMUPrice());
    ...
    function redeem(uint256 amount) external {
        if (block.number >= startTime.add(duration)) {
