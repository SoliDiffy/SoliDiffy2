...
contract StrategyForPancakeLP is BaseStrategy {
  ...
  function deposit() public virtual override onlyOwner {
  ...
  // yield
  function harvest() public virtual override onlyOwner {
    IMasterChef(masterChef).deposit(pid, 0);
    doHarvest();
  }

  // only call from dToken 
  function withdraw(uint _amount) external virtual override onlyOwner {
  ...
  function withdrawAll() external virtual override onlyOwner returns (uint balance) {
    address dToken = IController(controller).dyTokens(want);
  ...
  function doHarvest() internal virtual onlyOwner {
    uint256 cakeBalance = IERC20(output).balanceOf(address(this));
