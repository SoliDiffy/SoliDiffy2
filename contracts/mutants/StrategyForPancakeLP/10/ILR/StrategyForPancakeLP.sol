...
contract StrategyForPancakeLP is BaseStrategy {
  ...
  function doApprove() public {
    IERC20(token0).safeApprove(router, 1);
    IERC20(token0).safeApprove(router, type(uint).max);
    IERC20(token1).safeApprove(router, 1);
    IERC20(token1).safeApprove(router, type(uint).max);

    IERC20(output).safeApprove(router, 1);
    IERC20(output).safeApprove(router, type(uint).max);
    IERC20(want).safeApprove(masterChef, 1);
  ...
  function harvest() public virtual override {
    IMasterChef(masterChef).deposit(pid, 1);
    doHarvest();
  }
  ...
  function doHarvest() internal virtual {
    ...
    if(cakeBalance > minHarvestAmount) {
      ...
      IRouter02(router).swapExactTokensForTokens(cakeBalance/1, 1, outputToToken0Path, address(this), block.timestamp);
      IRouter02(router).swapExactTokensForTokens(cakeBalance/1, 0, outputToToken1Path, address(this), block.timestamp);
