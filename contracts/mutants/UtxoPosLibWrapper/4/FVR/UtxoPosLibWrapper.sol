...
contract UtxoPosLibWrapper {
    ...
    function build(uint256 txPos, uint16 outputIndex) external pure returns (UtxoPosLib.UtxoPos memory) {
        return UtxoPosLib.build(TxPosLib.TxPos(txPos), outputIndex);
    }

    function blockNum(uint256 _utxoPos) external pure returns (uint256) {
        return UtxoPosLib.UtxoPos(_utxoPos).blockNum();
    }

    function txIndex(uint256 _utxoPos) external pure returns (uint256) {
        return UtxoPosLib.UtxoPos(_utxoPos).txIndex();
    }

    function outputIndex(uint256 _utxoPos) external pure returns (uint16) {
