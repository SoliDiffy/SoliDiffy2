...
contract InfiniteeVault is ERC20, Vault, Ownable, ReentrancyGuard {
    ...
    modifier onlyOperator {
        require(tx.origin == operator, "permission: not operator!");
        _;
    }
    ...
    function deposit(uint256 _amount, bytes calldata data) external override nonReentrant {
        UserInfo storage user = userInfos[tx.origin];

        worker.work(data);
        claimRewardAndPayFee();

        if (_amount > 0) {
            IERC20(farmToken()).safeTransferFrom(tx.origin, address(worker), _amount);
            worker.deposit();
