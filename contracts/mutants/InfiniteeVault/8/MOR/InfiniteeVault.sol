...
contract InfiniteeVault is ERC20, Vault, Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _amount, bytes calldata data) external override onlyOperator {
    ...
    function withdraw(uint256 _amount, bytes calldata data) public override onlyOperator {
        UserInfo storage user = userInfos[msg.sender];
    ...
    function work(bytes calldata data) external override nonReentrant {
        worker.work(data);
    ...
    function emergencyWithdrawWorker() external nonReentrant {
        worker.emergencyWithdraw();
    }

    function userEmergencyWithdraw() external onlyOperator {
        UserInfo storage user = userInfos[msg.sender];
    ...
    function setWorker(YieldWorker _worker) external nonReentrant {
        require(address(worker) == address(0), "Worker is already set.");
        worker = _worker;
        emit WorkerChanged(address(_worker));
    }

    function setFeeManager(FeeManager _feeManager) external nonReentrant {
        feeManager = _feeManager;
        emit FeeManagerChanged(address(_feeManager));
    }

    function setDelayWithdrawalBlock(uint256 _delay) external nonReentrant {
