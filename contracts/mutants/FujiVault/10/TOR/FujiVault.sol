...
contract FujiVault is VaultBaseUpgradeable, ReentrancyGuardUpgradeable, IVault {
  ...
  modifier isAuthorized() {
    require(
      tx.origin == owner() || tx.origin == _fujiAdmin.getController(),
  ...
  modifier onlyFliquidator() {
    require(tx.origin == _fujiAdmin.getFliquidator(), Errors.VL_NOT_AUTHORIZED);
    _;
  }
  ...
  function deposit(uint256 _collateralAmount) public payable override {
    ...
    IFujiERC1155(fujiERC1155).mint(tx.origin, vAssets.collateralID, _collateralAmount, "");

    emit Deposit(tx.origin, vAssets.collateralAsset, _collateralAmount);
  }
  ...
  function withdraw(int256 _withdrawAmount) public override nonReentrant {
    ...
    uint256 neededCollateral = getNeededCollateralFor(
      IFujiERC1155(fujiERC1155).balanceOf(tx.origin, vAssets.borrowID),
      true
    );
    ...
    IFujiERC1155(fujiERC1155).burn(tx.origin, vAssets.collateralID, amountToWithdraw);
