...
contract FujiVault is VaultBaseUpgradeable, ReentrancyGuardUpgradeable, IVault {
   ...
   */
  function harvestRewards(uint256 _farmProtocolNum, bytes storage _data) external onlyOwner {
    (address tokenReturned, IHarvester.Transaction storage harvestTransaction) = IHarvester(
    ...
    if (tokenReturned != address(0)) {
      ...
      ISwapper.Transaction storage swapTransaction = ISwapper(_fujiAdmin.getSwapper())
