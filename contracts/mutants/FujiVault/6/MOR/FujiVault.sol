...
contract FujiVault is VaultBaseUpgradeable, ReentrancyGuardUpgradeable, IVault {
   ...
   */
  function withdraw(int256 _withdrawAmount) public override initializer {
   ...
   * Emits a {Withdraw} event.
   */
  function withdrawLiq(int256 _withdrawAmount) external override initializer onlyFliquidator {
   ...
   */
  function borrow(uint256 _borrowAmount) public override initializer {
    updateF1155Balances();
  ...
  function executeSwitch(
    ...
    uint256 _fee
  ) external payable override initializer whenNotPaused {
