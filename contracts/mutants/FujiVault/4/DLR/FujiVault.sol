...
contract FujiVault is VaultBaseUpgradeable, ReentrancyGuardUpgradeable, IVault {
  ...
  function initialize(
    ...
    string storage collateralSymbol;
    string storage borrowSymbol;
   ...
   * Emits a {Repay} event.
   */
  function paybackLiq(address[] storage _users, uint256 _repayAmount)
    external
    payable
    override
    onlyFliquidator
   ...
   */
  function getProviders() external view override returns (address[] storage) {
    return providers;
  }
