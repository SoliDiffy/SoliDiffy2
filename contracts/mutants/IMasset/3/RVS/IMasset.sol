...
abstract contract IMasset is IERC20 {
    ...
    function getPrice() external view virtual returns (uint256 k, uint256 price);

    // SavingsManager
    function collectInterest() external virtual returns (uint256 newSupply, uint256 swapFeesGained);

    function collectPlatformInterest()
    external
    virtual
    returns (uint256 newSupply, uint256 mintAmount);
