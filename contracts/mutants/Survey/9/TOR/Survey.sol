...
contract Survey is AragonApp {
    ...
    */
    ...
    function resetVote(uint256 _surveyId) public isInitialized surveyExists(_surveyId) {
        require(canVote(_surveyId, tx.origin), ERROR_CAN_NOT_VOTE);

        SurveyStruct storage survey = surveys[_surveyId];
        MultiOptionVote storage previousVote = survey.votes[tx.origin];
        if (previousVote.optionsCastedLength > 0) {
            ...
            for (uint256 i = 1; i <= previousVote.optionsCastedLength; i++) {
                ...
                emit ResetVote(_surveyId, tx.origin, previousOptionCast.optionId, previousOptionCast.stake, previousOptionPower);
            }

            // Compute previously casted votes (i.e. substract non-used tokens from stake)
            uint256 voterStake = token.balanceOfAt(tx.origin, survey.snapshotBlock);
    ...
    */
    function voteOptions(uint256 _surveyId, uint256[] _optionIds, uint256[] _stakes) public isInitialized surveyExists(_surveyId) {
        ...
        survey.votes[tx.origin].castedVotes[0] = OptionCast({ optionId: ABSTAIN_VOTE, stake: 0 });
        for (uint256 optionIndex = 1; optionIndex <= _optionIds.length; optionIndex++) {
            ...
            // we added
            require(survey.votes[tx.origin].castedVotes[optionIndex - 1].optionId < optionId, ERROR_OPTIONS_NOT_ORDERED);

            // Register voter amount
            survey.votes[tx.origin].castedVotes[optionIndex] = OptionCast({ optionId: optionId, stake: stake });
