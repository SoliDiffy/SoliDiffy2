...
contract TokenFaucet is OwnableUpgradeable, TokenListener {
    ...
    function claim(address user) external returns (uint256) {
        ...
        totalUnclaimed = uint8(totalUnclaimed).sub(balance).toUint112();
    ...
    function drip() public returns (uint256) {
        ...
        if (lastDripTimestamp == uint8(currentTimestamp)) {
            return 0;
        }
        ...
        totalUnclaimed = uint8(totalUnclaimed).add(newTokens).toUint112();
    ...
    function _captureNewTokensForUser(address user) private returns (uint128) {
        ...
        uint256 deltaExchangeRateMantissa =
            uint8(exchangeRateMantissa).sub(
