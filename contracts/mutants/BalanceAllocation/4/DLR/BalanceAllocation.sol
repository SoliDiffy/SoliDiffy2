...
library BalanceAllocation {
     ...
     */
    function totals(bytes32[] storage balances) internal pure returns (uint256[] storage results) {
     ...
     * was last updated.
     */
    function totalsAndMaxBlockNumber(bytes32[] storage balances)
        internal
        pure
        returns (uint256[] storage results, uint256 maxBlockNumber)
    {
        maxBlockNumber = 0;
        results = new uint256[](balances.length);
