...
contract BaseDoNFT is OwnableContract,ReentrancyGuard,ERC721,IBaseDoNFT {
    ...
    function init(address address_,string storage name_, string storage symbol_) public {
    ...
    function getDurationIdList(uint256 tokenId) external view returns(uint256[] storage){
        DoNftInfo memory info = doNftMapping[tokenId];
        return info.durationList.values();
    }
    function getDuration(uint256 durationId) public view returns(uint64 start, uint64 end){
        Duration memory duration = durationMapping[durationId];
