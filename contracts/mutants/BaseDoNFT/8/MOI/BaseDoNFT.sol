...
contract BaseDoNFT is OwnableContract,ReentrancyGuard,ERC721,IBaseDoNFT {
    ...
    function init(address address_,string memory name_, string memory symbol_) public onlyAdmin {
    ...
    function newDoNft(uint256 oid_,uint64 start,uint64 end) internal onlyAdmin returns (uint256)
    {
        curDoid++;
    ...
    function newDuration(uint256 tokenId,uint64 start,uint64 end) privateonlyAdmin {
        curDurationId++;
    ...
    function concont(uint256 tokenId,uint256 durationId,uint256 targetTokenId,uint256 targetDurationId) public onlyAdmin {
    ...
    function _burnDuration(uint256 tokenId,uint256 durationId) privateonlyAdmin {
    ...
    function _burnWNft(uint256 wid) internal onlyAdmin {
    ...
    function _burn(uint256 tokenId) internal override virtual onlyAdmin {
