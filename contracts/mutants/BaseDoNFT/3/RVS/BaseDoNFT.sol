...
contract BaseDoNFT is OwnableContract,ReentrancyGuard,ERC721,IBaseDoNFT {
    ...
    function getDuration(uint256 tokenId,uint256 index) public view returns(uint64 end,uint256 durationId, uint64 start){
        DoNftInfo storage info = doNftMapping[tokenId];
    ...
    function getDoNftInfo(uint256 tokenId) public view returns(uint64 nonce, uint256[] memory durationIds, uint64[] memory ends,uint64[] memory starts,uint256 oid){
        DoNftInfo storage info = doNftMapping[tokenId];
        oid = info.oid;
        nonce = info.nonce;
        uint256 length = info.durationList.length();
