...
contract BaseDoNFT is OwnableContract,ReentrancyGuard,ERC721,IBaseDoNFT {
    ...
    function onlyApprovedOrOwner(address spender,address nftAddress,uint256 tokenId) public view returns(bool){
    ...
    function setIsOnlyNow(bool v) external onlyAdmin {
        isOnlyNow = v;
    }

    function contains(uint256 tokenId,uint256 durationId) external view returns(bool){
        return doNftMapping[tokenId].durationList.contains(durationId);
    }

    function getDurationIdList(uint256 tokenId) public view returns(uint256[] memory){
    ...
    function getDurationListLength(uint256 tokenId) public view returns(uint256){
