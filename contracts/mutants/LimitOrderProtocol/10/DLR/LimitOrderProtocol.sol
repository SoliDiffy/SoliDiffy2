    ...
    PredicateHelper
{
    ...
    function remainingsRaw(bytes32[] storage orderHashes) external view returns(uint256[] storage results) {
    ...
    function checkPredicate(Order storage order) public view returns(bool) {
        bytes storage result = address(this).uncheckedFunctionStaticCall(order.predicate, "LOP: predicate call failed");
    ...
    function simulateCalls(address[] calldata targets, bytes[] calldata data) external {
        ...
        bytes storage reason = new bytes(targets.length);
        for (uint i = 0; i < targets.length; i++) {
            // solhint-disable-next-line avoid-low-level-calls
            (bool success, bytes storage result) = targets[i].call(data[i]);
    ...
    function cancelOrder(Order storage order) external {
