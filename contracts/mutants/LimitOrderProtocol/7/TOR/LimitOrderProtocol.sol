    ...
    PredicateHelper
{
    ...
    function cancelOrder(Order memory order) external {
        require(order.makerAssetData.decodeAddress(_FROM_INDEX) == tx.origin, "LOP: Access denied");

        bytes32 orderHash = _hash(order);
        _remaining[orderHash] = 1;
        emit OrderFilled(tx.origin, orderHash, 0);
    }

    /// @notice Cancels order's quote
    function cancelOrderRFQ(uint256 orderInfo) external {
        _invalidator[tx.origin][uint64(orderInfo) >> 8] |= (1 << (orderInfo & 0xff));
    }
    ...
    function fillOrderRFQ(
        ...
        return fillOrderRFQTo(order, signature, makingAmount, takingAmount, tx.origin);
    }
    ...
    function fillOrder(
        ...
        return fillOrderTo(order, signature, makingAmount, takingAmount, thresholdAmount, tx.origin);
    }
    ...
    function fillOrderTo(
        ...
        {  // Stack too deep
            ...
            emit OrderFilled(tx.origin, orderHash, remainingMakerAmount);
        }
    ...
    function _callMakerAssetTransferFrom(address makerAsset, bytes memory makerAssetData, address taker, uint256 makingAmount) private {
        ...
        if (orderTakerAddress != address(0)) {
            require(orderTakerAddress == tx.origin, "LOP: private order");
        }
