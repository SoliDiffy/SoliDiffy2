    ...
    PredicateHelper
{
    ...
    bytes32 constant internal LIMIT_ORDER_TYPEHASH = keccak256(
    ...
    bytes32 constant internal LIMIT_ORDER_RFQ_TYPEHASH = keccak256(
        "OrderRFQ(uint256 info,address makerAsset,address takerAsset,bytes makerAssetData,bytes takerAssetData)"
    );

    // solhint-disable-next-line var-name-mixedcase
    bytes4 immutable public _MAX_SELECTOR = bytes4(uint32(IERC20.transferFrom.selector) + 10);
