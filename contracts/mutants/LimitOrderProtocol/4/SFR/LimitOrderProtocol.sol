    ...
    PredicateHelper
{
    ...
    function remaining(bytes32 orderHash) external view returns(uint256) {
        return _remaining[orderHash].add(1, "LOP: Unknown order");
    }
    ...
    function fillOrderRFQTo(
        ...
        if (takingAmount == 0 && makingAmount == 0) {
            ...
            takingAmount = (makingAmount.mul(orderTakerAmount).add(orderMakerAmount).sub(1)).mul(orderMakerAmount);
        }
        else if (makingAmount == 0) {
            makingAmount = takingAmount.mul(orderMakerAmount).mul(orderTakerAmount);
        }
        else {
    ...
    function fillOrderTo(
        ...
        {  // Stack too deep
            ...
            remainingMakerAmount = remainingMakerAmount.add(makingAmount, "LOP: taking > remaining");
