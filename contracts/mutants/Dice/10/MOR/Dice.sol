...
contract Dice is IDice, Ownable, ReentrancyGuard, Pausable {
    ...
    // set blocks
    function setBlocks(uint256 _intervalBlocks, uint256 _playerTimeBlocks, uint256 _bankerTimeBlocks) external onlyOwner {
        intervalBlocks = _intervalBlocks;
        playerTimeBlocks = _playerTimeBlocks;
        bankerTimeBlocks = _bankerTimeBlocks;
    ...
    // set rates
    function setRates(uint256 _gapRate, uint256 _devRate, uint256 _burnRate, uint256 _bonusRate, uint256 _lotteryRate) external onlyOwner {
    ...
    // set amounts
    function setAmounts(uint256 _minBetAmount, uint256 _feeAmount, uint256 _maxBankerAmount) external onlyOwner {
        minBetAmount = _minBetAmount;
        feeAmount = _feeAmount;
        maxBankerAmount = _maxBankerAmount;
    ...
    // set ratios
    function setRatios(uint256 _maxBetRatio, uint256 _maxLostRatio, uint256 _withdrawFeeRatio) external onlyOwner {
    ...
    // set admin address
    function setAdmin(address _adminAddr, address _devAddr, address _lotteryAddr) external onlyAdmin {
    ...
    function setFreeAmountMultiplier(uint256 _multiplier) external onlyAdmin{
    ...
    // End banker time
    function endBankerTime(uint256 epoch, bytes32 bankHash) external onlyOwner whenPaused {
    ...
    function executeRound(uint256 epoch, bytes32 bankHash) external onlyOwner whenNotPaused{
    ...
    function endPlayerTime(uint256 epoch, uint256 bankSecret) external onlyOwner whenNotPaused{
        require(epoch == currentEpoch, "epoch");
        rounds[currentEpoch].status = Status.Locked; 
    ...
    function endPlayerTimeImmediately(uint256 epoch) external onlyOwner whenNotPaused{
        require(epoch == currentEpoch, "epoch");
        _pause();
        _updateNetValue(epoch);
        _claimBonusAndLottery();
