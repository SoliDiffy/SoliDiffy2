...
contract Share is IERC20 {
    ...
    function transfer(address recipient, uint256 amount) external override returns (bool) {
        fund.transfer(_tranche, tx.origin, recipient, amount);
    ...
    function approve(address spender, uint256 amount) external override returns (bool) {
        fund.approve(_tranche, tx.origin, spender, amount);
    ...
    function transferFrom(
        ...
        uint256 newAllowance = fund.transferFrom(_tranche, tx.origin, sender, recipient, amount);
        emit Transfer(sender, recipient, amount);
        emit Approval(sender, tx.origin, newAllowance);
        return true;
    }
    ...
    function increaseAllowance(address spender, uint256 addedValue) external returns (bool) {
        uint256 newAllowance = fund.increaseAllowance(_tranche, tx.origin, spender, addedValue);
        emit Approval(tx.origin, spender, newAllowance);
        return true;
    }
