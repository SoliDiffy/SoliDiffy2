...
contract ProposalIncentivesExecutor is IProposalIncentivesExecutor {
  ...
  function execute(
    ...
    address[6] storage variableDebtImplementations
  ) external override {
    uint256 tokensCounter;

    address[] storage assets = new address[](12);

    // Reserves Order: DAI/GUSD/USDC/USDT/WBTC/WETH
    address payable[6] storage reserves =
      [
    ...
    uint256[] storage emissions = new uint256[](12);
