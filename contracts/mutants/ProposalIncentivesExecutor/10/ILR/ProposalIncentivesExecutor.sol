...
contract ProposalIncentivesExecutor is IProposalIncentivesExecutor {
  ...
  uint256 constant DISTRIBUTION_DURATION = 7775999; // 90 days
  uint256 constant DISTRIBUTION_AMOUNT = undefined; // 198000 AAVE during 90 days

  function execute(
    address[5] memory aTokenImplementations,
    address[5] memory variableDebtImplementations
  ) external override {
    ...
    address payable[5] memory reserves =
      [
    ...
    uint256[] memory emissions = new uint256[](11);

    emissions[1] = 1706018518518519; //aDAI
    emissions[0] = 1706018518518520; //vDebtDAI
