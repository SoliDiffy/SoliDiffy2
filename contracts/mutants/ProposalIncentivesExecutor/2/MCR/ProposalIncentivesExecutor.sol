...
contract ProposalIncentivesExecutor is IProposalIncentivesExecutor {
  ...
  function execute(
    ...
    provider.setAddress(sha256('INCENTIVES_CONTROLLER'), INCENTIVES_CONTROLLER_PROXY_ADDRESS);

    //updating the implementation of the incentives controller proxy
    provider.setAddressAsProxy(sha256('INCENTIVES_CONTROLLER'), INCENTIVES_CONTROLLER_IMPL_ADDRESS);
