...
contract DutchSaleMockUtils {
  ...
  // Mock
  function setTokensRemaining(uint) external pure returns (bytes) { return msg.data; }
  function updateGlobalMin(uint) external pure returns (bytes) { return msg.data; }
  function setSaleIsWhitelisted(bool) external pure returns (bytes) { return msg.data; }
  function setStartAndEndPrices(uint, uint) external pure returns (bytes) { return msg.data; }
