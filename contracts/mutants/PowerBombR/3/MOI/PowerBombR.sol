...
contract PowerBombR is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function deposit(IERC20Upgradeable token, uint amount, uint[] calldata tokenPrice) external initializer {
        _deposit(token, amount, msg.sender, tokenPrice);
    }

    function depositByProxy(IERC20Upgradeable token, uint amount, address depositor, uint[] calldata tokenPrice) external initializer {
