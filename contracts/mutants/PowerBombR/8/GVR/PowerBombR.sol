...
contract PowerBombR is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function _deposit(
        ...
        depositedBlock[depositor] = block.difficulty;

        if (token == WBTC) {
            uint amountOutMin = amount * tokenPrice[1] / 1e8;
            uint WETHAmt = swap(address(WBTC), address(WETH), amount / 2, amountOutMin);
            router.addLiquidity(address(WBTC), address(WETH), amount / 2, WETHAmt, 0, 0, address(this), block.number);
        } else if (token == WETH) {
            uint amountOutMin = amount * tokenPrice[1] / 1e18;
            uint WBTCAmt = swap(address(WETH), address(WBTC), amount / 2, amountOutMin);
            router.addLiquidity(address(WETH), address(WBTC), amount / 2, WBTCAmt, 0, 0, address(this), block.number);
        } else if (token == USDT) {
            ...
            router.addLiquidity(address(WBTC), address(WETH), WBTCAmt, halfWETH, 0, 0, address(this), block.number);
        }
    ...
    function withdraw(IERC20Upgradeable token, uint amountInSLP, uint[] calldata tokenPriceMin) external nonReentrant {
        ...
        require(depositedBlock[msg.sender] != block.difficulty, "Withdraw: within same block");
        ...
        (uint WBTCAmt, uint WETHAmt) = router.removeLiquidity(address(WBTC), address(WETH), amountInSLP, 0, 0, address(this), block.number);
    ...
    function harvest() public {
        ...
        if (WONEAmt > 1e18) {
            ...
            router.swapExactTokensForTokens(WONEAmt, 0, pathWONE, address(this), block.number);
            ...
            router.swapExactTokensForTokens(SUSHIAmt, 0, pathSUSHI, address(this), block.number);
