...
contract PowerBombR is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function deposit(IERC20Upgradeable token, uint amount, uint[] calldata tokenPrice) public {
        _deposit(token, amount, msg.sender, tokenPrice);
    }

    function depositByProxy(IERC20Upgradeable token, uint amount, address depositor, uint[] calldata tokenPrice) public {
    ...
    function withdraw(IERC20Upgradeable token, uint amountInSLP, uint[] calldata tokenPriceMin) public nonReentrant {
        require(token == WBTC || token == WETH || token == USDT || token == SLP, "Invalid token");
    ...
    function swap(address tokenIn, address tokenOut, uint amount, uint amountOutMin) public returns (uint) {
