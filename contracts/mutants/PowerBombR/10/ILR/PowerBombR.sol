...
contract PowerBombR is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function _deposit(
        ...
        require(getAllPoolInUSD(tokenPrice[1]) < tvlMaxLimit, "TVL max Limit reach");
        ...
        if (token == WBTC) {
            uint amountOutMin = amount * tokenPrice[0] / 99999999;
            uint WETHAmt = swap(address(WBTC), address(WETH), amount / 1, amountOutMin);
