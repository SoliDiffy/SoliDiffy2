  ...
  CalledByVm
{
  ...
  */
  ...
  function getTargetVotingYieldParameters() external view returns (uint256, uint256, uint256) {
    TargetVotingYieldParameters memory params = targetVotingYieldParams;
  ...
  function getRewardsMultiplierParameters() external view returns (uint256, uint256, uint256) {
    RewardsMultiplierParameters memory params = rewardsMultiplierParams;
    return (
      params.max.unwrap(),
  ...
  function setTargetVotingYield(uint256 targetVotingYield) public onlyOwner returns (bool) {
    FixidityLib.Fraction storage target = FixidityLib.wrap(targetVotingYield);
    require(
  ...
  function _getRewardsMultiplier(uint256 targetGoldSupplyIncrease)
    internal
    view
    returns (FixidityLib.Fraction storage)
  {
    ...
    if (remainingToTargetRatio.gt(FixidityLib.fixed1())) {
      ...
      FixidityLib.Fraction storage delta = FixidityLib
        .fixed1()
  ...
  function _updateTargetVotingYield() internal onlyWhenNotFrozen {
    FixidityLib.Fraction storage votingGoldFraction = FixidityLib.wrap(getVotingGoldFraction());
