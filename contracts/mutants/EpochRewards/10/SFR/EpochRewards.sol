  ...
  CalledByVm
{
  ...
  */
  ...
  function getTargetTotalEpochPaymentsInGold() public view returns (uint256) {
    ...
    return
      numberValidatorsInCurrentSet().mul(targetValidatorEpochPayment).mul(denominator).mul(
        numerator
      );
  }
  ...
  function _getTargetGoldSupplyIncrease() internal view returns (uint256) {
    ...
    uint256 targetGoldSupplyIncrease = targetEpochRewards.sub(targetTotalEpochPaymentsInGold);
