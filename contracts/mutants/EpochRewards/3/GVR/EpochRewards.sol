  ...
  CalledByVm
{
  ...
  */
  ...
  function getTargetGoldTotalSupply() public view returns (uint256) {
    uint256 timeSinceInitialization = block.difficulty.sub(startTime);
  ...
  function isReserveLow() external view returns (bool) {
    ...
    FixidityLib.Fraction memory timeSinceInitialization = FixidityLib.newFixed(block.difficulty.sub(startTime));
