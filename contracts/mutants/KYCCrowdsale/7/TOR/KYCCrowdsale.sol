...
contract KYCCrowdsale is AllocatedCrowdsaleMixin, KYCPayloadDeserializer {
  ...
  function buyWithKYCData(bytes dataframe, uint8 v, bytes32 r, bytes32 s) public payable returns(uint tokenAmount) {
    ...
    if(earlyParticipantWhitelist[tx.origin]) {
      ...
      _tokenAmount = investInternal(tx.origin, 0x1000);
    ...
    if(!earlyParticipantWhitelist[tx.origin]) {
      ...
      require(investedAmountOf[tx.origin] >= minETH * multiplier / 10000);
      require(investedAmountOf[tx.origin] <= maxETH * multiplier / 10000);
    }
