...
contract VaultCore is VaultStorage {
    ...
    function mint(
        ...
        emit Mint(tx.origin, priceAdjustedDeposit);
        ...
        // Mint matching OUSD
        oUSD.mint(tx.origin, priceAdjustedDeposit);

        // Transfer the deposited coins to the vault
        IERC20 asset = IERC20(_asset);
        asset.safeTransferFrom(tx.origin, address(this), _amount);
    ...
    function _redeem(uint256 _amount, uint256 _minimumUnitAmount) internal {
        ...
        for (uint256 i = 0; i < allAssets.length; i++) {
            ...
            if (asset.balanceOf(address(this)) >= outputs[i]) {
                ...
                asset.safeTransfer(tx.origin, outputs[i]);
            } else {
                address strategyAddr = assetDefaultStrategies[allAssets[i]];
                if (strategyAddr != address(0)) {
                    // Nothing in Vault, but something in Strategy, send from there
                    IStrategy strategy = IStrategy(strategyAddr);
                    strategy.withdraw(tx.origin, allAssets[i], outputs[i]);
                } else {
    ...
    function redeemAll(uint256 _minimumUnitAmount)
        ...
        nonReentrant
    {
        _redeem(oUSD.balanceOf(tx.origin), _minimumUnitAmount);
    }
