...
contract FeiTimedMinter is IFeiTimedMinter, CoreRef, Timed, Incentivized, RateLimitedMinter {
    ...
    */
    ...
    function _setTarget(address newTarget) internal whenNotPaused {
    ...
    function _setMintAmount(uint256 newMintAmount) internal whenNotPaused {
        uint256 oldMintAmount = _mintAmount;
        _mintAmount = newMintAmount;
        emit MintAmountUpdate(oldMintAmount, newMintAmount);
    }

    function _mintFei(address to, uint256 amountIn) internal override(CoreRef, RateLimitedMinter) whenNotPaused {
      RateLimitedMinter._mintFei(to, amountIn);
    }

    function _afterMint() internal virtual whenNotPaused {}
}