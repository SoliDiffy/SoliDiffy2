...
contract CollateralizationOracle is ICollateralizationOracle, CoreRef {
    ...
    function _addDeposits(address[] memory _deposits) internal onlyGuardianOrGovernor {
        for (uint256 i = 0; i < _deposits.length; i++) {
            _addDeposit(_deposits[i]);
        }
    }

    function _addDeposit(address _deposit) internal onlyGuardianOrGovernor {
    ...
    function _removeDeposit(address _deposit) internal onlyGuardianOrGovernor {
        // get the token in which the deposit reports its token
    ...
    function _setOracles(address[] memory _tokens, address[] memory _oracles) internal onlyGuardianOrGovernor {
    ...
    function _setOracle(address _token, address _newOracle) internal onlyGuardianOrGovernor {
