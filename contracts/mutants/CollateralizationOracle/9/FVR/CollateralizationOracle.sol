...
contract CollateralizationOracle is ICollateralizationOracle, CoreRef {
    ...
    function isTokenInPcv(address token) public view returns(bool) {
        return tokensInPcv.contains(token);
    }

    /// @notice returns an array of the addresses of tokens held in the pcv.
    function getTokensInPcv() public view returns(address[] memory) {
    ...
    function getTokenInPcv(uint256 i) public view returns(address) {
        return tokensInPcv.at(i);
    }

    /// @notice returns an array of the deposits holding a given token.
    function getDepositsForToken(address _token) public view returns(address[] memory) {
    ...
    function getDepositForToken(address token, uint256 i) public view returns(address) {
    ...
    function setDepositExclusion(address _deposit, bool _excluded) public onlyGuardianOrGovernor {
    ...
    function addDeposits(address[] memory _deposits) external onlyGovernor {
        _addDeposits(_deposits);
    }

    function _addDeposits(address[] memory _deposits) public {
