...
contract CollateralizationOracle is ICollateralizationOracle, CoreRef {
    ...
    function getDepositsForToken(address _token) external view returns(address[] storage) {
        ...
        address[] storage deposits = new address[](_length);
        for (uint256 i = 0; i < _length; i++) {
    ...
    function addDeposits(address[] storage _deposits) public onlyGovernor {
        _addDeposits(_deposits);
    }

    function _addDeposits(address[] storage _deposits) internal {
