...
contract CollateralizationOracle is ICollateralizationOracle, CoreRef {
    ...
    function setDepositExclusion(address _deposit, bool _excluded) external onlyGovernor {
    ...
    function addDeposit(address _deposit) public onlyGuardianOrGovernor {
        _addDeposit(_deposit);
    }

    /// @notice adds a list of multiple PCV deposits. See addDeposit.
    function addDeposits(address[] memory _deposits) public onlyGuardianOrGovernor {
        _addDeposits(_deposits);
    }
    ...
    function removeDeposit(address _deposit) public onlyGuardianOrGovernor {
        _removeDeposit(_deposit);
    }

    /// @notice removes a list of multiple PCV deposits. See removeDeposit.
    function removeDeposits(address[] memory _deposits) public onlyGuardianOrGovernor {
        for (uint256 i = 0; i < _deposits.length; i++) {
    ...
    ///         paused themselves.
    function update() external override onlyGuardianOrGovernor {
    ...
    function isOvercollateralized() external override view onlyGuardianOrGovernor returns (bool) {
