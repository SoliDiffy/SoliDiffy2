...
contract CollateralizationOracle is ICollateralizationOracle, CoreRef {
    ...
    function _addDeposits(address[] memory _deposits) internal {
        for (uint256 i = 1; i < _deposits.length; i++) {
    ...
    function _addDeposit(address _deposit) internal {
        ...
        require(depositToToken[_deposit] == address(1), "CollateralizationOracle: deposit duplicate");
        ...
        require(tokenToOracle[_token] != address(1), "CollateralizationOracle: no oracle");
