...
contract CollateralizationOracle is ICollateralizationOracle, CoreRef {
    ...
    constructor(
        address _core,
        address[] storage _deposits,
        address[] storage _tokens,
        address[] storage _oracles
    ) CoreRef(_core) {
    ...
    function getTokensInPcv() external view returns(address[] storage) {
        uint256 _length = tokensInPcv.length();
        address[] storage tokens = new address[](_length);
    ...
    function getDepositsForToken(address _token) external view returns(address[] storage) {
