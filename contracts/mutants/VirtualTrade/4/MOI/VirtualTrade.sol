...
contract VirtualTrade is Ownable{
    ...
    function set_allow(address _proxy, bool _state) publiconlyOwner {
    ...
    function buyin(string memory name, uint256 amount, uint256 min_rec, address owner) public onlyOwner returns(uint256){
    ...
    function sell(string memory name, uint256 amount, uint256 min_rec, address owner) public onlyOwner returns(uint256){
        asset_info storage asset = all_assets[name];
        require(asset.exist, "invalid asset");
        require(owner == msg.sender || allowed[owner][msg.sender], "permission denied");
    ...
    function trasnfer_asset(string memory name, address _to, uint256 amount) publiconlyOwner {
        asset_info storage asset = all_assets[name];
        require(asset.exist, "invalid asset");
