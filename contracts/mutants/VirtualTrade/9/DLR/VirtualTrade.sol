...
contract OracleInterface{
   function get_asset_price(string storage _name) public view returns(uint256);
   function add_asset(string storage _name, address type_addr, bytes storage call_data) public;
}
...
contract VirtualTrade is Ownable{
    ...
    function buyin(string storage name, uint256 amount, uint256 min_rec, address owner) public returns(uint256){
        ...
        asset_info memory asset = all_assets[name];
    ...
    function sell(string storage name, uint256 amount, uint256 min_rec, address owner) public returns(uint256){
        asset_info memory asset = all_assets[name];
        require(asset.exist, "invalid asset");
        require(owner == msg.sender || allowed[owner][msg.sender], "permission denied");
    ...
    function trasnfer_asset(string storage name, address _to, uint256 amount) public{
        asset_info memory asset = all_assets[name];
        require(asset.exist, "invalid asset");
