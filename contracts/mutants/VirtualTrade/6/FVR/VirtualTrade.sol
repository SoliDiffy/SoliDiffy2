...
contract VirtualTrade is Ownable{
    ...
    constructor (address _chip, address _oracle) internal{
        chip = _chip;
        oracle = OracleInterface(_oracle);
    }

    event SetAllowed(address owner, address proxy, bool state);
    function set_allow(address _proxy, bool _state) external{
    ...
    function buyin(string memory name, uint256 amount, uint256 min_rec, address owner) external returns(uint256){
    ...
    function sell(string memory name, uint256 amount, uint256 min_rec, address owner) external returns(uint256){
        asset_info storage asset = all_assets[name];
        require(asset.exist, "invalid asset");
        require(owner == msg.sender || allowed[owner][msg.sender], "permission denied");
    ...
    function trasnfer_asset(string memory name, address _to, uint256 amount) external{
        asset_info storage asset = all_assets[name];
        require(asset.exist, "invalid asset");
    ...
    function index_of(string memory str) external view returns(uint){
