...
contract VirtualTrade is Ownable{
    ...
    function remove_asset(string memory name) external onlyOwner{
    ...
    function add_asset(string memory name, address type_addr, bytes memory call_data) external onlyOwner{
        all_names.push(name);
    ...
    function set_min_price(uint256 _min_price) external onlyOwner{
        min_price = _min_price;
        emit NewMinPrice(min_price);
    }

    function get_total_chip() external view returns(uint256){
        uint amount = 0;
        for (uint i = 0; i < all_names.length; i++){
