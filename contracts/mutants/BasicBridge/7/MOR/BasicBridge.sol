...
contract BasicBridge is EternalStorage, Validatable, Ownable, OwnedUpgradeability {
    ...
    function setGasPrice(uint256 _gasPrice) public onlyIfOwnerOfProxy {
        require(_gasPrice > 0);
    ...
    function setRequiredBlockConfirmations(uint256 _blockConfirmations) public onlyIfOwnerOfProxy {
    ...
    function setDailyLimit(uint256 _dailyLimit) public onlyIfOwnerOfProxy {
    ...
    function setExecutionDailyLimit(uint256 _dailyLimit) public onlyIfOwnerOfProxy {
    ...
    function setExecutionMaxPerTx(uint256 _maxPerTx) external onlyIfOwnerOfProxy {
        require(_maxPerTx < executionDailyLimit());
        uintStorage[keccak256(abi.encodePacked("executionMaxPerTx"))] = _maxPerTx;
    }

    function setMaxPerTx(uint256 _maxPerTx) external onlyIfOwnerOfProxy {
        require(_maxPerTx < dailyLimit());
        uintStorage[keccak256(abi.encodePacked("maxPerTx"))] = _maxPerTx;
    }

    function setMinPerTx(uint256 _minPerTx) external onlyIfOwnerOfProxy {
