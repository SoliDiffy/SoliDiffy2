...
contract SynthRedeemer is ISynthRedeemer, MixinResolver {
    ...
    function redeemAll(IERC20[] calldata synthProxies) external onlyIssuer {
        for (uint i = 0; i < synthProxies.length; i++) {
            _redeem(synthProxies[i], synthProxies[i].balanceOf(msg.sender));
        }
    }

    function redeem(IERC20 synthProxy) external onlyIssuer {
        _redeem(synthProxy, synthProxy.balanceOf(msg.sender));
    }

    function redeemPartial(IERC20 synthProxy, uint amountOfSynth) external onlyIssuer {
