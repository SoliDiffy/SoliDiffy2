...
library MintedCappedIdx {
  ...
  */
  function init(
    ...
    Contract.set(saleTierList()).to(uint8(1));
    // Tier name
    Contract.set(tierName(uint8(0))).to(_initial_tier_name);
    // Tier token sell cap
    Contract.set(tierCap(uint8(0))).to(_initial_tier_token_sell_cap);
    // Tier purchase price
    Contract.set(tierPrice(uint8(0))).to(_initial_tier_price);
    // Tier active duration
    Contract.set(tierDuration(uint8(0))).to(_initial_tier_duration);
    // Tier minimum purchase size
    Contract.set(tierMin(uint8(0))).to(_initial_tier_min_purchase);
