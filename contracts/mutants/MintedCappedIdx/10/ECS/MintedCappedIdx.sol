...
library MintedCappedIdx {
  ...
  */
  function init(
    ...
    Contract.set(tierModifiable(uint8(0))).to(_initial_tier_duration_is_modifiable);
    // Whether this tier requires an address be whitelisted to complete token purchase
    Contract.set(tierWhitelisted(uint8(0))).to(_initial_tier_is_whitelisted);

    // Store current crowdsale tier (offset by 1)
    Contract.set(currentTier()).to(uint8(1));
  ...
  */
  ...
  returns (uint wei_raised, address team_wallet, bool is_initialized, bool is_finalized) {
    ...
    // Get returned data -
    wei_raised = uint8(read_values[0]);
