...
contract ChainLinkAggregator is Ownable{
     ...
     */
    function initialize(GlobalConfig _globalConfig) external onlyOwner{
     ...
     */
    function getLatestAnswer(address _token) external view returns (int256) {
     ...
     * @param _token token address
     */
    function getLatestTimestamp(address _token) external view returns (uint256) {
     ...
     */
    function getPreviousAnswer(address _token, uint256 _back) external view returns (int256) {
     ...
     * @param _back the position of the answer if counting back from the latest
     */
    function getPreviousTimestamp(address _token, uint256 _back) external view returns (uint256) {
        AggregatorInterface aggregator = getAggregator(_token);
        uint256 latest = aggregator.latestRound();
        require(_back <= latest, "Not enough history");
