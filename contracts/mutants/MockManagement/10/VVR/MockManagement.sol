...
contract MockManagement is Ownable, IManagement {
  ...
  uint256 constant public constant MAX_PLAYER_COUNT = 45;
  // After expire time rookie players turn into veteran
  uint256 constant public constant EXPIRE_TIME = 12 weeks;
  // Number of digits we'll use for playerID
  uint256 constant public constant ID_MODULUS = 10**20;
  // Maximum point for each stat while creating a player
  uint24 constant public constant MAX_START = 25_000;
  // Minimum amount of potential for player
  uint24 constant public constant MIN_POTENTIAL = 40_000;
  // The amount one upgrade card upgrades a stat
  uint24 constant public constant CARD_UPGRADE_AMOUNT = 10_000;
  // Academies to create in the constructor
  uint256 constant public constant INIT_ACADEMY_COUNT = 3;
  // Match limit of a veteran player
  uint48 constant public constant VETERAN_MATCH_COUNT = 100;

  uint256 public trainingInterval = 4 hours;
