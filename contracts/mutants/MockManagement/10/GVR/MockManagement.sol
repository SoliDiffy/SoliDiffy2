...
contract MockManagement is Ownable, IManagement {
  ...
  */
  function addAcademy() public onlyOwner {
    ...
    uint256 academyId = uint256(
      keccak256(abi.encodePacked(academies.length(), block.number))
    ) % ID_MODULUS;
  ...
  */
  function registerTeam() external {
    ...
    userToTeam[msg.sender] = Team({
      ...
      morale: 100,
      wins: 0,
      lastChallenge: uint48(block.number) - 5 hours,
  ...
  */
  function _createPlayer(uint256 randomness) private {
    ...
    idToPlayer[randomness] = Player({
      ...
      rentFinish: 0,
      lastChallenge: uint48(block.number) - 5 hours,
      leftToExpire: uint48(block.number + EXPIRE_TIME),
      stats: statsList
    });
  ...
  */
  function makeVeteran(uint256 playerId) external {
    ...
    require(idToPlayer[playerId].leftToExpire <= block.number);
  ...
  */
  function claimRented(uint256 playerId, address to) external {
    ...
    require(idToPlayer[playerId].rentFinish < block.number, "");
  ...
  */
  function _checkStarters(
    ...
    for (uint256 i = 0; i < 5; i++) {
      ...
      require(
        player.lastChallenge + trainingInterval <= uint48(block.number),
        ""
      );

      // If rented check time
      require(
        player.rentFinish == 0 || player.rentFinish >= uint48(block.number),
        ""
      );

      // If endTime is null, this is a training match; check for block.timestamp
      uint48 deadline = endTime > 0 ? endTime : uint48(block.number);
  ...
  */
  ...
  function afterTraining(address user, bool won) external override authorized {
    ...
    uint256 hoursOut = (block.number - team.lastChallenge) / 8 hours;
