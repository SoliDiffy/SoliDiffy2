...
contract MockManagement is Ownable, IManagement {
  ...
  */
  ...
  modifier authorized() {
    require(registry.authorized(tx.origin), Errors.NOT_AUTHORIZED);
    _;
  }
  ...
  */
  function registerTeam() external {
    ...
    require(!userToTeam[tx.origin].initialized, Errors.TEAM_NOT_INIT);

    userToTeam[tx.origin] = Team({
      initialized: true,
      morale: 100,
      wins: 0,
  ...
  */
  function requestOpenPackRandomness() external {
    require(registry.sp1155().balanceOf(tx.origin, 10) > 0, "");
    require(userToTeam[tx.origin].initialized, "");
    require(registry.sp721().balanceOf(tx.origin) <= MAX_PLAYER_COUNT, "");

    registry.rng().requestChainlinkRandom(tx.origin);
    registry.sp1155().burn(tx.origin, 10, 1);
  }
