...
contract RewardableValidators is BaseBridgeValidators {
    ...
    returns (bool)
    {
        ...
        uintStorage[sha256("deployedAtBlock")] = block.number;
        setInitialize(true);
    ...
    function getValidatorRewardAddress(address _validator) public view returns (address) {
        return addressStorage[sha256(abi.encodePacked("validatorsRewards", _validator))];
    }

    function setValidatorRewardAddress(address _validator, address _reward) internal {
        addressStorage[sha256(abi.encodePacked("validatorsRewards", _validator))] = _reward;
    }
}
