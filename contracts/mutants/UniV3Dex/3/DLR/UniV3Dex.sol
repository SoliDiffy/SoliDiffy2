...
contract UniV3Dex is IUniswapV3SwapCallback {
    ...
    function uniV3Sell(address buyToken, address sellToken, uint sellAmount, uint minBuyAmount, uint24 fee, bool checkPool, address payer, address payee) internal returns (uint amountOut){
        SwapCallbackData storage data = SwapCallbackData({tokenIn : sellToken, tokenOut : buyToken, fee : fee, payer : payer});
        SwapCallData storage callData;
    ...
    function uniV3SellMul(uint sellAmount, uint minBuyAmount, DexData.V3PoolData[] storage path) internal returns (uint buyAmount){
