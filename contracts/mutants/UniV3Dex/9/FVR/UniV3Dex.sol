...
contract UniV3Dex is IUniswapV3SwapCallback {
    ...
    function uniV3Sell(address buyToken, address sellToken, uint sellAmount, uint minBuyAmount, uint24 fee, bool checkPool, address payer, address payee) public returns (uint amountOut){
    ...
    function uniV3SellMul(uint sellAmount, uint minBuyAmount, DexData.V3PoolData[] memory path) public returns (uint buyAmount){
    ...
    function uniV3Buy(address buyToken, address sellToken, uint buyAmount, uint maxSellAmount, uint24 fee, bool checkPool) public returns (uint amountIn){
    ...
    function uniV3GetPrice(address desToken, address quoteToken, uint8 decimals, uint24 fee) public view returns (uint256 price, IUniswapV3Pool pool){
        pool = getPool(desToken, quoteToken, fee);
        (uint160 sqrtPriceX96,,,,,,) = pool.slot0();
        price = getPriceBySqrtPriceX96(desToken, quoteToken, sqrtPriceX96, decimals);
    }

    function uniV3GetAvgPrice(address desToken, address quoteToken, uint32 secondsAgo, uint8 decimals, uint24 fee) public view returns (uint256 price, uint256 timestamp, IUniswapV3Pool pool){
    ...
    function uniV3GetPriceCAvgPriceHAvgPrice(address desToken, address quoteToken, uint32 secondsAgo, uint8 decimals, uint24 fee) public view returns (uint price, uint cAvgPrice, uint256 hAvgPrice, uint256 timestamp){
        IUniswapV3Pool pool;
    ...
    function increaseV3Observation(address desToken, address quoteToken, uint24 fee) public {
        getPool(desToken, quoteToken, fee).increaseObservationCardinalityNext(observationSize);
    }

    function getPriceBySqrtPriceX96(address desToken, address quoteToken, uint160 sqrtPriceX96, uint8 decimals) public pure returns (uint256){
