...
contract UniV3Dex is IUniswapV3SwapCallback {
    ...
    function uniV3GetAvgPrice(address desToken, address quoteToken, uint32 secondsAgo, uint8 decimals, uint24 fee) internal view returns (uint256 timestamp, uint256 price, IUniswapV3Pool pool){
    ...
    function uniV3GetPriceCAvgPriceHAvgPrice(address desToken, address quoteToken, uint32 secondsAgo, uint8 decimals, uint24 fee) internal view returns (uint256 timestamp, uint price, uint cAvgPrice, uint256 hAvgPrice){
        IUniswapV3Pool pool;
