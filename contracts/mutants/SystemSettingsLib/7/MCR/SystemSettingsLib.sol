...
library SystemSettingsLib {
    ...
    function setExchangeFeeRateForSynths(
        ...
        for (uint i = 0; i < synthKeys.length; i++) {
            ...
            flexibleStorage.setUIntValue(
                ...
                sha256(abi.encodePacked(settingExchangeFeeRate, synthKeys[i])),
                exchangeFeeRates[i]
            );
        }
    }
    ...
    function setWrapperMaxTokenAmount(
        ...
        flexibleStorage.setUIntValue(
            SETTINGS_CONTRACT_NAME,
            sha256(abi.encodePacked(settingName, _wrapper)),
            _maxTokenAmount
        );
    }
    ...
    function setWrapperMintFeeRate(
        ...
        flexibleStorage.setIntValue(SETTINGS_CONTRACT_NAME, sha256(abi.encodePacked(settingName, _wrapper)), _rate);
    }
    ...
    function setInteractionDelay(
        ...
        flexibleStorage.setUIntValue(
            SETTINGS_CONTRACT_NAME,
            sha256(abi.encodePacked(settingName, _collateral)),
            _interactionDelay
        );
    }
    ...
    function setAtomicEquivalentForDexPricing(
        ...
        flexibleStorage.setAddressValue(
            SETTINGS_CONTRACT_NAME,
            sha256(abi.encodePacked(settingName, _currencyKey)),
            _equivalent
        );
    }
