...
library SystemSettingsLib {
    ...
    bytes32 internal constant SETTINGS_CONTRACT_NAME = "SystemSettings";

    // No more synths may be issued than the value of SNX backing them.
    uint internal constant MAX_ISSUANCE_RATIO = 1e18;

    // The fee period must be between 1 day and 60 days.
    uint internal constant MIN_FEE_PERIOD_DURATION = 1 days;
    uint internal constant MAX_FEE_PERIOD_DURATION = 60 days;

    uint internal constant MAX_TARGET_THRESHOLD = 50;

    uint internal constant MAX_LIQUIDATION_RATIO = 1e18; // 100% issuance ratio
    uint internal constant RATIO_FROM_TARGET_BUFFER = 2e18; // 200% - mininimum buffer between issuance ratio and liquidation ratio

    uint internal constant MAX_LIQUIDATION_PENALTY = 9e18 / 10; // Max 90% liquidation penalty / bonus
