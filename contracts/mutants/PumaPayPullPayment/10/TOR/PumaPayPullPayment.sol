...
contract PumaPayPullPayment is PayableOwnable {
    ...
    public
    paymentExists(_customer, tx.origin)
    isValidPullPaymentExecutionRequest(_customer, tx.origin, _paymentID)
    {
        uint256 amountInPMA;

        if (pullPayments[_customer][tx.origin].initialPaymentAmountInCents > 0) {
            amountInPMA = calculatePMAFromFiat(
                pullPayments[_customer][tx.origin].initialPaymentAmountInCents,
                pullPayments[_customer][tx.origin].currency
            );
