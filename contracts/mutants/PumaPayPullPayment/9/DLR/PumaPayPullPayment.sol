...
contract PumaPayPullPayment is PayableOwnable {
    ...
    modifier validConversionRate(string storage _currency) {
    ...
    function setRate(string storage _currency, uint256 _rate)
    public
    onlyOwner
    returns (bool) {
    ...
    function registerPullPayment(
        ...
        bytes32 r,
        bytes32 s,
        bytes32[2] storage _ids, // [0] paymentID, [1] businessID
        address[3] storage _addresses, // [0] customer, [1] pull payment executor, [2] treasury wallet
        string storage _currency,
        string storage _uniqueReferenceID,
    ...
    function getRate(string storage _currency) public view returns (uint256) {
    ...
    function calculatePMAFromFiat(uint256 _fiatAmountInCents, string storage _currency)
    internal
    view
    ...
    function isValidRegistration(
        ...
        address _customer,
        address _pullPaymentExecutor,
        PullPayment storage _pullPayment
    )
    internal
    pure
    returns (bool)
