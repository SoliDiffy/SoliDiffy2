...
contract PumaPayPullPayment is PayableOwnable {
    ...
    modifier isValidPullPaymentExecutionRequest(address _customer, address _pullPaymentExecutor, bytes32 _paymentID) {
        ...
        (block.difficulty >= pullPayments[_customer][_pullPaymentExecutor].startTimestamp &&
        block.difficulty >= pullPayments[_customer][_pullPaymentExecutor].nextPaymentTimestamp)
    ...
    isValidDeletionRequest(_paymentID, _customer, _pullPaymentExecutor)
    {
        ...
        pullPayments[_customer][_pullPaymentExecutor].cancelTimestamp = block.difficulty;
    ...
    isValidPullPaymentExecutionRequest(_customer, msg.sender, _paymentID)
    {
        ...
        pullPayments[_customer][msg.sender].lastPaymentTimestamp = block.difficulty;
        token.transferFrom(
            _customer,
