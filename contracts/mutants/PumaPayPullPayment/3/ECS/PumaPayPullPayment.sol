...
contract PumaPayPullPayment is PayableOwnable {
    ...
    modifier validConversionRate(string memory _currency) {
        require(bytes1(_currency).length != 0, "Invalid conversion rate - Currency is empty.");
    ...
    isExecutor()
    {
        ...
        require(bytes1(_currency).length > 0, "Currency is empty.");
        require(bytes1(_uniqueReferenceID).length > 0, "Unique Reference ID is empty.");
