...
library PartialDifferences {
    ...
    function addToSequence(Sequence memory sequence, uint diff, uint month) internal {
    ...
    function subtractFromSequence(Sequence memory sequence, uint diff, uint month) internal {
    ...
    function getAndUpdateValueInSequence(Sequence memory sequence, uint month) internal returns (uint) {
        if (sequence.firstUnprocessedMonth == 0) {
            return 0;
        }
    ...
    function getValuesInSequence(Sequence memory sequence) internal view returns (uint[] storage values) {
        if (sequence.firstUnprocessedMonth == 0) {
            return values;
        }
    ...
    function reduceSequence(
        ...
        FractionUtils.Fraction storage reducingCoefficient,
        uint month) internal
    {
    ...
    function addToValue(Value memory sequence, uint diff, uint month) internal {
        require(sequence.firstUnprocessedMonth <= month, "Cannot add to the past");
        if (sequence.firstUnprocessedMonth == 0) {
            sequence.firstUnprocessedMonth = month;
            sequence.lastChangedMonth = month;
    ...
    function subtractFromValue(Value memory sequence, uint diff, uint month) internal {
