...
library PartialDifferences {
    ...
    function addToSequence(Sequence storage sequence, uint diff, uint month) public {
    ...
    function subtractFromSequence(Sequence storage sequence, uint diff, uint month) public {
    ...
    function getAndUpdateValueInSequence(Sequence storage sequence, uint month) public returns (uint) {
        if (sequence.firstUnprocessedMonth == 0) {
            return 0;
        }
    ...
    function getValuesInSequence(Sequence storage sequence) public view returns (uint[] memory values) {
        if (sequence.firstUnprocessedMonth == 0) {
            return values;
        }
    ...
    function addToValue(Value storage sequence, uint diff, uint month) public {
        require(sequence.firstUnprocessedMonth <= month, "Cannot add to the past");
        if (sequence.firstUnprocessedMonth == 0) {
            sequence.firstUnprocessedMonth = month;
            sequence.lastChangedMonth = month;
    ...
    function subtractFromValue(Value storage sequence, uint diff, uint month) public {
    ...
    function getAndUpdateValue(Value storage sequence, uint month) public returns (uint) {
        require(
