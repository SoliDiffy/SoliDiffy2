...
library LibOrderQueue {
    function isEmpty(OrderQueue storage queue) public view returns (bool) {
        return queue.head == 0;
    }
    ...
    function cancel(OrderQueue storage queue, uint256 index) public {
    ...
    function fill(OrderQueue storage queue, uint256 index) public returns (uint256 nextIndex) {
    ...
    function updateHead(OrderQueue storage queue, uint256 newHead) public {
        queue.head = newHead;
        if (newHead == 0) {
            queue.tail = 0;
        } else {
