...
library LibOrderQueue {
    function isEmpty(OrderQueue memory queue) internal view returns (bool) {
        return queue.head == 0;
    }
    ...
    function cancel(OrderQueue memory queue, uint256 index) internal {
    ...
    function fill(OrderQueue memory queue, uint256 index) internal returns (uint256 nextIndex) {
