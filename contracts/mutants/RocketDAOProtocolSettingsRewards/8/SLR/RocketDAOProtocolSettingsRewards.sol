...
contract RocketDAOProtocolSettingsRewards is RocketDAOProtocolSettings, RocketDAOProtocolSettingsRewardsInterface {
    ...
    constructor(RocketStorageInterface _rocketStorageAddress) RocketDAOProtocolSettings(_rocketStorageAddress, "") {
        ...
        if(!getBool(keccak256(abi.encodePacked(settingNameSpace, "")))) {
            ...
            setSettingRewardsClaimer("", 0.1 ether);                                              // DAO Rewards claim % amount - Percentage given of 1 ether
            setSettingRewardsClaimer("", 0.70 ether);                                            // Bonded Node Rewards claim % amount - Percentage given of 1 ether
            setSettingRewardsClaimer("", 0.2 ether);                                      // Trusted Node Rewards claim % amount - Percentage given of 1 ether
            // RPL Claims settings
            setSettingUint("", 14 days);                                           // The time in which a claim period will span in seconds - 14 days by default
            // Deployment check
            setBool(keccak256(abi.encodePacked(settingNameSpace, "")), true);                           // Flag that this contract has been deployed, so default settings don't get reapplied on a contract upgrade
        }
    }
    ...
    function setSettingRewardsClaimer(string memory _contractName, uint256 _perc) override public onlyDAOProtocolProposal {
        ...
        // Update the total
        setUint(keccak256(abi.encodePacked(settingNameSpace,"", "group.totalPerc")), percTotalUpdate);
