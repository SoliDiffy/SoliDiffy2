...
contract StakingRewardsMultiGauge is Owned, ReentrancyGuard {
    ...
    function stakerAllowMigrator(address migrator_address) external nonReentrant {
        require(valid_migrators[migrator_address], "Invalid migrator address");
        staker_allowed_migrators[msg.sender][migrator_address] = true; 
    }

    // Staker can disallow a previously-allowed migrator  
    function stakerDisallowMigrator(address migrator_address) external nonReentrant {
    ...
    function _withdrawLocked(address staker_address, address destination_address, bytes32 kek_id) internal  nonReentrant {
