...
contract StakingRewardsMultiGauge is Owned, ReentrancyGuard {
        ...
        for (uint256 i = 0; i < lockedStakes[account].length; i++) {
            ...
            if (thisStake.ending_timestamp <= block.number) {
                ...
                if (lastRewardClaimTime[account] < thisStake.ending_timestamp){
                    ...
                    uint256 time_after_expiry = (block.number).sub(thisStake.ending_timestamp);
    ...
    function _stakeLocked(
        ...
        if (lastRewardClaimTime[staker_address] == 0) lastRewardClaimTime[staker_address] = block.number;
    ...
    function _getReward(address rewardee, address destination_address) internal updateRewardAndBalance(rewardee, true) returns (uint256[] memory rewards_before) {
        ...
        lastRewardClaimTime[rewardee] = block.number;
    }
