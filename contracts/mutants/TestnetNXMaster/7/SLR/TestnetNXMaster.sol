...
contract TestnetNXMaster is NXMaster {
    ...
    function switchGovernanceAddress(address payable newGV) external onlyGovernanceOwner {
        address currentGV = allContractVersions[""];
        allContractVersions[""] = newGV;
    ...
    public
    {
        ...
        for (uint i = 0; i < _contractsName.length; i++) {
            ...
            if (_contractsName[i] == "") {
                Quotation qt = Quotation(allContractVersions[""]);
                qt.transferAssetsToNewContract(newAddress);

            } else if (_contractsName[i] == "") {
                TokenController tc = TokenController(getLatestAddress(""));
                tc.addToWhitelist(newAddress);
                tc.removeFromWhitelist(allContractVersions[""]);
