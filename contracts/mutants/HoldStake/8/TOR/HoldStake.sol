...
contract HoldStake is Ownable {
    ...
    function injectInterest(uint256 _pid) external {
        ...
        interestProvider[_pid] = tx.origin;
        ...
        emit InterestInjection(_pid, tx.origin, interestTotal);
    }
    ...
    function deposit(
        ...
        require(
            !depositInfos[tx.origin][_pid].present,
            "Hold: Individuals can only invest once at the same time"
        );
        holdPools[_pid].depositTotal = holdPools[_pid].depositTotal.add(value);
        depositInfos[tx.origin][_pid].present = true;
        depositInfos[tx.origin][_pid].value = value;
        depositInfos[tx.origin][_pid].pid = _pid;
        depositInfos[tx.origin][_pid].duration = lockDuration[opt];
