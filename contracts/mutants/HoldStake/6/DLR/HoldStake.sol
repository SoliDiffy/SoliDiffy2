...
contract HoldStake is Ownable {
    ...
    function addPool(
        ...
        pid++;
        HoldPool memory holdPool = holdPools[pid];
        holdPool.apy = apy;
    ...
    function injectInterest(uint256 _pid) external {
        ...
        HoldPool storage holdPool = holdPools[_pid];
        require(
    ...
    function harvest(uint256 _pid) external {
        require(_pid > 0 && _pid <= pid, "Hold: Can't find this pool");
        DepositInfo memory depositInfo = depositInfos[msg.sender][_pid];
        require(
    ...
    function holdInProgress() external view returns (HoldPool[] storage) {
        uint256 len;
        ...
        HoldPool[] storage pools = new HoldPool[](len);
        for (uint256 i = 1; i <= pid; i++) {
            if (holdPools[i].finishTime > block.timestamp) {
                pools[i] = holdPools[i];
            }
