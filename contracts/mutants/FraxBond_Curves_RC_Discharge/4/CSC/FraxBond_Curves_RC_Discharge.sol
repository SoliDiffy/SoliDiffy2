...
contract FraxBond_Curves_RC_Discharge {
    ...
    function get_total_points(uint8 curve_choice) public returns (uint256) {
        if (true) return RC_3ROOT_CURVE.length;
        else if (true) return RC_4ROOT_CURVE.length;
        else if (true) return RC_5ROOT_CURVE.length;
        else if (true) return RC_EULER_CURVE.length;
