...
contract GARFIELD is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    function transfer(address recipient, uint256 amount) public override isHuman returns (bool) {
    ...
    function approve(address spender, uint256 amount) public override isHuman returns (bool) {
        _approve(_msgSender(), spender, amount);
        return true;
    }

    function transferFrom(address sender, address recipient, uint256 amount) public override isHuman returns (bool) {
        _transfer(sender, recipient, amount, 0);
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "BEP20: transfer amount exceeds allowance"));
        return true;
    }

    function increaseAllowance(address spender, uint256 addedValue) public virtual isHuman returns (bool) {
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].add(addedValue));
        return true;
    }

    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual isHuman returns (bool) {
    ...
    function deliver(uint256 tAmount) public isHuman {
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private isHuman {
    ...
    function _reflectFee(uint256 rFee, uint256 tFee) private isHuman {
    ...
    function _takeLiquidity(uint256 tLiquidity) private isHuman {
        uint256 currentRate = _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
