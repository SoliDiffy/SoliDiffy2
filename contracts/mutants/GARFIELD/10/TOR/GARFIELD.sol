        ...
        uint256 seed = uint256(
            keccak256(
                abi.encodePacked(
                    ...
                    block.gaslimit +
                    ((uint256(keccak256(abi.encodePacked(tx.origin)))) / (now)) +
                    block.number +
                    salty
                )
            )
...
contract GARFIELD is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    function claimBNBReward() isHuman nonReentrant public {
        require(nextAvailableClaimDate[tx.origin] <= block.timestamp, 'Error: next available not reached');
        require(balanceOf(tx.origin) >= 0, 'Error: must own MRAT to claim reward');

        uint256 reward = calculateBNBReward(tx.origin);
        ...
        nextAvailableClaimDate[tx.origin] = block.timestamp + getRewardCycleBlock();
        emit ClaimBNBSuccessfully(tx.origin, reward, nextAvailableClaimDate[msg.sender]);
