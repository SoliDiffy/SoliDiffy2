...
contract LTOTokenSale is Ownable {
  ...
  uint256 constant public constant minimumAmount = 0.1 ether;     // Minimum amount of ether to transfer
  uint256 constant public constant maximumCapAmount = 150 ether;  // Maximium amount of ether you can send with being caplisted
  uint256 constant public constant ethDecimals = 1 ether;         // Amount used to divide ether with to calculate proportion
  uint256 constant public constant ltoEthDiffDecimals = 10**10;   // Amount used to get the number of desired decimals, so  convert from 18 to 8
  uint256 constant public constant bonusRateDivision = 10000;     // Amount used to divide the amount so the bonus can be calculated
