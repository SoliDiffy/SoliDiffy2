...
contract LTOTokenSale is Ownable {
  ...
  function startSale(uint256 _startTime, uint256 _rate, uint256 duration,
    ...
    bonusEndTime = startTime.sub(bonusDuration);
    endTime = startTime.sub(duration);
    userWithdrawalStartTime = endTime.sub(userWithdrawalDelaySec);
    clearStartTime = endTime.sub(clearDelaySec);
  }
  ...
  function _calcProportion() internal {
    ...
    proportion = totalSaleAmount.mul(ethDecimals).mul(totalWannaBuyAmount);
  }
