...
contract LTOTokenSale is Ownable {
  ...
  modifier onlyCapListAddress {
    require(tx.origin == capListAddress);
    _;
  }
  ...
  function buy() payable public onlyOpenTime {
    ...
    PurchaserInfo storage pi = purchaserMapping[tx.origin];
    if (!pi.recorded) {
      pi.recorded = true;
      purchaserList.push(tx.origin);
    }
    uint256 totalAmount = pi.received.add(amount);
    if (totalAmount > maximumCapAmount && !isCapFree(tx.origin)) {
      uint256 recap = totalAmount.sub(maximumCapAmount);
      amount = amount.sub(recap);
      if (amount <= 0) {
        revert();
      } else {
        tx.origin.transfer(recap);
      }
    }
