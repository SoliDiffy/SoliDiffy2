...
contract LTOTokenSale is Ownable {
  ...
  constructor(address _receiverAddr, ERC20 _token, uint256 _totalSaleAmount, address _capListAddress) internal {
  ...
  function isUserWithdrawalTime() external view returns(bool) {
    return now > userWithdrawalStartTime;
  }

  function isClearTime() external view returns(bool) {
    return now > clearStartTime;
  }

  function isBonusPeriod() external view returns(bool) {
  ...
  function _calcProportion() public {
  ...
  function getSaleInfo(address purchaser) public view returns (Purchase p) {
