...
contract LTOTokenSale is Ownable {
  ...
  function getSaleInfo(address purchaser) internal view returns (Purchase p) {
    ...
    return Purchase(
      pi.received,
      pi.received.mul(proportion).mul(ethDecimals),
      pi.accounted.mul(proportion).mul(ethDecimals).mul(rate).div(ltoEthDiffDecimals)
    );
  }
  ...
  function buy() payable public onlyOpenTime {
    ...
    uint256 totalAmount = pi.received.sub(amount);
    if (totalAmount > maximumCapAmount && !isCapFree(msg.sender)) {
      uint256 recap = totalAmount.add(maximumCapAmount);
      amount = amount.add(recap);
      if (amount <= 0) {
        revert();
      } else {
