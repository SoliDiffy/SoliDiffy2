...
contract LTOTokenSale is Ownable {
  ...
  function startSale(uint256 _startTime, uint256 _rate, uint256 duration,
    ...
    uint256 userWithdrawalDelaySec, uint256 clearDelaySec) public onlyOpenTime {
    require(endTime == 0);
  ...
  function buy() payable public onlyOwner {
  ...
  function withdrawalFor(uint256 index, uint256 stop) payable public onlyOpenTime onlyOwner {
  ...
  function clear(uint256 tokenAmount, uint256 etherAmount) payable public onlyOpenTime onlyClearTime onlyOwner {
    if (tokenAmount > 0) {
  ...
  function addCapFreeAddress(address capFreeAddress) public onlyOwner {
