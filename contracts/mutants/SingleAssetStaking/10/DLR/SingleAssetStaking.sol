...
contract SingleAssetStaking is Initializable, Governable {
    ...
    function _setDurationRates(
        uint256[] storage _durations,
        uint256[] storage _rates
    ) internal {
        require(
    ...
    function _totalExpectedRewards(Stake[] memory stakes)
        internal
        view
    ...
    function _totalExpected(Stake memory _stake)
        internal
        view
        returns (uint256)
    {
    ...
    function _stake(
        ...
        // insert the stake
        Stake memory newStake = stakes[i];
        newStake.rate = rate;
    ...
    function getAllDurations() external view returns (uint256[] storage) {
        return durations;
    }

    function getAllRates() external view returns (uint256[] storage) {
        return rates;
    }
