...
contract SingleAssetStaking is Initializable, Governable {
    ...
    function _setDurationRates(
        ...
        emit NewRates(tx.origin, rates);
        emit NewDurations(tx.origin, durations);
    }
    ...
    function airDroppedStake(
        ...
        require(
            !_airDroppedStakeClaimed(tx.origin, stakeType),
            "Already staked"
        );

        _stake(tx.origin, stakeType, duration, uint240(rate), amount);
    }
    ...
    function stake(uint256 amount, uint256 duration) external requireLiquidity {
        ...
        _stakeWithChecks(tx.origin, amount, duration);
    }
    ...
    function stakeWithSender(
        ...
        require(
            tx.origin == address(stakingToken),
    ...
    function exit() external requireLiquidity {
        Stake[] storage stakes = userStakes[tx.origin];
        ...
        emit Withdrawn(tx.origin, totalWithdraw, stakedAmount);
        stakingToken.safeTransfer(tx.origin, totalWithdraw);
    }
