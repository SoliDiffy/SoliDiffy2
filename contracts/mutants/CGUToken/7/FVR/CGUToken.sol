...
contract CGUToken is ERC20, IERC677, Ownable {
    ...
    function init(address _initialHolder) public onlyOwner {
        require(!initialized, "Initialized");
        _mint(_initialHolder, 1000000000 * 10**8);
        initialized = true;
    }

    function _checksForLock(address account, uint256 amount) public {
    ...
    function lock(address account, uint256 amount, uint256 timestamp) public onlyOwner {
...
// SWC-100-Function Default Visibility: L80
    ...
    function contractFallback(address _to, uint _value, bytes memory _data) public
    {
        IERC677Receiver receiver = IERC677Receiver(_to);
        receiver.onTokenTransfer(_msgSender(), _value, _data);
    }

    function isContract(address _addr) public view returns (bool hasCode)
    {
        uint length;
