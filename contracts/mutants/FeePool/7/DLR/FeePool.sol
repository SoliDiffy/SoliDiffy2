...
contract FeePool is Owned, Proxyable, LimitedSetup, MixinSystemSettings, IFeePool {
    ...
    function resolverAddressesRequired() public view returns (bytes32[] storage addresses) {
        ...
        bytes32[] storage newAddresses = new bytes32[](14);
    ...
    function _recentFeePeriodsStorage(uint index) internal view returns (FeePeriod memory) {
    ...
    function _closeSecondary(uint allNetworksSnxBackedDebt, uint allNetworksDebtSharesSupply) internal {
        ...
        FeePeriod memory periodClosing = _recentFeePeriodsStorage(FEE_PERIOD_LENGTH - 2);
        FeePeriod memory periodToRollover = _recentFeePeriodsStorage(FEE_PERIOD_LENGTH - 1);
