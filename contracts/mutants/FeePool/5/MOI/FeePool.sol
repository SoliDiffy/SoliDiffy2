...
contract FeePool is Owned, Proxyable, LimitedSetup, MixinSystemSettings, IFeePool {
     ...
     * @notice Close the current fee period and start a new one.
     */
    function _closeSecondary(uint allNetworksSnxBackedDebt, uint allNetworksDebtSharesSupply) internal onlyInternalContracts {
    ...
    function _claimFees(address claimingAddress) internal onlyInternalContracts returns (bool) {
        uint rewardsPaid = 0;
        uint feesPaid = 0;
        uint availableFees;
     ...
     */
    function _recordFeePayment(uint sUSDAmount) internal onlyInternalContracts returns (uint) {
     ...
     */
    function _recordRewardPayment(uint snxAmount) internal onlyInternalContracts returns (uint) {
        // Don't assign to the parameter
     ...
     */
    function _setLastFeeWithdrawal(address _claimingAddress, uint _feePeriodID) internal onlyInternalContracts {
