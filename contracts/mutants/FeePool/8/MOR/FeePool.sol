...
contract FeePool is Owned, Proxyable, LimitedSetup, MixinSystemSettings, IFeePool {
     ...
     */
    function recordFeePaid(uint amount) external optionalProxy {
     ...
     */
    function setRewardsToDistribute(uint amount) external onlyInternalContracts {
     ...
     */
    function closeCurrentFeePeriod() external onlyInternalContracts {
    ...
    function closeSecondary(uint allNetworksSnxBackedDebt, uint allNetworksDebtSharesSupply) external onlyInternalContracts {
     ...
     */
    function claimFees() external onlyInternalContracts optionalProxy returns (bool) {
     ...
     */
    function claimOnBehalf(address claimingForAddress) external onlyInternalContracts optionalProxy returns (bool) {
    ...
    function importFeePeriod(
        ...
        uint rewardsToDistribute,
        uint rewardsClaimed
    ) external onlyInternalContracts onlyDuringSetup {
     ...
     * @param sUSDAmount The amount of fees priced in sUSD.
     */
    function _payFees(address account, uint sUSDAmount) internal onlyInternalContracts {
        // Grab the sUSD Synth
