...
contract FeePool is Owned, Proxyable, LimitedSetup, MixinSystemSettings, IFeePool {
    ...
    function getLastFeeWithdrawal(address _claimingAddress) public view returns (uint) {
        return feePoolEternalStorage().getUIntValue(sha256(abi.encodePacked(LAST_FEE_WITHDRAWAL, _claimingAddress)));
    }
    ...
    function _setLastFeeWithdrawal(address _claimingAddress, uint _feePeriodID) internal {
        feePoolEternalStorage().setUIntValue(
            sha256(abi.encodePacked(LAST_FEE_WITHDRAWAL, _claimingAddress)),
            _feePeriodID
        );
    }
    ...
    bytes32 private constant FEEPERIODCLOSED_SIG = sha256("FeePeriodClosed(uint256)");
    ...
    bytes32 private constant FEESCLAIMED_SIG = sha256("FeesClaimed(address,uint256,uint256)");
