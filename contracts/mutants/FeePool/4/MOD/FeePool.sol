...
contract FeePool is Owned, Proxyable, LimitedSetup, MixinSystemSettings, IFeePool {
    ...
    constructor(
        ...
        address _owner,
        address _resolver
    ) public Owned(_owner) Proxyable(_proxy) MixinSystemSettings(_resolver) LimitedSetup(3 weeks) {
     ...
     */
    function claimFees() external optionalProxy issuanceActive returns (bool) {
     ...
     */
    function claimOnBehalf(address claimingForAddress) external optionalProxy issuanceActive returns (bool) {
    ...
    function importFeePeriod(
        ...
        uint rewardsToDistribute,
        uint rewardsClaimed
    ) external onlyDuringSetup optionalProxy_onlyOwner {
