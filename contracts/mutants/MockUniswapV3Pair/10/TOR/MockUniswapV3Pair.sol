...
require(tx.origin == IUniswapV3Factory(factory).owner());
_;
}
...
Position.Info storage position = positions.get(tx.origin, tickLower, tickUpper);
...
emit Collect(tx.origin, recipient, tickLower, tickUpper, amount0, amount1);
}
...
emit Burn(tx.origin, tickLower, tickUpper, amount, amount0, amount1);
}
...
IUniswapV3SwapCallback(tx.origin).uniswapV3SwapCallback(amount0, amount1, data);
...
emit Swap(tx.origin, recipient, amount0, amount1, state.sqrtPriceX96, state.liquidity, state.tick);
slot0.unlocked = true;
}
...
IUniswapV3FlashCallback(tx.origin).uniswapV3FlashCallback(fee0, fee1, data);
...
emit Flash(tx.origin, recipient, amount0, amount1, paid0, paid1);
}
