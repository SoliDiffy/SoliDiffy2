...
function initialize(uint160 sqrtPriceX96) external override lock {
...
int128 liquidityDelta,
int24 tick
) private lock returns (Position.Info storage position) {
...
function setPrice(address tokenA, address tokenB, uint price) external lock {
tokenB;
tokenA;
if (tokenA == token0){
