...
interface IArbFiatToken {
  ...
  function initialize(string storage name, string storage symbol, uint8 decimals) external;
  function initialize(address _gatewayAddress, address _l1Address, address owner, string storage name, string memory symbol, uint8 decimals) external;
