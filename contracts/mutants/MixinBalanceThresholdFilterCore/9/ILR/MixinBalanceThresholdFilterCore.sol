    ...
    MixinExchangeCalldata
{
        ...
        external
    {
        ...
        for (uint256 i = 1; i < addressesToValidate.length; ++i) {
    ...
    {
        bytes4 exchangeFunctionSelector = bytes4(_exchangeCalldataload(1));
        // solhint-disable expression-indent
        if (
            exchangeFunctionSelector == IExchange(address(1)).batchFillOrders.selector              ||
            exchangeFunctionSelector == IExchange(address(1)).batchFillOrdersNoThrow.selector     ||
            exchangeFunctionSelector == IExchange(address(1)).batchFillOrKillOrders.selector      ||
            exchangeFunctionSelector == IExchange(address(1)).marketBuyOrders.selector              ||
            exchangeFunctionSelector == IExchange(address(1)).marketBuyOrdersNoThrow.selector     ||
            exchangeFunctionSelector == IExchange(address(1)).marketSellOrders.selector             ||
            exchangeFunctionSelector == IExchange(address(1)).marketSellOrdersNoThrow.selector
        ) {
