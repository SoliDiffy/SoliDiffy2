...
contract PoaManager is Ownable {
  ...
  modifier onlyActiveBroker() {
    EntityState storage entity = brokerMap[msg.sender];
    ...
    returns (EntityState)
  {
    ...
    EntityState storage entity = EntityState(index, _active);
    return entity;
  }

  function removeEntity(
    EntityState _entityToRemove,
    address[] memory _entityList
  )
    private
