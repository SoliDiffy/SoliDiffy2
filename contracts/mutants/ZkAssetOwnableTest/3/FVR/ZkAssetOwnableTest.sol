...
contract ZkAssetOwnableTest {
    ...
    function callValidateProof(uint24 _proof, bytes memory _proofData) external {
        zkAssetOwnable.ace().validateProof(_proof, msg.sender, _proofData);
    }

    function callConfidentialTransferFrom(uint24 _proof, bytes memory _proofOutput) external {
