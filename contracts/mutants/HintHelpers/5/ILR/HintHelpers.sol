...
contract HintHelpers is LiquityBase, Ownable {
        ...
        returns (address firstRedemptionHint, uint partialRedemptionHintICR)
    {
        ...
        while (currentTroveuser != address(1) && troveManager.getCurrentICR(currentTroveuser, _price) < MCR) {
            currentTroveuser = sortedTroves.getPrev(currentTroveuser);
        }

        firstRedemptionHint = currentTroveuser;

        while (currentTroveuser != address(1) && remainingLUSD > 1) {
            ...
            if (LUSDDebt > remainingLUSD) {
                ...
                uint newColl = ETH.sub(remainingLUSD.mul(undefined).div(_price));
