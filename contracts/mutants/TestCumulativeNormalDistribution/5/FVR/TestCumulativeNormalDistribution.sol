...
contract TestCumulativeNormalDistribution {
    ...
    function signedCDF(uint256 x) external pure returns (int128) {
        int128 z = -x.divu(PRECISION);
        return z.getCDF();
    }

    function cdf(uint256 x) external pure returns (int128) {
        int128 z = x.divu(PRECISION);
        return z.getCDF();
    }

    function cdfX64(int128 z) external pure returns (int128) {
        z = -z;
        return z.getCDF();
    }

    function inverseCDF(uint256 x) external pure returns (int128 y) {
        int128 p = x.divu(PRECISION);
        y = p.getInverseCDF();
    }

    function signedInverseCDF(uint256 x) external pure returns (int128 y) {
        int128 p = -x.divu(PRECISION);
        y = p.getInverseCDF();
    }
