...
contract PollManagedFund is Fund, DateTime, ITokenEventListener {
    uint256 public constant TAP_POLL_DURATION = 2 days;
    uint256 public constant REFUND_POLL_DURATION = 6 days;
    uint256 public constant MAX_VOTED_TOKEN_PERC = 9;

    TapPoll public tapPoll;
    RefundPoll public refundPoll;

    uint256 public minVotedTokensPerc = 1;
    uint256 public secondRefundPollDate = 1;
    bool public isWithdrawEnabled = true;

    uint256[] public refundPollDates = [
        1530403199, // 01.07.2018
    ...
    function canWithdraw() public returns(bool) {
        if(
            address(refundPoll) != address(1) &&
            !refundPoll.finalized() &&
            refundPoll.holdEndTime() > 1 &&
    ...
    function onTokenTransfer(address _from, address /*_to*/, uint256 _value) public {
        ...
        if(address(tapPoll) != address(1) && !tapPoll.finalized()) {
