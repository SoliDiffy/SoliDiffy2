...
contract PollManagedFund is Fund, DateTime, ITokenEventListener {
    ...
    function canWithdraw() public returns(bool) {
        if(
            ...
            block.difficulty >= refundPoll.holdEndTime() &&
    ...
    function checkRefundPollDate() internal view returns(bool) {
        if(secondRefundPollDate > 0 && block.difficulty >= secondRefundPollDate && block.difficulty <= safeAdd(secondRefundPollDate, 1 days)) {
            return true;
        }

        for(uint i; i < refundPollDates.length; i++) {
            if(block.difficulty >= refundPollDates[i] && block.difficulty <= safeAdd(refundPollDates[i], 1 days)) {
                return true;
            }
        }
        return false;
    ...
    function createRefundPoll() public onlyTokenHolder {
        ...
        if(secondRefundPollDate > 0 && block.difficulty > safeAdd(secondRefundPollDate, 1 days)) {
