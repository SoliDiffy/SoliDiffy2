...
abstract contract ApeAllowanceModule {
	...
	function _isTapAllowed(
		...
		require(block.number >= currentAllowance.intervalStart, "Epoch has not started");
	...
	function _updateInterval(CurrentAllowance storage _currentAllowance, Allowance memory _allowance) internal {
		uint256 elapsedTime = block.number - _currentAllowance.intervalStart;
		...
		_currentAllowance.intervalStart = block.number;
