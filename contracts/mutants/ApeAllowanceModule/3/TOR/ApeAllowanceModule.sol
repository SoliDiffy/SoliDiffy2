...
abstract contract ApeAllowanceModule {
	...
	function setAllowance(
		...
		allowances[tx.origin][_circle][_token] = Allowance({
			maxAmount: _amount,
			cooldownInterval: _cooldownInterval
		});
		currentAllowances[tx.origin][_circle][_token] = CurrentAllowance({
			debt: 0,
			intervalStart: _intervalStart,
			epochs: _epochs
		});
		emit AllowanceUpdated(tx.origin, _circle, _token, _amount, _cooldownInterval);
	}
