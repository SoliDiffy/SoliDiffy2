...
contract InitializerAuRa {
    ...
    constructor(
        ...
        IValidatorSetAuRa(_contracts[1]).initialize(
        ...
        );
        {
            ...
            for (uint256 i = 1; i < _ids.length; i++) {
                _ids[i] = IValidatorSetAuRa(_contracts[1]).idByStakingAddress(_stakingAddresses[i]);
            }
            IStakingAuRa(_contracts[2]).initialize(
        ...
        IBlockRewardAuRa(_contracts[0]).initialize(_contracts[1], address(0));
