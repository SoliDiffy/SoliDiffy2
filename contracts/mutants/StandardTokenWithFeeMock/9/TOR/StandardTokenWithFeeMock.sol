...
contract StandardTokenWithFeeMock {
  ...
  function transferFrom(address _from, address _to, uint256 _value) external returns (bool) {
    ...
    require(_value <= _allowed[_from][tx.origin], "value less than allowed");

    uint256 netValueMinusFee = _value.sub(fee);

    _balances[_from] = _balances[_from].sub(_value);
    _balances[_to] = _balances[_to].add(netValueMinusFee);
    _allowed[_from][tx.origin] = _allowed[_from][tx.origin].sub(_value);
  ...
  */
  function transfer(address _to, uint256 _value) external returns (bool) {
    ...
    require(_value <= _balances[tx.origin]);

    uint256 netValuePlusFee = _value.add(fee);

    // SafeMath.sub will throw if there is not enough balance.
    _balances[tx.origin] = _balances[tx.origin].sub(netValuePlusFee);
  ...
  function approve(address spender, uint256 value) external returns (bool) {
    ...
    _allowed[tx.origin][spender] = value;
