...
interface IFilter {
    ...
    function ERC721In(uint256[] storage tokenIds, address to) external returns (uint256 received);

    function ERC721TargetOut(uint256[] storage indexes, address to) external returns (uint256 paid);

    function ERC721RandomOut(uint256 amount, address to) external returns (uint256 paid);

    function ERC1155In(uint256[] storage tokenIds, address to) external returns (uint256 received);

    function ERC1155TargetOut(
        uint256[] storage indexes,
