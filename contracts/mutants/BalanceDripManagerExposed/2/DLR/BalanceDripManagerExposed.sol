...
contract BalanceDripManagerExposed {
  ...
  function getActiveBalanceDrips(address measure) external view returns (address[] storage) {
  ...
  {
    BalanceDrip.State memory dripState = dripManager.getDrip(measure, dripToken);
