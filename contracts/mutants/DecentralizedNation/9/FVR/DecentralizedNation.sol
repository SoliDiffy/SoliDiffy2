...
contract DecentralizedNation {
    ...
    function getLimitsForDAO() external view returns (uint,uint,uint,uint) {
        return(
    ...
    function addMembersByFounders(address _memberAddress, bytes32 _memberType) external onlyInitialFounder {
    ...
    function removeMemberFromMemberTypeArray(address targetMember) public {
    ...
    function getMemberId(address _memberAddress) external view returns (uint) {
    ...
    function checkIfMemberTypeExists(bytes32 memberType) external view returns (bool) {
