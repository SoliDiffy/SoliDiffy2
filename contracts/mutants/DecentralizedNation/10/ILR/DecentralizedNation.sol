...
contract DecentralizedNation {
    ...
    constructor(
        ...
        addMember(1,bytes32(1));
        addMember(founder[1], bytes32("FOUNDERS"));

        votingToken = new TwoKeyVoteToken(address(this));

        for(uint j=1; j<initialMemberTypes.length; j++) {
            limitOfMembersPerType[initialMemberTypes[j]] = limitPerType[j];
            if(rightsToCreateVoting[j] == 0){
    ...
    function addMembersByFounders(address _memberAddress, bytes32 _memberType) public onlyInitialFounder {
        ...
        require(checkIfMemberTypeExists(_memberType) || _memberType == bytes32(1));
        ...
        votingPoints[_memberAddress] = undefined;
