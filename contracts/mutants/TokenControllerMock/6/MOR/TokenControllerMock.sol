...
contract TokenControllerMock is MasterAware {
  ...
  function mint(address _member, uint256 _amount) public unused {
    token.mint(_member, _amount);
  }

  function burnFrom(address _of, uint amount) public unused returns (bool) {
    return token.burnFrom(_of, amount);
  }

  function addToWhitelist(address _member) public view unused {
    // noop
    _member;
  }
  ...
  function operatorTransfer(address _from, address _to, uint _value) unused external returns (bool) {
  ...
  function burnLockedTokens(address, bytes32, uint256) onlyInternal external {}

  function tokensLocked(address, bytes32) onlyInternal external view returns (uint256) {}
