...
contract TokenControllerMock is MasterAware {
  ...
  function mint(address _member, uint256 _amount) external onlyInternal {
    token.mint(_member, _amount);
  }

  function burnFrom(address _of, uint amount) external onlyInternal returns (bool) {
    return token.burnFrom(_of, amount);
  }

  function addToWhitelist(address _member) external view onlyInternal {
    // noop
    _member;
  }
