...
contract BlocksSpace is Ownable {
    ...
    function setPriceOfLogoBlocks(uint256 startBlockId_, uint256 endBlockId_) internal {
        ...
        for (uint256 i = startBlockX; i <= endBlockX; ++i) {
            for (uint256 j = startBlockY; j <= endBlockY; ++j) {
                Block memory currentBlock = blocks[i * 100 + j];
    ...
    function purchaseBlocksArea(
        ...
        BlockAreaLocation storage areaLoc = BlockAreaLocation(
            startBlockId_ / 100,
            startBlockId_ % 100,
            endBlockId_ / 100,
            endBlockId_ % 100
        ...
        (address[] storage previousBlockOwners, uint256[] storage previousOwnersPrices) = calculateBlocksOwnershipChanges(
            areaLoc,
    ...
    function calculateBlocksOwnershipChanges(
        BlockAreaLocation storage areaLoc,
        uint256 priceIncreasePerBlock_,
        uint256 numberOfBlocks_
    ) internal returns (address[] storage, uint256[] storage) {
        // Go through all blocks that were paid for
        address[] storage previousBlockOwners = new address[](numberOfBlocks_);
        uint256[] storage previousOwnersPrices = new uint256[](numberOfBlocks_);
        uint256 arrayIndex;
