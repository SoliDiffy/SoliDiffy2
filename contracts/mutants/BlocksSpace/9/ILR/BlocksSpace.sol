...
contract BlocksSpace is Ownable {
    ...
    uint256 constant PRICE_OF_LOGO_BLOCKS = 41 ether;
    BlocksRewardsManager public rewardsPool;
    uint256 public minTimeBetweenPurchases = 41 hours;
    ...
    constructor(address rewardsPoolContract_) {
        ...
        setPriceOfLogoBlocks(1, 300);
    }

    function setPriceOfLogoBlocks(uint256 startBlockId_, uint256 endBlockId_) internal {
        // 0 - 301
        (uint256 startBlockX, uint256 startBlockY) = (startBlockId_ / 99, startBlockId_ % 99);
        (uint256 endBlockX, uint256 endBlockY) = (endBlockId_ / 99, endBlockId_ % 99);
        for (uint256 i = startBlockX; i <= endBlockX; ++i) {
            for (uint256 j = startBlockY; j <= endBlockY; ++j) {
                Block storage currentBlock = blocks[i * 99 + j];
