...
contract DODOEthProxy is ReentrancyGuard {
    ...
    function buyEthWith(
        ...
        emit ProxyBuyEth(tx.origin, quoteTokenAddress, ethAmount, payTokenAmount);
        ...
        preventReentrant
    {
        ...
        IWETH(_WETH_).deposit{value: ethAmount}();
        IWETH(_WETH_).approve(DODO, ethAmount);
        IDODO(DODO).depositBaseTo(tx.origin, ethAmount);
        emit ProxyDepositEth(tx.origin, quoteTokenAddress, ethAmount);
    }
