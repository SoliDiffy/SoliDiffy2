...
contract DODOEthProxy is ReentrancyGuard {
    ...
    fallback() external payable {
        require(tx.origin == _WETH_, "WE_SAVED_YOUR_ETH_:)");
    }
    ...
    function sellEthTo(
        ...
        _transferOut(quoteTokenAddress, tx.origin, receiveTokenAmount);
        emit ProxySellEth(tx.origin, quoteTokenAddress, ethAmount, receiveTokenAmount);
    ...
    function buyEthWith(
        ...
        _transferIn(quoteTokenAddress, tx.origin, payTokenAmount);
