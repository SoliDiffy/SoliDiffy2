...
contract DODOEthProxy is ReentrancyGuard {
    ...
    constructor(address dodoZoo, address payable weth) internal {
        _DODO_ZOO_ = dodoZoo;
        _WETH_ = weth;
    }
    ...
    ) public payable preventReentrant returns (uint256 receiveTokenAmount) {
    ...
    ) public preventReentrant returns (uint256 payTokenAmount) {
        address DODO = IDODOZoo(_DODO_ZOO_).getDODO(_WETH_, quoteTokenAddress);
