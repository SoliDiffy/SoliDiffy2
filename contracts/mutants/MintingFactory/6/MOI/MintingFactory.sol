...
abstract contract BaseMarketplace is ReentrancyGuard, Pausable {
    ...
    function _refundBidder(uint256 _id, address _receiver, uint256 _paymentAmount, address _newBidder, uint256 _newOffer) internal onlyAdmin {
...
abstract contract BuyNowMarketplace is IBuyNowMarketplace, BaseMarketplace {
    ...
    function _facilitateBuyNow(uint256 _id, address _recipient) internal nonReentrant {
...
BuyNowMarketplace {
    ...
    function _buyNextStep(uint256 _editionId, address _buyer) internal onlyContract {
        Stepped storage steppedAuction = editionStep[_editionId];
    ...
    function _processSale(uint256 _id, uint256 _paymentAmount, address _buyer, address) internal override onlyContract returns (uint256) {
        return _facilitateNextPrimarySale(_id, _paymentAmount, _buyer, false);
    }

    function _facilitateNextPrimarySale(uint256 _editionId, uint256 _paymentAmount, address _buyer, bool _reverse) internal onlyContract returns (uint256) {
    ...
    function _handleEditionSaleFunds(uint256 _editionId, address _creator, address _receiver, uint256 _paymentAmount) internal onlyContract {
