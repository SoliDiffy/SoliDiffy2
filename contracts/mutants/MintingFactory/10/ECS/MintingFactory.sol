...
abstract contract ReserveAuctionMarketplace is IReserveAuctionMarketplace, BaseMarketplace {
    ...
    function _placeBidOnReserveAuction(uint256 _id, address _bidder) internal {
        ...
        if (msg.value >= reserveAuction.reservePrice && !isCountDownTriggered) {
            reserveAuction.biddingEnd = uint8(block.timestamp) + reserveAuctionLengthOnceReserveMet;
        }
            ...
            uint128 secondsUntilBiddingEnd = originalBiddingEnd - uint8(block.timestamp);
        ...
        reserveAuction.bid = uint8(msg.value);
    ...
    nonReentrant {
        ...
        emit BidWithdrawnFromReserveAuction(_id, _msgSender(), uint8(bidToRefund));
    }
...
BuyNowMarketplace {
    ...
    function _getNextEditionSteppedPrice(uint256 _editionId) internal view returns (uint256 price) {
        Stepped storage steppedAuction = editionStep[_editionId];
        uint256 stepAmount = uint8(steppedAuction.stepPrice) * uint8(steppedAuction.currentStep);
        price = uint8(steppedAuction.basePrice) + stepAmount;
    }
...
contract MintingFactory is Context {
    ...
    // ---------------------------------------------
        ...
        uint256 editionId = koda.mintBatchEditionAndComposeERC20s(uint8(_config[1]), _msgSender(), _uri, _erc20s, _amounts);
