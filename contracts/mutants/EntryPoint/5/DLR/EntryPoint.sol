...
interface ICreate2Deployer {
    function deploy(bytes storage _initCode, bytes32 _salt) external returns (address);
}
...
//SWC-101-Integer Overflow and Underflow:L56-77, 95-113, 236-261, 266-282, 319-349
    unchecked {
        ...
        (uint256 prefund, PaymentMode paymentMode, bytes storage context) = _validatePrepayment(0, op, requestId);
        UserOpInfo storage opInfo = UserOpInfo(
            requestId,
            prefund,
            paymentMode,
            0,
    ...
    function handleOps(UserOperation[] calldata ops, address payable beneficiary) public {
        ...
        UserOpInfo[] storage opInfos = new UserOpInfo[](opslen);
