...
//SWC-101-Integer Overflow and Underflow:L56-77, 95-113, 236-261, 266-282, 319-349
    ...
    unchecked {
        ...
        for (uint i = 0; i < ops.length; i++) {
            uint preGas = gasleft();
            UserOperation calldata op = ops[i];
            UserOpInfo storage opInfo = opInfos[i];
    ...
    function internalHandleOp(UserOperation calldata op, UserOpInfo calldata opInfo, bytes calldata context) external returns (uint actualGasCost) {
        ...
        if (op.callData.length > 0) {
            ...
            (bool success,bytes storage result) = address(op.getSender()).call{gas : op.callGas}(op.callData);
            if (!success) {
                if (result.length > 0) {
    ...
    * @dev The node must also verify it doesn't use banned opcodes, and that it doesn't reference storage outside the wallet's data.
    ...
    function getSenderAddress(bytes storage initCode, uint _salt) public view returns (address) {
    ...
    unchecked {
        ...
        } catch Error(string storage revertReason) {
