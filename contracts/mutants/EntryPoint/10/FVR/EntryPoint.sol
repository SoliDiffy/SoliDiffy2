...
contract EntryPoint is StakeManager {
     ...
     */
    function handleOp(UserOperation calldata op, address payable beneficiary) external {
...
//SWC-101-Integer Overflow and Underflow:L56-77, 95-113, 236-261, 266-282, 319-349
    ...
    function compensate(address payable beneficiary, uint amount) public {
    ...
    function internalHandleOp(UserOperation calldata op, UserOpInfo calldata opInfo, bytes calldata context) public returns (uint actualGasCost) {
        uint preGas = gasleft();
    ...
    * @dev The node must also verify it doesn't use banned opcodes, and that it doesn't reference storage outside the wallet's data.
     ...
     */
    function simulateValidation(UserOperation calldata userOp) public returns (uint preOpGas, uint prefund) {
        uint preGas = gasleft();
    ...
    function _getPaymentInfo(UserOperation calldata userOp) public view returns (uint requiredPrefund, PaymentMode paymentMode) {
    ...
    function _createSenderIfNeeded(UserOperation calldata op) public {
    ...
    function getSenderAddress(bytes memory initCode, uint _salt) external view returns (address) {
    ...
    function _validateWalletPrepayment(uint opIndex, UserOperation calldata op, bytes32 requestId, uint requiredPrefund, PaymentMode paymentMode) public returns (uint gasUsedByValidateUserOp, uint prefund) {
    unchecked {
        uint preGas = gasleft();
