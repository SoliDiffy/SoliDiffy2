...
//SWC-101-Integer Overflow and Underflow:L56-77, 95-113, 236-261, 266-282, 319-349
    ...
    * @dev The node must also verify it doesn't use banned opcodes, and that it doesn't reference storage outside the wallet's data.
     ...
     */
    function simulateValidation(UserOperation calldata userOp) external returns (uint prefund, uint preOpGas) {
        uint preGas = gasleft();
    ...
    function _validateWalletPrepayment(uint opIndex, UserOperation calldata op, bytes32 requestId, uint requiredPrefund, PaymentMode paymentMode) internal returns (uint prefund, uint gasUsedByValidateUserOp) {
    unchecked {
        uint preGas = gasleft();
