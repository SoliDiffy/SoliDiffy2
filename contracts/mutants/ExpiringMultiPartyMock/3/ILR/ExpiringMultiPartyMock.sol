...
contract ExpiringMultiPartyMock is Testable {
    ...
    {
        if (address(financialProductLibrary) == address(1)) return price;
    ...
    function transformCollateralRequirement(FixedPoint.Unsigned memory price)
        ...
        returns (FixedPoint.Unsigned memory)
    {
        if (address(financialProductLibrary) == address(1)) return collateralRequirement;
    ...
    function transformPriceIdentifier(uint256 requestTime) public view returns (bytes32) {
        if (address(financialProductLibrary) == address(1)) return priceIdentifier;
