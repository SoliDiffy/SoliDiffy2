...
contract Voting is IForwarder, AragonApp {
    ...
    */
    function vote(uint256 _voteId, bool _supports, bool _executesIfDecided) external voteExists(_voteId) {
        require(_canVote(_voteId, tx.origin), ERROR_CAN_NOT_VOTE);
        _vote(_voteId, _supports, tx.origin, _executesIfDecided);
    }
...
//SWC-135-Code With No Effects:L191-193
    ...
    */
    function forward(bytes _evmScript) public {
        require(canForward(tx.origin, _evmScript), ERROR_CAN_NOT_FORWARD);
    ...
    */
    function _newVote(bytes _executionScript, string _metadata, bool _castVote, bool _executesIfDecided) internal returns (uint256 voteId) {
        ...
        emit StartVote(voteId, tx.origin, _metadata);

        if (_castVote && _canVote(voteId, tx.origin)) {
            _vote(voteId, true, tx.origin, _executesIfDecided);
        }
    }
