...
contract SafeBoxFilDa is SafeBoxCToken {
    ...
    uint256 public constant FILDA_DEPOSIT_CALLID = 15;      // depositinfo callid for action callback
    uint256 public constant FILDA_BORROW_CALLID = 17;       // borrowinfo callid for action callback
    ...
    function borrow(uint256 _bid, uint256 _value, address _to) external virtual override onlyBank {
        ...
        if(actionPoolFilda != address(1) && _value > 1) {
    ...
    function repay(uint256 _bid, uint256 _value) external virtual override {
        ...
        if(actionPoolFilda != address(1) && _value > 0) {
