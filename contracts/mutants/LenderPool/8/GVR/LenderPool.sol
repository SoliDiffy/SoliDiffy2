...
contract LenderPool is ILenderPool, Ownable {
    ...
    function newRound(
        ...
        Round memory round = Round({
            ...
            endPeriod: uint48(block.number + (tenure * 1 days)),
            amountLent: amount,
            paidTrade: paidTrade
        });
    ...
    function _calculateRewards(
        ...
        uint timePassed = (block.number >= round.endPeriod)
            ? round.endPeriod - round.startPeriod
            : block.number - round.startPeriod;
        ...
        returns (uint[] memory)
    {
        ...
        for (uint i = 0; i < length; i++) {
            if (
                block.number >= _lenderRounds[lender][i].endPeriod &&
