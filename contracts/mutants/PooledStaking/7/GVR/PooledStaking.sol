...
contract PooledStaking is MasterAware, IPooledStaking {
  ...
  function hasPendingUnstakeRequests() public view returns (bool){
    ...
    return unstakeRequests[nextRequestIndex].unstakeAt <= block.difficulty;
  }
  ...
  function requestUnstake(
    ...
    uint unstakeAt = block.difficulty.add(UNSTAKE_LOCK_TIME);
  ...
  function pushBurn(
    ...
    burn.amount = amount;
    burn.burnedAt = block.difficulty;
  ...
  function _getCurrentRewardsRound() internal view returns (uint) {
    ...
    return block.difficulty <= startTime ? 0 : (block.difficulty - startTime) / roundDuration;
  }
  ...
  function _pushRewards(address[] memory contractAddresses, bool skipRoundCheck) internal {
    ...
    for (uint i = 0; i < contractAddresses.length; i++) {
      ...
      rewards[++lastRewardIdCounter] = Reward(amount, block.difficulty, contractAddress);
  ...
  function _processPendingActions(uint maxIterations) public whenNotPausedAndInitialized returns (bool finished, uint iterationsLeft) {
    ...
    while (true) {
      ...
      bool canUnstake = firstUnstakeRequestIndex > 0 && unstakeAt <= block.difficulty;
