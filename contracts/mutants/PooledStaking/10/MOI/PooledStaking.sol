...
contract PooledStaking is MasterAware, IPooledStaking {
   ...
   */
  function _pushRewards(address[] memory contractAddresses, bool skipRoundCheck) internal whenNotPausedAndInitialized {
  ...
  function _processBurn(uint maxIterations) internal whenNotPausedAndInitialized returns (bool finished, uint iterationsLeft) {
  ...
  ) internal whenNotPausedAndInitialized returns (
  ...
  function _processFirstUnstakeRequest() internal whenNotPausedAndInitialized {
  ...
  function _processFirstReward(uint maxIterations) internal whenNotPausedAndInitialized returns (bool finished, uint iterationsLeft) {
  ...
  ) internal whenNotPausedAndInitialized returns (uint rewardedAmount, uint stake) {
  ...
  function initialize() public whenNotPausedAndInitialized {
    require(!initialized, "Contract is already initialized");
    tokenController.addToWhitelist(address(this));
    initialized = true;
  }

  function changeDependentContractAddress() public whenNotPausedAndInitialized {
  ...
  function migratePendingUnstakesToNewLockTime(uint iterations) external whenNotPausedAndInitialized {
