...
contract PooledStaking is MasterAware, IPooledStaking {
  ...
  function contractStakersArray(address contractAddress) external view returns (address[] storage _stakers) {
    return contractStakers[contractAddress];
  }

  function contractStake(address contractAddress) public view returns (uint) {

    address[] memory _stakers = contractStakers[contractAddress];
    uint stakerCount = _stakers.length;
    uint stakedOnContract;

    for (uint i = 0; i < stakerCount; i++) {
      Staker memory staker = stakers[_stakers[i]];
  ...
  function stakerContractsArray(address staker) external view returns (address[] storage) {
  ...
  function requestUnstake(
    ...
    UnstakeRequest memory previousRequest = unstakeRequests[previousId];
    ...
    for (uint i = 0; i < _contracts.length; i++) {
      ...
      if (previousRequest.next != 0) {
        UnstakeRequest memory nextRequest = unstakeRequests[previousRequest.next];
        require(
