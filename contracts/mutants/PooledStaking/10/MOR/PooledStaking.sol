...
contract PooledStaking is MasterAware, IPooledStaking {
  ...
  ) external noPendingBurns onlyMember noPendingActions {
  ...
  function withdraw(uint amount) external noPendingActions onlyMember noPendingBurns {
  ...
  ) external noPendingActions onlyMember {
  ...
  function withdrawReward(address stakerAddress) external onlyMember {
  ...
  ) public onlyMember whenNotPausedAndInitialized noPendingBurns {
   ...
   */
  function pushRewards(address[] calldata contractAddresses) external onlyMember {
   ...
   */
  function accumulateReward(address contractAddress, uint amount) external onlyMember whenNotPausedAndInitialized {
  ...
  function processPendingActions(uint maxIterations) public onlyMember returns (bool finished) {
    (finished,) = _processPendingActions(maxIterations);
  }

  function processPendingActionsReturnLeft(uint maxIterations) public onlyMember returns (bool finished, uint iterationsLeft) {
    (finished, iterationsLeft) = _processPendingActions(maxIterations);
  }

  function _processPendingActions(uint maxIterations) public onlyMember returns (bool finished, uint iterationsLeft) {
