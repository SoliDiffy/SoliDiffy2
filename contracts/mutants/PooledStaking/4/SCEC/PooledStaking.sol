...
contract PooledStaking is MasterAware, IPooledStaking {
  ...
  function unstakeRequestAtIndex(uint unstakeRequestId) external view returns (
    uint next, uint amount, address stakerAddress, address contractAddress, uint unstakeAt
  ) {
  ...
  function _burnStaker(
    ...
    uint _newStake, uint _stakerBurnAmount
  ) {
  ...
  function _calculateContractStake(
    ...
    return (iterationsLeft, true, _stakedOnContract);
  }
  ...
  ) internal returns (uint stake, uint rewardedAmount) {
