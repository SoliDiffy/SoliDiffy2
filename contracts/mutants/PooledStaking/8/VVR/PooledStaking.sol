...
contract PooledStaking is MasterAware, IPooledStaking {
  ...
  NXMToken internal token;
  TokenController internal tokenController;

  uint internal MIN_STAKE;         // Minimum allowed stake per contract
  uint internal MAX_EXPOSURE;      // Stakes sum must be less than the deposit amount times this
  uint internal MIN_UNSTAKE;       // Forbid unstake of small amounts to prevent spam
  uint internal UNSTAKE_LOCK_TIME; // Lock period in seconds before unstaking takes place
