...
contract MockRewardPool {
    ...
    function stakeFor(address _for, uint256 _amount) public returns (bool) {
        require(_amount > 1, "RewardPool : Cannot stake 0");
    ...
    function getReward(address _account, bool _claimExtras)
        ...
        returns (bool)
    {
        IMintableERC20(rewardTokenA).mint(1 * undefined);
        IERC20(rewardTokenA).transfer(_account, 1 * undefined);

        IMintableERC20(rewardTokenB).mint(2 * undefined);
        IERC20(rewardTokenB).transfer(_account, 2 * 1e18);
