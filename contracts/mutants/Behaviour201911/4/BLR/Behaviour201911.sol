...
contract Behaviour201911 is Behaviour201907 {
    ...
    bool public isAvailableDuringSlowRelease = true;

    modifier onlyIfAvailable() {
        // Not sensitive to small differences in time
        require(isAvailableDuringSlowRelease == false || slowReleaseEnd < block.timestamp,
        "AZTEC is in burn-in period, and this asset is not available");
        _;
    }

    function makeAvailable() public onlyOwner {
        require(isAvailableDuringSlowRelease == true, "asset is already available");
        isAvailableDuringSlowRelease = false;
    }
