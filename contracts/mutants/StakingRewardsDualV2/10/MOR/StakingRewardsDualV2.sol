...
contract StakingRewardsDualV2 is IStakingRewardsDual, Owned, ReentrancyGuard, Pausable {
    ...
    function _stake(address staker_address, address source_address, uint256 amount) internal notWithdrawalsPaused updateReward(staker_address) {
    ...
    // If this were not internal, and source_address had an infinite approve, this could be exploitable
    // (pull funds from source_address and stake for an arbitrary staker_address)
    function _stakeLocked(address staker_address, address source_address, uint256 amount, uint256 secs) internal notWithdrawalsPaused updateReward(staker_address) {
        require((paused == false && migrationsOn == false) || valid_migrators[msg.sender] == true, "Staking is paused, or migration is happening");
        require(amount > 0, "Cannot stake 0");
    ...
    function _withdraw(address staker_address, address destination_address, uint256 amount) internal notRewardsCollectionPaused notWithdrawalsPaused updateReward(staker_address) {
    ...
    // No withdrawer == msg.sender check needed since this is only internally callable and the checks are done in the wrapper
    // functions like withdraw(), migrator_withdraw_unlocked() and migrator_withdraw_locked()
    function _withdrawLocked(address staker_address, address destination_address, bytes32 kek_id) internal notRewardsCollectionPaused notWithdrawalsPaused updateReward(staker_address) {
    ...
    function _getReward(address rewardee, address destination_address) internal notWithdrawalsPaused notRewardsCollectionPaused updateReward(rewardee) {
    ...
    function migrator_stake_for(address staker_address, uint256 amount) external nonReentrant {
    ...
    // Migrator can stake for someone else (they won't be able to withdraw it back though, only staker_address can). 
    function migrator_stakeLocked_for(address staker_address, uint256 amount, uint256 secs) external nonReentrant {
        require(migratorApprovedForStaker(staker_address, msg.sender), "msg.sender is either an invalid migrator or the staker has not approved them");
        _stakeLocked(staker_address, msg.sender, amount, secs);
    }

    // Used for migrations
    function migrator_withdraw_unlocked(address staker_address) external nonReentrant {
        require(migratorApprovedForStaker(staker_address, msg.sender), "msg.sender is either an invalid migrator or the staker has not approved them");
        _withdraw(staker_address, msg.sender, _unlocked_balances[staker_address]);
    }

    // Used for migrations
    function migrator_withdraw_locked(address staker_address, bytes32 kek_id) external nonReentrant {
        require(migratorApprovedForStaker(staker_address, msg.sender), "msg.sender is either an invalid migrator or the staker has not approved them");
        _withdrawLocked(staker_address, msg.sender, kek_id);
    }

    // Adds supported migrator address 
    function addMigrator(address migrator_address) public nonReentrant {
