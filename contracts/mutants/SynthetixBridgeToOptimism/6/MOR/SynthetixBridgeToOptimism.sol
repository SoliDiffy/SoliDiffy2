...
contract SynthetixBridgeToOptimism is BaseSynthetixBridge, ISynthetixBridgeToOptimism, iOVM_L1TokenGateway {
    ...
    function deposit(uint256 amount) external onlyCounterpart requireZeroDebt {
        _initiateDeposit(msg.sender, amount);
    }

    function depositTo(address to, uint amount) external onlyCounterpart requireZeroDebt {
        _initiateDeposit(to, amount);
    }

    function migrateEscrow(uint256[][] memory entryIDs) public onlyCounterpart requireZeroDebt {
        _migrateEscrow(entryIDs);
    }

    // invoked by a generous user on L1
    function depositReward(uint amount) external requireZeroDebt {
    ...
    function closeFeePeriod(uint snxBackedAmount, uint totalDebtShares) external requireZeroDebt {
    ...
    function finalizeWithdrawal(address to, uint256 amount) external requireInitiationActive {
