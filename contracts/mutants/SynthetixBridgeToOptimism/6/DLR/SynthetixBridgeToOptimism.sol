...
contract SynthetixBridgeToOptimism is BaseSynthetixBridge, ISynthetixBridgeToOptimism, iOVM_L1TokenGateway {
    ...
    function resolverAddressesRequired() public view returns (bytes32[] storage addresses) {
        bytes32[] storage existingAddresses = BaseSynthetixBridge.resolverAddressesRequired();
        bytes32[] storage newAddresses = new bytes32[](4);
    ...
    function migrateEscrow(uint256[][] storage entryIDs) public requireInitiationActive requireZeroDebt {
    ...
    function depositAndMigrateEscrow(uint256 depositAmount, uint256[][] storage entryIDs)
        public
