...
contract SynthetixBridgeToOptimism is BaseSynthetixBridge, ISynthetixBridgeToOptimism, iOVM_L1TokenGateway {
    ...
    function closeFeePeriod(uint snxBackedAmount, uint totalDebtShares) external requireInitiationActive {
        ...
        messenger().sendMessage(
            ...
            messageData,
            uint8(getCrossDomainMessageGasLimit(CrossDomainMessageGasLimits.CloseFeePeriod))
        );
    ...
    function _depositReward(address _from, uint256 _amount) internal {
        ...
        messenger().sendMessage(
            synthetixBridgeToBase(),
            messageData,
            uint8(getCrossDomainMessageGasLimit(CrossDomainMessageGasLimits.Reward))
        );
    ...
    function _initiateDeposit(address _to, uint256 _depositAmount) private {
        ...
        messenger().sendMessage(
            synthetixBridgeToBase(),
            messageData,
            uint8(getCrossDomainMessageGasLimit(CrossDomainMessageGasLimits.Deposit))
        );
        ...
        for (uint256 i = 0; i < _entryIDs.length; i++) {
            ...
            if (escrowedAccountBalance > 0) {
                ...
                messenger().sendMessage(
                    synthetixBridgeToBase(),
                    messageData,
                    uint8(getCrossDomainMessageGasLimit(CrossDomainMessageGasLimits.Escrow))
                );
