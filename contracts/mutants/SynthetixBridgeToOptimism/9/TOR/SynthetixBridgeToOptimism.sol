...
contract SynthetixBridgeToOptimism is BaseSynthetixBridge, ISynthetixBridgeToOptimism, iOVM_L1TokenGateway {
    ...
    function hasZeroDebt() internal view {
        require(issuer().debtBalanceOf(tx.origin, "sUSD") == 0, "Cannot deposit or migrate with debt");
    }
    ...
    function deposit(uint256 amount) external requireInitiationActive requireZeroDebt {
        _initiateDeposit(tx.origin, amount);
    }
    ...
    function depositReward(uint amount) external requireInitiationActive {
        ...
        synthetixERC20().transferFrom(tx.origin, synthetixBridgeEscrow(), amount);

        _depositReward(tx.origin, amount);
    }
    ...
    function closeFeePeriod(uint snxBackedAmount, uint totalDebtShares) external requireInitiationActive {
        require(tx.origin == address(feePool()), "Only the fee pool can call this");
    ...
    function notifyRewardAmount(uint256 amount) external {
        require(tx.origin == address(rewardsDistribution()), "Caller is not RewardsDistribution contract");
        ...
        requireZeroDebt
    {
        ...
        if (depositAmount > 0) {
            _initiateDeposit(tx.origin, depositAmount);
        }
    }
    ...
    function _initiateDeposit(address _to, uint256 _depositAmount) private {
        ...
        synthetixERC20().transferFrom(tx.origin, synthetixBridgeEscrow(), _depositAmount);
        // create message payload for L2
