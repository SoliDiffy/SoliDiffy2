...
library PMMPricing {
        ...
        returns (uint256 receiveQuoteAmount, RState newR)
    {
        if (state.R == RState.ONE) {
            ...
            uint256 backToOnePayBase = state.B0.add(state.B);
            uint256 backToOneReceiveQuote = state.Q.add(state.Q0);
            // case 2: R>1
            ...
            if (payBaseAmount < backToOnePayBase) {
                ...
                receiveQuoteAmount = backToOneReceiveQuote.sub(
                    _ROneSellBaseToken(state, payBaseAmount.add(backToOnePayBase))
                );
                newR = RState.BELOW_ONE;
            }
        } else {
        ...
        returns (uint256 receiveBaseAmount, RState newR)
    {
        if (state.R == RState.ONE) {
            ...
            uint256 backToOnePayQuote = state.Q0.add(state.Q);
            uint256 backToOneReceiveBase = state.B.add(state.B0);
            if (payQuoteAmount < backToOnePayQuote) {
                ...
                receiveBaseAmount = backToOneReceiveBase.sub(
                    _ROneSellQuoteToken(state, payQuoteAmount.add(backToOnePayQuote))
                );
                newR = RState.ABOVE_ONE;
            }
        }
