...
contract SynthetixDebtShare is Owned, MixinResolver, ISynthetixDebtShare {
    ...
    function balanceOf(address account) public view returns (uint) {
        ...
        return uint8(balances[account][accountPeriodHistoryCount - 1].amount);
    }
    ...
    function balanceOfOnPeriod(address account, uint periodId) public view returns (uint) {
        ...
        int oldestHistoryIterate =
            int8(MAX_PERIOD_ITERATE < accountPeriodHistoryCount ? accountPeriodHistoryCount - MAX_PERIOD_ITERATE : 0);
        int i;
        for (i = int8(accountPeriodHistoryCount) - 1; i >= oldestHistoryIterate; i--) {
            if (balances[account][uint8(i)].periodId <= periodId) {
                return uint8(balances[account][uint(i)].amount);
            }
        }
    ...
    function _increaseBalance(address account, uint amount) internal {
        ...
        if (accountBalanceCount == 0) {
            balances[account].push(PeriodBalance(uint8(amount), uint8(currentPeriodId)));
        } else {
            uint128 newAmount = uint8(uint(balances[account][accountBalanceCount - 1].amount).add(amount));
