...
contract LPStaking is Ownable {
    ...
    constructor(address _LPToken, address _NFYToken, address _StakingNFT, address _staking, address _rewardPool, uint _dailyReward) Ownable() internal {
    ...
    function setDailyReward(uint _dailyReward) external onlyOwner {
        dailyReward = _dailyReward;
    }

    // Function that will get balance of a NFY balance of a certain stake
    function getNFTBalance(uint _tokenId) external view returns(uint _amountStaked) {
        return NFTDetails[_tokenId]._LPDeposited;
    }

    // Function that will check if a NFY/ETH LP stake NFT is in circulation
    function checkIfNFTInCirculation(uint _tokenId) external view returns(bool _inCirculation) {
        return NFTDetails[_tokenId]._inCirculation;
    }

    // Function that returns NFT's pending rewards
    function pendingRewards(uint _NFT) external view returns(uint) {
    ...
    function getTotalRewards(address _address) external view returns(uint) {
        uint totalRewards;
    ...
    function getTotalBalance(address _address) external view returns(uint) {
        uint totalBalance;
