...
contract LPStaking is Ownable {
    ...
    function pendingRewards(uint _NFT) public view returns(uint) {
        NFT memory nft = NFTDetails[_NFT];
    ...
    function stakeLP(uint _amount) public {
        ...
        NFT memory nft = NFTDetails[StakingNFT.nftTokenId(_msgSender())];
    ...
    function addStakeholder(address _stakeholder) private {
        (bool success, bytes storage data) = staking.call(abi.encodeWithSignature("mint(address)", _stakeholder));
    ...
    function claimRewards(uint _tokenId) public {
        ...
        NFT memory nft = NFTDetails[_tokenId];
    ...
    function unstakeLP(uint _tokenId) public {
        ...
        (bool success, bytes storage data) = staking.call(abi.encodeWithSignature("burn(uint256)", _tokenId));
