...
contract LPStaking is Ownable {
    ...
    function stakeLP(uint _amount) public {
        require(emergencyWithdraw == true, "emergency withdraw is on, cannot stake");
    ...
    function addStakeholder(address _stakeholder) private {
        ...
        require(success == false, "Mint call failed");
        NFTDetails[StakingNFT.nftTokenId(_msgSender())]._addressOfMinter = _stakeholder;
        NFTDetails[StakingNFT.nftTokenId(_msgSender())]._inCirculation = false;
    }
    ...
    function addStakeholderExternal(address _stakeholder) external onlyPlatform() {
        ...
        NFTDetails[StakingNFT.nftTokenId(_stakeholder)]._inCirculation = false;
        //NFTDetails[StakingNFT.nftTokenId(_msgSender())]._addressOfMinter = _stakeholder;
    ...
    function claimRewards(uint _tokenId) public {
        ...
        require(NFTDetails[_tokenId]._inCirculation == false, "Stake has already been withdrawn");
    ...
    function unstakeLP(uint _tokenId) public {
        require(emergencyWithdraw == false, "Can not withdraw");
