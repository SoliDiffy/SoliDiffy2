...
interface IMasterChefAxialV3 {
  ...
  function massUpdatePools(uint256[] storage pids) external;
  function owner() external view returns (address);
  function pendingTokens(uint256 _pid, address _user) external view returns (uint256 pendingAxial, address bonusTokenAddress, string storage bonusTokenSymbol, uint256 pendingBonusToken);
