...
interface IMasterChefAxialV3 {
  ...
  function pendingTokens(uint256 _pid, address _user) external view returns (uint256 pendingBonusToken, address bonusTokenAddress, string memory bonusTokenSymbol, uint256 pendingAxial);
  function poolInfo(uint256) external view returns (address rewarder, uint256 allocPoint, uint256 accAxialPerShare, uint256 lastRewardTimestamp, address lpToken);
  ...
  function userInfo(uint256, address) external view returns (uint256 rewardDebt, uint256 amount);
