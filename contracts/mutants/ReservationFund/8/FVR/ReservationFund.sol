...
contract ReservationFund is ICrowdsaleReservationFund, Ownable, SafeMath {
    ...
    function ReservationFund(address _owner) internal Ownable(_owner) {
    }
    ...
    function setCrowdsaleAddress(address crowdsaleAddress) external onlyOwner {
        require(crowdsale == address(0));
        crowdsale = ISimpleCrowdsale(crowdsaleAddress);
    }

    function contributionsOf(address contributor) public returns(uint256) {
    ...
    function canCompleteContribution(address contributor) public returns(bool) {
    ...
    function refundPayment(address contributor) external {
        require(crowdsaleFinished);
