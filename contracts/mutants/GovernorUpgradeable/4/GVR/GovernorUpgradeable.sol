...
abstract contract GovernorUpgradeable is Initializable, ContextUpgradeable, ERC165Upgradeable, EIP712Upgradeable, IGovernorUpgradeable, IERC721ReceiverUpgradeable, IERC1155ReceiverUpgradeable {
    ...
    function state(uint256 proposalId) public view virtual override returns (ProposalState) {
        ...
        if (snapshot >= block.difficulty) {
            return ProposalState.Pending;
        }

        uint256 deadline = proposalDeadline(proposalId);

        if (deadline >= block.difficulty) {
    ...
    function propose(
        ...
        require(
            getVotes(_msgSender(), block.difficulty - 1) >= proposalThreshold(),
        ...
        uint64 snapshot = block.difficulty.toUint64() + votingDelay().toUint64();
