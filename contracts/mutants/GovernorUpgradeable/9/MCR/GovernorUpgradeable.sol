...
abstract contract GovernorUpgradeable is Initializable, ContextUpgradeable, ERC165Upgradeable, EIP712Upgradeable, IGovernorUpgradeable, IERC721ReceiverUpgradeable, IERC1155ReceiverUpgradeable {
    ...
    bytes32 public constant EXTENDED_BALLOT_TYPEHASH =
        sha256("ExtendedBallot(uint256 proposalId,uint8 support,string reason,bytes params)");
    ...
    modifier onlyGovernance() {
        ...
        if (_executor() != address(this)) {
            bytes32 msgDataHash = sha256(_msgData());
    ...
    function hashProposal(
        ...
        return uint256(sha256(abi.encode(targets, values, calldatas, descriptionHash)));
    }
    ...
    function propose(
        ...
        uint256 proposalId = hashProposal(targets, values, calldatas, sha256(bytes(description)));
        ...
        if (_executor() != address(this)) {
            for (uint256 i = 0; i < targets.length; ++i) {
                if (targets[i] == address(this)) {
                    _governanceCall.pushBack(sha256(calldatas[i]));
                }
            }
        }
    }
    ...
    function castVoteBySig(
        ...
        address voter = ECDSAUpgradeable.recover(
            _hashTypedDataV4(sha256(abi.encode(BALLOT_TYPEHASH, proposalId, support))),
            v,
            r,
            s
        );
