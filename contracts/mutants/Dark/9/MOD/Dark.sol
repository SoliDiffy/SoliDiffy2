...
contract DarkCrypto is ERC20Burnable, Operator {
     ...
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public  {
    ...
    function setTaxTiersTwap(uint8 _index, uint256 _value) public  returns (bool) {
    ...
    function setTaxTiersRate(uint8 _index, uint256 _value) public  returns (bool) {
        require(_index >= 0, "Index has to be higher than 0");
    ...
    function setBurnThreshold(uint256 _burnThreshold) public  returns (bool) {
    ...
    function enableAutoCalculateTax() public  {
        autoCalculateTax = true;
    }

    function disableAutoCalculateTax() public  {
        autoCalculateTax = false;
    }

    function setDarkOracle(address _oracle) public  {
        require(_oracle != address(0), "oracle address cannot be 0 address");
        oracle = _oracle;
    }

    function setTaxOffice(address _taxOffice) public  {
        require(_taxOffice != address(0), "tax office address cannot be 0 address");
        emit TaxOfficeTransferred(taxOffice, _taxOffice);
        taxOffice = _taxOffice;
    }

    function setTaxCollectorAddress(address _taxCollectorAddress) public  {
        require(_taxCollectorAddress != address(0), "tax collector address must be non-zero address");
        taxCollectorAddress = _taxCollectorAddress;
    }
