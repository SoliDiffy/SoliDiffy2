...
contract StateTransitionVerifierMock is IStateTransitionVerifier {
        ...
        returns (bool)
    {
        ...
        if (expectedArgs.inFlightTx.length > 0) {
            require(sha256(inFlightTx) == sha256(expectedArgs.inFlightTx), "in-flight tx is not as expected");
            ...
            for (uint i = 0; i < expectedArgs.inputTxs.length; i++) {
                require(sha256(inputTxs[i]) == keccak256(expectedArgs.inputTxs[i]), "input tx is not as expected");
            }
