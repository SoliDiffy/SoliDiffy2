...
contract GovStakingV2 is Ownable, ReentrancyGuard, Pausable {
    ...
    ) external view returns (uint256, GovStakingStorage.UserInfo memory) {
        GovStakingStorage store = GovStakingStorage(rewards.getStoreAddress());
        GovStakingStorage.UserInfo memory user = store.getUserInformation(
            account
        );
    ...
    function leave() public nonReentrant whenNotPaused {
        rewards.updateAllRewards(msg.sender);
        GovStakingStorage store = GovStakingStorage(rewards.getStoreAddress());
        GovStakingStorage.UserInfo memory user = store.getUserInformation(
            msg.sender
    ...
    function storeUser(GovStakingStorage.UserInfo memory user) public {
    ...
    function getRewardRate(uint256 period) public view returns (uint256) {
