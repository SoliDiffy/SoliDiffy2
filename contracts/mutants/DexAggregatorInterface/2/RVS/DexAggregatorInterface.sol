...
interface DexAggregatorInterface {
    ...
    function getPrice(address desToken, address quoteToken, bytes memory data) external view returns (uint8 decimals, uint256 price);

    function getAvgPrice(address desToken, address quoteToken, uint32 secondsAgo, bytes memory data) external view returns (uint256 timestamp, uint256 price, uint8 decimals);
