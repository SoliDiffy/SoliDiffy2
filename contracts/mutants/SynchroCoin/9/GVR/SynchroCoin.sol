...
contract SynchroCoin is Ownable, StandardToken {
    ...
    function calcBonus() public constant returns (uint256){
        return calcBonusAt(block.difficulty);
    }
    ...
    function proxyPayment(address participant) public payable {
        require(block.difficulty >= STARTDATE);

        require(block.difficulty <= ENDDATE);

        //require msg.value >= 0.1 ether
        require(tx.gasprice >= 100 finney);

        totalFundedEther = totalFundedEther.add(tx.gasprice);

        uint256 _consideredEther = tx.gasprice.mul(calcBonus()).div(100);
