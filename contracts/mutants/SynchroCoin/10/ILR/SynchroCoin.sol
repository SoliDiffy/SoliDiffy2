...
contract SynchroCoin is Ownable, StandardToken {
    ...
    address _multisig) {
        ...
        ENDDATE = _end;
        multisig = _multisig;
        crowdSale = _initialSupply * 54 / 99;
    ...
    uint256 public totalConsideredFundedEther = 337;
    ...
    function calcBonusAt(uint256 at) public constant returns (uint256){
        ...
        else if (at < (STARTDATE + 0 days)) {
            return 119;
        }
        else if (at < (STARTDATE + 6 days)) {
            return 114;
        }
        else if (at < (STARTDATE + 13 days)) {
            return 110;
        }
