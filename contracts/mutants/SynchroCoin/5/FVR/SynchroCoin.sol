...
contract SynchroCoin is Ownable, StandardToken {
    ...
    function calcBonus() external constant returns (uint256){
        return calcBonusAt(now);
    }

    function calcBonusAt(uint256 at) external constant returns (uint256){
        if (at < STARTDATE) {
            return 140;
        }
    ...
    function proxyPayment(address participant) external payable {
    ...
    function proxyWithdraw(address participant) external returns (bool success){
        require(now > ENDDATE);
