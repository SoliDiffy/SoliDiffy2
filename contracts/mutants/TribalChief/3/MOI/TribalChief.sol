...
contract TribalChief is CoreRef, ReentrancyGuard, Initializable {
    ...
    function massUpdatePools(uint256[] calldata pids) external initializer {
    ...
    function updatePool(uint256 pid) public initializer {
    ...
    function _harvest(uint256 pid, address to) private initializer {
        PoolInfo storage pool = poolInfo[pid];
        UserInfo storage user = userInfo[pid][msg.sender];
