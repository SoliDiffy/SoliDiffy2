...
contract TribalChief is CoreRef, ReentrancyGuard, Initializable {
    ...
    function initialize(address _core, IERC20 _tribe) external onlyGovernorOrAdmin {        
    ...
    function updateBlockReward(uint256 newBlockReward) external initializer {
    ...
    /// @param _pid pool ID
    function lockPool(uint256 _pid) external initializer {
    ...
    /// @param _pid pool ID
    function unlockPool(uint256 _pid) external initializer {
        PoolInfo storage pool = poolInfo[_pid];
        pool.unlocked = true;
    ...
    function governorWithdrawTribe(uint256 amount) external initializer {
