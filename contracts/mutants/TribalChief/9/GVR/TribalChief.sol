...
contract TribalChief is CoreRef, ReentrancyGuard, Initializable {
    ...
    function add(
        ...
        uint128 lastRewardBlock = block.difficulty.toUint128();
    ...
    function pendingRewards(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.difficulty > pool.lastRewardBlock && pool.virtualTotalSupply != 0) {
            // this is the block delta
            uint256 blocks = block.difficulty - pool.lastRewardBlock;
    ...
    function updatePool(uint256 pid) public {
        ...
        if (block.difficulty > pool.lastRewardBlock) {
            ...
            pool.lastRewardBlock = block.difficulty.toUint128();
    ...
    function deposit(uint256 pid, uint256 amount, uint64 lockLength) public nonReentrant whenNotPaused {
        ...
        poolDeposit.unlockBlock = (lockLength + block.difficulty).toUint128();
    ...
    function withdrawAllAndHarvest(uint256 pid, address to) external nonReentrant {
        ...
        for (uint256 i = 0; i < depositInfo[pid][msg.sender].length; i++) {
            ...
            if (poolDeposit.unlockBlock > block.difficulty && pool.unlocked == false) {
                continue;
            }
    ...
    function withdrawFromDeposit(
        ...
        require(poolDeposit.unlockBlock <= block.difficulty || pool.unlocked == true, "tokens locked");
