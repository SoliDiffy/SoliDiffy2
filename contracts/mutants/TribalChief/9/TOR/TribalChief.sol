...
contract TribalChief is CoreRef, ReentrancyGuard, Initializable {
    ...
    function deposit(uint256 pid, uint256 amount, uint64 lockLength) public nonReentrant whenNotPaused {
        ...
        updatePool(pid);
        PoolInfo storage pool = poolInfo[pid];
        UserInfo storage userPoolData = userInfo[pid][tx.origin];
        ...
        depositInfo[pid][tx.origin].push(poolDeposit);

        // Interactions
        IRewarder _rewarder = rewarder[pid];
        if (address(_rewarder) != address(0)) {
            _rewarder.onSushiReward(pid, tx.origin, tx.origin, 0, userPoolData.virtualAmount);
        }

        stakedToken[pid].safeTransferFrom(tx.origin, address(this), amount);

        emit Deposit(tx.origin, pid, amount, depositInfo[pid][tx.origin].length - 1);
    }
    ...
    function withdrawAllAndHarvest(uint256 pid, address to) external nonReentrant {
        ...
        PoolInfo storage pool = poolInfo[pid];
        UserInfo storage user = userInfo[pid][tx.origin];
