...
contract TribalChief is CoreRef, ReentrancyGuard, Initializable {
    ...
    function initialize(address _core, IERC20 _tribe) public initializer {        
    ...
    function updateBlockReward(uint256 newBlockReward) public onlyGovernorOrAdmin {
    ...
    /// @param _pid pool ID
    function lockPool(uint256 _pid) public onlyGovernorOrAdmin {
    ...
    /// @param _pid pool ID
    function unlockPool(uint256 _pid) public onlyGovernorOrAdmin {
        PoolInfo storage pool = poolInfo[_pid];
        pool.unlocked = true;
    ...
    function governorWithdrawTribe(uint256 amount) public onlyGovernor {
