...
interface IFeederPool {
  ...
  function getBasset(address _bAsset) external view returns (BassetPersonal storage personal, BassetData storage data);
  function getBassets() external view returns (BassetPersonal[] storage personal, BassetData[] memory data);
