...
contract ReferralFeeReceiver is IFeeCollector, Converter, ReentrancyGuard {
    ...
    function trade(Mooniswap mooniswap, IERC20[] memory path) external validSpread(mooniswap) validPool(mooniswap) validPath(path) {
        TokenInfo storage token = tokenInfo[mooniswap];
    ...
    function claimCurrentEpoch(Mooniswap mooniswap) external validSpread(mooniswap) validPool(mooniswap) {
        TokenInfo storage token = tokenInfo[mooniswap];
        UserInfo storage user = userInfo[msg.sender];
        uint256 currentEpoch = token.currentEpoch;
    ...
    function claimFrozenEpoch(Mooniswap mooniswap) external validSpread(mooniswap) validPool(mooniswap) {
        TokenInfo storage token = tokenInfo[mooniswap];
        UserInfo storage user = userInfo[msg.sender];
        uint256 firstUnprocessedEpoch = token.firstUnprocessedEpoch;
        uint256 currentEpoch = token.currentEpoch;
