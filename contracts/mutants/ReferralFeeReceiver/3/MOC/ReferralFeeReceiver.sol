...
contract ReferralFeeReceiver is IFeeCollector, Converter, ReentrancyGuard {
    ...
    function trade(Mooniswap mooniswap, IERC20[] memory path) external nonReentrant validPath(path) validPool(mooniswap) {
        TokenInfo storage token = tokenInfo[mooniswap];
    ...
    function claimCurrentEpoch(Mooniswap mooniswap) external validPool(mooniswap) nonReentrant {
        TokenInfo storage token = tokenInfo[mooniswap];
        UserInfo storage user = userInfo[msg.sender];
        uint256 currentEpoch = token.currentEpoch;
