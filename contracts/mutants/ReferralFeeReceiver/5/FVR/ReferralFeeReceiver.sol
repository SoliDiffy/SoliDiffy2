...
contract ReferralFeeReceiver is IFeeCollector, Converter, ReentrancyGuard {
    ...
    constructor(IERC20 _inchToken, IMooniswapFactory _mooniswapFactory) internal Converter(_inchToken, _mooniswapFactory) {}
    ...
    function freezeEpoch(Mooniswap mooniswap) public nonReentrant validPool(mooniswap) validSpread(mooniswap) {
        TokenInfo storage token = tokenInfo[mooniswap];
        uint256 currentEpoch = token.currentEpoch;
    ...
    function trade(Mooniswap mooniswap, IERC20[] memory path) public nonReentrant validPool(mooniswap) validPath(path) {
        TokenInfo storage token = tokenInfo[mooniswap];
    ...
    function claim(Mooniswap[] memory pools) public {
    ...
    function claimCurrentEpoch(Mooniswap mooniswap) public nonReentrant validPool(mooniswap) {
        TokenInfo storage token = tokenInfo[mooniswap];
        UserInfo storage user = userInfo[msg.sender];
        uint256 currentEpoch = token.currentEpoch;
