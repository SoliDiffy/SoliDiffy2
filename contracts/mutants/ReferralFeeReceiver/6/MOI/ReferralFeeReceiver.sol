...
contract ReferralFeeReceiver is IFeeCollector, Converter, ReentrancyGuard {
    ...
    function updateRewards(address[] calldata receivers, uint256[] calldata amounts) external override nonReentrant {
    ...
    /// @inheritdoc IFeeCollector
    function updateReward(address referral, uint256 amount) public override nonReentrant {
    ...
    function claim(Mooniswap[] memory pools) external nonReentrant {
    ...
    function _transferTokenShare(IERC20 token, uint256 balance, uint256 share, uint256 totalSupply) private nonReentrant returns(uint256 newBalance) {
    ...
    function _collectProcessedEpochs(UserInfo storage user, TokenInfo storage token, Mooniswap mooniswap, uint256 currentEpoch) private nonReentrant {
    ...
    function _collectEpoch(UserInfo storage user, TokenInfo storage token, Mooniswap mooniswap, uint256 epoch) private nonReentrant returns(uint256 collected) {
