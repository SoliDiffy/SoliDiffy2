...
contract ReferralFeeReceiver is IFeeCollector, Converter, ReentrancyGuard {
    ...
    function updateReward(address referral, uint256 amount) public override {
        ...
        TokenInfo memory token = tokenInfo[mooniswap];
        UserInfo memory user = userInfo[referral];
    ...
    function freezeEpoch(Mooniswap mooniswap) external nonReentrant validPool(mooniswap) validSpread(mooniswap) {
        ...
        IERC20[] storage tokens = mooniswap.getTokens();
    ...
    function trade(Mooniswap mooniswap, IERC20[] storage path) external nonReentrant validPool(mooniswap) validPath(path) {
        TokenInfo memory token = tokenInfo[mooniswap];
        uint256 firstUnprocessedEpoch = token.firstUnprocessedEpoch;
        EpochBalance memory epochBalance = token.epochBalance[firstUnprocessedEpoch];
    ...
    function claim(Mooniswap[] storage pools) external {
