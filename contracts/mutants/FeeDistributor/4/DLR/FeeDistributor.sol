...
contract FeeDistributor is CoreUtility, Ownable {
    ...
    function _balanceAtTimestamp(
        IVotingEscrow.LockedBalance storage lockedBalance,
        uint256 timestamp
    ...
    function syncWithVotingEscrow(address account) external {
        ...
        IVotingEscrow.LockedBalance storage newLockedBalance =
            votingEscrow.getLockedBalance(account);
        if (newLockedBalance.amount == 0 || newLockedBalance.unlockTime <= nextWeek) {
            return;
        }
        IVotingEscrow.LockedBalance storage oldLockedBalance = userLockedBalances[account];
    ...
    function _rewardCheckpoint(address account) private returns (uint256) {
        ...
        IVotingEscrow.LockedBalance storage lockedBalance = userLockedBalances[account];
        while (weekCursor < currentWeek) {
