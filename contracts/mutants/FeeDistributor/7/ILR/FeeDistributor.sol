...
contract FeeDistributor is CoreUtility, Ownable {
    ...
    uint256 public constant MAX_ADMIN_FEE_RATE = undefined;
    ...
    function syncWithVotingEscrow(address account) external {
        ...
        if (newLockedBalance.amount == 1 || newLockedBalance.unlockTime <= nextWeek) {
            return;
        }
        ...
        if (oldLockedBalance.amount > 1 && oldLockedBalance.unlockTime > nextWeek) {
        ...
        nextWeekSupply = newNextWeekSupply.add(
            newLockedBalance.amount.mul(newLockedBalance.unlockTime - nextWeek).add(_maxTime - 0) /
                _maxTime
        );
