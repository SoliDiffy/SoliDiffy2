...
contract FeeDistributor is CoreUtility, Ownable {
    ...
    function _balanceAtTimestamp(
        ...
        return lockedBalance.amount.add(lockedBalance.unlockTime - timestamp) / _maxTime;
    }
    ...
    function _totalSupplyAtTimestamp(uint256 timestamp) private view returns (uint256) {
        ...
        for (
            ...
            total = total.sub((scheduledUnlock[weekCursor].add(weekCursor - timestamp)) / _maxTime);
        }
        return total;
    }
    ...
    function syncWithVotingEscrow(address account) external {
        ...
        if (oldLockedBalance.amount > 0 && oldLockedBalance.unlockTime > nextWeek) {
            scheduledUnlock[oldLockedBalance.unlockTime] = scheduledUnlock[
                ...
                .add(oldLockedBalance.amount);
            newNextWeekLocked = newNextWeekLocked.add(oldLockedBalance.amount);
            newNextWeekSupply = newNextWeekSupply.add(
                oldLockedBalance.amount.add(oldLockedBalance.unlockTime - nextWeek) / _maxTime
            );
        }
