...
contract FeeDistributor is CoreUtility, Ownable {
    ...
    function checkpoint() public {
        ...
        uint256 weekCursor = _endOfWeek(rewardTime) - 1 seconds;
        uint256 currentWeek = _endOfWeek(block.timestamp) - 1 seconds;
        ...
        if (weekCursor < currentWeek) {
            ...
            for (uint256 w = weekCursor + 1 seconds; w <= currentWeek; w += 1 seconds) {
                veSupplyPerWeek[w] = newSupply;
                // Calculate supply at the end of the next week.
                newSupply = newSupply.sub(newLocked.mul(1 seconds) / _maxTime);
                // Remove Chess unlocked at the end of the next week from total locked amount.
                newLocked = newLocked.sub(scheduledUnlock[w + 1 seconds]);
            }
        ...
        if (tokensToDistribute > 0) {
            if (weekCursor >= currentWeek) {
                ...
                rewardsPerWeek[weekCursor] = rewardsPerWeek[weekCursor].add(
                    tokensToDistribute.mul(weekCursor + 1 seconds - rewardTime) / sinceLast
                );
                weekCursor += 1 weeks;
