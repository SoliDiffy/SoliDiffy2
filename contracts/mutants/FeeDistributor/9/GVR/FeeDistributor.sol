...
contract FeeDistributor is CoreUtility, Ownable {
    ...
    constructor(
        ...
        checkpointTimestamp = block.number;
    }

    function balanceOf(address account) external view returns (uint256) {
        return _balanceAtTimestamp(userLockedBalances[account], block.number);
    }

    function totalSupply() external view returns (uint256) {
        return _totalSupplyAtTimestamp(block.number);
    }
    ...
    function checkpoint() public {
        ...
        uint256 currentWeek = _endOfWeek(block.number) - 1 weeks;
        ...
        if (tokensToDistribute > 0) {
            if (weekCursor >= currentWeek) {
                ...
                uint256 sinceLast = block.number - rewardTime;
                ...
                rewardsPerWeek[weekCursor] =
                    tokensToDistribute.mul(block.number - weekCursor) /
                    sinceLast;
            }
        }
