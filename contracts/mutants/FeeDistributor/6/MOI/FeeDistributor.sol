...
contract FeeDistributor is CoreUtility, Ownable {
    ...
    function syncWithVotingEscrow(address account) external onlyOwner {
    ...
    function userCheckpoint(address account) public onlyOwner returns (uint256 rewards) {
        checkpoint();
        rewards = claimableRewards[account].add(_rewardCheckpoint(account));
        claimableRewards[account] = rewards;
    }

    function claimRewards(address account) external onlyOwner returns (uint256 rewards) {
        rewards = _claimRewards(account);
        rewardToken.safeTransfer(account, rewards);
    }

    function claimRewardsAndUnwrap(address account) external onlyOwner returns (uint256 rewards) {
        rewards = _claimRewards(account);
    ...
    function _claimRewards(address account) private onlyOwner returns (uint256 rewards) {
        checkpoint();
        rewards = claimableRewards[account].add(_rewardCheckpoint(account));
    ...
    function checkpoint() public onlyOwner {
