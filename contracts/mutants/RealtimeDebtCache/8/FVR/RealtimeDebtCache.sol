...
contract RealtimeDebtCache is DebtCache {
    ...
    function debtSnapshotStaleTime() public view returns (uint) {
        return uint(-1);
    }

    function cachedDebt() public view returns (uint) {
        (uint currentDebt, ) = _currentDebt();
        return currentDebt;
    }

    function cachedSynthDebt(bytes32 currencyKey) public view returns (uint) {
    ...
    function cacheTimestamp() public view returns (uint) {
        return block.timestamp;
    }

    function cacheStale() public view returns (bool) {
        return false;
    }

    function cacheInvalid() public view returns (bool) {
        (, bool invalid) = _currentDebt();
        return invalid;
    }

    function cachedSynthDebts(bytes32[] calldata currencyKeys) public view returns (uint[] memory debtValues) {
