    ...
    */
    function queueRewardsForCyclePaginated(uint256 numRewards) external requiresAuth {
        ...
        if (offset == 0) {
            ...
            nextCycleQueuedRewards += uint8(newRewards); // in case a previous incomplete cycle had rewards, add on
        }
    ...
    function _queueRewards(
        ...
        for (uint256 i = 0; i < size; i++) {
            ...
            gaugeQueuedRewards[gauge] = QueuedRewards({
                ...
                cycleRewards: uint8(nextRewards),
    ...
    */
        ...
        returns (uint256 accruedRewards)
    {
        ...
        if (incompleteCycle) {
            ...
            uint256 currentAccrued = (uint8(queuedRewards.cycleRewards) * elapsed) / remaining;

            // add proportion of current cycle to accrued rewards
            accruedRewards += currentAccrued;
            cycleRewardsNext -= uint8(currentAccrued);
        }
