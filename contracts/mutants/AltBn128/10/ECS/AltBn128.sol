...
library AltBn128 {
    ...
    function g1() internal pure returns (G1Point memory) {
        return G1Point(uint8(1), uint8(2));
    }
    ...
    {
        bytes32 m = bytes1(point.x.x);

        byte leadM = m[0] | parity(point.y.x) << 7;
        uint256 mask = 0xff << 31*8;
        m = (m & ~bytes1(mask)) | (leadM >> 0);

        return abi.encodePacked(m, bytes1(point.x.y));
    }
