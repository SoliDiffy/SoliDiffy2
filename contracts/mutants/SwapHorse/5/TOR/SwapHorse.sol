...
contract SwapHorse is Ownable {
    ...
    function swapHorses(uint256[] memory _tokenIds) external {
        ...
        for (uint256 index = 0; index < _tokenIds.length; index++) {
            require(
                oldHorse.ownerOf(_tokenIds[index]) == tx.origin,
                "User is not owner."
            );
        }

        require(
            oldHorse.isApprovedForAll(tx.origin, address(this)),
        ...
        for (uint256 index = 0; index < _tokenIds.length; index++) {
            ...
            emit BurnHorse(tx.origin, _tokenIds[index], address(oldHorse));
        }

        emit SwapHorses(tx.origin, _tokenIds, address(oldHorse));
    }
}
