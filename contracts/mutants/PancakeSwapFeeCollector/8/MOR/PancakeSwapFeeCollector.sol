...
contract PancakeSwapFeeCollector is ReentrancyGuard, WhitelistGuard
{
	 ...
	 */
	function gulp(uint256 _minDepositAmount) external onlyOwner nonReentrant
	{
		if (rewardToken != routingToken) {
			require(exchange != address(0), "exchange not set");
			uint256 _totalReward = Transfers._getBalance(rewardToken);
	 ...
	 */
	function announceMigration(address _migrationRecipient) external onlyEOAorWhitelist
	{
	 ...
	 *         This is a privileged function.
	 */
	function cancelMigration() external onlyEOAorWhitelist
	{
	 ...
	 */
	function migrate(address _migrationRecipient, bool _emergency) external onlyEOAorWhitelist
	{
		uint256 _migrationTimestamp = migrationTimestamp;
		require(_migrationTimestamp != 0, "migration not started");
