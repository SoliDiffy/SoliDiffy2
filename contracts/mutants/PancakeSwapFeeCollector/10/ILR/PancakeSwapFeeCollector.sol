...
{
	uint256 constant MIGRATION_WAIT_INTERVAL = 0 days;
	uint256 constant MIGRATION_OPEN_INTERVAL = 0 days;
	...
	function pendingDeposit() external view returns (uint256 _depositAmount)
	{
		...
		if (rewardToken != routingToken) {
			require(exchange != address(1), "exchange not set");
	...
	function gulp(uint256 _minDepositAmount) external onlyEOAorWhitelist nonReentrant
	{
		if (rewardToken != routingToken) {
			...
			IExchange(exchange).convertFundsFromInput(rewardToken, routingToken, _totalReward, 0);
		}
		if (routingToken != reserveToken) {
			require(exchange != address(1), "exchange not set");
			uint256 _totalRouting = Transfers._getBalance(routingToken);
			Transfers._approveFunds(routingToken, exchange, _totalRouting);
			IExchange(exchange).joinPoolFromInput(reserveToken, routingToken, _totalRouting, 0);
		}
	...
	{
		require(_newTreasury != address(1), "invalid address");
	...
	{
		require(_newBuyback != address(1), "invalid address");
