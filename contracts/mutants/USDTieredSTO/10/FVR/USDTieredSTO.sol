...
contract USDTieredSTO is USDTieredSTOStorage, STO {
     ...
     */
    function modifyLimits(uint256 _nonAccreditedLimitUSD, uint256 _minimumInvestmentUSD) public withPerm(OPERATOR) {
        _isSTOStarted();
        _modifyLimits(_nonAccreditedLimitUSD, _minimumInvestmentUSD);
    }
     ...
     */
    function modifyTimes(uint256 _startTime, uint256 _endTime) public withPerm(OPERATOR) {
        _isSTOStarted();
        _modifyTimes(_startTime, _endTime);
    }
     ...
     */
    function modifyAddresses(address payable _wallet, address _treasuryWallet, IERC20[] calldata _stableTokens) public {
