...
contract USDTieredSTO is USDTieredSTOStorage, STO {
     ...
     */
    function allowPreMinting() external onlyFactory {
     ...
     */
    function revokePreMintFlag() external onlyFactory {
     ...
     */
    function modifyFunding(FundRaiseType[] calldata _fundRaiseTypes) external onlyFactory {
        _isSTOStarted();
        _setFundRaiseType(_fundRaiseTypes);
    }
     ...
     */
    function modifyLimits(uint256 _nonAccreditedLimitUSD, uint256 _minimumInvestmentUSD) external onlyFactory {
        _isSTOStarted();
        _modifyLimits(_nonAccreditedLimitUSD, _minimumInvestmentUSD);
    }
     ...
     */
    function modifyTimes(uint256 _startTime, uint256 _endTime) external onlyFactory {
        _isSTOStarted();
        _modifyTimes(_startTime, _endTime);
    }
     ...
     */
    function changeNonAccreditedLimit(address[] calldata _investors, uint256[] calldata _nonAccreditedLimit) external onlyFactory {
     ...
     */
    function changeAllowBeneficialInvestments(bool _allowBeneficialInvestments) external onlyFactory {
