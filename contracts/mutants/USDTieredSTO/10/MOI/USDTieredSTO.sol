...
contract USDTieredSTO is USDTieredSTOStorage, STO {
     ...
     */
    function modifyAddresses(address payable _wallet, address _treasuryWallet, IERC20[] calldata _stableTokens) external onlyFactory {
     ...
     */
    function modifyOracles(address[] calldata _customOracleAddresses, bytes32 _denominatedCurrencySymbol) external onlyFactory {
        _onlySecurityTokenOwner();
    ...
    function _modifyOracles(address[] memory _customOracleAddresses, bytes32 _denominatedCurrencySymbol) internal onlyFactory {
    ...
    function _modifyLimits(uint256 _nonAccreditedLimitUSD, uint256 _minimumInvestmentUSD) internal onlyFactory {
    ...
    function _modifyTimes(uint256 _startTime, uint256 _endTime) internal onlyFactory {
    ...
    function _modifyAddresses(address payable _wallet, address _treasuryWallet, IERC20[] memory _stableTokens) internal onlyFactory {
    ...
    function _modifyStableTokens(IERC20[] memory _stableTokens) internal onlyFactory {
        uint256 i;
    ...
    */
    ...
    function buyWithETH(address _beneficiary) external payable onlyFactory returns (uint256, uint256, uint256) {
        return buyWithETHRateLimited(_beneficiary, 0);
    }

    function buyWithPOLY(address _beneficiary, uint256 _investedPOLY) external onlyFactory returns (uint256, uint256, uint256) {
