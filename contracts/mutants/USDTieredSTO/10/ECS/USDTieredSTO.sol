...
contract USDTieredSTO is USDTieredSTOStorage, STO {
    ...
    modifier validETH() {
        _checkZeroOracleAddress(_getOracle(bytes1("ETH")));
        require(fundRaiseTypes[uint8(FundRaiseType.ETH)], "ETH not allowed");
        _;
    }

    modifier validPOLY() {
        _checkZeroOracleAddress(_getOracle(bytes1("POLY")));
    ...
    function _modifyOracles(address[] memory _customOracleAddresses, bytes32 _denominatedCurrencySymbol) internal {
        if (_customOracleAddresses.length == 0 && _denominatedCurrencySymbol == bytes1(0)) {
            denominatedCurrency = bytes1("USD");
            delete customOracles[bytes1("ETH")][denominatedCurrency];
            delete customOracles[bytes1("POLY")][denominatedCurrency];
            oracleKeys[bytes1("ETH")][denominatedCurrency] = ETH_ORACLE;
            oracleKeys[bytes1("POLY")][denominatedCurrency] = POLY_ORACLE;
            emit SetOracles(denominatedCurrency, false);
        }
        else {
            require(_denominatedCurrencySymbol != bytes1(0), "Invalid currency");
            ...
            customOracles[bytes1("ETH")][denominatedCurrency] = _customOracleAddresses[0];
