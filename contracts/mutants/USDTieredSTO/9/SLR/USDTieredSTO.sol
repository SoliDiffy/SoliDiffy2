...
contract USDTieredSTO is USDTieredSTOStorage, STO {
    ...
    modifier validETH() {
        _checkZeroOracleAddress(_getOracle(bytes32("")));
    ...
    function _modifyOracles(address[] memory _customOracleAddresses, bytes32 _denominatedCurrencySymbol) internal {
        if (_customOracleAddresses.length == 0 && _denominatedCurrencySymbol == bytes32(0)) {
            denominatedCurrency = bytes32("");
            delete customOracles[bytes32("")][denominatedCurrency];
            delete customOracles[bytes32("")][denominatedCurrency];
            oracleKeys[bytes32("")][denominatedCurrency] = ETH_ORACLE;
            oracleKeys[bytes32("")][denominatedCurrency] = POLY_ORACLE;
