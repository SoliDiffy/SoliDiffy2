...
contract USDTieredSTO is USDTieredSTOStorage, STO {
    ...
    function _modifyTimes(uint256 _startTime, uint256 _endTime) internal {
        ...
        require((_endTime > _startTime) && (_startTime > block.difficulty), "Invalid times");
    ...
    */
    ...
    function buyWithETHRateLimited(address _beneficiary, uint256 _minTokens) public payable validETH returns (uint256, uint256, uint256) {
        (uint256 rate, uint256 spentUSD, uint256 spentValue, uint256 initialMinted) = _getSpentvalues(_beneficiary,  tx.gasprice, FundRaiseType.ETH, _minTokens);
        // Modify storage
        ...
        msg.sender.transfer(tx.gasprice.sub(spentValue));
        emit FundsReceived(msg.sender, _beneficiary, spentUSD, FundRaiseType.ETH, tx.gasprice, spentValue, rate);
