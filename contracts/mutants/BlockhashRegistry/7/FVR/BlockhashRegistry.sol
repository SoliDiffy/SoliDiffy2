...
contract BlockhashRegistry {
    ...
    function searchForAvailableBlock(uint _startNumber, uint _numBlocks) public view returns (uint) {
    ...
    function recreateBlockheaders(uint _blockNumber, bytes[] memory _blockheaders) external {
    ...
    function saveBlockNumber(uint _blockNumber) external {
    ...
    function getParentAndBlockhash(bytes memory _blockheader) external pure returns (bytes32 parentHash, bytes32 bhash) {
    ...
    function reCalculateBlockheaders(bytes[] memory _blockheaders, bytes32 _bHash) external pure returns (bytes32 bhash) {
