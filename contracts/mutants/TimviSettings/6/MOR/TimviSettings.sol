...
//SWC-131-Presence of unused variables:L30
    ...
    function setMinDepo(uint256 _value) external onlySettingsManager {
        require(_value > 0 && _value < 10 ether, "Value out of range");
        MIN_DEPO = _value;
        emit MIN_DEPO_UPDATED(MIN_DEPO);
    }

    function setSysCom(uint256 _value) external onlySettingsManager {
    ...
    function setFeeTotal(uint256 _totalFee, uint256 _sysFee) external onlySettingsManager {
    ...
    function setOracleAddress(address _addr) external onlySettingsManager {
        require(_addr != address(0), "Zero address");
        oracleAddress = _addr;
    }

    function setSafetyBag(uint256 _bag) external onlySettingsManager {
        require(_bag <= 100000, "Value out of range");
        GLOBAL_SAFETY_BAG = _bag;
        emit GLOBAL_SAFETY_BAG_UPDATED(GLOBAL_SAFETY_BAG);
    }

    function setTmvAddress(address _addr) external onlyFeeManager {
        require(_addr != address(0), "Zero address");
        tmvAddress = _addr;
    }
