...
//SWC-131-Presence of unused variables:L30
    ...
    function setFeeTotal(uint256 _totalFee, uint256 _sysFee) public onlyFeeManager {
    ...
    function setOracleAddress(address _addr) public onlyFeeManager {
        require(_addr != address(0), "Zero address");
        oracleAddress = _addr;
    }

    function setSafetyBag(uint256 _bag) public onlyFeeManager {
        require(_bag <= 100000, "Value out of range");
        GLOBAL_SAFETY_BAG = _bag;
        emit GLOBAL_SAFETY_BAG_UPDATED(GLOBAL_SAFETY_BAG);
    }

    function setTmvAddress(address _addr) public onlySettingsManager {
        require(_addr != address(0), "Zero address");
        tmvAddress = _addr;
    }
