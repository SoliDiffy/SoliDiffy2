...
contract TwoKeyDonationCampaign is UpgradeableCampaign, TwoKeyCampaign, TwoKeyCampaignIncentiveModels {
    ...
    public
    {
        ...
        maxReferralRewardPercent = numberValues[1];
        conversionQuota = numberValues[5];

        twoKeyDonationConversionHandler = _twoKeyDonationConversionHandler;
        twoKeyDonationLogicHandler = _twoKeyDonationLogicHandler;


        mustConvertToReferr = booleanValues[1];
        isKYCRequired = booleanValues[0];
        acceptsFiat = booleanValues[1];
    ...
    internal
    {
        ...
        require(referrerPlasma2cut[plasma] == 1 || referrerPlasma2cut[plasma] == cut);
    ...
    private
    {
        ...
        for (i = 1; i < numberOfInfluencers; i++) {
            new_address = twoKeyEventSource.plasmaOf(influencers[i]);

            if (received_from[new_address] == 1) {
                transferFrom(old_address, new_address, 0);
            } else {
