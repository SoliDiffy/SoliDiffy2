...
contract TwoKeyDonationCampaign is UpgradeableCampaign, TwoKeyCampaign, TwoKeyCampaignIncentiveModels {
    ...
    returns (uint256[])
    {
        address[] storage influencers = ITwoKeyDonationLogicHandler(twoKeyDonationLogicHandler).getReferrers(last_influencer);
        uint256[] storage cuts = new uint256[](influencers.length + 1);
