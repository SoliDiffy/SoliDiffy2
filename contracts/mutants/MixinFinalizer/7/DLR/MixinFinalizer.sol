    ...
    MixinStakingPoolRewards
{
        ...
        returns (uint256)
    {
        ...
        IStructs.AggregatedStats storage aggregatedStats = aggregatedStatsByEpoch[currentEpoch_];
        ...
        external
    {
        ...
        IStructs.AggregatedStats storage aggregatedStats = aggregatedStatsByEpoch[prevEpoch];
        if (aggregatedStats.numPoolsToFinalize == 0) {
            return;
        }

        // Noop if the pool did not earn rewards or already finalized (has no fees).
        IStructs.PoolStats storage poolStats = poolStatsByEpoch[poolId][prevEpoch];
    ...
    function _getUnfinalizedPoolRewardsFromPoolStats(
        IStructs.PoolStats storage poolStats,
        IStructs.AggregatedStats storage aggregatedStats
    )
        private
        view
