...
contract HelperFunct {
  ...
  function _getComptrollerAddress() public pure returns (address) {
    return 0xFB558eCD2D24886e8d2956775C619deb22f154EF;
  }

  function _getCTokenAddr(address _asset) public view returns (address cTokenAddr) {
    if (_isETH(_asset)) {
   ...
   */
  function _enterCollatMarket(address _cTokenAddress) public {
   ...
   */
  function _exitCollatMarket(address _cTokenAddress) public {
    // Create a reference to the corresponding network Comptroller
    IFuseComptroller comptroller = IFuseComptroller(_getComptrollerAddress());
