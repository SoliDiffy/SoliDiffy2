...
contract OracleAggregator is OracleAggregatorErrors, ReentrancyGuard {
    ...
    function fetchData(address oracleId, uint256 timestamp) external payable nonReentrant enoughEtherProvided(oracleId, 1) {
    ...
    function recursivelyFetchData(address oracleId, uint256 timestamp, uint256 period, uint256 times) external payable nonReentrant enoughEtherProvided(oracleId, times) {
    ...
    function calculateFetchPrice(address oracleId) external returns(uint256 fetchPrice) {
    ...
    function _registerQuery(address oracleId, uint256 timestamp) public {
