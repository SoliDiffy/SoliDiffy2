...
contract ThorusMaster is Ownable, ReentrancyGuard {
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken) external onlyOwner {
        massUpdatePools();
        uint256 lastRewardSecond = block.number > startSecond
            ? block.number
            : startSecond;
    ...
    function pendingThorus(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardSecond && pool.lpSupply != 0 && totalAllocPoint > 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardSecond, block.number);
    ...
    function updatePool(uint256 _pid) public {
        PoolInfo storage pool = poolInfo[_pid];
        if (block.number <= pool.lastRewardSecond) {
            return;
        }
        if (pool.lpSupply == 0) {
            pool.lastRewardSecond = block.number;
            return;
        }
        uint256 multiplier = getMultiplier(pool.lastRewardSecond, block.number);
        uint256 thorusReward = multiplier * thorusPerSecond * pool.allocPoint / totalAllocPoint;
