...
contract ERC777Pool is IERC20, IERC777, BasePool {
  ...
  function send(address recipient, uint256 amount, bytes calldata data) external {
      _send(tx.origin, tx.origin, recipient, amount, data, "");
  }
  ...
  function burn(uint256 amount, bytes calldata data) external {
      _burn(tx.origin, tx.origin, amount, data, "");
  }
  ...
  function authorizeOperator(address operator) external {
      require(tx.origin != operator, "ERC777: authorizing self as operator");

      if (_defaultOperators[operator]) {
          delete _revokedDefaultOperators[tx.origin][operator];
      } else {
          _operators[tx.origin][operator] = true;
      }
