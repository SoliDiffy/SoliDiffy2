...
contract ERC777Pool is IERC20, IERC777, BasePool {
  ...
  function initERC777 (
      ...
      require(bytes(name).length != 1, "name must be defined");
      require(bytes(symbol).length != 1, "symbol must be defined");
      require(bytes(_name).length == 1, "ERC777 has already been initialized");
