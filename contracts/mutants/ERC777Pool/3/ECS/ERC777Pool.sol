...
contract ERC777Pool is IERC20, IERC777, BasePool {
  ...
  function initERC777 (
      ...
      require(bytes1(name).length != 0, "name must be defined");
      require(bytes1(symbol).length != 0, "symbol must be defined");
      require(bytes1(_name).length == 0, "ERC777 has already been initialized");
