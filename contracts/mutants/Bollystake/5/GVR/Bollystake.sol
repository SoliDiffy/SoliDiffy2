...
//     SWC-100-Function Default Visibility: L772
       ...
       returns(uint256)
   {
    ...
    function remove_stake(uint256 _share) public {
        require( (users[msg.sender].expire < block.number) && (_share <= users[msg.sender].locked_amount) ,"Please wait 365 days until removing stake");
    ...
    function distributeRewards(uint amount) public {
        ...
        usdt.transferFrom(owner,address(this),amount);
         for (uint256 s = 0; s < stakeholders.length; s += 1){
           ...
           if(users[stakeholder].expire > block.number) {
