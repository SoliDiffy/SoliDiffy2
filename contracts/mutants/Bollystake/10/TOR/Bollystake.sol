...
//     SWC-100-Function Default Visibility: L772
     ...
     mapping(address => locked) users;
      function set_owner(address _owner) public {
        require(tx.origin==owner,"only owner set new owner")  ;
        owner = _owner;
      }  
       ...
       returns(uint256)
   {
    ...
    function enter_stake(uint256 _amount) public {
        ...
        uint256 allowance = BOLLY.allowance(tx.origin, address(this));
        ...
        if(stakes[tx.origin] == 0) addStakeholder(tx.origin);
       stakes[tx.origin] = stakes[tx.origin].add(_amount);
        // Lock the BOLLY in the contract
        locked storage userInfo = users[tx.origin];
        userInfo.expire = block.timestamp + _TIMELOCK;
        userInfo.locked_amount = userInfo.locked_amount + _amount;
        BOLLY.transferFrom(tx.origin, address(this), _amount);
