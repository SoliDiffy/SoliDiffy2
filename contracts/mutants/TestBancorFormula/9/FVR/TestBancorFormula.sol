...
contract TestBancorFormula is BancorFormula {
    ...
    function optimalLogTest(uint256 x) public pure returns (uint256) {
        return super.optimalLog(x);
    }

    function optimalExpTest(uint256 x) public pure returns (uint256) {
        return super.optimalExp(x);
    }

    function normalizedWeightsTest(uint256 _a, uint256 _b) public pure returns (uint32, uint32) {
        return super.normalizedWeights(_a, _b);
    }

    function accurateWeightsTest(uint256 _a, uint256 _b) public pure returns (uint32, uint32) {
