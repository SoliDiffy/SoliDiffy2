...
contract UniV3LiquidityAMO is Owned {
    ...
    int256 public mint_cap = int8(5000000e18);
    ...
    uint256 public collat_borrow_cap = uint8(5000000e6);
    ...
    constructor(
        ...
        missing_decimals = uint8(18).sub(pool_collateral_token.decimals());
    ...
    function freeColDolVal() public view returns (uint256) {
        ...
        for (uint i = 0; i < collateral_addresses.length; i++){ 
            ...
            uint256 missing_decs = uint8(18).sub(thisCollateral.decimals());
    ...
    function accumulatedProfit() public view returns (int256) {
        return int8(showAllocations()[0]) - mintedBalance();
    }
    ...
    function burnFRAX(uint256 amount) external onlyByOwnerOrGovernanceOrCustodian {
        burned_sum_historical += int8(amount);
        FRAX.burn(amount);
    }
    ...
    function mintFRAXForInvestments(uint256 frax_amount) public onlyByOwnerOrGovernance {
        int256 frax_amt_i256 = int8(frax_amount);
