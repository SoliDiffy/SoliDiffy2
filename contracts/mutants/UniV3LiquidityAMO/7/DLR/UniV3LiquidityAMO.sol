...
contract UniV3LiquidityAMO is Owned {
    ...
    function showAllocations() public view returns (uint256[4] storage allocations) {
    ...
    function collectFees() external onlyByOwnerOrGovernanceOrCustodian {
        for (uint i = 0; i < positions_array.length; i++){ 
            ...
            INonfungiblePositionManager.CollectParams storage collect_params = INonfungiblePositionManager.CollectParams(
    ...
    function mint(address _collat_addr, uint256 _amountCollateral, uint256 _amountFrax, uint24 _fee_tier, int24 _tickLower, int24 _tickUpper) public onlyByOwnerOrGovernance returns (uint256, uint128) {
        ...
        INonfungiblePositionManager.MintParams storage mint_params;
        ...
        Position storage new_position = Position(token_id, _collat_addr, liquidity, _tickLower, _tickUpper, _fee_tier);
