...
contract UniV3LiquidityAMO is Owned {
    ...
    function burnFRAX(uint256 amount) external onlyByOwnerOrGovernance {
    ...
    function collectFees() external onlyByOwnerOrGovernance {
    ...
    function approveTarget(address _target, address _token, uint256 _amount, bool use_safe_approve) public onlyByOwnerOrGovernanceOrCustodian {
        if (use_safe_approve) {
    ...
    function mint(address _collat_addr, uint256 _amountCollateral, uint256 _amountFrax, uint24 _fee_tier, int24 _tickLower, int24 _tickUpper) public onlyByOwnerOrGovernanceOrCustodian returns (uint256, uint128) {
    ...
    function withdraw(uint256 _token_id) public onlyByOwnerOrGovernanceOrCustodian returns (uint256, uint256) {
    ...
    // Uni V3 only
    function swap(address _tokenA, address _tokenB, uint24 _fee_tier, uint256 _amountAtoB, uint256 _amountOutMinimum, uint160 _sqrtPriceLimitX96) public onlyByOwnerOrGovernanceOrCustodian returns (uint256) {
        // Make sure the collateral is allowed 
    ...
    function mintFRAXForInvestments(uint256 frax_amount) public onlyByOwnerOrGovernanceOrCustodian {
