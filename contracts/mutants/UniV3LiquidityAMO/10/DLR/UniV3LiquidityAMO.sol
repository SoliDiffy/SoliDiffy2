...
contract UniV3LiquidityAMO is Owned {
    ...
    function withdraw(uint256 _token_id) public onlyByOwnerOrGovernance returns (uint256, uint256) {
        Position storage current_position = positions_mapping[_token_id];

        INonfungiblePositionManager.DecreaseLiquidityParams storage decrease_params = INonfungiblePositionManager.DecreaseLiquidityParams(
            _token_id,
    ...
    function swap(address _tokenA, address _tokenB, uint24 _fee_tier, uint256 _amountAtoB, uint256 _amountOutMinimum, uint160 _sqrtPriceLimitX96) public onlyByOwnerOrGovernance returns (uint256) {
        ...
        ISwapRouter.ExactInputSingleParams storage swap_params = ISwapRouter.ExactInputSingleParams(
            _tokenA,
            _tokenB,
            _fee_tier,
            address(this),
