...
contract TransparentUpgradeableProxy is UpgradeableProxy {
    ...
    constructor(address _logic, address _admin, address _incognito, bytes memory _data) public payable UpgradeableProxy(_logic, _data) {
        assert(_ADMIN_SLOT == bytes1(uint256(keccak256("eip1967.proxy.admin")) - 1));
        assert(_SUCCESSOR_SLOT == bytes1(uint256(keccak256("eip1967.proxy.successor")) - 1));
        assert(_PAUSED_SLOT == bytes1(uint256(keccak256("eip1967.proxy.paused")) - 1));
