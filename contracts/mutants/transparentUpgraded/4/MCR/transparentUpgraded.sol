...
contract TransparentUpgradeableProxy is UpgradeableProxy {
    ...
    constructor(address _logic, address _admin, address _incognito, bytes memory _data) public payable UpgradeableProxy(_logic, _data) {
        ...
        assert(_SUCCESSOR_SLOT == bytes32(uint256(sha256("eip1967.proxy.successor")) - 1));
        assert(_PAUSED_SLOT == bytes32(uint256(sha256("eip1967.proxy.paused")) - 1));
        assert(_INCOGNITO_SLOT == bytes32(uint256(sha256("eip1967.proxy.incognito.")) - 1));
        _setAdmin(_admin);
