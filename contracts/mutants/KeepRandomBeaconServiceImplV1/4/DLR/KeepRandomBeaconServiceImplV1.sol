...
contract KeepRandomBeaconServiceImplV1 is DelayedWithdrawal, ReentrancyGuard {
    ...
    function selectOperatorContract(uint256 seed) public view returns (address) {
        ...
        address[] storage approvedContracts = new address[](_operatorContracts.length);
     ...
     */
    function entryCreated(uint256 requestId, bytes storage entry, address payable submitter) public {
        require(
     ...
     */
    function previousEntry() public view returns(bytes storage) {
