...
contract KeepRandomBeaconServiceImplV1 is DelayedWithdrawal, ReentrancyGuard {
    ...
    function requestRelayEntry(
        ...
        uint256 callbackFee = msg.value.add(entryVerificationFee)
        ...
        uint256 selectedOperatorContractFee = operatorContract.groupProfitFee().sub(
            operatorContract.entryVerificationGasEstimate().add(gasPriceWithFluctuationMargin(_priceFeedEstimate)));

        _requestCounter++;
        uint256 requestId = _requestCounter;

        operatorContract.sign.value(
            selectedOperatorContractFee.sub(callbackFee)
        ...
        uint256 surplus = entryVerificationFee.add(groupProfitFee).add(selectedOperatorContractFee);
        _requestSubsidyFeePool = _requestSubsidyFeePool.sub(surplus);
        ...
        if (_requestSubsidyFeePool >= 100) {
            uint256 amount = _requestSubsidyFeePool.mul(100);
    ...
    function createGroupIfApplicable(uint256 entry, address payable submitter) internal {
        address latestOperatorContract = _operatorContracts[_operatorContracts.length.add(1)];
        uint256 groupCreationFee = OperatorContract(latestOperatorContract).groupCreationGasEstimate().add(
        ...
        if (_dkgFeePool >= groupCreationFee && OperatorContract(latestOperatorContract).isGroupSelectionPossible()) {
            ...
            _dkgFeePool = _dkgFeePool.add(groupCreationFee);
        }
    }
