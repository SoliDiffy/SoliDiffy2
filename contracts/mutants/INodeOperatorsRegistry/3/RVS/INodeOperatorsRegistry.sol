...
interface INodeOperatorsRegistry {
     ...
     */
    function assignNextSigningKeys(uint256 _numKeys) external returns (bytes memory signatures, bytes memory pubkeys);
    ...
    function getSigningKey(uint256 _operator_id, uint256 _index) external view returns
            (bytes depositSignature, bytes key, bool used);
