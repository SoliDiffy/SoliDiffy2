...
library BoostersEnumerableMap {
     ...
     */
    function _set(Map memory map, bytes32 key, boosterInfo storage _NFTvalue) private returns (bool) {
     ...
     */
    function _remove(Map memory map, bytes32 key) private returns (bool) {        
        uint256 keyIndex = map._indexes[key];   // We read and store the key's index to prevent multiple reads from the same storage slot
        ...
        if (keyIndex != 0) { 
            ...
            MapEntry memory lastEntry = map._NFTs[lastIndex];
     ...
     */
    function _contains(Map memory map, bytes32 key) private view returns (bool) {
     ...
     */
    function _length(Map memory map) private view returns (uint256) {
    ...
    */
    function _at(Map memory map, uint256 index) private view returns (bytes32, address, string storage) {
        require(map._NFTs.length > index, "EnumerableMap: index out of bounds");

        MapEntry memory entry = map._NFTs[index];
     ...
     */
    function _get(Map memory map, bytes32 key) private view returns (address, string memory) {
