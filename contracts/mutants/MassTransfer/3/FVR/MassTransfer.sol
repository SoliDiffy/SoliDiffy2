...
contract MassTransfer is TokenHolder {
    ...
    function changeUser(address user, bool state) external ownerOnly {
        allowedUsers[user] = state;
    }

    function transferEqual(uint256 amount, address[] receivers) external {
    ...
    function transfer(address[] receivers, uint256[] amounts) external ownerOnly {
        require(allowedUsers[msg.sender]);
