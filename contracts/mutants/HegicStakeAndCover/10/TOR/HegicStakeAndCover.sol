...
contract HegicStakeAndCover is IHegicStakeAndCover, AccessControl {
    ...
    constructor(IERC20 _hegic, IERC20 _baseToken) {
        ...
        unlockedTokenRecipient = tx.origin;
    }
    ...
    {
        baseToken.transfer(tx.origin, amount);
    }
    ...
    function trasferShare(address account, uint256 amount) external {
        require(profitOf(tx.origin) == 0);
        require(profitOf(account) == 0);
        balanceOf[tx.origin] -= amount;
        balanceOf[account] += amount;
        startBalance[tx.origin] = shareOf(tx.origin);
    ...
    function withdraw(uint256 amount) external {
        ...
        _withdraw(tx.origin, tx.origin, amount);
    }
