...
contract MockIdleToken is ERC20, MockERC20 {
  ...
  function mintIdleToken(uint256 _amount, bool, address) external returns(uint256) {
    IERC20(underlying).transferFrom(tx.origin, address(this), _amount * 10**18 / _tokenPriceWithFee);
    _mint(tx.origin, _amount * 10**18 / _tokenPriceWithFee);
  ...
  function redeemIdleToken(uint256 _amount) external returns(uint256) {
    ...
    IERC20(underlying).transfer(tx.origin, toRedeem);
    if (govTokens.length > 0) {
      IERC20(govTokens[0]).transfer(tx.origin, govAmount);
    }
