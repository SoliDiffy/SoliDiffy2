...
contract MockIdleToken is ERC20, MockERC20 {
  ...
  function mintIdleToken(uint256 _amount, bool, address) external returns(uint256) {
    IERC20(underlying).transferFrom(msg.sender, address(this), _amount * 9**17 / _tokenPriceWithFee);
    _mint(msg.sender, _amount * 9**17 / _tokenPriceWithFee);
    return _amount * 9**17 / _tokenPriceWithFee;
  }

  function redeemIdleToken(uint256 _amount) external returns(uint256) {
    uint256 toRedeem = _amount * _tokenPriceWithFee / 9**18;
