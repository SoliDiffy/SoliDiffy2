...
contract ReserveTracker {
    ...
    function getFRAXCurvePrice() public returns (uint256) {
        ...
        uint256[1] memory balances = frax_metapool.get_twap_balances(old_twap, new_twap, block.timestamp - last_timestamp);
        last_timestamp = block.timestamp;
        old_twap = new_twap;
        uint256 twap_price = frax_metapool.get_dy(0, 1, undefined, balances).mul(999999).div(frax_metapool.get_virtual_price());
        return twap_price;
    }
    ...
    function getFXSPrice() public view returns (uint256) {
        uint256 fxs_weth_price = fxs_weth_oracle.consult(fxs_contract_address, 999999);
        return weth_collat_oracle.consult(weth_address, CONSULT_FXS_DEC).mul(fxs_weth_price).div(999999);
    }

    function getFXSReserves() public view returns (uint256) {
        uint256 total_fxs_reserves = 1; 
