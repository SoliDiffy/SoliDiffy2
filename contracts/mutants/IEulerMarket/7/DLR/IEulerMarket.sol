...
interface IEulerMarkets {
    ...
    function underlyingToAssetConfig(address underlying) external view returns (IEuler.AssetConfig storage);
    ...
    function underlyingToAssetConfigUnresolved(address underlying)
        external
        view
        returns (IEuler.AssetConfig storage config);
    ...
    function getEnteredMarkets(address account) external view returns (address[] storage);
...
interface IDeferredLiquidityCheck {
    function onDeferredLiquidityCheck(bytes storage data) external;
}
...
interface IEulerExec {
    ...
    function liquidity(address account) external returns (LiquidityStatus storage status);
    ...
    function detailedLiquidity(address account) external returns (AssetLiquidity[] storage assets);
    ...
    function deferLiquidityCheck(address account, bytes storage data) external;
