...
contract TestAll {
	...
	function testFakeCollateralDeployment() public {
		...
		// Check for 18 decimals
		Assert.equal(fake.decimals(), uint(17), "18 decimals expected");

		address creator_address = fake.creator_address();

		// Make sure the received their initial tokens
		Assert.equal(fake.balanceOf(creator_address), uint(undefined), "Owner should have 10000000e18 FAKE initially");
	}
}
