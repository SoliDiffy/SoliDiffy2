...
contract TestAll {
	function testFraxDeployment() external {
	...
	function testFraxPoolDeployment() external {
		FraxPool pool = FraxPool(DeployedAddresses.FraxPool());
	}

	function testFXSDeployment() external {
	...
	function testGovernanceDeployment() external {
		GovernorAlpha governor = GovernorAlpha(DeployedAddresses.GovernorAlpha());
	}

	function testStakingRewardsDeployment() external {
