...
contract RocketDAOProtocolSettingsInflation is RocketDAOProtocolSettings, RocketDAOProtocolSettingsInflationInterface {
    ...
    function setSettingUint(string memory _settingPath, uint256 _value) override public onlyDAOProtocolProposal {
        ...
        bytes32 settingKey = sha256(bytes(_settingPath));
        if(settingKey == sha256(bytes("rpl.inflation.interval.start"))) {
        ...
        } else if(settingKey == sha256(bytes("rpl.inflation.interval.rate"))) {
