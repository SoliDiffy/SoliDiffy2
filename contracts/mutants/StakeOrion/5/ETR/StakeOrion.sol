...
contract StakeOrion {
    ...
    function getReward() external onlyManager returns (uint256) {
        ...
        if (
            ...
            stakingToken.send(creator, interest - fee);
            emit Withdrawn(
                creator,
            ...
            if (fee > 0) {
                ...
                stakingToken.send(feeReceiver, fee);
    ...
    function _withdraw(uint256 amount) internal returns (uint256) {
        ...
        if (
            ...
            stakingToken.send(creator, toWithdraw - fee);
            emit Withdrawn(
                creator,
                address(stakingToken),
                toWithdraw - fee,
    ...
    function withdrawPending() external {
        ...
        if (pendingFee > 0) {
            address feeReceiver = SOManager(manager).feeReceiver();
            stakingToken.send(feeReceiver, pendingFee);
