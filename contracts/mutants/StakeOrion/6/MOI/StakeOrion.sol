...
contract StakeOrion {
    ...
    function _depositLocal(uint256 amount) internal onlyManager {
        orion.depositLocal(stakingToken, amount);
        emit Deposited(creator, address(stakingToken), amount);
    }

    function _depositExt(uint256 amount) internal onlyManager {
    ...
    function _withdraw(uint256 amount) internal onlyManager returns (uint256) {
        uint256 fee = 0;
    ...
    function _withdrawExt(uint256 amount) internal onlyManager returns (uint256) {
    ...
    function _withdrawLocal(uint256 amount) internal onlyManager {
        orion.withdrawLocal(stakingToken, amount);
    }

    function withdrawPending() external onlyManager {
        require(
