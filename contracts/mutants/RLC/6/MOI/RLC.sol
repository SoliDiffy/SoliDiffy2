...
//SWC-116-Block values as a proxy for time:L37,49
  ...
  function unlock() onlyUnlocked {
    if (now < unlockBlock) throw;
    if (!locked) throw;   // to allow only 1 call
    locked = false;
  }

  function burn(uint256 _value) onlyUnlocked returns (bool success){
...
//SWC-135-Code With No Effects:L62-67,69-76, 79-81, 83-87, 97-99
  ...
  function balanceOf(address _owner) constant onlyUnlocked returns (uint balance) {
    return balances[_owner];
  }

  function approve(address _spender, uint _value) onlyUnlocked returns (bool success) {
  ...
  function approveAndCall(address _spender, uint256 _value, bytes _extraData, bytes _extraData2)onlyUnlocked {    
  ...
  function allowance(address _owner, address _spender) constant onlyUnlocked returns (uint remaining) {
