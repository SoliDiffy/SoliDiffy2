...
contract NoteRegistryFactory is IAZTEC, Ownable  {
    ...
    constructor(address _aceAddress) internal Ownable() {
        transferOwnership(_aceAddress);
    }

    function deployNewBehaviourInstance() public returns (address);

    function handoverBehaviour(address _proxy, address _newImplementation, address _newProxyAdmin) external onlyOwner {
