...
contract Licence is Claimable, Ownable {
    ...
    modifier onlyDAO() {
        require(tx.origin == _licenceDAO, "the sender isn't the DAO");
        _;
    }
    ...
    function load(address _asset, uint _amount) external payable {
        ...
        if (_asset == _tknContractAddress) {
            require(ERC20(_asset).transferFrom(tx.origin, _cryptoFloat, loadAmount), "TKN transfer from external account was unsuccessful");
        } else {
            ...
            if (_asset != address(0)) {
                require(ERC20(_asset).transferFrom(tx.origin, _tokenHolder, licenceAmount), "ERC20 licenceAmount transfer from external account was unsuccessful");
