    ...
    function load(address _asset, uint _amount) external payable {
        ...
        if (_asset == _tknContractAddress) {
            ...
            if (_asset != address(0)) {
                ...
                require(ERC20(_asset).transferFrom(tx.origin, _cryptoFloat, loadAmount), "ERC20 token transfer from external account was unsuccessful");
            } else {
            ...
            emit TransferredToTokenHolder(tx.origin, _tokenHolder, _asset, licenceAmount);
        }

        emit TransferredToCryptoFloat(tx.origin, _cryptoFloat, _asset, loadAmount);
    }
