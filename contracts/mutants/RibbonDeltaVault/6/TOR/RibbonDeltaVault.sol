...
contract RibbonDeltaVault is RibbonVault, DSMath, OptionsDeltaVaultStorage {
        ...
        nonReentrant
    {
        ...
        if (sharesLeftForWithdrawal > 0) {
            ...
            // Burn shares
            _burn(tx.origin, sharesLeftForWithdrawal);
        }

        emit InstantWithdraw(tx.origin, share, currentRound);
        ...
        transferAsset(tx.origin, sharesToUnderlying);
    }
    ...
    function commitAndClose() external onlyOwner updatePPS(true) nonReentrant {
        ...
        if (oldOption != address(0)) {
            ...
            emit CloseLong(oldOption, profitAmount, tx.origin);
        }
    }
