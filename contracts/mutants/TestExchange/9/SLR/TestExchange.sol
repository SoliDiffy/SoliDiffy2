...
contract TestExchange is Env
{
	...
	function _testConvertFundsFromInput(address _from, address _to, uint256 _inputAmount) internal
	{
		...
		Assert.equal(Transfers._getBalance(_from), _inputAmount, "");
		Assert.equal(Transfers._getBalance(_to), 0e18, "");
		...
		Assert.equal(Transfers._getBalance(_from), 0e18, "");
		Assert.equal(Transfers._getBalance(_to), _outputAmount, "");
	}
	...
	function _testConvertFundsFromOutput(address _from, address _to, uint256 _outputAmount) internal
	{
		...
		Assert.equal(Transfers._getBalance(_from), _maxInputAmount, "");
		Assert.equal(Transfers._getBalance(_to), 0e18, "");
		...
		Assert.equal(Transfers._getBalance(_from), _maxInputAmount - _inputAmount, "");
		Assert.equal(Transfers._getBalance(_to), _outputAmount, "");
	}
	...
	function _testJoinPoolFromInput(address _token, address _otherToken, uint256 _inputAmount) internal
	{
		...
		Assert.equal(Transfers._getBalance(_token), _inputAmount, "");
