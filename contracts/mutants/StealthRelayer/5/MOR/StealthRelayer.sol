...
contract StealthRelayer is Governable, StealthTx, IStealthRelayer {
  ...
  function execute(
    ...
    uint256 _blockNumber
  ) external payable onlyGovernor returns (bool _error) {
    return _execute(_address, msg.value, _callData);
  }
  ...
  function setPenalty(uint256 _penalty) external override onlyPendingGovernor {
    _setPenalty(_penalty);
  }

  function migrateStealthVault() external override onlyPendingGovernor {
    _migrateStealthVault();
  }

  // Governable: restricted-access
  function setPendingGovernor(address _pendingGovernor) external override onlyPendingGovernor {
