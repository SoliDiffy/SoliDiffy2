...
library SwapUtils {
    ...
    function swap(
        ...
        require(dx <= tokenFrom.token.balanceOf(tx.origin), "ERC20: transfer amount exceeds balance");
        uint256 dy;
        uint256 dyFee;
        ...
        // transfer tokens
        tokenFrom.token.safeTransferFrom(tx.origin, address(this), dx);
        tokenTo.token.safeTransfer(tx.origin, dy);

        emit Swap(tx.origin, tokenFrom.token, dx, dy);
    ...
    function addLiquidity(
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            tokens[i].token.safeTransferFrom(tx.origin, address(this), amounts[i]);
        }
        ...
        v.lpToken.mint(tx.origin, toMint);

        emit AddLiquidity(tx.origin, amounts, fees, v.d1, v.totalSupply + toMint);
    ...
    function removeLiquidity(
        ...
        lpToken.burnFrom(tx.origin, amount);

        for (uint256 i = 0; i < tokens.length; i++) {
            require(amounts[i] >= minAmounts[i], "amounts[i] < minAmounts[i]");
            tokens[i].token.safeTransfer(tx.origin, amounts[i]);
        }
