...
library SwapUtils {
    ...
    function _getAPrecise(Amplification storage self) internal view returns (uint256) {
        ...
        if (block.number < t1) {
            ...
            if (a1 > a0) {
                ...
                return a0 + ((a1 - a0) * (block.number - t0)) / (t1 - t0);
            } else {
                // a0 - (a0 - a1) * (block.timestamp - t0) / (t1 - t0)
                return a0 - ((a0 - a1) * (block.number - t0)) / (t1 - t0);
            }
        } else {
            return a1;
        }
    ...
    function rampA(
        ...
        require(block.number >= self.initialATime + 1 days, "Wait 1 day before starting ramp");
        require(futureTime_ >= block.number + MIN_RAMP_TIME, "Insufficient ramp time");
        ...
        self.initialATime = block.number;
        self.futureATime = futureTime_;

        emit RampA(initialAPrecise, futureAPrecise, block.number, futureTime_);
    }
    ...
    function stopRampA(Amplification storage self) external {
        require(self.futureATime > block.number, "Ramp is already stopped");
