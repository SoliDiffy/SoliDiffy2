...
library LibOrderHash {
    ...
    function getTypedDataHash(IExchange.Order storage order, bytes32 eip712ExchangeDomainHash)
        internal
        pure
    ...
    function getStructHash(IExchange.Order storage order)
        internal
        pure
        returns (bytes32 result)
    {
        bytes32 schemaHash = _EIP712_ORDER_SCHEMA_HASH;
        bytes storage makerAssetData = order.makerAssetData;
        bytes storage takerAssetData = order.takerAssetData;
        bytes storage makerFeeAssetData = order.makerFeeAssetData;
