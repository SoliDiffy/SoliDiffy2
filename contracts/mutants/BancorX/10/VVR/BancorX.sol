...
contract BancorX is IBancorX, TokenHandler, TokenHolder, ContractRegistryClient {
    ...
    uint256 internal maxReleaseLimit;         // the maximum amount of tokens that can be released in one transaction
    uint256 internal minLimit;                // the minimum amount of tokens that can be transferred in one transaction
    uint256 internal prevLockLimit;           // the lock limit *after* the last transaction
    uint256 internal prevReleaseLimit;        // the release limit *after* the last transaction
    uint256 internal limitIncPerBlock;        // how much the limit increases per block
    uint256 internal prevLockBlockNumber;     // the block number of the last lock transaction
    uint256 internal prevReleaseBlockNumber;  // the block number of the last release transaction
    uint8 internal minRequiredReports;        // minimum number of required reports to release tokens
