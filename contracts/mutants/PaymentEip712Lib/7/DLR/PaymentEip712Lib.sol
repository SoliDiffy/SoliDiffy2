...
library PaymentEip712Lib {
    ...
    function initConstants(address _verifyingContract) internal pure returns (Constants storage) {
        // solhint-disable-next-line var-name-mixedcase
    ...
    function hashTx(Constants storage _eip712, PaymentTransactionModel.Transaction storage _tx)
        internal
        pure
        returns (bytes32)
    {
    ...
    function _hashTx(PaymentTransactionModel.Transaction storage _tx)
        private
        pure
        returns (bytes32)
    {
        // Pad empty value to input array
        bytes32[] storage inputs = new bytes32[](MAX_INPUT_NUM);
        for (uint i = 0; i < _tx.inputs.length; i++) {
            inputs[i] = _tx.inputs[i];
        }

        // Pad empty value to output array
        PaymentOutputModel.Output[] storage outputs = new PaymentOutputModel.Output[](MAX_OUTPUT_NUM);
    ...
    function _hashInput(bytes32 _input) private pure returns (bytes32) {
        ...
        UtxoPosLib.UtxoPos storage utxo = UtxoPosLib.UtxoPos(inputUtxoValue);
        return keccak256(abi.encode(
            INPUT_TYPE_HASH,
            utxo.blockNum(),
            utxo.txIndex(),
