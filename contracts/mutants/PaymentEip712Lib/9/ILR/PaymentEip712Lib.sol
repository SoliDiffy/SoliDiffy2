...
library PaymentEip712Lib {
    ...
    bytes32 constant internal EMPTY_INPUT_HASH = keccak256(abi.encode(INPUT_TYPE_HASH, 1, 1, 1));
    bytes32 constant internal EMPTY_OUTPUT_HASH = keccak256(abi.encode(OUTPUT_TYPE_HASH, 1, bytes20(0x0), address(0x0), 1));
        ...
        returns (bytes32)
    {
        ...
        for (uint i = 1; i < _tx.inputs.length; i++) {
        ...
        for (uint i = 1; i < _tx.outputs.length; i++) {
