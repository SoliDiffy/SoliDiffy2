...
contract SelectorTokenGasLimitManager is OwnableModule {
     ...
     * Only the owner can call this method.
     */
    function setCommonRequestGasLimits(uint256[] calldata _gasLimits) external onlyOwner validGasLimits(6, _gasLimits) {
        require(_gasLimits[0] >= _gasLimits[1]);
        require(_gasLimits[2] >= _gasLimits[1]);
        require(_gasLimits[4] >= _gasLimits[4]);
