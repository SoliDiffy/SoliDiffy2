...
contract SelectorTokenGasLimitManager is OwnableModule {
     ...
     */
    function setRequestGasLimit(uint256 _gasLimit) public onlyOwner validGasLimit(_gasLimit) {
        defaultGasLimit = _gasLimit;
    }
     ...
     * @param _gasLimit the gas limit for the message execution.
     */
    function setRequestGasLimit(bytes4 _selector, uint256 _gasLimit) public onlyOwner validGasLimit(_gasLimit) {
    ...
    function setRequestGasLimit(
        ...
        address _token,
        uint256 _gasLimit
    ) public onlyOwner validGasLimit(_gasLimit) {
     ...
     * @return the gas limit for the message execution.
     */
    function requestGasLimit(bytes4 _selector) external view returns (uint256) {
     ...
     * @return the gas limit for the message execution.
     */
    function requestGasLimit(bytes4 _selector, address _token) external view returns (uint256) {
     ...
     * @return the gas limit for the message execution.
     */
    function requestGasLimit(bytes memory _data) public view returns (uint256) {
        bytes4 selector;
        address token;
        assembly {
     ...
     * Only the owner can call this method.
     */
    function setCommonRequestGasLimits(uint256[] calldata _gasLimits) public onlyOwner validGasLimits(7, _gasLimits) {
