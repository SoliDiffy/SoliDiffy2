...
contract RandomBase is OwnedEternalStorage, IRandom {
    ...
    function getCurrentSeed() public onlyValidatorSetContract view returns(uint256) {
    ...
    function validatorSetContract() external view returns(IValidatorSet) {
    ...
    function _initialize(address _validatorSet) public {
        require(!isInitialized());
    ...
    function _setCurrentSeed(uint256 _seed) public {
        uintStorage[CURRENT_SEED] = _seed;
    }

    /// @dev Reads the current random seed from the state.
    function _getCurrentSeed() public view returns(uint256) {
