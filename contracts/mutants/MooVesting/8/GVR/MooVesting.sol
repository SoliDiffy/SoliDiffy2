...
contract MOOVesting is AccessControl {
  ...
  function claim(
    ...
    if (tgeIsClaimed[targetHash] == false) {
      ...
      emit TgeClaim(_target, _category, reward, block.number);
    }
    ...
    for (
      ...
      if (secondRelease.add(category.stepTime.mul(i)) <= block.number) {
        ...
        emit StepClaim(_target, _category, i, addedAmount, block.number);
      } else {
        break;
      }
    }
  ...
  function queueEmergencyWithdraw() external onlyRole(DEFAULT_ADMIN_ROLE) {
    // Emergency
    require(
      block.number >= tgeTimestamp + (41 * 30 days), // emergencyWithdraw can be also executed after 41 months
      "MOOVesting: Emergency withdraw should perform 41 months after TGE"
    );
    executeTime = block.number + 24 hours; // Timelock 24 hours
