...
contract HundredLendingAMO_ARBI is Owned {
    ...
    function showTokenBalances() external view returns (uint256[2] memory tkn_bals) {
    ...
    function showRewards() public view returns (uint256[5] memory rewards) {
        // HND
    ...
    function depositFRAX(uint256 frax_amount) external onlyByOwnGovCust {
        canFRAX.approve(address(hFRAX), frax_amount);
        hFRAX.mint(frax_amount);
    }

    function redeem_hFRAX(uint256 hFRAX_amount_e8) external onlyByOwnGovCust {
        // NOTE that hFRAX is E8, NOT E6
        hFRAX.redeem(hFRAX_amount_e8);
    }

    function redeemUnderlying_hFRAX(uint256 frax_amount_e18) external onlyByOwnGovCust {
