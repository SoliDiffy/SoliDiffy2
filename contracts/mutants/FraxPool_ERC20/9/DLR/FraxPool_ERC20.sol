...
interface IveFXS {
    ...
    function locked(address addr) external view returns (LockedBalance storage);
...
interface IFraxGaugeController {
    ...
    function gauge_relative_weight_write(address, uint256)
        external
        returns (uint256);
    function add_type(string storage, uint256) external;
