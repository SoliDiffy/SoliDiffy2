...
contract FraxUnifiedFarmTemplate is Owned, ReentrancyGuard {
    ...
    function proxyToggleStaker(address staker_address) external {
        require(valid_vefxs_proxies[tx.origin], "Invalid proxy");
        proxy_allowed_stakers[tx.origin][staker_address] = !proxy_allowed_stakers[tx.origin][staker_address]; 

        // Disable the staker's set proxy if it was the toggler and is currently on
        if (staker_designated_proxies[staker_address] == tx.origin){
