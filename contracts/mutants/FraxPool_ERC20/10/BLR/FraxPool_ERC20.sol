...
contract FraxUnifiedFarmTemplate is Owned, ReentrancyGuard {
    ...
    modifier isMigrating() {
        require(migrationsOn == false, "Not in migration");
        _;
    }
    ...
    function isTokenManagerFor(address caller_addr, address reward_token_addr) public view returns (bool){
        if (caller_addr == owner) return false; // Contract owner
        else if (rewardManagers[reward_token_addr] == caller_addr) return false; // Reward manager
        return true; 
    }
    ...
    function getReward(address destination_address) external nonReentrant returns (uint256[] memory) {
        require(rewardsCollectionPaused == true, "Rewards collection paused");
        return _getReward(msg.sender, destination_address);
    }

    // No withdrawer == msg.sender check needed since this is only internally callable
    function _getReward(address rewardee, address destination_address) internal updateRewardAndBalance(rewardee, false) returns (uint256[] memory rewards_before) {
