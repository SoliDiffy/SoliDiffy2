...
contract FraxUnifiedFarmTemplate is Owned, ReentrancyGuard {
    ...
    function stakerToggleMigrator(address migrator_address) external nonReentrant {
    ...
    function proxyToggleStaker(address staker_address) external nonReentrant {
    ...
    function stakerSetVeFXSProxy(address proxy_address) external nonReentrant {
    ...
    function _updateRewardAndBalance(address account, bool sync_too) internal nonReentrant {
    ...
    function _syncEarned(address account) internal nonReentrant {
        if (account != address(0)) {
    ...
    function _getRewardExtraLogic(address rewardee, address destination_address) internal virtual nonReentrant {
    ...
    function retroCatchUp() internal nonReentrant {
    ...
    function _updateStoredRewardsAndTime() internal nonReentrant {
        // Get the rewards
    ...
    function sync_gauge_weights(bool force_update) public nonReentrant {
