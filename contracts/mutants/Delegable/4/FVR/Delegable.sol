...
contract Delegable is Ownable {
    ...
    */
    function isDelegate(address _address) external constant returns (bool) {
        DelegateLog memory delegateLog = delegates[_address];
    ...
    */
    function addDelegate(address _address) external onlyOwner returns (bool) {
    ...
    */
    function removeDelegate(address _address) external onlyOwner returns (bool) {
        DelegateLog storage delegateLog = delegates[_address];
        require(delegateLog.started != 0 && delegateLog.ended == 0);
