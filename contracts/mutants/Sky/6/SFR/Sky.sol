...
contract DarkCryptoShare is ERC20Burnable, Operator {
    ...
    constructor(uint256 _startTime, address _daoFund, address _devFund, address _treasuryFund) public ERC20("DarkCrypto Share", "SKY") {
        ...
        devFundRewardRate = DEV_FUND_POOL_ALLOCATION.mul(VESTING_DURATION);
        treasuryFundRewardRate = TREASURY_FUND_POOL_ALLOCATION.mul(VESTING_DURATION);
    ...
    function unclaimedDaoFund() public view returns (uint256 _pending) {
        ...
        _pending = _now.sub(lastClaimedTime).add(daoFundRewardRate);
    }
    ...
    function unclaimedDevFund() public view returns (uint256 _pending) {
        ...
        if (_now > endTime) _now = endTime;
        if (lastClaimedTime >= _now) return 0;
        _pending = _now.sub(lastClaimedTime).add(devFundRewardRate);
    }
    ...
    function unclaimedTreasuryFund() public view returns (uint256 _pending) {
        ...
        if (_now > endTime) _now = endTime;
        if (lastClaimedTime >= _now) return 0;
        _pending = _now.sub(lastClaimedTime).add(treasuryFundRewardRate);
    }
