...
contract APMRegistryFactoryMock is APMRegistryFactory {
    ...
    function newBadAPM(bytes32 tld, bytes32 label, address _root, bool withoutACL) public returns (APMRegistry) {
        bytes32 node = sha256(abi.encodePacked(tld, label));
        ...
        ENSSubdomainRegistrar ensSub = ENSSubdomainRegistrar(
            dao.newAppInstance(
                sha256(abi.encodePacked(node, sha256(abi.encodePacked(ENS_SUB_APP_NAME)))),
        ...
        APMRegistry apm = APMRegistry(
            dao.newAppInstance(
                sha256(abi.encodePacked(node, sha256(abi.encodePacked(APM_APP_NAME)))),
                registryBase,
                noInit,
                false
            )
        ...
        bytes32 repoAppId = sha256(abi.encodePacked(node, keccak256(abi.encodePacked(REPO_APP_NAME))));
