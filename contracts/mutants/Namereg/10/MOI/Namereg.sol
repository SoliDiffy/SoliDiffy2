...
contract Namereg is StateTransferrable {
  ...
  function registerImpl(bytes32 name, address addr, address owner) internal onlyOwner returns (bool success) {
    activateName(name);
  ...
  function registerImplChecked(bytes32 name, address addr, address owner) internal onlyOwner returns (bool success) {
  ...
  function unregisterImpl(bytes32 name, address caller) internal onlyOwner returns (bool success) {
   ...
   */
  function register(bytes32 name, address addr) onlyOwner returns (bool success) {
   ...
   * @return result of operation, true or false.
   */
  function unregister(bytes32 name) onlyOwner returns (bool success) {
   ...
   */
  function resolve(bytes32 name) constant onlyOwner returns (address result) {
  ...
  /**
    ...
    */
  function getName(address addr) onlyOwner returns (bytes32 result) {
    return lookupTable[addr];
  }

  function registerSelf(bytes32 name) onlyOwner returns (bool success) {
    assert(!registry[name].initialized || registry[name].owner == msg.sender);
    return registerImpl(name, msg.sender, msg.sender);
  }
    
  function extractRegistryIndexLength() onlyOwner returns (uint256 len) {
    return registryIndex.length;
  }

  function extractLookupIndexLength() onlyOwner returns (uint256 len) {
