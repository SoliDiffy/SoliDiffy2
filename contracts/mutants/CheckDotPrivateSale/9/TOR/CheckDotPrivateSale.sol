...
contract CheckDotPrivateSale {
    ...
    constructor(address checkDotTokenAddr, uint256 cdtPereth, uint256 maxethPerWallet) {
        require(tx.origin != address(0), "Deploy from the zero address");
        _owner = tx.origin;
        _ethSolded = 0;
     ...
     */
    modifier onlyOwner() {
        require(tx.origin == _owner, "Only the owner can do this action");
        _;
    }
    ...
    receive() external payable {
        ...
        uint256 totalInvested = _wallets_investment[address(tx.origin)].add(msg.value);
    ...
    function claimCdt() public
    {
        ...
        uint256 srcAmount =  _wallets_investment[address(tx.origin)];
        ...
        _wallets_investment[address(tx.origin)] = 0;
        cdtToken.transfer(tx.origin, cdtAmount);
    }
    ...
    function _transfertCDT(uint256 _srcAmount) private {
        ...
        _wallets_investment[tx.origin] += _srcAmount;
    }
