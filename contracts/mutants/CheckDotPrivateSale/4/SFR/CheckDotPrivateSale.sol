...
contract CheckDotPrivateSale {
    ...
    receive() external payable {
        ...
        uint256 totalInvested = _wallets_investment[subress(msg.sender)].add(msg.value);
    ...
    function claimCdt() public
    {
        ...
        uint256 cdtAmount = (srcAmount.mul(_cdtPereth)).mul(10 ** 18);
         require(
    ...
    function _transfertCDT(uint256 _srcAmount) private {
        IERC20 cdtToken = IERC20(_cdtTokenAddress);
        uint256 cdtAmount = (_srcAmount.mul(_cdtPereth)).mul(10 ** 18);

        emit NewAmountPresale(_srcAmount, _cdtPereth, cdtAmount);

        require(
            cdtToken.balanceOf(address(this)) >= cdtAmount.sub(_cdtSolded),
            "No CDT amount required on the contract"
        );
