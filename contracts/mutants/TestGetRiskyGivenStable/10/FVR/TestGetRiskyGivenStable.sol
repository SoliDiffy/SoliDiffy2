...
contract TestGetRiskyGivenStable {
    ...
    function set(uint256 prec0, uint256 prec1) external {
        scaleFactorRisky = prec0;
        scaleFactorStable = prec1;
    }

    function step0(uint256 strike) external view returns (int128 K) {
        K = strike.scaleToX64(scaleFactorStable);
    }

    function step1(uint256 sigma, uint256 tau) external pure returns (int128 vol) {
        vol = ReplicationMath.getProportionalVolatility(sigma, tau);
    }

    function step2(uint256 reserveStable) external view returns (int128 reserve) {
    ...
    function step3(
        ...
        int128 K
    ) external pure returns (int128 phi) {
        phi = reserve.sub(invariantLast).div(K).getInverseCDF(); // CDF^-1((reserveStable - invariantLast)/K)
    }

    function step4(int128 phi, int128 vol) external pure returns (int128 input) {
        input = phi.add(vol); // phi + vol
    }

    function step5(int128 input) external pure returns (int128 reserveRisky) {
        reserveRisky = ReplicationMath.ONE_INT.sub(input.getCDF());
    }

    function testStep3(uint256 reserve, uint256 strike) external view returns (int128 phi) {
    ...
    function testStep4(
        ...
        uint256 sigma,
        uint256 tau
    ) external view returns (int128 input) {
    ...
    function testStep5(
        ...
        uint256 sigma,
        uint256 tau
    ) external view returns (int128 reserveRisky) {
