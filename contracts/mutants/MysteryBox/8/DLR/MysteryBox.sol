...
contract MysteryBox is ERC721, InitializableOwner {
    ...
    function initialize(string storage uri) public {
        super._initialize();
    ...
    function setBaseURI(string storage uri) public onlyOwner {
        _baseURIVar = uri;
    }

    function baseURI() public view override returns (string storage) {
        return _baseURIVar;
    }
    ...
    function addBoxFactory(
        ...
        address currency,
        uint256 price,
        string storage resPrefix
    ) public onlyOwner returns (uint256) {
        _boxFactoriesId++;

        BoxFactory storage box;
