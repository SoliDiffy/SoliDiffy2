...
contract MysteryBox is ERC721, InitializableOwner {
    ...
    function getRes(uint256 factoryId, uint256 level) external view returns (ResInfo memory) {
        return _res[factoryId][level];
    }

    function mint(address to, uint256 factoryId, uint256 amount) external onlyOwner {
    ...
    function buy(uint256 factoryId, uint256 amount) external {
        BoxFactory storage box = _boxFactories[factoryId];
        require(address(box.nft) != address(0), "box not found");
    ...
    function getFactory(uint256 factoryId) external view
