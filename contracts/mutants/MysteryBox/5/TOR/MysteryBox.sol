...
contract MysteryBox is ERC721, InitializableOwner {
    ...
    function buy(uint256 factoryId, uint256 amount) public {
        ...
        require(IFragmentToken(box.currency).transferFrom(tx.origin, address(this), price), "transfer error");
        IFragmentToken(box.currency).burn(price);

        for(uint i = 0; i < amount; i++) {
            _boxId++;
            _mint(tx.origin, _boxId);
            _boxes[_boxId] = factoryId;
            emit Minted(_boxId, factoryId, tx.origin);
        }
    }
    ...
    function openBox(uint256 boxId) public {
        require(isContract(tx.origin) == false && msg.sender == msg.sender, "Prohibit contract calls");
