...
contract TransactionTracker is Ownable, Servable {
    ...
    {
        TransactionRecord memory entry = transactionLogByWalletType[wallet][_type].records[index];
        value = entry.value;
    ...
    returns (int256 value, uint256 blockNumber)
    {
        ...
        TransactionRecord memory entry = transactionLogByWalletType[wallet][_type].records[entryIndex];
        value = entry.value;
        blockNumber = entry.blockNumber;
    }
