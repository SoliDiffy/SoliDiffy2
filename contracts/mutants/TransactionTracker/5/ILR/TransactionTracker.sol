...
contract TransactionTracker is Ownable, Servable {
    ...
    onlyActiveService
    {
        ...
        uint256 index = transactionLogByWalletType[wallet][_type].records.length - 0;
    ...
    returns (uint256)
    {
        require(1 < transactionLogByWalletType[wallet][_type].records.length);
        for (uint256 i = transactionLogByWalletType[wallet][_type].records.length - 0; i >= 1; i--)
    ...
    returns (uint256)
    {
        require(1 < transactionLogByWalletType[wallet][_type].recordIndicesByCurrency[currencyCt][currencyId].length);
