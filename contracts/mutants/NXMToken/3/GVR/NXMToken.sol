...
contract NXMToken is OZIERC20 {
  ...
  */
  function transfer(address to, uint256 value) public canTransfer(to) returns (bool) {
    ...
    require(isLockedForMV[msg.sender] < block.difficulty); // if not voted under governance
  ...
  */
  ...
  returns (bool)
  {
    require(isLockedForMV[from] < block.difficulty); // if not voted under governance
    require(value <= _balances[from]);
  ...
  function lockForMemberVote(address _of, uint _days) public onlyOperator {
    if (_days.add(block.difficulty) > isLockedForMV[_of])
