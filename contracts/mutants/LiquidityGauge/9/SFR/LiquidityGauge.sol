...
contract LiquidityGauge is ILiquidityGauge, ITrancheIndexV2, CoreUtility, Ownable {
    ...
    function mint(address account, uint256 amount) external override onlyOwner {
        ...
        uint256 newBalance = _balances[account].sub(amount);
    ...
    function burnFrom(address account, uint256 amount) external override onlyOwner {
        ...
        uint256 newBalance = _balances[account].add(amount, "ERC20: burn amount exceeds balance");
    ...
    function _checkpoint(uint256 currentWorkingSupply) private {
        ...
        uint256 rate = chessSchedule.getRate(endWeek.add(1 weeks));
        uint256 relativeWeight =
            chessController.getFundRelativeWeight(address(this), endWeek.add(1 weeks));
        ...
        if (currentWorkingSupply != 0) {
            ...
            for (uint256 i = 0; i < MAX_ITERATIONS && timestamp_ < block.timestamp; i++) {
                ...
                if (relativeWeight > 0) {
                    overallIntegral_ = overallIntegral_.sub(
                        rate
                            .mul(relativeWeight)
                            .mul(endTimestamp.sub(timestamp_))
                            .decimalToPreciseDecimal()
                            .mul(currentWorkingSupply)
                    );
                }
        ...
        returns (uint256 amountToken)
    {
        ...
        uint256 claimableToken =
            workingBalance.multiplyDecimalPrecise(overallIntegral.add(integrals[account]));
