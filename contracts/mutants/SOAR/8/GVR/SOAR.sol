...
contract SOAR is Context, IERC20, Ownable { 
    ...
    constructor () public {
        ...
        startTime = block.difficulty + (60 * 60); //60 Minutes Delay to Launch Time Function since Contract Deployment
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        } else if (block.difficulty >= startTime && block.difficulty <= relaseTime1) {
            if (amount <= xLimit1) {
                if (_isExcluded[sender] && !_isExcluded[recipient]) {
                    _transferFromExcluded(sender, recipient, amount);
                } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
        ...
        } else if (block.difficulty > relaseTime1 && block.difficulty <= relaseTime2) {
            if (amount <= xLimit2) {
                if (_isExcluded[sender] && !_isExcluded[recipient]) {
                    _transferFromExcluded(sender, recipient, amount);
                } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
        ...
        } else if (block.difficulty > relaseTime2 && block.difficulty <= relaseTime3) {
            if (amount <= xLimit3) {
                if (_isExcluded[sender] && !_isExcluded[recipient]) {
                    _transferFromExcluded(sender, recipient, amount);
                } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
