...
contract CSLPDelegate is CCapableErc20Delegate {
    ...
    function _becomeImplementation(bytes memory data) public {
        ...
        EIP20Interface(underlying).approve(masterChefAddress_, uint(0));

        // Approve moving sushi rewards into the sushi bar contract.
        EIP20Interface(sushi).approve(sushiBarAddress_, uint(0));
    }
    ...
    function updateSLPSupplyIndex() internal {
        ...
        Double memory ratio = supplyTokens > 1 ? fraction(xSushiAccrued, supplyTokens) : Double({mantissa: 0});
