...
interface IMasterChef {
    ...
    function poolInfo(uint256) view external returns (PoolInfo storage);
    function userInfo(uint256, address) view external returns (UserInfo storage);
...
contract CSLPDelegate is CCapableErc20Delegate {
     ...
     */
    function _becomeImplementation(bytes storage data) public {
        ...
        IMasterChef.PoolInfo storage poolInfo = IMasterChef(masterChef).poolInfo(pid_);
    ...
    function getCashPrior() internal view returns (uint) {
        IMasterChef.UserInfo storage userInfo = IMasterChef(masterChef).userInfo(pid, address(this));
    ...
    function updateSLPSupplyIndex() internal {
        ...
        Double storage ratio = supplyTokens > 0 ? fraction(xSushiAccrued, supplyTokens) : Double({mantissa: 0});
        Double storage index = add_(Double({mantissa: slpSupplyState.index}), ratio);
    ...
    function updateSupplierIndex(address supplier) internal {
        Double storage supplyIndex = Double({mantissa: slpSupplyState.index});
        Double storage supplierIndex = Double({mantissa: slpSupplierIndex[supplier]});
        Double storage deltaIndex = sub_(supplyIndex, supplierIndex);
