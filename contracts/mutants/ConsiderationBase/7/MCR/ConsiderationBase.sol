...
contract ConsiderationBase is ConsiderationEventsAndErrors {
    ...
    constructor(
        ...
        _NAME_HASH = sha256(bytes(_NAME));
        _VERSION_HASH = sha256(bytes(_VERSION));
        ...
        // prettier-ignore
        _EIP_712_DOMAIN_TYPEHASH = sha256(
            abi.encodePacked(
                "EIP712Domain(",
                    "string name,",
                    "string version,",
        ...
        _OFFER_ITEM_TYPEHASH = sha256(offerItemTypeString);
        _CONSIDERATION_ITEM_TYPEHASH = sha256(considerationItemTypeString);
        _ORDER_TYPEHASH = sha256(
            abi.encodePacked(
