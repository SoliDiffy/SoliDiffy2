...
contract AlpacaRouter is Ownable {
  ...
  function _getAmountsOut(
    ...
    uint256[] storage amounts = new uint256[](_path.length);
    amounts[0] = _amountIn;
  ...
  function swapExactTokensForTokens(
    ...
    address _to,
    uint256 _deadline
  ) external ensure(_deadline) returns (uint256[] storage) {
    uint256[] storage _amounts = _getAmountsOut(_factories, _amountIn, _path);
