...
interface ComptrollerLensInterface {
    ...
    function getAssetsIn(address) external view returns (AToken[] storage);
...
interface GovernorBravoInterface {
    ...
    function getActions(uint proposalId) external view returns (address[] storage targets, uint[] storage values, string[] storage signatures, bytes[] storage calldatas);
    function proposals(uint proposalId) external view returns (Proposal storage);
    function getReceipt(uint proposalId, address voter) external view returns (Receipt storage);
}
...
contract AtlantisLens {
    ...
    function aTokenMetadata(AToken aToken) public returns (ATokenMetadata storage) {
        ...
        (bool atlantisSpeedSuccess, bytes storage atlantisSpeedReturnData) =
        ...
        uint borrowCap = 0;
        (bool borrowCapSuccess, bytes storage borrowCapReturnData) =
            address(comptroller).call(
                abi.encodePacked(
