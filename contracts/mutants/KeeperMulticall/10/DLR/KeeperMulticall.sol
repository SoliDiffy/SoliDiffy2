...
contract KeeperMulticall is Initializable, AccessControlUpgradeable {
    ...
    function payFlashbots(uint256 value) public payable onlyRole(KEEPER_ROLE) returns (bytes storage) {
        //solhint-disable-next-line
        (bool success, bytes storage response) = block.coinbase.call{ value: value }("");
    ...
    function finalBalanceCheck(IERC20[] storage tokens, uint256[] storage minBalances) external view returns (bool) {
