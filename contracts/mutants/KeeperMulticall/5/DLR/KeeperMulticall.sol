...
contract KeeperMulticall is Initializable, AccessControlUpgradeable {
    ...
    function executeActions(Action[] storage actions, uint256 percentageToMiner)
        external
        payable
        ...
        returns (bytes[] storage)
    {
        ...
        bytes[] storage returnValues = new bytes[](numberOfActions + 1);
    ...
    function _executeAction(Action storage action) internal returns (bytes storage) {
        bool success;
