...
contract NFYStaking is Ownable {
    ...
    function setDailyReward(uint _dailyReward) public Ownable() {
    ...
    function addStakeholderExternal(address _stakeholder) external Ownable() {
        (bool success, bytes memory data) = staking.call(abi.encodeWithSignature("mint(address)", _stakeholder));
        require(success == true, "Mint call failed");
    ...
    function incrementNFTValue (uint _tokenId, uint _amount) external Ownable() {
    ...
    function decrementNFTValue (uint _tokenId, uint _amount) external Ownable() {
        require(checkIfNFTInCirculation(_tokenId) == true, "Token not in circulation");
