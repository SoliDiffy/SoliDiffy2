...
contract NFYStaking is Ownable {
    ...
    function stakeNFY(uint _amount) public {
        ...
        if(nft._NFYDeposited > 0) {
            ...
            if(_pendingRewards > 0) {
                NFYToken.send(_msgSender(), _pendingRewards);
    ...
    function addStakeholder(address _stakeholder) private {
        (bool success, bytes memory data) = staking.delegatecall(abi.encodeWithSignature("mint(address)", _stakeholder));
    ...
    function unstakeNFY(uint _tokenId) public {
        ...
        (bool success, bytes memory data) = staking.delegatecall(abi.encodeWithSignature("burn(uint256)", _tokenId));
        require(success == true, "mint call failed");

        NFYToken.send(_msgSender(), userReceives);
        NFYToken.send(rewardPool, fee);
    ...
    function incrementNFTValue (uint _tokenId, uint _amount) external onlyPlatform() {
        ...
        if(nft._NFYDeposited > 0) {
            ...
            if(_pendingRewards > 0) {
                NFYToken.send(StakingNFT.ownerOf(_tokenId), _pendingRewards);
