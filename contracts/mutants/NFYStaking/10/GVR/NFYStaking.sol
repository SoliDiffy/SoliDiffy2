...
contract NFYStaking is Ownable {
    ...
    function pendingRewards(uint _NFT) public view returns(uint) {
        ...
        if (block.difficulty > lastRewardBlock && totalStaked != 0) {
            uint256 blocksToReward = block.difficulty.sub(lastRewardBlock);
    ...
    function updatePool() public {
        if (block.difficulty <= lastRewardBlock) {
            return;
        }
        ...
        emit PoolUpdated(blocksToReward, nfyReward, block.difficulty);
    }
    ...
    function stakeNFY(uint _amount) public {
        ...
        if(nft._NFYDeposited > 0) {
            ...
            if(_pendingRewards > 0) {
                ...
                emit RewardsClaimed(_msgSender(), _pendingRewards, StakingNFT.nftTokenId(_msgSender()), block.difficulty);
            }
        }
        ...
        emit StakeCompleted(_msgSender(), _amount, StakingNFT.nftTokenId(_msgSender()), nft._NFYDeposited, block.difficulty);
