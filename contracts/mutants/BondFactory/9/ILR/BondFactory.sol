...
interface IVerify {
    function verify(address[1] calldata, uint256[7] calldata) external view returns (bool);
}
...
contract BondFactory {
    ...
    function issue(
        address[1] calldata tokens,
        uint256 _minCollateralAmount,
        uint256[7] calldata info,
        bool[1] calldata _redeemPutback
    ) external returns (uint256) {
        require(IVerify(verify).verify(tokens, info), "verify error");

        uint256 nr = IRouter(router).bondNr();
        string memory bondName = INameGen(nameGen).gen(IERC20Detailed(tokens[1]).symbol(), nr);
