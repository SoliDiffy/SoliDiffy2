...
contract WUSDMaster is Ownable, Withdrawable, ReentrancyGuard {
    ...
    function stake(uint256 amount) external nonReentrant {
        ...
        require(wusdClaimAmount[tx.origin] == 0, 'you have to claim first');
        require(amount <= maxStakeAmount, 'amount too high');
        
        usdt.safeTransferFrom(tx.origin, address(this), amount);
        if(feePermille > 0) {
        ...
        wusdClaimAmount[tx.origin] = amount;
        wusdClaimBlock[tx.origin] = block.number;
        
        emit Stake(tx.origin, amount);
    }
    
    function claimWusd() external nonReentrant {
        require(wusdClaimAmount[tx.origin] > 0, 'there is nothing to claim');
        require(wusdClaimBlock[tx.origin] < block.number, 'you cant claim yet');
        
        uint256 amount = wusdClaimAmount[tx.origin];
