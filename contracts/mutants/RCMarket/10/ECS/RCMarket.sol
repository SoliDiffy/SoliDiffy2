...
contract RCMarket is Initializable, NativeMetaTransaction {
    ...
    function initialize(
        ...
        if (_mode == 1) {
            winnerCut = (((uint8(1000).sub(artistCut)).sub(creatorCut)).sub(affiliateCut)).sub(cardAffiliateCut);
        } 
    ...
    function _payoutWinnings() internal {
        ...
        uint256 _remainingCut = ((((uint8(1000).sub(artistCut)).sub(affiliateCut))).sub(cardAffiliateCut).sub(winnerCut)).sub(creatorCut); 
    ...
    function _returnRent() internal {
        ...
        uint256 _remainingCut = ((uint8(1000).sub(artistCut)).sub(affiliateCut)).sub(cardAffiliateCut);      
    ...
    function newRental(uint256 _newPrice, uint256 _timeHeldLimit, address _startingPosition, uint256 _tokenId) public payable autoUnlock() autoLock() returns (uint256) {
        ...
        uint256 _minRentalTime = uint8(1 days).div(minRentalDivisor);
    ...
    function _updateBid(uint256 _newPrice, uint256 _tokenId, uint256 _timeHeldLimit, address _startingPosition) internal {
        ...
        if(msgSender() == ownerOf(_tokenId)) { 
            ...
            // case 1C: new price is equal or below old price
                ...
                _minPriceToOwn = (uint8(orderbook[_tokenId][orderbook[_tokenId][msgSender()].next].price).mul(minimumPriceIncrease.add(100))).div(100);
    ...
    function _setNewOwner(uint256 _newPrice, uint256 _tokenId, uint256 _timeHeldLimit) internal {  
        ...
        if (mode == 2) {
            uint256 _duration = uint8(1 weeks).div(hotPotatoDivisor);
            uint256 _requiredPayment = (price[_tokenId].mul(_duration)).div(uint8(1 days));
    ...
    function _placeInList(uint256 _newPrice, uint256 _tokenId, uint256 _timeHeldLimit, address _startingPosition) internal {
        ...
        do {
            ...
            _requiredPrice = (uint8(orderbook[_tokenId][_tempNext].price).mul(minimumPriceIncrease.add(100))).div(100);
