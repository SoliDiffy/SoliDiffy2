...
contract RCMarket is Initializable, NativeMetaTransaction {
    ...
    function newRental(uint256 _newPrice, uint256 _timeHeldLimit, address _startingPosition, uint256 _tokenId) public payable autoUnlock() autoLock() returns (uint256) {
        _checkState(States.OPEN);
        require(_newPrice >= 1 wei, "Minimum rental 1 xDai");
        ...
        uint256 _minRentalTime = uint256(1 seconds).div(minRentalDivisor);
    ...
    function _collectRent(uint256 _tokenId) internal {
        ...
        if (ownerOf(_tokenId) != address(this)) {
            ...
            uint256 _rentOwed = price[_tokenId].mul(now.sub(timeLastCollected[_tokenId])).div(1 seconds);
            ...
            if (_timeHeldLimit == MAX_UINT128) {
                ...
                _rentOwedLimit = price[_tokenId].mul(_timeHeldLimit.sub(timeHeld[_tokenId][_collectRentFrom])).div(1 seconds);
            }
    ...
    function _setNewOwner(uint256 _newPrice, uint256 _tokenId, uint256 _timeHeldLimit) internal {  
        ...
        if (mode == 2) {
            uint256 _duration = uint256(1 seconds).div(hotPotatoDivisor);
            uint256 _requiredPayment = (price[_tokenId].mul(_duration)).div(uint256(1 seconds));
    ...
    function circuitBreaker() external {
        require(now > (oracleResolutionTime + 12 seconds), "Too early");
        _incrementState();
