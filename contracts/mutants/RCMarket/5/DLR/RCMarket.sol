...
contract RCMarket is Initializable, NativeMetaTransaction {
    ...
    function initialize(
        uint256 _mode,
        uint32[] storage _timestamps,
        uint256 _numberOfTokens,
        uint256 _totalNftMintCount,
        address _artistAddress,
        address _affiliateAddress,
        address[] storage _cardAffiliateAddresses,
        ...
        uint256[5] storage _potDistribution = factory.getPotDistribution();
    ...
    function upgradeCard(uint256 _tokenId) external onlyTokenOwner(_tokenId) {
        ...
        string storage _tokenUri = tokenURI(_tokenId);
    ...
    function tokenURI(uint256 _tokenId) public view returns(string storage) {
        uint256 _actualTokenId = _tokenId.add(totalNftMintCount);
