...
// ========== CONSTRUCTOR ==========    
    ...
    */
    constructor(uint ein, address adminContractAddress) internal onlyAdminOrRole(adminContractAddress, IEndaomentAdmin.Role.ORG_FACTORY){
        taxId = ein;
    }
...
// ========== Org Management & Info ==========
     ...
     */
    function claimRequest(string memory fName, string memory lName, bool fSub, string memory eMail, address orgAdminAddress) external {
        require (fSub == true);
     ...
     * @param adminContractAddress Contract Address for Endaoment Admin
     */
    function approveClaim(uint index, address adminContractAddress) external onlyAdminOrRole(adminContractAddress, IEndaomentAdmin.Role.REVIEWER){
     ...
     * @param adminContractAddress Contract Address for Endaoment Admin
     */
    function cashOutOrg(address desiredWithdrawlAddress, address tokenAddress, address adminContractAddress) external onlyAdminOrRole(adminContractAddress, IEndaomentAdmin.Role.ACCOUNTANT){
...
//SWC-104-Unchecked Call Return Value:L93,95
    function setOrgWallet(address providedWallet, address adminContractAddress) external onlyAdminOrRole(adminContractAddress, IEndaomentAdmin.Role.REVIEWER){
