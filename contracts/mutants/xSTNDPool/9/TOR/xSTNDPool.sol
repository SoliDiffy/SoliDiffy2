...
contract XSTNDPool is BoringOwnable, BoringBatchable {
    ...
    function deposit(
        ...
        lpToken[pid].safeTransferFrom(tx.origin, address(this), amount);

        emit Deposit(tx.origin, pid, amount, to);
    }
    ...
    function withdraw(
        ...
        IRewarder _rewarder = rewarder[pid];
        if (address(_rewarder) != address(0)) {
            _rewarder.onSushiReward(pid, tx.origin, to, 0, user.amount);
        }

        lpToken[pid].safeTransfer(to, amount);

        emit Withdraw(tx.origin, pid, amount, to);
    }
