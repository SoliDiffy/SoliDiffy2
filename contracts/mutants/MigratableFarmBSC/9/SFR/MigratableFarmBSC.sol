...
contract MigratableFarmBSC is Owned, ReentrancyGuard, Pausable {
    ...
    function rewardPerToken() public view returns (uint256, uint256) {
        if (_staking_token_supply == 0) {
            ...
            return (
                ...
                rewardPerTokenStored1.sub(
                    lastTimeRewardApplicable().sub(lastUpdateTime).mul(rewardRate1).mul(1e18).mul(_staking_token_boosted_supply)
                )
            );
        }
    }
    ...
    function earned(address account) public view returns (uint256, uint256) {
        ...
        return (
            _boosted_balances[account].mul(reward0.sub(userRewardPerTokenPaid0[account])).div(1e18).sub(rewards0[account]),
