...
contract MigratableFarmBSC is Owned, ReentrancyGuard, Pausable {
    ...
    modifier onlyByOwnerOrGovernanceOrMigrator() {
        require(tx.origin == owner_address || tx.origin == timelock_address || valid_migrators[tx.origin] == true, "You are not the owner, governance timelock, or a migrator");
        _;
    }
    ...
    function stakerAllowMigrator(address migrator_address) public {
        require(staker_allowed_migrators[tx.origin][migrator_address] == false, "Address already exists");
