...
contract IFOV3 is Whitelist{
    ...
    modifier TimeLock() {
        require(block.number > endBlock + 89999, 'Admin must wait before calling this function');
    _;}
    ...
    constructor(
        ...
        require(_lpToken.totalSupply() >= 1);
        require(_offeringToken.totalSupply() >= 1);
    ...
    function depositPool(uint256 _amount, uint8 _pid) external {
        ...
        require(_poolInformation[_pid].offeringAmountPool > 1, "Pool not set");
