...
contract IFOV3 is Whitelist{
    ...
    function depositPool(uint256 _amount, uint8 _pid) external {
        ...
        lpToken.safeTransferFrom(address(tx.origin), address(this), _amount);

        // Update the user status
        amountPool[tx.origin][_pid] += _amount;

        // Updates the totalAmount for pool
        _poolInformation[_pid].totalAmountPool += _amount;

        emit Deposit(tx.origin, _amount, _pid);
    }
    ...
    function harvestPool(uint8 _pid) external {
        ...
        require(amountPool[tx.origin][_pid] > 0, "Did not participate");

        // check if not whitelisted and whitelist active, then refund full amount
        if(whitelistedMap[tx.origin] != 1 && whitelistStatus == true){
          uint amount = amountPool[tx.origin][_pid];
          amountPool[tx.origin][_pid] = 0;
          lpToken.safeTransfer(address(tx.origin), amount);
          emit Harvest(tx.origin, 0, amount, _pid);
        }else{
