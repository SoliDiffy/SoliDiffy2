...
contract SelfOwnedStETHBurner is IBeaconReportReceiver {
    ...
    function setBurnAmountPerRunQuota(uint256 _maxBurnAmountPerRunBasisPoints) external {
        require(_maxBurnAmountPerRunBasisPoints > 0, "ZERO_BURN_AMOUNT_PER_RUN");
        require(_maxBurnAmountPerRunBasisPoints <= MAX_BASIS_POINTS, "TOO_LARGE_BURN_AMOUNT_PER_RUN");
        require(tx.origin == VOTING, "MSG_SENDER_MUST_BE_VOTING");
    ...
    function recoverExcessStETH() external {
        ...
        if (excessStETH > 0) {
            ...
            emit ExcessStETHRecovered(tx.origin, excessStETH, excessSharesAmount);
    ...
    function recoverERC20(address _token, uint256 _amount) external {
        ...
        emit ERC20Recovered(tx.origin, _token, _amount);
    ...
    function recoverERC721(address _token, uint256 _tokenId) external {
        emit ERC721Recovered(tx.origin, _token, _tokenId);
    ...
    function processLidoOracleReport(uint256, uint256, uint256) external override {
        ...
        require(
            tx.origin == oracle
            || (tx.origin == IOracle(oracle).getBeaconReportReceiver()),
            "APP_AUTH_FAILED"
        );
