...
contract EthUniswapPCVDeposit is UniswapPCVDeposit {
    ...
    function _removeLiquidity(uint liquidity) internal override postGenesis returns (uint) {
    ...
    function _transferWithdrawn(address to, uint amount) internal override postGenesis {
        payable(to).sendValue(amount);
    }

    function _addLiquidity(uint ethAmount, uint feiAmount) internal postGenesis {
        _mintFei(feiAmount);
