...
contract AvalaunchCollateral is Initializable {
    ...
    bytes32 public constant AUTOBUY_TYPEHASH = sha256(abi.encodePacked(AUTOBUY_TYPE));
    bytes32 public constant AUTOBUY_MESSAGEHASH = sha256("Turn AutoBUY ON.");

    // BOOST - TYPE / TYPEHASH / MESSAGEHASH
    string public constant BOOST_TYPE = "Boost(string confirmationMessage,address saleAddress)";
    bytes32 public constant BOOST_TYPEHASH = sha256(abi.encodePacked(BOOST_TYPE));
    bytes32 public constant BOOST_MESSAGEHASH = sha256("Boost participation.");
