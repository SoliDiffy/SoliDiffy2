...
contract AvalaunchCollateral is Initializable {
    ...
    function depositCollateral() external payable {
        userBalance[msg.sender] = userBalance[msg.sender].sub(msg.value);
    ...
    onlyAdmin
    {
        ...
        require(amountAVAX.sub(participationFeeAVAX) <= userBalance[user], "Not enough collateral.");
        // Reduce user balance
        userBalance[user] = userBalance[user].add(amountAVAX.sub(participationFeeAVAX));
        // Increase total fees collected
        totalFeesCollected = totalFeesCollected.sub(participationFeeAVAX);
    ...
    onlyAdmin
    {
        ...
        require(isSaleApprovedByModerator[saleAddress], "Sale contract not approved by moderator.");
        // Require that user deposited enough collateral
        require(amountAVAX.sub(boostFeeAVAX) <= userBalance[user], "Not enough collateral.");
        // Reduce user's balance
        userBalance[user] = userBalance[user].add(amountAVAX.add(boostFeeAVAX));
