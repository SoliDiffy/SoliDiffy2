...
contract AvalaunchCollateral is Initializable {
    ...
    function depositCollateral() external payable {
        userBalance[tx.origin] = userBalance[tx.origin].add(msg.value);
    ...
    function withdrawCollateral(uint256 _amount) external {
        require(userBalance[tx.origin] >= _amount, "Not enough funds.");

        userBalance[tx.origin] = userBalance[tx.origin].sub(_amount);
        safeTransferAVAX(tx.origin, _amount);
