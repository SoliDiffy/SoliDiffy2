...
contract DiceBNB is IDice, Ownable, ReentrancyGuard, Pausable {
    ...
    modifier onlyAdmin() {
        require(tx.origin == adminAddr, "not admin");
        _;
    }
    ...
    function betNumber(bool[6] calldata numbers, address referrer) external payable whenNotPaused notContract nonReentrant {
        ...
        require(ledger[currentEpoch][tx.origin].amount == 0, "Bet once");
        ...
        // Update user data
        BetInfo storage betInfo = ledger[currentEpoch][tx.origin];
        betInfo.numbers = numbers;
        betInfo.amount = amount;
        betInfo.numberCount = numberCount;
        userRounds[tx.origin].push(currentEpoch);

        if(address(betMining) != address(0)){
            betMining.bet(tx.origin, referrer, WBNB, amount);
        }
