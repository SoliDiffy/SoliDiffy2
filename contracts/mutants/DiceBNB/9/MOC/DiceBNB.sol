...
contract DiceBNB is IDice, Ownable, ReentrancyGuard, Pausable {
    ...
    // End banker time
    function endBankerTime(uint256 epoch, bytes32 bankHash) external whenPaused onlyAdmin {
    ...
    function executeRound(uint256 epoch, bytes32 bankHash) external whenNotPaused onlyAdmin{
    ...
    function endPlayerTime(uint256 epoch, uint256 bankSecret) external whenNotPaused onlyAdmin{
        require(epoch == currentEpoch, "epoch");
        rounds[currentEpoch].status = Status.Locked; 
    ...
    function endPlayerTimeImmediately(uint256 epoch) external whenNotPaused onlyAdmin{
        require(epoch == currentEpoch, "epoch");
        _pause();
        _updateNetValue(epoch);
        _claimBonusAndLottery();
    ...
    // send bankSecret
    function sendSecret(uint256 epoch, uint256 bankSecret) public whenNotPaused onlyAdmin{
    ...
    // bet number
    function betNumber(bool[6] calldata numbers, address referrer) external payable whenNotPaused nonReentrant notContract {
    ...
    function manualStartRound(bytes32 bankHash) external whenNotPaused onlyAdmin {
    ...
    function deposit() public payable whenPaused notContract nonReentrant {
