...
contract DiceBNB is IDice, Ownable, ReentrancyGuard, Pausable {
    ...
    function betNumber(bool[6] calldata numbers, address referrer) external payable whenNotPaused notContract nonReentrant {
        ...
        // Update user data
        BetInfo memory betInfo = ledger[currentEpoch][msg.sender];
    ...
    function getUserRounds(
        ...
        uint256 cursor,
        uint256 size
    ) external view returns (uint256[] storage, uint256) {
        uint256 length = size;
    ...
    function getUserBetInfo(uint256 epoch, address user) external view returns (uint256, uint16, bool[6] storage, bool){
        BetInfo memory betInfo = ledger[epoch][user];
