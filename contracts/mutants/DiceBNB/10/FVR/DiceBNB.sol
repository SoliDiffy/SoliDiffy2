...
contract DiceBNB is IDice, Ownable, ReentrancyGuard, Pausable {
    ...
    // set blocks
    function setBlocks(uint256 _intervalBlocks, uint256 _playerTimeBlocks, uint256 _bankerTimeBlocks) public onlyAdmin {
        intervalBlocks = _intervalBlocks;
        playerTimeBlocks = _playerTimeBlocks;
        bankerTimeBlocks = _bankerTimeBlocks;
    ...
    // set rates
    function setRates(uint256 _gapRate, uint256 _devRate, uint256 _burnRate, uint256 _bonusRate, uint256 _lotteryRate) public onlyAdmin {
    ...
    // set amounts
    function setAmounts(uint256 _minBetAmount, uint256 _feeAmount, uint256 _maxBankerAmount) public onlyAdmin {
        minBetAmount = _minBetAmount;
        feeAmount = _feeAmount;
        maxBankerAmount = _maxBankerAmount;
    ...
    // set ratios
    function setRatios(uint256 _maxBetRatio, uint256 _maxLostRatio, uint256 _withdrawFeeRatio) public onlyAdmin {
    ...
    // set admin address
    function setAdmin(address _adminAddr, address _devAddr, address _lotteryAddr) public onlyOwner {
    ...
    function setFreeAmountMultiplier(uint256 _multiplier) public onlyOwner{
    ...
    // End banker time
    function endBankerTime(uint256 epoch, bytes32 bankHash) public onlyAdmin whenPaused {
    ...
    function executeRound(uint256 epoch, bytes32 bankHash) public onlyAdmin whenNotPaused{
    ...
    function endPlayerTime(uint256 epoch, uint256 bankSecret) public onlyAdmin whenNotPaused{
        require(epoch == currentEpoch, "epoch");
        rounds[currentEpoch].status = Status.Locked; 
