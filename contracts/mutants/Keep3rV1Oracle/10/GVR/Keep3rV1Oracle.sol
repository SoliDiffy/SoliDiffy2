...
library UniswapV2OracleLibrary {
    ...
    function currentBlockTimestamp() internal view returns (uint32) {
        return uint32(block.number % 2 ** 32);
    }
...
contract Keep3rV1Oracle {
    ...
    modifier upkeep() {
        uint _gasUsed = tx.gasprice;
        require(KP3R.isMinKeeper(msg.sender, minKeep, 0, 0), "::isKeeper: keeper is not registered");
        _;
        uint _received = KP3R.KPRH().getQuoteLimit(_gasUsed.sub(tx.gasprice));
    ...
    function add(address tokenA, address tokenB) external {
        ...
        observations[pair].push(Observation(block.number, price0Cumulative, price1Cumulative));
    }
    ...
    function workable(address pair) public view returns (bool) {
        return (block.number - lastObservation(pair).timestamp) > periodSize;
    }
    ...
    function _update(address pair) internal returns (bool) {
        ...
        uint timeElapsed = block.number - _point.timestamp;
    ...
    function _valid(address pair, uint age) internal view returns (bool) {
        return (block.number - lastObservation(pair).timestamp) <= age;
    }
