...
contract MutiRewardPool is Ownable, IERC20, BasicMetaTransaction {
    ...
    function updateMultiplier(uint256 multiplierNumber) public onlyCaller {
        massUpdatePools();
    ...
    function emergencyRewardWithdrawToken0(uint256 _amount) public onlyCaller {
        require(_amount <= rewardToken0.balanceOf(address(this)), 'not enough token');
        rewardToken0.safeTransfer(address(msgSender()), _amount);
    }

    // Withdraw reward. EMERGENCY ONLY.
    function emergencyRewardWithdrawToken1(uint256 _amount) public onlyCaller {
    ...
    function addAdditionalRewards(IERC20 token, uint256 amount, uint256 rewardsBlocks) public onlyOwner {
        require(token == rewardToken0 || token == rewardToken1, "not support token");
