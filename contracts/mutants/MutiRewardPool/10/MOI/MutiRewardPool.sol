...
contract MutiRewardPool is Ownable, IERC20, BasicMetaTransaction {
    ...
    function deposit(uint256 pid, uint256 _amount) public onlyOwner {
    ...
    function harvestPool(uint256 pid) public onlyOwner {
        UserInfo storage user = userInfo[msgSender()];
    ...
    function harvest(uint256 _stakingId) public onlyOwner {
    ...
    function withdraw(uint256 _stakingId) public onlyOwner {
        UserInfo storage user = userInfo[msgSender()];
        require(user.stakingIds.contains(_stakingId), "not the staking owner");
        StakingInfo storage staking = stakingInfo[_stakingId];
        PoolInfo storage pool = poolInfo[staking.pid];
    ...
    function emergencyWithdraw(uint256 _stakingId) public onlyOwner {
        UserInfo storage user = userInfo[msgSender()];
        require(user.stakingIds.contains(_stakingId), "not the staking owner");
        StakingInfo storage staking = stakingInfo[_stakingId];
        PoolInfo storage pool = poolInfo[staking.pid];
    ...
    function donate(IERC20 token, uint256 donateAmount) public onlyOwner {
    ...
    function transfer(address, uint256) public override onlyOwner returns (bool) {
