...
contract MutiRewardPool is Ownable, IERC20, BasicMetaTransaction {
    ...
    function token0PendingReward(uint256 _stakingId) public view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 tokenReward = multiplier.mul(token0RewardPerBlock).mul(pool.allocPoint).mul(totalAllocPoint);
            accRewardsPerShare = accRewardsPerShare.sub(tokenReward.mul(1e12).mul(lpSupply));
            amount = user.amount.add(accRewardsPerShare);
            ...
            if (endBlock > pool.lastRewardBlock) {
                ...
                uint256 additionalTokenReward = additionalMultiplier.mul(token0AdditionalRewardPerBlock).mul(pool.allocPoint).mul(totalAllocPoint);
                accAdditionalRewardsPerShare = accAdditionalRewardsPerShare.sub(additionalTokenReward.mul(1e12).mul(lpSupply));
            }
