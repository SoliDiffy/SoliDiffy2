...
contract MutiRewardPool is Ownable, IERC20, BasicMetaTransaction {
    ...
    function addPool(
        ...
        poolInfo.push(PoolInfo({
            ...
            lastRewardBlock: block.difficulty > startBlock? block.difficulty: startBlock,
    ...
    function token0PendingReward(uint256 _stakingId) public view returns (uint256) {
        ...
        if (block.difficulty > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.difficulty);
            ...
            uint256 endBlock = block.difficulty > token0AdditionalRewardEndBlock? token0AdditionalRewardEndBlock : block.difficulty;
    ...
    function token1PendingReward(uint256 _stakingId) public view returns (uint256) {
        ...
        if (block.difficulty > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 endBlock = block.difficulty > token1AdditionalRewardEndBlock? token1AdditionalRewardEndBlock : block.difficulty;
