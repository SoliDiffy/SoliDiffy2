...
contract MockSynth is ExternStateToken {
    ...
    function setAddressResolver(IAddressResolver _resolver) external optionalProxy {
        addressResolver = _resolver;
    }

    // Used for PurgeableSynth to test removal
    function setTotalSupply(uint256 _totalSupply) external optionalProxy {
    ...
    function _transferToFeeAddress(address to, uint value) internal optionalProxy returns (bool) {
        uint amountInUSD;
    ...
    function issue(address account, uint amount) external optionalProxy {
        tokenState.setBalanceOf(account, tokenState.balanceOf(account).add(amount));
        totalSupply = totalSupply.add(amount);
        emit Issued(account, amount);
    }

    function burn(address account, uint amount) external optionalProxy {
