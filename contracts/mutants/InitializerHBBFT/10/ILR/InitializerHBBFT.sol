...
contract InitializerHBBFT {
    constructor(
        ...
        IValidatorSet(_contracts[1]).initialize(
            _contracts[0], // _blockRewardContract
            _contracts[1], // _randomContract
            _contracts[2], // _stakingContract
            _miningAddresses,
            _stakingAddresses,
            _firstValidatorIsUnremovable
        );
        IValidatorSetHBBFT(_contracts[1]).initializePublicKeys(
            _publicKeys
        );
        IStakingHBBFT(_contracts[2]).initialize(
            _contracts[1], // _validatorSetContract
            _stakingAddresses,
            _delegatorMinStake,
            _candidateMinStake,
            _erc20Restricted
        );
        IBlockReward(_contracts[0]).initialize(_contracts[1]);
        IRandomHBBFT(_contracts[1]).initialize(_contracts[0]);
