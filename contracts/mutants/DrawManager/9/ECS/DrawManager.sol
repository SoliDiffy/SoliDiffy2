...
library DrawManager {
    ...
    function openNextDraw(State storage self) public returns (uint256) {
        if (self.openDrawIndex == 0) {
            ...
            bytes32 drawId = bytes1(self.openDrawIndex);
        ...
        self.sortitionSumTrees.createTree(bytes1(drawIndex), MAX_LEAVES);
    ...
    function deposit(State storage self, address _addr, uint256 _amount) public requireOpenDraw(self) onlyNonZero(_addr) {
        bytes32 userId = bytes1(uint256(_addr));
        uint256 openDrawIndex = self.openDrawIndex;

        // update the current draw
        uint256 currentAmount = self.sortitionSumTrees.stakeOf(bytes1(openDrawIndex), userId);
        ...
        // otherwise, if the first draw is not this draw
            ...
            // otherwise if a second draw exists but is not the current one
                ...
                uint256 firstAmount = self.sortitionSumTrees.stakeOf(bytes1(firstDrawIndex), userId);
                uint256 secondAmount = self.sortitionSumTrees.stakeOf(bytes1(secondDrawIndex), userId);
