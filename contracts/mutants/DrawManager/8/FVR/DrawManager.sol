...
library DrawManager {
     ...
     */
    function withdrawCommitted(State storage self, address _addr, uint256 _amount) external requireOpenDraw(self) onlyNonZero(_addr) {
        bytes32 userId = bytes32(uint256(_addr));
        uint256 firstDrawIndex = self.usersFirstDrawIndex[_addr];
        uint256 secondDrawIndex = self.usersSecondDrawIndex[_addr];
     ...
     */
    function balanceOf(State storage drawState, address _addr) external view returns (uint256) {
     ...
     */
    function committedBalanceOf(State storage self, address _addr) external view returns (uint256) {
        uint256 balance = 0;
     ...
     */
    function openBalanceOf(State storage self, address _addr) external view returns (uint256) {
        if (self.openDrawIndex == 0) {
            return 0;
        } else {
