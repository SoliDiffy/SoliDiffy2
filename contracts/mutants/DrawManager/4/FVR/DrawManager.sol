...
library DrawManager {
     ...
     */
    function openNextDraw(State storage self) external returns (uint256) {
     ...
     */
    function deposit(State storage self, address _addr, uint256 _amount) external requireOpenDraw(self) onlyNonZero(_addr) {
     ...
     * @param _amount The amount to deposit
     */
    function depositCommitted(State storage self, address _addr, uint256 _amount) external requireOpenDraw(self) onlyNonZero(_addr) {
        bytes32 userId = bytes32(uint256(_addr));
        uint256 firstDrawIndex = self.usersFirstDrawIndex[_addr];
     ...
     */
    function withdraw(State storage self, address _addr) external requireOpenDraw(self) onlyNonZero(_addr) {
        uint256 firstDrawIndex = self.usersFirstDrawIndex[_addr];
        uint256 secondDrawIndex = self.usersSecondDrawIndex[_addr];
