...
library DrawManager {
     ...
     */
    function deposit(State storage self, address _addr, uint256 _amount) public onlyNonZero(_addr) requireOpenDraw(self) {
     ...
     * @param _amount The amount to deposit
     */
    function depositCommitted(State storage self, address _addr, uint256 _amount) public onlyNonZero(_addr) requireOpenDraw(self) {
        bytes32 userId = bytes32(uint256(_addr));
        uint256 firstDrawIndex = self.usersFirstDrawIndex[_addr];
     ...
     */
    function withdraw(State storage self, address _addr) public onlyNonZero(_addr) requireOpenDraw(self) {
        uint256 firstDrawIndex = self.usersFirstDrawIndex[_addr];
        uint256 secondDrawIndex = self.usersSecondDrawIndex[_addr];
