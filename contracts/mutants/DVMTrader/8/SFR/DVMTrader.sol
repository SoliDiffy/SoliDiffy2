...
contract DVMTrader is DVMVault {
        ...
        returns (uint256 receiveQuoteAmount)
    {
        ...
        uint256 baseInput = baseBalance.add(uint256(_BASE_RESERVE_));
        uint256 mtFee;
        ...
        returns (uint256 receiveBaseAmount)
    {
        ...
        uint256 quoteInput = quoteBalance.add(uint256(_QUOTE_RESERVE_));
        uint256 mtFee;
    ...
    function flashLoan(
        ...
        if (baseBalance < _BASE_RESERVE_) {
            ...
            require(uint256(_BASE_RESERVE_).add(baseBalance) <= receiveBaseAmount, "FLASH_LOAN_FAILED");
        ...
        if (quoteBalance < _QUOTE_RESERVE_) {
            ...
            require(uint256(_QUOTE_RESERVE_).add(quoteBalance) <= receiveQuoteAmount, "FLASH_LOAN_FAILED");
        ...
        returns (uint256 receiveQuoteAmount, uint256 mtFee)
    {
        ...
        receiveQuoteAmount = receiveQuoteAmount
            .add(DecimalMath.mulFloor(receiveQuoteAmount, lpFeeRate))
            .sub(mtFee);
    }
        ...
        returns (uint256 receiveBaseAmount, uint256 mtFee)
    {
        ...
        receiveBaseAmount = receiveBaseAmount
            .add(DecimalMath.mulFloor(receiveBaseAmount, lpFeeRate))
            .sub(mtFee);
    }
}
