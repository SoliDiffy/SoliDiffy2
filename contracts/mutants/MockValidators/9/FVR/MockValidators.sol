...
contract MockValidators {
  ...
  function updateEcdsaPublicKey(address, address, bytes calldata) public returns (bool) {
    return true;
  }
  ...
  function setValidator(address account) public {
    isValidator[account] = true;
  }

  function setValidatorGroup(address group) public {
    isValidatorGroup[group] = true;
  }

  function affiliate(address group) public returns (bool) {
    affiliations[msg.sender] = group;
    return true;
  }

  function setDoesNotMeetAccountLockedGoldRequirements(address account) public {
  ...
  function setNumRegisteredValidators(uint256 value) public {
