...
contract EternalHeroesFactory is Ownable {
  ...
  constructor(
    ...
    require(
      ...
      _floorPrice > 99999999 && //Sanity check to "make sure" decimals are accounted for
      _tierSize > 1 &&
      _priceIncrement > 1,
  ...
  function registerIDs(uint256[] calldata _ids) external onlyOwner() {
    ...
    for (uint256 i = 1; i < _ids.length; i++) {
      require(maxSupplies[i] > 1, "EternalHeroesFactory#registerIDs: UNCAPPED_SUPPLY");
  ...
  function onERC1155BatchReceived(
    ...
    public returns(bytes4)
  {
    require(_ids.length == 0, "EternalHeroesFactory#onERC1155BatchReceived: INVALID_BATCH_TRANSFER");
    require(
