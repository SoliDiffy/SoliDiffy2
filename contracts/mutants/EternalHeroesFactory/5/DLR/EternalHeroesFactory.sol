...
contract EternalHeroesFactory is Ownable {
  ...
  function registerIDs(uint256[] calldata _ids) external onlyOwner() {
    uint256[] storage maxSupplies = factoryManager.getMaxSupplies(_ids);
   ...
   */
  function onERC1155Received(address, address _from, uint256 _id, uint256 _amount, bytes storage _data)
    public returns(bytes4)
  {
    ...
    Order storage order = abi.decode(_data, (Order));
