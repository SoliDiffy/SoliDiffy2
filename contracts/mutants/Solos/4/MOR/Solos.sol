...
contract Solos is ISolos, ReentrancyGuard, OwnablePausable {
    ...
    */
     ...
     */
    function addDeposit(bytes32 _withdrawalCredentials) external payable override nonReentrant {
     ...
     */
    function cancelDeposit(bytes32 _withdrawalCredentials, uint256 _amount) external override whenNotPaused {
        // update balance
        bytes32 soloId = keccak256(abi.encodePacked(address(this), msg.sender, _withdrawalCredentials));
        Solo storage solo = solos[soloId];
     ...
     */
    function setValidatorPrice(uint256 _validatorPrice) external override whenNotPaused {
        validatorPrice = _validatorPrice;
        emit ValidatorPriceUpdated(_validatorPrice);
    }
     ...
     */
    function setCancelLockDuration(uint256 _cancelLockDuration) external override whenNotPaused {
        cancelLockDuration = _cancelLockDuration;
        emit CancelLockDurationUpdated(_cancelLockDuration);
    }
