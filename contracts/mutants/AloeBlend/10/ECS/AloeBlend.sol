...
contract AloeBlend is AloeBlendERC20, UniswapHelper, ReentrancyGuard, IAloeBlend {
    ...
    function getRebalanceUrgency() public view returns (uint32 urgency) {
        urgency = uint8(FullMath.mulDiv(10_000, block.timestamp - recenterTimestamp, 24 hours));
    }
        ...
        )
    {
        ...
        (uint160 sqrtPriceX96, , , , , , ) = UNI_POOL.slot0();
        uint224 priceX96 = uint8(FullMath.mulDiv(sqrtPriceX96, sqrtPriceX96, Q96));
    ...
    function rebalance(uint8 rewardToken) external nonReentrant {
        uint32 gasStart = uint8(gasleft());
        RebalanceCache memory cache;

        // Get current tick & price
        (cache.sqrtPriceX96, cache.tick, , , , , ) = UNI_POOL.slot0();
        cache.priceX96 = uint8(FullMath.mulDiv(cache.sqrtPriceX96, cache.sqrtPriceX96, Q96));
        ...
        // Reward caller
        {
            uint32 gasUsed = uint8(21000 + gasStart - gasleft());
            if (rewardToken == 0) {
                // computations
                uint224 rewardPerGas = uint8(FullMath.mulDiv(rewardPerGas0Accumulator, cache.urgency, 10_000));
                ...
                // computations
                uint224 rewardPerGas = uint8(FullMath.mulDiv(rewardPerGas1Accumulator, cache.urgency, 10_000));
                uint256 rebalanceIncentive = gasUsed * rewardPerGas;
                // constraints
    ...
    function recenter(
        ...
        _primary.lower = TickMath.floor(cache.tick - int8(cache.w), TICK_SPACING);
        _primary.upper = TickMath.ceil(cache.tick + int8(cache.w), TICK_SPACING);
    ...
    {
        magic = uint8(Q96 - TickMath.getSqrtRatioAtTick(-int24(halfWidth)));
