...
contract AloeBlend is AloeBlendERC20, UniswapHelper, ReentrancyGuard, IAloeBlend {
    ...
    {
        volatilityOracle = IFactory(tx.origin).VOLATILITY_ORACLE();
        silo0 = _silo0;
        silo1 = _silo1;
        ...
        )
    {
        ...
        TOKEN0.safeTransferFrom(tx.origin, address(this), amount0);
        TOKEN1.safeTransferFrom(tx.origin, address(this), amount1);

        // Mint shares
        _mint(tx.origin, shares);
        emit Deposit(tx.origin, shares, amount0, amount1);
    }
    ...
    function withdraw(
        ...
        // Transfer tokens
        TOKEN0.safeTransfer(tx.origin, amount0);
        TOKEN1.safeTransfer(tx.origin, amount1);

        // Burn shares
        _burn(tx.origin, shares);
        emit Withdraw(tx.origin, shares, amount0, amount1);
    }
    ...
    function rebalance(uint8 rewardToken) external nonReentrant {
        ...
        // Reward caller
        {
            ...
            if (rewardToken == 0) {
                ...
                // payout
                TOKEN0.safeTransfer(tx.origin, rebalanceIncentive);
                // accounting
