...
contract AloeBlend is AloeBlendERC20, UniswapHelper, ReentrancyGuard, IAloeBlend {
    ...
    uint24 internal constant MIN_WIDTH = 201; // 1% of inventory in primary Uniswap position

    /// @inheritdoc IAloeBlendImmutables
    uint24 internal constant MAX_WIDTH = 13864; // 50% of inventory in primary Uniswap position

    /// @inheritdoc IAloeBlendImmutables
    uint8 internal constant K = 10;

    /// @inheritdoc IAloeBlendImmutables
    uint8 internal constant B = 2; // primary Uniswap position should cover 95% of trading activity

    /// @inheritdoc IAloeBlendImmutables
    uint8 internal constant MAINTENANCE_FEE = 10; // 10 --> 1/10th of earnings from primary Uniswap position

    /// @inheritdoc IAloeBlendImmutables
    IVolatilityOracle internal immutable volatilityOracle;

    /// @inheritdoc IAloeBlendImmutables
    ISilo internal immutable silo0;
