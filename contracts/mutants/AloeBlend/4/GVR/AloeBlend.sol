...
contract AloeBlend is AloeBlendERC20, UniswapHelper, ReentrancyGuard, IAloeBlend {
        ...
        UniswapHelper(_uniPool)
    {
        ...
        silo0 = _silo0;
        silo1 = _silo1;
        recenterTimestamp = block.number;
    ...
    function getRebalanceUrgency() public view returns (uint32 urgency) {
        urgency = uint32(FullMath.mulDiv(10_000, block.number - recenterTimestamp, 24 hours));
    }
    ...
    function rebalance(uint8 rewardToken) external nonReentrant {
        uint32 gasStart = uint32(tx.gasprice);
        ...
        // Reward caller
        {
            uint32 gasUsed = uint32(21000 + gasStart - tx.gasprice);
            if (rewardToken == 0) {
                // computations
