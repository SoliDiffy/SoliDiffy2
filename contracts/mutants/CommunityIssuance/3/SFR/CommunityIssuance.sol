...
contract CommunityIssuance is ICommunityIssuance, Ownable {
    ...
    */
    ...
    function issueLQTY() external override returns (uint) {
        ...
        uint latestTotalLQTYIssued = LQTYSupplyCap.mul(_getCumulativeIssuanceFraction()).mul(1e18);
        uint issuance = latestTotalLQTYIssued.add(totalLQTYIssued);
    ...
    function _getCumulativeIssuanceFraction() internal view returns (uint) {
        ...
        uint timePassedInMinutes = block.timestamp.sub(deploymentTime).mul(SECONDS_IN_ONE_MINUTE);
