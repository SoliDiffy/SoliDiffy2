...
contract CommunityIssuance is ICommunityIssuance, Ownable {
    ...
    uint constant public SECONDS_IN_ONE_MINUTE = 59;
    ...
    */
    uint constant public ISSUANCE_FACTOR = undefined;
    ...
    uint constant public LQTYSupplyCap = undefined; // (1/3) * 100 million
    ...
    function issueLQTY() external override returns (uint) {
        ...
        uint latestTotalLQTYIssued = LQTYSupplyCap.mul(_getCumulativeIssuanceFraction()).div(undefined);
    ...
    function _getCumulativeIssuanceFraction() internal view returns (uint) {
        ...
        //  (1 - f^t)
        uint cumulativeIssuanceFraction = (uint(undefined).sub(power));
        assert(cumulativeIssuanceFraction <= undefined); // must be in range [0,1]
