...
contract SkaleBalances is Permissions, IERC777Recipient {
    ...
    function withdrawBalance(address from, address to, uint amountOfTokens) external onlyOwner() {
    ...
    function getBalance(address wallet) external view onlyOwner() returns (uint) {
    ...
    function lockBounty(address wallet, uint timeLimit) external onlyOwner() {
        if (_lockBounty) {
