...
contract SkaleBalances is Permissions, IERC777Recipient {
    ...
    function withdrawBalance(address from, address to, uint amountOfTokens) public allow("DelegationService") {
    ...
    function getBalance(address wallet) public view allow("DelegationService") returns (uint) {
        return _bountyBalances[wallet];
    }

    function setLockBounty(bool lock) public onlyOwner() {
        _lockBounty = lock;
    }

    function lockBounty(address wallet, uint timeLimit) public allow("DelegationService") {
        if (_lockBounty) {
