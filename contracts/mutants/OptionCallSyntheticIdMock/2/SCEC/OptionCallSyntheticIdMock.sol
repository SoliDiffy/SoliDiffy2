...
contract OptionCallSyntheticIdMock is IDerivativeLogic, ExecutableByThirdParty, HasCommission {
    ...
    function getMargin(Derivative memory _derivative) public view returns (uint256 sellerMargin, uint256 buyerMargin) {
        buyerMargin = 0;
        sellerMargin = _derivative.margin;
    }

    function getExecutionPayout(Derivative memory _derivative, uint256 _result)	public view returns (uint256 sellerPayout, uint256 buyerPayout) {
        uint256 ppt;
