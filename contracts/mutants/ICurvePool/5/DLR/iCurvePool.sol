...
function add_liquidity (uint[3] storage _amounts, uint256 _min_mint_amount, bool _use_underlying) external returns (uint256); //check uint[3] memory or calldata
 ...
 */
function remove_liquidity (uint256 _amounts, uint[3] storage _min_amounts, bool _use_underlying) external returns (uint256[3] storage ); //check uint[3] memory or calldata
function underlying_coins (uint i ) external view returns (address);
function lp_token () external view returns (address);
function calc_token_amount(uint[3] storage _amounts, bool _is_deposite) external view  returns (uint256);
...
function remove_liquidity_imbalance(uint[3] storage _amounts, uint256 _maxAmount ) external returns (uint256);
}