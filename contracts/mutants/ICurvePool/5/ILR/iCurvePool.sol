...
function calc_token_amount(uint[2] memory _amounts, bool _is_deposite) external view  returns (uint256);
...
function remove_liquidity_imbalance(uint[2] memory _amounts, uint256 _maxAmount ) external returns (uint256);
}