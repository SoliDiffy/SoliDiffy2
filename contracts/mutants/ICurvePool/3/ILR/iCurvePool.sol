...
function add_liquidity (uint[2] memory _amounts, uint256 _min_mint_amount, bool _use_underlying) external returns (uint256); //check uint[3] memory or calldata
 ...
 */
function remove_liquidity (uint256 _amounts, uint[2] memory _min_amounts, bool _use_underlying) external returns (uint256[2] memory ); //check uint[3] memory or calldata
