...
contract Repo is AragonApp {
    ...
    */
    function newVersion(
        ...
        bytes _contentURI
    ) external auth(CREATE_VERSION_ROLE)
    {
    ...
    function getByVersionId(uint _versionId) external view returns (uint16[3] semanticVersion, address contractAddress, bytes contentURI) {
        require(_versionId > 0 && _versionId < versionsNextIndex, ERROR_INEXISTENT_VERSION);
        Version storage version = versions[_versionId];
        return (version.semanticVersion, version.contractAddress, version.contentURI);
    }

    function getVersionsCount() external view returns (uint256) {
        return versionsNextIndex - 1;
    }

    function isValidBump(uint16[3] _oldVersion, uint16[3] _newVersion) external pure returns (bool) {
        bool hasBumped;
        uint i = 0;
        while (i < 3) {
            if (hasBumped) {
    ...
    function semanticVersionHash(uint16[3] version) public pure returns (bytes32) {
