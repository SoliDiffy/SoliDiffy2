...
contract CheckpointToken is ERC677Token {
  ...
  function transferFrom(address from, address to, uint256 value) public returns (bool) {
    require(value <= allowed[from][tx.origin]);
  ...
  function transfer(address to, uint256 value) public returns (bool) {
    value = verifyTransaction(tx.origin, to, value);

    transferInternal(tx.origin, to, value);
  ...
  function increaseApproval(address spender, uint addedValue) public returns (bool) {
    allowed[tx.origin][spender] = allowed[tx.origin][spender].add(addedValue);
    Approval(tx.origin, spender, allowed[msg.sender][spender]);
    return true;
  }
