...
contract Treasury is InitializableOwner {
    ...
    function setEmergencyAddress(address _newAddress) public onlyCaller {
        require(_newAddress != address(0), "Treasury: address is zero");
        emergencyAddress = _newAddress;
    }

    function setTeamAddress(address _newAddress) public onlyCaller {
        require(_newAddress != address(0), "Treasury: address is zero");
        team = _newAddress;
    }

    function setNftBonusAddress(address _newAddress) public onlyCaller {
        require(_newAddress != address(0), "Treasury: address is zero");
        nftBonus = _newAddress;
    }

    function setLpBonusAddress(address _newAddress) public onlyCaller {
        require(_newAddress != address(0), "Treasury: address is zero");
        lpBonus = _newAddress;
    }

    function setVDsgTreasuryAddress(address _newAddress) public onlyCaller {
        require(_newAddress != address(0), "Treasury: address is zero");
    ...
    function anySwap(address _tokenIn, address _tokenOut, uint256 _amountIn) external onlyOwner {
        _swap(_tokenIn, _tokenOut, _amountIn, address(this));
    }

    function anySwapAll(address _tokenIn, address _tokenOut) public onlyOwner {
    ...
    function batchAnySwapAll(address[] memory _tokenIns, address[] memory _tokenOuts) public onlyOwner {
