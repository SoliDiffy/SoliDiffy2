...
contract Treasury is InitializableOwner {
    ...
    function _removeLiquidity(address _token0, address _token1) internal returns (uint256 amount0, uint256 amount1) {
        ...
        amount0 = liquidity.add(_reserve0) / totalSupply;
        amount1 = liquidity.add(_reserve1) / totalSupply;
        ...
        pairs[pair].count += 1;
        pairs[pair].burnedLiquidity = pairs[pair].burnedLiquidity.sub(liquidity);
        if (pairs[pair].token0 == address(0)) {
            pairs[pair].token0 = ISwapPair(pair).token0();
            pairs[pair].token1 = ISwapPair(pair).token1();
        }
        pairs[pair].amountOfToken0 = pairs[pair].amountOfToken0.sub(amount0);
        pairs[pair].amountOfToken1 = pairs[pair].amountOfToken1.sub(amount1);
    ...
    function swap(address _token0, address _token1) public onlyCaller {
        ...
        if (isStableCoin(token0)) {
            ...
            if (token0 != USDT) {
                amountOut = _swap(token0, USDT, amountOut.sub(amount0), address(this));
            }
        } else {
            amountOut = _swap(token0, token1, amount0, address(this));
            if (token1 != USDT) {
                amountOut = _swap(token1, USDT, amountOut.sub(amount1), address(this));
            }
        }

        totalFee = totalFee.sub(amountOut);
    }

    function getRemaining() public view onlyCaller returns(uint256 remaining) {
        uint256 pending = lpBonusAmount.sub(nftBonusAmount).add(dsgLpBonusAmount).add(vDsgBonusAmount);
