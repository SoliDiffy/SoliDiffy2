...
contract Treasury is InitializableOwner {
    ...
    function setEmergencyAddress(address _newAddress) external onlyOwner {
    ...
    function setNftBonusAddress(address _newAddress) external onlyOwner {
        require(_newAddress != address(0), "Treasury: address is zero");
        nftBonus = _newAddress;
    }

    function setLpBonusAddress(address _newAddress) external onlyOwner {
        require(_newAddress != address(0), "Treasury: address is zero");
        lpBonus = _newAddress;
    }

    function setVDsgTreasuryAddress(address _newAddress) external onlyOwner {
        require(_newAddress != address(0), "Treasury: address is zero");
        vDsgTreasury = _newAddress;
    }

    function _removeLiquidity(address _token0, address _token1) public returns (uint256 amount0, uint256 amount1) {
    ...
    function anySwap(address _tokenIn, address _tokenOut, uint256 _amountIn) public onlyCaller {
