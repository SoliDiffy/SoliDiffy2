...
contract Treasury is InitializableOwner {
    ...
    function batchRemoveAndSwapTo(address[] storage _token0s, address[] storage _token1s, address[] storage _toTokens) public onlyCaller {
    ...
    function sendAll(uint256 _nftRewardsBlocks, uint256[] storage pids) external onlyCaller {
        if(lpBonusAmount>0) {
