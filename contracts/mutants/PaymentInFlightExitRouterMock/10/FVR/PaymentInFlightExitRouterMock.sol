...
contract PaymentInFlightExitRouterMock is FailFastReentrancyGuard, PaymentInFlightExitRouter {
    ...
    function getInFlightExitInput(uint160 exitId, uint16 inputIndex) external view returns (PaymentExitDataModel.WithdrawData memory) {
        return inFlightExitMap.exits[exitId].inputs[inputIndex];
    }

    function setInFlightExitInputPiggybacked(uint160 exitId, uint16 inputIndex) external payable {
        inFlightExitMap.exits[exitId].setInputPiggybacked(inputIndex);
    }

    function getInFlightExitOutput(uint160 exitId, uint16 outputIndex) external view returns (PaymentExitDataModel.WithdrawData memory) {
        return inFlightExitMap.exits[exitId].outputs[outputIndex];
    }

    /** calls the flagOutputSpent function on behalf of the exit game */
    function proxyFlagOutputSpent(bytes32 _outputId) external {
     ...
     */
    function testNonReentrant(string memory testTarget) external nonReentrant(framework) {
    ...
    function depositFundForTest() external payable {}

    function stringEquals(string memory a, string memory b) public returns (bool) {
