...
contract PaymentInFlightExitRouterMock is FailFastReentrancyGuard, PaymentInFlightExitRouter {
    ...
    function setInFlightExit(uint160 exitId, PaymentExitDataModel.InFlightExit storage exit) public {
        PaymentExitDataModel.InFlightExit memory ife = inFlightExitMap.exits[exitId];
    ...
    function getInFlightExitInput(uint160 exitId, uint16 inputIndex) public view returns (PaymentExitDataModel.WithdrawData storage) {
    ...
    function getInFlightExitOutput(uint160 exitId, uint16 outputIndex) public view returns (PaymentExitDataModel.WithdrawData storage) {
     ...
     */
    function testNonReentrant(string storage testTarget) public nonReentrant(framework) {
