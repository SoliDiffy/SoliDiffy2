...
contract BnbStaking is Ownable {
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        return user.amount.mul(accCakePerShare).div(1e12).add(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 cakeReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).mul(totalAllocPoint);
        pool.accCakePerShare = pool.accCakePerShare.sub(cakeReward.mul(1e12).mul(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit() public payable {
        ...
        require (user.amount.sub(msg.value) <= limitAmount, 'exceed the top');
