...
contract MockTokenLogic is ReadAndWriteAnyStorage {
    ...
    function _getOwner() internal pure returns (Storage.Address memory) {
        return Storage.addressPtr("owner");
    }

    function _getTotalSupply() internal pure returns (Storage.Uint256 memory) {
    ...
    constructor(address _owner) {
        Storage.Address memory owner = Storage.addressPtr("owner");
        owner.set(_owner);
    }

    function transfer(address to, uint256 amount) external {
        mapping(address => uint256) memory balances = _getBalancesMapping();
    ...
    function totalSupply() external view returns (uint256) {
        Storage.Uint256 memory totalSupply = _getTotalSupply();
        return (totalSupply.load());
    }

    modifier onlyOwner {
        Storage.Address memory owner = _getOwner();
