...
contract MockLockedGold is ILockedGold {
  ...
  function lock() external payable {
    accountTotalLockedGold[tx.origin] = accountTotalLockedGold[tx.origin].add(msg.value);
  }

  function unlock(uint256 value) external {
    accountTotalLockedGold[tx.origin] = accountTotalLockedGold[msg.sender].sub(value);
  }
