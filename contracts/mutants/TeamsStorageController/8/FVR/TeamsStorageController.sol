...
contract TeamsStorageController {
    ...
    function storageGetNumberOfTeams (address db) public view returns(uint) {
        return StorageInterface(db).getUint(keccak256(Storage.teams));
    }
    // SWC-104-Unchecked Call Return Value: L48
    function storageGetTeamMemberIndexByAddress (address db, uint teamId, address memberAddress) public view returns(uint) {
        uint i = 0;
        address addr;
        do {
    ...
    function storageGetTeamMemberAgreementValue (address db, uint teamId, uint memberIndex) public view returns(uint) {
        return StorageInterface(db).getUint(keccak256(Storage.teams, teamId, memberIndex, Member.agreementValue));
    }

    function storageGetTeamMemberAgreementMinutes (address db, uint teamId, uint memberIndex) public view returns(uint) {
        return StorageInterface(db).getUint(keccak256(Storage.teams, teamId, memberIndex, Member.agreementMinutes));
    }

    function storageGetTeamMemberPayoutDate (address db, uint teamId, uint memberIndex) public view returns(uint) {
        return StorageInterface(db).getUint(keccak256(Storage.teams, teamId, memberIndex, Member.payoutDate));
    }

    function storageGetTeamMemberAddress (address db, uint teamId, uint memberIndex) public view returns(address) {
        return StorageInterface(db).getAddress(keccak256(Storage.teams, teamId, memberIndex));
    }

    function storageGetNumberOfMembers (address db, uint teamId) public view returns(uint) {
        return StorageInterface(db).getUint(keccak256(Storage.teams, teamId));
    }

    function storageGetTeamMemberSingleTermAgreement (address db, uint teamId, uint memberIndex) public view returns(bool) {
