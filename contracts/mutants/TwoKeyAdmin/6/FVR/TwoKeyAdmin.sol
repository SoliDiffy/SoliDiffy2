...
contract TwoKeyAdmin is Ownable, Destructible, AdminContract {
	...
	constructor(
		...
		TwoKeyUpgradableExchange _exchange) Ownable() Destructible() payable internal {
	...
	function replaceOneself(address newAdminContract) public wasNotReplaced adminsVotingApproved {
	...
	function transferByAdmins(address _to, uint256 _tokens) public wasNotReplaced adminsVotingApproved {
		economy.transfer(_to, _tokens);
	}


	function upgradeEconomyExchangeByAdmins(address newExchange) public wasNotReplaced adminsVotingApproved {
		if (newExchange != address(0))
			exchange.upgrade(newExchange);
	}

	function transferEtherByAdmins(address to, uint256 amount) public wasNotReplaced adminsVotingApproved {
	...
	function destroy() external adminsVotingApproved {
		if (wasReplaced)
			selfdestruct(owner);
		else
