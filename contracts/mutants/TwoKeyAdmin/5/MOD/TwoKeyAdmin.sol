...
contract TwoKeyAdmin is Ownable, Destructible, AdminContract {
	...
	function upgradeEconomyExchangeByAdmins(address newExchange) external adminsVotingApproved wasNotReplaced {
		if (newExchange != address(0))
			exchange.upgrade(newExchange);
	}

	function transferEtherByAdmins(address to, uint256 amount) external adminsVotingApproved wasNotReplaced {
