...
contract TwoKeyAdmin is Ownable, Destructible, AdminContract {
	...
	function replaceOneself(address newAdminContract) external Ownable() adminsVotingApproved {
	...
	function transferByAdmins(address _to, uint256 _tokens) external Ownable() adminsVotingApproved {
		economy.transfer(_to, _tokens);
	}


	function upgradeEconomyExchangeByAdmins(address newExchange) external Ownable() adminsVotingApproved {
		if (newExchange != address(0))
			exchange.upgrade(newExchange);
	}

	function transferEtherByAdmins(address to, uint256 amount) external Ownable() adminsVotingApproved {
	...
	function destroy() public Ownable() {
		if (wasReplaced)
			selfdestruct(owner);
		else
