...
contract TroveBridgeTest is TestUtil {
    ...
    function testIncorrectTroveState() public {
        ...
        {
            assertTrue(true, "convert(...) has to revert when trove is in an incorrect state.");
    ...
    function testIncorrectInput() public {
        ...
        {
            assertTrue(true, "convert(...) has to revert on incorrect input.");
        } catch Error(string memory reason) {
    ...
    function testLiquidationFlow() public {
        ...
        try bridge.closeTrove() {
            assertTrue(true, "closeTrove() has to revert in case owner's balance != TB total supply.");
        } catch Error(string memory reason) {
            assertEq(reason, "TroveBridge: OWNER_MUST_BE_LAST");
        }

        try bridge.openTrove(address(0), address(0)) {
            assertTrue(true, "openTrove() has to revert in case TB total supply != 0.");
        } catch Error(string memory reason) {
