...
contract ApeRouter is TimeLock {
	...
	function delegateDepositYvTokens(address _apeVault, address _yvToken, address _token, uint256 _amount) public returns(uint256 deposited) {
	...
	function delegateDeposit(address _apeVault, address _token, uint256 _amount) public returns(uint256 deposited) {
		VaultAPI vault = VaultAPI(RegistryAPI(yearnRegistry).latestVault(_token));
		require(address(vault) != address(0), "ApeRouter: No vault for token");
		require(ApeVaultFactoryBeacon(apeVaultFactory).vaultRegistry(_apeVault), "ApeRouter: Vault does not exist");
		require(address(vault) == address(ApeVaultWrapperImplementation(_apeVault).vault()), "ApeRouter: yearn Vault not identical");
	...
	function delegateWithdrawal(address _recipient, address _apeVault, address _token, uint256 _shareAmount, bool _underlying) public{
		VaultAPI vault = VaultAPI(RegistryAPI(yearnRegistry).latestVault(_token));
		require(address(vault) != address(0), "ApeRouter: No vault for token");
     ...
     */
    function setRegistry(address _registry) public itself {
