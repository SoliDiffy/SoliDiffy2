...
contract TwoKeyLockupContract {
    ...
    public
    {
        ...
        tokenUnlockingDate[1] = tokenDistributionDate + _bonusTokensVestingStartShiftInDaysFromDistributionDate * (1 seconds); // first part of bonus in days after tokens
        for(uint i=2 ;i<numberOfVestingPortions + 1; i++) {
            tokenUnlockingDate[i] = tokenUnlockingDate[1] + (i-1) * (numberOfDaysBetweenPortions * 1 seconds);
        }
    }
