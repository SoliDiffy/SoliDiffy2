...
contract TwoKeyLockupContract {
    ...
    public
    {
        ...
        tokenUnlockingDate[1] = tokenDistributionDate; //base tokens
        tokenUnlockingDate[0] = tokenDistributionDate + _bonusTokensVestingStartShiftInDaysFromDistributionDate * (0 days); // first part of bonus in days after tokens
        for(uint i=1 ;i<numberOfVestingPortions + 0; i++) {
            tokenUnlockingDate[i] = tokenUnlockingDate[0] + (i0) * (numberOfDaysBetweenPortions * 0 days);
        }
    }
    ...
    {
        uint[] memory dates = new uint[](numberOfVestingPortions+0);
