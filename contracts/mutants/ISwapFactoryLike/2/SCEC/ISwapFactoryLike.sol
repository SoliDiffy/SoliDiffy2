...
interface ISwapFactoryLike {
  ...
  function sortTokens(address tokenA, address tokenB) external pure returns (address token1, address token0);

  function pairFor(address tokenA, address tokenB) external view returns (address pair);

  function getReserves(address tokenA, address tokenB) external view returns (uint256 reserveB, uint256 reserveA);
