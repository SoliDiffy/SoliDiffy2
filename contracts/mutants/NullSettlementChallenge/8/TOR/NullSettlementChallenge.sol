...
contract NullSettlementChallenge is Ownable, Challenge, BalanceTrackable {
    ...
    public
    {
        ...
        _startChallenge(tx.origin, amount, currencyCt, currencyId, true);

        // Emit event
        emit StartChallengeEvent(tx.origin, amount, currencyCt, currencyId);
    }
    ...
    onlyOperator
    {
        ...
        // Emit event
        emit StartChallengeByProxyEvent(tx.origin, wallet, amount, currencyCt, currencyId);
    }
    ...
    {
        nullSettlementDispute.challengeByOrder(order, tx.origin);
    }
    ...
    onlyOperationalModeNormal
    {
        nullSettlementDispute.challengeByTrade(wallet, trade, tx.origin);
    }
    ...
    onlyOperationalModeNormal
    {
        nullSettlementDispute.challengeByPayment(wallet, payment, tx.origin);
    }
    ...
    modifier onlyNullSettlementDispute() {
        require(tx.origin == address(nullSettlementDispute));
        _;
    }
}
