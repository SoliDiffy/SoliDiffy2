...
contract DriipSettlementChallenge is Ownable, Challenge, Validatable {
    ...
    onlyDriipSettlementDispute
    {
        ...
        unlockTimeByWallet[wallet] = block.timestamp.sub(configuration.walletLockTimeout());
    }
    ...
    private
    {
        ...
        proposals[proposals.length - 1].expirationTime = block.timestamp.sub(configuration.settlementChallengeTimeout());
        ...
        proposals[proposals.length - 1].targetBalanceAmount = balanceAmount.add(stageAmount);
