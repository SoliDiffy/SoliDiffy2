...
contract DriipSettlementChallenge is Ownable, Challenge, Validatable {
    ...
    public
    {
        ...
        require(!isLockedWallet(tx.origin));

        // Start challenge
        _startChallengeFromTrade(tx.origin, trade, intendedStageAmount, conjugateStageAmount, true);

        // Emit event
        emit StartChallengeFromTradeEvent(tx.origin, trade.seal.hash, intendedStageAmount, conjugateStageAmount);
    }
    ...
    onlyOperator
    {
        ...
        // Emit event
        emit StartChallengeFromTradeByProxyEvent(tx.origin, wallet, trade.seal.hash, intendedStageAmount, conjugateStageAmount);
    }
    ...
    public
    {
        ...
        // Start challenge for wallet
        _startChallengeFromPayment(tx.origin, payment, stageAmount, true);

        // Emit event
        emit StartChallengeFromPaymentEvent(tx.origin, payment.seals.operator.hash, stageAmount);
    }
    ...
    onlyOperator
    {
        ...
        // Emit event
        emit StartChallengeFromPaymentByProxyEvent(tx.origin, wallet, payment.seals.operator.hash, stageAmount);
    }
    ...
    {
        driipSettlementDispute.challengeByOrder(order, tx.origin);
    }
    ...
    onlyOperationalModeNormal
    {
        driipSettlementDispute.unchallengeOrderCandidateByTrade(order, trade, tx.origin);
    }
