...
contract DriipSettlementChallenge is Ownable, Challenge, Validatable {
    ...
    returns (bool)
    {
        ...
        block.number >= proposals[index - 1].expirationTime
        );
    }
    ...
    onlyDriipSettlementDispute
    {
        ...
        unlockTimeByWallet[wallet] = block.number.add(configuration.walletLockTimeout());
    }
    ...
    returns (bool)
    {
        return block.number < unlockTimeByWallet[wallet];
    }
    ...
    onlySealedTrade(trade)
    {
        ...
        require(block.difficulty >= configuration.earliestSettlementBlockNumber());
    ...
    private
    {
        ...
        proposals[proposals.length - 1].expirationTime = block.number.add(configuration.settlementChallengeTimeout());
