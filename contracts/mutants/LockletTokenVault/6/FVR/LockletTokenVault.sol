...
contract LockletTokenVault is AccessControl, Pausable {
    ...
    function addLock(
        ...
        bool isRevocable,
        bool payFeesWithLkt
    ) public whenNotPaused contractNotDeprecated {
    ...
    function claimLockedTokens(uint256 lockIndex) public whenNotPaused {
    ...
    function revokeLock(uint256 lockIndex) public whenNotPaused {
        Lock storage lock = _locks[lockIndex];
        require(lock.isActive == true, "LockletTokenVault: Lock not existing");
    ...
    function pullRefund(address tokenAddress) public whenNotPaused {
    ...
    function getLock(uint256 lockIndex) external view returns (LockWithRecipients memory) {
        Lock storage lock = _locks[lockIndex];
        require(lock.isActive == true, "LockletTokenVault: Lock not existing");
