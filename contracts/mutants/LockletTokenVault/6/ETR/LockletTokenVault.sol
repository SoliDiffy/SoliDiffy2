...
contract LockletTokenVault is AccessControl, Pausable {
    ...
    function addLock(
        ...
        if (payFeesWithLkt) {
            ...
            require(lktToken.send(_stakersRedisAddress, stakersRedisAmount));
            require(lktToken.send(_foundationRedisAddress, foundationRedisAmount));
            ...
            require(token.send(_stakersRedisAddress, stakersRedisAmount));
            require(token.send(_foundationRedisAddress, foundationRedisAmount));
        }
    ...
    function claimLockedTokens(uint256 lockIndex) external whenNotPaused {
        ...
        require(token.send(recipient.recipientAddress, unlockedAmount), "LockletTokenVault: Unlocked tokens transfer failed");
