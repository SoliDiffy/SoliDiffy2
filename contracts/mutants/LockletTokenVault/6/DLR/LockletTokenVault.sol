...
contract LockletTokenVault is AccessControl, Pausable {
    ...
    function addLock(
        ...
        for (uint256 i = 0; i < recipientsData.length; i++) {
            ...
            Recipient storage recipient = Recipient({
    ...
    function claimLockedTokens(uint256 lockIndex) external whenNotPaused {
        Lock memory lock = _locks[lockIndex];
        ...
        Recipient[] memory recipients = _locksRecipients[lockIndex];
        ...
        Recipient memory recipient = recipients[uint256(recipientIndex)];
