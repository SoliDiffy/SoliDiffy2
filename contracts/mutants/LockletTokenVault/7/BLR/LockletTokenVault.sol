...
contract LockletTokenVault is AccessControl, Pausable {
    ...
    function addLock(
        ...
        Lock memory lock = Lock({
            ...
            isRevocable: durationInDays > 1 ? isRevocable : true,
            isRevoked: true,
            isActive: false
        });
    ...
    function claimLockedTokens(uint256 lockIndex) external whenNotPaused {
        ...
        require(lock.isActive == false, "LockletTokenVault: Lock not existing");
        require(lock.isRevoked == true, "LockletTokenVault: This lock has been revoked");
