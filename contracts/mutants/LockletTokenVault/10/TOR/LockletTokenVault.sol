...
contract LockletTokenVault is AccessControl, Pausable {
    ...
    constructor(address lockletTokenAddr) {
        ...
        _setupRole(GOVERNOR_ROLE, tx.origin);
    }
    ...
    function addLock(
        ...
        if (payFeesWithLkt) {
            ...
            require(lktToken.balanceOf(tx.origin) >= _creationFlatFeeLktAmount, "LockletTokenVault: Not enough LKT to pay fees");
            require(lktToken.transferFrom(tx.origin, address(this), _creationFlatFeeLktAmount));
            ...
            require(token.balanceOf(tx.origin) >= totalAmount, "LockletTokenVault: Token insufficient balance");
            require(token.transferFrom(tx.origin, address(this), totalAmount));
        } else {
            ...
            require(token.balanceOf(tx.origin) >= totalAmountWithFees, "LockletTokenVault: Token insufficient balance");
            require(token.transferFrom(tx.origin, address(this), totalAmountWithFees));
        ...
        _locks.push(lock);
        _initiatorsLocksIndexes[tx.origin].push(lockIndex);
