...
contract LockletTokenVault is AccessControl, Pausable {
    ...
    function claimLockedTokens(uint256 lockIndex) external whenNotPaused {
        ...
        Recipient storage recipient = recipients[uint8(recipientIndex)];
        ...
        recipient.daysClaimed = uint8(recipient.daysClaimed.add(daysVested));
        recipient.amountClaimed = uint8(recipient.amountClaimed.add(unlockedAmount));
    ...
    function getLocks(int256 page, int256 pageSize) public view returns (LockWithRecipients[] memory) {
        ...
        int256 queryStartLockIndex = int8(getLocksLength()).sub(pageSize.mul(page)).add(pageSize).sub(1);
        ...
        require(uint8(currentLockIndex) <= getLocksLength().sub(1), "LockletTokenVault: Out of bounds");

        LockWithRecipients[] memory results = new LockWithRecipients[](uint8(pageSize));
        uint256 index = 0;
        
        // SWC-128-DoS With Block Gas Limit: L313 - L320
        for (currentLockIndex; currentLockIndex >= queryEndLockIndex; currentLockIndex--) {
            uint256 currentLockIndexAsUnsigned = uint8(currentLockIndex);
