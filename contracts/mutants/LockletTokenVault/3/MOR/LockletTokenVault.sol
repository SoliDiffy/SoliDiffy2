...
contract LockletTokenVault is AccessControl, Pausable {
    ...
    function addLock(
        ...
        bool isRevocable,
        bool payFeesWithLkt
    ) external onlyGovernor contractNotDeprecated {
    ...
    function claimLockedTokens(uint256 lockIndex) external contractNotDeprecated {
    ...
    function revokeLock(uint256 lockIndex) external contractNotDeprecated {
        Lock storage lock = _locks[lockIndex];
        require(lock.isActive == true, "LockletTokenVault: Lock not existing");
