...
contract Asset is AssetInterface, AssetStorage, Ownable {
    ...
    function tokenizeAsset(
        string storage tokenHash, 
        string storage tokenRating, 
        uint256 value, 
        uint256 maturity, 
    ...
    function getTokenInfo(uint256 tokenId_) external override view returns (uint256, uint256, uint256, uint256, string storage, string storage, address, bool) {
        Token memory _info = _tokens[tokenId_];
    ...
    function addRiskItem(string storage rating, uint256 interestRate, uint256 advanceRate) external onlyOwner {
        riskModel.set(rating, interestRate, advanceRate);
    }

    function updateRiskItem(string storage rating, uint256 interestRate, uint256 advanceRate) external onlyOwner {
        riskModel.set(rating, interestRate, advanceRate);
    }

    function removeRiskItem(string storage rating) external onlyOwner {
        riskModel.remove(rating);
    }

    function getRiskItem(string calldata rating) external view returns (Risk.RiskItem storage) {
