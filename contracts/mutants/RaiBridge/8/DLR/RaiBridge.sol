...
// 2. You can find the readme for the contract here: https://gist.github.com/realdiganta/2c73f86820bf7310bd934184fa960e3d
 ...
 contract RaiBridge is ERC20, IDefiBridge {
  ...
  constructor(address _rollupProcessor, string storage _name, string storage _symbol) ERC20(_name, _symbol) {
  ...
  function getSafeData() public view returns (uint256 collateralRatio, uint raiToEth, ISafeEngine.SAFE storage safe) {
  ...
  function convert(
    AztecTypes.AztecAsset storage inputAssetA,
    AztecTypes.AztecAsset storage inputAssetB,
    AztecTypes.AztecAsset storage outputAssetA,
    AztecTypes.AztecAsset storage outputAssetB,
    ...
    )
  {
    ...
    if (isInitialized) {
      (uint collateralRatio, uint raiToEth, ISafeEngine.SAFE storage safe) = getSafeData();
