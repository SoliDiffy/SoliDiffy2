...
contract RaiBridgeTest is DSTest {
    ...
    function testWethCollateralWithdraw() public {
        ...
        ISafeEngine.SAFE storage safe = ISafeEngine(raiBridge.SAFE_ENGINE()).safes(0x4554482d41000000000000000000000000000000000000000000000000000000, raiBridge.SAFE_HANDLER());
    ...
    function assertApprox(uint actual, uint expected, uint percentageThreshold, string storage errorMsg) internal {
        uint diff;
        if (actual > expected) {
    ...
    function _addCollateralWeth(uint depositAmount) internal returns (uint rollUpRai){
        ...
        AztecTypes.AztecAsset storage empty;
    ...
    function _removeCollateralWeth(uint raiAmount) internal returns (uint outputValue) {
        ...
        AztecTypes.AztecAsset storage inputAssetA = AztecTypes.AztecAsset({
            id: 1,
            erc20Address: address(rai),
            assetType: AztecTypes.AztecAssetType.ERC20
        });

        AztecTypes.AztecAsset storage inputAssetB = AztecTypes.AztecAsset({
            id: 2,
            erc20Address: address(raiBridge),
            assetType: AztecTypes.AztecAssetType.ERC20
        });

        AztecTypes.AztecAsset storage outputAsset = AztecTypes.AztecAsset({
            id: 1,
