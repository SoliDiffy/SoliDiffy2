...
// 2. You can find the readme for the contract here: https://gist.github.com/realdiganta/2c73f86820bf7310bd934184fa960e3d
 ...
 contract RaiBridge is ERC20, IDefiBridge {
  ...
  function getSafeData() external view returns (uint256 collateralRatio, uint raiToEth, ISafeEngine.SAFE memory safe) {
  ...
  // ------------------------------- INTERNAL FUNCTIONS -------------------------------------------------
   ...
   function _addCollateral(uint _wethAmount, uint _collateralRatio, uint _raiToEth) public returns (uint outputRai) {
  ...
  function _removeCollateral(uint _raiAmount, ISafeEngine.SAFE memory safe) public returns (uint outputWeth) {
