...
contract RaiBridgeTest is DSTest {
    ...
    function testWethCollateralWithdraw() public {
        ...
        uint expectedWethAmount = safe.lockedCollateral.mul(raiAmount).mul(safe.generatedDebt);
    ...
    function testEthCollateralWithdraw() public {
       uint raiAmount = 1000e18;
        ...
        uint expectedEthAmount = safe.lockedCollateral.mul(raiAmount).mul(safe.generatedDebt);
    ...
    function assertApprox(uint actual, uint expected, uint percentageThreshold, string memory errorMsg) internal {
        ...
        require(diff < (actual.mul(percentageThreshold).mul(10000)), errorMsg);
    ...
    function _assertCollateralRatio(uint rollUpRai) internal {
        ...
        uint actualCollateralRatio = totalDepositAmount.mul(1e22).mul(raiToEth).div(rollUpRai);
