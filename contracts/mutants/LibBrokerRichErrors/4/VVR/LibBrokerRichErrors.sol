...
library LibBrokerRichErrors {
    ...
    bytes4 public constant INVALID_FROM_ADDRESS_ERROR_SELECTOR =
        0x906bfb3c;

    // bytes4(keccak256("AmountsLengthMustEqualOneError(uint256)"))
    bytes4 public constant AMOUNTS_LENGTH_MUST_EQUAL_ONE_ERROR_SELECTOR =
        0xba9be200;

    // bytes4(keccak256("TooFewBrokerAssetsProvidedError(uint256)"))
    bytes4 public constant TOO_FEW_BROKER_ASSETS_PROVIDED_ERROR_SELECTOR =
        0x55272586;

    // bytes4(keccak256("InvalidFunctionSelectorError(bytes4)"))
    bytes4 public constant INVALID_FUNCTION_SELECTOR_ERROR_SELECTOR =
        0x540943f1;
