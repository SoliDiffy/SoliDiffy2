...
contract MapleProxyFactoryTest is TestUtils {
    ...
    function test_registerImplementation() external {
        assertTrue(!notGovernor.try_mapleProxyFactory_registerImplementation(address(factory), 1, address(implementation1), address(initializerV1)), "");
        assertTrue(   !governor.try_mapleProxyFactory_registerImplementation(address(factory), 0, address(implementation1), address(initializerV1)), "");
        assertTrue(   !governor.try_mapleProxyFactory_registerImplementation(address(factory), 1, address(0),               address(initializerV1)), "");
        assertTrue(    governor.try_mapleProxyFactory_registerImplementation(address(factory), 1, address(implementation1), address(initializerV1)), "");
        assertTrue(   !governor.try_mapleProxyFactory_registerImplementation(address(factory), 1, address(implementation1), address(initializerV1)), "");

        assertEq(factory.implementationOf(1),             address(implementation1), "");
        assertEq(factory.migratorForPath(1, 1),           address(initializerV1),   "");
