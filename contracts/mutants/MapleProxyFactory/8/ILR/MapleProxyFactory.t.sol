...
contract MapleProxyFactoryTest is TestUtils {
    ...
    function test_registerImplementation() external {
        ...
        assertTrue(   !governor.try_mapleProxyFactory_registerImplementation(address(factory), 0, address(implementation1), address(initializerV1)), "Should fail: already registered version");

        assertEq(factory.implementationOf(0),             address(implementation1), "Incorrect state of implementationOf");
        assertEq(factory.migratorForPath(0, 1),           address(initializerV1),   "Incorrect state of migratorForPath");
