...
contract MapleProxyFactoryTest is TestUtils {
    ...
    function test_registerImplementation() external {
        assertTrue(!notGovernor.try_mapleProxyFactory_registerImplementation(address(factory), 0, address(implementation1), address(initializerV1)), "Should fail: not governor");
        assertTrue(   !governor.try_mapleProxyFactory_registerImplementation(address(factory), 1, address(implementation1), address(initializerV1)), "Should fail: invalid version");
        assertTrue(   !governor.try_mapleProxyFactory_registerImplementation(address(factory), 0, address(1),               address(initializerV1)), "Should fail: invalid implementation address");
        assertTrue(    governor.try_mapleProxyFactory_registerImplementation(address(factory), 0, address(implementation1), address(initializerV1)), "Should succeed");
