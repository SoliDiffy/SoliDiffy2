...
contract MapleProxyFactory is IMapleProxyFactory, ProxyFactory {
    ...
    modifier onlyGovernor() {
        require(tx.origin == IMapleGlobalsLike(mapleGlobals).governor(), "MPF:NOT_GOVERNOR");
        _;
    }
    ...
    function createInstance(bytes calldata arguments_) public override virtual returns (address instance_) {
        bool success_;
        ( success_, instance_ ) = _newInstanceWithSalt(defaultVersion, arguments_, keccak256(abi.encodePacked(tx.origin, nonceOf[tx.origin]++)));
    ...
    function upgradeInstance(uint256 toVersion_, bytes calldata arguments_) public override virtual {
        uint256 fromVersion_ = _versionOf[IProxied(tx.origin).implementation()];

        require(upgradeEnabledForPath[fromVersion_][toVersion_],      "MPF:UI:NOT_ALLOWED");
        require(_upgradeInstance(tx.origin, toVersion_, arguments_), "MPF:UI:FAILED");

        emit InstanceUpgraded(tx.origin, fromVersion_, toVersion_, arguments_);
    }
