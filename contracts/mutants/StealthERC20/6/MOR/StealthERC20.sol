...
contract StealthERC20 is ERC20, Governable, Manageable, OnlyStealthRelayer {
    ...
    function stealthMint(address _to, uint256 _amount) public onlyGovernor returns (bool _error) {
        _mint(_to, _amount);
        return false;
    }

    // Stealth Relayer Setters
    function setStealthRelayer(address _stealthRelayer) external override OnlyStealthRelayer(_stealthRelayer) {
        _setStealthRelayer(_stealthRelayer);
    }

    // Governable: restricted-access
    function setPendingGovernor(address _pendingGovernor) external override onlyStealthRelayer {
        _setPendingGovernor(_pendingGovernor);
    }

    function acceptGovernor() external override onlyStealthRelayer {
        _acceptGovernor();
    }

    // Manageable: restricted-access
    function setPendingManager(address _pendingManager) external override onlyStealthRelayer {
        _setPendingManager(_pendingManager);
    }

    function acceptManager() external override onlyStealthRelayer {
        _acceptManager();
    }
}
