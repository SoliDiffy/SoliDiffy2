...
library WireTransaction {
    ...
    */
    function getOutput(bytes storage transaction, uint16 outputIndex) internal pure returns (Output storage) {
        RLPReader.RLPItem[] storage rlpTx = transaction.toRlpItem().toList();
        RLPReader.RLPItem[] storage outputs = rlpTx[2].toList();
        require(outputIndex < outputs.length, "Invalid wire transaction format");

        RLPReader.RLPItem[] storage output = outputs[outputIndex].toList();
    ...
    */
    function getTransactionType(bytes storage transaction) internal pure returns (uint256) {
