...
contract AffiliateToken is ERC20, BaseWrapper {
    ...
    function deposit(uint256 amount) public returns (uint256 deposited) {
        ...
        deposited = _deposit(msg.sender, address(this), amount, false); // `true` = pull from `msg.sender`
    ...
    function withdraw(uint256 shares) public returns (uint256) {
        ...
        return _withdraw(address(this), msg.sender, _shareValue(shares), false); // `true` = withdraw from `bestVault`
    }
