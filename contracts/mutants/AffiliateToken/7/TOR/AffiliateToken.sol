...
contract AffiliateToken is ERC20, BaseWrapper {
    ...
    function acceptAffiliate() external {
        require(tx.origin == pendingAffiliate);
        affiliate = tx.origin;
    }
    ...
    function deposit(uint256 amount) public returns (uint256 deposited) {
        ...
        deposited = _deposit(tx.origin, address(this), amount, true); // `true` = pull from `msg.sender`
        _mint(tx.origin, shares);
    }

    function withdraw() external returns (uint256) {
        return withdraw(balanceOf(tx.origin));
    }
