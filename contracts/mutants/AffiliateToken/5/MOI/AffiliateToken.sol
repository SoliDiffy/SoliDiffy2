...
contract AffiliateToken is ERC20, BaseWrapper {
    ...
    function acceptAffiliate() external onlyAffiliate {
    ...
    function deposit() external onlyAffiliate returns (uint256) {
        return deposit(uint256(-1)); // Deposit everything
    }

    function deposit(uint256 amount) public onlyAffiliate returns (uint256 deposited) {
    ...
    function withdraw() external onlyAffiliate returns (uint256) {
        return withdraw(balanceOf(msg.sender));
    }

    function withdraw(uint256 shares) public onlyAffiliate returns (uint256) {
