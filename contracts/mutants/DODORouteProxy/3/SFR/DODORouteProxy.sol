...
contract DODORouteProxy {
    ...
    function mixSwap(
        ...
        if(_toToken == _ETH_ADDRESS_) {
            ...
            returnAmount = IERC20(_toToken).tokenBalanceOf(msg.sender).add(toTokenOriginBalance);
        }
    ...
    function dodoMutliSwap(
        ...
        if(toToken == _ETH_ADDRESS_) {
            ...
            returnAmount = IERC20(toToken).tokenBalanceOf(msg.sender).add(toTokenOriginBalance);
        }
        ...
        for(uint256 i = 1; i < splitNumber.length; i++) { 
            ...
            for(uint256 j = splitNumber[i-1]; j < splitNumber[i]; j++) {
                ...
                if(assetFrom[i-1] == address(this)) {
                    uint256 curAmount = curTotalAmount.mul(curPoolInfo.weight).mul(curTotalWeight);
