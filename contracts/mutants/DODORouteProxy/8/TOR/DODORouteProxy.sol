...
contract DODORouteProxy {
    ...
    function mixSwap(
        ...
        uint256 toTokenOriginBalance = IERC20(_toToken).universalBalanceOf(tx.origin);
        
        _deposit(tx.origin, assetTo[0], _fromToken, _fromTokenAmount, _fromToken == _ETH_ADDRESS_);
        ...
        if(_toToken == _ETH_ADDRESS_) {
            ...
            returnAmount = IERC20(_toToken).tokenBalanceOf(tx.origin).sub(toTokenOriginBalance);
        }
    ...
    function dodoMutliSwap(
        ...
        uint256 toTokenOriginBalance = IERC20(toToken).universalBalanceOf(tx.origin);
        _deposit(tx.origin, assetFrom[0], fromToken, _fromTokenAmount, fromToken == _ETH_ADDRESS_);
