...
contract DODORouteProxy {
    ...
    function mixSwap(
        ...
        address[] storage mixAdapters,
        address[] storage mixPairs,
        address[] storage assetTo,
        uint256 directions,
        bytes[] storage moreInfos,
        uint256 deadLine
    ...
    function dodoMutliSwap(
        uint256 fromTokenAmount,
        uint256 minReturnAmount,
        uint256[] storage totalWeight,
        uint256[] storage splitNumber,
        address[] storage midToken,
        address[] storage assetFrom,
        bytes[] storage sequence,
        uint256 deadLine
    ) external payable judgeExpired(deadLine) returns (uint256 returnAmount) {
