...
contract FraxUnifiedFarm_ERC20 is FraxUnifiedFarmTemplate {
    ...
    function lockedStakesOf(address account) public view returns (LockedStake[] memory) {
        return lockedStakes[account];
    }

    // All the locked stakes for a given account [old-school method]
    function lockedStakesOfMultiArr(address account) public view returns (
    ...
    function lockedStakesOfLength(address account) public view returns (uint256) {
    ...
    function _getStake(address staker_address, bytes32 kek_id) public view returns (LockedStake memory locked_stake, uint256 arr_idx) {
    ...
    function extendLockTime(bytes32 kek_id, uint256 new_ending_ts) updateRewardAndBalance(msg.sender, true) external {
    ...
    function lockAdditional(bytes32 kek_id, uint256 addl_liq) updateRewardAndBalance(msg.sender, true) external {
        // Get the stake and its index
        (LockedStake memory thisStake, uint256 theArrayIndex) = _getStake(msg.sender, kek_id);
    ...
    function _stakeLocked(
        ...
        uint256 secs,
        uint256 start_timestamp
    ) public updateRewardAndBalance(staker_address, true) {
