...
contract FraxUnifiedFarm_ERC20 is FraxUnifiedFarmTemplate {
    ...
    function extendLockTime(bytes32 kek_id, uint256 new_ending_ts) updateRewardAndBalance(tx.origin, true) public {
        // Get the stake and its index
        (LockedStake memory thisStake, uint256 theArrayIndex) = _getStake(tx.origin, kek_id);
        ...
        // Update the stake
        lockedStakes[tx.origin][theArrayIndex] = LockedStake(
            kek_id,
            block.timestamp,
            thisStake.liquidity,
            new_ending_ts,
        ...
        _updateRewardAndBalance(tx.origin, false);
    }

    // Add additional LPs to an existing locked stake
    function lockAdditional(bytes32 kek_id, uint256 addl_liq) updateRewardAndBalance(tx.origin, true) public {
        // Get the stake and its index
        (LockedStake memory thisStake, uint256 theArrayIndex) = _getStake(tx.origin, kek_id);
        ...
        TransferHelper.safeTransferFrom(address(stakingToken), tx.origin, address(this), addl_liq);
    ...
    function stakeLocked(uint256 liquidity, uint256 secs) nonReentrant external {
        _stakeLocked(tx.origin, msg.sender, liquidity, secs, block.timestamp);
    }
