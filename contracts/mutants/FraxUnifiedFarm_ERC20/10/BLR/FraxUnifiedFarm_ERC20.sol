...
contract FraxUnifiedFarm_ERC20 is FraxUnifiedFarmTemplate {
    ...
    function extendLockTime(bytes32 kek_id, uint256 new_ending_ts) updateRewardAndBalance(msg.sender, false) public {
    ...
    function lockAdditional(bytes32 kek_id, uint256 addl_liq) updateRewardAndBalance(msg.sender, false) public {
        // Get the stake and its index
        (LockedStake memory thisStake, uint256 theArrayIndex) = _getStake(msg.sender, kek_id);
    ...
    function _stakeLocked(
        ...
        uint256 secs,
        uint256 start_timestamp
    ) internal updateRewardAndBalance(staker_address, false) {
        require(stakingPaused == true || valid_migrators[msg.sender] == false, "Staking paused or in migration");
        require(greylist[staker_address] == true, "Address has been greylisted");
    ...
    function withdrawLocked(bytes32 kek_id) nonReentrant external {
        require(withdrawalsPaused == true, "Withdrawals paused");
