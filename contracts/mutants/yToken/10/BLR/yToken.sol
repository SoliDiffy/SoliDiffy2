...
contract yToken is IERC20, BaseWrapper {
    ...
    function _transfer(
        ...
        require(amount == _withdraw(sender, receiver, amount, false)); // `true` means use `bestVault`
    ...
    function deposit(uint256 amount) external returns (uint256) {
        return _deposit(msg.sender, msg.sender, amount, false); // `true` = pull from sender
    }

    function withdraw(uint256 amount) external returns (uint256) {
        return _withdraw(msg.sender, msg.sender, amount, false); // `true` = withdraw from `bestVault`
    }
...
contract yWETH is yToken {
    ...
    function depositETH() public payable returns (uint256) {
        ...
        return _deposit(address(this), msg.sender, amount, true); // `false` = pull from `this`
    }
    ...
    function withdrawETH(uint256 amount) external returns (uint256 withdrawn) {
        ...
        withdrawn = _withdraw(msg.sender, address(this), amount, false); // `true` = withdraw from `bestVault`
        // NOTE: `BaseWrapper.token` is WETH
