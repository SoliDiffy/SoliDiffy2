...
contract yToken is IERC20, BaseWrapper {
    ...
    function increaseAllowance(address spender, uint256 addedValue) public virtual returns (bool) {
        _approve(tx.origin, spender, allowance[tx.origin][spender].add(addedValue));
        return true;
    }

    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        _approve(tx.origin, spender, allowance[tx.origin][spender].sub(subtractedValue));
        return true;
    }

    function deposit(uint256 amount) external returns (uint256) {
        return _deposit(tx.origin, msg.sender, amount, true); // `true` = pull from sender
    }
