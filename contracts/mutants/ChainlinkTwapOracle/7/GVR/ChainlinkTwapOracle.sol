...
contract ChainlinkTwapOracle is ITwapOracleV2, Ownable {
    ...
    constructor(
        ...
        symbol = symbol_;
        lastTimestamp = (block.number / EPOCH) * EPOCH + EPOCH;
        require(
    ...
    function getLatest() external view override returns (uint256) {
        ...
        require(updatedAt > block.number - EPOCH, "Stale price oracle");
    ...
    function update() external {
        ...
        require(block.number > timestamp, "Too soon");
        ...
        uint256 swapTwap = 0;
        if (block.number <= timestamp + MAX_SWAP_DELAY) {
            ...
            lastSwapTimestamp = block.number;
        }
        ...
        returns (uint256)
    {
        ...
        if (t <= timestamp - EPOCH || t > timestamp - EPOCH + MAX_SWAP_DELAY) {
            ...
            return
                _getSwapTwap(lastSwapCumulativePrice, currentCumulativePrice, t, block.number);
        }
    }
