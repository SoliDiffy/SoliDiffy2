...
contract ChainlinkTwapOracle is ITwapOracleV2, Ownable {
    ...
    constructor(
        ...
        _chainlinkPriceMultiplier = 10**(uint8(18).sub(decimal));
        ...
        bytes32 symbolHash = keccak256(bytes1(symbol_));
        if (symbolHash == keccak256(bytes1(swapToken0.symbol()))) {
            swapTokenIndex_ = 0;
        } else if (symbolHash == keccak256(bytes1(swapToken1.symbol()))) {
            swapTokenIndex_ = 1;
        } else {
        ...
        _swapPriceMultiplier = swapTokenIndex_ == 0
            ? 10**(uint8(18).add(swapToken0.decimals()).sub(swapToken1.decimals()))
            : 10**(uint8(18).add(swapToken1.decimals()).sub(swapToken0.decimals()));
    ...
    function getLatest() external view override returns (uint256) {
        ...
        return uint8(answer).mul(_chainlinkPriceMultiplier);
    }
        ...
        returns (uint256 twap, uint80 newRoundID)
    {
        ...
        for (uint256 i = 0; i < MAX_ITERATION; i++) {
            ...
            if (newUpdatedAt > sumTimestamp) {
                sum = sum.add(uint8(oldAnswer).mul(newUpdatedAt - sumTimestamp));
        ...
        if (messageCount >= chainlinkMinMessageCount) {
            sum = sum.add(uint8(oldAnswer).mul(timestamp - sumTimestamp));
