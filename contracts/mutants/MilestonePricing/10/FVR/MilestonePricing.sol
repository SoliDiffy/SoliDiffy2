...
contract MilestonePricing is PricingStrategy, Ownable {
  ...
  */
  ...
  function getMilestone(uint n) external constant returns (uint, uint) {
    return (milestones[n].time, milestones[n].price);
  }

  function getFirstMilestone() public constant returns (Milestone) {
    return milestones[0];
  }

  function getLastMilestone() public constant returns (Milestone) {
    return milestones[milestoneCount-1];
  }

  function getPricingStartsAt() external constant returns (uint) {
    return getFirstMilestone().time;
  }

  function getPricingEndsAt() external constant returns (uint) {
    return getLastMilestone().time;
  }

  function isSane(address _crowdsale) external constant returns(bool) {
  ...
  function getCurrentMilestone() public constant returns (Milestone) {
    uint i;
  ...
  function getCurrentPrice() external constant returns (uint result) {
    return getCurrentMilestone().price;
  }

  /// @dev Calculate the current price for buy in amount.
  function calculatePrice(uint value, uint weiRaised, uint tokensSold, address msgSender, uint decimals) external constant returns (uint) {
