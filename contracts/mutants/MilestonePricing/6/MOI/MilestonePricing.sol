...
contract MilestonePricing is PricingStrategy, Ownable {
  ...
  */
  ...
  function getMilestone(uint n) public constant onlyOwner returns (uint, uint) {
    return (milestones[n].time, milestones[n].price);
  }

  function getFirstMilestone() private constant onlyOwner returns (Milestone) {
    return milestones[0];
  }

  function getLastMilestone() private constant onlyOwner returns (Milestone) {
