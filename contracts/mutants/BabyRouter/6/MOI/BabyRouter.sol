...
contract BabyRouter is IBabyRouter02, Ownable {
    ...
    function _addLiquidity(
        ...
        uint amountAMin,
        uint amountBMin
    ) internal virtual onlyOwner returns (uint amountA, uint amountB) {
    ...
    ) external virtual override onlyOwner returns (uint amountA, uint amountB) {
        address pair = BabyLibrary.pairFor(factory, tokenA, tokenB);
    ...
    function removeLiquidityETHWithPermit(
        ...
        uint deadline,
        bool approveMax, uint8 v, bytes32 r, bytes32 s
    ) external virtual override onlyOwner returns (uint amountToken, uint amountETH) {
        address pair = BabyLibrary.pairFor(factory, token, WETH);
        uint value = approveMax ? uint(-1) : liquidity;
        IBabyPair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    ...
    function removeLiquidityETHWithPermitSupportingFeeOnTransferTokens(
        ...
        uint deadline,
        bool approveMax, uint8 v, bytes32 r, bytes32 s
    ) external virtual override onlyOwner returns (uint amountETH) {
        address pair = BabyLibrary.pairFor(factory, token, WETH);
        uint value = approveMax ? uint(-1) : liquidity;
        IBabyPair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual onlyOwner {
    ...
    // requires the initial amount to have already been sent to the first pair
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual onlyOwner {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = BabyLibrary.sortTokens(input, output);
