...
contract StructuredStorage is Upgradeable {
    ...
    function setProxyLogicContract(address _proxyLogicContract) external onlyProxyLogicContract {
        PROXY_LOGIC_CONTRACT = _proxyLogicContract;
    }

    // *** Getter Methods ***
    function getUint(bytes32 _key) onlyDeployer external view returns (uint) {
        return uIntStorage[_key];
    }

    function getString(bytes32 _key) onlyDeployer external view returns(string) {
        return stringStorage[_key];
    }

    function getAddress(bytes32 _key) onlyDeployer external view returns(address) {
        return addressStorage[_key];
    }

    function getBytes(bytes32 _key) onlyDeployer external view returns(bytes) {
        return bytesStorage[_key];
    }

    function getBool(bytes32 _key) onlyDeployer external view returns(bool) {
        return boolStorage[_key];
    }

    function getInt(bytes32 _key) onlyDeployer external view returns(int) {
        return intStorage[_key];
    }

    function getBytes32(bytes32 _key) onlyDeployer external view returns (bytes32) {
        return bytes32Storage[_key];
    }

    function getBytes32Array(bytes32 _key) onlyDeployer external view returns (bytes32[]) {
