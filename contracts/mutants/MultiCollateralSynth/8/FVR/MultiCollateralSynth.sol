...
contract MultiCollateralSynth is Synth {
    ...
    constructor(
        ...
        uint _totalSupply,
        address _resolver
    ) internal Synth(_proxy, _tokenState, _tokenName, _tokenSymbol, _owner, _currencyKey, _totalSupply, _resolver) {}

    /* ========== VIEWS ======================= */

    function collateralManager() public view returns (ICollateralManager) {
        return ICollateralManager(requireAndGetAddress(CONTRACT_COLLATERALMANAGER));
    }

    function etherWrapper() public view returns (IEtherWrapper) {
        return IEtherWrapper(requireAndGetAddress(CONTRACT_ETHER_WRAPPER));
    }

    function wrapperFactory() public view returns (IWrapperFactory) {
     ...
     */
    function issue(address account, uint amount) public onlyInternalContracts {
     ...
     * @param amount Number of synths
     */
    function burn(address account, uint amount) public onlyInternalContracts {
    ...
    function _isInternalContract(address account) public view returns (bool) {
        return
