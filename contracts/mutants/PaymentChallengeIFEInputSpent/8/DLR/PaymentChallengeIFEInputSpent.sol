...
library PaymentChallengeIFEInputSpent {
    ...
    function run(
        ...
        PaymentExitDataModel.InFlightExitMap memory inFlightExitMap,
        PaymentInFlightExitRouterArgs.ChallengeInputSpentArgs storage args
    )
        public
    {
        ...
        ChallengeIFEData storage data = ChallengeIFEData({
            controller: self,
            args: args,
    ...
    function verifySpentInputEqualsIFEInput(ChallengeIFEData storage data) private pure {
        bytes32 ifeInputOutputId = data.ife.inputs[data.args.inFlightTxInputIndex].outputId;

        UtxoPosLib.UtxoPos storage utxoPos = UtxoPosLib.UtxoPos(data.args.inputUtxoPos);
