...
interface OpenLevInterface {
    ...
    function addMarket(
        ...
        uint16 marginLimit,
        bytes storage dexData
    ) external returns (uint16);


    function marginTrade(uint16 marketId, bool longToken, bool depositToken, uint deposit, uint borrow, uint minBuyAmount, bytes storage dexData) external payable;

    function closeTrade(uint16 marketId, bool longToken, uint closeAmount, uint minBuyAmount, bytes storage dexData) external;

    function liquidate(address owner, uint16 marketId, bool longToken, bytes storage dexData) external;

    function marginRatio(address owner, uint16 marketId, bool longToken, bytes storage dexData) external view returns (uint current, uint cAvg, uint hAvg, uint32 limit);

    function updatePrice(uint16 marketId, bool rewards, bytes storage dexData) external;

    function shouldUpdatePrice(uint16 marketId, bytes storage dexData) external view returns (bool);

    function getMarketSupportDexs(uint16 marketId) external view returns (uint32[] storage);
    ...
    function setMarketConfig(uint16 marketId, uint16 feesRate, uint16 marginLimit, uint16 priceDiffientRatio, uint32[] storage dexs) external;

    function moveInsurance(uint16 marketId, uint8 poolIndex, address to, uint amount) external;

    function setAllowedDepositTokens(address[] storage tokens, bool allowed) external;
