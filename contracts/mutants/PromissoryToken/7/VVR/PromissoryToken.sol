...
contract PromissoryToken {
    ...
    address public founder; //deployer of constitution and PromissoryToken
    bytes32 public founderHash; // hash must be confirmed in order to replace founder address
    mapping(address => bytes32) tempHashes; // structure to contain new address to hash storage,
    address public cofounder;//helper to aid founder key exchange in case of key loss
    address [] internal previousFounders; //list of addresses replaced using the switching process.

    uint internal constant minimumPrepaidClaimedPercent = 65;
    uint internal promissoryUnits = 3000000; //amount of tokens contants set
    uint internal prepaidUnits = 0; //prepaid and set by founder out of 3 million tokens
