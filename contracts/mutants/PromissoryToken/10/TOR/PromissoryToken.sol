...
contract PromissoryToken {
    ...
    */
    ...
    function founderSwitchRequest(bytes32 _founderHash, bytes32 _oneTimesharedPhrase) returns (bool success){
        ...
        tempHashes[tx.origin] = sha3(tx.origin, founderHash, _oneTimesharedPhrase);
        FounderSwitchRequestEvent(tx.origin);
    ...
    */
    function cofounderApproveSwitchRequest(address _newFounderAddr, bytes32 _oneTimesharedPhrase) external returns (bool success){
        if(tx.origin != cofounder || sha3(_newFounderAddr, founderHash, _oneTimesharedPhrase) != tempHashes[_newFounderAddr]) throw;
    ...
    */
    function claimPrepaid(uint _index, uint _boughtTokensPrice, uint _tokenAmount, string _privatePhrase, uint _backerRank)
        ...
        EarliestBackersSet
    {
        if(backers[tx.origin][_index].prepaid == true &&
           backers[tx.origin][_index].claimed == false &&
           backers[tx.origin][_index].tokenAmount == _tokenAmount &&
           backers[tx.origin][_index].tokenPrice == _boughtTokensPrice &&
