...
contract TokenAccessControl is Ownable {
	...
	function changePauseStatus(bool _status) public onlyOwner {
		require(!foreverUnpaused, "AccessControl: Contract is unpaused forever");
		paused = _status;
	} 


	function disablePausingForever() public onlyOwner {
		require(!foreverUnpaused, "AccessControl: Contract is unpaused forever");
		foreverUnpaused = true;
		paused = false;
	}

	function addMinters(address[] calldata _minters) public onlyOwner {
		require(!mintingDisabled, "AccessControl: Contract cannot mint tokens anymore");
	...
	function removeMinters(address[] calldata _minters) public onlyOwner {
		require(!mintingDisabled, "AccessControl: Contract cannot mint tokens anymore");
	...
	function addWhitelistedAddresses(address[] calldata _addresses) public onlyOwner {
		require(!whitelistDisabled, "AccessControl: Whitelist already disabled");
	...
	function removeWhitelistedAddresses(address[] calldata _addresses) public onlyOwner {
		require(!whitelistDisabled, "AccessControl: Whitelist already disabled");
