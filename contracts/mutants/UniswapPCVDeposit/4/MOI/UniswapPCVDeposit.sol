...
contract UniswapPCVDeposit is IUniswapPCVDeposit, PCVDeposit, UniRef {
    ...
    function _addLiquidity(uint256 tokenAmount, uint256 feiAmount) internal whenNotPaused {
    ...
    function _approveToken(address _token) internal whenNotPaused {
        uint256 maxTokens = type(uint256).max;
        IERC20(_token).approve(address(router), maxTokens);
    }

    // Wrap all held ETH
    function _wrap() internal whenNotPaused {
