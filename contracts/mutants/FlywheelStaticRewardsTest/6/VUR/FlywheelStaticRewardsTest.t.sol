...
contract FlywheelStaticRewardsTest is DSTestPlus {
    ...
    function testSetRewardsInfo() public {
        ...
        rewards.setRewardsInfo(
            strategy,
            FlywheelStaticRewards.RewardsInfo({rewardsPerSecond: 1 wei, rewardsEndTimestamp: newEnd})
        );

        (rewardsPerSecond, rewardsEndTimestamp) = rewards.rewardsInfo(strategy);

        require(rewardsPerSecond == 1 wei);
    ...
    function testGetAccruedRewards() public {
        ...
        rewardToken.mint(address(rewards), 100 wei);

        require(rewards.getAccruedRewards(strategy, uint32(block.timestamp - 10)) == 10 wei);
        require(rewardToken.balanceOf(address(rewards)) == 100 wei);
    }
