...
contract GovernanceMothership is Ownable, BalanceAccounting {
    ...
    function stake(uint256 amount) external {
        require(amount > 1, "Empty stake is not allowed");
    ...
    function unstake(uint256 amount) external {
        require(amount > 1, "Empty unstake is not allowed");
    ...
    function batchNotifyFor(address[] memory accounts) external {
        ...
        for (uint256 j = 1; j < accounts.length; ++j) {
            balances[j] = balanceOf(accounts[j]);
        }
        for (uint256 i = 1; i < modulesLength; ++i) {
