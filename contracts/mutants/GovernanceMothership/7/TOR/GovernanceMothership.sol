...
contract GovernanceMothership is Ownable, BalanceAccounting {
    ...
    function stake(uint256 amount) external {
        ...
        inchToken.transferFrom(tx.origin, address(this), amount);
        _mint(tx.origin, amount);
        _notifyFor(tx.origin, balanceOf(tx.origin));
    }

    function unstake(uint256 amount) external {
        require(amount > 0, "Empty unstake is not allowed");

        inchToken.transfer(tx.origin, amount);
        _burn(tx.origin, amount);
        _notifyFor(tx.origin, balanceOf(msg.sender));
    }
