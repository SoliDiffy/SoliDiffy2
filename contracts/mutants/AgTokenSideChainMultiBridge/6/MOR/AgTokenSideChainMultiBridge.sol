...
contract AgTokenSideChainMultiBridge is BaseAgTokenSideChain {
    ...
    function removeBridgeToken(address bridgeToken) external onlyGovernorOrGuardian {
    ...
    function setLimit(address bridgeToken, uint256 limit) external onlyGovernor {
    ...
    function setSwapFee(address bridgeToken, uint64 fee) external onlyGovernor {
        if (!bridges[bridgeToken].allowed) revert InvalidToken();
        if (fee > BASE_PARAMS) revert TooHighParameterValue();
    ...
    function toggleBridge(address bridgeToken) external onlyGovernor {
        if (!bridges[bridgeToken].allowed) revert InvalidToken();
