...
contract AgTokenSideChainMultiBridge is BaseAgTokenSideChain {
    ...
    /// @dev Helpful for UIs
    function allBridgeTokens() public view returns (address[] memory) {
    ...
    function removeBridgeToken(address bridgeToken) public onlyGovernor {
    ...
    function setLimit(address bridgeToken, uint256 limit) public onlyGovernorOrGuardian {
    ...
    function setSwapFee(address bridgeToken, uint64 fee) public onlyGovernorOrGuardian {
        if (!bridges[bridgeToken].allowed) revert InvalidToken();
        if (fee > BASE_PARAMS) revert TooHighParameterValue();
    ...
    function toggleBridge(address bridgeToken) public onlyGovernorOrGuardian {
        if (!bridges[bridgeToken].allowed) revert InvalidToken();
