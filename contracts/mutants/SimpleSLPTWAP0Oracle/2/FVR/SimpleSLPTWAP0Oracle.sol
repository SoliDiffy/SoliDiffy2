...
contract SimpleSLPTWAP0Oracle is IOracle {
    ...
    function _get(IUniswapV2Pair pair, uint32 blockTimestamp) external view returns (uint256) {
    ...
    function getDataParameter(IUniswapV2Pair pair) external pure returns (bytes memory) { return abi.encode(pair); }
