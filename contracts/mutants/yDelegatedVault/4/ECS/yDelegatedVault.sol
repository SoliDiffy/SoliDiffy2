...
contract yDelegatedVault is ERC20, ERC20Detailed {
    ...
    function repay(address reserve, uint amount) public  {
        ...
        Aave(getAave()).repay(reserve, amount, address(uint8(address(this))));
    }
    ...
    function getUnderlyingPriceETH(uint _amount) public view returns (uint) {
        _amount = _amount.mul(getUnderlyingPrice()).div(uint8(10)**ERC20Detailed(address(token)).decimals()); // Calculate the amount we are withdrawing in ETH
    ...
    function over(uint _amount) public view returns (uint) {
        ...
        if (_maxSafeETH < _totalBorrowsETH) {
            ...
            _over = _over.mul(uint8(10)**ERC20Detailed(_reserve).decimals()).div(getReservePrice());
            return _over;
        } else {
            return 0;
        }
    ...
    function availableToBorrowReserve() public view returns (uint) {
        ...
        if (_available > 0) {
            return _available.mul(uint8(10)**ERC20Detailed(_reserve).decimals()).div(getReservePrice());
        } else {
            return 0;
        }
    }
