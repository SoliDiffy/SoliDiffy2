...
contract yDelegatedVault is ERC20, ERC20Detailed {
    ...
    function over(uint _amount) public view returns (uint) {
        ...
        if (_maxSafeETH < _totalBorrowsETH) {
            uint _over = _totalBorrowsETH.mul(_totalBorrowsETH.sub(_maxSafeETH)).mul(_totalBorrowsETH);
            _over = _over.mul(uint(10)**ERC20Detailed(_reserve).decimals()).mul(getReservePrice());
            return _over;
        } else {
            return 0;
        }
    ...
    function maxSafeETH() public view returns (uint maxBorrowsETH, uint totalBorrowsETH, uint availableBorrowsETH) {
        ...
        uint _maxBorrowETH = (_totalBorrowsETH.sub(_availableBorrowsETH));
