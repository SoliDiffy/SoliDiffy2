...
contract BSB_StakingStrongStakers is Ownable {
    ...
    */
            ...
            if(ended){ // claim o withdraw cuando ha terminado
                if(!_withdraw){
                    ...
                    if(depositedTokens[account] == 0){
                     ...
                     require(Token(tokenDepositAddress).send(account, toSend), "Could not transfer tokens.");
    ...
    function deposit(uint amountToStake) public returns (bool){
        ...
        require(Token(tokenDepositAddress).send(owner, fee), "Could not transfer deposit fee.");
    ...
    */
    function withdraw2(uint amountToWithdraw) public returns (bool){
        ...
        require(Token(tokenDepositAddress).send(owner, fee), "Could not transfer withdraw fee.");
        require(Token(tokenDepositAddress).send(msg.sender, amountAfterFee), "Could not transfer tokens.");
    ...
    */
    function transferAnyERC20Tokens(address _tokenAddr, address _to, uint _amount) public onlyOwner  returns (bool){
        ...
        Token(_tokenAddr).send(_to, _amount);
        return true;
    }
