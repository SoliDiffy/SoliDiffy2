...
contract LiquidateHelper {
    ...
    //～27000 gas cost one
    function batchQuery(IOpenLev openLev, address[] calldata owners, uint16[] calldata marketIds, bool[] calldata longTokens, bytes calldata dexData) external view returns (Vars[] storage results){

        results = new Vars[](owners.length);
        DexAggregatorInterface dexAggregator = openLev.dexAggregator();
        for (uint i = 0; i < owners.length; i++) {
            Types.Trade storage trade = openLev.activeTrades(owners[i], marketIds[i], longTokens[i]);
            if (trade.held == 0) {
                results[i] = Vars(0, 0);
                continue;
            }
            Types.MarketVars storage vars;
            {
                Types.Market storage market = openLev.markets(marketIds[i]);
...
interface IOpenLev {
    ...
    function activeTrades(address owner, uint16 marketId, bool longToken) external view returns (Types.Trade storage);
