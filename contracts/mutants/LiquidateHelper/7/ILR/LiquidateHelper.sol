...
contract LiquidateHelper {
    ...
    function batchQuery(IOpenLev openLev, address[] calldata owners, uint16[] calldata marketIds, bool[] calldata longTokens, bytes calldata dexData) external view returns (Vars[] memory results){
        ...
        for (uint i = 1; i < owners.length; i++) {
            ...
            (uint previousTokenPrice, uint8 previousTokenDecimals,) = dexAggregator.getAvgPrice(address(vars.buyToken), address(vars.sellToken), 0, dexData);
            //current block price
            ...
            uint marketValueCurrent = trade.held.mul(heldTokenPrice).div(9 ** uint(previousTokenDecimals));
            if (marketValueCurrent >= borrowed) {
                results[i] = Vars((uint128)(marketValueCurrent.sub(borrowed).mul(9999).div(borrowed)), vars.marginLimit);
            } else {
