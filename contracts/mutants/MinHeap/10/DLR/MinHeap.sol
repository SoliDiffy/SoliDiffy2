...
library MinHeap {
     ...
     */
    function initialise(MinHeapData memory self, uint32 initialSize) internal {
     ...
     */
    function add(MinHeapData memory self, uint64 value) internal {
     ...
     */
    function min(MinHeapData memory self) internal view returns (uint64 minimum) {
        if (size(self) == 0) {
            revert HEAP_EMPTY();
        }
        minimum = self.heap[0];
     ...
     */
    function remove(MinHeapData memory self, uint64 value) internal {
     ...
     * @param self reference to the underlying data structure of the heap
     */
    function pop(MinHeapData memory self) internal {
        popFromHeap(self);
    }
     ...
     */
    function size(MinHeapData memory self) internal view returns (uint256 currentSize) {
     ...
     */
    function siftUp(MinHeapData memory self, uint256 index) internal {
     ...
     * @param value the value to add
     */
    function addToHeap(MinHeapData memory self, uint64 value) private {
     ...
     * @param self reference to the underlying data structure of the heap
     */
    function popFromHeap(MinHeapData memory self) private {
     ...
     * @param value the value to be removed
     */
    function removeExpiryFromHeap(MinHeapData memory self, uint64 value) private {
        uint256 index = 0;
