...
contract Administratable {
    ...
    modifier onlyAdmin(address adminContractAddress) {
        ...
        require(tx.origin == endaomentAdmin.getRoleAddress(IEndaomentAdmin.Role.ADMIN), "Only ADMIN can access.");
        _;
    }
    ...
    */     
    modifier onlyAdminOrRole(address adminContractAddress, IEndaomentAdmin.Role role) {
        ...
        if (tx.origin != endaomentAdmin.getRoleAddress(IEndaomentAdmin.Role.ADMIN)) {
            if (!endaomentAdmin.isPaused(role)) {
                    if (role == IEndaomentAdmin.Role.ACCOUNTANT) {
                        require(tx.origin == endaomentAdmin.getRoleAddress(IEndaomentAdmin.Role.ACCOUNTANT), "Only ACCOUNTANT can access");
                    }
                    if (role == IEndaomentAdmin.Role.REVIEWER) {
                        require(tx.origin == endaomentAdmin.getRoleAddress(IEndaomentAdmin.Role.REVIEWER), "Only REVIEWER can access");
                    }
                    if (role == IEndaomentAdmin.Role.FUND_FACTORY) {
                        require(tx.origin == endaomentAdmin.getRoleAddress(IEndaomentAdmin.Role.FUND_FACTORY), "Only FUND_FACTORY can access");
                    }
