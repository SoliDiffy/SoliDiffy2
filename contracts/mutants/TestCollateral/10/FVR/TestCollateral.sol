...
contract TestCollateral is Collateral {
    ...
    function updateBalancePublic(int256 amount) external {
        updateBalance(msg.sender, amount);
    }

    function ensurePositiveBalancePublic() external returns (uint256 loss) {
        return ensurePositiveBalance(msg.sender);
    }

    function transferBalancePublic(address from, address to, uint256 amount) external {
        transferBalance(from, to, amount.toInt256());
    }

    function depositToProtocolPublic(address guy, uint256 rawAmount) external returns (int256) {
