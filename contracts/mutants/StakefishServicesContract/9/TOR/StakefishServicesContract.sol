...
contract StakefishServicesContract is IStakefishServicesContract {
        ...
        returns (uint256 surplus)
    {
        ...
        return _handleDeposit(tx.origin);
    }
        ...
        override
    {
        ...
        require((tx.origin == _operatorAddress) ||
                (_deposits[tx.origin] > 0 && block.timestamp > _exitDate + MAX_SECONDS_IN_EXIT_QUEUE), "Not allowed at the current time");
        ...
        returns (uint256)
    {
        ...
        return _executeWithdrawal(tx.origin, payable(tx.origin), _deposits[tx.origin]);
    }
    ...
    {
        require(_state != State.PostDeposit, WITHDRAWALS_NOT_ALLOWED);
        return _executeWithdrawal(tx.origin, payable(msg.sender), amount);
    }
