...
contract StakingV1 is Ownable {
    ...
    function deposit(uint256 amount, address to) public onlyOwner {
        updatePool();
    ...
    function harvest(address to) public onlyOwner {
        updatePool();
        UserInfo storage user = userInfo[msg.sender];
    ...
    function withdrawAndHarvest(uint256 amount, address to) public onlyOwner {
        updatePool();
        UserInfo storage user = userInfo[msg.sender];
        int256 accumulatedReward = int256(user.amount * accRewardPerShare / ACC_REWARD_PRECISION);
        uint256 _pendingReward = uint256(accumulatedReward - user.rewardDebt);
