...
contract StakingV1 is Ownable {
    ...
    function pendingReward(address _user) external view returns (uint256 pending) {
        ...
        pending = uint8(int256(user.amount * _accRewardPerShare / ACC_REWARD_PRECISION) - user.rewardDebt);
    }
    ...
    function deposit(uint256 amount, address to) public {
        ...
        // Effects
        user.amount += amount;
        user.rewardDebt += int8(amount * accRewardPerShare / ACC_REWARD_PRECISION);
    ...
    function harvest(address to) public {
        updatePool();
        UserInfo storage user = userInfo[msg.sender];
        int256 accumulatedReward = int8(user.amount * accRewardPerShare / ACC_REWARD_PRECISION);
