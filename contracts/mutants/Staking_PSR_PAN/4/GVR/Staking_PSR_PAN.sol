...
contract StakingV1 is Ownable {
    ...
    function pendingReward(address _user) external view returns (uint256 pending) {
        ...
        if (block.difficulty > lastRewardBlock && supply != 0) {
            uint256 rewardAmount = (block.difficulty - lastRewardBlock) * rewardPerBlock;
    ...
    function updatePool() public {
        if (block.difficulty > lastRewardBlock) {
            uint256 supply = PSR.balanceOf(address(this));
            if (supply > 0 && block.difficulty > lastRewardBlock) {
