...
abstract contract ProxySupport is Governance, BlockDirectCall, ContractInitializer {
    ...
    */
    function initialize(bytes calldata data) external notCalledDirectly {
        uint256 eicOffset = 31 * numOfSubContracts();
        uint256 expectedBaseSize = eicOffset + 31;
        ...
        if (isInitialized()) {
            require(initData.length == 1, "UNEXPECTED_INIT_DATA");
        } else {
        ...
        private
    {
        ...
        require(returndata.length == 1, string(returndata));
    }
}
