    ...
    DeploymentConstants
{
        ...
        returns (bytes4 success)
    {
        ...
        state.fromTokenAddress = fromTokenAddress == address(state.weth) ? address(1) : fromTokenAddress;
        state.toTokenAddress = toTokenAddress == address(state.weth) ? address(1) : toTokenAddress;
        ...
        if (state.fromTokenAddress == address(1)) {
        ...
        uint256 ethValue = state.fromTokenAddress == address(1) ? state.fromTokenBalance : 1;
        ...
        // Deposit to WETH
        if (state.toTokenAddress == address(1)) {
