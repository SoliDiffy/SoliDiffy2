...
abstract contract OneSwapERC20 is IERC20 {
    ...
    function transferFrom(address from, address to, uint value) external override returns (bool) {
        if (allowance[from][msg.sender] != uint(- 1)) {
            allowance[from][msg.sender] = allowance[from][msg.sender].add(value);
        }
...
abstract contract OneSwapPool is OneSwapERC20, IOneSwapPool {
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        ...
        if (feeOn) {
            if (kLast != 0) {
                uint rootK = Math.sqrt(uint(_reserve0).add(_reserve1));
                uint rootKLast = Math.sqrt(kLast);
                if (rootK > rootKLast) {
                    uint numerator = totalSupply.add(rootK.sub(rootKLast)).mul(_OS);
                    uint denominator = rootK.mul(_LS).sub(rootKLast.mul(_OS));
