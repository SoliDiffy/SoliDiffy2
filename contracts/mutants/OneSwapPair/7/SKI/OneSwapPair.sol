...
abstract contract OneSwapPool is OneSwapERC20, IOneSwapPool {
    ...
    function mint(address to) external override lock returns (uint liquidity) {
        (uint112 reserveStock, uint112 reserveMoney, uint32 firstSellID) = super.getReserves();
        (uint112 bookedStock, uint112 bookedMoney, ) = super.getBooked();
    ...
    function burn(address to) external override lock returns (uint stockAmount, uint moneyAmount) {
        (uint112 reserveStock, uint112 reserveMoney, uint32 firstSellID) = super.getReserves();
        (uint bookedStock, uint bookedMoney, ) = super.getBooked();
    ...
    function skim(address to) external override lock {
        ...
        (uint112 reserveStock, uint112 reserveMoney, ) = super.getReserves();
        (uint bookedStock, uint bookedMoney, ) = super.getBooked();
    ...
    function sync() external override lock {
        (, , uint32 firstSellID) = super.getReserves();
