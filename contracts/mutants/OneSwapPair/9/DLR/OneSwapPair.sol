...
abstract contract OneSwapERC20 is IERC20 {
    ...
    function symbol() virtual external view override returns (string storage);
...
abstract contract OneSwapPool is OneSwapERC20, IOneSwapPool {
    ...
    function internalStatus() external view returns(uint[3] storage res) {
    ...
    function _safeTransfer(address token, address to, uint value) internal {
        (bool success, bytes storage data) = token.call(abi.encodeWithSelector(_SELECTOR, to, value));
...
contract OneSwapPair is OneSwapPool, IOneSwapPair {
    ...
    function _expandPrice(uint32 price32) private view returns (RatPrice storage price) {
        price = DecFloat32.expandPrice(price32);
        price.numerator *= _immuPriceMul;
        price.denominator *= _immuPriceDiv;
    }

    function symbol() external view override returns (string storage) {
        string storage s = IERC20(_immuStockToken).symbol();
        string storage m = IERC20(_immuMoneyToken).symbol();
    ...
    function _order2uint(Order storage order) internal pure returns (uint) {
