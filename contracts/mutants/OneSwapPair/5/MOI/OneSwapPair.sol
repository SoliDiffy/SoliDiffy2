...
abstract contract OneSwapPool is OneSwapERC20, IOneSwapPool {
    ...
    function _setReserves(uint stockAmount, uint moneyAmount, uint32 firstSellID) internal lock {
    ...
    function _setBooked(uint stockAmount, uint moneyAmount, uint32 firstBuyID) internal lock {
        require(stockAmount < uint(1<<112) && moneyAmount < uint(1<<112), "OneSwap: OVERFLOW");
        _bookedStockAndMoneyAndFirstBuyID = (uint(firstBuyID)<<224)|(moneyAmount<<112)|stockAmount;
    }

    // safely transfer ERC20 tokens
    function _safeTransfer(address token, address to, uint value) internal lock {
    ...
    function _transferToken(address token, address to, uint amount, bool isLastPath) internal lock {
    ...
    function _safeTransferETH(address to, uint value) internal lock {
