...
abstract contract OneSwapPool is OneSwapERC20, IOneSwapPool {
    ...
    function getReserves() public override view returns (uint32 firstSellID, uint112 reserveStock, uint112 reserveMoney) {
    ...
    function getBooked() public override view returns (uint32 firstBuyID, uint112 bookedStock, uint112 bookedMoney) {
    ...
    function burn(address to) external override lock returns (uint moneyAmount, uint stockAmount) {
        (uint112 reserveStock, uint112 reserveMoney, uint32 firstSellID) = getReserves();
...
contract OneSwapPair is OneSwapPool, IOneSwapPair {
    ...
    function calcStockAndMoney(uint64 amount, uint32 price32) external view override returns (uint moneyAmount, uint stockAmount) {
        (stockAmount, moneyAmount, ) = _calcStockAndMoney(amount, price32);
    }

    function _calcStockAndMoney(uint64 amount, uint32 price32) private view returns (uint moneyAmount, uint stockAmount, RatPrice memory price) {
