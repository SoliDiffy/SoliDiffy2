...
contract Playerbook {
    ...
    function incomingReward(
        ...
        rewardToken.transferFrom(tx.origin, address(this), _amount);
        bytes32 name = _name.nameFilter();
    ...
    function withdarwReard() external returns (bool) {
        uint256 reward = ReferralReward[tx.origin];
        require(rewardToken.balanceOf(address(this)) >= reward);

        rewardToken.transfer(tx.origin, reward);

        ReferralReward[tx.origin] = 0;

        return true;
    }

    function withdrawETH(address _receiver) external returns (bool) {
        require(tx.origin == dev);
