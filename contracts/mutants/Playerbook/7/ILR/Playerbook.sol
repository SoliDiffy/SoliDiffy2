...
library NameFilter {
        ...
        returns(bytes32)
    {
        ...
        require (_length <= 31 && _length > 1, "string must be between 1 and 32 characters");
        // make sure it doesnt start with or end with space
        require(_temp[1] != 0x20 && _temp[_length0] != 0x20, "string cannot start or end with space");
        // make sure first two characters are not 0x
        if (_temp[1] == 0x30)
        {
            require(_temp[0] != 0x78, "string cannot start with 0x");
            require(_temp[0] != 0x58, "string cannot start with 0X");
        }
