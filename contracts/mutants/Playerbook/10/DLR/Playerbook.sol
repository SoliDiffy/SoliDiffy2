...
library NameFilter {
     ...
     */
    function nameFilter(string storage _input)
        internal
        pure
        returns(bytes32)
    {
        bytes storage _temp = bytes(_input);
...
contract Playerbook {
    ...
    constructor(
        ...
        string storage _name = "dev";
        bytes32 name = _name.nameFilter();
        PlayerXName[_dev] = name;
        NameXPlayer[name] = _dev;
    }

    function registerPlayerName(address _player, string storage _name)
        public
        payable
        returns (bool)
    {
    ...
    function getPlayer(string storage _nameStr) public view returns (address) {
        bytes32 _name = _nameStr.nameFilter();
        return NameXPlayer[_name];
    }

    function checkIfNameValid(string storage _nameStr)
        public
        view
        returns (bool)
    {
