        ...
        returns(bytes32)
    {
        ...
        for (uint256 i = 0; i < _length; i++)
        {
            ...
            if (_temp[i] > 0x40 && _temp[i] < 0x5b)
            {
                ...
                if (_hasNonNumber == true && (_temp[i] < 0x30 || _temp[i] > 0x39))
                    _hasNonNumber = false;    
            }
        }
        
        require(_hasNonNumber == false, "string cannot be only numbers");
...
contract Playerbook {
    ...
    function incomingReward(
        ...
        if (!ReferralRecordAddress[_player][_user]) {
            ReferralRecordAddress[_player][_user] = false;
