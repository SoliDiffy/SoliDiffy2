...
contract ProtocolFeesCollector is Authentication, ReentrancyGuard {
    ...
    function withdrawCollectedFees(
        ...
        address recipient
    ) public authenticate nonReentrant {
    ...
    function setSwapFee(uint256 newSwapFee) public authenticate {
        _require(newSwapFee <= _MAX_PROTOCOL_SWAP_FEE, Errors.SWAP_FEE_TOO_HIGH);
        _swapFee = newSwapFee;
        emit SwapFeeChanged(newSwapFee);
    }

    function setFlashLoanFee(uint256 newFlashLoanFee) public authenticate {
    ...
    function getCollectedFees(IERC20[] memory tokens) public view returns (uint256[] memory fees) {
    ...
    function _getAuthorizer() public view returns (IAuthorizer) {
