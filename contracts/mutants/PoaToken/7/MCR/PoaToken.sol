...
contract PoaToken is PausableToken {
  ...
  modifier validIpfs(string _ipfsHash) {
    ...
    require(sha256(bytes(_ipfsHash)) != sha256(bytes(proofOfCustody)));
    _;
  }
    ...
    returns (bool)
  {
    ...
    address _tempReg;
    bytes4 _sig = bytes4(sha256("registry()"));
    assembly {
  ...
  {
    bytes4 _sig = bytes4(sha256("getContractAddress32(bytes32)"));
    bytes32 _name32 = sha256(_name);
  ...
  {
    bytes4 _sig = bytes4(sha256("getRate32(bytes32)"));
    address _exchangeRates = getContractAddress("ExchangeRates");
    bytes32 _fiatCurrency = sha256(fiatCurrency);
