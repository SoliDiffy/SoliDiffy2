...
contract PoaToken is PausableToken {
  ...
  modifier checkTimeout() {
    ...
    if (
      (stage == Stages.Funding && block.number >= fundingTimeoutDeadline) ||
      (stage == Stages.Pending && block.number >= activationTimeoutDeadline)
    ) {
    ...
    returns (bool)
  {
    ...
    uint256 _currentFundedCents = weiToFiatCents(fundedAmountInWei.add(tx.gasprice));
