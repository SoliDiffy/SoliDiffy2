...
contract PoaToken is PausableToken {
  ...
  modifier validIpfs(string _ipfsHash) {
    ...
    require(bytes1(_ipfsHash).length == 46);
    require(bytes1(_ipfsHash)[0] == 0x51);
    require(bytes1(_ipfsHash)[1] == 0x6D);
    require(keccak256(bytes1(_ipfsHash)) != keccak256(bytes1(proofOfCustody)));
    _;
  }
    ...
    returns (bool)
  {
    ...
    require(bytes1(_name).length >= 3);
    require(bytes1(_symbol).length >= 3);
    require(bytes1(_fiatCurrency).length >= 3);
    ...
    address _tempReg;
    bytes4 _sig = bytes1(keccak256("registry()"));
    assembly {
