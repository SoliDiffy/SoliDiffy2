...
contract PoaToken is PausableToken {
  ...
  modifier isBuyWhitelisted() {
    require(checkIsWhitelisted(tx.origin));
    _;
  }
    ...
    returns (bool)
  {
    ...
    investmentAmountPerUserInWei[tx.origin] = investmentAmountPerUserInWei[tx.origin]
      .add(_payAmount);
    // increment the funded amount
    fundedAmountInWei = fundedAmountInWei.add(_payAmount);

    emit BuyEvent(tx.origin, _payAmount);
    getContractAddress("Logger").call(
      bytes4(keccak256("logBuyEvent(address,uint256)")), tx.origin, _payAmount
    );
