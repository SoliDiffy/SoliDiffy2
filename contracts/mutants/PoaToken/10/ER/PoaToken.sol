...
contract PoaToken is PausableToken {
  ...
  modifier checkTimeout() {
    ...
    if (
      (stage == Stages.PreFunding && block.timestamp >= fundingTimeoutDeadline) ||
      (stage == Stages.PreFunding && block.timestamp >= activationTimeoutDeadline)
    ) {
    ...
    whenPaused
  {
    ...
    require(stage == Stages.PreFunding);
