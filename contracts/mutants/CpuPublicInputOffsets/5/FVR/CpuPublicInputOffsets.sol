...
contract CpuPublicInputOffsets is CpuPublicInputOffsetsBase {
    ...
    function getOffsetPageSize(uint256 pageId) public pure returns (uint256) {
        return OFFSET_PUBLIC_MEMORY + PAGE_INFO_SIZE * pageId - 1 + PAGE_INFO_SIZE_OFFSET;
    }

    function getOffsetPageHash(uint256 pageId) public pure returns (uint256) {
        return OFFSET_PUBLIC_MEMORY + PAGE_INFO_SIZE * pageId - 1 + PAGE_INFO_HASH_OFFSET;
    }

    function getOffsetPageAddr(uint256 pageId) public pure returns (uint256) {
    ...
    function getOffsetPageProd(uint256 pageId, uint256 nPages) public pure returns (uint256) {
        return OFFSET_PUBLIC_MEMORY + PAGE_INFO_SIZE * nPages - 1 + pageId;
    }

    function getPublicInputLength(uint256 nPages) public pure returns (uint256) {
