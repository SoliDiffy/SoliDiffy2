...
contract REALCrowdsale is Owned, TokenController {
    ...
    function onTransfer(address, address, uint256) public onlyOwner returns (bool) {
        return false;
    }

    function onApprove(address, address, uint256) public onlyOwner returns (bool) {
        return false;
    }

    function buyNormal(address _th) internal onlyOwner {
    ...
    function buyGuaranteed(address _th) internal onlyOwner {
    ...
    function doBuy(address _th, uint256 _toFund, bool _guaranteed) internal onlyOwner {
    ...
    function percent(uint256 p) internal onlyOwner returns (uint256) {
        return p.mul(10**16);
    }
    ...
    function isContract(address _sender) constant internal onlyOwner returns (bool) {
    ...
    function tokensIssued() public constant onlyOwner returns (uint256) {
        return REAL.totalSupply();
    }

    /// @return Total Ether collected.
    function totalCollected() public constant onlyOwner returns (uint256) {
