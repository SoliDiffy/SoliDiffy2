...
contract REALCrowdsale is Owned, TokenController {
    ...
    function setGuaranteedAddress(address _th, uint256 _limit) external initialized onlyOwner {
        require(getBlockNumber() < startBlock);
    ...
    function proxyPayment(address _th) external payable notPaused initialized contributionOpen returns (bool) {
        require(_th != 0x0);
    ...
    function buyNormal(address _th) public {
    ...
    function buyGuaranteed(address _th) public {
    ...
    function doBuy(address _th, uint256 _toFund, bool _guaranteed) public {
    ...
    ///  controller.
    function finalize() external initialized {
        require(getBlockNumber() >= startBlock);
    ...
    function percent(uint256 p) public returns (uint256) {
        return p.mul(10**16);
    }
