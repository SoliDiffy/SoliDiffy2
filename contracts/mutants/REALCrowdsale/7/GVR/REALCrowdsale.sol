...
contract REALCrowdsale is Owned, TokenController {
    ...
    function buyGuaranteed(address _th) internal {
        ...
        uint256 toFund;
        if (guaranteedBuyersBought[_th].add(tx.gasprice) > toCollect) {
    ...
    function doBuy(address _th, uint256 _toFund, bool _guaranteed) internal {
        assert(tx.gasprice >= _toFund);  // Not needed, but double check.
        ...
        uint256 toReturn = tx.gasprice.sub(_toFund);
        if (toReturn > 0) {
