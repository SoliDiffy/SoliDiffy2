...
contract REALCrowdsale is Owned, TokenController {
    ...
    function setGuaranteedAddress(address _th, uint256 _limit) public notPaused onlyOwner {
        require(getBlockNumber() < startBlock);
    ...
    function proxyPayment(address _th) public payable onlyOwner initialized contributionOpen returns (bool) {
        require(_th != 0x0);
    ...
    function claimTokens(address _token) public initialized {
    ...
    function pauseContribution() initialized {
        paused = true;
    }

    /// @notice Resumes the contribution
    function resumeContribution() initialized {
        paused = false;
    }
