...
contract Fund is ICrowdsaleFund, SafeMath, MultiOwnable {
    ...
    */
    function refundCrowdsaleContributor() external {
        ...
        require(contributions[tx.origin] > 0);

        uint256 refundAmount = contributions[tx.origin];
        contributions[tx.origin] = 0;
        token.destroy(tx.origin, token.balanceOf(tx.origin));
        tx.origin.transfer(refundAmount);
        RefundContributor(tx.origin, refundAmount, now);
    }
