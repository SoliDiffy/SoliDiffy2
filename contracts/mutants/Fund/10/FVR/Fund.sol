...
contract Fund is ICrowdsaleFund, SafeMath, MultiOwnable {
     ...
     */
    function processContribution(address contributor) public payable onlyCrowdsale {
     ...
     */
    function enableCrowdsaleRefund() public onlyCrowdsale {
        require(state == FundState.Crowdsale);
    ...
    */
    function refundCrowdsaleContributor() public {
     ...
     */
    function decTap(uint256 _tap) public onlyOwner {
        require(state == FundState.TeamWithdraw);
        require(_tap < tap);
        tap = _tap;
    }

    function getCurrentTapAmount() external constant returns(uint256) {
    ...
    function calcTapAmount() public view returns(uint256) {
