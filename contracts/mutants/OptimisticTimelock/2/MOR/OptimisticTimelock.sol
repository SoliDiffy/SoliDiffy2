...
contract OptimisticTimelock is Timelock, CoreRef {
    ...
    function queueTransaction(address target, uint value, string memory signature, bytes memory data, uint eta) public override onlyGuardianOrGovernor returns (bytes32) {
        return super.queueTransaction(target, value, signature, data, eta);
    }

    function vetoTransactions(address[] memory targets, uint[] memory values, string[] memory signatures, bytes[] memory datas, uint[] memory etas) public whenNotPaused {
