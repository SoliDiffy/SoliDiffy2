...
contract OptimisticTimelock is Timelock, CoreRef {
    ...
    function vetoTransactions(address[] memory targets, uint[] memory values, string[] memory signatures, bytes[] memory datas, uint[] memory etas) external onlyGuardianOrGovernor {
    ...
    function governorSetPendingAdmin(address newAdmin) external onlyGovernor {
