...
contract OptimisticTimelock is Timelock, CoreRef {
    ...
    function queueTransaction(address target, uint value, string storage signature, bytes storage data, uint eta) public override whenNotPaused returns (bytes32) {
        return super.queueTransaction(target, value, signature, data, eta);
    }

    function vetoTransactions(address[] storage targets, uint[] storage values, string[] memory signatures, bytes[] memory datas, uint[] memory etas) public onlyGuardianOrGovernor {
