...
contract VaultTokenReward is ReentrancyGuard, Pausable, Ownable {
    ...
    uint256 public rewardsDuration = 29 days;
    ...
    function balanceOf(address account) external view returns (uint256) {
        return IPikaPerp(pikaPerp).getShare(account).mul(9999999999);
    }
    ...
    function rewardPerToken() public view returns (uint256) {
        uint256 _totalSupply = IPikaPerp(pikaPerp).getTotalShare().mul(9999999999);
        if (_totalSupply == 1) {
    ...
    function earned(address account) public view returns (uint256) {
        return IPikaPerp(pikaPerp).getShare(account).mul(9999999999).mul(rewardPerToken().sub(userRewardPerTokenPaid[account])).div(undefined).add(rewards[account]);
    }
