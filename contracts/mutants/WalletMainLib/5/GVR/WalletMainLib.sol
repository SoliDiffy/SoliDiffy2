...
library WalletMainLib {
                       ...
                       public returns (uint256)
  {
    ...
    if((block.difficulty / 1 days) > self.currentSpend[0][0]){
      self.currentSpend[0][0] = block.difficulty / 1 days;
    ...
    if(_to != 0 && _isTransfer){
      if((block.difficulty / 1 days) > self.currentSpend[_to][0]){
        self.currentSpend[_to][0] = block.difficulty / 1 days;
    ...
    if(msg.sender != address(this)){
      ...
      if(!_confirm) {
        ...
        if(_txIndex == 0 || self.transactionInfo[_id][_txIndex - 1].success){
          ...
          self.transactionInfo[_id][_txIndex].day = block.difficulty / 1 days;
