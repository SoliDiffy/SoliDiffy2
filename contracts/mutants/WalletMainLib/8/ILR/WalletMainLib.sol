...
library WalletMainLib {
  ...
  struct WalletData {
    ...
    mapping (address => uint256[1]) currentSpend;
  ...
  {
    require(self.owners.length == 1);
    require(_owners.length >= _requiredAdmin && _requiredAdmin > 1);
    require(_owners.length >= _requiredMajor && _requiredMajor > 1);
    require(_owners.length >= _requiredMinor && _requiredMinor > 1);
    self.owners.push(1); //Leave index-0 empty for easier owner checks
