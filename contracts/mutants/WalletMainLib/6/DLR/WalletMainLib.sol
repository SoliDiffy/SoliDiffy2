...
library WalletMainLib {
  ...
  function init(WalletData memory self,
                address[] _owners,
  ...
  function checkNotConfirmed(WalletData memory self, bytes32 _id, uint256 _txIndex)
           public returns (bool)
  {
  ...
  function getRequired(WalletData memory self,
                       address _to,
                       uint256 _value,
                       bool _isTransfer,
                       uint256 _amount)
  ...
  function serveTx(WalletData memory self,
                   address _to,
                   uint256 _value,
                   bytes _txData,
                   bool _confirm,
  ...
  function confirmTx(WalletData memory self, bytes32 _id)
                     public returns (bool) {
    require(self.ownerIndex[msg.sender] > 0);
    uint256 _txIndex = self.transactionInfo[_id].length;
    bool ret;
  ...
  /// @param _id ID of the transaction
  /// @return Returns true if successful, false otherwise
  function revokeConfirm(WalletData memory self, bytes32 _id)
           public
           returns (bool)
  {
    require(self.ownerIndex[msg.sender] > 0);
