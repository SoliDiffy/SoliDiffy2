...
library WalletMainLib {
  ...
  function init(WalletData storage self,
                ...
                uint256 _majorThreshold) external returns (bool)
  {
  ...
  function calcConfirmsNeeded(uint256 _required, uint256 _count) external pure returns (uint256){
  ...
  function getAmount(bytes _txData) external pure returns (bool,uint256) {
    bytes32 getSig;
    bytes4 sig;
  ...
  function createContract(bytes _txData, uint256 _value) external {
