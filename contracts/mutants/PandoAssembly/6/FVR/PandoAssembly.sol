...
contract PandoAssembly is Ownable, IERC721Receiver {
    ...
    function info(address _user) public view returns(uint256[] memory _nftIds){
        UserInfo storage user = userInfo[_user];
        _nftIds = EnumerableSet.values(user.nftIds);
    }

    function originalPower(address _user) external view returns (uint256 res) {
        UserInfo storage user = userInfo[_user];
    ...
    function getRewardForDuration(uint256 _from, uint256 _to) external view returns (uint256) {
