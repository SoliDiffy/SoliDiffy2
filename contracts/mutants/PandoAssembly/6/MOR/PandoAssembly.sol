...
contract PandoAssembly is Ownable, IERC721Receiver {
    ...
    function allocateMoreRewards(uint256 _addedReward, uint256 _days) external onlyOwner {
        updatePool();
    ...
    function rescueFund(uint256 _amount) external onlyReserveFund {
    ...
    function setPayment(address _paymentToken, uint256 _price, uint256 _coef) external onlyReserveFund {
        paymentToken = _paymentToken;
    ...
    function changeDroidBotAddress(address _newAddr) external onlyReserveFund {
        droidBot = IDroidBot(_newAddr);
        emit DroidBotAddressChanged(_newAddr);
    }

    function setReceivingFund(address _addr) external onlyReserveFund {
        receivingFund = _addr;
