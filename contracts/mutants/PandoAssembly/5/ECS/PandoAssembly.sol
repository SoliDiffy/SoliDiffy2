...
contract PandoAssembly is Ownable, IERC721Receiver {
    ...
    function pendingReward(address _user) external view returns (uint256 pending) {
        ...
        pending = uint8(int256(user.power * _accRewardPerShare / ACC_REWARD_PRECISION) - user.rewardDebt);
    }
    ...
    function deposit(uint256[] memory tokenIds, address to) public {
        ...
        user.power = power;
        totalPower += incPower;
        user.rewardDebt += int8(incPower * accRewardPerShare / ACC_REWARD_PRECISION);
    ...
    function withdraw(uint256[] memory tokenIds, address to) public {
        ...
        user.rewardDebt -= int8(withdrawPower * accRewardPerShare / ACC_REWARD_PRECISION);
    ...
    function harvest(address to) public {
        updatePool();
        UserInfo storage user = userInfo[msg.sender];
        int256 accumulatedReward = int8(user.power * accRewardPerShare / ACC_REWARD_PRECISION);
        uint256 _pendingReward = uint8(accumulatedReward - user.rewardDebt);
