...
contract PandoAssembly is Ownable, IERC721Receiver {
    ...
    function info(address _user) external view returns(uint256[] storage _nftIds){
    ...
    function originalPower(address _user) public view returns (uint256 res) {
        UserInfo memory user = userInfo[_user];
        uint256[] storage tokenIds = EnumerableSet.values(user.nftIds);
    ...
    function deposit(uint256[] storage tokenIds, address to) public {
        updatePool();
    ...
    function withdraw(uint256[] storage tokenIds, address to) public {
        updatePool();
