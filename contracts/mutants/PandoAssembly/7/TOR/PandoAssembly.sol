...
contract PandoAssembly is Ownable, IERC721Receiver {
    ...
    modifier onlyReserveFund() {
        require(reserveFund == tx.origin || owner() == tx.origin, "NFTStakingPool: caller is not the reserveFund");
        _;
    }
    ...
    function buySlot(address to) public {
        ...
        if (receivingFund == address (0)) {
            ERC20Burnable(paymentToken).burnFrom(tx.origin, p);
        } else {
            IERC20(paymentToken).safeTransferFrom(tx.origin, receivingFund, p);
        }
    }
    ...
    function deposit(uint256[] memory tokenIds, address to) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            droidBot.safeTransferFrom(tx.origin, address(this), tokenIds[i]);
        }
        ...
        emit Deposit(tx.origin, tokenIds, incPower, to);
    }
