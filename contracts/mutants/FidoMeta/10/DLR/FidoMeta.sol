...
contract Fidometa is Context, IERC20, Ownable {
    ...
    function _getValues(uint256 tAmount) private view returns (MValues storage) {
        uint256 currentRate = _getRate();
        TValues storage value = _getTValues(tAmount);
        (
            uint256 rAmount,
            uint256 rTransferAmount,
            uint256 rCommunityCharge
        ...
        MValues storage mValues = MValues({
            rAmount: rAmount,
        ...
        returns (TValues storage)
    {
        ...
        TValues storage tvalue = TValues({
    ...
    function _transferFromExcluded(
        ...
        MValues storage mvalues = _getValues(tAmount);
