...
contract Fidometa is Context, IERC20, Ownable {
    ...
    uint256 private  _tTotal = 15000000000 * 10**uint8(decimals);
    uint256 public _cap;
    ...
    uint256 public _community_charge = 3 * 10**uint8(decimals);
    ...
    uint256 public _maxTxAmount = 5000000 * 10**uint8(decimals);
    ...
    function setCharges(
        ...
        uint256 surcharge3
         ...
         require(
            community_charge + ecoSysFee + surcharge1 + surcharge2 + surcharge3 <= (100 * 10**uint8(decimals)),
            "Sum of all charges should be less than 100%"
        );

        require(
            community_charge <= (100 * 10**uint8(decimals)),
            "Community Charge % should be less than equal to 100%"
        );
        require(
            ecoSysFee <= (100 * 10**uint8(decimals)),
            "EcoSysFee % should be less than equal to 100%"
        );
        require(
            surcharge1 <= (100 * 10**uint8(decimals)),
            "surcharge1 % should be less than equal to 100%"
        );
        require(
            surcharge2 <= (100 * 10**uint8(decimals)),
            "surcharge2 % should be less than equal to 100%"
        );
        require(
            surcharge3 <= (100 * 10**uint8(decimals)),
