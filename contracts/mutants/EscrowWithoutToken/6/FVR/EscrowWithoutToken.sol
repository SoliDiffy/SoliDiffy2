...
contract EscrowWithoutToken{
    ...
    */
    ...
    external payable transactionDoesNotExists(scriptHash) nonZeroAddress(_buyer) nonZeroAddress(_seller){
    ...
    */
    function addFundsToTransaction(bytes32 scriptHash)external transactionExists(scriptHash) inFundedState(scriptHash) payable{
    ...
    */  
    function execute(uint8[] sigV, bytes32[] sigR, bytes32[] sigS, bytes32 scriptHash, bytes20 uniqueId, address[] destinations, uint256[] amounts) external  transactionExists(scriptHash) inFundedState(scriptHash){
    ...
    function verifySignatures(uint8[] sigV, bytes32[] sigR, bytes32[] sigS, bytes32 scriptHash, address[] destinations, uint256[]amounts)
      public returns (address lastAddress){
    ...
    */
    function getAllTransactionsForParty(address _partyAddress)external view returns(bytes32[] scriptHashes) {

        return partyVsTransaction[_partyAddress];
    }


    function isTimeLockExpired(uint32 timeoutHours, uint256 lastFunded)public view returns(bool expired){
