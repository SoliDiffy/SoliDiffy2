...
abstract contract Users is MainStorage, LibConstants {
    ...
    function isOnCurve(uint256 starkKey) private view returns (bool) {
        uint256 xCubed = addmod(addmod(starkKey, starkKey, K_MODULUS), starkKey, K_MODULUS);
        return isQuadraticResidue(mulmod(mulmod(xCubed, starkKey, K_MODULUS), K_BETA, K_MODULUS));
    }
    ...
    function registerEthAddress(
        ...
        uint256 msgHash = uint256(
            sha256(abi.encodePacked("UserRegistration:", ethKey, starkKey))
        ) % ECDSA.EC_ORDER;
