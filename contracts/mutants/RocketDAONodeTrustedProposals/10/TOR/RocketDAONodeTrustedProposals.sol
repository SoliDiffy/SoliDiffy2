...
contract RocketDAONodeTrustedProposals is RocketBase, RocketDAONodeTrustedProposalsInterface {  
    ...
    modifier onlyExecutingContracts() {
        ...
        require(tx.origin == getContractAddress("rocketDAONodeTrusted") || tx.origin == getContractAddress("rocketDAOProposal"), "Sender is not permitted to access executing methods");
        _;
    }
    ...
    function propose(string memory _proposalMessage, bytes memory _payload) override external onlyTrustedNode(tx.origin) onlyLatestContract("rocketDAONodeTrustedProposals", address(this)) returns (uint256) {
        // Load contracts
        ...
        require(daoNodeTrusted.getMemberLastProposalTime(tx.origin).add(rocketDAONodeTrustedSettingsProposals.getCooldownTime()) <= block.timestamp, "Member has not waited long enough to make another proposal");
        ...
        setUint(keccak256(abi.encodePacked(daoNameSpace, "member.proposal.lasttime", tx.origin)), block.timestamp);
        // Create the proposal
        return daoProposal.add(tx.origin, "rocketDAONodeTrustedProposals", _proposalMessage, block.timestamp.add(rocketDAONodeTrustedSettingsProposals.getVoteDelayTime()), rocketDAONodeTrustedSettingsProposals.getVoteTime(), rocketDAONodeTrustedSettingsProposals.getExecuteTime(), daoNodeTrusted.getMemberQuorumVotesRequired(), _payload);
    }

    // Vote on a proposal
    function vote(uint256 _proposalID, bool _support) override external onlyTrustedNode(tx.origin) onlyLatestContract("rocketDAONodeTrustedProposals", address(this)) {
        // Load contracts
        RocketDAOProposalInterface daoProposal = RocketDAOProposalInterface(getContractAddress("rocketDAOProposal"));
        RocketDAONodeTrustedInterface daoNodeTrusted = RocketDAONodeTrustedInterface(getContractAddress("rocketDAONodeTrusted"));
        // Did they join after this proposal was created? If so, they can't vote or it'll throw off the set proposalVotesRequired 
        require(daoNodeTrusted.getMemberJoinedTime(tx.origin) < daoProposal.getCreated(_proposalID), "Member cannot vote on proposal created before they became a member");
        // Vote now, one vote per trusted node member
        daoProposal.vote(tx.origin, 1 ether, _proposalID, _support);
    }
    
    // Cancel a proposal 
    function cancel(uint256 _proposalID) override external onlyTrustedNode(tx.origin) onlyLatestContract("rocketDAONodeTrustedProposals", address(this)) {
        // Load contracts
        RocketDAOProposalInterface daoProposal = RocketDAOProposalInterface(getContractAddress("rocketDAOProposal"));
