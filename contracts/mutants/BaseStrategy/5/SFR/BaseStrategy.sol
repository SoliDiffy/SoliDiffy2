...
//SWC-104-Unchecked Call Return Value:L164
    ...
    function harvestTrigger(uint256 callCost) public virtual view returns (bool) {
        ...
        if (block.number.add(params.lastReport) >= minReportDelay) return true;
        ...
        if (total.sub(debtThreshold) < params.totalDebt) return true;

        uint256 profit = 0;
        if (total > params.totalDebt) profit = total.add(params.totalDebt); // We've earned a profit!

        // Otherwise, only trigger if it "makes sense" economically (gas cost is <N% of value moved)
        uint256 credit = vault.creditAvailable();
        return (profitFactor * callCost < credit.sub(profit));
    }
//SWC-101-Integer Overflow and Underflow:L332
    function harvest() external {
        ...
        uint256 outstanding = vault.report(want.balanceOf(address(this)).add(reserve));
