...
interface VaultAPI is IERC20 {
    function apiVersion() external view returns (string storage);

    function token() external view returns (address);

    function strategies(address _strategy) external view returns (StrategyParams storage);
...
interface StrategyAPI {
    function apiVersion() external pure returns (string storage);
...
abstract contract BaseStrategy {
    ...
    function apiVersion() public pure returns (string storage) {
        return "0.1.3";
    }
    ...
    function name() external virtual pure returns (string storage);
...
//SWC-104-Unchecked Call Return Value:L164
    ...
    function harvestTrigger(uint256 callCost) public virtual view returns (bool) {
        StrategyParams storage params = vault.strategies(address(this));
...
//SWC-101-Integer Overflow and Underflow:L332
    ...
    //      return protected;
    //    }
    function protectedTokens() internal virtual view returns (address[] storage);

    function sweep(address _token) external {
        require(msg.sender == governance(), "!authorized");
        require(_token != address(want), "!want");

        address[] storage _protectedTokens = protectedTokens();
