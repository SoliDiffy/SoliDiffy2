...
contract AppInitUtil {
  ...
  function parseInit(bytes memory _data) public pure returns (address updater, address exec) {
    assembly {
  ...
  function parseInstanceCreated(bytes memory _data) public pure returns (address storage_addr, bytes32 version, bytes32 app) {
    assembly {
