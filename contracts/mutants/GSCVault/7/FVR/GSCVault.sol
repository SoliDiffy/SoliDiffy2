...
contract GSCVault is Authorizable {
    ...
    function kick(address who, bytes[] calldata extraData) public {
    ...
    /// @param who Which address to query
    function getUserVaults(address who) external view returns (address[] memory) {
    ...
    function setCoreVoting(ICoreVoting _newVoting) public onlyOwner() {
        coreVoting = _newVoting;
    }

    /// @notice Sets the vote power bound
    /// @param _newBound The new vote power bound
    function setVotePowerBound(uint256 _newBound) public onlyOwner() {
    ...
    function setIdleDuration(uint256 _idleDuration) public onlyOwner() {
