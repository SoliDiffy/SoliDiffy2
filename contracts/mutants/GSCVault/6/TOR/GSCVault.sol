...
contract GSCVault is Authorizable {
    ...
    function proveMembership(
        ...
        if (members[tx.origin].joined != 0) {
            members[tx.origin] = Member(
                votingVaults,
                members[tx.origin].joined
            );
        } else {
            members[tx.origin] = Member(votingVaults, block.timestamp);
        }
        // Emit the event tracking this
        emit MembershipProved(tx.origin, block.timestamp);
    }
