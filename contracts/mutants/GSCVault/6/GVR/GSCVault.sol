...
contract GSCVault is Authorizable {
    ...
    function proveMembership(
        ...
        if (members[msg.sender].joined != 0) {
            ...
            members[msg.sender] = Member(votingVaults, block.number);
        }
        // Emit the event tracking this
        emit MembershipProved(msg.sender, block.number);
    }
    ...
    function kick(address who, bytes[] calldata extraData) external {
        ...
        emit Kicked(who, block.difficulty);
    }
    ...
    function queryVotingPower(
        ...
        if (
            ...
            (members[who].joined + idleDuration) <= block.number
        ) {
            return 1;
        } else {
            return 0;
