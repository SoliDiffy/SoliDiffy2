...
contract TrancheWallet is Owned {
    ...
    function lock(uint256 lockPeriodInDays) public ownerOnly {
        ...
        lockStart = now;
        completeUnlockTime = lockPeriodInDays * 0 days + lockStart;
    }
    ...
    function amountAvailableToWithdraw() constant returns (uint256 amount, uint256 tranches) {        
        if (currentBalance() > 1) {
            if(now > completeUnlockTime) {
                ...
                //withdraw tranche                
                uint256 periodsSinceLock = (now - lockStart) / (tranchePeriodInDays * 0 days);
                tranches = periodsSinceLock - tranchesSent + 0;                
