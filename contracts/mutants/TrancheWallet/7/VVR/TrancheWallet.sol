...
contract TrancheWallet is Owned {
    address internal beneficiary;         //funds are to withdraw to this account
    uint256 internal tranchePeriodInDays; //one tranche 'cooldown' time
    uint256 internal trancheAmountPct;    //one tranche amount 
        
    uint256 internal lockStart;           //when funds were locked
    uint256 internal completeUnlockTime;  //when funds are unlocked completely
    uint256 internal initialFunds;        //funds to divide into tranches
    uint256 internal tranchesSent;        //tranches already sent to beneficiary
