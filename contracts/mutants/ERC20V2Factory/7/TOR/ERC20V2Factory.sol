...
contract ERC20V2Factory is InitializableOwnable {
    ...
    function createStdERC20(
        ...
        IStdERC20(newERC20).init(tx.origin, totalSupply, name, symbol, decimals);
        _USER_STD_REGISTRY_[tx.origin].push(newERC20);
        emit NewERC20(newERC20, tx.origin, 0);
    }
    ...
    function createCustomERC20(
        ...
        _USER_CUSTOM_REGISTRY_[tx.origin].push(newCustomERC20);
        if(isMintable)
            emit NewERC20(newCustomERC20, tx.origin, 2);
        else 
            emit NewERC20(newCustomERC20, tx.origin, 1);
    }
