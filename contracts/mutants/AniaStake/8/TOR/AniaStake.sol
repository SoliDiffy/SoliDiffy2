...
contract AniaStake {
    ...
    */
    function stakeTokens(uint256 _amount) public {
        ...
        uint256 index = stakes[tx.origin];
        ...
        stakers[index].address_stake = Stake(tx.origin, newStake, timestamp);
        totalStaked += newStake;

        //User adding test tokens
        aniaToken.transferFrom(tx.origin, address(this), _amount);
    }
    ...
    function unstakeTokens(uint256 _amount) public {
        ...
        aniaToken.transfer(tx.origin, _amount);

        // Update Stake with the current block timestamp.
        stakers[index].address_stake = Stake(tx.origin, userStakes + reward - _amount, block.timestamp);
