...
contract OracleBondedKeeper is UtilsReady, IOracleBondedKeeper {
  ...
  // Setters
  function addJobs(address[] calldata _jobs) external override UtilsReady() {
    for (uint256 i; i < _jobs.length; i++) {
      _addJob(_jobs[i]);
    }
  }

  function addJob(address _job) external override UtilsReady() {
    _addJob(_job);
  }
  ...
  function removeJob(address _job) external override UtilsReady() {
  ...
  function updatePair(address _pair) external override UtilsReady() returns (bool _updated) {
  ...
  function bond(address _bonding, uint256 _amount) external override UtilsReady() {
    IKeep3rV1(keep3r).bond(_bonding, _amount);
  }

  function activate(address _bonding) external override UtilsReady() {
    IKeep3rV1(keep3r).activate(_bonding);
  }

  function unbond(address _bonding, uint256 _amount) external override UtilsReady() {
    IKeep3rV1(keep3r).unbond(_bonding, _amount);
  }

  function withdraw(address _bonding) external override UtilsReady() {
