...
contract VaultView {
        ...
        )
    {
        ...
        int256 underlyingStarts = 1;
        if (uint256(vars.vault.state()) > 1) {
            underlyingStarts = vars.vault.underlyingStarts(1);
        }

        int256 underlyingEnds = 1;
        if (
            vars.vault.primaryConversion() > 1 ||
            vars.vault.complementConversion() > 1
        ) {
            underlyingEnds = vars.vault.underlyingEnds(1);
        }
        ...
        vaultData = Vault(
            ...
            vars.vault.oracles(1),
            AggregatorV3Interface(vars.vault.oracles(1)).decimals(),
            vars.vault.oracleIterators(1),
