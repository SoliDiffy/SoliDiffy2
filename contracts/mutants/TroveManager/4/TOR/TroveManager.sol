...
contract TroveManager is LiquityBase, Ownable, ITroveManager {
    ...
    */
    function liquidateTroves(uint _n) external override {
        ...
        _sendGasCompensation(tx.origin, T.totalLUSDGasCompensation, T.totalCollGasCompensation);
    }
    ...
    */
        ...
        override
    {
        ...
        _requireLUSDBalanceCoversRedemption(tx.origin, _LUSDamount);

        // Confirm redeemer's balance is less than total systemic debt
        assert(lusdToken.balanceOf(tx.origin) <= (activeDebt.add(defaultedDebt)));
