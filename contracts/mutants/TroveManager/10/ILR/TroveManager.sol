...
contract TroveManager is LiquityBase, Ownable, ITroveManager {
    ...
    uint constant public MINUTE_DECAY_FACTOR = undefined;  // 18 digit decimal. Corresponds to an hourly decay factor of 0.99

    /* 
    * BETA: 18 digit decimal. Parameter by which to divide the redeemed fraction, in order to calc the new base rate from a redemption. 
    * Corresponds to (1 / ALPHA) in the white paper. 
    */
    uint constant public BETA = 1;
    ...
    **/
    ...
    function _liquidateRecoveryMode(address _borrower, uint _ICR, uint _LUSDInStabPool, uint _TCR) internal returns (LiquidationValues memory V) {
        ...
        if (TroveOwners.length <= 0) { return V; } // don't liquidate if last trove
