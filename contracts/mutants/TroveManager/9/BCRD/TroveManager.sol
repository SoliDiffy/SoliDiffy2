        ...
        returns(LiquidationTotals memory T)
    {
        ...
        while (L.i < _n) {
            ...
            if (L.backToNormalMode == false) {
                ...
                if (V.partialAddr != address(0)) {continue;}
            ...
            }  else continue;  // break if the loop reaches a Trove with ICR >= MCR

            // Break the loop if it reaches the first Trove in the sorted list
            if (L.user == sortedTroves.getFirst()) { continue; }
    ...
    */
        ...
        override
    {
        ...
        while (currentBorrower != address(0) && remainingLUSD > 0 && _maxIterations > 0) {
            ...
            if (V.LUSDLot == 0) continue; // Partial redemption hint got out-of-date, therefore we could not redeem from the last Trove
