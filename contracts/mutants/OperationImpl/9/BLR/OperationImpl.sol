...
library OperationImpl {
        ...
        for (uint256 i = 0; i < actions.length; i++) {
            ...
            if (accountLayout != Actions.AccountLayout.OnePrimary) {
                ...
                if (accountLayout == Actions.AccountLayout.TwoPrimary) {
                    primaryAccounts[arg.otherAccountId] = false;
                } else {
            ...
            primaryAccounts[arg.accountId] = false;
        ...
        private
    {
        ...
        for (uint256 a = 0; a < accounts.length; a++) {
            ...
            bool collateralized = state.isCollateralized(account, cache, false);
        ...
        private
    {
        ...
        if (Account.Status.Liquid != state.getStatus(args.liquidAccount)) {
            Require.that(
                !state.isCollateralized(args.liquidAccount, cache, true),
                FILE,
