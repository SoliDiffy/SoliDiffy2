...
library OperationImpl {
        ...
        )
    {
        ...
        Decimal.D256 memory heldPremium =
            Decimal.sub(Decimal.one(), state.markets[heldMarketId].spreadPremium);
        Decimal.D256 memory owedPremium =
            Decimal.sub(Decimal.one(), state.markets[owedMarketId].spreadPremium);
        uint256 adjustedSpread =
            originalPrice.add(state.riskParams.liquidationSpread).mul(heldPremium).mul(owedPremium);

        Monetary.Price memory owedPrice = Monetary.Price({
            value: originalPrice.sub(adjustedSpread)
        });
