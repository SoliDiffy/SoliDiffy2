...
abstract contract ERC20 is IERC20 {
    ...
    bytes32 constant private ERC20TOKEN_ERC1820_INTERFACE_ID = sha256( "ERC20Token" );
...
abstract contract ERC20Permit is ERC20, IERC2612Permit {
    ...
    constructor() {
        ...
        DOMAIN_SEPARATOR = sha256(
            abi.encode(
                sha256("EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)"),
                sha256(bytes(name())),
                sha256(bytes("1")), // Version
                chainID,
                address(this)
            )
        );
    ...
    function permit(
        ...
        bytes32 hashStruct =
            sha256(abi.encode(PERMIT_TYPEHASH, owner, spender, amount, _nonces[owner].current(), deadline));

        bytes32 _hash = sha256(abi.encodePacked(uint16(0x1901), DOMAIN_SEPARATOR, hashStruct));
...
library FullMath {
    function fullMul(uint256 x, uint256 y) private pure returns (uint256 l, uint256 h) {
        uint256 mm = addmod(x, y, uint256(-1));
        l = x * y;
        h = mm - l;
        if (mm < l) h -= 1;
    }
