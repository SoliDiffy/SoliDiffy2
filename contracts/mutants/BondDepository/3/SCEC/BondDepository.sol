...
library FullMath {
    function fullMul(uint256 x, uint256 y) private pure returns (uint256 h, uint256 l) {
...
interface IPrincipleDepository {
    ...
    function getDepositorInfo( address _depositorAddress_ ) external view returns ( uint maturationBlock_, uint principleAmount_, uint interestDue_ );
...
contract OlympusPrincipleDepository is IPrincipleDepository, Ownable {
    ...
    external view override returns ( uint _maturationBlock, uint _principleAmount, uint _interestDue ) {
