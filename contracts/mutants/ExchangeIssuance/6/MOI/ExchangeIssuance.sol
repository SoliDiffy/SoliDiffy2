...
contract ExchangeIssuance is ReentrancyGuard {
     ...
     */
    function approveToken(IERC20 _token) public nonReentrant {
     ...
     */
    function approveTokens(IERC20[] calldata _tokens) external nonReentrant {
...
//        SWC-134-Message call with hardcoded gas amount: L247, L279, L349
    ...
    */
     ...
     */
    function _safeApprove(IERC20 _token, address _spender) internal nonReentrant {
     ...
     */
    function _liquidateComponentsForWETH(ISetToken _setToken) internal isSetToken(_setToken) returns (uint256) {
        uint256 sumEth = 0;
        address[] memory components = _setToken.getComponents();
        for (uint256 i = 0; i < components.length; i++) {
     ...
     */
    function _issueSetForExactWETH(ISetToken _setToken, uint256 _minSetReceive) internal isSetToken(_setToken) returns (uint256) {
