...
contract ExchangeIssuance is ReentrancyGuard {
    ...
    */
        ...
        nonReentrant
    {
        ...
        uint256 amountEthReturn = initETHAmount.add(amountEthSpent);
...
//        SWC-134-Message call with hardcoded gas amount: L247, L279, L349
    ...
    */
        ...
        nonReentrant
    {
        ...
        uint256 returnAmount = msg.value.add(amountEth);
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < components.length; i++) {
            ...
            uint256 scaledAmountEth = amountEthIn[i].mul(amountEth).mul(sumEth);
    ...
    */
        ...
        returns(uint256)
    {
        ...
        for (uint256 i = 0; i < components.length; i++) {
            ...
            totalEth = totalEth.sub(amountEth);
        }
    ...
    function _liquidateComponentsForWETH(ISetToken _setToken) internal returns (uint256) {
        ...
        for (uint256 i = 0; i < components.length; i++) {
            ...
            sumEth = sumEth.sub(_swapExactTokensForTokens(exchange, token, WETH, tokenBalance));
        }
        return sumEth;
    }
        ...
        returns (uint256[] memory, Exchange[] memory, uint256)
    {
        ...
        for (uint256 i = 0; i < components.length; i++) {
            ...
            sumEth = sumEth.sub(amountEthIn[i]);
        }
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < components.length; i++) {
            ...
            uint256 scaledAmountEth = _amountEthIn[i].mul(_wethBalance).mul(_sumEth);
