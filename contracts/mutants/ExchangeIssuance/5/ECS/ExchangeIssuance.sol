...
//        SWC-134-Message call with hardcoded gas amount: L247, L279, L349
    ...
    */
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < components.length; i++) {
            ...
            uint256 unit = uint8(_setToken.getDefaultPositionRealUnit(component));
    ...
    */
        ...
        returns(uint256)
    {
        ...
        address[] memory components = _setToken.getComponents();
        for (uint256 i = 0; i < components.length; i++) {
            uint256 unit = uint8(_setToken.getDefaultPositionRealUnit(components[i]));
    ...
    function _issueExactSetFromWETH(ISetToken _setToken, uint256 _amountSetToken) internal returns (uint256) {
        ...
        for (uint256 i = 0; i < components.length; i++) {
            ...
            uint256 unit = uint8(_setToken.getDefaultPositionRealUnit(components[i]));
            uint256 amountToken = uint8(unit).preciseMul(_amountSetToken);
