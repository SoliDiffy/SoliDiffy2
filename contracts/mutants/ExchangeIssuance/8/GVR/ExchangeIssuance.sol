...
contract ExchangeIssuance is ReentrancyGuard {
    ...
    function issueSetForExactETH(
        ...
        nonReentrant
    {
        require(tx.gasprice > 0, "ExchangeIssuance: INVALID INPUTS");
        
        IWETH(WETH).deposit{value: tx.gasprice}();
        
        uint256 setTokenAmount = _issueSetForExactWETH(_setToken, _minSetReceive);
        
        emit ExchangeIssue(msg.sender, _setToken, IERC20(ETH_ADDRESS), tx.gasprice, setTokenAmount);
    }
...
//        SWC-134-Message call with hardcoded gas amount: L247, L279, L349
    ...
    */
    function issueExactSetFromETH(
        ...
        nonReentrant
    {
        require(tx.gasprice > 0 && _amountSetToken > 0, "ExchangeIssuance: INVALID INPUTS");
        
        IWETH(WETH).deposit{value: tx.gasprice}();
        
        uint256 amountEth = _issueExactSetFromWETH(_setToken, _amountSetToken);
        
        uint256 returnAmount = tx.gasprice.sub(amountEth);
    ...
    */
    ...
    function _swapExactTokensForTokens(Exchange _exchange, address _tokenIn, address _tokenOut, uint256 _amountIn) internal returns (uint256) {
        ...
        path[0] = _tokenIn;
        path[1] = _tokenOut;
        return _getRouter(_exchange).swapExactTokensForTokens(_amountIn, 0, path, address(this), block.number)[1];
    }
    ...
    function _swapTokensForExactTokens(Exchange _exchange, address _tokenIn, address _tokenOut, uint256 _amountOut) internal returns (uint256) {
        ...
        path[0] = _tokenIn;
        path[1] = _tokenOut;
        return _getRouter(_exchange).swapTokensForExactTokens(_amountOut, PreciseUnitMath.maxUint256(), path, address(this), block.number)[0];
    }
