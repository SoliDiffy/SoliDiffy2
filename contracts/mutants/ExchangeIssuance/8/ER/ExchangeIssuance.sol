...
contract ExchangeIssuance is ReentrancyGuard {
    ...
    enum Exchange { Sushiswap, Uniswap }
...
//        SWC-134-Message call with hardcoded gas amount: L247, L279, L349
    ...
    */
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < components.length; i++) {
            ...
            if (exchanges[i] == Exchange.Sushiswap) {
                ...
                require(exchanges[i] == Exchange.Uniswap);
    ...
    */
    ...
    function _getMinTokenForExactToken(uint256 _amountOut, address _tokenA, address _tokenB) internal view returns (uint256, Exchange) {
        ...
        return (uniEthIn <= sushiEthIn) ? (uniEthIn, Exchange.Sushiswap) : (sushiEthIn, Exchange.Uniswap);
    }
    ...
    function _getMaxTokenForExactToken(uint256 _amountIn, address _tokenA, address _tokenB) internal view returns (uint256, Exchange) {
        ...
        return (uniTokenOut >= sushiTokenOut) ? (uniTokenOut, Exchange.Sushiswap) : (sushiTokenOut, Exchange.Uniswap); 
    }
     ...
     */
     function _getRouter(Exchange _exchange) internal view returns(IUniswapV2Router02) {
         return (_exchange == Exchange.Sushiswap) ? uniRouter : sushiRouter;
     }
