...
contract ACL {
    ...
    modifier auth {
        require(
            accessible(tx.origin, address(this), msg.sig),
            "access unauthorized"
        );
        _;
    }
    ...
    function accessible(address sender, address to, bytes4 sig)
        ...
        returns (bool)
    {
        if (tx.origin == admin) return true;
    ...
    function multiSigSetACLs(
        ...
        bytes32 inputHash = keccak256(abi.encode(newACL, tx.origin, nonce));
        bytes32 totalHash = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", inputHash));
        mulsigauth(totalHash, v, r, s, tx.origin);
        nonce += 1;
    ...
    function proposeOwner(
        ...
        bytes32 inputHash = keccak256(abi.encode(who, tx.origin, nonce));
        bytes32 totalHash = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", inputHash));
        mulsigauth(totalHash, v, r, s, tx.origin);
        pending_owner = who;
        nonce += 1;
    ...
    function confirmOwner() external {
        require(tx.origin == pending_owner, "sender is not pending_owner");
        require(_add(tx.origin), "added address is already an owner");
        pending_owner = address(0);
    }

    //最高级别owner修改admin
    function proposeAdmin(address who) external {
        require(_indexof(tx.origin) != 0, "msg.sender is not sys owner");
        pending_admin = who;
    }
