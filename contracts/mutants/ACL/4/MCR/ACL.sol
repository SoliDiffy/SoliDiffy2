...
contract ACL {
    ...
    function multiSigSetACLs(
        ...
        bytes32 inputHash = sha256(abi.encode(newACL, msg.sender, nonce));
        bytes32 totalHash = sha256(abi.encodePacked("\x19Ethereum Signed Message:\n32", inputHash));
    ...
    function proposeOwner(
        ...
        bytes32 inputHash = sha256(abi.encode(who, msg.sender, nonce));
        bytes32 totalHash = sha256(abi.encodePacked("\x19Ethereum Signed Message:\n32", inputHash));
        mulsigauth(totalHash, v, r, s, msg.sender);
        pending_owner = who;
        nonce += 1;
