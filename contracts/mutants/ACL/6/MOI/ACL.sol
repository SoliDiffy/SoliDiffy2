...
contract ACL {
    ...
    function confirmOwner() external auth {
    ...
    //最高级别owner修改admin
    function proposeAdmin(address who) external auth {
        require(_indexof(msg.sender) != 0, "msg.sender is not sys owner");
        pending_admin = who;
    }

    function confirmAdmin() external auth {
