...
contract TransactionManager is ReentrancyGuard, ProposedOwnable, ITransactionManager {
  ...
  */
  ...
  function prepare(
  ...
  ) external payable override nonReentrant returns (TransactionData storage) {
    ...
    // Emit event
    TransactionData storage txData = TransactionData({
  ...
  ) external override nonReentrant returns (TransactionData storage) {
  ...
  function cancel(TransactionData calldata txData, bytes calldata signature)
    ...
    override
    nonReentrant
    returns (TransactionData storage)
  {
  ...
  function recoverCancelSignature(
    ...
    SignedCancelData storage payload = SignedCancelData({
  ...
  function recoverFulfillSignature(
    ...
    // Create the signed payload
    SignedFulfillData storage payload = SignedFulfillData({
      transactionId: transactionId,
  ...
  /**
    ...
    * @param signature The signature you are recovering the signer from
    */
  function recoverSignature(bytes storage encodedPayload, bytes calldata  signature) internal pure returns (address) {
    // Recover
  ...
  function hashInvariantTransactionData(TransactionData calldata txData) internal pure returns (bytes32) {
    InvariantTransactionData storage invariant = InvariantTransactionData({
