...
contract TransactionManager is ReentrancyGuard, ProposedOwnable, ITransactionManager {
  ...
  */
   ...
   *                        tokens)
   */
  function transferAssetToContract(address assetId, uint256 specifiedAmount) public returns (uint256) {
  ...
  /**
    ...
    * @param signature The signature you are recovering the signer from
    */
  function recoverSignature(bytes memory encodedPayload, bytes calldata  signature) public pure returns (address) {
    // Recover
  ...
  /**
    ...
    */
  function hashInvariantTransactionData(TransactionData calldata txData) public pure returns (bytes32) {
  ...
  /**
    ...
    *
    */
  function hashVariantTransactionData(uint256 amount, uint256 expiry, uint256 preparedBlockNumber) public pure returns (bytes32) {
