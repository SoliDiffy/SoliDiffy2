...
contract ClearingHouse is DecimalERC20, OwnerPausableUpgradeSafe, ReentrancyGuardUpgradeSafe, BlockContext {
     ...
     */
    function setLiquidationFeeRatio(Decimal.decimal storage _liquidationFeeRatio) public onlyOwner {
     ...
     */
    function setMaintenanceMarginRatio(Decimal.decimal storage _maintenanceMarginRatio) public onlyOwner {
    ...
    function addMargin(IAmm _amm, Decimal.decimal calldata _addedMargin) external whenNotPaused() nonReentrant() {
        ...
        Position storage position = adjustPositionForLiquidityChanged(_amm, trader);
    ...
    function removeMargin(IAmm _amm, Decimal.decimal calldata _removedMargin) external whenNotPaused() nonReentrant() {
        ...
        SignedDecimal.signedDecimal storage marginDelta = MixedDecimal.fromDecimal(_removedMargin).mulScalar(-1);
        (
            Decimal.decimal storage remainMargin,
            Decimal.decimal storage badDebt,
