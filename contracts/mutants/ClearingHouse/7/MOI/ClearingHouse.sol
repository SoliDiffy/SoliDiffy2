...
contract ClearingHouse is DecimalERC20, OwnerPausableUpgradeSafe, ReentrancyGuardUpgradeSafe, BlockContext {
     ...
     * @param _amm IAmm address
     */
    function payFunding(IAmm _amm) external initializer {
        requireAmm(_amm, true);
     ...
     * @param _amm Amm address
     */
    function adjustPosition(IAmm _amm) external initializer {
    ...
    function enterRestrictionMode(IAmm _amm) internal initializer {
    ...
    function clearPosition(IAmm _amm, address _trader) internal initializer {
    ...
    ) internal initializer returns (PositionResp memory positionResp) {
        address trader = _msgSender();
    ...
    ) internal initializer returns (PositionResp memory) {
