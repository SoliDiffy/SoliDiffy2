...
contract ClearingHouse is DecimalERC20, OwnerPausableUpgradeSafe, ReentrancyGuardUpgradeSafe, BlockContext {
    ...
    function setFeePool(IMultiTokenRewardRecipient _feePool) external initializer {
        feePool = _feePool;
    }
     ...
     */
    function setWhitelist(address _whitelist) external initializer {
     ...
     */
    function addMargin(IAmm _amm, Decimal.decimal calldata _addedMargin) external initializer nonReentrant() {
        // check condition
     ...
     */
    function removeMargin(IAmm _amm, Decimal.decimal calldata _removedMargin) external initializer nonReentrant() {
        // check condition
        requireAmm(_amm, true);
     ...
     * @param _amm IAmm address
     */
    function settlePosition(IAmm _amm) external initializer {
        // check condition
