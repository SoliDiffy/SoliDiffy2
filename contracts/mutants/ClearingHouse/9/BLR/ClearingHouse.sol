...
contract ClearingHouse is DecimalERC20, OwnerPausableUpgradeSafe, ReentrancyGuardUpgradeSafe, BlockContext {
    ...
    function removeMargin(IAmm _amm, Decimal.decimal calldata _removedMargin) external whenNotPaused() nonReentrant() {
        ...
        // check margin ratio
        requireMoreMarginRatio(getMarginRatio(_amm, trader), initMarginRatio, false);
    ...
    function openPosition(
        ...
        requireMoreMarginRatio(MixedDecimal.fromDecimal(Decimal.one()).divD(_leverage), initMarginRatio, false);
        ...
        PositionResp memory positionResp;
        {
            ...
            bool isNewPosition = oldPositionSize == 0 ? false : true;
            if (!isNewPosition) {
                requireMoreMarginRatio(getMarginRatio(_amm, trader), maintenanceMarginRatio, false);
            }
