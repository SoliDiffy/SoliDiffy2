...
//            }
    ...
    returns (bytes)
    {
        ...
        if(tx.origin == contractor || tx.origin == eth_address) {
            flag = true;
        } else {
    ...
    public
    {
        require(tx.origin == twoKeyAcquisitionCampaign);
...
//    }
    ...
    {
        require(ITwoKeyMaintainersRegistry(twoKeyMaintainersRegistry).onlyMaintainer(tx.origin));
    ...
    returns (uint,uint,uint,uint[])
    {
        if(_referrer != address(0)) {
            require(tx.origin == _referrer || tx.origin == contractor || ITwoKeyMaintainersRegistry(twoKeyMaintainersRegistry).onlyMaintainer(tx.origin));
