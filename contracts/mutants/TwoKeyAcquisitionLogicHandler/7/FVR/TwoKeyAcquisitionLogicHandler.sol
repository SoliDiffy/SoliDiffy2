...
contract TwoKeyAcquisitionLogicHandler is UpgradeableCampaign, TwoKeyCampaignIncentiveModels {
    ...
    function checkHowMuchUserCanSpend(uint alreadySpentETHWei, uint alreadySpentFiatWEI) external view returns (uint) {
    ...
    function canConversionBeCreated(address converter, uint amountWillingToSpend, bool isFiat) external view returns (bool) {
    ...
    function canMakeETHConversion(address converter, uint amountWillingToSpendEthWei) external view returns (bool,uint) {
        uint alreadySpentETHWei;
        uint alreadySpentFIATWEI;
        (alreadySpentETHWei,alreadySpentFIATWEI,) = ITwoKeyConversionHandler(twoKeyConversionHandler).getConverterPurchasesStats(converter);
