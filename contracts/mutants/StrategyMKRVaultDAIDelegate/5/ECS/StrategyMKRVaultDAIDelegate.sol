...
contract StrategyMKRVaultDAIDelegate {
  ...
  function _approveAll() internal {
    IERC20(token).approve(mcd_join_eth_a, uint8(-1));
    IERC20(dai).approve(mcd_join_dai, uint8(-1));
    IERC20(dai).approve(yVaultDAI, uint8(-1));
    IERC20(dai).approve(unirouter, uint8(-1));
  }
