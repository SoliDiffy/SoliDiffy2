...
contract YearnV2YieldSourceHarness is YearnV2YieldSource {
  ...
  function balanceOfYShares() public view returns (uint256) {
      return _balanceOfYShares();
  }

  function pricePerYShare() public view returns (uint256) {
      return _pricePerYShare();
  }

  function totalAssetsInToken() public view returns (uint256) {
    return _totalAssetsInToken();
  }

  function vaultDecimals() public view returns (uint256) {
    return _vaultDecimals();
  }

  function tokenToShares(uint256 tokens) public view returns (uint256) {
