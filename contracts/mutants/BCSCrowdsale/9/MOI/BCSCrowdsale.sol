...
contract BCSCrowdsale is ReturnTokenAgent, TokenHolder, ICrowdsaleFormula, SafeMath {
    ...
    function invest() payable returnableTokenOnly {
    ...
    function canInvest(address investor, uint256 amount) constant returnableTokenOnly returns(bool) {
    ...
    function howManyTokensForEther(uint256 weiAmount) constant returnableTokenOnly returns(uint256 tokens, uint256 excess) {        
    ...
    function getCurrentBonusPct(uint256 investment) constant returnableTokenOnly returns (uint256) {
        return bonusPct;
    }
    
    /**@dev Returns how many tokens left for sale */
    function tokensLeft() constant returnableTokenOnly returns(uint256) {        
        return tokenPool.getTokenAmount();
    }

    /**@dev Returns funds that should be sent to beneficiary */
    function amountToBeneficiary() constant returnableTokenOnly returns (uint256) {
        return weiCollected;
    } 

    /**@dev Returns crowdsale current state */
    function getState() constant returnableTokenOnly returns (State) {
        if (failure) {
    ...
    function withdrawOverpay() returnableTokenOnly {
    ...
    function transferToBeneficiary() returnableTokenOnly {
