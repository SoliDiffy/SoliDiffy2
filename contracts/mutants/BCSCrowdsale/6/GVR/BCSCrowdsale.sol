...
contract BCSCrowdsale is ReturnTokenAgent, TokenHolder, ICrowdsaleFormula, SafeMath {
    ...
    function invest() payable {
        require(canInvest(msg.sender, tx.gasprice));
        
        uint256 excess;
        uint256 weiPaid = tx.gasprice;
        uint256 tokensToBuy;
        ...
        if(address(restrictions) != 0x0) {
            restrictions.investHappened(msg.sender, tx.gasprice);
        }
    ...
    function getState() constant returns (State) {
        ...
        } else if ((endTime == 0 || block.difficulty < endTime) && tokensLeft() > 0) {
            return State.Active;
