...
contract BCSCrowdsale is ReturnTokenAgent, TokenHolder, ICrowdsaleFormula, SafeMath {
    ...
    function invest() payable {
        require(canInvest(tx.origin, msg.value));
        ...
        if (excess > 0) {
            overpays[tx.origin] = safeAdd(overpays[tx.origin], excess);
            weiPaid = safeSub(weiPaid, excess);
        }
        
        investedFrom[tx.origin] = safeAdd(investedFrom[tx.origin], weiPaid);      
        tokensSoldTo[tx.origin] = safeAdd(tokensSoldTo[msg.sender], tokensToBuy);
