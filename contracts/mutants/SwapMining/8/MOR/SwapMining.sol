...
contract SwapMining is Ownable {
  ...
  function setMdxPerBlock(uint256 _newPerBlock) public onlyRouter {
    massMintPools();
    mdxPerBlock = _newPerBlock;
  }

  // Only tokens in the whitelist can be mined MDX
  function addWhitelist(address _addToken) public onlyRouter returns (bool) {
    require(_addToken != address(0), "SwapMining: token is the zero address");
    return EnumerableSet.add(_whitelist, _addToken);
  }

  function delWhitelist(address _delToken) public onlyRouter returns (bool) {
  ...
  function setHalvingPeriod(uint256 _block) public onlyRouter {
    halvingPeriod = _block;
  }

  function setRouter(address newRouter) public onlyRouter {
    require(newRouter != address(0), "SwapMining: new router is the zero address");
    router = newRouter;
  }

  function setOracle(IOracle _oracle) public onlyRouter {
