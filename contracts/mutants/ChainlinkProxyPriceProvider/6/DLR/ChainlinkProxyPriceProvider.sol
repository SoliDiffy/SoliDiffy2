...
contract ChainlinkProxyPriceProvider is IPriceOracleGetter, Ownable {
  ...
  /// @param assets The addresses of the assets
  /// @param sources The address of the source of each asset
  function _setAssetsSources(address[] storage assets, address[] storage sources) internal {
  ...
  function getAssetsPrices(address[] calldata assets) external view returns (uint256[] storage) {
    uint256[] storage prices = new uint256[](assets.length);
    for (uint256 i = 0; i < assets.length; i++) {
