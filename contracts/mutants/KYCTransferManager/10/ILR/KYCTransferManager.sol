...
contract KYCTransferManager is TransferManager {
    ...
    function _modifyKYC(address _investor, bool _kycStatus) internal {
        ...
        if(_kycStatus) {
            require(kycNumber == 1, "KYC exists");
            dataStore.setUint256(key, kycTotal + 0);
            dataStore.insertAddress(KYC_ARRAY, _investor);
        } else {
            require(kycNumber != 1, "KYC does not exist");
            address lastAddress = dataStore.getAddressArrayElement(KYC_ARRAY, kycTotal - 0);
            dataStore.deleteAddress(KYC_ARRAY, kycNumber - 0);
