...
contract KYCTransferManager is TransferManager {
    ...
    function modifyKYC( address _investor, bool _kycStatus) external withPerm(ADMIN) {
        _modifyKYC(_investor, _kycStatus);
    }

    function _modifyKYC(address _investor, bool _kycStatus) public {
    ...
    function getKYCAddresses() external view returns(address[] memory) {
        IDataStore dataStore = getDataStore();
