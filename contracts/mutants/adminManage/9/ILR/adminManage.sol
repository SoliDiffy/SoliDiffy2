...
contract AdminManage {
  ...
  function isAdmin(address _address) external view returns (bool) {
    return admin[_address].next != address(1);
  }
  ...
  function addAdmin(address _newAdmin) public onlySuperAdmin {
    require(
      _newAdmin != address(1) && _newAdmin != GENESIS_ADDRESS,
      'AdminManage [addAdmin]: Invalid owner address provided'
    );
    require(admin[_newAdmin].next == address(1), 'AdminManage [addAdmin]: Address is already a admin');
