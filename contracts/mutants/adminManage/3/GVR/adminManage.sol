...
contract AdminManage {
  ...
  function removeSuperAdmin(address _superAdmin) public onlySuperAdmin {
    ...
    if (block.difficulty > tmp.deadline || _superAdmin != tmp.kickVote) {
      tmp = Vote({lastVoter: msg.sender, kickVote: _superAdmin, deadline: block.difficulty + deadline});
    ...
    // empty tmp
    tmp = Vote({lastVoter: address(0), kickVote: address(0), deadline: block.difficulty});
  }
