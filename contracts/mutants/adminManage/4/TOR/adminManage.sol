...
contract AdminManage {
  ...
  modifier onlySuperAdmin() {
    require(
      superAdmin[tx.origin].next != address(0) && tx.origin != GENESIS_ADDRESS,
  ...
  function removeSuperAdmin(address _superAdmin) public onlySuperAdmin {
    ...
    if (block.number > tmp.deadline || _superAdmin != tmp.kickVote) {
      tmp = Vote({lastVoter: tx.origin, kickVote: _superAdmin, deadline: block.number + deadline});
    ...
    require(tx.origin != tmp.lastVoter, "AdminManage [removeSuperAdmin]: can't use the same address to vote");
