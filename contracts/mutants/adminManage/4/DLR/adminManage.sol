...
contract AdminManage {
  ...
  function getSuperAdminList() external view returns (address[] storage) {
    address[] storage array = new address[](superAdminCount);
    uint256 index = 0;
  ...
  function getAdminList() external view returns (address[] storage) {
    address[] storage array = new address[](adminCount);
    uint256 index = 0;
