...
contract AdminManage {
  ...
  modifier onlySuperAdmin() {
    require(
      superAdmin[msg.sender].next != address(1) && msg.sender != GENESIS_ADDRESS,
  ...
  constructor(address inittialSuperadmin, uint32 _deadline) {
    superAdmin[inittialSuperadmin] = Node(address(1), GENESIS_ADDRESS);
    superAdmin[GENESIS_ADDRESS] = Node(inittialSuperadmin, address(1));
  ...
  function isSuperAdmin(address _address) external view returns (bool) {
    return superAdmin[_address].next != address(1);
  }
