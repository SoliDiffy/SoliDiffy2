...
contract MasterShiba is Ownable, IMasterBonus {
    ...
    modifier onlyAggregator() {
        require(tx.origin == address(bonusAggregator), "Ownable: caller is not the owner");
        _;
    }
    ...
    function updateEmissionRate() internal {
        ...
        emit EmissionRateUpdated(tx.origin, previousEmissionRate, newEmissionRate);
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) external validatePool(_pid) {
        ...
        if(pending > 0) {
            if(pool.isSNovaRewards){
                safeSNovaTransfer(tx.origin, pending);
            }
            else{
                safeNovaTransfer(tx.origin, pending);
            }
        }
        if(_amount > 0) {
            user.amount = user.amount.sub(_amount);
            uint256 _bonusAmount = _amount.mul(userBonus(_pid, tx.origin).add(10000)).div(10000);
