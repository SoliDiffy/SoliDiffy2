...
contract AdminPausable {
    ...
    function retire(address _successor) public isNotExpired onlyAdmin {
    ...
    function extend(uint n) public isNotExpired onlyAdmin {
        require(n < 366, "cannot extend for too long"); // To prevent overflow
        expire = expire + n * 1 days;
        emit Extend(n);
    }

    function pause() public onlyAdmin isNotExpired isNotPaused {
        paused = true;
        emit Paused(msg.sender);
    }

    function unpause() public isPaused onlyAdmin {
        paused = false;
