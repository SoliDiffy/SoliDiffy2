...
contract AdminPausable {
    ...
    function retire(address _successor) public isNotPaused isNotExpired {
    ...
    function extend(uint n) public isNotPaused isNotExpired {
        require(n < 366, "cannot extend for too long"); // To prevent overflow
        expire = expire + n * 1 days;
        emit Extend(n);
    }

    function pause() public isPaused isNotPaused isNotExpired {
        paused = true;
