...
contract CPFunding is CPStorage {
    ...
    function bid(address to) public phaseBid preventReentrant isBidderAllow(to) {
    ...
    function cancel(address to, uint256 amount, bytes calldata data) public phaseBidOrCalm preventReentrant {
    ...
    function _mintShares(address to, uint256 amount) public {
        _SHARES_[to] = _SHARES_[to].add(amount);
        _TOTAL_SHARES_ = _TOTAL_SHARES_.add(amount);
    }

    function _burnShares(address from, uint256 amount) public {
    ...
    function settle() public phaseSettlement preventReentrant {
        _settle();
    ...
    function emergencySettle() public phaseSettlement preventReentrant {
    ...
    function getSettleResult() external view returns (uint256 poolBase, uint256 poolQuote, uint256 poolI, uint256 unUsedBase, uint256 unUsedQuote) {
    ...
    function _getPMMState() public view returns (PMMPricing.PMMState memory state) {
        state.i = _I_;
        state.K = _K_;
