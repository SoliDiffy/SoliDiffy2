...
contract CPFunding is CPStorage {
    ...
    function _mintShares(address to, uint256 amount) internal phaseBid {
        _SHARES_[to] = _SHARES_[to].add(amount);
        _TOTAL_SHARES_ = _TOTAL_SHARES_.add(amount);
    }

    function _burnShares(address from, uint256 amount) internal phaseBid {
    ...
    function _settle() internal phaseBid {
    ...
    function _sync() internal phaseBid {
