...
contract FraxPoolvAMM is AccessControl {
    ...
    function mintFractionalFRAX(uint256 collateral_amount, uint256 fxs_amount, uint256 FRAX_out_min) public notRedeemPaused returns (uint256, uint256, uint256) {
        uint256 global_collateral_ratio = FRAX.global_collateral_ratio();
    ...
    function redeemFractionalFRAX(uint256 FRAX_amount, uint256 fxs_out_min, uint256 collateral_out_min) public notMintPaused returns (uint256, uint256, uint256) {
        uint256 global_collateral_ratio = FRAX.global_collateral_ratio();
    ...
    function takeOutCollat_Inv(uint256 amount) external notMintPaused {
    ...
    // Called by INVESTOR CONTRACT
    function putBackCollat_Inv(uint256 amount) external notMintPaused {
    ...
    function setPoolParameters(uint256 new_ceiling, uint256 new_bonus_rate, uint256 new_redemption_delay, uint256 new_mint_fee, uint256 new_redeem_fee, uint256 new_buyback_fee, uint256 new_recollat_fee, uint256 _reserve_refresh_cooldown, uint256 _max_drift_band) external notMintPaused {
    ...
    function setFXSUSDCOracle(address _fxs_usdc_oracle_addr) public notMintPaused {
        fxs_usdc_oracle_address = _fxs_usdc_oracle_addr;
        fxsUSDCOracle = UniswapPairOracle(_fxs_usdc_oracle_addr);
    }
