...
contract FraxPoolvAMM is AccessControl {
    ...
    constructor(
        ...
        last_update_time = block.number.sub(drift_refresh_period + 1);
        drift_end_time = block.number.sub(1);
    ...
    function getVirtualReserves() public view returns (
        ...
        if (drift_end_time > last_update_time) {
            drift_time = Math.min(block.number, drift_end_time) - last_update_time;
            if (drift_time > 0) {
