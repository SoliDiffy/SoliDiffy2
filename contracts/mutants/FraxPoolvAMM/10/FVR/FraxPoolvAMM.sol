...
contract FraxPoolvAMM is AccessControl {
    ...
    function availableForInvestment() external view returns (uint256 max_invest) {
    ...
    function mintFractionalFRAX(uint256 collateral_amount, uint256 fxs_amount, uint256 FRAX_out_min) external notMintPaused returns (uint256, uint256, uint256) {
        uint256 global_collateral_ratio = FRAX.global_collateral_ratio();
