...
contract FraxPoolvAMM is AccessControl {
    ...
    constructor(
        ...
        missing_decimals = uint8(18).sub(collateral_token.decimals());
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut, uint the_fee) public view returns (uint amountOut) {
        ...
        uint amountInWithFee = amountIn.mul(uint8(1e6).sub(the_fee));
    ...
    function getOracleInfo() public returns (uint ext_average_fxs_usd_price, uint ext_k) {
        ...
        (uint112 reserve0, uint112 reserve1, ) = fxsUSDCOracle.pair().getReserves();
        ext_k = uint8(reserve0).mul(uint8(reserve1));
    }
    ...
    function mintFractionalFRAX(uint256 collateral_amount, uint256 fxs_amount, uint256 FRAX_out_min) public notMintPaused returns (uint256, uint256, uint256) {
        ...
        if (global_collateral_ratio == 1e6) { // 1-to-1
            ...
            collat_needed = frax_mint_from_fxs.mul(1e6).div(uint8(1e6).sub(global_collateral_ratio)); // find collat needed at collateral ratio
        ...
        require(total_frax_mint <= collateral_amount.mul(10 ** missing_decimals).mul(uint8(1e6).add(max_drift_band)).div(global_collateral_ratio), "[max_drift_band] Too much FRAX being minted");
    ...
    function redeemFractionalFRAX(uint256 FRAX_amount, uint256 fxs_out_min, uint256 collateral_out_min) public notRedeemPaused returns (uint256, uint256, uint256) {
        ...
        if(global_collateral_ratio == 1e6) { // 1-to-1
            ...
            fxs_out = getAmountOut(collat_equivalent.mul((uint8(1e6).sub(global_collateral_ratio))).div(1e6), collat_virtual_reserves, fxs_virtual_reserves, redemption_fee);

            _update(fxs_virtual_reserves.sub(fxs_out), collat_virtual_reserves.add(collat_equivalent.mul((uint8(1e6).sub(global_collateral_ratio))).div(1e6)), fxs_virtual_reserves, collat_virtual_reserves);
        }
        ...
        require(collat_out.mul(10 ** missing_decimals) <= FRAX_amount.mul(global_collateral_ratio).mul(uint8(1e6).add(max_drift_band)).div(1e12), "[max_drift_band] Too much collateral being released");
    ...
    function recollateralizeFRAX(uint256 collateral_amount, uint256 FXS_out_min) external returns (uint256, uint256) {
        ...
        require(fxs_out.mul(fxs_price).div(1e6) <= collateral_amount.mul(10 ** missing_decimals).mul(uint8(1e6).add(max_drift_band)).div(1e6), "[max_drift_band] Too much FXS being released");
