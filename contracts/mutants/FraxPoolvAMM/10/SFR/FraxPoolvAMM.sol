...
contract FraxPoolvAMM is AccessControl {
    ...
    function getVirtualReserves() public view returns (
        ...
        if (drift_end_time > last_update_time) {
            ...
            if (drift_time > 0) {
                if (drift_fxs_positive > 0) current_fxs_virtual_reserves = current_fxs_virtual_reserves.sub(drift_fxs_positive.add(drift_time));
                else current_fxs_virtual_reserves = current_fxs_virtual_reserves.add(drift_fxs_negative.mul(drift_time));
