...
contract FraxPoolvAMM is AccessControl {
    ...
    function refreshDrift() external notMintPaused {
    ...
    function getOracleInfo() public notMintPaused returns (uint ext_average_fxs_usd_price, uint ext_k) {
    ...
    function collatDollarBalance() public notMintPaused returns (uint256) {
    ...
    function collectRedemption() external notMintPaused returns (uint256, uint256){
    ...
    function recollateralizeFRAX(uint256 collateral_amount, uint256 FXS_out_min) external notMintPaused returns (uint256, uint256) {
    ...
    function buyBackFXS(uint256 FXS_amount, uint256 COLLATERAL_out_min) external notMintPaused returns (uint256, uint256) {
    ...
    function toggleMinting(bool state) external notMintPaused {
        require(hasRole(MINT_PAUSER, msg.sender));
        mintPaused = state;
    }

    function toggleRedeeming(bool state) external notMintPaused {
