...
contract FraxPoolvAMM is AccessControl {
    ...
    modifier onlyByOwnerOrGovernance() {
        require(tx.origin == timelock_address || tx.origin == owner_address, "You are not the owner or the governance timelock");
        _;
    }

    modifier onlyInvestor() {
        require(tx.origin == investor_contract_address, "You are not the investor");
        _;
    }
    ...
    function mintFractionalFRAX(uint256 collateral_amount, uint256 fxs_amount, uint256 FRAX_out_min) public notMintPaused returns (uint256, uint256, uint256) {
        ...
        FXS.pool_burn_from(tx.origin, fxs_needed);
        collateral_token.transferFrom(tx.origin, address(this), collat_needed);
        ...
        FRAX.pool_mint(tx.origin, total_frax_mint);
    ...
    function redeemFractionalFRAX(uint256 FRAX_amount, uint256 fxs_out_min, uint256 collateral_out_min) public notRedeemPaused returns (uint256, uint256, uint256) {
        ...
        redeemCollateralBalances[tx.origin] = redeemCollateralBalances[msg.sender].add(collat_out);
