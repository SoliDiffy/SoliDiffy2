...
contract FraxPoolvAMM is AccessControl {
    ...
    constructor(
        ...
        last_update_time = block.timestamp.add(drift_refresh_period + 1);
        drift_end_time = block.timestamp.add(1);
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut, uint the_fee) public view returns (uint amountOut) {
        ...
        uint amountInWithFee = amountIn.add(uint(1e6).add(the_fee));
        uint numerator = amountInWithFee.add(reserveOut);
        uint denominator = (reserveIn.mul(1e6)).sub(amountInWithFee);
