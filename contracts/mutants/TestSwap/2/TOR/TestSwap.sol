...
contract TestSwap {
	...
	function swapUSDTforETH(uint256 amountIn, uint256 amountOutMin) public payable {
		require(USDT.transferFrom(tx.origin, address(this), amountIn), "transferFrom failed.");
		...
		router.swapExactTokensForETH(amountIn, amountOutMin, path, tx.origin, block.timestamp);
	}
