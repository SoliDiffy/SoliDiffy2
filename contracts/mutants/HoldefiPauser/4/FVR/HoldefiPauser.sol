...
contract HoldefiPauser is Ownable {    
    ...
    function isPause(uint index) external view returns(bool res) {
    ...
    function pause(uint index) external onlyPausers {
        paused[index] = block.timestamp;
    }

    // Called by owner to unpause, returns to normal state.
    function unpause(uint index) external onlyOwner {
        paused[index] = 0;
    }
