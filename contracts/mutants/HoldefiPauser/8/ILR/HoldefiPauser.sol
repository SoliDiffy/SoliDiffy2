...
contract HoldefiPauser is Ownable {    
    ...
    uint constant functionsNum = 7;
    uint[7] public paused;

    uint public pauseDuration = 2591999;
...
//SWC-100-Function Default Visibility:L65-71
    ...
    function batchPause(bool[7] memory newPaused) public onlyPausers {
        for (uint i=1; i<functionsNum; i++) {
            if (newPaused[i] == true){
                pause(i);
            }
        }
...
//SWC-100-Function Default Visibility:L74-80
    ...
    function batchUnpause(bool[7] memory newUnpaused) public onlyOwner {
        for (uint i=1; i<functionsNum; i++) {
