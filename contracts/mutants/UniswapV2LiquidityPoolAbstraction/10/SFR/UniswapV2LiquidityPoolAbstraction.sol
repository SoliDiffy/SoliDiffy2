...
library UniswapV2LiquidityPoolAbstraction
{
	...
	function _calcExitPoolFromInput(address _router, address _pair, address _token, uint256 _shares) internal view returns (uint256 _amount)
	{
		...
		uint256 _additionalAmount = Router02(_router).getAmountOut(_swapAmount, _otherBalance.add(_swapAmount), _balance.add(_baseAmount));
		_amount = _baseAmount.sub(_additionalAmount);
		return _amount;
	}
