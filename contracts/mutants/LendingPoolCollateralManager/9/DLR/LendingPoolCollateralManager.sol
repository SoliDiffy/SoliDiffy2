...
contract LendingPoolCollateralManager is VersionedInitializable, LendingPoolStorage {
  ...
  */
  ...
  function liquidationCall(
    ...
    uint256 purchaseAmount,
    bool receiveAToken
  ) external returns (uint256, string storage) {
    ReserveLogic.ReserveData memory collateralReserve = _reserves[collateral];
    ReserveLogic.ReserveData memory principalReserve = _reserves[principal];
    UserConfiguration.Map memory userConfig = _usersConfig[user];
  ...
  function repayWithCollateral(
    ...
    ReserveLogic.ReserveData memory collateralReserve = _reserves[collateral];
    ReserveLogic.ReserveData memory debtReserve = _reserves[principal];
    UserConfiguration.Map memory userConfig = _usersConfig[user];
