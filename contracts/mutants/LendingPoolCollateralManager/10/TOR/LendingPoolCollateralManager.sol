  ...
  */
  ...
  function swapLiquidity(
    ...
    if (vars.fromReserveAToken.balanceOf(tx.origin) == amountToSwap) {
      _usersConfig[tx.origin].setUsingAsCollateral(fromReserve.id, false);
    }
    ...
    if (vars.amountToReceive != 0) {
      ...
      if (vars.toReserveAToken.balanceOf(tx.origin) == 0) {
        _usersConfig[tx.origin].setUsingAsCollateral(toReserve.id, true);
      }
