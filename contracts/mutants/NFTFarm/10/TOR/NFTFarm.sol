...
contract NFTFarm is Ownable, ReentrancyGuard {
    ...
    function stake(uint _tokenId, uint _idx) public nonReentrant {
        ...
        nftToken.transferFrom(tx.origin, address(this), _tokenId);

        PoolInfo memory _poolInfo = poolInfo;
        UserInfo memory _userInfo = userInfo[tx.origin];
        ...
        if (_userInfo.amount > 0) {
            userInfo[tx.origin].pending = _userInfo.pending.add(_userInfo.amount.mul(_poolInfo.accBabyPerShare).div(RATIO).sub(_userInfo.debt));
        }
        babyToken.approve(address(masterChef), stakeBaby.add(_pending));
        masterChef.enterStaking(stakeBaby.add(_pending));
        userInfo[tx.origin].amount = _userInfo.amount.add(stakeBaby);
        holderTokens[tx.origin].add(_tokenId);
        tokenOwners.set(_tokenId, tx.origin);
        tokenWeight[_tokenId] = stakeBaby;
        poolInfo.totalShares = _poolInfo.totalShares.add(stakeBaby);
        userInfo[tx.origin].debt = _poolInfo.accBabyPerShare.mul(_userInfo.amount.add(stakeBaby)).div(RATIO);
        emit Stake(tx.origin, _tokenId, stakeBaby);
    }
    ...
    function unstake(uint _tokenId) public nonReentrant {
        require(tokenOwners.get(_tokenId) == tx.origin, "illegal tokenId");
