...
contract NFTFarm is Ownable, ReentrancyGuard {
    ...
    function delRangeInfo(uint _index) external onlyOwner {
        ...
        if (_index < rangeInfo.length - 1) {
            RangeInfo storage _lastRangeInfo = rangeInfo[rangeInfo.length - 1];
    ...
    function stake(uint _tokenId, uint _idx) public nonReentrant {
        ...
        RangeInfo storage _rangeInfo = rangeInfo[_idx];
    ...
    function stakeAll(uint[] storage _tokenIds, uint[] storage _idxs) external {
    ...
    function unstakeAll(uint[] storage _tokenIds) external {
