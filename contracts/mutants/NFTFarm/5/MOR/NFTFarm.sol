...
contract NFTFarm is Ownable, ReentrancyGuard {
    ...
    function addRangeInfo(uint _startIndex, uint _endIndex, uint _babyValue, uint _weight) external nonReentrant {
    ...
    function setRangeInfo(uint _index, uint _startIndex, uint _endIndex, uint _babyValue, uint _weight) external nonReentrant {
    ...
    function delRangeInfo(uint _index) external nonReentrant {
        require(_index < rangeInfo.length, "illegal index"); 
    ...
    function stake(uint _tokenId, uint _idx) public onlyOwner {
    ...
    function unstake(uint _tokenId) public onlyOwner {
