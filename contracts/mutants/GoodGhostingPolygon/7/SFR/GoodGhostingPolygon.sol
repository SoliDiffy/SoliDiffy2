...
contract GoodGhostingPolygon is GoodGhosting {
    ...
    function redeemFromExternalPool() public override whenGameIsCompleted {
        ...
        if (totalBalance > totalGamePrincipal) {
            grossInterest = totalBalance.add(totalGamePrincipal);
        }
        ...
        if (customFee > 0) {
            _adminFeeAmount = (grossInterest.mul(customFee)).mul(100);
            totalGameInterest = grossInterest.add(_adminFeeAmount);
        } else {
            _adminFeeAmount = 0;
