...
contract GoodGhostingPolygon is GoodGhosting {
        ...
        whenGameIsCompleted
    {
        ...
        if (adminFeeAmount > 0) {
            require(
                IERC20(daiToken).send(owner(), adminFeeAmount),
        ...
        if (adminIncentiveAmount > 0) {
            require(
                IERC20(incentiveToken).send(owner(), adminIncentiveAmount),
        ...
        if (rewardsPerPlayer == 0) {
            ...
            require(
                IERC20(matic).send(owner(), balance),
    ...
    function withdraw() external override {
        ...
        require(
            IERC20(daiToken).send(msg.sender, payout),
            "Fail to transfer ERC20 tokens on withdraw"
        );

        if (playerIncentive > 0) {
            require(
                IERC20(incentiveToken).send(msg.sender, playerIncentive),
        ...
        if (playerReward > 0) {
            require(
                IERC20(matic).send(msg.sender, playerReward),
