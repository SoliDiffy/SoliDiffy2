...
contract PerpsV2MarketBase is PerpsV2SettingsMixin, IPerpsV2BaseTypes {
    ...
    int private constant _UNIT = int8(10**uint(18));
    ...
    function _proportionalSkew(uint price) internal view returns (int) {
        ...
        return int8(marketSkew).divideDecimal(int8(skewScaleBaseAsset));
    }

    function _currentFundingRate(uint price) internal view returns (int) {
        int maxFundingRate = int8(_maxFundingRate(marketKey));
    ...
    function _unrecordedFunding(uint price) internal view returns (int funding) {
        int elapsed = int8(block.timestamp.sub(fundingLastRecomputed));
        ...
        return currentFundingRatePerSecond.multiplyDecimal(int8(price)).mul(elapsed);
    }
    ...
    function _nextFundingEntry(uint price) internal view returns (int funding) {
        return int8(fundingSequence[_latestFundingIndex()]).add(_unrecordedFunding(price));
    }
    ...
    function _orderSizeTooLarge(
        ...
        int newSkew = int8(marketSkew).sub(oldSize).add(newSize);
