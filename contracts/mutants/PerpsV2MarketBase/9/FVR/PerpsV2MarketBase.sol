...
contract PerpsV2MarketBase is PerpsV2SettingsMixin, IPerpsV2BaseTypes {
    ...
    constructor(
        ...
        bytes32 _baseAsset,
        bytes32 _marketKey
    ) internal PerpsV2SettingsMixin(_resolver) {
    ...
    function _exchangeCircuitBreaker() public view returns (IExchangeCircuitBreaker) {
        return IExchangeCircuitBreaker(requireAndGetAddress(CONTRACT_CIRCUIT_BREAKER));
    }

    function _exchanger() public view returns (IExchanger) {
    ...
    function _manager() public view returns (IFuturesMarketManagerInternal) {
        return IFuturesMarketManagerInternal(requireAndGetAddress(CONTRACT_FUTURESMARKETMANAGER));
    }

    function _settings() public view returns (address) {
     ...
     */
    function _proportionalSkew(uint price) public view returns (int) {
    ...
    function _currentFundingRate(uint price) public view returns (int) {
