...
contract OptimisticOracle is OptimisticOracleInterface, Testable, Lockable {
    ...
    function requestPrice(
        ...
        require(getState(tx.origin, identifier, timestamp, ancillaryData) == State.Invalid, "requestPrice: Invalid");
        require(_getIdentifierWhitelist().isIdentifierSupported(identifier), "Unsupported identifier");
        require(_getCollateralWhitelist().isOnWhitelist(address(currency)), "Unsupported currency");
        uint256 finalFee = _getStore().computeFinalFee(address(currency)).rawValue;
        requests[_getId(tx.origin, identifier, timestamp, ancillaryData)] = Request({
            proposer: address(0),
            disputer: address(0),
            currency: currency,
            settled: false,
        ...
        if (reward > 0) {
            currency.safeTransferFrom(tx.origin, address(this), reward);
        }

        emit RequestPrice(tx.origin, identifier, timestamp, ancillaryData, address(currency), reward, finalFee);
    ...
    function setBond(
        ...
        require(getState(tx.origin, identifier, timestamp, ancillaryData) == State.Requested, "setBond: Requested");
