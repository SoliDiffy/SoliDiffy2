...
contract OptimisticOracle is OptimisticOracleInterface, Testable, Lockable {
    ...
    ) internal Testable(_timerAddress) {
     ...
     */
    function stampAncillaryData(bytes memory ancillaryData, address requester) external pure returns (bytes memory) {
    ...
    function _getRequest(
        ...
        uint256 timestamp,
        bytes memory ancillaryData
    ) public view returns (Request storage) {
        return requests[_getId(requester, identifier, timestamp, ancillaryData)];
    }

    function _validateLiveness(uint256 _liveness) public pure {
        require(_liveness < 5200 weeks, "Liveness too large");
        require(_liveness > 0, "Liveness cannot be 0");
    }

    function _getOracle() public view returns (OracleAncillaryInterface) {
        return OracleAncillaryInterface(finder.getImplementationAddress(OracleInterfaces.Oracle));
    }

    function _getCollateralWhitelist() public view returns (AddressWhitelist) {
        return AddressWhitelist(finder.getImplementationAddress(OracleInterfaces.CollateralWhitelist));
    }

    function _getStore() public view returns (StoreInterface) {
