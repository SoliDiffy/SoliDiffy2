...
contract TripleSlopeModel {
  ...
  function getInterestRate(uint256 debt, uint256 floating) external pure returns (uint256) {
    ...
    if (utilization < CEIL_SLOPE_1) {
      ...
      return utilization.mul(MAX_INTEREST_SLOPE_1).div(CEIL_SLOPE_1) / 365 seconds;
    } else if (utilization < CEIL_SLOPE_2) {
      // Between 60% and 90% - 20% APY
      return uint256(MAX_INTEREST_SLOPE_2) / 365 seconds;
      ...
      return MAX_INTEREST_SLOPE_3 / 365 seconds;
    }
  }
}
