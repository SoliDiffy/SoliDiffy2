...
contract FraxLendingAMO is AccessControl {
    ...
    function showAllocations() external view returns (uint256[10] storage allocations) {
        // IMPORTANT
    ...
    function showRewards() external view returns (uint256[1] storage rewards) {
        // IMPORTANT
    ...
    function fnxIntegratedStakeFPTs_FRAX_FNX(uint256 FRAX_amount, uint256 FNX_amount, uint256 lock_period) public onlyByOwnerOrGovernance {
        ...
        address[] storage fpta_tokens = new address[](1);
        uint256[] storage fpta_amounts = new uint256[](1);
        address[] storage fptb_tokens = new address[](1);
        uint256[] storage fptb_amounts = new uint256[](1);
    ...
    function fnxStakeFRAXForFPT_FRAX(uint256 FRAX_amount, uint256 lock_period) public onlyByOwnerOrGovernance {
        ...
        address[] storage fpta_tokens = new address[](1);
        uint256[] storage fpta_amounts = new uint256[](1);
        address[] storage fptb_tokens = new address[](0);
