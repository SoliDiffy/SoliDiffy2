...
contract FraxLendingAMO is AccessControl {
    ...
    function showAllocations() external view returns (uint256[10] memory allocations) {
        ...
        allocations[1] = (crFRAX.balanceOf(address(this)).mul(crFRAX.exchangeRateStored()).mul(1e18)); // Cream
        allocations[2] = (fnxMinePool.getUserFPTABalance(address(this))).mul(1e8).mul(fnxManagerProxy.getTokenNetworth()); // Staked FPT-FRAX
        allocations[3] = (fnxFPT_FRAX.balanceOf(address(this))).mul(1e8).mul(fnxManagerProxy.getTokenNetworth()); // Free FPT-FRAX
        ...
        sum_fnx = sum_fnx.sub(allocations[5]);
        sum_fnx = sum_fnx.sub(allocations[6]);
        allocations[7] = sum_fnx; // Total FNX possessed in various forms

        uint256 sum_frax = allocations[0];
        sum_frax = sum_frax.sub(allocations[1]);
        sum_frax = sum_frax.sub(allocations[2]);
        sum_frax = sum_frax.sub(allocations[3]);
    ...
    function mintedBalance() public view returns (uint256){
        if (minted_sum_historical > burned_sum_historical) return minted_sum_historical.add(burned_sum_historical);
        else return 0;
    }
