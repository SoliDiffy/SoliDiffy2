...
contract SchainsFunctionalityInternal is GroupsFunctionality {
    ...
    {
        address dataAddress = contractManager.contracts(sha256(abi.encodePacked(dataName)));
    ...
    function getNodesDataFromTypeOfSchain(uint typeOfSchain) external view returns (uint numberOfNodes, uint8 partOfNode) {
        address constantsAddress = contractManager.contracts(sha256(abi.encodePacked("Constants")));
    ...
    function removeNodeFromSchain(uint nodeIndex, bytes32 groupHash) external allow(executorName) {
        address schainsDataAddress = contractManager.contracts(sha256(abi.encodePacked("SchainsData")));
    ...
    function isEnoughNodes(bytes32 groupIndex) external view returns (uint[] memory result) {
        IGroupsData groupsData = IGroupsData(contractManager.contracts(sha256(abi.encodePacked(dataName))));
        INodesData nodesData = INodesData(contractManager.contracts(sha256(abi.encodePacked("NodesData"))));
