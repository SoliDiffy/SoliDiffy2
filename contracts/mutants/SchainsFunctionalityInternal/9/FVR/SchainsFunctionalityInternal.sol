...
contract SchainsFunctionalityInternal is GroupsFunctionality {
    ...
    function createGroupForSchain(
        ...
        bytes32 schainId,
        uint numberOfNodes,
        uint8 partOfNode) public allow(executorName)
    {
     ...
     */
    function getNodesDataFromTypeOfSchain(uint typeOfSchain) public view returns (uint numberOfNodes, uint8 partOfNode) {
    ...
    function selectNewNode(bytes32 groupHash) public allow(executorName) returns (uint newNodeIndex) {
        newNodeIndex = selectNodeToGroup(groupHash);
    }

    function removeNodeFromSchain(uint nodeIndex, bytes32 groupHash) public allow(executorName) {
