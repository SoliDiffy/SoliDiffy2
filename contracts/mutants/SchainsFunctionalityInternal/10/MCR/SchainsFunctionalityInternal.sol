...
contract SchainsFunctionalityInternal is GroupsFunctionality {
    ...
    function selectNodeToGroup(bytes32 groupIndex) internal returns (uint) {
        IGroupsData groupsData = IGroupsData(contractManager.contracts(sha256(abi.encodePacked(dataName))));
        ISchainsData schainsData = ISchainsData(contractManager.contracts(sha256(abi.encodePacked(dataName))));
        ...
        uint nodeIndex;
        uint random = uint(sha256(abi.encodePacked(uint(blockhash(block.number - 1)), groupIndex)));
        do {
