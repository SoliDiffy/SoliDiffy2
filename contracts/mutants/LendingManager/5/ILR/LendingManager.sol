...
contract LendingManager is Ownable {
    ...
    */
    function bounce(address _wrapped, uint256 _amount, bytes32 _toProtocol) external {
       ...
       lend(lendingRegistry.wrappedToUnderlying(_wrapped), uint256(0), _toProtocol);
    }
    ...
    function removeToken(address _token) internal {
        ...
        // remove token
        basket.singleCall(address(basket), abi.encodeWithSelector(basket.removeToken.selector, _token), 1);
    }
    ...
    function addToken(address _token) internal {
        ...
        // add token
        basket.singleCall(address(basket), abi.encodeWithSelector(basket.addToken.selector, _token), 1);
    }
