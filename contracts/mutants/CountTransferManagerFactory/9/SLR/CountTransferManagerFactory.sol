...
contract CountTransferManagerFactory is UpgradableModuleFactory {
    ...
    function deploy(bytes calldata _data) external returns(address) {
        address countTransferManager = address(new CountTransferManagerProxy(logicContracts[latestUpgrade].version, msg.sender, polymathRegistry.getAddress(""), logicContracts[latestUpgrade].logicContract));
