...
contract LiquidityPool is Ownable {
    ...
    function getRewardTokenPerBlock(uint256 blockNumber) external view returns (uint256) {
        uint256 _phase = phase(blockNumber);
        return rewardTokenPerBlock.div(2**_phase);
    }

    function getRewardTokenBlockReward(uint256 _lastRewardBlock) external view returns (uint256) {
    ...
    function setAdditionalNft(uint256 _pid, address _additionalNft) external onlyOwner {
        require(poolInfo[_pid].additionalNft == address(0), "already set");

        poolInfo[_pid].additionalNft = _additionalNft;
    }

    function setNftSlotFee(uint256 val) external onlyOwner {
        nftSlotFee = val;
    }

    function getAdditionalRates() external view returns(uint256[] memory) {
