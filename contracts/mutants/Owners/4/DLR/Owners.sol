...
function getOwnerByIndex(uint256 index) public view  returns( address) {
        ...
        Sowners storage p= _Sowners[ index ];
...
function getAllOwners() public view returns(uint256[] storage, address[] storage ) {
  
    uint256[] storage indexs=new uint256[](_SownersCount);
