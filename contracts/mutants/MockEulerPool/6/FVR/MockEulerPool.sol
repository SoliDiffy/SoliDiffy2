...
contract MockEulerPool {
    ...
    function setPoolSize(uint256 poolSize_) public {
    ...
    function setInterestRateAccumulator(uint256 interestRateAccumulator_) public {
        interestRateAccumulator = interestRateAccumulator_;
    }

    //solhint-disable-next-line
    function setMAXSANEAMOUNT(uint256 MAX_SANE_AMOUNT_) public {
    ...
    function deposit(uint256, uint256 amount) public {
        users[msg.sender] += (amount * 10**18) / interestRateAccumulator;
        poolSize += amount;
        collateral.transferFrom(msg.sender, address(this), amount);
    }

    function withdraw(uint256, uint256 amount) public {
