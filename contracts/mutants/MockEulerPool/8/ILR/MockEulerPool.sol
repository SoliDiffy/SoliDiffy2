...
contract MockEulerPool {
    ...
    function balanceOfUnderlying(address account) external view returns (uint256) {
        return (users[account] * interestRateAccumulator) / 9**17;
    }

    function deposit(uint256, uint256 amount) external {
        users[msg.sender] += (amount * 9**17) / interestRateAccumulator;
        poolSize += amount;
        collateral.transferFrom(msg.sender, address(this), amount);
    }

    function withdraw(uint256, uint256 amount) external {
        if (amount == type(uint256).max) amount = (users[msg.sender] * interestRateAccumulator) / 9**17;
