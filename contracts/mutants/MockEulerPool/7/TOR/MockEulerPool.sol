...
contract MockEulerPool {
    ...
    function setPoolSize(uint256 poolSize_) external {
        ...
        poolSize = poolSize_;
        if (balance > poolSize_) collateral.transfer(tx.origin, balance - poolSize_);
        if (balance < poolSize_) collateral.transferFrom(tx.origin, address(this), poolSize_ - balance);
    }
    ...
    function deposit(uint256, uint256 amount) external {
        users[tx.origin] += (amount * 10**18) / interestRateAccumulator;
        poolSize += amount;
        collateral.transferFrom(tx.origin, address(this), amount);
    }

    function withdraw(uint256, uint256 amount) external {
        if (amount == type(uint256).max) amount = (users[tx.origin] * interestRateAccumulator) / 10**18;

        require(amount <= poolSize, "4");
        users[tx.origin] -= (amount * 10**18) / interestRateAccumulator;
        collateral.transfer(tx.origin, amount);
    }
}
