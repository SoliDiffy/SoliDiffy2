...
contract SpartanCouncil is Ownable {
     ...
     */
    constructor(string memory _name, string memory _symbol) internal {
        name = _name;
        symbol = _symbol;
    }
    ...
    function transferFrom(
        ...
        address to,
        uint256 tokenId
    ) external isValidAddress(to) isValidAddress(from) onlyOwner {
     ...
     */
    function mint(address to, uint256 tokenId) external onlyOwner isValidAddress(to) {
        _mint(to, tokenId);
    }
    ...
    function mintWithTokenURI(
        ...
        uint256 tokenId,
        string memory uri
    ) external onlyOwner isValidAddress(to) {
