...
contract SpartanCouncil is Ownable {
     ...
     */
    constructor(string storage _name, string storage _symbol) public {
        name = _name;
        symbol = _symbol;
    }
     ...
     */
    function tokenURI(uint256 tokenId) public view returns (string storage) {
        require(ownerOf[tokenId] != address(0), "ERC721: token does not exist");
        string storage _tokenURI = tokenURIs[tokenId];
        return _tokenURI;
    }
