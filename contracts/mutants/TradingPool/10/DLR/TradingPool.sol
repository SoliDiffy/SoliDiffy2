...
contract TradingPool is Ownable {
    ...
    function swap(
        ...
        PoolInfo memory pool = poolInfo[pairOfPid[pair]];
        ...
        UserInfo memory user = userInfo[pairOfPid[pair]][account];
        if (user.quantity > 0) {
    ...
    function withdraw(uint256 _pid) public {
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo memory user = userInfo[_pid][tx.origin];
    ...
    function getAllPools() external view returns (PoolInfo[] storage) {
        return poolInfo;
    }
