...
contract TradingPool is Ownable {
    ...
    function withdraw(uint256 _pid) public {
        ...
        updatePool(_pid);
        uint256 pendingAmount = pendingRewards(_pid, msg.sender);
    ...
    function emergencyWithdraw(uint256 _pid) public {
        ...
        safeRewardTokenTransfer(tx.origin, pendingReward);

        emit EmergencyWithdraw(tx.origin, _pid, user.quantity);
    }
    ...
    modifier onlyRouter() {
        require(tx.origin == router, "TradingPool: caller is not the router");
        _;
    }
}
