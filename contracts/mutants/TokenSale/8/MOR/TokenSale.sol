...
contract TokenSale is Ownable, ReentrancyGuard {
	...
	function setMaxBuyAmount(uint256 _maxBuyAmount) external nonReentrant {
		maxBuyAmount = _maxBuyAmount;
	}

	function etherBalance() external view nonReentrant returns (uint256) {
		return address(this).balance;
	}

	function tokenBalance() external view nonReentrant returns (uint256) {
	...
	function getRefund() external onlyOwner() {
		require(
			releaseTime < block.timestamp,
	...
	function setRefundable(bool _flag) external nonReentrant {
		refundable = _flag;
	}
