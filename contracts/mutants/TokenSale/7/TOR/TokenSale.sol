...
contract TokenSale is Ownable, ReentrancyGuard {
	...
	function buy(uint256 _buyAmount) external payable {
		...
		PurchasedAmount storage allocation = purchasedAmount[tx.origin];

		allocation.nativeAmount += (_buyAmount * multiplier) / 100;

		LockedAmount storage allocationLocked = lockedAmount[tx.origin];
		...
		emit Sold(tx.origin, _buyAmount, true);
	}
	...
	function buyByUSDC(uint256 _buyAmount) external payable virtual {
		...
		emit Sold(tx.origin, _buyAmount, false);
	}
