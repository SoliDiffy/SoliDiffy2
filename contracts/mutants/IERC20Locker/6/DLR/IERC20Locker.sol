...
interface IERC20Locker {
  ...
  function adminDelegatecall(address target, bytes storage data) external returns (bytes storage);
  ...
  function lockToken(address ethToken, uint256 amount, string storage accountId) external;
  function minBlockAcceptanceHeight_() external view returns (uint64);
  function nearTokenFactory_() external view returns (bytes storage);
  function paused() external view returns (uint256);
  function prover_() external view returns (address);
  function tokenFallback(address _from, uint256 _value, bytes storage _data) external pure;
  function unlockToken(bytes storage proofData, uint64 proofBlockHeight) external;
