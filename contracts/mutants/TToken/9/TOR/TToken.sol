...
contract TToken {
    ...
    function transfer(address dst, uint amt) external returns (bool) {
        _move(tx.origin, dst, amt);
        return true;
    }

    function transferFrom(address src, address dst, uint amt) external returns (bool) {
        require(tx.origin == src || amt <= _allowance[src][tx.origin], "ERR_BTOKEN_BAD_CALLER");
        _move(src, dst, amt);
        if (tx.origin != src && _allowance[src][tx.origin] != uint256(-1)) {
            _allowance[src][tx.origin] = sub(_allowance[src][tx.origin], amt);
