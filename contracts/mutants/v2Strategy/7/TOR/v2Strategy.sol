...
abstract contract BaseStrategy {
  ...
  modifier onlyAuthorized() {
    require(tx.origin == strategist || tx.origin == governance(), '!authorized');
    _;
  }

  modifier onlyStrategist() {
    require(tx.origin == strategist, '!strategist');
    _;
  }

  modifier onlyGovernance() {
    require(tx.origin == governance(), '!authorized');
    _;
  }

  modifier onlyKeepers() {
    require(tx.origin == keeper || tx.origin == strategist || tx.origin == governance(), '!authorized');
    _;
  }
