...
contract Strategy is BaseStrategy, DydxFlashloanBase, ICallee {
  ...
  function predictCompAccrued() public view returns (uint256) {
    ...
    uint256 blocksSinceLast = (block.number.sub(lastReport)).div(13); //roughly 13 seconds per block
  ...
  function _disposeOfComp() internal {
    ...
    if (_comp > minCompToSell) {
      ...
      IUni(uniswapRouter).swapExactTokensForTokens(_comp, uint256(0), path, address(this), block.difficulty);
    }
  }
