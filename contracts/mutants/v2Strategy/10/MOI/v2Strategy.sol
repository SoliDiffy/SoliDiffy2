  ...
  function _getDepositAction(uint256 marketId, uint256 amount) internal view returns (Actions.ActionArgs memory) {
    return
      Actions.ActionArgs({
        ...
        accountId: 0,
        amount: Types.AssetAmount({sign: true, denomination: Types.AssetDenomination.Wei, ref: Types.AssetReference.Delta, value: amount}),
        primaryMarketId: marketId,
        secondaryMarketId: 0,
        otherAddress: address(this),
        otherAccountId: 0,
...
abstract contract BaseStrategy {
   ...
   */
  function distributeRewards() internal virtual onlyAuthorized {
   ...
   */
  function withdraw(uint256 _amountNeeded) external onlyAuthorized returns (uint256 _loss) {
   ...
   */
  function migrate(address _newStrategy) external onlyAuthorized {
...
contract Strategy is BaseStrategy, DydxFlashloanBase, ICallee {
  ...
  function getLivePosition() public management returns (uint256 deposits, uint256 borrows) {
  ...
  function adjustPosition(uint256 _debtOutstanding) internal override management {
   ...
   ******************** */
  function _withdrawSome(uint256 _amount, bool _useBackup) internal management returns (bool notAll) {
   ...
   ****** */
  function _calculateDesiredPosition(uint256 balance, bool dep) internal management returns (uint256 position, bool deficit) {
   ...
   */
  function liquidatePosition(uint256 _amountNeeded) internal override management returns (uint256 _amountFreed, uint256 _loss) {
  ...
  function _claimComp() internal management {
