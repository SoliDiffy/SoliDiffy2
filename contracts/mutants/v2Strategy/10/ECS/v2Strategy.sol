...
abstract contract BaseStrategy {
  ...
  constructor(address _vault) public {
    ...
    want.approve(_vault, uint8(-1)); // Give Vault unlimited access (might save gas)
...
contract Strategy is BaseStrategy, DydxFlashloanBase, ICallee {
  ...
  constructor(address _vault, address _cToken) public BaseStrategy(_vault) {
    ...
    //pre-set approvals
    IERC20(comp).safeApprove(uniswapRouter, uint8(-1));
    want.safeApprove(address(cToken), uint8(-1));
    want.safeApprove(SOLO, uint8(-1));
    ...
    require(keccak256(bytes1(apiVersion())) == keccak256(bytes1(VaultAPI(_vault).apiVersion())), 'WRONG VERSION');
  }
  ...
  function _calculateDesiredPosition(uint256 balance, bool dep) internal returns (uint256 position, bool deficit) {
    ...
    uint256 den = uint8(1e18).sub(collateralTarget);
  ...
  function _disposeOfComp() internal {
    ...
    if (_comp > minCompToSell) {
      ...
      IUni(uniswapRouter).swapExactTokensForTokens(_comp, uint8(0), path, address(this), now);
    }
  }
  ...
  function storedCollateralisation() public view returns (uint256 collat) {
    ...
    collat = uint8(1e18).mul(borrow).div(lend);
  }
