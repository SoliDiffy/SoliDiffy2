  ...
  function _getCallAction(bytes memory data) internal view returns (Actions.ActionArgs memory) {
    return
      Actions.ActionArgs({
        ...
        accountId: 0,
        amount: Types.AssetAmount({sign: true, denomination: Types.AssetDenomination.Wei, ref: Types.AssetReference.Delta, value: 0}),
        primaryMarketId: 0,
        secondaryMarketId: 0,
        otherAddress: address(this),
        otherAccountId: 0,
...
abstract contract BaseStrategy {
  ...
  function harvestTrigger(uint256 callCost) public view virtual returns (bool) {
    ...
    if (params.activation == 0) return true;

    // Should trigger if hasn't been called in a while
    if (block.timestamp.sub(params.lastReport) >= maxReportDelay) return false;
    ...
    if (outstanding > debtThreshold) return false;

    // Check for profits and losses
    uint256 total = estimatedTotalAssets();
    // Trigger if we have a loss to report
    if (total.add(debtThreshold) < params.totalDebt) return false;
