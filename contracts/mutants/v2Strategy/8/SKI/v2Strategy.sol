...
abstract contract BaseStrategy {
  ...
  function isActive() public view returns (bool) {
    return vault.strategies(address(this)).debtRatio > 0 || super.estimatedTotalAssets() > 0;
  }
  ...
  function tend() external onlyKeepers {
    ...
    super.adjustPosition(vault.debtOutstanding());
  }
  ...
  function harvestTrigger(uint256 callCost) public view virtual returns (bool) {
    ...
    uint256 total = super.estimatedTotalAssets();
  ...
  function harvest() external onlyKeepers {
    ...
    if (emergencyExit) {
      ...
      uint256 totalAssets = super.estimatedTotalAssets();
      // NOTE: use the larger of total assets or debt outstanding to book losses properly
      (debtPayment, loss) = super.liquidatePosition(totalAssets > debtOutstanding ? totalAssets : debtOutstanding);
      ...
      (profit, loss, debtPayment) = super.prepareReturn(debtOutstanding);
    }
    ...
    super.adjustPosition(debtOutstanding);
  ...
  function withdraw(uint256 _amountNeeded) external returns (uint256 _loss) {
    ...
    uint256 amountFreed;
    (amountFreed, _loss) = super.liquidatePosition(_amountNeeded);
