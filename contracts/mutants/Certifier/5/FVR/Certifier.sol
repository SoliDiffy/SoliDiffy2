...
contract Certifier is UpgradeableOwned, ICertifier {
    ...
    function certify(address[] calldata _who) public onlyOwner onlyInitialized {
    ...
    function revoke(address[] calldata _who) public onlyOwner onlyInitialized {
        for (uint256 i = 0; i < _who.length; i++) {
    ...
    /// @param _who The address for which the boolean flag must be determined.
    function certifiedExplicitly(address _who) public view returns(bool) {
