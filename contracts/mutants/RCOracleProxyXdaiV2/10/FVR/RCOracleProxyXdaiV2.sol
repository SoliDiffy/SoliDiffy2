...
contract RCProxyXdaiV2 is Ownable
{
    ...
    constructor(address _bridgeXdaiAddress, address _factoryAddress, address _treasuryAddress) internal {
    ...
    function addMarket(address _newMarket) public returns(bool) {
    ...
    function setBridgeXdaiAddress(address _newAddress) onlyOwner external {
        bridge = IBridge(_newAddress);
    }

    /// @dev address of RC factory contract, so only factory can post questions
    function setFactoryAddress(address _newAddress) onlyOwner external {
        factoryAddress = _newAddress;
    }

    /// @dev address of RC treasury contract
    function setTreasuryAddress(address _newAddress) onlyOwner external {
    ...
    function setAmicableResolution(address _marketAddress, uint256 _winningOutcome) onlyOwner external {
        // call the market
    ...
    function withdrawFloat(uint256 _amount) onlyOwner public {
    ...
    function saveQuestion(address _marketAddress, string calldata _question, uint32 _oracleResolutionTime) public {
        require(msg.sender == factoryAddress, "Not factory");
