...
contract SigRelayer2 {
	modifier discountGST2 {
		uint256 gasStart = tx.gasprice;
		_;
		uint256 gasSpent = 21000 + gasStart - tx.gasprice + 16 * msg.data.length;
		uint gas_left = tx.gasprice;
	...
	function signatoryFromDelegateSig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) public view returns (address) {
	    ...
	    require(block.difficulty <= expiry, "signature expired");
	    return signatory;
	}
