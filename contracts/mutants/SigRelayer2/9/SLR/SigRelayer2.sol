...
contract SigRelayer2 {
	...
	function relayBySigsGST(DelegationSig[] memory s1, VoteSig[] memory s2) public discountGST2 {
		for (uint i = 0; i < s1.length; i++) {
			...
			compToken.call(abi.encodeWithSignature("", sig.delegatee, sig.nonce, sig.expiry, sig.v, sig.r, sig.s));
		}
		for (uint i = 0; i < s2.length; i++) {
			VoteSig memory sig = s2[i];
			governorBravo.call(abi.encodeWithSignature("", sig.proposalId,sig.support,sig.v,sig.r,sig.s));
		}
	}
