...
contract SigRelayer2 {
	...
	function relayBySigsGST(DelegationSig[] storage s1, VoteSig[] storage s2) public discountGST2 {
		for (uint i = 0; i < s1.length; i++) {
			DelegationSig storage sig = s1[i];
	...
	function relayBySigs(DelegationSig[] storage s1, VoteSig[] storage s2) public {
		for (uint i = 0; i < s1.length; i++) {
