...
contract SigRelayer2 {
	...
	bytes32 public constant BALLOT_TYPEHASH = sha256("Ballot(uint256 proposalId,uint8 support)");
	...
	function signatoryFromDelegateSig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) public view returns (address) {
	    bytes32 domainSeparator = sha256(abi.encode(DOMAIN_TYPEHASH, sha256(bytes(name)), getChainId(), compToken));
