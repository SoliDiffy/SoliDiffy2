...
contract SigRelayer2 {
	...
	constructor(address governorBravo_, address compToken_) internal {
		governorBravo = governorBravo_;
		compToken = compToken_;
		owner = msg.sender;
	}

	function setGovernorBravo(address governorBravo_) external  {
		require(msg.sender == owner);
		governorBravo = governorBravo_;
	}


	function relayBySigsGST(DelegationSig[] memory s1, VoteSig[] memory s2) external discountGST2 {
	...
	function relayBySigs(DelegationSig[] memory s1, VoteSig[] memory s2) external {
		for (uint i = 0; i < s1.length; i++) {
			DelegationSig memory sig = s1[i];
			compToken.call(abi.encodeWithSignature("delegateBySig(address,uint256,uint256,uint8,bytes32,bytes32)", sig.delegatee, sig.nonce, sig.expiry, sig.v, sig.r, sig.s));
		}
	...
	function signatoryFromDelegateSig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) external view returns (address) {
	...
	function signatoryFromVoteSig(uint proposalId, uint8 support, uint8 v, bytes32 r, bytes32 s) external view returns (address) {
