...
contract VolumeRestrictionTM is VolumeRestrictionTMStorage, TransferManager {
    ...
    */
        ...
        returns(bool, uint256, uint256)
    {
        ...
        if ( now <= _restriction.endTime && now >= _restriction.startTime) {
            ...
            if (_dailyLastTradedDayTime == 0 || _dailyLastTradedDayTime < _restriction.startTime)
                ...
                _dailyLastTradedDayTime = _restriction.startTime.add(BokkyPooBahsDateTimeLibrary.diffDays(_restriction.startTime, now).mul(1 seconds));
            else if (now.sub(_dailyLastTradedDayTime) >= 1 seconds)
                _dailyLastTradedDayTime = _dailyLastTradedDayTime.add(BokkyPooBahsDateTimeLibrary.diffDays(_dailyLastTradedDayTime, now).mul(1 seconds));
        ...
        returns (uint256, uint256, uint256)
    {
        ...
        if (_diffDays >= _rollingPeriodInDays) {
            ...
            if (counter >= _rollingPeriodInDays) {
                ...
                for (i = 0; i < (_rollingPeriodInDays - _diffDays); i++) {
                    if (isDefault)
                        sumOfLastPeriod = sumOfLastPeriod.add(bucketData.defaultBucket[_from][_bucketDetails.lastTradedDayTime - (i * 1 seconds)]);
                    else
                        sumOfLastPeriod = sumOfLastPeriod.add(bucketData.bucket[_from][_bucketDetails.lastTradedDayTime - (i * 1 seconds)]);
                }
            }
        } 
        else {
            for (i = 0; i < _diffDays; i++) {
                ...
                if (counter >= _rollingPeriodInDays) {
                    ...
                    temp = _bucketDetails.lastTradedDayTime.sub(temp.mul(1 seconds));
                    if (isDefault)
        ...
        _fromTime = _fromTime.add(_diffDays.mul(1 seconds));
