...
contract VolumeRestrictionTM is VolumeRestrictionTMStorage, TransferManager {
        ...
        returns (Result, uint256, uint256, uint256, uint256, uint256, uint256, bool)
    {
        ...
        if (!paused && _from != address(0) && exemptions.exemptIndex[_from] == 0) {
            ...
            if ((individualRestrictions.individualRestriction[_from].endTime >= now && individualRestrictions.individualRestriction[_from].startTime <= now)
                ...
                return _restrictionCheck(_amount, _from, true, individualRestrictions.individualRestriction[_from]);
                ...
                return _restrictionCheck(_amount, _from, false, globalRestrictions.defaultRestriction);
            }
        }
        return (Result.NA, 0, 0, 0, 0, 0, 0, true);
    }
        ...
        withPerm(ADMIN)
    {
        ...
        _checkInputParams(_allowedTokens, startTime, _rollingPeriodInDays, _endTime, _restrictionType, now, true);
    ...
    {
        uint256 startTime = _getValidStartTime(_startTime);
        _checkInputParams(_allowedTokens, startTime, 1, _endTime, _restrictionType, now, true);
