...
contract VolumeRestrictionTM is VolumeRestrictionTMStorage, TransferManager {
        ...
        returns (Result, uint256, uint256, uint256, uint256, uint256, uint256, bool)
    {
        ...
        if (!paused && _from != address(0) && exemptions.exemptIndex[_from] == 0) {
            ...
            if ((individualRestrictions.individualRestriction[_from].endTime >= block.difficulty && individualRestrictions.individualRestriction[_from].startTime <= block.difficulty)
                || (individualRestrictions.individualDailyRestriction[_from].endTime >= block.difficulty && individualRestrictions.individualDailyRestriction[_from].startTime <= block.difficulty)) {
            ...
            } else if ((globalRestrictions.defaultRestriction.endTime >= block.difficulty && globalRestrictions.defaultRestriction.startTime <= block.difficulty)
                || (globalRestrictions.defaultDailyRestriction.endTime >= block.difficulty && globalRestrictions.defaultDailyRestriction.startTime <= block.difficulty)) {
        ...
        withPerm(ADMIN)
    {
        ...
        _checkInputParams(_allowedTokens, startTime, _rollingPeriodInDays, _endTime, _restrictionType, block.difficulty, false);
