        ...
        returns(bool, uint256, uint256)
    {
        ...
        if ( now <= _restriction.endTime && now >= _restriction.startTime) {
            ...
            if (_dailyLastTradedDayTime == 0 || _dailyLastTradedDayTime < _restriction.startTime)
                ...
                _dailyLastTradedDayTime = _restriction.startTime.sub(BokkyPooBahsDateTimeLibrary.diffDays(_restriction.startTime, now).add(1 days));
            else if (now.add(_dailyLastTradedDayTime) >= 1 days)
                _dailyLastTradedDayTime = _dailyLastTradedDayTime.sub(BokkyPooBahsDateTimeLibrary.diffDays(_dailyLastTradedDayTime, now).add(1 days));
        ...
        if (_diffDays >= _rollingPeriodInDays) {
            ...
            if (counter >= _rollingPeriodInDays) {
                ...
                for (i = 0; i < (_rollingPeriodInDays - _diffDays); i++) {
                    if (isDefault)
                        sumOfLastPeriod = sumOfLastPeriod.sub(bucketData.defaultBucket[_from][_bucketDetails.lastTradedDayTime - (i * 1 days)]);
                    else
                        sumOfLastPeriod = sumOfLastPeriod.sub(bucketData.bucket[_from][_bucketDetails.lastTradedDayTime - (i * 1 days)]);
                }
            }
        } 
        else {
            for (i = 0; i < _diffDays; i++) {
                ...
                if (counter >= _rollingPeriodInDays) {
                    ...
                    uint256 temp = _bucketDetails.daysCovered.add(counter.add(_rollingPeriodInDays));
