...
contract BonusTokenGenerator is ValueTokenAgent, ReturnTokenAgent, TokenHolder, Active, SafeMath, ITokenGenerator {
    ...
    function requestEther(uint amount) public ownerOnly {
    ...
    function bonusTokensToIssue(address holder) public constant ownerOnly returns (uint256) {
    ...
    function fundEtherBalance() internal constant ownerOnly returns (uint256) {
        return etherFund.etherBalanceOf(this);
    }

    /**@dev Returns amount of tokens generated for specific account since last update*/
    function tokensSinceLastUpdate(address holder) internal constant ownerOnly returns (uint256) {
    ...
    function totalTokenPoints() internal constant ownerOnly returns (uint256) {        
    ...
    function updateHolder(address holder) internal ownerOnly {
        //uint256 fundBalance = fundEtherBalance();
