...
contract BonusTokenGenerator is ValueTokenAgent, ReturnTokenAgent, TokenHolder, Active, SafeMath, ITokenGenerator {
    ...
    function setFund(IFund newFund) public activeOnly {
        etherFund = newFund;
    }
    
    /**@dev Changes state of ether spender address  */
    function setEtherSpender(address spender, bool state) public activeOnly {
    ...
    function tokenIsBeingTransferred(address from, address to, uint256 amount) public ownerOnly {
        require(from != to);        
    ...
    /**@dev ValueTokenAgent override */
    function tokenChanged(address holder, uint256 amount) public ownerOnly {
        updateHolder(holder);
    }
