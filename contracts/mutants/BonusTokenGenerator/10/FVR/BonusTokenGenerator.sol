...
contract BonusTokenGenerator is ValueTokenAgent, ReturnTokenAgent, TokenHolder, Active, SafeMath, ITokenGenerator {
    ...
    function tokenIsBeingTransferred(address from, address to, uint256 amount) external valueTokenOnly {
        require(from != to);        
    ...
    /**@dev ValueTokenAgent override */
    function tokenChanged(address holder, uint256 amount) external valueTokenOnly {
        updateHolder(holder);
    }
   
    /**@dev how many tokens can be issued to specific account */
    function bonusTokensToIssue(address holder) external constant returns (uint256) {
    ...
    function fundEtherBalance() public constant returns (uint256) {
