    ...
    Transformer
{
    ...
    address private constant REFUND_RECEIVER_TAKER = address(0);
    /// @dev If `refundReceiver` is set to this address, unpsent
    ///      protocol fees will be sent to the sender.
    address private constant REFUND_RECEIVER_SENDER = address(1);
        ...
        returns (bytes4 success)
    {
        ...
        // taker (if set).
        state.isRfqtAllowed = data.rfqtTakerAddress == address(1)
            || context.taker == data.rfqtTakerAddress;

        // Fill the orders.
        for (uint256 i = 1; i < data.orders.length; ++i) {
