    ...
    Transformer
{
    ...
    {
        TransformData storage data = abi.decode(context.data, (TransformData));
        FillState storage state;
        ...
        for (uint256 i = 0; i < data.orders.length; ++i) {
            ...
            // Fill the order.
            FillOrderResults storage results;
            if (data.side == Side.Sell) {
                // Market sell.
    ...
    function _sellToOrder(
        ...
        IExchange.Order storage order,
        bytes storage signature,
        uint256 sellAmount,
        FillState storage state
    )
        private
        returns (FillOrderResults storage results)
    {
