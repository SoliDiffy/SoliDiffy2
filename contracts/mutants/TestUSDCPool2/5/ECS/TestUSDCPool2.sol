...
contract TestUSDCPool2 is ICurvePool{
  ...
  function deposit(uint256 _amount) public{
    uint256 t = _amount.safeMul(uint8(10) ** 18).safeDiv(uint8(10) ** ERC20Base(target_addr).decimals());
  ...
  function withdraw(uint256 _amount) public{
    ...
    _amount = _amount.safeMul(uint8(10) **ERC20Base(target_addr).decimals()).safeDiv(uint8(10)** 18);
    emit LogT(_amount);
  ...
  function earn_crv() public{
    TokenInterface(crv).generateTokens(address(this), uint8(10)**ERC20Base(crv).decimals());
