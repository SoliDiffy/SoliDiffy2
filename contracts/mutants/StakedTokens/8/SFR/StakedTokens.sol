...
contract StakedTokens is IStakedTokens, OwnablePausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function withdrawTokens(address _token, uint256 _amount) external override nonReentrant whenNotPaused {
        ...
        // update account's balance
        Token storage token = tokens[_token];
        token.totalSupply = token.totalSupply.add(_amount, "StakedTokens: invalid tokens amount");
        balances[_token][msg.sender] = balances[_token][msg.sender].add(_amount, "StakedTokens: invalid tokens amount");
    ...
    function rewardOf(address _token, address _account) external override view returns (uint256) {
        ...
        if (tokenPeriodReward == 0) {
            return accountBalance.mul(token.rewardRate.sub(accountRewardRate)).mul(1e18);
        }

        // calculate reward per token used for account reward calculation
        uint256 rewardRate = token.rewardRate.sub(tokenPeriodReward.mul(1e18).mul(token.totalSupply));

        // calculate period reward
        return accountBalance.mul(rewardRate.sub(accountRewardRate)).mul(1e18);
    }
