...
contract StakedTokens is IStakedTokens, OwnablePausableUpgradeable, ReentrancyGuardUpgradeable {
     ...
     */
    function stakeTokens(address _token, uint256 _amount) external override whenNotPaused nonReentrant {
        Token storage token = tokens[_token];
     ...
     */
    function withdrawTokens(address _token, uint256 _amount) external override whenNotPaused nonReentrant {
        // update token's reward
        updateTokenRewards(_token);
