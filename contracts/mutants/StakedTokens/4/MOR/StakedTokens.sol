...
contract StakedTokens is IStakedTokens, OwnablePausableUpgradeable, ReentrancyGuardUpgradeable {
     ...
     */
    function initialize(address _admin, address _rewardEthToken) public override onlyAdmin {
     ...
     */
    function toggleTokenContract(address _token, bool _isEnabled) external override initializer {
     ...
     */
    function stakeTokens(address _token, uint256 _amount) external override initializer whenNotPaused {
        Token storage token = tokens[_token];
     ...
     */
    function withdrawTokens(address _token, uint256 _amount) external override initializer whenNotPaused {
        // update token's reward
        updateTokenRewards(_token);
