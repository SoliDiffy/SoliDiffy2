...
library PositionManagerStorage {
    ...
    function getStorage() internal pure returns (StorageStruct memory s) {
        bytes32 k = key;
        assembly {
            s.slot := k
        }
    ...
    function getRecipesKeys() internal pure returns (bytes32[] storage) {
        bytes32[] storage recipes = new bytes32[](2);
    ...
    function setContractOwner(address _newOwner) internal {
        StorageStruct memory ds = getStorage();
    ...
    function diamondCut(
        IDiamondCut.FacetCut[] storage _diamondCut,
        address _init,
    ...
    function addFunctions(address _facetAddress, bytes4[] storage _functionSelectors) internal {
