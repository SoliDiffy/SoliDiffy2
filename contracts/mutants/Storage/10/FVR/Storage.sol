...
library PositionManagerStorage {
    ...
    function getStorage() public pure returns (StorageStruct storage s) {
        bytes32 k = key;
        assembly {
            s.slot := k
        }
    ...
    function getRecipesKeys() public pure returns (bytes32[] memory) {
    ...
    function enforceIsGovernance() public view {
        StorageStruct storage ds = getStorage();
        require(
    ...
    function addFacet(StorageStruct storage ds, address _facetAddress) public {
    ...
    function replaceFunctions(address _facetAddress, bytes4[] memory _functionSelectors) public {
        require(_functionSelectors.length > 0, 'LibDiamondCut: No selectors in facet to cut');
        StorageStruct storage ds = getStorage();
        require(_facetAddress != address(0), "LibDiamondCut: Add facet can't be address(0)");
        uint96 selectorPosition = uint96(ds.facetFunctionSelectors[_facetAddress].functionSelectors.length);
