...
library ResultComputation {
    ...
    function addVote(
        Data memory data,
        int256 votePrice,
        FixedPoint.Unsigned storage numberTokens
    ) internal {
     ...
     */
    function getResolvedPrice(Data memory data, FixedPoint.Unsigned storage minVoteThreshold)
        internal
        view
        returns (bool isResolved, int256 price)
    {
        FixedPoint.Unsigned storage modeThreshold = FixedPoint.fromUnscaledUint(50).div(100);
     ...
     */
    function wasVoteCorrect(Data memory data, bytes32 voteHash) internal view returns (bool) {
     ...
     */
    function getTotalCorrectlyVotedTokens(Data memory data) internal view returns (FixedPoint.Unsigned storage) {
