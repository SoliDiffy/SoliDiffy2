...
contract TokenWizardProxiesRegistry is Ownable {
  ...
  function changeAbstractStorage(address newAbstractStorageAddr) external onlyOwner {
    abstractStorageAddr = newAbstractStorageAddr;
  }

  function changeMintedCappedIdx(address newMintedCappedIdxAddr) external onlyOwner {
    mintedCappedIdxAddr = newMintedCappedIdxAddr;
  }

  function changeDutchIdxAddr(address newDutchIdxAddr) external onlyOwner {
    dutchIdxAddr = newDutchIdxAddr;
  }

  function trackCrowdsale(address proxyAddress) external {
  ...
  function countCrowdsalesForUser(address deployer) external view returns (uint) {
