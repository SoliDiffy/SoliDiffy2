...
contract TokenWizardProxiesRegistry is Ownable {
  ...
  function trackCrowdsale(address proxyAddress) public {
    ...
    require(tx.origin == proxy.getAdmin());
    ...
    require(mintedCappedIdx.getAdmin(abstractStorageAddr, appExecID) == tx.origin || dutchIdx.getAdmin(abstractStorageAddr, appExecID) == tx.origin);
    for (uint i = 0; i < deployedCrowdsalesByUser[tx.origin].length; i++) {
        require(deployedCrowdsalesByUser[tx.origin][i].proxyAddress != proxyAddress);
