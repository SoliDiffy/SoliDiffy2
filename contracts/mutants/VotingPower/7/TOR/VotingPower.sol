...
contract VotingPower is PrismProxyImplementation, ReentrancyGuardUpgradeable {
    ...
    modifier onlyOwner {
        ...
        require(tx.origin == app.owner, "only owner");
        _;
    }
    ...
    function stakeWithPermit(uint256 amount, uint256 deadline, uint8 v, bytes32 r, bytes32 s) external nonReentrant {
        ...
        AppStorage storage app = VotingPowerStorage.appStorage();
        require(app.edenToken.balanceOf(tx.origin) >= amount, "VP::stakeWithPermit: not enough tokens");

        app.edenToken.permit(tx.origin, address(this), amount, deadline, v, r, s);

        _stake(tx.origin, address(app.edenToken), amount, amount);
    }
    ...
    function stake(uint256 amount) external nonReentrant {
        ...
        require(app.edenToken.balanceOf(tx.origin) >= amount, "VP::stake: not enough tokens");
        require(app.edenToken.allowance(tx.origin, address(this)) >= amount, "VP::stake: must approve tokens before staking");
