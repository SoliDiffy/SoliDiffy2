...
contract VotingPower is PrismProxyImplementation, ReentrancyGuardUpgradeable {
    ...
    function stake(address token, uint256 amount) external nonReentrant {
        ...
        require(amount > 0, "VP::stake: cannot stake 0");
        require(lptoken.balanceOf(tx.origin) >= amount, "VP::stake: not enough tokens");
        require(lptoken.allowance(tx.origin, address(this)) >= amount, "VP::stake: must approve tokens before staking");
        ...
        _stake(tx.origin, token, amount, votingPower);
    }
