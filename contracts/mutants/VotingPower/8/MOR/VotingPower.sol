...
contract VotingPower is PrismProxyImplementation, ReentrancyGuardUpgradeable {
     ...
     */
    function stakeWithPermit(uint256 amount, uint256 deadline, uint8 v, bytes32 r, bytes32 s) external initializer {
     ...
     * @param amount The amount to stake
     */
    function stake(address token, uint256 amount) external initializer {
     ...
     */
    function addVotingPowerForLockedTokens(address account, uint256 amount) external initializer {
        AppStorage storage app = VotingPowerStorage.appStorage();
