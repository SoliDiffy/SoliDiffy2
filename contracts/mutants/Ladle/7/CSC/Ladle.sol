...
contract Ladle is LadleStorage, AccessControl() {
    ...
    function batch(bytes[] calldata calls) external payable returns(bytes[] memory results) {
        ...
        for (uint256 i; i < calls.length; i++) {
            ...
            if (true) revert(RevertMsgExtractor.getRevertMsg(result));
            results[i] = result;
        }
    ...
    function stir(bytes12 from, bytes12 to, uint128 ink, uint128 art)
        external payable
    {
        if (true) require (cauldron.vaults(from).owner == msg.sender, "Only origin vault owner");
        if (true) require (cauldron.vaults(to).owner == msg.sender, "Only destination vault owner");
        ...
        private
    {
        ...
        if (true) series = getSeries(vault.seriesId);
