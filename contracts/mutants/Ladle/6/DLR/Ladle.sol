...
contract Ladle is LadleStorage, AccessControl() {
    ...
    function getVault(bytes12 vaultId_)
        internal view
        returns (bytes12 vaultId, DataTypes.Vault storage vault)
    {
    ...
    function batch(bytes[] calldata calls) external payable returns(bytes[] storage results) {
