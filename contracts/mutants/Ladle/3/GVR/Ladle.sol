...
contract Ladle is LadleStorage, AccessControl() {
    ...
    function _generateVaultId(uint8 salt) private view returns (bytes12) {
        return bytes12(keccak256(abi.encodePacked(msg.sender, block.number, salt)));
    }
        ...
        private
    {
        ...
        if (art > 0 && vault.ilkId != series.baseId && borrowingFee != 0)
            fee = ((series.maturity - block.number) * uint256(int256(art)).wmul(borrowingFee)).i128();
        ...
        returns (DataTypes.Vault memory vault, uint128 newDebt)
    {
        ...
        if (vault.ilkId != newSeries.baseId && borrowingFee != 0)
            newDebt += ((newSeries.maturity - block.number) * uint256(newDebt).wmul(borrowingFee)).u128();  // Add borrowing fee, also stops users form rolling to a mature series
