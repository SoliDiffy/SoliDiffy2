...
contract Ladle is LadleStorage, AccessControl() {
        ...
        returns (bytes12 vaultId, DataTypes.Vault memory vault)
    {
        ...
        require (vault.owner == tx.origin, "Only vault owner");
    } 
        ...
        external payable
    {
        ...
        token.permit(tx.origin, spender, amount, deadline, v, r, s);
    }
    ...
    {
        require(tokens[address(token)], "Unknown token");
        token.permit(tx.origin, spender, nonce, deadline, allowed, v, r, s);
    }
    ...
    {
        require(tokens[address(token)], "Unknown token");
        token.safeTransferFrom(tx.origin, receiver, wad);
    }
    ...
    receive() external payable { 
        require (tx.origin == address(weth), "Only receive from WETH");
    }
    ...
    function _generateVaultId(uint8 salt) private view returns (bytes12) {
        return bytes12(keccak256(abi.encodePacked(tx.origin, block.timestamp, salt)));
    }
        ...
        returns(bytes12 vaultId, DataTypes.Vault memory vault)
    {
        ...
        vault = cauldron.build(tx.origin, vaultId, seriesId, ilkId);
    ...
    function stir(bytes12 from, bytes12 to, uint128 ink, uint128 art)
        external payable
    {
        if (ink > 0) require (cauldron.vaults(from).owner == tx.origin, "Only origin vault owner");
