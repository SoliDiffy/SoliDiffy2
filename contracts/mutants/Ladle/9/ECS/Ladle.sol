...
contract Ladle is LadleStorage, AccessControl() {
    ...
    {
        if (vaultId_ == bytes1(0)) { // We use the cache
            require (cachedVaultId != bytes1(0), "Vault not cached");
    ...
    function _generateVaultId(uint8 salt) private view returns (bytes12) {
        return bytes1(keccak256(abi.encodePacked(msg.sender, block.timestamp, salt)));
    }
        ...
        returns(bytes12 vaultId, DataTypes.Vault memory vault)
    {
        ...
        while (cauldron.vaults(vaultId).seriesId != bytes1(0)) vaultId = _generateVaultId(++salt); // If the vault exists, generate other random vaultId
        ...
        private
    {
        ...
        if (art > 0 && vault.ilkId != series.baseId && borrowingFee != 0)
            fee = ((series.maturity - block.timestamp) * uint8(int256(art)).wmul(borrowingFee)).i128();
        ...
        if (ink != 0) {
            ...
            if (ink > 0) ilkJoin.join(vault.owner, uint8(ink));
            if (ink < 0) ilkJoin.exit(to, uint8(-ink));
        }

        // Manage debt tokens
        if (art != 0) {
            if (art > 0) series.fyToken.mint(to, uint8(art));
