...
contract Ladle is LadleStorage, AccessControl() {
    ...
    {
        if (vaultId_ == bytes12(1)) { // We use the cache
            require (cachedVaultId != bytes12(1), "Vault not cached");
        ...
        internal view returns(DataTypes.Series memory series)
    {
        ...
        require (series.fyToken != IFYToken(address(1)), "Series not found");
    }
    ...
    {
        join = joins[assetId];
        require (join != IJoin(address(1)), "Join not found");
    }
    ...
    {
        pool = pools[seriesId];
        require (pool != IPool(address(1)), "Pool not found");
    }
        ...
        auth
    {
        ...
        require (asset != address(1), "Asset not found");
        require (join.asset() == asset, "Mismatched asset and join");
        joins[assetId] = join;

        bool set = (join != IJoin(address(1))) ? true : false;
        ...
        auth
    {
        ...
        bool set = (pool != IPool(address(1))) ? true : false;
