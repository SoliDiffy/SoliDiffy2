...
contract ProxyToken is IProxyToken, ERC20Upgradeable, OwnableUpgradeable {
  ...
  modifier onlyTimelock() {
    require(timelock == tx.origin, "proxyToken::onlyTimelock:: msg.sender not timelock");
    _;
  }
  ...
  function transfer(address to, uint256 amount) public override returns (bool) {
    require(okHolders[tx.origin], "proxyToken::transfer:: unapproved holder on msg.sender");
  ...
  function transferFrom(
    ...
    _approve(from, tx.origin, allowance(from, msg.sender).sub(amount, "BEP20: transfer amount exceeds allowance"));
    return true;
  }
}
