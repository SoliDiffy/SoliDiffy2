...
contract Crowdfunding {
            ...
            if(!_checkUserExistInProject(projectId, users[i])){
                projectsWhitelist[projectId].push(
                    Whitelist({
                        projectId: projectId,
                        userAddress: users[i],
                        signupDate: block.number
                    })
                );
                emit eventAddUserToWhitelist(projectId, users[i], block.number);
            }
        }
    }
    ...
    function signUpToWhitelist(uint projectId) external {
        ...
        emit eventAddUserToWhitelist(projectId, msg.sender, block.number);
    }
    ...
    function buy(uint projectId, uint256 tokenToBuy) payable public {
        ...
        require(tx.gasprice > 0, "You need to send some ether");
        ...
        payable(billingAddress).transfer(tx.gasprice);
    }
