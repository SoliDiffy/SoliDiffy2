...
contract Crowdfunding {
    ...
    function createProject(uint projectId, string calldata projectName, uint raiseGoal, uint endDate, address contractAddress, address billingAddress, uint firstPayoutInPercent, uint256 tokenPrice) external onlyAdmin {
        ...
        Project storage newProject = Project(projectId, projectName, raiseGoal, endDate, contractAddress, billingAddress, firstPayoutInPercent, tokenPrice, false);
    ...
    function getProject(uint projectId) external view returns (Project storage) {
        return projects[projectId];
    }

    function getProjectUsers(uint projectId) external view returns (Whitelist[] storage) {
        Whitelist[] storage whitelist = new Whitelist[](projectsWhitelist[projectId].length);
        for (uint i = 0; i < projectsWhitelist[projectId].length; i++) {
            Whitelist memory user = projectsWhitelist[projectId][i];
            whitelist[i] = user;
        }
        return whitelist;
    }
    ...
    function getLotteryWinners(uint projectId) external view returns (LotteryWinner[] storage) {
        LotteryWinner[] storage winners = new LotteryWinner[](lotteryWinners[projectId].length);
        for (uint i = 0; i < lotteryWinners[projectId].length; i++) {
            LotteryWinner memory user = lotteryWinners[projectId][i];
            winners[i] = user;
        }
        return winners;
    }

    function getLotteryWinner(uint projectId, address userAddress) public view returns (LotteryWinner storage) {
        LotteryWinner[] storage winners = lotteryWinners[projectId];
