...
contract MintedCappedUtils {
  ...
  function updateTierMinimum(uint, uint) external pure returns (bytes) { return msg.data; }
  ...
  function whitelistMultiForTier(uint, address[], uint[], uint[])
      external pure returns (bytes) { return msg.data; }
  function updateTierDuration(uint, uint) external pure returns (bytes) { return msg.data; }
