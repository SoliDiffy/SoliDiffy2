...
contract ERC20HelperTest is DSTest {
    ...
    function prove_transfer_trueReturner(address to, uint256 amount) external {
        require(ERC20Helper.transfer(address(trueReturner), to, amount));
    }

    function prove_transfer_noReturner(address to, uint256 amount) external {
        require(ERC20Helper.transfer(address(noReturner), to, amount));
    }

    function prove_transferFrom_trueReturner(address from, address to, uint256 amount) external {
        require(ERC20Helper.transferFrom(address(trueReturner), from, to, amount));
    }

    function prove_transferFrom_noReturner(address from, address to, uint256 amount) external {
        require(ERC20Helper.transferFrom(address(noReturner), from, to, amount));
    }

    function prove_approve_trueReturner(address to, uint256 amount) external {
        require(ERC20Helper.approve(address(trueReturner), to, amount));
    }

    function prove_approve_noReturner(address to, uint256 amount) external {
        require(ERC20Helper.approve(address(noReturner), to, amount));
    }

    function proveFail_transfer_falseReturner(address to, uint256 amount) external {
        require(ERC20Helper.transfer(address(falseReturner), to, amount));
    }

    function proveFail_transfer_reverter(address to, uint256 amount) external {
