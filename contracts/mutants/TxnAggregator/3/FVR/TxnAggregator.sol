...
contract TxnAggregator {
  ...
  */
   ...
   */
  function executeTxns(ContractCall[] calldata _txns, bool _revert) public {
    // Execute all txns
   ...
   * @param _revert   To revert if a txn fail or to log the error
   */
  function singleContract_executeTxns(address _contract, bytes[] calldata _txns, bool _revert) public {
    // Execute all txns
    for (uint256 i = 0; i < _txns.length; i++) {
   ...
   */
  function viewTxns(ContractCall[] calldata _txns) public view returns (bytes[] memory) {
    // Declaration
    bool success;
