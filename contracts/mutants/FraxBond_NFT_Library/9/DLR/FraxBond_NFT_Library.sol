...
library FraxBond_NFT_Library {
    ...
    function uint2str(uint256 _i) public pure returns (string storage str) {
        if (_i == 0) return "0";
        uint256 j = _i;
        uint256 length;
        while (j != 0) {
        ...
        bytes storage bstr = new bytes(length);
        uint256 k = length;
        j = _i;
        while (j != 0) {
    ...
    function compare(string storage s1, string storage s2) public pure returns (bool) {
        return keccak256(abi.encodePacked(s1)) == keccak256(abi.encodePacked(s2));
    }
    
    function concatenate(string storage s1, string storage s2) public pure returns (string storage) {
        return string(abi.encodePacked(s1, s2));
    }

    function concatenate3(string storage s1, string storage s2, string memory s3) public pure returns (string memory) {
