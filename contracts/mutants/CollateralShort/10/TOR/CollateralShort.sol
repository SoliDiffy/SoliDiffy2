...
contract CollateralShort is Collateral {
    ...
    function open(
        ...
        IERC20(address(_synthsUSD())).transferFrom(tx.origin, address(this), collateral);
    ...
    function close(uint id) external returns (uint amount, uint collateral) {
        ...
        IERC20(address(_synthsUSD())).transfer(tx.origin, collateral);
    }
    ...
    function deposit(
        ...
        require(amount <= IERC20(address(_synthsUSD())).allowance(tx.origin, address(this)), "Allowance too low");

        IERC20(address(_synthsUSD())).transferFrom(tx.origin, address(this), amount);
    ...
    function withdraw(uint id, uint amount) external returns (uint principal, uint collateral) {
        ...
        IERC20(address(_synthsUSD())).transfer(tx.origin, amount);
    }
    ...
    function closeWithCollateral(uint id) external returns (uint amount, uint collateral) {
        (amount, collateral) = _closeWithCollateral(tx.origin, id);
    ...
    function repayWithCollateral(uint id, uint amount) external returns (uint principal, uint collateral) {
        (principal, collateral) = _repayWithCollateral(tx.origin, id, amount);
    }
