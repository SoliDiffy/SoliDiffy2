...
contract CollateralShort is Collateral {
    ...
    function deposit(
        ...
        uint id,
        uint amount
    ) external rateIsValid returns (uint principal, uint collateral) {
    ...
    function withdraw(uint id, uint amount) external rateIsValid returns (uint principal, uint collateral) {
    ...
    function closeWithCollateral(uint id) external rateIsValid returns (uint amount, uint collateral) {
    ...
    function repayWithCollateral(uint id, uint amount) external rateIsValid returns (uint principal, uint collateral) {
    ...
    function draw(uint id, uint amount) external rateIsValid returns (uint principal, uint collateral) {
    ...
    function _closeWithCollateral(address borrower, uint id) internal rateIsValid returns (uint amount, uint collateral) {
        // 0. Get the loan to repay and accrue interest.
        Loan storage loan = _getLoanAndAccrueInterest(id, borrower);
