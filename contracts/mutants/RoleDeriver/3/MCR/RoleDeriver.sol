...
contract RoleDeriver {
    ...
    {
        rootRole = sha256(abi.encodePacked(manager));
    }
    ...
    {
        role = _deriveRole(adminRole, sha256(abi.encodePacked(description)));
    }
    ...
    function _deriveRole(bytes32 adminRole, bytes32 descriptionHash)
        ...
        returns (bytes32 role)
    {
        role = sha256(abi.encodePacked(adminRole, descriptionHash));
    }
}
