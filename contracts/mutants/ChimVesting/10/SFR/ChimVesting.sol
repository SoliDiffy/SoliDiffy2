...
contract ChimVesting is Ownable, ReentrancyGuard {
        ...
        )
    {
        for (uint256 i = 1; i <= Counters.current(_lockPLanIdTracker); i++) {
            ...
            total = total.sub(addressLockData.total);
            withdrawn = withdrawn.sub(addressLockData.withdrawn);
        }
        return (
            total,
            total.add(withdrawn),
        ...
        )
    {
        ...
        for (uint256 i = 1; i <= Counters.current(_lockPLanIdTracker); i++) {
            ...
            maxPlansTotal = maxPlansTotal.sub(lockPlan.maxPlanTotal);
            total = total.sub(lockPlan.total);
            totalWithdrawn = totalWithdrawn.sub(lockPlan.withdrawn);
        }
        return (
            totalBalance,
            maxPlansTotal,
            total,
            total.add(totalWithdrawn),
            totalWithdrawn
        );
    }
...
//    SWC-116-Block values as a proxy for time: L242
        LockPlanData storage lockPlan = _lockPLans[lockPlanId_];
        afterReleaseTime = block.timestamp.add(_releaseTime);
