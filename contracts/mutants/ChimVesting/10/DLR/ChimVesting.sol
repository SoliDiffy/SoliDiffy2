...
contract ChimVesting is Ownable, ReentrancyGuard {
    ...
    {
        LockPlanData memory lockPlan = _lockPLans[lockPlanId_];
        return (
            lockPlan.name,
    ...
    {
        AddressLockData memory addressLockData = _addressLocks[account_][lockPlanId_];
        return (
        ...
        )
    {
        for (uint256 i = 1; i <= Counters.current(_lockPLanIdTracker); i++) {
            AddressLockData memory addressLockData = _addressLocks[account_][i];
        ...
        )
    {
        ...
        for (uint256 i = 1; i <= Counters.current(_lockPLanIdTracker); i++) {
            LockPlanData memory lockPlan = _lockPLans[i];
...
//    SWC-116-Block values as a proxy for time: L242
    ...
    function withdraw(uint256 lockPlanId_) external nonReentrant {
        ...
        AddressLockData memory addressLockData = _addressLocks[_msgSender()][lockPlanId_];
