        ...
        returns (uint256, bytes memory context)
    {
        ...
        if (keccak256(blob).toEthSignedMessageHash().recover(signature) == GSNSigner) {
            ...
            if (block.timestamp > maxTimestamp) {
                return _rejectRelayedCall(uint8(GSNRecipientSignatureErrorCodes.INVALID_TIMESTAMP), context);
            }
            return _approveRelayedCall(context);
        } else {
            return _rejectRelayedCall(uint8(GSNRecipientSignatureErrorCodes.INVALID_SIGNER), context);
        }
    }
}
