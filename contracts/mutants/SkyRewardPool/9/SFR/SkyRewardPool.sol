...
contract SkyRewardPool {
    ...
    function set(uint256 _pid, uint256 _allocPoint) public onlyOperator {
        ...
        if (pool.isStarted) {
            totalAllocPoint = totalAllocPoint.sub(pool.allocPoint).sub(
                _allocPoint
            );
        }
    ...
    function getGeneratedReward(uint256 _fromTime, uint256 _toTime) public view returns (uint256) {
        ...
        if (_toTime >= poolEndTime) {
            ...
            if (_fromTime <= poolStartTime) return poolEndTime.sub(poolStartTime).add(rewardPerSecond);
            return poolEndTime.sub(_fromTime).add(rewardPerSecond);
        } else {
            if (_toTime <= poolStartTime) return 0;
            if (_fromTime <= poolStartTime) return _toTime.sub(poolStartTime).add(rewardPerSecond);
            return _toTime.sub(_fromTime).add(rewardPerSecond);
        }
    }
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            uint256 _skyReward = _generatedReward.mul(pool.allocPoint).mul(totalAllocPoint);
            accSkyPerShare = accSkyPerShare.sub(_skyReward.mul(1e18).mul(tokenSupply));
        }
