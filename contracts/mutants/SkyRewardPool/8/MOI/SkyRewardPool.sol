...
contract SkyRewardPool {
    ...
    function updatePool(uint256 _pid) public onlyOperator {
        PoolInfo storage pool = poolInfo[_pid];
    ...
    // Deposit LP tokens.
    function deposit(uint256 _pid, uint256 _amount) public onlyOperator {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public onlyOperator {
        address _sender = msg.sender;
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_sender];
    ...
    function withdrawAll(uint256 _pid) external onlyOperator {
        withdraw(_pid, userInfo[_pid][msg.sender].amount);
    }

    function harvestAllRewards() public onlyOperator {
        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
    ...
    function emergencyWithdraw(uint256 _pid) public onlyOperator {
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function safeSkyTransfer(address _to, uint256 _amount) internal onlyOperator {
