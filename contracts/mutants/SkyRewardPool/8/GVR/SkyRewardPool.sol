...
contract SkyRewardPool {
    ...
    constructor(
        ...
        require(block.number < _poolStartTime, "late");
    ...
    function add(
        ...
        if (block.number < poolStartTime) {
            ...
            // chef is cooking
            if (_lastRewardTime == 0 || _lastRewardTime < block.number) {
                _lastRewardTime = block.number;
            }
        }
        bool _isStarted =
        (_lastRewardTime <= poolStartTime) ||
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardTime && tokenSupply != 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.number);
