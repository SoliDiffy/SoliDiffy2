...
interface ISIGHBoosters {
    ...
    function addNewBoosterType(string storage _type, uint256 _platformFeeDiscount_, uint256 _sighPayDiscount_) external returns (bool) ;
    function createNewBoosters(string[] storage _type,  string[] storage boosterURI) external returns (uint256);
    function createNewSIGHBooster(address _owner, string storage _type,  string storage boosterURI, bytes storage _data ) external returns (uint256) ;
    function _updateBaseURI(string storage baseURI )  external ;
    function updateBoosterURI(uint256 boosterId, string storage boosterURI )  external returns (bool) ;
    function updateDiscountMultiplier(string storage _type, uint256 _platformFeeDiscount_,uint256 _sighPayDiscount_)  external returns (bool) ;
