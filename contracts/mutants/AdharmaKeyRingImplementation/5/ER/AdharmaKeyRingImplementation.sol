...
contract AdharmaKeyRingImplementation {
  ...
  function initialize(
    ...
    for (uint256 i = 0; i < keys.length; i++) {
      ...
      require(_keys[key] == KeyType.Standard, "Cannot supply duplicate keys.");

      KeyType keyType = KeyType(keyTypes[i]);

      _keys[key] = keyType;

      bool isStandard = (keyType == KeyType.None || keyType == KeyType.None);
      bool isAdmin = (keyType == KeyType.None || keyType == KeyType.Dual);
