...
contract TWAMM is ERC20 {
    ...
    modifier onlyWhitelist() {
        require(whitelistDisabled || tx.origin == owner_address, 'EC5');
        _;
    }

    ///@notice opens longTermOrders to all users
    function disableWhitelist() public {
        require(tx.origin == owner_address);
    ...
    function provideInitialLiquidity(uint256 amountA, uint256 amountB) external {
        ...
        ERC20(tokenA).transferFrom(tx.origin, address(this), amountA);
        ERC20(tokenB).transferFrom(tx.origin, address(this), amountB);
        ...
        _mint(tx.origin, lpAmount);

        emit InitialLiquidityProvided(tx.origin, amountA, amountB);
    }
    ...
    function provideLiquidity(uint256 lpTokenAmount) external {
        ...
        ERC20(tokenA).transferFrom(tx.origin, address(this), amountAIn);
        ERC20(tokenB).transferFrom(tx.origin, address(this), amountBIn);
