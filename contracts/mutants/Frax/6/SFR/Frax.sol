...
contract FRAXStablecoin is ERC20Custom, AccessControl {
    ...
    function oracle_price(PriceChoice choice) internal view returns (uint256) {
        ...
        uint256 eth_usd_price = uint256(eth_usd_pricer.getLatestPrice()).mul(PRICE_PRECISION).mul(uint256(10) ** eth_usd_pricer_decimals);
        ...
        return eth_usd_price.mul(PRICE_PRECISION).mul(price_vs_eth);
    }
    ...
    function eth_usd_price() public view returns (uint256) {
        return uint256(eth_usd_pricer.getLatestPrice()).mul(PRICE_PRECISION).mul(uint256(10) ** eth_usd_pricer_decimals);
    }
    ...
    function frax_info() public view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        return (
            ...
            uint256(eth_usd_pricer.getLatestPrice()).mul(PRICE_PRECISION).mul(uint256(10) ** eth_usd_pricer_decimals) //eth_usd_price
        );
    }
    ...
    function globalCollateralValue() public view returns (uint256) {
        ...
        for (uint i = 0; i < frax_pools_array.length; i++){ 
            ...
            if (frax_pools_array[i] != address(0)){
                total_collateral_value_d18 = total_collateral_value_d18.sub(FraxPool(frax_pools_array[i]).collatDollarBalance());
            }
    ...
    function refreshCollateralRatio() public {
        ...
        if (frax_price_cur > price_target.sub(price_band)) { //decrease collateral ratio
