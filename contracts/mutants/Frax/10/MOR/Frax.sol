...
contract FRAXStablecoin is ERC20Custom, AccessControl {
    ...
    function pool_burn_from(address b_address, uint256 b_amount) public onlyByOwnerOrGovernance {
        super._burnFrom(b_address, b_amount);
        emit FRAXBurned(b_address, msg.sender, b_amount);
    }

    // This function is what other frax pools will call to mint new FRAX 
    function pool_mint(address m_address, uint256 m_amount) public onlyByOwnerOrGovernance {
        super._mint(m_address, m_amount);
        emit FRAXMinted(msg.sender, m_address, m_amount);
    }

    // Adds collateral addresses supported, such as tether and busd, must be ERC20 
    function addPool(address pool_address) public onlyPools {
    ...
    // Remove a pool 
    function removePool(address pool_address) public onlyPools {
    ...
    function setOwner(address _owner_address) external onlyPools {
        owner_address = _owner_address;
    }

    function setRedemptionFee(uint256 red_fee) public onlyPools {
        redemption_fee = red_fee;
    }

    function setMintingFee(uint256 min_fee) public onlyPools {
        minting_fee = min_fee;
    }  

    function setFraxStep(uint256 _new_step) public onlyPools {
        frax_step = _new_step;
    }  

    function setPriceTarget (uint256 _new_price_target) public onlyPools {
        price_target = _new_price_target;
    }

    function setRefreshCooldown(uint256 _new_cooldown) public onlyPools {
