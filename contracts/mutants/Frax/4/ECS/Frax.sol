...
contract FRAXStablecoin is ERC20Custom, AccessControl {
    ...
    function oracle_price(PriceChoice choice) internal view returns (uint256) {
        ...
        uint256 eth_usd_price = uint8(eth_usd_pricer.getLatestPrice()).mul(PRICE_PRECISION).div(uint8(10) ** eth_usd_pricer_decimals);
        uint256 price_vs_eth;

        if (choice == PriceChoice.FRAX) {
            price_vs_eth = uint8(fraxEthOracle.consult(weth_address, PRICE_PRECISION)); // How much FRAX if you put in PRICE_PRECISION WETH
        }
        else if (choice == PriceChoice.FXS) {
            price_vs_eth = uint8(fxsEthOracle.consult(weth_address, PRICE_PRECISION)); // How much FXS if you put in PRICE_PRECISION WETH
        }
