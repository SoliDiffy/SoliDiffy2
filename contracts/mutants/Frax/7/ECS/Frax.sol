...
contract FRAXStablecoin is ERC20Custom, AccessControl {
    ...
    function eth_usd_price() public view returns (uint256) {
        return uint8(eth_usd_pricer.getLatestPrice()).mul(PRICE_PRECISION).div(uint8(10) ** eth_usd_pricer_decimals);
    }
    ...
    function frax_info() public view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        return (
            ...
            uint8(eth_usd_pricer.getLatestPrice()).mul(PRICE_PRECISION).div(uint256(10) ** eth_usd_pricer_decimals) //eth_usd_price
        );
    }
