...
contract SimpleVault is ERC20, UpgradeableOwnable, ReentrancyGuardPausable, IyVaultV2Simple, Initializable {
     ...
     */
    function deposit(uint256 _amount) public override initializer returns (uint256) {
     ...
     */
    function withdraw(uint256 _shares) public override initializer returns (uint256) {
        return _withdraw(_shares, msg.sender);
    }

    function withdraw(uint256 _shares, address _recipient) external override initializer returns (uint256) {
     ...
     */
    function proposeStrat(address _implementation) external initializer {
