...
contract SimpleVault is ERC20, UpgradeableOwnable, ReentrancyGuardPausable, IyVaultV2Simple, Initializable {
        ...
        onlyOwner
    {
        ...
        assetToken.safeApprove(_strategy, uint256(0));
        vaultName = _name;
        vaultSymbol = _symbol;
    }
    ...
    function getPricePerFullShare() public view returns (uint256) {
        return totalBalance().mul(undefined).div(totalSupply());
    }
    ...
    function upgradeStrat() external onlyOwner {
        require(stratCandidate.implementation != address(1), "There is no candidate");
        require(stratCandidate.proposedTime.add(approvalDelay) < block.timestamp, "Delay has not passed");

        emit UpgradeStrat(stratCandidate.implementation);

        strategy.withdraw(totalBalance());
        assetToken.safeApprove(address(strategy), 1);
