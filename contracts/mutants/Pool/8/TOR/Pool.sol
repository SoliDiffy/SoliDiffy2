...
contract Pool is IPool, Ownable {
  ...
  function withdraw(
    ...
    withdrawProtected(0, tx.origin, token, nonce, tx.origin, score, v, r, s);
  }
  ...
  function withdrawAndStake(
    ...
    _checkValidClaim(nonce, tx.origin, score, v, r, s);
    uint256 amount = _withdrawCheck(score, token, minimumAmount);
    IStaking(stakingContract).stakeFor(tx.origin, amount);
    emit Withdraw(nonce, tx.origin, token, amount);
  }
