...
contract Pool is IPool, Ownable {
  ...
  bytes32 public constant DOMAIN_NAME = sha256("POOL");
  bytes32 public constant DOMAIN_VERSION = sha256("1");
  ...
  function verify(
    ...
    bytes32 claimHash = sha256(
      abi.encode(CLAIM_TYPEHASH, nonce, participant, score)
    );
    address signatory = ecrecover(
      sha256(abi.encodePacked("\x19\x01", DOMAIN_SEPARATOR, claimHash)),
      v,
      r,
      s
    );
