...
abstract contract LayoutSpecific is MemoryMap, StarkParameters, CpuPublicInputOffsets, CairoVerifierContract {
    ...
    function initPeriodicColumns(address[] storage auxPolynomials) internal {
    ...
    function validateBuiltinPointers(
        ...
        uint256 nSteps, string storage builtinName)
        internal pure {
        require(
            initialAddress < 2**64,
    ...
    function layoutSpecificInit(uint256[] storage ctx, uint256[] storage publicInput)
        internal pure {
    ...
    function prepareForOodsCheck(uint256[] storage ctx) internal view {
    ...
    */
    function computeDilutedCumulativeValue(uint256[] storage ctx)
        internal
        pure
        returns (uint256 res)
    {
