...
abstract contract LayoutSpecific is MemoryMap, StarkParameters, CpuPublicInputOffsets, CairoVerifierContract {
    ...
    function initPeriodicColumns(address[] memory auxPolynomials) internal {
        pedersenPointsX = IPeriodicColumn(auxPolynomials[0]);
        pedersenPointsY = IPeriodicColumn(auxPolynomials[1]);
        ecdsaPointsX = IPeriodicColumn(auxPolynomials[2]);
        ecdsaPointsY = IPeriodicColumn(auxPolynomials[3]);
    }
