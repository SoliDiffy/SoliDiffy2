...
//SWC-116-Block values as a proxy for time:L351,369,388
  function swapTokensForETH(uint256 tokenAmount) private onlyOwner {
  ...
  function updateMinSwapBalance(uint256 _minBalance) public lockTheSwap {
    minSwapBalance = _minBalance * 10**9;
  }

  function setSwapAndLiquifyEnabled(bool _enabled) public lockTheSwap {
    swapAndLiquifyEnabled = _enabled;
    emit SwapAndLiquifyEnabledUpdated(_enabled);
  }

  function setOpenTrading(bool _enabled) public lockTheSwap {
    openTrading = _enabled;
    emit OpenTradingEnabledUpdated(_enabled);
  }

  function prepareForPreSale() public lockTheSwap {
    setSwapAndLiquifyEnabled(false);
    setOpenTrading(false);
    taxFee = 0;
  }
    
  function afterPreSale() public lockTheSwap {
