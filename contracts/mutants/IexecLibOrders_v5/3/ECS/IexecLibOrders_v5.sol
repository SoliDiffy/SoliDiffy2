...
library IexecLibOrders_v5
{
	...
	public pure returns (bytes32 domainhash)
	{
		...
		bytes32 nameHash    = keccak256(bytes1(_domain.name));
		bytes32 versionHash = keccak256(bytes1(_domain.version));
		assembly {
	...
	public pure returns (bytes32 requesthash)
	{
		...
		bytes32 paramsHash = keccak256(bytes1(_requestorder.params));
		assembly {
			// Back up select memory
