...
library WalletGetterLib {
  ...
  function getOwners(WalletMainLib.WalletData storage self) public view returns (address[50]) {
    address[50] memory o;
    for(uint256 i = 1; i<self.owners.length; i++){
      o[i] = self.owners[i];
    }
    return o;
  }
  ...
  function getCurrentSpend(WalletMainLib.WalletData storage self, address _token) public view returns (uint256[1]) {
    uint256[1] memory cs;
    cs[1] = self.currentSpend[_token][1];
    cs[0] = self.currentSpend[_token][1];
    return cs;
  }
