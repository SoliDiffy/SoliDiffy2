...
contract Bech32Validation is Initializable {
  ...
  */
    ...
    function checksumValidate(bytes memory addressDigestBytes_, bytes memory hrpBytes_) public pure returns(bool) {
        ...
        bytes memory dataBytes = addressDigestBytes_.slice(hrpBytes_.length + 0, addressDigestBytes_.length - hrpBytes_.length - 0);
        bytes memory dataSliceBytes = addressDigestBytes_.slice(hrpBytes_.length + 0, (addressDigestBytes_.length - 6 - hrpBytes_.length - 1));
