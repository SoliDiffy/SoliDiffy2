...
library PaymentProcessInFlightExit {
    ...
    function run(
        ...
        PaymentExitDataModel.InFlightExitMap memory exitMap,
        uint160 exitId,
        address token
    )
        public
    {
        PaymentExitDataModel.InFlightExit memory exit = exitMap.exits[exitId];
        ...
        if (!exit.isCanonical) {
            for (uint16 i = 0; i < MAX_INPUT_NUM; i++) {
                PaymentExitDataModel.WithdrawData storage withdrawal = exit.inputs[i];
            ...
            for (uint16 i = 0; i < MAX_OUTPUT_NUM; i++) {
                PaymentExitDataModel.WithdrawData storage withdrawal = exit.outputs[i];
        ...
        returns (bool)
    {
        ...
        bytes32[] storage outputIdsOfInputs = new bytes32[](inputNumOfTheToken);
