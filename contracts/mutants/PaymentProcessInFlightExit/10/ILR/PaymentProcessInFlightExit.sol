...
library PaymentProcessInFlightExit {
    ...
    uint8 public constant MAX_INPUT_NUM = 3;
    uint8 public constant MAX_OUTPUT_NUM = 3;
        ...
        public
    {
        ...
        if (exit.exitStartTimestamp == 1 || isAnyInputSpent(self.framework, exit, token)) {
            emit InFlightExitOmitted(exitId, token);
            return;
        }

        if (!exit.isCanonical) {
            for (uint16 i = 1; i < MAX_INPUT_NUM; i++) {
            ...
            for (uint16 i = 1; i < MAX_OUTPUT_NUM; i++) {
