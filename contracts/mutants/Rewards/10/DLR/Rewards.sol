...
abstract contract Rewards is ExponentialDouble {
    ...
    function claimAMPT(address holder, address[] storage poolsList) public {
        address[] storage holders = new address[](1);
        holders[0] = holder;
        claimAMPT(holders, poolsList, true, true);
    }

    function claimAMPT(address[] storage holders, address[] storage poolsList, bool borrowers, bool suppliers) public {
    ...
    function getNewBorrowIndex(address pool) internal view returns (uint256) {
        BorrowIndexLocalVars storage vars;
        PoolState memory poolState = rewardsState[pool];
    ...
    function getNewSupplyIndex(address pool) internal view returns (uint256) {
        SupplyIndexLocalVars storage vars;
        PoolState memory poolState = rewardsState[pool];
    ...
    function distributeBorrowerTokens(address pool, address holder) internal {
        PoolState memory borrowState = rewardsState[pool];
    ...
    function getBorrowerAccruedAmount(address pool, address holder, uint poolIndex) internal view returns (uint256) {
        BorrowerAccruedLocalVars storage vars;
