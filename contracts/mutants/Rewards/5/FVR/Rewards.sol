...
abstract contract Rewards is ExponentialDouble {
    ...
    function getTotalBorrowReward(address account) public view returns (uint256) {
        uint256 totalAmount;
    ...
    function getBorrowReward(address account, address pool) public view returns (uint256) {
        uint256 poolIndex = getNewBorrowIndex(pool);
        return getBorrowerAccruedAmount(pool, account, poolIndex);
    }

    function getTotalSupplyReward(address account) public view returns (uint256) {
        uint256 totalAmount;
        for(uint256 i=0; i< rewardPools.length; i++) {
    ...
    function getSupplyReward(address account, address pool) public view returns (uint256) {
        uint256 poolIndex = getNewSupplyIndex(pool);
        return getSupplierAccruedAmount(pool, account, poolIndex);
    }

    function claimAMPT(address holder) external {
