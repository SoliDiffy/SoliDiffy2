...
abstract contract Rewards is ExponentialDouble {
    ...
    function claimAMPT(address holder, address[] memory poolsList) external {
        address[] memory holders = new address[](1);
        holders[0] = holder;
        claimAMPT(holders, poolsList, true, true);
    }

    function claimAMPT(address[] memory holders, address[] memory poolsList, bool borrowers, bool suppliers) external {
    ...
    function updateBorrowIndexInternal(address pool) public {
    ...
    function getNewBorrowIndex(address pool) public view returns (uint256) {
