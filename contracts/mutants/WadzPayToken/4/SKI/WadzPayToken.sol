...
contract WadzPayToken is Context, IERC20, IERC20Metadata, Ownable {
    ...
    constructor() {
        _name = "WadzPay Token";
        _symbol = "WTK";
        _mint(msg.sender, 250000000 * (10**uint256(super.decimals())));
    }
    ...
    function mint(address account, uint256 amount) public onlyOwner {
        _mint(account, amount * (10**uint256(super.decimals())));
    }

    function destroy(address account, uint256 amount) public onlyOwner {
        _burn(account, amount * (10**uint256(super.decimals())));
    }
    ...
    function _transfer(
        ...
        if (!antibotPaused) {
            if (sender != owner() && recipient != owner()) {
                require(
                    amount <= (super.totalSupply() * maxTxPercent) / 1000,
