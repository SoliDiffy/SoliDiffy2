...
contract BoomstarterPreICO is ArgumentsChecker, ReentrancyGuard, EthPriceDependent {
     ...
     */
    uint internal c_dateTo = 1532563200; // 26-Jul-18 00:00:00 UTC

    /// @dev current amount of tokens sold
    uint internal m_currentTokensSold = 0;
    /// @dev limit of tokens to be sold during presale
    uint internal c_maximumTokensSold = uint(5000000) * uint(10) ** uint(18); // 5 million tokens

    /// @notice usd price of BoomstarterToken in cents 
    uint internal c_centsPerToken = 60; // $0.6
}
