...
contract NodeRotation is Permissions {
     ...
     */
    function exitFromSchain(uint nodeIndex) external onlyOwner returns (bool) {
     ...
     */
    function freezeSchains(uint nodeIndex) external onlyOwner {
        SchainsInternal schainsInternal = SchainsInternal(contractManager.getContract("SchainsInternal"));
     ...
     */
    function removeRotation(bytes32 schainIndex) external onlyOwner {
     ...
     */
    function skipRotationDelay(bytes32 schainIndex) external initializer {
    ...
    function initialize(address newContractsAddress) public override onlyOwner {
