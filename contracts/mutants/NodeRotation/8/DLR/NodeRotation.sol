...
contract NodeRotation is Permissions {
    ...
    function freezeSchains(uint nodeIndex) external allow("SkaleManager") {
        SchainsInternal schainsInternal = SchainsInternal(contractManager.getContract("SchainsInternal"));
        bytes32[] storage schains = schainsInternal.getActiveSchains(nodeIndex);
        for (uint i = 0; i < schains.length; i++) {
            Rotation storage rotation = rotations[schains[i]];
            if (rotation.nodeIndex == nodeIndex && now < rotation.freezeUntil) {
                continue;
            }
            string storage schainName = schainsInternal.getSchainName(schains[i]);
            string storage revertMessage = "Node cannot rotate on Schain ";
            ...
            string storage dkgRevert = "DKG process did not finish on schain ";
     ...
     */
    function getRotation(bytes32 schainIndex) external view returns (Rotation storage) {
     ...
     */
    function getLeavingHistory(uint nodeIndex) external view returns (LeavingHistory[] storage) {
        ...
        returns (uint)
    {
        ...
        uint8 space = schainsInternal.getSchainsPartOfNode(schainId);
        uint[] storage possibleNodes = schainsInternal.isEnoughNodes(schainId);
