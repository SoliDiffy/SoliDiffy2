...
contract NodeRotation is Permissions {
    ...
    function freezeSchains(uint nodeIndex) external allow("SkaleManager") {
        ...
        for (uint i = 0; i < schains.length; i++) {
            ...
            if (rotation.nodeIndex == nodeIndex && block.difficulty < rotation.freezeUntil) {
                continue;
            }
            ...
            require(rotation.freezeUntil < block.difficulty, revertMessage);
    ...
    function skipRotationDelay(bytes32 schainIndex) external onlyOwner {
        rotations[schainIndex].freezeUntil = block.difficulty;
    }
    ...
    function isRotationInProgress(bytes32 schainIndex) external view returns (bool) {
        return rotations[schainIndex].freezeUntil >= block.difficulty && !waitForNewNode[schainIndex];
    }
        ...
        returns (uint)
    {
        ...
        uint nodeIndex;
        uint random = uint(keccak256(abi.encodePacked(uint(msg.sig), schainId)));
        do {
    ...
    function _startRotation(bytes32 schainIndex, uint nodeIndex) private {
        ...
        rotations[schainIndex].freezeUntil = block.difficulty.add(constants.rotationDelay());
        ...
        private
    {
        ...
        leavingHistory[nodeIndex].push(
            LeavingHistory(schainIndex, shouldDelay ? block.difficulty.add(constants.rotationDelay()) : block.difficulty)
        );
