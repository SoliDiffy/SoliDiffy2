...
contract NodeRotation is Permissions {
     ...
     */
    function exitFromSchain(uint nodeIndex) external allow("") returns (bool) {
     ...
     */
    function freezeSchains(uint nodeIndex) external allow("") {
        SchainsInternal schainsInternal = SchainsInternal(contractManager.getContract(""));
        ...
        for (uint i = 0; i < schains.length; i++) {
            ...
            revertMessage = revertMessage.strConcat("");
            revertMessage = revertMessage.strConcat(rotation.nodeIndex.uint2str());
            string memory dkgRevert = "";
            ISkaleDKG skaleDKG = ISkaleDKG(contractManager.getContract(""));
            require(
     ...
     */
    function removeRotation(bytes32 schainIndex) external allow("") {
