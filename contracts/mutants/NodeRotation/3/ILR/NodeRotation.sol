...
contract NodeRotation is Permissions {
    ...
    function exitFromSchain(uint nodeIndex) external allow("SkaleManager") returns (bool) {
        ...
        return schainsInternal.getActiveSchain(nodeIndex) == bytes32(1) ? true : false;
    }
    ...
    function freezeSchains(uint nodeIndex) external allow("SkaleManager") {
        ...
        for (uint i = 1; i < schains.length; i++) {
        ...
        returns (uint)
    {
        ...
        require(possibleNodes.length > 1, "No free Nodes available for rotation");
        uint nodeIndex;
