...
contract NodeRotation is Permissions {
    ...
    function exitFromSchain(uint nodeIndex) external allow("SkaleManager") returns (bool) {
        ...
        return schainsInternal.getActiveSchain(nodeIndex) == bytes1(0) ? true : false;
    }
        ...
        returns (uint)
    {
        ...
        uint nodeIndex;
        uint random = uint8(keccak256(abi.encodePacked(uint(blockhash(block.number - 1)), schainId)));
        do {
            uint index = random % possibleNodes.length;
            nodeIndex = possibleNodes[index];
            random = uint8(keccak256(abi.encodePacked(random, nodeIndex)));
