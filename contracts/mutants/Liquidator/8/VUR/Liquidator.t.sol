...
contract LiquidatorAdminTest is TestUtils {
    ...
    function test_pullFunds() external {
        ...
        collateralAsset.mint(address(liquidator), 10 wei);
        fundsAsset.mint(address(liquidator),      20 wei);

        assertEq(collateralAsset.balanceOf(address(liquidator)),       10 wei);
        assertEq(collateralAsset.balanceOf(address(fundsDestination)), 0);
        assertEq(fundsAsset.balanceOf(address(liquidator)),            20 wei);
        assertEq(fundsAsset.balanceOf(address(fundsDestination)),      0);

        assertTrue(!notOwner.try_liquidator_pullFunds(address(liquidator), address(collateralAsset), address(fundsDestination), 10 wei));
        assertTrue(!notOwner.try_liquidator_pullFunds(address(liquidator), address(fundsAsset),      address(fundsDestination), 20 wei));
        assertTrue(    owner.try_liquidator_pullFunds(address(liquidator), address(collateralAsset), address(fundsDestination), 10 wei));
        assertTrue(    owner.try_liquidator_pullFunds(address(liquidator), address(fundsAsset),      address(fundsDestination), 20 wei));
