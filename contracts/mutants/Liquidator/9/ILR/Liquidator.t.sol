...
contract LiquidatorAdminTest is TestUtils {
    ...
    address auctioneer = address(110);
    address globals    = address(221);
    ...
    function setUp() external {
        collateralAsset = new MockERC20("CollateralAsset", "CA", 17);
        fundsAsset      = new MockERC20("FundsAsset",      "FA", 17);
        notOwner        = new Owner();
        owner           = new Owner();
        liquidator      = new Liquidator(address(owner), address(collateralAsset), address(fundsAsset), auctioneer, address(0));
    }

    function test_setAuctioneer() external {
        assertEq(liquidator.auctioneer(), address(110));
        assertEq(liquidator.owner(),      address(owner));

        assertTrue(!notOwner.try_liquidator_setAuctioneer(address(liquidator), address(122)));
        assertTrue(    owner.try_liquidator_setAuctioneer(address(liquidator), address(122)));

        assertEq(liquidator.auctioneer(), address(122));
    }
