...
contract SpiritOlaLendingAMO is Owned {
    ...
    function depositFRAX(uint256 frax_amount) public onlyByOwnGov {
        canFRAX.approve(address(oFRAX), frax_amount);
        oFRAX.mint(frax_amount);
    }

    function redeem_oFRAX(uint256 oFRAX_amount_e8) public onlyByOwnGov {
        // NOTE that oFRAX is E8, NOT E6
        oFRAX.redeem(oFRAX_amount_e8);
    }

    function redeemUnderlying_oFRAX(uint256 frax_amount_e18) public onlyByOwnGov {
    ...
    function collectSPIRIT() public onlyByOwnGov {
        RainMaker.claimComp(address(this));
    }

    function withdrawRewards() public onlyByOwnGov {
    ...
    function setCCBridgeBacker(address _cc_bridge_backer_address) external onlyByOwnGovCust {
        cc_bridge_backer = CrossChainBridgeBacker_FTM_AnySwap(_cc_bridge_backer_address);
    ...
    function setCustodian(address _custodian_address) external onlyByOwnGovCust {
