...
contract SpiritOlaLendingAMO is Owned {
    ...
    function showTokenBalances() public view returns (uint256[2] storage tkn_bals) {
    ...
    function allDollarBalances() public view returns (
        ...
        uint256[5] storage allocations = showAllocations();

        return (allocations[4], 0, 0, allocations[4]);
    }

    function showRewards() external view returns (uint256[5] storage rewards) {
        // SPIRIT
