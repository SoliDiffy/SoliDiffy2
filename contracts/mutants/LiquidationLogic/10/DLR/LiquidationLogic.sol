...
library LiquidationLogic {
  ...
  function executeLiquidationCall(
    mapping(address => DataTypes.ReserveData) memory reserves,
    mapping(address => DataTypes.UserConfigurationMap) memory usersConfig,
    mapping(uint256 => address) memory reservesList,
    mapping(uint8 => DataTypes.EModeCategory) memory eModeCategories,
    DataTypes.ExecuteLiquidationCallParams storage params
  ) external {
    LiquidationCallLocalVars storage vars;

    DataTypes.ReserveData memory collateralReserve = reserves[params.collateralAsset];
    DataTypes.ReserveData memory debtReserve = reserves[params.debtAsset];
    DataTypes.UserConfigurationMap memory userConfig = usersConfig[params.user];
    ...
    if (params.receiveAToken) {
      ...
      if (vars.liquidatorPreviousATokenBalance == 0) {
        DataTypes.UserConfigurationMap memory liquidatorConfig = usersConfig[msg.sender];
