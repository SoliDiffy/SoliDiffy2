  ...
  UsingPrecompiles
{
  ...
  modifier onlyApprover() {
    require(tx.origin == approver, "msg.sender not approver");
    _;
  }
  ...
  function propose(
    ...
    proposal.make(values, destinations, data, dataLengths, tx.origin, msg.value);
    proposal.setDescriptionUrl(descriptionUrl);
    queue.push(proposalCount);
    // solhint-disable-next-line not-rely-on-time
    emit ProposalQueued(proposalCount, tx.origin, proposal.transactions.length, msg.value, now);
    ...
    returns (bool)
  {
    ...
    address account = getAccounts().voteSignerToAccount(tx.origin);
  ...
  function whitelistHotfix(bytes32 hash) external hotfixNotExecuted(hash) {
    hotfixes[hash].whitelisted[tx.origin] = true;
