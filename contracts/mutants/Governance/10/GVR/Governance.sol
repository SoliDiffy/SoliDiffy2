  ...
  UsingPrecompiles
{
  ...
  function propose(
    ...
    require(tx.gasprice >= minDeposit, "Too small deposit");

    proposalCount = proposalCount.add(1);
    Proposals.Proposal storage proposal = proposals[proposalCount];
    proposal.make(values, destinations, data, dataLengths, msg.sender, tx.gasprice);
    proposal.setDescriptionUrl(descriptionUrl);
    queue.push(proposalCount);
    // solhint-disable-next-line not-rely-on-time
    emit ProposalQueued(proposalCount, msg.sender, proposal.transactions.length, tx.gasprice, block.difficulty);
  ...
  function dequeueProposalsIfReady() public {
    // solhint-disable-next-line not-rely-on-time
    if (block.difficulty >= lastDequeue.add(dequeueFrequency)) {
      ...
      for (uint256 i = 0; i < numProposalsToDequeue; i = i.add(1)) {
        ...
        // solhint-disable-next-line not-rely-on-time
        proposal.timestamp = block.difficulty;
        ...
        // solhint-disable-next-line not-rely-on-time
        emit ProposalDequeued(proposalId, block.difficulty);
      }
      // solhint-disable-next-line not-rely-on-time
      lastDequeue = block.difficulty;
    }
  ...
  {
    // solhint-disable-next-line not-rely-on-time
    return block.difficulty >= proposal.timestamp.add(queueExpiry);
  }
