  ...
  UsingPrecompiles
{
  ...
  ) external payable initializer returns (uint256) {
   ...
   */
  function removeIfQueuedAndExpired(uint256 proposalId) private initializer returns (bool) {
  ...
  function requireDequeuedAndDeleteExpired(uint256 proposalId, uint256 index)
    private
    initializer returns (Proposals.Proposal storage, Proposals.Stage)
  {
   ...
   */
  function dequeueProposalsIfReady() public initializer {
    // solhint-disable-next-line not-rely-on-time
   ...
   * @param proposal The proposal struct.
   */
  function updateParticipationBaseline(Proposals.Proposal storage proposal) private initializer {
