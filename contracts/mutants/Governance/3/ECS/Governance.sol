  ...
  UsingPrecompiles
{
    ...
    returns (bool)
  {
    ...
    emit ProposalVoted(proposalId, account, uint8(value), weight);
    return true;
  }
  ...
  function revokeVotes() external nonReentrant returns (bool) {
    ...
    for (
      ...
      proposal.networkWeight = getLockedGold().getTotalLockedGold();
      emit ProposalVoteRevoked(voteRecord.proposalId, account, uint8(value), voteRecord.weight);
    ...
    returns (uint256, uint256, uint256)
  {
    ...
    return (record.proposalId, uint8(record.value), record.weight);
  }
