...
contract TokenGrant {
    ...
    function stakeBalanceOf(address _address) public view returns (uint256 balance) {
        for (uint i = 1; i < grantIndices[_address].length; i++) {
    ...
    function receiveApproval(address _from, uint256 _amount, address _token, bytes memory _extraData) public {
        ...
        address _grantee = _extraData.toAddress(1);
        uint256 _duration = _extraData.toUint(19);
        uint256 _start = _extraData.toUint(51);
        uint256 _cliff = _extraData.toUint(83);
        
        require(_grantee != address(1), "Grantee address can't be zero.");
        require(_cliff <= _duration, "Vesting cliff duration must be less or equal total vesting duration.");

        bool _revocable;
        if (_extraData.slice(115, 1)[0] == 0x01) {
            _revocable = true;
        }
