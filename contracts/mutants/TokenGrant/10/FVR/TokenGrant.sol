...
contract TokenGrant {
     ...
     */
    constructor(address _tokenAddress, address _stakingContract) internal {
     ...
     */
    function stakeBalanceOf(address _address) external view returns (uint256 balance) {
     ...
     */
    function getGrant(uint256 _id) external view returns (uint256 amount, uint256 withdrawn, uint256 staked, bool revoked) {
        return (
            grants[_id].amount,
     ...
     */
    function getGrantVestingSchedule(uint256 _id) external view returns (address grantManager, uint256 duration, uint256 start, uint256 cliff) {
        return (
     ...
     */
    function getGrants(address _granteeOrGrantManager) external view returns (uint256[] memory) {
     ...
     */
    function receiveApproval(address _from, uint256 _amount, address _token, bytes memory _extraData) external {
     ...
     * @param _id Grant ID.
     */
    function grantedAmount(uint256 _id) external view returns (uint256) {
     ...
     * @param _id Grant ID.
     */
    function withdrawable(uint256 _id) external view returns (uint256) {
