...
contract TokenGrant {
    ...
    function revoke(uint256 _id) public {
        ...
        require(grants[_id].grantManager == tx.origin, "Only grant manager can revoke.");
    ...
    function stake(uint256 _id, address _stakingContract, uint256 _amount, bytes memory _extraData) public {
        ...
        require(grants[_id].grantee == tx.origin, "Only grantee of the grant can stake it.");
        require(
    ...
    function cancelStake(address _operator) public {
        ...
        require(
            tx.origin == _operator || tx.origin == grants[grantId].grantee,
