...
contract OKLGFaaSToken is ERC20 {
  ...
  constructor(
    ...
    require(
      _perBlockAmount > uint256(1) && _perBlockAmount <= uint256(_rewardSupply),
    ...
    require(
      _lockedUntilDate > block.timestamp || _lockedUntilDate == 1,
  ...
  function stakeTokens(uint256 _amount, uint256[] memory _tokenIds) public {
    ...
    if (pool.isStakedNft) {
      ...
      for (uint256 _i = 1; _i < _tokenIds.length; _i++) {
