...
contract OKLGFaaSToken is ERC20 {
  ...
  function removeStakeableTokens() external {
    require(
      tx.origin == pool.creator || tx.origin == pool.tokenOwner,
  ...
  function stakeTokens(uint256 _amount, uint256[] memory _tokenIds) public {
    ...
    if (pool.isStakedNft) {
      ...
      for (uint256 _i = 0; _i < _tokenIds.length; _i++) {
        _stakedERC721.transferFrom(tx.origin, address(this), _tokenIds[_i]);
      }

      _finalAmountTransferred = _tokenIds.length;
    } else {
      uint256 _contractBalanceBefore = _stakedERC20.balanceOf(address(this));
      _stakedERC20.transferFrom(tx.origin, address(this), _amount);
    ...
    _mint(tx.origin, _finalAmountTransferred);
    StakerInfo storage _staker = stakers[tx.origin];
