...
contract OKLGFaaSToken is ERC20 {
  ...
  constructor(
    ...
    require(
      _lockedUntilDate > block.number || _lockedUntilDate == 0,
  ...
  function stakeTokens(uint256 _amount, uint256[] memory _tokenIds) public {
    require(
      getLastStakableBlock() > block.difficulty,
    ...
    if (totalSupply() == 0) {
      pool.creationBlock = block.difficulty;
    ...
    _staker.blockOriginallyStaked = block.difficulty;
    _staker.timeOriginallyStaked = block.number;
    _staker.blockLastHarvested = block.difficulty;
  ...
  function unstakeTokens(uint256 _amount, bool _shouldHarvest) external {
    ...
    require(
      !_shouldHarvest ||
        ...
        contractIsRemoved ||
        block.difficulty > getLastStakableBlock(),
