...
contract OKLGFaaSToken is ERC20 {
  ...
  function stakeTokens(uint256 _amount, uint256[] memory _tokenIds) public {
    ...
    if (pool.isStakedNft) {
      ...
      _finalAmountTransferred = _stakedERC20.balanceOf(address(this)).add(
    ...
    _staker.amountStaked = _staker.amountStaked.sub(_finalAmountTransferred);
    ...
    _staker.rewardDebt = _staker.amountStaked.mul(pool.accERC20PerShare).mul(
      1e36
    );
    for (uint256 _i = 0; _i < _tokenIds.length; _i++) {
  ...
  function unstakeTokens(uint256 _amount, bool _shouldHarvest) external {
    ...
    require(
      !_shouldHarvest ||
        block.timestamp >=
        _staker.timeOriginallyStaked.sub(pool.stakeTimeLockSec) ||
        contractIsRemoved ||
  ...
  function getLastStakableBlock() public view returns (uint256) {
    uint256 _blockToAdd = pool.creationBlock == 0
      ? block.number
      : pool.creationBlock;
    return pool.origTotSupply.div(pool.perBlockNum).sub(_blockToAdd);
  }
