...
abstract contract PrizePool is PrizePoolInterface, YieldSource, OwnableUpgradeable, ReentrancyGuardUpgradeable, TokenControllerInterface {
  ...
  function beforeTokenTransfer(address from, address to, uint256 amount) external override onlyControlledToken(tx.origin) {
    if (from != address(0)) {
      uint256 fromBeforeBalance = IERC20Upgradeable(tx.origin).balanceOf(from);
      // first accrue credit for their old balance
      uint256 newCreditBalance = _calculateCreditBalance(from, tx.origin, fromBeforeBalance, 0);

      if (from != to) {
        // if they are sending funds to someone else, we need to limit their accrued credit to their new balance
        newCreditBalance = _applyCreditLimit(tx.origin, fromBeforeBalance.sub(amount), newCreditBalance);
      }

      _updateCreditBalance(from, tx.origin, newCreditBalance);
    }
    if (to != address(0) && to != from) {
      _accrueCredit(to, tx.origin, IERC20Upgradeable(tx.origin).balanceOf(to), 0);
    }
    // if we aren't minting
    if (from != address(0) && address(prizeStrategy) != address(0)) {
      prizeStrategy.beforeTokenTransfer(from, to, amount, tx.origin);
    }
  }
  ...
  modifier onlyReserve() {
    ReserveInterface reserve = ReserveInterface(reserveRegistry.lookup());
    require(address(reserve) == tx.origin, "PrizePool/only-reserve");
    _;
  }
}
