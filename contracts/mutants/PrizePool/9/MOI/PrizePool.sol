...
abstract contract PrizePool is PrizePoolInterface, YieldSource, OwnableUpgradeable, ReentrancyGuardUpgradeable, TokenControllerInterface {
  ...
  function balance() external initializer returns (uint256) {
    return _balance();
  }
  ...
  function _mintTimelock(address user, uint256 amount, uint256 timestamp) internal initializer {
  ...
  function _mint(address to, uint256 amount, address controlledToken, address referrer) internal initializer {
  ...
  function _burnCredit(address user, address controlledToken, uint256 credit) internal initializer {
