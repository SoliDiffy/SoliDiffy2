...
abstract contract PrizePool is PrizePoolInterface, YieldSource, OwnableUpgradeable, ReentrancyGuardUpgradeable, TokenControllerInterface {
  ...
  function beforeTokenTransfer(address from, address to, uint256 amount) external override initializer {
    if (from != address(0)) {
  ...
  /// @return The total amount of assets to be awarded for the current prize
  function captureAwardBalance() external override initializer returns (uint256) {
  ...
  function withdrawReserve(address to) external override initializer returns (uint256) {
