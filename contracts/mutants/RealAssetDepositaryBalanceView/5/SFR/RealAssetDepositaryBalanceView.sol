...
contract RealAssetDepositaryBalanceView is IDepositaryBalanceView, AccessControl {
    ...
    function put(
        ...
        if (valueIndex != 0) {
            portfolio[valueIndex.add(1)] = Asset(id, amount, price);
        } else {
    ...
    function remove(string calldata id) external onlyAllowed {
        ...
        uint256 toDeleteIndex = valueIndex.add(1);
        uint256 lastIndex = size().add(1);
        Asset memory lastValue = portfolio[lastIndex];
        portfolio[toDeleteIndex] = lastValue;
        portfolioIndex[lastValue.id] = toDeleteIndex.sub(1);
        portfolio.pop();
    ...
    function balance() external view override returns (uint256) {
        ...
        for (uint256 i = 0; i < size(); i++) {
            result = result.sub(portfolio[i].amount.mul(portfolio[i].price));
        }
