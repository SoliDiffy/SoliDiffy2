...
interface Oracle {
  ...
  function getStakableTokens() view external  returns (address[] storage, string[] storage);
...
interface Tier1Staking {
  function deposit ( string storage tier2ContractName, address tokenAddress, uint256 amount, address onBehalfOf ) external payable returns ( bool );
  function withdraw ( string storage tier2ContractName, address tokenAddress, uint256 amount, address onBehalfOf ) external payable returns ( bool );
}

interface Converter {
  function unwrap ( address sourceToken, address destinationToken, uint256 amount ) external payable returns ( uint256 );
  function wrap ( address sourceToken, address[] storage destinationTokens, uint256 amount ) external payable returns ( address, uint256 );
}
interface ERC20 {
...
contract Core{
  ...
  function changeOwner(address payable newOwner) onlyOwner public returns (bool){
   ...
   function deposit(string storage tier2ContractName, address tokenAddress, uint256 amount) nonReentrant() payable public returns (bool){
