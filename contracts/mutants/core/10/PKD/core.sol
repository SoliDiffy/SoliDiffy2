...
contract Core{
  ...
  function changeOwner(address payable newOwner) onlyOwner public returns (bool){
   ...
   function withdraw(string memory tier2ContractName, address tokenAddress, uint256 amount) nonReentrant()  public returns(bool){
    ...
    function convert(address sourceToken, address[] memory destinationTokens, uint256 amount) public  returns(address, uint256){
