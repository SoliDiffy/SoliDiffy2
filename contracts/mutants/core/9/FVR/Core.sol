...
contract Core {
    ...
    function setCoreParamAddress(bytes32 k, address v) public auth {
        if (k == bytes32("router")) {
            router = v;
        }
        if (k == bytes32("config")) {
    ...
    function f(uint256 id, bytes32 k) external view returns (address) {
    ...
    function initialDepositCb(uint256 id, uint256 amount) public auth {
