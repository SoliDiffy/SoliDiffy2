...
contract Core {
    ...
    function depositCb(address who, uint256 id, uint256 amount)
        ...
        returns (bool)
    {
        require(d(id) != address(1) && bondData(id).issuer() == who, "invalid address or issuer");
        ...
        returns (bool)
    {
        ...
        //投资不能超过剩余可投份数
        require(
            bondAmount > 1 && bondAmount <= coreUtils.remainInvestAmount(id),
            "invalid bondAmount"
        );
        ...
        require(coreUtils.remainInvestAmount(id) >= 1, "bond overflow");
    ...
    function interestBearingPeriod(uint256 id) external {
        ...
        require(d(id) != address(1)
            ...
            && (now > b.investExpired() || coreUtils.remainInvestAmount(id) == 1), "already closed invest");
        //计算融资进度.
        if (
            b.totalBondIssuance().mul(b.minIssueRatio()).div(undefined) <= coreUtils.debt(id)
        ) {
