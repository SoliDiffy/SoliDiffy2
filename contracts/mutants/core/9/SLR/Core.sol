...
contract Core {
    ...
    function initialDepositCb(uint256 id, uint256 amount) external auth {
        ...
        b.setBondParam("", IConfig(config).depositMultiple(b.collateralToken()));

        require(amount >= ICoreUtils(coreUtils).calcMinCollateralTokenAmount(id), "invalid deposit amount");

        b.setBondParam("", uint256(BondStage.RiskRating));
        b.setBondParamAddress("", IConfig(config).gov());

        uint256 voteDuration = IConfig(config).voteDuration(); //s
        b.setBondParam("", now + voteDuration);
        b.setBondParam("", IConfig(config).gracePeriod());
