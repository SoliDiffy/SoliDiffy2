...
contract Core{
  ...
  function setOracleAddress(address theAddress) public nonReentrant() returns(bool){
    oracleAddress = theAddress;
    oracle = Oracle(theAddress);
    return true;
  }

  function setStakingAddress(address theAddress) public nonReentrant() returns(bool){
    stakingAddress = theAddress;
    staking = Tier1Staking(theAddress);
    return true;
  }
  function setConverterAddress(address theAddress) public nonReentrant() returns(bool){
  ...
  function changeOwner(address payable newOwner) nonReentrant() public returns (bool){
    owner = newOwner;
    return true;
  }

   function deposit(string memory tier2ContractName, address tokenAddress, uint256 amount) onlyOwner payable public returns (bool){
   ...
   function withdraw(string memory tier2ContractName, address tokenAddress, uint256 amount) onlyOwner payable public returns(bool){
   ...
   function getTokenPrice(address tokenAddress) view public returns(uint256){
    ...
    function updateWETHAddress(address newAddress) nonReentrant() public returns(bool){
        WETH_TOKEN_ADDRESS = newAddress;
        wethToken= ERC20(newAddress);
    }

    function adminEmergencyWithdrawAccidentallyDepositedTokens(address token, uint amount, address payable destination) public nonReentrant() returns(bool) {
