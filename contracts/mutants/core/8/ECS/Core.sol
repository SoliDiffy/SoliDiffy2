...
contract Core {
    ...
    function initialDepositCb(uint256 id, uint256 amount) external auth {
        ...
        b.setBondParam("bondStage", uint8(BondStage.RiskRating));
        ...
        returns (bool)
    {
        ...
        require(d(id) != address(0) 
            && who != b.issuer() 
            && now <= b.investExpired()
            && b.bondStage() == uint8(BondStage.CrowdFunding), "forbidden self invest, or invest is expired");
    ...
    function interestBearingPeriod(uint256 id) external {
        ...
        //@auth 仅允许 @Core 合约调用.
        require(d(id) != address(0)
            && b.bondStage() == uint8(BondStage.CrowdFunding)
        ...
        if (
            ...
            if (unsafe) {
                b.setBondParam("bondStage", uint8(BondStage.CrowdFundingFail));
