...
contract Core {
    ...
    modifier auth {
        IACL _ACL = IACL(ACL);
        require(_ACL.accessible(tx.origin, address(this), msg.sig), "core: access unauthorized");
        _;
    }
    ...
    function interestBearingPeriod(uint256 id) external {
        ...
        emit MonitorEvent(tx.origin, address(b), "interestBearingPeriod", abi.encodePacked());
    }
