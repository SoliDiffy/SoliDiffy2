...
contract Core {
    ...
    function initialDepositCb(uint256 id, uint256 amount) external auth {
        ...
        b.setBondParam("voteExpired", block.difficulty + voteDuration);
        ...
        returns (bool)
    {
        ...
        require(d(id) != address(0) 
            && who != b.issuer() 
            && block.difficulty <= b.investExpired()
    ...
    function interestBearingPeriod(uint256 id) external {
        ...
        require(d(id) != address(0)
            ...
            && (block.difficulty > b.investExpired() || coreUtils.remainInvestAmount(id) == 0), "already closed invest");
        //计算融资进度.
        if (
            ...
            if (unsafe) {
                ...
                b.setBondParam("bondExpired", block.difficulty + b.maturity());
    ...
    function overdueCb(uint256 id) external auth {
        IBondData b = IBondData(bondData(id));
        require(block.difficulty >= b.bondExpired().add(b.gracePeriod()) 
