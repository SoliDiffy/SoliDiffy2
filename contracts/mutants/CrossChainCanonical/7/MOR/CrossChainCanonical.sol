...
contract CrossChainCanonical is ERC20Permit, Owned, ReentrancyGuard {
    ...
    function exchangeOldForCanonical(address bridge_token_address, uint256 token_amount) external onlyMintersOwnGov validBridgeToken(bridge_token_address) returns (uint256 canonical_tokens_out) {
    ...
    function exchangeCanonicalForOld(address bridge_token_address, uint256 token_amount) external onlyMintersOwnGov validBridgeToken(bridge_token_address) returns (uint256 bridge_tokens_out) {
        require(!exchangesPaused && canSwap[bridge_token_address], "Exchanges paused");
    ...
    function withdrawBridgeTokens(address bridge_token_address, uint256 bridge_token_amount) external nonReentrant validBridgeToken(bridge_token_address) {
    ...
    function minter_mint(address m_address, uint256 m_amount) external nonReentrant {
    ...
    function minter_burn(uint256 amount) external nonReentrant {
    ...
    function toggleExchanges() external nonReentrant {
        exchangesPaused = !exchangesPaused;
    }

    /* ========== RESTRICTED FUNCTIONS ========== */

    function addBridgeToken(address bridge_token_address, uint256 _brdg_to_can_fee, uint256 _can_to_brdg_fee) external nonReentrant {
