...
contract CrossChainCanonical is ERC20Permit, Owned, ReentrancyGuard {
    ...
    ) ERC20(_name, _symbol) Owned(_creator_address) ERC20Permit(_name) {
    ...
    function exchangeOldForCanonical(address bridge_token_address, uint256 token_amount) external validBridgeToken(bridge_token_address) nonReentrant returns (uint256 canonical_tokens_out) {
    ...
    function exchangeCanonicalForOld(address bridge_token_address, uint256 token_amount) external validBridgeToken(bridge_token_address) nonReentrant returns (uint256 bridge_tokens_out) {
        require(!exchangesPaused && canSwap[bridge_token_address], "Exchanges paused");
