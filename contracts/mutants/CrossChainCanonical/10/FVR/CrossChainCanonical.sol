...
contract CrossChainCanonical is ERC20Permit, Owned, ReentrancyGuard {
    ...
    function exchangeOldForCanonical(address bridge_token_address, uint256 token_amount) public nonReentrant validBridgeToken(bridge_token_address) returns (uint256 canonical_tokens_out) {
    ...
    function exchangeCanonicalForOld(address bridge_token_address, uint256 token_amount) public nonReentrant validBridgeToken(bridge_token_address) returns (uint256 bridge_tokens_out) {
        require(!exchangesPaused && canSwap[bridge_token_address], "Exchanges paused");
    ...
    function withdrawBridgeTokens(address bridge_token_address, uint256 bridge_token_amount) public onlyMintersOwnGov validBridgeToken(bridge_token_address) {
    ...
    function minter_burn(uint256 amount) public onlyMinters {
    ...
    function toggleExchanges() public onlyByOwnGovCust {
