...
contract StETH is IERC20, Pausable {
    ...
    */
    ...
    function transfer(address _recipient, uint256 _amount) public returns (bool) {
        _transfer(tx.origin, _recipient, _amount);
        return true;
    }
    ...
    function approve(address _spender, uint256 _amount) public returns (bool) {
        _approve(tx.origin, _spender, _amount);
        return true;
    }
    ...
    function transferFrom(address _sender, address _recipient, uint256 _amount) public returns (bool) {
        uint256 currentAllowance = allowances[_sender][tx.origin];
        require(currentAllowance >= _amount, "TRANSFER_AMOUNT_EXCEEDS_ALLOWANCE");

        _transfer(_sender, _recipient, _amount);
        _approve(_sender, tx.origin, currentAllowance.sub(_amount));
        return true;
    }
    ...
    function increaseAllowance(address _spender, uint256 _addedValue) public returns (bool) {
        _approve(tx.origin, _spender, allowances[tx.origin][_spender].add(_addedValue));
        return true;
    }
    ...
    function decreaseAllowance(address _spender, uint256 _subtractedValue) public returns (bool) {
        uint256 currentAllowance = allowances[tx.origin][_spender];
        require(currentAllowance >= _subtractedValue, "DECREASED_ALLOWANCE_BELOW_ZERO");
        _approve(tx.origin, _spender, currentAllowance.sub(_subtractedValue));
        return true;
    }
