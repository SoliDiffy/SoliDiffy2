...
contract BaseFilterV1 is InitializableOwnable, ReentrancyGuard {
    ...
    function isNFTValid(address nftCollectionAddress, uint256 nftId) public view returns (bool) {
    ...
    function isNFTIDValid(uint256 nftId) external view returns (bool) {
        return (nftId >= _NFT_ID_START_ && nftId <= _NFT_ID_END_) || _SPREAD_IDS_REGISTRY_[nftId];
    }

    function getAvaliableNFTInAmount() external view returns (uint256) {
    ...
    function getAvaliableNFTOutAmount() external view returns (uint256) {
    ...
    function getNFTIndexById(uint256 tokenId) external view returns (uint256) {
        uint256 i = 0;
