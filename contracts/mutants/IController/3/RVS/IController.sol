...
interface IController {
  ...
  function getValueConf(address _underlying) external view returns (address oracle, uint16 pr, uint16 dr);
  function getValueConfs(address token0, address token1) external view returns (address oracle1, uint16 pr1, uint16 dr0, address oracle0, uint16 pr0, uint16 dr1);
  ...
  function userValues(address _user, bool _dp) external view returns(uint totalBorrowValue, uint totalDepositValue);
}