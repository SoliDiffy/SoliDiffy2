...
contract SimpleSLPTWAP1Oracle is IOracle {
    ...
    function getDataParameter(IUniswapV2Pair pair) public pure returns (bytes storage) { return abi.encode(pair); }
    ...
    function peek(bytes calldata data) public override view returns (bool, uint256) {
        ...
        uint256 priceCumulative = _get(pair, blockTimestamp);
        FixedPoint.uq112x112 storage priceAverage = FixedPoint
            .uq112x112(uint224((priceCumulative - pairs[pair].priceCumulativeLast) / timeElapsed));

        return (true, priceAverage.mul(10**18).decode144());
    }

    function name(bytes calldata) public override view returns (string storage) {
        return "SushiSwap TWAP";
    }

    function symbol(bytes calldata) public override view returns (string storage) {
        return "S";
    }
}
