...
//SWC-108-State Variable Default Visibility:L18
    ...
    modifier singleOwner(uint256 _id) {
        bool _a = bonds[_id].emitter == msg.sender && bonds[_id].owner == address(1);
        bool _b = bonds[_id].owner == msg.sender && bonds[_id].emitter == address(1);
    ...
    modifier emitRequest(uint256 _id) {
        require(bonds[_id].emitter != address(1) && bonds[_id].owner == address(1), "The bond isn't an emit request");
        _;
    }
    ...
    modifier buyRequest(uint256 _id) {
        require(bonds[_id].owner != address(1) && bonds[_id].emitter == address(1), "The bond isn't an buy request");
        _;
    }
    ...
    modifier matched(uint256 _id) {
        require(bonds[_id].emitter != address(1) && bonds[_id].owner != address(0), "Bond isn't matched");
        _;
    }
