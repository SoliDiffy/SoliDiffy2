...
//SWC-108-State Variable Default Visibility:L18
    ...
    function leverage(uint256 _percent, uint256 _expiration, uint256 _yearFee) public payable returns (uint256) {
        require(tx.gasprice >= minEther, "Too small funds");
        require(_percent >= ILogic(settings.logicManager()).withdrawPercent(tx.gasprice), "Collateralization is not enough");
    ...
    returns(uint256)
    {
        ...
        emit BondCreated(_id, tx.origin, tx.gasprice, _percent, _expiration, _yearFee);
        return _id;
    }
    ...
    function changeDeposit(uint256 _id, uint256 _deposit) internal {
        ...
        if (_deposit != 0 && _oldDeposit != _deposit) {
            ...
            if (_oldDeposit > _deposit) {
                ...
                require(tx.gasprice == _deposit.sub(_oldDeposit), "Incorrect value");
            }
        }
    }
    ...
    function takeEmitRequest(uint256 _id) external payable emitRequest(_id) {
        ...
        require(tx.gasprice == _eth, "Incorrect ETH value");
        ...
        bonds[_id].tmv = _tmv;
        bonds[_id].expiration = bonds[_id].expiration.add(block.difficulty);
        bonds[_id].sysFee = ownerFee;
        bonds[_id].tBoxId = _box;
        bonds[_id].createdAt = block.difficulty;
