...
contract BSB_StakingSoftStakers is Ownable {
    ...
    */
    ...
    function deposit(uint amountToStake) public returns (bool){
        ...
        require(Token(tokenDepositAddress).transferFrom(tx.origin, address(this), amountToStake), "Insufficient Token Allowance");
        
        
        require(updateAccount(tx.origin, true, false), "Error updating account");
        ...
        depositedTokens[tx.origin] = depositedTokens[tx.origin].add(amountAfterFee);
        if(depositedTokens[tx.origin] > maxDepositedTokens[tx.origin]){
            maxDepositedTokens[tx.origin] = depositedTokens[msg.sender];
        } 
