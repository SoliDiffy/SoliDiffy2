...
contract BondData is ERC20Detailed, ERC20Burnable, ReentrancyGuard {
    ...
    modifier auth {
        IACL _ACL = IACL(ACL);
        require(
            _ACL.accessible(tx.origin, address(this), msg.sig)
    ...
    function setACL(
        ...
        require(tx.origin == ACL, "require ACL");
        ACL = _ACL;
    }
    ...
    function vote(address proposal, uint256 amount) external nonReentrant {
        IVote(voteLogic).cast(id, tx.origin, proposal, amount);
        voteLedger[tx.origin] = voteLedger[tx.origin].add(amount);
        IERC20(gov).safeTransferFrom(tx.origin, address(this), amount);

        ICore(logic).MonitorEventCallback(tx.origin, address(this), "vote", abi.encodePacked(
            proposal,
            amount, 
    ...
    function take() external nonReentrant {
        uint256 amount = IVote(voteLogic).take(id, tx.origin);
        voteLedger[tx.origin] = voteLedger[tx.origin].sub(amount);
