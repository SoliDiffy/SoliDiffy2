...
contract BondData is ERC20Detailed, ERC20Burnable, ReentrancyGuard {
    ...
    function setLogics(address _logic, address _voteLogic) external nonReentrant {
        logic = _logic;
        voteLogic = _voteLogic;
    }

    function setBondParam(bytes32 k, uint256 v) external nonReentrant {
    ...
    function setBondParamAddress(bytes32 k, address v) external nonReentrant {
        if (k == bytes32("gov")) {
            gov = v;
        }
    ...
    function setLiquidating(bool _liquidating) external nonReentrant {
    ...
    function setPr(address who, address proposal, uint256 reason) external nonReentrant {
        pr.who = who;
    ...
    function setBondParamMapping(bytes32 name, address k, uint256 v) external nonReentrant {
    ...
    function vote(address proposal, uint256 amount) external auth {
