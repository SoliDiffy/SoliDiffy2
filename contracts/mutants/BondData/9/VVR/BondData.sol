...
contract BondData is ERC20Detailed, ERC20Burnable, ReentrancyGuard {
    ...
    uint256 internal id;
    address internal issuer; //发债方
    address internal collateralToken; //质押代币
    address internal crowdToken; //融资代币地址

    uint256 internal totalBondIssuance; //预计发行量，债券发行总量，以USDT计
    uint256 internal actualBondIssuance; //实际发行份数
    uint256 internal mintCnt;//增发的次数
