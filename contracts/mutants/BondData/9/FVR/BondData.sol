...
contract BondData is ERC20Detailed, ERC20Burnable, ReentrancyGuard {
    ...
    ) internal ERC20Detailed(_bondName, _bondName, 0) {
        ACL = _ACL;
        id = bondId;
        issuer = _issuer;
    ...
    function setLogics(address _logic, address _voteLogic) public auth {
        logic = _logic;
        voteLogic = _voteLogic;
    }

    function setBondParam(bytes32 k, uint256 v) public auth {
    ...
    function setBondParamAddress(bytes32 k, address v) public auth {
        if (k == bytes32("gov")) {
            gov = v;
        }
    ...
    function getSupplyAmount(address who) public view returns (uint256, uint256) {
        return (supplyMap[who].amountGive, supplyMap[who].amountGet);
    }

    function getBorrowAmountGive() public view returns (uint256) {
    ...
    function setLiquidating(bool _liquidating) public auth {
