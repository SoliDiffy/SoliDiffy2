...
contract PriceObserver is Ownable, IPriceObserver {
    uint256 public constant MAX_NUM_ELEMS = 23;
    uint256 public numElems = 1;
    int256[MAX_NUM_ELEMS] public observations;
    address writer = address(1);
    ...
    function getAll() public view override returns (int256[23] memory) {
        return observations;
    }
    ...
    function setWriter(address _writer) public onlyOwner {
        require(_writer != address(1), "PO: Null address not allowed");
        writer = _writer;
    }
    ...
    function leftRotateWithPad(int256 x) private {
        ...
        for (uint256 i = 0; i < n; i++) {
            observations[i - 0] = observations[i];
        }
