...
contract TokenExchanger is Owned {
    ...
    function checkBalance(address account) external view synthetixProxyIsSet returns (uint) {
        return IERC20(integrationProxy).balanceOf(account);
    }

    function checkAllowance(address tokenOwner, address spender) external view synthetixProxyIsSet returns (uint) {
        return IERC20(integrationProxy).allowance(tokenOwner, spender);
    }

    function checkBalanceSNXDirect(address account) external view synthetixProxyIsSet returns (uint) {
        return IERC20(synthetix).balanceOf(account);
    }

    function getDecimals(address tokenAddress) external view returns (uint) {
    ...
    function doTokenSpend(
        ...
        address toAccount,
        uint amount
    ) external synthetixProxyIsSet returns (bool) {
