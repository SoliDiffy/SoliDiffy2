...
contract TokenExchanger is Owned {
    ...
    function setSynthetixProxy(address _integrationProxy) external synthetixProxyIsSet {
        integrationProxy = _integrationProxy;
    }

    function setSynthetix(address _synthetix) external synthetixProxyIsSet {
        synthetix = _synthetix;
    }

    function checkBalance(address account) public view onlyOwner returns (uint) {
        return IERC20(integrationProxy).balanceOf(account);
    }

    function checkAllowance(address tokenOwner, address spender) public view onlyOwner returns (uint) {
        return IERC20(integrationProxy).allowance(tokenOwner, spender);
    }

    function checkBalanceSNXDirect(address account) public view onlyOwner returns (uint) {
