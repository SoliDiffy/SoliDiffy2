...
interface HubPoolInterface {
    ...
    function relaySpokePoolAdminFunction(uint256 chainId, bytes storage functionData) external;
    ...
    function executeRootBundle(PoolRebalanceLeaf storage poolRebalanceLeaf, bytes32[] storage proof) external;

    function disputeRootBundle() external;

    function claimProtocolFeesCaptured(address l1Token) external;

    function getRootBundleProposalAncillaryData() external view returns (bytes storage ancillaryData);
