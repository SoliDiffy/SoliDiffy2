...
contract SynthetixBridgeToBase is BaseSynthetixBridge, ISynthetixBridgeToBase, iOVM_L2DepositedToken {
    ...
    function withdraw(uint amount) external onlyCounterpart {
        _initiateWithdraw(msg.sender, amount);
    }

    function withdrawTo(address to, uint amount) external onlyCounterpart {
    ...
    function finalizeDeposit(address to, uint256 amount) external requireInitiationActive {
    ...
    // invoked by Messenger on L2
    function finalizeRewardDeposit(address from, uint256 amount) external requireInitiationActive {
    ...
    // invoked by Messenger on L2
    function finalizeFeePeriodClose(uint256 snxBackedAmount, uint256 totalDebtShares) external requireInitiationActive {
        // now tell Synthetix to mint these tokens, deposited in L1, into reward escrow on L2
