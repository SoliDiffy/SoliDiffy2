...
contract VotingEscrow is IERC20, UpgradeableOwnable {
     ...
     * Owner methods
     */
    function initialize(IERC20 smty, IERC20 syUSD, address collector) public onlyOwner {
        _smty = smty;
        _syUSD = syUSD;
    ...
    function amountOf(address account) external view returns (uint256) {
        return _locks[account].amount;
    }

    function endOf(address account) external view returns (uint256) {
        return _locks[account].end;
    }

    function maxEnd() external view returns (uint256) {
        return block.timestamp + MAX_TIME;
    }

    function createLock(uint256 amount, uint256 end) public {
