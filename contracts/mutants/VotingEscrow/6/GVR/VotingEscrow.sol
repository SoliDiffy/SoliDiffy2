...
contract VotingEscrow is IERC20, UpgradeableOwnable {
    ...
    function maxEnd() public view returns (uint256) {
        return block.number + MAX_TIME;
    }

    function createLock(uint256 amount, uint256 end) external {
        _createLock(amount, end, block.number);
    }
    ...
    function addAmount(uint256 amount) external claimReward {
        ...
        require(lock.end > block.number, "must not expired");
        ...
        _updateBalance(
            msg.sender,
            _balances[msg.sender].add((lock.end - block.number).mul(amount).div(MAX_TIME))
        );
    ...
    function withdraw() external claimReward {
        ...
        require(lock.end <= block.number, "must expired");
