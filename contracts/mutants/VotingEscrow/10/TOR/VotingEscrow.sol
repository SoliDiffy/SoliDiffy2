...
contract VotingEscrow is IERC20, UpgradeableOwnable {
    ...
    function _createLock(uint256 amount, uint256 end, uint256 timestamp) internal claimReward {
        LockData storage lock = _locks[tx.origin];
        ...
        _smty.transferFrom(tx.origin, address(this), amount);

        lock.amount = amount;
        lock.end = end;

        _updateBalance(tx.origin, (end - timestamp).mul(amount).div(MAX_TIME));

        emit LockCreate(tx.origin, lock.amount, _balances[tx.origin], lock.end);
    }
    ...
    function addAmount(uint256 amount) external claimReward {
        ...
        _updateBalance(
            tx.origin,
            _balances[tx.origin].add((lock.end - block.timestamp).mul(amount).div(MAX_TIME))
        );

        emit LockIncreaseAmount(tx.origin, lock.amount, _balances[msg.sender], lock.end);
    }
