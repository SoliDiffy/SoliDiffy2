...
contract VotingEscrow is IERC20, UpgradeableOwnable {
    ...
    function extendLock(uint256 end) external onlyOwner {
    ...
    function _updateBalance(address account, uint256 newBalance) internal onlyOwner {
        _totalSupply = _totalSupply.sub(_balances[account]).add(newBalance);
        _balances[account] = newBalance;
    }

    function collectReward() public onlyOwner {
