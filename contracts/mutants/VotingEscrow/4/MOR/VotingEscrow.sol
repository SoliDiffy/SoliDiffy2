...
contract VotingEscrow is IERC20, UpgradeableOwnable {
     ...
     * Owner methods
     */
    function initialize(IERC20 smty, IERC20 syUSD, address collector) external claimReward {
        _smty = smty;
        _syUSD = syUSD;
    ...
    function _createLock(uint256 amount, uint256 end, uint256 timestamp) internal onlyOwner {
    ...
    function addAmount(uint256 amount) external onlyOwner {
        LockData storage lock = _locks[msg.sender];
    ...
    function _extendLock(uint256 end, uint256 timestamp) internal onlyOwner {
        LockData storage lock = _locks[msg.sender];
        require(lock.amount != 0, "must locked");
