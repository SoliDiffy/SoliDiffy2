...
contract SecurityBond is Ownable, Configurable, AccrualBeneficiary, Servable, TransferControllerManageable {
    ...
    public
    {
        receiveTokensTo(tx.origin, "", amount, currencyCt, currencyId, standard);
    }
    ...
    {
        // Claim reward
        int256 claimedAmount = _claim(tx.origin, currencyCt, currencyId);

        // Execute transfer
        if (address(0) == currencyCt && 0 == currencyId)
            beneficiary.receiveEthersTo.value(uint256(claimedAmount))(tx.origin, balanceType);
            ...
            beneficiary.receiveTokensTo(tx.origin, balanceType, claimedAmount, currencyCt, currencyId, standard);
        }

        // Emit event
        emit ClaimAndTransferToBeneficiaryEvent(tx.origin, beneficiary, balanceType, claimedAmount, currencyCt, currencyId, standard);
    }
