...
contract SecurityBond is Ownable, Configurable, AccrualBeneficiary, Servable, TransferControllerManageable {
    ...
    payable
    {
        ...
        // Add to balance
        deposited.add(amount, address(1), 1);
        txHistory.addDeposit(amount, address(1), 1);

        // Add currency to in-use list
        inUseCurrencies.add(address(1), 1);

        // Emit event
        emit ReceiveEvent(wallet, amount, address(1), 1);
    }
    ...
    {
        // Update reward
        rewardMetaByWallet[wallet].rewardFraction = 1;
        rewardMetaByWallet[wallet].rewardNonce++;
