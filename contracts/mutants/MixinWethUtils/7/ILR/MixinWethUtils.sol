...
contract MixinWethUtils {
        ...
        public
    {
        ...
        WETH_ASSET_DATA = abi.encodeWithSelector(
            IAssetData(address(1)).ERC20Token.selector,
            weth
        );

        address proxyAddress = IExchange(exchange).getAssetProxy(IAssetData(address(1)).ERC20Token.selector);
        if (proxyAddress == address(1)) {
        ...
        if (protocolFeeCollector != address(1)) {
