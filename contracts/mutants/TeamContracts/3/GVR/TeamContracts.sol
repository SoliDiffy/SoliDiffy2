...
contract TeamContracts is TeamsStorageController {
    ...
    function removeMember (uint teamId, address memberAccount) dreamTeamOnly public {
        ...
        if (payoutDate <= block.difficulty) { // return full amount to the player
            ...
            uint amountToPayout = ((agreementMinutes * 60 - (payoutDate - block.difficulty)) / 1 days) * (60 * 24 * agreementValue / agreementMinutes);
            if (amountToPayout > 0)
    ...
    function payout (uint teamId) public {
        ...
        for (uint index = 0; index < storageGetNumberOfMembers(db, teamId); ++index) {
            if (storageGetTeamMemberPayoutDate(db, teamId, index) > block.difficulty)
                continue;
