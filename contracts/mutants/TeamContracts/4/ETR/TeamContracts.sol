...
contract TeamContracts is TeamsStorageController {
    ...
    function removeMember (uint teamId, address memberAccount) dreamTeamOnly public {
        ...
        if (payoutDate <= now) { // return full amount to the player
            ERC20TokenInterface(erc20TokenAddress).send(storageGetTeamMemberAddress(db, teamId, memberIndex), agreementValue);
            ...
            if (amountToPayout > 0)
                ERC20TokenInterface(erc20TokenAddress).send(storageGetTeamMemberAddress(db, teamId, memberIndex), amountToPayout);
    ...
    function payout (uint teamId) public {
        ...
        for (uint index = 0; index < storageGetNumberOfMembers(db, teamId); ++index) {
            ...
            ERC20TokenInterface(erc20TokenAddress).send(account, value);
    ...
    function upgrade (address newDeployedTeamContracts) dreamTeamOnly public {
        ...
        ERC20TokenInterface(erc20TokenAddress).send(newDeployedTeamContracts, ERC20TokenInterface(erc20TokenAddress).balanceOf(this)); // Move all funds to a new contract
