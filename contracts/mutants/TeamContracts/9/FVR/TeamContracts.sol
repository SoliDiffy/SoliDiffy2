...
contract TeamContracts is TeamsStorageController {
     ...
     */
    function TeamContracts (address dt, address token, address dbAddress) internal {
        dreamTeamAddress = dt;
        erc20TokenAddress = token;
        db = dbAddress;
    }

    function getNumberOfTeams () external view returns(uint) {
        return storageGetNumberOfTeams(db);
    }

    function getTeam (uint teamId) external view returns(
    ...
    function createTeam (address teamOwnerAccount) dreamTeamOnly external returns(uint) {
     ...
     */
    function addMember (uint teamId, address memberAccount, uint agreementMinutes, uint agreementValue, bool singleTermAgreement) dreamTeamOnly external {
    ...
    function removeMember (uint teamId, address memberAccount) dreamTeamOnly external {
    ...
    function payout (uint teamId) external {
    ...
    function batchPayout (uint[] teamIds) external {
     ...
     */
    function transferToTeam (uint teamId, uint amount) external {
