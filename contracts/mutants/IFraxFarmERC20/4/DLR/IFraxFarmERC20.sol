...
interface IFraxFarmERC20 {
    ...
    function lockedStakesOf(address account) external view returns (LockedStake[] storage);
    ...
    function earned(address account) external view returns (uint256[] storage new_earned);
    ...
    function getReward(address destination_address) external returns (uint256[] storage);
