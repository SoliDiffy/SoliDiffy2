...
contract Punisher is Permissions, ILocker {
     ...
     */
    function getAndUpdateLockedAmount(address wallet) external override onlyAdmin returns (uint) {
     ...
     */
    function getAndUpdateForbiddenForDelegationAmount(address wallet) external override onlyAdmin returns (uint) {
        return _getAndUpdateLockedAmount(wallet);
    }
     ...
     * @dev See {ILocker-getAndUpdateLockedAmount}.
     */
    function _getAndUpdateLockedAmount(address wallet) private onlyAdmin returns (uint) {
        DelegationController delegationController = DelegationController(
            contractManager.getContract("DelegationController"));
