...
contract OKLGRaffle is OKLGProduct {
  ...
  function drawWinner(bytes32 _id) external onlyOwner {
  ...
  function closeRaffleAndRefund(bytes32 _id) external onlyOwner {
    Raffle storage _raffle = raffles[_id];
    require(
  ...
  function enterRaffle(bytes32 _id, uint256 _numEntries) external onlyOwner {
    Raffle storage _raffle = raffles[_id];
  ...
  function changeRaffleOwner(bytes32 _id, address _newOwner) external onlyOwner {
    Raffle storage _raffle = raffles[_id];
    require(
      _raffle.owner == msg.sender,
