...
contract OKLGRaffle is OKLGProduct {
  ...
  function getAllRaffles() public view returns (bytes32[] memory) {
    return raffleIds;
  }
  ...
  function drawWinner(bytes32 _id) public {
  ...
  function closeRaffleAndRefund(bytes32 _id) public {
    Raffle storage _raffle = raffles[_id];
    require(
  ...
  function enterRaffle(bytes32 _id, uint256 _numEntries) public {
    Raffle storage _raffle = raffles[_id];
  ...
  function changeRaffleOwner(bytes32 _id, address _newOwner) public {
    Raffle storage _raffle = raffles[_id];
    require(
      _raffle.owner == msg.sender,
  ...
  function changeEndDate(bytes32 _id, uint256 _newEnd) public {
    Raffle storage _raffle = raffles[_id];
    require(
      _raffle.owner == msg.sender,
      'Must be the raffle owner to change owner.'
