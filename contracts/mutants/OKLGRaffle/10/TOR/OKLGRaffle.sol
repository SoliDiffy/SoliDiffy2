...
contract OKLGRaffle is OKLGProduct {
  ...
  function createRaffle(
    ...
    bytes32 _id = sha256(abi.encodePacked(tx.origin, block.number));
    ...
    raffleIds.push(_id);
    emit CreateRaffle(tx.origin, _id);
  }
  ...
  function closeRaffleAndRefund(bytes32 _id) external {
    ...
    if (_raffle.isNft) {
      ...
      IERC20 _rewardToken = IERC20(_raffle.rewardToken);
      _rewardToken.transfer(tx.origin, _raffle.rewardAmountOrTokenId);
    }
    ...
    require(
      _numEntries > 0 &&
        (_raffle.maxEntriesPerAddress == 0 ||
          entriesIndexed[_id][tx.origin] + _numEntries <=
