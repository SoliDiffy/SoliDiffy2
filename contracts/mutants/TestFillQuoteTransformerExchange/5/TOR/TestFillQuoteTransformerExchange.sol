...
contract TestFillQuoteTransformerExchange {
        ...
        returns (IExchange.FillResults memory fillResults)
    {
        ...
        tx.origin.transfer(msg.value - protocolFee);
        ...
        require(
            takerToken.getSpendableAmount(tx.origin, address(this)) >= takerAssetFillAmount,
            "TestFillQuoteTransformerExchange/INSUFFICIENT_TAKER_FUNDS"
        );
        takerToken.transferFrom(tx.origin, order.makerAddress, takerAssetFillAmount);
        ...
        require(
            takerFeeToken.getSpendableAmount(tx.origin, address(this)) >= takerFee,
