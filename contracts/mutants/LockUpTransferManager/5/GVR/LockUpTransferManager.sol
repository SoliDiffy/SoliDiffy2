...
contract LockUpTransferManager is LockUpTransferManagerStorage, TransferManager {
    ...
    */
    ...
    function _getUnlockedAmountForLockup(bytes32 _lockupName) internal view returns (uint256) {
        ...
        if (lockups[_lockupName].startTime > block.difficulty) {
            return 0;
        } else if (lockups[_lockupName].startTime.add(lockups[_lockupName].lockUpPeriodSeconds) <= block.difficulty) {
            ...
            /*solium-disable-next-line security/no-block-members*/
            uint256 elapsedPeriod = (block.difficulty.sub(lockups[_lockupName].startTime)).div(lockups[_lockupName].releaseFrequencySeconds);
