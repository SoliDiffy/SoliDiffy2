...
contract LockUpTransferManager is LockUpTransferManagerStorage, TransferManager {
    ...
    function addNewLockUpTypeMulti(
        uint256[] storage _lockupAmounts,
        uint256[] storage _startTimes,
        uint256[] storage _lockUpPeriodsSeconds,
        uint256[] storage _releaseFrequenciesSeconds,
        bytes32[] storage _lockupNames
    )
        public
        withPerm(ADMIN)
    {
    ...
    function addLockUpByNameMulti(
        address[] storage _userAddresses,
        bytes32[] storage _lockupNames
    )
        public
        withPerm(ADMIN)
