...
library UserAccounting {
    ...
    function farmedPerToken(
        Info storage info,
        address context,
        function(address) public view returns(uint256) lazyGetSupply,
    ...
    function farmed(Info storage info, address account, uint256 balance, uint256 fpt) public view returns(uint256) {
        // SWC-101-Integer Overflow and Underflow: L30
        return uint256(int256(balance * fpt) - info.corrections[account]) / 1e18;
    }

    function eraseFarmed(Info storage info, address account, uint256 balance, uint256 fpt) public {
