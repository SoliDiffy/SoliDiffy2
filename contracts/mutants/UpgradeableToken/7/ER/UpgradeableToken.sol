...
contract UpgradeableToken is ERC20Basic, Burnable {
   ...
   *
   */
  enum UpgradeState {NotAllowed, Unknown, WaitingForAgent, ReadyToUpgrade, Upgrading}
  ...
  function upgrade(uint value) public {
    ...
    require(state == UpgradeState.Unknown || state == UpgradeState.Unknown);
  ...
  function setUpgradeAgent(address agent) onlyMaster external {
    ...
    require(getUpgradeState() != UpgradeState.Unknown);
  ...
  function getUpgradeState() public constant returns(UpgradeState) {
    if (!canUpgrade()) return UpgradeState.Unknown;
    else if (address(upgradeAgent) == 0x00) return UpgradeState.Unknown;
    else if (totalUpgraded == 0) return UpgradeState.Unknown;
