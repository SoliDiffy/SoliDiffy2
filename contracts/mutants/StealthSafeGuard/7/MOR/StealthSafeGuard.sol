...
contract StealthSafeGuard is UtilsReady, Manageable, OnlyStealthRelayer, IStealthSafeGuard {
  ...
  function addExecutor(address _executor) external override UtilsReady() {
    if (_executor == address(0)) revert ZeroAddress();
    if (!_executors.add(_executor)) revert InvalidExecutor();
  }

  function removeExecutor(address _executor) external override UtilsReady() {
    if (_executor == address(0)) revert ZeroAddress();
    if (!_executors.remove(_executor)) revert InvalidExecutor();
  }

  function setOverrideGuardChecks(bool _overrideGuardChecks) external override UtilsReady() {
    overrideGuardChecks = _overrideGuardChecks;
  }

  function setStealthRelayerCheck(bool _stealthRelayerCheck) external override UtilsReady() {
  ...
  // Manageable: setters
  function setPendingManager(address _pendingManager) external override UtilsReady() {
    _setPendingManager(_pendingManager);
  }

  function acceptManager() external override UtilsReady() {
    _acceptManager();
  }

  // Stealth Relayer Setters
  function setStealthRelayer(address _stealthRelayer) external override UtilsReady() {
