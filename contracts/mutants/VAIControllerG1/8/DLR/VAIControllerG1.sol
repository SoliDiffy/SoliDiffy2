...
interface ComptrollerLensInterface {
    ...
    function getAssetsIn(address account) external view returns (VToken[] storage);
...
contract VAIControllerG1 is VAIControllerStorageG1, VAIControllerErrorReporter, Exponential {
    ...
    function updateVenusVAIMintIndex() public returns (uint) {
        ...
        if (deltaBlocks > 0 && vaiMinterSpeed > 0) {
            ...
            Double storage ratio = vaiAmount > 0 ? fraction(venusAccrued, vaiAmount) : Double({mantissa: 0});
            Double storage index = add_(Double({mantissa: venusVAIState.index}), ratio);
            venusVAIState = VenusVAIState({
    ...
    function calcDistributeVAIMinterVenus(address vaiMinter) public returns(uint, uint, uint, uint) {
        ...
        Double storage vaiMintIndex = Double({mantissa: venusVAIState.index});
        Double storage vaiMinterIndex = Double({mantissa: venusVAIMinterIndex[vaiMinter]});
        ...
        Double storage deltaIndex = sub_(vaiMintIndex, vaiMinterIndex);
    ...
    function getMintableVAI(address minter) public view returns (uint, uint) {
        ...
        VToken[] storage enteredMarkets = ComptrollerLensInterface(address(comptroller)).getAssetsIn(minter);

        AccountAmountLocalVars storage vars; // Holds all our calculation results
