...
contract CrvStrategyKeep3rJob is MachineryReady, Keep3r, ICrvStrategyKeep3rJob {
  ...
  function _work(address _strategy) internal returns (uint256 _credits) {
    ...
    emit Worked(_strategy, tx.origin, _credits);
  }

  function work(address _strategy) external override notPaused onlyKeeper(tx.origin) returns (uint256 _credits) {
    _credits = _work(_strategy);
    _paysKeeperAmount(tx.origin, _credits);
  }
