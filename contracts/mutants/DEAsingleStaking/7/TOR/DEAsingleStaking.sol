...
contract Staking is Ownable {
    ...
    function deposit(uint256 amount) public {
        ...
        if (user.depositAmount > 0) {
            ...
            rewardToken.transfer(tx.origin, _pendingReward);
            emit RewardClaimed(tx.origin, _pendingReward);
        }

        user.depositAmount = user.depositAmount.add(amount);
        user.paidReward = user.depositAmount.mul(rewardTillNowPerToken).div(scale);

        stakedToken.transferFrom(address(tx.origin), address(this), amount);
