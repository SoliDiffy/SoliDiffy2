...
contract HubPool is HubPoolInterface, Testable, Lockable, MultiCaller, Ownable {
     ...
     */
    function executeRootBundle(PoolRebalanceLeaf memory poolRebalanceLeaf, bytes32[] memory proof) external nonReentrant {
    ...
    function _cancelBundle(bytes memory ancillaryData) public {
    ...
    function _getOptimisticOracle() public view returns (SkinnyOptimisticOracleInterface) {
        return
            SkinnyOptimisticOracleInterface(finder.getImplementationAddress(OracleInterfaces.SkinnyOptimisticOracle));
    }

    function _getBondTokenFinalFee() public view returns (uint256) {
        return
