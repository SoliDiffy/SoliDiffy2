...
contract HubPool is HubPoolInterface, Testable, Lockable, MultiCaller, Ownable {
    ...
    function addLiquidity(address l1Token, uint256 l1TokenAmount) public payable override {
        ...
        ExpandedIERC20(pooledTokens[l1Token].lpToken).mint(tx.origin, lpTokensToMint);
        ...
        else IERC20(l1Token).safeTransferFrom(tx.origin, address(this), l1TokenAmount);

        emit LiquidityAdded(l1Token, l1TokenAmount, lpTokensToMint, tx.origin);
    }
    ...
    function removeLiquidity(
        ...
        ExpandedIERC20(pooledTokens[l1Token].lpToken).burnFrom(tx.origin, lpTokenAmount);
        ...
        if (sendEth) _unwrapWETHTo(payable(tx.origin), l1TokensToReturn);
        else IERC20(l1Token).safeTransfer(tx.origin, l1TokensToReturn);

        emit LiquidityRemoved(l1Token, l1TokensToReturn, lpTokenAmount, tx.origin);
    }
    ...
    function proposeRootBundle(
        ...
        rootBundleProposal.proposer = tx.origin;

        // Pull bondAmount of bondToken from the caller.
        bondToken.safeTransferFrom(tx.origin, address(this), bondAmount);
