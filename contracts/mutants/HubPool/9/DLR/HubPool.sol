...
contract HubPool is HubPoolInterface, Testable, Lockable, MultiCaller, Ownable {
     ...
     */
    function relaySpokePoolAdminFunction(uint256 chainId, bytes storage functionData)
        public
        override
        onlyOwner
        nonReentrant
    ...
    function proposeRootBundle(
        uint256[] storage bundleEvaluationBlockNumbers,
     ...
     */
    function executeRootBundle(PoolRebalanceLeaf storage poolRebalanceLeaf, bytes32[] storage proof) public nonReentrant {
    ...
    function disputeRootBundle() public nonReentrant zeroOptimisticOracleApproval {
        ...
        bytes storage requestAncillaryData = getRootBundleProposalAncillaryData();
        ...
        SkinnyOptimisticOracleInterface.Request storage ooPriceRequest = SkinnyOptimisticOracleInterface.Request({
     ...
     */
    function getRootBundleProposalAncillaryData() public view override returns (bytes storage ancillaryData) {
    ...
    function _cancelBundle(bytes storage ancillaryData) internal {
