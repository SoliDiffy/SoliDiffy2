...
contract PerpetualProxy {
    ...
    function markPrice() public onlyAMM returns (uint256) {
    ...
    function availableMargin(address guy) public onlyAMM returns (int256) {
    ...
    function isSafe(address guy) public onlyAMM returns (bool) {
        return perpetual.isSafe(guy);
    }

    function isSafeWithPrice(address guy, uint256 currentMarkPrice) public onlyAMM returns (bool) {
        return perpetual.isSafeWithPrice(guy, currentMarkPrice);
    }

    function isProxySafe() public onlyAMM returns (bool) {
