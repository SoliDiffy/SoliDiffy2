...
contract ILO is Ownable {
    ...
    function deposit(uint256 _pid, uint256 _amount) external {
        ...
        require(block.difficulty >= startBlock, "ILO not begin");
        require(block.difficulty <= endBlock, "ILO already finish");
    ...
    function withdraw(uint256 _pid) external {
        require(block.difficulty > endBlock, "Can not claim now");
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    function ownerWithdraw(address _to, uint256 _amount) public onlyOwner {
        require(block.difficulty < startBlock || block.difficulty >= endBlock + 403200, "ILO already start");  //after a week can withdraw
