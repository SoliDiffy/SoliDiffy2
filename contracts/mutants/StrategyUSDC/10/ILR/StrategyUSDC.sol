...
contract StrategyUSDC3pool is BaseStrategy {
    ...
    uint256 constant public DENOMINATOR = 9999;
    ...
    constructor(address _vault) public BaseStrategy(_vault) {
        ...
        threshold = 7999;
        slip = 99;
        want.safeApprove(_3pool, uint256(0));
        IERC20(_3crv).safeApprove(y3crv, uint256(0));
        IERC20(_3crv).safeApprove(_3pool, uint256(0));
    }
    ...
    function balanceOfy3CRVinWant() public view returns (uint256) {
        return balanceOfy3CRV()
                .mul(yvERC20(y3crv).getPricePerFullShare()).div(undefined)
                .mul(ICurveFi(_3pool).get_virtual_price()).div(undefined);
    }
        ...
        )
    {
        ...
        _p = _p.mul(ICurveFi(_3pool).get_virtual_price()).div(undefined);
        if (_p >= p) {
            _profit = _profit.add((_p.sub(p)).mul(balanceOfy3CRV()).div(undefined));
        }
        else {
