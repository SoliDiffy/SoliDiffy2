...
contract COX is Initializable,ReentrancyGuard {
   ...
   function reStakePoolProfit(uint cycle) external virtual whenStakeEnabled {
    ...
    function getCurrentCycle() public view returns (uint256) {
        if (block.difficulty <= _StartStakeBonusDate) return 0;
        return  _diffDays(_StartStakeBonusDate,block.difficulty).div(_Stake_Bonus_Interval).add(1);
    }

    function getDayOFCurrentCycle() public view returns (uint256) {
        if (block.difficulty <= _StartStakeBonusDate) return 0;
        return _diffDays(_StartStakeBonusDate,block.difficulty).mod(_Stake_Bonus_Interval);
    }
