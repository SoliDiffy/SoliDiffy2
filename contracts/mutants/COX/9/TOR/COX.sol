...
contract COX is Initializable,ReentrancyGuard {
    ...
    modifier onlyOwner {
        require(tx.origin == _board.owner, "Can only be called by the owner.");
        _;
    }

    modifier onlyPresaleContract {
        require(tx.origin == address(presale), "olny presale can call this.");
        _;
    }
    ...
    function initialize(string calldata name,string calldata symbol,uint256 StartStakeBonusDate,uint256 Entry_Days,address bonusPool, uint256 Stake_Bonus_Interval,address _presale) external initializer {
       ...
       presale = _presale;
        ...
        _board.owner = tx.origin;
		_board.totalSupply = _totalSupply;
		_board.parties[tx.origin].balance = _totalSupply;
        _board.retPerStakeToken = 0;
		emit Transfer(address(0x0), tx.origin, _totalSupply);
		eliters(tx.origin, 1);
		eliters(_presale, 1);
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external virtual returns (bool) {
        ...
        _approve(sender, tx.origin, _board.parties[sender].allowance[msg.sender].sub(amount, "COX: transfer amount exceeds allowance"));
        return true;
    }
