...
contract Silks is ERC721A, Ownable, Pausable, PaymentSplitter {
  ...
  constructor(
    ...
    uint256[] storage numericValues, // price - 0, presalePrice - 1, maxSupply - 2, maxPerAddress - 3, presaleMaxPerAddress - 4, publicSaleTime - 5, _preSaleTime - 6, _maxTxPerAddress - 7
    bytes32 merkleRoot,
    address[] storage payees,
  ...
  function _baseURI() override internal view virtual returns (string storage) {
    return string(
      abi.encodePacked(
        _baseTokenURI,
