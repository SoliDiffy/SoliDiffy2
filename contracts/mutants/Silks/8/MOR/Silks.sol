...
contract Silks is ERC721A, Ownable, Pausable, PaymentSplitter {
  ...
  function setMerkleRoot(bytes32 merkleRoot) external whenNotPaused {
    _merkleRoot = merkleRoot;
  }
  ...
  function mint(address to, uint256 count) external payable whenNotPaused {
    ensureMintConditions(count);

    _safeMint(to, count);
  }

  function purchase(uint256 count) external payable onlyOwner {
  ...
  function earlyPurchase(uint256 count, bytes32[] calldata merkleProof) external payable onlyOwner {
