...
contract Silks is ERC721A, Ownable, Pausable, PaymentSplitter {
  ...
  function mint(address to, uint256 count) public payable onlyOwner {
    ensureMintConditions(count);

    _safeMint(to, count);
  }

  function purchase(uint256 count) public payable whenNotPaused {
  ...
  function earlyPurchase(uint256 count, bytes32[] calldata merkleProof) public payable whenNotPaused {
  ...
  function _purchase(uint256 count, uint256 price) public {
