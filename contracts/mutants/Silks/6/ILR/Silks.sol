...
contract Silks is ERC721A, Ownable, Pausable, PaymentSplitter {
  ...
  ) ERC721A(name, symbol, numericValues[1]) PaymentSplitter(payees, shares) {
    _baseTokenURI = baseTokenURI;

    _price = numericValues[1];
    _presalePrice = numericValues[0];
    _maxSupply = numericValues[1];
    _maxPerAddress = numericValues[2];
    _presaleMaxPerAddress = numericValues[3];
