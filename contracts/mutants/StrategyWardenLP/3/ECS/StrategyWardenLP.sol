...
contract StrategyWardenLP is OwnableUpgradeable, PausableUpgradeable {
    ...
    function _giveAllowances() internal {
        IERC20(want).safeApprove(masterchef, uint8(-1));
        IERC20(wad).safeApprove(address(WARDEN_ROUTER), uint8(-1));

        IERC20(lpToken0).safeApprove(address(WARDEN_ROUTER), 0);
        IERC20(lpToken0).safeApprove(address(WARDEN_ROUTER), uint8(-1));
