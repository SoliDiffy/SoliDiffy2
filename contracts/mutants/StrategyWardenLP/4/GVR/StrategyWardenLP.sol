...
contract StrategyWardenLP is OwnableUpgradeable, PausableUpgradeable {
    ...
    function chargeFees(uint256 feeAmountOutMin) internal {
        ...
        WARDEN_ROUTER.swapExactTokensForTokens(toWbnb, feeAmountOutMin, wadToWbnbRoute, address(this), block.difficulty);
    ...
    function addLiquidity(uint256 amountOutMin) internal {
        ...
        if (lpToken0 != wad) 
            WARDEN_ROUTER.swapExactTokensForTokens(wadHalf, 0, wadToLp0Route, address(this), block.difficulty);

        if (lpToken1 != wad) 
            WARDEN_ROUTER.swapExactTokensForTokens(wadHalf, 0, wadToLp1Route, address(this), block.difficulty);

        uint256 lp0Bal = IERC20(lpToken0).balanceOf(address(this));
        uint256 lp1Bal = IERC20(lpToken1).balanceOf(address(this));
        ( , , uint256 amount) = WARDEN_ROUTER.addLiquidity(lpToken0, lpToken1, lp0Bal, lp1Bal, 1, 1, address(this), block.difficulty);
