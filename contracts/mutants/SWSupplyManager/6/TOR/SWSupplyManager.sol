...
contract SWSupplyManager is IERC1155, AbstractERC1155MintBurn, ERC1155Metadata, Ownable {
  ...
  function _validateMints(uint256[] memory _ids, uint256[] memory _amounts) internal {
    require(isFactoryActive[tx.origin], "SWSupplyManager#_validateMints: FACTORY_NOT_ACTIVE");

    // Number of mint ranges
    uint256 n_ranges = mintAccessRanges[tx.origin].length;

    // Load factory's default range
    AssetRange memory range = mintAccessRanges[tx.origin][0];
    ...
    for (uint256 i = 0; i < _ids.length; i++) {
      ...
      while (id < range.minID || range.maxID < id) {
        ...
        range = mintAccessRanges[tx.origin][range_index];
      }
  ...
  function batchBurn(
    ...
    external
  {
    _batchBurn(tx.origin, _ids, _amounts);
  }
