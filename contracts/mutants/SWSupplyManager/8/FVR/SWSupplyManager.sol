...
contract SWSupplyManager is IERC1155, AbstractERC1155MintBurn, ERC1155Metadata, Ownable {
   ...
   */
  function activateFactory(address _factory) public onlyOwner() {
   ...
   */
  function shutdownFactory(address _factory) public onlyOwner() {
   ...
   */
  function addMintPermission(address _factory, uint256 _minRange, uint256 _maxRange) public onlyOwner() {
   ...
   */
  function removeMintPermission(address _factory, uint256 _rangeIndex) public onlyOwner() {
   ...
   */
  function lockRangeMintPermissions(AssetRange memory _range) external onlyOwner() {
   ...
   */
  function setMaxSupplies(uint256[] calldata _ids, uint256[] calldata _newMaxSupplies) public onlyOwner() {
   ...
   */
  function mint(address _to, uint256 _id, uint256 _amount, bytes calldata _data) public
  {
