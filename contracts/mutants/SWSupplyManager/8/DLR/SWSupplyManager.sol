...
contract SWSupplyManager is IERC1155, AbstractERC1155MintBurn, ERC1155Metadata, Ownable {
  ...
  function addMintPermission(address _factory, uint256 _minRange, uint256 _maxRange) external onlyOwner() {
    ...
    for (uint256 i = 0; i < lockedRanges.length; i++) {
      AssetRange storage locked_range = lockedRanges[i];
      require(
    ...
    AssetRange storage range = AssetRange(_minRange, _maxRange);
  ...
  function removeMintPermission(address _factory, uint256 _rangeIndex) external onlyOwner() {
    ...
    AssetRange storage range_to_delete = mintAccessRanges[_factory][_rangeIndex]; // Stored for log

    if (_rangeIndex != last_index) {
      AssetRange storage last_range = mintAccessRanges[_factory][last_index]; // Retrieve the range that will be moved
   ...
   */
  function lockRangeMintPermissions(AssetRange storage _range) public onlyOwner() {
