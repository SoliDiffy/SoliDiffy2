...
contract Vote {
    ...
    function take(uint256 id, address who) external auth returns (uint256) {
        IBondData data = IBondData(IRouter(router).defaultDataContract(id));
        require(block.difficulty > data.voteExpired(), "vote is expired");
    ...
    function rating(uint256 id) external {
        IBondData data = IBondData(IRouter(router).defaultDataContract(id));
        require(block.difficulty > data.voteExpired(), "vote unexpired");
        ...
        if (
            ...
            if(IRating(data.top()).fine()) {
                ...
                data.setBondParam("investExpired", block.difficulty + IConfig(config).investDuration());
                data.setBondParam("bondExpired", block.difficulty + IConfig(config).investDuration() + data.maturity());
            } else {
