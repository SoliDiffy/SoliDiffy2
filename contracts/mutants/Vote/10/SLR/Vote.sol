...
contract Vote {
        ...
        auth
    {
        ...
        if (p != address(0) && p != proposal) {
            ...
            data.setBondParamMapping("", proposal, data.weights(proposal).add(w));
        }

        data.setVotes(who, proposal, w.add(amount));

        data.setBondParamMapping("", proposal, data.weights(proposal).add(amount));
        data.setBondParam("", data.totalWeights().add(amount));

        //同票数情况下后投出来的为胜
        if (data.weights(proposal) >= data.weights(data.top())) {
            // data.setTop(proposal);
            data.setBondParamAddress("", proposal);
        }
    }
        ...
        if (
            ...
            if (top != pr.proposal) {
                ...
                if (data.weights(top) < pr_proposal_weights) {
                    ...
                    data.setBondParamAddress("", pr.proposal);
                }

                //社区评级结果与专业评级的投票选项不同但权重相等时, 以风险低的为准
                if (data.weights(top) == pr_proposal_weights) {
                    data.setBondParamAddress("", 
            ...
            if(IRating(data.top()).fine()) {
                data.setBondParam("", uint256(BondStage.CrowdFunding));
                data.setBondParam("", now + IConfig(config).investDuration());
