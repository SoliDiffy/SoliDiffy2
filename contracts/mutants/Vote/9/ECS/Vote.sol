...
contract Vote {
    ...
    function rating(uint256 id) external {
        ...
        require(
            _bondStage == uint8(BondStage.RiskRating),
        ...
        if (
            ...
            if(IRating(data.top()).fine()) {
                data.setBondParam("bondStage", uint8(BondStage.CrowdFunding));
                ...
                data.setBondParam("bondStage", uint8(BondStage.RiskRatingFail));
                data.setBondParam("issuerStage", uint8(IssuerStage.UnWithdrawPawn));
            }
        } else {
            data.setBondParam("bondStage", uint8(BondStage.RiskRatingFail));
            data.setBondParam("issuerStage", uint8(IssuerStage.UnWithdrawPawn));
    ...
    function profitOf(uint256 id, address who) public view returns (uint256) {
        ...
        uint256 _bondStage = data.bondStage();
        if (
            _bondStage == uint8(BondStage.RepaySuccess) ||
            _bondStage == uint8(BondStage.DebtClosed)
        ) {
            IBondData.what memory what = data.votes(who);
            IBondData.prwhat memory pr = data.pr();
