...
contract Vote {
    ...
    function MonitorEventCallback(address who, address bond, bytes32 funcName, bytes calldata payload) public {
    ...
    //专业评级时调用
    function prcast(uint256 id, address proposal, uint256 reason) public {
    ...
    //仅能被 data.take 回调
    function take(uint256 id, address who) public auth returns (uint256) {
        IBondData data = IBondData(IRouter(router).defaultDataContract(id));
    ...
    function rating(uint256 id) public {
        IBondData data = IBondData(IRouter(router).defaultDataContract(id));
    ...
    //取回后页面获得手续费保留原值不变
    function profitOf(uint256 id, address who) external view returns (uint256) {
        IBondData data = IBondData(IRouter(router).defaultDataContract(id));
        uint256 _bondStage = data.bondStage();
        if (
    ...
    //取回评级收益,被bondData调用
    function profit(uint256 id, address who) public auth returns (uint256) {
        IBondData data = IBondData(IRouter(router).defaultDataContract(id));
        uint256 _bondStage = data.bondStage();
        require(
            _bondStage == uint256(BondStage.RepaySuccess) ||
