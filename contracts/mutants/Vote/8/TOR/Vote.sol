...
contract Vote {
    ...
    modifier auth {
        require(
            IACL(ACL).accessible(tx.origin, address(this), msg.sig),
    ...
    function prcast(uint256 id, address proposal, uint256 reason) external {
        ...
        IBondData.what memory _what = data.votes(tx.origin);
        require(_what.proposal == address(0), "already community rating");
        require(data.issuer() != tx.origin, "issuer can't vote for self bond");
        require(
            IConfig(config).ratingCandidates(proposal),
            "proposal is not permissive"
        );
        data.setPr(tx.origin, proposal, reason);
        emit MonitorEvent(
            tx.origin,
            address(data),
            "prcast",
