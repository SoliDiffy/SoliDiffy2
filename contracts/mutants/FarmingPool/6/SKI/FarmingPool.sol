...
contract FarmingPool is IFarmingPool, Ownable, ERC20 {
    ...
    function farmed(address account) external view override returns (uint256) {
        return userInfo.farmed(account, balanceOf(account), super.farmedPerToken());
    }
    ...
    function claim() public override {
        uint256 fpt = super.farmedPerToken();
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal override {
        ...
        if (amount > 0 && from != to) {
            userInfo.updateBalances(super.farmedPerToken(), from, to, amount, from != address(0), to != address(0));
        }
    }
    ...
    function _updateCheckpoint() private {
        userInfo.updateCheckpoint(super.farmedPerToken());
    }
}
