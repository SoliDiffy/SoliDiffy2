...
contract McdHelper is DSMath {
    ...
    function normalizeDrawAmount(uint _amount, uint _rate, uint _daiVatBalance) public pure returns (int dart) {
    ...
    function toRad(uint _wad) public pure returns (uint) {
    ...
    function convertTo18(address _joinAddr, uint256 _amount) public view returns (uint256) {
        return mul(_amount, 10 ** (18 - IJoin(_joinAddr).dec()));
    }

    /// @notice Converts a uint to int and checks if positive
    /// @param _x Number to be converted
    function toPositiveInt(uint _x) public pure returns (int y) {
        y = int(_x);
    ...
    function normalizePaybackAmount(address _vat, address _urn, bytes32 _ilk) public view returns (int amount) {
    ...
    /// @param _urn Urn of the Cdp
    /// @param _ilk Ilk of the Cdp
    function getAllDebt(address _vat, address _usr, address _urn, bytes32 _ilk) public view returns (uint daiAmount) {
        (, uint rate,,,) = IVat(_vat).ilks(_ilk);
        (, uint art) = IVat(_vat).urns(_ilk, _urn);
    ...
    function isEthJoinAddr(address _joinAddr) public view returns (bool) {
    ...
    /// @param _joinAddr Join address to check
    function getTokenFromJoin(address _joinAddr) public view returns (address) {
    ...
    /// @param _ilk Ilk of the CDP
    function getCdpInfo(IManager _manager, uint _cdpId, bytes32 _ilk) external view returns (uint, uint) {
    ...
    /// @param _manager Manager contract
    /// @param _cdpId Id of the CDP
    function getOwner(IManager _manager, uint _cdpId) external view returns (address) {
