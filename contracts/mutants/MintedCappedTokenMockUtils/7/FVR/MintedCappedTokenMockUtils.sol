...
contract MintedCappedTokenMockUtils {
  ...
  // Mock
  function setBalance(address, uint) external pure returns (bytes) { return msg.data; }
  function unlockToken() external pure returns (bytes) { return msg.data; }
  function setTransferAgent(address, bool) external pure returns (bytes) { return msg.data; }
  function setTotalSold(uint) external pure returns (bytes) { return msg.data; }
