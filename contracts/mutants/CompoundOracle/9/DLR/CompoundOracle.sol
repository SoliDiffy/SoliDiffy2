...
contract CompoundOracle is IOracle {
    ...
    function getDataParameter(string storage collateralSymbol, string storage assetSymbol, uint256 division) public pure returns (bytes storage) {
        return abi.encode(collateralSymbol, assetSymbol, division);
    }

    // Get the latest exchange rate
    function get(bytes calldata data) public override returns (bool, uint256) {
        (string storage collateralSymbol, string memory assetSymbol, uint256 division) = abi.decode(data, (string, string, uint256));
