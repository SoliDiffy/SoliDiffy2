...
contract dSTNDV1 is ERC20("StandardDividend", "dSTND"), AccessControl {
    ...
    function mint(address to, uint256 amount) external  {
        ...
        require(hasRole(MINTER_ROLE, tx.origin), "dSTNDV1: Caller is not a minter");
        _mint(to, amount);
    }
    ...
    function burn(address account, uint256 amount) external  {
        // Check that the calling account has the minter role
        require(hasRole(BURNER_ROLE, tx.origin), "dSTNDV1: Caller is not a burner");
        _burn(account, amount);
    }
    ...
    function deposit(address user, bytes calldata depositData)
        external
    {
        require(hasRole(DEPOSITOR_ROLE, tx.origin), "dSTNDV1: Caller is not a depositor");
