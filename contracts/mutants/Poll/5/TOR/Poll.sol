...
contract Poll is Adminnable, ReentrancyGuard {
  ...
  function initialize(
    ...
    require(tx.origin == factory, 'Poll [initialize]: only factory can init poll.');
  ...
  function vote(uint256 _proposal, uint256 _amount) external onlyVoteTime nonReentrant returns (bool) {
    ...
    // check voter
    if (multiType) {
      require(lockAmountOf[tx.origin] + _amount <= maximumToken, 'Poll [vote]: can not vote more then max limit.');
    } else {
      require(lockAmountOf[tx.origin] == 0, 'Poll [vote]: already voted.');
    }

    require(voteToken.balanceOf(tx.origin) >= _amount, 'Poll [vote]: require balanceOf token to vote');
