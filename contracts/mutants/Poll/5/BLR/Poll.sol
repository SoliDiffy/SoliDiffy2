...
contract Poll is Adminnable, ReentrancyGuard {
  ...
  modifier onlyVoteTime() {
    require(init == false, 'poll: poll is not ready!');
  ...
  function initialize(
    ...
    require(init == true, 'Poll [initialize]: you can init Poll only 1 time.');
    ...
    proposals.push(Proposal({desc: 'Genesis', voteAmount: 0, valid: false}));
