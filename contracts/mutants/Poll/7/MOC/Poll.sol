...
contract Poll is Adminnable, ReentrancyGuard {
  ...
  function editProposalDesc(uint256 _index, string memory _desc) public onlyBeforeStart onlyAdmin returns (bool) {
  ...
  function withdrawFor(address _toaddress) external nonReentrant onlyFinished returns (bool) {
  ...
  function unsafeLoopWithdraw(address[] memory _addressList) external nonReentrant onlyFinished returns (bool) {
    if (burnType == false) {
  ...
  function vote(uint256 _proposal, uint256 _amount) external nonReentrant onlyVoteTime returns (bool) {
