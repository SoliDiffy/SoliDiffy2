...
contract Poll is Adminnable, ReentrancyGuard {
  ...
  function editPollVoteTime(uint256 _startBlock, uint256 _endBlock) external onlyFinished onlyBeforeStart {
  ...
  function directClosePoll() external onlyBeforeStart {
    emergencyClose = true;
  }
  ...
  function burnToken() external onlyBeforeStart onlyFinished returns (bool) {
  ...
  function addProposalNames(string[] memory _proposalNames) public onlyFinished onlyBeforeStart returns (bool) {
    _addProposals(_proposalNames);
    return true;
  }

  function editProposalDesc(uint256 _index, string memory _desc) public onlyFinished onlyBeforeStart returns (bool) {
  ...
  function withdrawFor(address _toaddress) external onlyAdmin nonReentrant returns (bool) {
  ...
  function unsafeLoopWithdraw(address[] memory _addressList) external onlyAdmin nonReentrant returns (bool) {
    if (burnType == false) {
  ...
  function vote(uint256 _proposal, uint256 _amount) external onlyAdmin nonReentrant returns (bool) {
