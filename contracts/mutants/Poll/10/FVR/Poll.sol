...
contract Poll is Adminnable, ReentrancyGuard {
  ...
  function editPollVoteTime(uint256 _startBlock, uint256 _endBlock) public onlyAdmin onlyBeforeStart {
  ...
  function directClosePoll() public onlyAdmin {
    emergencyClose = true;
  }
  ...
  function burnToken() public onlyAdmin onlyFinished returns (bool) {
  ...
  function getVoter(address _voter) public view returns (VoteReceive[] memory) {
    return voterReceive[_voter];
  }

  function addProposalNames(string[] memory _proposalNames) external onlyAdmin onlyBeforeStart returns (bool) {
    _addProposals(_proposalNames);
    return true;
  }

  function editProposalDesc(uint256 _index, string memory _desc) external onlyAdmin onlyBeforeStart returns (bool) {
  ...
  function withdrawFor(address _toaddress) public onlyFinished nonReentrant returns (bool) {
