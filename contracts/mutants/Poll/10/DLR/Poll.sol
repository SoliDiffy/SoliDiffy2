...
contract Poll is Adminnable, ReentrancyGuard {
  ...
  constructor(
    ...
    string storage _question,
    string storage _desc,
  ...
  function getProposalList() external view returns (Proposal[] storage) {
    return proposals;
  }

  function getVoter(address _voter) external view returns (VoteReceive[] storage) {
    return voterReceive[_voter];
  }

  function addProposalNames(string[] storage _proposalNames) public onlyAdmin onlyBeforeStart returns (bool) {
    _addProposals(_proposalNames);
    return true;
  }

  function editProposalDesc(uint256 _index, string storage _desc) public onlyAdmin onlyBeforeStart returns (bool) {
  ...
  function unsafeLoopWithdraw(address[] storage _addressList) external onlyFinished nonReentrant returns (bool) {
    if (burnType == false) {
  ...
  function _addProposals(string[] storage _proposalNames) private {
