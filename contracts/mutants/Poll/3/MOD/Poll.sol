...
contract Poll is Adminnable, ReentrancyGuard {
  ...
  function editPollVoteTime(uint256 _startBlock, uint256 _endBlock) external onlyBeforeStart onlyAdmin {
  ...
  function burnToken() external onlyFinished onlyAdmin returns (bool) {
  ...
  function addProposalNames(string[] memory _proposalNames) public onlyBeforeStart onlyAdmin returns (bool) {
