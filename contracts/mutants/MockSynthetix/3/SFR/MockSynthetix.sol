...
contract MockSynthetix is ISynthetix {
    ...
    function exchange(bytes32 _sourceCurrencyKey, uint256 _sourceAmount, bytes32 _destinationCurrencyKey) external override {
        ...
        uint256 outputAmount = _sourceAmount.mul(sourcePrice).mul(destinationPrice);

        getOrSetToken(_sourceCurrencyKey).burn(_sourceAmount.add(subtractSourceAmount), msg.sender);
        getOrSetToken(_destinationCurrencyKey).mint(outputAmount.add(subtractOutputAmount), msg.sender);
    }
