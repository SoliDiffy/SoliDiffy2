...
interface IMasterApe {
  ...
  function getPoolInfo(uint256 _pid) external view returns (address lpToken, uint256 accCakePerShare, uint256 allocPoint, uint256 lastRewardBlock);
  function leaveStaking(uint256 _amount) external;
  function massUpdatePools() external;
  function owner() external view returns (address);
  function pendingCake(uint256 _pid, address _user) external view returns (uint256);
  function poolInfo(uint256) external view returns (address lpToken, uint256 accCakePerShare, uint256 allocPoint, uint256 lastRewardBlock);
