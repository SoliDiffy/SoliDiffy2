...
contract FeePolicy is Manageable {
    ...
    uint16 constant public constant MAXPERMILLE = 1000;

    IProductStorage internal productStorage;
    uint16 internal defaultFee;               //default fee for genereal products (permille)
    uint16 internal escrowFee;                //additional fee for escrow usage (permille)
    //uint256 public minEscrowFeePermille;    //minimum escrow fee permille in weis, can't be discounted further
    uint16 internal fiatPriceFee;             //additional fee for fiat price usage (permille)
    address internal feeWallet;
    
    IERC20Token internal token;               // token to check minimum token balance
    uint256 internal minTokenForDiscount;     // min token amount to get fee discount
    uint256 internal termDuration;            // term duration in seconds
