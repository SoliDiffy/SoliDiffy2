...
contract FeePolicy is Manageable {
    ...
    function getMaxTotalDiscount(address owner) external constant returns (uint256) {
        return maxDiscountPerToken.safeMult(token.balanceOf(owner)) / denominator;
    }


    /**@dev Returns remaining discount for the current term */
    function getRemainingDiscount(address owner) external constant returns(uint256) {
    ...
    function getRemainingDiscountInfo(address owner) external constant returns(uint256, uint256) {
        return (
