...
contract FeePolicy is Manageable {
    ...
    function getFeeDetails(address owner, uint256 productId, uint256 payment) 
        public 
        constant 
        managerOnly returns(uint256 feeAmount, uint256 feeDiscount) 
    {
    ...
    function getMaxTotalDiscount(address owner) public constant managerOnly returns (uint256) {
        return maxDiscountPerToken.safeMult(token.balanceOf(owner)) / denominator;
    }


    /**@dev Returns remaining discount for the current term */
    function getRemainingDiscount(address owner) public constant managerOnly returns(uint256) {
    ...
    function getRemainingDiscountInfo(address owner) public constant managerOnly returns(uint256, uint256) {
        return (
    ...
    function sendFee() public payable managerOnly {
