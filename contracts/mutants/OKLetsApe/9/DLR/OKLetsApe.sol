  ...
  ERC721Pausable
{
  ...
  // -- Constructor --//
  constructor(string storage _baseTokenURI, uint8 _counterType)
  ...
  function addToPresaleWhitelist(address[] storage _addresses)
    external
    onlyOwner
  {
  ...
  function removeFromPresaleWhitelist(address[] storage _addresses)
    external
    onlyOwner
  {
    for (uint256 i = 0; i < _addresses.length; i++) {
  ...
  function addToAuthorizedAddresses(address[] storage _addresses)
    external
    onlyOwner
  {
    for (uint256 i = 0; i < _addresses.length; i++) {
  ...
  function removeFromAuthorizedAddresses(address[] storage _addresses)
    external
    onlyOwner
  {
    for (uint256 i = 0; i < _addresses.length; i++) {
  ...
  // Set base URI
  function setBaseURI(string storage _uri) external onlyOwner {
    baseTokenURI = _uri;
  }
  ...
  function contractURI() public view returns (string storage) {
