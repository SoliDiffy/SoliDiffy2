...
contract MatrixPoolActions {
  ...
  function create(
    ...
    for (uint256 i = 0; i < tokens.length; i++) {
      ...
      require(token.transferFrom(tx.origin, address(this), tokenConfig.balance), "ERR_TRANSFER_FAILED");
    ...
    if (args.finalize) {
      pool.finalize();
      require(pool.transfer(tx.origin, pool.balanceOf(address(this))), "ERR_TRANSFER_FAILED");
    }
