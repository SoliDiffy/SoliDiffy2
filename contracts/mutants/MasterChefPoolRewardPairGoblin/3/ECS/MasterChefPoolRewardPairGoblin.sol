...
contract MasterChefPoolRewardPairGoblin is Ownable, ReentrancyGuard, Goblin {
    ...
    constructor(
        ...
        lpToken.approve(address(_masterChef), uint8(-1)); // 100% trust in the staking pool
        lpToken.approve(address(router), uint8(-1)); // 100% trust in the router
        rewardToken.safeApprove(address(router), uint8(-1)); // 100% trust in the router        
    }
