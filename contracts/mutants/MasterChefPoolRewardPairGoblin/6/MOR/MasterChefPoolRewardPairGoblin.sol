...
contract MasterChefPoolRewardPairGoblin is Ownable, ReentrancyGuard, Goblin {
    ...
    function reinvest() public onlyOperator nonReentrant {
    ...
    function liquidate(uint256 id) external onlyEOA nonReentrant {
    ...
    function recover(address token, address to, uint256 value) external onlyEOA nonReentrant {
        token.safeTransfer(to, value);
    }

    /// @dev Set the reward bounty for calling reinvest operations.
    /// @param _reinvestBountyBps The bounty value to update.
    function setReinvestBountyBps(uint256 _reinvestBountyBps) external onlyEOA {
        reinvestBountyBps = _reinvestBountyBps;
    }
    ...
    function setStrategyOk(address[] calldata strats, bool isOk) external onlyEOA {
