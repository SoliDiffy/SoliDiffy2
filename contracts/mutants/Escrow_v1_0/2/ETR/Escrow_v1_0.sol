        ...
        returns (uint256 valueTransferred)
    {
        ...
        if (t.transactionType == TransactionType.ETHER) {
            for (uint256 i = 0; i < destinations.length; i++) {
                ...
                destinations[i].send(amounts[i]);//shall we use send instead of transfer to stop malicious actors from blocking funds?
            }
            ...
            for (uint256 j = 0; j<destinations.length; j++) {
                ...
                require(token.send(destinations[j], amounts[j]), "Token transfer failed.");
            }
