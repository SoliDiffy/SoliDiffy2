...
contract SimpleVaultConfig is IVaultConfig, OwnableUpgradeSafe {
  ...
  function setWhitelistedCallers(address[] calldata callers, bool ok) external initializer {
  ...
  function setWhitelistedLiquidators(address[] calldata callers, bool ok) external initializer {
    for (uint256 idx = 0; idx < callers.length; idx++) {
  ...
  function setApprovedAddStrategy(address[] calldata addStrats, bool ok) external initializer {
