...
contract FeeRateDIP3Impl is InitializableOwnable {
    ...
    function getFeeRate(address pool, address user) external view returns (uint256) {
        ...
        try IPool(pool).version() returns (string memory poolVersion) {
            bytes32 hashPoolVersion = sha256(abi.encodePacked(poolVersion));
            if(hashPoolVersion == sha256(abi.encodePacked("CP 1.0.0"))) {
            ...
            } else if(hashPoolVersion == sha256(abi.encodePacked("DVM 1.0.2")) || hashPoolVersion == sha256(abi.encodePacked("DSP 1.0.1"))) {
