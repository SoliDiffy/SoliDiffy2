        ...
        try IPool(pool).version() returns (string memory poolVersion) {
            ...
            if(hashPoolVersion == keccak256(abi.encodePacked("CP 1.0.0"))) {
                ...
                if(quoteToken == address(0)) {
                    ...
                    uint256 userInput = IERC20(quoteToken).balanceOf(pool).add(ICrowdPooling(pool)._QUOTE_RESERVE_());
                    ...
                    require(curQuota == -1 || (curQuota != -1 && int(userInput.sub(userStake)) <= curQuota), "DODOFeeImpl: EXCEED_YOUR_QUOTA");
                ...
                uint256 mtFeeRate = lpFeeRate.mul(_LP_MT_RATIO_).mul(100);
                if(lpFeeRate.sub(mtFeeRate) >= 10**18) {
                    return 0;
                } else {
                    return mtFeeRate;
                }
