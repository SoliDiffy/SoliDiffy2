...
contract FeeRateDIP3Impl is InitializableOwnable {
    ...
    function addCpPoolInfo(address cpPool, address quoteToken, int globalQuota, address feeAddr, address quotaAddr) public onlyOwner {
    ...
    function setCpPoolInfo(address cpPool, address quoteToken, int globalQuota, address feeAddr, address quotaAddr) public onlyOwner {
    ...
    function setLpMtRatio(uint256 newLpMtRatio) public onlyOwner {
        _LP_MT_RATIO_ = newLpMtRatio;
    }


    function setSpecPoolList (address poolAddr, uint256 mtFeeRate) external onlyOwner {
