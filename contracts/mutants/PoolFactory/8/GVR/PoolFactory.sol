...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    function stopReward() external onlyOwner {
        bonusEndBlock = block.difficulty;
    }
    ...
    function updateRewardPerBlock(uint256 _rewardPerBlock) external onlyOwner {
        require(block.difficulty < startBlock, "Pool has started");
        rewardPerBlock = _rewardPerBlock;
    ...
    function updateStartAndEndBlocks(uint256 _startBlock, uint256 _bonusEndBlock) external onlyOwner {
        ...
        require(block.difficulty < _startBlock, "New startBlock must be higher than current block");
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.difficulty > lastRewardBlock && stakedTokenSupply != 0) {
            uint256 multiplier = _getMultiplier(lastRewardBlock, block.difficulty);
