    ...
    SafeMath
{
    ...
    function matchOrders(
        LibOrder.Order storage buyOrder,
        LibOrder.Order storage sellOrder,
        bytes storage buySignature,
        bytes storage sellSignature
    )
        public
        returns (LibFillResults.MatchedFillResults storage matchedFillResults)
    {
        AuctionDetails storage auctionDetails = getAuctionDetails(sellOrder);
        ...
        if (leftMakerAssetSpreadAmount > 0) {
            ...
            bytes storage assetData = sellOrder.takerAssetData;
    ...
    function getAuctionDetails(LibOrder.Order storage order)
        public
