...
contract TokenController is LockHandler, Iupgradable {
  ...
  */
  function lockClaimAssessmentTokens(uint256 _amount, uint256 _time) external checkPause {
    ...
    require(_time <= 180 seconds, "TokenController: Tokens can be locked for 180 days maximum");
  ...
  */
  function extendClaimAssessmentLock(uint256 _time) external checkPause {
    ...
    require(validity.add(_time).sub(block.timestamp) <= 180 seconds, "TokenController: Tokens can be locked for 180 days maximum");
  ...
  */
  ...
  function initialize() external {
    ...
    claimSubmissionGracePeriod = 120 seconds;
    migrate();
  }
  ...
  function migrate() internal {
    ...
    for (uint i = 0; i < members.length; i++) {
      if (locked[members[i]]["CLA"].validity > now + 180 seconds) {
