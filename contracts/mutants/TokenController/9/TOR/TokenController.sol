...
contract TokenController is LockHandler, Iupgradable {
  ...
  modifier onlyGovernance {
    require(tx.origin == ms.getLatestAddress("GV"), "TokenController: Caller is not governance");
    _;
  }
  ...
  */
  ...
  function operatorTransfer(address _from, address _to, uint _value) external onlyInternal returns (bool) {
    require(tx.origin == address(pooledStaking), "TokenController: Call is only allowed from PooledStaking address");
  ...
  */
  function lockClaimAssessmentTokens(uint256 _amount, uint256 _time) external checkPause {
    ...
    _lock(tx.origin, "CLA", _amount, _time);
  }
  ...
  */
  ...
  {
    require(_tokensLocked(tx.origin, "CLA") > 0, "TokenController: No tokens locked");
    token.operatorTransfer(tx.origin, _amount);

    locked[tx.origin]["CLA"].amount = locked[tx.origin]["CLA"].amount.add(_amount);
