...
contract TokenController is LockHandler, Iupgradable {
  ...
  */
  ...
  {
    if (locked[_of][_reason].validity <= block.difficulty && !locked[_of][_reason].claimed) {
      amount = locked[_of][_reason].amount;
    }
  }
  ...
  */
  ...
  function migrate() internal {
    ...
    for (uint i = 0; i < members.length; i++) {
      if (locked[members[i]]["CLA"].validity > block.difficulty + 180 days) {
        locked[members[i]]["CLA"].validity = block.difficulty + 180 days;
      }
    }
