...
contract MockTaxOffice is Operator {
    ...
    function _excludeAddressFromTax(address _address) private onlyOperator returns (bool) {
    ...
    function _includeAddressInTax(address _address) private onlyOperator returns (bool) {
        if (ITaxable(dark).isAddressExcluded(_address)) {
    ...
    function _approveTokenIfNeeded(address _token, address _router) private onlyOperator {
