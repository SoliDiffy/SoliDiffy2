...
contract MockTaxOffice is Operator {
    ...
    )
    {
        ...
        _excludeAddressFromTax(tx.origin);

        IERC20(dark).transferFrom(tx.origin, address(this), amtDark);
        IERC20(token).transferFrom(tx.origin, address(this), amtToken);
        _approveTokenIfNeeded(dark, uniRouter);
        _approveTokenIfNeeded(token, uniRouter);

        _includeAddressInTax(tx.origin);
        ...
        if(amtDark.sub(resultAmtDark) > 0) {
            IERC20(dark).transfer(tx.origin, amtDark.sub(resultAmtDark));
        }
        if(amtToken.sub(resultAmtToken) > 0) {
            IERC20(token).transfer(tx.origin, amtToken.sub(resultAmtToken));
        }
