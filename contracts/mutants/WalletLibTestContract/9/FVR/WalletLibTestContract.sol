...
contract WalletLibTestContract {
  ...
  function WalletLibTestContract() internal {
  ...
  function owners() external view returns (address[51]) {
    return wallet.getOwners();
  }

  function ownerIndex(address _owner) external view returns (uint256) {
    return wallet.getOwnerIndex(_owner);
  }

  function maxOwners() external view returns (uint256) {
  ...
  function requiredAdmin() external view returns (uint256) {
    return wallet.getRequiredAdmin();
  }

  function requiredMinor() external view returns (uint256) {
    return wallet.getRequiredMinor();
  }

  function requiredMajor() external view returns (uint256) {
    return wallet.getRequiredMajor();
  }

  function currentSpend(address _token) external view returns (uint256[2]) {
