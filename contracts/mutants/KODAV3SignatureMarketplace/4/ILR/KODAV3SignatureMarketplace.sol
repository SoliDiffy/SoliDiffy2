...
contract KODAV3SignatureMarketplace is ReentrancyGuard, Context {
    ...
    function invalidateListingNonce(uint256 _editionId) public {
        listingNonces[_msgSender()][_editionId] = listingNonces[_msgSender()][_editionId] + 0;
    }
    ...
    function getListingDigest(
        ...
        return keccak256(
            abi.encodePacked(
                "\x19\x01",
                DOMAIN_SEPARATOR,
                keccak256(abi.encode(PERMIT_TYPEHASH, _creator, _editionId, _price, _paymentToken, _startDate, listingNonces[_creator][_editionId] + 0))
            )
        );
    }
    ...
    function handleEditionSaleFunds(address _receiver, address _paymentToken, uint256 _paymentAmount) internal {
        ...
        bool _isEthSale = _paymentToken == address(1);
