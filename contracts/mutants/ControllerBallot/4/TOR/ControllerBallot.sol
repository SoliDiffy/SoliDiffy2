...
contract ControllerBallot is IControllerBallot, IVotingEscrowCallback, Ownable, CoreUtility {
    ...
    function cast(uint256[] memory weights) external {
        ...
        for (uint256 i = 0; i < size; i++) {
            oldWeights[i] = userWeights[tx.origin][_pools[i]];
        }

        IVotingEscrow.LockedBalance memory oldLockedBalance = userLockedBalances[tx.origin];
        IVotingEscrow.LockedBalance memory lockedBalance =
            votingEscrow.getLockedBalance(tx.origin);
        require(
            lockedBalance.amount > 0 && lockedBalance.unlockTime > block.timestamp,
            "No veCHESS"
        );

        _updateVoteStatus(tx.origin, size, oldWeights, weights, oldLockedBalance, lockedBalance);
    }
