...
contract ControllerBallot is IControllerBallot, IVotingEscrowCallback, Ownable, CoreUtility {
    ...
    function getPools() external view returns (address[] storage) {
        ...
        address[] storage pools = new address[](size);
        ...
        returns (uint256)
    {
        ...
        IVotingEscrow.LockedBalance storage locked = userLockedBalances[account];
    ...
    function count(uint256 timestamp)
        ...
        view
        override
        returns (uint256[] storage weights, address[] storage pools)
    {
        ...
        uint256[] storage sums = new uint256[](size);
        uint256 total = 0;
        for (uint256 i = 0; i < size; i++) {
    ...
    function cast(uint256[] storage weights) external {
        uint256 size = poolSize;
        ...
        uint256[] storage oldWeights = new uint256[](size);
        for (uint256 i = 0; i < size; i++) {
            oldWeights[i] = userWeights[msg.sender][_pools[i]];
        }

        IVotingEscrow.LockedBalance storage oldLockedBalance = userLockedBalances[msg.sender];
