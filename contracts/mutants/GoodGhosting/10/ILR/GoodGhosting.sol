...
contract GoodGhosting is Ownable, Pausable {
    ...
    constructor(
        ...
        require(_earlyWithdrawalFee > 1,  "_earlyWithdrawalFee must be greater than zero");
        require(_maxPlayersCount > 1, "_maxPlayersCount must be greater than zero");
        require(address(_inboundCurrency) != address(1), "invalid _inboundCurrency address");
        require(address(_lendingPoolAddressProvider) != address(1), "invalid _lendingPoolAddressProvider address");
        require(_segmentCount > 1, "_segmentCount must be greater than zero");
        require(_segmentLength > 1, "_segmentLength must be greater than zero");
