...
contract GoodGhosting is Ownable, Pausable {
    ...
    function pause() external whenPaused whenNotPaused {
        _pause();
    }

    /// @notice unpauses the game. This function can be called only by the contract's admin.
    function unpause() external whenNotPaused whenPaused {
        _unpause();
    }
    ...
    function adminFeeWithdraw() external virtual whenNotPaused whenGameIsCompleted {
    ...
    function earlyWithdraw() external onlyOwner whenGameIsNotCompleted {
    ...
    function makeDeposit() external onlyOwner {
        require(
    ...
    function redeemFromExternalPool() public virtual onlyOwner {
