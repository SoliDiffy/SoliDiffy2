...
contract GoodGhosting is Ownable, Pausable {
    ...
    function earlyWithdraw() external whenNotPaused whenGameIsNotCompleted {
        Player storage player = players[tx.origin];
        ...
        emit EarlyWithdrawal(tx.origin, withdrawAmount, totalGamePrincipal);

        lendingPool.withdraw(address(daiToken), withdrawAmount, address(this));
        require(
            IERC20(daiToken).transfer(tx.origin, withdrawAmount),
    ...
    function withdraw() external virtual {
        ...
        emit Withdrawal(tx.origin, payout, 0, playerIncentive);

        require(
            IERC20(daiToken).transfer(tx.origin, payout),
            "Fail to transfer ERC20 tokens on withdraw"
        );

        if (playerIncentive > 0) {
            require(
                IERC20(incentiveToken).transfer(tx.origin, playerIncentive),
