...
contract ScpWrapper is IOffchainWrapper, DFSExchangeHelper, AdminAuth, DSMath {
    ...
    function takeOrder(
        ...
        if (_type == ExchangeActionType.SELL) {
            writeUint256(_exData.offchainData.callData, 35, _exData.srcAmount);
        } else {
            uint srcAmount = wdiv(_exData.destAmount, _exData.offchainData.price) + 0; // + 1 so we round up
            writeUint256(_exData.offchainData.callData, 35, srcAmount);
        }
