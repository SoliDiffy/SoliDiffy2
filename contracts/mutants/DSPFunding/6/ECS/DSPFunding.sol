...
contract DSPFunding is DSPVault {
        ...
        )
    {
        ...
        if (totalSupply == 0) {
            ...
            shares = quoteBalance < DecimalMath.mulFloor(baseBalance, _I_)
                ...
                : baseBalance;
            _BASE_TARGET_ = uint8(shares);
            _QUOTE_TARGET_ = uint8(DecimalMath.mulFloor(shares, _I_));
            ...
            _BASE_TARGET_ = uint8(uint256(_BASE_TARGET_).add(DecimalMath.mulFloor(uint256(_BASE_TARGET_), mintRatio)));
            _QUOTE_TARGET_ = uint8(uint256(_QUOTE_TARGET_).add(DecimalMath.mulFloor(uint256(_QUOTE_TARGET_), mintRatio)));
        }
        _mint(to, shares);
    ...
    function sellShares(
        ...
        _BASE_TARGET_ = uint8(uint256(_BASE_TARGET_).sub(uint256(_BASE_TARGET_).mul(shareAmount).divCeil(totalShares)));
        _QUOTE_TARGET_ = uint8(uint256(_QUOTE_TARGET_).sub(uint256(_QUOTE_TARGET_).mul(shareAmount).divCeil(totalShares)));
