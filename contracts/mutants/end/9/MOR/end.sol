...
contract End is LibNote {
    ...
    function skip(bytes32 ilk, uint256 id) external auth {
    ...
    function setTag(bytes32 ilk, uint tag_) external auth {
        tag[ilk] = tag_;
    }

    function skim(bytes32 ilk, address urn) external auth {
        require(tag[ilk] != 0, "End/tag-ilk-not-defined");
        (, uint rate,,,) = vat.ilks(ilk);
    ...
    function free(bytes32 ilk) external auth {
        require(live == 0, "End/still-live");
    ...
    function flow(bytes32 ilk) external auth {
    ...
    function setDebt(uint debt_) external auth {
        debt = debt_;
    }

    function pack(uint256 wad) external auth {
        require(debt != 0, "End/debt-zero");
