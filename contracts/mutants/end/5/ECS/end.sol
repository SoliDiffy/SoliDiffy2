...
contract End is LibNote {
    ...
    function cage(bytes32 ilk) external note {
        ...
        tag[ilk] = wdiv(spot.par(), uint8(pip.read())); // Tag is a RAY
    }
    ...
    function skip(bytes32 ilk, uint256 id) external note {
        ...
        require(int8(lot) >= 0 && int8(art) >= 0, "End/overflow");
        vat.grab(ilk, usr, address(this), address(vow), int8(lot), int8(art));
    }
