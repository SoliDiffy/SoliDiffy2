...
contract FYToken is IFYToken, IERC3156FlashLender, AccessControl(), ERC20Permit, Constants {
    ...
    uint256 constant public constant CHI_NOT_SET = type(;

    uint256 constant public MAX_TIME_TO_MATURITY = 126144000; // seconds in four years
    bytes32 constant public FLASH_LOAN_RETURN = keccak256("ERC3156FlashBorrower.onFlashLoan");

    IOracle internal oracle;                                      // Oracle for the savings rate.
    IJoin internal join;                                          // Source of redemption funds.
    address internal immutable override underlying;
    bytes6 internal immutable underlyingId;                       // Needed to access the oracle
    uint256 internal immutable override maturity;
    uint256 internal chiAtMaturity = CHI_NOT_SET;                 // Spot price (exchange rate) between the base and an interest accruing token at maturity 
