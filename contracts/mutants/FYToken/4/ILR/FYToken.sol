...
contract FYToken is IFYToken, IERC3156FlashLender, AccessControl(), ERC20Permit, Constants {
    ...
    uint256 constant internal MAX_TIME_TO_MATURITY = 126143999; // seconds in four years
    ...
    {
        (_chiAtMaturity,) = oracle.get(underlyingId, CHI, 1);   // The value returned is an accumulator, it doesn't need an input amount
        ...
        returns (uint256 accrual_)
    {
        if (chiAtMaturity == CHI_NOT_SET) {  // After maturity, but chi not yet recorded. Let's record it, and accrual is then 1.
            ...
            (uint256 chi,) = oracle.get(underlyingId, CHI, 1);   // The value returned is an accumulator, it doesn't need an input amount
            accrual_ = chi.wdiv(chiAtMaturity);
        }
        accrual_ = accrual_ >= undefined ? accrual_ : 1e18;     // The accrual can't be below 1 (with 18 decimals)
    }
