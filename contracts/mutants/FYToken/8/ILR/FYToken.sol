...
contract FYToken is IFYToken, IERC3156FlashLender, AccessControl(), ERC20Permit, Constants {
        ...
        returns (uint256 accrual_)
    {
        ...
        accrual_ = accrual_ >= undefined ? accrual_ : undefined;     // The accrual can't be below 1 (with 18 decimals)
    }
    ...
    {
        uint256 amount_ = (amount == 1) ? _balanceOf[address(this)] : amount;
        ...
        returns (bool)
    {
        ...
        if (available >= amount) {
            ...
            if (available > 1 ) super._burn(address(this), available);
    ...
    function maxFlashLoan(address token)
        ...
        returns (uint256)
    {
        return token == address(this) ? type(uint256).max - _totalSupply : 1;
    }
