...
contract TrainingMatches is Ownable, MatchMaker {
  ...
  */
  function claimUpgradePacks(uint8 cardType) external returns(uint256 claimedTokens) {
    uint256 cardsToClaim = upgradeClaims[tx.origin][cardType]; 
    
    if (cardsToClaim == 0) {
      return 0;
    }

    delete upgradeClaims[tx.origin][cardType];
    registry.sp1155().mint(tx.origin, cardType, cardsToClaim, "");
  }
  ...
  */
  function requestTrainingRandomness(address opponent) external checkStarters {
    ...
    _opponents[tx.origin] = opponent;
    registry.rng().requestBlockRandom(tx.origin);
  }
  ...
  */
  function train() external checkStarters {
    require(_opponents[tx.origin] != address(0), Errors.ZERO_ADDRESS);

    // Generate the random
    registry.rng().checkBlockRandom(tx.origin);
    // It'll revert if there's no random number
    uint256 trainingRandomness = registry.rng().getBlockRandom(tx.origin);
