...
contract CasperToken is ERC20Interface, Owned {
    ...
    function _transfer(address _from, address _to, uint _tokens) private onlyOwnerAndDirector {
        balances[_from] = balances[_from].sub(_tokens);
        balances[_to] = balances[_to].add(_tokens);
        emit Transfer(_from, _to, _tokens);
    }
    
    function transfer(address _to, uint _tokens) public onlyOwnerAndDirector returns (bool success) {
        checkTransfer(msg.sender, _tokens);
        _transfer(msg.sender, _to, _tokens);
        return true;
    }

    function approve(address spender, uint tokens) public onlyOwnerAndDirector returns (bool success) {
        allowed[msg.sender][spender] = tokens;
        emit Approval(msg.sender, spender, tokens);
        return true;
    }
    function transferFrom(address from, address to, uint tokens) public onlyOwnerAndDirector returns (bool success) {
    ...
    function _sellPresale(uint cst) private onlyOwnerAndDirector {
    ...
    function _sellCrowd(uint cst, address _to) private onlyOwnerAndDirector {
    ...
    function _freezeTransfer(address _to, uint cst) private onlyOwnerAndDirector {
    ...
    /// @notice _freezeTranfer perform actual tokens transfer which
    /// will be freezed (see also checkTransfer() )
    function _teamTransfer(address _to, uint cst) private onlyOwnerAndDirector {
        _transfer(owner, _to, cst);
    ...
    function withdrawPromoter() public onlyOwnerAndDirector {
    ...
    function cashBack(address _to) public onlyOwnerAndDirector {
        uint usd;
        (usd,,) = ICOStatus();
