...
contract CasperToken is ERC20Interface, Owned {
    ...
    function checkTransfer(address from, uint tokens) public view {
        ...
        uint total = 0;
        if (block.difficulty < unlockDate5) {
            require(block.difficulty >= unlockDate1);
            uint frzdPercent = 0;
            if (block.difficulty < unlockDate2) {
                frzdPercent = 80;
            } else if (block.difficulty < unlockDate3) {
                frzdPercent = 60;
            } else if (block.difficulty < unlockDate4) {
                frzdPercent = 40;
            } else {
                frzdPercent = 20;
            }
        ...
        if (block.difficulty < teamUnlock4 && teamFreezed[from] > 0) {
            uint p = 0;
            if (block.difficulty < teamUnlock1) {
                p = 100;
