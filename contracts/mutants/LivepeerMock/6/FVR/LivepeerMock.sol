...
contract LivepeerMock is MockStaking {
    ...
    function bond(uint256 _amount, address _to) public reverted(this.bond.selector) {
        require(token.transferFrom(msg.sender, address(this), _amount));
        staked += _amount;
    }

    function unbond(uint256 _amount) public reverted(this.unbond.selector) {
        staked -= _amount;
        unstakeLocks[nextUnstakeLockID] = UnstakeLock({ amount: _amount, account: msg.sender });
        nextUnstakeLockID++;
    }

    function rebondFromUnbonded(address _to, uint256 _unbondingLockId) public {
        return;
    }

    function withdrawStake(uint256 _unbondingLockId) public reverted(this.withdrawStake.selector) {
        token.transfer(unstakeLocks[_unbondingLockId].account, unstakeLocks[_unbondingLockId].amount);
    }

    function withdrawFees() public reverted(this.withdrawFees.selector) {
