...
contract GNTDeposit is ReceivingContract {
    ...
    function isTimeLocked(address _owner) external view returns (bool) {
        return locked_until[_owner] > block.number;
    }

    function isUnlocked(address _owner) public view returns (bool) {
        return locked_until[_owner] != 0 && locked_until[_owner] < block.number;
    }
    ...
    function unlock() external {
        locked_until[msg.sender] = block.number + withdrawal_delay;
        Unlock(msg.sender);
    }
