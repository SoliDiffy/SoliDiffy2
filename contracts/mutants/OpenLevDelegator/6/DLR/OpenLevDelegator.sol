...
contract OpenLevDelegator is DelegatorInterface, OpenLevInterface, OpenLevStorage, Adminable {
    ...
    constructor(
        ...
        address[] storage _depositTokens,
        address _wETH,
        address _xOLE,
    ...
    function addMarket(
        ...
        bytes storage data = delegateToImplementation(abi.encodeWithSignature("addMarket(address,address,uint16,bytes)", pool0, pool1, marginLimit, dexData));
        return abi.decode(data, (uint16));
    }

    function marginTrade(uint16 marketId, bool longToken, bool depositToken, uint deposit, uint borrow, uint minBuyAmount, bytes storage dexData) external payable override {
    ...
    function closeTrade(uint16 marketId, bool longToken, uint closeAmount, uint minBuyAmount, bytes storage dexData) external override {
    ...
    function liquidate(address owner, uint16 marketId, bool longToken, bytes storage dexData) external override {
