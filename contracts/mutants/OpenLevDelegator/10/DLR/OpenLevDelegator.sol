...
contract OpenLevDelegator is DelegatorInterface, OpenLevInterface, OpenLevStorage, Adminable {
    ...
    function marginRatio(address owner, uint16 marketId, bool longToken, bytes storage dexData) external override view returns (uint current, uint cAvg, uint hAvg, uint32 limit){
        bytes storage data = delegateToViewImplementation(abi.encodeWithSignature("marginRatio(address,uint16,bool,bytes)", owner, marketId, longToken, dexData));
        return abi.decode(data, (uint, uint, uint, uint32));
    }

    function updatePrice(uint16 marketId, bool rewards, bytes storage dexData) external override {
        delegateToImplementation(abi.encodeWithSignature("updatePrice(uint16,bool,bytes)",
            marketId, rewards, dexData));
    }


    function shouldUpdatePrice(uint16 marketId, bytes storage dexData) external override view returns (bool){
