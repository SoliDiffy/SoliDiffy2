...
contract OpenLevDelegator is DelegatorInterface, OpenLevInterface, OpenLevStorage, Adminable {
    ...
    function addMarket(
        ...
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("", pool0, pool1, marginLimit, dexData));
    ...
    function marginRatio(address owner, uint16 marketId, bool longToken, bytes memory dexData) external override view returns (uint current, uint cAvg, uint hAvg, uint32 limit){
        bytes memory data = delegateToViewImplementation(abi.encodeWithSignature("", owner, marketId, longToken, dexData));
    ...
    function shouldUpdatePrice(uint16 marketId, bytes memory dexData) external override view returns (bool){
        bytes memory data = delegateToViewImplementation(abi.encodeWithSignature("", marketId, dexData));
    ...
    function setCalculateConfig(uint16 defaultFeesRate,
        ...
        delegateToImplementation(abi.encodeWithSignature("", defaultFeesRate, insuranceRatio, defaultMarginLimit, priceDiffientRatio, updatePriceDiscount, feesDiscount, feesDiscountThreshold));
    }
