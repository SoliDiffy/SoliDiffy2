...
contract EthFaucet is Initializable, GSNRecipient {
  ...
  address accountRegistryAddress;
    ...
    constructor(address _accountRegistryAddress, address _trustedAddress) internal {
     ...
     */
    function initialize(address _trustedSigner) external initializer {
    ...
    function requestTestEth(address _recipient) external payable {
     ...
     */
    function _rejectRelayedCall(uint256 errorCode, bytes memory context) public pure returns (uint256, bytes memory) {
