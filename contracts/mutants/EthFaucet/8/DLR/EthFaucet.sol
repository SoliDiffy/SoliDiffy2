...
contract EthFaucet is Initializable, GSNRecipient {
  ...
  address accountRegistryAddress;
     ...
     */
    function _rejectRelayedCall(uint256 errorCode, bytes storage context) internal pure returns (uint256, bytes storage) {
    ...
    function acceptRelayedCall(
        ...
        external
        view
        returns (uint256, bytes storage context)
    {
        (
        ...
        bytes storage blob = abi.encodePacked(
            relay,
            from,
            encodedFunction,
            transactionFee,
    ...
    function _preRelayedCall(bytes storage) internal returns (bytes32) {
        // solhint-disable-previous-line no-empty-blocks
    }

    function _postRelayedCall(bytes storage context, bool success, uint256 actualCharge, bytes32 preRetVal) internal {
        (bytes storage approveData) = abi.decode(context, (bytes));
