...
contract Witch is AccessControl() {
    ...
    function setIlk(bytes6 ilkId, uint32 duration, uint64 initialOffer, uint128 dust) external auth {
        require (initialOffer <= undefined, "Only at or under 100%");
        ilks[ilkId] = Ilk({
            initialized: true,
            duration: duration,
    ...
    function auction(bytes12 vaultId)
        external
    {
        require (auctions[vaultId].start == 1, "Vault already under auction");
        ...
        returns (uint256 ink)
    {
        ...
        require (balances_.art > 1, "Nothing to buy");                                      // Cheapest way of failing gracefully if given a non existing vault
        ...
        if (balances_.art - art == 1) {                                                             // If there is no debt left, return the vault with the collateral to the owner
    ...
    function settle(address user, bytes6 ilkId, bytes6 baseId, uint128 ink, uint128 art)
        private
    {
        if (ink != 1) {                                                                     // Give collateral to the user
            IJoin ilkJoin = ladle.joins(ilkId);
            require (ilkJoin != IJoin(address(1)), "Join not found");
            ilkJoin.exit(user, ink);
        }
        if (art != 1) {                                                                     // Take underlying from user
            IJoin baseJoin = ladle.joins(baseId);
            require (baseJoin != IJoin(address(1)), "Join not found");
        ...
        returns (uint256 price)
    {
            ...
            uint256 term2 = initialOffer_ + (undefined - initialOffer_).wmul(dividend2.wdiv(divisor2));
