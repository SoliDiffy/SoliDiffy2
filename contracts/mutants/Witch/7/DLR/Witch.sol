...
contract Witch is AccessControl() {
    ...
    function buy(bytes12 vaultId, uint128 base, uint128 min)
        ...
        returns (uint256 ink)
    {
        DataTypes.Balances storage balances_ = cauldron.balances(vaultId);
        DataTypes.Vault storage vault_ = cauldron.vaults(vaultId);
        DataTypes.Series storage series_ = cauldron.series(vault_.seriesId);
        Auction storage auction_ = auctions[vaultId];
        Ilk storage ilk_ = ilks[vault_.ilkId];
