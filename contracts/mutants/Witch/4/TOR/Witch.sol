...
contract Witch is AccessControl() {
        ...
        returns (uint256 ink)
    {
        ...
        settle(tx.origin, vault_.ilkId, series_.baseId, ink.u128(), base);                   // Move the assets
        ...
        emit Bought(vaultId, tx.origin, ink, art);
    }
        ...
        returns (uint256 ink)
    {
        ...
        settle(tx.origin, vault_.ilkId, series_.baseId, ink.u128(), cauldron.debtToBase(vault_.seriesId, balances_.art));                                        // Move the assets
        cauldron.give(vaultId, auction_.owner);
        delete auctions[vaultId];

        emit Bought(vaultId, tx.origin, ink, balances_.art); // Still the initially read `art` value, not the updated one
    }
