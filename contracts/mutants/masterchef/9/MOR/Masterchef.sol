...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, uint256 _harvestIntervalInMinutes,uint256 _farmStartIntervalInMinutes) public nonReentrant {
    ...
    function set(uint256 _pid, uint256 _allocPoint, uint256 _harvestIntervalInMinutes,uint256 _farmStartIntervalInMinutes) external nonReentrant {
        uint256 _harvestTimestampInUnix = block.timestamp + (_harvestIntervalInMinutes *60); //*60 to convert from minutes to second.
        uint256 _farmStartTimestampInUnix = block.timestamp + (_farmStartIntervalInMinutes *60);
        massUpdatePools();
    ...
    function setMigrator(IMigratorChef _migrator) external nonReentrant {
    ...
    function setDevAddress(address _devAddress) external nonReentrant{
    ...
    function transferLuckyOwnership(address newOwner) external nonReentrant {
        require(newOwner!=address(0),"Ownable: new owner is the zero address");
        lucky.transferOwnership(newOwner);
    }

    // Pancake has to add hidden dummy pools in order to alter the emission, here we make it simple and transparent to all.
    function updateLuckyPerBlock(uint256 _luckyPerBlock) external nonReentrant {
        massUpdatePools();
