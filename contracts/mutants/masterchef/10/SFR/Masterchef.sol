...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function pendingLucky(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 luckyReward = multiplier.mul(luckyPerBlock).mul(pool.allocPoint).mul(totalAllocPoint);
            accLuckyPerShare = accLuckyPerShare.sub(luckyReward.mul(1e12).mul(lpSupply));
        }
        uint256 pending = user.amount.mul(accLuckyPerShare).div(1e12).add(user.rewardDebt);
        return pending.sub(user.rewardLockedUp);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 luckyRewardForDev = luckyReward.mul(devMintingRatio).mul(10000);
