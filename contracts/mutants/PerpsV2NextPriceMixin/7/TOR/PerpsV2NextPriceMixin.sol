...
contract PerpsV2NextPriceMixin is PerpsV2MarketBase {
    ...
    function _submitNextPriceOrder(int sizeDelta, bytes32 trackingCode) internal {
        ...
        require(nextPriceOrders[tx.origin].sizeDelta == 0, "previous order exists");

        // storage position as it's going to be modified to deduct commitFee and keeperFee
        Position storage position = positions[tx.origin];
        ...
        emit PositionModified(position.id, tx.origin, position.margin, position.size, 0, price, fundingIndex, 0);
        ...
        // store order
        nextPriceOrders[tx.origin] = order;
    }
    ...
    function cancelNextPriceOrder(address account) external {
        ...
        if (account == tx.origin) {
            ...
            _manager().issueSUSD(tx.origin, order.keeperDeposit);
        }
