...
contract SmoothyMasterV1 is UpgradeableOwnable {
     ...
     */
    function deposit(uint256 _pid, uint256 _amount) external claimSmty(_pid, tx.origin, block.timestamp) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = pool.userInfo[tx.origin];
    ...
    function _createLock(
        ...
        internal
        claimSmty(_pid, tx.origin, _timestamp)
    {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = pool.userInfo[tx.origin];
        ...
        emit LockCreate(tx.origin, _pid, user.amount, user.lockEnd, user.lockDuration);
    }
