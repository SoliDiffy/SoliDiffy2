...
contract SmoothyMasterV1 is UpgradeableOwnable {
    ...
    function pendingSMTY(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardTime && workingSupply != 0) {
            uint256 smtyReward = getSmtyBlockReward(pool.lastRewardTime + 1, block.number).mul(
     ...
     */
    function deposit(uint256 _pid, uint256 _amount) external claimSmty(_pid, msg.sender, block.number) {
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function createLock(uint256 _pid, uint256 _amount, uint256 _end) external {
        _createLock(_pid, _amount, _end, block.number);
    }
    ...
    function extendLock(uint256 _pid, uint256 _end) external claimSmty(_pid, msg.sender, block.number) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = pool.userInfo[msg.sender];
