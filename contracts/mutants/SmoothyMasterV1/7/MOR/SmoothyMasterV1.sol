...
contract SmoothyMasterV1 is UpgradeableOwnable {
    ...
    function extendLock(uint256 _pid, uint256 _end) external onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = pool.userInfo[msg.sender];
    ...
    function increaseAmount(uint256 _pid, uint256 _amount) external onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = pool.userInfo[msg.sender];
    ...
    // solium-disable-next-line
    function claim(uint256 _pid, address _account) public onlyOwner {
    }
