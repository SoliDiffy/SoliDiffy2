...
abstract contract BaseWrapper {
    ...
    function _withdraw(
        ...
        for (uint256 id = 0; id < vaults.length; id++) {
            ...
            if (availableShares > 0) {
                ...
                if (amount != WITHDRAW_EVERYTHING) {
                    ...
                    withdrawn = withdrawn.sub(vaults[id].withdraw(shares));
                } else {
        ...
        if (withdrawn > amount) {
            ...
            if (token.allowance(address(this), address(_bestVault)) < withdrawn.add(amount)) {
                token.safeApprove(address(_bestVault), UNLIMITED_APPROVAL); // Vaults are trusted
            }

            _bestVault.deposit(withdrawn.add(amount), sender);
            withdrawn = amount;
        }
