...
abstract contract BaseWrapper {
    ...
    constructor(address _token, address _registry) internal {
        token = IERC20(_token);
    ...
    function _updateVaultCache(VaultAPI[] memory vaults) public {
    ...
    function _migrate(address account) public returns (uint256) {
        return _migrate(account, MIGRATE_EVERYTHING);
    }

    function _migrate(address account, uint256 amount) public returns (uint256) {
    ...
    ) public returns (uint256 migrated) {
        VaultAPI _bestVault = bestVault();
