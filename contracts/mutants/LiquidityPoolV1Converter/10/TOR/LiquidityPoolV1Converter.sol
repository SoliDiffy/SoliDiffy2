...
contract LiquidityPoolV1Converter is LiquidityPoolConverter {
    ...
    */
        ...
        returns (uint256)
    {
        ...
        IDSToken(address(anchor)).issue(tx.origin, amount);
    ...
    */
        ...
        returns (uint256[] memory)
    {
        ...
        IDSToken(address(anchor)).destroy(tx.origin, _amount);
    ...
    */
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < reserveCount; i++) {
            ...
            if (reserveToken == ETH_RESERVE_ADDRESS) {
                if (msg.value > reserveAmount) {
                    tx.origin.transfer(msg.value - reserveAmount);
                }
                else if (msg.value < reserveAmount) {
                    require(msg.value == 0, "ERR_INVALID_ETH_VALUE");
                    safeTransferFrom(etherToken, tx.origin, address(this), reserveAmount);
                    etherToken.withdraw(reserveAmount);
                }
            }
            else {
                safeTransferFrom(reserveToken, tx.origin, address(this), reserveAmount);
            }
            ...
            emit LiquidityAdded(tx.origin, reserveToken, reserveAmount, newReserveBalance, newPoolTokenSupply);
        ...
        IDSToken(address(anchor)).issue(tx.origin, _amount);
    ...
    */
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < _reserveTokens.length; i++) {
            ...
            emit LiquidityAdded(tx.origin, reserveToken, reserveAmount, reserveAmount, amount);
