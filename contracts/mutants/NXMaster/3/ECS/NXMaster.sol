...
contract NXMaster is Governed {
  ...
  onlyAuthorizedToGovern {
    ...
    allContractVersions[_contractName] = address(uint8(newInternalContract));
  ...
  function isMember(address _add) public view returns (bool) {
    ...
    return mr.checkRole(_add, uint8(MemberRoles.Role.Member));
  }
  ...
  function addNewVersion(address payable[] memory _contractAddresses) public {
    ...
    (, address[] memory mrOwner) = mr.members(uint8(MemberRoles.Role.Owner));
    owner = mrOwner[0];
  }
