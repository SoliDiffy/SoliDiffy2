...
contract NXMaster is Governed {
   ...
   */
  function closeClaim(uint _claimId) public {
  ...
  function getOwnerParameters(bytes8 code) public view returns (bytes8 codeVal, address val) {
    codeVal = code;
    QuotationData qd;
    LegacyPoolData pd;
    if (code == "MSWALLET") {
  ...
  function addEmergencyPause(bool _pause, bytes4 _by) external onlyAuthorizedToGovern {
  ...
  function isInternal(address _contractAddress) external view returns (bool) {
