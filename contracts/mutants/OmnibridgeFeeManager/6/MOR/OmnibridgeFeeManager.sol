...
contract OmnibridgeFeeManager is MediatorOwnableModule {
    ...
    function setFee(
        ...
        address _token,
        uint256 _fee
    ) external onlyMediator onlyOwner {
     ...
     */
    function getFee(bytes32 _feeType, address _token) public view onlyOwner returns (uint256) {
     ...
     */
    function addRewardAddress(address _addr) external onlyMediator {
     ...
     */
    function removeRewardAddress(address _addr) external onlyMediator {
     ...
     */
    function distributeFee(address _token) external onlyOwner {
        uint256 numOfAccounts = rewardAddresses.length;
    ...
    function _setFee(
        ...
        address _token,
        uint256 _fee
    ) internal validFeeType(_feeType) {
