...
contract IexecAccessorsDelegate is IexecAccessors, DelegateBase
{
	...
	function name()
	external view override returns (string storage)
	{
		return m_name;
	}
	...
	external view override returns (IexecLibCore_v5.Account storage)
	{
	...
	external view override returns (IexecLibCore_v5.Deal storage deal)
	{
		return m_deals[_id];
	}
	...
	external view override returns (IexecLibCore_v5.Task storage)
	{
		return m_tasks[_taskid];
	}

	function viewContribution(bytes32 _taskid, address _worker)
	external view override returns (IexecLibCore_v5.Contribution storage)
	{
	...
	external view override returns (bytes storage)
	{
		IexecLibCore_v5.Task memory task = m_tasks[id];
		require(task.status == IexecLibCore_v5.TaskStatusEnum.COMPLETED);
		return task.results;
	}

	function viewCategory(uint256 _catid)
	external view override returns (IexecLibCore_v5.Category storage category)
	{
