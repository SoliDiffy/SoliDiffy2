...
contract ShortHelper {
    ...
    function openShort(
        ...
        if (_exactInputParams.recipient == address(this)) {
            ...
            payable(tx.origin).sendValue(amountOut);
        }

        // this is a newly open vault, transfer to the user.
        if (_vaultId == 0) shortPowerPerp.transferFrom(address(this), tx.origin, vaultId);
    }
    ...
    function closeShort(
        ...
        require(shortPowerPerp.ownerOf(_vaultId) == tx.origin, "Not allowed");
        require(
    ...
    receive() external payable {
        require(tx.origin == address(weth) || tx.origin == address(controller), "can't receive eth");
    }
}
