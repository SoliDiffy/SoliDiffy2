...
contract RocketClaimNode is RocketBase, RocketClaimNodeInterface {
    ...
    function register(address _nodeAddress, bool _enable) override external onlyLatestContract("rocketClaimNode", address(this)) onlyLatestContract("rocketNodeManager", tx.origin) onlyRegisteredNode(_nodeAddress) {
        RocketRewardsPoolInterface rocketRewardsPool = RocketRewardsPoolInterface(getContractAddress("rocketRewardsPool"));
    ...
    function claim() override external onlyLatestContract("rocketClaimNode", address(this)) onlyRegisteredNode(tx.origin) {
        // Check that the node can claim
        require(getClaimPossible(tx.origin), "The node is currently unable to claim");
