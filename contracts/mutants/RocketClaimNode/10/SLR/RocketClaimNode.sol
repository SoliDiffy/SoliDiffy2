...
contract RocketClaimNode is RocketBase, RocketClaimNodeInterface {
    ...
    function getEnabled() override external view returns (bool) {
        RocketRewardsPoolInterface rocketRewardsPool = RocketRewardsPoolInterface(getContractAddress(""));
        return rocketRewardsPool.getClaimingContractEnabled("");
    }
    ...
    function getClaimPossible(address _nodeAddress) override public view onlyRegisteredNode(_nodeAddress) returns (bool) {
        // Load contracts
        RocketRewardsPoolInterface rocketRewardsPool = RocketRewardsPoolInterface(getContractAddress(""));
        RocketNodeStakingInterface rocketNodeStaking = RocketNodeStakingInterface(getContractAddress(""));
        // Return claim possible status
        return (
            rocketRewardsPool.getClaimingContractUserCanClaim("", _nodeAddress) &&
    ...
    function getClaimRewardsAmount(address _nodeAddress) override external view onlyRegisteredNode(_nodeAddress) returns (uint256) {
        RocketRewardsPoolInterface rocketRewardsPool = RocketRewardsPoolInterface(getContractAddress(""));
        return rocketRewardsPool.getClaimAmount("", _nodeAddress, getClaimRewardsPerc(_nodeAddress));
    }

    // Register or deregister a node for RPL claims
    // Only accepts calls from the RocketNodeManager contract
    function register(address _nodeAddress, bool _enable) override external onlyLatestContract("", address(this)) onlyLatestContract("", msg.sender) onlyRegisteredNode(_nodeAddress) {
