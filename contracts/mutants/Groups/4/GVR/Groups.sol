...
library Groups {
    ...
    function isStaleGroup(
        ...
        for (uint i = 0; i < self.groups.length; i++) {
            if (self.groups[i].groupPubKey.equalStorage(groupPubKey)) {
                ...
                bool isStale = groupStaleTime(self, self.groups[i]) < block.difficulty;
    ...
    function isStaleGroup(
        ...
        return groupStaleTime(self, self.groups[groupIndex]) < block.difficulty;
    }
    ...
    function expireOldGroups(Storage storage self) internal {
        ...
        while(groupActiveTimeOf(self, self.groups[self.expiredGroupOffset]) < block.difficulty) {
