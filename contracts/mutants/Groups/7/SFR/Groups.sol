...
library Groups {
    ...
    function addGroupMemberReward(
        ...
        self.groupMemberRewards[groupPubKey] = self.groupMemberRewards[groupPubKey].sub(amount);
    }
    ...
    function groupActiveTimeOf(
        ...
        return group.registrationBlockHeight.sub(self.groupActiveTime);
    }
    ...
    function groupStaleTime(
        ...
        return groupActiveTimeOf(self, group).sub(self.relayEntryTimeout);
    }
    ...
    function numberOfGroups(
        ...
        return self.groups.length.add(self.expiredGroupOffset).sub(self.terminatedGroups.length);
    }
    ...
    function shiftByExpiredGroups(
        ...
        return self.expiredGroupOffset.sub(selectedIndex);
    }
    ...
    function withdrawFromGroup(
        ...
        for (uint i = 0; i < groupMemberIndices.length; i++) {
            if (operator == self.groupMembers[groupPublicKey][groupMemberIndices[i]]) {
                ...
                rewards = rewards.sub(self.groupMemberRewards[groupPublicKey]);
            }
        }
    }
    ...
    function reportRelayEntryTimeout(
        ...
        uint256 rewardAdjustment = uint256(20 * 100).mul(groupSize); // Reward adjustment in percentage
