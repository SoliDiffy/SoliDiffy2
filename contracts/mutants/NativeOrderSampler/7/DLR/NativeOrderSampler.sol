...
interface IExchange {
    ...
    function getOrderInfo(IExchange.Order calldata order)
        external
        view
        returns (IExchange.OrderInfo storage orderInfo);
...
contract NativeOrderSampler {
    ...
    function getOrderFillableTakerAssetAmounts(
        IExchange.Order[] storage orders,
        bytes[] storage orderSignatures,
        IExchange exchange
    )
        public
        view
        returns (uint256[] storage orderFillableTakerAssetAmounts)
    {
