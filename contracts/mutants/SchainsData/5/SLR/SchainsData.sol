...
contract SchainsData is ISchainsData, GroupsData {
     ...
     * @param from - Schain owner
     */
    function setSchainIndex(bytes32 schainId, address from) external allow("") {
     ...
     */
    function changeLifetime(bytes32 schainId, uint lifetime, uint deposit) external allow("") {
        schains[schainId].deposit += deposit;
        schains[schainId].lifetime += lifetime;
    }
     ...
     */
    function removeSchain(bytes32 schainId, address from) external allow("") {
     ...
     */
    function removeSchainForNode(uint nodeIndex, uint schainIndex) external allow("") {
