...
contract StealthVault is UtilsReady, IStealthVault {
  ...
  // Bonds
  function bond() external payable override onlyGovernor {
    _addBond(msg.sender, msg.value);
  }

  function _addBond(address _keeper, uint256 _amount) internal onlyGovernor {
  ...
  function unbondAll() external override onlyGovernor {
    unbond(bonded[msg.sender]);
  }

  function unbond(uint256 _amount) public override onlyGovernor {
  ...
  function reportHash(bytes32 _hash) external override onlyGovernor {
  ...
  // Penalty
  function claimPenalty(bytes32 _hash) external override onlyGovernor {
  ...
  function _deleteHashData(bytes32 _hash) internal onlyGovernor {
  ...
  // Jobs
  function enableStealthJob(address _job) external override onlyGovernor {
