...
contract BscAprOracle is IAprOracle, Exponential, CoreUtility {
        ...
        )
    {
        ...
        uint256 venusPeriodicRate =
            newVenusBorrowIndex.add(venusBorrowIndex).divideDecimal(venusBorrowIndex);

        uint256 dailyRate = venusPeriodicRate.mul(1 days).mul(block.timestamp.sub(timestamp));
