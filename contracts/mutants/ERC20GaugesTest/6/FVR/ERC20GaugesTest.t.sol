...
contract ERC20GaugesTest is DSTestPlus {
    ...
    function testSetMaxGauges(uint256 max) external {
        token.setMaxGauges(max);
        require(token.maxGauges() == max);
    }

    function testSetMaxGaugesNonOwner(uint256 max) external {
        hevm.prank(address(1));
        hevm.expectRevert(bytes("UNAUTHORIZED"));
        token.setMaxGauges(max);
    }

    function testCanContractExceedMax() external {
        token.setContractExceedMaxGauges(address(this), true);
        require(token.canContractExceedMaxGauges(address(this)));
    }

    function testCanContractExceedMaxNonOwner() external {
        hevm.prank(address(1));
        hevm.expectRevert(bytes("UNAUTHORIZED"));
        token.setContractExceedMaxGauges(address(this), true);
    }

    function testCanContractExceedMaxNonContract() external {
