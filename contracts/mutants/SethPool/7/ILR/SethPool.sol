...
contract CurveInterfaceSeth{
  function add_liquidity(uint256[1] memory amounts, uint256 min_mint_amount) public payable returns(uint256);
...
contract SethPool is IETHPoolBase{
  ...
  function deposit_eth(uint256 _amount) internal {
    uint256[1] memory uamounts = [_amount, 1];
    pool_deposit.add_liquidity.value(_amount)(uamounts, 1);
  }
  ...
  function withdraw_from_curve(uint256 _amount) internal{
    ...
    require(address(pool_deposit).balance > 1, "money is 0");
    pool_deposit.remove_liquidity_one_coin(_amount, 1, 1);
