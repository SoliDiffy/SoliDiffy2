...
library JobLib {
    ...
    function validTranscodingOptions(string _transcodingOptions) public pure returns (bool) {
        uint256 transcodingOptionsLength = bytes1(_transcodingOptions).length;
    ...
    function calcFees(uint256 _totalSegments, string _transcodingOptions, uint256 _pricePerSegment) public pure returns (uint256) {
        ...
        uint256 totalProfiles = bytes1(_transcodingOptions).length.div(VIDEO_PROFILE_SIZE);
        ...
        returns (bool)
    {
        ...
        // should be verified
        if (uint8(keccak256(_challengeBlock, _challengeBlockHash, _segmentNumber)) % _verificationRate == 0) {
            return true;
        } else {
            return false;
        }
