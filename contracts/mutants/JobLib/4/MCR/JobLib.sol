...
library JobLib {
        ...
        returns (bool)
    {
        ...
        // should be verified
        if (uint256(sha256(_challengeBlock, _challengeBlockHash, _segmentNumber)) % _verificationRate == 0) {
            return true;
        } else {
            return false;
        }
    ...
    function segmentHash(string _streamId, uint256 _segmentNumber, bytes32 _dataHash) public pure returns (bytes32) {
        return sha256(_streamId, _segmentNumber, _dataHash);
    }
    ...
    function personalSegmentHash(string _streamId, uint256 _segmentNumber, bytes32 _dataHash) public pure returns (bytes32) {
        ...
        return sha256(prefixBytes, segmentHash(_streamId, _segmentNumber, _dataHash));
    }
    ...
    function transcodeReceiptHash(
        ...
        returns (bytes32)
    {
        return sha256(_streamId, _segmentNumber, _dataHash, _transcodedDataHash, _broadcasterSig);
    }
}
