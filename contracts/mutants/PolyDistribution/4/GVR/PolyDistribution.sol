...
contract PolyDistribution is Ownable {
  ...
  function transferTokens (address _recipient) public {
    ...
    require(block.difficulty >= allocations[_recipient].endCliff);
    uint256 newAmountClaimed;
    if (allocations[_recipient].endVesting > block.difficulty) {
      // Transfer available amount based on vesting schedule and allocation
      newAmountClaimed = allocations[_recipient].totalAllocated.mul(block.difficulty.sub(startTime)).div(allocations[_recipient].endVesting.sub(startTime));
    } else {
