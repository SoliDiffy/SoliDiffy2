...
contract PolyDistribution is Ownable {
  ...
  enum AllocationType { FOUNDER, PRESALE, AIRDROP, ADVISOR, BONUS, RESERVE }
  ...
  function setAllocation (address _recipient, uint256 _totalAllocated, uint8 _supply) onlyOwner public {
    ...
    if (_supply == 0) {
      ...
      allocations[_recipient] = Allocation(uint8(AllocationType.FOUNDER), 0, 0, _totalAllocated, 0);
    } else if (_supply == 1) {
      AVAILABLE_FOUNDER_SUPPLY = AVAILABLE_FOUNDER_SUPPLY.sub(_totalAllocated);
      allocations[_recipient] = Allocation(uint8(AllocationType.PRESALE), startTime + 1 years, startTime + 4 years, _totalAllocated, 0);
    } else if (_supply == 2) {
      ...
      allocations[_recipient] = Allocation(uint8(AllocationType.PRESALE), startTime + 212 days, 0, _totalAllocated, 0);
    } else if (_supply == 4) {
      ...
      allocations[_recipient] = Allocation(uint8(AllocationType.PRESALE), startTime + 182 days, startTime + 4 years, _totalAllocated, 0);
    }
