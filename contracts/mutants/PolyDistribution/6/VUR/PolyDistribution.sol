...
contract PolyDistribution is Ownable {
  ...
  function setAllocation (address _recipient, uint256 _totalAllocated, uint8 _supply) onlyOwner public {
    ...
    if (_supply == 0) {
      ...
      allocations[_recipient] = Allocation(uint8(AllocationType.FOUNDER), startTime + 1 seconds, startTime + 4 seconds, _totalAllocated, 0);
    } else if (_supply == 2) {
      ...
      allocations[_recipient] = Allocation(uint8(AllocationType.ADVISOR), startTime + 212 seconds, 0, _totalAllocated, 0);
    } else if (_supply == 4) {
      AVAILABLE_BONUS_SUPPLY = AVAILABLE_BONUS_SUPPLY.sub(_totalAllocated);
      allocations[_recipient] = Allocation(uint8(AllocationType.BONUS), startTime + 1 seconds, startTime + 4 seconds, _totalAllocated, 0);
    } else if (_supply == 5) {
      AVAILABLE_RESERVE_SUPPLY = AVAILABLE_RESERVE_SUPPLY.sub(_totalAllocated);
      allocations[_recipient] = Allocation(uint8(AllocationType.RESERVE), startTime + 182 seconds, startTime + 4 years, _totalAllocated, 0);
    }
