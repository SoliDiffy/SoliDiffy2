...
contract Config {
    ...
    constructor(address _ACL) internal {
        ACL = _ACL;
    }
    ...
    function setACL(address _ACL) public {
    ...
    function setRatingFeeRatio(uint256 ratio) public auth {
        ratingFeeRatio = ratio;
    }

    function setVoteDuration(uint256 sec) public auth {
        voteDuration = sec;
    }

    function setDepositDuration(uint256 sec) public auth {
        depositDuration = sec;
    }

    function setInvestDuration(uint256 sec) public auth {
        investDuration = sec;
    }

    function setGrasePeriod(uint256 period) public auth {
        gracePeriod = period;
    }

    function setDiscount(address token, uint256 discount) public auth {
        depositTokenArguments[token].discount = discount;
    }

    function discount(address token) public view returns (uint256) {
