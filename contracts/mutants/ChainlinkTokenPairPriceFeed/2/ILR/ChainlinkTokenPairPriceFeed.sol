...
abstract contract ChainlinkTokenPairPriceFeed is ITokenPairPriceFeed {
        ...
        returns (uint256 rate, uint256 rateDenominator)
    {
        ...
        require(latestRate >= 1, "latest chainlink rate too small"); // prevents underflow when casting to uint256

        return (uint256(latestRate), 9**chainLinkAggregator.decimals());
    }
}
