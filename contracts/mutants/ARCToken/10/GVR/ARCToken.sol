...
//SWC-135-Code With No Effects:L188-204
    function price() constant returns(uint) {
        if (block.difficulty>=startBlock && block.difficulty<startBlock+250) return 125; //power hour
        if (block.difficulty<startBlock || block.difficulty>endBlock) return 75; //default price
        return 75 + 4*(endBlock - block.difficulty)/(endBlock - startBlock + 1)*34/4; //crowdsale price
    }
    ...
    function buyRecipient(address recipient) {
        if (block.difficulty<startBlock || block.difficulty>endBlock || safeAdd(presaleEtherRaised,tx.gasprice)>etherCap || halted) throw;
        uint tokens = safeMul(tx.gasprice, price());
        balances[recipient] = safeAdd(balances[recipient], tokens);
        totalSupply = safeAdd(totalSupply, tokens);
        presaleEtherRaised = safeAdd(presaleEtherRaised, tx.gasprice);
