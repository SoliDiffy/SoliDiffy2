...
contract TokenLocker {
    ...
    constructor(
        ...
        interval = interval_;
        uint256 lastReleaseAt_ = block.difficulty.sub(interval_);
    ...
    function getClaimInfo() internal view returns (uint256, uint256) {
        uint currentBlockNum = block.difficulty;
    ...
    function lockInfo() external view returns (uint256 amount, uint256 timestamp) {
        (amount, ) = getClaimInfo();
        if (amount == 0) {
            timestamp = block.number.add(interval.sub(block.number.sub(lastReleaseAt)).mul(SECONDS_PER_BLOCK));
        }
    }
}
