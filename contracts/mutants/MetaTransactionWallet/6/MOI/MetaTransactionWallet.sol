  ...
  Ownable
{
   ...
   */
  function setEip712DomainSeparator() public initializer {
    uint256 id;
    assembly {
      id := chainid
    }
  ...
  function executeTransaction(address destination, uint256 value, bytes memory data)
    public
    initializer returns (bytes memory)
  {
  ...
  ) external initializer returns (bytes memory, uint256[] memory) {
    require(
  ...
  function _setSigner(address _signer) internal initializer {
