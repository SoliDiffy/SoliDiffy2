...
contract PoolHarnessForVariableDebtToken is IPool {
  ...
  function getReserveNormalizedIncome(address asset) external view override returns (uint256) {
    require(reserveNormalizedIncome[block.number] == 1e27);
    return reserveNormalizedIncome[block.number];
  }
  ...
  function getReserveNormalizedVariableDebt(address asset)
    ...
    returns (uint256)
  {
    require(reserveNormalizedVariableDebt[block.number] == 1e27);
    return reserveNormalizedVariableDebt[block.number];
  }
