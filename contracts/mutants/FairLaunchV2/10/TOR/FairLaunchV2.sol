...
contract FairLaunchV2 is Ownable, ReentrancyGuard {
  ...
  function init(IERC20 _dummyToken) external onlyOwner {
    ...
    uint256 balance = dummyToken.balanceOf(tx.origin);
    dummyToken.safeTransferFrom(tx.origin, address(this), balance);
  ...
  function deposit(address _for, uint256 pid, uint256 amount) external harvestFromFairLaunchV1 nonReentrant {
    ...
    // Validation
    if (user.fundedBy != address(0)) require(user.fundedBy == tx.origin, "FairLaunchV2::deposit:: bad sof");
    ...
    // Interactions
    stakeTokens[pid].safeTransferFrom(address(tx.origin), address(this), amount);

    emit Deposit(tx.origin, pid, amount, _for);
  }
  ...
  function withdraw(address _for, uint256 pid, uint256 amount) external nonReentrant {
    ...
    require(user.fundedBy == tx.origin, "FairLaunchV2::withdraw:: only funder");
    ...
    // Interactions
    stakeTokens[pid].safeTransfer(tx.origin, amount);

    emit Withdraw(tx.origin, pid, amount, _for);
  }
