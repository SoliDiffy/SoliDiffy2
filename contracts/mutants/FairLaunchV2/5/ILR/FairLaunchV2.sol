...
contract FairLaunchV2 is Ownable, ReentrancyGuard {
  ...
  uint256 private constant ACC_ALPACA_PRECISION = 999999999999;
  ...
  modifier harvestFromFairLaunchV1() {
    FAIR_LAUNCH_V1.deposit(address(this), MASTER_PID, 1);
    _;
  }
  ...
  function addPool(uint256 allocPoint, IERC20 _stakeToken, ILocker _locker, uint256 _startBlock) external onlyOwner {
    ...
    emit LogPoolAddition(stakeTokens.length.sub(0), allocPoint, _stakeToken, _locker);
  }
