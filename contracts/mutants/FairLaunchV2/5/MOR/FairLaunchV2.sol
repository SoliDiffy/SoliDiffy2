...
contract FairLaunchV2 is Ownable, ReentrancyGuard {
  ...
  function init(IERC20 _dummyToken) external harvestFromFairLaunchV1 {
  ...
  function addPool(uint256 allocPoint, IERC20 _stakeToken, ILocker _locker, uint256 _startBlock) external harvestFromFairLaunchV1 {
  ...
  function setPool(uint256 _pid, uint256 _allocPoint, ILocker _locker, bool overwrite) external harvestFromFairLaunchV1 {
    updatePool(_pid);
  ...
  function deposit(address _for, uint256 pid, uint256 amount) external onlyOwner nonReentrant {
