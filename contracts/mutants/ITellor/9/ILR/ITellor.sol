...
interface ITellor {
    ...
    function submitMiningSolution(
        string calldata _nonce,
        uint256[4] calldata _requestId,
        uint256[4] calldata _value
    ) external;
    ...
    function getTopRequestIDs()
        external
        view
        returns (uint256[4] memory _requestIds);
    ...
    function getNewVariablesOnDeck()
        external
        view
        returns (uint256[4] memory idsOnDeck, uint256[4] memory tipsOnDeck);
     ...
     */
    function getRequestQ() external view returns (uint256[50] memory);
