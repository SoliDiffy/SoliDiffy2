...
contract Define is ERC20, Ownable {
    ...
    function getTradingIsEnabled() public view returns (bool) {
        return block.number >= tradingEnabledTimestamp;
    }
    ...
    function _transfer(
        ...
        if(isFixedSaleBuy) {
            require(block.number >= fixedSaleStartTimestamp, "Define: The fixed-sale has not started yet.");

            bool openToEveryone = block.number.sub(fixedSaleStartTimestamp) >= fixedSaleEarlyParticipantDuration ||
