    ...
    RewardsDistributionRecipient
{
        ...
        updateReward(tx.origin)
    {
        ...
        if (userContracts[tx.origin] == address(0)) {
            ...
            userContracts[tx.origin] = address(newContract);
        }

        stakingToken.safeTransferFrom(
            tx.origin,
            userContracts[tx.origin],
            amount
        );
        ...
        StakeOrion(userContracts[tx.origin]).deposit(amount);
    }
