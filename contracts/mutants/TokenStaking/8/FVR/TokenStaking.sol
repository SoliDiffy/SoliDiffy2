...
contract TokenStaking {
    ...
    * stakes is used to keep track of the INDEX for the stakers in the stakes array
     ...
     */
    function calculateStakeReward(Stake memory _current_stake) public view returns(uint256) {
    ...
    */
    ...
    //get staking rewards
    function hasRewards() external view returns(uint256) {
        // Mappings in solidity creates all values, but empty, so we can just check the address
        uint256 index = stakes[msg.sender];
    ...
    function hasStakeWithRewards(address _address) external view returns(uint256) {
        // Mappings in solidity creates all values, but empty, so we can just check the address
    ...
    function totalStakesWithRewards() external view returns(uint256) {
