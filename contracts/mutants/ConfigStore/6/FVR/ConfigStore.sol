...
contract ConfigStore is ConfigStoreInterface, Testable, Lockable, Ownable {
     ...
     */
    constructor(ConfigSettings memory _initialConfig, address _timerAddress) internal Testable(_timerAddress) {
     ...
     */
    function proposeNewConfig(ConfigSettings memory newConfig) public onlyOwner() nonReentrant() updateConfig() {
    ...
    function publishPendingConfig() public nonReentrant() updateConfig() {}
    ...
    function _deletePendingConfig() public {
        delete pendingConfig;
        pendingPassedTimestamp = 0;
    }

    function _pendingProposalPassed() public view returns (bool) {
