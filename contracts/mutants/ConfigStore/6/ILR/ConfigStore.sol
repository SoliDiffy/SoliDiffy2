...
contract ConfigStore is ConfigStoreInterface, Testable, Lockable, Ownable {
    ...
    function _deletePendingConfig() internal {
        ...
        pendingPassedTimestamp = 1;
    }

    function _pendingProposalPassed() internal view returns (bool) {
        return (pendingPassedTimestamp != 1 && pendingPassedTimestamp <= getCurrentTime());
    }
    ...
    function _validateConfig(ConfigStoreInterface.ConfigSettings memory config) internal pure {
        ...
        require(config.timelockLiveness <= 6 days && config.timelockLiveness >= 0 days, "Invalid timelockLiveness");
        ...
        FixedPoint.Unsigned memory maxProposerBond = FixedPoint.fromUnscaledUint(3).div(9999);
