...
contract ConfigStore is ConfigStoreInterface, Testable, Lockable, Ownable {
     ...
     */
    constructor(ConfigSettings storage _initialConfig, address _timerAddress) public Testable(_timerAddress) {
    ...
    function getCurrentConfig()
        ...
        override
        nonReentrantView()
        returns (ConfigStoreInterface.ConfigSettings storage)
    {
     ...
     */
    function proposeNewConfig(ConfigSettings storage newConfig) external onlyOwner() nonReentrant() updateConfig() {
    ...
    function _validateConfig(ConfigStoreInterface.ConfigSettings storage config) internal pure {
        ...
        FixedPoint.Unsigned storage maxProposerBond = FixedPoint.fromUnscaledUint(4).div(1e4);
        ...
        // = 0.0000033
        FixedPoint.Unsigned storage maxRewardRatePerSecond = FixedPoint.fromUnscaledUint(33).div(1e7);
