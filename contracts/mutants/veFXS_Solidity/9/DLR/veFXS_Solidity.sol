...
contract veFXS_Solidity is ReentrancyGuard{
    ...
    */
    ...
    function EMPTY_POINT_FACTORY() internal view returns (Point storage){
        return Point({
            bias: 0, 
            slope: 0, 
            ts: 0, 
    ...
    // Constant structs not allowed yet, so this will have to do
    function EMPTY_LOCKED_BALANCE_FACTORY() internal view returns (LockedBalance storage){
    ...
    */
    function balanceOf(address addr, uint256 _t) public view returns (uint256) {
        ...
        if (_epoch == 0) {
            ...
            Point storage last_point = user_point_history[addr][_epoch];
    ...
    */
    function balanceOfAt(address addr, uint256 _block) external view returns (uint256) {
        ...
        Point storage upoint = user_point_history[addr][_min];
