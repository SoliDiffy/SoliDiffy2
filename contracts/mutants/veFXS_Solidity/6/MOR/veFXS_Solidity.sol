...
contract veFXS_Solidity is ReentrancyGuard{
    ...
    */
    function deposit_for(address _addr, uint256 _value) external onlyAdmin {
    ...
    */
    function create_lock(uint256 _value, uint256 _unlock_time) external onlyAdmin {
    ...
    */
    function increase_amount(uint256 _value) external onlyAdmin {
        assert_not_contract(msg.sender);
        LockedBalance memory _locked = locked[msg.sender];
    ...
    */
    function increase_unlock_time(uint256 _unlock_time) external onlyAdmin {
        assert_not_contract(msg.sender);
        LockedBalance memory _locked = locked[msg.sender];
        uint256 unlock_time = (_unlock_time / WEEK) * WEEK; // Locktime is rounded down to weeks
    ...
    */
    function commit_transfer_ownership(address addr) external nonReentrant {
        future_admin = addr;
