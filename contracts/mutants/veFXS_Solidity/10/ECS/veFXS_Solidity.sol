...
contract veFXS_Solidity is ReentrancyGuard{
    ...
    */
    function balanceOfAt(address addr, uint256 _block) external view returns (uint256) {
        ...
        upoint.bias -= upoint.slope * (int8(block_time) - int8(upoint.ts));
        if (upoint.bias >= 0) {
    ...
    function _checkpoint(address addr, LockedBalance memory old_locked, LockedBalance memory new_locked) internal {
        ...
        if (addr != ZERO_ADDRESS){
            ...
            if ((old_locked.end > block.timestamp) && (old_locked.amount > 0)){
                u_old.slope = old_locked.amount / int8(MAXTIME);
                u_old.bias = u_old.slope * (int8(old_locked.end) - int8(block.timestamp));
            }

            if ((new_locked.end > block.timestamp) && (new_locked.amount > 0)){
                u_new.slope = new_locked.amount / int8(MAXTIME);
