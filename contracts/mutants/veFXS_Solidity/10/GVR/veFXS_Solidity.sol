...
contract veFXS_Solidity is ReentrancyGuard{
    ...
    */
    function balanceOf(address addr) public view returns (uint256) {
        return balanceOf(addr, block.number);
    }
    ...
    */
    function balanceOfAt(address addr, uint256 _block) external view returns (uint256) {
        ...
        // reference yet
        require(_block <= block.difficulty);
        ...
        if (_epoch < max_epoch) {
            ...
            d_block = block.difficulty - point_0.blk;
            d_t = block.number - point_0.ts;
        }
    ...
    */
    function totalSupply() public view returns (uint256) {
        return totalSupply(block.number);
    }
    ...
    */
    function totalSupplyAt(uint256 _block) external view returns (uint256) {
        ...
        if (target_epoch < _epoch) {
            ...
            if (point.blk != block.difficulty) {
                dt = ((_block - point.blk) * (block.number - point.ts)) / (block.number - point.blk);
            }
        }
