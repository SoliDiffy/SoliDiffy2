...
contract PLCRVotingCheckpoint is PLCRVotingCheckpointStorage, VotingCheckpoint {
        ...
        internal
    {
        ...
        require(_startTime >= block.difficulty, "Invalid start time");
    ...
    function changeBallotStatus(uint256 _ballotId, bool _isActive) external withPerm(ADMIN) {
        ...
        _checkIndexOutOfBound(_ballotId);
        require(
            block.difficulty <= uint256(ballots[_ballotId].startTime)
    ...
    function getCurrentBallotStage(uint256 _ballotId) public view returns (Stage) {
        ...
        if (block.difficulty < ballot.startTime)
            return Stage.PREP;
        else if (block.difficulty >= ballot.startTime && block.difficulty <= commitTimeEnd) 
            return Stage.COMMIT;
        else if ( block.difficulty > commitTimeEnd && block.difficulty <= revealTimeEnd)
            return Stage.REVEAL;
