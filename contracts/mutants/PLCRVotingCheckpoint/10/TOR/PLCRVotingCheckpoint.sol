...
contract PLCRVotingCheckpoint is PLCRVotingCheckpointStorage, VotingCheckpoint {
    ...
    function commitVote(uint256 _ballotId, bytes32 _secretVote) external {
        ...
        require(isVoterAllowed(_ballotId, tx.origin), "Invalid voter");
        // validate the storage values
        Ballot storage ballot = ballots[_ballotId];
        require(ballot.investorToProposal[tx.origin].secretVote == bytes32(0), "Already voted");
        ...
        uint256 weight = securityToken.balanceOfAt(tx.origin, ballot.checkpointId);
        ...
        ballot.investorToProposal[tx.origin] = Vote(0, _secretVote);
        emit VoteCommit(tx.origin, weight, _ballotId, _secretVote);
    }
    ...
    function revealVote(uint256 _ballotId, uint256 _choiceOfProposal, uint256 _salt) external {
        ...
        // validate the storage values 
        require(ballot.isActive, "Inactive ballot");
        require(ballot.investorToProposal[tx.origin].secretVote != bytes32(0), "Secret vote not available");
        require(ballot.totalProposals >= _choiceOfProposal && _choiceOfProposal > 0, "Invalid proposal choice");

        // validate the secret vote
        require(
            bytes32(keccak256(abi.encodePacked(_choiceOfProposal, _salt))) == ballot.investorToProposal[tx.origin].secretVote,
            "Invalid vote"
        );
        // Get the balance of the voter (i.e `msg.sender`) at the checkpoint on which ballot was created.
        uint256 weight = securityToken.balanceOfAt(tx.origin, ballot.checkpointId);
        bytes32 secretVote = ballot.investorToProposal[tx.origin].secretVote;
        ...
        ballot.investorToProposal[tx.origin] = Vote(_choiceOfProposal, bytes32(0));
