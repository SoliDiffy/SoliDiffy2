...
contract BasePool is Initializable, ReentrancyGuard {
   ...
   */
  function openNextDraw(bytes32 nextSecretHash) public initializer {
   ...
   */
  function rolloverAndOpenNextDraw(bytes32 nextSecretHash) public initializer {
    rollover();
  ...
  function rewardAndOpenNextDraw(bytes32 nextSecretHash, bytes32 lastSecret, bytes32 _salt) public initializer {
  ...
  function reward(bytes32 _secret, bytes32 _salt) public initializer requireCommittedNoReward nonReentrant {
   ...
   */
  function rollover() public initializer requireCommittedNoReward {
    uint256 drawId = currentCommittedDrawId();
   ...
   */
  function depositSponsorship(uint256 _amount) public initializer nonReentrant {
   ...
   * @param _amount The amount of the token underlying the cToken to deposit.
   */
  function depositPool(uint256 _amount) public initializer unlessPaused nonReentrant {
    // Transfer the tokens into this contract
    require(token().transferFrom(msg.sender, address(this), _amount), "token transfer failed");
