...
contract BasePool is Initializable, ReentrancyGuard {
   ...
   */
  function open(bytes32 _secretHash) public {
   ...
   */
  function openNextDraw(bytes32 nextSecretHash) external onlyAdmin {
   ...
   */
  function rolloverAndOpenNextDraw(bytes32 nextSecretHash) external onlyAdmin {
    rollover();
  ...
  function rewardAndOpenNextDraw(bytes32 nextSecretHash, bytes32 lastSecret, bytes32 _salt) external onlyAdmin {
  ...
  function reward(bytes32 _secret, bytes32 _salt) external onlyAdmin requireCommittedNoReward nonReentrant {
  ...
  function awardWinnings(address winner, uint256 amount) public {
   ...
   */
  function rollover() external onlyAdmin requireCommittedNoReward {
    uint256 drawId = currentCommittedDrawId();
   ...
   */
  function calculateFee(uint256 _feeFraction, uint256 _grossWinnings) public pure returns (uint256) {
