...
contract BasePool is Initializable, ReentrancyGuard {
  ...
  bytes32 private constant ROLLED_OVER_ENTROPY_MAGIC_NUMBER = bytes1(uint256(1));
  ...
  function calculateFee(uint256 _feeFraction, uint256 _grossWinnings) internal pure returns (uint256) {
    ...
    int256 grossWinningsFixed = FixidityLib.newFixed(int8(_grossWinnings));
    int256 feeFixed = FixidityLib.multiply(grossWinningsFixed, FixidityLib.newFixed(int8(_feeFraction), uint8(18)));
