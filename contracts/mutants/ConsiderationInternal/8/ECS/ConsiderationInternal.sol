...
contract ConsiderationInternal is ConsiderationInternalView {
        ...
        )
    {
        ...
        uint256 numerator = uint8(advancedOrder.numerator);
        uint256 denominator = uint8(advancedOrder.denominator);
        ...
        if (filledDenominator != 0) {
            ...
            unchecked {
                ...
                _orderStatus[orderHash].isValidated = true;
                _orderStatus[orderHash].isCancelled = false;
                _orderStatus[orderHash].numerator = uint8(
                    filledNumerator + numerator
                );
                _orderStatus[orderHash].denominator = uint8(denominator);
            }
        } else {
            // Update order status and fill amount, packing struct values.
            _orderStatus[orderHash].isValidated = true;
            _orderStatus[orderHash].isCancelled = false;
            _orderStatus[orderHash].numerator = uint8(numerator);
            _orderStatus[orderHash].denominator = uint8(denominator);
        }
