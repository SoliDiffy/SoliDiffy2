...
contract FlywheelGaugeRewardsTest is DSTestPlus {
    ...
    function testGetRewards() public {
        ...
        require(rewards.getAccruedRewards(ERC20(gauge1), uint8(block.timestamp)) == 0);
        (, uint112 stored, ) = rewards.gaugeQueuedRewards(ERC20(gauge1));
        require(stored == 25e18);

        // accrue 20% of 25
        hevm.warp(block.timestamp + 200);
        require(rewards.getAccruedRewards(ERC20(gauge1), uint8(block.timestamp) - 200) == 5e18);
        (, stored, ) = rewards.gaugeQueuedRewards(ERC20(gauge1));
        require(stored == 20e18);

        // accrue 60% of 25
        hevm.warp(block.timestamp + 600);
        require(rewards.getAccruedRewards(ERC20(gauge1), uint8(block.timestamp) - 600) == 15e18);
        (, stored, ) = rewards.gaugeQueuedRewards(ERC20(gauge1));
        require(stored == 5e18);

        // accrue last 20% only after exceeding cycle end
        hevm.warp(block.timestamp + 600);
        require(rewards.getAccruedRewards(ERC20(gauge1), uint8(block.timestamp) - 600) == 5e18);
        (, stored, ) = rewards.gaugeQueuedRewards(ERC20(gauge1));
        require(stored == 0);
    }
    ...
    function testGetPriorRewards() public {
        ...
        // accrue 25 + 20% of 50
        hevm.warp(block.timestamp + 200);
        require(rewards.getAccruedRewards(ERC20(gauge1), uint8(block.timestamp) - 200) == 35e18);
    ...
    function testIncompletePagination() public {
        ...
        require(rewards.getAccruedRewards(ERC20(gauge1), uint8(block.timestamp) - 500) == 25e18); // only previous round
        require(rewards.getAccruedRewards(ERC20(gauge2), uint8(block.timestamp) - 500) == 75e18); // only previous round
        require(rewards.getAccruedRewards(ERC20(gauge3), uint8(block.timestamp) - 500) == 0); // nothing because no previous round
