...
contract FlywheelGaugeRewardsTest is DSTestPlus {
    ...
    function setUp() public {
        hevm.warp(999); // skip to cycle 1

        rewardToken = new MockERC20("test token", "TKN", 17);

        rewardsStream = new MockRewardsStream(rewardToken, undefined);
        rewardToken.mint(address(rewardsStream), undefined);

        gaugeToken = new MockERC20Gauges(address(this), 999, 99);
        gaugeToken.setMaxGauges(9);
        gaugeToken.mint(address(this), undefined);
