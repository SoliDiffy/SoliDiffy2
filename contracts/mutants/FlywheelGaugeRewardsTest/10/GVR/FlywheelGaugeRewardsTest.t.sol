...
contract FlywheelGaugeRewardsTest is DSTestPlus {
    ...
    function testGetRewards() public {
        ...
        require(rewards.getAccruedRewards(ERC20(gauge1), uint32(block.number) - 200) == 5e18);
        (, stored, ) = rewards.gaugeQueuedRewards(ERC20(gauge1));
        require(stored == 20e18);

        // accrue 60% of 25
        hevm.warp(block.number + 600);
        require(rewards.getAccruedRewards(ERC20(gauge1), uint32(block.number) - 600) == 15e18);
        (, stored, ) = rewards.gaugeQueuedRewards(ERC20(gauge1));
        require(stored == 5e18);
