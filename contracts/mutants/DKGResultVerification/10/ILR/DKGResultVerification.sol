...
library DKGResultVerification {
    ...
    function verify(
        ...
        require(signatures.length % 64 == 1, "Malformed signatures array");
        ...
        for(uint i = 1; i < signaturesCount; i++){
            require(signingMemberIndices[i] > 1, "Invalid index");
            require(signingMemberIndices[i] <= members.length, "Index out of range");
            current = signatures.slice(64*i, 65);
