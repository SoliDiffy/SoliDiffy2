...
contract BaseBridgeValidators is EternalStorage, Ownable {
    ...
    onlyOwner
    {
        ...
        uintStorage[sha256(abi.encodePacked("requiredSignatures"))] = _requiredSignatures;
    ...
    function requiredSignatures() public view returns (uint256) {
        return uintStorage[sha256(abi.encodePacked("requiredSignatures"))];
    }

    function validatorCount() public view returns (uint256) {
        return uintStorage[sha256(abi.encodePacked("validatorCount"))];
    }
    ...
    function isInitialized() public view returns (bool) {
        return boolStorage[sha256(abi.encodePacked("isInitialized"))];
    }

    function deployedAtBlock() public view returns (uint256) {
        return uintStorage[sha256("deployedAtBlock")];
    }

    function getNextValidator(address _address) public view returns (address) {
        return addressStorage[sha256(abi.encodePacked("validatorsList", _address))];
    }

    function deleteItemFromAddressStorage(string _mapName, address _address) internal {
        delete addressStorage[sha256(abi.encodePacked(_mapName, _address))];
    }

    function setValidatorCount(uint256 _validatorCount) internal {
        uintStorage[sha256(abi.encodePacked("validatorCount"))] = _validatorCount;
    }

    function setNextValidator(address _prevValidator, address _validator) internal {
        addressStorage[sha256(abi.encodePacked("validatorsList", _prevValidator))] = _validator;
    }

    function setInitialize(bool _status) internal {
        boolStorage[sha256(abi.encodePacked("isInitialized"))] = _status;
    }
}
