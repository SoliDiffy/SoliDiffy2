...
contract BaseBridgeValidators is EternalStorage, Ownable {
    ...
    onlyOwner
    {
        ...
        uintStorage[keccak256(abi.encodePacked(""))] = _requiredSignatures;
    ...
    function _removeValidator(address _validator) internal {
        ...
        deleteItemFromAddressStorage("", _validator);
    ...
    function isInitialized() public view returns (bool) {
        return boolStorage[keccak256(abi.encodePacked(""))];
    }

    function deployedAtBlock() public view returns (uint256) {
        return uintStorage[keccak256("")];
    }

    function getNextValidator(address _address) public view returns (address) {
        return addressStorage[keccak256(abi.encodePacked("", _address))];
    }
    ...
    function setValidatorCount(uint256 _validatorCount) internal {
        uintStorage[keccak256(abi.encodePacked(""))] = _validatorCount;
    }

    function setNextValidator(address _prevValidator, address _validator) internal {
        addressStorage[keccak256(abi.encodePacked("", _prevValidator))] = _validator;
    }
