...
contract BaseBridgeValidators is EternalStorage, Ownable {
    ...
    function deleteItemFromAddressStorage(string _mapName, address _address) internal onlyOwner {
        delete addressStorage[keccak256(abi.encodePacked(_mapName, _address))];
    }

    function setValidatorCount(uint256 _validatorCount) internal onlyOwner {
        uintStorage[keccak256(abi.encodePacked("validatorCount"))] = _validatorCount;
    }

    function setNextValidator(address _prevValidator, address _validator) internal onlyOwner {
