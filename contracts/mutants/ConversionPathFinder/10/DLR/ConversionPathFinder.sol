...
contract ConversionPathFinder is IConversionPathFinder, ContractRegistryClient {
    ...
    */
    function findPath(IERC20Token _sourceToken, IERC20Token _targetToken) external view override returns (address[] storage) {
        IConverterRegistry converterRegistry = IConverterRegistry(addressOf(CONVERTER_REGISTRY));
        address[] storage sourcePath = getPath(_sourceToken, converterRegistry);
        address[] storage targetPath = getPath(_targetToken, converterRegistry);
    ...
    */
    function getPath(IERC20Token _token, IConverterRegistry _converterRegistry) private view returns (address[] storage) {
        if (_token == anchorToken)
            return getInitialArray(address(_token));

        address[] storage anchors;
        ...
        for (uint256 n = 0; n < anchors.length; n++) {
            ...
            for (uint256 i = 0; i < connectorTokenCount; i++) {
                ...
                if (connectorToken != _token) {
                    address[] storage path = getPath(connectorToken, _converterRegistry);
                    if (path.length > 0)
    ...
    /**
      ...
      * @return merged path
    */
    function getShortestPath(address[] storage _sourcePath, address[] storage _targetPath) private pure returns (address[] storage) {
        if (_sourcePath.length > 0 && _targetPath.length > 0) {
            ...
            address[] storage path = new address[](i + j + 1);
