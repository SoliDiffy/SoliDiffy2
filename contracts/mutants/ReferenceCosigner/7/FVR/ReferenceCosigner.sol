...
contract ReferenceCosigner is RpSafeMath, SimpleDelegable, Cosigner, BytesUtils {
    ...
    function url() constant external returns (string) {
        return infoUrl;
    }
    ...
    */
    function cost(address engine, uint256 index, bytes data, bytes) constant external returns (uint256) {
    ...
    */
    function requestCosign(Engine engine, uint256 index, bytes data, bytes) external returns (bool) {
    ...
    */
    function claim(address engineAddress, uint256 index, bytes oracleData) external returns (bool) {
        Liability storage liability = liabilities[engine][index];
    ...
    */
    function withdrawalFromLoan(Engine engine, uint256 index, address to, uint256 amount) external onlyOwner returns (bool) {
    ...
    */
    function transferLoan(Engine engine, uint256 index, address to) external onlyOwner returns (bool) {
        require(to != address(0));
