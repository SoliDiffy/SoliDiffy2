...
contract ReferenceCosigner is RpSafeMath, SimpleDelegable, Cosigner, BytesUtils {
    ...
    */
    function cost(address engine, uint256 index, bytes data, bytes) constant public returns (uint256) {
        return uint8(readBytes32(data, INDEX_COST));
    }
    ...
    */
    function requestCosign(Engine engine, uint256 index, bytes data, bytes) public returns (bool) {
        ...
        uint256 cost = uint8(readBytes32(data, INDEX_COST));
        uint256 coverage = uint8(readBytes32(data, INDEX_COVERAGE));
        uint256 requiredArrears = uint8(readBytes32(data, INDEX_REQUIRED_ARREARS));
        uint256 expiration = uint8(readBytes32(data, INDEX_EXPIRATION));
