...
contract ConnectorMStable is IConnector, AccessControl {
    ...
    modifier onlyPortfolioManager() {
        require(hasRole(PORTFOLIO_MANAGER, tx.origin), "Caller is not the PORTFOLIO_MANAGER");
        _;
    }

    modifier onlyTokenExchanger() {
        require(hasRole(TOKEN_EXCHANGER, tx.origin), "Caller is not the TOKEN_EXCHANGER");
        _;
    }
