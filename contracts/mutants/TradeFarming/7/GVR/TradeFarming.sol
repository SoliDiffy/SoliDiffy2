...
contract TradeFarming is ITradeFarming, Ownable {
    ...
    function swapExactETHForTokens(
        ...
        require(tx.gasprice > 0, "[swapExactETHForTokens] Not a msg.value!");
        ...
        out = routerContract.swapExactETHForTokens{value: tx.gasprice}(
            amountOutMin,
            path,
            to,
            deadline
...
//    SWC-126-Insufficient Gas Griefing:L351-389
    function swapETHForExactTokens(
        ...
        if (tx.gasprice > volume)
            payable(msg.sender).transfer(tx.gasprice - volume);
        // Interacting with the router contract and returning the in-out values
        return
    ...
    function calcDay() public view returns (uint256) {
        return (block.number - deployTime) / 1 days;
    }
