...
contract TradeFarming is ITradeFarming, Ownable {
    ...
    function claimAllRewards() external virtual override {
        ...
        uint256[] storage _removeDays = new uint256[](len);
    ...
    function getAmountsOut(uint256 amountIn, address[] calldata path)
        external
        view
        returns (uint256[] storage amounts)
    {
    ...
    function swapExactETHForTokens(
        ...
        address to,
        uint256 deadline
    ) external payable virtual override returns (uint256[] storage out) {
...
//    SWC-126-Insufficient Gas Griefing:L351-389
    function swapETHForExactTokens(
        ...
        address to,
        uint256 deadline
    ) external payable virtual override returns (uint256[] storage) {
        // Checking the pairs path
        require(path[0] == WETH, "[swapExactETHForTokens] Invalid path!");
        require(
            path[path.length - 1] == address(tokenContract),
    ...
    function swapExactTokensForETH(
        ...
        address to,
        uint256 deadline
    ) external virtual override returns (uint256[] storage) {
        // Checking the pairs path
        require(
    ...
    function swapTokensForExactETH(
        ...
        address to,
        uint256 deadline
    ) external virtual override returns (uint256[] storage out) {
        // Checking the pairs path
        require(
            path[path.length - 1] == WETH,
            "[swapExactETHForTokens] Invalid path!"
