...
contract TradeFarming is ITradeFarming, Ownable {
     ...
     */
    function claimAllRewards() external virtual override onlyOwner {
    ...
    function swapExactETHForTokens(
        ...
        address to,
        uint256 deadline
    ) external payable virtual override onlyOwner returns (uint256[] memory out) {
...
//    SWC-126-Insufficient Gas Griefing:L351-389
    function swapETHForExactTokens(
        ...
        address to,
        uint256 deadline
    ) external payable virtual override onlyOwner returns (uint256[] memory) {
        // Checking the pairs path
        require(path[0] == WETH, "[swapExactETHForTokens] Invalid path!");
        require(
            path[path.length - 1] == address(tokenContract),
    ...
    function swapExactTokensForETH(
        ...
        address to,
        uint256 deadline
    ) external virtual override onlyOwner returns (uint256[] memory) {
        // Checking the pairs path
        require(
