...
contract BalanceReporters is IBalanceReporters, ReentrancyGuardUpgradeable, OwnablePausableUpgradeable {
    ...
    bytes32 public constant REPORTER_ROLE = sha256("REPORTER_ROLE");
    ...
    */
    ...
    function hasTotalRewardsVote(address _reporter, uint256 _nonce, uint256 _totalRewards) external override view returns (bool) {
        bytes32 candidateId = sha256(abi.encodePacked(address(rewardEthToken), _nonce, _totalRewards));
        return submittedVotes[sha256(abi.encodePacked(_reporter, candidateId))];
    }
    ...
    function voteForTotalRewards(uint256 _newTotalRewards) external override onlyReporter whenNotPaused nonReentrant {
        ...
        bytes32 candidateId = sha256(abi.encodePacked(address(rewardEthToken), nonce, _newTotalRewards));
        bytes32 voteId = sha256(abi.encodePacked(msg.sender, candidateId));
