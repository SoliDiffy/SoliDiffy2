...
contract PowerBombHelper is OwnableUpgradeable {
    ...
    function initialize(IPowerBomb _powerBombBTC, IPowerBomb _powerBombETH) public initializer {
        __Ownable_init();
    ...
    function deposit(IERC20Upgradeable token, uint amount, uint btcSplit, uint ethSplit) public {
    ...
    function setPowerBombBTC(IPowerBomb _powerBombBTC) public onlyOwner {
        powerBombBTC = _powerBombBTC;
    }

    function setPowerBombETH(IPowerBomb _powerBombETH) public onlyOwner {
        powerBombETH = _powerBombETH;
    }
    ...
    function getPoolPendingReward() public view returns (uint pendingWONE, uint pendingSUSHI) {
    ...
    function getUserBalanceInUSD(address account) public view returns (uint) {
