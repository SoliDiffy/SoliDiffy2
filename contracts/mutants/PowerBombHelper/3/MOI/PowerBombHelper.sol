...
contract PowerBombHelper is OwnableUpgradeable {
    ...
    function deposit(IERC20Upgradeable token, uint amount, uint btcSplit, uint ethSplit) external initializer {
    ...
    function harvest() external initializer {
        powerBombBTC.harvest();
        powerBombETH.harvest();
    }

    function claimReward(address account) external initializer {
