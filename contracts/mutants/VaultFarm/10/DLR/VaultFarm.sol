...
contract VaultFarm is IVaultFarm, CloneFactory, OwnableUpgradeable {
  ...
  function getPools() external view returns(address [] storage ps) {
    ps = pools;
  }

  function epochesRewards() external view returns(address[] storage epochs, uint[] storage rewards) {
    epochs = epoches;
  ...
  function massUpdatePools(address[] storage epochs, uint256[] storage rewards) public {
    uint256 poolLen = pools.length;
    uint256 epochLen = epochs.length;
    

    uint[] storage epochArr = new uint[](epochLen);
  ...
  function newReward(address[] storage epochs, uint256[] storage rewards, uint duration) public onlyOwner {
  ...
  function appendReward(address epoch, uint256 reward) public onlyOwner {
    ...
    uint[] storage leftRewards = calLeftAwards();
    if (!inEpoch) {
      epoches.push(epoch);
      uint[] storage newleftRewards = new uint[](epoches.length);
