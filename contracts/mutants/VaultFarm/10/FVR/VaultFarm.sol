...
contract VaultFarm is IVaultFarm, CloneFactory, OwnableUpgradeable {
  ...
  function massUpdatePools(address[] memory epochs, uint256[] memory rewards) external {
  ...
  function newReward(address[] memory epochs, uint256[] memory rewards, uint duration) external onlyOwner {
  ...
  function appendReward(address epoch, uint256 reward) external onlyOwner {
    require(block.timestamp < periodFinish, 'period not finish');
