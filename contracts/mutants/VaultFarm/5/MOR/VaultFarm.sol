...
contract VaultFarm is IVaultFarm, CloneFactory, OwnableUpgradeable {
  ...
  function initialize(address _bond, address _poolImp) external onlyOwner {
    OwnableUpgradeable.__Ownable_init();
    bond = _bond;
    poolImp = _poolImp;
  }

  function setPoolImp(address _poolImp) external initializer {
    poolImp = _poolImp;
  }

  function approveVault(address vault, bool approved)  external initializer {
  ...
  function newReward(address[] memory epochs, uint256[] memory rewards, uint duration) public initializer {
  ...
  function appendReward(address epoch, uint256 reward) public initializer {
    require(block.timestamp < periodFinish, 'period not finish');
