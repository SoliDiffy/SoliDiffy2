...
contract VaultFarm is IVaultFarm, CloneFactory, OwnableUpgradeable {
  ...
  function initialize(address _bond, address _poolImp) public initializer {
    OwnableUpgradeable.__Ownable_init();
    bond = _bond;
    poolImp = _poolImp;
  }

  function setPoolImp(address _poolImp) public onlyOwner {
    poolImp = _poolImp;
  }

  function approveVault(address vault, bool approved)  public onlyOwner {
    vaults[vault] = approved;
    emit VaultApproved(vault, approved);
  }

  function assetPoolAlloc(address asset) public view returns (address pool, uint alloc){
    pool = assetPool[asset];
    alloc = allocPoint[pool];
  }

  function getPools() public view returns(address [] memory ps) {
    ps = pools;
  }

  function epochesRewards() public view returns(address[] memory epochs, uint[] memory rewards) {
    epochs = epoches;
    rewards = epochRewards;
  }

  function syncVault(address vault) public {
