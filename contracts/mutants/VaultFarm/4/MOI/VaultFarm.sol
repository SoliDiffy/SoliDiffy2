...
contract VaultFarm is IVaultFarm, CloneFactory, OwnableUpgradeable {
  ...
  function syncVault(address vault) external initializer {
  ...
  function syncDeposit(address _user, uint256 _amount, address asset) external override initializer {
  ...
  function syncWithdraw(address _user, uint256 _amount, address asset) external override initializer {
    require(vaults[msg.sender], "invalid vault");
    address pooladdr = assetPool[asset];
    if (pooladdr != address(0)) {
  ...
  function syncLiquidate(address _user, address asset) external override initializer {
    require(vaults[msg.sender], "invalid vault");
    address pooladdr = assetPool[asset];
    if (pooladdr != address(0)) {
