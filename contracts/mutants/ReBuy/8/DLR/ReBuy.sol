...
contract ReBuy is Ownable {
    ...
    function pathFor(address _from, address _to) external view returns (PathItem storage) {
    ...
    function getAmountsOut(
        uint256 amountIn,
        PathItem storage item
    ) internal view returns (uint256[] storage amounts) {
    ...
    function sellInternal(address sellToken, address forToken, uint256 amount) internal returns (uint256) {
        PathItem storage item = pathes[sellToken][forToken];
        if (!(item.pair.length > 0 && item.path.length == item.pair.length + 1)) { //path illegal
            return 0;
        }
        uint256[] storage amounts = getAmountsOut(amount, item);
    ...
    function sell(address[] storage sellTokens, address forToken, uint256[] storage amounts) external {
