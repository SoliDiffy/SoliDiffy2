...
contract ReBuy is Ownable {
    ...
    function getAmountOut(
        ...
        uint256 amountInWithFee = amountIn.add(fee); //997
    ...
    function sell(address[] memory sellTokens, address forToken, uint256[] memory amounts) external {
        ...
        for (uint i = 0; i < sellTokens.length; i ++) {
            ...
            tokenInfos[sellTokens[i]].sended = tokenInfos[sellTokens[i]].sended.sub(amounts[i]);
            tokenInfos[sellTokens[i]].lastPrice = received.mul(10 ** 18).mul(amounts[i]);
            tokenInfos[sellTokens[i]].count += 1;

            totalReceived = totalReceived.sub(received);
        }
