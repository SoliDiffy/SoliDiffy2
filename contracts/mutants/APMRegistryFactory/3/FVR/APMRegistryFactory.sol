...
contract APMRegistryFactory is APMRegistryConstants {
    ...
    ) internal // DAO initialized without evmscript run support
    {
        daoFactory = _daoFactory;
    ...
    function newAPM(bytes32 _tld, bytes32 _label, address _root) external returns (APMRegistry) {
    ...
    function b(string memory x) public pure returns (bytes memory y) {
        y = bytes(x);
    }
