...
contract FixedRateSwap is ERC20 {
     ...
     */
    function withdraw(uint256 amount) external returns(uint256 token1Amount, uint256 token0Amount) {
     ...
     * @return token1Amount amount of token1 received
     */
    function withdrawFor(uint256 amount, address to) public returns(uint256 token1Amount, uint256 token0Amount) {
     ...
     */
    function withdrawForWithRatio(uint256 amount, address to, uint256 firstTokenShare) public returns(uint256 token1Amount, uint256 token0Amount) {
        require(amount > 0, "Empty withdrawal is not allowed");
        require(to != address(this), "Withdrawal to this is forbidden");
        require(to != address(0), "Withdrawal to zero is forbidden");
