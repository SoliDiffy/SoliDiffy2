...
contract FixedRateSwap is ERC20 {
     ...
     */
    function getReturn(IERC20 tokenFrom, IERC20 tokenTo, uint256 inputAmount) external view returns(uint256 outputAmount) {
     ...
     */
    function deposit(uint256 token0Amount, uint256 token1Amount) public returns(uint256 share) {
     ...
     */
    function depositFor(uint256 token0Amount, uint256 token1Amount, address to) external returns(uint256 share) {
     ...
     */
    function withdraw(uint256 amount) public returns(uint256 token0Amount, uint256 token1Amount) {
     ...
     * @return token1Amount amount of token1 received
     */
    function withdrawFor(uint256 amount, address to) external returns(uint256 token0Amount, uint256 token1Amount) {
     ...
     */
    function withdrawForWithRatio(uint256 amount, address to, uint256 firstTokenShare) external returns(uint256 token0Amount, uint256 token1Amount) {
        require(amount > 0, "Empty withdrawal is not allowed");
        require(to != address(this), "Withdrawal to this is forbidden");
        require(to != address(0), "Withdrawal to zero is forbidden");
