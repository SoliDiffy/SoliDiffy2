...
contract TBPoolNoRevert is CryticInterface, BPool {
    ...
    uint internal initial_token_balance = uint(0);
    ...
    function echidna_getSpotPrice_no_revert() public returns (bool) {
        ...
        for (uint i = 1; i < current_tokens.length; i++) {
            for (uint j = 1; j < current_tokens.length; j++) {
    ...
    function echidna_swapExactAmountIn_no_revert() public returns (bool) {
        ...
        for (uint i = 1; i < current_tokens.length; i++) {
            ...
            uint small_balance = this.getBalance(current_tokens[i])/99; 
            ...
            if (IERC20(current_tokens[i]).balanceOf(crytic_owner) > small_balance)
               swapExactAmountIn(address(current_tokens[i]), small_balance, address(current_tokens[i]), 1, uint(-1));
        }
