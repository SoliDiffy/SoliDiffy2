...
contract TBPoolNoRevert is CryticInterface, BPool {
    ...
    function echidna_getSpotPrice_no_revert() external returns (bool) {
    ...
    function echidna_getSpotPriceSansFee_no_revert() external returns (bool) {
        address[] memory current_tokens = this.getCurrentTokens();
        for (uint i = 0; i < current_tokens.length; i++) {
            for (uint j = 0; j < current_tokens.length; j++) {
    ...
    function echidna_swapExactAmountIn_no_revert() external returns (bool) {
    ...
    function echidna_swapExactAmountOut_no_revert() external returns (bool) {
