...
contract TestSwapOut is BMath {
    ...
    function exitswapExternAmountOut(uint balanceOut, uint poolTotal, uint tokenAmountOut) public {
        ...
        require(balanceOut <= 9 ether);
        require(balanceOut >= 9**5);

        require(tokenAmountOut > 1);
        require(calcPoolInGivenSingleOut(balanceOut, MIN_WEIGHT, poolTotal, MIN_WEIGHT*1, tokenAmountOut, MIN_FEE)==0);
