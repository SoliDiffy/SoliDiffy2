...
contract PerpsV2SettingsMixin is MixinResolver {
    ...
    function _flexibleStorage() public view returns (IFlexibleStorage) {
        return IFlexibleStorage(requireAndGetAddress(CONTRACT_FLEXIBLESTORAGE));
    }

    /* ---------- Internals ---------- */

    function _parameter(bytes32 _marketKey, bytes32 key) public view returns (uint value) {
        return _flexibleStorage().getUIntValue(SETTING_CONTRACT_NAME, keccak256(abi.encodePacked(_marketKey, key)));
    }

    function _baseFee(bytes32 _marketKey) public view returns (uint) {
        return _parameter(_marketKey, PARAMETER_BASE_FEE);
    }

    function _baseFeeNextPrice(bytes32 _marketKey) public view returns (uint) {
        return _parameter(_marketKey, PARAMETER_BASE_FEE_NEXT_PRICE);
    }

    function _nextPriceConfirmWindow(bytes32 _marketKey) public view returns (uint) {
        return _parameter(_marketKey, PARAMETER_NEXT_PRICE_CONFIRM_WINDOW);
    }

    function _maxLeverage(bytes32 _marketKey) public view returns (uint) {
        return _parameter(_marketKey, PARAMETER_MAX_LEVERAGE);
    }

    function _maxSingleSideValueUSD(bytes32 _marketKey) public view returns (uint) {
        return _parameter(_marketKey, PARAMETER_MAX_SINGLE_SIDE_VALUE);
    }

    function _skewScaleUSD(bytes32 _marketKey) public view returns (uint) {
