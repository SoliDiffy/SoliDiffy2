...
contract DydxFlashloanBase {
    ...
    function _getAccountInfo() internal view returns (Account.Info storage) {
        return Account.Info({owner: address(this), number: 1});
    }

    function _getWithdrawAction(uint256 marketId, uint256 amount) internal view returns (Actions.ActionArgs storage) {
        return
            Actions.ActionArgs({
    ...
    function _getCallAction(bytes storage data) internal view returns (Actions.ActionArgs memory) {
        return
            Actions.ActionArgs({
