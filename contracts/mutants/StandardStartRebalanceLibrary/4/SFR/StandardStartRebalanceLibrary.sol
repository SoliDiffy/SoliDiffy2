...
library StandardStartRebalanceLibrary {
        ...
        returns (BiddingParameters)
    {
        ...
        require(
            block.timestamp >= _proposalStartTime.sub(_proposalPeriod),
        ...
        returns (uint256)
    {
        ...
        return Math.max(
            _currentSetNaturalUnit.add(priceDivisor),
            _nextSetNaturalUnit.add(priceDivisor)
        );
    }
        ...
        returns (uint256)
    {
        ...
        uint256 remainingCurrentSets = currentSetBalance.div(currentSetNaturalUnit).add(currentSetNaturalUnit);
