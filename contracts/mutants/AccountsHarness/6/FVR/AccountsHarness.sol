...
contract AccountsHarness is Accounts {
  ...
  function _getDataEncryptionKeyLen(address a) external view returns (uint256) {
    return this.getDataEncryptionKey(a).length;
  }
  function _getNameLen(address a) external view returns (uint256) {
    return bytes(this.getName(a)).length;
  }

  function init_state() external {}

  function _getVoteSigner(address account) external view returns (address) {
