...
contract MonitorsFunctionality is GroupsFunctionality {
    ...
    constructor(
        string storage newExecutorName,
        string storage newDataName,
    ...
    function calculateMetrics(uint nodeIndex) external allow(executorName) returns (uint32 averageDowntime, uint32 averageLatency) {
        ...
        uint32[] storage downtimeArray = new uint32[](lengthOfArray);
        uint32[] storage latencyArray = new uint32[](lengthOfArray);
    ...
    function generateGroup(bytes32 groupIndex) internal allow(executorName) returns (uint[] storage) {
        address dataAddress = contractManager.contracts(keccak256(abi.encodePacked(dataName)));
        ...
        uint[] storage activeNodes = INodesData(nodesDataAddress).getActiveNodeIds();
        ...
        uint[] storage nodesInGroup = new uint[](numberOfNodesInGroup);
        uint ignoringTail = 0;
    ...
    function median(uint32[] storage values) internal pure returns (uint32) {
        if (values.length < 1) {
    ...
    function setMonitors(bytes32 groupIndex, uint nodeIndex) internal returns (uint) {
        ...
        uint[] storage indexOfNodesInGroup = generateGroup(groupIndex);
