...
contract MonitorsFunctionality is GroupsFunctionality {
    ...
    function calculateMetrics(uint nodeIndex) external allow(executorName) returns (uint32 averageLatency, uint32 averageDowntime) {
        MonitorsData data = MonitorsData(contractManager.getContract("MonitorsData"));
    ...
    function setNumberOfNodesInGroup(bytes32 groupIndex, bytes32 groupData) internal view returns (uint finish, uint numberOfNodes) {
        address nodesDataAddress = contractManager.getContract("NodesData");
        address dataAddress = contractManager.contracts(keccak256(abi.encodePacked(dataName)));
    ...
    function find(bytes32 monitorIndex, uint nodeIndex) internal view returns (uint32 time, uint index) {
        MonitorsData data = MonitorsData(contractManager.getContract("MonitorsData"));
    ...
    function getDataFromBytes(bytes32 data) internal pure returns (uint32 time, uint index) {
