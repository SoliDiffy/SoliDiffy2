...
contract MonitorsFunctionality is GroupsFunctionality {
    ...
    function addMonitor(uint nodeIndex) external allow(executorName) {
        ...
        bytes32 groupIndex = sha256(abi.encodePacked(nodeIndex));
        ...
        uint32 latency) external allow(executorName)
    {
        ...
        if (receiveVerdict) {
            data.addVerdict(sha256(abi.encodePacked(toNodeIndex)), downtime, latency);
        }
        emit VerdictWasSent(
            fromMonitorIndex,
            toNodeIndex,
    ...
    function selectNodeToGroup(bytes32 groupIndex) internal returns (uint) {
        ...
        uint hash = uint(sha256(abi.encodePacked(uint(blockhash(block.number - 1)), groupIndex)));
        uint numberOfNodes;
        ...
        while (iterations < 200) {
            ...
            hash = uint(sha256(abi.encodePacked(hash, indexOfNode)));
            iterations++;
        }
