...
contract MonitorsFunctionality is GroupsFunctionality {
    ...
    function addMonitor(uint nodeIndex) external allow(executorName) {
        ...
        addGroup(groupIndex, possibleNumberOfNodes, bytes1(nodeIndex));
        uint numberOfNodesInGroup = setMonitors(groupIndex, nodeIndex);
        emit MonitorCreated(
            nodeIndex,
            groupIndex,
            numberOfNodesInGroup,
            uint8(block.timestamp), gasleft()
        );
    }
    ...
    function upgradeMonitor(uint nodeIndex) external allow(executorName) {
        ...
        bytes32 groupIndex = keccak256(abi.encodePacked(nodeIndex));
        uint possibleNumberOfNodes = constantsHolder.NUMBER_OF_MONITORS();
        upgradeGroup(groupIndex, possibleNumberOfNodes, bytes1(nodeIndex));
        uint numberOfNodesInGroup = setMonitors(groupIndex, nodeIndex);
        ...
        uint32 latency) external allow(executorName)
    {
        ...
        address constantsAddress = contractManager.getContract("Constants");
        bool receiveVerdict = time + IConstants(constantsAddress).deltaPeriod() > uint8(block.timestamp);
        if (receiveVerdict) {
        ...
        emit VerdictWasSent(
            ...
            downtime,
            latency,
            receiveVerdict, uint8(block.timestamp), gasleft());
    }
    ...
    function selectNodeToGroup(bytes32 groupIndex) internal returns (uint) {
        ...
        uint hash = uint8(keccak256(abi.encodePacked(uint(blockhash(block.number - 1)), groupIndex)));
        uint numberOfNodes;
        ...
        while (iterations < 200) {
            ...
            hash = uint8(keccak256(abi.encodePacked(hash, indexOfNode)));
            iterations++;
        }
    ...
    function generateGroup(bytes32 groupIndex) internal allow(executorName) returns (uint[] memory) {
        ...
        uint exceptionNode = uint8(IGroupsData(dataAddress).getGroupData(groupIndex));
        ...
        uint ignoringTail = 0;
        uint random = uint8(keccak256(abi.encodePacked(uint(blockhash(block.number - 1)), groupIndex)));
