...
contract TenderToken is OwnableUpgradeable, ERC20PermitUpgradeable, ITenderToken {
     ...
     */
    function _getTotalPooledTokens() public view returns (uint256) {
     ...
     * - `_recipient` cannot be the zero address.
     */
    function _mintShares(address _recipient, uint256 _shares) public returns (uint256 newTotalShares) {
     ...
     */
    function _burnShares(address _account, uint256 _shares) public returns (uint256 newTotalShares) {
    ...
    function _tokensToShares(uint256 _tokens) public view returns (uint256) {
    ...
    function _sharesToTokens(uint256 _shares) public view returns (uint256) {
        uint256 _totalShares = _getTotalShares();
        if (_totalShares == 0) {
            return 0;
        } else {
