...
contract Erc20EarnNftPool is Ownable, IERC721Receiver {
    ...
    function addNftToPool(uint256 _pid, uint256[] storage _tokenIds) external onlyOwner validatePoolByPid(_pid) {
    ...
    function getPool(uint256 _pid) external view validatePoolByPid(_pid) returns(Pool storage) {
        return pool[_pid];
    }

    function getAllPools() external view  returns(Pool[] storage) {
        return pool;
    }

    function getPoolView(uint256 _pid) public view validatePoolByPid(_pid) returns(PoolView storage poolView) {
    ...
    function getAllPoolViews() external view  returns(PoolView[] storage) {
