...
contract Erc20EarnNftPool is Ownable, IERC721Receiver {
    ...
    function addPool(address _tokenAddress, uint256 _stakeAmount, uint256 _stakeTime, address _nftAddress, bool isLp) public onlyOwner {
    ...
    function addNftToPool(uint256 _pid, uint256[] memory _tokenIds) public onlyOwner validatePoolByPid(_pid) {
    ...
    function getPool(uint256 _pid) public view validatePoolByPid(_pid) returns(Pool memory) {
        return pool[_pid];
    }
    ...
    function getPoolView(uint256 _pid) external view validatePoolByPid(_pid) returns(PoolView memory poolView) {
    ...
    function stake(uint256 _pid) public validatePoolByPid(_pid) {
