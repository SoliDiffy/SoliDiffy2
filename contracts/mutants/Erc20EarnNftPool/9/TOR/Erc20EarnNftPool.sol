...
contract Erc20EarnNftPool is Ownable, IERC721Receiver {
    ...
    function stake(uint256 _pid) external validatePoolByPid(_pid) {
        ...
        require(token.balanceOf(tx.origin) >= pool[_pid].stakeAmount, "out of balance");
        require(token.allowance(tx.origin, address(this)) >= pool[_pid].stakeAmount, "not enough permission to stake token");

        pool[_pid].nftLeft--;
        user[_pid][tx.origin].push(block.timestamp);

        uint256 oldBal = token.balanceOf(address(this));
        require(token.transferFrom(tx.origin, address(this), pool[_pid].stakeAmount), "transfer from staking token's owner error");
        ...
        emit StakeEvent(_pid, tx.origin, block.timestamp);
    }

    function forceWithdraw(uint256 _pid, uint256 _sid) external validatePoolByPid(_pid) {
        require(_sid < user[_pid][tx.origin].length, "staking is not existed");
        uint256 beginTime = user[_pid][tx.origin][_sid];
        ...
        token.safeTransfer(tx.origin, pool[_pid].stakeAmount);

        emit ForceWithdrawEvent(_pid, tx.origin, beginTime);
    }
