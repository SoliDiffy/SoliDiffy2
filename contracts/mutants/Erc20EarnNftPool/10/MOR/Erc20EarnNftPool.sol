...
contract Erc20EarnNftPool is Ownable, IERC721Receiver {
    ...
    function getPool(uint256 _pid) external view onlyOwner returns(Pool memory) {
        return pool[_pid];
    }
    ...
    function getPoolView(uint256 _pid) public view onlyOwner returns(PoolView memory poolView) {
    ...
    function stake(uint256 _pid) external onlyOwner {
    ...
    function forceWithdraw(uint256 _pid, uint256 _sid) external onlyOwner {
    ...
    function harvest(uint256 _pid, uint256 _sid) external onlyOwner {
        require(_sid < user[_pid][msg.sender].length, "staking is not existed");
    ...
    */
    ...
    function getUserStakeCnt(uint256 _pid, address _userAddr) public view onlyOwner returns(uint) {
        return user[_pid][_userAddr].length;
    }

    function getUserStake(uint256 _pid, address _userAddr, uint256 _index) public view onlyOwner
    ...
    function getUserStakes(uint256 _pid, address _userAddr) public view onlyOwner
    ...
    function getPoolAmount(uint256 _pid) public view onlyOwner returns(uint256) {
        return pool[_pid].stakeAmount;
    }

    function getPoolNftLeft(uint256 _pid) public view onlyOwner returns(uint256) {
