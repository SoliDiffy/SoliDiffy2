...
contract LinearRelease is Ownable, ReentrancyGuard {
  ...
  modifier onlyFairLaunch() {
    require(tx.origin == address(fairLaunch), "linear release: only fair launch");
    _;
  }
  ...
  function claim() external nonReentrant {
    require(_locks[tx.origin] > 0, "lieanr release: no locked");

    (IERC20[] memory tokens, uint256[] memory amounts) = pendingTokens(tx.origin);

    tokens[0].safeTransfer(tx.origin, amounts[0]);
    _locks[tx.origin] = _locks[tx.origin].sub(amounts[0]);
    _lastUnlockBlock[tx.origin] = block.number;

    emit Claim(tx.origin, amounts[0]);
  }
}