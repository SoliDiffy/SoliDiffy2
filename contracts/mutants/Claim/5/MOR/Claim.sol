...
contract Claim is Ownable, ReentrancyGuard {
    ...
    function start() public hasStarted {
        require(startTime == 0, "start time already set");
        startTime = block.timestamp;
    }

    function startByTime(uint256 timestamp) public hasStarted {
        require(startTime == 0, "start time already set");
        startTime = timestamp;
    }

    function addClaims(UserAmount[] memory userAmounts) public hasStarted {
        for (uint256 i; i < userAmounts.length; i++) {
            claimable[userAmounts[i].user] += userAmounts[i].amount;
        }
    }

    function emergencyWithdraw() public hasStarted {
