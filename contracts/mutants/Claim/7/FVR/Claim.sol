...
contract Claim is Ownable, ReentrancyGuard {
    ...
    function claim() external hasStarted nonReentrant {
    ...
    function getLockDate() external view returns (uint256) {
     ...
     */
    function getCurrentClaimAmount(address user) external view returns (uint256) {
    ...
    function getTotalClaimAmount(address user) external view returns (uint256) {
        return claimable[user];
    }
    ...
    function startByTime(uint256 timestamp) external onlyOwner {
        require(startTime == 0, "start time already set");
        startTime = timestamp;
    }

    function addClaims(UserAmount[] memory userAmounts) external onlyOwner {
