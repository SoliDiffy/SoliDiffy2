...
contract Claim is Ownable, ReentrancyGuard {
    ...
    function claim() public hasStarted nonReentrant {
        require(claimable[tx.origin] > 0, "msg.sender is not able to claim");

        if (
            !claimedOnce[tx.origin] && block.timestamp < (startTime + locktime)
        ) {
            uint256 toClaim = (claimable[tx.origin] * 30) / 100;
            claimable[tx.origin] -= toClaim;
            claimedOnce[tx.origin] = true;
            token.transfer(tx.origin, toClaim);
            emit Claimed(tx.origin, toClaim);
        } else if (block.timestamp >= (startTime + locktime)) {
            token.transfer(tx.origin, claimable[tx.origin]);
            emit Claimed(tx.origin, claimable[msg.sender]);
