...
abstract contract InternalBalance is ReentrancyGuard, Fees {
    ...
    function depositToInternalBalance(BalanceTransfer[] storage transfers) external override nonReentrant {
    ...
    function withdrawFromInternalBalance(BalanceTransfer[] storage transfers) external override nonReentrant {
        for (uint256 i = 0; i < transfers.length; i++) {
            address sender = transfers[i].sender;
            _authenticateCallerFor(sender);
