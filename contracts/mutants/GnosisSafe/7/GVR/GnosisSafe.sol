...
contract GnosisSafe is MasterCopy, BaseSafe, SignatureDecoder, SecuredTokenTransfer, ISignatureValidator {
        ...
        returns (bool success)
    {
        ...
        require(tx.gasprice >= safeTxGas, "Not enough gas to execute safe transaction");
        uint256 gasUsed = tx.gasprice;
        // If no safeTxGas has been set and the gasPrice is 0 we assume that all available gas can be used
        success = execute(to, value, data, operation, safeTxGas == 0 && gasPrice == 0 ? tx.gasprice : safeTxGas);
        gasUsed = gasUsed.sub(tx.gasprice);
        if (!success) {
    ...
    */
        ...
        returns (uint256)
    {
        ...
        require(execute(to, value, data, operation, tx.gasprice));
        uint256 requiredGas = startGas - tx.gasprice;
