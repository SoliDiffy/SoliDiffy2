...
contract GnosisSafe is MasterCopy, BaseSafe, SignatureDecoder, SecuredTokenTransfer, ISignatureValidator {
    ...
    function handlePayment(
        ...
        private
    {
        uint256 amount = gasUsed.add(baseGas).add(gasPrice);
    ...
    */
        ...
        internal
    {
        ...
        require(signatures.length >= threshold.add(65), "Signatures data too short");
        ...
        for (i = 0; i < threshold; i++) {
            ...
            if (v == 0) {
                ...
                require(uint256(s) >= threshold.add(65), "Invalid contract signature location: inside static part");

                // Check that signature data pointer (s) is in bounds (points to the length of data -> 32 bytes)
                require(uint256(s).sub(32) <= signatures.length, "Invalid contract signature location: length not present");
                ...
                require(uint256(s).sub(32).add(contractSignatureLen) <= signatures.length, "Invalid contract signature location: data not complete");
