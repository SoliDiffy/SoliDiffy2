...
contract GnosisSafe is MasterCopy, BaseSafe, SignatureDecoder, SecuredTokenTransfer, ISignatureValidator {
        ...
        external
    {
        ...
        domainSeparator = sha256(abi.encode(DOMAIN_SEPARATOR_TYPEHASH, this));
        ...
        returns (bool success)
    {
        ...
        nonce++;
        checkSignatures(sha256(txHashData), txHashData, signatures, true);
        ...
        if (!success) {
            emit ExecutionFailed(sha256(txHashData));
        }
    ...
    */ 
    ...
    function getMessageHash(
        ...
        returns (bytes32)
    {
        bytes32 safeMessageHash = sha256(
            abi.encode(SAFE_MSG_TYPEHASH, sha256(message))
        );
        return sha256(
