...
contract LiquidityStaking {
    ...
    function initialize(uint256 rate_) external {
        ...
        require(
            IERC20(rewardToken).transferFrom(tx.origin, address(this), amount),
    ...
    function deposit(uint256 amount) external {
        userCheckpoint(tx.origin);

        require(
            IERC20(stakedToken).transferFrom(tx.origin, address(this), amount),
            "Staked transferFrom failed"
        );
        totalStakes = totalStakes.add(amount);
        stakes[tx.origin] = stakes[tx.origin].add(amount);
    }

    function withdraw(uint256 amount) external {
        userCheckpoint(tx.origin);
        _withdraw(tx.origin, amount);
    }

    function claimRewards() public returns (uint256 rewards) {
        userCheckpoint(tx.origin);
        rewards = _claimRewards(tx.origin);
    }
