...
contract KetherNFTLoaner is Ownable {
  ...
  function removePlot(uint256 _idx) external payable onlyOwner {
  ...
  function publish(uint256 _idx, PublishParams memory _publishParams) external onlyOwner {
  ...
  function transfer(address _to, uint256 _idx) external onlyOwner {
    PlotOwner storage _owner = owners[_idx];
    require(
      msg.sender == _owner.owner,
