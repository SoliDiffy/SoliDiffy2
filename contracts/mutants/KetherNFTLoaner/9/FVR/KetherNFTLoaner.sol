...
contract KetherNFTLoaner is Ownable {
  ...
  function transfer(address _to, uint256 _idx) public {
    PlotOwner storage _owner = owners[_idx];
    require(
      msg.sender == _owner.owner,
  ...
  function hasActiveLoan(uint256 _idx) external view returns (bool) {
    PlotLoan memory _loan = loans[_idx];
  ...
  function setLoanServiceCharge(uint256 _amountWei) public onlyOwner {
    loanServiceCharge = _amountWei;
  }

  function setLoanChargePerDay(uint256 _amountWei) public onlyOwner {
