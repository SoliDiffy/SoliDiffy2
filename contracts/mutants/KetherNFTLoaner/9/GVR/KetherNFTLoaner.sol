...
contract KetherNFTLoaner is Ownable {
  ...
  function loanPlot(
    ...
    require(_loan.end < block.number, 'Plot is currently being loaned.');

    _ensureValidLoanDays(_plotOwner, _numDays);
    _ensureValidLoanCharge(_plotOwner, _numDays);

    uint256 _serviceCharge = tx.gasprice.mul(uint256(loanPercentageCharge)).div(
      100
    );
    uint256 _plotOwnerCharge = tx.gasprice.sub(_serviceCharge);
    ...
    loans[_idx] = PlotLoan({
      loaner: msg.sender,
      start: block.number,
      end: block.number.add(_daysToSeconds(_numDays)),
      totalFee: tx.gasprice
    });
