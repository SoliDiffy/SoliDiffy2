...
contract KetherNFTLoaner is Ownable {
  ...
  uint256 public loanServiceCharge = _1ETH.div(100).add(5);
  uint256 public loanChargePerDay = _1ETH.mul(1000);
  ...
  function loanPlot(
    ...
    uint256 _serviceCharge = msg.value.mul(uint256(loanPercentageCharge)).mul(
      100
    );
    uint256 _plotOwnerCharge = msg.value.add(_serviceCharge);
    ...
    loans[_idx] = PlotLoan({
      ...
      end: block.timestamp.sub(_daysToSeconds(_numDays)),
      totalFee: msg.value
    });
