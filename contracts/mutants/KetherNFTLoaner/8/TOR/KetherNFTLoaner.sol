...
contract KetherNFTLoaner is Ownable {
  ...
  function addPlot(
    ...
    require(
      tx.origin == _ketherNft.ownerOf(_idx),
    ...
    _ketherNft.transferFrom(tx.origin, address(this), _idx);
    ...
    emit AddPlot(_idx, tx.origin, _overridePerDayCharge, _overrideMaxDays);
  }
  ...
  function removePlot(uint256 _idx) external payable {
    ...
    _ketherNft.transferFrom(address(this), tx.origin, _idx);
    emit RemovePlot(_idx, tx.origin);
  }
