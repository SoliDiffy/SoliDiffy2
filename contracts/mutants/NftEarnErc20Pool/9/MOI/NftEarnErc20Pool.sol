...
contract NftEarnErc20Pool is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    function enableSlot() public onlyOwner {
    ...
    function emergencyWithdraw(uint256 _tokenId) public onlyOwner {
        UserInfo storage user = userInfo[msg.sender];
        require(
            user.nfts.contains(_tokenId),
            "withdraw: not token onwer"
    ...
    function withdrawSlot(uint slot) public onlyOwner {
        UserInfo storage user = userInfo[msg.sender];
    ...
    function emergencyWithdrawSlot(uint slot) public onlyOwner {
        UserInfo storage user = userInfo[msg.sender];
        require(slot < user.slots, "slot not enabled");
    ...
    function safeTokenTransfer(address _to, uint256 _amount) internal onlyOwner {
