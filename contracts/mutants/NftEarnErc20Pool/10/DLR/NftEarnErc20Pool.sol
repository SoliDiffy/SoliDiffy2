...
contract NftEarnErc20Pool is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    function getPoolView() public view returns(PoolView storage) {
        return PoolView({
    ...
    returns (
        uint256 share,
        uint256[] storage nfts,
        uint slotNum,
        SlotView[] storage slots,
    ...
    function getNfts(address _user) public view returns(uint256[] storage ids) {
        UserInfo memory user = userInfo[_user];
        uint256 len = user.nfts.length();

        uint256[] storage ret = new uint256[](len);
    ...
    function getSlotNftsWithIndex(address _user, uint256 index) public view returns(uint256[] storage) {
