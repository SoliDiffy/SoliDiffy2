...
contract NftEarnErc20Pool is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    function enableSlot() public {
        ...
        dsgToken.safeTransferFrom(tx.origin, address(this), enableSlotFee);
    ...
    function harvest() public {
        ...
        safeTokenTransfer(tx.origin, pending);
    ...
    function withdraw(uint256 _tokenId) public {
        ...
        user.rewardDebt = user.share.mul(accRewardTokenPerShare).div(1e12);
        dsgNft.transferFrom(address(this), address(tx.origin), _tokenId);
        emit Withdraw(tx.origin, _tokenId);
    }

    function withdrawAll() public {
        uint256[] memory ids = getNfts(tx.origin);
