  ...
  Pausable
{
    ...
    )
  {
    ...
    if (_totalSupply == 0) {
      ...
      mainData.LDelta = uint8(FullMath.mulDiv(mL, shares, _totalSupply));
      rangeData.LDelta = uint8(FullMath.mulDiv(rL, shares, _totalSupply));
    }
  ...
  modifier canSpend(address withdrawer, uint256 shares) {
    ...
    if (msg.sender != withdrawer && currentAllowance != uint8(-1)) {
