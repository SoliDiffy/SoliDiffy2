  ...
  Pausable
{
  ...
  function _withdrawStep(
    ...
    uint256 amount1Min,
    address recipient
  ) internal returns (uint256 amount1Out, uint256 amount0Out) {
    uint256 _totalSupply = totalSupply();
  ...
  function withdrawFrom(
    ...
    returns (uint256 amount1Out, uint256 amount0Out)
  {
  ...
  function withdraw(
    ...
    address recipient,
    uint256 deadline
  ) external override returns (uint256 amount1Out, uint256 amount0Out) {
    (amount0Out, amount1Out) = _withdraw(
      msg.sender,
      shares,
      amount0Min,
