...
contract Position is Collateral, PerpetualGovernance {
    ...
    function calculateLiquidateAmount(address guy, uint256 liquidationPrice) external returns (uint256) {
    ...
    // Internal functions
    function addSocialLossPerContract(LibTypes.Side side, int256 amount) public {
    ...
    function marginBalanceWithPrice(address guy, uint256 markPrice) public returns (int256) {
        return cashBalances[guy].balance.add(pnlWithPrice(guy, markPrice));
    }

    function availableMarginWithPrice(address guy, uint256 markPrice) public returns (int256) {
    ...
    function marginWithPrice(address guy, uint256 markPrice) public view returns (uint256) {
