...
contract Position is Collateral, PerpetualGovernance {
    ...
    function getPosition(address guy) public view returns (LibTypes.PositionAccount storage) {
    ...
    function socialLoss(LibTypes.PositionAccount storage account) internal view returns (int256) {
        return socialLossWithAmount(account, account.size);
    }

    function socialLossWithAmount(LibTypes.PositionAccount storage account, uint256 amount)
        internal
        view
        returns (int256)
    {
    ...
    function fundingLoss(LibTypes.PositionAccount storage account) internal returns (int256) {
        return fundingLossWithAmount(account, account.size);
    }

    function fundingLossWithAmount(LibTypes.PositionAccount storage account, uint256 amount) internal returns (int256) {
    ...
    function calculatePnl(LibTypes.PositionAccount storage account, uint256 tradePrice, uint256 amount)
        internal
        returns (int256)
    {
        if (account.size == 0) {
