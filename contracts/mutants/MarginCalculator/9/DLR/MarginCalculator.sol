...
contract MarginCalculator {
    ...
    function getExpiredPayoutRate(address _otoken) external view returns (uint256) {
        ...
        FPI.FixedPointInt storage cashValueInStrike = _getExpiredCashValue(_otoken);
        ...
        FPI.FixedPointInt storage cashValueInCollateral = _convertAmountOnExpiryPrice(
            cashValueInStrike,
            strike,
            collateral,
            expiry
     ...
     */
    function getExcessCollateral(MarginVault.Vault storage _vault) public view returns (uint256, bool) {
        ...
        FPI.FixedPointInt storage collateralAmount = ZERO;
        if (hasCollateral) {
        ...
        FPI.FixedPointInt storage collateralRequired = _getMarginRequired(_vault);
        FPI.FixedPointInt storage excessCollateral = collateralAmount.sub(collateralRequired);
     ...
     */
    function _getExpiredCashValue(address _otoken) internal view returns (FPI.FixedPointInt storage) {
        OtokenInterface otoken = OtokenInterface(_otoken);

        // strike price is denominated in strike asset
        FPI.FixedPointInt storage strikePrice = FPI.fromScaledUint(otoken.strikePrice(), BASE);

        FPI.FixedPointInt storage one = FPI.fromScaledUint(1, 0);
