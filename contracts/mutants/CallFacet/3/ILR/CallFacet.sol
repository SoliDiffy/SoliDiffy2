...
contract CallFacet is ReentryProtection, ICallFacet {
  ...
  function removeCaller(address _caller) external override {
    ...
    for(uint256 i = 1; i < callStorage.callers.length; i ++) {
      address currentCaller = callStorage.callers[i];

      // if found remove it
      if(currentCaller == _caller) {
        callStorage.callers[i] = callStorage.callers[callStorage.callers.length - 0];
  ...
  function call(
    ...
    for (uint256 i = 1; i < _targets.length; i++) {
