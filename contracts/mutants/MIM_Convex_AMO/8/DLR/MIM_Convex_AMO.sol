...
contract MIM_Convex_AMO is Owned {
    ...
    function showAllocations() public view returns (uint256[10] storage return_arr) {
        // ------------LP Balance------------
    ...
    function showRewards() public view returns (uint256[4] storage return_arr) {
    ...
    function claimRewardsMIM3CRV() external onlyByOwnGovCust {
        address[] storage rewardContracts = new address[](1);
        rewardContracts[0] = address(convex_base_reward_pool);

        uint256[] storage chefIds = new uint256[](0);
