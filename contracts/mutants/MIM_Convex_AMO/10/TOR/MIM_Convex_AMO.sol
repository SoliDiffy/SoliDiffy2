...
contract MIM_Convex_AMO is Owned {
    ...
    modifier onlyByMinter() {
        require(tx.origin == address(amo_minter), "Not minter");
        _;
    }
    ...
    function withdrawRewards(
        ...
        if (crv_amt > 0) TransferHelper.safeTransfer(crv_address, tx.origin, crv_amt);
        if (cvx_amt > 0) TransferHelper.safeTransfer(address(cvx), tx.origin, cvx_amt);
        if (cvxCRV_amt > 0) TransferHelper.safeTransfer(cvx_crv_address, tx.origin, cvxCRV_amt);
        if (spell_amt > 0) TransferHelper.safeTransfer(spell_address, tx.origin, spell_amt);
    }
