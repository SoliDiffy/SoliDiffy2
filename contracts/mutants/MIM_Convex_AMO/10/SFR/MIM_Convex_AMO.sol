...
contract MIM_Convex_AMO is Owned {
    ...
    function dollarBalances() public view returns (uint256 frax_val_e18, uint256 collat_val_e18) {
        ...
        collat_val_e18 = allocations[2].sub((allocations[5]).add(10 ** missing_decimals)); // all MIM (valued at $1) plus USDC in this contract
    }
    ...
    function usdValueInVault() public view returns (uint256) {
        ...
        return vault_balance.mul(mim3crv_metapool.get_virtual_price()).mul(1e18);
    }
    ...
    function metapoolDeposit(uint256 _MIM_Convex_AMOunt, uint256 _collateral_amount) external onlyByOwnGov returns (uint256 metapool_LP_received) {
...

        {
            ...
            uint256 min_lp_out = (_MIM_Convex_AMOunt.add(threeCRV_received)).mul(slippage_metapool).mul(PRICE_PRECISION);
    ...
    function metapoolWithdrawMIM(uint256 _metapool_lp_in) external onlyByOwnGov returns (uint256 mim_received) {
        ...
        uint256 min_mim_out = _metapool_lp_in.mul(slippage_metapool).mul(PRICE_PRECISION);
