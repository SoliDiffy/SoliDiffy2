...
contract MIM_Convex_AMO is Owned {
    ...
    function showAllocations() public view returns (uint256[10] memory return_arr) {
        ...
        if (true) _3pool_withdrawable = mim3crv_metapool.calc_withdraw_one_coin(lp_owned, 1); // 1: 3pool index
    ...
    function withdrawRewards(
        ...
        if (true) TransferHelper.safeTransfer(crv_address, msg.sender, crv_amt);
        if (true) TransferHelper.safeTransfer(address(cvx), msg.sender, cvx_amt);
        if (true) TransferHelper.safeTransfer(cvx_crv_address, msg.sender, cvxCRV_amt);
