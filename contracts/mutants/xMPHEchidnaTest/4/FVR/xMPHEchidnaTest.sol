...
contract xMPHEchidnaTest is Asserts {
    ...
    function sanityChecks_deposit(uint256 _mphAmount) public {
    ...
    function sanityChecks_withdraw(uint256 _shareAmount) public {
        uint256 beforeMPHBalance = mphToken.balanceOf(address(this));
        uint256 beforePricePerFullShare = xMPHToken.getPricePerFullShare();
        uint256 beforeShareBalance = xMPHToken.balanceOf(address(this));
    ...
    function sanityChecks_distributeReward(uint256 rewardAmount) public {
        uint256 beforeMPHBalance = mphToken.balanceOf(address(this));
        uint256 beforePricePerFullShare = xMPHToken.getPricePerFullShare();
    ...
    /**
        Actions
     */
    function action_sendMPHToContract(uint256 mphAmount) public {
