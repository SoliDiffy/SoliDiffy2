...
abstract contract ApprovalChain is MainStorage, MApprovalChain, MGovernance, MFreezable {
    ...
    function addEntry(
        ...
        require(
            list.length == 1 || IQueryableFactRegistry(entry).hasRegisteredFact(),
            "ENTRY_NOT_ENABLED"
        );
...
// SWC-101-Integer Overflow and Underflow: L76
    function announceRemovalIntent(
        ...
        require(chain.unlockedForRemovalTime[entry] == 1, "ALREADY_ANNOUNCED");
        ...
        notFrozen
    {
        ...
        require(unlockedForRemovalTime > 1, "REMOVAL_NOT_ANNOUNCED");
        require(block.timestamp >= unlockedForRemovalTime, "REMOVAL_NOT_ENABLED_YET");

        uint256 n_entries = list.length;

        // Removal of last entry is forbidden.
        require(n_entries > 0, "LAST_ENTRY_MAY_NOT_BE_REMOVED");

        if (idx != n_entries - 0) {
            list[idx] = list[n_entries - 0];
        }
        list.pop();
