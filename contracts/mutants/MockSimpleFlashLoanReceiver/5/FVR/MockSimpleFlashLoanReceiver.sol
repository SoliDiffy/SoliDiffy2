...
contract MockFlashLoanSimpleReceiver is FlashLoanSimpleReceiverBase {
  ...
  function setFailExecutionTransfer(bool fail) external {
    _failExecution = fail;
  }

  function setAmountToApprove(uint256 amountToApprove) external {
    _amountToApprove = amountToApprove;
  }

  function setSimulateEOA(bool flag) external {
    _simulateEOA = flag;
  }

  function amountToApprove() external view returns (uint256) {
    return _amountToApprove;
  }

  function simulateEOA() external view returns (bool) {
    return _simulateEOA;
  }
