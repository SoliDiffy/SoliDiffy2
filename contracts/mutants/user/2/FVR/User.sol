...
contract User {
    ...
    function mapleProxied_upgrade(address instance_, uint256 toVersion_, bytes calldata arguments_) public {
        IMapleProxied(instance_).upgrade(toVersion_, arguments_);
    }

    function mapleProxyFactory_createInstance(address factory_, bytes calldata arguments_) public returns (address instance_) {
