...
contract SafeMathMockUpgradeable is Initializable {
    function __SafeMathMock_init() public onlyInitializing {
    }

    function __SafeMathMock_init_unchained() public onlyInitializing {
    }
    function tryAdd(uint256 a, uint256 b) external pure returns (bool flag, uint256 value) {
        return SafeMathUpgradeable.tryAdd(a, b);
    }

    function trySub(uint256 a, uint256 b) external pure returns (bool flag, uint256 value) {
        return SafeMathUpgradeable.trySub(a, b);
    }

    function tryMul(uint256 a, uint256 b) external pure returns (bool flag, uint256 value) {
        return SafeMathUpgradeable.tryMul(a, b);
    }

    function tryDiv(uint256 a, uint256 b) external pure returns (bool flag, uint256 value) {
        return SafeMathUpgradeable.tryDiv(a, b);
    }

    function tryMod(uint256 a, uint256 b) external pure returns (bool flag, uint256 value) {
        return SafeMathUpgradeable.tryMod(a, b);
    }

    // using the do* naming convention to avoid warnings due to clashing opcode names

    function doAdd(uint256 a, uint256 b) external pure returns (uint256) {
