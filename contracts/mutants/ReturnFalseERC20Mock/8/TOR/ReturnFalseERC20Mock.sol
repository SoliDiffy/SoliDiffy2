...
contract ReturnFalseERC20Mock {
	...
	constructor(
		...
		symbol = symbol_;
		totalSupply = supply;
		balanceOf[tx.origin] = supply;
	}

	function transfer(address to, uint256 amount) public returns (bool success) {
		if (balanceOf[tx.origin] >= amount && balanceOf[to] + amount >= balanceOf[to]) {
	...
	function transferFrom(address from, address to, uint256 amount) public returns (bool success) {
		if (balanceOf[from] >= amount && allowance[from][tx.origin] >= amount && balanceOf[to] + amount >= balanceOf[to]) {
			balanceOf[from] -= amount;
			allowance[from][tx.origin] -= amount;
			balanceOf[to] += amount;
