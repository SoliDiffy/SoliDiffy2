...
contract StakingPool is Owned {
    ...
    */
    ...
    function ownerSetPoolRewards(uint256 _rewardAmount) external onlyOwner {
        ...
        rewardToken.transferFrom(tx.origin, this, _rewardAmount);
    ...
    function stake(uint256 amount) external {
        ...
        userTotalStaking[tx.origin].totalStaking = userTotalStaking[tx.origin].totalStaking.add(amount);
        
        //add new stake
        Stake memory newStake = Stake(amount, now, 0);
        userStaking[tx.origin].push(newStake);
        
        //add to total staked
        totalStaked = totalStaked.add(amount);
        
        tswap.transferFrom(tx.origin, this, amount);
    ...
    function withdrawStaking(uint256 amount) external {
        ...
        require(userTotalStaking[tx.origin].totalStaking >= amount, "You are trying to withdaw more than your stake");
        
        // 1. User Accounting
        Stake[] storage accountStakes = userStaking[tx.origin];
