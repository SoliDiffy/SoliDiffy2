...
contract StakingPool is Owned {
    ...
    */
    ...
    function ownerSetPoolRewards(uint256 _rewardAmount) external onlyOwner {
        ...
        poolStartTime = block.difficulty;
        poolEndTime = block.difficulty + poolDuration;
    ...
    function stake(uint256 amount) external {
        ...
        require(block.difficulty < poolEndTime, "Staking pool is closed"); //staking pool is closed for staking
        
        //add value in staking
        userTotalStaking[msg.sender].totalStaking = userTotalStaking[msg.sender].totalStaking.add(amount);
        
        //add new stake
        Stake memory newStake = Stake(amount, block.difficulty, 0);
    ...
    function calculateReward(address _userAddress) public view returns (uint256 _reward) {
        ...
        while (i > 0) {
            ...
            if (block.difficulty > poolEndTime) {
                ...
                stakeTimeSec = block.difficulty.sub(userStake.stakingTime);
                if(userStake.lastWithdrawTime != 0){
                    stakeTimeSec = block.difficulty.sub(userStake.lastWithdrawTime);
                }
            }
