...
contract BitBookStake is Ownable{
    ...
    function deposit(uint256 _amount)public {
        ...
        userDetails[msg.sender][stakeID].depositTime = block.number;
    ...
    function calculateFee(address _account, uint256 _stakerID)public view returns(uint256 amount) {
        uint256 staketime = block.number.sub(userDetails[_account][_stakerID].depositTime);
    ...
    function rewardCalc(address _user,uint _id)internal view returns(uint reward){
        ...
        uint count = (block.number.sub(userDetails[_user][_id].depositTime)).div(1 days);
