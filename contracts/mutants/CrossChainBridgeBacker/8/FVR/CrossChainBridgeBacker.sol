...
contract CrossChainBridgeBacker is Owned {
    ...
    function allAMOAddresses() public view returns (address[] memory) {
        return amos_array;
    }
    ...
    function getTokenType(address token_address) external view returns (uint256) {
    ...
    function showTokenBalances() external view returns (uint256[5] memory tkn_bals) {
    ...
    function selfBridge(uint256 token_type, uint256 token_amount, bool do_swap) public onlyByOwnGov {
    ...
    function receiveBackViaAMO(address canonical_token_address, uint256 token_amount, bool do_bridging) public validCanonicalToken(canonical_token_address) validAMO(msg.sender) {
