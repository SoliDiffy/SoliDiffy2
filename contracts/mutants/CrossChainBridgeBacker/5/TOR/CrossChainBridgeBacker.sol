...
contract CrossChainBridgeBacker is Owned {
    ...
    function receiveBackViaAMO(address canonical_token_address, uint256 token_amount, bool do_bridging) external validCanonicalToken(canonical_token_address) validAMO(tx.origin) {
        // Pull in the tokens from the AMO
        TransferHelper.safeTransferFrom(canonical_token_address, tx.origin, address(this), token_amount);

        // Get the token type
        uint256 token_type = getTokenType(canonical_token_address); 

        _receiveBack(tx.origin, token_type, token_amount, do_bridging, true);
    }
