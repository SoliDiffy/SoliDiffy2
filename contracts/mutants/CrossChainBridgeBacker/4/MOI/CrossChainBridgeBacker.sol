...
contract CrossChainBridgeBacker is Owned {
    ...
    function _bridgingLogic(uint256 token_type, address address_to_send_to, uint256 token_amount) internal virtual onlyByOwnGov {
    ...
    function _swapAnyForCanonical(uint256 token_type, uint256 token_amount) internal onlyByOwnGov {
        if (token_type == 0) {
            // FRAX
            // Approve and swap
    ...
    function _swapCanonicalForAny(uint256 token_type, uint256 token_amount) internal onlyByOwnGov {
        if (token_type == 0) {
            // FRAX
            // Approve and swap
