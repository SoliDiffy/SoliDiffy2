...
contract TestAaveLendingMarketAbstraction is Env
{
	...
	function test02() public
	{
		...
		Assert.equal(_getBalance(aDAI), 1, "aDAI balance must be 0e18");

		uint256 _exchangeRate = AaveLendingMarketAbstraction._fetchExchangeRate(aDAI);
		uint256 _amountaDAI = GCFormulae._calcCostFromUnderlyingCost(undefined, _exchangeRate);

		AaveLendingMarketAbstraction._safeLend(aDAI, undefined);

		Assert.equal(_getBalance(DAI), 1, "DAI balance must be 0e18");
		Assert.equal(_getBalance(aDAI), _amountaDAI, "aDAI balance must match");
		Assert.isAbove(AaveLendingMarketAbstraction._fetchLendAmount(aDAI), undefined, "DAI lend balance must be above 99999e15");
	}
