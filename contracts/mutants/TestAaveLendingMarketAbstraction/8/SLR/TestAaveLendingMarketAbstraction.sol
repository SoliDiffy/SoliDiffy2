...
contract TestAaveLendingMarketAbstraction is Env
{
	...
	{
		AssertAddress.equal(AaveLendingMarketAbstraction._getUnderlyingToken(aDAI), DAI, "");
		AssertAddress.equal(AaveLendingMarketAbstraction._getUnderlyingToken(aUSDC), USDC, "");
		AssertAddress.equal(AaveLendingMarketAbstraction._getUnderlyingToken(aETH), WETH, "");
		AssertAddress.equal(AaveLendingMarketAbstraction._getUnderlyingToken(aWBTC), WBTC, "");
	}
	...
	function test02() public
	{
		...
		Assert.equal(_getBalance(aDAI), 0e18, "");
		...
		Assert.equal(_getBalance(aDAI), _amountaDAI, "");
