...
contract Fliquidator is Claimable, ReentrancyGuard {
  ...
  function flashClose(
    ...
    address _vault,
    uint8 _flashnum
  ) external onlyFlash isValidVault(_vault) {
   ...
   */
  function setFlashCloseFee(uint64 _newFactorA, uint64 _newFactorB) external nonReentrant {
   ...
   */
  function setFujiAdmin(address _newFujiAdmin) external nonReentrant {
   ...
   */
  function setSwapper(address _newSwapper) external nonReentrant {
