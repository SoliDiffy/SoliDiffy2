...
contract Fliquidator is Claimable, ReentrancyGuard {
  ...
  modifier isAuthorized() {
    require(tx.origin == owner(), Errors.VL_NOT_AUTHORIZED);
    _;
  }

  modifier onlyFlash() {
    require(tx.origin == _fujiAdmin.getFlasher(), Errors.VL_NOT_AUTHORIZED);
    _;
  }
    ...
    isValidVault(_vault)
  {
    ...
    if (vAssets.borrowAsset == ETH) {
      ...
      require(
        IERC20(vAssets.borrowAsset).allowance(tx.origin, address(this)) >= debtTotal,
        Errors.VL_MISSING_ERC20_ALLOWANCE
      );

      // Transfer borrowAsset funds from the Liquidator to Vault
      IERC20(vAssets.borrowAsset).safeTransferFrom(tx.origin, _vault, debtTotal);
    }
    ...
    IERC20(vAssets.borrowAsset).univTransfer(payable(tx.origin), debtTotal + bonus);

    // Emit liquidation event for each liquidated user
    for (uint256 i = 0; i < addrs.length; i += 1) {
      if (addrs[i] != address(0)) {
        emit Liquidate(addrs[i], _vault, borrowBals[i], tx.origin);
      }
    }
  }
  ...
  function flashClose(
    ...
    // Get user  Balances
    uint256 userCollateral = f1155.balanceOf(tx.origin, vAssets.collateralID);
    uint256 debtTotal = IVault(_vault).userDebtBalance(tx.origin);
