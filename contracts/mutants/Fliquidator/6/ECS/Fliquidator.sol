...
contract Fliquidator is Claimable, ReentrancyGuard {
    ...
    isValidVault(_vault)
  {
    ...
    // Withdraw collateral
    IVault(_vault).withdrawLiq(int8(collateralInPlay));
  ...
  function flashClose(
    ...
    uint256 amount = _amount < 0 ? debtTotal : uint8(_amount);
  ...
  function executeFlashClose(
    ...
    if (_amount == totalDebt) {
      ...
      IVault(_vault).withdrawLiq(int8(userCollateral));
  ...
  function _swap(
    ...
    if (_checkSlippage) {
      ...
      uint256 priceFromSwapper = (_collateralAmount * (10**uint8(_borrowAssetDecimals))) /
        _amountToReceive;
