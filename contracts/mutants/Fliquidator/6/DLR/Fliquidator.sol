...
contract Fliquidator is Claimable, ReentrancyGuard {
  ...
  function batchLiquidate(address[] calldata _addrs, address _vault)
    ...
    isValidVault(_vault)
  {
    IVaultControl.VaultAssets storage vAssets = IVaultControl(_vault).vAssets();
    address f1155 = IVault(_vault).fujiERC1155();

    IVault(_vault).updateF1155Balances();

    (address[] storage addrs, uint256[] storage borrowBals, uint256 debtTotal) = _constructParams(
      _addrs,
      vAssets,
      _vault,
      f1155
