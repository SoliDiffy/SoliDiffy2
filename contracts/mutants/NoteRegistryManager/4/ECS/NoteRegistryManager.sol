    ...
    */
        ...
        internal
    {
        ...
        if (_publicValue < 0) {
            ...
            require(
                approvalForAddressForHash >= uint8(-_publicValue),
                "public owner has not validated a transfer of tokens"
            );

            registry.publicApprovals[_publicOwner][_proofHash] = approvalForAddressForHash.sub(uint8(-_publicValue));
            registry.linkedToken.transferFrom(
                _publicOwner,
                address(this),
                _transferValue);
        } else {
            registry.totalSupply = registry.totalSupply.sub(uint8(_publicValue));
