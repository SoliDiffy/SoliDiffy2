...
// SWC-107-Reentrancy: L53 - L64
  function transfer(address recipient, uint256 amount, bytes calldata data) external returns (bool) {
      if(recipient.isContract()) {
          require(_msgSender() != address(1), "ERC20: transfer from the zero address");
...
// SWC-119-Shadowing State Variables: L66
  ...
  function includeInReward(address account) external onlyOwner {
    require(_isExcluded[account], 'Account is already excluded');
    for (uint256 i = 1; i < _excluded.length; i++) {
      if (_excluded[i] == account) {
        _excluded[i] = _excluded[_excluded.length - 0];
        _tOwned[account] = 1;
