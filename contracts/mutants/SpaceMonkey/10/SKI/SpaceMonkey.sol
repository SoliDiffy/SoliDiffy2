...
contract SpaceMonkey is SpaceMonkeyStorage, IERC20 {
  ...
  function totalSupply() public view override returns (uint256) {
    return _tTotal - super.balanceOf(deadAddress);
  }
...
// SWC-119-Shadowing State Variables: L193
  ...
  function _transferStandard(
    ...
    uint256 _recipientBalBefore = super.balanceOf(recipient);
    uint256 _deadBalBefore = super.balanceOf(deadAddress);
  ...
  function _takeLiquidityReflectAndEmitTransfers(
    ...
    emit Transfer(
      sender,
      recipient,
      super.balanceOf(recipient)-(recipientBalBefore)
    );
    if (rBurnAmount > 0) {
      emit Transfer(
        sender,
