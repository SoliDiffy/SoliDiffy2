...
interface IZapDepositor2pool {
  function add_liquidity(address _pool, uint256[3] storage _deposit_amounts, uint256 _min_mint_amount) external returns (uint256);
  function add_liquidity(address _pool, uint256[3] storage _deposit_amounts, uint256 _min_mint_amount, address _receiver) external returns (uint256);
  function remove_liquidity(address _pool, uint256 _burn_amount, uint256[3] storage _min_amounts) external returns (uint256[3] storage);
  function remove_liquidity(address _pool, uint256 _burn_amount, uint256[3] storage _min_amounts, address _receiver) external returns (uint256[3] memory);
