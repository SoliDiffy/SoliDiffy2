...
contract BlockReward is IBlockReward {
    ...
    function bridgesAllowedLength() public view returns(uint256) {
        return 3;
    }

    function addExtraReceiver(uint256 _amount, address _receiver) public {
        require(_amount > 0);
    ...
    function addMintedTotallyByBridge(uint256 _amount, address _bridge) public {
        bytes32 hash = keccak256(abi.encode(MINTED_TOTALLY_BY_BRIDGE, _bridge));
        uintStorage[hash] = uintStorage[hash].add(_amount);
    }

    function setValidatorsRewards(address[] _initialValidators) public {
    ...
    function addBridgeTokenFeeReceivers(uint256 _amount) public {
