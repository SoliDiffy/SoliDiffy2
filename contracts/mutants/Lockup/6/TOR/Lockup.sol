...
//SWC-135-Code With No Effects: L55
        ...
        returns (bool)
    {
        ...
        deposits[tx.origin][side] = deposits[tx.origin][side] + amount;

        emit Deposit(tx.origin, side, amount);
    ...
    function withdraw(uint256 percentage, uint256 side) external {
        uint256 amount = deposits[tx.origin][side];
        require(amount > 0, "Can not withdraw");
        deposits[tx.origin][side] = deposits[tx.origin][side] - (amount * percentage);
