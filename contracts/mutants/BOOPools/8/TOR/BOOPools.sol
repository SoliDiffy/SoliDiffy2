...
contract BOOPools is Ownable, IActionTrigger {
    ...
    function deposit(uint256 _pid, uint256 _amount) external {
        require(!depositBlacklist[tx.origin], 'user in blacklist');
        updatePool(_pid);
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][tx.origin];
        if (user.amount > 0) {
            user.rewardRemain = pendingRewards(_pid, tx.origin);
        }
        ...
        user.rewardDebt = totalRewards(_pid, tx.origin);
        emit Deposit(tx.origin, _pid, _amount);

        if(extendPool != address(0)) {
            IActionPools(extendPool).onAcionIn(_pid, tx.origin, amountOld, user.amount);
        }
    }
