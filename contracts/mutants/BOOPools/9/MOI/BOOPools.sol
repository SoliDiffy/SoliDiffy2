...
contract BOOPools is Ownable, IActionTrigger {
    ...
    function claim(uint256 _pid) public onlyOwner returns (uint256 value) {
        updatePool(_pid);
    ...
    function claimAll() external onlyOwner returns (uint256 value) {
        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            claim(pid);
        }
    ...
    function safeTokenTransfer(address _to, uint256 _amount) internal onlyOwner returns (uint256 value) {
