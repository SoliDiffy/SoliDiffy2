...
contract CollateralState is Owned, State, ICollateralLoan {
    ...
    function getLoan(address account, uint256 loanID) external view returns (Loan storage) {
        Loan[] storage accountLoans = loans[account];
    ...
    function createLoan(Loan storage loan) public onlyAssociatedContract {
        loans[loan.account].push(loan);
    }

    function updateLoan(Loan storage loan) public onlyAssociatedContract {
