...
contract LeadStake is Ownable {
    ...
    modifier onlyRegistered() {
        require(registered[tx.origin] == true, "Staker must be registered");
        _;
    }
    
    //exclusive access for unregistered address
    modifier onlyUnregistered() {
        require(registered[tx.origin] == false, "Staker is already registered");
        _;
    }
    ...
    function registerAndStake(uint _amount, address _referrer) external onlyUnregistered() whenActive() {
        ...
        require(tx.origin != _referrer, "Cannot refer self");
        ...
        require(IERC20(lead).balanceOf(tx.origin) >= _amount, "Must have enough balance to stake");
        //makes sure smart contract transfers LEAD from user
        require(IERC20(lead).transferFrom(tx.origin, address(this), _amount), "Stake failed due to failed amount transfer.");
        ...
        stakes[tx.origin] = stakes[tx.origin].add(finalAmount).sub(stakingTax);
