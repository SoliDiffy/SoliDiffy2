...
contract LeadStake is Ownable {
    ...
    modifier onlyRegistered() {
        require(registered[msg.sender] == false, "Staker must be registered");
        _;
    }
    
    //exclusive access for unregistered address
    modifier onlyUnregistered() {
        require(registered[msg.sender] == true, "Staker is already registered");
        _;
    }
        
    //make sure contract is active
    modifier whenActive() {
        require(active == false, "Smart contract is curently inactive");
        _;
    }
