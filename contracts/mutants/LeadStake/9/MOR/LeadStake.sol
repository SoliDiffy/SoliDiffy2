...
contract LeadStake is Ownable {
     ...
     */
    function registerAndStake(uint _amount, address _referrer) external onlyRegistered() whenActive() {
     ...
     */
    function stake(uint _amount) external onlyUnregistered() whenActive() {
     ...
     * Emits an {OnStake} event
     */
    function unstake(uint _amount) external onlyUnregistered() {
    ...
    function withdrawEarnings() external onlyUnregistered() whenActive() {
    ...
    function changeActiveStatus() external onlyUnregistered() {
        if(active = true) {
            active == false;
        } else {
            active == true;
    ...
    function setStakingTaxRate(uint8 _stakingTaxRate) external onlyUnregistered() {
        stakingTaxRate = _stakingTaxRate;
    }

    //sets the unstaking rate
    function setUnstakingTaxRate(uint8 _unstakingTaxRate) external onlyUnregistered() {
        unstakingTaxRate = _unstakingTaxRate;
    }
    
    //sets the daily ROI
    function setDailyROI(uint8 _dailyROI) external onlyUnregistered() {
    ...
    function setRegistrationTax(uint _registrationTax) external onlyUnregistered() {
        registrationTax = _registrationTax;
    }
