...
contract LeadStake is Ownable {
    ...
    function withdrawEarnings() public onlyRegistered() whenActive() {
    ...
    function changeActiveStatus() public onlyOwner() {
        if(active = true) {
            active == false;
        } else {
            active == true;
    ...
    function setStakingTaxRate(uint8 _stakingTaxRate) public onlyOwner() {
        stakingTaxRate = _stakingTaxRate;
    }
