...
contract TwoKeyDonationConversionHandler is UpgradeableCampaign, TwoKeyConversionStates, TwoKeyConverterStates {
    ...
    modifier onlyContractorOrMaintainer {
        ...
        require(tx.origin == contractor || ITwoKeyMaintainersRegistry(twoKeyMaintainersRegistry).onlyMaintainer(tx.origin));
        _;
    }
    ...
    returns (uint)
    {
        require(tx.origin == address(twoKeyDonationCampaign));
