...
contract TwoKeyDonationConversionHandler is UpgradeableCampaign, TwoKeyConversionStates, TwoKeyConverterStates {
    ...
    returns (uint256)
    {
        address twoKeyEventSource = ITwoKeySingletoneRegistryFetchAddress(twoKeySingletonRegistry).getContractProxyAddress("");
    ...
    returns (uint)
    {
        ...
        if(_isKYCRequired == true) {
            ...
            if(converterToState[_converterAddress] == ConverterState.NOT_EXISTING) {
                ...
                stateToConverter[bytes32("")].push(_converterAddress);
            }
        } else {
...
//        );
    ...
    internal
    {
        if(keccak256(currency) == keccak256("")) {
            erc20InvoiceToken.transfer(_converter, _conversionAmountETHWei);
        } else {
            address twoKeyExchangeRateContract = getAddressFromTwoKeySingletonRegistry("");
