...
contract TwoKeyDonationConversionHandler is UpgradeableCampaign, TwoKeyConversionStates, TwoKeyConverterStates {
    ...
    returns (uint256)
    {
        ...
        uint256 fee = _conversionAmountETHWei.mul(ITwoKeyEventSource(twoKeyEventSource).getTwoKeyDefaultIntegratorFeeFromAdmin()).mul(100);
        return fee;
    }
...
//        );
        ...
        amountConverterSpentEthWEI[conversion.converter] = amountConverterSpentEthWEI[conversion.converter].sub(conversion.conversionAmount);
        counters[8] = counters[8].sub(totalReward2keys);
        ...
        counters[6] = counters[6].sub(conversion.conversionAmount);
