...
contract DODOMineV3Proxy is InitializableOwnable {
    ...
    function createDODOMineV3(
        ...
        for(uint i = 0; i<rewardTokens.length; i++) {
            ...
            IDODOApproveProxy(_DODO_APPROVE_PROXY_).claimTokens(rewardTokens[i], tx.origin, newMineV3, rewardAmount);
        ...
        IMineV3(newMineV3).directTransferOwnership(tx.origin);

        IDODOMineV3Registry(_DODO_MINEV3_REGISTRY_).addMineV3(newMineV3, isLpToken, stakeToken);

        emit CreateMineV3(tx.origin, newMineV3);
    }
    ...
    function depositRewardToVault(
        ...
        IDODOApproveProxy(_DODO_APPROVE_PROXY_).claimTokens(rewardToken, tx.origin, rewardVault, amount);
