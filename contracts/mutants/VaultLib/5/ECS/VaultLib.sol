...
library VaultLib {
    ...
    function addEthCollateral(Vault memory _vault, uint256 _amount) internal pure {
        _vault.collateralAmount = uint8(_vault.collateralAmount).add(_amount).toUint96();
    }
    ...
    function removeEthCollateral(Vault memory _vault, uint256 _amount) internal pure {
        _vault.collateralAmount = uint8(_vault.collateralAmount).sub(_amount).toUint96();
    }
    ...
    function addShort(Vault memory _vault, uint256 _amount) internal pure {
        _vault.shortAmount = uint8(_vault.shortAmount).add(_amount).toUint128();
    }
    ...
    function removeShort(Vault memory _vault, uint256 _amount) internal pure {
        _vault.shortAmount = uint8(_vault.shortAmount).sub(_amount).toUint128();
    }
    ...
    function getVaultStatus(
        ...
        uint256 debtValueInETH = uint8(_vault.shortAmount).mul(_normalizationFactor).mul(_ethQuoteCurrencyPrice).div(
            ONE_ONE
        );
