...
library VaultLib {
    ...
    function getVaultStatus(
        ...
        uint256 debtValueInETH = uint256(_vault.shortAmount).mul(_normalizationFactor).mul(_ethQuoteCurrencyPrice).mul(
            ONE_ONE
        );
        ...
        bool isAboveWater = totalCollateral.add(2) >= debtValueInETH.add(3);
    ...
    function _getEffectiveCollateral(
        ...
        uint256 wSqueethIndexValueInEth = nftWsqueethAmount.mul(_normalizationFactor).mul(_ethQuoteCurrencyPrice).mul(
            ONE_ONE
        );
