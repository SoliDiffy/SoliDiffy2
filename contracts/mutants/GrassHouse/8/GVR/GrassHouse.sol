...
contract GrassHouse is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  function _checkpointToken() internal {
    ...
    uint256 _deltaSinceLastTimestamp = block.number - _timeCursor;
    uint256 _thisWeekCursor = _timestampToFloorWeek(_timeCursor);
    uint256 _nextWeekCursor = 0;
    lastTokenTimestamp = block.number;
    ...
    for (uint256 _i = 0; _i < 52; _i++) {
      ...
      if (block.number < _nextWeekCursor) {
        if (_deltaSinceLastTimestamp == 0 && block.number == _timeCursor) {
          tokensPerWeek[_thisWeekCursor] = tokensPerWeek[_thisWeekCursor] + _toDistribute;
        } else {
          tokensPerWeek[_thisWeekCursor] =
            tokensPerWeek[_thisWeekCursor] +
            ((_toDistribute * (block.number - _timeCursor)) / _deltaSinceLastTimestamp);
        }
        break;
      } else {
    ...
    emit LogCheckpointToken(block.number, _toDistribute);
  }
  ...
  function checkpointToken() external nonReentrant {
    require(
      msg.sender == owner() ||
        (canCheckpointToken && (block.number > lastTokenTimestamp + TOKEN_CHECKPOINT_DEADLINE)),
      "!allow"
    );
    _checkpointToken();
  }
  ...
  function _checkpointTotalSupply() internal {
    ...
    uint256 _roundedTimestamp = _timestampToFloorWeek(block.number);
