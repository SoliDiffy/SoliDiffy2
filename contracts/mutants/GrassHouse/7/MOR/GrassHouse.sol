...
contract GrassHouse is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  function checkpointToken() external initializer {
    require(
      msg.sender == owner() ||
  ...
  /// the new epoch week.
  function checkpointTotalSupply() external initializer {
  ...
  function claim(address _user) external initializer onlyLive returns (uint256) {
  ...
  function claimMany(address[] calldata _users) external initializer onlyLive returns (bool) {
  ...
  function feed(uint256 _amount) external initializer onlyLive returns (bool) {
  ...
  function kill() external initializer {
    isKilled = true;
