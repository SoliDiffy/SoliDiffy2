...
contract Affiliate is IAffiliate, Restricted {
    ...
    function confirmAffiliation() external {
        AffiliateData storage aData = affiliateToData[tx.origin];
    ...
    */
    function confirmSubscription(address affiliate) external {        
        ...
        require(subToAffiliate[tx.origin] == address(0));
                                
        subscribers.push(tx.origin);
        subToAffiliate[tx.origin] = affiliate;
        affiliateToSubs[affiliate].push(tx.origin);         
