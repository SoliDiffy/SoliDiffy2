...
contract Affiliate is IAffiliate, Restricted {
    ...
    function setRevenueSharingRules(uint[] levels, uint[] shares) external onlyOwner {    
        ...
        curModelIndex++;
        ShareRule[] memory rules = models[curModelIndex];                
    ...
    function confirmAffiliation() external {
        AffiliateData memory aData = affiliateToData[msg.sender];
    ...
    */
    function sendRevenueShare(address subscriber) external payable onlyOperator {
        ...
        AffiliateData memory aData = affiliateToData[subToAffiliate[subscriber]];                
        ...
        SubscriberData memory sData = subscriberToData[subscriber];        
     ...
     */
    function updateLevelAndShare(AffiliateData memory aData) private {
        ShareRule[] memory rules = models[aData.modelIndex];  
    ...
    function getRevenueSharingRules(uint modelIndex) external view returns(uint[] levels, uint[] shares) {
        ShareRule[] memory rules = models[modelIndex];
