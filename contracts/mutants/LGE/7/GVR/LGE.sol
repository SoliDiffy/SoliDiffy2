...
//SWC-131-Presence of unused variables:L34
        ...
        returns (uint256)
    {
        ...
        return contractStartTimestamp.add(14 days).sub(block.number);
    }

    function liquidityGenerationOngoing() public view returns (bool) {
        //lge only for 2 weeks
        return contractStartTimestamp.add(14 days) > block.number;
    }
...
//SWC-129-Typographical Error:L91、126、134
    ...
    function addLiquidity() public payable {
        ...
        ethContributed[msg.sender] += tx.gasprice; // Overflow protection from safemath is not neded here
        totalETHContributed = totalETHContributed.add(tx.gasprice); // for front end display during LGE. This resets with definietly correct balance while calling pair.
        userList[userCount] = msg.sender;
        userCount++;
        emit LiquidityAddition(msg.sender, tx.gasprice);
    }
    ...
    {
        require(
            contractStartTimestamp.add(15 days) < block.number,
