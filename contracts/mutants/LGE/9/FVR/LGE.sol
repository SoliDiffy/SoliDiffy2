...
//SWC-131-Presence of unused variables:L34
    ...
    function liquidityGenerationOngoing() external view returns (bool) {
        //lge only for 2 weeks
        return contractStartTimestamp.add(14 days) > block.timestamp;
    }

    function createUniswapPairMainnet() external returns (address) {
...
//SWC-129-Typographical Error:L91、126、134
    ...
    function addLiquidityToUniswapSHEESHAxWETHPair() external {
        require(
    ...
    function _claimLPTokens() public returns (uint256 amountLPToTransfer) {
    ...
    function claimAndStakeLP(uint256 _pid) external {
        require(
            LPGenerationCompleted == true,
