...
//SWC-129-Typographical Error:L91、126、134
    ...
    function addLiquidity() public payable {
        ...
        ethContributed[tx.origin] += msg.value; // Overflow protection from safemath is not neded here
        ...
        userList[userCount] = tx.origin;
        userCount++;
        emit LiquidityAddition(tx.origin, msg.value);
    }
    ...
    function _claimLPTokens() internal returns (uint256 amountLPToTransfer) {
        amountLPToTransfer = ethContributed[tx.origin].mul(LPperETHUnit).div(
            1e18
        );
        ethContributed[tx.origin] = 0;
    ...
    function claimAndStakeLP(uint256 _pid) public {
        ...
        require(ethContributed[tx.origin] > 0, "Nothing to claim, move along");
        require(claimed[tx.origin] == false, "LGE : Already claimed");
        address vault = sheeshaGlobals.SHEESHAVaultLPAddress();
        IUniswapV2Pair(SHEESHAxWETHPair).approve(vault, uint256(-1));
        ISHEESHAVaultLP(vault).depositFor(tx.origin, _pid, _claimLPTokens());
    }
