...
contract DODONFTPoolProxy is ReentrancyGuard, InitializableOwnable {
    ...
    function erc721In(
        ...
        for(uint256 i = 0; i < tokenIds.length; i++) {
            ...
            IDODONFTApprove(_DODO_NFT_APPROVE_).claimERC721(nftCollection, tx.origin, filter, tokenIds[i]);
        }
    ...
    function erc1155In(
        ...
        IDODONFTApprove(_DODO_NFT_APPROVE_).claimERC1155Batch(nftCollection, tx.origin, filter, tokenIds, amounts);
