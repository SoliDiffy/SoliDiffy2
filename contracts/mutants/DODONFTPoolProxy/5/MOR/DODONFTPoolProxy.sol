...
contract DODONFTPoolProxy is ReentrancyGuard, InitializableOwnable {
    ...
    //====================== Ownable ========================
    function changeMaintainer(address newMaintainer) external preventReentrant {
        _MAINTAINER_ = newMaintainer;
    }

    function changeFilterAdminTemplate(address newFilterAdminTemplate) external preventReentrant {
        _FILTER_ADMIN_TEMPLATE_ = newFilterAdminTemplate;
    }

    function changeController(address newControllerModel) external preventReentrant {
        _CONTROLLER_ = newControllerModel;
    }

    function setFilterTemplate(uint256 idx, address newFilterTemplate) external preventReentrant {
