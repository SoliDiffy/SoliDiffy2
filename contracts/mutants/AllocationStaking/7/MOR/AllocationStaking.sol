...
contract AllocationStaking is OwnableUpgradeable {
    ...
    function setSalesFactory(address _salesFactory) external initializer {
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public initializer {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    // Set deposit fee
    function setDepositFee(uint256 _depositFeePercent, uint256 _depositFeePrecision) public initializer {
        setDepositFeeInternal(_depositFeePercent, _depositFeePrecision);
    }
    ...
    function set(uint256 _pid, uint256 _allocPoint, bool _withUpdate) public initializer {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function setTokensUnlockTime(uint256 _pid, address _user, uint256 _tokensUnlockTime) external initializer {
        UserInfo storage user = userInfo[_pid][_user];
