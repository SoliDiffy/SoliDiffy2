...
contract AllocationStaking is OwnableUpgradeable {
    ...
    function fund(uint256 _amount) public initializer {
    ...
    function setDepositFeeInternal(uint256 _depositFeePercent, uint256 _depositFeePrecision) internal initializer {
    ...
    function deposit(uint256 _pid, uint256 _amount) public initializer {
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function withdraw(uint256 _pid, uint256 _amount, uint256 nonce, bytes memory signature) public initializer {
    ...
    function compound(uint256 _pid) public initializer {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    function emergencyWithdraw(uint256 _pid) public initializer {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(
