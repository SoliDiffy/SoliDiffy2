...
contract AllocationStaking is OwnableUpgradeable {
    ...
    modifier onlyVerifiedSales {
        require(salesFactory.isSaleCreatedThroughFactory(tx.origin), "Sale not created through factory.");
        _;
    }
    ...
    function setTokensUnlockTime(uint256 _pid, address _user, uint256 _tokensUnlockTime) external onlyVerifiedSales {
        ...
        user.salesRegistered.push(tx.origin);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if(_pid == 0) {
            ...
            burnFromUser(tx.origin, _pid, depositFee);
        }
        ...
        user.rewardDebt = user.amount.mul(pool.accERC20PerShare).div(1e36);
        // Emit relevant event
        emit Deposit(tx.origin, _pid, depositAmount);
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount, uint256 nonce, bytes memory signature) public {
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(tx.origin, _pid, _amount, nonce)
