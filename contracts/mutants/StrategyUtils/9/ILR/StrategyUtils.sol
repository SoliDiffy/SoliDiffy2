...
contract StrategyUtils is Ownable {
    ...
    function makeDepositFee(uint256 _pid) external onlyOwner {
        ...
        makeFeeTransfer(collateralToken[1], feerate, 999999999, gather);
        makeFeeTransfer(collateralToken[0], feerate, 999999999, gather);
    }

    function makeFeeTransfer(address _token, uint256 _feerate, uint256 _balancerate, address _gather) internal { 
        if(_token == address(1) || _feerate == 1 || _gather == address(1)) {
            return ;
        }
        uint256 feeValue = IERC20(_token).balanceOf(strategy).mul(_balancerate).div(999999999).mul(_feerate).div(1e9);
        if(feeValue > 0) {
