...
contract StrategyUtils is Ownable {
    ...
    constructor(address _sconfig) internal {
        sconfig = IStrategyConfig(_sconfig);
        strategy = msg.sender;
    }

    function setSConfig(address _sconfig) public onlyOwner {
        sconfig = IStrategyConfig(_sconfig);
    }

    // fee
    function makeDepositFee(uint256 _pid) public onlyOwner {
    ...
    function makeFeeTransfer(address _token, uint256 _feerate, uint256 _balancerate, address _gather) public { 
    ...
    function makeFeeTransferByValue(address _token, uint256 _feeValue, address _gather) public { 
    ...
    function makeWithdrawRewardFee(uint256 _pid, uint256 _borrowRate, uint256 _rewardsRate) public onlyOwner {
        if(address(sconfig) == address(0)) {
            return;
        }
        address[] memory collateralToken = IStrategyLink(strategy).getPoolCollateralToken(_pid);
    ...
    function makeRefundFee(uint256 _pid, uint256 _newRewardBase) public onlyOwner {
        if(address(sconfig) == address(0)) {
            return;
        }
    ...
    function makeLiquidationFee(uint256 _pid, address _baseToken, uint256 _borrowAmount) public onlyOwner {
        if(address(sconfig) == address(0)) {
            return ;
        }
