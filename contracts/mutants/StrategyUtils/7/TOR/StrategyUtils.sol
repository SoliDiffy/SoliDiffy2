...
contract StrategyUtils is Ownable {
    ...
    function makeRepay(uint256 _pid, address _borrowFrom, address _account, uint256 _rate, bool _fast)
        ...
        uint256 bid = ISafeBox(_borrowFrom).getBorrowId(tx.origin, _pid, _account);
        if( bid <= 0) {
            return ;
        }
    ...
    function getTokenOut(address _tokenIn, address _tokenOut, uint256 _amountOut) 
        ...
        IERC20(_tokenIn).safeTransferFrom(tx.origin, address(this), amountIn);
        value = getTokenInTo(tx.origin, _tokenIn, amountIn, _tokenOut);
        transferFromAllToken(address(this), tx.origin, _tokenIn, _tokenOut);
    }
    
    function getTokenIn(address _tokenIn, uint256 _amountIn, address _tokenOut) 
            public virtual onlyOwner returns (uint256 value) {
        IERC20(_tokenIn).safeTransferFrom(tx.origin, address(this), _amountIn);
        value = getTokenInTo(tx.origin, _tokenIn, _amountIn, _tokenOut);
