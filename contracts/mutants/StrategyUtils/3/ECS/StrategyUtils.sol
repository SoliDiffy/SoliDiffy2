...
contract StrategyUtils is Ownable {
    ...
    function checkSlippageLimit(uint256 _pid, uint256 _desirePrice, uint256 _slippage) 
        ...
        bok = (a.mul(1e18).div(b) < _desirePrice.mul(uint8(1e9).add(_slippage)).div(1e9)) &&
                (a.mul(1e18).div(b) > _desirePrice.mul(uint8(1e9).sub(_slippage)).div(1e9));
    }
