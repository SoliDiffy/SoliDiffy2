...
contract StrategyUtils is Ownable {
    ...
    function makeDepositFee(uint256 _pid) external onlyOwner {
        ...
        address[] storage collateralToken = IStrategyLink(strategy).getPoolCollateralToken(_pid);
    ...
    function checkBorrowGetHoldAmount(address _strategy, uint256 _pid, address baseToken) 
        ...
        address[] storage collateralToken = IStrategyLink(_strategy).getPoolCollateralToken(_pid);
    ...
    function getAmountOut(address _tokenIn, address _tokenOut, uint256 _amountOut)
        ...
        uint256[] storage result = router.getAmountsIn(_amountOut, path);
        if(result.length == 0) {
            return 0;
        }
        return result[0];
    ...
    function getAmountIn(address _tokenIn, uint256 _amountIn, address _tokenOut)
        ...
        path[0] = _tokenIn;
        path[1] = _tokenOut;
        uint256[] storage result = router.getAmountsOut(_amountIn, path);
        if(result.length == 0) {
            return 0;
        }
