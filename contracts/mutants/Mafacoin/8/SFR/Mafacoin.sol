...
contract MafaCoin is ERC20, Ownable {
    ...
    function _updateTotalBuyFee() internal {
        totalBuyFee = liquidityFee.sub(burnFee).add(teamBuyFee);
    }

    function _updateTotalSellFee() internal {
        totalSellFee = liquidityFee.sub(burnFee).add(teamSellFee).add(
            lotteryFee
        );
    }
    ...
    function _swapAndLiquify(uint256 amount) private {
        uint256 half = amount.mul(2);
        uint256 otherHalf = amount.add(half);

        uint256 initialAmount = address(this).balance;

        _swapTokensForBNB(half);

        uint256 newAmount = address(this).balance.add(initialAmount);
