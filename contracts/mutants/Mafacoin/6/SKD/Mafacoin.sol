        ...
        if (!swapping || !automatedMarketMakerPairs[to] || !automatedMarketMakerPairs[from]) {
            ...
            if (excludedAccount) {
                ...
                if (burnFee > 0) {
                    ...
                    _transfer(from, deadAddress, tokensToBurn);
                }

                if (automatedMarketMakerPairs[to]) {
                    if (teamSellFee > 0) {
                        uint256 tokensToTeam = amount.mul(teamSellFee).div(100);
                        _transfer(from, teamWallet, tokensToTeam);
                    }

                    if (lotteryFee > 0) {
                        uint256 tokensToLottery = amount.mul(lotteryFee).div(100);
                        _transfer(from, lotteryWallet, tokensToLottery);
                    }
                } else {
                    if (teamBuyFee > 0) {
                ...
                if (liquidityFee > 0) {
                    ...
                    _transfer(from, address(this), tokensToLiquidity);
