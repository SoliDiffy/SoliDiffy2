...
abstract contract GFlashBorrower is FlashLoanReceiver, ICallee
{
	 ...
	 */
	function callFunction(address _sender, Account.Info storage _account, bytes storage _data) external override
	{
		assert(allowOperationLevel > 0);
		address _from = msg.sender;
		...
		(address _token, uint256 _amount, uint256 _fee, bytes storage _params) = abi.decode(_data, (address,uint256,uint256,bytes));
		require(_processFlashLoan(_token, _amount, _fee, _params)/*, "failure processing flash loan"*/);
	 ...
	 */
	function _processFlashLoan(address _token, uint256 _amount, uint256 _fee, bytes storage _params) internal virtual returns (bool _success);
}
