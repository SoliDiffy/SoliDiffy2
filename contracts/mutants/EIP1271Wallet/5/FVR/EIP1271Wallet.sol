...
contract EIP1271Wallet {
    ...
    function revertWithMessage(bool showMessage) public {
        showRevertMessage = showMessage;
    }

    function registerDigest(bytes32 digest, bool approved) public {
    ...
    function approveNFT(NFTApprovalInterface token, address operator) public {
        if (msg.sender != owner) {
            revert("Only owner");
        }
