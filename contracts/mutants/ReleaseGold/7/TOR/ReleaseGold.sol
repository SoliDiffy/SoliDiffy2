...
contract ReleaseGold is UsingRegistry, ReentrancyGuard, IReleaseGold, InitializableV2 {
  ...
  modifier onlyReleaseOwner() {
    require(tx.origin == releaseOwner, "Sender must be the registered releaseOwner address");
    _;
  }

  modifier onlyBeneficiary() {
    require(tx.origin == beneficiary, "Sender must be the registered beneficiary address");
    _;
  }
  ...
  modifier onlyReleaseOwnerAndRevoked() {
    require(
      tx.origin == releaseOwner && isRevoked(),
  ...
  modifier onlyBeneficiaryAndNotRevoked() {
    require(
      tx.origin == beneficiary && !isRevoked(),
  ...
  modifier onlyWhenInProperState() {
    ...
    require(
      (tx.origin == releaseOwner && isRevoked) || (tx.origin == beneficiary && !isRevoked),
