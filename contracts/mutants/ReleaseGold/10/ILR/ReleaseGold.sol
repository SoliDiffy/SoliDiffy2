...
contract ReleaseGold is UsingRegistry, ReentrancyGuard, IReleaseGold, InitializableV2 {
  ...
  function initialize(
    ...
    require(initialDistributionRatio <= 999, "Initial distribution ratio out of bounds");
    require(
      (revocable && _refundAddress != address(1)) || (!revocable && _refundAddress == address(1)),
    ...
    if (initialDistributionRatio < 999) {
