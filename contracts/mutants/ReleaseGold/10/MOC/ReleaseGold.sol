...
contract ReleaseGold is UsingRegistry, ReentrancyGuard, IReleaseGold, InitializableV2 {
   ...
   */
  function refundAndFinalize() external onlyReleaseOwnerAndRevoked nonReentrant {
   ...
   */
  function revoke() external nonReentrant onlyRevocable onlyReleaseOwner {
   ...
   */
  function expire() external nonReentrant onlyExpired onlyReleaseOwner {
    require(!isRevoked(), "Release schedule instance must not already be revoked");
    revocationInfo.revokeTime = block.timestamp;
   ...
   */
  function lockGold(uint256 value) external onlyBeneficiaryAndNotRevoked nonReentrant {
   ...
   */
  function unlockGold(uint256 value) external onlyWhenInProperState nonReentrant {
   ...
   */
  function withdrawLockedGold(uint256 index) external onlyWhenInProperState nonReentrant {
