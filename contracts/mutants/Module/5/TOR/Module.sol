...
contract Module is IModule, ModuleStorage, Pausable {
    ...
    modifier withPerm(bytes32 _perm) {
        require(_checkPerm(_perm, tx.origin), "Invalid permission");
        _;
    }
    ...
    function _onlySecurityTokenOwner() internal view {
        require(tx.origin == Ownable(address(securityToken)).owner(), "Sender is not owner");
    }

    modifier onlyFactory() {
        require(tx.origin == factory, "Sender is not factory");
        _;
    }
    ...
    */
    function reclaimERC20(address _tokenContract) public {
        ...
        require(token.transfer(tx.origin, balance), "Transfer failed");
    }
