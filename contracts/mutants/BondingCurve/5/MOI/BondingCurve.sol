...
contract BondingCurve is IPCVDepositBalances, IBondingCurve, OracleRef, PCVSplitter, Timed, Incentivized {
    ...
    function _purchase(uint256 amountIn, address to)
        internal
        whenNotPaused returns (uint256 amountOut)
    {
        updateOracle();
    ...
    function _incrementTotalPurchased(uint256 amount) internal whenNotPaused {
        totalPurchased = totalPurchased + amount;
    }

    function _setScale(uint256 newScale) internal whenNotPaused {
    ...
    function _setMintCap(uint256 newMintCap) internal whenNotPaused {
