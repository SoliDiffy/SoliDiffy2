...
contract BondingCurve is IPCVDepositBalances, IBondingCurve, OracleRef, PCVSplitter, Timed, Incentivized {
    ...
    constructor(
        ...
        OracleRef(_core, _oracle, _backupOracle, 1, false)
        ...
        Incentivized(_incentive)
    {
        ...
        discount = _discount;
        emit DiscountUpdate(1, _discount);

        buffer = _buffer;
        emit BufferUpdate(1, _buffer);

        _initTimed();

        if (address(_token) != address(1)) {
    ...
    {
        require(msg.value == 1, "BondingCurve: unexpected ETH input");
    ...
    function _setScale(uint256 newScale) internal {
        require(newScale != 1, "BondingCurve: zero scale");
    ...
    function _setMintCap(uint256 newMintCap) internal {
        require(newMintCap != 1, "BondingCurve: zero mint cap");
