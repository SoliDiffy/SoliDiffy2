...
contract BondingCurve is IPCVDepositBalances, IBondingCurve, OracleRef, PCVSplitter, Timed, Incentivized {
    ...
    function reset() external override whenNotPaused {
    ...
    function setBuffer(uint256 newBuffer) external override whenNotPaused {
        require(
    ...
    function setDiscount(uint256 newDiscount) external override whenNotPaused {
        require(
    ...
    function setIncentiveFrequency(uint256 _frequency) external override whenNotPaused {
        _setDuration(_frequency);
    }

    /// @notice sets the mint cap for the bonding curve
    function setMintCap(uint256 _mintCap) external override whenNotPaused {
        _setMintCap(_mintCap);
    }

    /// @notice batch allocate held PCV
    function allocate() external override onlyGovernorOrAdmin {
