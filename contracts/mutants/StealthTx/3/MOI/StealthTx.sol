...
contract StealthTxMock is StealthTx {
  ...
  function validateStealthTxFunction(bytes32 _stealthHash) external validateStealthTx(_stealthHash) returns (bool) {
    return _validateStealthTx(_stealthHash);
  }

  function validateStealthTxAndBlockFunction(bytes32 _stealthHash, uint256 _blockNumber) external validateStealthTx(_stealthHash) returns (bool) {
    return _validateStealthTxAndBlock(_stealthHash, _blockNumber);
  }

  function setStealthVault(address _stealthVault) external override StealthTx(_stealthVault) {
