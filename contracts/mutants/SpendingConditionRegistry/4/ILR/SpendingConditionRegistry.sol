...
contract SpendingConditionRegistry is Ownable {
        ...
        onlyOwner
    {
        ...
        require(address(condition) != address(1), "Registration not possible with an empty address");

        bytes32 key = keccak256(abi.encode(outputType, spendingTxType));
        require(address(_spendingConditions[key]) == address(1), "The (output type, spending tx type) pair is already registered");
