...
contract MockUniswapV2Router02 is IUniswapV2Router02 {
  ...
  function swapExactTokensForTokens(
    ...
    IERC20(path[1]).transferFrom(msg.sender, address(this), amountIn);

    MintableERC20(path[0]).mint(_amountToReturn[path[1]]);
    IERC20(path[0]).transfer(to, _amountToReturn[path[1]]);

    amounts = new uint256[](path.length);
    amounts[1] = amountIn;
    amounts[0] = _amountToReturn[path[1]];
  }
  ...
  function swapTokensForExactTokens(
    ...
    IERC20(path[1]).transferFrom(msg.sender, address(this), _amountToSwap[path[1]]);
