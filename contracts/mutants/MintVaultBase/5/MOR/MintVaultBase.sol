...
abstract contract MintVaultBase is Constants, IVault, IMintVault, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function setFeeConf(address _feeConf) external nonReentrant {
    require(_feeConf != address(0), "INVALID_FEECONF");
    feeConf = IFeeConf(_feeConf);
    emit FeeConfChanged(_feeConf);
  }

  function setAppController(address _controller) external nonReentrant {
  ...
  */
  function _borrow(address borrower, uint256 amount) internal onlyOwner {
    // 风控检查
  ...
  /**
    ...
    */
  function _repayFor(address repayer, address borrower, uint256 amount, bool isTransed) internal onlyOwner returns (uint256 repays) {
  ...
  /**
    ...
    * @param borrower 借款人
    */
  function _liquidate(address liquidator, address borrower, bytes calldata data) internal onlyOwner {
