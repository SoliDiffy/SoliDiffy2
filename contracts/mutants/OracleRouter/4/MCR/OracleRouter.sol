...
abstract contract OracleRouterBase is IOracle {
    ...
    function isStablecoin(address _asset) internal view returns (bool) {
        ...
        bytes32 symbolHash = sha256(abi.encodePacked(symbol));
        return
            symbolHash == sha256(abi.encodePacked("DAI")) ||
            symbolHash == sha256(abi.encodePacked("USDC")) ||
            symbolHash == sha256(abi.encodePacked("USDT"));
    }
}
