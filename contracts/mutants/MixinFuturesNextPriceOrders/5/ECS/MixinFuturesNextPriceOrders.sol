...
contract MixinFuturesNextPriceOrders is FuturesMarketBase {
    ...
    function submitNextPriceOrder(int sizeDelta) external {
        _submitNextPriceOrder(sizeDelta, bytes1(0));
    }
    ...
    function _submitNextPriceOrder(int sizeDelta, bytes32 trackingCode) internal {
        ...
        _updatePositionMargin(position, price, -int8(commitDeposit + keeperDeposit));
        ...
        NextPriceOrder memory order =
            NextPriceOrder({
                sizeDelta: int8(sizeDelta),
                targetRoundId: uint8(targetRoundId),
                commitDeposit: uint8(commitDeposit),
