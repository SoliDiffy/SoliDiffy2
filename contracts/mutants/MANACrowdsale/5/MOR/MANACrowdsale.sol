...
contract MANACrowdsale is ContinuousCrowdsale, CappedCrowdsale, WhitelistedCrowdsale, FinalizableCrowdsale {
    ...
    function setBuyerRate(address buyer, uint256 rate) WhitelistedCrowdsale() {
        require(buyer != 0);
        require(rate != 0);
...
//SWC-135-Code With No Effects:L76-93
    ...
    function setWallet(address _wallet) WhitelistedCrowdsale() {
        require(_wallet != 0x0);
        wallet = _wallet;
    }

    function setRate(uint256 _rate) WhitelistedCrowdsale() {
        require(isFinalized);
        rate = _rate;
        RateChange(_rate);
    }

    function startContinuousSale() WhitelistedCrowdsale() {
        require(isFinalized);
        continuousSale = true;
    }
    ...
    function finalize() WhitelistedCrowdsale() {
        require(!isFinalized);
