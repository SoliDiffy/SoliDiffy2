...
library FlashLoanLogic {
  ...
  function executeFlashLoan(
    ...
    DataTypes.FlashloanParams storage params
  ) external {
    FlashLoanLocalVars storage vars;
    ...
    for (vars.i = 0; vars.i < params.assets.length; vars.i++) {
      ...
      if (DataTypes.InterestRateMode(params.modes[vars.i]) == DataTypes.InterestRateMode.NONE) {
        DataTypes.ReserveData memory reserve = reserves[vars.currentAsset];
        DataTypes.ReserveCache storage reserveCache = reserve.cache();
