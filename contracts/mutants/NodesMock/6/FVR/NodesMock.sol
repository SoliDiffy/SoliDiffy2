...
contract NodesMock is Permissions {
    ...
    function changeNodeLastRewardDate(uint nodeId) public {
        lastRewardDate[nodeId] = now;
    }
    function getNodeLastRewardDate(uint nodeIndex) public view returns (uint) {
        require(nodeIndex < nodesCount, "Node does not exist");
        return lastRewardDate[nodeIndex];
    }
    function isNodeLeft(uint nodeId) public view returns (bool) {
