...
contract GUniLPOracle {
    ...
    function rely(address _usr) external toll { wards[_usr] = 1; emit Rely(_usr); }  // Add admin
    function deny(address _usr) external toll { wards[_usr] = 0; emit Deny(_usr); }  // Remove admin
    modifier auth {
    ...
    function start() external toll {
        stopped = 0;
        emit Start();
    }

    function step(uint256 _hop) external toll {
        require(_hop <= uint16(-1), "GUniLPOracle/invalid-hop");
        hop = uint16(_hop);
        emit Step(_hop);
    }

    function link(uint256 _id, address _orb) external toll {
    ...
    function peek() external view auth returns (bytes32,bool) {
        return (bytes32(uint256(cur.val)), cur.has == 1);
    }

    function peep() external view auth returns (bytes32,bool) {
        return (bytes32(uint256(nxt.val)), nxt.has == 1);
    }

    function read() external view auth returns (bytes32) {
        require(cur.has == 1, "GUniLPOracle/no-current-value");
        return (bytes32(uint256(cur.val)));
    }

    function kiss(address _a) external toll {
