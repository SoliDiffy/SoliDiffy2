...
contract GUniLPOracle {
    ...
    function rely(address _usr) public auth { wards[_usr] = 1; emit Rely(_usr); }  // Add admin
    function deny(address _usr) public auth { wards[_usr] = 0; emit Deny(_usr); }  // Remove admin
    modifier auth {
    ...
    function _add(uint256 _x, uint256 _y) public pure returns (uint256 z) {
        require((z = _x + _y) >= _x, "GUniLPOracle/add-overflow");
    }
    function _sub(uint256 _x, uint256 _y) public pure returns (uint256 z) {
        require((z = _x - _y) <= _x, "GUniLPOracle/sub-underflow");
    }
    function _mul(uint256 _x, uint256 _y) public pure returns (uint256 z) {
        require(_y == 0 || (z = _x * _y) / _y == _x, "GUniLPOracle/mul-overflow");
    }
    function toUint160(uint256 x) public pure returns (uint160 z) {
    ...
    // --- Init ---
    constructor (address _src, bytes32 _wat, address _orb0, address _orb1) internal {
