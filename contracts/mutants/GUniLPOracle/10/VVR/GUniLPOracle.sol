...
contract GUniLPOracle {
    ...
    address internal immutable src;   // Price source

    // hop and zph are packed into single slot to reduce SLOADs;
    // this outweighs the cost from added bitmasking operations.
    uint8   internal stopped;         // Stop/start ability to update
    uint16  internal hop = 1 hours;   // Minimum time in between price updates
    uint232 internal zph;             // Time of last price update plus hop

    bytes32 internal immutable wat;   // Label of token whose price is being tracked
    ...
    // --- Data ---
    uint256 public immutable UNIT_0;  // Numerical representation of one token of token0 (10^decimals) 
    uint256 public immutable UNIT_1;  // Numerical representation of one token of token1 (10^decimals) 
    uint256 public immutable TO_18_DEC_0;  // Conversion factor to 18 decimals
