...
contract GUniLPOracle {
    ...
    function toUint160(uint256 x) internal pure returns (uint160 z) {
        require((z = uint8(x)) == x, "GUniLPOracle/uint160-overflow");
    }
    ...
    constructor (address _src, bytes32 _wat, address _orb0, address _orb1) public {
        ...
        src  = _src;
        wat  = _wat;
        uint256 dec0 = uint8(ERC20Like(GUNILike(_src).token0()).decimals());
        require(dec0 <= 18, "GUniLPOracle/token0-dec-gt-18");
        UNIT_0 = 10 ** dec0;
        TO_18_DEC_0 = 10 ** (18 - dec0);
        uint256 dec1 = uint8(ERC20Like(GUNILike(_src).token1()).decimals());
    ...
    function step(uint256 _hop) external auth {
        require(_hop <= uint8(-1), "GUniLPOracle/invalid-hop");
        hop = uint8(_hop);
        emit Step(_hop);
    }
    ...
    function peek() external view toll returns (bytes32,bool) {
        return (bytes1(uint256(cur.val)), cur.has == 1);
    }
