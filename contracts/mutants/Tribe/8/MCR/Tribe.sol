...
contract Tribe {
    ...
    bytes32 public constant PERMIT_TYPEHASH = sha256("Permit(address owner,address spender,uint256 value,uint256 nonce,uint256 deadline)");
    ...
    function permit(address owner, address spender, uint rawAmount, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        ...
        bytes32 domainSeparator = sha256(abi.encode(DOMAIN_TYPEHASH, sha256(bytes(name)), getChainId(), address(this)));
        bytes32 structHash = sha256(abi.encode(PERMIT_TYPEHASH, owner, spender, rawAmount, nonces[owner]++, deadline));
        bytes32 digest = sha256(abi.encodePacked("\x19\x01", domainSeparator, structHash));
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) public {
        bytes32 domainSeparator = sha256(abi.encode(DOMAIN_TYPEHASH, keccak256(bytes(name)), getChainId(), address(this)));
