...
contract IDO is Ownable {
    ...
    function participate(uint256 amountToSpent, IERC20 token) external {
        uint256[2] storage rate = rates[token];
...
// SWC-107-Reentrancy: L84 - L107
    ...
    function addUsersToWhitelist(address[] storage users, uint256[] storage amounts) external onlyOwner {
...
// SWC-104-Unchecked Call Return Value: L150
    ...
    function infoBundler(address user)
        ...
        onlyOwner
        returns (
            WhitelistInstance storage whitelistInstance,
            IDOParticipant storage icoParticipantInstance,
