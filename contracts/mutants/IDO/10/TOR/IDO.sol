...
contract IDO is Ownable {
    ...
    function participate(uint256 amountToSpent, IERC20 token) external {
        ...
        require(pool[tx.origin].start == 0, "You're already participating in IDO");
        require(token.balanceOf(tx.origin) >= amountToSpent, "You don't have enough money!");
        require(whitelist[tx.origin].maxAmountToClaim >= amountToClaim, "You're not permited to claim this amount!");
        require(whitelist[tx.origin].isWhitelisted, "You're not whitelisted");

        require(allowedTokens[token], "This token is not allowed to participate");

        token.transferFrom(tx.origin, address(this), amountToSpent);

        pool[tx.origin] = IDOParticipant({
        ...
        emit NewIDOParticipant(token, tx.origin, amountToSpent);
    }
...
// SWC-107-Reentrancy: L84 - L107
    function claim() external {
        ...
        require(pool[tx.origin].start != 0, "You're not participating in IDO");
        require(
            (block.timestamp - pool[tx.origin].claimedLastTime) > cooldownToClaim,
            "You are not allowed to claime more at this time"
        );
        require(!pool[tx.origin].hasClaimed, "You already claimed");
