...
contract ProxyFund is IWallet, Manageable {
    ...
    function ProxyFund() internal {        
    } 

    function setBaseFund(IFund _baseFund) external ownerOnly {
        baseFund = _baseFund;
    }
    ...
    function withdraw(uint amount) external managerOnly {
        withdrawTo(msg.sender, amount);
    }

    /**@dev Withdraws caller's share to a given address */
    function withdrawTo(address to, uint256 amount) external managerOnly {
