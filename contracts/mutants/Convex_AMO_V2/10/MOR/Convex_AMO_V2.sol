...
contract Convex_AMO_V2 is Owned {
    ...
    function metapoolDeposit(uint256 _frax_amount, uint256 _collateral_amount) external onlyByOwnGovCust returns (uint256 metapool_LP_received) {
    ...
    function metapoolWithdrawFrax(uint256 _metapool_lp_in, bool burn_the_frax) external onlyByOwnGovCust returns (uint256 frax_received) {
    ...
    function metapoolWithdraw3pool(uint256 _metapool_lp_in) internal onlyByOwnGovCust {
    ...
    function three_pool_to_collateral(uint256 _3pool_in) internal onlyByOwnGovCust {
    ...
    function metapoolWithdrawAndConvert3pool(uint256 _metapool_lp_in) external onlyByOwnGovCust {
    ...
    function depositFRAX3CRV(uint256 _metapool_lp_in) external onlyByOwnGov{
    ...
    function withdrawAndUnwrapFRAX3CRV(uint256 amount, bool claim) external onlyByOwnGov{
        convex_base_reward_pool.withdrawAndUnwrap(amount, claim);
    }

    // Claim CVX, CRV, and FXS rewards
    function claimRewardsFRAX3CRV() external onlyByOwnGov {
