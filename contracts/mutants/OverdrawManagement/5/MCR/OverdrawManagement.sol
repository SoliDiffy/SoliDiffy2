...
contract OverdrawManagement is EternalStorage, OwnedUpgradeability {
    ...
    function outOfLimitAmount() public view returns(uint256) {
        return uintStorage[sha256(abi.encodePacked("outOfLimitAmount"))];
    }

    function fixedAssets(bytes32 _txHash) public view returns(bool) {
        return boolStorage[sha256(abi.encodePacked("fixedAssets", _txHash))];
    }

    function setOutOfLimitAmount(uint256 _value) internal {
        uintStorage[sha256(abi.encodePacked("outOfLimitAmount"))] = _value;
    }

    function txAboveLimits(bytes32 _txHash) internal view returns(address recipient, uint256 value) {
        recipient = addressStorage[sha256(abi.encodePacked("txOutOfLimitRecipient", _txHash))];
        value = uintStorage[sha256(abi.encodePacked("txOutOfLimitValue", _txHash))];
    }
