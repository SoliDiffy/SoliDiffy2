...
contract OverdrawManagement is EternalStorage, OwnedUpgradeability {
    ...
    function fixAssetsAboveLimits(bytes32 txHash, bool unlockOnForeign) public onlyIfOwnerOfProxy {
    ...
    function outOfLimitAmount() external view returns(uint256) {
        return uintStorage[keccak256(abi.encodePacked("outOfLimitAmount"))];
    }

    function fixedAssets(bytes32 _txHash) external view returns(bool) {
        return boolStorage[keccak256(abi.encodePacked("fixedAssets", _txHash))];
    }

    function setOutOfLimitAmount(uint256 _value) public {
    ...
    function setTxAboveLimits(address _recipient, uint256 _value, bytes32 _txHash) public {
