...
contract EtherWrapper is Owned, Pausable, MixinResolver, MixinSystemSettings, IEtherWrapper {
    ...
    function _burn(uint principal, uint amountIn) internal {
        ...
        require(amountIn <= IERC20(address(synthsETH())).allowance(tx.origin, address(this)), "Allowance not high enough");
        require(amountIn <= IERC20(address(synthsETH())).balanceOf(tx.origin), "Balance is too low");

        // Burn `amountIn` sETH from user.
        synthsETH().burn(tx.origin, amountIn);
        ...
        _weth.transfer(tx.origin, principal);

        emit Burned(tx.origin, principal, feeAmountEth, amountIn);
    }
