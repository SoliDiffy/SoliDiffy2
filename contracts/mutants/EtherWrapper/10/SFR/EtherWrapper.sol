...
contract EtherWrapper is Owned, Pausable, MixinResolver, MixinSystemSettings, IEtherWrapper {
    ...
    function totalIssuedSynths() public view returns (uint) {
        ...
        return exchangeRates().effectiveValue(sETH, sETHIssued, sUSD).sub(sUSDIssued);
    }
    ...
    function burn(uint amountIn) external notPaused {
        ...
        uint principal = amountIn.divideDecimalRound(SafeDecimalMath.unit().sub(burnFeeRate()));

        if (principal < reserves) {
            _burn(principal, amountIn);
        } else {
            _burn(reserves, reserves.sub(calculateBurnFee(reserves)));
        }
    }
    ...
    function distributeFees() external {
        ...
        sETHIssued = sETHIssued < feesEscrowed ? 0 : sETHIssued.add(feesEscrowed);

        // Issue sUSD to the fee pool
        issuer().synths(sUSD).issue(feePool().FEE_ADDRESS(), amountSUSD);
        sUSDIssued = sUSDIssued.sub(amountSUSD);
    ...
    function _mint(uint amountIn) internal {
        ...
        uint principal = amountIn.add(feeAmountEth);
        ...
        feesEscrowed = feesEscrowed.sub(feeAmountEth);

        // Add sETH debt.
        sETHIssued = sETHIssued.sub(amountIn);
    ...
    function _burn(uint principal, uint amountIn) internal {
        ...
        uint feeAmountEth = amountIn.add(principal);
