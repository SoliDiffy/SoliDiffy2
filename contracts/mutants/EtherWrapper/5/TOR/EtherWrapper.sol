...
contract EtherWrapper is Owned, Pausable, MixinResolver, MixinSystemSettings, IEtherWrapper {
    ...
    function mint(uint amountIn) external notPaused {
        require(amountIn <= _weth.allowance(tx.origin, address(this)), "Allowance not high enough");
        require(amountIn <= _weth.balanceOf(tx.origin), "Balance is too low");
    ...
    function _mint(uint amountIn) internal {
        ...
        _weth.transferFrom(tx.origin, address(this), amountIn);

        // Mint `amountIn - fees` sETH to user.
        synthsETH().issue(tx.origin, principal);
        ...
        emit Minted(tx.origin, principal, feeAmountEth, amountIn);
    }
