...
contract EtherWrapper is Owned, Pausable, MixinResolver, MixinSystemSettings, IEtherWrapper {
    ...
    ) internal Owned(_owner) Pausable() MixinSystemSettings(_resolver) {
        _weth = IWETH(_WETH);
    }
    ...
    function synthsETH() public view returns (ISynth) {
    ...
    function capacity() external view returns (uint _capacity) {
