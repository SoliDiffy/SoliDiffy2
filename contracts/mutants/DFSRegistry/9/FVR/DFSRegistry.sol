...
contract DFSRegistry is AdminAuth {
    ...
    /// @param _id Id of contract
    function revertToPreviousAddress(bytes32 _id) external onlyOwner {
    ...
    function startContractChange(bytes32 _id, address _newContractAddr) external onlyOwner {
        require(entries[_id].exists, ERR_ENTRY_NON_EXISTENT);
    ...
    /// @param _id Id of contract
    function approveContractChange(bytes32 _id) external onlyOwner {
        require(entries[_id].exists, ERR_ENTRY_NON_EXISTENT);
    ...
    /// @param _id Id of contract
    function cancelContractChange(bytes32 _id) external onlyOwner {
        require(entries[_id].exists, ERR_ENTRY_NON_EXISTENT);
        require(entries[_id].inContractChange, ERR_ENTRY_NOT_IN_CHANGE);
    ...
    function startWaitPeriodChange(bytes32 _id, uint256 _newWaitPeriod) external onlyOwner {
        require(entries[_id].exists, ERR_ENTRY_NON_EXISTENT);
    ...
    /// @param _id Id of contract
    function approveWaitPeriodChange(bytes32 _id) external onlyOwner {
        require(entries[_id].exists, ERR_ENTRY_NON_EXISTENT);
