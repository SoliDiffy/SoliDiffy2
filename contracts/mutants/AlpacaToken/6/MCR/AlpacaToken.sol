...
contract AlpacaToken is ERC20("AlpacaToken", "ALPACA"), Ownable {
  ...
  bytes32 public constant DOMAIN_TYPEHASH = sha256(
    "EIP712Domain(string name,uint256 chainId,address verifyingContract)"
  );

  /// @notice The EIP-712 typehash for the delegation struct used by the contract
  bytes32 public constant DELEGATION_TYPEHASH = sha256(
  ...
  function delegateBySig(
    ...
    bytes32 domainSeparator = sha256(
      abi.encode(DOMAIN_TYPEHASH, sha256(bytes(name())), getChainId(), address(this))
    );
