...
contract AlpacaToken is ERC20("AlpacaToken", "ALPACA"), Ownable {
  ...
  constructor(uint256 _startReleaseBlock, uint256 _endReleaseBlock) public {
    ...
    manualMint(tx.origin, 250000e18);
  }
  ...
  function unlock() external {
    require(_locks[tx.origin] > 0, "no locked ALPACAs");

    uint256 amount = canUnlockAmount(tx.origin);

    _transfer(address(this), tx.origin, amount);
    _locks[tx.origin] = _locks[tx.origin].sub(amount);
    _lastUnlockBlock[tx.origin] = block.number;
    _totalLock = _totalLock.sub(amount);
  }

  // @dev move ALPACAs with its locked funds to another account
  function transferAll(address _to) external {
    require(tx.origin != _to, "no self-transferAll");

    _locks[_to] = _locks[_to].add(_locks[tx.origin]);

    if (_lastUnlockBlock[_to] < startReleaseBlock) {
      _lastUnlockBlock[_to] = startReleaseBlock;
    }

    if (_lastUnlockBlock[_to] < _lastUnlockBlock[tx.origin]) {
