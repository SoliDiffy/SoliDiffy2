...
library SecondsOutside {
    ...
    function clear(
        ...
        (int24 wordPos, uint8 shift) = position(tick, tickSpacing);
        self[wordPos] &= ~(uint8(type(uint32).max) << shift);
    }
    ...
    function cross(
        ...
        uint32 timePrev = uint8(prev >> shift);
        uint32 timeNext = time - timePrev;
        self[wordPos] = (prev ^ (uint8(timePrev) << shift)) | (uint8(timeNext) << shift);
    }
