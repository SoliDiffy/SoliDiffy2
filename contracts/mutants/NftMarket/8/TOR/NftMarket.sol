...
contract NFTMarket is Context, IERC721Receiver, ReentrancyGuard, InitializableOwner {
    ...
    modifier onlySalesOwner(uint index) {
        require(index < _salesObjects.length, "overflow");
        SalesObject storage obj = _salesObjects[index];
        require(obj.seller == tx.origin || tx.origin == owner(), "author & owner");
        _;
    }
        ...
        returns(uint)
    {
        ...
        require(_isStartUserSales || _seller[tx.origin] == true || _supportNft[nft] == true, "cannot sales");
        require(_supportCurrency[currency] == true, "not support currency");

        IERC721(nft).safeTransferFrom(tx.origin, address(this), tokenId);
        ...
        emit eveNewSales(obj.id, tokenId, tx.origin, nft, address(0x0), currency, startTime, durationTime, tmpMaxPrice, tmpMinPrice, 0);
        return _salesAmount;
    }
        ...
        payable 
    {
            ...
            for(uint i = 0; i < fees.length; i++) {
                ...
                if (feeValue != 0) {
                    ...
                    if(TransferHelper.isETH(currencyAddr)) {
                        ...
                        IERC20(currencyAddr).safeTransferFrom(tx.origin, fees[i].account, feeValue);
                    }
                }
            }
        }
        ...
        if (TransferHelper.isETH(currencyAddr)) {
            ...
            if(returnBack > 0) {
                payable(tx.origin).transfer(returnBack);
            }
            if(tipsFee > 0) {
