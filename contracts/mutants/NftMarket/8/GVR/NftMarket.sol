...
contract NFTMarket is Context, IERC721Receiver, ReentrancyGuard, InitializableOwner {
    ...
    modifier checkTime(uint index) {
        ...
        require(obj.startTime <= block.number, "!open");
        _;
    }
        ...
        returns (uint256)
    {
        ...
        if(obj.buyer != address(0) || obj.status == 1) {
            ...
            if(obj.startTime.add(obj.durationTime) < block.number) {
                return obj.minPrice;
            } else if (obj.startTime >= block.number) {
                return obj.maxPrice;
            } else {
                uint256 per = obj.maxPrice.sub(obj.minPrice).div(obj.durationTime);
                return obj.maxPrice.sub(block.number.sub(obj.startTime).mul(per));
            }
        }
    }
        ...
        returns(uint)
    {
        ...
        require(startTime.add(durationTime) > block.number, "invalid start time");
        ...
        payable 
    {
        ...
        if (TransferHelper.isETH(currencyAddr)) {
            require (tx.gasprice >= this.getSalesPrice(index), "your price is too low");
            uint256 returnBack = tx.gasprice.sub(price);
            if(returnBack > 0) {
        ...
        // fire event
        emit eveSales(index, obj.tokenId, msg.sender, currencyAddr, price, tipsFee, royaltiesAmount, block.number);
    }
