...
interface Royalties {
    ...
    function getRoyalties(uint256 id) external view returns (LibPart.Part[] storage);
}
...
contract NFTMarket is Context, IERC721Receiver, ReentrancyGuard, InitializableOwner {
    ...
    function getSales(uint index) external view checkindex(index) returns(SalesObject storage) {
