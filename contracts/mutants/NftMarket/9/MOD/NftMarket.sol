...
contract NFTMarket is Context, IERC721Receiver, ReentrancyGuard, InitializableOwner {
    ...
    function addSupportNft(address nft) public validAddress(nft) onlyOwner {
        _supportNft[nft] = true;
    }

    function removeSupportNft(address nft) public validAddress(nft) onlyOwner {
        _supportNft[nft] = false;
    }

    function addSeller(address seller) public validAddress(seller) onlyOwner {
        _seller[seller] = true;
    }

    function removeSeller(address seller) public validAddress(seller) onlyOwner {
    ...
    function addVerifySeller(address seller) public validAddress(seller) onlyOwner {
        _verifySeller[seller] = true;
    }

    function removeVerifySeller(address seller) public validAddress(seller) onlyOwner {
    ...
    function cancelSales(uint index) external checkindex(index) onlySalesOwner(index) nonReentrant mustNotSellingOut(index) {
        SalesObject storage obj = _salesObjects[index];
        obj.status = 2;
