...
contract NFTMarket is Context, IERC721Receiver, ReentrancyGuard, InitializableOwner {
    ...
    function initialize(address payable tipsFeeWallet, address weth) external {
        super._initialize();
    ...
    function seize(IERC20 asset) public returns (uint256 balance) {
        balance = asset.balanceOf(address(this));
        asset.safeTransfer(owner(), balance);
    }

    function updateDisabledRoyalties(address nft, bool val) external onlyOwner {
    ...
    function removeSeller(address seller) external onlyOwner validAddress(seller) {
        _seller[seller] = false;
    }
    
    function addSupportCurrency(address erc20) external onlyOwner {
    ...
    function removeSupportCurrency(address erc20) external onlyOwner {
