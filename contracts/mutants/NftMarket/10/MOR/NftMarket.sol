...
contract NFTMarket is Context, IERC721Receiver, ReentrancyGuard, InitializableOwner {
    ...
    function updateDisabledRoyalties(address nft, bool val) public validAddress(nft) {
        _disabledRoyalties[nft] = val;
    }

    function addSupportNft(address nft) public nonReentrant validAddress(nft) {
        _supportNft[nft] = true;
    }

    function removeSupportNft(address nft) public nonReentrant validAddress(nft) {
        _supportNft[nft] = false;
    }

    function addSeller(address seller) public nonReentrant validAddress(seller) {
        _seller[seller] = true;
    }

    function removeSeller(address seller) public nonReentrant validAddress(seller) {
        _seller[seller] = false;
    }
    
    function addSupportCurrency(address erc20) public nonReentrant {
    ...
    function removeSupportCurrency(address erc20) public nonReentrant {
    ...
    function addVerifySeller(address seller) public nonReentrant validAddress(seller) {
        _verifySeller[seller] = true;
    }

    function removeVerifySeller(address seller) public nonReentrant validAddress(seller) {
        _verifySeller[seller] = false;
    }

    function setIsStartUserSales(bool isStartUserSales) public nonReentrant {
