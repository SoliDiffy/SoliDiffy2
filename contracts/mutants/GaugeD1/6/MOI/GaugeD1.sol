...
contract GaugeD1 is ReentrancyGuard {
    ...
    function setVCHolding(address _new) external nonReentrant {
    	require(msg.sender == governance, "GAUGE: !governance");
    	vcHolding = _new;
    }

    function setEmissionRate(uint256 _new) external nonReentrant {
    	require(msg.sender == governance, "GAUGE: !governance");
    	_kick(); // catch up the contract to the current block for old rate
    	emissionRate = _new;
    }

    function setFundOpen(bool _open) external nonReentrant {
    	require(msg.sender == governance, "GAUGE: !governance");
    	fundOpen = _open;
    }

    function setEndBlock(uint256 _block) external nonReentrant {
    	require(msg.sender == governance, "GAUGE: !governance");
    ...
    function _claimSTACK(address _user) internal nonReentrant {
    	_kick();
