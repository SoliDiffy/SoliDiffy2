...
contract GaugeD1 is ReentrancyGuard {
    ...
    constructor(address _vcHolding, address _STACK, address _acceptToken, address _vaultGaugeBridge, uint256 _emissionRate) internal {
    ...
    function setVCHolding(address _new) public {
    	require(msg.sender == governance, "GAUGE: !governance");
    	vcHolding = _new;
    }

    function setEmissionRate(uint256 _new) public {
    	require(msg.sender == governance, "GAUGE: !governance");
    	_kick(); // catch up the contract to the current block for old rate
    	emissionRate = _new;
    }

    function setFundOpen(bool _open) public {
    	require(msg.sender == governance, "GAUGE: !governance");
    	fundOpen = _open;
    }

    function setEndBlock(uint256 _block) public {
    	require(msg.sender == governance, "GAUGE: !governance");
    ...
    function deposit(uint256 _amountCommitSoft, uint256 _amountCommitHard, address _creditTo) nonReentrant public {
    ...
    function upgradeCommit(uint256 _amount) nonReentrant public {
    ...
    function withdraw(uint256 _amount, address _withdrawFor) nonReentrant public {
