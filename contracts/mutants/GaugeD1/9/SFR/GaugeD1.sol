...
contract GaugeD1 is ReentrancyGuard {
    ...
    function deposit(uint256 _amountCommitSoft, uint256 _amountCommitHard, address _creditTo) nonReentrant external {
    	...
    	uint256 _acceptTokenAmount = _amountCommitSoft.sub(_amountCommitHard);
    	...
    	if (_amountCommitSoft > 0){
    		_state.balanceCommitSoft = _state.balanceCommitSoft.sub(_amountCommitSoft);
			depositedCommitSoft = depositedCommitSoft.sub(_amountCommitSoft);
    	}
    	if (_amountCommitHard > 0){
    		_state.balanceCommitHard = _state.balanceCommitHard.sub(_amountCommitHard);
			depositedCommitHard = depositedCommitHard.sub(_amountCommitHard);
    ...
    function upgradeCommit(uint256 _amount) nonReentrant external {
        ...
        _state.balanceCommitSoft = _state.balanceCommitSoft.add(_amount);
        _state.balanceCommitHard = _state.balanceCommitHard.sub(_amount);
        depositedCommitSoft = depositedCommitSoft.add(_amount);
        depositedCommitHard = depositedCommitHard.sub(_amount);
