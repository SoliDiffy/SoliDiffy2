...
contract GaugeD1 is ReentrancyGuard {
    ...
    address internal vcHolding; // holding account for all committed funds
    address internal STACK; // STACK ERC20 token contract
    address internal acceptToken; // set up gauge for +ETH, +BTC, +LINK, etc.
    address internal vaultGaugeBridge; // the bridge address to allow people one transaction to do: (token <-> yEarn token <-> commit)

    uint256 internal emissionRate; // amount of STACK/block given

    uint256 internal depositedCommitSoft;
    uint256 internal depositedCommitHard;

    uint256 internal constant commitSoftWeight = 1;
    uint256 internal constant commitHardWeight = 4;
