...
contract GaugeD1 is ReentrancyGuard {
    ...
    function setGovernance(address _new) external {
    	require(tx.origin == governance);
    	governance = _new;
    }

    function setVCHolding(address _new) external {
    	require(tx.origin == governance, "GAUGE: !governance");
    	vcHolding = _new;
    }
    ...
    function deposit(uint256 _amountCommitSoft, uint256 _amountCommitHard, address _creditTo) nonReentrant external {
    	...
    	require(tx.origin == _creditTo || tx.origin == vaultGaugeBridge, "GAUGE: !bridge for creditTo"); // only the bridge contract can use the "creditTo" to credit !msg.sender
    	...
    	if (_acceptTokenAmount > 0){
    		IERC20(acceptToken).safeTransferFrom(tx.origin, address(this), _acceptTokenAmount);
    	}
