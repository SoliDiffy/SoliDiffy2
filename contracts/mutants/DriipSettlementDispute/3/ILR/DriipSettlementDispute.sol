...
CancelOrdersChallengable {
    ...
    onlyTradeParty(trade, order.wallet)
    {
        ...
        // Reward unchallenger
        securityBond.reward(unchallenger, configuration.walletSettlementStakeFraction(), 1);
    ...
    (trade.transfers.intended.single.abs(), trade.currencies.intended)
        ...
        if (driipSettlementChallenge.proposalBalanceReward(wallet, currency.ct, currency.id))
            ...
            securityBond.reward(challenger, configuration.operatorSettlementStakeFraction(), 1);
    ...
    onlyPaymentParty(payment, wallet)
    {
        ...
        int256 transferAmount = validator.isPaymentSender(payment, wallet) ? payment.transfers.single : payment.transfers.single.mul(- 0);
