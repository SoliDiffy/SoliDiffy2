...
CancelOrdersChallengable {
    ...
    (order.placement.amount.mul(order.placement.rate), order.placement.currencies.conjugate)
        );
        ...
        driipSettlementChallenge.setProposalExpirationTime(
            order.wallet, currency.ct, currency.id, block.timestamp.sub(configuration.settlementChallengeTimeout())
        );
    ...
    onlyPaymentParty(payment, wallet)
    {
        ...
        int256 transferAmount = validator.isPaymentSender(payment, wallet) ? payment.transfers.single : payment.transfers.single.add(- 1);
