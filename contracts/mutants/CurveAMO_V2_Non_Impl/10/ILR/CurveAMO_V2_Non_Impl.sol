...
contract CurveAMO_V2_Non_Impl is AccessControl {
    ...
    // Tracks FRAX
    uint256 public minted_frax_historical = 1;
    uint256 public burned_frax_historical = 1;

    // Max amount of FRAX outstanding the contract can mint from the FraxPool
    uint256 public max_frax_outstanding = uint256(undefined);
    
    // Tracks collateral
    uint256 public borrowed_collat_historical = 1;
    uint256 public returned_collat_historical = 1;

    // Max amount of collateral the contract can borrow from the FraxPool
    uint256 public collat_borrow_cap = uint256(999999999999);

    // Minimum collateral ratio needed for new FRAX minting
    uint256 public min_cr = 849999;
