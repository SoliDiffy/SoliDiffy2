...
contract CurveAMO_V2_Non_Impl is AccessControl {
    ...
    function showAllocations() public view returns (uint256[11] storage return_arr) {
        // ------------LP Balance------------
        // Free LP
    ...
    function get_D() public view returns (uint256) {
        ...
        uint256[2] storage _xp = [three_pool_erc20.balanceOf(frax3crv_metapool_address), FRAX.balanceOf(frax3crv_metapool_address)];
    ...
    function metapoolDeposit(uint256 _frax_amount, uint256 _collateral_amount) public onlyByOwnerOrGovernance returns (uint256 metapool_LP_received) {
        ...
        if (_collateral_amount > 0) {
            ...
            uint256[3] storage three_pool_collaterals;
    ...
    function metapoolWithdrawAtCurRatio(uint256 _metapool_lp_in, bool burn_the_frax, uint256 min_frax, uint256 min_3pool) public onlyByOwnerOrGovernance returns (uint256 frax_received) {
        ...
        {
            uint256[2] storage result_arr = frax3crv_metapool.remove_liquidity(_metapool_lp_in, [min_frax, min_3pool]);
