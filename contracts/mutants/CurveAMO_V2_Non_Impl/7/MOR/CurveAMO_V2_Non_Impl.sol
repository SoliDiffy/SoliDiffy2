...
contract CurveAMO_V2_Non_Impl is AccessControl {
    ...
    function mintRedeemPart1(uint256 frax_amount) public onlyCustodian {
    ...
    function mintRedeemPart2() public onlyCustodian {
        pool.collectRedemption();
    }

    // Give USDC profits back
    function giveCollatBack(uint256 amount) public onlyCustodian {
    ...
    function burnFRAX(uint256 frax_amount) public onlyCustodian {
        FRAX.burn(frax_amount);
        burned_frax_historical = burned_frax_historical.add(frax_amount);
    }
   
    function burnFXS(uint256 amount) public onlyCustodian {
        FXS.approve(address(this), amount);
        FXS.pool_burn_from(address(this), amount);
    }

    function metapoolDeposit(uint256 _frax_amount, uint256 _collateral_amount) public onlyCustodian returns (uint256 metapool_LP_received) {
    ...
    function metapoolWithdrawAtCurRatio(uint256 _metapool_lp_in, bool burn_the_frax, uint256 min_frax, uint256 min_3pool) public onlyCustodian returns (uint256 frax_received) {
