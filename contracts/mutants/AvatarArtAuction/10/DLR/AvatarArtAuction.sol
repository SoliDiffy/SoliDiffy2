...
contract AvatarArtAuction is AvatarArtBase, IAvatarArtAuction{
     ...
     */ 
    function getActiveAuctionByTokenId(uint256 tokenId) public view returns(bool, Auction storage, uint256){
        for(uint256 index = _auctions.length; index > 0; index--){
            Auction storage auction = _auctions[index - 1];
     ...
     */
    function getAuction(uint256 auctionIndex) external view returns(Auction storage){
        require(auctionIndex < getAuctionCount());
     ...
     */ 
    function getAuctionWinnersByTokenId(uint256 tokenId) public view returns(Auction[] storage){
        ...
        Auction[] storage result = new Auction[](resultCount);
        resultCount = 0;
        for(uint256 index = 0; index < _auctions.length; index++){
            Auction storage auction = _auctions[index];
            if(auction.tokenId == tokenId && auction.status == EAuctionStatus.Completed){
