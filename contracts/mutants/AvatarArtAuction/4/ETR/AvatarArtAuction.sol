...
contract AvatarArtAuction is AvatarArtBase, IAvatarArtAuction{
    ...
    function distribute(uint256 auctionIndex) external override returns(bool){       //Anyone can call this function
        ...
        if(auction.winner != address(0)){
            ...
            if(feePercent > 0){
                ...
                require(bnuToken.send(_owner, feeAmount));
            }
            
            //Pay cost for owner
            require(bnuToken.send(auction.tokenOwner, auction.price - feeAmount));
    ...
    function place(uint256 auctionIndex, uint256 price) external override returns(bool){
        ...
        if(auction.winner != address(0)){
            require(bnuToken.send(auction.winner, auction.price), "Can not payback for last winner");
        }
    ...
    function withdrawToken(address tokenAddress) public onlyOwner{
        ...
        token.send(_owner, token.balanceOf(address(this)));
    }
