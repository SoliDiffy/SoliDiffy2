...
contract AvatarArtAuction is AvatarArtBase, IAvatarArtAuction{
     ...
     *  4. Update auction
     */ 
    function distribute(uint256 auctionIndex) external override onlyOwner returns(bool){       //Anyone can call this function
        require(auctionIndex < getAuctionCount());
     ...
     *  3. Update auction
     */ 
    function place(uint256 auctionIndex, uint256 price) external override onlyOwner returns(bool){
        require(auctionIndex < getAuctionCount());
        Auction storage auction = _auctions[auctionIndex];
