...
contract Hermez is InstantWithdrawManager {
    ...
    function withdrawCircuit(
        ...
        uint256 input = uint256(
            sha256(abi.encodePacked(exitRoot, tx.origin, tokenID, amount, idx))
        ) % _RFIELD;
    ...
    function addToken(address tokenAddress, bytes calldata permit) public {
        ...
        _safeTransferFrom(tokenHEZ, tx.origin, address(this), feeAddToken);
    ...
    function _withdrawFunds(
        ...
        if (instantWithdraw) {
            _safeTransfer(tokenList[tokenID], tx.origin, amount);
        } else {
            if (tokenID == 0) {
