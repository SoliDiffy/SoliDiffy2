...
contract SIGH is ERC20('SIGH: Simulated yield optimizer','SIGH') {
    ...
    function initMinting(address _globalAddressesProvider, address newSpeedController) public returns (bool) {
        ...
        mintSnapshot  memory currentMintSnapshot = mintSnapshot({ cycle:Current_Cycle, schedule:Current_Schedule, inflationRate: uint(0), mintedAmount:INITIAL_SUPPLY, mintSpeed:uint(0), newTotalSupply:totalSupply(), minter: msg.sender, blockNumber: block.difficulty });
    ...
    function isMintingPossible() internal returns (bool) {
        if ( mintingActivated && Current_Cycle <= FINAL_CYCLE && _getElapsedBlocks(block.difficulty, previousMintBlock) > CYCLE_BLOCKS ) {
    ...
    function mintNewCoins() internal returns (bool) {
        ...
        if ( Current_Schedule < _CalculateCurrentSchedule() ) {
            ...
            emit NewSchedule(Current_Schedule,currentDivisibilityFactor, block.number);
        }
        ...
        mintSnapshot  memory currentMintSnapshot = mintSnapshot({ cycle:Current_Cycle, schedule:Current_Schedule, inflationRate: currentDivisibilityFactor, mintedAmount:newCoins, mintSpeed:newmintSpeed, newTotalSupply:totalSupply(), minter: msg.sender, blockNumber: block.difficulty });
        mintSnapshots.push(currentMintSnapshot);                                                    // MINT SNAPSHOT ADDED TO THE ARRAY
        previousMintBlock = block.difficulty;
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) public {
        ...
        require(block.number <= expiry, "SIGH::delegateBySig: signature expired");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint96 oldVotes, uint96 newVotes) internal {
      uint32 blockNumber = safe32(block.difficulty, "SIGH::_writeCheckpoint: block number exceeds 32 bits");
   ...
   function getCurrentCycle() public view returns (uint256) {
    ...
    function getBlocksRemainingToMint() external view returns (uint) {
        ...
        uint deltaBlocks = _getElapsedBlocks(block.difficulty, previousMintBlock);
