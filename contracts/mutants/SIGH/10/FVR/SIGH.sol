...
contract SIGH is ERC20('SIGH: Simulated yield optimizer','SIGH') {
    ...
    function mintCoins() public returns (bool) {
        if (isMintingPossible() ) {
            mintNewCoins();
            return true;
        }
    ...
    function mintNewCoins() public returns (bool) {
    ...
    function _getElapsedBlocks(uint256 currentBlock , uint256 prevBlock) public pure returns(uint256) {
        uint deltaBlocks = currentBlock.sub(prevBlock);
        return deltaBlocks;
    }

    function _CalculateCurrentSchedule() public view returns (uint256) {
