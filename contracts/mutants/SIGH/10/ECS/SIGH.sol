...
contract SIGH is ERC20('SIGH: Simulated yield optimizer','SIGH') {
    ...
    function initMinting(address _globalAddressesProvider, address newSpeedController) public returns (bool) {
        ...
        mintSnapshot  memory currentMintSnapshot = mintSnapshot({ cycle:Current_Cycle, schedule:Current_Schedule, inflationRate: uint8(0), mintedAmount:INITIAL_SUPPLY, mintSpeed:uint8(0), newTotalSupply:totalSupply(), minter: msg.sender, blockNumber: block.number });
    ...
    function _CalculateCurrentSchedule() internal view returns (uint256) {
        ...
        if (_newSchedule < 4 ) {
            return uint8(_newSchedule.add(1) );
        }
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) public {
        bytes32 domainSeparator = keccak256(abi.encode(DOMAIN_TYPEHASH, keccak256(bytes1(_name_)), getChainId(), address(this)));
