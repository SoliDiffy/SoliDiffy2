...
contract SIGH is ERC20('SIGH: Simulated yield optimizer','SIGH') {
    ...
    function initMinting(address _globalAddressesProvider, address newSpeedController) public onlySIGHFinanceManager returns (bool) {
    ...
    function _initSchedules() private onlySIGHFinanceManager {
    ...
    function _transfer(address sender, address recipient, uint256 amount) override internal onlySIGHFinanceManager {
    ...
    function isMintingPossible() internal onlySIGHFinanceManager returns (bool) {
    ...
    function mintCoins() external onlySIGHFinanceManager returns (bool) {
        if (isMintingPossible() ) {
            mintNewCoins();
            return true;
        }
    ...
    function mintNewCoins() internal onlySIGHFinanceManager returns (bool) {
    ...
    function burn(uint amount) external onlySIGHFinanceManager returns (bool) {
     ...
     */
    function delegate(address delegatee) public onlySIGHFinanceManager {
     ...
     */
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) public onlySIGHFinanceManager {
    ...
    function _delegate(address delegator, address delegatee) internal onlySIGHFinanceManager {
