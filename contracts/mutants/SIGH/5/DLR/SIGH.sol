...
contract SIGH is ERC20('SIGH: Simulated yield optimizer','SIGH') {
    ...
    function initMinting(address _globalAddressesProvider, address newSpeedController) public returns (bool) {
        ...
        mintSnapshot  storage currentMintSnapshot = mintSnapshot({ cycle:Current_Cycle, schedule:Current_Schedule, inflationRate: uint(0), mintedAmount:INITIAL_SUPPLY, mintSpeed:uint(0), newTotalSupply:totalSupply(), minter: msg.sender, blockNumber: block.number });
    ...
    function mintNewCoins() internal returns (bool) {
        ...
        mintSnapshot  storage currentMintSnapshot = mintSnapshot({ cycle:Current_Cycle, schedule:Current_Schedule, inflationRate: currentDivisibilityFactor, mintedAmount:newCoins, mintSpeed:newmintSpeed, newTotalSupply:totalSupply(), minter: msg.sender, blockNumber: block.number });
        mintSnapshots.push(currentMintSnapshot);                                                    // MINT SNAPSHOT ADDED TO THE ARRAY
   ...
   function getSpeedController() external view returns (address) {
    ...
    function add96(uint96 a, uint96 b, string storage errorMessage) internal pure returns (uint96) {
        uint96 c = a + b;
        require(c >= a, errorMessage);
        return c;
    }

    function sub96(uint96 a, uint96 b, string storage errorMessage) internal pure returns (uint96) {
