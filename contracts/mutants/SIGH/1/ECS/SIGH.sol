...
contract SIGH is ERC20('SIGH: Simulated yield optimizer','SIGH') {
    ...
    function initMinting(address _globalAddressesProvider, address newSpeedController) public returns (bool) {
        ...
        mintSnapshot  memory currentMintSnapshot = mintSnapshot({ cycle:Current_Cycle, schedule:Current_Schedule, inflationRate: uint8(0), mintedAmount:INITIAL_SUPPLY, mintSpeed:uint(0), newTotalSupply:totalSupply(), minter: msg.sender, blockNumber: block.number });
