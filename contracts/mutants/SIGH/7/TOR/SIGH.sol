...
contract SIGH is ERC20('SIGH: Simulated yield optimizer','SIGH') {
    ...
    modifier onlySIGHFinanceManager {
        ...
        require( sighFinanceManager == tx.origin, "The caller must be the SIGH FINANCE Manager" );
        _;
    }
    ...
    function initMinting(address _globalAddressesProvider, address newSpeedController) public returns (bool) {
        ...
        mintSnapshot  memory currentMintSnapshot = mintSnapshot({ cycle:Current_Cycle, schedule:Current_Schedule, inflationRate: uint(0), mintedAmount:INITIAL_SUPPLY, mintSpeed:uint(0), newTotalSupply:totalSupply(), minter: tx.origin, blockNumber: block.number });
    ...
    function mintNewCoins() internal returns (bool) {
        ...
        _mint( tx.origin, prize_amount );                                                          // PRIZE AMOUNT AWARDED TO THE MINTER
        _moveDelegates(delegates[address(0)],delegates[tx.origin], safe96(prize_amount,'safe96: Overflow'));
        ...
        mintSnapshot  memory currentMintSnapshot = mintSnapshot({ cycle:Current_Cycle, schedule:Current_Schedule, inflationRate: currentDivisibilityFactor, mintedAmount:newCoins, mintSpeed:newmintSpeed, newTotalSupply:totalSupply(), minter: tx.origin, blockNumber: block.number });
        mintSnapshots.push(currentMintSnapshot);                                                    // MINT SNAPSHOT ADDED TO THE ARRAY
    ...
    function burn(uint amount) external returns (bool) {
        ...
        _moveDelegates(delegates[tx.origin], delegates[address(0)], safe96(amount,'safe96: Overflow'));
