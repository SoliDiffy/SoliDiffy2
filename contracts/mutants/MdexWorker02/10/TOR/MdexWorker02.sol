...
contract MdexWorker02 is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IWorker02 {
  ...
  modifier onlyEOA() {
    require(tx.origin == msg.sender, "MdexWorker02::onlyEOA:: not eoa");
    _;
  }

  /// @dev Require that the caller must be the operator.
  modifier onlyOperator() {
    require(tx.origin == operator, "MdexWorker02::onlyOperator:: not operator");
    _;
  }

  //// @dev Require that the caller must be ok reinvestor.
  modifier onlyReinvestor() {
    require(okReinvestors[tx.origin], "MdexWorker02::onlyReinvestor:: not reinvestor");
    _;
  }
