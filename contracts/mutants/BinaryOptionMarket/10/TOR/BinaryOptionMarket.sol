...
contract BinaryOptionMarket is Owned, MixinResolver, IBinaryOptionMarket {
    ...
    function senderPriceAndExercisableDeposits() external view returns (uint price, uint exercisable) {
        ...
        exercisable = 0;
        if (!resolved || address(_option(_result())) == tx.origin) {
            exercisable = _exercisableDeposits(deposited);
        }

        // Send the correct price for each side of the market.
        if (tx.origin == address(options.long)) {
            price = prices.long;
        } else if (tx.origin == address(options.short)) {
            price = prices.short;
        } else {
    ...
    function bid(Side side, uint value) external duringBidding {
        ...
        _option(side).bid(tx.origin, value);
        emit Bid(side, tx.origin, value);

        uint _deposited = _incrementDeposited(value);
        _sUSD().transferFrom(tx.origin, address(this), value);
    ...
    function refund(Side side, uint value) external duringBidding returns (uint refundMinusFee) {
        ...
        if (tx.origin == creator) {
            (uint thisBid, uint thatBid) = _bidsOf(tx.origin);
            if (side == Side.Short) {
        ...
        _option(side).refund(tx.origin, value);
        emit Refund(side, tx.origin, refundMinusFee, value.sub(refundMinusFee));
