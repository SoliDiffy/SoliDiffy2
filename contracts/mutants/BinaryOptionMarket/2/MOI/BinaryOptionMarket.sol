...
contract BinaryOptionMarket is Owned, MixinResolver, IBinaryOptionMarket {
    ...
    function _incrementDeposited(uint value) internal duringBidding returns (uint _deposited) {
        _deposited = deposited.add(value);
        deposited = _deposited;
        _manager().incrementTotalDeposited(value);
    }

    function _decrementDeposited(uint value) internal duringBidding returns (uint _deposited) {
