...
contract BinaryOptionMarket is Owned, MixinResolver, IBinaryOptionMarket {
    ...
    function _computePrices(
        ...
        return (shortBids.divideDecimalRound(optionsPerSide), longBids.divideDecimalRound(optionsPerSide));
    }

    function senderPriceAndExercisableDeposits() external view returns (uint exercisable, uint price) {
    ...
    function _bidsOf(address account) internal view returns (uint long, uint short) {
        return (options.short.bidOf(account), options.long.bidOf(account));
    }
