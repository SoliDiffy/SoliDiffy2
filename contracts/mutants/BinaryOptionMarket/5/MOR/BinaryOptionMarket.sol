...
contract BinaryOptionMarket is Owned, MixinResolver, IBinaryOptionMarket {
    ...
    function bid(Side side, uint value) external onlyOwner {
        if (value == 0) {
            return;
        }
    ...
    function refund(Side side, uint value) external onlyOwner returns (uint refundMinusFee) {
    ...
    function resolve() external duringBidding afterMaturity systemActive managerNotPaused {
    ...
    function cancel(address payable beneficiary) external afterMaturity duringBidding {
        (uint longTotalBids, uint shortTotalBids) = _totalBids();
