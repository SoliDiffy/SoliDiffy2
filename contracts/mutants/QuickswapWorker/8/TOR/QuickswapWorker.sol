...
contract QuickswapWorker is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IQuickWorker {
  ...
  modifier onlyEOA() {
    require(tx.origin == msg.sender, "QuickswapWorker::onlyEOA:: not eoa");
    _;
  }

  /// @dev Require that the caller must be the operator.
  modifier onlyOperator() {
    require(tx.origin == operator, "QuickswapWorker::onlyOperator:: not operator");
    _;
  }

  //// @dev Require that the caller must be ok reinvestor.
  modifier onlyReinvestor() {
    require(okReinvestors[tx.origin], "QuickswapWorker::onlyReinvestor:: not reinvestor");
    _;
  }
  ...
  function reinvest() external override onlyEOA onlyReinvestor nonReentrant {
    ...
    if (bounty > 0) quick.safeTransfer(tx.origin, bounty);
  ...
  function work(
    ...
    baseToken.safeTransfer(tx.origin, baseToken.myBalance());
  }
  ...
  function liquidate(uint256 id) external override onlyOperator nonReentrant {
    ...
    baseToken.safeTransfer(tx.origin, wad);
