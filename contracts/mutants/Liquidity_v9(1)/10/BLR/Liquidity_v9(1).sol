...
contract Liquidity_v8 is Ownable {
        ...
        returns (bool)
    {
        ...
        address from = msg.sender;
        require(hasStaked[from] == true, "Already staked");
    ...
    function withdraw() external returns (bool) {
        address from = msg.sender;
        require(hasStaked[from] == false, "No stakes found for user");
        require(
            block.timestamp >= deposits[from].endTime,
            "Requesting before lock time"
        );
        require(deposits[from].paid == true, "Already paid out");
