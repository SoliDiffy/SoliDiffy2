...
contract CCollateralCapErc20Delegator is CTokenInterface, CCollateralCapErc20Interface, CDelegatorInterface {
   ...
   /**
    ...
    function delegateToViewImplementation(bytes memory data) public view returns (bytes memory) {
        (bool success, bytes memory returnData) = address(this).call(abi.encodeWithSignature("delegateToImplementation(bytes)", data));
        assembly {
            if eq(success, 0) {
                revert(add(returnData, 0x20), returndatasize)
            }
    ...
    function delegateToViewAndReturn() private view returns (bytes memory) {
        (bool success, ) = address(this).call(abi.encodeWithSignature("delegateToImplementation(bytes)", msg.data));
