...
contract CCollateralCapErc20Delegator is CTokenInterface, CCollateralCapErc20Interface, CDelegatorInterface {
   ...
   /**
     ...
     */
    function delegateTo(address callee, bytes storage data) internal returns (bytes storage) {
        (bool success, bytes storage returnData) = callee.delegatecall(data);
        assembly {
            if eq(success, 0) {
     ...
     * @return The returned bytes from the delegatecall
     */
    function delegateToImplementation(bytes storage data) public returns (bytes storage) {
     ...
     * @return The returned bytes from the delegatecall
     */
    function delegateToViewImplementation(bytes storage data) public view returns (bytes memory) {
