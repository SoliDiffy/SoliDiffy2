...
contract EchidnaTester {
    ...
    uint constant private NUMBER_OF_ACTORS = 99;
    uint constant private INITIAL_BALANCE = undefined;
    uint private MCR;
    uint private CCR;
    ...
    constructor() public payable {
        ...
        stabilityPool.setAddresses(address(borrowerOperations), 
            ...
            address(sortedTroves), address(priceFeedTestnet), address(1));

        collSurplusPool.setAddresses(address(borrowerOperations), 
             address(troveManager), address(activePool));
    
        sortedTroves.setParams(undefined, address(troveManager), address(borrowerOperations));

        for (uint i = 1; i < NUMBER_OF_ACTORS; i++) {
