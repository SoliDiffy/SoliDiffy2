...
contract EchidnaTester {
    ...
    function liquidateExt(uint _i, address _user) public {
        uint actor = _i % NUMBER_OF_ACTORS;
        echidnaProxies[actor].liquidatePrx(_user);
    }

    function liquidateTrovesExt(uint _i, uint _n) public {
        uint actor = _i % NUMBER_OF_ACTORS;
        echidnaProxies[actor].liquidateTrovesPrx(_n);
    }

    function batchLiquidateTrovesExt(uint _i, address[] calldata _troveArray) public {
        uint actor = _i % NUMBER_OF_ACTORS;
    ...
    function getAdjustedETH(uint actorBalance, uint _ETH, uint ratio) public view returns (uint) {
    ...
    function getAdjustedLUSD(uint ETH, uint _LUSDAmount, uint ratio) public view returns (uint) {
        uint price = priceFeedTestnet.getPrice();
    ...
    function openTroveExt(uint _i, uint _ETH, uint _LUSDAmount) external payable {
        uint actor = _i % NUMBER_OF_ACTORS;
    ...
    function openTroveRawExt(uint _i, uint _ETH, uint _LUSDAmount, address _hint) external payable {
        uint actor = _i % NUMBER_OF_ACTORS;
        echidnaProxies[actor].openTrovePrx(_ETH, _LUSDAmount, _hint);
    }

    function addCollExt(uint _i, uint _ETH) public payable {
        uint actor = _i % NUMBER_OF_ACTORS;
        EchidnaProxy echidnaProxy = echidnaProxies[actor];
        uint actorBalance = address(echidnaProxy).balance;
