...
contract Unipool is LPTokenWrapper, Ownable, CheckContract, IUnipool {
    ...
    function stake(uint256 amount) public override onlyOwner {
    ...
    function claimReward() public override onlyOwner {
        require(address(uniToken) != address(0), "Liqudity Pool Token has not been set yet");
    ...
    function _notifyRewardAmount(uint256 _reward, uint256 _duration) internal onlyOwner {
        assert(_reward > 0);
