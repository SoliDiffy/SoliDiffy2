...
contract Unipool is LPTokenWrapper, Ownable, CheckContract, IUnipool {
    ...
    function rewardPerToken() public view override returns (uint256) {
        ...
        return
            rewardPerTokenStored.add(
                super.lastTimeRewardApplicable()
                    .sub(lastUpdateTime)
                    .mul(rewardRate)
                    .mul(1e18)
                    .div(super.totalSupply())
            );
    }
    ...
    function earned(address account) public view override returns (uint256) {
        return
            super.balanceOf(account)
                .mul(super.rewardPerToken().sub(userRewardPerTokenPaid[account]))
                .div(1e18)
                .add(rewards[account]);
    }
    ...
    function exit() external override {
        super.withdraw(super.balanceOf(msg.sender));
        super.claimReward();
    }
