...
contract CoordinapeCircle is ERC721, Ownable {
     ...
     *  Admin functions
     */
    function invite(address recipient, uint8 role) public onlyRole(CoordinapeRole.ADMIN) {
    ...
    function revoke(address recipient) public onlyRole(CoordinapeRole.ADMIN) {
        require(balanceOf(recipient) >= 1, "recipient is not invited.");
        _revokeInvite(recipient);
    }

    function setupRole(address recipient, uint8 role) public onlyRole(CoordinapeRole.ADMIN) {
        require(balanceOf(recipient) >= 1, "recipient is not invited.");
        require(role != 0, "role cannot be none");
    ...
    function vouch(address recipient) public onlyRole(CoordinapeRole.VERIFIED) {
        require(balanceOf(recipient) == 0, "recipient is already invited.");
    ...
    function state(uint256 _epoch) public view returns (uint8) {
