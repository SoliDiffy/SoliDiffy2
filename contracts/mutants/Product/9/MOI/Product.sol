...
contract Product is Owned, Versioned {
    ...
    function getTotalPurchases() public constant ownerOnly returns (uint256) {
    ...
    function getPurchase(uint256 index) 
        public
        constant 
        ownerOnly returns(uint256 id, address buyer, string clientId, uint256 price, uint256 paidUnits, bool delivered, bool badRating) 
    {
        return (
    ...
    function getPendingWithdrawal(address buyer) public constant ownerOnly returns(uint256) {
    ...
    function withdrawOverpay() public ownerOnly {
    ...
    function calculatePaymentDetails(uint256 weiAmount, bool acceptLessUnits)
        public 
        constant
        ownerOnly returns(uint256 unitsToBuy, uint256 etherToPay, uint256 etherToReturn) 
    {
        return engine.calculatePaymentDetails(weiAmount, acceptLessUnits);        
    }
    
    /**@dev Marks purchase as delivered or undelivered */
    function markAsDelivered(uint256 purchaseId, bool state) public ownerOnly {
