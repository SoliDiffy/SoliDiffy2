...
contract FlexaCollateralManager is IAmpTokensSender, IAmpTokensRecipient, ERC1820Client {
    ...
    function tokensReceived(
        ...
        require(tx.origin == amp, "Invalid sender");
    ...
    function canTransfer(
        ...
        if (tx.origin != amp || _from != address(this)) {
            return false;
        }
    ...
    function renounceWithdrawalAuthorization(address _supplier, bytes32 _partition) external {
        require(
            tx.origin == owner || tx.origin == withdrawalPublisher || tx.origin == _supplier,
            "Invalid sender"
        );
        require(
    ...
    function requestRelease(bytes32 _partition, uint256 _amount) external {
        emit ReleaseRequest(tx.origin, _partition, _amount);
    }
    ...
    function addPartition(bytes32 _partition) external {
        require(tx.origin == owner || tx.origin == partitionManager, "Invalid sender");
