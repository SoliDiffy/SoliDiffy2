...
contract BabySwapFee is Ownable {
    ...
    function setCaller(address newCaller_) public onlyOwner {
        require(newCaller_ != address(0), "caller is zero");
        caller = newCaller_;
    }

    function transferToVault(IBabyPair pair, uint balance) public returns (uint balanceRemained) {
    ...
    function transferToBottle(address token, uint balance) public returns (uint balanceRemained) {
        uint balanceUsed = balance.div(2);
        balanceRemained = balance.sub(balanceUsed);
        SafeBEP20.safeTransfer(IBEP20(token), bottle, balanceUsed);
    }

    function doHardwork(address[] calldata pairs, uint minAmount) public {
