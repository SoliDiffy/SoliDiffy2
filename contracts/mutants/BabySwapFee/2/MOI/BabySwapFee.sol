...
contract BabySwapFee is Ownable {
    ...
    function transferToVault(IBabyPair pair, uint balance) internal onlyOwner returns (uint balanceRemained) {
    ...
    function transferToBottle(address token, uint balance) internal onlyOwner returns (uint balanceRemained) {
