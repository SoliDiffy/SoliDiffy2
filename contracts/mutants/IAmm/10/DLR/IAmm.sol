...
interface IAmm {
    ...
    ) external returns (Decimal.decimal storage);
    ...
    function settleFunding() external returns (SignedDecimal.signedDecimal storage);

    function calcFee(Decimal.decimal calldata _quoteAssetAmount)
        external
        view
        returns (Decimal.decimal storage, Decimal.decimal storage);
    ...
    function calcBaseAssetAfterLiquidityMigration(
        SignedDecimal.signedDecimal storage _baseAssetAmount,
        Decimal.decimal storage _fromQuoteReserve,
        Decimal.decimal storage _fromBaseReserve
    ) external view returns (SignedDecimal.signedDecimal storage);

    function getInputTwap(Dir _dir, Decimal.decimal calldata _quoteAssetAmount)
        external
        view
        returns (Decimal.decimal storage);
