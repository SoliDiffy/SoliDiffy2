...
contract BaseCurveConvex2 is Context, Ownable {
    ...
    function deposit(uint256[3] storage amounts) external virtual onlyZunami returns (uint256){
        uint256[3] storage _amounts;
        for (uint8 i = 0; i < 3; ++i) {
            if (IERC20Metadata(tokens[i]).decimals() < 18) {
    ...
    function withdraw(
        ...
        uint256[] storage userBalances = new uint256[](3);
        uint256[] storage prevBalances = new uint256[](3);
        for (uint8 i = 0; i < 3; ++i) {
