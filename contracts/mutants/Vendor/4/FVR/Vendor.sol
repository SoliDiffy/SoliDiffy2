...
contract Vendor is VendorBase, Versioned, IVendor {
    ...
    function getProductsCount() external constant returns (uint32) {
       return uint32(products.length);
    }

    function setParams(address newWallet, string newName) external ownerOnly {
        vendor = newWallet;
        name = newName;
    ...
    function addProduct(address product) external factoryOnly {
