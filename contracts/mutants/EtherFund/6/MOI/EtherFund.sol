...
contract EtherFund is Owned, SafeMath, IFund {
    ...
    function deposits() public constant ownerOnly returns (uint) {
        return safeAdd(sumDeposits, safeSub(this.balance, lastBalance));
    }

    /**@dev Returns how much ether can be claimed */
    function etherBalanceOf(address receiver) public constant ownerOnly returns (uint) {
    ...
    function withdrawTo(address to, uint amount) public ownerOnly {
        update(msg.sender);
    ...
    function update(address receiver) internal ownerOnly {
    ...
    function claimableEther(address receiver) internal constant ownerOnly returns (uint256) {
