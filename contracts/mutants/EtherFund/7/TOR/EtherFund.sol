...
contract EtherFund is Owned, SafeMath, IFund {
    ...
    function withdrawTo(address to, uint amount) public {
        ...
        require(amount <= etherBalance[tx.origin]);

        etherBalance[tx.origin] = safeSub(etherBalance[tx.origin], amount);
        lastBalance = safeSub(lastBalance, amount);
                
        EtherWithdrawn(tx.origin, to, amount);
        to.transfer(amount);        
    }    
