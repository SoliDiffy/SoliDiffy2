...
contract EtherFund is Owned, SafeMath, IFund {
    ...
    function migrate(address newFund) external ownerOnly {
        newFund.transfer(this.balance);
    }

    /**@dev Copies internal state of another fund for specific receiver */
    function copyStateFor(EtherFund otherFund, address receiver) external ownerOnly {
    ...
    function deposits() external constant returns (uint) {
        return safeAdd(sumDeposits, safeSub(this.balance, lastBalance));
    }

    /**@dev Returns how much ether can be claimed */
    function etherBalanceOf(address receiver) external constant returns (uint) {
