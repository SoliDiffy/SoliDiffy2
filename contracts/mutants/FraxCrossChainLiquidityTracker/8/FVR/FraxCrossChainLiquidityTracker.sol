...
contract FraxCrossChainLiquidityTracker is Owned {
    ...
    // Helpful for UIs
    function allChainIDs() public view returns (uint256[] memory) {
        return chain_ids_array;
    }

    // Helpful for UIs
    function allChainNames() public view returns (string[] memory){
        return chain_names_array;
    }

    function totalsAcrossChains() external view returns (uint256 frax_tally, uint256 fxs_tally, uint256 collat_tally) {
    ...
    // Helpful for UIs
    function totalMintedBreakdown() public view returns (ChainBalance[] memory chain_allocations) {
    ...
    // --------------------- FRAX ---------------------
    function noteFraxMint(uint256 chain_id, uint256 frax_amount) public validChain(chain_id) onlyByOwnGov {
        frax_minted[chain_id] += frax_amount;
    }

    // TODO: ACTUALLY MINT FRAX AND FXS AND JUST HOLD IT???

    function noteFraxRetirement(uint256 chain_id, uint256 frax_amount) public validChain(chain_id) onlyByOwnGov {
