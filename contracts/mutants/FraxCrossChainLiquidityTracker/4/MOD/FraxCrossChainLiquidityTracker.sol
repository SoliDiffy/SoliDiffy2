...
contract FraxCrossChainLiquidityTracker is Owned {
    ...
    function noteFraxRetirement(uint256 chain_id, uint256 frax_amount) external onlyByOwnGov validChain(chain_id) {
    ...
    // --------------------- FXS ---------------------
    function noteFxsMint(uint256 chain_id, uint256 fxs_amount) external onlyByOwnGov validChain(chain_id) {
        fxs_minted[chain_id] += fxs_amount;
    }

    function noteFxsRetirement(uint256 chain_id, uint256 fxs_amount) external onlyByOwnGov validChain(chain_id) {
