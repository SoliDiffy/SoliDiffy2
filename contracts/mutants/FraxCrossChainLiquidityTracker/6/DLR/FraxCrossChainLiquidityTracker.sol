...
contract FraxCrossChainLiquidityTracker is Owned {
    ...
    // Helpful for UIs
    function allChainIDs() external view returns (uint256[] storage) {
        return chain_ids_array;
    }

    // Helpful for UIs
    function allChainNames() external view returns (string[] storage){
    ...
    // Helpful for UIs
    function totalMintedBreakdown() external view returns (ChainBalance[] storage chain_allocations) {
    ...
    // https://chainlist.org/
    function addChain(uint256 chain_id, string storage chain_name) public onlyByOwnGov {
