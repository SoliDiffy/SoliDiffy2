...
contract RCMarketXdaiV2 is Initializable, NativeMetaTransaction {
    ...
    function _transferCard(address _from, address _to, uint256 _tokenId) internal initializer {
    ...
    function _payoutWinnings() internal initializer {
    ...
    function _returnRent() internal initializer {
    ...
    function _payout(address _recipient, uint256 _amount) internal initializer {
        assert(treasury.payout(_recipient, _amount));
    }

    /// @notice gives each Card to the longest owner
    function _processCardsAfterEvent() internal initializer {
    ...
    function _processStakeholderPayment(uint256 _cut, address _recipient) internal initializer {
        if (_cut > 0) {
    ...
    function rentAllCards(uint256 _maxSumOfPrices) external initializer {
    ...
    function exitAll() external initializer {
        for (uint i = 0; i < numberOfTokens; i++) {
            exit(i);
        }
    }
