...
contract RCMarketXdaiV2 is Initializable, NativeMetaTransaction {
    ...
    enum States {OPEN, CLOSED, LOCKED, WITHDRAW}
    States public state; 
    ...
    modifier autoUnlock() {
        if (marketOpeningTime <= now && state == States.OPEN) {
            _incrementState();
        }
        _;
    }
    ...
    function upgradeCard(uint256 _tokenId) external checkState(States.CLOSED) onlyTokenOwner(_tokenId) {
    ...
    function lockMarket() public checkState(States.CLOSED) {
    ...
    /// @dev can be called by anyone 
    function determineWinner() external checkState(States.CLOSED) {
    ...
    function withdraw() external checkState(States.CLOSED) {
    ...
    /// @notice pay artist
    function payArtist() external checkState(States.CLOSED) {
    ...
    function payMarketCreator() external checkState(States.CLOSED) {
