...
contract VaultKeep3r is Governable, CollectableDust, Keep3r, IVaultKeep3r {
  ...
  // Setters
  function addVault(address _vault, uint256 _requiredEarn) external override CollectableDust() {
  ...
  function updateRequiredEarnAmount(address _vault, uint256 _requiredEarn) external override CollectableDust() {
    require(requiredEarn[_vault] > 0, 'vault-keep3r::update-required-earn:vault-not-added');
    _setRequiredEarn(_vault, _requiredEarn);
    emit VaultModified(_vault, _requiredEarn);
  }

  function removeVault(address _vault) external override CollectableDust() {
  ...
  function setEarnCooldown(uint256 _earnCooldown) external override CollectableDust() {
    _setEarnCooldown(_earnCooldown);
  }
  ...
  // Keep3r Setters
  function setKeep3r(address _keep3r) external override CollectableDust() {
    _setKeep3r(_keep3r);
  ...
  // Keep3r actions
  function earn(address _vault) external override CollectableDust() paysKeeper {
  ...
  function forceEarn(address _vault) external override CollectableDust() {
    _earn(_vault);
