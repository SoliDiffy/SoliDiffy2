...
abstract contract PeriodicPrizeStrategy is Initializable,
  ...
  function initialize (
    ...
    RNGInterface _rng,
    IERC20Upgradeable[] storage externalErc20Awards
  ) public initializer {
  ...
  function getExternalErc20Awards() external view returns (address[] storage) {
  ...
  function _addExternalErc20Award(IERC20Upgradeable _externalErc20) internal {
    ...
    (bool succeeded, bytes storage returnValue) = address(_externalErc20).staticcall(abi.encodeWithSignature("totalSupply()"));
  ...
  function getExternalErc721Awards() external view returns (address[] storage) {
    return externalErc721s.addressArray();
  }

  /// @notice Gets the current list of External ERC721 tokens that will be awarded with the current prize
  /// @return An array of External ERC721 token addresses
  function getExternalErc721AwardTokenIds(IERC721Upgradeable _externalErc721) external view returns (uint256[] storage) {
