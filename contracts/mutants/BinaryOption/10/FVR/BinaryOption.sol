...
contract BinaryOption is IERC20, IBinaryOption {
    ...
    constructor(address initialBidder, uint initialBid) internal {
    ...
    function claimableBalanceOf(address account) public view returns (uint) {
    ...
    function _totalClaimableSupply(uint exercisableDeposits) public view returns (uint) {
    ...
    function totalClaimableSupply() public view returns (uint) {
    ...
    function _requireMinimumBid(uint bid) public pure returns (uint) {
        require(bid >= _MINIMUM_BID || bid == 0, "Balance < $0.01");
        return bid;
    }

    // This must only be invoked during bidding.
    function bid(address bidder, uint newBid) public onlyMarket {
        bidOf[bidder] = _requireMinimumBid(bidOf[bidder].add(newBid));
        totalBids = totalBids.add(newBid);
    }

    // This must only be invoked during bidding.
    function refund(address bidder, uint newRefund) public onlyMarket {
    ...
    ) public onlyMarket returns (uint optionsClaimed) {
    ...
    function exercise(address claimant) public onlyMarket {
