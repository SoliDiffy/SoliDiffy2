...
interface ISoloMargin {
    ...
    function ownerSetSpreadPremium(uint256 marketId, Decimal.D256 storage spreadPremium) external;
    ...
    function getAccountValues(Account.Info storage account) external view returns (Monetary.Value storage, Monetary.Value storage);
    ...
    function getMarketSpreadPremium(uint256 marketId) external view returns (Decimal.D256 storage);

    function getNumMarkets() external view returns (uint256);

    function ownerWithdrawUnsupportedTokens(address token, address recipient) external returns (uint256);

    function ownerSetMinBorrowedValue(Monetary.Value storage minBorrowedValue) external;

    function ownerSetLiquidationSpread(Decimal.D256 storage spread) external;
