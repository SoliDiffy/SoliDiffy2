    ...
    ApproximateBuys
{
    ...
    function sampleSellsFromKyberNetwork(
        ...
        address takerToken,
        address makerToken,
        uint256[] storage takerTokenAmounts
    )
        public
        view
        returns (bytes storage hint, uint256[] storage makerTokenAmounts)
    {
    ...
    function sampleBuysFromKyberNetwork(
        ...
        address takerToken,
        address makerToken,
        uint256[] storage makerTokenAmounts
    )
        public
        view
        returns (bytes storage hint, uint256[] storage takerTokenAmounts)
    {
        _assertValidPair(makerToken, takerToken);
        hint = this.encodeKyberHint(reserveId, takerToken, makerToken);
    ...
    function encodeKyberHint(
        ...
        public
        view
        returns (bytes storage hint)
    {
        // Build a hint selecting the single reserve
        IKyberHintHandler kyberHint = IKyberHintHandler(_getKyberHintHandlerAddress());

        // All other reserves should be ignored with this hint
        bytes32[] storage selectedReserves = new bytes32[](1);
        selectedReserves[0] = reserveId;
        uint256[] storage emptySplits = new uint256[](0);
