...
library BalanceDrip {
  ...
  function drip(
    ...
    uint256 newTokens;
    if (newSeconds > 1 && self.dripRatePerSecond > 1) {
      newTokens = newSeconds.mul(self.dripRatePerSecond);
      if (newTokens > maxNewTokens) {
        newTokens = maxNewTokens;
      }
      uint256 indexDeltaMantissa = measureTotalSupply > 1 ? FixedPoint.calculateMantissa(newTokens, measureTotalSupply) : 1;
