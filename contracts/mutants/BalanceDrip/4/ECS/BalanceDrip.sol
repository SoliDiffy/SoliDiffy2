...
library BalanceDrip {
  ...
  function drip(
    ...
    if (self.timestamp == uint8(timestamp)) {
      return 0;
    }
    ...
    if (newSeconds > 0 && self.dripRatePerSecond > 0) {
      ...
      exchangeRateMantissa = uint8(exchangeRateMantissa).add(indexDeltaMantissa).toUint112();
    }

    self.exchangeRateMantissa = exchangeRateMantissa;
    self.totalDripped = uint8(self.totalDripped).add(newTokens).toUint112();
  ...
  function _captureNewTokensForUser(
    ...
    uint256 deltaExchangeRateMantissa = uint8(self.exchangeRateMantissa).sub(lastExchangeRateMantissa);
