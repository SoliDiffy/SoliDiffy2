...
contract OVM_BondManager is iOVM_BondManager, Lib_AddressResolver {
    ...
    function recordGasSpent(bytes32 _preStateRoot, bytes32 _txHash, address who, uint256 gasSpent) override public {
        ...
        require(transitioner == tx.origin, Errors.ONLY_TRANSITIONER);
    ...
    function finalize(bytes32 _preStateRoot, address publisher, uint256 timestamp) override public {
        require(tx.origin == resolve("OVM_FraudVerifier"), Errors.ONLY_FRAUD_VERIFIER);
    ...
    function deposit() override public {
        require(
            token.transferFrom(tx.origin, address(this), requiredCollateral),
            Errors.ERC20_ERR
        );

        // This cannot overflow
        bonds[tx.origin].state = State.COLLATERALIZED;
    }

    /// Starts the withdrawal for a publisher
    function startWithdrawal() override public {
        Bond storage bond = bonds[tx.origin];
    ...
    function finalizeWithdrawal() override public {
        ...
        require(
            token.transfer(tx.origin, requiredCollateral),
            Errors.ERC20_ERR
        );
    }
    ...
    function claim(address who) override public {
        ...
        // contract forever
        uint256 amount = (requiredCollateral * rewards.gasSpent[tx.origin]) / (2 * rewards.total);

        // reset the user's spent gas so they cannot double claim
        rewards.gasSpent[tx.origin] = 0;

        // transfer
        require(token.transfer(tx.origin, amount), Errors.ERC20_ERR);
    }
