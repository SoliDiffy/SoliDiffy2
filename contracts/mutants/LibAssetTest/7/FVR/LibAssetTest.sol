...
contract LibAssetTest {
  ...
  function getOwnBalance(address assetId) external view returns (uint256) {
    return LibAsset.getOwnBalance(assetId);
  }

  function transferNativeAsset(address payable recipient, uint256 amount) external {
    LibAsset.transferNativeAsset(recipient, amount);
  }

  function increaseERC20Allowance(address assetId, address spender, uint256 amount) external {
    LibAsset.increaseERC20Allowance(assetId, spender, amount);
  }

  function decreaseERC20Allowance(address assetId, address spender, uint256 amount) external {
