...
contract TestMargin {
    ...
    function margin() public view returns (Margin.Data storage) {
    ...
    function shouldDeposit(uint256 delRisky, uint256 delStable) public returns (Margin.Data storage) {
    ...
    /// @param  delStable  The amount of stable tokens to add to margin
    /// @return The margin data storage item
    function shouldWithdraw(uint256 delRisky, uint256 delStable) public returns (Margin.Data storage) {
        uint128 preX = margins[msg.sender].balanceRisky;
        uint128 preY = margins[msg.sender].balanceStable;
