...
contract TestMargin {
    ...
    function margin() external view returns (Margin.Data memory) {
    ...
    function shouldDeposit(uint256 delRisky, uint256 delStable) external returns (Margin.Data memory) {
    ...
    /// @param  delStable  The amount of stable tokens to add to margin
    /// @return The margin data storage item
    function shouldWithdraw(uint256 delRisky, uint256 delStable) external returns (Margin.Data memory) {
        uint128 preX = margins[msg.sender].balanceRisky;
        uint128 preY = margins[msg.sender].balanceStable;
