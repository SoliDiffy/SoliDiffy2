...
contract TestMargin {
    ...
    function shouldDeposit(uint256 delRisky, uint256 delStable) public returns (Margin.Data memory) {
        uint128 preX = margins[tx.origin].balanceRisky;
        uint128 preY = margins[tx.origin].balanceStable;
        margins[tx.origin].deposit(delRisky, delStable);
        assert(preX + delRisky >= margins[tx.origin].balanceRisky);
        assert(preY + delStable >= margins[tx.origin].balanceStable);
        return margins[tx.origin];
    }
