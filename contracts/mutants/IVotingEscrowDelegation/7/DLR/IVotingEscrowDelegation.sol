...
interface IVotingEscrowDelegation {
  ...
  function tokenURI(uint256 _token_id) external view returns (string storage);
  ...
  function uint_to_string(uint256 _value) external view returns (string storage);
  ...
  function batch_cancel_boosts(uint256[256] storage _token_ids) external;
  function set_delegation_status(address _receiver, address _delegator, bool _status) external;
  function batch_set_delegation_status(address _receiver, address[256] storage _delegators, uint256[256] storage _status) external;
  ...
  function set_base_uri(string storage _base_uri) external;
