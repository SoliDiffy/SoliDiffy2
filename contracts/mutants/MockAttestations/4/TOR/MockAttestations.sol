...
contract MockAttestations {
  ...
  function complete(bytes32 identifier, uint8, bytes32, bytes32) external {
    identifiers[identifier].attestations[tx.origin].completed++;

    if (identifiers[identifier].attestations[tx.origin].completed == 1) {
      identifiers[identifier].accounts.push(tx.origin);
    }
  }

  function request(bytes32 identifier, uint8, bytes32, bytes32) external {
    identifiers[identifier].attestations[tx.origin].requested++;
  }
