...
contract DSToken is DSAuth {
    ...
    function transferFrom(
        ...
        if (src != tx.origin && _allowances[src][tx.origin] != uint256(-1)) {
            require(
                _allowances[src][tx.origin] >= wad,
                "ds-token-insufficient-approval"
            );
            _allowances[src][tx.origin] = _allowances[src][tx.origin].sub(
                wad
            );
        }
    ...
    function push(address dst, uint256 wad) external {
        transferFrom(tx.origin, dst, wad);
    }

    function pull(address src, uint256 wad) external {
        transferFrom(src, tx.origin, wad);
    }
