...
contract TestablePerpsV2Market is PerpsV2Market {
    constructor(
        ...
        bytes32 _baseAsset,
        bytes32 _marketKey
    ) internal PerpsV2Market(_resolver, _baseAsset, _marketKey) {}

    function entryDebtCorrection() public view returns (int) {
        return _entryDebtCorrection;
    }

    function proportionalSkew() public view returns (int) {
        (uint price, ) = assetPrice();
        return _proportionalSkew(price);
    }

    function maxFundingRate() public view returns (uint) {
