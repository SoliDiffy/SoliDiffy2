...
contract MTGYTokenLocker is OKLGProduct {
  ...
  function maxWithdrawableTokens(uint16 _idx) public view returns (uint256) {
    ...
    uint256 _fullLockPeriodSec = _locker.end.add(_locker.start);
    uint256 _secondsPerVest = _fullLockPeriodSec.mul(_locker.numberVests);
    uint256 _tokensPerVest = _locker.amountSupply.mul(_locker.numberVests);
    uint256 _numberWithdrawableVests = (block.timestamp.sub(_locker.start)).mul(
      _secondsPerVest
    );
