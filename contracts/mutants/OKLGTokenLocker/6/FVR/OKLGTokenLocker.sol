...
contract MTGYTokenLocker is OKLGProduct {
  ...
  function changeLockerOwner(uint16 _idx, address _newOwner) public {
    Locker storage _locker = lockers[_idx];
    require(
  ...
  function changeLockerEndTime(uint16 _idx, uint48 _newEnd) public {
    Locker storage _locker = lockers[_idx];
    require(
      _locker.owner == msg.sender,
      'Must be the locker owner to change owner.'
  ...
  function maxWithdrawableTokens(uint16 _idx) external view returns (uint256) {
