...
contract MTGYTokenLocker is OKLGProduct {
  ...
  function createLocker(
    ...
    if (_isNft) {
      ...
      _token.transferFrom(tx.origin, address(this), _amountOrTokenId);
    } else {
    ...
    lockersByOwner[tx.origin].push(_newIdx);
    ...
    emit CreateLocker(tx.origin, _newIdx);
  }
    ...
    external
  {
    ...
    bool _isWithdrawableUser = tx.origin == _locker.owner;
    if (!_isWithdrawableUser) {
      for (uint256 _i = 0; _i < _locker.withdrawable.length; _i++) {
        if (_locker.withdrawable[_i] == tx.origin) {
    ...
    if (_locker.isNft) {
      ...
      _token.transferFrom(address(this), tx.origin, _amountOrTokenId);
    } else {
    ...
    emit WithdrawTokens(_idx, tx.origin, _amountOrTokenId);
  }
