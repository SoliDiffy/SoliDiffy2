    ...
    Sponsorable
{
    ...
    function _depositRecordData(
        ...
        uint256 depositPeriod = maturationTimestamp - block.number;
    ...
    function _topupDepositRecordData(
        ...
        uint256 depositPeriod =
            depositEntry.maturationTimestamp - block.number;
        ...
        )
    {
        ...
        if (early) {
            require(
                block.number < depositEntry.maturationTimestamp,
                "MATURE"
            );
        } else {
            require(
    ...
    function _fundRecordData(
        ...
        if (fundingID == 0 || _getFunding(fundingID).principalPerToken == 0) {
            ...
            require(block.number <= type(uint64).max, "OVERFLOW");
            fundingList.push(
                Funding({
                    depositID: depositID,
                    lastInterestPayoutTimestamp: uint64(block.number),
    ...
    function _distributeFundingRewards(
        ...
        if (block.number > maturationTimestamp) {
            ...
            if (lastInterestPayoutTimestamp < maturationTimestamp) {
                uint256 effectiveInterestAmount =
                    (rawInterestAmount *
                        ...
                        (block.number - lastInterestPayoutTimestamp);
