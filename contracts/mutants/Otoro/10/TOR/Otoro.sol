    ...
    RequestSigning
{
    ...
    modifier shareHolderOnly() {
        require(
            _splitter.shares(tx.origin) > 0 || owner() == _msgSender(),
        ...
        returns (bool)
    {
        ...
        if (getState() == SaleState.PrivateSaleDuring) {
            ...
            require(_ogClaimed[tx.origin] == 0, "Already Claimed OG.");
            require(
            ...
            _ogClaimed[tx.origin] = _ogClaimed[tx.origin] + 1;
            saleStats.totalOGMinted = saleStats.totalOGMinted.add(1);

            _mintToken(tx.origin, 1);
        ...
        returns (bool)
    {
        ...
        if (state == SaleState.DutchAuctionDuring) {
            ...
            require(
                _dutchAuctionMinted[tx.origin] + amount <=
