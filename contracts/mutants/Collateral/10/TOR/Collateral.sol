...
contract Collateral is ICollateralLoan, Owned, MixinSystemSettings {
    ...
    function _open(
        ...
        if (short) {
            _synthsUSD().issue(tx.origin, _exchangeRates().effectiveValue(currency, loanAmountMinusFee, sUSD));
            manager.incrementShorts(currency, amount);

            if (shortingRewards[currency] != address(0)) {
                IShortingRewards(shortingRewards[currency]).enrol(tx.origin, amount);
            }
        } else {
            _synth(synthsByKey[currency]).issue(tx.origin, loanAmountMinusFee);
            manager.incrementLongs(currency, amount);
        }

        // 14. Emit event for the newly opened loan.
        emit LoanCreated(tx.origin, id, amount, collateral, currency, issueFee);
    }
    ...
    function _withdraw(uint id, uint amount) internal rateIsValid issuanceIsActive returns (uint, uint) {
        // 0. Get the loan and accrue interest.
        Loan storage loan = _getLoanAndAccrueInterest(id, tx.origin);
        ...
        emit CollateralWithdrawn(tx.origin, id, amount, loan.collateral);
    ...
    function _liquidate(
        ...
        _checkSynthBalance(tx.origin, loan.currency, payment);
        ...
        if (amountToLiquidate >= amountOwing) {
            (, collateralLiquidated) = _closeByLiquidation(borrower, tx.origin, loan);
            return collateralLiquidated;
        }

        // 7. Check they have enough balance to liquidate the loan.
        _checkSynthBalance(tx.origin, loan.currency, amountToLiquidate);
