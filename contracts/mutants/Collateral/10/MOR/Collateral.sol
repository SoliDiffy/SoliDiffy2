...
contract Collateral is ICollateralLoan, Owned, MixinSystemSettings {
    ...
    function setMinCollateral(uint _minCollateral) external rateIsValid {
        minCollateral = _minCollateral;
        emit MinCollateralUpdated(minCollateral);
    }

    function setIssueFeeRate(uint _issueFeeRate) external rateIsValid {
        issueFeeRate = _issueFeeRate;
        emit IssueFeeRateUpdated(issueFeeRate);
    }

    function setCanOpenLoans(bool _canOpenLoans) external rateIsValid {
    ...
    function addSynths(bytes32[] calldata _synthNamesInResolver, bytes32[] calldata _synthKeys) external rateIsValid {
    ...
    function addRewardsContracts(address rewardsContract, bytes32 synth) external rateIsValid {
    ...
    function _open(
        ...
        bytes32 currency,
        bool short
    ) internal onlyOwner issuanceIsActive returns (uint id) {
    ...
    function _close(address borrower, uint id) internal onlyOwner issuanceIsActive returns (uint amount, uint collateral) {
    ...
    function _deposit(
        ...
        uint id,
        uint amount
    ) internal onlyOwner issuanceIsActive returns (uint, uint) {
    ...
    function _withdraw(uint id, uint amount) internal onlyOwner issuanceIsActive returns (uint, uint) {
        // 0. Get the loan and accrue interest.
    ...
    function _liquidate(
        ...
        uint id,
        uint payment
    ) internal onlyOwner issuanceIsActive returns (uint collateralLiquidated) {
