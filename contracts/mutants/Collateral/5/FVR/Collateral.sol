...
contract Collateral is ICollateralLoan, Owned, MixinSystemSettings {
    ...
    constructor(
        ...
        uint _minCratio,
        uint _minCollateral
    ) internal Owned(_owner) MixinSystemSettings(_resolver) {
        manager = _manager;
    ...
    function _synth(bytes32 synthName) public view returns (ISynth) {
        return ISynth(requireAndGetAddress(synthName));
    }

    function _synthsUSD() public view returns (ISynth) {
