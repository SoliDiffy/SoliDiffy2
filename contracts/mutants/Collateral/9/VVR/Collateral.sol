...
contract Collateral is ICollateralLoan, Owned, MixinSystemSettings {
    ...
    bytes32 internal collateralKey;

    // Stores open loans.
    mapping(uint => Loan) public loans;

    ICollateralManager internal manager;

    // The synths that this contract can issue.
    bytes32[] internal synths;
    ...
    uint internal minCratio;

    // The minimum amount of collateral to create a loan.
    uint internal minCollateral;

    // The fee charged for issuing a loan.
    uint internal issueFeeRate;

    bool internal canOpenLoans = true;
