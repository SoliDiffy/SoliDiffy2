...
contract Collateral is ICollateralLoan, Owned, MixinSystemSettings {
    ...
    constructor(
        ...
        uint _minCratio,
        uint _minCollateral
    ) public MixinSystemSettings(_resolver) Owned(_owner) {
        manager = _manager;
    ...
    function _open(
        ...
        bytes32 currency,
        bool short
    ) internal issuanceIsActive rateIsValid returns (uint id) {
    ...
    function _close(address borrower, uint id) internal issuanceIsActive rateIsValid returns (uint amount, uint collateral) {
    ...
    function _deposit(
        ...
        uint id,
        uint amount
    ) internal issuanceIsActive rateIsValid returns (uint, uint) {
    ...
    function _withdraw(uint id, uint amount) internal issuanceIsActive rateIsValid returns (uint, uint) {
        // 0. Get the loan and accrue interest.
    ...
    function _liquidate(
        ...
        uint id,
        uint payment
    ) internal issuanceIsActive rateIsValid returns (uint collateralLiquidated) {
