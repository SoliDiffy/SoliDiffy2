...
contract Collateral is ICollateralLoan, Owned, MixinSystemSettings {
    ...
    function _closeByLiquidation(
        ...
        address liquidator,
        Loan storage loan
    ) internal onlyOwner returns (uint amount, uint collateral) {
    ...
    function _accrueInterest(Loan storage loan) internal onlyOwner {
    ...
    function _processPayment(Loan storage loan, uint payment) internal onlyOwner {
        require(payment > 0, "Payment must be above 0");
    ...
    function _payFees(uint amount, bytes32 synth) internal onlyOwner {
        if (amount > 0) {
            if (synth != sUSD) {
    ...
    function _recordLoanAsClosed(Loan storage loan) internal onlyOwner {
        loan.amount = 0;
        loan.collateral = 0;
    ...
    function _getLoanAndAccrueInterest(uint id, address owner) internal onlyOwner returns (Loan storage loan) {
        loan = loans[id];
