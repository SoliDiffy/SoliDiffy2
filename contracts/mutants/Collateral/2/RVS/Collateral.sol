...
contract Collateral is ICollateralLoan, Owned, MixinSystemSettings {
    ...
    function _close(address borrower, uint id) internal rateIsValid issuanceIsActive returns (uint collateral, uint amount) {
    ...
    function _closeByLiquidation(
        ...
        address liquidator,
        Loan storage loan
    ) internal returns (uint collateral, uint amount) {
