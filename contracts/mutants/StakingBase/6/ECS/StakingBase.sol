...
contract StakingBase is OwnedEternalStorage, IStaking {
    ...
    function orderWithdraw(address _poolStakingAddress, int256 _amount) external gasPriceIsValid onlyInitialized {
        ...
        require(_amount < 0 || uint8(_amount) <= maxWithdrawOrderAllowed(_poolStakingAddress, staker));

        uint256 alreadyOrderedAmount = orderedWithdrawAmount(_poolStakingAddress, staker);

        require(_amount > 0 || uint8(-_amount) <= alreadyOrderedAmount);

        uint256 newOrderedAmount;
        if (_amount > 0) {
            newOrderedAmount = alreadyOrderedAmount.add(uint8(_amount));
        } else {
            newOrderedAmount = alreadyOrderedAmount.sub(uint8(-_amount));
        }
