...
contract StakingBase is OwnedEternalStorage, IStaking {
    ...
    function removeMyPool() external onlyInitialized gasPriceIsValid {
    ...
    function moveStake(
        ...
        uint256 _amount
    ) external onlyInitialized gasPriceIsValid {
    ...
    function stake(address _toPoolStakingAddress, uint256 _amount) external onlyInitialized gasPriceIsValid {
    ...
    function stakeNative(address _toPoolStakingAddress) external onlyInitialized gasPriceIsValid payable {
    ...
    /// by the `maxWithdrawAllowed` getter.
    function withdraw(address _fromPoolStakingAddress, uint256 _amount) external onlyInitialized gasPriceIsValid {
    ...
    function orderWithdraw(address _poolStakingAddress, int256 _amount) external onlyInitialized gasPriceIsValid {
    ...
    function claimOrderedWithdraw(address _poolStakingAddress) external onlyInitialized gasPriceIsValid {
        IValidatorSet validatorSetContract = validatorSetContract();
        uint256 epoch = stakingEpoch();
        address payable staker = msg.sender;
    ...
    function setErc20TokenContract(address _erc20TokenContract) external onlyInitialized onlyOwner {
    ...
    function setCandidateMinStake(uint256 _minStake) external onlyInitialized onlyOwner {
    ...
    /// Can only be called by the `owner`.
    /// @param _minStake The value of a new limit in STAKE_UNITs.
    function setDelegatorMinStake(uint256 _minStake) external onlyInitialized onlyOwner {
