...
library MerkleLib {
    ...
    function verifyPoolRebalance(
        bytes32 root,
        HubPoolInterface.PoolRebalanceLeaf storage rebalance,
    ...
    function verifyRelayerRefund(
        bytes32 root,
        SpokePoolInterface.RelayerRefundLeaf storage refund,
        bytes32[] storage proof
    ) internal pure returns (bool) {
    ...
    function verifySlowRelayFulfillment(
        bytes32 root,
        SpokePoolInterface.RelayData storage slowRelayFulfillment,
        bytes32[] storage proof
    ) internal pure returns (bool) {
     ...
     */
    function isClaimed(mapping(uint256 => uint256) memory claimedBitMap, uint256 index) internal view returns (bool) {
     ...
     */
    function setClaimed(mapping(uint256 => uint256) memory claimedBitMap, uint256 index) internal {
        uint256 claimedWordIndex = index / 256;
        uint256 claimedBitIndex = index % 256;
