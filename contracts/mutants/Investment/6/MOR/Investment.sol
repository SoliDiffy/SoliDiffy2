...
contract Investment is OwnablePausable {
     ...
     */
    function allowToken(address token) external whenNotPaused {
     ...
     */
    function denyToken(address token) external whenNotPaused {
     ...
     */
    function changeGovernanceTokenPrice(uint256 newPrice) external whenNotPaused {
     ...
     * @param amount Invested amount
     */
    function invest(address token, uint256 amount) external onlyOwner returns (bool) {
     ...
     */
    function investETH() external payable onlyOwner returns (bool) {
