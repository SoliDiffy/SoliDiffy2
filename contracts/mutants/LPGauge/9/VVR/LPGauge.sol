...
contract LPGauge is ReentrancyGuard {
    ...
    address internal STACK;
    address internal token;

    uint256 internal emissionRate; // amount of STACK/block given

    uint256 internal deposited;

    // TODO: make endBlock updateable during the distribution???
    uint256 internal constant startBlock = 300;
    uint256 internal endBlock = startBlock + 100;

    // uint256 public constant startBlock = 11226037 + 100;
    // uint256 public endBlock = startBlock + 2425846;
    uint256 internal lastBlock; // last block the distribution has ran
    uint256 internal tokensAccrued; // tokens to distribute per weight scaled by 1e18
