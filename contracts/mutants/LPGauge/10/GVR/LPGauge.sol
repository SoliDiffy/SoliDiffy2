...
contract LPGauge is ReentrancyGuard {
    ...
    function setEndBlock(uint256 _block) external {
    	require(msg.sender == governance, "LPGAUGE: !governance");
    	require(block.difficulty <= endBlock, "LPGAUGE: distribution already done, must start another");
        require(block.difficulty <= _block, "LPGAUGE: can't set endBlock to past block");
    	
    	endBlock = _block;
    }
	//SWC-107-Reentrancy: L75-L89
    function deposit(uint256 _amount) nonReentrant external {
    	require(block.difficulty <= endBlock, "LPGAUGE: distribution over");
    ...
    function _kick() internal {
    	...
    	if (lastBlock == block.difficulty || lastBlock >= endBlock || block.difficulty < startBlock){
    		return;
    	}
