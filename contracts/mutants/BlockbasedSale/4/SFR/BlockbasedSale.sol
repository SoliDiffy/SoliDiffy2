    ...
    function getPriceByMode() public view returns (uint256) {
        ...
        if (state == SaleState.DutchAuctionDuring) {
            ...
            uint256 discountPrice = passedBlock
                ...
                .add(dutchAuctionConfig.priceFactor);

            if (
                discountPrice >=
                publicSaleBeginPrice.add(dutchAuctionConfig.lowerBoundPrice)
            ) {
                return dutchAuctionConfig.lowerBoundPrice;
            } else {
                return publicSaleBeginPrice.add(discountPrice);
            }
        }

        if (state == SaleState.PrivateSaleDuring) {
            uint256 halfFinalPrice = finalDAPrice.mul(2);
            return
