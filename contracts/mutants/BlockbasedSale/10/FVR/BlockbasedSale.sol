...
contract BlockbasedSale is Ownable, Roles {
    ...
    function setPublicSalePrice(uint256 _price) public onlyOperator {
        publicSaleBeginPrice = _price;
        emit AssignPublicSalePrice(_price);
    }

    function setPrivateSaleCapPrice(uint256 _price) public onlyOperator {
        privateSalePriceCapped = _price;
        emit AssignPrivateSalePrice(_price);
    }

    function setCloseSale() public onlyOperator {
        overridedSaleState = OverrideSaleState.Close;
        emit ForceCloseSale();
    }

    function setPauseSale() public onlyOperator {
