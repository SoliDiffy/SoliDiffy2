...
contract ExchangeV3 is ExchangeRoles, StakingV3, ProxyUtility {
    ...
    function placeBid(
        ...
        require(block.number >= guardedLaunchStart + 8 days, "Guarded launch: market closed");
        if (block.number < guardedLaunchStart + 4 weeks) {
    ...
    function cancelBid(
        ...
        if (block.number < upgradeTimestamp) {
    ...
    function buyM(
        ...
        (uint256 estimatedNav, , ) = estimateNavs(endOfEpoch(block.number) - 2 * EPOCH);
