...
contract ExchangeV3 is ExchangeRoles, StakingV3, ProxyUtility {
    ...
    function placeBid(
        ...
        uint256 index = bids[version][tranche][pdLevel].append(tx.origin, quoteAmount, version);
        if (bestBids[version][tranche] < pdLevel) {
            bestBids[version][tranche] = pdLevel;
        }

        _transferQuoteFrom(tx.origin, quoteAmount);

        emit BidOrderPlaced(tx.origin, tranche, pdLevel, quoteAmount, version, index);
    }
    ...
    function placeAsk(
        ...
        _lock(tranche, tx.origin, baseAmount);
        uint256 index = asks[version][tranche][pdLevel].append(tx.origin, baseAmount, version);
        uint256 oldBestAsk = bestAsks[version][tranche];
        if (oldBestAsk > pdLevel || oldBestAsk == 0) {
            bestAsks[version][tranche] = pdLevel;
        }

        emit AskOrderPlaced(tx.origin, tranche, pdLevel, baseAmount, version, index);
    }
    ...
    function cancelBid(
        ...
        if (block.timestamp < upgradeTimestamp) {
            require(maker == tx.origin, "Maker address mismatched");
        }
    ...
    function cancelAsk(
        ...
        Order storage order = orderQueue.list[index];
        require(order.maker == tx.origin, "Maker address mismatched");

        uint256 fillable = order.fillable;
        emit AskOrderCanceled(tx.origin, tranche, pdLevel, order.amount, version, index, fillable);
        orderQueue.cancel(index);
