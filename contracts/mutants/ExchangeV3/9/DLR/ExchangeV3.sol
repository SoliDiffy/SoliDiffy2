...
contract ExchangeV3 is ExchangeRoles, StakingV3, ProxyUtility {
    ...
    {
        Order memory order = bids[version][tranche][pdLevel].list[index];
        maker = order.maker;
        amount = order.amount;
    ...
    {
        Order memory order = asks[version][tranche][pdLevel].list[index];
        maker = order.maker;
        amount = order.amount;
        fillable = order.fillable;
    }
    ...
    function cancelBid(
        ...
        OrderQueue memory orderQueue = bids[version][tranche][pdLevel];
    ...
    function cancelAsk(
        ...
        OrderQueue memory orderQueue = asks[version][tranche][pdLevel];
        Order memory order = orderQueue.list[index];
    ...
    function _buy(
        ...
        UnsettledBuyTrade storage totalTrade;
        ...
        UnsettledBuyTrade storage currentTrade;
