...
contract Router {
    ...
    function addField(uint id, bytes32 field, address data) public auth {
        routerDataMap[id].fields[field] = data;
    }

    function setBondNr(uint _bondNr) public auth {
        bondNr = _bondNr;
    }

    //根据field找出合约地址
    function f(uint id, bytes32 field) public view returns (address) {
