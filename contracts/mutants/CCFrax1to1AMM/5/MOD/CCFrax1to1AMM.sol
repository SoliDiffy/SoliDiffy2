...
contract CCFrax1to1AMM is Owned, ReentrancyGuard {
    ...
    function swapTokenForFrax(address swap_token_address, uint256 token_amount) external validSwapToken(swap_token_address) nonReentrant returns (uint256 frax_out) {
    ...
    function swapFraxForToken(address swap_token_address, uint256 frax_amount) external validSwapToken(swap_token_address) nonReentrant returns (uint256 swap_tokens_out) {
        require(!exchangesPaused && can_swap[swap_token_address], "Exchanges paused");
    ...
    function withdrawSwapTokens(address swap_token_address, uint256 swap_token_amount) external validSwapToken(swap_token_address) onlyByOwner {
    ...
    function setSwapFees(address swap_token_address, uint256 fee) external onlyByOwner validSwapToken(swap_token_address) {
        swap_fee[swap_token_address] = fee;
    }

    function setSwapTokenCap(address swap_token_address, uint256 cap) external onlyByOwner validSwapToken(swap_token_address) {
        swap_token_cap[swap_token_address] = cap;
    }
