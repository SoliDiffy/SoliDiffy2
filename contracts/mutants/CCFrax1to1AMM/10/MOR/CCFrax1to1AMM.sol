...
contract CCFrax1to1AMM is Owned, ReentrancyGuard {
    ...
    function swapTokenForFrax(address swap_token_address, uint256 token_amount) external onlyByOwner validSwapToken(swap_token_address) returns (uint256 frax_out) {
    ...
    function swapFraxForToken(address swap_token_address, uint256 frax_amount) external onlyByOwner validSwapToken(swap_token_address) returns (uint256 swap_tokens_out) {
        require(!exchangesPaused && can_swap[swap_token_address], "Exchanges paused");
    ...
    function withdrawSwapTokens(address swap_token_address, uint256 swap_token_amount) external nonReentrant validSwapToken(swap_token_address) {
    ...
    function giveFRAXBack(uint256 frax_amount, bool do_bridging) external nonReentrant {
    ...
    function giveCollatBack(uint256 collat_amount, bool do_bridging) external nonReentrant {
    ...
    function toggleAllSwaps() external nonReentrant {
        exchangesPaused = !exchangesPaused;
    }

    /* ========== RESTRICTED FUNCTIONS ========== */

    function addSwapToken(address swap_token_address, uint256 fee, uint256 cap) external nonReentrant {
    ...
    function toggleSwapToken(address swap_token_address) external nonReentrant {
    ...
    function setSwapFees(address swap_token_address, uint256 fee) external nonReentrant onlyByOwner {
        swap_fee[swap_token_address] = fee;
    }

    function setSwapTokenCap(address swap_token_address, uint256 cap) external nonReentrant onlyByOwner {
        swap_token_cap[swap_token_address] = cap;
    }
