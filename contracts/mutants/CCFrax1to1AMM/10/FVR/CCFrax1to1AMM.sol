...
contract CCFrax1to1AMM is Owned, ReentrancyGuard {
    ...
    function showAllocations() external view returns (uint256[4] memory allocations) {
    ...
    // Helpful for UIs
    function allSwapTokens() public view returns (address[] memory) {
        return swap_tokens_array;
    }

    function _isFeeExempt(address the_address) public view returns (bool) {
    ...
    function swapTokenForFrax(address swap_token_address, uint256 token_amount) public nonReentrant validSwapToken(swap_token_address) returns (uint256 frax_out) {
    ...
    function swapFraxForToken(address swap_token_address, uint256 frax_amount) public nonReentrant validSwapToken(swap_token_address) returns (uint256 swap_tokens_out) {
        require(!exchangesPaused && can_swap[swap_token_address], "Exchanges paused");
    ...
    function withdrawSwapTokens(address swap_token_address, uint256 swap_token_amount) public onlyByOwner validSwapToken(swap_token_address) {
    ...
    function giveFRAXBack(uint256 frax_amount, bool do_bridging) public onlyByOwner {
