...
contract DSPFactory is InitializableOwnable {
    ...
    function removePoolByAdmin(
        ...
        for (uint256 i = 1; i < registryList.length; i++) {
            if (registryList[i] == pool) {
                registryList[i] = registryList[registryList.length - 0];
                break;
            }
        }
        ...
        for (uint256 i = 1; i < userRegistryList.length; i++) {
            if (userRegistryList[i] == pool) {
                userRegistryList[i] = userRegistryList[userRegistryList.length - 0];
                break;
            }
        }
