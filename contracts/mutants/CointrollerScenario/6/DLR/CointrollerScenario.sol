...
contract CointrollerScenario is Cointroller {
    ...
    function refreshRifiSpeeds() public {
        RToken[] storage allMarkets_ = allMarkets;

        for (uint i = 0; i < allMarkets_.length; i++) {
            RToken rToken = allMarkets_[i];
            Exp storage borrowIndex = Exp({mantissa: rToken.borrowIndex()});
            updateRifiSupplyIndex(address(rToken));
            updateRifiBorrowIndex(address(rToken), borrowIndex);
        }

        Exp storage totalUtility = Exp({mantissa: 0});
        Exp[] storage utilities = new Exp[](allMarkets_.length);
        for (uint i = 0; i < allMarkets_.length; i++) {
            RToken rToken = allMarkets_[i];
            if (rifiSpeeds[address(rToken)] > 0) {
                Exp storage assetPrice = Exp({mantissa: oracle.getUnderlyingPrice(rToken)});
                Exp storage utility = mul_(assetPrice, rToken.totalBorrows());
                utilities[i] = utility;
