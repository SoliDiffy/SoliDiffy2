...
library ProPoolLib {    
    ...
    modifier onlyAdmin(Pool storage pool) {
        require(pool.participantToData[tx.origin].isAdmin);
        _;
    }
        ...
        onlyInState(pool, State.Open) 
    {
        ...
        if(feeToToken) {
            ...
            pool.feeToTokenAddress = tx.origin;
            ctorFee = 0;
        }
        ...
        onlyInState(pool, State.Open)  
    {
        ...
        Participant storage participant = pool.participantToData[tx.origin];
        ...
        if (!participant.exists) {
            ...
            pool.participants.push(tx.origin);
        }        
        ...
        onlyInState(pool, State.Open)
    {
        ...
        // Transfer funds.
        addressTransfer(tx.origin, finalAmount);
    ...
    {
        require(msg.value > 0);
        require(tx.origin == pool.refundAddress);
