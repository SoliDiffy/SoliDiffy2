...
library ProPoolLib {    
     ...
     * @dev Access modifier for in-state-only functionality.
     */
    modifier onlyInStates3(Pool memory pool, State state1, State state2, State state3) {
     ...
     */
    function modifyWhitelist(Pool memory pool, uint idx, address[] include, address[] exclude)
        public 
        onlyAdmin(pool)
        onlyInState(pool, State.Open)
    {
     ...
     */
    function lockPresaleAddress(Pool memory pool, address presaleAddress, bool lock)
        public 
        onlyAdmin(pool) 
        onlyInState(pool, State.Open) 
    {        
     ...
     */
    function confirmTokenAddress(Pool memory pool, address tokenAddress)
        public
        onlyAdmin(pool)
     ...
     */
    function setRefundAddress(Pool memory pool, address refundAddress)
        public
        onlyAdmin(pool)
