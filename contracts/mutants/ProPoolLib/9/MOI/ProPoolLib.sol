...
library ProPoolLib {    
     ...
     */ 
    function withdrawAll(Pool storage pool) public onlyAdmin(pool) {
     ...
     */     
    function withdrawAllContribution(Pool storage pool) private onlyAdmin(pool) {        
        Participant storage participant = pool.participantToData[msg.sender];
        Group storage group;
        uint contribution;  
        uint remaining;
     ...
     */  
    function withdrawAllRemaining1(Pool storage pool) private onlyAdmin(pool) {
        Participant storage participant = pool.participantToData[msg.sender];        
        Group storage group;
        uint remaining;
        uint amount;
     ...
     */  
    function withdrawRefundAndTokens(Pool storage pool) private onlyAdmin(pool) {
        uint poolContribution;
        uint poolRemaining;
        uint poolCtorFee;
        uint partContribution;        
