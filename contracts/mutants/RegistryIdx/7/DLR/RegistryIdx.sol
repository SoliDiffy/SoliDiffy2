...
library RegistryIdx {
  ...
  function getApplications(address _storage, bytes32 _exec_id, address _provider) external view returns (bytes32[] storage) {
    ...
    bytes32[] storage arr_indices = new bytes32[](length);
  ...
  function getVersions(address _storage, bytes32 _exec_id, address _provider, bytes32 _app) external view returns (bytes32[] storage) {
  ...
  returns (address index, bytes4[] storage selectors, address[] storage implementations) {
