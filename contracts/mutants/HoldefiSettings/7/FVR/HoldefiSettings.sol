...
contract HoldefiSettings is Ownable {
	...
	constructor (address newOwnerChanger) Ownable(newOwnerChanger) internal {
	}

	// Disposable function to Get in touch with Holdefi contract
	function setHoldefiContract(HoldefiInterface holdefiContractAddress) public onlyOwner {
	...
	function getInterests (address market, uint totalSupply, uint totalBorrow) public view returns(uint borrowRate, uint supplyRate) {
	...
	function getMarketsList() public view returns (address[] memory res){
		res = marketsList;
	}
//SWC-100-Function Default Visibility:L120-122
	// Returns true if an asset is in the market list
	function getMarket(address market) public view returns (bool active){
		active = marketAssets[market].isActive;
	}

	// Returns the features of a collateral (Is active- VTL rate- Penalty rate- Bonus rate)
	function getCollateral(address collateral) public view returns (bool, uint, uint, uint){
		return(
	...
	function setBorrowRate (address market, uint newBorrowRate) public onlyOwner {
