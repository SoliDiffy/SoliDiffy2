...
abstract contract FarmBossV1 {
	...
	constructor(address payable _governance, address _daoMultisig, address _treasury, address _underlying) internal {
	...
	function getWhitelist(address _contract, bytes4 _fnSig) public view returns (uint256){
		return whitelist[_contract][_fnSig];
	}

	function changeFarmers(address[] calldata _newFarmers, address[] calldata _rmFarmers) public {
		require(msg.sender == governance, "FARMBOSSV1: !governance");
	...
	function emergencyRemoveFarmers(address[] calldata _rmFarmers) public {
	...
	function changeWhitelist(WhitelistData[] calldata _newActions, WhitelistData[] calldata _rmActions, Approves[] calldata _newApprovals, Approves[] calldata _newDepprovals) public {
		require(msg.sender == governance, "FARMBOSSV1: !governance");
	...
	function _addWhitelist(address _contract, bytes4 _fnSig, bool _msgValueAllowed) public {
		if (_msgValueAllowed){
	...
	function _approveMax(address _token, address _account) public {
