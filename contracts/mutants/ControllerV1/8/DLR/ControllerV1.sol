...
contract ControllerV1 is DelegateInterface, ControllerInterface, ControllerStorage, Adminable {
    ...
    function createLPoolPair(address token0, address token1, uint16 marginLimit, bytes storage dexData) external override {
        ...
        LPoolPairVar storage pairVar = LPoolPairVar(token0, token1, marginLimit, dexData, "OpenLeverage LToken", "LToken");
    ...
    function liquidateAllowed(uint marketId, address liquidator, uint liquidateAmount, bytes storage dexData) external override onlyOpenLevOperator(msg.sender) {
        // Shh - currently unused
        liquidateAmount;
        dexData;
    ...
    function initDistribution(uint totalAmount, uint64 startTime, uint64 duration) internal pure returns (ControllerStorage.LPoolDistribution storage distribution){
    ...
    function updateDistribution(ControllerStorage.LPoolDistribution memory distribution, uint addAmount) internal {
    ...
    function rewardPerToken(LPoolInterface lpool, bool isBorrow) internal view returns (uint256) {
        LPoolDistribution storage distribution = lpoolDistributions[lpool][isBorrow];
    ...
    function earnedInternal(LPoolInterface lpool, address account, bool isBorrow) internal view returns (uint256) {
        LPoolRewardByAccount storage accountReward = lPoolRewardByAccounts[lpool][isBorrow][account];
