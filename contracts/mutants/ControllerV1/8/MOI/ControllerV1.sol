...
contract ControllerV1 is DelegateInterface, ControllerInterface, ControllerStorage, Adminable {
    ...
    function createLPoolPair(address token0, address token1, uint16 marginLimit, bytes memory dexData) external override onlyAdmin {
    ...
    function updateDistribution(ControllerStorage.LPoolDistribution storage distribution, uint addAmount) internal onlyAdmin {
    ...
    function updateReward(LPoolInterface lpool, address account, bool isBorrow) internal onlyLPoolSender(lpool) returns (bool) {
    ...
    function stake(LPoolInterface lpool, address account, uint256 amount) internal onlyLPoolSender(lpool) returns (bool) {
    ...
    function withdraw(LPoolInterface lpool, address account, uint256 amount) internal onlyLPoolSender(lpool) returns (bool)  {
        bool updateSucceed = updateReward(lpool, account, false);
    ...
    function getRewardInternal(LPoolInterface lpool, address account, bool isBorrow) internal onlyLPoolSender(lpool) {
    ...
    function getSupplyRewards(LPoolInterface[] calldata lpools, address account) external override onlyAdmin {
        uint rewards;
