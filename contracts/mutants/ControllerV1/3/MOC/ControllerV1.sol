...
contract ControllerV1 is DelegateInterface, ControllerInterface, ControllerStorage, Adminable {
    ...
    /*** Policy Hooks ***/
    function mintAllowed(address lpool, address minter, uint lTokenAmount) external override onlyLPoolSender(lpool) onlyNotSuspended() onlyLPoolAllowed(lpool) {
    ...
    function redeemAllowed(address lpool, address redeemer, uint lTokenAmount) external override onlyNotSuspended() onlyLPoolSender(lpool) {
        if (withdraw(LPoolInterface(lpool), redeemer, lTokenAmount)) {
            getRewardInternal(LPoolInterface(lpool), redeemer, false);
        }
    }

    function borrowAllowed(address lpool, address borrower, address payee, uint borrowAmount) external override onlyLPoolSender(lpool) onlyLPoolAllowed(lpool) onlyNotSuspended() onlyOpenLevOperator(payee) {
