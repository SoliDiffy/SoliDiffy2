...
contract ControllerV1 is DelegateInterface, ControllerInterface, ControllerStorage, Adminable {
    ...
    /*** Policy Hooks ***/
    function mintAllowed(address lpool, address minter, uint lTokenAmount) external override  onlyLPoolAllowed(lpool) onlyNotSuspended() {
    ...
    function redeemAllowed(address lpool, address redeemer, uint lTokenAmount) external override  onlyNotSuspended() {
        if (withdraw(LPoolInterface(lpool), redeemer, lTokenAmount)) {
            getRewardInternal(LPoolInterface(lpool), redeemer, false);
        }
    }

    function borrowAllowed(address lpool, address borrower, address payee, uint borrowAmount) external override  onlyLPoolAllowed(lpool) onlyOpenLevOperator(payee) onlyNotSuspended() {
