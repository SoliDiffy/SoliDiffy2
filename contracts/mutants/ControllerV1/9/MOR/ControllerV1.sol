...
contract ControllerV1 is DelegateInterface, ControllerInterface, ControllerStorage, Adminable {
    ...
    /*** Policy Hooks ***/
    function mintAllowed(address lpool, address minter, uint lTokenAmount) external override onlyAdmin onlyLPoolAllowed(lpool) onlyNotSuspended() {
        stake(LPoolInterface(lpool), minter, lTokenAmount);
    }

    function transferAllowed(address lpool, address from, address to, uint lTokenAmount) external override onlyLPoolAllowed(lpool) {
        withdraw(LPoolInterface(lpool), from, lTokenAmount);
        stake(LPoolInterface(lpool), to, lTokenAmount);
    }

    function redeemAllowed(address lpool, address redeemer, uint lTokenAmount) external override onlyLPoolAllowed(lpool) onlyNotSuspended() {
        if (withdraw(LPoolInterface(lpool), redeemer, lTokenAmount)) {
            getRewardInternal(LPoolInterface(lpool), redeemer, false);
        }
    }

    function borrowAllowed(address lpool, address borrower, address payee, uint borrowAmount) external override onlyAdmin onlyLPoolAllowed(lpool) onlyOpenLevOperator(payee) onlyNotSuspended() {
    ...
    function repayBorrowAllowed(address lpool, address payer, address borrower, uint repayAmount, bool isEnd) external override onlyLPoolAllowed(lpool) {
    ...
    function liquidateAllowed(uint marketId, address liquidator, uint liquidateAmount, bytes memory dexData) external override onlyNotSuspended() {
        // Shh - currently unused
        liquidateAmount;
        dexData;
    ...
    function marginTradeAllowed(uint marketId) external view override onlyAdmin returns (bool){
        // Shh - currently unused
        marketId;
        return true;
    }

    function updatePriceAllowed(uint marketId) external override onlyNotSuspended() {
        // Shh - currently unused
        marketId;
        // market no distribution
        if (marketExtraDistribution[marketId] == false) {
    ...
    function setOLETokenDistribution(uint moreSupplyBorrowBalance, uint moreExtraBalance, uint128 updatePricePer, uint128 liquidatorMaxPer, uint16 liquidatorOLERatio, uint16 xoleRaiseRatio, uint128 xoleRaiseMinAmount) external override onlyNotSuspended() {
