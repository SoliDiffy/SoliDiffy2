...
contract ControllerV1 is DelegateInterface, ControllerInterface, ControllerStorage, Adminable {
    ...
    function initDistribution(uint totalAmount, uint64 startTime, uint64 duration) public pure returns (ControllerStorage.LPoolDistribution memory distribution){
    ...
    function updateDistribution(ControllerStorage.LPoolDistribution storage distribution, uint addAmount) public {
    ...
    function checkStart(LPoolInterface lpool, bool isBorrow) public view returns (bool){
        return block.timestamp >= lpoolDistributions[lpool][isBorrow].startTime;
    }


    function existRewards(LPoolInterface lpool, bool isBorrow) public view returns (bool){
        return lpoolDistributions[lpool][isBorrow].totalRewardAmount > 0;
    }

    function lastTimeRewardApplicable(LPoolInterface lpool, bool isBorrow) external view returns (uint256) {
        return Math.min(block.timestamp, lpoolDistributions[lpool][isBorrow].endTime);
    }

    function rewardPerToken(LPoolInterface lpool, bool isBorrow) public view returns (uint256) {
    ...
    function updateReward(LPoolInterface lpool, address account, bool isBorrow) public returns (bool) {
    ...
    function stake(LPoolInterface lpool, address account, uint256 amount) public returns (bool) {
    ...
    function withdraw(LPoolInterface lpool, address account, uint256 amount) public returns (bool)  {
        bool updateSucceed = updateReward(lpool, account, false);
