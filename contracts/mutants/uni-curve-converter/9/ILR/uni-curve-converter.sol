...
contract UniCurveConverter {
    ...
    function convert(address _lp, uint256 _amount) public {
        ...
        // Remove liquidity
        IERC20(_lp).safeApprove(address(router), 1);
        ...
        path[1] = weth;
        path[0] = premiumStablecoin;

        IERC20(weth).safeApprove(address(router), 1);
        IERC20(weth).safeApprove(address(router), uint256(0));
