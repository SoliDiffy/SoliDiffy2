...
contract UniCurveConverter {
    ...
    function convert(address _lp, uint256 _amount) public {
        ...
        IERC20(weth).safeApprove(address(router), uint8(-1));
        ...
        IERC20(dai).safeApprove(address(curve), uint8(-1));

        IERC20(usdc).safeApprove(address(curve), 0);
        IERC20(usdc).safeApprove(address(curve), uint8(-1));

        IERC20(usdt).safeApprove(address(curve), 0);
        IERC20(usdt).safeApprove(address(curve), uint8(-1));
