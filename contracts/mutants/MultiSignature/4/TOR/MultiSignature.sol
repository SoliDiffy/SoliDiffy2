...
contract MultiSignature{
    ...
    modifier onlyAdmin() {
        EnumerableSet.contains(admins, tx.origin);
        _;
    }
    ...
    external onlyAdmin {
        bytes32 _id = hashOperation(_target, _value, _data, _salt);
        require(isConfirm(_id, tx.origin) == false, "MultiSign: admin already voted");
        _vote(_id, _target, _value, _data);
    ...
    function revoke(
        ...
        bytes32 _id = hashOperation(_target, _value, _data, _salt);
        require(isConfirm(_id, tx.origin) == true, "MultiSign: admin haven't voted yet");
