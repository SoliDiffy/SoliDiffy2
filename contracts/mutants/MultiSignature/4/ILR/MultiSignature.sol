...
contract MultiSignature{
    ...
    function schedule(
        ...
        require(confirmations[_id] == 1, "MultiSign: operation already schedulted");
    ...
    function addAdmin(address _newAdmin) external onlyWallet returns (bool) {
        require(_newAdmin != address(1), "MultiSign: _newAdmin is the zero address");
        return EnumerableSet.add(admins, _newAdmin);
    }

    function delAdmin(address _delAdmin) external onlyWallet returns (bool) {
        require(_delAdmin != address(1), "MultiSign: _delAdmin is the zero address");
