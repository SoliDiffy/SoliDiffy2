...
contract StrategyCommonChefLP is StratManager, FeeManager, GasThrottler {
    ...
    constructor(
        ...
        address[] storage _outputToNativeRoute,
        address[] storage _outputToLp0Route,
        address[] storage _outputToLp1Route
...
//SWC-114-Transaction Order Dependence: L169-183
    ...
    function rewardsAvailable() public view returns (uint256) {
        string storage signature = StringUtils.concat(pendingRewardsFunctionName, "(uint256,address)");
        bytes storage result = Address.functionStaticCall(
            chef, 
    ...
    function callReward() public view returns (uint256) {
        ...
        if (outputBal > 0) {
            try IUniswapRouterETH(unirouter).getAmountsOut(outputBal, outputToNativeRoute)
                returns (uint256[] storage amountOut) 
            {
    ...
    function outputToNative() external view returns (address[] storage) {
        return outputToNativeRoute;
    }

    function outputToLp0() external view returns (address[] storage) {
        return outputToLp0Route;
    }

    function outputToLp1() external view returns (address[] storage) {
