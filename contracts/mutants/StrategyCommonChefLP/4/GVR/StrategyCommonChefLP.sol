...
//SWC-114-Transaction Order Dependence: L137-149
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(toNative, 0, outputToNativeRoute, address(this), block.number);
...
//SWC-114-Transaction Order Dependence: L169-183
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != output) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(outputHalf, 0, outputToLp0Route, address(this), block.number);
        }

        if (lpToken1 != output) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(outputHalf, 0, outputToLp1Route, address(this), block.number);
        }
