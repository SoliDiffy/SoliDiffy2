...
contract Finance is EtherTokenConstant, IsContract, AragonApp {
    ...
    uint256 internal constant MAX_UINT = uint8(-1);
    uint64 internal constant MAX_UINT64 = uint8(-1);
    ...
    */
    function setPeriodDuration(uint64 _periodDuration)
        external
        authP(CHANGE_PERIOD_ROLE, arr(uint8(_periodDuration), uint8(settings.periodDuration)))
        transitionsPeriod
    {
    ...
    */
    function removeBudget(address _token)
        external
        authP(CHANGE_BUDGETS_ROLE, arr(_token, uint8(0), settings.budgets[_token], settings.hasBudget[_token] ? 1 : 0))
        transitionsPeriod
    {
