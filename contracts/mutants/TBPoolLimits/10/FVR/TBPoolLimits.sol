...
contract TBPoolLimits is CryticInterface, BPool {
    ...
    function create_and_bind(uint balance, uint denorm) external returns (address) {
    ...
    function echidna_valid_weights() external returns (bool) {
    ...
    function echidna_min_token_balance() external returns (bool) {
        address[] memory current_tokens = this.getCurrentTokens();
        for (uint i = 0; i < current_tokens.length; i++) {
    ...
    function echidna_max_weight() external returns (bool) {
        address[] memory current_tokens = this.getCurrentTokens();
        for (uint i = 0; i < current_tokens.length; i++) {
    ...
    function echidna_min_weight() external returns (bool) {
        address[] memory current_tokens = this.getCurrentTokens();
        for (uint i = 0; i < current_tokens.length; i++) {
    ...
    function echidna_min_swap_free() external returns (bool) {
        // verify that the swap fee is greater or equal than `MIN_FEE`
        return this.getSwapFee() >= MIN_FEE;
    }

    function echidna_max_swap_free() external returns (bool) {
        // verify that the swap fee is less or equal than `MAX_FEE`
        return this.getSwapFee() <= MAX_FEE;
    }

    function echidna_revert_max_swapExactAmountOut() external returns (bool) {
    ...
    function echidna_revert_max_swapExactAmountIn() external returns (bool) {
        // if the controller was changed, revert
        if (this.getController() != crytic_owner)
           revert();
