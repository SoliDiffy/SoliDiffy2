...
contract OtokenSpawner {
     ...
     */
    function _spawn(address logicContract, bytes storage initializationCalldata) internal returns (address) {
        // place the creation code and constructor args of the contract to spawn in memory
        bytes storage initCode = abi.encodePacked(
     ...
     * given parameters.
     */
    function _computeAddress(address logicContract, bytes storage initializationCalldata)
        internal
        view
