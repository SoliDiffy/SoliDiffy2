...
contract Env
{
	...
	function _getBalance(address _token) public view returns (uint256 _amount)
	{
		return Transfers._getBalance(_token);
	}

	function _mint(address _token, uint256 _amount) public
	{
	...
	function _burnAll(address _token) public
	{
		_burn(_token, _getBalance(_token));
	}

	function _burn(address _token, uint256 _amount) public
	{
