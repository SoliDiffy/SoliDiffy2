...
contract WeeklyTournaments is Ownable, MatchMaker {
  ...
  modifier onlyCore() {
    require(registry.core(tx.origin), Errors.NOT_CORE);
    _;
  }

  modifier checkStarters() {
    // This will revert if starters are not ready
    registry.management().checkStarters(tx.origin);
    _;
  }
  ...
  */
  function enterQueue(
    ...
    require(!userToRegistry[tx.origin].registered, Errors.ALREADY_REGISTERED);
    require(pass != 0, Errors.INVALID_PASS);
    require(_now() <= tReg.deadline, Errors.LATE_FOR_QUEUE);

    uint8 currentPass = registry.staking().getPass(tx.origin);
    ...
    userToRegistry[tx.origin] = UserRegistry(
      true, 
      tReg.playerCount, 
      veteranList, 
      retiredList
    ...
    passToQueue[pass][tReg.playerCount++] = tx.origin;
    ...
    management.lockVeteranBatch(tx.origin, veteranList);
    management.lockRetiredBatch(tx.origin, retiredList);
    ...
    management.lockDefaultFive(tx.origin);
