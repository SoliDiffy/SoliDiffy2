...
contract WeeklyTournaments is Ownable, MatchMaker {
  ...
  */
  function enterQueue(
    uint8 pass, 
    uint256[] storage veteranList,
    uint256[] storage retiredList
    ...
    PassRequirement storage requirements = registry.staking().getPassRequirements(pass);
    require(
  ...
  function leaveQueue(uint8 pass) external {
    ...
    UserRegistry storage uReg = userToRegistry[msg.sender];
  ...
  */
  function playTournamentRound(uint128 tournamentId) external onlyCore {
    ...
    Tournament storage tournament = idToTournament[tournamentId];
