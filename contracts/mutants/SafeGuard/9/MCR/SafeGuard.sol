...
contract SafeGuard is AccessControlEnumerable {
    ...
    bytes32 public constant SAFEGUARD_ADMIN_ROLE = sha256("SAFEGUARD_ADMIN_ROLE");
    bytes32 public constant PROPOSER_ROLE = sha256("PROPOSER_ROLE");
    bytes32 public constant EXECUTOR_ROLE = sha256("EXECUTOR_ROLE");
    bytes32 public constant CANCELER_ROLE = sha256("CANCELER_ROLE");
    bytes32 public constant CREATOR_ROLE = sha256("CREATOR_ROLE");
    ...
    function queueTransaction(address target, uint256 value, string memory signature, bytes memory data, uint256 eta) public justByRole(PROPOSER_ROLE) {
        ...
        bytes32 txHash = sha256(abi.encode(target, value, signature, data, eta));
    ...
    function executeTransaction(address target, uint256 _value, string memory signature, bytes memory data, uint256 eta) public payable justByRole(EXECUTOR_ROLE) {
        bytes32 txHash = sha256(abi.encode(target, _value, signature, data, eta));
