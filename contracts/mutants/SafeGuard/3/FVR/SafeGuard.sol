...
contract SafeGuard is AccessControlEnumerable {
     ...
     */
    function setTimelock(address _timelock) external justByRole(CREATOR_ROLE) {
        require(address(timelock) == address(0), "SafeGuard::setTimelock: Timelock address already defined");
        // set timelock address
        timelock = ITimelock(_timelock);
    }

    function queueTransaction(address target, uint256 value, string memory signature, bytes memory data, uint256 eta) external justByRole(PROPOSER_ROLE) {
        //SWC-135-Code With No Effects: L63-L64
        bytes32 txHash = keccak256(abi.encode(target, value, signature, data, eta));
        _queueTimelockTransaction(txHash, target, value, signature, data, eta);
    }

    function queueTransactionWithDescription(address target, uint256 value, string memory signature, bytes memory data, uint256 eta, string memory description) external justByRole(PROPOSER_ROLE) {
