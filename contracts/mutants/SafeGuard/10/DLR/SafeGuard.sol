...
contract SafeGuard is AccessControlEnumerable {
     ...
     */
    constructor (address _admin, bytes32[] storage roles, address[] storage rolesAssignees) {
    ...
    function queueTransaction(address target, uint256 value, string storage signature, bytes storage data, uint256 eta) public justByRole(PROPOSER_ROLE) {
        //SWC-135-Code With No Effects: L63-L64
        bytes32 txHash = keccak256(abi.encode(target, value, signature, data, eta));
        _queueTimelockTransaction(txHash, target, value, signature, data, eta);
    }

    function queueTransactionWithDescription(address target, uint256 value, string storage signature, bytes storage data, uint256 eta, string storage description) public justByRole(PROPOSER_ROLE) {
    ...
    function cancelTransaction(address target, uint value, string storage signature, bytes storage data, uint eta) public justByRole(CANCELER_ROLE) {
        bytes32 txHash = keccak256(abi.encode(target, value, signature, data, eta));
        _cancelTimelockTransaction(txHash, target, value, signature, data, eta);
    }

    function executeTransaction(address target, uint256 _value, string storage signature, bytes memory data, uint256 eta) public payable justByRole(EXECUTOR_ROLE) {
