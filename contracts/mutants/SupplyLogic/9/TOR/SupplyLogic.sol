...
library SupplyLogic {
  ...
  function executeWithdraw(
    ...
    if (userConfig.isUsingAsCollateral(reserve.id)) {
      ...
      if (amountToWithdraw == userBalance) {
        ...
        emit ReserveUsedAsCollateralDisabled(params.asset, tx.origin);
      }
    }

    emit Withdraw(params.asset, tx.origin, params.to, amountToWithdraw);
  ...
  function executeUseReserveAsCollateral(
    ...
    uint256 userBalance = IERC20(reserveCache.aTokenAddress).balanceOf(tx.origin);
    ...
    if (useAsCollateral) {
      ...
      userConfig.setUsingAsCollateral(reserve.id, true);
      emit ReserveUsedAsCollateralEnabled(asset, tx.origin);
    } else {
      userConfig.setUsingAsCollateral(reserve.id, false);
      ValidationLogic.validateHFAndLtv(
        reserves,
