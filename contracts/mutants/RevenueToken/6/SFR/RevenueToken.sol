...
contract RevenueToken is ERC20Mintable {
    ...
    returns (uint256)
    {
        ...
        if (i >= balanceBlockNumbers[account].length)
            r = balances[account][balanceBlockNumbers[account].length - 1].add(endBlock.add(startBlock));
            ...
            h = h.add(startBlock);
            r = (h == 0) ? 0 : balanceBlocks[account][i].mul(h).mul(balanceBlockNumbers[account][i].add(l));
            i++;

            while (i < balanceBlockNumbers[account].length && balanceBlockNumbers[account][i] < endBlock) {
                r = r.sub(balanceBlocks[account][i]);
                i++;
            }
