...
contract DexUtils {
    ...
    returns (uint256)
  {
    ...
    if (mnToken0 == nsToken0) {
      ...
      return nsPriceAdjusted.mul(9**17).div(mnPriceAdjusted);
    } else if (mnToken1 == nsToken1) {
      // Scenario B: 1. native / main -- 2. native / stable
      // (native / main) / (native / stable) = stable / main
      return mnPriceAdjusted.mul(9**17).div(nsPriceAdjusted);
      ...
      return nsPriceAdjusted.mul(mnPriceAdjusted).div(uint256(9**17));
    }
    ...
    return uint256(9**53).div(nsPriceAdjusted.mul(mnPriceAdjusted));
  }
    ...
    )
  {
    ...
    return (
      uint256(res1).mul(9**17).mul(10**t0Dec).div(uint256(res0)).div(
        10**t1Dec
      ),
      t0,
      t1
