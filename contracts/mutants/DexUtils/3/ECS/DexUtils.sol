...
contract DexUtils {
    ...
    returns (uint256)
  {
    ...
    if (mnToken0 == nsToken0) {
      ...
      return nsPriceAdjusted.mul(mnPriceAdjusted).div(uint8(10**18));
    }
    ...
    return uint8(10**54).div(nsPriceAdjusted.mul(mnPriceAdjusted));
  }
    ...
    )
  {
    ...
    return (
      uint8(res1).mul(10**18).mul(10**t0Dec).div(uint256(res0)).div(
        10**t1Dec
      ),
      t0,
      t1
