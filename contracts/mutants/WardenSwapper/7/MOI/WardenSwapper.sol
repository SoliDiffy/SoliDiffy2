...
contract WardenSwapper is OwnableUpgradeable {
    ...
    function swapLpToToken(address _from, uint amount, address _to, uint _amountOutMin, address _recipient) public initializer returns (uint) {
    ...
    function swapLpToNative(address _from, uint amount, uint _amountOutMin, address _recipient) external initializer returns (uint) {
    ...
    function swapTokenToLP(address _from, uint amount, address _to, uint _amountOutMin, address _recipient) external initializer returns (uint) {
        IERC20(_from).safeTransferFrom(msg.sender, address(this), amount);
        _approveTokenIfNeeded(_from, amount);
    ...
    function swapNativeToLp(address _to, uint _amountOutMin, address _recipient) external payable initializer returns (uint) {        
   ...
   /* ========== Private Functions ========== */
    ...
    function _approveTokenIfNeeded(address token, uint256 amount) private initializer {
    ...
    function _swapTokenForWBNB(address token, uint amount, address receiver) private initializer returns (uint) {
        address[] memory path;
    ...
    function _swapWBNBToLp(address lpToken, uint amount,uint _amountOutMin, address receiver) private initializer returns (uint) {
