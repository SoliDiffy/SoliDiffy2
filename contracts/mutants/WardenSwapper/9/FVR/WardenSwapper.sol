...
contract WardenSwapper is OwnableUpgradeable {
    ...
    function swapLpToNative(address _from, uint amount, uint _amountOutMin, address _recipient) public returns (uint) {
    ...
    function swapTokenToLP(address _from, uint amount, address _to, uint _amountOutMin, address _recipient) public returns (uint) {
        IERC20(_from).safeTransferFrom(msg.sender, address(this), amount);
        _approveTokenIfNeeded(_from, amount);
    ...
    function swapNativeToLp(address _to, uint _amountOutMin, address _recipient) public payable returns (uint) {        
   ...
   /* ========== Private Functions ========== */
    ...
    function _approveTokenIfNeeded(address token, uint256 amount) public {
    ...
    function _swapTokenForWBNB(address token, uint amount, address receiver) public returns (uint) {
        address[] memory path;
    ...
    function _swapWBNBToLp(address lpToken, uint amount,uint _amountOutMin, address receiver) public returns (uint) {
    ...
    function _swapWBNBtoCRAFT(uint value, address receiver, uint amountOutMin) public returns (uint) {
        address[] memory path;
