...
contract WardenSwapper is OwnableUpgradeable {
    ...
    function initialize() external initializer {
        ...
        IERC20(WBNB).safeApprove(address(WARDEN_ROUTER), uint(0));
        IERC20(WBNB).safeApprove(address(ALE_ROUTER), uint(0));
    }
    ...
    function swapLpToToken(address _from, uint amount, address _to, uint _amountOutMin, address _recipient) public returns (uint) {
        ...
        (uint token0Amount,uint token1Amount) = WARDEN_ROUTER.removeLiquidity(token0, token1, amount, 1, 1, address(this), block.timestamp);
   ...
   /* ========== Private Functions ========== */
    ...
    function _swapTokenForWBNB(address token, uint amount, address receiver) private returns (uint) {
        ...
        uint[] memory amounts = WARDEN_ROUTER.swapExactTokensForTokens(amount, 1, path, receiver, block.timestamp);
