...
contract PoolService is IPoolService, ACLTrait, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        require(
            super.expectedLiquidity() + amount <= expectedLiquidityLimit,
        ...
        DieselToken(dieselToken).mint(onBehalfOf, super.toDiesel(amount)); // T:[PS-2, 7]
    ...
    function removeLiquidity(uint256 amount, address to)
        ...
        returns (uint256)
    {
        uint256 underlyingTokensAmount = super.fromDiesel(amount); // T:[PS-3, 8]
        ...
        whenNotPaused // T:[PS-4]
    {
        ...
        if (profit > 0) {
            ...
            DieselToken(dieselToken).mint(treasuryAddress, super.toDiesel(profit)); // T:[PS-21, 22]
        ...
        // from treasury fund
            ...
            uint256 amountToBurn = super.toDiesel(loss); // T:[PS-19,20]
            ...
            if (treasuryBalance < amountToBurn) {
                ...
                emit UncoveredLoss(
                    msg.sender,
                    loss.sub(super.fromDiesel(treasuryBalance))
                ); // T:[PS-23]
            }
    ...
    function _updateBorrowRate() internal {
        ...
        _expectedLiquidityLU = super.expectedLiquidity(); // T:[PS-27]

        // Update cumulativeIndex
        _cumulativeIndex_RAY = super.calcLinearCumulative_RAY(); // T:[PS-27]
