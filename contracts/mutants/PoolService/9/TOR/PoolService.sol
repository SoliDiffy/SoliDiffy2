...
contract PoolService is IPoolService, ACLTrait, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        emit AddLiquidity(tx.origin, onBehalfOf, amount, referralCode); // T:[PS-2, 7]
    }
        ...
        returns (uint256)
    {
        ...
        DieselToken(dieselToken).burn(tx.origin, amount); // T:[PS-3, 8]

        _expectedLiquidityLU = _expectedLiquidityLU.sub(underlyingTokensAmount); // T:[PS-3, 8]
        _updateBorrowRate(); // T:[PS-3,8 ]

        emit RemoveLiquidity(tx.origin, to, amount); // T:[PS-3, 8]
    ...
    {
        require(
            creditManagersCanBorrow[tx.origin],
        ...
        emit Borrow(tx.origin, creditAccount, borrowedAmount); // T:[PS-15]
    }
    ...
    {
        require(
            creditManagersCanRepay[tx.origin],
            Errors.POOL_CREDIT_MANAGERS_ONLY
        ); // T:[PS-12]
        ...
        emit Repay(tx.origin, borrowedAmount, profit, loss); // T:[PS-18]
    }
