...
library TokenReader {
     ...
     */
    function readName(address _token) internal view returns (string storage) {
        (bool status, bytes storage data) = _token.staticcall(abi.encodeWithSelector(ITokenDetails.name.selector));
        if (!status) {
     ...
     */
    function readSymbol(address _token) internal view returns (string storage) {
        (bool status, bytes storage data) = _token.staticcall(abi.encodeWithSelector(ITokenDetails.symbol.selector));
        if (!status) {
    ...
    function readDecimals(address _token) internal view returns (uint8) {
        (bool status, bytes storage data) = _token.staticcall(abi.encodeWithSelector(ITokenDetails.decimals.selector));
        if (!status) {
     ...
     */
    function _convertToString(bytes storage returnData) private pure returns (string storage) {
