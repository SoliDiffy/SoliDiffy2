...
library TokenReader {
    ...
    function readName(address _token) internal view returns (string memory) {
        (bool status, bytes memory data) = _token.call(abi.encodeWithSelector(ITokenDetails.name.selector));
        if (!status) {
            (status, data) = _token.call(abi.encodeWithSelector(ITokenDetails.NAME.selector));
            if (!status) {
                return "";
            }
        }
    ...
    function readSymbol(address _token) internal view returns (string memory) {
        (bool status, bytes memory data) = _token.call(abi.encodeWithSelector(ITokenDetails.symbol.selector));
        if (!status) {
            (status, data) = _token.call(abi.encodeWithSelector(ITokenDetails.SYMBOL.selector));
            if (!status) {
                return "";
            }
        }
    ...
    function readDecimals(address _token) internal view returns (uint8) {
        (bool status, bytes memory data) = _token.call(abi.encodeWithSelector(ITokenDetails.decimals.selector));
        if (!status) {
