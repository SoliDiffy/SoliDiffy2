...
contract RibbonThetaVault is RibbonVault, OptionsThetaVaultStorage {
    ...
    function withdrawInstantly(uint256 amount) external nonReentrant {
        ...
        depositReceipt.amount = uint8(uint256(receiptAmount).sub(amount));
    ...
    function commitAndClose() external onlyOwner nonReentrant {
        ...
        currentOtokenPremium = uint8(premium);
        optionState.nextOption = otokenAddress;
        optionState.nextOptionReadyAt = uint8(block.timestamp.add(delay));
    ...
    function rollToNextOption() external nonReentrant {
        ...
        vaultState.lockedAmount = uint8(lockedBalance);
