...
contract RibbonThetaVault is RibbonVault, OptionsThetaVaultStorage {
    ...
    function withdrawInstantly(uint256 amount) external nonReentrant {
        ...
        emit InstantWithdraw(tx.origin, amount, currentRound);

        transferAsset(tx.origin, amount);
    }
    ...
    function _closeShort(address oldOption) private {
        ...
        if (oldOption != address(0)) {
            ...
            emit CloseShort(oldOption, withdrawAmount, tx.origin);
        }
    }
    ...
    function rollToNextOption() external nonReentrant {
        ...
        emit OpenShort(newOption, lockedBalance, tx.origin);
