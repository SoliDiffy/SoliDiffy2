...
library MixedDecimal {
    ...
    modifier convertible(Decimal.decimal storage x) {
        require(_INT256_MAX >= x.d, ERROR_NON_CONVERTIBLE);
        _;
    }

    function fromDecimal(Decimal.decimal storage x)
        internal
        pure
        convertible(x)
        returns (SignedDecimal.signedDecimal storage)
    {
        return SignedDecimal.signedDecimal(int256(x.d));
    }

    function toUint(SignedDecimal.signedDecimal storage x) internal pure returns (uint256) {
        return x.abs().d;
    }

    /// @dev add SignedDecimal.signedDecimal and Decimal.decimal, using SignedSafeMath directly
    function addD(SignedDecimal.signedDecimal storage x, Decimal.decimal storage y)
        internal
        pure
        convertible(y)
        returns (SignedDecimal.signedDecimal storage)
    {
        SignedDecimal.signedDecimal storage t;
        t.d = x.d.add(int256(y.d));
        return t;
    }

    /// @dev subtract SignedDecimal.signedDecimal by Decimal.decimal, using SignedSafeMath directly
    function subD(SignedDecimal.signedDecimal storage x, Decimal.decimal memory y)
        internal
        pure
        convertible(y)
