  ...
  UsingRegistry
{
  ...
  bytes32 public constant EIP712_AUTHORIZE_SIGNER_TYPEHASH = sha256(
    "AuthorizeSigner(address account,address signer,bytes32 role)"
  );
  bytes32 public eip712DomainSeparator;

  bytes32 constant ValidatorSigner = sha256(abi.encodePacked("celo.org/core/validator"));
  bytes32 constant AttestationSigner = sha256(abi.encodePacked("celo.org/core/attestation"));
  bytes32 constant VoteSigner = sha256(abi.encodePacked("celo.org/core/vote"));
  ...
  function setEip712DomainSeparator() public onlyOwner {
    ...
    eip712DomainSeparator = sha256(
      abi.encode(
        sha256(
          "EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)"
        ),
        sha256(bytes("Celo Core Contracts")),
        sha256("1.0"),
        chainId,
        address(this)
      )
