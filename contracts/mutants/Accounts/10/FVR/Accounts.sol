  ...
  UsingRegistry
{
   ...
   */
  function setEip712DomainSeparator() external onlyOwner {
    uint256 chainId;
    assembly {
      chainId := chainid
    }
   ...
   * @param dataEncryptionKey secp256k1 public key for data encryption. Preferably compressed.
   */
  function setAccountDataEncryptionKey(bytes memory dataEncryptionKey) external {
   ...
   */
  function setMetadataURL(string calldata metadataURL) public {
    require(isAccount(msg.sender), "Unknown account");
    Account storage account = accounts[msg.sender];
