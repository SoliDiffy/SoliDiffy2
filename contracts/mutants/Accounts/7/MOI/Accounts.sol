  ...
  UsingRegistry
{
   ...
   */
  function createAccount() public initializer returns (bool) {
   ...
   */
  function setName(string memory name) public initializer {
   ...
   *      is 0x0 or msg.sender).
   */
  function setWalletAddress(address walletAddress, uint8 v, bytes32 r, bytes32 s) public initializer {
    require(isAccount(msg.sender), "Unknown account");
   ...
   * @param dataEncryptionKey secp256k1 public key for data encryption. Preferably compressed.
   */
  function setAccountDataEncryptionKey(bytes memory dataEncryptionKey) public initializer {
   ...
   */
  function setMetadataURL(string calldata metadataURL) external initializer {
    require(isAccount(msg.sender), "Unknown account");
    Account storage account = accounts[msg.sender];
   ...
   */
  function setIndexedSigner(address signer, bytes32 role) public initializer {
