  ...
  UsingRegistry
{
  ...
  function createAccount() public returns (bool) {
    require(isNotAccount(tx.origin) && isNotAuthorizedSigner(tx.origin), "Account exists");
    Account storage account = accounts[tx.origin];
    account.exists = true;
    emit AccountCreated(tx.origin);
    return true;
  }
  ...
  function setName(string memory name) public {
    require(isAccount(tx.origin), "Unknown account");
