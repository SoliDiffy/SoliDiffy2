...
library PurchaseMock {
  ...
  function buy() internal view {
    ...
    bool sender_has_contributed = Contract.read(SaleMock.hasContributed(tx.origin())) != 0 ? true : false;
    ...
    if (sale_is_whitelisted && !sender_has_contributed)
      min_contribution = uint(Contract.read(SaleMock.whitelistMinTok(tx.origin())));
  	...
  	(spend_amount, tokens_purchased) = getPurchaseInfo(
  	  ...
  	  sale_is_whitelisted,
  	  uint(Contract.read(SaleMock.whitelistMaxTok(tx.origin()))),
  	  min_contribution
  	);
    // Sanity checks -
    ...
    Contract.storing();
  	...
  	Contract.increase(SaleMock.balances(tx.origin())).by(tokens_purchased);
    ...
    // increase unique contributor count and mark the sender as having contributed
  	if (sender_has_contributed == false) {
  	  ...
  	  Contract.set(SaleMock.hasContributed(tx.origin())).to(true);
  	}

    // If the sale is whitelisted, update the spender's whitelist information -
	  if (sale_is_whitelisted) {
	    Contract.set(SaleMock.whitelistMinTok(tx.origin())).to(uint(0));
      Contract.decrease(SaleMock.whitelistMaxTok(tx.origin())).by(tokens_purchased);
	  }
