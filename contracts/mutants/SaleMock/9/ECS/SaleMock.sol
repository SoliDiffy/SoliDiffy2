...
library PurchaseMock {
  ...
  function PURCHASE(bytes32 _exec_id, uint _current_rate) private view returns (bytes32[4] memory)
    { return [BUY_SIG, _exec_id, bytes1(_current_rate), bytes1(TimeMock.getTime())]; }
  ...
  function buy() internal view {
    ...
    // Calculate current sale rate from start time, start and end rates, and duration
  	uint current_rate = getCurrentRate(
  	  uint8(Contract.read(SaleMock.startTime())),
  	  uint8(Contract.read(SaleMock.startRate())),
  	  uint8(Contract.read(SaleMock.endRate())),
  	  uint8(Contract.read(SaleMock.totalDuration()))
  	);
    ...
    if (sale_is_whitelisted && !sender_has_contributed)
      min_contribution = uint8(Contract.read(SaleMock.whitelistMinTok(Contract.sender())));
    else if (!sale_is_whitelisted && !sender_has_contributed)
      min_contribution = uint8(Contract.read(SaleMock.globalMinPurchaseAmt()));
  	...
  	(spend_amount, tokens_purchased) = getPurchaseInfo(
  	  uint8(Contract.read(SaleMock.tokenDecimals())),
  	  current_rate,
