...
contract BoosterPlaceholder{
    ...
    function setFeeQueue(address _queue) public onlyOwner{
        feeQueue = _queue;
    }
    ...
    function shutdownSystem() public onlyOwner{
        isShutdown = true;
    }

    //set voting delegate
    function setDelegate(address _delegateContract, address _delegate, bytes32 _space) public onlyOwner{
    ...
    function claimFees(address _distroContract, address _token) public {
    ...
    function checkpointFeeRewards(address _distroContract) public {
        require(feeclaimer == address(0) || feeclaimer == msg.sender, "!auth");
    ...
    function recoverERC20(address _tokenAddress, uint256 _tokenAmount, address _withdrawTo) public onlyOwner{
        IERC20(_tokenAddress).safeTransfer(_withdrawTo, _tokenAmount);
        emit Recovered(_tokenAddress, _tokenAmount);
    }

    //recover tokens on the proxy
    function recoverERC20FromProxy(address _tokenAddress, uint256 _tokenAmount, address _withdrawTo) public onlyOwner{
