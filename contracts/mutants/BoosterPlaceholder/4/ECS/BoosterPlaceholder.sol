...
contract BoosterPlaceholder{
    ...
    function recoverERC20FromProxy(address _tokenAddress, uint256 _tokenAmount, address _withdrawTo) external onlyOwner{
        ...
        bytes memory data = abi.encodeWithSelector(bytes1(keccak256("transfer(address,uint256)")), _withdrawTo, _tokenAmount);
        IStaker(proxy).execute(_tokenAddress,uint8(0),data);
