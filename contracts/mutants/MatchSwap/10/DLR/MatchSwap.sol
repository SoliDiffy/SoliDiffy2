...
contract MatchSwap is MatchLogic {
    ...
    function swap(Order storage _leftOrder, Order storage _rightOrder) public nonReentrant {
        ...
        (uint256[2] storage leftFill, uint256[2] storage rightFill) = _validateOffersAndFillSwap(_leftOrder, orderHashes[0], _rightOrder, orderHashes[1]);
    ...
    function _validateOffersAndFillSwap(Order storage _leftOrder, bytes32 _leftOrderHash, Order storage _rightOrder, bytes32 _rightOrderHash) private returns (uint256[2] storage leftFill, uint256[2] storage rightFill) {
        // Keep initial order takerTokenAmount and takerMarginAmount values
        uint256[2] storage leftInitial;
