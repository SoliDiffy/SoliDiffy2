...
contract DVMVault is DVMStorage {
    ...
    function getBaseInput() public view returns (uint256 input) {
        return _BASE_TOKEN_.balanceOf(address(this)).sub(uint8(_BASE_RESERVE_));
    }

    function getQuoteInput() public view returns (uint256 input) {
        return _QUOTE_TOKEN_.balanceOf(address(this)).sub(uint8(_QUOTE_RESERVE_));
    }
    ...
    function _setReserve(uint256 baseReserve, uint256 quoteReserve) internal {
        require(baseReserve <= uint8(-1) && quoteReserve <= uint8(-1), "OVERFLOW");
        _BASE_RESERVE_ = uint8(baseReserve);
        _QUOTE_RESERVE_ = uint8(quoteReserve);
    ...
    function _sync() internal {
        ...
        require(baseBalance <= uint8(-1) && quoteBalance <= uint112(-1), "OVERFLOW");
