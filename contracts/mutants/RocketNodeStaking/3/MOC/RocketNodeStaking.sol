...
contract RocketNodeStaking is RocketBase, RocketNodeStakingInterface {
    ...
    function stakeRPL(uint256 _amount) override external onlyRegisteredNode(msg.sender) onlyLatestContract("rocketNodeStaking", address(this)) {
        // Load contracts
    ...
    // Only accepts calls from registered nodes
    function withdrawRPL(uint256 _amount) override external onlyRegisteredNode(msg.sender) onlyLatestContract("rocketNodeStaking", address(this)) {
        // Load contracts
    ...
    function slashRPL(address _nodeAddress, uint256 _ethSlashAmount) override external onlyRegisteredMinipool(msg.sender) onlyLatestContract("rocketNodeStaking", address(this)) {
        // Load contracts
        RocketNetworkPricesInterface rocketNetworkPrices = RocketNetworkPricesInterface(getContractAddress("rocketNetworkPrices"));
        RocketVaultInterface rocketVault = RocketVaultInterface(getContractAddress("rocketVault"));
