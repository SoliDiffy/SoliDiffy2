...
contract RocketNodeStaking is RocketBase, RocketNodeStakingInterface {
    ...
    function stakeRPL(uint256 _amount) override external onlyLatestContract("rocketNodeStaking", address(this)) onlyRegisteredNode(tx.origin) {
        // Load contracts
        ...
        // Transfer RPL tokens
        require(rplToken.transferFrom(tx.origin, address(this), _amount), "Could not transfer RPL to staking contract");
        ...
        uint256 rplStake = getNodeRPLStake(tx.origin);
        // Update RPL stake amounts & node RPL staked block
        increaseTotalRPLStake(_amount);
        increaseNodeRPLStake(tx.origin, _amount);
        updateTotalEffectiveRPLStake(tx.origin, rplStake, rplStake.add(_amount));
        setNodeRPLStakedTime(tx.origin, block.timestamp);
        // Emit RPL staked event
        emit RPLStaked(tx.origin, _amount, block.timestamp);
    }

    // Withdraw staked RPL back to the node account
    // Only accepts calls from registered nodes
    function withdrawRPL(uint256 _amount) override external onlyLatestContract("rocketNodeStaking", address(this)) onlyRegisteredNode(tx.origin) {
        // Load contracts
