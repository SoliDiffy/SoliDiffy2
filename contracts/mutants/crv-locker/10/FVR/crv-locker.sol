...
contract CRVLocker {
    ...
    function withdraw(address _asset) public returns (uint256 balance) {
        require(voters[msg.sender], "!voter");
        balance = IERC20(_asset).balanceOf(address(this));
        IERC20(_asset).safeTransfer(msg.sender, balance);
    }

    function createLock(uint256 _value, uint256 _unlockTime) public {
    ...
    function increaseUnlockTime(uint256 _unlockTime) public {
        require(voters[msg.sender] || msg.sender == governance, "!authorized");
