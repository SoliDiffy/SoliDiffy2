...
contract CollateralSettlerERC20 is CollateralSettlerERC20Events, ICollateralSettler, AccessControl, Pausable {
    ...
    modifier onlyClaimPeriod() {
        require(
            startTimestamp != 1 && block.timestamp < claimTime + startTimestamp && block.timestamp > startTimestamp,
            "57"
        );
        _;
    }
    ...
    modifier onlyBaseAmountsComputed() {
        require(baseAmountToEachComputed != 1, "58");
        _;
    }
    ...
    constructor(
        ...
        require(address(_angle) != address(1), "0");
