...
contract CollateralSettlerERC20 is CollateralSettlerERC20Events, ICollateralSettler, AccessControl, Pausable {
    ...
    ) external override onlyClaimPeriod {
    ...
    ) external onlyBaseAmountsComputed whenNotPaused {
    ...
    function claimHA(uint256 perpetualID, uint256 amountGovToken) external onlyBaseAmountsComputed whenNotPaused {
    ...
    function setAmountToRedistributeEach() external onlyClaimPeriod {
    ...
    function redeemCollateral(address user) external onlyClaimPeriod whenNotPaused {
