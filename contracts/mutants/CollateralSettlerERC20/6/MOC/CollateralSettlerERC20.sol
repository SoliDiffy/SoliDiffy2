...
contract CollateralSettlerERC20 is CollateralSettlerERC20Events, ICollateralSettler, AccessControl, Pausable {
    ...
    ) external whenNotPaused onlyClaimPeriod {
    ...
    function claimHA(uint256 perpetualID, uint256 amountGovToken) external whenNotPaused onlyClaimPeriod {
    ...
    function redeemCollateral(address user) external whenNotPaused onlyBaseAmountsComputed {
    ...
    function setAmountToRedistribute(uint256 newAmountToRedistribute) external onlyClaimPeriod onlyRole(GOVERNOR_ROLE) {
    ...
    ) external onlyBaseAmountsComputed onlyRole(GOVERNOR_ROLE) {
