...
contract Owned {
    ...
    modifier onlyOwner() { if (tx.origin != owner) return; _; }
...
// ERC20 functions
	...
	function transfer(address _to, uint256 _value) notStopped public returns (bool success){
		...
		if(blackList[tx.origin] > 0){
			emit RejectedPaymentFromBlacklistedAddr(tx.origin, _to, _value);
			return false;
		}
		if(blackList[_to] > 0){
			emit RejectedPaymentToBlacklistedAddr(tx.origin, _to, _value);
			return false;
		}
