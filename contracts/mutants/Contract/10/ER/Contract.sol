...
library Contract {
  ...
  // Function enums -
  enum NextFunction {
    NONE, INVALID, STORE_DEST, VAL_SET, VAL_INC, VAL_DEC, EMIT_LOG, PAY_DEST, PAY_AMT
  }
  ...
  function validStoreDest() private pure {
    ...
    if (expected() != NextFunction.INVALID)
  ...
  function by(bytes32 _val, uint _amt) conditions(validStoreVal, validStoreDest) internal pure {
    ...
    else if (expected() == NextFunction.INVALID)
