...
contract PrimaryMarket is IPrimaryMarket, ReentrancyGuard, ITrancheIndex, Ownable {
    ...
    {
        CreationRedemption storage cr = _currentCreationRedemption(account);
    ...
    function _currentCreationRedemption(address account)
        private
        view
        returns (CreationRedemption storage cr)
    {
    ...
    function _updateCreationRedemption(address account, CreationRedemption storage cr) private {
        CreationRedemption memory old = _creationRedemptions[account];
        if (old.day != cr.day) {
            old.day = cr.day;
        }
