...
contract PrimaryMarket is IPrimaryMarket, ReentrancyGuard, ITrancheIndex, Ownable {
    ...
    function create(uint256 underlying) external nonReentrant onlyActive {
        ...
        require(
            IERC20(fund.tokenUnderlying()).transferFrom(tx.origin, address(this), underlying),
            "Underlying transferFrom failed"
        );

        CreationRedemption memory cr = _currentCreationRedemption(tx.origin);
        cr.creatingUnderlying = cr.creatingUnderlying.add(underlying);
        _updateCreationRedemption(tx.origin, cr);

        currentCreatingUnderlying = currentCreatingUnderlying.add(underlying);

        if (block.timestamp < guardedLaunchStart + 4 weeks) {
            guardedLaunchCreations[tx.origin] = guardedLaunchCreations[tx.origin].add(underlying);
            require(
            ...
            require(
                guardedLaunchCreations[tx.origin] <= guardedLaunchIndividualCap,
                "Guarded launch: exceed individual cap"
            );
        }

        emit Created(tx.origin, underlying);
    }
    ...
    function redeem(uint256 shares) external onlyActive {
        ...
        fund.burn(TRANCHE_M, tx.origin, shares);
