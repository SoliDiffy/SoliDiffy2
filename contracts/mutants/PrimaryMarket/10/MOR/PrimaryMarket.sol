...
contract PrimaryMarket is IPrimaryMarket, ReentrancyGuard, ITrancheIndex, Ownable {
    ...
    function create(uint256 underlying) external Ownable() onlyActive {
    ...
    function redeem(uint256 shares) external Ownable() {
    ...
    function split(uint256 inM) external Ownable() {
        require(
    ...
    function merge(uint256 inA) external Ownable() {
        (uint256 weightA, uint256 weightB) = fund.trancheWeights();
    ...
    function updateRedemptionFeeRate(uint256 newRedemptionFeeRate) external Ownable() {
    ...
    function updateSplitFeeRate(uint256 newSplitFeeRate) external Ownable() {
        require(newSplitFeeRate <= MAX_SPLIT_FEE_RATE, "Exceed max split fee rate");
        splitFeeRate = newSplitFeeRate;
    }

    function updateMergeFeeRate(uint256 newMergeFeeRate) external Ownable() {
