...
contract TestACLInterpreter is ACL, ACLHelper {
    ...
    function testEqualityAddr() public {
        assertEval(arr(tx.origin), 0, Op.EQ, uint256(tx.origin), true);
        assertEval(arr(tx.origin), 0, Op.EQ, uint256(this), false);
    }
    ...
    function testInequalityAddr() public {
        assertEval(arr(tx.origin), 0, Op.NEQ, uint256(msg.sender), false);
