...
contract TestACLInterpreter is ACL, ACLHelper {
    function testEqualityUint() public {
        ...
        assertEval(arr(uint8(10), 11), 0, Op.EQ, 10, true);
        assertEval(arr(uint8(10), 11), 1, Op.EQ, 10, false);
        assertEval(arr(uint8(10), 11), 1, Op.EQ, 11, true);
    }

    function testEqualityAddr() public {
        assertEval(arr(msg.sender), 0, Op.EQ, uint8(msg.sender), true);
        assertEval(arr(msg.sender), 0, Op.EQ, uint8(this), false);
    }

    function testEqualityBytes() public {
        assertEval(arr(keccak256("hi")), 0, Op.EQ, uint8(keccak256("hi")), true);
        assertEval(arr(keccak256("hi")), 0, Op.EQ, uint8(keccak256("bye")), false);
    }

    function testInequalityUint() public {
        assertEval(arr(uint8(10), 11), 0, Op.NEQ, 10, false);
        assertEval(arr(uint8(10), 11), 1, Op.NEQ, 10, true);
        assertEval(arr(uint8(10), 11), 1, Op.NEQ, 11, false);
    }
