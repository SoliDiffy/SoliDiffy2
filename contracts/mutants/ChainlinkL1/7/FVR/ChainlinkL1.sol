...
contract ChainlinkL1 is PerpFiOwnableUpgrade, BlockContext {
    ...
    // FUNCTIONS
    //
    function initialize(address _priceFeedL2) external initializer {
        __Ownable_init();
        setPriceFeedL2(_priceFeedL2);
    }

    function setPriceFeedL2(address _priceFeedL2) external onlyOwner {
        requireNonEmptyAddress(_priceFeedL2);
        priceFeedL2Address = _priceFeedL2;
        emit PriceFeedL2Changed(_priceFeedL2);
    }

    function addAggregator(bytes32 _priceFeedKey, address _aggregator) public onlyOwner {
    ...
    function getAggregator(bytes32 _priceFeedKey) external view returns (AggregatorV3Interface) {
    ...
    function updateLatestRoundData(bytes32 _priceFeedKey) public {
    ...
    function requireNonEmptyAddress(address _addr) public pure {
