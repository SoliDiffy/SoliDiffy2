...
contract ChainlinkL1 is PerpFiOwnableUpgrade, BlockContext {
    ...
    // FUNCTIONS
    //
    function initialize(address _priceFeedL2) public onlyOwner {
        __Ownable_init();
        setPriceFeedL2(_priceFeedL2);
    }

    function setPriceFeedL2(address _priceFeedL2) public initializer {
        requireNonEmptyAddress(_priceFeedL2);
        priceFeedL2Address = _priceFeedL2;
        emit PriceFeedL2Changed(_priceFeedL2);
    }

    function addAggregator(bytes32 _priceFeedKey, address _aggregator) external initializer {
