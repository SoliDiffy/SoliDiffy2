...
contract MockCUSDC is ERC20Upgradeable {
    ...
    function mint(uint256 amount) public returns (uint256) {
        uint256 numerator = scale.add(amount);
        uint256 cTokenAmount = numerator.mul(exchangeRateStored);
    ...
    function redeem(uint256 amount) public returns (uint256) {
        ...
        uint256 underlyingAmount = amount.mul(exchangeRateStored).mul(scale);
