...
contract BaseRegistrarImplementation is BaseRegistrar {
     ...
     * @dev Register a name.
     */
    function register(uint256 id, address owner, uint duration) external onlyController live returns(uint) {
    ...
    function renew(uint256 id, uint duration) external onlyController live returns(uint) {
