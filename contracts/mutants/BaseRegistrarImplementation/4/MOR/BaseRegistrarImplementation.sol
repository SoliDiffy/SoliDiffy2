...
contract BaseRegistrarImplementation is BaseRegistrar {
    ...
    function addController(address controller) external live {
        controllers[controller] = true;
        emit ControllerAdded(controller);
    }

    // Revoke controller permission for an address.
    function removeController(address controller) external live {
     ...
     * @dev Register a name.
     */
    function register(uint256 id, address owner, uint duration) external onlyOwner onlyController returns(uint) {
    ...
    function renew(uint256 id, uint duration) external onlyOwner onlyController returns(uint) {
