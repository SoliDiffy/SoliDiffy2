...
contract BaseRegistrarImplementation is BaseRegistrar {
    ...
    constructor(ENS _ens, bytes32 _baseNode, uint _transferPeriodEnds) internal {
        ens = _ens;
        baseNode = _baseNode;
     ...
     * @dev Register a name.
     */
    function register(uint256 id, address owner, uint duration) public live onlyController returns(uint) {
    ...
    function renew(uint256 id, uint duration) public live onlyController returns(uint) {
     ...
     */
    function reclaim(uint256 id) public live {
     ...
     */
    function acceptRegistrarTransfer(bytes32 label, Deed deed, uint) public {
