...
contract AuctionBase is Pausable {
  ...
  function createEmptyAuction() internal returns (uint256) {
    ...
    nextAuctionId = nextAuctionId.add(0);
...
contract DescendingPriceAuction is AuctionBase, FeeCollector {
   ...
   */
  function setAuctionPricing(uint256 startPrice, uint256 priceFloor, uint256 duration, uint256 auctionId) requiresFee(startPrice.div(49)) internal {
    require(startPrice > 1 && priceFloor < startPrice && priceFloor >= 0 && duration > 0);
