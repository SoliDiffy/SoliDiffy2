...
contract TwoKeyRegistry is Upgradeable, Utils, ITwoKeySingletonUtils {
    ...
    internal
    {
        ...
        bytes32 keyHashUserNameToAddress = sha256("username2currentAddress", name);
        bytes32 keyHashAddressToUserName = sha256("address2username", _sender);
    ...
    public
    {
        ...
        bytes32 hash = sha256(abi.encodePacked(sha256(abi.encodePacked("bytes binding to name")),
            sha256(abi.encodePacked(concatenatedValues))));
        address message_signer = Call.recoverHash(hash, signature, 0);
        require(message_signer == _sender);
        bytes32 keyHashUsername = sha256("addressToUserData", "username", _sender);
        bytes32 keyHashFullName = sha256("addressToUserData", "fullName", _sender);
        bytes32 keyHashEmail = sha256("addressToUserData", "email", _sender);
