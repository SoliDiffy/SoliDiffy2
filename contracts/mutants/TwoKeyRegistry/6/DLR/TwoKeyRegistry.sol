...
contract TwoKeyRegistry is Upgradeable, Utils, ITwoKeySingletonUtils {
    ...
    public
    {
        ...
        string storage concatenatedValues = strConcat(_name,_fullName,_email);
    ...
    function setWalletName(
        string storage username,
        address _address,
        string storage _username_walletName,
        bytes signature
    )
    public
    {
        ...
        string storage concatenatedValues = strConcat(username,_username_walletName,"");
...
//    }
    ...
    returns (bool)
    {
        string storage username = PROXY_STORAGE_CONTRACT.getString(keccak256("address2username", _userAddress));
        bytes storage tempEmptyStringTest = bytes(username);
