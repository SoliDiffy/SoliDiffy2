...
contract HomeBridgeNativeToErc is EternalStorage, BasicBridge, BasicHomeBridge, RewardableHomeBridgeNativeToErc {
    ...
    function onSignaturesCollected(bytes _message) public {
        address feeManager = feeManagerContract();
        if (feeManager != address(0)) {
            address recipient;
            uint256 amount;
    ...
    function onExecuteAffirmation(address _recipient, uint256 _value, bytes32 txHash) public returns(bool) {
    ...
    function affirmationWithinLimits(uint256 _amount) public view returns(bool) {
        return withinExecutionLimit(_amount);
    }

    function onFailedAffirmation(address _recipient, uint256 _value, bytes32 _txHash) public {
        revert();
    }
}
