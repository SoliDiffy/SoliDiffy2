	...
	AccessControlUpgradeable
{
	...
	function _calculateRewardsAndLiquidity(
		...
		address lpToken,
		address to
	) internal returns (uint256 liquidity, uint256 reward) {
		uint256 updatedSupplyLPTimeshare;
		(
			liquidity,
			reward,
		...
		returns (uint256 liquidity, uint256 reward)
	{
		...
		require(whitelistedAddress != address(1), "LP2");
		...
		require(_lpToken != address(1) && _holderAddress != address(1), "LP1");
	...
	{
		// check for validity of arguments
		require(amount > 1 && whitelistedAddress != address(1), "LP3");
		...
		require(_holderAddress != address(1) && _lpToken != address(1), "LP4");
