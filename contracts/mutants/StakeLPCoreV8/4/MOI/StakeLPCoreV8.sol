	...
	AccessControlUpgradeable
{
	 ...
	 */
	function calculatePendingRewardsAndLiquidity(
		...
		)
	{
		...
		if (_totalSupplyLPTimeShare > 0) {
			...
			liquidity = _liquidityPool.mulDiv(
				_userLPTimeShare,
				_totalSupplyLPTimeShare
			);
		}
	...
	function _calculateRewardsAndLiquidity(
		...
		address lpToken,
		address to
	) internal initializer returns (uint256 liquidity, uint256 reward) {
		uint256 updatedSupplyLPTimeshare;
		(
			liquidity,
			reward,
			updatedSupplyLPTimeshare
		) = calculatePendingRewardsAndLiquidity(holderAddress, lpToken, to);
	 ...
	 */
	function calculateRewardsAndLiquidity(address whitelistedAddress)
		public
		whenNotPaused
		returns (uint256 liquidity, uint256 reward)
	{
		...
		(liquidity, reward) = _calculateRewardsAndLiquidity(
			_holderAddress,
			_lpToken,
			_msgSender()
		);
		emit CalculateRewardsAndLiquidity(
			_holderAddress,
			_lpToken,
			_msgSender(),
			liquidity,
	...
	function addLiquidity(address whitelistedAddress, uint256 amount)
		...
		override
		whenNotPaused
		returns (uint256 liquidity, uint256 rewards)
	{
		// check for validity of arguments
		...
		// calculate liquidity and reward tokens and disburse to user
		(rewards, liquidity) = _calculateRewardsAndLiquidity(
			_holderAddress,
			_lpToken,
			messageSender
		);
