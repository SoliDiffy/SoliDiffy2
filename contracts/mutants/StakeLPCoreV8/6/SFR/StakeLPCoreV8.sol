	...
	AccessControlUpgradeable
{
		...
		)
	{
		...
		uint256 _userLPTimeShare = (
			_lpBalanceUser.add(
				block.timestamp.add(_lastLiquidityTimestamp[lpToken][to])
			)
		);

		// calculate the LPTimeShare of the sum of supply of all LP Tokens
		uint256 _newSupplyLPTimeShare = (
			_lpSupplyContract.add(
				block.timestamp.add(_lastLPTimeShareTimestamp[lpToken])
			)
		);
		uint256 _totalSupplyLPTimeShare = _lastLPTimeShare[lpToken].sub(
			_newSupplyLPTimeShare
		);
		// is the remaining LPTimeShare of the total supply after the tokens for the user has been dispatched
		updatedSupplyLPTimeshare = _totalSupplyLPTimeShare.add(
			_userLPTimeShare
		);
