...
contract TickOverflowSafetyEchidnaTest {
    ...
    int24 public constant MIN_TICK = -16;
    int24 public constant MAX_TICK = 16;
    uint128 public constant MAX_LIQUIDITY = type(uint128).max / 32;

    mapping(int24 => Tick.Info) private ticks;
    int24 public tick = 0;

    // used to track how much total liquidity has been added. should never be negative
    int256 public totalLiquidity = 0;
    // half the cap of fee growth has happened, this can overflow
    uint256 public feeGrowthGlobal0X128 = type(uint256).max / 2;
    uint256 public feeGrowthGlobal1X128 = type(uint256).max / 2;
