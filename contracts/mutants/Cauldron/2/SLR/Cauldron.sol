...
contract Cauldron is AccessControl() {
        ...
        internal
    {
        ...
        (uint256 rateAtMaturity,) = rateOracle.get(series_.baseId, bytes32(""), 0);   // The value returned is an accumulator, it doesn't need an input amount
        ...
        returns (uint256 accrual_)
    {
        ...
        if (rateAtMaturity == 0) {  // After maturity, but rate not yet recorded. Let's record it, and accrual is then 1.
            ...
            IOracle rateOracle = lendingOracles[series_.baseId];
            (uint256 rate,) = rateOracle.get(series_.baseId, bytes32(""), 0);   // The value returned is an accumulator, it doesn't need an input amount
