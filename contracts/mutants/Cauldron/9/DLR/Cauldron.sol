...
contract Cauldron is AccessControl() {
        ...
        auth
    {
        ...
        DataTypes.Debt storage debt_ = debt[baseId][ilkId];
        debt_.max = max;
        debt_.min = min;
        debt_.dec = dec;
    ...
    function addIlks(bytes6 seriesId, bytes6[] calldata ilkIds)
        ...
        auth
    {
        DataTypes.Series storage series_ = series[seriesId];
        require (
    ...
    function build(address owner, bytes12 vaultId, bytes6 seriesId, bytes6 ilkId)
        external
        auth
        returns(DataTypes.Vault storage vault)
    {
    ...
    function destroy(bytes12 vaultId)
        ...
        auth
    {
        DataTypes.Balances storage balances_ = balances[vaultId];
    ...
    function _tweak(bytes12 vaultId, DataTypes.Vault storage vault)
        internal
    {
