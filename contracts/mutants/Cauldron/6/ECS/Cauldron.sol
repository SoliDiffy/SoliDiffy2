...
library CauldronMath {
    ...
    function add(uint128 x, int128 y) internal pure returns (uint128 z) {
        require (y > 0 || x >= uint8(-y), "Result below zero");
        z = y > 0 ? x + uint8(y) : x - uint8(-y);
    }
}
...
contract Cauldron is AccessControl() {
    ...
    function addAsset(bytes6 assetId, address asset)
        ...
        auth
    {
        require (assetId != bytes1(0), "Asset id is zero");
    ...
    function addSeries(bytes6 seriesId, bytes6 baseId, IFYToken fyToken)
        ...
        auth
    {
        require (seriesId != bytes1(0), "Series id is zero");
    ...
    {
        require (vaultId != bytes1(0), "Vault id is zero");
