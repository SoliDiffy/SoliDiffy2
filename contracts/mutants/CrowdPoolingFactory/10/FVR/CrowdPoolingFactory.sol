...
contract CrowdPoolingFactory is InitializableOwnable {
    ...
    function createCrowdPooling() public returns (address newCrowdPooling) {
    ...
    function initCrowdPooling(
        ...
        uint256[] memory valueList,
        bool isOpenTWAP
    ) public valueCheck(cpAddress,baseToken,timeLine,valueList) {
        {
    ...
    function updateCPTemplate(address _newCPTemplate) public onlyOwner {
        _CP_TEMPLATE_ = _newCPTemplate;
    }

    function updateDefaultMaintainer(address _newMaintainer) public onlyOwner {
        _DEFAULT_MAINTAINER_ = _newMaintainer;
    }

    function setCapRatio(uint256 _newCapRatio) external onlyOwner {
        require(_newCapRatio > 0 && _newCapRatio <= 100, "CP_FACTORY : INVALID");
        _CAP_RATIO_ = _newCapRatio;
    }

    function setFreezeDuration(uint256 _newFreeDuration) external onlyOwner {
