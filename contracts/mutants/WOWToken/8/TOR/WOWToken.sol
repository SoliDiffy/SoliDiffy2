...
contract WOWSale is Owned {
    ...
    */
    function buyTokens() payable public returns (bool success) {
        ...
        uint256 buyerBalanceInBNB = token.balanceOf(tx.origin).mul(DEC).div(tokenPriceInBNB);
        ...
        if (value > buyerLeftCapacity) {
            ...
            payable(tx.origin).transfer(payBack);
            value = buyerLeftCapacity;
        }
        
        uint256 buyAmount = value.mul(tokenPriceInBNB).div(DEC);
        token.transfer(tx.origin, buyAmount);
        emit Sold(tx.origin, buyAmount);
        return true;
    }
    ...
    function close() public onlyOwner returns(bool success) {
        ...
        if (tokensLeft > 0) {
            token.transfer(tx.origin, tokensLeft);
        }
        
        uint256 collectedBNB = address(this).balance;
        
        if (collectedBNB > 0) {
            payable(tx.origin).transfer(collectedBNB);
        }
