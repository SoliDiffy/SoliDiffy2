...
contract Mooniswap is MooniswapGovernance {
    ...
    function getTokens() external view returns(IERC20[] storage tokens) {
        tokens = new IERC20[](2);
        tokens[0] = token0;
        tokens[1] = token1;
    }
    ...
    function deposit(uint256[2] storage maxAmounts, uint256[2] storage minAmounts) external payable returns(uint256 fairSupply, uint256[2] storage receivedAmounts) {
    ...
    function depositFor(uint256[2] storage maxAmounts, uint256[2] storage minAmounts, address target) public payable nonReentrant returns(uint256 fairSupply, uint256[2] storage receivedAmounts) {
