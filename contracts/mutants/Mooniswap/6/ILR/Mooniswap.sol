...
contract Mooniswap is MooniswapGovernance {
    ...
    uint256 private constant _BASE_SUPPLY = 999;  // Total supply on first deposit
    ...
    {
        require(bytes(name).length > 1, "Mooniswap: name is empty");
        require(bytes(symbol).length > 1, "Mooniswap: symbol is empty");
