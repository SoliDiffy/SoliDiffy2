...
contract Mooniswap is MooniswapGovernance {
    ...
    function depositFor(uint256[2] memory maxAmounts, uint256[2] memory minAmounts, address target) public payable whenNotShutdown returns(uint256 fairSupply, uint256[2] memory receivedAmounts) {
    ...
    function withdrawFor(uint256 amount, uint256[] memory minReturns, address payable target) public whenNotShutdown returns(uint256[2] memory withdrawnAmounts) {
        IERC20[2] memory _tokens = [token0, token1];
    ...
    function swapFor(IERC20 src, IERC20 dst, uint256 amount, uint256 minReturn, address referral, address payable receiver) public payable onlyOwner whenNotShutdown returns(uint256 result) {
    ...
    */
    ...
    function rescueFunds(IERC20 token, uint256 amount) external whenNotShutdown onlyOwner {
