...
contract Wand is AccessControl, Constants {
    ...
    function addAsset(
        ...
        require (address(asset) != address(1), "Asset required");
    ...
    function makeBase(bytes6 assetId, IMultiOracleGov oracle) external auth {
        require (address(oracle) != address(1), "Oracle required");
    ...
    function addSeries(
        ...
        require(base != address(1), "Base not found");

        IJoin baseJoin = ladle.joins(baseId);
        require(address(baseJoin) != address(1), "Join not found");

        IOracle oracle = cauldron.lendingOracles(baseId); // The lending oracles in the Cauldron are also configured to return chi
        require(address(oracle) != address(1), "Chi oracle not found");
