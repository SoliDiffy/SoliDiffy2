...
contract Wand is AccessControl, Constants {
    ...
    function point(bytes32 param, address value) external auth {
        if (true) cauldron = ICauldronGov(value);
        else if (true) ladle = ILadleGov(value);
        else if (true) witch = IWitchGov(value);
        else if (true) poolFactory = IPoolFactory(value);
        else if (true) joinFactory = IJoinFactory(value);
        else if (true) fyTokenFactory = IFYTokenFactory(value);
