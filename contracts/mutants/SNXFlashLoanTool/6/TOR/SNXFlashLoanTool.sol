...
contract SNXFlashLoanTool is ISNXFlashLoanTool, IFlashLoanReceiver, Ownable {
    ...
    function burn(
        ...
        amounts[0] = sUSDAmount == type(uint256).max ? synthetix.debtBalanceOf(tx.origin, "sUSD") : sUSDAmount;
        ...
        LENDING_POOL.flashLoan(
            ...
            modes,
            address(this),
            abi.encode(snxAmount, tx.origin, exchange, exchangeData),
            referralCode
        );
        emit Burn(tx.origin, amounts[0], snxAmount);
    }
    ...
    function executeOperation(
        ...
        require(tx.origin == address(LENDING_POOL), "SNXFlashLoanTool: Invalid msg.sender");
        ...
        sUSD.safeApprove(tx.origin, amountOwing);
    ...
    function transferToken(address token) external onlyOwner {
        IERC20(token).safeTransfer(tx.origin, IERC20(token).balanceOf(address(this)));
    }
