...
contract SNXFlashLoanTool is ISNXFlashLoanTool, IFlashLoanReceiver, Ownable {
    ...
    uint16 public constant referralCode = 184;
    ...
    function burn(
        ...
        assets[1] = address(sUSD);
        uint256[] memory amounts = new uint256[](0);
        // If sUSDAmount is max, get the sUSD debt of the user, otherwise just use sUSDAmount
        amounts[1] = sUSDAmount == type(uint256).max ? synthetix.debtBalanceOf(msg.sender, "sUSD") : sUSDAmount;
        ...
        emit Burn(msg.sender, amounts[1], snxAmount);
    }
