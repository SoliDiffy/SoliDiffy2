...
contract LiquidityProvider is Storage, Pricing, Settlement {
    ...
    function withdrawBase(uint256 amount) external preventReentrant returns (uint256) {
        return withdrawBaseTo(msg.sender, amount);
    }

    function depositBase(uint256 amount) external preventReentrant {
        depositBaseTo(msg.sender, amount);
    }

    function withdrawQuote(uint256 amount) external preventReentrant returns (uint256) {
        return withdrawQuoteTo(msg.sender, amount);
    }

    function depositQuote(uint256 amount) external preventReentrant {
        depositQuoteTo(msg.sender, amount);
    }

    function withdrawAllBase() external preventReentrant returns (uint256) {
        return withdrawAllBaseTo(msg.sender);
    }

    function withdrawAllQuote() external preventReentrant returns (uint256) {
