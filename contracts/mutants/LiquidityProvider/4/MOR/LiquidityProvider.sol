...
contract LiquidityProvider is Storage, Pricing, Settlement {
    ...
    function depositBaseTo(address to, uint256 amount) public depositQuoteAllowed depositBaseAllowed {
    ...
    function withdrawQuoteTo(address to, uint256 amount) public depositQuoteAllowed returns (uint256) {
        // calculate capital
        (, uint256 quoteTarget) = _getExpectedTarget();
        uint256 totalQuoteCapital = getTotalQuoteCapital();
    ...
    function withdrawBaseTo(address to, uint256 amount) public depositQuoteAllowed returns (uint256) {
        // calculate capital
        (uint256 baseTarget, ) = _getExpectedTarget();
        uint256 totalBaseCapital = getTotalBaseCapital();
