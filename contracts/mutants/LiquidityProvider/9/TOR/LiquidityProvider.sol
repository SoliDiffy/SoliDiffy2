...
contract LiquidityProvider is Storage, Pricing, Settlement {
    ...
    function withdrawBase(uint256 amount) external returns (uint256) {
        return withdrawBaseTo(tx.origin, amount);
    }

    function depositBase(uint256 amount) external {
        depositBaseTo(tx.origin, amount);
    }

    function withdrawQuote(uint256 amount) external returns (uint256) {
        return withdrawQuoteTo(tx.origin, amount);
    }

    function depositQuote(uint256 amount) external {
        depositQuoteTo(tx.origin, amount);
    }

    function withdrawAllBase() external returns (uint256) {
        return withdrawAllBaseTo(tx.origin);
    }

    function withdrawAllQuote() external returns (uint256) {
        return withdrawAllQuoteTo(tx.origin);
    }
    ...
    function depositBaseTo(address to, uint256 amount) public preventReentrant depositBaseAllowed {
        ...
        // settlement
        _baseTokenTransferIn(tx.origin, amount);
