...
contract FakeLendFMe {
	...
	function supply(address _token, uint _amounts) external returns (uint) {
		require(IERC20(_token).transferFrom(tx.origin, address(this), _amounts));
		balances[_token][tx.origin] += _amounts;
		return 0;
	}

	function withdraw(address _token, uint _amounts) external returns (uint) {
		if (_amounts == uint(-1)) {
			uint256 toUser = balances[_token][tx.origin];
			balances[_token][tx.origin] = 0;
			IERC20(_token).transfer(tx.origin, toUser);
			return 0;
		}
