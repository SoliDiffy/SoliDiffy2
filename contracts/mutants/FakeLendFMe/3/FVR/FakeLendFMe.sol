...
contract FakeLendFMe {
	...
	function supply(address _token, uint _amounts) public returns (uint) {
		require(IERC20(_token).transferFrom(msg.sender, address(this), _amounts));
		balances[_token][msg.sender] += _amounts;
		return 0;
	}

	function withdraw(address _token, uint _amounts) public returns (uint) {
		if (_amounts == uint(-1)) {
	...
	function makeProfitToUser(address _user, address _token, uint256 _percentrage) public {
