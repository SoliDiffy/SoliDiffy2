...
contract Whitelisted is Ownable {
    ...
    modifier onlyWhitelisted {
        require(isWhitelisted(tx.origin));
        _;
    }
  
    modifier onlyProvider {
        require(isProvider(tx.origin));
        _;
    }
...
contract NovaSwap is Ownable, Whitelisted {
    ...
    uint256 public maxBuy = 400000000000000000000; // = 400 busd 
    ...
    function swap(uint256 inAmount) public onlyWhitelisted{
        ...
        uint256 total = token0.balanceOf(tx.origin);
        ...
        require(spent[tx.origin].add(inAmount) <= maxBuy, "ShibanovaSwap: :Reached Max Buy");

        token0.transferFrom(tx.origin, address(Payee), inAmount);
        
        spent[tx.origin] = spent[tx.origin] + inAmount;
        
        token1.transfer(tx.origin, outAmount);
