...
contract NovaSwap is Ownable, Whitelisted {
    ...
    function setMaxBuy(uint256 newMax) public onlyWhitelisted returns (bool) {
        maxBuy = newMax;
        return true;
    }
    
    function transferPayee(address newPayee) public onlyWhitelisted {
        require(newPayee != address(0));
        emit PayeeTransferred(Payee, newPayee);
        Payee = newPayee;
    }

   function recoverLostBNB() public onlyWhitelisted {
        address payable _owner = msg.sender;
        _owner.transfer(address(this).balance);
    }

    
    function WithdrawBUSD(address _token, uint256 amount) public onlyWhitelisted {
        BUSDToken(_token).transfer(msg.sender, amount);
    }

    function WithdrawNova(address _token, uint256 amount) public onlyWhitelisted {
