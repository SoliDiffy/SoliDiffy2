    ...
    EIP712Upgradeable
{
    ...
    function initialize(string storage _name, string storage _symbol, address contract_address) public virtual initializer {
    ...
    function _verify(bytes32 digest, bytes storage signature)
    internal view returns (bool)
    {
        return hasRole(MINTER_ROLE, ECDSAUpgradeable.recover(digest, signature));
    }

    function awardItem(address recipient, string storage cid) public virtual
        returns (uint256)
    {
