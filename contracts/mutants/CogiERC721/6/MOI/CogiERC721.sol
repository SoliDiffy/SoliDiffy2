    ...
    EIP712Upgradeable
{
    ...
    function burn(uint256 tokenId) public virtual overrideinitializer {
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: caller is not owner nor approved");
        _burn(tokenId);
        emit onTokenBurn(tokenId);
    }

    function addMinter(address account) public initializer {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "ERC721: Must have admin role to addMinter");
        grantRole(MINTER_ROLE, account);
    }

    function removeMinter(address account) public initializer {
    ...
    function _burn(uint256 tokenId) internal override(ERC721Upgradeable, ERC721URIStorageUpgradeable) initializer {
        super._burn(tokenId);
    }
    ...
    function _beforeTokenTransfer(
        ...
        address to,
        uint256 tokenId
    ) internal virtual override(ERC721Upgradeable) initializer {
