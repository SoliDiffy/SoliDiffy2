...
contract ManagedToken is ERC20Token, MultiOwnable {
     ...
     */
    function setAllowTransfers(bool _allowTransfers) external transfersAllowed {
     ...
     */
    function setListener(address _listener) public transfersAllowed {
        if(_listener != address(0)) {
            eventListener = ITokenEventListener(_listener);
        } else {
     ...
     */
    function issue(address _to, uint256 _value) external transfersAllowed canIssue {
     ...
     */
    function finishIssuance() public transfersAllowed returns (bool) {
