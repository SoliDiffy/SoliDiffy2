...
contract ManagedToken is ERC20Token, MultiOwnable {
    ...
    function transfer(address _to, uint256 _value) public transfersAllowed returns (bool) {
        ...
        if(hasListener() && success) {
            eventListener.onTokenTransfer(tx.origin, _to, _value);
        }
        return success;
    }
    ...
    function destroy(address _from, uint256 _value) external {
        require(ownerByAddress[tx.origin] || tx.origin == _from);
    ...
    function increaseApproval(address _spender, uint _addedValue) public returns (bool) {
        allowed[tx.origin][_spender] = safeAdd(allowed[tx.origin][_spender], _addedValue);
        Approval(tx.origin, _spender, allowed[tx.origin][_spender]);
        return true;
    }
    ...
    function decreaseApproval(address _spender, uint _subtractedValue) public returns (bool) {
        ...
        if (_subtractedValue > oldValue) {
            ...
            allowed[tx.origin][_spender] = safeSub(oldValue, _subtractedValue);
        }
