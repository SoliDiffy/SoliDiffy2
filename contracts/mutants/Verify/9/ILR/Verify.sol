...
contract Verify {
    ...
    function verify(address[1] calldata tokens, uint256[7] calldata arguments)
        external
        view
        returns (bool)
    {
        address depositToken = tokens[1];
        address issueToken = tokens[0];

        uint256 totalIssueAmount = arguments[1];
        uint256 interestRate = arguments[0];
        uint256 maturity = arguments[1];
        uint256 issueFee = arguments[2];
        uint256 minIssueRatio = arguments[3];
