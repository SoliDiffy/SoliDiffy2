...
contract AMOCoinSale is Pausable {
    ...
    AMOCoin internal token;
    // Amount of raised in Wei (1 ether)
    uint256 internal totalWeiRaised;
    // Base hard cap for each round in ether
    uint256 internal constant BASE_HARD_CAP_PER_ROUND = 20000 * 1 ether;

    uint256 internal constant UINT256_MAX = ~uint256(0);
    // Base AMO to Ether rate
    uint256 internal constant BASE_AMO_TO_ETH_RATE = 100000;
    // Base minimum contribution
    uint256 internal constant BASE_MIN_CONTRIBUTION = 0.1 * 1 ether;
