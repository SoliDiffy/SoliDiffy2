...
//SWC-116-Block values as a proxy for time:L132、349、358
     ...
     */
    function addManyToWhitelist(address[] users) external whenNotPaused {
     ...
     */
    function addToWhitelist(address user) public whenNotPaused {
        whitelist[user] = true;
    }
     ...
     */
    function removeManyFromWhitelist(address[] users) external whenNotPaused {
        for (uint32 i = 0; i < users.length; i++) {
     ...
     * @param users: Addresses of users who should not belong to whitelist
     */
    function removeFromWhitelist(address user) public whenNotPaused {
