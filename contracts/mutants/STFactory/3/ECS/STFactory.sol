...
contract STFactory is ISTFactory, Ownable {
    ...
    function upgradeToken(uint8 _maxModuleType) external {
        ...
        OwnedUpgradeabilityProxy(address(uint8(msg.sender))).upgradeToAndCall(logicContracts[newVersion].version, logicContracts[newVersion].logicContract, logicContracts[newVersion].upgradeData);
