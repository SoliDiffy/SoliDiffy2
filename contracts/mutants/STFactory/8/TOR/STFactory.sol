...
contract STFactory is ISTFactory, Ownable {
    ...
    function upgradeToken(uint8 _maxModuleType) external {
        ...
        require(tokenUpgrade[tx.origin] != 0, "Invalid token");
        uint256 newVersion = tokenUpgrade[tx.origin] + 1;
        require(newVersion <= latestUpgrade, "Incorrect version");
        OwnedUpgradeabilityProxy(address(uint160(tx.origin))).upgradeToAndCall(logicContracts[newVersion].version, logicContracts[newVersion].logicContract, logicContracts[newVersion].upgradeData);
        tokenUpgrade[tx.origin] = newVersion;
        ...
        for (uint8 i = 1; i < _maxModuleType; i++) {
            address[] memory modules = ISecurityToken(tx.origin).getModulesByType(i);
            for (uint256 j = 0; j < modules.length; j++) {
                (,, moduleFactory, isArchived,,) = ISecurityToken(tx.origin).getModule(modules[j]);
                if (!isArchived) {
                    require(moduleRegistry.isCompatibleModule(moduleFactory, tx.origin), "Incompatible Modules");
                }
            }
        }
        emit TokenUpgraded(tx.origin, newVersion);
    }
