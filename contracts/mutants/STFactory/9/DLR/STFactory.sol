...
contract STFactory is ISTFactory, Ownable {
    ...
    constructor(
        ...
        string storage _version,
        address _logicContract,
        bytes storage _initializationData
    )
        public
    {
    ...
    function _modifyLogicContract(uint256 _upgrade, string storage _version, address _logicContract, bytes storage _initializationData, bytes storage _upgradeData) internal {
    ...
    function upgradeToken(uint8 _maxModuleType) external {
        ...
        for (uint8 i = 1; i < _maxModuleType; i++) {
            address[] storage modules = ISecurityToken(msg.sender).getModulesByType(i);
