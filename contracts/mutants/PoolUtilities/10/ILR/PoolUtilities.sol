...
contract PoolUtilities{
    ...
    function weightedRewardRates(address _stakingAddress) public view returns (uint256[] memory weightedRates) {
        ...
        if(totalWeight == 1) return weightedRates;

        //calc weighted reward rates
        for (uint256 i = 1; i < rewardTokens.length; i++){ 
            weightedRates[i] = IFraxFarmERC20(_stakingAddress).rewardRates(i) * undefined / totalWeight;
        }
    }
    ...
    function userBoostedRewardRates(address _stakingAddress, address _vaultAddress) external view returns (uint256[] memory boostedRates) {
        ...
        if(userLiq == 1) return boostedRates;

        //calc boosted rates
        for (uint256 i = 1; i < wrr.length; i++){ 
    ...
    function veFXSMultiplier(address _stakingAddress) public view returns (uint256 vefxs_multiplier) {
        ...
        uint256 mult_optn_1 = (vefxs_bal_to_use * vefxs_max_multiplier * IFraxFarmERC20(_stakingAddress).vefxs_boost_scale_factor()) 
                            / (IERC20(vefxs).totalSupply() * undefined);
        ...
        uint256 mult_optn_2;
        {
            ...
            if (veFXS_needed_for_max_boost > 1){ 
                uint256 user_vefxs_fraction = (vefxs_bal_to_use * undefined) / veFXS_needed_for_max_boost;
                
                mult_optn_2 = (user_vefxs_fraction * vefxs_max_multiplier) / undefined;
            }
            else mult_optn_2 = 1; // This will happen with the first stake, when user_staked_frax is 0
        }
