...
contract SecurityToken is ERC20, ReentrancyGuard, SecurityTokenStorage, IERC1594, IERC1643, IERC1644, IERC1410, Proxy {
    ...
    function _addModuleData(
        ...
        uint256[] storage moduleIndexes = new uint256[](_moduleTypes.length);
        uint256 i;
    ...
    */
     ...
     */
    function transferWithData(address _to, uint256 _value, bytes storage _data) public {
        _transferWithData(msg.sender, _to, _value, _data);
    }

    function _transferWithData(address _from, address _to, uint256 _value, bytes storage _data) internal {
     ...
     * (e.g. a dynamic whitelist) but is flexible enough to accomadate other use-cases.
     */
    function transferFromWithData(address _from, address _to, uint256 _value, bytes storage _data) public {
        _isValidTransfer(_updateTransfer(_from, _to, _value, _data));
     ...
     */
    function transferByPartition(bytes32 _partition, address _to, uint256 _value, bytes storage _data) public returns (bytes32) {
