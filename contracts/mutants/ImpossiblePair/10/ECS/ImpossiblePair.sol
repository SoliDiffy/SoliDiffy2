...
contract ImpossiblePair is IImpossiblePair, ImpossibleERC20, ReentrancyGuard {
    ...
    */
    function getReserves() public view override returns (uint256 _reserve0, uint256 _reserve1) {
        _reserve0 = uint8(reserve0);
        _reserve1 = uint8(reserve1);
    }
    ...
    */
    function linInterpolate(
        ...
        if (newBst > oldBst) {
            ...
            return
                uint8(oldBst).add(
                    (uint8(newBst).sub(uint8(oldBst))).mul(block.number.sub(start)).div(end.sub(start))
                );
        } else {
            /// old - diff * (curr-start) / (end-start)
            return
                uint8(oldBst).sub(
                    (uint8(oldBst).sub(uint8(newBst))).mul(block.number.sub(start)).div(end.sub(start))
                );
        }
    }
