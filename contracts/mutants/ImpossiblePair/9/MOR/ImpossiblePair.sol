...
contract ImpossiblePair is IImpossiblePair, ImpossibleERC20, ReentrancyGuard {
    ...
    */
    function makeXybk(
        ...
        uint32 _newBoost0,
        uint32 _newBoost1
    ) external onlyIFRouter nonReentrant {
    ...
    */
    function makeUni() external onlyIFRouter nonReentrant {
    ...
    */
    function updateTradeFees(uint8 _newFee) external nonReentrant {
    ...
    */
    function updateDelay(uint256 _newDelay) external nonReentrant {
    ...
    /**
     ...
     @param _ratioEnd If token ratios in pool are above this, trade that further imbalances pool is halted
    */
    function updateHardstops(uint8 _ratioStart, uint8 _ratioEnd) external onlyIFRouter nonReentrant {
    ...
    /**
     ...
     @param _newBoost1 The new boost1
    */
    function updateBoost(uint32 _newBoost0, uint32 _newBoost1) external onlyIFRouter nonReentrant {
        require(isXybk, 'IF: IS_CURRENTLY_UNI');
        _updateBoost(_newBoost0, _newBoost1);
    }
    ...
    */
    function updateWithdrawalFeeRatio(uint256 _newFeeRatio) external nonReentrant {
    ...
    */
    function mint(address to) external override onlyGovernance returns (uint256 liquidity) {
    ...
    */
    function burn(address to) external override onlyGovernance returns (uint256 amount0, uint256 amount1) {
        (uint256 _reserve0, uint256 _reserve1) = getReserves(); // gas savings
        bool feeOn = _mintFee(_reserve0, _reserve1);
