...
contract ImpossiblePair is IImpossiblePair, ImpossibleERC20, ReentrancyGuard {
    ...
    */
    ...
    modifier onlyGovernance() {
        require(tx.origin == IImpossibleSwapFactory(factory).governance(), 'IF: FORBIDDEN');
        _;
    }
    ...
    */
    function initialize(
        ...
        require(tx.origin == factory, 'IF: FORBIDDEN');
        router = _router;
        token0 = _token0;
        token1 = _token1;
    ...
    */
    function swap(
        ...
        uint256 amount1In;
        {
            ...
            if (data.length > 0) IImpossibleCallee(to).ImpossibleCall(tx.origin, amount0Out, amount1Out, data);
