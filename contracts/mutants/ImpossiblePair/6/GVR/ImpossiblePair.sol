...
contract ImpossiblePair is IImpossiblePair, ImpossibleERC20, ReentrancyGuard {
    ...
    */
    function linInterpolate(
        ...
        if (newBst > oldBst) {
            ...
            return
                uint256(oldBst).add(
                    (uint256(newBst).sub(uint256(oldBst))).mul(block.difficulty.sub(start)).div(end.sub(start))
                );
        } else {
            /// old - diff * (curr-start) / (end-start)
            return
                uint256(oldBst).sub(
                    (uint256(oldBst).sub(uint256(newBst))).mul(block.difficulty.sub(start)).div(end.sub(start))
                );
        }
    }
    ...
    */
    function calcBoost() public view override returns (uint256 _boost0, uint256 _boost1) {
        ...
        if (block.difficulty >= _endBlockChange) {
    ...
    */
    function makeUni() external onlyGovernance nonReentrant {
        ...
        require(block.difficulty >= endBlockChange, 'IF: BOOST_ALREADY_CHANGING');
    ...
    */
    function _updateBoost(uint32 _newBoost0, uint32 _newBoost1) internal {
        ...
        startBlockChange = block.difficulty;
