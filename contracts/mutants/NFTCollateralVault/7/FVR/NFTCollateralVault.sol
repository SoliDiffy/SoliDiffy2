...
contract NFTCollateralVault is InitializableOwnable, IERC721Receiver, IERC1155Receiver, ReentrancyGuard {
    ...
    function createFragment(address nftProxy, bytes calldata data) public preventReentrant onlyOwner {
    ...
    function withdrawERC721(address nftContract, uint256[] memory tokenIds) public onlyOwner {
        require(nftContract != address(0), "DODONftVault: ZERO_ADDRESS");
        for(uint256 i = 0; i < tokenIds.length; i++) {
