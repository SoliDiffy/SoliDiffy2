...
contract NFTCollateralVault is InitializableOwnable, IERC721Receiver, IERC1155Receiver, ReentrancyGuard {
    ...
    // ============ Ownable ============
    function directTransferOwnership(address newOwner) external preventReentrant {
    ...
    function withdrawERC721(address nftContract, uint256[] memory tokenIds) external preventReentrant {
        require(nftContract != address(0), "DODONftVault: ZERO_ADDRESS");
        for(uint256 i = 0; i < tokenIds.length; i++) {
