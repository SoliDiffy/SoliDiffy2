...
contract DODODropsProxy is ReentrancyGuard {
    ...
    function buyTickets(address payable dodoDrops, uint256 ticketAmount) external payable preventReentrant {
        ...
        (uint256 payAmount,) = IDropsFeeModel(feeModel).getPayAmount(dodoDrops, tx.origin, curPrice, ticketAmount);
        ...
        if(buyToken == _BASE_COIN_) {
            ...
            IDODOApproveProxy(_DODO_APPROVE_PROXY_).claimTokens(buyToken, tx.origin, dodoDrops, payAmount);
        }

        IDODODrops(dodoDrops).buyTickets(tx.origin, ticketAmount);
