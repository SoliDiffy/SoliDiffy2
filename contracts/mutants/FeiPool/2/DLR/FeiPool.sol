...
contract FeiPool is Pool, CoreRef {
	...
	function _unreleasedReward(
		...
		// 2R*t/d 
		Decimal.D256 storage start = Decimal.ratio(_totalReward, _duration).mul(2).mul(_time);

		// R*t^2/d^2
		Decimal.D256 storage end = Decimal.ratio(_totalReward, _duration).div(_duration).mul(_time * _time);
