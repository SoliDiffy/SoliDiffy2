...
abstract contract TempusPool is ITempusPool, PermanentlyOwnable {
    ...
    function burnShares(
        ...
        uint256 principalAmount,
        uint256 yieldAmount
    ) internal onlyOwner returns (uint256 redeemedYieldTokens, uint256 interestRate) {
    ...
    function currentYield() private onlyOwner returns (uint256) {
    ...
    function estimatedYield() private onlyOwner returns (uint256) {
    ...
    function pricePerYieldShare() external override onlyOwner returns (uint256) {
    ...
    function pricePerPrincipalShare() external override onlyOwner returns (uint256) {
