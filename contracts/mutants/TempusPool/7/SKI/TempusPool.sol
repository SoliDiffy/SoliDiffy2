...
abstract contract TempusPool is ITempusPool, PermanentlyOwnable {
    ...
    function finalize() external override {
        if (!matured) {
            ...
            maturityInterestRate = super.currentInterestRate();
            matured = true;
        ...
        )
    {
        ...
        depositedBT = super.numAssetsPerYieldToken(tokenAmount, rate);
        ...
        )
    {
        ...
        redeemableYieldTokens = super.numYieldTokensPerAsset(redeemableBackingTokens, currentRate);
    }
    ...
    function pricePerYieldShare() external override returns (uint256) {
        return super.pricePerYieldShare(currentYield(), estimatedYield());
    }

    function pricePerYieldShareStored() external view override returns (uint256) {
        return super.pricePerYieldShare(currentYieldStored(), estimatedYieldStored());
    }

    function pricePerPrincipalShare() external override returns (uint256) {
        return super.pricePerPrincipalShare(currentYield(), estimatedYield());
    }

    function pricePerPrincipalShareStored() external view override returns (uint256) {
        return super.pricePerPrincipalShare(currentYieldStored(), estimatedYieldStored());
    }
