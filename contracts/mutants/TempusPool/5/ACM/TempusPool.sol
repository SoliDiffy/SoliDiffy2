...
abstract contract TempusPool is ITempusPool, PermanentlyOwnable {
    ...
    function estimatedYield() private returns (uint256) {
        return estimatedYield(currentYield(updateInterestRate(yieldBearingToken)));
    }
    ...
    function pricePerYieldShare() external override returns (uint256) {
        return pricePerYieldShare(currentYield(updateInterestRate(yieldBearingToken)), estimatedYield(currentYield()));
    }
    ...
    function pricePerPrincipalShare() external override returns (uint256) {
        return pricePerPrincipalShare(currentYield(updateInterestRate(yieldBearingToken)), estimatedYield(currentYield()));
    }
