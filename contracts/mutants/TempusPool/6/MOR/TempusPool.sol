...
abstract contract TempusPool is ITempusPool, PermanentlyOwnable {
    ...
    function setFeesConfig(FeesConfig calldata newFeesConfig) external override onlyController {
        feesConfig = newFeesConfig;
    }

    function transferFees(address recipient, uint256 amount) external override onlyController {
    ...
    function redeem(
        ...
        uint256 yieldAmount,
        address recipient
    ) external override onlyOwner returns (uint256 redeemedYieldTokens, uint256 rate) {
        (redeemedYieldTokens, rate) = burnShares(from, principalAmount, yieldAmount);
