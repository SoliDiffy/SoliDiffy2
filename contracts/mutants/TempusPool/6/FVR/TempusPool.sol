...
abstract contract TempusPool is ITempusPool, PermanentlyOwnable {
    ...
    function burnShares(
        ...
        uint256 principalAmount,
        uint256 yieldAmount
    ) public returns (uint256 redeemedYieldTokens, uint256 interestRate) {
    ...
    function effectiveRate(uint256 currentRate) public view returns (uint256) {
        if (matured) {
    ...
    function currentYield(uint256 interestRate) public view returns (uint256) {
        return (effectiveRate(interestRate) - initialInterestRate).divf18(initialInterestRate);
    }

    function currentYield() public returns (uint256) {
