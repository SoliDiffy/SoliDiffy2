...
abstract contract TempusPool is ITempusPool, PermanentlyOwnable {
    ...
    function currentYieldStored() public view returns (uint256) {
        return currentYield(storedInterestRate(yieldBearingToken));
    }

    function estimatedYield() public returns (uint256) {
        return estimatedYield(currentYield());
    }

    function estimatedYieldStored() public view returns (uint256) {
        return estimatedYield(currentYieldStored());
    }

    function estimatedYield(uint256 yieldCurrent) public view returns (uint256) {
        if (matured) {
            return yieldCurrent;
        }
