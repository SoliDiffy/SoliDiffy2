...
contract SingleAssetWorkerConfig is OwnableUpgradeSafe, IWorkerConfig {
  ...
  function initialize(PriceOracle _oracle, IPancakeRouter02 _router) public initializer {
  ...
  function setOracle(PriceOracle _oracle) public onlyOwner {
    oracle = _oracle;
    emit SetOracle(_msgSender(), address(oracle));
  }

  /// @dev Set worker configurations. Must be called by owner.
  function setConfigs(address[] calldata addrs, Config[] calldata configs) public onlyOwner {
  ...
  function emergencySetAcceptDebt(address[] calldata addrs, bool isAcceptDebt) public onlyGovernor {
    uint256 len = addrs.length;
    for (uint256 idx = 0; idx < len; idx++) {
