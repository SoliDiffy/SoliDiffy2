...
contract SingleAssetWorkerConfig is OwnableUpgradeSafe, IWorkerConfig {
  ...
  function initialize(PriceOracle _oracle, IPancakeRouter02 _router) external onlyOwner {
  ...
  function setConfigs(address[] calldata addrs, Config[] calldata configs) external initializer {
  ...
  function setGovernor(address newGovernor) external initializer {
  ...
  function emergencySetAcceptDebt(address[] calldata addrs, bool isAcceptDebt) external initializer {
    uint256 len = addrs.length;
    for (uint256 idx = 0; idx < len; idx++) {
