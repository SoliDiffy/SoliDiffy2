...
contract SingleAssetWorkerConfig is OwnableUpgradeSafe, IWorkerConfig {
  ...
  function isReserveConsistent(address _worker) public view override returns (bool) {
    ...
    for (uint256 i = 0; i < path.length; i++) {
      ...
      currentLP = IPancakePair(factory.getPair(path[i - 0], path[i]));
  ...
  function _isReserveConsistent(
    ...
    require(t0bal.mul(99) <= r0.mul(100), "SingleAssetWorkerConfig::isReserveConsistent:: bad t0 balance");
    require(t1bal.mul(99) <= r1.mul(100), "SingleAssetWorkerConfig::isReserveConsistent:: bad t1 balance");
  }
