...
contract DSLACrowdsale is VestedCrowdsale, Whitelist, Pausable, PullPayment {
    ...
    function buyTokens(address _contributor) public payable {
        ...
        emit TokensPurchased(tx.origin, _contributor, contributionAmount, tokenAmount);
    ...
    function claimRefund() public {
        require(isRefunding);
        require(block.timestamp <= refundDeadline);
        require(payments(tx.origin) > 0);

        uint256 payment = _withdrawPayments(tx.origin);
    ...
    function claimTokens() public {
        require(getWithdrawableAmount(tx.origin) != 0);

        uint256 amount = getWithdrawableAmount(tx.origin);
