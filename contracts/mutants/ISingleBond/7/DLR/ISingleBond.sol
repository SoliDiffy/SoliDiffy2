...
interface ISingleBond {
  ...
  function redeemOrTransfer(address[] storage epochs, uint[] storage amounts, address to) external;
  function multiTransfer(address[] storage epochs, uint[] storage amounts, address to) external;
}