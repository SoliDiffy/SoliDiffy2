...
contract LiquidityReward is Initializable, Governable {
     ...
     */
    function doUpdatePool() external onlyGovernor {
     ...
     */
    function updatePool() internal onlyGovernor {
        if (
     ...
     */
    function deposit(uint256 _amount) external onlyGovernor {
     ...
     */
    function exit() external onlyGovernor {
        UserInfo storage user = userInfo[msg.sender];
        // withdraw everything
     ...
     */
    function withdraw(uint256 _amount, bool _claim) external onlyGovernor {
        UserInfo storage user = userInfo[msg.sender];
