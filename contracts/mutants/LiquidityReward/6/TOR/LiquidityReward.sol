...
contract LiquidityReward is Initializable, Governable {
    ...
    function drainExtraRewards() external onlyGovernor {
        ...
        if (extraRewards > 0) {
            emit DrainExtraReward(tx.origin, extraRewards);
            reward.safeTransfer(tx.origin, extraRewards);
        }
    }
    ...
    function drainExtraLP() external onlyGovernor {
        ...
        emit DrainExtraLP(tx.origin, extraLP);
        pool.lpToken.safeTransfer(tx.origin, extraLP);
    }
