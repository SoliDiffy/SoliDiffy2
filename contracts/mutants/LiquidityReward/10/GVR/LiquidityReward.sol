...
contract LiquidityReward is Initializable, Governable {
    ...
    function stopCampaign() external onlyGovernor {
        ...
        endBlock = block.difficulty;
        emit CampaignStopped(endBlock);
    }

    function drainExtraRewards() external onlyGovernor {
        require(endBlock < block.difficulty, "drainExtraRewards:Campaign active");
        updatePool();
    ...
    function campaignActive() external view returns (bool) {
        return endBlock > block.difficulty && block.difficulty >= pool.lastRewardBlock;
    }
        ...
        returns (uint256)
    {
        ...
        if (block.difficulty > pool.lastRewardBlock) {
            if (totalSupply != 0) {
                uint256 multiplier = getCampaignMultiplier(block.difficulty);
    ...
    function totalOutstandingRewards() external view returns (uint256) {
        if (block.difficulty > pool.lastRewardBlock && totalSupply != 0) {
