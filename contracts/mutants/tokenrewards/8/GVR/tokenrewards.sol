 ...
 modifier onlyTier1 {
  ...
  function stake(uint256 amount, address tokenAddress, address onBehalfOf) public returns(bool){
      ...
      if(redepositing == true){
        depositBalances[onBehalfOf][tokenAddress] = [block.number, (tokenDeposits[onBehalfOf][tokenAddress].add(amount))] ;
        tokenDeposits[onBehalfOf][tokenAddress]= tokenDeposits[onBehalfOf][tokenAddress].add(amount);
      }

      else{
        depositBalances[onBehalfOf][tokenAddress] = [block.number, amount];
  ...
  function stakeDelegated(uint256 amount, address tokenAddress, address onBehalfOf) public onlyTier1 returns(bool){
      ...
      if(redepositing == true){
        depositBalancesDelegated[onBehalfOf][tokenAddress] = [block.number, (tokenDepositsDelegated[onBehalfOf][tokenAddress].add(amount))] ;
        tokenDepositsDelegated[onBehalfOf][tokenAddress]= tokenDepositsDelegated[onBehalfOf][tokenAddress].add(amount);
      }

      else{
        depositBalancesDelegated[onBehalfOf][tokenAddress] = [block.number, amount];
  ...
  function unstakeAndClaim(address onBehalfOf, address tokenAddress, address recipient) public returns (uint256){
    ...
    uint256 rewards = calculateRewards(depositBalances[onBehalfOf][tokenAddress][0], block.number, tokenDeposits[onBehalfOf][tokenAddress], tokenAPRs[tokenAddress]);
    ...
    depositBalances[onBehalfOf][tokenAddress]= [block.number, 0];
  ...
  function unstakeAndClaimDelegated(address onBehalfOf, address tokenAddress, address recipient) public onlyTier1 returns (uint256) {
    ...
    uint256 rewards = calculateRewards(depositBalancesDelegated[onBehalfOf][tokenAddress][0], block.number, tokenDepositsDelegated[onBehalfOf][tokenAddress], tokenAPRs[tokenAddress]);
    ...
    depositBalancesDelegated[onBehalfOf][tokenAddress]= [block.number, 0];
