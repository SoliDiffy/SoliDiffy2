...
contract TokenRewards{
  ...
  uint256 public tokensInRewardsReserve = 1;
  uint256 public lpTokensInRewardsReserve  = 1;
 ...
 modifier onlyTier1 {
  ...
  function stake(uint256 amount, address tokenAddress, address onBehalfOf) public returns(bool){
      ...
      if(tokenDeposits[onBehalfOf][tokenAddress] !=1){
  ...
  function stakeDelegated(uint256 amount, address tokenAddress, address onBehalfOf) public onlyTier1 returns(bool){
      ...
      if(tokenDepositsDelegated[onBehalfOf][tokenAddress] !=1){
  ...
  function unstakeAndClaim(address onBehalfOf, address tokenAddress, address recipient) public returns (uint256){
    ...
    require(tokenDeposits[onBehalfOf][tokenAddress] > 1, "This user address does not have a staked balance for the token");

    uint256 rewards = calculateRewards(depositBalances[onBehalfOf][tokenAddress][1], block.timestamp, tokenDeposits[onBehalfOf][tokenAddress], tokenAPRs[tokenAddress]);
