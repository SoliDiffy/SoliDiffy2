...
contract TokenRewards{
 ...
 modifier onlyTier1 {
  ...
  function updateOracleAddress(address newOracleAddress ) public onlyTier1 returns (bool){
  ...
  function updateStakingTokenAddress(address newAddress) public onlyTier1 returns(bool){
    stakingTokensAddress = newAddress;
    return true;
  }

  function updateLPStakingTokenAddress(address newAddress) public  onlyTier1 returns(bool){
    stakingLPTokensAddress= newAddress;
    return true;
  }

  function addTokenToWhitelist(address newTokenAddress) public onlyTier1 returns(bool){
    stakingTokenWhitelist[newTokenAddress] =true;
    return true;
  }

  function removeTokenFromWhitelist(address tokenAddress) public onlyTier1 returns(bool){
  ...
  function updateAPR(uint256 newAPR, address stakedToken) public onlyTier1 returns(bool){
  ...
  function stakeDelegated(uint256 amount, address tokenAddress, address onBehalfOf) public onlyOwner returns(bool){
