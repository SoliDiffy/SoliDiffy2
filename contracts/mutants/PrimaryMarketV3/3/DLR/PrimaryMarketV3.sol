...
contract PrimaryMarketV3 is IPrimaryMarketV3, ReentrancyGuard, ITrancheIndexV2, Ownable {
    ...
    ) external view returns (uint256[] storage indices, uint256 underlying) {
    ...
    function queueRedemption(
        ...
        QueuedRedemption memory newRedemption = queuedRedemptions[index];
        ...
        returns (uint256 underlying)
    {
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            QueuedRedemption memory redemption = queuedRedemptions[indices[i]];
