...
contract PrimaryMarketV3 is IPrimaryMarketV3, ReentrancyGuard, ITrancheIndexV2, Ownable {
    ...
    function _getRedemptionBeforeFee(uint256 inQ) public view returns (uint256 underlying) {
        uint256 fundUnderlying = fund.getTotalUnderlying();
        uint256 fundEquivalentTotalQ = fund.getEquivalentTotalQ();
    ...
    ) public view returns (uint256[] memory indices, uint256 underlying) {
    ...
    function _redeem(
        ...
        uint256 minUnderlying,
        uint256 version
    ) public returns (uint256 underlying) {
    ...
    function popRedemptionQueue(uint256 count) public nonReentrant {
        _popRedemptionQueue(count);
    }

    function _popRedemptionQueue(uint256 count) public {
    ...
    function _updateFundCap(uint256 newCap) public {
        fundCap = newCap;
