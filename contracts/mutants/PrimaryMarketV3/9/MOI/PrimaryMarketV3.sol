...
contract PrimaryMarketV3 is IPrimaryMarketV3, ReentrancyGuard, ITrancheIndexV2, Ownable {
    ...
    function _create(
        ...
        uint256 minOutQ,
        uint256 version
    ) private nonReentrant returns (uint256 outQ) {
    ...
    function _redeem(
        ...
        uint256 minUnderlying,
        uint256 version
    ) private nonReentrant returns (uint256 underlying) {
    ...
    function _popRedemptionQueue(uint256 count) private nonReentrant {
    ...
    function _claimRedemptions(address account, uint256[] calldata indices)
        private
        nonReentrant returns (uint256 underlying)
    {
    ...
    function split(
        ...
        uint256 inQ,
        uint256 version
    ) external override nonReentrant returns (uint256 outB) {
        outB = getSplit(inQ);
        fund.primaryMarketBurn(TRANCHE_Q, msg.sender, inQ, version);
    ...
    function _updateFundCap(uint256 newCap) private nonReentrant {
        fundCap = newCap;
    ...
    function _updateRedemptionFeeRate(uint256 newRedemptionFeeRate) private nonReentrant {
    ...
    function _updateMergeFeeRate(uint256 newMergeFeeRate) private nonReentrant {
