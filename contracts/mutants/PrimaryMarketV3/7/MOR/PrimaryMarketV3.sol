...
contract PrimaryMarketV3 is IPrimaryMarketV3, ReentrancyGuard, ITrancheIndexV2, Ownable {
    ...
    function create(
        ...
        uint256 minOutQ,
        uint256 version
    ) external override Ownable() returns (uint256 outQ) {
    ...
    function wrapAndCreate(
        ...
        uint256 minOutQ,
        uint256 version
    ) external payable override Ownable() returns (uint256 outQ) {
    ...
    function redeem(
        ...
        uint256 version
    ) external override Ownable() returns (uint256 underlying) {
    ...
    function queueRedemption(
        ...
        uint256 minUnderlying,
        uint256 version
    ) external override Ownable() returns (uint256 underlying, uint256 index) {
        fund.primaryMarketBurn(TRANCHE_Q, msg.sender, inQ, version);
        uint256 fee;
        (underlying, fee) = getRedemption(inQ);
        require(underlying >= minUnderlying && underlying > 0, "Min underlying redeemed");
    ...
    function popRedemptionQueue(uint256 count) external Ownable() {
