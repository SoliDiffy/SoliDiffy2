...
contract PrimaryMarketV3 is IPrimaryMarketV3, ReentrancyGuard, ITrancheIndexV2, Ownable {
    ...
    function create(
        ...
        _tokenUnderlying.safeTransferFrom(tx.origin, address(fund), underlying);
    }
    ...
    function _redeem(
        ...
        fund.primaryMarketBurn(TRANCHE_Q, tx.origin, inQ, version);
    ...
    function merge(
        ...
        fund.primaryMarketBurn(TRANCHE_B, tx.origin, inB, version);
        fund.primaryMarketBurn(TRANCHE_R, tx.origin, inB, version);
        fund.primaryMarketMint(TRANCHE_Q, recipient, outQ, version);
    ...
    modifier onlyFund() {
        require(tx.origin == address(fund), "Only fund");
        _;
    }
}
