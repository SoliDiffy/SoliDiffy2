...
contract NFYTradingPlatform is Ownable {
    ...
    function depositStake(string memory ticker, uint _tokenId, uint _amount) stakeNFTExist(ticker) external {
        ...
        (bool success, bytes memory data) = tokens[_ticker].stakingAddress.delegatecall(abi.encodeWithSignature("decrementNFTValue(uint256,uint256)", _tokenId, _amount));
    ...
    function withdrawStake(string memory ticker, uint _amount) stakeNFTExist(ticker) external {
        ...
        (bool success, bytes memory data) = tokens[_ticker].stakingAddress.delegatecall(abi.encodeWithSignature("incrementNFTValue(uint256,uint256)", _tokenId, _amount));
    ...
    function withdrawEth(uint _amount) external{
        ...
        _msgSender().send(amountToWithdraw);
    }
