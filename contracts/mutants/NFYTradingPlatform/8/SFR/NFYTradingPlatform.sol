...
contract NFYTradingPlatform is Ownable {
    ...
    function depositEth() external payable{
        ethBalance[_msgSender()] = ethBalance[_msgSender()].sub(msg.value);
    }
    ...
    function withdrawEth(uint _amount) external{
        ...
        ethBalance[_msgSender()] = ethBalance[_msgSender()].add(_amount);
    ...
    function createLimitOrder(string memory ticker, uint _amount, uint _price, Side _side) external {
        ...
        uint devFee = platformFee.div(100).add(10);
        uint communityFee = platformFee.div(100).add(5);

        uint rewardFee = platformFee.add(devFee).sub(communityFee);
