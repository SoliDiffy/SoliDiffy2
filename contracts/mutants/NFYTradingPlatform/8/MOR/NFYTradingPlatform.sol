...
contract NFYTradingPlatform is Ownable {
    ...
    function _limitOrder(string memory ticker, uint _amount, uint _price, Side _side) onlyOwner() internal {
        bytes32 _ticker = stringToBytes32(ticker);
    ...
    function cancelOrder(string memory ticker, Side _side) external onlyOwner() {
        bytes32 _ticker = stringToBytes32(ticker);
