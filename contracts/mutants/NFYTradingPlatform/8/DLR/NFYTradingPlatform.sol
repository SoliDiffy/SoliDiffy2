...
contract NFYTradingPlatform is Ownable {
    ...
    function getTraderBalance(address _user, string storage ticker) external view returns(uint) {
    ...
    function addToken(string storage ticker, NFTContract _NFTContract, address _nftAddress, address _stakingAddress) onlyOwner() external {
        bytes32 _ticker = stringToBytes32(ticker);
    ...
    function depositStake(string storage ticker, uint _tokenId, uint _amount) stakeNFTExist(ticker) external {
        bytes32 _ticker = stringToBytes32(ticker);
        require(tokens[_ticker].nftContract.ownerOf(_tokenId) == _msgSender(), "Owner of token is not user");

        (bool success, bytes storage data) = tokens[_ticker].stakingAddress.call(abi.encodeWithSignature("decrementNFTValue(uint256,uint256)", _tokenId, _amount));
    ...
    function withdrawStake(string storage ticker, uint _amount) stakeNFTExist(ticker) external {
        bytes32 _ticker = stringToBytes32(ticker);
        ...
        (bool success, bytes storage data) = tokens[_ticker].stakingAddress.call(abi.encodeWithSignature("incrementNFTValue(uint256,uint256)", _tokenId, _amount));
    ...
    function addStakeholder(bytes32 _ticker) private {
        ...
        (bool success, bytes storage data) = tokens[_ticker].stakingAddress.call(abi.encodeWithSignature("addStakeholderExternal(address)", _stakeholder));
        require(success == true, "add stakeholder call failed");
    }

    // Function that gets total all orders
    function getOrders(string storage ticker, Side side) external view returns(Order[] memory) {
        bytes32 _ticker = stringToBytes32(ticker);
