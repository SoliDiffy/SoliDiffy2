...
contract NFYTradingPlatform is Ownable {
    ...
    function setFee(uint _fee) external onlyDev() {
        platformFee = _fee;
    }
    ...
    function setCommunityFeeAddress(address _communityAddress) external onlyDev() {
    ...
    function addToken(string memory ticker, NFTContract _NFTContract, address _nftAddress, address _stakingAddress) onlyDev() external {
        bytes32 _ticker = stringToBytes32(ticker);
    ...
    function depositStake(string memory ticker, uint _tokenId, uint _amount) onlyOwner() external {
        bytes32 _ticker = stringToBytes32(ticker);
    ...
    function withdrawStake(string memory ticker, uint _amount) onlyOwner() external {
        bytes32 _ticker = stringToBytes32(ticker);
