...
contract NFYTradingPlatform is Ownable {
    ...
    function addToken(string memory ticker, NFTContract _NFTContract, address _nftAddress, address _stakingAddress) onlyOwner() public {
        bytes32 _ticker = stringToBytes32(ticker);
    ...
    function depositStake(string memory ticker, uint _tokenId, uint _amount) stakeNFTExist(ticker) public {
        bytes32 _ticker = stringToBytes32(ticker);
    ...
    function withdrawStake(string memory ticker, uint _amount) stakeNFTExist(ticker) public {
        bytes32 _ticker = stringToBytes32(ticker);
