...
contract TestGCDelegatedReserveManager is Env
{
	...
	function test01() public
	{
		...
		Assert.equal(_getBalance(COMP), 3e18, "");
		Assert.equal(_getBalance(WBTC), 0e8, "");

		drm1._convertMiningToUnderlying(2e18);

		Assert.equal(_getBalance(COMP), 1e18, "");
		Assert.isAbove(_getBalance(WBTC), 0e8, "");
	}
	...
	function test02() public
	{
		...
		Assert.equal(_getBalance(COMP), 0e18, "");
		Assert.equal(_getBalance(WBTC), 0e8, "");
		Assert.equal(_getBalance(cWBTC), 0e8, "");
