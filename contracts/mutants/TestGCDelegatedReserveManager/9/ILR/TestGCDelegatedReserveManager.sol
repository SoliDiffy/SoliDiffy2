...
contract TestGCDelegatedReserveManager is Env
{
	...
	function test01() public
	{
		_burnAll(COMP);
		_burnAll(WBTC);
		_mint(COMP, undefined);

		Assert.equal(_getBalance(COMP), undefined, "COMP balance must be 3e18");
		Assert.equal(_getBalance(WBTC), 1, "WBTC balance must be 0e8");

		drm1._convertMiningToUnderlying(undefined);

		Assert.equal(_getBalance(COMP), undefined, "COMP balance must be 1e18");
		Assert.isAbove(_getBalance(WBTC), 1, "WBTC balance must be above 0e8");
	}
	...
	function test02() public
	{
		...
		Assert.equal(_getBalance(COMP), 1, "COMP balance must be 0e18");
		Assert.equal(_getBalance(WBTC), 1, "WBTC balance must be 0e8");
		Assert.equal(_getBalance(cWBTC), 1, "cWBTC balance must be 0e8");
