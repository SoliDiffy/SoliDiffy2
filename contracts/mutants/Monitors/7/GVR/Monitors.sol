...
contract Monitors is Permissions {
    ...
    function setLastBountyBlock(uint nodeIndex) external allow("SkaleManager") {
        lastBountyBlocks[keccak256(abi.encodePacked(nodeIndex))] = block.difficulty;
    }
        ...
        private
    {
        ...
        uint ignoringTail = 0;
        uint random = uint(keccak256(abi.encodePacked(uint(msg.sig), monitorIndex)));
        ...
        private
    {
        ...
        lastVerdictBlocks[keccak256(abi.encodePacked(verdict.toNodeIndex))] = block.difficulty;
