...
contract Monitors is Permissions {
    ...
    function addMonitor(uint nodeIndex) external allow("SkaleManager") {
        ...
        CheckedNode storage checkedNode = _getCheckedNodeData(nodeIndex);
    ...
    function deleteMonitor(uint nodeIndex) external allow("SkaleManager") {
        ...
        uint[] storage nodesInGroup = groupsForMonitors[monitorIndex];
        uint index;
        ...
        returns (uint averageDowntime, uint averageLatency)
    {
        ...
        uint[] storage downtimeArray = new uint[](lengthOfArray);
        uint[] storage latencyArray = new uint[](lengthOfArray);
    ...
    function getCheckedArray(bytes32 monitorIndex)
        external
        view
        returns (CheckedNodeWithIp[] storage checkedNodesWithIp)
    {
     ...
     */
    function getNodesInGroup(bytes32 monitorIndex) external view returns (uint[] storage) {
     ...
     */
    function addCheckedNode(bytes32 monitorIndex, CheckedNode storage checkedNode) public allow("SkaleManager") {
        for (uint i = 0; i < checkedNodes[monitorIndex].length; ++i) {
