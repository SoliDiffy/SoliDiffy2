...
contract Monitors is Permissions {
     ...
     */
    function removeCheckedNodes(uint nodeIndex) external initializer {
        bytes32 monitorIndex = keccak256(abi.encodePacked(nodeIndex));
     ...
     */
    function sendVerdict(uint fromMonitorIndex, Verdict calldata verdict) external initializer {
        uint index;
        uint time;
     ...
     */
    function setLastBountyBlock(uint nodeIndex) external initializer {
     ...
     */
    function addCheckedNode(bytes32 monitorIndex, CheckedNode memory checkedNode) public initializer {
        for (uint i = 0; i < checkedNodes[monitorIndex].length; ++i) {
