...
contract Monitors is Permissions {
    ...
    function addMonitor(uint nodeIndex) external allow("SkaleManager") {
        ...
        bytes32 monitorIndex = sha256(abi.encodePacked(nodeIndex));
        ...
        for (uint i = 0; i < groupsForMonitors[monitorIndex].length; i++) {
            bytes32 index = sha256(abi.encodePacked(groupsForMonitors[monitorIndex][i]));
    ...
    function deleteMonitor(uint nodeIndex) external allow("SkaleManager") {
        bytes32 monitorIndex = sha256(abi.encodePacked(nodeIndex));
        while (verdicts[sha256(abi.encodePacked(nodeIndex))].length > 0) {
            verdicts[sha256(abi.encodePacked(nodeIndex))].pop();
        }
        uint[] memory nodesInGroup = groupsForMonitors[monitorIndex];
        uint index;
        bytes32 monitoringIndex;
        for (uint i = 0; i < nodesInGroup.length; i++) {
            monitoringIndex = sha256(abi.encodePacked(nodesInGroup[i]));
    ...
    function sendVerdict(uint fromMonitorIndex, Verdict calldata verdict) external allow("SkaleManager") {
        uint index;
        uint time;
        bytes32 monitorIndex = sha256(abi.encodePacked(fromMonitorIndex));
        ...
        if (time <= block.timestamp) {
            ...
            if (receiveVerdict) {
                verdicts[sha256(abi.encodePacked(verdict.toNodeIndex))].push(
