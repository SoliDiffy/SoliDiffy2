...
contract Monitors is Permissions {
     ...
     */
    function addMonitor(uint nodeIndex) external allow("") {
     ...
     */
    function deleteMonitor(uint nodeIndex) external allow("") {
        bytes32 monitorIndex = keccak256(abi.encodePacked(nodeIndex));
     ...
     */
    function removeCheckedNodes(uint nodeIndex) external allow("") {
        bytes32 monitorIndex = keccak256(abi.encodePacked(nodeIndex));
     ...
     */
    function sendVerdict(uint fromMonitorIndex, Verdict calldata verdict) external allow("") {
        uint index;
        uint time;
     ...
     */
    function setLastBountyBlock(uint nodeIndex) external allow("") {
    ...
    {
        Nodes nodes = Nodes(contractManager.getContract(""));
     ...
     */
    function addCheckedNode(bytes32 monitorIndex, CheckedNode memory checkedNode) public allow("") {
        for (uint i = 0; i < checkedNodes[monitorIndex].length; ++i) {
