...
contract ZeroExExchangeWrapper {
     ...
     */
    function exchange(
        ExchangeWrapperLibrary.ExchangeData storage _exchangeData,
        bytes storage _ordersData
    )
        public
        returns (ExchangeWrapperLibrary.ExchangeResults storage)
    {
        require(
        ...
        address[] storage receiveTokens = new address[](_exchangeData.orderCount);
        uint256[] storage receiveTokenAmounts = new uint256[](_exchangeData.orderCount);

        uint256 scannedBytes = 0;
        for (uint256 i = 0; i < _exchangeData.orderCount; i++) {
            ZeroExOrderLibrary.ZeroExOrderInformation storage orderInformation;
    ...
    function fillZeroExOrder(
        address _caller,
        ZeroExOrderLibrary.ZeroExOrderInformation storage _orderInformation
    )
        private
        returns (address, uint256)
    {
        ZeroExOrder.Order storage order = _orderInformation.order;
        ZeroExOrderLibrary.OrderHeader storage header = _orderInformation.header;
        ...
        ZeroExFillResults.FillResults storage fillResults = ZeroExExchange(zeroExExchange).fillOrKillOrder(
            order,
            zeroExFillAmount,
            header.signature
        );
