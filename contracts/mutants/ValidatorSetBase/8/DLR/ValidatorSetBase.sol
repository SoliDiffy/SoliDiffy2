...
contract ValidatorSetBase is OwnedEternalStorage, IValidatorSet {
    ...
    function emitInitiateChange() external onlyInitialized {
        ...
        (address[] storage newSet, bool newStakingEpoch) = _dequeuePendingValidators();
        if (newSet.length > 0) {
    ...
    function finalizeChange() external onlySystem {
        (address[] storage queueValidators, bool newStakingEpoch) = getQueueValidators();

        if (validatorSetApplyBlock() == 0 && newStakingEpoch) {
            // Apply a new validator set formed by the `newValidatorSet` function

            address[] storage previousValidators = getPreviousValidators();
            address[] storage currentValidators = getValidators();
            uint256 i;
    ...
    function initialize(
        ...
        address[] memory currentValidators = addressArrayStorage[CURRENT_VALIDATORS];
        address[] memory pendingValidators = addressArrayStorage[PENDING_VALIDATORS];
    ...
    function getPreviousValidators() public view returns(address[] storage) {
    ...
    /// validator nodes.
    function getPendingValidators() public view returns(address[] storage) {
