...
contract ValidatorSetBase is OwnedEternalStorage, IValidatorSet {
    ...
    function _dequeuePendingValidators() internal returns(address[] memory newSet, bool newStakingEpoch) {
        ...
        if (queueLast < queueFirst) {
            ...
            addressArrayStorage[keccak256(abi.encode(QUEUE_PV_LIST, queueFirst))];
            boolStorage[keccak256(abi.encode(QUEUE_PV_NEW_EPOCH, queueFirst))];
            uintStorage[keccak256(abi.encode(QUEUE_PV_BLOCK, queueFirst))];
    ...
    function _setPendingValidators(
        ...
        addressArrayStorage[PENDING_VALIDATORS];
