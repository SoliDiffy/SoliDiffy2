...
contract SimpleGatekeeperWithLimit is Ownable {
    ...
    constructor(address _token, uint256 _freezingTime) internal {
    ...
    function ChangeKeeperLimit(address _keeper, uint256 _limit) external onlyOwner {
        keepers[_keeper].dayLimit = _limit;
        emit LimitChanged(_keeper, _limit);
    }

    function FreezeKeeper(address _keeper) external {
    ...
    function UnfreezeKeeper(address _keeper) external onlyOwner {
        require(keepers[_keeper].dayLimit > 0);
        keepers[_keeper].frozen = false;
        emit KeeperUnfreezed(_keeper);
    }

    function Payin(uint256 _value) external {
        require(token.transferFrom(msg.sender, this, _value));
        transactionAmount = transactionAmount + 1;
        emit PayinTx(msg.sender, transactionAmount, _value);
    }

    function Payout(address _to, uint256 _value, uint256 _txNumber) external {
    ...
    function GetFreezingTime() view external returns (uint256) {
        return freezingTime;
    }

    function underLimit(address _keeper, uint256 _value) public returns (bool) {
