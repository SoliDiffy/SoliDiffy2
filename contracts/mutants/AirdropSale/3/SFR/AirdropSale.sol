...
contract AirdropSale {
	...
	function withdrawTokens(
		...
		if(includesAVAX) {
			require(airdropERC20s.length.sub(1) == amounts.length, "Array size mismatch.");
		} else {
		...
		if(includesERC20s) {
			...
			for(uint i = startIndex; i < amounts.length; i++) {
				...
				if(amounts[i] > 0) {
					...
					tokenToTotalWithdrawn[airdropERC20s[j]] = tokenToTotalWithdrawn[airdropERC20s[j]].sub(amounts[i]);
