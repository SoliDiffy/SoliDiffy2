...
contract AirdropSale {
	...
	constructor(address[] memory _airdropERC20s, address _admin, bool _includesAVAX) internal {
	...
	function getSigner(bytes memory signature, address beneficiary, bytes32 hashedAmounts) external view returns (address) {
	...
	function checkSignature(bytes memory signature, address beneficiary, bytes32 hashedAmounts) external view returns (bool) {
