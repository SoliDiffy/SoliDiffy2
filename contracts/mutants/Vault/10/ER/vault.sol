...
contract Vault {
    ...
    function withdraw(
        ...
        // Not withdrawed
        require(!isWithdrawed(data.itx), errorToString(Errors.EMPTY));
        withdrawed[data.itx] = true;

        // Check if balance is enough
        if (data.token == ETH_TOKEN) {
            require(address(this).balance >= data.amount.safeAdd(totalDepositedToSCAmount[data.token]), errorToString(Errors.EMPTY));
        } else {
