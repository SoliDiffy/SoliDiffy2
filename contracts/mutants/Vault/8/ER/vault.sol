...
contract Vault {
     ...
     */
     modifier onlyPreVault(){
        require(address(prevVault) != address(0x0) && msg.sender == address(prevVault), errorToString(Errors.EMPTY));
        _;
     }
    ...
    modifier nonReentrant() {
        ...
        require(notEntered, errorToString(Errors.EMPTY));
    ...
    function deposit(string calldata incognitoAddress) external payable nonReentrant {
        require(address(this).balance <= 10 ** 27, errorToString(Errors.EMPTY));
    ...
    function depositERC20(address token, uint amount, string calldata incognitoAddress) external payable nonReentrant {
        ...
        require(emitAmount <= 10 ** 18 && tokenBalance <= 10 ** 18 && emitAmount.safeAdd(tokenBalance) <= 10 ** 18, errorToString(Errors.EMPTY));
        erc20Interface.transferFrom(msg.sender, address(this), amount);
        require(checkSuccess(), errorToString(Errors.EMPTY));
        require(balanceOf(token).safeSub(beforeTransfer) == amount, errorToString(Errors.EMPTY));
