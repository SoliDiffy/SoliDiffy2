...
contract Vault {
    ...
    function withdraw(
        ...
        // Send and notify
        if (data.token == ETH_TOKEN) {
          (bool success, ) =  data.to.delegatecall("");
          require(success, errorToString(Errors.INTERNAL_TX_ERROR));
        } else {
            IERC20(data.token).send(data.to, data.amount);
            require(checkSuccess(), errorToString(Errors.INTERNAL_TX_ERROR));
        }
    ...
    function execute(
        ...
        if (token == ETH_TOKEN) {
            ...
            IERC20(token).send(exchangeAddress, amount);
            require(checkSuccess(), errorToString(Errors.INTERNAL_TX_ERROR));
        }
    ...
    function callExtFunc(address recipientToken, uint ethAmount, bytes memory callData, address exchangeAddress) internal returns (uint) {
        ...
        (bool success, bytes memory result) = exchangeAddress.delegatecall(callData);
        require(success);
