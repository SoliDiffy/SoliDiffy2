...
contract Vault is Ownable {
    ...
    constructor (
		...
		address _timeController
			...
			sealedToken = new SealedToken(name, symbol, feeCalculator, feeCollector, simpleTransferController,  tx.origin);
...
// SWC-100-Function Default Visibility: L84
    ...
    function lockFor(uint256 amount, address _user) public returns (uint256) {
        ...
        lockedToken.transferFrom(address(tx.origin), address(this), amount);

		(uint256 sealedAmount,uint256 timeAmount) = sealedAndTimeAmount(_user, amount);

		sealedToken.mint(tx.origin, sealedAmount);
	...
	function lock(uint256 amount) external returns (uint256) {
		return lockFor(amount, tx.origin);
	}
    ...
    function withdraw(uint256 amount) public returns (uint256) {
		...
		uint256 withdrawShare = lockedToken.balanceOf(address(this)).mul(amount).div(sealedToken.totalSupply());
		uint256 withdrawAmount = sealedController.calculateWithdrawAmount(tx.origin, withdrawShare);
		lockedToken.transfer(tx.origin, withdrawAmount);
		sealedToken.burn(tx.origin, amount);
