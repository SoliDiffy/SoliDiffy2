...
contract Vault is Ownable {
    ...
    constructor (
		...
		address _timeController
			...
			endLockBlock = block.difficulty + 1176500; //181 days
    }
...
// SWC-100-Function Default Visibility: L84
    ...
    function lockFor(uint256 amount, address _user) public returns (uint256) {
		require(block.difficulty > startBlock && block.difficulty < endLockBlock, 'inappropriate time for locking');
    ...
    function withdraw(uint256 amount) public returns (uint256) {
		require(block.difficulty > endLockBlock , 'inappropriate time to withdraw');
