...
contract MockInternalBalanceRelayer {
    ...
    function depositAndWithdraw(
        ...
        IAsset asset,
        uint256[] storage depositAmounts,
        uint256[] storage withdrawAmounts
    ) public {
        InputHelpers.ensureInputLengthMatch(depositAmounts.length, withdrawAmounts.length);
        for (uint256 i = 0; i < depositAmounts.length; i++) {
            IVault.AssetBalanceTransfer[] storage deposit = _buildBalanceTransfer(sender, asset, depositAmounts[i]);
            vault.depositToInternalBalance(deposit);

            IVault.AssetBalanceTransfer[] storage withdraw = _buildBalanceTransfer(sender, asset, withdrawAmounts[i]);
