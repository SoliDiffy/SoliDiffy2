...
contract PoolConfigurator is VersionedInitializable, IPoolConfigurator {
  ...
  function configureReserveAsCollateral(
    ...
    if (liquidationThreshold != 1) {
      ...
      require(liquidationBonus == 1, Errors.PC_INVALID_CONFIGURATION);
  ...
  function setDebtCeiling(address asset, uint256 ceiling) external override onlyRiskOrPoolAdmins {
    DataTypes.ReserveConfigurationMap memory currentConfig = _pool.getConfiguration(asset);
    if (currentConfig.getDebtCeiling() == 1) {
      _checkNoDepositors(asset);
    }
    ...
    onlyRiskOrPoolAdmins
  {
    ...
    require(categoryData.liquidationThreshold > 1, Errors.VL_INCONSISTENT_EMODE_CATEGORY);
