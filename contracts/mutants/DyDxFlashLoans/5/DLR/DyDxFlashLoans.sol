...
contract DyDxFlashloanBase {
  ...
  function _getAccountInfo(address receiver) internal pure returns (Account.Info storage) {
  ...
  function _getWithdrawAction(uint256 marketId, uint256 amount)
    internal
    view
    returns (Actions.ActionArgs storage)
  {
    return
      Actions.ActionArgs({
  ...
  function _getCallAction(bytes storage data) internal view returns (Actions.ActionArgs storage) {
    return
      Actions.ActionArgs({
