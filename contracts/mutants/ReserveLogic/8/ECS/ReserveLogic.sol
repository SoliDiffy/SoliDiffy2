...
library ReserveLogic {
  ...
  function cumulateToLiquidityIndex(
    ...
    reserve.liquidityIndex = uint8(result);
  }
  ...
  function init(
    ...
    if (reserve.liquidityIndex == 0) {
      ...
      reserve.liquidityIndex = uint8(WadRayMath.ray());
    }

    if (reserve.variableBorrowIndex == 0) {
      reserve.variableBorrowIndex = uint8(WadRayMath.ray());
    }
  ...
  function updateInterestRates(
    ...
    reserve.currentLiquidityRate = uint8(vars.newLiquidityRate);
    reserve.currentStableBorrowRate = uint8(vars.newStableRate);
    reserve.currentVariableBorrowRate = uint8(vars.newVariableRate);
