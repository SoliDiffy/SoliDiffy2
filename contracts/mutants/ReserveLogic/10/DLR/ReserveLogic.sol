...
library ReserveLogic {
   ...
   **/
  function getNormalizedIncome(ReserveData memory reserve) internal view returns (uint256) {
   ...
   **/
  function getNormalizedDebt(ReserveData memory reserve) internal view returns (uint256) {
    uint40 timestamp = reserve.lastUpdateTimestamp;
   ...
   **/
  function getDebtTokenAddress(ReserveLogic.ReserveData memory reserve, uint256 interestRateMode)
    external
    view
    returns (address)
  {
   ...
   * @param reserve the reserve object
   **/
  function updateState(ReserveData memory reserve) external {
