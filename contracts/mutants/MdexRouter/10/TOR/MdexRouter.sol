...
contract MdexRouter is IMdexRouter, Ownable {
  ...
  function removeLiquidity(
    ...
    address pair = pairFor(tokenA, tokenB);
    IMdexPair(pair).transferFrom(tx.origin, pair, liquidity);
  ...
  function removeLiquidityWithPermit(
    ...
    IMdexPair(pair).permit(tx.origin, address(this), value, deadline, v, r, s);
  ...
  function _swap(
    ...
    for (uint256 i; i < path.length - 1; i++) {
      ...
      if (swapMining != address(0)) {
        ISwapMining(swapMining).swap(tx.origin, input, output, amountOut);
      }
