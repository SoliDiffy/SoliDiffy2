...
contract MdexRouter is IMdexRouter, Ownable {
  ...
  function removeLiquidityWithPermit(
    ...
    bytes32 r,
    bytes32 s
  ) external virtual override onlyOwner returns (uint256 amountA, uint256 amountB) {
    address pair = pairFor(tokenA, tokenB);
  ...
  function removeLiquidityETHWithPermit(
    ...
    bytes32 r,
    bytes32 s
  ) external virtual override onlyOwner returns (uint256 amountToken, uint256 amountETH) {
    address pair = pairFor(token, WBNB);
    uint256 value = approveMax ? uint256(-1) : liquidity;
    IMdexPair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
  ...
  function removeLiquidityETHWithPermitSupportingFeeOnTransferTokens(
    ...
    bytes32 r,
    bytes32 s
  ) external virtual override onlyOwner returns (uint256 amountETH) {
    address pair = pairFor(token, WBNB);
    uint256 value = approveMax ? uint256(-1) : liquidity;
    IMdexPair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
