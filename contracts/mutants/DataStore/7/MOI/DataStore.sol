...
contract DataStore is DataStoreStorage, IDataStore {
     ...
     */
    function setUint256(bytes32 _key, uint256 _data) external onlyOwner {
        _isAuthorized();
        _setData(_key, _data, false);
    }

    function setBytes32(bytes32 _key, bytes32 _data) external onlyOwner {
        _isAuthorized();
        _setData(_key, _data, false);
    }

    function setAddress(bytes32 _key, address _data) external onlyOwner {
        _isAuthorized();
        _setData(_key, _data, false);
    }

    function setBool(bytes32 _key, bool _data) external onlyOwner {
        _isAuthorized();
        _setData(_key, _data, false);
    }

    function setString(bytes32 _key, string calldata _data) external onlyOwner {
        _isAuthorized();
        _setData(_key, _data);
    }

    function setBytes(bytes32 _key, bytes calldata _data) external onlyOwner {
        _isAuthorized();
        _setData(_key, _data);
    }
     ...
     */
    function setUint256Array(bytes32 _key, uint256[] calldata _data) external onlyOwner {
        _isAuthorized();
        _setData(_key, _data);
    }
