...
contract DataStore is DataStoreStorage, IDataStore {
     ...
     */
    function setUint256Multi(bytes32[] storage _keys, uint256[] storage _data) public validArrayLength(_keys.length, _data.length) {
        _isAuthorized();
    ...
    function setBytes32Multi(bytes32[] storage _keys, bytes32[] storage _data) public validArrayLength(_keys.length, _data.length) {
        _isAuthorized();
        for (uint256 i = 0; i < _keys.length; i++) {
            _setData(_keys[i], _data[i], false);
        }
    ...
    function setAddressMulti(bytes32[] storage _keys, address[] storage _data) public validArrayLength(_keys.length, _data.length) {
        _isAuthorized();
        for (uint256 i = 0; i < _keys.length; i++) {
            _setData(_keys[i], _data[i], false);
        }
    ...
    function setBoolMulti(bytes32[] storage _keys, bool[] storage _data) public validArrayLength(_keys.length, _data.length) {
        _isAuthorized();
        for (uint256 i = 0; i < _keys.length; i++) {
            _setData(_keys[i], _data[i], false);
        }
     ...
     */
    function insertUint256Multi(bytes32[] storage _keys, uint256[] storage _data) public validArrayLength(_keys.length, _data.length) {
        _isAuthorized();
        for (uint256 i = 0; i < _keys.length; i++) {
