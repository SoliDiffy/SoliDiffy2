...
contract DataStore is DataStoreStorage, IDataStore {
    ...
    function setBytes32Array(bytes32 _key, bytes32[] calldata _data) external onlyOwner {
        _isAuthorized();
        _setData(_key, _data);
    }

    function setAddressArray(bytes32 _key, address[] calldata _data) external onlyOwner {
        _isAuthorized();
        _setData(_key, _data);
    }

    function setBoolArray(bytes32 _key, bool[] calldata _data) external onlyOwner {
        _isAuthorized();
        _setData(_key, _data);
    }
