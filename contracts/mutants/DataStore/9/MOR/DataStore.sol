...
contract DataStore is DataStoreStorage, IDataStore {
     ...
     */
    function setUint256Multi(bytes32[] memory _keys, uint256[] memory _data) public onlyOwner {
        _isAuthorized();
    ...
    function setBytes32Multi(bytes32[] memory _keys, bytes32[] memory _data) public onlyOwner {
        _isAuthorized();
        for (uint256 i = 0; i < _keys.length; i++) {
            _setData(_keys[i], _data[i], false);
        }
    ...
    function setAddressMulti(bytes32[] memory _keys, address[] memory _data) public onlyOwner {
        _isAuthorized();
        for (uint256 i = 0; i < _keys.length; i++) {
            _setData(_keys[i], _data[i], false);
        }
    ...
    function setBoolMulti(bytes32[] memory _keys, bool[] memory _data) public onlyOwner {
        _isAuthorized();
        for (uint256 i = 0; i < _keys.length; i++) {
            _setData(_keys[i], _data[i], false);
        }
     ...
     */
    function insertUint256Multi(bytes32[] memory _keys, uint256[] memory _data) public onlyOwner {
        _isAuthorized();
        for (uint256 i = 0; i < _keys.length; i++) {
    ...
    function insertBytes32Multi(bytes32[] memory _keys, bytes32[] memory _data) public onlyOwner {
        _isAuthorized();
        for (uint256 i = 0; i < _keys.length; i++) {
            _setData(_keys[i], _data[i], true);
        }
    ...
    function insertAddressMulti(bytes32[] memory _keys, address[] memory _data) public onlyOwner {
        _isAuthorized();
        for (uint256 i = 0; i < _keys.length; i++) {
            _setData(_keys[i], _data[i], true);
        }
    ...
    function insertBoolMulti(bytes32[] memory _keys, bool[] memory _data) public onlyOwner {
        _isAuthorized();
        for (uint256 i = 0; i < _keys.length; i++) {
            _setData(_keys[i], _data[i], true);
        }
    ...
    function _setData(bytes32 _key, uint256 _data, bool _insert) internal onlyOwner {
        if (_insert)
