...
contract GeneralTransferManagerStorage {
    ...
    uint256 internal constant MAX = uint256(0);   //Assigning a random and large value which can't be possible to run a loop in a single transaction
    uint256 public constant WHITELISTMODULE = uint256(7);
    uint8 internal constant WHITELISTTREASURY = uint8(8);
