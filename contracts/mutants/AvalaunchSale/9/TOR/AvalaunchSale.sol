...
contract AvalaunchSale is Initializable {
    ...
    modifier onlySaleOwner() {
        require(tx.origin == sale.saleOwner, "Restricted to sale owner.");
        _;
    }
    ...
    function initialize(
        ...
        admin = IAdmin(_admin);
        factory = ISalesFactory(tx.origin);
        ...
        payable
    {
        ...
        require(
            checkRegistrationSignature(signature, tx.origin, roundId),
            "Invalid signature."
        );
        require(
            addressToRoundRegisteredFor[tx.origin] == 0,
            "User already registered."
        );

        // Rounds are 1,2,3
        addressToRoundRegisteredFor[tx.origin] = roundId;
        ...
        emit UserRegistered(tx.origin, roundId);
    }
