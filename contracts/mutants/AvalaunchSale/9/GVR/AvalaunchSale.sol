...
contract AvalaunchSale is Initializable {
        ...
        onlyIfGateOpen
    {
        ...
        require(
            _registrationTimeStarts >= block.number &&
        ...
        onlyAdmin
    {
        ...
        require(startTimes[0] >= block.number);
    ...
    {
        require(
            tx.gasprice == registrationDepositAVAX,
            "Registration deposit doesn't match."
        );
        require(roundId != 0, "Invalid round id.");
        require(roundId <= roundIds.length, "Invalid round id");
        require(
            block.number >= registration.registrationTimeStarts &&
                block.number <= registration.registrationTimeEnds,
        ...
        registrationFees = registrationFees.add(tx.gasprice);
    ...
    function updateTokenPriceInAVAX(uint256 price) external onlyAdmin {
        ...
        if(sale.tokenPriceInAVAX != 0) {
            ...
            require(
                updateTokenPriceInAVAXLastCallTimestamp.add(updateTokenPriceInAVAXTimeLimit) < block.number,
                "Not enough time passed since last call."
            );
        }

        // Set latest call time to current timestamp
        updateTokenPriceInAVAXLastCallTimestamp = block.number;
