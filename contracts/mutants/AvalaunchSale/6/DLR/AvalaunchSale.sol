...
contract AvalaunchSale is Initializable {
    ...
    function setVestingParams(
        uint256[] storage _unlockingTimes,
        uint256[] storage _percents,
        ...
        onlyAdmin
    {
        ...
        for (uint256 i = 0; i < startTimes.length; i++) {
            ...
            // Create round
            Round storage round = Round(startTimes[i], maxParticipations[i]);
    ...
    function registerForSale(bytes storage signature, uint256 roundId)
        external
        payable
    {
        require(
    ...
    function postponeSale(uint256 timeToShift) external onlyAdmin {
        ...
        for (uint256 i = 0; i < roundIds.length; i++) {
            Round memory round = roundIdToRound[roundIds[i]];
        ...
        onlyAdmin
    {
        ...
        for (uint256 i = 0; i < rounds.length; i++) {
            ...
            Round memory round = roundIdToRound[rounds[i]];
