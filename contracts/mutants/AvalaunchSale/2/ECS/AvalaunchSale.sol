...
contract AvalaunchSale is Initializable {
    ...
    function _participate(
        ...
        uint256 amountOfTokensBuying =
            (amountAVAX).mul(uint8(10) ** IERC20Metadata(address(sale.token)).decimals()).div(sale.tokenPriceInAVAX);
    ...
    function boostParticipation(
        ...
        // Compute the amount of tokens user is buying
        uint256 amountOfTokensBuying =
            (msg.value).mul(uint8(10) ** IERC20Metadata(address(sale.token)).decimals()).div(sale.tokenPriceInAVAX);
