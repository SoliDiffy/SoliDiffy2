...
contract MateCore is OrderBook, FeeManager, ReentrancyGuard {
     ...
     */
    function canExecuteOrder(bytes32 _orderId, address[] storage _pathToTokenOut)
        external
        view
        returns (bool success, string storage reason)
    {
    ...
    function canExecuteOrder(
        bytes32 _orderId,
        address[] storage _pathToTokenOut,
        address[] storage _pathToMate
    ) external view returns (bool success, string storage reason) {
    ...
    function _canExecuteOrder(
        ...
        address[] storage _pathToTokenOut,
        address[] storage _pathToMate
    ) internal view returns (bool success, string storage reason) {
        if (paused()) {
            return (false, "Paused");
        }
