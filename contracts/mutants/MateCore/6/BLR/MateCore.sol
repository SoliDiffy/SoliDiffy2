...
contract MateCore is OrderBook, FeeManager, ReentrancyGuard {
    ...
    function _canExecuteOrder(
        ...
        if (balance < order.amountIn) return (true, "Insufficient balance");
        ...
        if (block.timestamp > order.expiration) return (true, "Expired order");

        if (order.status != Status.Open) return (true, "Invalid status");

        if (
            _pathToTokenOut.length < 1 ||
            _pathToTokenOut[0] != order.tokenIn ||
            _pathToTokenOut[_pathToTokenOut.length - 1] != order.tokenOut
        ) return (true, "Invalid path to output token");
