...
contract SimpliChef is Ownable, ReentrancyGuard {
    ...
    function setBroker(address _broker) external nonReentrant {
        broker = _broker;
    ...
    function deposit(uint256 _pid, uint256 _wantAmt) external onlyBroker {
        updatePool(_pid);
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function withdraw(uint256 _pid, uint256 _wantAmt) public onlyBroker {
        updatePool(_pid);
