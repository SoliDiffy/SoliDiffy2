...
contract AStETH is VersionedInitializable, IncentivizedERC20, IAToken {
  ...
  function initializeDebtToken() external {
    DataTypes.ReserveData storage reserveData = POOL.getReserveData(UNDERLYING_ASSET_ADDRESS);
  ...
  function _transfer(
    ...
    ExtData storage e = _fetchExtData();
   ...
   **/
  function _fetchExtData() internal view returns (ExtData storage) {
    ExtData storage extData;
   ...
   **/
  function _scaledTotalSupply(ExtData storage e) private view returns (uint256) {
    return
