...
contract ArthPoolInvestorForV2 is AccessControl {
    ...
    constructor(
        ...
        missing_decimals = uint256(18).add(collateralToken.decimals());
        ...
        returns (uint256[5] memory allocations)
    {
        ...
        allocations[3] = (
            cUSDC.balanceOf(address(this)).mul(cUSDC.exchangeRateStored()).mul(
                1e18
            )
    ...
    function getCollateralGMUBalance() external view returns (uint256) {
        ...
        if (pool.collateralPricePaused() == true) {
            ...
            uint256 collat_usd_price =
                eth_usd_price.mul(PRICE_PRECISION).mul(eth_collat_price);
            return
                borrowed_balance
                    .mul(10**missing_decimals)
                    .mul(collat_usd_price)
                    .mul(PRICE_PRECISION); //.mul(getCollateralPrice()).div(1e6);
        }
    }
        ...
        onlyByOwnerOrGovernance
    {
        ...
        uint256 redeem_amount_E6 =
            (arth_amount.mul(uint256(1e6).sub(redemptionFee))).mul(1e6).div(
                10**missing_decimals
            );
