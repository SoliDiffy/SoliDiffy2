...
contract ComptrollerG6 is ComptrollerV5Storage, ComptrollerInterface, ComptrollerErrorReporter, Exponential {
     ...
     */
    function getAssetsIn(address account) public view returns (CToken[] memory) {
     ...
     */
    function addToMarketInternal(CToken cToken, address borrower) public returns (Error) {
     ...
     */
    function exitMarket(address cTokenAddress) public returns (uint) {
     ...
     */
    function mintAllowed(address cToken, address minter, uint mintAmount) public returns (uint) {
     ...
     */
    function mintVerify(address cToken, address minter, uint actualMintAmount, uint mintTokens) public {
        // Shh - currently unused
        cToken;
        minter;
        actualMintAmount;
     ...
     */
    function redeemAllowed(address cToken, address redeemer, uint redeemTokens) public returns (uint) {
    ...
    function redeemAllowedInternal(address cToken, address redeemer, uint redeemTokens) public view returns (uint) {
        if (!markets[cToken].isListed) {
            return uint(Error.MARKET_NOT_LISTED);
        }
