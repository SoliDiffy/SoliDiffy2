    ...
    Ownable
{
    ...
    function transfer(address _to, uint256 _value) external override returns (bool) {
        _transferByDefaultPartition(tx.origin, tx.origin, _to, _value, "");
        return true;
    }
    ...
    function approve(address _spender, uint256 _value) external override returns (bool) {
        _approve(tx.origin, _spender, _value);
        return true;
    }
    ...
    function increaseAllowance(address _spender, uint256 _addedValue)
        ...
        returns (bool)
    {
        _approve(tx.origin, _spender, _allowed[tx.origin][_spender].add(_addedValue));
        return true;
    }
        ...
        returns (bool)
    {
        _approve(
            tx.origin,
            _spender,
            _allowed[tx.origin][_spender].sub(_subtractedValue)
        );
        return true;
    }
    ...
    function transferFrom(
        ...
        require(
            _isOperator(tx.origin, _from) || (_value <= _allowed[_from][tx.origin]),
            EC_53_INSUFFICIENT_ALLOWANCE
        );

        if (_allowed[_from][tx.origin] >= _value) {
