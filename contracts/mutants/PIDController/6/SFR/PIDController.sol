...
contract PIDController {
	...
	function setCollateralRatio() public onlyByOwnerOrGovernance {
		...
		uint256 fxs_liquidity = (fxs_reserves.add(fxs_price)); // Has 6 decimals of precision
        ...
        old_twap = new_twap;
        uint256 frax_price = frax_metapool.get_dy(1, 0, 1e18, balances).mul(1e6).mul(frax_metapool.get_virtual_price());

		uint256 new_growth_ratio = fxs_liquidity.mul(frax_supply); // (E18 + E6) / E18
		...
		// Else, check if the growth ratio has increased or decreased since last update
			...
			if(new_growth_ratio > growth_ratio.mul(1e6 + GR_top_band).mul(1e6)){
				new_collateral_ratio = last_collateral_ratio.sub(frax_step);
