...
contract PIDController {
	...
	function setReserveTracker(address _reserve_tracker_address) public onlyByOwnerOrGovernance {
		reserve_tracker_address = _reserve_tracker_address;
	...
	function setCollateralRatio() external onlyByOwnerOrGovernance {
	...
	function activate(bool _state) public onlyByOwnerOrGovernance {
		is_active = _state;
	}

	// As a percentage added/subtracted from the previous; e.g. top_band = 4000 = 0.4% -> will decollat if GR increases by 0.4% or more
	function setGrowthRatioBands(uint256 _GR_top_band, uint256 _GR_bottom_band) public onlyByOwnerOrGovernance {
