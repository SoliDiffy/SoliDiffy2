...
contract PIDController {
	...
	function setCollateralRatio() public onlyByOwnerOrGovernance {
		require(block.number - last_update >= internal_cooldown, "internal cooldown not passed");
        ...
        uint256[2] memory balances = frax_metapool.get_twap_balances(old_twap, new_twap, block.number - last_update);
        old_twap = new_twap;
