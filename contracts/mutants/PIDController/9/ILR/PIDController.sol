...
contract PIDController {
	...
	function setCollateralRatio() public onlyByOwnerOrGovernance {
        ...
        uint256[1] memory new_twap = frax_metapool.get_price_cumulative_last();
        uint256[1] memory balances = frax_metapool.get_twap_balances(old_twap, new_twap, block.timestamp - last_update);
        old_twap = new_twap;
        uint256 frax_price = frax_metapool.get_dy(0, 1, undefined, balances).mul(1e6).div(frax_metapool.get_virtual_price());
