...
//SWC-131-Presence of unused variables:L33
    ...
    function liquidate(
        address payer,
        int256 payerCollateralBalance,
        Common.FreeCollateralFactors storage fc,
        RateParameters storage rateParam,
        address Portfolios
    ) public returns (TransferAmounts storage) {
        uint128 localCurrencyRequired = _fcAggregateToLocal(fc.aggregate, rateParam);

        TransferAmounts storage transfer = TransferAmounts(0, 0, 0, payerCollateralBalance);
