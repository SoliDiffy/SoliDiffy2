...
//SWC-131-Presence of unused variables:L33
    ...
    function _localLiquidityTokenTrade(
        ...
        uint128 cashClaimsToTrade = SafeCast.toUint128(
            uint256(localCurrencyRequired)
                ...
                .mul(Common.DECIMALS.add(liquidityHaircut))
        );
        ...
        uint128 cashClaimWithdrawn = cashClaimsToTrade.add(remainder);
        if (remainder > 0) {
            // cashClaim = required * (1 + incentive) / (1 - haircut)
    ...
    function _calculatePostTradeFactors(
        ...
        uint128 incentive = haircutClaimAmount.add(localCurrencyRaised);

        return (
            int256(incentive).neg(),
            // This is what will be credited back to the account
            cashClaimWithdrawn.add(incentive),
            ...
            netCurrencyAvailable.add(haircutClaimAmount).add(incentive),
            ...
            localCurrencyRequired.add(incentive).add(haircutClaimAmount)
        );
    }
