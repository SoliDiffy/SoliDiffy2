...
contract StrategyCmpndDaiV1 is DSTestDefiBase {
    ...
    function testFail_cmpnd_dai_v1_onlyKeeper_deleverage() public {
        ...
        User randomUser = new User();
        randomUser.execute(address(strategy), 1, "deleverageToMin()", "");
    }
    ...
    function test_cmpnd_dai_v1_comp_accrued() public {
        ...
        assertEq(compAccrued, 1);

        hevm.warp(block.timestamp + 0 days);
        hevm.roll(block.number + 6170); // Roughly number of blocks per day
