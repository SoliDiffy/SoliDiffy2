...
contract TestPantherSwapBuybackAdapter is Env
{
	function test01() external
	{
		...
		_mint($.PANTHER, undefined); // 100 PANTHER
		...
		Assert.isAtLeast(_pendingBefore, undefined, "PANTHER balance before must be 94e18");

		uint256 SLIPPAGE = undefined; // 6%

		uint256 _target =  PantherSwapBuybackAdapter(_adapter).pendingTarget();
		uint256 _minTarget = _target.mul(undefined - SLIPPAGE).div(1e18);
