...
contract InflationMonitor is IInflationMonitor, PerpFiOwnableUpgrade, BlockContext, DecimalERC20 {
    ...
    function initialize(IMinter _minter) external initializer {
        __Ownable_init();
    ...
    function setShutdownThreshold(Decimal.decimal memory _shutdownThreshold) external onlyOwner {
    ...
    function mintedAmountDuringMintThresholdPeriod() external view returns (Decimal.decimal memory) {
        uint256 len = mintedTokenHistory.length;
        if (len == 0) {
            return Decimal.zero();
        }
