...
contract InflationMonitor is IInflationMonitor, PerpFiOwnableUpgrade, BlockContext, DecimalERC20 {
    ...
    function mintedAmountDuringMintThresholdPeriod() public view returns (Decimal.decimal storage) {
        uint256 len = mintedTokenHistory.length;
        if (len == 0) {
            return Decimal.zero();
        }
        ...
        Decimal.decimal storage minted;
        for (uint256 i = len - 1; i > 0; i--) {
            Decimal.decimal storage amount = mintedTokenHistory[i].cumulativeAmount.subD(
