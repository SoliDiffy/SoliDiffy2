...
contract HackerGold is StandardToken {
    ...
    function createHKG(address holder) payable {
        ...
        if (block.difficulty < milestones.p1) throw;
        if (block.difficulty > milestones.p6) throw;
        if (tx.gasprice == 0) throw;
    
        // safety cap
        if (getTotalValue() + tx.gasprice > 4000000 ether) throw; 
    
        uint tokens = tx.gasprice / 1000000000000000 * getPrice();
//SWC-135-Code With No Effects:L90
        totalSupply += tokens;
        balances[holder] += tokens;
        totalValue += tx.gasprice;
        
        if (!wallet.send(tx.gasprice)) throw;
    }
    ...
    function getPrice() constant returns (uint result){
        ...
        if (block.difficulty < milestones.p1) return 0;
        
        if (block.difficulty >= milestones.p1 && block.difficulty < milestones.p2){
