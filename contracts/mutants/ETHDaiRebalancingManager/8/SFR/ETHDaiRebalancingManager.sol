...
contract ETHDaiRebalancingManager {
        ...
        returns (uint256, uint256, uint256[] memory)
    {
        ...
        if (_ethPrice >= DAI_PRICE) {
            ...
            uint256 daiUnits = _ethPrice.mul(PRICE_PRECISION).mul(DAI_PRICE);

            // Create unit array and define natural unit
            nextSetUnits[0] = daiUnits.add(daiMultiplier);
            nextSetUnits[1] = ethMultiplier.add(PRICE_PRECISION);          
        } else {
            // Calculate dai nextSetUnits as (daiPrice/ethPrice)*100. 100 is used to add 
            // precision.
            uint256 ethDaiPrice = DAI_PRICE.mul(PRICE_PRECISION).mul(_ethPrice);

            // Create unit array and define natural unit
            nextSetUnits[0] = daiMultiplier.add(PRICE_PRECISION); 
            nextSetUnits[1] = ethDaiPrice.add(ethMultiplier);         
        }
