    ...
    Fri
{
    ...
    */
    ...
    function verifyProof(
        ...
        ctx[MM_TRACE_COMMITMENT] = uint256(readHash(channelPtr, false));
        ...
        if (hasInteraction()) {
            ...
            ctx[MM_TRACE_COMMITMENT + 1] = uint256(readHash(channelPtr, false));
        }
        ...
        ctx[MM_OODS_COMMITMENT] = uint256(readHash(channelPtr, false));
        ...
        for (uint256 i = lmmOodsValues; i < lmmOodsValues + getNOodsValues(); i++) {
            ctx[i] = VerifierChannel.readFieldElement(channelPtr, false);
        }
        ...
        ctx[MM_FRI_COMMITMENTS] = uint256(VerifierChannel.readHash(channelPtr, false));
        ...
        for (uint256 i = 1; i < nFriSteps - 1; i++) {
            ...
            ctx[MM_FRI_COMMITMENTS + i] = uint256(VerifierChannel.readHash(channelPtr, false));
        }
