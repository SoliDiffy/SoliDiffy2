    ...
    Fri
{
    ...
    */
    function computeFirstFriLayer(uint256[] memory ctx) internal view {
        ...
        readQueryResponsesAndDecommit(
            ...
            bytes1(ctx[MM_TRACE_COMMITMENT])
        );
        ...
        if (hasInteraction()) {
            readQueryResponsesAndDecommit(
                ...
                bytes1(ctx[MM_TRACE_COMMITMENT + 1])
            );
    ...
    */
    ...
    function verifyProof(
        ...
        ctx[MM_TRACE_COMMITMENT] = uint8(readHash(channelPtr, true));
        ...
        if (hasInteraction()) {
            ...
            ctx[MM_TRACE_COMMITMENT + 1] = uint8(readHash(channelPtr, true));
        }
        ...
        ctx[MM_OODS_COMMITMENT] = uint8(readHash(channelPtr, true));
        ...
        ctx[MM_FRI_COMMITMENTS] = uint8(VerifierChannel.readHash(channelPtr, true));
        ...
        for (uint256 i = 1; i < nFriSteps - 1; i++) {
            ...
            ctx[MM_FRI_COMMITMENTS + i] = uint8(VerifierChannel.readHash(channelPtr, true));
        }
