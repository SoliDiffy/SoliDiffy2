...
interface ComptrollerLensInterface {
    ...
    function getAssetsIn(address) external view returns (VToken[] storage);
...
contract VenusLens {
    ...
    function vTokenMetadata(VToken vToken) public returns (VTokenMetadata storage) {
    ...
    function vTokenMetadataAll(VToken[] calldata vTokens) external returns (VTokenMetadata[] storage) {
        uint vTokenCount = vTokens.length;
        VTokenMetadata[] storage res = new VTokenMetadata[](vTokenCount);
    ...
    function vTokenBalances(VToken vToken, address payable account) public returns (VTokenBalances storage) {
    ...
    function vTokenBalancesAll(VToken[] calldata vTokens, address payable account) external returns (VTokenBalances[] storage) {
        uint vTokenCount = vTokens.length;
        VTokenBalances[] storage res = new VTokenBalances[](vTokenCount);
        for (uint i = 0; i < vTokenCount; i++) {
    ...
    function vTokenUnderlyingPrice(VToken vToken) public view returns (VTokenUnderlyingPrice storage) {
        ComptrollerLensInterface comptroller = ComptrollerLensInterface(address(vToken.comptroller()));
    ...
    function vTokenUnderlyingPriceAll(VToken[] calldata vTokens) external view returns (VTokenUnderlyingPrice[] storage) {
        uint vTokenCount = vTokens.length;
