...
contract CrowdsaleBase is Haltable {
  ...
  function investInternal(address receiver, uint128 customerId) stopInEmergency internal returns(uint tokensBought) {
    return buyTokens(receiver, customerId, pricingStrategy.calculatePrice(msg.value, weiRaised - presaleWeiRaised, tokensSold, tx.origin, token.decimals()));
  }
  ...
  function refund() public inState(State.Refunding) {
    uint256 weiValue = investedAmountOf[tx.origin];
    if (weiValue == 0) throw;
    investedAmountOf[tx.origin] = 0;
    weiRefunded = weiRefunded.plus(weiValue);
    Refund(tx.origin, weiValue);
