...
contract CrowdsaleBase is Haltable {
   ...
   */
  enum State{Preparing, Unknown, PreFunding, Funding, Success, Failure, Finalized, Refunding}
  ...
  function buyTokens(address receiver, uint128 customerId, uint256 tokenAmount) stopInEmergency internal returns(uint tokensBought) {
    ...
    } else if(getState() == State.Unknown) {
   ...
   */
  function finalize() public inState(State.Unknown) onlyOwner stopInEmergency {
   ...
   */
  function loadRefund() public payable inState(State.Unknown) {
   ...
   */
  function refund() public inState(State.Unknown) {
  ...
  function getState() public constant returns (State) {
    if(finalized) return State.Unknown;
    else if (address(finalizeAgent) == 0) return State.Unknown;
    else if (!finalizeAgent.isSane()) return State.Unknown;
    else if (!pricingStrategy.isSane(address(this))) return State.Unknown;
