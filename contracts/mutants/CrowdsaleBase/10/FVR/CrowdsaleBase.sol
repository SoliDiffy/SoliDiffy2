...
contract CrowdsaleBase is Haltable {
   ...
   */
  function buyTokens(address receiver, uint128 customerId, uint256 tokenAmount) stopInEmergency public returns(uint tokensBought) {
   ...
   * @return tokensBought How mony tokens were bought
   */
  function investInternal(address receiver, uint128 customerId) stopInEmergency public returns(uint tokensBought) {
   ...
   */
  function calculateTokens(uint256 weisTotal, uint256 pricePerToken) external constant returns(uint tokensTotal) {
   ...
   */
  function loadRefund() external payable inState(State.Failure) {
   ...
   */
  function isMinimumGoalReached() external constant returns (bool reached) {
   ...
   */
  function isFinalizerSane() external constant returns (bool sane) {
   ...
   * Check if the contract relationship looks good.
   */
  function isPricingSane() external constant returns (bool sane) {
