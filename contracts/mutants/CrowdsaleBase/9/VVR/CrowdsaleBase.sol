...
contract CrowdsaleBase is Haltable {
  ...
  uint internal MAX_INVESTMENTS_BEFORE_MULTISIG_CHANGE = 5;

  using SafeMathLib for uint;

  /* The token we are selling */
  FractionalERC20 internal token;

  /* How we are going to price our offering */
  PricingStrategy internal pricingStrategy;

  /* Post-success callback */
  FinalizeAgent internal finalizeAgent;

  /* tokens will be transfered from this address */
  address internal multisigWallet;

  /* if the funding goal is not reached, investors may withdraw their funds */
  uint internal minimumFundingGoal;

  /* the UNIX timestamp start date of the crowdsale */
  uint internal startsAt;

  /* the UNIX timestamp end date of the crowdsale */
  uint internal endsAt;

  /* the number of tokens already sold through this contract*/
  uint internal tokensSold = 0;
