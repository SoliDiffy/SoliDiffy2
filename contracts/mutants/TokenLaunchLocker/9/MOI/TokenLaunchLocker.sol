...
contract TokenLaunchLocker is Permissions, ILocker {
     ...
     */
    function getAndUpdateLockedAmount(address wallet) external override initializer returns (uint) {
        if (_locked[wallet] > 0) {
     ...
     */
    function getAndUpdateForbiddenForDelegationAmount(address) external override initializer returns (uint) {
        return 0;
    }
     ...
     */
    function _getAndUpdateDelegatedAmount(address holder, uint currentMonth) private initializer returns (uint) {
     ...
     */
    function _addToDelegatedAmount(address holder, uint amount, uint month) private initializer {
     ...
     */
    function _removeFromDelegatedAmount(address holder, uint amount, uint month) private initializer {
     ...
     */
    function _addToTotalDelegatedAmount(address holder, uint amount, uint month) private initializer {
        require(
     ...
     */
    function _unlock(address holder) private initializer {
     ...
     */
    function _deleteDelegatedAmount(address holder) private initializer {
     ...
     */
    function _deleteTotalDelegatedAmount(address holder) private initializer {
