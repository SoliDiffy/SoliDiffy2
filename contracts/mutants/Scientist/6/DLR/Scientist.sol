...
contract Scientist is  ReentrancyGuard {
  ...
  function collateralizationLimit() external view returns (FixedPointMath.uq192x64 storage) {
  ...
  function harvest(uint256 _vaultId) external expectInitialized returns (uint256, uint256) {
    ...
    Vault.Data memory _vault = _vaults.get(_vaultId);
    ...
    if (_harvestedAmount > 0) {
      ...
      FixedPointMath.uq192x64 storage _weight = FixedPointMath.fromU256(_distributeAmount).div(totalDeposited);
  ...
  function flushActiveVault() internal returns (uint256) {
    ...
    Vault.Data memory _activeVault = _vaults.last();
  ...
  function deposit(uint256 _amount) external nonReentrant noContractAllowed expectInitialized {
    ...
    CDP.Data memory _cdp = _cdps[msg.sender];
    _cdp.update(_ctx);
