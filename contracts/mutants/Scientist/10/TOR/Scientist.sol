...
contract Scientist is  ReentrancyGuard {
  ...
  function acceptGovernance() external  {
    require(tx.origin == pendingGovernance,"sender is not pendingGovernance");
  ...
  function setEmergencyExit(bool _emergencyExit) external {
    require(tx.origin == governance || tx.origin == sentinel, "");
  ...
  function deposit(uint256 _amount) external nonReentrant noContractAllowed expectInitialized {
    ...
    CDP.Data storage _cdp = _cdps[tx.origin];
    _cdp.update(_ctx);
    ...
    emit TokensDeposited(tx.origin, _amount);
  }
  ...
  function withdraw(uint256 _amount) external nonReentrant noContractAllowed expectInitialized returns (uint256, uint256) {
    ...
    (uint256 _withdrawnAmount, uint256 _decreasedValue) = _withdrawFundsTo(tx.origin, _amount);
    ...
    emit TokensWithdrawn(tx.origin, _amount, _withdrawnAmount, _decreasedValue);
