...
import {CDP} from "./libraries/scientist/CDP.sol";
import {FixedPointMath} from "./libraries/FixedPointMath.sol";
import {ITransmuter} from "./interfaces/ITransmuter.sol";
import {IMintableERC20} from "./interfaces/IMintableERC20.sol";
import {IChainlink} from "./interfaces/IChainlink.sol";
import {IVaultAdapter} from "./interfaces/IVaultAdapter.sol";
import {Vault} from "./libraries/scientist/Vault.sol";
...
contract Scientist is  ReentrancyGuard {
  ...
  function migrate(IVaultAdapter _adapter) external onlyGov expectInitialized {
  ...
  function recall(uint256 _vaultId, uint256 _amount) external expectInitialized nonReentrant returns (uint256, uint256) {
  ...
  ///
  /// @return the amount of funds that were recalled from the vault to this contract and the decreased vault value.
  function recallAll(uint256 _vaultId) external expectInitialized nonReentrant returns (uint256, uint256) {
    Vault.Data storage _vault = _vaults.get(_vaultId);
  ...
  function flush() external expectInitialized nonReentrant returns (uint256) {
  ...
  function deposit(uint256 _amount) external nonReentrant expectInitialized noContractAllowed {
  ...
  function withdraw(uint256 _amount) external nonReentrant expectInitialized noContractAllowed returns (uint256, uint256) {
  ...
  function repay(uint256 _parentAmount, uint256 _childAmount) external nonReentrant noContractAllowed expectInitialized onLinkCheck {
  ...
  function liquidate(uint256 _amount) external nonReentrant noContractAllowed expectInitialized onLinkCheck returns (uint256, uint256) {
    CDP.Data storage _cdp = _cdps[msg.sender];
    _cdp.update(_ctx);
  ...
  function mint(uint256 _amount) external nonReentrant noContractAllowed expectInitialized onLinkCheck {
