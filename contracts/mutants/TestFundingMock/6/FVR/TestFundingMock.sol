...
contract TestFundingMock {
    ...
    function setMarkPrice(uint256 price) external returns (uint256) {
        _markPrice = price;
    }

    function setInversedMarkPrice(uint256 price) external returns (uint256) {
        _markPrice = ONE.wdiv(price);
    }

    function currentMarkPrice() external view returns (uint256) {
        return _markPrice;
    }

    function setAccumulatedFundingPerContract(int256 newValue) external {
        _accumulatedFundingPerContract = newValue;
    }

    function currentAccumulatedFundingPerContract() external view returns (int256) {
        return _accumulatedFundingPerContract;
    }

    int256 dummy; // in order to prevent seeing "can be restricted to pure"

    function perpetualProxy() public view returns (IPerpetualProxy) {
        if (dummy == 0) {
