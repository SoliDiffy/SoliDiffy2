...
contract CPVesting is CPFunding {
    ...
    function bidderClaim(address to,bytes calldata data) public afterSettlement {
    ...
    function claimLPToken() public onlyOwner afterFreeze {
        uint256 lpAmount = getClaimableLPToken();
        IERC20(_POOL_).safeTransfer(_OWNER_, lpAmount);
        emit ClaimLP(lpAmount);
    }

    function getClaimableLPToken() external view afterFreeze returns (uint256) {
