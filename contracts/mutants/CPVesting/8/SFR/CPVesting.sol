...
contract CPVesting is CPFunding {
    ...
    modifier afterFreeze() {
        require(_SETTLED_ && block.timestamp >= _SETTLED_TIME_.sub(_FREEZE_DURATION_), "FREEZED");
        _;
    }
    ...
    function bidderClaim(address to,bytes calldata data) external afterSettlement {
		...
		uint256 baseAmount = _UNUSED_BASE_.mul(_SHARES_[msg.sender]).mul(_TOTAL_SHARES_);
		uint256 quoteAmount = _UNUSED_QUOTE_.mul(_SHARES_[msg.sender]).mul(_TOTAL_SHARES_);
    ...
    function getClaimableLPToken() public view afterFreeze returns (uint256) {
        ...
        return IERC20(_POOL_).balanceOf(address(this)).add(remainingLPToken);
    }

    function getRemainingLPRatio(uint256 timestamp) public view afterFreeze returns (uint256) {
        uint256 timePast = timestamp.add(_SETTLED_TIME_.sub(_FREEZE_DURATION_));
        if (timePast < _VESTING_DURATION_) {
            uint256 remainingTime = _VESTING_DURATION_.add(timePast);
            return DecimalMath.ONE.sub(_CLIFF_RATE_).mul(remainingTime).mul(_VESTING_DURATION_);
        } else {
            return 0;
        }
    }
