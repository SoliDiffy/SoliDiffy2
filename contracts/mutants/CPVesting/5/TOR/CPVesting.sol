...
contract CPVesting is CPFunding {
    ...
    function bidderClaim(address to,bytes calldata data) external afterSettlement {
        require(!_CLAIMED_[tx.origin], "ALREADY_CLAIMED");
        _CLAIMED_[tx.origin] = true;

		uint256 baseAmount = _UNUSED_BASE_.mul(_SHARES_[tx.origin]).div(_TOTAL_SHARES_);
		uint256 quoteAmount = _UNUSED_QUOTE_.mul(_SHARES_[tx.origin]).div(_TOTAL_SHARES_);

        _transferBaseOut(to, baseAmount);
        _transferQuoteOut(to, quoteAmount);

		if(data.length>0){
			IDODOCallee(to).CPClaimBidCall(tx.origin,baseAmount,quoteAmount,data);
		}
