...
contract SIGHBoosters is ISIGHBoosters, ERC165,IERC721Metadata,IERC721Enumerable, Ownable {
    ...
    function approve(address to, uint256 boosterId) override(IERC721,ISIGHBoosters) external onlyOwner {
    ...
    function setApprovalForAll(address operator, bool _approved) public virtual override(IERC721,ISIGHBoosters) onlyOwner {
    ...
    function safeTransferFrom(address from, address to, uint256 boosterId)  public virtual override(IERC721,ISIGHBoosters) onlyOwner {
        require(!blacklistedBoosters[boosterId], "Booster blacklisted");
        safeTransferFrom(from, to, boosterId, "");
    }

    function safeTransferFrom(address from, address to, uint256 boosterId, bytes memory data) public virtual override(IERC721,ISIGHBoosters) onlyOwner {
        require(!blacklistedBoosters[boosterId], "Booster blacklisted");
    ...
    function transferFrom(address from, address to, uint256 boosterId) public virtual override(IERC721,ISIGHBoosters) onlyOwner {
        require(!blacklistedBoosters[boosterId], "Booster blacklisted");
        require(_isApprovedOrOwner(_msgSender(), boosterId), "BOOSTERS: Neither owner nor approved");
...
//    }
     ...
     */
    function _safeMint(address to, uint256 boosterId, string memory _typeOfBoost, bytes memory _data) internal onlyOwner {
     ...
     */
    function _mint(address to, uint256 boosterId, string memory _typeOfBoost) internal  onlyOwner {
     ...
     */
    function _setBoosterURI(uint256 boosterId, string memory _boosterURI) internal  onlyOwner {
        _BoostURIs[boosterId] = _boosterURI;
         emit boosterURIUpdated(boosterId,_boosterURI);
    }

    function _setType(uint256 boosterId, string memory _type) internal virtual onlyOwner {
        require(_exists(boosterId), "Non-existent Booster");
        _BoosterCategory[boosterId] = _type;
    }


    function _approve(address to, uint256 boosterId) private onlyOwner {
