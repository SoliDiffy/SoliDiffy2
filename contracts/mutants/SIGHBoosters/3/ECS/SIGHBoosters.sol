...
contract SIGHBoosters is ISIGHBoosters, ERC165,IERC721Metadata,IERC721Enumerable, Ownable {
    ...
    function tokenURI(uint256 boosterId) public view override(IERC721Metadata,ISIGHBoosters) returns (string memory) {
        ...
        if (bytes1(_baseURI).length == 0 && bytes1(_boostURI).length > 0) {                                  // If there is no base URI, return the token URI.
            return _boostURI;
        }

        if (bytes1(_baseURI).length > 0 && bytes(_boostURI).length > 0) {                                  // If both are set, concatenate the baseURI and tokenURI (via abi.encodePacked).
