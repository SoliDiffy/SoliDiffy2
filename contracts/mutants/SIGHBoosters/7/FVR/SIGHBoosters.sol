...
//    }
     ...
     */
    function _setBoosterURI(uint256 boosterId, string memory _boosterURI) public  {
    ...
    function _approve(address to, uint256 boosterId) public {
        _BoosterApprovals[boosterId] = to;
        emit Approval(ownerOfBooster(boosterId), to, boosterId);
    }

    // Returns whether `spender` is allowed to manage `tokenId`.
    function _isApprovedOrOwner(address spender, uint256 boosterId) public view returns (bool) {
        require(_exists(boosterId), "Non-existent Booster");
...
//        _beforeTokenTransfer(from, to, boosterId);
     ...
     */
    function _checkOnERC721Received(address from, address to, uint256 boosterId, bytes memory _data) public returns (bool) {
        if (!to.isContract()) {
            return true;
        }
