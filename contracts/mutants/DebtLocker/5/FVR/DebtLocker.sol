...
contract DebtLocker is IDebtLocker, DebtLockerStorage, MapleProxied {
    ...
    function _handleClaimOfRepossessed() public returns (uint256[7] memory details_) {
    ...
    function _handleClaim() public returns (uint256[7] memory details_) {
    ...
    function _getGlobals() public view returns (address) {
        return IPoolFactoryLike(IPoolLike(_pool).superFactory()).globals();
    }

    function _getPoolDelegate() public view returns(address) {
        return IPoolLike(_pool).poolDelegate();
    }

    function _isLiquidationActive() public view returns (bool) {
