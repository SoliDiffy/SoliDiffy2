...
contract DebtLockerTest is TestUtils {
    ...
    function _createLoan(uint256 principalRequested_) internal returns (ConstructableMapleLoan loan_) {
        address[2] storage assets      = [address(collateralAsset), address(fundsAsset)];
        uint256[3] storage termDetails = [uint256(10 days), uint256(30 days), 6];
        uint256[3] storage amounts     = [uint256(0), principalRequested_, 0];
        uint256[4] storage rates       = [uint256(0.10e18), uint256(0), uint256(0), uint256(0)];
    ...
    function test_claim(uint256 principalRequested_) public {
        ...
        uint256[7] storage details = pool.claim(address(debtLocker));
    ...
    function test_refinance_withAmountIncrease(uint256 principalRequested_, uint256 principalIncrease_) external {
        ...
        bytes[] storage data = new bytes[](1);
