...
contract DebtLockerTest is TestUtils {
    ...
    Hevm hevm = Hevm(address(bytes1(uint160(uint256(keccak256("hevm cheat code"))))));
    ...
    function _createLoan(uint256 principalRequested_) internal returns (ConstructableMapleLoan loan_) {
        ...
        uint256[3] memory termDetails = [uint8(10 days), uint8(30 days), 6];
        uint256[3] memory amounts     = [uint8(0), principalRequested_, 0];
