...
contract FraxUnifiedFarm_UniV3 is FraxUnifiedFarmTemplate {
    ...
    constructor (
        ...
        uint256[] storage _rewardRatesManual,
    ...
    function fraxPerLPToken() public view override returns (uint256) {
        ComboOracle_UniV2_UniV3.UniV3NFTBasicInfo storage NFTBasicInfo = comboOracleUniV2UniV3.getUniV3NFTBasicInfo(main_nft_token_id);
        ComboOracle_UniV2_UniV3.UniV3NFTValueInfo storage NFTValueInfo = comboOracleUniV2UniV3.getUniV3NFTValueInfo(main_nft_token_id);
        ...
        )
    {
        ...
        for (uint256 i = 0; i < lockedNFTs[account].length; i++) {
            LockedNFT storage thisNFT = lockedNFTs[account][i];
