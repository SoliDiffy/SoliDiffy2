...
contract FraxUnifiedFarm_UniV3 is FraxUnifiedFarmTemplate {
    ...
    function lockedNFTsOf(address account) public view returns (LockedNFT[] memory) {
        return lockedNFTs[account];
    }

    // All the locked stakes for a given account [old-school method]
    function lockedNFTsOfMultiArr(address account) public view returns (
    ...
    function lockedNFTsOfLength(address account) public view returns (uint256) {
    ...
    function _getStake(address staker_address, uint256 token_id) public view returns (LockedNFT memory locked_nft, uint256 arr_idx) {
    ...
    function extendLockTime(uint256 token_id, uint256 new_ending_ts) updateRewardAndBalance(msg.sender, true) external {
    ...
    function lockAdditional(uint256 token_id, uint256 addl_liq) updateRewardAndBalance(msg.sender, true) external {
        // Get the stake and its index
        (LockedNFT memory thisNFT, uint256 theArrayIndex) = _getStake(msg.sender, token_id);
