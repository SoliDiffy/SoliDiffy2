...
abstract contract Swaps is ReentrancyGuard, PoolRegistry {
    ...
    function _swapWithPool(
        ...
        LastSwapData memory previous,
        SwapKind kind
    ) private nonReentrant returns (uint256 amountIn, uint256 amountOut) {
     ...
     */
    function _processSwapRequest(InternalSwapRequest memory request, SwapKind kind) private nonReentrant returns (uint256) {
    ...
    function _processTwoTokenPoolSwapRequest(
        ...
        SwapKind kind
    ) private nonReentrant returns (uint256 amountCalculated) {
