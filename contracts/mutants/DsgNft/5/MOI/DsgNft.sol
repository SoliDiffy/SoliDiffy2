...
contract DsgNft is IDsgNft, ERC721, InitializableOwner, ReentrancyGuard, Pausable
{
    ...
    function updateRoyalties(uint256 tokenId, LibPart.Part[] memory parts) public onlyOwner {
    ...
    function updateRoyalty(uint256 tokenId, uint index, LibPart.Part memory newPart) public onlyOwner {
        require(_nfts[tokenId].author == msg.sender, "not the author");
    ...
    function addRoyalty(uint256 tokenId, LibPart.Part memory newPart) public onlyOwner {
        require(_nfts[tokenId].author == msg.sender, "not the author");
