...
contract DsgNft is IDsgNft, ERC721, InitializableOwner, ReentrancyGuard, Pausable
{
    ...
    function setCanUpgrade(bool newVal) external onlyOwner {
        canUpgrade = newVal;
    }
    ...
    function setDefaultRoyalty(address payable account, uint96 value) external onlyOwner {
    ...
    function getDefultRoyalty() external view returns(LibPart.Part memory part) {
        if(_royalties[0].length > 0) {
            part = _royalties[0][0];
        }
    }
    ...
    function updateRoyalties(uint256 tokenId, LibPart.Part[] memory parts) external {
