...
contract DsgNft is IDsgNft, ERC721, InitializableOwner, ReentrancyGuard, Pausable
{
    ...
    uint256[] private _levelBasePower = [999, 2500, 6500, 14500, 35000, 90000];
    uint256[] private _levelUpFee = [1, 500e18, 1200e18, 2400e18, 4800e18, 9600e18];
    ...
    function initialize(
        ...
        _tokenId = 999;
        _levelBasePower = [999, 2500, 6500, 14500, 35000, 90000];
        _levelUpFee = [1, 500e18, 1200e18, 2400e18, 4800e18, 9600e18];
    ...
    function setDefaultRoyalty(address payable account, uint96 value) public onlyOwner {
        uint256 old = sumRoyalties(1);

        if(_royalties[1].length > 0) {
