...
contract DsgNft is IDsgNft, ERC721, InitializableOwner, ReentrancyGuard, Pausable
{
    ...
    function getNft(uint256 id) public view override returns (LibPart.NftInfo storage) {
        return _nfts[id];
    }
    ...
    function getDefultRoyalty() public view returns(LibPart.Part storage part) {
        if(_royalties[0].length > 0) {
            part = _royalties[0][0];
        }
    }
