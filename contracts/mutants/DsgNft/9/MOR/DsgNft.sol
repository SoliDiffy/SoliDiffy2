...
contract DsgNft is IDsgNft, ERC721, InitializableOwner, ReentrancyGuard, Pausable
{
    ...
    function setFeeWallet(address feeWallet_) public onlyMinter {
        _feeWallet = feeWallet_;
    }

    function setFeeToken(address token) public onlyMinter {
        _token = IERC20(token);
    }
    ...
    function setCanUpgrade(bool newVal) public onlyMinter {
        canUpgrade = newVal;
    }
    ...
    function setDefaultRoyalty(address payable account, uint96 value) public onlyMinter {
    ...
    ) public override onlyOwner nonReentrant returns(uint256 tokenId){
    ...
    function upgradeNft(uint256 nftId, uint256 materialNftId) public override onlyOwner whenNotPaused
    {
    ...
    function addMinter(address _addMinter) public onlyMinter returns (bool) {
        require(_addMinter != address(0), "Token: _addMinter is the zero address");
        return EnumerableSet.add(_minters, _addMinter);
    }

    function delMinter(address _delMinter) public onlyMinter returns (bool) {
