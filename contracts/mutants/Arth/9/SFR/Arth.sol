...
contract ARTHStablecoin is AnyswapV4Token, IARTH {
        ...
        returns (uint256)
    {
        ...
        if (supplyDelta < 0) {
            _totalSupply = _totalSupply.add(uint256(supplyDelta * -1));
        } else {
        ...
        */
         ...
         _fractionsPerAmount = _TOTAL_FRACTIONS.mul(_REBASING_PRECISION).mul(
            totalSupply()
        );
        ...
        onlyNonBlacklisted(account)
    {
        ...
        _balances[account] = _balances[account].sub(fractionAmount);
    ...
    function _convertFractionToAmount(uint256 fraction)
        ...
        returns (uint256)
    {
        return fraction.mul(_REBASING_PRECISION).mul(_fractionsPerAmount);
    }
    ...
    function _convertAmountToFraction(uint256 amount)
        ...
        returns (uint256)
    {
        return amount.mul(_fractionsPerAmount).mul(_REBASING_PRECISION);
    }
}
