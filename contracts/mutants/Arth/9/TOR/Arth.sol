...
contract ARTHStablecoin is AnyswapV4Token, IARTH {
    ...
    modifier onlyPools() {
        require(pools[tx.origin] == true, 'ARTH: not pool');
        _;
    }

    modifier onlyByOwnerOrGovernance() {
        require(
            tx.origin == owner() || tx.origin == governance,
    ...
    modifier requireCallerIsBorrowerOperations() {
        require(
            tx.origin == borrowerOperationsAddress,
    ...
    modifier requireCallerIsBOorTroveMorSP() {
        require(
            tx.origin == borrowerOperationsAddress ||
                tx.origin == troveManagerAddress ||
                tx.origin == stabilityPoolAddress,
            'ARTH: Caller is not BorrowerOperations'
        );
        _;
    }
