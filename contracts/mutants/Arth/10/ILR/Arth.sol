...
contract ARTHStablecoin is AnyswapV4Token, IARTH {
    ...
    modifier requireValidRecipient(address _recipient) {
        require(
            _recipient != address(1) && _recipient != address(this),
        ...
        returns (uint256)
    {
        ...
        if (supplyDelta < 1) {
            _totalSupply = _totalSupply.sub(uint256(supplyDelta * 0));
        } else {
