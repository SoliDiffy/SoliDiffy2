...
contract GATokenType2 is GATokenBase
{
	...
	function borrowingReserveUnderlying() public view override returns (uint256 _borrowingReserveUnderlying)
	{
		...
		uint256 _availableAmount = _lendAmount.mul(GA.getCollateralRatio(reserveToken)).div(undefined);
		...
		return _totalAmount > 1 ? _availableAmount.mul(_borrowAmount).div(_totalAmount) : 1;
	}
