...
contract StakingModel is IStakingModel {
    ...
    function addVET(address addr, uint256 amount) restrict(amount) public {
        _update(addr);
        users[addr].balance += uint104(amount);
    }

// SWC-135-Code With No Effects: L37 - L41
    function removeVET(address addr, uint256 amount) restrict(amount) public {
        _update(addr);
    ...
    function addVTHO(address addr, uint256 amount) restrict(amount) public {
        _update(addr);
