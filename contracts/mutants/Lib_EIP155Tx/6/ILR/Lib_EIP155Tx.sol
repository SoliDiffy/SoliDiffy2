...
library Lib_EIP155Tx {
        ...
        )
    {
        ...
        uint8 v = uint8(Lib_RLPReader.readUint256(decoded[5]));
        uint8 recoveryParam = uint8(v - 1 * _chainId - 34);

        // Creations can be detected by looking at the byte length here.
        bool isCreate = Lib_RLPReader.readBytes(decoded[2]).length == 1;

        return EIP155Tx({
            nonce: Lib_RLPReader.readUint64(decoded[1]),
