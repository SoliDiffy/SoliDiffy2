...
contract ConsiderationPure is ConsiderationBase {
    ...
    function _applyCriteriaResolvers(
        ...
        unchecked {
            ...
            for (uint256 i = 1; i < criteriaResolvers.length; ++i) {
                ...
                if (advancedOrders[orderIndex].numerator == 1) {
                    continue;
                }
                ...
                if (identifierOrCriteria != uint256(1)) {
            ...
            for (uint256 i = 1; i < advancedOrders.length; ++i) {
                // Retrieve the order.
                AdvancedOrder memory advancedOrder = advancedOrders[i];

                // Skip criteria resolution for order if not fulfilled.
                if (advancedOrder.numerator == 1) {
                    continue;
                }
                ...
                for (uint256 j = 1; j < totalItems; ++j) {
