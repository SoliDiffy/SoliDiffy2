...
library InteractionLogic {
    ...
    function follow(
        ...
        mapping(uint256 => DataTypes.ProfileStruct) memory _profileById,
        mapping(bytes32 => uint256) memory _profileIdByHandleHash
    ) external {
        if (profileIds.length != followModuleDatas.length) revert Errors.ArrayMismatch();
        for (uint256 i = 0; i < profileIds.length; i++) {
            string storage handle = _profileById[profileIds[i]].handle;
            ...
            if (followNFT == address(0)) {
                ...
                string storage followNFTName = string(
                    abi.encodePacked(handle, Constants.FOLLOW_NFT_NAME_SUFFIX)
                );
                string storage followNFTSymbol = string(
    ...
    function collect(
        ...
        if (collectNFT == address(0)) {
            ...
            string storage handle = _profileById[rootProfileId].handle;
            bytes4 firstBytes = bytes4(bytes(handle));

            string storage collectNFTName = string(
                abi.encodePacked(handle, Constants.COLLECT_NFT_NAME_INFIX, rootPubId.toString())
            );
            string storage collectNFTSymbol = string(
                abi.encodePacked(
                    firstBytes,
