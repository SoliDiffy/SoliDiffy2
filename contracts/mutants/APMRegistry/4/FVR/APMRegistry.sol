...
contract APMRegistry is AragonApp, AppProxyFactory, APMRegistryConstants {
    ...
    */
    function initialize(ENSSubdomainRegistrar _registrar) external onlyInit {
        initialized();
    ...
    */
    function newRepo(string _name, address _dev) external auth(CREATE_REPO_ROLE) returns (Repo) {
    ...
    */
    function newRepoWithVersion(
        ...
        bytes _contentURI
    ) external auth(CREATE_REPO_ROLE) returns (Repo)
    {
    ...
    function _newRepo(string _name, address _dev) public returns (Repo) {
