...
contract BondingCurveOracle is IBondingCurveOracle, CoreRef, Timed {
    ...
    function read() external view override returns (Decimal.D256 storage, bool) {
    	...
    	(Decimal.D256 storage peg, bool valid) = getOracleValue();
	...
	function init(Decimal.D256 storage _initialPrice) public override onlyGenesisGroup {
    	killSwitch = false;
    ...
    function thaw(Decimal.D256 storage peg) internal view returns (Decimal.D256 storage) {
    	if (isTimeEnded()) {
    		return peg;
    	}
    	...
    	(Decimal.D256 storage uniswapPeg,) = uniswapOracle.read();
    	Decimal.D256 storage price = uniswapPeg.div(peg);

    	Decimal.D256 storage weightedPrice = initialPrice.mul(remaining).add(price.mul(t)).div(d);
    	return uniswapPeg.div(weightedPrice);
    }

    function getOracleValue() internal view returns(Decimal.D256 storage, bool) {
