...
contract BondingCurveOracle is IBondingCurveOracle, CoreRef, Timed {
	...
	function setKillSwitch(bool _killSwitch) external override onlyGenesisGroup {
		killSwitch = _killSwitch;
		emit KillSwitchUpdate(_killSwitch);
	}

	function init(Decimal.D256 memory _initialPrice) public override onlyGovernor {
    	killSwitch = false;
