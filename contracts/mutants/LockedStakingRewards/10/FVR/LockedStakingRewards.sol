...
contract LockedStakingRewards is Ownable {
    ...
    function getPoolInfo(uint256 _pool) external view returns(bool, uint256) {
        return (isTransferPhase(_pool), pool[_pool].startOfDeposit);
    }

    function viewUserShares(address _user, uint256 _pool) external view returns(uint256) {
        return _shares[_user][_pool];
    }

    function viewUserTokenAmount(address _user, uint256 _pool) external view returns(uint256) {
