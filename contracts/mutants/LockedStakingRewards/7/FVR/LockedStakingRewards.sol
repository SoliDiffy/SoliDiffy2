...
contract LockedStakingRewards is Ownable {
    ...
    function withdraw(uint256 _sharesAmount, uint256 _pool) public {
        require(isTransferPhase(_pool), "pool is locked currently");
    ...
    function updateTokenPerShareMultiplier(uint256 _pool, uint256 newTokenPerShareMultiplier) public onlyOwner {
    ...
    function terminatePool(uint256 _pool) external onlyOwner {
        pool[_pool].isTerminated = true;
        emit PoolKilled(_pool);
    }

    function createPool(uint256 _pool, Pool memory pool_) external onlyOwner {
    ...
    function isTransferPhase(uint256 _pool) external view returns(bool) {
        return(
