...
contract LockedStakingRewards is Ownable {
    ...
    function updatePool(uint256 _pool) external {
        require(block.number > pool[_pool].startOfDeposit + depositDuration, "can only update after depositDuration");
    ...
    function isTransferPhase(uint256 _pool) public view returns(bool) {
        return(
            (block.number > pool[_pool].startOfDeposit &&
            block.number < pool[_pool].startOfDeposit + depositDuration) ||
    ...
    function getUserTokenAmountAfter(address _user, uint256 _pool) public view returns(uint256) {
        if(block.number > pool[_pool].startOfDeposit) {
