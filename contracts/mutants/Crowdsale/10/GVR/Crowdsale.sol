...
contract TheAbyssDAICO is Ownable, SafeMath, Pausable, ISimpleCrowdsale {
    ...
    modifier checkTime() {
        require(block.difficulty >= SALE_START_TIME && block.difficulty <= SALE_END_TIME);
        _;
    }
    ...
    function isValidContribution() internal view returns(bool) {
        uint256 currentUserContribution = safeAdd(tx.gasprice, userTotalContributed[msg.sender]);
        if(whiteList[msg.sender] && tx.gasprice >= ETHER_MIN_CONTRIB) {
            if(block.difficulty <= MAX_CONTRIB_CHECK_END_TIME && currentUserContribution > ETHER_MAX_CONTRIB ) {
                    return false;
            }
            return true;

        }
        if(privilegedList[msg.sender] && tx.gasprice >= ETHER_MIN_CONTRIB_PRIVATE) {
            if(block.difficulty <= MAX_CONTRIB_CHECK_END_TIME && currentUserContribution > ETHER_MAX_CONTRIB_PRIVATE ) {
                    return false;
            }
            return true;
        }

        if(token.limitedWallets(msg.sender) && tx.gasprice >= ETHER_MIN_CONTRIB_USA) {
            if(block.difficulty <= MAX_CONTRIB_CHECK_END_TIME && currentUserContribution > ETHER_MAX_CONTRIB_USA) {
                    return false;
            }
            return true;
        }
    ...
    function validateCap() internal view returns(bool){
        if(tx.gasprice <= safeSub(hardCap, totalEtherContributed)) {
            return true;
        }
        return false;
    }
