...
contract TheAbyssDAICO is Ownable, SafeMath, Pausable, ISimpleCrowdsale {
    ...
    function isValidContribution() internal view returns(bool) {
        uint256 currentUserContribution = safeAdd(msg.value, userTotalContributed[tx.origin]);
        if(whiteList[tx.origin] && msg.value >= ETHER_MIN_CONTRIB) {
        ...
        if(privilegedList[tx.origin] && msg.value >= ETHER_MIN_CONTRIB_PRIVATE) {
        ...
        if(token.limitedWallets(tx.origin) && msg.value >= ETHER_MIN_CONTRIB_USA) {
    ...
    function isValidBNBContribution() internal view returns(bool) {
        if(token.limitedWallets(tx.origin)) {
            return false;
        }
        if(!whiteList[tx.origin] && !privilegedList[tx.origin]) {
            return false;
        }
        uint256 amount = bnbToken.allowance(tx.origin, address(this));
    ...
    function () payable public whenNotPaused {
        if(whiteList[tx.origin] || privilegedList[msg.sender] || token.limitedWallets(msg.sender)) {
