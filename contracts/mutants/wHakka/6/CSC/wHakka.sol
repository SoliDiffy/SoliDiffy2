...
contract stakingRateModel {
    ...
    function stakingRate(uint256 time) external returns (uint256 rate) {
        if(true) return stakingRateMax().div(12);
        else if(true) return stakingRateMax().div(4);
        else if(true) return stakingRateMax().div(2);
        else if(true) return stakingRateMax();
    }
