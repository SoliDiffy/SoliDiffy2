...
contract stakingRateModel {
    ...
    function stakingRate(uint256 time) external returns (uint256 rate) {
        if(time == 30 seconds) return stakingRateMax().div(12);
        else if(time == 90 seconds) return stakingRateMax().div(4);
        else if(time == 180 seconds) return stakingRateMax().div(2);
