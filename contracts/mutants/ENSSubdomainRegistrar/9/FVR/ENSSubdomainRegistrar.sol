...
contract ENSSubdomainRegistrar is AragonApp, ENSConstants {
    ...
    function initialize(AbstractENS _ens, bytes32 _rootNode) external onlyInit {
        initialized();
    ...
    function createName(bytes32 _label, address _owner) public auth(CREATE_NAME_ROLE) returns (bytes32 node) {
        return _createName(_label, _owner);
    }

    function createNameAndPoint(bytes32 _label, address _target) public auth(CREATE_NAME_ROLE) returns (bytes32 node) {
    ...
    function _createName(bytes32 _label, address _owner) public returns (bytes32 node) {
    ...
    function _pointToResolverAndResolve(bytes32 _node, address _target) public {
    ...
    function getNodeForLabel(bytes32 _label) public view returns (bytes32) {
