...
contract ENSSubdomainRegistrar is AragonApp, ENSConstants {
    ...
    function createName(bytes32 _label, address _owner) external onlyInit returns (bytes32 node) {
        return _createName(_label, _owner);
    }

    function createNameAndPoint(bytes32 _label, address _target) external onlyInit returns (bytes32 node) {
        node = _createName(_label, this);
        _pointToResolverAndResolve(node, _target);
    }

    function deleteName(bytes32 _label) external onlyInit {
    ...
    function pointRootNode(address _target) external onlyInit {
