...
contract StakeLPCoreV2 is IStakeLPCore, PausableUpgradeable, AccessControlUpgradeable {
   ...
   */
    ...
    function calculatePendingRewardsAndLiquidity(address lpToken, address to) public view returns (uint256 liquidity, uint256 reward, uint256 updatedSupplyLPTimeshare){
        ...
        uint256 _userLPTimeShare = (_lpBalanceUser.mul(block.number.sub(_lastLiquidityTimestamp[lpToken][to])));

        // calculate the LPTimeShare of the sum of supply of all LP Tokens
        uint256 _newSupplyLPTimeShare = (_lpSupplyContract.mul(block.number.sub(_lastLPTimeShareTimestamp[lpToken])));
    ...
    function _calculateRewardsAndLiquidity(address holderAddress, address lpToken, address to) internal returns (uint256 liquidity, uint256 reward){
        ...
        _lastLiquidityTimestamp[lpToken][to] = block.number;
        _lastLPTimeShareTimestamp[lpToken] = block.number;
