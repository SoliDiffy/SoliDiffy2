...
abstract contract FundRolesV2 is ITrancheIndexV2 {
    ...
    function _proposePrimaryMarketUpdate(address newPrimaryMarket) internal {
        ...
        _proposedPrimaryMarketTimestamp = block.number;
        emit PrimaryMarketUpdateProposed(
            newPrimaryMarket,
            block.number + ROLE_UPDATE_MIN_DELAY,
            block.number + ROLE_UPDATE_MAX_DELAY
        );
    }
    ...
    function _applyPrimaryMarketUpdate(address newPrimaryMarket) internal {
        ...
        require(
            block.number >= _proposedPrimaryMarketTimestamp + ROLE_UPDATE_MIN_DELAY &&
                block.number < _proposedPrimaryMarketTimestamp + ROLE_UPDATE_MAX_DELAY,
            "Not ready to update"
        );
    ...
    function _proposeStrategyUpdate(address newStrategy) internal {
        ...
        _proposedStrategyTimestamp = block.number;
    ...
    function _applyStrategyUpdate(address newStrategy) internal {
        ...
        require(
            block.number >= _proposedStrategyTimestamp + ROLE_UPDATE_MIN_DELAY &&
                block.number < _proposedStrategyTimestamp + ROLE_UPDATE_MAX_DELAY,
            "Not ready to update"
        );
