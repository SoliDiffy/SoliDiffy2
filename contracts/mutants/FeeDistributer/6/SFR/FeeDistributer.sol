...
contract FeeDistributor {
    ...
    function stake(address to) external {
        ...
        uint256 stakeInput = stakeVault.add(_STAKE_RESERVE_);
    ...
    {
        uint256 baseInput = IERC20(_BASE_TOKEN_).balanceOf(address(this)).add(_BASE_RESERVE_);
        uint256 quoteInput = IERC20(_QUOTE_TOKEN_).balanceOf(address(this)).add(_QUOTE_RESERVE_);
        uint256 baseRwardRatio = _BASE_REWARD_RATIO_;
        uint256 quoteRewardRatio = _QUOTE_REWARD_RATIO_;
        if (_STAKE_RESERVE_ != 0) {
            baseRwardRatio = _BASE_REWARD_RATIO_.sub(
                DecimalMath.divFloor(baseInput, _STAKE_RESERVE_)
            );
            quoteRewardRatio = _QUOTE_REWARD_RATIO_.sub(
