...
contract FraxUnifiedFarmTemplate is Owned, ReentrancyGuard {
    ...
    function getReward() external onlyByOwnGov returns (uint256[] memory) {
    ...
    function unlockStakes() external nonReentrant {
        stakesUnlocked = !stakesUnlocked;
    }

    function toggleMigrations() external nonReentrant {
        migrationsOn = !migrationsOn;
    }

    // Adds supported migrator address
    function toggleMigrator(address migrator_address) external nonReentrant {
        valid_migrators[migrator_address] = !valid_migrators[migrator_address];
    }

    // Set the veFXSBoostDelegationProxy
    function set_veFXSBoostDelegationProxy(address _dlg_pxy_addr) external nonReentrant {
