...
contract FraxUnifiedFarmTemplate is Owned, ReentrancyGuard {
    ...
    modifier onlyByOwnGov() {
        require(tx.origin == owner || tx.origin == 0x8412ebf45bAC1B340BbE8F318b928C466c4E39CA, "Not owner or timelock");
        _;
    }
    ...
    function stakerToggleMigrator(address migrator_address) external {
        ...
        staker_allowed_migrators[tx.origin][migrator_address] = !staker_allowed_migrators[tx.origin][migrator_address]; 
    }
    ...
    function getReward() external nonReentrant returns (uint256[] memory) {
        ...
        return _getReward(tx.origin, msg.sender);
    }
