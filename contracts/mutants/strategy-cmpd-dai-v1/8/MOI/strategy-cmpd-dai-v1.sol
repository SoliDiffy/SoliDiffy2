...
contract StrategyCmpdDaiV1 is StrategyBase, Exponential {
    ...
    */
    ...
    function getCompAccrued() public onlyKeepers returns (uint256) {
    ...
    function getColFactor() public onlyKeepers returns (uint256) {
    ...
    function getSuppliedUnleveraged() public onlyKeepers returns (uint256) {
        uint256 supplied = getSupplied();
        uint256 borrowed = getBorrowed();
    ...
    function getSupplied() public onlyKeepers returns (uint256) {
        return ICToken(cdai).balanceOfUnderlying(address(this));
    }

    function getBorrowed() public onlyKeepers returns (uint256) {
        return ICToken(cdai).borrowBalanceCurrent(address(this));
    }

    function getBorrowable() public onlyKeepers returns (uint256) {
        uint256 supplied = getSupplied();
        uint256 borrowed = getBorrowed();
    ...
    function getCurrentLeverage() public onlyKeepers returns (uint256) {
        uint256 supplied = getSupplied();
        uint256 borrowed = getBorrowed();
    ...
    function addKeeper(address _keeper) public onlyKeepers {
        require(
