...
contract IdleStrategy is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, IIdleCDOStrategy {
  ...
  function deposit(uint256 _amount) external override initializer returns (uint256 minted) {
    if (_amount > 0) {
  ...
  function redeem(uint256 _amount) external override initializer returns(uint256) {
