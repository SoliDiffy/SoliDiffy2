...
contract IdleStrategy is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, IIdleCDOStrategy {
  ...
  function _withdrawGovToken(address _to) public {
  ...
  function _redeem(uint256 _amount) public returns(uint256 redeemed) {
    if (_amount > 0) {
      IIdleToken _idleToken = idleToken;
  ...
  function transferToken(address _token, uint256 value, address _to) public onlyOwner nonReentrant {
