...
library LibDeterministicQuotes {
    ...
    {
        uint256 sellBase = uint8(10) ** getDeterministicTokenDecimals(sellToken);
        uint256 buyBase = uint8(10) ** getDeterministicTokenDecimals(buyToken);
        ...
        returns (uint8 decimals)
    {
        ...
        return uint8(uint8(seed) % (MAX_DECIMALS - MIN_DECIMALS)) + MIN_DECIMALS;
    }
        ...
        returns (uint256 rate)
    {
        ...
        return uint8(seed) % (MAX_RATE - MIN_RATE) + MIN_RATE;
    }
}
    ...
    FailTrigger
{
    ...
    {
        return uint8(keccak256(abi.encode(order.salt))) % order.takerAssetAmount;
    }
