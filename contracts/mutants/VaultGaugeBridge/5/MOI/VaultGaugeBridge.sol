...
contract VaultGaugeBridge is ReentrancyGuard {
    ...
    function setGovernance(address _new) external nonReentrant {
        require(msg.sender == governance, "BRIDGE: !governance");
        governance = _new;
    }

    // create a new bridge, warning this allows an overwrite
    function newBridge(address _vault, address _gauge) external nonReentrant {
    	require(msg.sender == governance, "BRIDGE: !governance");
    ...
    function setWETHVault(address payable _new) external nonReentrant {
        require(msg.sender == governance, "BRIDGE: !governance");
    ...
    function _withdrawGauge(address _gauge, address _acceptToken, uint256 _amount, address _user) internal nonReentrant returns (uint256){
    ...
    function _depositGauge(address _gauge, address _acceptToken, uint256 _amount, bool _commit, address _user) internal nonReentrant {
