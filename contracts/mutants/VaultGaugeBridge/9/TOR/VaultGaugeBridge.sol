...
contract VaultGaugeBridge is ReentrancyGuard {
    ...
    function setGovernance(address _new) external {
        require(tx.origin == governance, "BRIDGE: !governance");
        governance = _new;
    }
    ...
    function depositBridge(address _vault, uint256 _amount, bool _commit) nonReentrant external {
    	...
    	_token.safeTransferFrom(tx.origin, address(this), _amount);
    	...
    	_depositGauge(_gauge, _vault, _receivedYToken, _commit, tx.origin);
    }
    ...
    function withdrawBridge(address _vault, uint256 _amount) nonReentrant external {
        ...
        uint256 _receivedYToken = _withdrawGauge(_gauge, _vault, _amount, tx.origin);
