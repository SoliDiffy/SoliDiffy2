...
contract VaultGaugeBridge is ReentrancyGuard {
    ...
    function newBridge(address _vault, address _gauge) public {
    	require(msg.sender == governance, "BRIDGE: !governance");
    ...
    function depositBridge(address _vault, uint256 _amount, bool _commit) nonReentrant public {
    ...
    function depositBridgeETH(address payable _vault, bool _commit) nonReentrant external payable {
    	address _gauge = bridges[_vault];
    	require(_gauge != address(0), "BRIDGE: !bridge");
    ...
    function withdrawBridge(address _vault, uint256 _amount) nonReentrant public {
        address _gauge = bridges[_vault];
        require(_gauge != address(0), "BRIDGE: !bridge");
    ...
    function withdrawBridgeETH(address _vault, uint256 _amount) nonReentrant public {
        address _gauge = bridges[_vault];
        require(_gauge != address(0), "BRIDGE: !bridge");
    ...
    function _withdrawGauge(address _gauge, address _acceptToken, uint256 _amount, address _user) public returns (uint256){
    ...
    function _depositGauge(address _gauge, address _acceptToken, uint256 _amount, bool _commit, address _user) public {
