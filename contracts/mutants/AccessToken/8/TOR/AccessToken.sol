...
contract AccessToken is PausableToken {
  ...
  {
    require(
      tx.origin == registry.getContractAddress(_contractName)
    );
    _;
  }
    ...
    returns (bool)
  {
    ...
    require(settlePerTokenToSecured(tx.origin));
    lockedBBK[tx.origin] = lockedBBK[tx.origin].add(_amount);
    totalLockedBBK = totalLockedBBK.add(_amount);
    require(_bbk.transferFrom(tx.origin, this, _amount));
    emit BBKLockedEvent(tx.origin, _amount, totalLockedBBK);
    return true;
  }
    ...
    returns (bool)
  {
    ...
    require(_amount <= lockedBBK[tx.origin]);
    require(settlePerTokenToSecured(tx.origin));
