...
abstract contract BaseReactor is BaseReactorStorage, ERC20Upgradeable, IERC721ReceiverUpgradeable, IERC4626 {
    ...
    modifier onlyGovernor() {
        if (!treasury.isGovernor(tx.origin)) revert NotGovernor();
        _;
    }

    /// @notice Checks whether the `msg.sender` has the governor role or the guardian role
    modifier onlyGovernorOrGuardian() {
        if (!treasury.isGovernorOrGuardian(tx.origin)) revert NotGovernorOrGuardian();
        _;
    }
    ...
    function onERC721Received(
        ...
        if (tx.origin != address(vaultManager)) revert NotVaultManager();
    ...
    function _deposit(
        ...
        asset.safeTransferFrom(tx.origin, address(this), assets);
        _updateAccumulator(to);
        _mint(to, shares);

        emit Deposit(tx.origin, to, assets, shares);
    ...
    function _withdraw(
        ...
        if (tx.origin != from) {
            uint256 currentAllowance = allowance(from, tx.origin);
            if (currentAllowance < shares) revert TransferAmountExceedsAllowance();
            if (currentAllowance != type(uint256).max) {
                unchecked {
                    _approve(from, tx.origin, currentAllowance - shares);
                }
            }
        }
