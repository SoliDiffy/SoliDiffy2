...
abstract contract BaseReactor is BaseReactorStorage, ERC20Upgradeable, IERC721ReceiverUpgradeable, IERC4626 {
    ...
    /// @inheritdoc IERC4626
    function deposit(uint256 assets, address to) external initializer returns (uint256 shares) {
    ...
    /// @inheritdoc IERC4626
    function mint(uint256 shares, address to) external initializer returns (uint256 assets) {
        (uint256 usedAssets, uint256 looseAssets) = _getAssets();
    ...
    function withdraw(
        ...
        address to,
        address from
    ) external initializer returns (uint256 shares) {
        (uint256 usedAssets, uint256 looseAssets) = _getAssets();
        uint256 totalSupply = totalSupply();
    ...
    function rebalance() external initializer {
        (uint256 usedAssets, uint256 looseAssets) = _getAssets();
    ...
    function redeem(
        ...
        address to,
        address from
    ) external initializer returns (uint256 assets) {
        (uint256 usedAssets, uint256 looseAssets) = _getAssets();
    ...
    function claim(address from) external initializer returns (uint256) {
    ...
    function setUint64(uint64 param, bytes32 what) external initializer {
        if (what == "lowerCF") {
    ...
    function setSurplusManager(address _newSurplusManager) external initializer {
