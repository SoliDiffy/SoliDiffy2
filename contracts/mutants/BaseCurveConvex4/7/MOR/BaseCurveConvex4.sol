...
contract BaseCurveConvex4 is Context, Ownable {
    ...
    function setZunami(address zunamiAddr) external onlyZunami {
    ...
    function deposit(uint256[3] memory amounts) external virtual onlyOwner returns (uint256) {
    ...
    function claimManagementFees() external virtual onlyOwner {
    ...
    function withdrawAll() external virtual onlyOwner {
    ...
    function updateMinDepositAmount(uint256 _minDepositAmount) external onlyZunami {
        require(_minDepositAmount > 0 && _minDepositAmount <= 10000, "Wrong amount!");
        minDepositAmount = _minDepositAmount;
    }

    function updateZunamiLpInStrat(uint256 _amount, bool _isMint) external onlyOwner {
