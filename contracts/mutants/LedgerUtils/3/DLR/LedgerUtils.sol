...
library LedgerUtils {
    ...
    function getTotalUnlocking(Types.OracleData storage report, uint64 _eraId) internal pure returns (uint128, uint128) {
        uint128 _total = 0;
    ...
    function getFreeBalance(Types.OracleData storage report) internal pure returns (uint128) {
        return report.stashBalance - report.totalBalance;
    }

    /// @notice Return true if report is consistent
    function isConsistent(Types.OracleData storage report) internal pure returns (bool) {
