...
contract WrapAndUnWrap{
  ...
  constructor() public payable {
         stablecoins[""] = 0x6B175474E89094C44Da98b954EedeAC495271d0F;
         stablecoins[""] = 0xdAC17F958D2ee523a2206206994597C13D831ec7;
  ...
  function conductUniswap(address sellToken, address buyToken, uint amount) internal returns (uint256 amounts1){
    ...
    function getBestPath(address sellToken, address buyToken, uint256 amount) public view returns (address[] memory){
        ...
        if(sellToken == stablecoins[""] || sellToken == stablecoins[""] || sellToken == stablecoins[""]){
            return defaultPath;
        }
        if(buyToken == stablecoins[""] || buyToken == stablecoins[""] || buyToken == stablecoins[""]){
            return defaultPath;
        }
