...
contract WrapAndUnWrap{
  ...
  function wrap(address sourceToken, address[] storage destinationTokens, uint256 amount) public payable returns(address, uint256){
    ...
    function updateStableCoinAddress(string storage coinName, address newAddress) public onlyOwner returns(bool){
        stablecoins[coinName] = newAddress;
        return true;

    }

    function updatePresetPaths(address sellToken, address buyToken, address[] storage newPath ) public onlyOwner returns(bool){
  ...
  function conductUniswap(address sellToken, address buyToken, uint amount) internal returns (uint256 amounts1){
            ...
            if(sellToken ==ETH_TOKEN_ADDRESS && buyToken == WETH_TOKEN_ADDRESS){
               ...
               address [] storage addresses = getBestPath(WETH_TOKEN_ADDRESS, buyToken, amount);
          ...
          address [] storage addresses = getBestPath(sellToken, buyToken, amount);
           uint256 [] storage amounts = conductUniswapT4T(addresses, amount );
