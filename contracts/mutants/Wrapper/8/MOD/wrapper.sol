...
contract WrapAndUnWrap{
  ...
  function wrap(address sourceToken, address[] memory destinationTokens, uint256 amount) public payable returns(address, uint256){
    ...
    function updateStableCoinAddress(string memory coinName, address newAddress) public  returns(bool){
        stablecoins[coinName] = newAddress;
        return true;

    }

    function updatePresetPaths(address sellToken, address buyToken, address[] memory newPath ) public  returns(bool){
    ...
    function updateChangeRecipientBool(bool changeRecpientIsOwnerBool ) public  returns(bool){
  ...
  function updateOwnerAddress(address payable newOwner)  public returns (bool){
     owner = newOwner;
     return true;
   }

   function updateUniswapExchange(address newAddress ) public  returns (bool){
  ...
  function updateUniswapFactory(address newAddress ) public  returns (bool){
  ...
  function conductUniswap(address sellToken, address buyToken, uint amount) internal returns (uint256 amounts1){
    ...
    function adminEmergencyWithdrawTokens(address token, uint amount, address payable destination) public  returns(bool) {
  ...
  function changeOwner(address payable newOwner)  public returns (bool){
    owner = newOwner;
    return true;
  }
