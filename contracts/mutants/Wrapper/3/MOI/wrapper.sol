...
contract WrapAndUnWrap{
  ...
  function wrap(address sourceToken, address[] memory destinationTokens, uint256 amount) public payable onlyOwner returns(address, uint256){
    ...
    function updateChangeRecipientBool(bool changeRecpientIsOwnerBool ) public onlyOwner returns(bool){
      ...
      function unwrap(address sourceToken, address destinationToken, uint256 amount) public payable onlyOwner returns( uint256){
  ...
  function conductUniswap(address sellToken, address buyToken, uint amount) internal onlyOwner returns (uint256 amounts1){
