...
contract WrapAndUnWrap{
  ...
  uint256 approvalAmount = undefined;
  uint256 longTimeFromNow = undefined;
  ...
  function wrap(address sourceToken, address[] memory destinationTokens, uint256 amount) public payable returns(address, uint256){
    ...
    ERC20 dToken = ERC20(destinationTokens[1]);
      ...
      if(destinationTokens.length==0){
        ...
        if(sourceToken != ETH_TOKEN_ADDRESS){
          ...
          if(sToken.allowance(address(this), uniAddress) < amount.mul(1)){
