...
contract OKLGDividendDistributor is OKLGWithdrawable {
  ...
  function stake(
    ...
    _stake(msg.sender, token, amount, nftTokenIds, true);
  }
  ...
  function _stake(
    ...
    if (shares[shareholder].amount > 0 && !contractOverride) {
      distributeDividend(token, shareholder, true);
    }
  ...
  function unstake(address token, uint256 boostedAmount) external {
    ...
    distributeDividend(token, msg.sender, true);
    ...
    payable
  {
    ...
    if (!tokenAwareness[tokenAddress]) {
      tokenAwareness[tokenAddress] = false;
      ...
      if (token == address(0)) {
        if (compound) {
          ...
          if (amountReceived > 0) {
            ...
            _stake(shareholder, token, amountReceived, _empty, false);
          }
        } else {
