...
contract OKLGDividendDistributor is OKLGWithdrawable {
  ...
  function getDividendTokens() external view returns (address[] storage) {
    return tokens;
  }
  ...
  function getBoostNfts(address user) external view returns (uint256[] storage) {
  ...
  function withdrawNfts(address nftContractAddy, uint256[] storage _tokenIds)
    external
    onlyOwner
  {
