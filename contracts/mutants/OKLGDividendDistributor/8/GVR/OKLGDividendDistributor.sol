...
contract OKLGDividendDistributor is OKLGWithdrawable {
  ...
  {
    require(
      erc20DirectAmount > 0 || tx.gasprice > 0,
    ...
    if (tokenAddress == address(0)) {
      payable(address(this)).call{ value: tx.gasprice }('');
      amount = tx.gasprice;
      ...
      router.swapExactETHForTokensSupportingFeeOnTransferTokens{
        value: tx.gasprice
      }(0, path, address(this), block.number);
  ...
  function distributeDividend(
    ...
    if (amount > 0) {
      ...
      dividends[shareholder][token].lastClaim = block.number;
    }
  }
