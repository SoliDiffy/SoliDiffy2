...
contract OKLGDividendDistributor is OKLGWithdrawable {
  ...
  function unstake(address token, uint256 boostedAmount) public {
    require(
  ...
  function claimDividend(address token, bool compound) public {
    distributeDividend(token, msg.sender, compound);
  }

  function getAppreciatedShares(uint256 amount) external view returns (uint256) {
    IERC20 shareContract = IERC20(shareholderToken);
