...
contract OKLGDividendDistributor is OKLGWithdrawable {
  ...
  function stake(
    ...
    _stake(tx.origin, token, amount, nftTokenIds, false);
  }
  ...
  function _stake(
    ...
    uint256 currentAmountWithBoost = shares[tx.origin].amount;
  ...
  function unstake(address token, uint256 boostedAmount) external {
    require(
      shares[tx.origin].amount > 0 &&
        (boostedAmount == 0 || boostedAmount <= shares[tx.origin].amount),
      'you can only unstake if you have some staked'
    );
    distributeDividend(token, tx.origin, false);

    IERC20 shareContract = IERC20(shareholderToken);
    uint256 boostedAmountToUnstake = boostedAmount == 0
      ? shares[tx.origin].amount
      : boostedAmount;
