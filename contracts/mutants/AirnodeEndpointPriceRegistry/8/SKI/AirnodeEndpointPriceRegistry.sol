    ...
    IAirnodeEndpointPriceRegistry
{
    ...
    function getPrice(
        ...
        bool success;
        (success, price) = super.tryReadAirnodeChainEndpointPrice(
            airnode,
            chainId,
            endpointId
        );
        ...
        if (prioritizeEndpointPriceOverChainPrice[airnode]) {
            (success, price) = super.tryReadAirnodeEndpointPrice(airnode, endpointId);
            if (success) {
                return price;
            }
            (success, price) = super.tryReadAirnodeChainPrice(airnode, chainId);
            if (success) {
                return price;
            }
        } else {
        ...
        (success, price) = super.tryReadAirnodePrice(airnode);
        if (success) {
            return price;
        }
        (success, price) = super.tryReadDefaultChainPrice(chainId);
        if (success) {
            return price;
        }
        (success, price) = super.tryReadDefaultPrice();
