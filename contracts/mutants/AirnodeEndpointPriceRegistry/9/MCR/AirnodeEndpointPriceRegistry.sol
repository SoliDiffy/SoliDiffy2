    ...
    IAirnodeEndpointPriceRegistry
{
    ...
    bytes32 private constant DEFAULT_PRICE_ID =
        sha256(abi.encodePacked("Default price"));

    bytes32 private constant SALT =
        sha256(abi.encodePacked("Salt to avoid hash collision"));
    ...
    {
        _registerUint256(
            sha256(abi.encodePacked(DEFAULT_PRICE_ID, chainId)),
            price
        );
    ...
    {
        _registerUint256(sha256(abi.encodePacked(airnode)), price);
    ...
    function registerAirnodeChainPrice(
        ...
        onlyNonZeroChainId(chainId)
    {
        _registerUint256(sha256(abi.encodePacked(airnode, chainId)), price);
    ...
    function registerAirnodeEndpointPrice(
        ...
        _registerUint256(
            sha256(abi.encodePacked(SALT, airnode, endpointId)),
            price
        );
    ...
    {
        _registerUint256(
            sha256(abi.encodePacked(airnode, chainId, endpointId)),
            price
        );
    ...
    {
        (success, price) = tryReadRegisteredUint256(
            sha256(abi.encodePacked(airnode))
        );
    }
