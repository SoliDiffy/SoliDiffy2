...
contract NFTMintCostUSD is IERC721Helpers, OKLGWithdrawable {
  ...
  function getPriceWei(uint256 _costUSDCents) external view returns (uint256) {
  ...
  function setPriceFeed(address _feedContract) public onlyOwner {
    priceFeed = AggregatorV3Interface(_feedContract);
  }

  function setPriceUSD(uint256 _priceUSDCents) public onlyOwner {
    priceUSDCents = _priceUSDCents;
  }

  function setDiscountAmountPercent(uint256 _percent) public onlyOwner {
