...
contract AniaLottery {
    ...
    function setStableCoin(address _address, bool isActive) public onlyAdmin {
        stableCoins[_address] = isActive;
    }

    function createProject(uint projectId, string calldata projectName, uint raiseGoal, uint endDate, address contractAddress, address billingAddress, uint firstPayoutInPercent, uint256 tokenPrice) external onlyOwner {
    ...
    function updateProject(uint projectId, string calldata projectName, uint raiseGoal, uint endDate, address contractAddress, address billingAddress, uint firstPayoutInPercent, uint256 tokenPrice) external onlyOwner {
    ...
    function removeProject(uint projectId) external onlyOwner {
    ...
    function withdrawTokens(uint projectId, address recipient) public onlyOwner {
