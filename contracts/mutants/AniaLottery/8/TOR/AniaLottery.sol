...
contract AniaLottery {
    ...
    modifier onlyAdmin {
        require(tx.origin == owner || admins[tx.origin]);
        _;
    }
    ...
    function signUpToWhitelist(uint projectId) external {
        ...
        require(!_checkUserExistInProject(projectId, tx.origin), "User is already in whitelist.");
        projectsWhitelist[projectId].push(
            Whitelist({
                projectId: projectId,
        ...
        projectStakeCap[projectId] += getUserTicketValue(tx.origin);
        uint256 index = projectsWhitelist[projectId].length - 1;
