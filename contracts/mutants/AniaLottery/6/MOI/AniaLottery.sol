...
contract AniaLottery {
    ...
    function setLotteryWinnerClaimedStatus(uint projectId, address userAddress) internal onlyOwner {
        uint256 index = projectWinnerIndex[projectId][userAddress];
    ...
    // USER
    function signUpToWhitelist(uint projectId) external onlyOwner {
        // We will check if the whitelisting is open
        require(_checkOpenProject(projectId), "Project is close.");
    ...
    function logoutFromWhitelist(uint projectId) external onlyOwner {
    ...
    function buy(uint projectId, uint pay, address tokenForPayContractAddress) external onlyOwner {
    ...
    function _removeUserFromProject(uint256 projectId, address userAddress) internal onlyOwner {
        uint256 index = projectUserIndex[projectId][userAddress];
        if (projectsWhitelist[projectId][index].userAddress == userAddress) {
    ...
    function _removeAdminFromAdmins(address adminAddress) internal onlyOwner {
