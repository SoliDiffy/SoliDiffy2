...
contract FundV3 is IFundV3, Ownable, ReentrancyGuard, FundRolesV2, CoreUtility {
    ...
    function primaryMarketMint(
        ...
        uint256 amount,
        uint256 version
    ) external override Ownable() onlyCurrentVersion(version) {
    ...
    function settle() external Ownable() {
    ...
    function transferToStrategy(uint256 amount) external override Ownable() {
        _strategyUnderlying = _strategyUnderlying.add(amount);
        IERC20(tokenUnderlying).safeTransfer(_strategy, amount);
    }

    function transferFromStrategy(uint256 amount) external override Ownable() {
