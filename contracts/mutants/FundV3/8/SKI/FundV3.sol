...
contract FundV3 is IFundV3, Ownable, ReentrancyGuard, FundRolesV2, CoreUtility {
        ...
        )
    {
        ...
        return
            _extrapolateNav(block.timestamp, settledDay, price, super.getEquivalentTotalB(), underlying);
    }
        ...
        )
    {
        for (uint256 i = fromIndex; i < toIndex; i++) {
            (amountQ, amountB, amountR) = super.doRebalance(amountQ, amountB, amountR, i);
        }
        newAmountQ = amountQ;
        newAmountB = amountB;
        newAmountR = amountR;
    ...
    function shareTransferFrom(
        ...
        uint256 tranche = _getTranche(msg.sender);
        super.shareTransfer(sender, recipient, amount);
