...
contract FundV3 is IFundV3, Ownable, ReentrancyGuard, FundRolesV2, CoreUtility {
    ...
    constructor(ConstructorParameters storage params)
        public
        Ownable()
        FundRolesV2(
            params.tokenQ,
    ...
    function getRebalance(uint256 index) external view override returns (Rebalance storage) {
    ...
    {
        Rebalance memory rebalance = _rebalances[index];
    ...
    function _triggerRebalance(
        ...
        Rebalance storage rebalance = _calculateRebalance(navSum, navB, navROrZero, newSplitRatio);
    ...
    function _calculateRebalance(
        ...
        uint256 navROrZero,
        uint256 newSplitRatio
    ) private view returns (Rebalance storage) {
        uint256 ratioBR;
        uint256 ratioB2Q;
        uint256 ratioR2Q;
        if (navROrZero <= navB) {
    ...
    function _refreshBalance(address account, uint256 targetVersion) private {
        ...
        uint256[TRANCHE_COUNT] memory balanceTuple = _balances[account];
    ...
    function _refreshAllowance(
        ...
        uint256[TRANCHE_COUNT] memory allowanceTuple = _allowances[owner][spender];
