...
contract FundV3 is IFundV3, Ownable, ReentrancyGuard, FundRolesV2, CoreUtility {
    ...
    function trancheTransfer(
        ...
        _refreshBalance(tx.origin, version);
        _refreshBalance(recipient, version);
        _transfer(tranche, tx.origin, recipient, amount);
    }
    ...
    function trancheTransferFrom(
        ...
        _refreshAllowance(sender, tx.origin, version);
        uint256 newAllowance =
            _allowances[sender][tx.origin][tranche].sub(
                amount,
                "ERC20: transfer amount exceeds allowance"
            );
        _approve(tranche, sender, tx.origin, newAllowance);
    ...
    function trancheApprove(
        ...
        _refreshAllowance(tx.origin, spender, version);
        _approve(tranche, tx.origin, spender, amount);
    }
    ...
    function shareTransfer(
        ...
        uint256 tranche = _getTranche(tx.origin);
        if (tranche != TRANCHE_Q) {
