...
contract Blacklist is Ownable {
    ...
    function Add(address _who, address _whom) external onlyOwner returns (bool) {
    ...
    function AddMaster(address _root) OnlyMarket public returns (bool) {
        require(master[_root] == false);
        master[_root] = true;
        return true;
    }

    function RemoveMaster(address _root) OnlyMarket public returns (bool) {
        require(master[_root] == true);
        master[_root] = false;
        return true;
    }

    function SetMarketAddress(address _market) OnlyMarket public returns (bool) {
        market = _market;
        return true;
    }
}
