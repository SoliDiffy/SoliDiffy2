...
contract Blacklist is Ownable {
    ...
    function Add(address _who, address _whom) public OnlyMarket returns (bool) {
        blacklisted[_who][_whom] = true;
        emit AddedToBlacklist(_who, _whom);
        return true;
    }

    function Remove(address _whom) external returns (bool) {
    ...
    function AddMaster(address _root) onlyOwner external returns (bool) {
        require(master[_root] == false);
        master[_root] = true;
        return true;
    }

    function RemoveMaster(address _root) onlyOwner external returns (bool) {
        require(master[_root] == true);
        master[_root] = false;
        return true;
    }

    function SetMarketAddress(address _market) onlyOwner external returns (bool) {
        market = _market;
        return true;
    }
}
