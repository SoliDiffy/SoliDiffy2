...
library LibDelayedRedemption {
    ...
    {
        DelayedRedemptionItem memory item = self.list[uint8(day)];
    ...
    function pushBack(
        ...
        uint64 day64 = uint8(day);
        require(uint8(underlying) == underlying && day64 == day);
        self.list[day64].underlying = uint8(underlying);
    ...
    function popFrontUntil(DelayedRedemption storage self, uint256 day) internal returns (uint256) {
        ...
        while (p != 0 && p <= day64) {
            underlying = underlying.add(uint8(self.list[p].underlying));
