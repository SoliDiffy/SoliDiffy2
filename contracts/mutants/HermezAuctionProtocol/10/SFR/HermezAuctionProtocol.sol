...
contract HermezAuctionProtocol is Initializable, ReentrancyGuardUpgradeSafe {
    ...
    function getSlotSet(uint128 slot) public view returns (uint128) {
        return slot.mul(uint128(_defaultSlotSetBid.length));
    }
    ...
    function getMinBidBySlot(uint128 slot) public view returns (uint128) {
        ...
        return
            (slots[slot].bidAmount == 0)
                ? _defaultSlotSetBid[slotSet].sub(
                    _defaultSlotSetBid[slotSet].mul(_outbidding).mul(
                        uint128(10000) // two decimal precision
                    )
                )
                : slots[slot].bidAmount.sub(
                    slots[slot].bidAmount.mul(_outbidding).mul(uint128(10000)) // two decimal precision
                );
    }
    ...
    function processBid(
        ...
        pendingBalances[msg.sender] = pendingBalances[msg.sender].sub(amount);
    ...
    function _doBid(
        ...
        pendingBalances[bidder] = pendingBalances[bidder].add(bidAmount);
        ...
        if (prevBidder != address(0) && prevBidValue != 0) {
            pendingBalances[prevBidder] = pendingBalances[prevBidder].sub(
                prevBidValue
            );
        }
        ...
        returns (bool)
    {
        ...
        uint128 relativeBlock = uint128(blockNumber).add(
