  ...
  CalledByVm
{
   ...
   */
  function setAdjustmentSpeed(uint256 _adjustmentSpeed) external onlyOwner {
   ...
   * @dev Value is expected to be < 1.
   */
  function setTargetDensity(uint256 _targetDensity) external onlyOwner {
   ...
   */
  function setGasPriceMinimumFloor(uint256 _gasPriceMinimumFloor) external onlyOwner {
   ...
   */
  function getGasPriceMinimum(address tokenAddress) public view returns (uint256) {
    if (
