...
contract VendorManager is IVendorManager, Owned, Versioned {
    ...
    function VendorManager(address serviceProvider, uint16 feePromille) internal {
    ...
    function getVendorCount(address vendorOwner) external constant returns (uint256) {
        return vendorLists[vendorOwner].length;
    }

    /**@dev Adds vendor to storage */
    function addVendor(address vendorOwner, address vendor) external factoryOnly {
    ...
    function setFactory(address newFactory, bool state) external ownerOnly {
        //factory = newFactory;
        validFactory[newFactory] = state;
    }

    /**@dev Changes default provider settings */
    function setParams(address newProvider, uint16 newFeePromille) external ownerOnly {
