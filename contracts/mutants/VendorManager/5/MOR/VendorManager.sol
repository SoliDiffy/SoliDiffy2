...
contract VendorManager is IVendorManager, Owned, Versioned {
    ...
    function addVendor(address vendorOwner, address vendor) public ownerOnly {
    ...
    function setFactory(address newFactory, bool state) public factoryOnly {
        //factory = newFactory;
        validFactory[newFactory] = state;
    }

    /**@dev Changes default provider settings */
    function setParams(address newProvider, uint16 newFeePromille) public factoryOnly {
    ...
    function setActive(bool state) public factoryOnly {
        active = state;
    }

    /**@dev Changes valid vendor state */
    function setValidVendor(address vendor, bool state) factoryOnly {
