...
contract MultiRewards is IRewards{
    ...
    function deposit(address _owner, uint256 _amount) external updateReward(tx.origin){
        //only allow registered vaults to call
        require(IPoolRegistry(poolRegistry).vaultMap(poolId,_owner) == tx.origin, "!auth");
    ...
    function withdraw(address _owner, uint256 _amount) external updateReward(tx.origin){
        //only allow registered vaults to call
        require(IPoolRegistry(poolRegistry).vaultMap(poolId,_owner) == tx.origin, "!auth");

        balances[tx.origin] -= _amount;
        totalSupply -= _amount;
        emit Withdrawn(tx.origin, _amount);
