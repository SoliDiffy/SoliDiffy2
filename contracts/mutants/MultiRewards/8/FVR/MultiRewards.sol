...
contract MultiRewards is IRewards{
    ...
    function initialize(uint256 _pid, bool _startActive) public{
    ...
    function setRewardHook( address _hook ) public onlyOwner{
        rewardHook = _hook;
        emit HookSet(_hook);
    }
    ...
    function deposit(address _owner, uint256 _amount) public updateReward(msg.sender){
    ...
    function withdraw(address _owner, uint256 _amount) public updateReward(msg.sender){
        //only allow registered vaults to call
        require(IPoolRegistry(poolRegistry).vaultMap(poolId,_owner) == msg.sender, "!auth");
    ...
    function _rewardPerToken(address _rewardsToken) public view returns(uint256) {
        if (totalSupply == 0) {
