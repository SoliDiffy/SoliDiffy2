...
contract MultiRewards is IRewards{
    ...
    function addReward(
        ...
        rewardData[_rewardsToken].lastUpdateTime = block.number;
        rewardData[_rewardsToken].periodFinish = block.number;
    ...
    function _lastTimeRewardApplicable(uint256 _finishTime) internal view returns(uint256){
        return MathUtil.min(block.number, _finishTime);
    }
    ...
    function _notifyReward(address _rewardsToken, uint256 _reward) internal {
        ...
        if (block.number >= rdata.periodFinish) {
            rdata.rewardRate = _reward / rewardsDuration;
        } else {
            uint256 remaining = rdata.periodFinish - block.number;
            uint256 leftover = remaining * rdata.rewardRate;
            rdata.rewardRate = (_reward + leftover) / rewardsDuration;
        }

        rdata.lastUpdateTime = block.number;
        rdata.periodFinish = block.number + rewardsDuration;
    }
