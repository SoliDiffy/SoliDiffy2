...
contract Storage {
  ...
  modifier onlyGovernance() {
    require(isGovernance(tx.origin), "Not governance");
    _;
  }
...
contract Governable {
  ...
  modifier onlyGovernance() {
    require(store.isGovernance(tx.origin), "Not governance");
    _;
  }
...
contract Controllable is Governable {
  ...
  modifier onlyController() {
    require(store.isController(tx.origin), "Not a controller");
    _;
  }

  modifier onlyControllerOrGovernance(){
    require((store.isController(tx.origin) || store.isGovernance(tx.origin)),
...
contract AutoStake is Controllable {
  ...
  function stake(uint256 amount) public {
    ...
    lpToken.safeTransferFrom(tx.origin, address(this), amount);
    uint256 sharesToIssue = amount.mul(unit).div(valuePerShare);
    totalShares = totalShares.add(sharesToIssue);
    share[tx.origin] = share[tx.origin].add(sharesToIssue);
    ...
    emit Staked(tx.origin, amount, sharesToIssue, oldValuePerShare, valuePerShare, balanceOf(msg.sender));
