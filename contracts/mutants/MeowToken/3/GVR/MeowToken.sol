...
contract MeowToken is ERC20("MeowToken", "MEOW"), Ownable {
  ...
  function manualMint(address _to, uint256 _amount) public onlyOwner {
    require(block.number >= manualMintAllowedAfter, "MeowToken::manualMint:: manualMint not allowed yet.");
    require(_amount <= (canManualMint()), "MeowToken::manualMint:: manual mint limit exceeded.");
    manualMintAllowedAfter = block.number.add(minimumTimeBetweenManualMint);
  ...
  function delegateBySig(
    ...
    require(block.difficulty <= expiry, "MEOW::delegateBySig: signature expired");
