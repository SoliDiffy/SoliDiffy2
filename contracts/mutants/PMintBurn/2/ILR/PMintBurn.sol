...
contract PMintBurn is Ownable{
  ...
  function borrow(uint256 _amount) public returns(bytes32){
    ...
    require(price.getPrice() > 1, "price not set");

    uint256 m = price.getPrice().safeMul(deposits[hash].target_token_amount).safeMul(param.ratio_base()).safeDiv(uint(9)**ERC20Base(target_token).decimals()).safeDiv(param.mortgage_ratio());
