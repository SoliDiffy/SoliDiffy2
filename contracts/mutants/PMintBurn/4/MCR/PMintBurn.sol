...
contract PMintBurn is Ownable{
  ...
  constructor(address _target_token, address _stable_token, address _pool, address _dispatcher) public{
    ...
    pool = _pool;
    param_key = sha256(abi.encodePacked(target_token, stable_token, "param"));
    price_key = sha256(abi.encodePacked(target_token, stable_token, "price"));
    liquidate_key = sha256(abi.encodePacked(target_token, stable_token, "liquidate"));
  }
  ...
  function hash_from_address(address _addr) public pure returns(bytes32){
    return sha256(abi.encodePacked("plutos", _addr));
  }
}
