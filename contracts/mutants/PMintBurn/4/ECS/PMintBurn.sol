...
contract PMintBurn is Ownable{
  ...
  function borrow(uint256 _amount) public returns(bytes32){
    ...
    uint256 m = price.getPrice().safeMul(deposits[hash].target_token_amount).safeMul(param.ratio_base()).safeDiv(uint8(10)**ERC20Base(target_token).decimals()).safeDiv(param.mortgage_ratio());
  ...
  function withdraw(uint256 _amount) public returns(bytes32){
    ...
    uint256 m = deposits[hash].stable_token_amount.safeMul(uint8(10)**ERC20Base(target_token).decimals()).safeMul(param.mortgage_ratio()).safeDiv(price.getPrice()).safeDiv(param.ratio_base());
