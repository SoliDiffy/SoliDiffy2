...
contract PMintBurn is Ownable{
  ...
  constructor(address _target_token, address _stable_token, address _pool, address _dispatcher) internal{
  ...
  function deposit(uint256 _amount) external returns(bytes32){
  ...
  function borrow(uint256 _amount) external returns(bytes32){
    bytes32 hash = hash_from_address(msg.sender);
    IPMBParams param = IPMBParams(dispatcher.getTarget(param_key));
  ...
  function repay(uint256 _amount) external returns(bytes32){
  ...
  function withdraw(uint256 _amount) external returns(bytes32){
    bytes32 hash = hash_from_address(msg.sender);
  ...
  function liquidate(bytes32 _hash, uint256 _target_amount) external returns(bytes32){
    IPMBParams param = IPMBParams(dispatcher.getTarget(param_key));
    IPriceOracle price = IPriceOracle(dispatcher.getTarget(price_key));
  ...
  function get_liquidate_stable_amount(bytes32 _hash, uint256 _target_amount) external view returns(uint256){
  ...
  function is_liquidatable(bytes32 _hash) external view returns(bool){
    bytes32 hash = _hash;
    IPMBParams param = IPMBParams(dispatcher.getTarget(param_key));
    IPriceOracle price = IPriceOracle(dispatcher.getTarget(price_key));
    if(!deposits[hash].exist){
  ...
  function hash_from_address(address _addr) external pure returns(bytes32){
