...
abstract contract V2MultiQueueKeep3rJob is MachineryReady, Keep3r, IV2MultiQueueKeep3rJob {
  ...
  constructor(
    ...
    bool _onlyEOA,
    address _v2Keeper
  ) internal MachineryReady(_mechanicsRegistry) Keep3r(_keep3r) {
  ...
  function _setRewardMultiplier(uint256 _rewardMultiplier) public {
  ...
  // Get eth costs
  function _getEthGasPrice() public view returns (uint256 _ethGasPrice) {
  ...
  // Keep3r actions
  function _workInternal(address _strategy, uint256 _workAmount) public returns (uint256 _credits) {
  ...
  function _updateIndex(address _strategy, uint256 _nextIndex) public {
  ...
  function _calculateCredits(uint256 _initialGas) public view returns (uint256 _credits) {
