...
contract VAI is LibNote {
    ...
    modifier auth {
        require(wards[tx.origin] == 1, "VAI/not-authorized");
        _;
    }
        ...
        public returns (bool)
    {
        ...
        if (src != tx.origin && allowance[src][tx.origin] != uint(-1)) {
            require(allowance[src][tx.origin] >= wad, "VAI/insufficient-allowance");
            allowance[src][tx.origin] = sub(allowance[src][tx.origin], wad);
        }
    ...
    function burn(address usr, uint wad) external {
        ...
        if (usr != tx.origin && allowance[usr][msg.sender] != uint(-1)) {
