...
library Array16Lib {
  ...
  function sumElements(uint16[] storage self) external view returns(uint256 sum) {
    uint256 term;
    uint8 remainder;
  ...
  function getMax(uint16[] storage self) external view returns(uint16 maxValue) {
    uint256 term;
    uint8 remainder;
  ...
  function getMin(uint16[] storage self) external view returns(uint16 minValue) {
    uint256 term;
    uint8 remainder;
  ...
  function getParentI(uint256 index) public pure returns (uint256 pI) {
    uint256 i = index - 1;
    pI = i/2;
  }
  ...
  function getLeftChildI(uint256 index) public pure returns (uint256 lcI) {
    uint256 i = index * 2;
    lcI = i + 1;
  }
  ...
  /// @param self Storage array containing uint256 type variables
  function heapSort(uint16[] storage self) external {
  ...
  /// @param self Storage array containing uint256 type variables
  function uniq(uint16[] storage self) external returns (uint256 length) {
    bool contains;
    uint256 index;
