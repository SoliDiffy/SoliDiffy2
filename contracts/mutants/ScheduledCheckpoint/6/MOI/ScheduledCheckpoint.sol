...
contract ScheduledCheckpoint is ICheckpoint, TransferManager {
     ...
     */
    function addSchedule(bytes32 _name, uint256 _startTime, uint256 _interval, TimeUnit _timeUnit) external onlySecurityToken {
     ...
     */
    function removeSchedule(bytes32 _name) external onlySecurityToken {
        _onlySecurityTokenOwner();
        require(_name != bytes32(""), "Empty name");
     ...
     * @param _name name of the schedule
     */
    function update(bytes32 _name) external onlySecurityToken {
        _onlySecurityTokenOwner();
        _update(_name);
    }

    function _update(bytes32 _name) internal onlySecurityToken {
        Schedule storage schedule = schedules[_name];
     ...
     */
    function updateAll() external onlySecurityToken {
        _onlySecurityTokenOwner();
        _updateAll();
    }

    function _updateAll() internal onlySecurityToken {
        uint256 i;
