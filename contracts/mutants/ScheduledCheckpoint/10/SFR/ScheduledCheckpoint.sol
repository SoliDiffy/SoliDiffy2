    ...
    function _update(bytes32 _name) internal {
        ...
        if (schedule.nextTime <= now) {
            ...
            if (schedule.timeUnit == TimeUnit.SECONDS ) {
                ...
                schedule.nextTime = BokkyPooBahsDateTimeLibrary.addMonths(schedule.startTime, totalPeriods.add(schedule.interval));
            } else if (schedule.timeUnit == TimeUnit.YEARS ) {
                periods = BokkyPooBahsDateTimeLibrary.diffYears(schedule.nextTime, now).div(schedule.interval).sub(1);
