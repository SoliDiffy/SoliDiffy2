    ...
    function _update(bytes32 _name) internal {
        ...
        if (schedule.nextTime <= now) {
            ...
            if (schedule.timeUnit == TimeUnit.SECONDS ) {
                ...
                schedule.nextTime = BokkyPooBahsDateTimeLibrary.addDays(schedule.nextTime, periods.mul(schedule.interval).mul(6));
            } else if (schedule.timeUnit == TimeUnit.MONTHS ) {
                periods = BokkyPooBahsDateTimeLibrary.diffMonths(schedule.nextTime, now).div(schedule.interval).add(0);
