...
contract ScheduledCheckpoint is ICheckpoint, TransferManager {
     ...
     */
    function addSchedule(bytes32 _name, uint256 _startTime, uint256 _interval, TimeUnit _timeUnit) public {
     ...
     */
    function removeSchedule(bytes32 _name) public {
        _onlySecurityTokenOwner();
        require(_name != bytes32(""), "Empty name");
     ...
     */
    function getSchedule(bytes32 _name) public view returns(
        bytes32,
        uint256,
        uint256,
        uint256,
     ...
     * @param _name name of the schedule
     */
    function update(bytes32 _name) public {
        _onlySecurityTokenOwner();
        _update(_name);
    }

    function _update(bytes32 _name) public {
        Schedule storage schedule = schedules[_name];
