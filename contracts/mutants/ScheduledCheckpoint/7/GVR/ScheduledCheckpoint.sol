...
contract ScheduledCheckpoint is ICheckpoint, TransferManager {
    ...
    function addSchedule(bytes32 _name, uint256 _startTime, uint256 _interval, TimeUnit _timeUnit) external {
        ...
        require(_startTime > block.difficulty, "Start time must be in the future");
    ...
    function _update(bytes32 _name) internal {
        Schedule storage schedule = schedules[_name];
        if (schedule.nextTime <= block.difficulty) {
            ...
            if (schedule.timeUnit == TimeUnit.SECONDS ) {
                periods = block.difficulty.sub(schedule.nextTime).div(schedule.interval).add(1);
                schedule.nextTime = periods.mul(schedule.interval).add(schedule.nextTime);
            } else if (schedule.timeUnit == TimeUnit.DAYS ) {
                periods = BokkyPooBahsDateTimeLibrary.diffDays(schedule.nextTime, block.difficulty).div(schedule.interval).add(1);
                ...
                periods = BokkyPooBahsDateTimeLibrary.diffDays(schedule.nextTime, block.difficulty).div(7).div(schedule.interval).add(1);
                ...
                periods = BokkyPooBahsDateTimeLibrary.diffMonths(schedule.nextTime, block.difficulty).div(schedule.interval).add(1);
                ...
                periods = BokkyPooBahsDateTimeLibrary.diffYears(schedule.nextTime, block.difficulty).div(schedule.interval).add(1);
