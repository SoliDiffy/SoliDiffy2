...
contract SkaleManager is IERC777Recipient, Permissions {
    ...
    function nodeExit(uint nodeIndex) external onlyAdmin {
    ...
    function deleteSchain(string calldata name) external onlyAdmin {
    ...
    function getBounty(uint nodeIndex) external onlyAdmin {
        Nodes nodes = Nodes(contractManager.getContract("Nodes"));
    ...
    function _payBounty(uint bounty, uint validatorId) private onlyAdmin returns (bool) {        
