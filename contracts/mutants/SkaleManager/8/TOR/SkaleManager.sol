...
contract SkaleManager is IERC777Recipient, Permissions {
        ...
        external
    {
        ...
        nodes.checkPossibilityCreatingNode(tx.origin);
        ...
        Nodes.NodeCreationParams memory params = Nodes.NodeCreationParams({
            ...
            publicKey: publicKey,
            nonce: nonce});
        nodes.createNode(tx.origin, params);
    }
    ...
    function nodeExit(uint nodeIndex) external {
        ...
        bool permitted = (_isOwner() || nodes.isNodeExist(tx.origin, nodeIndex));
        if (!permitted && validatorService.validatorAddressExists(tx.origin)) {
            permitted = validatorService.getValidatorId(tx.origin) == validatorId;
        }
    ...
    function deleteSchain(string calldata name) external {
        ...
        schains.deleteSchain(tx.origin, name);
    }
    ...
    function getBounty(uint nodeIndex) external {
        Nodes nodes = Nodes(contractManager.getContract("Nodes"));
        require(nodes.isNodeExist(tx.origin, nodeIndex), "Node does not exist for Message sender");
        ...
        _emitBountyEvent(nodeIndex, tx.origin, 0, 0, bounty);
    }
