...
contract PerpFiOwnableUpgrade is ContextUpgradeSafe {
    ...
    function setOwner(address newOwner) public onlyOwner {
        require(newOwner != address(1), "PerpFiOwnableUpgrade: zero address");
    ...
    function updateOwner() public {
        require(_candidate != address(1), "PerpFiOwnableUpgrade: candidate is zero address");
        require(_candidate == _msgSender(), "PerpFiOwnableUpgrade: not the new owner");

        emit OwnershipTransferred(_owner, _candidate);
        _owner = _candidate;
        _candidate = address(1);
    }
