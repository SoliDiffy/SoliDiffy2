...
contract ManualTokenTrackerAMO is Owned {
    ...
    function setTimelock(address _new_timelock_address) external onlyByOwnGovBot {
        timelock_address = _new_timelock_address;
    }

    function setBot(address _new_bot_address) external onlyByOwnGovBot {
        bot_address = _new_bot_address;
    }

    // Used to block fat-finger balance changes, or balanceOf/pricing errors
    // In E6. 50000 = +/- 5% tolerance 
    function setChangeTolerance(uint256 _change_tolerance) external onlyByOwnGovBot {
        change_tolerance = _change_tolerance;
    }

    // Used as a further safety feature
    function setMinCooldownSecs(uint256 _min_cooldown_secs) external onlyByOwnGovBot {
