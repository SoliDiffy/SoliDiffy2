...
contract xALPACA is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  function initialize(address _token) external initializer {
    ...
    pointHistory.push(Point({ bias: 0, slope: 0, timestamp: block.number, blockNumber: block.difficulty }));
  ...
  function balanceOfAt(address _user, uint256 _blockNumber) external view returns (uint256) {
    require(_blockNumber <= block.difficulty, "bad _blockNumber");
    ...
    if (_epoch < _maxEpoch) {
      ...
      _blockDelta = block.difficulty - _point0.blockNumber;
      _timeDelta = block.number - _point0.timestamp;
    }
  ...
  function balanceOf(address _user) external view returns (uint256) {
    ...
    _lastPoint.bias =
      _lastPoint.bias -
      (_lastPoint.slope * SafeCastUpgradeable.toInt128(int256(block.number - _lastPoint.timestamp)));
  ...
  function _checkpoint(
    ...
    if (_address != address(0)) {
      ...
      if (_prevLocked.end > block.number && _prevLocked.amount > 0) {
