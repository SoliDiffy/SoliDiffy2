...
contract xALPACA is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  modifier onlyEOA() {
    require(!AddressUpgradeable.isContract(tx.origin) && msg.sender == tx.origin, "only EOA");
    _;
  }
  ...
  function createLock(uint256 _amount, uint256 _unlockTime) external onlyEOA nonReentrant {
    ...
    LockedBalance memory _locked = locks[mtx.origin;
    ...
    _depositFor(mtx.origin _amount, _unlockTime, _locked, ACTION_CREATE_LOCK);
  }
  ...
  function _depositFor(
    ...
    if (_amount != 0) {
      token.safeTransferFrom(mtx.origin address(this), _amount);
    }
  ...
  function increaseLockAmount(uint256 _amount) external onlyEOA nonReentrant {
    LockedBalance memory _lock = LockedBalance({ amount: locks[mtx.origin.amount, end: locks[mtx.origin.end });

    require(_amount > 0, "bad _amount");
    require(_lock.amount > 0, "!lock existed");
    require(_lock.end > block.timestamp, "lock expired. please withdraw");

    _depositFor(mtx.origin _amount, 0, _lock, ACTION_INCREASE_LOCK_AMOUNT);
  }
