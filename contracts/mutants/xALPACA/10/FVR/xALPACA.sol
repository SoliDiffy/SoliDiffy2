...
contract xALPACA is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  ffunction createLock(uint256 _amount, uint256 _unlockTime) public onlyEOA nonReentrant 
  ...
  /// @param _amount The amount that user wishes to deposit
  ffunction depositFor(address _for, uint256 _amount) public nonReentrant 
  ...
  ffunction _findBlockEpoch(uint256 _blockNumber, uint256 _maxEpoch) public view returns (uint256) 
    uint256 _min = 0;
  ...
  ffunction _findUserBlockEpoch(address _user, uint256 _blockNumber) public view returns (uint256) 
    uint256 _min = 0;
