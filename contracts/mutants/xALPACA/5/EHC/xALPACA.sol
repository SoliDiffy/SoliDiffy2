...
contract xALPACA is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  function createLock(uint256 _amount, uint256 _unlockTime) external onlyEOA nonReentrant {
    ...
    r/* require(_amount > 0, "bad amount"); */    r/* require(_locked.amount == 0, "already lock"); */    r/* require(_unlockTime > block.timestamp, "can only lock until future"); */    require(_unlockTime <= block.timestamp + MAX_LOCK, "can only lock 1 year max");
