...
contract xALPACA is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  function balanceOfAt(address _user, uint256 _blockNumber) external view returns (uint256) {
    ...
    Point storage _userPoint = userPointHistory[_user][_userEpoch];
    uint256 _maxEpoch = epoch;
    uint256 _epoch = _findBlockEpoch(_blockNumber, _maxEpoch);
    Point storage _point0 = pointHistory[_epoch];
    uint256 _blockDelta = 0;
    uint256 _timeDelta = 0;
    if (_epoch < _maxEpoch) {
      Point storage _point1 = pointHistory[_epoch + 1];
  ...
  function balanceOf(address _user) external view returns (uint256) {
    ...
    Point storage _lastPoint = userPointHistory[_user][_epoch];
    _lastPoint.bias =
      _lastPoint.bias -
  ...
  function _checkpoint(
    address _address,
    LockedBalance storage _prevLocked,
