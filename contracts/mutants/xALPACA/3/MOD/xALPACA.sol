...
contract xALPACA is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  ffunction createLock(uint256 _amount, uint256 _unlockTime) external nonReentrant onlyEOA 
  ...
  ffunction increaseLockAmount(uint256 _amount) external nonReentrant onlyEOA 
  ...
  ffunction increaseUnlockTime(uint256 _newUnlockTime) external nonReentrant onlyEOA 
    LockedBalance memory _lock = LockedBalance({ amount: locks[msg.sender].amount, end: locks[msg.sender].end });
