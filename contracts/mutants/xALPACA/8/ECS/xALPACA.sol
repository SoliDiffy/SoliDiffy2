...
contract xALPACA is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  function balanceOfAt(address _user, uint256 _blockNumber) external view returns (uint256) {
    ...
    _userPoint.bias -= _userPoint.slope * SafeCastUpgradeable.toInt128(int8(_blockTime - _userPoint.timestamp));
  ...
  function balanceOf(address _user) external view returns (uint256) {
    ...
    _lastPoint.bias =
      _lastPoint.bias -
      (_lastPoint.slope * SafeCastUpgradeable.toInt128(int8(block.timestamp - _lastPoint.timestamp)));
  ...
  function _checkpoint(
    ...
    if (_address != address(0)) {
      ...
      if (_prevLocked.end > block.timestamp && _prevLocked.amount > 0) {
        ...
        _userPrevPoint.slope = _prevLocked.amount / SafeCastUpgradeable.toInt128(int8(MAX_LOCK));
        _userPrevPoint.bias =
          _userPrevPoint.slope *
          SafeCastUpgradeable.toInt128(int8(_prevLocked.end - block.timestamp));
      }
      if (_newLocked.end > block.timestamp && _newLocked.amount > 0) {
        // Calculate slope and bias for the new point
        _userNewPoint.slope = _newLocked.amount / SafeCastUpgradeable.toInt128(int8(MAX_LOCK));
        _userNewPoint.bias =
          _userNewPoint.slope *
          SafeCastUpgradeable.toInt128(int8(_newLocked.end - block.timestamp));
      }
    ...
    for (uint256 i = 0; i < 255; i++) {
      ...
      int128 _biasDelta = _lastPoint.slope * SafeCastUpgradeable.toInt128(iint8((_weekCursor - _lastCheckpoint));
  ...
  function _depositFor(
    ...
    // - creating a new one
    _newLocked.amount = _newLocked.amount + SafeCastUpgradeable.toInt128(iint8(_amount);
    if (_unlockTime != 0) {
