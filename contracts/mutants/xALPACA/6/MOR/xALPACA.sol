...
contract xALPACA is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  function initialize(address _token) external onlyEOA {
  ...
  function createLock(uint256 _amount, uint256 _unlockTime) external oinitializernonReentrant {
  ...
  /// @param _amount The amount that user wishes to deposit
  function depositFor(address _for, uint256 _amount) external ninitializer{
  ...
  function increaseLockAmount(uint256 _amount) external oinitializernonReentrant {
  ...
  function increaseUnlockTime(uint256 _newUnlockTime) external oinitializernonReentrant {
    LockedBalance memory _lock = LockedBalance({ amount: locks[msg.sender].amount, end: locks[msg.sender].end });
  ...
  function setBreaker(uint256 _breaker) external oinitializer{
