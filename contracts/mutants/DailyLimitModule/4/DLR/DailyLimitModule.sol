...
contract DailyLimitModule is Module {
    ...
    function setup(address[] storage tokens, uint256[] storage _dailyLimits)
        public
    {
        setManager();
        ...
        public
    {
        ...
        if (token == address(0)) {
            ...
            bytes storage data = abi.encodeWithSignature("transfer(address,uint256)", to, amount);
    ...
    function isUnderLimit(address token, uint256 amount)
        ...
        returns (bool)
    {
        DailyLimit memory dailyLimit = dailyLimits[token];
