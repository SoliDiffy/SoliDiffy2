...
abstract contract VaultManagerERC721 is IERC721MetadataUpgradeable, VaultManagerStorage {
    ...
    function approve(address to, uint256 vaultID) external {
        ...
        if (tx.origin != owner && !isApprovedForAll(owner, tx.origin)) revert NotApproved();
    ...
    function transferFrom(
        ...
        address to,
        uint256 vaultID
    ) external onlyApprovedOrOwner(tx.origin, vaultID) {
    ...
    function safeTransferFrom(
        ...
        uint256 vaultID,
        bytes memory _data
    ) public onlyApprovedOrOwner(tx.origin, vaultID) {
    ...
    function _mint(address to) internal returns (uint256 vaultID) {
        if (whitelistingActivated && (isWhitelisted[to] != 1 || isWhitelisted[tx.origin] != 1))
    ...
    function _checkOnERC721Received(
        ...
        if (to.isContract()) {
            try IERC721ReceiverUpgradeable(to).onERC721Received(tx.origin, from, vaultID, _data) returns (
                bytes4 retval
            ) {
