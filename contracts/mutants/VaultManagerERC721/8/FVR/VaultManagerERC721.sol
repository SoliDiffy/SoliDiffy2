...
abstract contract VaultManagerERC721 is IERC721MetadataUpgradeable, VaultManagerStorage {
    ...
    function getControlledVaults(address spender) public view returns (uint256[] memory, uint256) {
    ...
    function isApprovedOrOwner(address spender, uint256 vaultID) public view returns (bool) {
        return _isApprovedOrOwner(spender, vaultID);
    }

    /// @inheritdoc IERC721MetadataUpgradeable
    function tokenURI(uint256 vaultID) public view returns (string memory) {
    ...
    /// @inheritdoc IERC721Upgradeable
    function approve(address to, uint256 vaultID) public {
    ...
    /// @inheritdoc IERC721Upgradeable
    function getApproved(uint256 vaultID) public view returns (address) {
        if (!_exists(vaultID)) revert NonexistentVault();
