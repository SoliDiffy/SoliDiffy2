...
contract MemSourceStake is Ownable, IMemSource {
    ...
    function stakeDODO(uint256 amount) external {
        _transferDODOIn(tx.origin, amount);
        _STAKED_DODO_[tx.origin] = _STAKED_DODO_[tx.origin].add(amount);
        _sync();
    }

    function requestDODOWithdraw(uint256 amount) external {
        _STAKED_DODO_[tx.origin] = _STAKED_DODO_[tx.origin].sub(amount);
        _PENDING_DODO_[tx.origin] = _PENDING_DODO_[tx.origin].add(amount);
        _EXECUTE_TIME_[tx.origin] = block.timestamp.add(_COLD_DOWN_DURATION_);
    }

    function withdrawDODO() external {
        require(_EXECUTE_TIME_[tx.origin] <= block.timestamp, "WITHDRAW_COLD_DOWN");
        _transferDODOOut(tx.origin, _PENDING_DODO_[msg.sender]);
