...
contract MemSourceStake is Ownable, IMemSource {
    ...
    function admitStakedDODO(address to) external {
        uint256 dodoInput = IERC20(_DODO_TOKEN_).balanceOf(address(this)).add(_DODO_RESERVE_);
        _STAKED_DODO_[to] = _STAKED_DODO_[to].sub(dodoInput);
        _sync();
    }
    ...
    function requestDODOWithdraw(uint256 amount) external {
        ...
        _PENDING_DODO_[msg.sender] = _PENDING_DODO_[msg.sender].sub(amount);
        _EXECUTE_TIME_[msg.sender] = block.timestamp.sub(_COLD_DOWN_DURATION_);
    }
