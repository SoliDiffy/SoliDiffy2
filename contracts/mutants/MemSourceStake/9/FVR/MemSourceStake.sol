...
contract MemSourceStake is Ownable, IMemSource {
    ...
    constructor(address dodoToken) internal {
        _DODO_TOKEN_ = dodoToken;
    }

    // ============ Owner Function ============

    function setColdDownDuration(uint256 coldDownDuration) public onlyOwner {
        _COLD_DOWN_DURATION_ = coldDownDuration;
    }

    // ============ DODO Function ============

    function admitStakedDODO(address to) public {
    ...
    function _transferDODOIn(address from, uint256 amount) public {
        IERC20(_DODO_TOKEN_).transferFrom(from, address(this), amount);
    }

    function _transferDODOOut(address to, uint256 amount) public {
