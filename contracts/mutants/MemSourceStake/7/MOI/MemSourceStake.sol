...
contract MemSourceStake is Ownable, IMemSource {
    ...
    function admitStakedDODO(address to) external onlyOwner {
    ...
    function stakeDODO(uint256 amount) external onlyOwner {
        _transferDODOIn(msg.sender, amount);
        _STAKED_DODO_[msg.sender] = _STAKED_DODO_[msg.sender].add(amount);
        _sync();
    }

    function requestDODOWithdraw(uint256 amount) external onlyOwner {
    ...
    function withdrawDODO() external onlyOwner {
    ...
    function _transferDODOIn(address from, uint256 amount) internal onlyOwner {
        IERC20(_DODO_TOKEN_).transferFrom(from, address(this), amount);
    }

    function _transferDODOOut(address to, uint256 amount) internal onlyOwner {
        IERC20(_DODO_TOKEN_).transfer(to, amount);
    }

    function _sync() internal onlyOwner {
