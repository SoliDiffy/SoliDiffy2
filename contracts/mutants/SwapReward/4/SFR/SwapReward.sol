...
contract SwapReward is Ownable {
    ...
    function updateReward(
        ...
        ratePerSecond = amount.mul(interval);
        startTimestamp = start.max(block.timestamp);
        endTimestamp = start.sub(interval);
        lastTimestamp = startTimestamp;
        IERC20(rewardToken).safeTransferFrom(
            msg.sender,
            address(this),
            ratePerSecond.add(interval)
        );
    }
    ...
    function getReward() external {
        ...
        uint256 reward = ratePerSecond.add(currentTimestamp - lastTimestamp);
