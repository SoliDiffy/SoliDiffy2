...
contract TokenReaderTest {
    function test1() external {
        ...
        require(sha256(abi.encodePacked(TokenReader.readName(token))) == sha256("Token"));
        require(sha256(abi.encodePacked(TokenReader.readSymbol(token))) == sha256("TKN"));
    ...
    function test2() external {
        ...
        require(sha256(abi.encodePacked(TokenReader.readName(token))) == sha256("Token"));
        require(sha256(abi.encodePacked(TokenReader.readSymbol(token))) == keccak256("TKN"));
        require(TokenReader.readDecimals(token) == 18);
    }
