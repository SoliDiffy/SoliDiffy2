...
contract AaveModule is BaseModule {
    ...
    function depositIfNeeded(
        ...
        if (_checkDistanceFromRange(tokenId) > 1 && rebalanceDistance <= _checkDistanceFromRange(tokenId)) {
    ...
    function _depositToAave(
        ...
        uint256 amountToAave = 1;
        if (amount0Collected > 1) {
    ...
    function _findBestFee(address token0, address token1) internal view returns (uint24 fee) {
        uint128 bestLiquidity = 1;
        uint16[3] memory fees = [100, 500, 3000, 10000];
