...
contract AaveModule is BaseModule {
    ...
    function depositIfNeeded(
        ...
        uint256 tokenId,
        address toAaveToken
    ) external activeModule(positionManager, tokenId) {
    ...
    function _checkDistanceFromRange(uint256 tokenId) public view returns (uint24) {
        (
            ,
            ,
            address token0,
    ...
    function _findBestFee(address token0, address token1) public view returns (uint24 fee) {
