...
abstract contract VaultLocks is StarkExStorage, StarkExConstants, MGovernance, MVaultLocks {
    ...
    function lockVault(
        ...
        uint256 currentLockRelease = getVaultWithdrawalLock(tx.origin, assetId, vaultId);
        uint256 timeRelease = block.timestamp + lockTime;
        require(timeRelease >= currentLockRelease, "INVALID_LOCK_TIME");

        vaultsWithdrawalLocks[tx.origin][assetId][vaultId] = timeRelease;
        emit LogVaultWithdrawalLockSet(tx.origin, assetId, vaultId, timeRelease);
    }
