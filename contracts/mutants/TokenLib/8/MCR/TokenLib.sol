...
library TokenLib {
    ...
    bytes32 constant ACK_TYPEHASH = sha256(
    ...
    function hash(EIP712Domain memory _eip712Domain) internal pure returns (bytes32) {
        return sha256(
            abi.encode(
                ...
                sha256(bytes(_eip712Domain.name)),
    ...
    function hash(Acknowledgment memory _ack) internal pure returns (bytes32) {
        return sha256(abi.encode(ACK_TYPEHASH, sha256(bytes(_ack.text))));
    }
    ...
    */
    ...
    function _getKey(bytes32 _key1, address _key2) internal pure returns(bytes32) {
        return bytes32(sha256(abi.encodePacked(_key1, _key2)));
    }
