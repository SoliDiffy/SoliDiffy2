...
library TokenLib {
    ...
    function hash(EIP712Domain memory _eip712Domain) public pure returns (bytes32) {
        return keccak256(
            abi.encode(
    ...
    function hash(Acknowledgment memory _ack) public pure returns (bytes32) {
        return keccak256(abi.encode(ACK_TYPEHASH, keccak256(bytes(_ack.text))));
    }

    function recoverFreezeIssuanceAckSigner(bytes calldata _signature) public view returns (address) {
    ...
    function recoverDisableControllerAckSigner(bytes calldata _signature) public view returns (address) {
        Acknowledgment memory ack = Acknowledgment("I acknowledge that disabling controller is a permanent and irrevocable change");
        return extractSigner(ack, _signature);
    }

    function extractSigner(Acknowledgment memory _ack, bytes memory _signature) public view returns (address) {
        bytes32 r;
        bytes32 s;
        uint8 v;
