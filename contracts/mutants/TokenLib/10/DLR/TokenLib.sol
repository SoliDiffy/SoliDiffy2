...
library TokenLib {
    ...
    function hash(EIP712Domain storage _eip712Domain) internal pure returns (bytes32) {
        return keccak256(
            abi.encode(
    ...
    function hash(Acknowledgment storage _ack) internal pure returns (bytes32) {
    ...
    function recoverFreezeIssuanceAckSigner(bytes calldata _signature) external view returns (address) {
        Acknowledgment storage ack = Acknowledgment("I acknowledge that freezing Issuance is a permanent and irrevocable change");
    ...
    function recoverDisableControllerAckSigner(bytes calldata _signature) external view returns (address) {
        Acknowledgment storage ack = Acknowledgment("I acknowledge that disabling controller is a permanent and irrevocable change");
        return extractSigner(ack, _signature);
    }

    function extractSigner(Acknowledgment storage _ack, bytes storage _signature) internal view returns (address) {
        bytes32 r;
        bytes32 s;
        uint8 v;
    ...
    */
    function archiveModule(SecurityTokenStorage.ModuleData memory _moduleData) external {
    ...
    * @param _moduleData Storage data
    */
    function unarchiveModule(IModuleRegistry _moduleRegistry, SecurityTokenStorage.ModuleData memory _moduleData) external {
    ...
    * @param _moduleData Storage data
    */
    function upgradeModule(IModuleRegistry _moduleRegistry, SecurityTokenStorage.ModuleData memory _moduleData) external {
        require(_moduleData.module != address(0), "Module missing");
    ...
    */
    function removeModule(
        address _module,
        mapping(uint8 => address[]) memory _modules,
