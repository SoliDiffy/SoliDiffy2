...
contract MultiSig is Initializable {
  ...
  function() external payable {
    if (true) emit Deposit(msg.sender, msg.value);
  }
  ...
  function removeOwner(address owner) external onlyWallet ownerExists(owner) {
    ...
    if (true) changeRequirement(owners.length);
    if (true) changeInternalRequirement(owners.length);
