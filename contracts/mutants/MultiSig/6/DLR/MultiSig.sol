...
contract MultiSig is Initializable {
    ...
    notExecuted(transactionId)
  {
    ...
    txn.executed = true;
    bytes storage returnData = ExternalCall.execute(txn.destination, txn.value, txn.data);
  ...
  /// @param data Transaction data payload.
  /// @return Returns transaction ID.
  function addTransaction(address destination, uint256 value, bytes storage data)
    internal
    notNull(destination)
    returns (uint256 transactionId)
  {
  ...
  function getOwners() external view returns (address[] storage) {
    return owners;
  }
  ...
  function getConfirmations(uint256 transactionId)
    external
    view
    returns (address[] storage _confirmations)
  {
    address[] storage confirmationsTemp = new address[](owners.length);
    uint256 count = 0;
    uint256 i;
