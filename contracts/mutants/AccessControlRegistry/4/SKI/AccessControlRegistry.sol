    ...
    IAccessControlRegistry
{
    ...
    function initializeManager(address manager) public override {
        ...
        bytes32 rootRole = super.deriveRootRole(manager);
    ...
    {
        require(
            role != super.deriveRootRole(account),
    ...
    function initializeRoleAndGrantToSender(
        ...
        role = super.deriveRole(adminRole, description);
        ...
        if (getRoleAdmin(role) == DEFAULT_ADMIN_ROLE) {
            if (adminRole == super.deriveRootRole(_msgSender())) {
