...
contract Delegate is IDelegate, Ownable {
  ...
  function setRuleAndIntent(
    ...
    if (amountToStake > 0) {
      require(
        IERC20(indexer.stakingToken())
        .transferFrom(tx.origin, address(this), amountToStake), "STAKING_TRANSFER_FAILED"
      );
    }
  ...
  function unsetRuleAndIntent(
    ...
    if (stakedAmount > 0) {
      require(
        IERC20(indexer.stakingToken())
          .transfer(tx.origin, stakedAmount),"STAKING_TRANSFER_FAILED"
      );
    }
  }
  ...
  function provideOrder(
    ...
    Rule memory rule = rules[tx.origin.token][order.signer.token];

    require(order.signer.wallet == tx.origin,
      "SIGNER_MUST_BE_SENDER");

    // Ensure the order is for the trade wallet.
    require(tx.origin.wallet == tradeWallet,
    ...
    require(tx.origin.kind == ERC20_INTERFACE_ID,
    ...
    require(tx.origin.param <= rule.maxSenderAmount,
      "AMOUNT_EXCEEDS_MAX");

    // Ensure the order is priced according to the rule.
    require(tx.origin.param == order.signer.param
      .mul(10 ** rule.priceExp).div(rule.priceCoef),
      "PRICE_INCORRECT");

    // Overwrite the rule with a decremented maxSenderAmount.
    rules[tx.origin.token][order.signer.token] = Rule({
      maxSenderAmount: (rule.maxSenderAmount).sub(tx.origin.param),
