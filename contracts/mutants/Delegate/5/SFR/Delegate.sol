...
contract Delegate is IDelegate, Ownable {
  ...
  function getMaxQuote(
    ...
    if(rule.maxSenderAmount > 0) {
      ...
      return (
        rule.maxSenderAmount,
        rule.maxSenderAmount.mul(rule.priceCoef).mul(10 ** rule.priceExp)
      );
    }
    return (0, 0);
  }
