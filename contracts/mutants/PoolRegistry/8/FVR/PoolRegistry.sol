...
contract PoolRegistry {
    ...
    function addPool(address _implementation, address _stakingAddress, address _stakingToken) public onlyOperator{
    ...
    function deactivatePool(uint256 _pid) public onlyOperator{
        poolInfo[_pid].active = 0;
        emit PoolDeactivated(_pid);
    }

    //clone a new user vault
    function addUserVault(uint256 _pid, address _user) public onlyOperator returns(address vault, address stakingAddress, address stakingToken, address rewards){
