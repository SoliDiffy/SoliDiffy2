...
contract VerifierChannel is Prng {
    ...
    function getPrngPtr(uint256 channelPtr) public pure returns (uint256) {
    ...
    */
    ...
    function readBytes(uint256 channelPtr, bool mix) public pure returns (bytes32) {
        uint256 proofPtr;
        bytes32 val;
    ...
    function readHash(uint256 channelPtr, bool mix) public pure returns (bytes32) {
    ...
    /*
      ...
      gas costs.
    */
    function readFieldElement(uint256 channelPtr, bool mix) public pure returns (uint256) {
        uint256 val = fromMontgomery(uint256(readBytes(channelPtr, mix)));

        return val;
    }

    function verifyProofOfWork(uint256 channelPtr, uint256 proofOfWorkBits) public pure {
