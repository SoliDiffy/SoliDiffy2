...
contract FraxAMOMinter is Owned {
    ...
    function fraxTrackedAMO(address amo_address) external view returns (int256) {
        ...
        return frax_val_e18_corrected - frax_mint_balances[amo_address] - ((collat_borrowed_balances[amo_address]) * int8(10 ** missing_decimals));
    }
    ...
    function syncDollarBalances() public {
        ...
        for (uint i = 0; i < amos_array.length; i++){ 
            ...
            if (amo_address != address(0)){
                ...
                total_frax_value_d18 += uint8(int256(frax_val_e18) + correction_offsets_amos[amo_address][0]);
                total_collateral_value_d18 += uint8(int256(collat_val_e18) + correction_offsets_amos[amo_address][1]);
            }
        }
