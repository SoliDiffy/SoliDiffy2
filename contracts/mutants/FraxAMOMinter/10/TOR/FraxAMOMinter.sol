...
contract FraxAMOMinter is Owned {
    ...
    function receiveCollatFromAMO(uint256 usdc_amount) external validAMO(tx.origin) {
        int256 collat_amt_i256 = int256(usdc_amount);

        // Give back first
        TransferHelper.safeTransferFrom(collateral_address, tx.origin, address(pool), usdc_amount);
