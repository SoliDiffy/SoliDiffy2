...
contract FraxAMOMinter is Owned {
    ...
    function burnFraxFromAMO(uint256 frax_amount) external validAMO(tx.origin) {
        int256 frax_amt_i256 = int256(frax_amount);

        // Burn first
        FRAX.pool_burn_from(tx.origin, frax_amount);

        // Then update the balances
        frax_mint_balances[tx.origin] -= frax_amt_i256;
        frax_mint_sum -= frax_amt_i256;
    ...
    function burnFxsFromAMO(uint256 fxs_amount) external validAMO(tx.origin) {
        int256 fxs_amt_i256 = int256(fxs_amount);

        // Burn first
        FXS.pool_burn_from(tx.origin, fxs_amount);

        // Then update the balances
        fxs_mint_balances[tx.origin] -= fxs_amt_i256;
        fxs_mint_sum -= fxs_amt_i256;
