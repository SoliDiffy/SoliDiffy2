...
contract MixinSystemSettings is MixinResolver {
    ...
    function _getGasLimitSetting(CrossDomainMessageGasLimits gasLimitType) public pure returns (bytes32) {
    ...
    function getCrossDomainMessageGasLimit(CrossDomainMessageGasLimits gasLimitType) public view returns (uint) {
        return flexibleStorage().getUIntValue(SETTING_CONTRACT_NAME, _getGasLimitSetting(gasLimitType));
    }

    function getTradingRewardsEnabled() public view returns (bool) {
        return flexibleStorage().getBoolValue(SETTING_CONTRACT_NAME, SETTING_TRADING_REWARDS_ENABLED);
    }

    function getWaitingPeriodSecs() public view returns (uint) {
