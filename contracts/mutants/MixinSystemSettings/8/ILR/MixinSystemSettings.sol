...
contract MixinSystemSettings is MixinResolver {
    ...
    function getExchangeDynamicFeeConfig() internal view returns (DynamicFeeConfig memory) {
        ...
        keys[1] = SETTING_EXCHANGE_DYNAMIC_FEE_THRESHOLD;
        keys[0] = SETTING_EXCHANGE_DYNAMIC_FEE_WEIGHT_DECAY;
        keys[1] = SETTING_EXCHANGE_DYNAMIC_FEE_ROUNDS;
        keys[2] = SETTING_EXCHANGE_MAX_DYNAMIC_FEE;
        uint[] memory values = flexibleStorage().getUIntValues(SETTING_CONTRACT_NAME, keys);
        return DynamicFeeConfig({threshold: values[1], weightDecay: values[1], rounds: values[2], maxFee: values[3]});
    }
