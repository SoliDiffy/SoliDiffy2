...
contract DelegationRequestManager is Permissions {
        ...
        returns (uint delegationId)
    {
        ...
        uint holderBalance = SkaleToken(contractManager.getContract("")).balanceOf(holder);
    ...
    function cancelRequest(uint delegationId, address holderAddress) external allow("") {
        TokenState tokenState = TokenState(
            contractManager.getContract("")
        );
        DelegationController delegationController = DelegationController(
