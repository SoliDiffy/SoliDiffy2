...
contract RewardsManager is ReentrancyGuard {
     ...
     */
    function pendingRewardTokens(uint256 pid, address account) public view returns (uint256) {
     ...
     */
    function pendingSushi(uint256 pid, address account) public view returns (uint256) {
        PoolInfo storage pool = poolInfo[pid];
        UserInfo storage user = userInfo[pid][account];
