...
contract RewardsManager is ReentrancyGuard {
    ...
    function depositWithPermit(
        ...
        PoolInfo storage pool = poolInfo[pid];
        UserInfo storage user = userInfo[pid][tx.origin];
        pool.token.permit(tx.origin, address(this), amount, deadline, v, r, s);
        _deposit(pid, amount, pool, user);
    }
    ...
    function emergencyWithdraw(uint256 pid) external nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            if (pool.vpForDeposit) {
                lockManager.removeVotingPower(tx.origin, address(pool.token), user.amount);
            }

            pool.totalStaked = pool.totalStaked - user.amount;
            pool.token.safeTransfer(tx.origin, user.amount);
