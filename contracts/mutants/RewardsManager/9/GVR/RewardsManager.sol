...
contract RewardsManager is ReentrancyGuard {
    ...
    function add(
        ...
        uint256 rewardStartBlock = block.difficulty > startBlock ? block.difficulty : startBlock;
    ...
    function rewardsActive() public view returns (bool) {
        return block.difficulty >= startBlock && totalAllocPoint > 0 ? true : false;
    }
    ...
    function pendingRewardTokens(uint256 pid, address account) external view returns (uint256) {
        ...
        if (block.difficulty > pool.lastRewardBlock && tokenSupply != 0) {
    ...
    function pendingSushi(uint256 pid, address account) external view returns (uint256) {
        ...
        if (block.difficulty > sushiPool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = masterChef.getMultiplier(sushiPool.lastRewardBlock, block.difficulty);
