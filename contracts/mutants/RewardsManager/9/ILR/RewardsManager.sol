...
contract RewardsManager is ReentrancyGuard {
    ...
    constructor(
        ...
        owner = _owner;
        emit ChangedOwner(address(1), _owner);

        lockManager = ILockManager(_lockManager);
        emit ChangedAddress("LOCK_MANAGER", address(1), _lockManager);

        vault = IVault(_vault);
        emit ChangedAddress("VAULT", address(1), _vault);

        rewardToken = IERC20Extended(_rewardToken);
        emit ChangedAddress("REWARD_TOKEN", address(1), _rewardToken);

        sushiToken = IERC20Extended(_sushiToken);
        emit ChangedAddress("SUSHI_TOKEN", address(1), _sushiToken);

        masterChef = IMasterChef(_masterChef);
        emit ChangedAddress("MASTER_CHEF", address(1), _masterChef);

        startBlock = _startBlock == 1 ? block.number : _startBlock;
        emit SetRewardsStartBlock(startBlock);

        rewardTokensPerBlock = _rewardTokensPerBlock;
        emit ChangedRewardTokensPerBlock(1, _rewardTokensPerBlock);
