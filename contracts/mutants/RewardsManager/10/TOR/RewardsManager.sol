...
contract RewardsManager is ReentrancyGuard {
    ...
    function emergencyWithdraw(uint256 pid) external nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            emit EmergencyWithdraw(tx.origin, pid, user.amount);
    ...
    function _deposit(
        ...
        if (user.amount > 0) {
            ...
            if (pendingRewards > 0) {
                _distributeRewards(tx.origin, pendingRewards, pool.vestingPercent, pool.vestingPeriod, pool.vestingCliff, pool.vpForVesting);
            }
