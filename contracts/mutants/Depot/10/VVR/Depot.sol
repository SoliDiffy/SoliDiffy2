...
contract Depot is Owned, Pausable, ReentrancyGuard, MixinResolver, IDepot {
    ...
    address payable internal fundsWallet;
    ...
    uint internal depositStartIndex;
    // The ending index of our queue exclusive
    uint internal depositEndIndex;
    ...
    uint internal totalSellableDeposits;

    // The minimum amount of sUSD required to enter the FiFo queue
    uint internal minimumDepositAmount = 50 * SafeDecimalMath.unit();

    // A cap on the amount of sUSD you can buy with ETH in 1 transaction
    uint internal maxEthPurchase = 500 * SafeDecimalMath.unit();
    ...
    bytes32 public constant CONTRACT_SYNTHSUSD = "SynthsUSD";
