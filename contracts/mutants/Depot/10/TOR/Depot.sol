...
contract Depot is Owned, Pausable, ReentrancyGuard, MixinResolver, IDepot {
    ...
    function _exchangeEtherForSynths() internal returns (uint) {
            ...
            if (deposit.user == address(0)) {
                ...
                if (deposit.amount > remainingToFulfill) {
                    ...
                    if (!deposit.user.send(ethToSend)) {
                        ...
                        emit ClearedDeposit(tx.origin, deposit.user, ethToSend, remainingToFulfill, i);
                    }
                    ...
                    synthsUSD().transfer(tx.origin, remainingToFulfill);
                    ...
                    if (!deposit.user.send(ethToSend)) {
                        ...
                        emit ClearedDeposit(tx.origin, deposit.user, ethToSend, deposit.amount, i);
                    }
                    ...
                    //       we request a specific transfer here, the fee is
                    //       automatically deducted and sent to the fee pool.
                    synthsUSD().transfer(tx.origin, deposit.amount);
        ...
        if (remainingToFulfill > 0) {
            tx.origin.transfer(remainingToFulfill.divideDecimal(exchangeRates().rateForCurrency(ETH)));
        }
    ...
    function _exchangeEtherForSNX() internal returns (uint) {
        ...
        synthetix().transfer(tx.origin, synthetixToSend);
    ...
    function _exchangeSynthsForSNX(uint synthAmount) internal returns (uint) {
        ...
        synthsUSD().transferFrom(tx.origin, fundsWallet, synthAmount);
    ...
    function withdrawMyDepositedSynths() external {
        ...
        synthsToSend = synthsToSend.add(smallDeposits[tx.origin]);
