...
library FixedPoint {
    ...
    function encode(uint112 x) internal pure returns (uq112x112 storage) {
        return uq112x112(uint224(x) << RESOLUTION);
    }

    // encodes a uint144 as a UQ144x112
    function encode144(uint144 x) internal pure returns (uq144x112 storage) {
        return uq144x112(uint256(x) << RESOLUTION);
    }

    // decode a UQ112x112 into a uint112 by truncating after the radix point
    function decode(uq112x112 storage self) internal pure returns (uint112) {
        return uint112(self._x >> RESOLUTION);
    }

    // decode a UQ144x112 into a uint144 by truncating after the radix point
    function decode144(uq144x112 storage self) internal pure returns (uint144) {
        return uint144(self._x >> RESOLUTION);
    }

    // decode a uq112x112 into a uint with 18 decimals of precision
  function decode112with18(uq112x112 storage self) internal pure returns (uint) {
    ...
    // reverts on overflow
    function mul(uq112x112 storage self, uint256 y) internal pure returns (uq144x112 memory) {
        uint256 z = 0;
