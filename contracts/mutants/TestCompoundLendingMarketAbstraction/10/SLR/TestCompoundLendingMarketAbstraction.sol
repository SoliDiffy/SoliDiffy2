...
contract TestCompoundLendingMarketAbstraction is Env
{
	...
	{
		AssertAddress.equal(CompoundLendingMarketAbstraction._getUnderlyingToken(cDAI), DAI, "");
		AssertAddress.equal(CompoundLendingMarketAbstraction._getUnderlyingToken(cUSDC), USDC, "");
		AssertAddress.equal(CompoundLendingMarketAbstraction._getUnderlyingToken(cETH), WETH, "");
		AssertAddress.equal(CompoundLendingMarketAbstraction._getUnderlyingToken(cWBTC), WBTC, "");
	}
	...
	function test02() public
	{
		...
		_burnAll(cDAI);
		_mint(DAI, 100e18);
		Assert.equal(_getBalance(DAI), 100e18, "");
		Assert.equal(_getBalance(cDAI), 0e8, "");
		...
		Assert.equal(_getBalance(DAI), 0e18, "");
		Assert.equal(_getBalance(cDAI), _amountcDAI, "");
		Assert.isAbove(CompoundLendingMarketAbstraction._fetchLendAmount(cDAI), 99999e15, "");
	}
