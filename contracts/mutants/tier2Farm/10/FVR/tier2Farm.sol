...
contract Tier2FarmController{
  ...
  function addOrEditStakingContract(string memory name, address stakingAddress, address stakingToken ) external onlyOwner returns (bool){
  ...
  function updateCommission(uint amount) external onlyOwner returns(bool){
      commission = amount;
      return true;
  }

  function deposit(address tokenAddress, uint256 amount, address onBehalfOf) payable onlyOwner external returns (bool){
   ...
   function stake(uint256 amount, address onBehalfOf, address tokenAddress) public returns(bool){
   ...
   function unstake(uint256 amount, address onBehalfOf, address tokenAddress) public returns(bool){
      StakingInterface staker  =  StakingInterface(tokenToFarmMapping[tokenAddress]);
      staker.exit();
      return true;
