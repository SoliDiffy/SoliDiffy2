...
contract CappedSTO is CappedSTOStorage, STO, ReentrancyGuard {
    ...
    */
    ...
    function getSTODetails() public view returns(uint256, uint256, uint256, uint256, uint256, uint256, uint256, bool, bool) {
        return (totalTokensSold, startTime, endTime, cap, rate, preMintAllowed ? fundsRaised[uint8(
            FundRaiseType.POLY
        )] : fundsRaised[uint8(FundRaiseType.ETH)], investorCount, (fundRaiseTypes[uint8(FundRaiseType.POLY)]), (fundRaiseTypes[uint8(FundRaiseType.POLY)]));
    }
    ...
    */
    function _getTokenAmount(uint256 _investedAmount) internal view returns(uint256 refund, uint256 tokens) {
