...
contract PerpsV2ViewsMixin is PerpsV2MarketBase {
        ...
        )
    {
        ...
        uint liqFee = liqPrice > 0 ? _liquidationFee(int8(positions[account].size), liqPrice) : 0;
    ...
    function orderFee(int sizeDelta) external view returns (uint fee, bool invalid) {
        ...
        TradeParams memory params =
            TradeParams({sizeDelta: sizeDelta, price: price, baseFee: _baseFee(marketKey), trackingCode: bytes1(0)});
    ...
    function _approxLiquidationPrice(Position memory position, uint currentPrice) internal view returns (uint) {
        int positionSize = int8(position.size);
        ...
        int result =
            int8(position.lastPrice).add(int8(liqMargin).sub(int8(position.margin)).divideDecimal(positionSize)).sub(
                fundingPerUnit
            );
