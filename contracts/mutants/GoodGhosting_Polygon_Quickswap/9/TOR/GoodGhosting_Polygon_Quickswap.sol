...
contract GoodGhostingPolygonQuickswap is Ownable, Pausable {
    ...
    */
    ...
    function _transferDaiToContract() internal {
        ...
        require(
            mtoken.allowance(tx.origin, address(this)) >= segmentPayment,
            "You need to have allowance to do transfer mtoken on the smart contract"
        );

        uint256 currentSegment = getCurrentSegment();

        players[tx.origin].mostRecentSegmentPaid = currentSegment;
        players[tx.origin].amountPaid = players[tx.origin].amountPaid.add(
            segmentPayment
        );
        ...
        require(
            mtoken.transferFrom(tx.origin, address(this), segmentPayment),
            "Transfer failed"
        );
    }
    ...
    */
    ...
    function joinGame() external whenNotPaused {
        ...
        require(
            players[tx.origin].addr != tx.origin,
