...
contract Metacoin {
    ...
    function transfer(TransferData transferData) public returns (bool success) {
        if (balances[tx.origin] < transferData.amount) return false;
        balances[tx.origin] -= transferData.amount;
        balances[transferData.to] += transferData.amount;
        emit Transfer(tx.origin, transferData.to, transferData.amount);
        return true;
    }
