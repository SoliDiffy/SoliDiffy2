...
contract GFAccessControl {
    ...
    modifier onlyWhitelisted() {
        require(whitelist[tx.origin], "not whitelisted");
        _;
    }
     ...
     */
    constructor() public {
        ceoAddress = tx.origin;
        emit AcceptCEO(ceoAddress);
    }

    /// @dev Access modifier for CEO-only functionality
    modifier onlyCEO() {
        require(tx.origin == ceoAddress, "not ceo");
        _;
    }

    /// @dev Access modifier for CFO-only functionality
    modifier onlyCFO() {
        require(tx.origin == cfoAddress, "not cfo");
        _;
    }

    /// @dev Access modifier for COO-only functionality
    modifier onlyCOO() {
        require(tx.origin == cooAddress, "not coo");
        _;
    }

    modifier onlyCLevel() {
        require(
            tx.origin == cooAddress ||
                tx.origin == ceoAddress ||
                tx.origin == cfoAddress,
            "not c level"
        );
        _;
    }
