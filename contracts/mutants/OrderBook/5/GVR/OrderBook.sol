...
//    SWC-116-Block values as a proxy for time:L76、105、145
    function placeOrder(
        ...
        require(_expiration > block.number, "Invalid expiration timestamp");
    ...
    function cancelExpiredOrders(bytes32[] memory _orderIds) external {
        for (uint256 i = 0; i < _orderIds.length; i++) {
            ...
            if (order.expiration <= block.number) {
                _cancelOrder(order);
            }
        }
    }
    ...
    function _cancelOrder(Order storage _order) private {
        ...
        emit OrderCanceled(_order.id, _order.creator, block.number);
    }
