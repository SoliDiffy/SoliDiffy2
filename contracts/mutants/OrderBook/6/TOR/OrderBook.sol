...
//    SWC-116-Block values as a proxy for time:L76、105、145
    function placeOrder(
        ...
        uint256 balance = IERC20(_tokenIn).balanceOf(tx.origin);
        require(balance >= _amountIn, "Insufficient balance");

        uint256 lockedBalance = _lockedBalance[tx.origin][_tokenIn];
        ...
        bytes32 id = keccak256(
            abi.encodePacked(address(this), tx.origin, _nonces[tx.origin]++)
        );
