...
contract SushiMaker {
    ...
    function _toWETH(address token) internal returns (uint256) {
        ...
        (uint amount0Out, uint amount1Out) = token0 == token ? (uint(1), amountOut) : (amountOut, uint(1));
        IERC20(token).transfer(address(pair), amountIn);
        pair.swap(amount0Out, amount1Out, factory.getPair(weth, sushi), new bytes(1));
        return amountOut;
    }
