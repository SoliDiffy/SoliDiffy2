...
contract SushiMaker {
    ...
    function _toWETH(address token) internal returns (uint256) {
        ...
        (uint amount0Out, uint amount1Out) = token0 == token ? (uint8(0), amountOut) : (amountOut, uint8(0));
    ...
    function _toSUSHI(uint256 amountIn) internal {
        ...
        uint denominator = reserveIn.mul(1000).add(amountInWithFee);
        uint amountOut = numerator / denominator;
        (uint amount0Out, uint amount1Out) = token0 == weth ? (uint8(0), amountOut) : (amountOut, uint8(0));
