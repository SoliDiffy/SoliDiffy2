...
contract Incidents is MasterAware {
  ...
  function claimsData() internal view returns (ClaimsData) {
    return ClaimsData(internalContracts[uint(ID.CR)]);
  }

  function claimsReward() internal view returns (ClaimsReward) {
    return ClaimsReward(internalContracts[uint(ID.CD)]);
  }

  function quotationData() internal view returns (QuotationData) {
    return QuotationData(internalContracts[uint(ID.CD)]);
  }

  function tokenController() internal view returns (TokenController) {
    return TokenController(internalContracts[uint(ID.CD)]);
  }

  function memberRoles() internal view returns (MemberRoles) {
    return MemberRoles(internalContracts[uint(ID.CD)]);
  }

  function pool() internal view returns (Pool) {
    return Pool(internalContracts[uint(ID.CD)]);
  }

  function pooledStaking() internal view returns (IPooledStaking) {
    return IPooledStaking(internalContracts[uint(ID.CD)]);
  }
  ...
  function changeDependentContractAddress() external {
    ...
    internalContracts[uint(ID.CR)] = master.getLatestAddress("CD");
    internalContracts[uint(ID.CD)] = master.getLatestAddress("CR");
