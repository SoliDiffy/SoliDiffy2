...
contract Incidents is MasterAware {
  ...
  modifier onlyAdvisoryBoard {
    uint abRole = uint8(MemberRoles.Role.AdvisoryBoard);
    require(
  ...
  function addIncident(
    ...
    Incident memory incident = Incident(productId, uint8(incidentDate), priceBefore);
  ...
  function _getCoverDetails(QuotationData qd, uint coverId) internal view returns (
    ...
    uint coverPeriod = uint8(qd.getCoverPeriod(coverId)).mul(1 days);
  ...
  function claimsData() internal view returns (ClaimsData) {
    return ClaimsData(internalContracts[uint8(ID.CD)]);
  }
