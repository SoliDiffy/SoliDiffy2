...
contract Incidents is MasterAware {
  ...
  function initialize(uint burnRate) public {
  ...
  ) public returns (uint claimId, uint payoutAmount) {
  ...
  function pushBurns(address productId, uint iterations) public {
  ...
  function withdrawAsset(address asset, address destination, uint amount) public onlyGovernance {
  ...
  function _getCoverDetails(QuotationData qd, uint coverId) public view returns (
    address productId,
    address coverOwner,
    uint coverStartDate,
    uint coverExpirationDate,
