...
contract Incidents is MasterAware {
  ...
  function initialize(uint burnRate) external onlyAdvisoryBoard {
  ...
  ) external onlyAdvisoryBoard returns (uint claimId, uint payoutAmount) {
  ...
  function pushBurns(address productId, uint iterations) external onlyAdvisoryBoard {
