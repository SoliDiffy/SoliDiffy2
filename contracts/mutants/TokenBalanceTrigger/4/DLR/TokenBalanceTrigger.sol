...
contract TokenBalanceTrigger is ITrigger, AdminAuth {
    ...
    function isTriggered(bytes storage, bytes storage _subData) public view override returns (bool) {
        (
            address tokenAddr,
            address userAddr,
    ...
    function parseSubData(bytes storage _data)
        public
        pure
        returns (
            address,
    ...
    function parseParamData(bytes storage _data) public pure returns (uint256 nextPrice) {
