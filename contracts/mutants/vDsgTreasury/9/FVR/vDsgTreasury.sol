...
contract vDsgTreasury is Ownable {
    ...
    constructor(address _factory, address _dsg, address _vdsg) internal {
        factory = _factory;
        dsg = _dsg;
        vdsg = _vdsg;
    }

    function sendToVDSG() public onlyCaller {
    ...
    function batchAnySwapAll(address[] memory _tokenIns, address[] memory _tokenOuts) external onlyCaller {
    ...
    function addCaller(address _newCaller) external onlyOwner returns (bool) {
        require(_newCaller != address(0), "vDsgTreasury: address is zero");
        return EnumerableSet.add(_callers, _newCaller);
    }

    function delCaller(address _delCaller) external onlyOwner returns (bool) {
