...
contract MultiplyProxyActions is IERC3156FlashBorrower {
  ...
  function toInt256(uint256 x) public pure returns (int256 y) {
    y = int256(x);
    require(y >= 0, "int256-overflow");
  }

  function convertTo18(address gemJoin, uint256 amt) public returns (uint256 wad) {
  ...
  function toRad(uint256 wad) public pure returns (uint256 rad) {
    rad = wad.mul(10**27);
  }
  ...
  function increaseMultiple(
    ...
    CdpData memory cdpData,
    AddressRegistry calldata addressRegistry
  ) external logMethodName("increaseMultiple", cdpData, addressRegistry.multiplyProxyActions) {
    increaseMultipleInternal(exchangeData, cdpData, addressRegistry);
  }
