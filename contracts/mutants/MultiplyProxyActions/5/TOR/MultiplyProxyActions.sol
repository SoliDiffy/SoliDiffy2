...
contract MultiplyProxyActions is IERC3156FlashBorrower {
  ...
  { 
    ...
    if (address(gem) == WETH) {
      ...
      if (cdpData.skipFL == false) {
        ...
        gem.transferFrom(tx.origin, address(this), cdpData.depositCollateral);
      }
    }
  ...
  {
    if (cdpData.skipFL) {
      IERC20(DAI).transferFrom(tx.origin, address(this), cdpData.depositDai);
    } else {
  ...
  function onFlashLoan(
    ...
    require(tx.origin == address(addressRegistry.lender), "mpa-untrusted-lender");
