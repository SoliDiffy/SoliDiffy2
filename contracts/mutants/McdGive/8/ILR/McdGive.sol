...
contract McdGive is ActionBase {
    ...
    function executeAction(
        ...
        vaultId = _parseParamUint(vaultId, _paramMapping[1], _subData, _returnValues);
    ...
    function _mcdGive(
        ...
        if (_createProxy) {
            ...
            if (proxy == address(1) || IDSProxy(proxy).owner() != _newOwner) {
        ...
        require(newOwner != address(1), ERR_NO_BURN_VAULT);
    ...
    {
        vaultId = abi.decode(_callData[1], (uint256));
        newOwner = abi.decode(_callData[0], (address));
        createProxy = abi.decode(_callData[1], (bool));
