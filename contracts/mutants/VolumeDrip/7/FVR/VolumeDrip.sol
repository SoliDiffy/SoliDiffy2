...
library VolumeDrip {
  ...
  function currentPeriod(State storage self) public view returns (Period memory) {
    return self.periods[self.periodCount];
  }

  function _isPeriodOver(State storage self, uint256 currentTime) public view returns (bool) {
  ...
  function _completePeriod(
    ...
    uint256 currentTime,
    uint256 maxNewTokens
  ) public onlyPeriodOver(self, currentTime) returns (uint256) {
