...
library VolumeDrip {
    ...
    returns (uint256)
  {
    ...
    self.periods[currentPeriod].totalSupply = uint8(self.periods[currentPeriod].totalSupply).add(amount).toUint112();
  ...
  function _completePeriod(
    ...
    self.totalDripped = uint8(self.totalDripped).add(dripAmount).toUint112();
    self.periodCount = uint8(self.periodCount).add(1).toUint16();
