...
library VolumeDrip {
  ...
  function currentPeriod(State memory self) internal view returns (Period storage) {
    return self.periods[self.periodCount];
  }

  function _isPeriodOver(State memory self, uint256 currentTime) private view returns (bool) {
