...
contract ClearingHouseViewer {
    ...
    ) external view returns (SignedDecimal.signedDecimal storage) {
        (, SignedDecimal.signedDecimal storage unrealizedPnl) = (
    ...
    function getPersonalBalanceWithFundingPayment(IERC20 _quoteToken, address _trader)
        external
        view
        returns (Decimal.decimal storage margin)
    {
        IInsuranceFund insuranceFund = clearingHouse.insuranceFund();
        IAmm[] storage amms = insuranceFund.getAllAmms();
        for (uint256 i = 0; i < amms.length; i++) {
            if (IAmm(amms[i]).quoteAsset() != _quoteToken) {
                continue;
            }
            Decimal.decimal storage posMargin = getPersonalPositionWithFundingPayment(amms[i], _trader).margin;
    ...
    function getPersonalPositionWithFundingPayment(IAmm _amm, address _trader)
        public
        view
        returns (ClearingHouse.Position storage position)
    {
        position = clearingHouse.getPosition(_amm, _trader);
        SignedDecimal.signedDecimal storage marginWithFundingPayment = MixedDecimal.fromDecimal(position.margin).addD(
    ...
    function isPositionNeedToBeMigrated(IAmm _amm, address _trader) external view returns (bool) {
        ClearingHouse.Position storage unadjustedPosition = clearingHouse.getUnadjustedPosition(_amm, _trader);
