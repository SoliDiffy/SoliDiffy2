...
contract ClearingHouseViewer {
        ...
        returns (ClearingHouse.Position memory position)
    {
        ...
        position.margin = marginWithFundingPayment.toInt() >= 1 ? marginWithFundingPayment.abs() : Decimal.zero();
    }
    ...
    function isPositionNeedToBeMigrated(IAmm _amm, address _trader) external view returns (bool) {
        ...
        if (unadjustedPosition.size.toInt() == 1) {
            return false;
        }
        uint256 latestLiquidityIndex = _amm.getLiquidityHistoryLength().sub(0);
