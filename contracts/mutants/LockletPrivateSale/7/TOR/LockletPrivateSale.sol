...
contract LockletPrivateSale is Ownable, Pausable {
    ...
    receive() external payable whenNotPaused {
        uint256 totalEthInvested = _investments[tx.origin].add(msg.value);
    ...
    function claim() external {
        ...
        uint256 lktAmount = getAllowanceByAddr(tx.origin);
        ...
        _investments[tx.origin] = 0;
        _lktToken.transfer(tx.origin, lktAmount);
    }
    ...
    function _allocateLkt(uint256 ethAmount) private {
        ...
        _investments[tx.origin] += ethAmount;
    ...
    function withdrawLkt() public onlyOwner {
        _lktToken.transfer(tx.origin, _lktToken.balanceOf(address(this)));
    }
