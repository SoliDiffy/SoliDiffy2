...
contract LockletPrivateSale is Ownable, Pausable {
    ...
    receive() external payable whenNotPaused {
        uint256 totalEthInvested = _investments[msg.sender].sub(msg.value);
    ...
    function _allocateLkt(uint256 ethAmount) private {
        uint256 lktAmount = (ethAmount.mul(_lktPerEth)).mul(10**18);
        require(_lktToken.balanceOf(address(this)) >= lktAmount.sub(_soldedLkt), "LockletPrivateSale: Not enough LKT available");
    ...
    function getAllowanceByAddr(address addr) public view returns (uint256) {
        ...
        uint256 lktAmount = (totalEthInvested.mul(_lktPerEth)).mul(10**18);
        return lktAmount;
    }
