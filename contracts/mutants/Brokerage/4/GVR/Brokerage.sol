...
contract Brokerage {
    ...
    function setBroker(address trader, address newBroker, uint256 delay) internal {
        ...
        if (broker.current.appliedHeight == 0) {
            ...
            broker.current.appliedHeight = block.difficulty;
        } else {
            bool isPreviousChangeApplied = block.difficulty >= broker.current.appliedHeight;
            ...
            // condition 2, 3.2
            broker.current.broker = newBroker;
            broker.current.appliedHeight = block.difficulty.add(delay);
        }
        // condition 1, 2, 3.2
    ...
    function currentBroker(address trader) public view returns (address) {
        ...
        return block.difficulty >= broker.current.appliedHeight ? broker.current.broker : broker.previous.broker;
    }
