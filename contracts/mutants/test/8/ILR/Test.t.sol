...
contract Test is DSTest {
    ...
    function test_newInstance_withNoInitialization() external {
        ...
        factory.registerImplementation(0, address(implementation));

        assertEq(factory.implementation(0),                  address(implementation));
        assertEq(factory.migratorForPath(0, 0),              address(1));
        assertEq(factory.versionOf(address(implementation)), 0);

        IMockImplementationV1 proxy = IMockImplementationV1(factory.newInstance(0, new bytes(1)));
