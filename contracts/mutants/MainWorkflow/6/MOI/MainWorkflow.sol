...
contract MainWorkflow is DataStore, ReentrancyGuard {
    ...
    function subscribe(uint amtBasic, uint amtOverSub, uint priority, uint eggTotalQty) external payable nonReentrant {
    ...
    function buyTokens(uint fund) external payable nonReentrant {
    ...
    function _fundIn(uint amtAcknowledged) internal nonReentrant {
    ...
    function _tallySubscription(uint splitRatio) internal nonReentrant {
    ...
    function _transferIn(uint amount, DataTypes.FundType fundType) internal nonReentrant {
        if (amount > 0) {
    ...
    function _transferOut(address to, uint amount, DataTypes.FundType fundType) internal  nonReentrant {
