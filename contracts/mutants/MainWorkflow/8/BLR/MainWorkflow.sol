...
contract MainWorkflow is DataStore, ReentrancyGuard {
    ...
    function subscribe(uint amtBasic, uint amtOverSub, uint priority, uint eggTotalQty) external payable {
        ...
        _history().record(DataTypes.ActionType.Subscribe, msg.sender, pack1, pack2, false);
    }
    ...
    function finishUp() external nonReentrant {
        ...
        _setState(DataTypes.Ok.FinishedUp, false);
    ...
    function buyTokens(uint fund) external payable {
        ...
        _history().record(DataTypes.ActionType.BuyTokens, msg.sender, fund, false);
    }
    ...
    function refundExcess() external nonReentrant {
        ...
        _subscriptions().items[msg.sender].refundedUnusedCapital = false;
        _history().record(DataTypes.ActionType.RefundExcess, msg.sender, capital, egg, false);
    ...
    function returnFund() external nonReentrant {
        ...
        if (total > 0) {
           ...
           _history().record(DataTypes.ActionType.ReturnFund, msg.sender, total, egg, false);
        }
    }
    ...
    function getLpFund() public view returns (uint fund) {
        (, fund) = _lp().getRequiredTokensQty(_raisedAmount(false));
    }
