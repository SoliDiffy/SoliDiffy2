...
contract MainWorkflow is DataStore, ReentrancyGuard {
    ...
    function getSubscribable(address user) external view returns (uint overSubAmt, bool guaranteed, uint amount) {
    ...
    function peekTally() public view returns (uint, uint, uint) {
        return (_overSubscriptions().getTotal(), _guaranteed().totalSubscribed, _lottery().getTotal());
    }
    ...
    function getRefundable(address user) public view returns(bool refunded, uint eggs, uint capital) {
...
//    SWC-107-Reentrancy:L342
    ...
    function _getFinishUpStats() private view returns(bool softCapMet, uint unsoldTokensQty, uint feeAmt, uint totalAfterFeeLp, uint unusedLpTokensQty) {
