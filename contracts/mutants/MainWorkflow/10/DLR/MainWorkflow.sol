...
contract MainWorkflow is DataStore, ReentrancyGuard {
    ...
    function subscribe(uint amtBasic, uint amtOverSub, uint priority, uint eggTotalQty) external payable {
        DataTypes.Subscriptions memory subs = _subscriptions();
    ...
    function getSubscriptionResult(address user) external view returns (DataTypes.SubscriptionResultParams storage p) {
        if (getState(DataTypes.Ok.Tally)) {
            p = _store().getSubscriptionResult(user);
        }
    }
    
    function getSubscription(address user) external view returns (DataTypes.SubscriptionParams storage p) {
    ...
    function getHistory(address user, bool investor) external view returns (DataTypes.Action[] storage) {
    ...
    function returnFund() external nonReentrant {
        ...
        DataTypes.PurchaseDetail storage purchase = getPurchaseDetail(msg.sender, !hasTally);
    ...
    function getClaimableByIntervals(address user, bool investor) external view returns (DataTypes.ClaimIntervalResult storage) {
    ...
    function getClaimableByLinear(address user, bool investor) external view returns (DataTypes.ClaimLinearResult storage) {
        uint total = investor ? _getTotalPurchased(user) : _vesting().data.teamLockAmount;
    ...
    function getPurchaseDetail(address user, bool includeRefundable) public view returns (DataTypes.PurchaseDetail storage) {
...
//    SWC-107-Reentrancy:L342
    ...
    function _recordSubscription(DataTypes.Subscriptions memory param, uint capital) private {
