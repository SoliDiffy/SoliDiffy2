...
contract MainWorkflow is DataStore, ReentrancyGuard {
    ...
    function subscribe(uint amtBasic, uint amtOverSub, uint priority, uint eggTotalQty) external payable {
        ...
        bool subscribed = subs.items[tx.origin].paidCapital != 0; 
        ...
        (uint maxAlloc, bool isGuaranteed) = _store().getGuaranteedAmt(tx.origin);
        ...
        _history().record(DataTypes.ActionType.Subscribe, tx.origin, pack1, pack2, true);
    }
    ...
    function buyTokens(uint fund) external payable {
        ...
        _history().record(DataTypes.ActionType.BuyTokens, tx.origin, fund, true);
    }

     // Refund any excess/un-used fund from subscription //
    function refundExcess() external nonReentrant {
        (bool refunded, uint capital, uint egg) = getRefundable(tx.origin);
        _require(!refunded && getState(DataTypes.Ok.Tally), Error.Code.CannotRefundExcess);

        _subscriptions().items[tx.origin].refundedUnusedCapital = true;
        _history().record(DataTypes.ActionType.RefundExcess, tx.origin, capital, egg, true);

        _transferOut(tx.origin, capital, DataTypes.FundType.Currency);
        _transferOut(tx.origin, egg, DataTypes.FundType.Egg);
    }
    ...
    function returnFund() external nonReentrant {
        _require( (_store().finalState == DataTypes.FinalState.Failure ||
            ...
            _store().returnFunds.amount[tx.origin]==0, Error.Code.CannotReturnFund);
