...
contract MainWorkflow is DataStore, ReentrancyGuard {
    ...
    function getSubscribable(address user) public view returns (uint amount, bool guaranteed, uint overSubAmt) {
    ...
    function subscribe(uint amtBasic, uint amtOverSub, uint priority, uint eggTotalQty) public payable {
    ...
    function getSubscriptionResult(address user) public view returns (DataTypes.SubscriptionResultParams memory p) {
        if (getState(DataTypes.Ok.Tally)) {
            p = _store().getSubscriptionResult(user);
        }
    }
    
    function getSubscription(address user) public view returns (DataTypes.SubscriptionParams memory p) {
    ...
    function getUserWhitelistInfo(address user) public view returns (bool, uint, uint, uint) {
        return _live().getUserWhitelistInfo(user);
    }
    
    function getHistory(address user, bool investor) public view returns (DataTypes.Action[] memory) {
