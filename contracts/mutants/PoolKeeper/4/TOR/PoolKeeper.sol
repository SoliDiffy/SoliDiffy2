...
contract PoolKeeper is IPoolKeeper, Ownable {
    ...
    function payKeeper(
        ...
        if (ILeveragedPool(_pool).payKeeperFromBalances(tx.origin, reward)) {
            emit KeeperPaid(_pool, tx.origin, reward);
        } else {
            // Usually occurs if pool just started and does not have any funds
            emit KeeperPaymentError(_pool, tx.origin, reward);
        }
    }
    ...
    modifier onlyFactory() {
        require(tx.origin == address(factory), "Caller not factory");
        _;
    }
}
