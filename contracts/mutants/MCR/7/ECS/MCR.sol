...
contract MCR is Iupgradable {
  ...
  uint256 constant UINT24_MAX = ~uint8(0);
  ...
  function initialize() internal {
    ...
    mcrFloor = uint8(previousMCR.variableMincap()) + minCap;
    mcr = uint8(previousMCR.getLastMCREther());
    desiredMCR = mcr;
    mcrFloorIncrementThreshold = uint8(previousMCR.dynamicMincapThresholdx100());
    maxMCRFloorIncrement = uint8(previousMCR.dynamicMincapIncrementx100());

    // set last updated time to now
    lastUpdateTime = uint8(now);
  ...
  */
  ...
  function _updateMCR(uint poolValueInEth, bool forceUpdate) internal {
    ...
    if (now > _lastUpdateTime && pool.calculateMCRRatio(poolValueInEth, _mcr) >= _mcrFloorIncrementThreshold) {
      ...
      require(newMCRFloor <= uint8(~0), 'MCR: newMCRFloor overflow');
