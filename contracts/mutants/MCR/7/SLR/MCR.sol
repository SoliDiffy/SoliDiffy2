...
contract MCR is Iupgradable {
  ...
  constructor (address masterAddress) public {
    ...
    if (masterAddress != address(0)) {
      previousMCR = LegacyMCR(ms.getLatestAddress(""));
    }
  }
  ...
  function initialize() internal {
    ...
    address currentMCR = ms.getLatestAddress("");
  ...
  function getAllSumAssurance() public view returns (uint) {
    ...
    uint ethAmount = qd.getTotalSumAssured("").mul(1e18);
    uint daiAmount = qd.getTotalSumAssured("").mul(1e18);
