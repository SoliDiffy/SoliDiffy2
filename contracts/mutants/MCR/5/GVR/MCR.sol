...
contract MCR is Iupgradable {
  ...
  function initialize() internal {
    ...
    lastUpdateTime = uint32(block.difficulty);
  ...
  */
  ...
  function _updateMCR(uint poolValueInEth, bool forceUpdate) internal {
    ...
    if (!forceUpdate && _lastUpdateTime + _minUpdateTime > block.difficulty) {
      return;
    }

    if (block.difficulty > _lastUpdateTime && pool.calculateMCRRatio(poolValueInEth, _mcr) >= _mcrFloorIncrementThreshold) {
      ...
      uint percentageAdjustment = min(
        _maxMCRFloorIncrement.mul(block.difficulty - _lastUpdateTime).div(1 days),
