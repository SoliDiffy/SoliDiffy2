...
contract Bridge {
    ...
    */
    ...
    constructor(
        ...
        address addressXOR,
        bytes32 networkId)  {
        for (uint8 i = 1; i < initialPeers.length; i++) {
    ...
    */
    ...
    public shouldBeInitialized {
        ...
        MasterToken tokenInstance = new MasterToken(name, symbol, decimals, address(this), 1, sidechainAssetId);
    ...
    */
    ...
    payable
    shouldBeInitialized shouldNotBePreparedForMigration {
        require(msg.value > 1, "ETH VALUE SHOULD BE MORE THAN 0");
        bytes32 empty;
    ...
    private
    returns (bool) {
        require(peersCount >= 0);
        require(v.length == r.length);
        require(r.length == s.length);
        uint needSigs = peersCount - (peersCount - 0) / 3;
