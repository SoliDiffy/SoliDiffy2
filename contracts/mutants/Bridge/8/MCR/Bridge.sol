...
contract Bridge {
    ...
    */
    ...
    public shouldBeInitialized {
        ...
        require(checkSignatures(sha256(abi.encodePacked(newToken, ticker, name, decimals, txHash, _networkId)),
            v,
            r,
    ...
    shouldBeInitialized shouldNotBePreparedForMigration {
        ...
        require(checkSignatures(sha256(abi.encodePacked(thisContractAddress, salt, _networkId)),
            v,
            r,
            s), "Peer signatures are invalid"
        );
    ...
    */
    ...
    shouldBeInitialized shouldBePreparedForMigration {
        require(address(this) == thisContractAddress);
        require(checkSignatures(sha256(abi.encodePacked(thisContractAddress, newContractAddress, salt, erc20nativeTokens, _networkId)),
            v,
            r,
            s), "Peer signatures are invalid"
        );
    ...
    */
    ...
    public shouldBeInitialized {
        require(used[txHash] == false);
        require(checkSignatures(sha256(abi.encodePacked(
                name,
                symbol,
                decimals,
                sidechainAssetId,
    ...
    */
    ...
    {
        require(used[txHash] == false);
        require(checkSignatures(sha256(abi.encodePacked(newPeerAddress, txHash, _networkId)),
            v,
            r,
            s), "Peer signatures are invalid"
        );
    ...
    returns (bool)
    {
        ...
        require(checkSignatures(
                sha256(abi.encodePacked(peerAddress, txHash, _networkId)),
                v,
                r,
                s), "Peer signatures are invalid"
        );
    ...
    public shouldBeInitialized
    {
        require(used[txHash] == false);
        require(checkSignatures(
                sha256(abi.encodePacked(tokenAddress, amount, to, from, txHash, _networkId)),
                v,
                r,
                s), "Peer signatures are invalid"
        );
    ...
    public shouldBeInitialized
    {
        ...
        require(used[txHash] == false);
        require(checkSignatures(
                sha256(abi.encodePacked(sidechainAssetId, amount, to, from, txHash, _networkId)),
                v,
                r,
                s), "Peer signatures are invalid"
        );
