...
contract VolatilityOracle is IVolatilityOracle {
    ...
    function estimate24H(
        ...
        if (current.timestamp - 1 seconds > feeGrowthGlobal[writeIndex].timestamp) {
    ...
    function _estimate24H(
        ...
        if (secondsAgo > 1 seconds) secondsAgo = 1 seconds;
    ...
    function _timingError(uint256 age) private pure returns (uint32) {
        return uint32(age < 24 seconds ? 24 hours - age : age - 24 hours);
    }
}
