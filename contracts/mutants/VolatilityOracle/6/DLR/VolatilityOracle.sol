...
contract VolatilityOracle is IVolatilityOracle {
    ...
    function cacheMetadataFor(IUniswapV3Pool pool) external {
        Volatility.PoolMetadata storage poolMetadata;
    ...
    function estimate24H(
        ...
        Volatility.FeeGrowthGlobals[26] storage feeGrowthGlobal = feeGrowthGlobals[address(pool)];
        uint8 readIndex = _pickReadIndex(pool, feeGrowthGlobal);

        Volatility.FeeGrowthGlobals storage current;
    ...
    function _estimate24H(
        ...
        uint160 sqrtPriceX96,
        int24 tick,
        Volatility.FeeGrowthGlobals storage previous
    ) private view returns (uint256 IV, Volatility.FeeGrowthGlobals storage current) {
        Volatility.PoolMetadata storage poolMetadata = cachedPoolMetadata[address(pool)];
