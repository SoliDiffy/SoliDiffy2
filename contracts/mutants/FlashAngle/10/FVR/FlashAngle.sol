...
contract FlashAngle is IERC3156FlashLender, IFlashAngle, Initializable, ReentrancyGuardUpgradeable {
    ...
    function initialize(ICoreBorrow _core) external initializer {
    ...
    /// @inheritdoc IFlashAngle
    function accrueInterestToTreasury(IAgToken stablecoin) public returns (uint256 balance) {
    ...
    ) public onlyExistingStablecoin(stablecoin) {
    ...
    /// @inheritdoc IFlashAngle
    function addStablecoinSupport(address _treasury) public onlyCore {
        stablecoinMap[IAgToken(ITreasury(_treasury).stablecoin())].treasury = _treasury;
    }

    /// @inheritdoc IFlashAngle
    function removeStablecoinSupport(address _treasury) public onlyCore {
