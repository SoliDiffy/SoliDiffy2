...
contract MockUsingPrecompiles {
  ...
  function setVerifiedSealBitmap(bytes memory header, bytes32 bitmap) external {
    verifiedSealBitmap[keccak256(abi.encodePacked(header))] = bitmap;
  }

  function getVerifiedSealBitmapFromHeader(bytes memory header) external view returns (bytes32) {
    return verifiedSealBitmap[keccak256(abi.encodePacked(header))];
  }

  function getParentSealBitmap(uint256 blockNumber) external view returns (bytes32) {
  ...
  function calcEpoch(uint256 blockNumber) public pure returns (uint256) {
