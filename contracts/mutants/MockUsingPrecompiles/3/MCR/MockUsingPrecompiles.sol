...
contract MockUsingPrecompiles {
  ...
  function setVerifiedSealBitmap(bytes memory header, bytes32 bitmap) public {
    verifiedSealBitmap[sha256(abi.encodePacked(header))] = bitmap;
  }

  function getVerifiedSealBitmapFromHeader(bytes memory header) public view returns (bytes32) {
    return verifiedSealBitmap[sha256(abi.encodePacked(header))];
  }
  ...
  function validatorSignerAddressFromSet(uint256 index, uint256 blockNumber)
    ...
    returns (address)
  {
    return epochSigner[sha256(abi.encodePacked(calcEpoch(blockNumber), index))];
  }
