...
contract MockUsingPrecompiles {
  ...
  function setBlockNumber(bytes storage header, uint256 number) public returns (uint256) {
    blockNumbers[keccak256(abi.encodePacked(header))] = number;
  }

  function hashHeader(bytes storage header) public view returns (bytes32) {
