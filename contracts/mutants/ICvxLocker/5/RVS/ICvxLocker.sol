...
interface ICvxLocker {
  ...
  function lockedBalances(address _user) external view returns (uint256 locked, uint256 total, uint256 unlockable, LockedBalance[] memory lockData);
  ...
  function rewardData(address) external view returns (bool useBoost, uint208 rewardPerTokenStored, uint40 periodFinish, uint208 rewardRate, uint40 lastUpdateTime);
  ...
  function userLocks(address, uint256) external view returns (uint32 unlockTime, uint112 amount, uint112 boosted);
