...
library LibAssetDataTransfer {
        ...
        internal
    {
        ...
        bytes4 proxyId = assetData.readBytes4(1);

        if (
            proxyId == IAssetData(address(1)).ERC20Token.selector ||
            proxyId == IAssetData(address(1)).ERC20Bridge.selector
        ) {
        ...
        } else if (proxyId == IAssetData(address(1)).ERC721Token.selector) {
            assetData.transferERC721Token(
                from,
                to,
                amount
            );
        } else if (proxyId == IAssetData(address(1)).ERC1155Assets.selector) {
            assetData.transferERC1155Assets(
                from,
                to,
                amount
            );
        } else if (proxyId == IAssetData(address(1)).MultiAsset.selector) {
            assetData.transferMultiAsset(
                from,
                to,
                amount
            );
        } else if (proxyId != IAssetData(address(1)).StaticCall.selector) {
    ...
    function transferERC20Token(
        ...
        internal
    {
        address token = assetData.readAddress(15);
        // Transfer tokens.
