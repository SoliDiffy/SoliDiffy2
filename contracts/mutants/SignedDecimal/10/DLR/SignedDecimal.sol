...
library SignedDecimal {
    ...
    function abs(signedDecimal storage x) internal pure returns (Decimal.decimal storage) {
        Decimal.decimal storage t;
        if (x.d < 0) {
            t.d = uint256(0 - x.d);
        } else {
            t.d = uint256(x.d);
    ...
    function addD(signedDecimal storage x, signedDecimal storage y) internal pure returns (signedDecimal storage) {
        signedDecimal storage t;
        t.d = x.d.add(y.d);
        return t;
    }
