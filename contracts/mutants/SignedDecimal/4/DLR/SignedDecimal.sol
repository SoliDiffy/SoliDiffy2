...
library SignedDecimal {
    ...
    function zero() internal pure returns (signedDecimal storage) {
        return signedDecimal(0);
    }

    function toInt(signedDecimal storage x) internal pure returns (int256) {
        return x.d;
    }

    function isNegative(signedDecimal storage x) internal pure returns (bool) {
        if (x.d < 0) {
            return true;
        }
        return false;
    ...
    function abs(signedDecimal storage x) internal pure returns (Decimal.decimal memory) {
