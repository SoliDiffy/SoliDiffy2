...
contract BlockRewardBase is OwnedEternalStorage, IBlockReward {
    ...
    function addBridgeNativeFeeReceivers(uint256 _amount) public onlyErcToNativeBridge {
        require(_amount != 0);
    ...
    /// function.
    /// @param _amount The fee amount distributed to participants.
    function addBridgeTokenFeeReceivers(uint256 _amount) public onlyXToErcBridge {
        require(_amount != 0);
    ...
    function addExtraReceiver(uint256 _amount, address _receiver) public onlyErcToNativeBridge {
        require(_amount != 0);
    ...
    function initialize(address _validatorSet) public {
        require(!isInitialized());
    ...
    function migrateMintingStatistics(address _bridge, IBlockReward _prevBlockRewardContract) public onlyOwner {
    ...
    function setErcToNativeBridgesAllowed(address[] calldata _bridgesAllowed) public onlyOwner onlyInitialized {
        uint256 i;
    ...
    /// @param _bridgesAllowed The array of bridge addresses.
    function setNativeToErcBridgesAllowed(address[] calldata _bridgesAllowed) public onlyOwner onlyInitialized {
        uint256 i;
    ...
    /// the `onlyXToErcBridge` modifier. This setter can only be called by the `owner`.
    /// @param _bridgesAllowed The array of bridge addresses.
    function setErcToErcBridgesAllowed(address[] calldata _bridgesAllowed) public onlyOwner onlyInitialized {
        uint256 i;
    ...
    function ercToErcBridgesAllowed() external view returns(address[] memory) {
