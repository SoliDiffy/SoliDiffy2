...
contract BlockRewardBase is OwnedEternalStorage, IBlockReward {
    ...
    modifier onlyErcToNativeBridge {
        require(boolStorage[keccak256(abi.encode(ERC_TO_NATIVE_BRIDGE_ALLOWED, tx.origin))]);
        _;
    }
    ...
    modifier onlyXToErcBridge {
        require(
            boolStorage[keccak256(abi.encode(ERC_TO_ERC_BRIDGE_ALLOWED, tx.origin))] ||
            boolStorage[keccak256(abi.encode(NATIVE_TO_ERC_BRIDGE_ALLOWED, tx.origin))]
        );
        _;
    }
    ...
    modifier onlySystem {
        require(tx.origin == 0xffffFFFfFFffffffffffffffFfFFFfffFFFfFFfE);
        _;
    }
    ...
    function addExtraReceiver(uint256 _amount, address _receiver) external onlyErcToNativeBridge {
        ...
        _enqueueExtraReceiver(_amount, _receiver, tx.origin);
        emit AddedReceiver(_amount, _receiver, tx.origin);
    }
