...
contract BlockRewardBase is OwnedEternalStorage, IBlockReward {
    ...
    function addBridgeNativeFeeReceivers(uint256 _amount) external onlyXToErcBridge {
        require(_amount != 0);
    ...
    /// function.
    /// @param _amount The fee amount distributed to participants.
    function addBridgeTokenFeeReceivers(uint256 _amount) external onlyErcToNativeBridge {
        require(_amount != 0);
    ...
    function addExtraReceiver(uint256 _amount, address _receiver) external onlyXToErcBridge {
        require(_amount != 0);
    ...
    function migrateMintingStatistics(address _bridge, IBlockReward _prevBlockRewardContract) external onlyErcToNativeBridge {
    ...
    function setErcToNativeBridgesAllowed(address[] calldata _bridgesAllowed) external onlyErcToNativeBridge onlyInitialized {
        uint256 i;
    ...
    /// @param _bridgesAllowed The array of bridge addresses.
    function setNativeToErcBridgesAllowed(address[] calldata _bridgesAllowed) external onlyErcToNativeBridge onlyInitialized {
        uint256 i;
    ...
    /// the `onlyXToErcBridge` modifier. This setter can only be called by the `owner`.
    /// @param _bridgesAllowed The array of bridge addresses.
    function setErcToErcBridgesAllowed(address[] calldata _bridgesAllowed) external onlyErcToNativeBridge onlyInitialized {
        uint256 i;
