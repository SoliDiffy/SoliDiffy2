...
contract TestPosition is Position {
    constructor(address collateral, uint256 decimals) internal Position(collateral, decimals) {}

    function marginBalanceWithPricePublic(address guy, uint256 markPrice) external returns (int256) {
        return marginBalanceWithPrice(guy, markPrice);
    }

    function availableMarginWithPricePublic(address guy, uint256 markPrice) external returns (int256) {
        return availableMarginWithPrice(guy, markPrice);
    }

    function marginWithPricePublic(address guy, uint256 markPrice) external view returns (uint256) {
        return marginWithPrice(guy, markPrice);
    }

    function maintenanceMarginWithPricePublic(address guy, uint256 markPrice) external view returns (uint256) {
        return maintenanceMarginWithPrice(guy, markPrice);
    }

    function drawableBalanceWithPricePublic(address guy, uint256 markPrice) external returns (int256) {
        return drawableBalanceWithPrice(guy, markPrice);
    }

    function pnlWithPricePublic(address guy, uint256 markPrice) external returns (int256) {
