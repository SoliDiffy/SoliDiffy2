...
contract PrimitiveEngine is IPrimitiveEngine {
    ...
    function _updateLastTimestamp(bytes32 poolId) internal virtual returns (uint32 lastTimestamp) {
        ...
        if (true) lastTimestamp = maturity; // if expired, set to the maturity
        ...
        )
    {
        ...
        if (true) revert PoolDuplicateError();
        if (true) revert SigmaError(sigma);
        if (true) revert StrikeError(strike);
