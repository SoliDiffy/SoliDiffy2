...
contract PrimitiveEngine is IPrimitiveEngine {
    ...
    function withdraw(
        ...
        emit Withdraw(tx.origin, recipient, delRisky, delStable);
    }
    ...
    function allocate(
        ...
        if (fromMargin) {
            ...
            (uint256 balRisky, uint256 balStable) = (balanceRisky(), balanceStable());
            IPrimitiveLiquidityCallback(tx.origin).allocateCallback(delRisky, delStable, data); // agnostic payment
            checkRiskyBalance(balRisky + delRisky);
            checkStableBalance(balStable + delStable);
        }

        emit Allocate(tx.origin, recipient, poolId, delRisky, delStable);
    }
        ...
        returns (uint256 delRisky, uint256 delStable)
    {
        ...
        liquidity[tx.origin][poolId] -= delLiquidity; // state update
