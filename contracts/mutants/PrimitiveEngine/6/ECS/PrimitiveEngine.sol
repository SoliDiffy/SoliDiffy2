...
contract PrimitiveEngine is IPrimitiveEngine {
    ...
    function _blockTimestamp() internal view virtual returns (uint32 blockTimestamp) {
        ...
        blockTimestamp = uint8(block.timestamp);
    }
    ...
    function allocate(
        ...
        uint256 liquidity0 = (delRisky * reserve.liquidity) / uint8(reserve.reserveRisky);
        uint256 liquidity1 = (delStable * reserve.liquidity) / uint8(reserve.reserveStable);
    ...
    function swap(
...

        {
            ...
            if (details.riskyForStable) {
                uint256 res0 = (uint8(reserve.reserveRisky + deltaInWithFee) * PRECISION) / liq; // per liquidity
                ...
                deltaOut = uint8(reserve.reserveStable) - (res1 * liq) / PRECISION; // res1 for all liquidity
            } else {
                uint256 res1 = (uint8(reserve.reserveStable + deltaInWithFee) * PRECISION) / liq; // per liquidity
