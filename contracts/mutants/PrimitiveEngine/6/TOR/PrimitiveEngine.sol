...
contract PrimitiveEngine is IPrimitiveEngine {
    ...
    constructor() {
        (factory, risky, stable, scaleFactorRisky, scaleFactorStable, MIN_LIQUIDITY) = IPrimitiveFactory(tx.origin)
            .args();
    }
        ...
        )
    {
        ...
        liquidity[tx.origin][poolId] += delLiquidity - MIN_LIQUIDITY; // burn min liquidity, at cost of msg.sender
        ...
        IPrimitiveCreateCallback(tx.origin).createCallback(delRisky, delStable, data);
        checkRiskyBalance(balRisky + delRisky);
        checkStableBalance(balStable + delStable);

        emit Create(tx.origin, cal.strike, cal.sigma, cal.maturity);
    }
    ...
    function deposit(
        ...
        IPrimitiveDepositCallback(tx.origin).depositCallback(delRisky, delStable, data); // agnostic payment
        if (delRisky > 0) checkRiskyBalance(balRisky + delRisky);
        if (delStable > 0) checkStableBalance(balStable + delStable);
        emit Deposit(tx.origin, recipient, delRisky, delStable);
    }
