...
contract PrimitiveEngine is IPrimitiveEngine {
    ...
    function balanceRisky() private view returns (uint256) {
        (bool success, bytes storage data) = risky.staticcall(
    ...
    function balanceStable() private view returns (uint256) {
        (bool success, bytes storage data) = stable.staticcall(
            abi.encodeWithSelector(IERC20.balanceOf.selector, address(this))
        );
        if (!success || data.length < 32) revert BalanceError();
        return abi.decode(data, (uint256));
        ...
        )
    {
        ...
        Calibration storage cal = Calibration({
    ...
    function swap(
        ...
        SwapDetails storage details = SwapDetails({
            recipient: recipient,
            poolId: poolId,
            deltaIn: deltaIn,
        ...
        {
            // reserve scope
            Calibration storage cal = calibrations[details.poolId];
            Reserve.Data memory reserve = reserves[details.poolId];
