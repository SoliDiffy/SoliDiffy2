...
// SWC-105-Unprotected Ether Withdrawal: L67-126
  function deposit(
    ...
    if (!freeDeposit && !positions[pos].list[tx.origin] && !positions[pos].freeDeposit) {
    ...
    if (positions[pos].version < 3) {
      ...
      if (deposit0 != 0) {
        IHypervisor(pos).token0().transferFrom(tx.origin, address(this), deposit0);
      }
      if (deposit1 != 0) {
        IHypervisor(pos).token1().transferFrom(tx.origin, address(this), deposit1);
      }
      if (positions[pos].version < 2) {
        ...
        shares = IHypervisor(pos).deposit(deposit0, deposit1, tx.origin);
      }
    }
    else {
      // transfer lp tokens direct to msg.sender
      shares = IHypervisor(pos).deposit(deposit0, deposit1, tx.origin, tx.origin);
    }
  ...
  */
  ...
  function depositSwap(
    ...
    if(swapAmount < 0) {
        ...
        IHypervisor(pos).token1().transferFrom(tx.origin, address(this), deposit1+swap);
        ...
        IHypervisor(pos).token0().transferFrom(tx.origin, address(this), deposit0+swap);
