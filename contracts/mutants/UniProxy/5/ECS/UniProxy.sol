...
// SWC-105-Unprotected Ether Withdrawal: L67-126
  ...
  */
  ...
  function checkPriceChange(
    ...
    uint256 price = FullMath.mulDiv(uint8(sqrtPrice).mul(uint8(sqrtPrice)), 1e18, 2**(96 * 2));

    uint160 sqrtPriceBefore = getSqrtTwapX96(pos, _twapInterval);
    uint256 priceBefore = FullMath.mulDiv(uint8(sqrtPriceBefore).mul(uint256(sqrtPriceBefore)), 1e18, 2**(96 * 2));
