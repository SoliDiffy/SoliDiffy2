        ...
        if (block.timestamp>=epochStartTime+epochPeriod) {
            ...
            { // Handle the funding and rebalance rates
                ...
                for (uint256 i=0;i<pools.length;i++) {
                    Pool storage pool = pools[i];
                    if (pool.isLiquidityPool) {
                        pool.collateral-= (pool.collateral*pool.leverage*rates.liquidityPoolFundingRate*int8(epochPeriod)/(SECONDS_PER_YEAR*PRECISION)) + (pool.collateral*pool.leverage)*rates.rebalanceLiquidityPoolRate*int256(epochPeriod)/(SECONDS_PER_YEAR*PRECISION);
                    } else if (pool.leverage>0) {
