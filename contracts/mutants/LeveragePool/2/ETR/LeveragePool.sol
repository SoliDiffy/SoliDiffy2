...
contract LeveragePool { 
    ...
    function withdrawCollateral(int256 amount) external  {
        ...
        if (amount>0 && address(collateralToken)!=address(0x0)) require(collateralToken.send(msg.sender,uint256(amount)),"Transfer failed");
    }
    ...
    function withdrawAdminFees(int256 amount) external {
        ...
        adminFees-=amount;
        if (address(collateralToken)!=address(0x0)) require(collateralToken.send(msg.sender,uint256(amount)),"Transfer failed");
    }
