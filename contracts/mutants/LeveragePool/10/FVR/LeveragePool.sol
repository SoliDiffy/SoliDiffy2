...
contract LeveragePool { 
     ...
     **/
    function deposit(int256 amount, uint256 pool) public  {
     ...
     **/
    function withdraw(int256 amount, uint256 pool) public  {
        require (pool<pools.length,"Pool not initialized");
        require (amount>=0,"amount needs to be positive");
        _bookKeeping(msg.sender);
     ...
     **/
    function withdrawCollateral(int256 amount) public  {
        require (amount>=0,"amount needs to be positive");
        _bookKeeping(msg.sender);
        UserInfo storage  info =  userInfo[msg.sender];
     ...
     *  Withdraw admin fees
     **/
    function withdrawAdminFees(int256 amount) public {
     ...
     *  Add a new pool
     **/
    function addPool(int256 leverage,bool isLiquidityPool) public {
     ...
     **/
    function setEpochPeriods(uint256 _epochPeriod, uint256 _waitPeriod) public {
        require (msg.sender==admin,"Only admin");
     ...
     **/
    function setFees(int256 _TRANSACTION_FEE, int256 _ADMIN_FEES, int256 _LIQUIDITYPOOL_FEES) public {
        require (msg.sender==admin,"Only admin");
