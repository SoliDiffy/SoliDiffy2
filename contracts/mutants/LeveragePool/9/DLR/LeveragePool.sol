...
contract LeveragePool { 
    ...
    function deposit(int256 amount, uint256 pool) external  {
        ...
        PoolEpochData memory data = poolEpochData[action.epoch][action.pool];
    ...
    **/
     ...
     **/
    function calculatePoolAmounts() external view returns (PoolAmounts storage amounts) {
     ...
     **/
    function getUserShares(address user) external returns (int256[] storage shares) {
        _bookKeeping(user);
