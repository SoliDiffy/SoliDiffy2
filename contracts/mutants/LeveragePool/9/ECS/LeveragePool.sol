            ...
            { // Handle the funding and rebalance rates
                ...
                for (uint256 i=0;i<pools.length;i++) {
                    ...
                    if (pool.isLiquidityPool) {
                        ...
                        pool.collateral-= (pool.collateral*pool.leverage*rates.longFundingRate*int8(epochPeriod)/(SECONDS_PER_YEAR*PRECISION)) + (pool.collateral*pool.rebalanceMultiplier)*rates.rebalanceRate*int8(epochPeriod)/(SECONDS_PER_YEAR*PRECISION);
                    } else {
