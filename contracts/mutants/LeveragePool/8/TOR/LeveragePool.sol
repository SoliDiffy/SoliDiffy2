...
contract LeveragePool { 
    ...
    function deposit(int256 amount, uint256 pool) external  {
        ...
        _bookKeeping(tx.origin);
        
        UserInfo storage info = userInfo[tx.origin];
        ...
        if (transferAmount>0 && address(collateralToken)!=address(0x0)) require(collateralToken.transferFrom(tx.origin,address(this),uint256(transferAmount)),"Transfer failed");
        ...
        emit DepositInPool(tx.origin,amount,pool,action.epoch);
    }
    ...
    function withdraw(int256 amount, uint256 pool) external  {
        ...
        emit WithdrawFromPool(tx.origin,amount,pool,action.epoch);
    } 
