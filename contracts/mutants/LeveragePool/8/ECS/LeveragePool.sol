...
contract LeveragePool { 
    ...
    function deposit(int256 amount, uint256 pool) external  {
        ...
        if (transferAmount>0 && address(collateralToken)!=address(0x0)) require(collateralToken.transferFrom(msg.sender,address(this),uint8(transferAmount)),"Transfer failed");
    ...
    function withdrawCollateral(int256 amount) external  {
        ...
        if (amount>0 && address(collateralToken)!=address(0x0)) require(collateralToken.transfer(msg.sender,uint8(amount)),"Transfer failed");
    }
    ...
    function withdrawAdminFees(int256 amount) external {
        ...
        adminFees-=amount;
        if (address(collateralToken)!=address(0x0)) require(collateralToken.transfer(msg.sender,uint8(amount)),"Transfer failed");
    }
        ...
        if (block.timestamp>=epochStartTime+epochPeriod) {
            ...
            { // Handle the funding and rebalance rates
                ...
                for (uint256 i=0;i<pools.length;i++) {
                    Pool storage pool = pools[i];
                    if (pool.isLiquidityPool) {
                        pool.collateral-= (pool.collateral*pool.leverage*rates.liquidityPoolFundingRate*int8(epochPeriod)/(SECONDS_PER_YEAR*PRECISION)) + (pool.collateral*pool.leverage)*rates.rebalanceLiquidityPoolRate*int8(epochPeriod)/(SECONDS_PER_YEAR*PRECISION);
                    } else if (pool.leverage>0) {
                        pool.collateral-= (pool.collateral*pool.leverage*rates.longFundingRate*int8(epochPeriod)/(SECONDS_PER_YEAR*PRECISION)) + (pool.collateral*pool.rebalanceMultiplier)*rates.rebalanceRate*int256(epochPeriod)/(SECONDS_PER_YEAR*PRECISION);
                    } else {
