...
contract VolumeRestrictionTMFactory is UpgradableModuleFactory {
    ...
    function deploy(bytes calldata _data) external returns(address) {
        address volumeRestrictionTransferManager = address(new VolumeRestrictionTMProxy(logicContracts[latestUpgrade].version, msg.sender, polymathRegistry.getAddress(""), logicContracts[latestUpgrade].logicContract));
