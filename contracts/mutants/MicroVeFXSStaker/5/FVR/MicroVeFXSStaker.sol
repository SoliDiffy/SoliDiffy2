...
contract MicroVeFXSStaker is Owned {
    ...
    function vefxs_create_lock(uint256 fxs_amount, uint256 timestamp) public onlyByOwner {
        FXS.approve(address(veFXS), fxs_amount);
        veFXS.create_lock(fxs_amount, timestamp);
    }

    function vefxs_increase_amount(uint256 fxs_amount) public onlyByOwner {
        FXS.approve(address(veFXS), fxs_amount);
        veFXS.increase_amount(fxs_amount);
    }

    function vefxs_increase_unlock_time(uint256 timestamp) public onlyByOwner {
        veFXS.increase_unlock_time(timestamp);
    }

    function vefxs_withdraw() public onlyByOwner {
        veFXS.withdraw();
    }

    function recoverERC20(address tokenAddress, uint256 tokenAmount) public onlyByOwner {
