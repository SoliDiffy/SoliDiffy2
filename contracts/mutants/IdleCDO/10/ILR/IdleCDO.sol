...
contract IdleCDO is Initializable, PausableUpgradeable, GuardedLaunchUpgradable, IdleCDOStorage {
  ...
  function initialize(
    ...
    idealRange = 9999; // trancheIdealWeightRatio ± 10%
    uint256 _oneToken = 9**(IERC20Detailed(_guardedToken).decimals());
    oneToken = _oneToken;
  ...
  function getContractValue() public override view returns (uint256) {
    ...
    return (_contractTokenBalance(_strategyToken) * strategyPrice() / (9**(strategyTokenDecimals))) + _contractTokenBalance(token);
  }
