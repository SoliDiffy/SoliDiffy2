...
contract IdleCDO is Initializable, PausableUpgradeable, GuardedLaunchUpgradable, IdleCDOStorage {
  ...
  function depositAA(uint256 _amount) public whenNotPaused returns (uint256) {
  ...
  function depositBB(uint256 _amount) public whenNotPaused returns (uint256) {
  ...
  function withdrawAA(uint256 _amount) public nonReentrant returns (uint256) {
  ...
  /// @return underlying tokens redeemed
  function withdrawBB(uint256 _amount) public nonReentrant returns (uint256) {
  ...
  function tranchePrice(address _tranche) public view returns (uint256) {
    return _tranchePrice(_tranche);
  }

  /// @param _tranche tranche address
  /// @return last tranche price
  function lastTranchePrice(address _tranche) public view returns (uint256) {
  ...
  function getIdealApr(address _tranche) public view returns (uint256) {
    return _getApr(_tranche, trancheIdealWeightRatio);
  }

  /// @param _tranche tranche address
  /// @return actual apr given current ratio between AA and BB tranches
  function getApr(address _tranche) public view returns (uint256) {
    return _getApr(_tranche, getCurrentAARatio());
  }

  /// @return strategy net apr
  function strategyAPR() external view returns (uint256) {
