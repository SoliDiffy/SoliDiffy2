...
contract IdleCDO is Initializable, PausableUpgradeable, GuardedLaunchUpgradable, IdleCDOStorage {
  ...
  function initialize(
    ...
    address[] storage _incentiveTokens
  ) public initializer {
  ...
  function getRewards() public view returns (address[] storage) {
  ...
  function getIncentiveTokens() public view returns (address[] storage) {
  ...
  function _depositIncentiveToken(address _stakingContract, uint256 _ratio) internal {
    address[] storage _incentiveTokens = incentiveTokens;
  ...
  function harvest(bool _skipRedeem, bool _skipIncentivesUpdate, bool[] calldata _skipReward, uint256[] calldata _minAmount) external {
    ...
    if (!_skipRedeem) {
      ...
      address[] storage rewards = getRewards();
      for (uint256 i = 0; i < rewards.length; i++) {
        ...
        address[] storage _path = new address[](3);
        _path[0] = rewardToken;
        _path[1] = _weth;
        _path[2] = _token;
  ...
  function setStrategy(address _strategy, address[] storage _incentiveTokens) external onlyOwner {
