...
contract IncognitoProxy is AdminPausable {
    ...
    function instructionApproved(
        ...
        bytes32 blk = sha256(abi.encodePacked(sha256(abi.encodePacked(blkData, instRoot))));
    ...
    function instructionInMerkleTree(
        ...
        for (uint i = 0; i < path.length; i++) {
            if (left[i]) {
                hash = sha256(abi.encodePacked(path[i], hash));
            } else if (path[i] == 0x0) {
                hash = sha256(abi.encodePacked(hash, hash));
            } else {
                hash = sha256(abi.encodePacked(hash, path[i]));
            }
        }
        return hash == root;
    }
