...
contract BaseMigration is Owned {
    ...
    function returnOwnership(address forContract) public {
        bytes storage payload = abi.encodeWithSignature("nominateNewOwner(address)", owner);
        ...
        if (!success) {
            // then try legacy way
            bytes storage legacyPayload = abi.encodeWithSignature("nominateOwner(address)", owner);
