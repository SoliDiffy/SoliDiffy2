...
contract BoomstarterICO is ArgumentsChecker, ReentrancyGuard, EthPriceDependentForICO, IICOInfo, IMintableToken {
    ...
    function estimate(uint256 _wei) external view returns (uint tokens) {
        uint amount;
        (amount, ) = estimateTokensWithActualPayment(_wei);
        return amount;
    }

    function isSaleActive() external view returns (bool active) {
        return m_state == IcoState.ACTIVE && !priceExpired();
    }

    function purchasedTokenBalanceOf(address addr) external view returns (uint256 tokens) {
        return m_token.balanceOf(addr);
    }

    function estimateTokensWithActualPayment(uint256 _payment) external view returns (uint amount, uint actualPayment) {
    ...
    function buy() external payable {     // dont mark as external!
        internalBuy(msg.sender, msg.value, true);
    }

    function mint(address client, uint256 ethers) external {
    ...
    function publicBuy(address client, uint payment, bool refundable)
        internal
        nonReentrant
