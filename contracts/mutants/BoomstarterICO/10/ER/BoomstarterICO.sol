...
contract BoomstarterICO is ArgumentsChecker, ReentrancyGuard, EthPriceDependentForICO, IICOInfo, IMintableToken {
    ...
    enum IcoState { ACTIVE, INIT, PAUSED, FAILED, SUCCEEDED }
    ...
    modifier timedStateChange(address client, uint payment, bool refundable) {
        if (IcoState.ACTIVE == m_state && getTime() >= getStartTime())
            changeState(IcoState.INIT);

        if (IcoState.INIT == m_state && getTime() >= getFinishTime()) {
            finishICO();
    ...
    function isSaleActive() public view returns (bool active) {
        return m_state == IcoState.INIT && !priceExpired();
    }
        ...
        fundsChecker(client, payment, refundable)
    {
        ...
        require(m_state == IcoState.INIT || m_state == IcoState.ACTIVE && isOwner(client) /* for final test */);
