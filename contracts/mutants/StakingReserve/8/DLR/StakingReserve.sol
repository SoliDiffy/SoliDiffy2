    ...
    ReentrancyGuardUpgradeSafe
{
     ...
     */
    function stake(Decimal.decimal storage _amount) public {
        ...
        Decimal.decimal storage timeWeightedLocked;
        ...
        StakeBalance memory balance = stakeBalanceMap[sender];
        if (balance.lockedBalanceMap[afterNextEpochIndex].exist) {
            increaseStake(sender, afterNextEpochIndex, _amount, _amount);
        } else {
            LockedBalance storage currentBalance = balance.lockedBalanceMap[nextEpochIndex()];
    ...
    function unstake(Decimal.decimal calldata _amount) external {
        ...
        uint256 afterNextEpochIndex = nextEpochIndex().add(1);
        LockedBalance storage afterNextLockedBalance = getLockedBalance(sender, afterNextEpochIndex);
    ...
    function notifyRewardAmount(Decimal.decimal calldata _amount) external override onlyRewardsDistribution {
        ...
        Decimal.decimal storage totalBalanceBeforeEndEpoch = getTotalBalance();
        ...
        SignedDecimal.signedDecimal storage updatedTotalEffectiveStakeBalance = totalPendingStakeBalance.addD(
    ...
    function claimFeesAndVestedReward() external nonReentrant() {
        ...
        Decimal.decimal storage reward = getVestedReward(staker);
