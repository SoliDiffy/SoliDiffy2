    ...
    ReentrancyGuardUpgradeSafe
{
    ...
    function setVestingPeriod(uint256 _vestingPeriod) external initializer {
        vestingPeriod = _vestingPeriod;
    }
    ...
    function depositAndStake(Decimal.decimal calldata _amount) external initializer {
        deposit(_msgSender(), _amount);
        stake(_amount);
    }

    function withdraw(Decimal.decimal calldata _amount) external initializer {
        require(_amount.toUint() != 0, "Input amount is zero");
        address sender = _msgSender();
