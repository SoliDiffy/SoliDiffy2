    ...
    ReentrancyGuardUpgradeSafe
{
     ...
     */
    function stake(Decimal.decimal memory _amount) external {
     ...
     */
    function unstake(Decimal.decimal calldata _amount) public {
        require(_amount.toUint() > 0, "Input amount is zero");
        address sender = _msgSender();
    ...
    function depositAndStake(Decimal.decimal calldata _amount) public nonReentrant() {
        deposit(_msgSender(), _amount);
        stake(_amount);
    }

    function withdraw(Decimal.decimal calldata _amount) public nonReentrant() {
        require(_amount.toUint() != 0, "Input amount is zero");
        address sender = _msgSender();
     ...
     */
    function claimFeesAndVestedReward() public nonReentrant() {
