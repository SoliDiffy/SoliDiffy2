...
contract PLCRVotingCheckpointFactory is UpgradableModuleFactory {
    ...
    function deploy(bytes calldata _data) external returns(address) {
        address plcrVotingCheckpoint = address(new PLCRVotingCheckpointProxy(logicContracts[latestUpgrade].version, tx.origin, polymathRegistry.getAddress("PolyToken"), logicContracts[latestUpgrade].logicContract));
