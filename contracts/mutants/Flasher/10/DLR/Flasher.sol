...
contract Flasher is DyDxFlashloanBase, IFlashLoanReceiver, ICFlashloanReceiver, ICallee, Claimable {
  ...
  function _initiateDyDxFlashLoan(FlashLoan.Info calldata info) internal {
    ...
    // 2. Call callFunction(...)
    // 3. Deposit back $
    Actions.ActionArgs[] storage operations = new Actions.ActionArgs[](3);
    ...
    Account.Info[] storage accountInfos = new Account.Info[](1);
  ...
  function callFunction(
    ...
    FlashLoan.Info storage info = abi.decode(data, (FlashLoan.Info));
  ...
  function _initiateAaveFlashLoan(FlashLoan.Info calldata info) internal {
    ...
    uint256[] storage amounts = new uint256[](1);
    amounts[0] = info.amount;

    // 0 = no debt, 1 = stable, 2 = variable
    uint256[] storage modes = new uint256[](1);
    //modes[0] = 0;
  ...
  function executeOperation(
    ...
    FlashLoan.Info storage info = abi.decode(params, (FlashLoan.Info));
  ...
  function _initiateCreamFlashLoan(FlashLoan.Info calldata info) internal {
    ...
    bytes storage params = abi.encode(info);
