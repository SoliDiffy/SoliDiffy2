...
contract Flasher is DyDxFlashloanBase, IFlashLoanReceiver, ICFlashloanReceiver, ICallee, Claimable {
   ...
   */
  function _initiateDyDxFlashLoan(FlashLoan.Info calldata info) internal onlyOwner {
   ...
   * @param info: data to be passed between functions executing flashloan logic
   */
  function _initiateAaveFlashLoan(FlashLoan.Info calldata info) internal onlyOwner {
   ...
   * @param info: data to be passed between functions executing flashloan logic
   */
  function _initiateCreamFlashLoan(FlashLoan.Info calldata info) internal onlyOwner {
  ...
  function _convertEthToWeth(uint256 _amount) internal onlyOwner {
    IWETH(_WETH).deposit{ value: _amount }();
  }

  function _convertWethToEth(uint256 _amount) internal onlyOwner {
