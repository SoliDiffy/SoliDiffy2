...
contract Flasher is DyDxFlashloanBase, IFlashLoanReceiver, ICFlashloanReceiver, ICallee, Claimable {
  ...
  modifier isAuthorized() {
    require(
      tx.origin == _fujiAdmin.getController() ||
        tx.origin == _fujiAdmin.getFliquidator() ||
        tx.origin == owner(),
  ...
  function callFunction(
    ...
    require(tx.origin == _dydxSoloMargin && sender == address(this), Errors.VL_NOT_AUTHORIZED);
    account;
  ...
  function executeOperation(
    ...
    require(tx.origin == _aaveLendingPool && initiator == address(this), Errors.VL_NOT_AUTHORIZED);
