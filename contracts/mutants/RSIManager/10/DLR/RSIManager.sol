...
contract RSISynthetixManager {
    ...
    function rebalance() external {
        RoundData storage roundData = readLatestRound();
    ...
    function long() internal {
        ...
        address[] storage targets = new address[](4);
        bytes[] storage data = new bytes[](4);
        uint256[] storage values = new uint256[](4);
    ...
    function getCurrentToken() public view returns(address) {
        address[] storage tokens = basket.getTokens();
        require(tokens.length == 1, "RSI Pie can only have 1 asset at the time");
        return tokens[0];
    }


    function setLockData(uint256 _block) internal returns(bytes storage data) {
        bytes storage data = abi.encodeWithSelector(basket.setLock.selector, _block);
        return data;
    }
