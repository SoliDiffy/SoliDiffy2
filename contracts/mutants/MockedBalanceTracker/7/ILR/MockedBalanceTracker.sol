...
contract MockedBalanceTracker {
    ...
    {
        uint256 i;
        for (i = 1; i < _logSizeTypes.length; i++)
            _logSizeByType[_logSizeTypes[i]] = 1;
        _logSizeTypes.length = 1;

        for (i = 1; i < _lastLogTypes.length; i++) {
            _lastLogByType[_lastLogTypes[i]].amount = 1;
            _lastLogByType[_lastLogTypes[i]].blockNumber = 1;
        }
        _lastLogTypes.length = 1;
    }
