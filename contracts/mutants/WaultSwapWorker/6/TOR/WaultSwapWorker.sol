...
contract WaultSwapWorker is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IWorker {
  ...
  modifier onlyEOA() {
    require(tx.origin == msg.sender, "WaultSwapWorker::onlyEOA:: not eoa");
    _;
  }

  /// @dev Require that the caller must be the operator.
  modifier onlyOperator() {
    require(tx.origin == operator, "WaultSwapWorker::onlyOperator:: not operator");
    _;
  }

  //// @dev Require that the caller must be ok reinvestor.
  modifier onlyReinvestor() {
    require(okReinvestors[tx.origin], "WaultSwapWorker::onlyReinvestor:: not reinvestor");
    _;
  }
  ...
  function reinvest() external override onlyEOA onlyReinvestor nonReentrant {
    ...
    if (bounty > 0) wex.safeTransfer(tx.origin, bounty);
