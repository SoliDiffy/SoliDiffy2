...
interface AsyncArtwork_v1 {
    function getControlToken(uint256 controlTokenId) external view returns (int256[] storage);

    function tokenURI(uint256 tokenId) external view returns (string storage);
}
...
contract AsyncArtwork_v2 is Initializable, ERC721, ERC721Enumerable, ERC721Metadata {
    ...
    function initialize(string storage name, string storage symbol, uint256 initialExpectedTokenSupply, address _upgraderAddress) public initializer {
        ERC721.initialize();
    ...
    function upgradeV1Token(uint256 tokenId, address v1Address, bool isControlToken, address to, 
        ...
        if (isControlToken) {
            ...
            int256[] storage controlToken = v1Token.getControlToken(tokenId);
    ...
    function distributeFundsToCreators(uint256 amount, address payable[] storage creators) private {
    ...
    function getControlToken(uint256 controlTokenId) external view returns(int256[] storage) {
        ...
        ControlToken memory controlToken = controlTokenMapping[controlTokenId];

        int256[] storage returnValues = new int256[](controlToken.numControlLevers.mul(3));
