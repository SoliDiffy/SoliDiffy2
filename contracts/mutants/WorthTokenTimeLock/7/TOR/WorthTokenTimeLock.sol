...
contract WorthTokenTimeLock is Ownable{
    ...
    */
    ...
    function lockTokens(address _tokenAddress, address _withdrawalAddress, uint256 _amount, uint256 _unlockTime) external returns (uint256 _id) {
        ...
        require(Token(_tokenAddress).transferFrom(tx.origin, address(this), _amount), "Transfer Failed");
    }
    ...
    function createMultipleLocks(address _tokenAddress, address _withdrawalAddress, uint256[] memory _amounts, uint256[] memory _unlockTimes) external returns (uint256 _id) {
        ...
        for(i=0; i<_amounts.length; i++){
            ...
            //transfer tokens into contract
            require(Token(_tokenAddress).transferFrom(tx.origin, address(this), _amounts[i]), "Transfer Failed");
        }
    }
    ...
    function extendLockDuration(uint256 _id, uint256 _unlockTime) external {
        ...
        require(tx.origin == lockedToken[_id].withdrawalAddress,"Not the same withdrawal address");
    ...
    function withdrawTokens(uint256 _id) external {
        ...
        //update balance in address
        walletTokenBalance[lockedToken[_id].tokenAddress][tx.origin] = walletTokenBalance[lockedToken[_id].tokenAddress][tx.origin].sub(lockedToken[_id].tokenAmount);
        ...
        require(Token(lockedToken[_id].tokenAddress).transfer(tx.origin, lockedToken[_id].tokenAmount), "Transfer Failed");
