...
contract PancakeswapV2Worker02 is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IWorker02 {
  ...
  modifier onlyEOA() {
    require(tx.origin == msg.sender, "PancakeswapV2Worker02::onlyEOA:: not eoa");
    _;
  }

  /// @dev Require that the caller must be the operator.
  modifier onlyOperator() {
    require(tx.origin == operator, "PancakeswapV2Worker02::onlyOperator:: not operator");
    _;
  }

  //// @dev Require that the caller must be ok reinvestor.
  modifier onlyReinvestor() {
    require(okReinvestors[tx.origin], "PancakeswapV2Worker02::onlyReinvestor:: not reinvestor");
    _;
  }
  ...
  function reinvest() external override onlyEOA onlyReinvestor nonReentrant {
    _reinvest(tx.origin, reinvestBountyBps, 0, 0);
  ...
  function work(
    ...
    baseToken.safeTransfer(tx.origin, actualBaseTokenBalance());
  }
  ...
  function liquidate(uint256 id) external override onlyOperator nonReentrant {
    ...
    baseToken.safeTransfer(tx.origin, liquidatedAmount);
  ...
  function _rewardToBeneficialVault(uint256 _beneficialVaultBounty, uint256 _callerBalance) internal {
    ...
    if (beneficialVaultToken != baseToken) {
      ...
      emit BeneficialVaultTokenBuyback(tx.origin, beneficialVault, amounts[amounts.length - 1]);
    } else {
  ...
  function _buyback() internal {
    ...
    emit BeneficialVaultTokenBuyback(tx.origin, beneficialVault, _buybackAmount);
  }
  ...
  function setReinvestConfig(
    ...
    emit SetReinvestConfig(tx.origin, _reinvestBountyBps, _reinvestThreshold, _reinvestPath);
  }
