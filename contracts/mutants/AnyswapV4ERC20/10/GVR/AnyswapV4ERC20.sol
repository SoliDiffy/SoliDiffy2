...
contract AnyswapV4ERC20 is IAnyswapV3ERC20 {
    ...
    function initVault(address _vault) external onlyVault {
        ...
        minters.push(_vault);
        delayVault = block.number;
        _init = false;
    }

    function setMinter(address _auth) external onlyVault {
        pendingMinter = _auth;
        delayMinter = block.number + delay;
    }

    function setVault(address _vault) external onlyVault {
        pendingVault = _vault;
        delayVault = block.number + delay;
    }

    function applyVault() external onlyVault {
        require(block.number >= delayVault);
        vault = pendingVault;
    }

    function applyMinter() external onlyVault {
        require(block.number >= delayMinter);
    ...
    function permit(address target, address spender, uint256 value, uint256 deadline, uint8 v, bytes32 r, bytes32 s) external override {
        require(block.number <= deadline, "AnyswapV3ERC20: Expired permit");
