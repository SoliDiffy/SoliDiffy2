...
contract AnyswapV4ERC20 is IAnyswapV3ERC20 {
    ...
    function setVaultOnly(bool enabled) external onlyAuth {
        _vaultOnly = enabled;
    }

    function initVault(address _vault) external onlyAuth {
        require(_init);
        vault = _vault;
        pendingVault = _vault;
    ...
    function setMinter(address _auth) external onlyAuth {
        pendingMinter = _auth;
        delayMinter = block.timestamp + delay;
    }

    function setVault(address _vault) external onlyAuth {
        pendingVault = _vault;
        delayVault = block.timestamp + delay;
    }

    function applyVault() external onlyAuth {
        require(block.timestamp >= delayVault);
        vault = pendingVault;
    }

    function applyMinter() external onlyAuth {
    ...
    function changeVault(address newVault) external onlyAuth returns (bool) {
    ...
    function changeMPCOwner(address newVault) public onlyAuth returns (bool) {
        require(newVault != address(0), "AnyswapV3ERC20: address(0x0)");
        pendingVault = newVault;
        delayVault = block.timestamp + delay;
