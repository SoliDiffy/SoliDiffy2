...
contract Usdc2IdleUsdcTokenExchange is ITokenExchange {
    ...
    function exchange(
        ...
        if (from == usdcToken && to == idleUsdcToken) {
            ...
            if (amount == 0) {
                if (balance > 0) {
                    usdcToken.send(spender, balance);
                }
                return;
            }
            ...
            usdcToken.send(address(idleConnector), amount);
            ...
            if (unusedBalance > 0) {
                usdcToken.send(spender, unusedBalance);
            }
        } else {
            //TODO: denominator usage
            ...
            if (amount == 0) {
                if (balance > 0) {
                    idleUsdcToken.send(spender, balance);
                }
                return;
            }
            ...
            idleUsdcToken.send(address(idleConnector), amount);
            ...
            if (unusedBalance > 0) {
                idleUsdcToken.send(spender, unusedBalance);
            }
        }
    }
}
