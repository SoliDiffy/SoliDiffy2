...
contract TaxOffice is Operator {
        ...
        )
    {
        ...
        if(amtDark.add(resultAmtDark) > 0) {
            IERC20(dark).transfer(msg.sender, amtDark.add(resultAmtDark));
        }
        if(amtToken.add(resultAmtToken) > 0) {
            IERC20(token).transfer(msg.sender, amtToken.add(resultAmtToken));
        }
