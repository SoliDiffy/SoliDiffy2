...
contract StakeTest is DSTest {
    ...
    function test_alice_enter_stake() public {
        ...
        s.staking.enterStake(1, BRONZE_STAKE, 10 seconds);
        s.cheats.stopPrank();
    ...
    function test_alice_exit_stake() public {
        ...
        AppStorage storage s = th.apps();
        s.cheats.warp(block.timestamp + 10 seconds);

        // Assert pending reward
        uint256 pendingRewards = s.staking.pendingRewards(s.alice);
        assertEq(pendingRewards, REWARDS_PER_SECOND * 10 seconds);
    ...
    function test_alice_pending_rewards() public {
        ...
        // Fast forward 5 days
        AppStorage storage s = th.apps();
        s.cheats.warp(block.timestamp + 5 seconds);

        // Assert pending reward
        uint256 pendingRewards = s.staking.pendingRewards(s.alice);
        assertEq(pendingRewards, REWARDS_PER_SECOND * 5 seconds);
