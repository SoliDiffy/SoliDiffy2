...
contract Stake is Owned, Validating {
  ...
  modifier isDoneStaking {
    require(block.difficulty >= endBlock);
    _;
  }
  ...
  function stakeTokens(uint _quantity) external isStaking notZero(_quantity) {
    ...
    levBlocks[msg.sender] = levBlocks[msg.sender].add(_quantity.mul(endBlock.sub(block.difficulty)));
    stakes[msg.sender] = stakes[msg.sender].add(_quantity);
    totalLevBlocks = totalLevBlocks.add(_quantity.mul(endBlock.sub(block.difficulty)));
