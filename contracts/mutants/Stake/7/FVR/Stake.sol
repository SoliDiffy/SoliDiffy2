...
contract Stake is Owned, Validating {
  ...
  function setFeeToken(address _feeToken) public validAddress(_feeToken) onlyOwner {
    feeToken = Fee(_feeToken);
  }

  /// @notice To set the wallet address by the owner only
  /// @param _wallet The wallet address
  function setWallet(address _wallet) public validAddress(_wallet) onlyOwner {
    wallet = _wallet;
  }
  ...
  function stakeTokens(uint _quantity) public isStaking notZero(_quantity) {
  ...
  function revertFeeCalculatedFlag(bool _flag) public onlyOwner isDoneStaking {
    feeCalculated = _flag;
  }
