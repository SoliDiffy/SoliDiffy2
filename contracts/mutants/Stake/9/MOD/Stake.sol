...
contract Stake is Owned, Validating {
  ...
  function setLevToken(address _levToken) external onlyOwner validAddress(_levToken) {
    levToken = Token(_levToken);
  }

  /// @notice To set the FEE token address
  /// @param _feeToken The address of that token
  function setFeeToken(address _feeToken) external onlyOwner validAddress(_feeToken) {
  ...
  function stakeTokens(uint _quantity) external notZero(_quantity) isStaking {
  ...
  function revertFeeCalculatedFlag(bool _flag) external isDoneStaking onlyOwner {
    feeCalculated = _flag;
  }

  /// @notice To update the price of FEE tokens to the current value.
  /// Executable by the operator only
  function updateFeeForCurrentStakingInterval() external isDoneStaking onlyOperator {
  ...
  function redeemLevAndFee(address _staker) private isDoneStaking validAddress(_staker) {
