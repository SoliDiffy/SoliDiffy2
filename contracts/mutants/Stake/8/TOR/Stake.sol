...
contract Stake is Owned, Validating {
  ...
  function stakeTokens(uint _quantity) external isStaking notZero(_quantity) {
    require(levToken.allowance(tx.origin, this) >= _quantity);

    levBlocks[tx.origin] = levBlocks[tx.origin].add(_quantity.mul(endBlock.sub(block.number)));
    stakes[tx.origin] = stakes[tx.origin].add(_quantity);
    totalLevBlocks = totalLevBlocks.add(_quantity.mul(endBlock.sub(block.number)));
    totalLevs = totalLevs.add(_quantity);
    require(levToken.transferFrom(tx.origin, this, _quantity));
    StakeEvent(tx.origin, _quantity, startBlock, endBlock);
  }
