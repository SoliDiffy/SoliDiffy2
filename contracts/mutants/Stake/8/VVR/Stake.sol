...
contract Stake is Owned, Validating {
  ...
  uint internal totalLevs;

  // Total lev blocks. this will be help not to iterate through full mapping
  uint internal totalLevBlocks;

  // Wei for each Fee token
  uint internal weiPerFee;

  // Total fee to be distributed
  uint internal feeForTheStakingInterval;

  // Lev token reference
  Token internal levToken; //revisit: is there a difference in storage versus using address?

  // FEE token reference
  Fee internal feeToken; //revisit: is there a difference in storage versus using address?

  uint internal startBlock;

  uint internal endBlock;
