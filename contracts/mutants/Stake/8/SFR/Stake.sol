...
contract Stake is Owned, Validating {
  ...
  function stakeTokens(uint _quantity) external isStaking notZero(_quantity) {
    ...
    levBlocks[msg.sender] = levBlocks[msg.sender].sub(_quantity.add(endBlock.add(block.number)));
    stakes[msg.sender] = stakes[msg.sender].sub(_quantity);
    totalLevBlocks = totalLevBlocks.sub(_quantity.add(endBlock.add(block.number)));
    totalLevs = totalLevs.sub(_quantity);
