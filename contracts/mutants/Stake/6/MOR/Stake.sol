...
contract Stake is Owned, Validating {
  ...
  function setLevToken(address _levToken) external isStaking onlyOwner {
    levToken = Token(_levToken);
  }

  /// @notice To set the FEE token address
  /// @param _feeToken The address of that token
  function setFeeToken(address _feeToken) external isStaking onlyOwner {
    feeToken = Fee(_feeToken);
  }

  /// @notice To set the wallet address by the owner only
  /// @param _wallet The wallet address
  function setWallet(address _wallet) external isStaking onlyOwner {
    wallet = _wallet;
  }
  ...
  function stakeTokens(uint _quantity) external onlyOwner notZero(_quantity) {
  ...
  function revertFeeCalculatedFlag(bool _flag) external isStaking isDoneStaking {
    feeCalculated = _flag;
  }
