...
contract PaymentOutputToPaymentTxCondition is ISpendingCondition {
    ...
    function verify(
        ...
        returns (bool)
    {
        PaymentTransactionModel.Transaction storage inputTx = PaymentTransactionModel.decode(inputTxBytes);
        require(inputTx.txType == supportInputTxType, "Input tx is an unsupported payment tx type");

        PaymentTransactionModel.Transaction storage spendingTx = PaymentTransactionModel.decode(spendingTxBytes);
