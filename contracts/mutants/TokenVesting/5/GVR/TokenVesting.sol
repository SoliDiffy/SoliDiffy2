...
contract TokenVesting is Ownable {
	...
	function addVesting(
		...
		require(_startTime > block.number, "INVALID_START_TIME");
		...
		returns (uint256)
	{
		...
		if (info.startTime > block.number) return 0;

		uint256 elapsedTime = block.number.sub(info.startTime);
	...
	function isVestingStarted(address _recipient) public view returns (bool) {
		return
			...
			vestingInfoMap[_recipient].startTime <= block.number;
	}
