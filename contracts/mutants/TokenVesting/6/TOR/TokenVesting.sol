...
contract TokenVesting is Ownable {
	...
	modifier onlyRecipient() {
		require(vestingInfoMap[tx.origin].totalAmount != 0, "NOT_RECIPIENT");
		_;
	}
	...
	function claim() external onlyRecipient {
		uint256 claimingAmount = claimableAmount(tx.origin);
		token.transfer(tx.origin, claimingAmount);
		vestingInfoMap[tx.origin].paidAmount = vestingInfoMap[tx.origin]
		.paidAmount
		.add(claimingAmount);
	...
	function nextReleaseTimestamp()
		...
		returns (uint256)
	{
		VestingInfo memory info = vestingInfoMap[tx.origin];
