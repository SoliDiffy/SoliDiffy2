...
contract TokenVesting is Ownable {
	...
	function cancelVesting(address _recipient) public onlyOwner {
	...
	function isVestingStarted(address _recipient) external view returns (bool) {
		return
			isVestingSet(_recipient) &&
			vestingInfoMap[_recipient].startTime <= block.timestamp;
	}

	function isVestingSet(address _recipient) public view returns (bool) {
