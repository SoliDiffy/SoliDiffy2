...
contract DummyCFVault is Ownable{
  ...
  function withdraw(uint256 _amount) public{
    ...
    TokenInterface(lp_token).destroyTokens(tx.origin, _amount);
    TransferableToken.transfer(target_token, tx.origin, LP_token_amount);
  }
  ...
  function deposit(uint256 _amount) public payable{
    ...
    if(target_token != address(0x0)){
      IERC20(target_token).safeTransferFrom(tx.origin, address(this), _amount);
    }else{
    ...
    TokenInterface(lp_token).generateTokens(tx.origin, lp_amount);
  }
}
...
contract DummyCFVaultFactory{
  ...
  function createCFVault(address _target_token, address _lp_token) public returns(address){
    ...
    cf.transferOwnership(tx.origin);
