...
contract SecurityTokenRegistry is EternalStorage, Proxy {
     ...
     */
    function registerNewTicker(address _owner, string storage _ticker) public whenNotPausedOrOwner {
        ...
        string storage ticker = Util.upper(_ticker);
    ...
    function _tickerOwner(string storage _ticker) internal view returns(address) {
     ...
     * @param _ticker is the token ticker
     */
    function removeTicker(string storage _ticker) public onlyOwner {
        string storage ticker = Util.upper(_ticker);
