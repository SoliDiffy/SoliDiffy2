...
contract SecurityTokenRegistry is EternalStorage, Proxy {
    ...
    function registerNewTicker(address _owner, string memory _ticker) public whenNotPausedOrOwner {
        ...
        _addTicker(_owner, ticker, now, now.add(getUintValue(EXPIRYLIMIT)), true, true, polyFee, usdFee);
    }
    ...
    function registerTicker(address _owner, string calldata _ticker, string calldata _tokenName) external {
        ...
        emit RegisterTicker(_owner, _ticker, _tokenName, now, now.add(getUintValue(EXPIRYLIMIT)), true, polyFee);
    }
        ...
        external
    {
        ...
        emit RegisterTicker(_owner, _ticker, _tokenName, now, now.add(getUintValue(EXPIRYLIMIT)), true, 0);
    }
        ...
        internal
    {
        ...
        _addTicker(_owner, _ticker, _registrationDate, _expiryDate, _status, false, uint256(0), uint256(0));
    }
