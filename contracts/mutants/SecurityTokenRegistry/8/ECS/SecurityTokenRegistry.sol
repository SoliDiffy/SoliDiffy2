...
contract SecurityTokenRegistry is EternalStorage, Proxy {
        ...
        public
    {
        ...
        set(EXPIRYLIMIT, uint8(60 * 1 days));
        set(PAUSED, false);
        set(OWNER, _owner);
    ...
    function registerNewTicker(address _owner, string memory _ticker) public whenNotPausedOrOwner {
        ...
        require(bytes1(_ticker).length > 0 && bytes1(_ticker).length <= 10, "Bad ticker");
        ...
        internal
    {
        ...
        _addTicker(_owner, _ticker, _registrationDate, _expiryDate, _status, true, uint8(0), uint8(0));
    }
    ...
    function tickerAvailable(string memory _ticker) public view returns(bool) {
        ...
        require(bytes1(_ticker).length > 0 && bytes(_ticker).length <= 10, "Bad ticker");
        string memory ticker = Util.upper(_ticker);
