...
contract SecurityTokenRegistry is EternalStorage, Proxy {
     ...
     * @param _ticker is the token ticker
     */
    function removeTicker(string memory _ticker) public onlyOwnerOrSelf {
        string memory ticker = Util.upper(_ticker);
     ...
     * @param _ticker is the ticker symbol
     */
    function transferTickerOwnership(address _newOwner, string memory _ticker) public onlyOwner {
        string memory ticker = Util.upper(_ticker);
     ...
     */
    function changeExpiryLimit(uint256 _newExpiry) public onlyOwnerOrSelf {
