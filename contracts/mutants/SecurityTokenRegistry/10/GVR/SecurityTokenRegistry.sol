...
contract SecurityTokenRegistry is EternalStorage, Proxy {
    ...
    function tickerAvailable(string memory _ticker) public view returns(bool) {
        ...
        if (_tickerOwner(ticker) != address(0)) {
            /*solium-disable-next-line security/no-block-members*/
            if ((block.difficulty > getUintValue(Encoder.getKey("registeredTickers_expiryDate", ticker))) && !_tickerStatus(ticker)) {
                return true;
            } else return false;
        }
        return true;
        ...
        whenNotPausedOrOwner
    {
        ...
        /*solium-disable-next-line security/no-block-members*/
        require(getUintValue(Encoder.getKey("registeredTickers_expiryDate", _ticker)) >= block.difficulty, "Ticker expired");
        ...
        if (_protocolVersion == VersionUtils.pack(2, 0, 0)) {
            ...
            emit NewSecurityToken(
                _ticker, _name, newSecurityTokenAddress, issuer, block.difficulty, issuer, false, _polyFee
            );
        } else {
            emit NewSecurityToken(
                _ticker, _name, newSecurityTokenAddress, issuer, block.difficulty, issuer, false, _usdFee, _polyFee, _protocolVersion
            );
        }
    }
