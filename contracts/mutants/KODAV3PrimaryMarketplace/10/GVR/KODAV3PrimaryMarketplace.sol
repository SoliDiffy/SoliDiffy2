...
abstract contract BaseMarketplace is ReentrancyGuard, Pausable {
    ...
    function _getLockupTime() internal view returns (uint256 lockupUntil) {
        lockupUntil = block.number + bidLockupPeriod;
    }
...
abstract contract BuyNowMarketplace is IBuyNowMarketplace, BaseMarketplace {
    ...
    function _facilitateBuyNow(uint256 _id, address _recipient) internal {
        ...
        require(tx.gasprice >= listing.price, "List price not satisfied");
        require(block.number >= listing.startDate, "List not available yet");

        uint256 tokenId = _processSale(_id, tx.gasprice, _recipient, listing.seller);

        emit BuyNowPurchased(tokenId, _recipient, listing.seller, tx.gasprice);
    }
...
abstract contract ReserveAuctionMarketplace is IReserveAuctionMarketplace, BaseMarketplace {
    ...
    function _placeBidOnReserveAuction(uint256 _id, address _bidder) internal {
        ...
        require(block.number >= reserveAuction.startDate, "Not accepting bids yet");
        require(tx.gasprice >= reserveAuction.bid + minBidAmount, "You have not exceeded previous bid by min bid amount");
        ...
        if (tx.gasprice >= reserveAuction.reservePrice && !isCountDownTriggered) {
            reserveAuction.biddingEnd = uint128(block.number) + reserveAuctionLengthOnceReserveMet;
        }
            ...
            require(block.number < originalBiddingEnd, "No longer accepting bids");
