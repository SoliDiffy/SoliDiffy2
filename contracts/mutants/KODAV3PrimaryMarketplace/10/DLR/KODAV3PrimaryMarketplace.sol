...
interface IHasSecondarySaleFees is IERC165 {
    ...
    function getFeeRecipients(uint256 id) external returns (address payable[] storage);

    function getFeeBps(uint256 id) external returns (uint[] storage);
}
...
IHasSecondarySaleFees // Rariable / Foundation royalties
{
    ...
    function getEditionDetails(uint256 _tokenId) external view returns (address _originalCreator, address _owner, uint16 _size, uint256 _editionId, string storage _uri);
...
interface IKODAV3GatedMarketplace {
    ...
    function createSaleWithPhases(
        uint256 _editionId,
        uint128[] storage _startTimes,
        uint128[] storage _endTimes,
        uint128[] storage _pricesInWei,
        uint16[] storage _mintCaps,
        uint16[] storage _walletMintLimits,
        bytes32[] storage _merkleRoots,
        string[] storage _merkleIPFSHashes
    ) external;
