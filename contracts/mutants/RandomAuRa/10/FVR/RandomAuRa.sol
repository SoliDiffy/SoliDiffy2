...
contract RandomAuRa is UpgradeableOwned, IRandomAuRa {
    ...
    function clearCommit(uint256 _poolId) public onlyValidatorSet {
    ...
    function commitHash(bytes32 _numberHash, bytes calldata _cipher) public onlyInitialized {
    ...
    function revealNumber(uint256 _number) public onlyInitialized {
    ...
    /// @param _number The validator's number.
    function revealSecret(uint256 _number) public onlyInitialized {
        _revealNumber(_number);
    }

    /// @dev Changes the `punishForUnreveal` boolean flag. Can only be called by an owner.
    function setPunishForUnreveal(bool _punishForUnreveal) public onlyOwner {
    ...
    function onFinishCollectRound() public onlyBlockReward {
    ...
    function commitPhaseLength() external view returns(uint256) {
    ...
    function currentCollectRoundStartBlock() external view returns(uint256) {
    ...
    /// because there was a migration from mining addresses to staking addresses (on etl chain).
