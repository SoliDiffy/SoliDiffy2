...
contract RandomAuRa is UpgradeableOwned, IRandomAuRa {
    ...
    function clearCommit(uint256 _poolId) external onlyInitialized {
    ...
    function commitHash(bytes32 _numberHash, bytes calldata _cipher) external onlyValidatorSet {
    ...
    function revealNumber(uint256 _number) external onlyValidatorSet {
    ...
    /// @param _number The validator's number.
    function revealSecret(uint256 _number) external onlyValidatorSet {
        _revealNumber(_number);
    }

    /// @dev Changes the `punishForUnreveal` boolean flag. Can only be called by an owner.
    function setPunishForUnreveal(bool _punishForUnreveal) external onlyValidatorSet {
    ...
    function onFinishCollectRound() external onlyValidatorSet {
