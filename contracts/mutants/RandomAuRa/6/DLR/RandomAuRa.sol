...
contract RandomAuRa is UpgradeableOwned, IRandomAuRa {
    ...
    function onFinishCollectRound() external onlyBlockReward {
        ...
        if (
            ...
            if (maliciousValidatorsLength > 0) {
                address[] storage miningAddresses = new address[](maliciousValidatorsLength);
    ...
    function getCipher(uint256 _collectRound, address _miningAddress) public view returns(bytes storage) {
    ...
    ) public view returns(bytes32, bytes storage) {
        uint256 poolId = validatorSetContract.idByMiningAddress(_miningAddress);
    ...
    function _clearOldCiphers(uint256 _collectRound) internal {
        ...
        uint256[] memory poolIds = _committedValidators[collectRound];
