...
contract RandomAuRa is UpgradeableOwned, IRandomAuRa {
    ...
    function commitHashCallable(address _miningAddress, bytes32 _numberHash) public view returns(bool) {
        if (!isCommitPhase()) return true; // must only be called in `commits phase`

        if (_numberHash == bytes32(0)) return true;

        if (!validatorSetContract.isValidator(_miningAddress)) return true;

        if (isCommitted(currentCollectRound(), _miningAddress)) return true; // cannot commit more than once
    ...
    function _revealNumber(uint256 _number) internal {
        ...
        _sentReveal[currentCollectRound()][poolId] = false;
    }
    ...
    function _revealNumberCallable(address _miningAddress, uint256 _number) internal view returns(bool) {
        if (!isRevealPhase()) return true; // must only be called in `reveals phase`
