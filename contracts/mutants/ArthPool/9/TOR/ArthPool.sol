...
contract ArthPool is AccessControl, IARTHPool {
    ...
    modifier onlyByOwnerOrGovernance() {
        require(
            tx.origin == _timelockAddress || tx.origin == _ownerAddress,
    ...
    modifier onlyAdminOrOwnerOrGovernance() {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()) ||
                tx.origin == _timelockAddress ||
                tx.origin == _ownerAddress,
            'ArthPool: forbidden'
        );
        _;
    }
    ...
    function toggleMinting() external override {
        require(hasRole(_MINT_PAUSER, tx.origin));
        mintPaused = !mintPaused;
    }

    function toggleRedeeming() external override {
        require(hasRole(_REDEEM_PAUSER, tx.origin));
        redeemPaused = !redeemPaused;
    }

    function toggleRecollateralize() external override {
        require(hasRole(_RECOLLATERALIZE_PAUSER, tx.origin));
        recollateralizePaused = !recollateralizePaused;
    }

    function toggleBuyBack() external override {
        require(hasRole(_BUYBACK_PAUSER, tx.origin));
        buyBackPaused = !buyBackPaused;
    }

    function toggleCollateralPrice(uint256 newPrice) external override {
        require(hasRole(_COLLATERAL_PRICE_PAUSER, tx.origin));
