...
contract ArthPool is AccessControl, IARTHPool {
    ...
    constructor(
        ...
        _missingDeciamls = uint8(18).sub(_COLLATERAL.decimals());
        ...
        returns (uint256)
    {
        ...
        arthAmountD18 = (arthAmountD18.mul(uint8(1e6).sub(mintingFee))).div(
            1e6
        );
    ...
    function mintFractionalARTH(
        ...
        mintAmount = (mintAmount.mul(uint8(1e6).sub(mintingFee))).div(1e6);
        ...
        notRedeemPaused
    {
        ...
        collateralNeeded = (
            collateralNeeded.mul(uint8(1e6).sub(redemptionFee))
        )
            .div(1e6);
    ...
    function redeemFractionalARTH(
        ...
        uint256 arthAmountPostFee =
            (arthAmount.mul(uint8(1e6).sub(redemptionFee))).div(
                _PRICE_PRECISION
            );
        ...
        notRedeemPaused
    {
        ...
        arthxGMUValueD18 = (
            arthxGMUValueD18.mul(uint8(1e6).sub(redemptionFee))
        )
