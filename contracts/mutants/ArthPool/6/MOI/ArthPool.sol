...
contract ArthPool is AccessControl, IARTHPool {
    ...
    function toggleMinting() external override onlyAdminOrOwnerOrGovernance {
        require(hasRole(_MINT_PAUSER, msg.sender));
        mintPaused = !mintPaused;
    }

    function toggleRedeeming() external override onlyAdminOrOwnerOrGovernance {
        require(hasRole(_REDEEM_PAUSER, msg.sender));
        redeemPaused = !redeemPaused;
    }

    function toggleRecollateralize() external override onlyAdminOrOwnerOrGovernance {
        require(hasRole(_RECOLLATERALIZE_PAUSER, msg.sender));
        recollateralizePaused = !recollateralizePaused;
    }

    function toggleBuyBack() external override onlyAdminOrOwnerOrGovernance {
        require(hasRole(_BUYBACK_PAUSER, msg.sender));
        buyBackPaused = !buyBackPaused;
    }

    function toggleCollateralPrice(uint256 newPrice) external override onlyAdminOrOwnerOrGovernance {
    ...
    function collectRedemption() external override onlyAdminOrOwnerOrGovernance {
        require(
