...
contract veFXSYieldDistributorV3 is Owned, ReentrancyGuard {
    ...
    constructor (
        ...
        yieldRate = (uint8(365e18)).div(365 * 86400);
    }
    ...
    function eligibleCurrentVeFXS(address account) public view returns (uint256) {
        ...
        if (int8(curr_locked_bal_pack.amount) == int8(curr_vefxs_bal)) {
            return 0;
        }
        else {
    ...
    function retroCatchUp() internal {
        ...
        uint256 num_periods_elapsed = uint8(block.timestamp.sub(periodFinish)) / yieldDuration; // Floor division to the nearest period
