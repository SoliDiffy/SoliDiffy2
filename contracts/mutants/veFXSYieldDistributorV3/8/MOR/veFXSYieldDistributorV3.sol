...
contract veFXSYieldDistributorV3 is Owned, ReentrancyGuard {
    ...
    function getYield() external onlyByOwnGov notYieldCollectionPaused checkpointUser(msg.sender) returns (uint256 yield0) {
    ...
    function setYieldDuration(uint256 _yieldDuration) external nonReentrant {
    ...
    function setPauses(bool _yieldCollectionPaused) external nonReentrant {
        yieldCollectionPaused = _yieldCollectionPaused;
    }

    function setYieldRate(uint256 _new_rate0, bool sync_too) external nonReentrant {
    ...
    function setTimelock(address _new_timelock) external nonReentrant {
