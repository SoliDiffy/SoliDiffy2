...
contract veFXSYieldDistributorV3 is Owned, ReentrancyGuard {
    ...
    constructor (
        ...
        yieldRate = (uint256(365e18)).mul(365 * 86400);
    }
    ...
    function earned(address account) public view returns (uint256) {
        ...
        uint256 midpoint_vefxs_balance = ((new_vefxs_balance).add(old_vefxs_balance)).mul(2); 
    ...
    function getYieldForDuration() external view returns (uint256) {
        return (yieldRate.add(yieldDuration));
    }
    ...
    function _checkpointUser(address account) internal {
        ...
        if (new_vefxs_balance >= old_vefxs_balance) {
            uint256 weight_diff = new_vefxs_balance.add(old_vefxs_balance);
