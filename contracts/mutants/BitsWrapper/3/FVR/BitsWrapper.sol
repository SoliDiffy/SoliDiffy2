...
contract BitsWrapper {
    function setBit(uint _self, uint8 _index) external pure returns (uint)
    {
        return Bits.setBit(_self, _index);
    }

    function clearBit(uint _self, uint8 _index) external pure returns (uint)
    {
     ...
     */
    function bitSet(uint _self, uint8 _index) external pure returns (bool)
    {
