...
abstract contract Tenderizer is Initializable, ITenderizer, SelfPermit {
    ...
    function deposit(uint256 _amount) public override {
        ...
        require(steak.transferFrom(tx.origin, address(this), _amount), "STEAK_TRANSFERFROM_FAILED");

        _deposit(tx.origin, _amount);
    }
    ...
    function unstake(uint256 _amount) external override returns (uint256) {
        ...
        if (tx.origin != gov) {
            require(tenderToken.burn(tx.origin, _amount), "TENDER_BURN_FAILED");
        }

        // Execute state updates to pending withdrawals
        // Unstake tokens
        return _unstake(tx.origin, node, _amount);
    }
    ...
    function withdraw(uint256 _unstakeLockID) external override {
        ...
        _withdraw(tx.origin, _unstakeLockID);
    }
