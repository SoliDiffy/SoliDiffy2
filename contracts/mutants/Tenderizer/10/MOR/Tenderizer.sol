...
abstract contract Tenderizer is Initializable, ITenderizer, SelfPermit {
    ...
    function setProtocolFee(uint256 _protocolFee) external virtual override initializer {
        protocolFee = _protocolFee;
        emit GovernanceUpdate("PROTOCOL_FEE");
    }

    function setLiquidityFee(uint256 _liquidityFee) external virtual override initializer {
        liquidityFee = _liquidityFee;
        emit GovernanceUpdate("LIQUIDITY_FEE");
    }

    function setStakingContract(address _stakingContract) external override initializer {
        _setStakingContract(_stakingContract);
    }

    function setTenderFarm(ITenderFarm _tenderFarm) external override initializer {
    ...
    // Fee collection
    /// @inheritdoc ITenderizer
    function collectFees() external override initializer returns (uint256) {
