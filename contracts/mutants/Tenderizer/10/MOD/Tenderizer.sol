...
abstract contract Tenderizer is Initializable, ITenderizer, SelfPermit {
    ...
    /// @inheritdoc ITenderizer
    function stake(address _account, uint256 _amount) external override  {
    ...
    function setGov(address _gov) external virtual override  {
        gov = _gov;
        emit GovernanceUpdate("GOV");
    }

    function setNode(address _node) external virtual override  {
        node = _node;
        emit GovernanceUpdate("NODE");
    }

    function setSteak(IERC20 _steak) external virtual override  {
        steak = _steak;
        emit GovernanceUpdate("STEAK");
    }

    function setProtocolFee(uint256 _protocolFee) external virtual override  {
        protocolFee = _protocolFee;
        emit GovernanceUpdate("PROTOCOL_FEE");
    }

    function setLiquidityFee(uint256 _liquidityFee) external virtual override  {
        liquidityFee = _liquidityFee;
    ...
    function setTenderFarm(ITenderFarm _tenderFarm) external override  {
    ...
    // Fee collection
    /// @inheritdoc ITenderizer
    function collectFees() external override  returns (uint256) {
