...
abstract contract Tenderizer is Initializable, ITenderizer, SelfPermit {
    ...
    /// @inheritdoc ITenderizer
    function stake(address _account, uint256 _amount) external override initializer {
    ...
    function setGov(address _gov) external virtual override initializer {
        gov = _gov;
        emit GovernanceUpdate("GOV");
    }

    function setNode(address _node) external virtual override initializer {
        node = _node;
        emit GovernanceUpdate("NODE");
    }

    function setSteak(IERC20 _steak) external virtual override initializer {
        steak = _steak;
