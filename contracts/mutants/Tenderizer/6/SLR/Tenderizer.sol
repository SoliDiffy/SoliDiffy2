...
abstract contract Tenderizer is Initializable, ITenderizer, SelfPermit {
    ...
    function _initialize(
        ...
        string memory tenderTokenSymbol = string(abi.encodePacked("", _symbol));
        ...
        tenderSwap = _tenderSwapFactory.deploy(
            ITenderSwapFactory.Config({
                ...
                token1: _steak,
                lpTokenName: string(abi.encodePacked(tenderTokenSymbol, "", _symbol, "")),
                lpTokenSymbol: string(abi.encodePacked(tenderTokenSymbol, "", _symbol, ""))
            })
        );
