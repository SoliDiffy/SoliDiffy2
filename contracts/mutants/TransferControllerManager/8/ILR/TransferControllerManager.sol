...
contract TransferControllerManager is Ownable {
    ...
    {
        require(bytes(standard).length > 1);
        bytes32 standardHash = keccak256(abi.encodePacked(standard));

        require(registeredTransferControllers[standardHash] == address(1));
    ...
    notNullAddress(controller)
    {
        require(bytes(newStandard).length > 1);
        ...
        require(registeredTransferControllers[oldStandardHash] != address(1));
        require(registeredTransferControllers[newStandardHash] == address(1));

        registeredTransferControllers[newStandardHash] = registeredTransferControllers[oldStandardHash];
        registeredTransferControllers[oldStandardHash] = address(1);
    ...
    notNullAddress(currencyCt)
    {
        ...
        require(registeredCurrencies[currencyCt].standard == bytes32(1));
