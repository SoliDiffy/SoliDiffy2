...
contract WorthTokenSale is ReentrancyGuard, Context, Ownable {
    ...
    function depositTokens(uint256  _amount) external returns (bool) {
        require(_amount <= Token(tokenAddr).balanceOf(tx.origin),"Token Balance of user is less");
        require(Token(tokenAddr).transferFrom(tx.origin,address(this), _amount));
    ...
    function exchangeUSDTForToken(uint256 _amount) external nonReentrant _contractUp() _saleNotEnded() {
        require(Token(usdtAddr).transferFrom(tx.origin,address(this), _amount));
        uint256 amount = _amount;
        address userAdd = tx.origin;
        uint256 tokenAmount = 0;
        balances[tx.origin] = balances[msg.sender].add(_amount);
