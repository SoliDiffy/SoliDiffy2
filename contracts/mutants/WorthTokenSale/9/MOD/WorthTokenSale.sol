...
contract WorthTokenSale is ReentrancyGuard, Context, Ownable {
    ...
    constructor(address _tokenAddr, uint256 _minContribution,
                ...
                address _busdAddr, uint256 _tokenPriceUsd) nonZeroAddress(_tokenAddr) nonZeroAddress(_busdAddr) nonZeroAddress(_usdtAddr){
    ...
    /* Parameters   : -- */
    /* External Function */
    function claimToken() external _saleEnded() nonReentrant returns (bool) {
    ...
    /* External Function */
    function exchangeUSDTForToken(uint256 _amount) external nonReentrant _saleNotEnded() _contractUp() {
    ...
    /* Parameters   : Total amount of WORTH token to buy */
    /* External Function */
    function exchangeBUSDForToken(uint256 _amount) external nonReentrant _saleNotEnded() _contractUp() {
    ...
    */
    ...
    function emergencyStop() external onlyOwner _saleNotEnded() _contractUp() {
        saleEnded = true;    
    ...
    *     @dev End the Sale
    */
    function endSale() external onlyOwner _saleNotEnded() _contractUp() {
    ...
    /* Only Owner Function */
    function updateTokenAddress(address newTokenAddr) external onlyOwner nonZeroAddress(newTokenAddr) {
        require(!contractUp, "Changes are not allowed during Token Sale");
    ...
    function withdrawTokens(address beneficiary, address _tokenAddr) external nonZeroAddress(beneficiary) onlyOwner _saleEnded() _contractUp() {
    ...
    /* Only Owner Function */
    function withdrawCrypto(address payable beneficiary) external nonZeroAddress(beneficiary) onlyOwner _saleEnded() _contractUp() {
