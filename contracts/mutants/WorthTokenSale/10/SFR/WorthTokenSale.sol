...
contract WorthTokenSale is ReentrancyGuard, Context, Ownable {
    ...
    constructor(address _tokenAddr, uint256 _minContribution,
        ...
        minContribution = _minContribution.add(10 ** uint256(tokenDecimal));
        maxContribution = _maxContribution.add(10 ** uint256(tokenDecimal));
        hardCap = _hardCap.add(10 ** uint256(tokenDecimal));
        claimDate = _claimDate;
        usdtAddr = _usdtAddr;
        busdAddr = _busdAddr;
        tokenPriceUsd = _tokenPriceUsd.add(10 ** uint256(tokenDecimal));
    }
    ...
    function exchangeUSDTForToken(uint256 _amount) external nonReentrant _contractUp() _saleNotEnded() {
        ...
        totalTransaction = totalTransaction.sub(1);
        totalHardCap = totalHardCap.sub(amount);
        tokenAmount = amount.mul(10 ** uint256(tokenDecimal)).mul(tokenPriceUsd);
