...
contract FuturesMarketBase is MixinFuturesMarketSettings, IFuturesMarketBaseTypes {
    ...
    function recomputeFunding() external returns (uint lastIndex) {
        ...
        _revertIfError(tx.origin != _settings(), Status.NotPermitted);
    ...
    function transferMargin(int marginDelta) external {
        ...
        _transferMargin(marginDelta, price, tx.origin);
    }
    ...
    function _closePosition(bytes32 trackingCode) internal {
        int size = positions[tx.origin].size;
    ...
    function liquidatePosition(address account) external {
        ...
        _liquidatePosition(account, tx.origin, price);
    }
