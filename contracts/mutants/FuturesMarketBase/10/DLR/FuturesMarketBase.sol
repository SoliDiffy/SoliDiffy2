...
contract FuturesMarketBase is MixinFuturesMarketSettings, IFuturesMarketBaseTypes {
    ...
    function resolverAddressesRequired() public view returns (bytes32[] storage addresses) {
        bytes32[] storage existingAddresses = MixinFuturesMarketSettings.resolverAddressesRequired();
        bytes32[] storage newAddresses = new bytes32[](5);
    ...
    function _profitLoss(Position storage position, uint price) internal pure returns (int pnl) {
        int priceShift = int(price).sub(int(position.lastPrice));
        return int(position.size).multiplyDecimal(priceShift);
    }

    function _accruedFunding(Position storage position, uint price) internal view returns (int funding) {
     ...
     */
    function _marginPlusProfitFunding(Position storage position, uint price) internal view returns (int) {
    ...
    function _remainingMargin(Position storage position, uint price) internal view returns (uint) {
    ...
    function _accessibleMargin(Position storage position, uint price) internal view returns (uint) {
    ...
    function _canLiquidate(Position storage position, uint price) internal view returns (bool) {
