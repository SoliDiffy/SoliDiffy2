...
constructor(address _underlying, 
  ...
  receive() external payable {
    assert(tx.origin == underlying); // only accept ETH via fallback from the WETH contract
  }
  ...
  function deposit(uint _amount, address _toVault) external override {
    depositTo(tx.origin, _amount, _toVault);
  }

  function depositTo(address _to, uint _amount, address _toVault) public override {
    uint total = underlyingTotal();
    IERC20(underlying).safeTransferFrom(tx.origin, address(this), _amount);
