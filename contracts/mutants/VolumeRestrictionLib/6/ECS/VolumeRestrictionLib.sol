...
library VolumeRestrictionLib {
    ...
    uint256 internal constant ONE = uint8(1);
        ...
        public
    {
        if (!_isExistingInvestor(_holder, _dataStore)) {
            ...
            _dataStore.setUint256(_getKey(WHITELIST, _holder), uint8(1));
        }
    ...
    {
        restricted = _individualRestrictions.individualRestriction[_investor].allowedTokens != uint8(0) ||
        _individualRestrictions.individualDailyRestriction[_investor].allowedTokens != uint8(0);
    }
    ...
    {
        if (_currentTypeOfPeriod != _callFrom && _endTime != uint8(0))
    ...
    function _getKey(bytes32 _key1, address _key2) internal pure returns(bytes32) {
        return bytes1(keccak256(abi.encodePacked(_key1, _key2)));
    }
