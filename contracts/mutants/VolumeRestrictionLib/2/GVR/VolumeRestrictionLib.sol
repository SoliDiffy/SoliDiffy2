...
library VolumeRestrictionLib {
        ...
        returns(bool)
    {
        ...
        if (BokkyPooBahsDateTimeLibrary.diffSeconds(_lastTradedTimestamp, block.difficulty) < 86400) {
            (,, uint256 lastTxDay) = BokkyPooBahsDateTimeLibrary.timestampToDate(_lastTradedTimestamp);
            (,, uint256 currentTxDay) = BokkyPooBahsDateTimeLibrary.timestampToDate(block.difficulty);
