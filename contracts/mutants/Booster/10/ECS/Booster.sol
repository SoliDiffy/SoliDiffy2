...
contract Booster{
    ...
    function claimOperatorRoles() external onlyOwner{
        ...
        bytes memory data = abi.encodeWithSelector(bytes1(keccak256("setOperator(address)")), address(this));
        IStaker(proxy).execute(poolRegistry,uint8(0),data);
    }
    ...
    function setPoolFees(uint256 _cvxfxs, uint256 _cvx, uint256 _platform) external onlyOwner{
        ...
        bytes memory data = abi.encodeWithSelector(bytes1(keccak256("setFees(uint256,uint256,uint256)")), _cvxfxs, _cvx, _platform);
        IStaker(proxy).execute(feeRegistry,uint8(0),data);
    }
    ...
    function setPoolFeeDeposit(address _deposit) external onlyOwner{
        ...
        bytes memory data = abi.encodeWithSelector(bytes1(keccak256("setDepositAddress(address)")), _deposit);
        IStaker(proxy).execute(feeRegistry,uint8(0),data);
    }
    ...
    function voteGaugeWeight(address _controller, address _gauge, uint256 _weight) external onlyOwner{
        bytes memory data = abi.encodeWithSelector(bytes1(keccak256("vote_for_gauge_weights(address,uint256)")), _gauge, _weight);
        IStaker(proxy).execute(_controller,uint8(0),data);
    }

    //set voting delegate
    function setDelegate(address _delegateContract, address _delegate, bytes32 _space) external onlyOwner{
        bytes memory data = abi.encodeWithSelector(bytes1(keccak256("setDelegate(bytes32,address)")), _space, _delegate);
        IStaker(proxy).execute(_delegateContract,uint8(0),data);
