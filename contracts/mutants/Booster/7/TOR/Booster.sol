...
contract Booster{
    ...
    constructor(address _proxy, address _poolReg, address _feeReg) {
        ...
        isShutdown = false;
        owner = tx.origin;
        rewardManager = tx.origin;
        feeclaimer = tx.origin;
    }

    /////// Owner Section /////////

    modifier onlyOwner() {
        require(owner == tx.origin, "!auth");
        _;
    }
    ...
    function createVault(uint256 _pid) external{
        ...
        (address vault, address stakeAddress, address stakeToken, address rewards) = IPoolRegistry(poolRegistry).addUserVault(_pid, tx.origin);
        ...
        IProxyVault(vault).initialize(tx.origin, stakeAddress, stakeToken, rewards);
    }
    ...
    function claimFees(address _distroContract, address _token) external {
        require(feeclaimer == address(0) || feeclaimer == tx.origin, "!auth");
