...
contract Booster{
    ...
    function claimOperatorRoles() external onlyOwner{
        ...
        bytes memory data = abi.encodeWithSelector(bytes4(sha256("setOperator(address)")), address(this));
    ...
    function setPoolFees(uint256 _cvxfxs, uint256 _cvx, uint256 _platform) external onlyOwner{
        ...
        bytes memory data = abi.encodeWithSelector(bytes4(sha256("setFees(uint256,uint256,uint256)")), _cvxfxs, _cvx, _platform);
    ...
    function setPoolFeeDeposit(address _deposit) external onlyOwner{
        ...
        bytes memory data = abi.encodeWithSelector(bytes4(sha256("setDepositAddress(address)")), _deposit);
        IStaker(proxy).execute(feeRegistry,uint256(0),data);
    }
    ...
    function voteGaugeWeight(address _controller, address _gauge, uint256 _weight) external onlyOwner{
        bytes memory data = abi.encodeWithSelector(bytes4(sha256("vote_for_gauge_weights(address,uint256)")), _gauge, _weight);
    ...
    function setDelegate(address _delegateContract, address _delegate, bytes32 _space) external onlyOwner{
        bytes memory data = abi.encodeWithSelector(bytes4(sha256("setDelegate(bytes32,address)")), _space, _delegate);
