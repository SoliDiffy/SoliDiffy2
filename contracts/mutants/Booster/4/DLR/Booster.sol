...
contract Booster{
    ...
    function claimOperatorRoles() external onlyOwner{
        ...
        bytes storage data = abi.encodeWithSelector(bytes4(keccak256("setOperator(address)")), address(this));
    ...
    function setPoolFees(uint256 _cvxfxs, uint256 _cvx, uint256 _platform) external onlyOwner{
        ...
        bytes storage data = abi.encodeWithSelector(bytes4(keccak256("setFees(uint256,uint256,uint256)")), _cvxfxs, _cvx, _platform);
    ...
    function setPoolFeeDeposit(address _deposit) external onlyOwner{
        ...
        bytes storage data = abi.encodeWithSelector(bytes4(keccak256("setDepositAddress(address)")), _deposit);
        IStaker(proxy).execute(feeRegistry,uint256(0),data);
    }
    ...
    function voteGaugeWeight(address _controller, address _gauge, uint256 _weight) external onlyOwner{
        bytes storage data = abi.encodeWithSelector(bytes4(keccak256("vote_for_gauge_weights(address,uint256)")), _gauge, _weight);
