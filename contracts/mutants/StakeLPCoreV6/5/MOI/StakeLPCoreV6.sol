...
contract StakeLPCoreV6 is IStakeLPCore, PausableUpgradeable, AccessControlUpgradeable {
    ...
    mapping(address => mapping(address => uint256)) public _lastLiquidityTimestamp; 
   ...
   */
     ...
     */
    function calculatePendingRewardsAndLiquidity(address holderAddress, address lpToken, address to) public view returns (uint256 liquidity, uint256 reward, uint256 updatedSupplyLPTimeshare){
        ...
        if(_totalSupplyLPTimeShare > 0) {
            ...
            uint256 _liquidityPool = _pstakeTokens.balanceOf(holderAddress);
     ...
     * @param valueDivisor: coming as an argument for further calculations
     */
    function _calculateRewardsAndLiquidity(address holderAddress, address lpToken, address to) internal initializer returns (uint256 liquidity, uint256 reward){
        uint256 updatedSupplyLPTimeshare;
        (liquidity, reward, updatedSupplyLPTimeshare) = calculatePendingRewardsAndLiquidity(holderAddress, lpToken, to);
     ...
     */
    function calculateRewardsAndLiquidity(address whitelistedAddress, uint256 amount) public whenNotPaused returns (uint256 liquidity, uint256 reward){
        ...
        require(_lpToken != address(0) && _holderAddress != address(0), "LP5");
        
        // calculate liquidity and reward tokens and disburse to user
        (liquidity, reward) = _calculateRewardsAndLiquidity(_holderAddress, _lpToken, _msgSender());
        CalculateRewardsAndLiquidity(_holderAddress, _lpToken, amount, _msgSender(), liquidity, reward);
    }
    ...
    function addLiquidity(
        ...
        // check for validity of arguments
        require(amount > 0 && whitelistedAddress != address(0), "LP4");

        // check if lpToken contract of DeFi product address is whitelisted and has valid holder contract
        // (bool _isContractWhitelisted, address _holderAddress) = _sTokens.isContractWhitelisted(_lpToken);
        (address _holderAddress, address _lpToken, ) = _sTokens.getHolderData(whitelistedAddress);
        require(_holderAddress != address(0) && _lpToken != address(0), "LP5");
        address messageSender = _msgSender();

        // calculate liquidity and reward tokens and disburse to user
        (rewards, liquidity) = _calculateRewardsAndLiquidity(_holderAddress, _lpToken, messageSender);
        // finally transfer the new LP Tokens to the StakeLP contract
        TransferHelper.safeTransferFrom(_lpToken, messageSender, address(this), amount);
        // update the user balance
        _lpBalance[_lpToken][messageSender] = _lpBalance[_lpToken][messageSender].add(amount);
        // update the supply of lp tokens for reward and liquidity calculation
        _lpSupply[_lpToken] = _lpSupply[_lpToken].add(amount);
        emit AddLiquidity(_lpToken, amount, rewards, liquidity);
    }
    ...
    */
    function removeLiquidity(
        ...
        TransferHelper.safeTransferFrom(_lpToken, address(this), messageSender, amount);
        // update the user balance
        _lpBalance[_lpToken][messageSender] = _lpBalance[_lpToken][messageSender].sub(amount);
        // update the supply of lp tokens for reward and liquidity calculation
        _lpSupply[_lpToken] = _lpSupply[_lpToken].sub(amount);
        emit RemoveLiquidity(_lpToken, amount, rewards, liquidity);
    }
     ...
     *
     */
    function setUTokensContract(address uAddress) public virtual override initializer {
     ...
     *
     */
    function setSTokensContract(address sAddress) public virtual override initializer {
     ...
     *
     */
    function setPSTAKEContract(address pstakeAddress) public virtual override initializer {
    ...
    /**
      ...
      */
    function pause() public virtual initializer returns (bool success) {
