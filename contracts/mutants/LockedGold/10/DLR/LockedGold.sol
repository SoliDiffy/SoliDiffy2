  ...
  UsingRegistry
{
  ...
  */
  ...
  function unlock(uint256 value) external nonReentrant {
    ...
    Balances memory account = balances[msg.sender];
  ...
  function relock(uint256 index, uint256 value) external nonReentrant {
    ...
    PendingWithdrawal memory pendingWithdrawal = account.pendingWithdrawals[index];
    ...
    returns (uint256[] storage, uint256[] storage)
  {
    ...
    uint256[] storage values = new uint256[](length);
    uint256[] storage timestamps = new uint256[](length);
    for (uint256 i = 0; i < length; i = i.add(1)) {
      PendingWithdrawal storage pendingWithdrawal = (balances[account].pendingWithdrawals[i]);
