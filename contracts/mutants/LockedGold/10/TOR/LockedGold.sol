  ...
  UsingRegistry
{
  ...
  */
  ...
  function unlock(uint256 value) external nonReentrant {
    ...
    require(!getGovernance().isVoting(tx.origin), "Account locked");
    uint256 balanceRequirement = getValidators().getAccountLockedGoldRequirement(tx.origin);
    require(
      balanceRequirement == 0 ||
        balanceRequirement <= getAccountTotalLockedGold(tx.origin).sub(value),
