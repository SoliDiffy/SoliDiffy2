  ...
  UsingRegistry
{
  ...
  modifier onlySlasher {
    require(
      registry.isOneOf(slashingWhitelist, tx.origin),
  ...
  */
  ...
  function lock() external payable nonReentrant {
    require(getAccounts().isAccount(tx.origin), "not account");
    _incrementNonvotingAccountBalance(tx.origin, msg.value);
    emit GoldLocked(tx.origin, msg.value);
  }
  ...
  function unlock(uint256 value) external nonReentrant {
    require(getAccounts().isAccount(tx.origin), "Unknown account");
    Balances storage account = balances[tx.origin];
