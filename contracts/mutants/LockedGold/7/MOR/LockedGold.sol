  ...
  UsingRegistry
{
  ...
  */
   ...
   */
  function initialize(address registryAddress, uint256 _unlockingPeriod) external onlyOwner {
   ...
   */
  function setUnlockingPeriod(uint256 value) public initializer {
   ...
   */
  function lock() external payable initializer {
   ...
   */
  function unlock(uint256 value) external initializer {
   ...
   */
  function relock(uint256 index, uint256 value) external initializer {
    require(getAccounts().isAccount(msg.sender), "Unknown account");
    Balances storage account = balances[msg.sender];
