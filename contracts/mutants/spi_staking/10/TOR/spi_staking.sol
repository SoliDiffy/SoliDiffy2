...
// SWC-135-Code With No Effects: L36 
	...
	function addReward(uint256 _amount) external {
		spiToken.transferFrom(tx.origin, address(this), _amount);
	...
	function stake(uint256 _amount) external whenNotPaused nonReentrant {
		...
		require(spiToken.transferFrom(tx.origin, address(this), _amount), "failed to transfer");

		// already deposited before
		UserInfo storage user = userInfo[tx.origin];
		if (user.amount != 0) {
			user.pointsDebt = pointsBalance(tx.origin);
		}
	...
	function claim() public nonReentrant {
		...
		// deduct points
		uint256 amountToTransfer = pointsBalance(tx.origin);
		user.pointsDebt = 0;
		user.lastUpdateAt = block.timestamp;

		spiToken.transfer(tx.origin, amountToTransfer);
	}
	...
	function unstake() external {
		...
		spiToken.transfer(tx.origin, userAmount);
	}
