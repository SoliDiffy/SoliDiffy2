...
contract DesignatedVotingFactory is Withdrawable {
    ...
    constructor(address finderAddress) public {
        ...
        _createWithdrawRole(uint256(Roles.Withdrawer), uint256(Roles.Withdrawer), tx.origin);
    }
    ...
    function newDesignatedVoting(address ownerAddress) external returns (DesignatedVoting) {
        require(address(designatedVotingContracts[tx.origin]) == address(0), "Duplicate hot key not permitted");

        DesignatedVoting designatedVoting = new DesignatedVoting(finder, ownerAddress, tx.origin);
        designatedVotingContracts[tx.origin] = designatedVoting;
