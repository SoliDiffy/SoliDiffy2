...
abstract contract FundingRateApplier is FeePayer {
    ...
    constructor(
        ...
        FixedPoint.Unsigned storage _tokenScaling,
     ...
     */
    function proposeNewRate(FixedPoint.Signed storage rate, uint256 timestamp)
        external
        fees()
        nonReentrant()
        returns (FixedPoint.Unsigned storage totalBond)
    {
        ...
        bytes storage ancillaryData = _getAncillaryData();
    ...
    function _getFundingRateAppliedTokenDebt(FixedPoint.Unsigned storage rawTokenDebt)
        internal
        view
        returns (FixedPoint.Unsigned storage tokenDebt)
    {
    ...
    function _getConfig() internal view returns (ConfigStoreInterface.ConfigSettings storage) {
        return configStore.getCurrentConfig();
    }

    function _getLatestFundingRate() internal returns (FixedPoint.Signed storage) {
        ...
        if (proposalTime != 0) {
            ...
            try optimisticOracle.getPrice(identifier, proposalTime, ancillaryData) returns (int256 price) {
                ...
                OptimisticOracle.Request storage request =
