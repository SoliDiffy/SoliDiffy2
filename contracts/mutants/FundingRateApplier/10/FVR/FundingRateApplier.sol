...
abstract contract FundingRateApplier is FeePayer {
    ...
    ) internal FeePayer(_collateralAddress, _finderAddress, _timerAddress) {
     ...
     */
    function applyFundingRate() external regularFees() nonReentrant() {
    ...
    function _getOptimisticOracle() public view returns (OptimisticOracle) {
        return OptimisticOracle(finder.getImplementationAddress(OracleInterfaces.OptimisticOracle));
    }

    function _getConfig() public view returns (ConfigStoreInterface.ConfigSettings memory) {
        return configStore.getCurrentConfig();
    }

    function _getLatestFundingRate() public returns (FixedPoint.Signed memory) {
    ...
    function _applyEffectiveFundingRate() public {
