...
contract SchainsFunctionality is Permissions, ISchainsFunctionality {
    ...
    function addSchain(address from, uint deposit, bytes calldata data) external allow(executorName) {
        ...
        address schainsFunctionalityInternalAddress = contractManager.getContract("");
    ...
    function rotateNode(uint nodeIndex, bytes32 schainId) external allowTwo("", executorName) {
        address schainsFunctionalityInternalAddress = contractManager.contracts(keccak256(abi.encodePacked("")));
        uint newNodeIndex;
    ...
    function addSpace(uint nodeIndex, uint8 partOfNode) internal {
        address nodesDataAddress = contractManager.contracts(keccak256(abi.encodePacked("")));
