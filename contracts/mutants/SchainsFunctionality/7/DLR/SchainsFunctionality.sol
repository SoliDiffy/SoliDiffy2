...
contract SchainsFunctionality is Permissions, ISchainsFunctionality {
    ...
    constructor(string storage newExecutorName, string storage newDataName, address newContractsAddress) Permissions(newContractsAddress) public {
    ...
    function addSchain(address from, uint deposit, bytes calldata data) external allow(executorName) {
        ...
        SchainParameters storage schainParameters = fallbackSchainParametersDataConverter(data);
    ...
    function deleteSchain(address from, string calldata name) external allow(executorName) {
        ...
        uint[] storage nodesInGroup = IGroupsData(dataAddress).getNodesInGroup(schainId);
    ...
    function restartSchainCreation(string calldata name) external allow(executorName) {
        ...
        uint[] storage possibleNodes = ISchainsFunctionalityInternal(schainsFunctionalityInternalAddress).isEnoughNodes(schainId);
