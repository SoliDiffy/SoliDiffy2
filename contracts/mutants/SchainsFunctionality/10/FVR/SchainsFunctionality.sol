...
contract SchainsFunctionality is Permissions, ISchainsFunctionality {
    ...
    constructor(string memory newExecutorName, string memory newDataName, address newContractsAddress) Permissions(newContractsAddress) internal {
     ...
     */
    function addSchain(address from, uint deposit, bytes calldata data) public allow(executorName) {
        uint numberOfNodes;
        uint8 partOfNode;
     ...
     */
    function deleteSchain(address from, string calldata name) public allow(executorName) {
    ...
    function deleteSchainByRoot(string calldata name) public allow(executorName) {
        bytes32 schainId = keccak256(abi.encodePacked(name));
        address dataAddress = contractManager.contracts(keccak256(abi.encodePacked(dataName)));
        address schainsFunctionalityInternalAddress = contractManager.getContract("SchainsFunctionalityInternal");
    ...
    function rotateNode(uint nodeIndex, bytes32 schainId) public allowTwo("SkaleDKG", executorName) {
    ...
    function restartSchainCreation(string calldata name) public allow(executorName) {
        bytes32 schainId = keccak256(abi.encodePacked(name));
        address dataAddress = contractManager.contracts(keccak256(abi.encodePacked(dataName)));
    ...
    public pure returns (SchainParameters memory schainParameters)
    {
     ...
     */
    function addSpace(uint nodeIndex, uint8 partOfNode) public {
