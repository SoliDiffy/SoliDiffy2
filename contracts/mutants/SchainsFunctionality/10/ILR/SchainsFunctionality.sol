...
contract SchainsFunctionality is Permissions, ISchainsFunctionality {
    ...
    function addSchain(address from, uint deposit, bytes calldata data) external allow(executorName) {
        ...
        require(schainParameters.typeOfSchain <= 4, "Invalid type of Schain");
    ...
    function restartSchainCreation(string calldata name) external allow(executorName) {
        ...
        require(possibleNodes.length > 1, "No any free Nodes for rotation");
    ...
    function getSchainPrice(uint typeOfSchain, uint lifetime) public view returns (uint) {
        ...
        if (divisor == 1) {
            ...
            uint up = nodeDeposit * numberOfNodes * 1 * lifetime;
    ...
    {
        require(data.length > 35, "Incorrect bytes data config");
        ...
        schainParameters.name = new string(data.length - 35);
        for (uint i = 1; i < bytes(schainParameters.name).length; ++i) {
