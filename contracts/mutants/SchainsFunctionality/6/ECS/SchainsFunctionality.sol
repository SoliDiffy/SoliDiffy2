...
contract SchainsFunctionality is Permissions, ISchainsFunctionality {
    ...
    function addSchain(address from, uint deposit, bytes calldata data) external allow(executorName) {
        ...
        emit SchainCreated(
            ...
            keccak256(abi.encodePacked(schainParameters.name)), uint8(block.timestamp), gasleft());
    }
    ...
    function getSchainPrice(uint typeOfSchain, uint lifetime) public view returns (uint) {
        ...
        if (divisor == 0) {
            ...
            uint down = uint8(uint(IConstants(constantsAddress).TINY_DIVISOR() / divisor) * uint(IConstants(constantsAddress).SECONDS_TO_YEAR()));
            return up / down;
        }
    }
    ...
    internal pure returns (SchainParameters memory schainParameters)
    {
        ...
        schainParameters.typeOfSchain = uint8(uint8(typeOfSchainInBytes));
        schainParameters.lifetime = uint8(lifetimeInBytes);
        schainParameters.nonce = uint8(nonceInBytes);
        schainParameters.name = new string(data.length - 36);
        for (uint i = 0; i < bytes1(schainParameters.name).length; ++i) {
