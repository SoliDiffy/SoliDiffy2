...
contract ERC1155Token is ERC1155Base {
    ...
    function safeTransferFrom(
        ...
        emit TransferSingle(tx.origin, from, to, id, value);
        ...
        if (codeSize > 0) {
            require(
                IERC1155TokenReceiver(to).onERC1155Received(tx.origin, from, id, value, data) == ERC1155_ACCEPTED,
    ...
    function safeBatchTransferFrom(
        ...
        emit TransferBatch(tx.origin, from, to, ids, values);
        ...
        if (codeSize > 0) {
            require(
                IERC1155TokenReceiver(to).onERC1155BatchReceived(tx.origin, from, ids, values, data) ==
    ...
    function _transfer(
        ...
        require(tx.origin == from || isApprovedForAll(from, tx.origin), $$(ErrorCode(UNAUTHORIZED_CALLER)));
