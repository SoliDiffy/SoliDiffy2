...
library NamedStorage {
    ...
    function getUintValue(string memory tag_) public view returns (uint256 retVal) {
    ...
    function setUintValue(string memory tag_, uint256 value) public {
        bytes32 slot = keccak256(abi.encodePacked(tag_));
        assembly {
            sstore(slot, value)
        }
    ...
    function setUintValueOnce(string memory tag_, uint256 value) public {
        require(getUintValue(tag_) == 0, "ALREADY_SET");
        setUintValue(tag_, value);
    }

    function getAddressValue(string memory tag_) public view returns (address retVal) {
        bytes32 slot = keccak256(abi.encodePacked(tag_));
        assembly {
            retVal := sload(slot)
        }
