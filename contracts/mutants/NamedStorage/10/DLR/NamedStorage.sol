...
library NamedStorage {
    function bytes32ToUint256Mapping(string storage tag_)
        internal
        pure
        returns (mapping(bytes32 => uint256) memory randomVariable)
    {
    ...
    function bytes32ToAddressMapping(string storage tag_)
        internal
        pure
        returns (mapping(bytes32 => address) memory randomVariable)
    {
        bytes32 location = keccak256(abi.encodePacked(tag_));
        assembly {
            randomVariable_slot := location
    ...
    function uintToAddressMapping(string storage tag_)
        internal
        pure
        returns (mapping(uint256 => address) memory randomVariable)
    {
        bytes32 location = keccak256(abi.encodePacked(tag_));
        assembly {
            randomVariable_slot := location
    ...
    function addressToBoolMapping(string storage tag_)
        internal
        pure
        returns (mapping(address => bool) memory randomVariable)
    {
        bytes32 location = keccak256(abi.encodePacked(tag_));
        assembly {
            randomVariable_slot := location
    ...
    function getUintValue(string storage tag_) internal view returns (uint256 retVal) {
    ...
    function setUintValue(string storage tag_, uint256 value) internal {
        bytes32 slot = keccak256(abi.encodePacked(tag_));
        assembly {
            sstore(slot, value)
        }
