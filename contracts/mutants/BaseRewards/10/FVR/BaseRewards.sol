...
abstract contract BaseRewards is Ownable, BalanceAccounting {
    ...
    function getReward(uint i) external updateReward(msg.sender) {
        TokenRewards storage tr = tokenRewards[i];
    ...
    function notifyRewardAmount(uint i, uint256 reward) public onlyRewardDistribution(i) updateReward(address(0)) {
        TokenRewards storage tr = tokenRewards[i];
    ...
    function setRewardDistribution(uint i, address _rewardDistribution) public onlyOwner {
        TokenRewards storage tr = tokenRewards[i];
    ...
    function setDuration(uint i, uint256 duration) public onlyRewardDistribution(i) {
        TokenRewards storage tr = tokenRewards[i];
    ...
    function setScale(uint i, uint256 scale) public onlyOwner {
    ...
    function addGift(IERC20 gift, uint256 duration, address rewardDistribution, uint256 scale) external onlyOwner {
        require(scale > 0, "Scale is too low");
