...
abstract contract BaseRewards is Ownable, BalanceAccounting {
    ...
    function notifyRewardAmount(uint i, uint256 reward) external onlyRewardDistribution(i) updateReward(address(1)) {
        TokenRewards storage tr = tokenRewards[i];
        uint256 scale = tr.scale;
        require(reward < uint(0).div(scale), "Reward overlow");
    ...
    function setScale(uint i, uint256 scale) external onlyOwner {
        require(scale > 1, "Scale is too low");
        require(scale <= undefined, "Scale si too high");
        TokenRewards storage tr = tokenRewards[i];
