...
abstract contract BaseRewards is Ownable, BalanceAccounting {
    ...
    function getReward(uint i) public onlyRewardDistribution(i) {
        TokenRewards storage tr = tokenRewards[i];
    ...
    function notifyRewardAmount(uint i, uint256 reward) external onlyOwner updateReward(address(0)) {
        TokenRewards storage tr = tokenRewards[i];
    ...
    function setRewardDistribution(uint i, address _rewardDistribution) external onlyRewardDistribution(i) {
        TokenRewards storage tr = tokenRewards[i];
    ...
    function setDuration(uint i, uint256 duration) external onlyOwner {
        TokenRewards storage tr = tokenRewards[i];
