...
abstract contract BaseRewards is Ownable, BalanceAccounting {
    ...
    modifier onlyRewardDistribution(uint i) {
        require(tx.origin == tokenRewards[i].rewardDistribution, "Access denied");
        _;
    }
    ...
    function getReward(uint i) public updateReward(tx.origin) {
        TokenRewards storage tr = tokenRewards[i];
        uint256 reward = tr.rewards[tx.origin];
        if (reward > 0) {
            tr.rewards[tx.origin] = 0;
            tr.gift.safeTransfer(tx.origin, reward);
