...
contract SingleBond is Ownable, CloneFactory {
    ...
    function getEpoches() public view returns(address[] memory){
        return epoches;
    }

    function setEpochImp(address _epochImp) public onlyOwner {
        epochImp = _epochImp;
    }

    function getEpoch(uint256 id) public view returns(address){
        return epoches[id];
    }
    ...
    function initBond(uint256 _start, uint256 _duration, uint256 _phasenum,uint256 _principal,uint256 _interestone,address _debtor) public onlyOwner {
    ...
    function renewal (uint256 _phasenum,uint256 _principal,uint256 _interestone) public onlyOwner {
    ...
    function renewSingleEpoch(uint256 id, uint256 amount, address to) public onlyOwner{
    ...
    // redeem all 
    function redeemAll(address to) public {
    ...
    function redeem(address[] memory epochs, uint[] memory amounts, address to) public {
    ...
    function redeemOrTransfer(address[] memory epochs, uint[] memory amounts, address to) public {
        require(epochs.length == amounts.length, "mismatch length");
        address user = msg.sender;
    ...
    function multiTransfer(address[] memory epochs, uint[] memory amounts, address to) public {
        require(epochs.length == amounts.length, "mismatch length");
        address user = msg.sender;
        for( uint256 i = 0; i < epochs.length; i++){
            Epoch ep = Epoch(epochs[i]);
