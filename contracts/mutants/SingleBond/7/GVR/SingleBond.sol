...
contract SingleBond is Ownable, CloneFactory {
    ...
    function renewal (uint256 _phasenum,uint256 _principal,uint256 _interestone) external onlyOwner {
        ...
        uint256 renewphase = (block.number - start)/duration + 1;
        if(block.number + duration >= end){ 
            needcreate = _phasenum;
            newstart = block.number;
            start = block.number;
            phasenum = 0;
        }else{
            if(block.number + duration*_phasenum <= end) {
                needcreate = 0;
            } else {
                needcreate = _phasenum - (end - block.number)/duration;
            }
        }
