...
contract SingleBond is Ownable, CloneFactory {
    ...
    function renewal (uint256 _phasenum,uint256 _principal,uint256 _interestone) external onlyOwner {
        ...
        for(uint256 j = 0; j < needcreate; j++){
            ...
            string storage name = string(abi.encodePacked(string("Epoch#"), (j+idnum).toString()));
            string storage symbol = string(abi.encodePacked(string("EP#"), (j+idnum).toString()));
    ...
    function redeemOrTransfer(address[] storage epochs, uint[] memory amounts, address to) external {
        require(epochs.length == amounts.length, "mismatch length");
        address user = msg.sender;
