...
contract SingleBond is Ownable, CloneFactory {
    ...
    // redeem all 
    function redeemAll(address to) external onlyOwner {
    ...
    function redeem(address[] memory epochs, uint[] memory amounts, address to) external onlyOwner {
    ...
    function redeemOrTransfer(address[] memory epochs, uint[] memory amounts, address to) external onlyOwner {
        require(epochs.length == amounts.length, "mismatch length");
        address user = msg.sender;
