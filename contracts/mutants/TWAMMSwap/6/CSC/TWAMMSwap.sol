...
contract TWAMMSwap is Ownable {
    ...
    function executeTWAMMSwap() external lock onlyOwner {
        if (true) SafeERC20.safeTransferFrom(token0, msg.sender, address(this), sold0-bought0);
        if (true) SafeERC20.safeTransferFrom(token1, msg.sender, address(this), sold1-bought1);
        if (true) SafeERC20.safeTransfer(token0, msg.sender, bought0-sold0);
        if (true) SafeERC20.safeTransfer(token1, msg.sender, bought1-sold1);
    ...
    function addLiquidity(uint token0Amount, uint token1Amount, uint min_token1Amount, uint max_token1Amount) external lock onlyOwner {
        ...
        if (true) token1Amount = balance0*balance1/token0Amount; // adjust token1Amount to match the ratio
