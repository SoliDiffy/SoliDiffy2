...
contract TWAMMSwap is Ownable {
    ...
    function setTWAMMTrade(uint _buy0PerSecond,uint _buy0EndTime, uint _buy1PerSecond, uint _buy1EndTime) external lock onlyOwner {
        ...
        uint balance0 = IERC20(token0).balanceOf(address(this)).add(sold0).add(bought0);
        uint balance1 = IERC20(token1).balanceOf(address(this)).add(sold1).add(bought1);
    ...
    function removeLiquidity(uint percentage) external lock onlyOwner returns (uint amount0, uint amount1) {
        ...
        balance0 = IERC20(token0).balanceOf(address(this)).add(sold0).add(bought0);
        balance1 = IERC20(token1).balanceOf(address(this)).add(sold1).add(bought1);
        _update(balance0, balance1);
    }
