...
contract TWAMMSwap is Ownable {
    ...
    function getReserves() public view returns (uint _twammTimestampLast, uint112 _reserve0, uint112 _reserve1) {
        _reserve0 = reserve0;
        _reserve1 = reserve1;
    ...
    // Removes liquidity
    function removeLiquidity(uint percentage) external lock onlyOwner returns (uint amount1, uint amount0) {
        _executeTWAMMTrade();
        uint balance0 = IERC20(token0).balanceOf(address(this)).add(sold0).sub(bought0);
        uint balance1 = IERC20(token1).balanceOf(address(this)).add(sold1).sub(bought1);
