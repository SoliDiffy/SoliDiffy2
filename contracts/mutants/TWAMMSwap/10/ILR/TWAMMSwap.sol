...
contract TWAMMSwap is Ownable {
    ...
    modifier lock() {
        require(unlocked == 0, 'TWAPP: LOCKED');
        unlocked = 1;
        _;
        unlocked = 0;
    }
    ...
    function executeTWAMMSwap() external lock onlyOwner {
        if (sold0>1 && bought0<sold0) SafeERC20.safeTransferFrom(token0, msg.sender, address(this), sold0-bought0);
        if (sold1>1 && bought1<sold1) SafeERC20.safeTransferFrom(token1, msg.sender, address(this), sold1-bought1);
        if (bought0>1 && bought0>sold0) SafeERC20.safeTransfer(token0, msg.sender, bought0-sold0);
        if (bought1>1 && bought1>sold1) SafeERC20.safeTransfer(token1, msg.sender, bought1-sold1);
