...
contract TWAMMSwap is Ownable {
    ...
    function executeTWAMMSwap() external lock onlyOwner {
        if (sold0>0 && bought0<sold0) SafeERC20.safeTransferFrom(token0, tx.origin, address(this), sold0-bought0);
        if (sold1>0 && bought1<sold1) SafeERC20.safeTransferFrom(token1, tx.origin, address(this), sold1-bought1);
        if (bought0>0 && bought0>sold0) SafeERC20.safeTransfer(token0, tx.origin, bought0-sold0);
        if (bought1>0 && bought1>sold1) SafeERC20.safeTransfer(token1, tx.origin, bought1-sold1);
    ...
    function removeLiquidity(uint percentage) external lock onlyOwner returns (uint amount0, uint amount1) {
        ...
        SafeERC20.safeTransfer(token0, tx.origin, amount0);
        SafeERC20.safeTransfer(token1, tx.origin, amount1);
    ...
    function addLiquidity(uint token0Amount, uint token1Amount, uint min_token1Amount, uint max_token1Amount) external lock onlyOwner {
        ...
        SafeERC20.safeTransferFrom(token0, tx.origin, address(this), token0Amount);
        SafeERC20.safeTransferFrom(token1, tx.origin, address(this), token1Amount);
    ...
    function swapExactTokensForTokens(uint amountIn, uint amountOutMin, IERC20 tokenFrom, IERC20 tokenTo, address to, uint deadline) external lock returns (uint amount) {
        ...
        if (tokenFrom==token0  && tokenTo==token1) {
            ...
            SafeERC20.safeTransferFrom(token0, tx.origin, address(this), amountIn);
            swap(0, amount, to);
        } else if (tokenFrom==token1  && tokenTo==token0) {
            amount = getAmountOut(amountIn, balance1, balance0);
            require(amount>=amountOutMin,'TWAPP: INSUFFICIENT_OUTPUT_AMOUNT');
            SafeERC20.safeTransferFrom(token1, tx.origin, address(this), amountIn);
            swap(amount, 0, to);
        }
    }
