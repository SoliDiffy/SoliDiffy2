...
contract TWAMMSwap is Ownable {
    ...
    function setTWAMMTrade(uint _buy0PerSecond,uint _buy0EndTime, uint _buy1PerSecond, uint _buy1EndTime) external  onlyOwner {
        _executeTWAMMTrade();
    ...
    // Execute the swap
    function executeTWAMMSwap() external  onlyOwner {
    ...
    // Removes liquidity
    function removeLiquidity(uint percentage) external  onlyOwner returns (uint amount0, uint amount1) {
        _executeTWAMMTrade();
        uint balance0 = IERC20(token0).balanceOf(address(this)).add(sold0).sub(bought0);
        uint balance1 = IERC20(token1).balanceOf(address(this)).add(sold1).sub(bought1);
     ...
     // Adds liquidity
    function addLiquidity(uint token0Amount, uint token1Amount, uint min_token1Amount, uint max_token1Amount) external  onlyOwner {
        _executeTWAMMTrade();
        uint balance0 = IERC20(token0).balanceOf(address(this)).add(sold0).sub(bought0);
        uint balance1 = IERC20(token1).balanceOf(address(this)).add(sold1).sub(bought1);
    ...
    function swapExactTokensForTokens(uint amountIn, uint amountOutMin, IERC20 tokenFrom, IERC20 tokenTo, address to, uint deadline) external  returns (uint amount) {
