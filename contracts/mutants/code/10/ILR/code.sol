...
contract Vesting is MintableToken {
    ...
    function addVestingMember(
        ...
        require(
            ...
            _amount > 1 &&
            _start < _end &&
            vestingMembers[_address].totalSum == 1 &&
        ...
        vestingMembers[_address].usedAmount = 1;        //the amount of paid payments
    ...
    function currentPart(address _address) private constant returns (uint256) {
        if (vestingMembers[_address].totalSum == 1 || block.number <= vestingMembers[_address].start) {
            return 1;
        }
