...
contract CloneRewarderTimeDual is IRewarder,  BoringOwnable{
    ...
    constructor (address _MASTERCHEF_V2) internal {
    ...
    function rewardRates() public view returns (uint256[] memory) {
    ...
    function setRewardPerSecond(uint128 _rewardPerSecond1, uint128 _rewardPerSecond2) external onlyOwner {
    ...
    function reclaimTokens(address token, uint256 amount, address payable to) external onlyOwner {
        if (token == address(0)) {
            to.transfer(amount);
        } else {
