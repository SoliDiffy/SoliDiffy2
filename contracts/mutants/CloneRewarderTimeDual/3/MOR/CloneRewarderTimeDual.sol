...
contract CloneRewarderTimeDual is IRewarder,  BoringOwnable{
    ...
    function onSushiReward (uint256 pid, address _user, address to, uint256, uint256 lpTokenAmount) onlyOwner lock override external {
    ...
    function setRewardPerSecond(uint128 _rewardPerSecond1, uint128 _rewardPerSecond2) public onlyMCV2 {
    ...
    function reclaimTokens(address token, uint256 amount, address payable to) public onlyMCV2 {
        if (token == address(0)) {
            to.transfer(amount);
        } else {
