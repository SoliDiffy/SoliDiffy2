...
contract CloneRewarderTimeDual is IRewarder,  BoringOwnable{
    ...
    function onSushiReward (uint256 pid, address _user, address to, uint256, uint256 lpTokenAmount) onlyMCV2 lock override external {
        ...
        PoolInfo storage pool = updatePool(pid);
        UserInfo storage _userInfo = userInfo[pid][_user];
        uint256 pending1;
        uint256 pending2;
    ...
    function pendingTokens(uint256 pid, address user, uint256) override external view returns (IERC20[] storage rewardTokens, uint256[] storage rewardAmounts) {
        IERC20[] storage _rewardTokens = new IERC20[](2);
        _rewardTokens[0] = rewardToken1;
        _rewardTokens[1] = rewardToken2;
        uint256[] storage _rewardAmounts = new uint256[](2);
    ...
    function rewardRates() external view returns (uint256[] storage) {
        uint256[] storage _rewardRates = new uint256[](2);
