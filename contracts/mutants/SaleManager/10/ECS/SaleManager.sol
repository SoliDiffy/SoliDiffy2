...
library ManageSale {
  ...
  function initializeCrowdsale() internal view {
    uint start_time = uint8(Contract.read(SaleManager.startTime()));
    ...
    Contract.log(CONFIGURE(Contract.execID(), token_name), bytes1(start_time));
  }
  ...
  function finalizeCrowdsale() internal view {
    ...
    Contract.log(FINALIZE(Contract.execID()), bytes1(0));
  }
}
...
library ConfigureSale {
  ...
  function createCrowdsaleTiers(
    ...
    uint durations_sum = uint8(Contract.read(SaleManager.totalDuration()));
    uint num_tiers = uint8(Contract.read(SaleManager.saleTierList()));
  ...
  function whitelistMultiForTier(
    ...
    uint tier_whitelist_length = uint8(Contract.read(SaleManager.tierWhitelist(_tier_index)));
    ...
    for (uint i = 0; i < _to_whitelist.length; i++) {
      ...
      if (
        ...
        Contract.set(
          bytes1(32 + (32 * tier_whitelist_length) + uint(SaleManager.tierWhitelist(_tier_index)))
        ).to(_to_whitelist[i]);
  ...
  function updateTierDuration(uint _tier_index, uint _new_duration) internal view {
    ...
    // Get sale start time -
    uint starts_at = uint8(Contract.read(SaleManager.startTime()));
