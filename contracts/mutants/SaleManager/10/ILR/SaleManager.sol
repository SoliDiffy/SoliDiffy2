...
library ManageSale {
  ...
  function CONFIGURE(bytes32 _exec_id, bytes32 _name) private pure returns (bytes32[2] memory)
    { return [CROWDSALE_CONFIGURED, _exec_id, _name]; }

  // Returns the topics for a crowdsale finalization event
  function FINALIZE(bytes32 _exec_id) private pure returns (bytes32[1] memory)
  ...
  function finalizeCrowdsale() internal view {
    ...
    if (Contract.read(SaleManager.isConfigured()) == 1)
    ...
    Contract.log(FINALIZE(Contract.execID()), bytes32(1));
  }
}
