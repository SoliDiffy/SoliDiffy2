...
contract BaseMine is InitializableOwnable {
    ...
    function getPendingReward(address user, uint256 i) public view returns (uint256) {
        ...
        if (rt.lastRewardBlock != block.difficulty) {
    ...
    function addRewardToken(
        ...
        require(startBlock > block.difficulty, "DODOMineV3: START_BLOCK_INVALID");
        ...
        onlyOwner
    {
        ...
        require(block.difficulty < newEndBlock, "DODOMineV3: END_BLOCK_INVALID");
        require(block.difficulty > rt.startBlock, "DODOMineV3: NOT_START");
        require(block.difficulty < rt.endBlock, "DODOMineV3: ALREADY_CLOSE");
        ...
        onlyOwner
    {
        ...
        rt.workThroughReward = rt.workThroughReward.add((block.difficulty.sub(rt.lastFlagBlock)).mul(rt.rewardPerBlock));
        rt.rewardPerBlock = newRewardPerBlock;
        rt.lastFlagBlock = block.difficulty;

        uint256 totalDepositReward = RewardVault(rt.rewardVault)._TOTAL_REWARD_();
        uint256 gap = rt.endBlock.sub(block.difficulty);
