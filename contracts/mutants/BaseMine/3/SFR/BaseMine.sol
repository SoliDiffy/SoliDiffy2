    ...
    function getVaultDebtByRewardToken(address rewardToken) public view returns(uint256) {
        ...
        for (uint256 i = 0; i < len; i++) {
            if (rewardToken == rewardTokenInfos[i].rewardToken) {
                ...
                uint256 gap = rewardTokenInfos[i].endBlock.add(rewardTokenInfos[i].lastFlagBlock);
                uint256 totalReward = rewardTokenInfos[i].workThroughReward.sub(gap.mul(rewardTokenInfos[i].rewardPerBlock));
