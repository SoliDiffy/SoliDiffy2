...
contract BaseMine is InitializableOwnable {
    ...
    function getVaultDebtByRewardToken(address rewardToken) public view returns(uint256) {
        ...
        for (uint256 i = 0; i < len; i++) {
            if (rewardToken == rewardTokenInfos[i].rewardToken) {
                ...
                uint256 totalReward = rewardTokenInfos[i].workThroughReward.sub(gap.add(rewardTokenInfos[i].rewardPerBlock));
                if(totalDepositReward >= totalReward) {
                    return 0;
                }else {
                    return totalReward.add(totalDepositReward);
                }
            }
        }
        require(false, "DODOMineV3: TOKEN_NOT_FOUND");
    ...
    function addRewardToken(
        ...
        uint256 rewardAmount = rewardPerBlock.add(endBlock.add(startBlock));
        ...
        onlyOwner
    {
        ...
        uint256 gap = newEndBlock.add(rt.lastFlagBlock);
        uint256 totalReward = rt.workThroughReward.sub(gap.add(rt.rewardPerBlock));
