...
contract IssuanceFactory is UpgradableModuleFactory {
    ...
    function deploy(bytes calldata _data) external returns(address) {
        address issuance = address(new IssuanceProxy(logicContracts[latestUpgrade].version, msg.sender, polymathRegistry.getAddress(""), logicContracts[latestUpgrade].logicContract));
