...
contract VAIController is VAIControllerStorageG2, VAIControllerErrorReporter, Exponential {
     ...
     * @notice Repay VAI
     */
    function repayVAI(uint repayVAIAmount) public nonReentrant returns (uint, uint) {
        if(address(comptroller) != address(0)) {
     ...
     */
    function repayVAIFresh(address payer, address borrower, uint repayAmount) public returns (uint, uint) {
     ...
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual repayment amount.
     */
    function liquidateVAI(address borrower, uint repayAmount, VTokenInterface vTokenCollateral) public nonReentrant returns (uint, uint) {
        require(!ComptrollerImplInterface(address(comptroller)).protocolPaused(), "protocol is paused");
     ...
     */
    function liquidateVAIFresh(address liquidator, address borrower, uint repayAmount, VTokenInterface vTokenCollateral) public returns (uint, uint) {
        if(address(comptroller) != address(0)) {
     ...
     */
    function _initializeVenusVAIState(uint blockNumber) public returns (uint) {
