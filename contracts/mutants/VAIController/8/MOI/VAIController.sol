...
contract VAIController is VAIControllerStorageG2, VAIControllerErrorReporter, Exponential {
     ...
     */
    function repayVAIFresh(address payer, address borrower, uint repayAmount) internal nonReentrant returns (uint, uint) {
     ...
     */
    function liquidateVAIFresh(address liquidator, address borrower, uint repayAmount, VTokenInterface vTokenCollateral) internal nonReentrant returns (uint, uint) {
        if(address(comptroller) != address(0)) {
     ...
     */
    function _initializeVenusVAIState(uint blockNumber) external nonReentrant returns (uint) {
     ...
     */
    function updateVenusVAIMintIndex() public nonReentrant returns (uint) {
     ...
     */
    function calcDistributeVAIMinterVenus(address vaiMinter) public nonReentrant returns(uint, uint, uint, uint) {
    ...
    /**
      ...
      */
    function _setComptroller(ComptrollerInterface comptroller_) external nonReentrant returns (uint) {
        // Check caller is admin
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_COMPTROLLER_OWNER_CHECK);
        }
    ...
    function _become(VAIUnitroller unitroller) external nonReentrant {
    ...
    function _setTreasuryData(address newTreasuryGuardian, address newTreasuryAddress, uint newTreasuryPercent) external nonReentrant returns (uint) {
        // Check caller is admin
