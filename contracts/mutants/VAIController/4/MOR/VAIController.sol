...
contract VAIController is VAIControllerStorageG2, VAIControllerErrorReporter, Exponential {
    ...
    function mintVAI(uint mintVAIAmount) external onlyAdmin returns (uint) {
     ...
     * @notice Repay VAI
     */
    function repayVAI(uint repayVAIAmount) external onlyAdmin returns (uint, uint) {
        if(address(comptroller) != address(0)) {
     ...
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual repayment amount.
     */
    function liquidateVAI(address borrower, uint repayAmount, VTokenInterface vTokenCollateral) external onlyAdmin returns (uint, uint) {
        require(!ComptrollerImplInterface(address(comptroller)).protocolPaused(), "protocol is paused");
    ...
    function initialize() nonReentrant public {
