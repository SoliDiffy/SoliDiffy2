...
contract TestReserve {
    ...
    function res() external view returns (Reserve.Data memory) {
    ...
    function _blockTimestamp() external view returns (uint32 blockTimestamp) {
        blockTimestamp = uint32(timestamp);
    }

    /// @notice Increments the timestamp used for testing
    function step(uint256 timestep) external {
        timestamp += uint32(timestep);
    }

    /// @notice Adds amounts to cumulative reserves
    function shouldUpdate(bytes32 resId) external returns (Reserve.Data memory) {
    ...
    function shouldSwap(
        ...
        uint256 deltaIn,
        uint256 deltaOut
    ) external returns (Reserve.Data memory) {
