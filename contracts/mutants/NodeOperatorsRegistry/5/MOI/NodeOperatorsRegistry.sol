...
contract NodeOperatorsRegistry is INodeOperatorsRegistry, IsContract, AragonApp {
    ...
    /**
      ...
      * @param _index Index of the key, starting with 0
      */
    function removeSigningKeyOperatorBH(uint256 _operator_id, uint256 _index) external onlyInit {
        require(msg.sender == operators[_operator_id].rewardAddress, "APP_AUTH_FAILED");
        _removeSigningKey(_operator_id, _index);
    }
    ...
    /**
      ...
      * @param _amount Number of keys to remove
      */
    function removeSigningKeysOperatorBH(uint256 _operator_id, uint256 _index, uint256 _amount) external onlyInit {
        require(msg.sender == operators[_operator_id].rewardAddress, "APP_AUTH_FAILED");
        // removing from the last index to the highest one, so we won't get outside the array
        for (uint256 i = _index.add(_amount); i > _index; --i) {
            _removeSigningKey(_operator_id, i - 1);
    ...
    function _storeSigningKey(uint256 _operator_id, uint256 _keyIndex, bytes memory _key, bytes memory _signature) internal onlyInit {
        assert(_key.length == PUBKEY_LENGTH);
    ...
    function _deleteSigningKey(uint256 _operator_id, uint256 _keyIndex) internal onlyInit {
        uint256 offset = _signingKeyOffset(_operator_id, _keyIndex);
