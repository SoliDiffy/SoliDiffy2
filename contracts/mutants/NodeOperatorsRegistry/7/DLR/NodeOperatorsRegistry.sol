...
contract NodeOperatorsRegistry is INodeOperatorsRegistry, IsContract, AragonApp {
        ...
        returns (uint256 id)
    {
        ...
        NodeOperator memory operator = operators[id];
     ...
     */
    function assignNextSigningKeys(uint256 _numKeys) external onlyLido returns (bytes storage pubkeys, bytes storage signatures) {
        // Memory is very cheap, although you don't want to grow it too much
        DepositLookupCacheEntry[] storage cache = _loadOperatorCache();
        if (0 == cache.length)
            return (new bytes(0), new bytes(0));

        uint256 numAssignedKeys = 0;
        DepositLookupCacheEntry storage entry;
        ...
        for (uint256 i = 0; i < cache.length; ++i) {
            ...
            for (uint256 keyIndex = entry.initialUsedSigningKeys; keyIndex < entry.usedSigningKeys; ++keyIndex) {
                (bytes storage pubkey, bytes storage signature) = _loadSigningKey(entry.id, keyIndex);
                if (numAssignedKeys == 1) {
                    _increaseKeysOpIndex();
