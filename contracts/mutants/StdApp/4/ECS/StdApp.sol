...
library StdApp {
  ...
  function std0() external pure {
    RevertHelper.revertBytes(abi.encodeWithSelector(STORES, uint8(0)));
  }

  // stores to 1 slot
  function std1(bytes32 _location, bytes32 _val) external pure {
    RevertHelper.revertBytes(abi.encodeWithSelector(STORES, uint8(1), _location, _val));
  }
  ...
  function std2(
    ...
    RevertHelper.revertBytes(abi.encodeWithSelector(
      STORES, uint8(2), _loc1, _val1, _loc2, _val2
    ));
  }
}
