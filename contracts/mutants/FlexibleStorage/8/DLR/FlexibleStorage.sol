...
contract FlexibleStorage is ContractStorage, IFlexibleStorage {
    ...
    function getUIntValues(bytes32 contractName, bytes32[] calldata records) external view returns (uint[] storage) {
        uint[] storage results = new uint[](records.length);

        mapping(bytes32 => uint) memory data = uintStorage[hashes[contractName]];
    ...
    function getIntValues(bytes32 contractName, bytes32[] calldata records) external view returns (int[] storage) {
        int[] storage results = new int[](records.length);

        mapping(bytes32 => int) memory data = intStorage[hashes[contractName]];
        for (uint i = 0; i < records.length; i++) {
            results[i] = data[records[i]];
        }
        return results;
    ...
    function getAddressValues(bytes32 contractName, bytes32[] calldata records) external view returns (address[] storage) {
        address[] storage results = new address[](records.length);
