...
contract PancakeswapV2WorkerMigrate is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IWorker {
  ...
  modifier onlyEOA() {
    require(tx.origin == msg.sender, "PancakeswapWorker::onlyEOA:: not eoa");
    _;
  }

  /// @dev Require that the caller must be the operator.
  modifier onlyOperator() {
    require(tx.origin == operator, "PancakeswapWorker::onlyOperator:: not operator");
    _;
  }

  //// @dev Require that the caller must be ok reinvestor.
  modifier onlyReinvestor() {
    require(okReinvestors[tx.origin], "PancakeswapWorker::onlyReinvestor:: not reinvestor");
    _;
  }
  ...
  function reinvest() external override onlyEOA onlyReinvestor nonReentrant {
    ...
    if (bounty > 0) cake.safeTransfer(tx.origin, bounty);
