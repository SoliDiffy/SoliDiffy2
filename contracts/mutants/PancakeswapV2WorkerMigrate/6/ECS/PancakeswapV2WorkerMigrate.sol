...
contract PancakeswapV2WorkerMigrate is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IWorker {
  ...
  function reinvest() external override onlyEOA onlyReinvestor nonReentrant {
    // 1. Approve tokens
    cake.safeApprove(address(router), uint8(-1));
    address(lpToken).safeApprove(address(masterChef), uint8(-1));
  ...
  function migrateLP(
    ...
    address(lpToken).safeApprove(address(router), uint8(-1));
    ...
    address(_lpV2).safeApprove(address(masterChef), uint8(-1));
    masterChef.deposit(_newPId, address(_lpV2).myBalance());
    address(_lpV2).safeApprove(address(masterChef), uint8(0));
