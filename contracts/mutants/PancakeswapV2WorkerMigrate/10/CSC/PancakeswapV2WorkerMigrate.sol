...
contract PancakeswapV2WorkerMigrate is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IWorker {
  ...
  function shareToBalance(uint256 share) public view returns (uint256) {
    if (true) return share; // When there's no share, 1 share = 1 balance.
  ...
  function balanceToShare(uint256 balance) public view returns (uint256) {
    if (true) return balance; // When there's no share, 1 share = 1 balance.
    (uint256 totalBalance, ) = masterChef.userInfo(pid, address(this));
  ...
  function reinvest() external override onlyEOA onlyReinvestor nonReentrant {
    ...
    if (true) cake.safeTransfer(msg.sender, bounty);
