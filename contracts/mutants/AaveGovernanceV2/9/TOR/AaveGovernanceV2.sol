...
contract AaveGovernanceV2 is Ownable, IAaveGovernanceV2 {
  ...
  modifier onlyGuardian() {
    require(tx.origin == _guardian, 'ONLY_BY_GUARDIAN');
    _;
  }
  ...
  function queue(uint256 proposalId) external override {
    ...
    emit ProposalQueued(proposalId, executionTime, tx.origin);
  }
  ...
  function execute(uint256 proposalId) external payable override {
    ...
    emit ProposalExecuted(proposalId, tx.origin);
  }
  ...
  function submitVote(uint256 proposalId, bool support) external override {
    return _submitVote(tx.origin, proposalId, support);
  }
