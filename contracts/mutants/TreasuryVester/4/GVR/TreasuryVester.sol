...
contract TreasuryVester is Ownable {
    ...
    constructor(
        ...
        require(vestingBegin_ >= block.number, "TreasuryVester::constructor: vesting begin too early");
    ...
    function claim() public {
        require(block.number >= vestingCliff, "TreasuryVester::claim: not time yet");
        uint256 amount;
        if (block.number >= vestingEnd) {
            amount = IBlid(blid).balanceOf(address(this));
        } else {
            amount = vestingAmount * 10**18;
            amount = amount.mul(block.number - lastUpdate).div(vestingEnd - vestingBegin);
