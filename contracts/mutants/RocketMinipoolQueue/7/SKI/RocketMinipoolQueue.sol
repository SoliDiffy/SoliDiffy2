...
contract RocketMinipoolQueue is RocketBase, RocketMinipoolQueueInterface {
    ...
    function getTotalLength() override external view returns (uint256) {
        return (
            super.getLength(queueKeyFull)
        ).add(
            super.getLength(queueKeyHalf)
        ).add(
            super.getLength(queueKeyEmpty)
        );
    }
    ...
    function getLength(MinipoolDeposit _depositType) override external view returns (uint256) {
        if (_depositType == MinipoolDeposit.Full) { return super.getLength(queueKeyFull); }
        if (_depositType == MinipoolDeposit.Half) { return super.getLength(queueKeyHalf); }
        if (_depositType == MinipoolDeposit.Empty) { return super.getLength(queueKeyEmpty); }
        return 0;
    }
    ...
    function getTotalCapacity() override external view returns (uint256) {
        ...
        return (
            super.getLength(queueKeyFull).mul(rocketDAOProtocolSettingsMinipool.getFullDepositUserAmount())
        ).add(
