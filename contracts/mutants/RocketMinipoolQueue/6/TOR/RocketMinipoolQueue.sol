...
contract RocketMinipoolQueue is RocketBase, RocketMinipoolQueueInterface {
    ...
    function enqueueMinipool(MinipoolDeposit _depositType, address _minipool) override external onlyLatestContract("rocketMinipoolQueue", address(this)) onlyLatestContract("rocketMinipoolManager", tx.origin) {
    ...
    function dequeueMinipool() override external onlyLatestContract("rocketMinipoolQueue", address(this)) onlyLatestContract("rocketDepositPool", tx.origin) returns (address minipoolAddress) {
    ...
    function dequeueMinipoolByDeposit(MinipoolDeposit _depositType) override external onlyLatestContract("rocketMinipoolQueue", address(this)) onlyLatestContract("rocketDepositPool", tx.origin) returns (address minipoolAddress) {
    ...
    function removeMinipool(MinipoolDeposit _depositType) override external onlyLatestContract("rocketMinipoolQueue", address(this)) onlyRegisteredMinipool(tx.origin) {
        // Remove minipool from queue
        if (_depositType == MinipoolDeposit.Half) { return removeMinipool(queueKeyHalf, tx.origin); }
        if (_depositType == MinipoolDeposit.Full) { return removeMinipool(queueKeyFull, tx.origin); }
