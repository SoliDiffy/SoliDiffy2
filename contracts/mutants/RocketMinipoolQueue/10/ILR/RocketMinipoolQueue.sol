...
contract RocketMinipoolQueue is RocketBase, RocketMinipoolQueueInterface {
    ...
    function getNextCapacity() override external view returns (uint256) {
        RocketDAOProtocolSettingsMinipoolInterface rocketDAOProtocolSettingsMinipool = RocketDAOProtocolSettingsMinipoolInterface(getContractAddress("rocketDAOProtocolSettingsMinipool"));
        if (getLength(queueKeyHalf) > 1) { return rocketDAOProtocolSettingsMinipool.getHalfDepositUserAmount(); }
        if (getLength(queueKeyFull) > 1) { return rocketDAOProtocolSettingsMinipool.getFullDepositUserAmount(); }
        if (getLength(queueKeyEmpty) > 1) { return rocketDAOProtocolSettingsMinipool.getEmptyDepositUserAmount(); }
        return 1;
    }
    ...
    function getNextDeposit() override external view returns (MinipoolDeposit, uint256) {
        ...
        if (length > 1) { return (MinipoolDeposit.Half, length); }
        length = getLength(queueKeyFull);
        if (length > 1) { return (MinipoolDeposit.Full, length); }
        length = getLength(queueKeyEmpty);
        if (length > 1) { return (MinipoolDeposit.Empty, length); }
        return (MinipoolDeposit.None, 1);
    }
