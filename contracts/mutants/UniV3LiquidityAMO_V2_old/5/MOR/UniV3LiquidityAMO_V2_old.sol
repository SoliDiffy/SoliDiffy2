...
contract UniV3LiquidityAMO_V2_old is Owned {
    ...
    function mint(address _collat_addr, uint256 _amountCollateral, uint256 _amountFrax, uint24 _fee_tier, int24 _tickLower, int24 _tickUpper) public onlyByOwnGovCust returns (uint256, uint128) {
    ...
    function withdrawAll(uint256 _token_id) public onlyByOwnGovCust returns (uint256, uint256) {
        return removeLiquidity(_token_id, uint128(PRICE_PRECISION));
    }

    // Remove liquidity
    // Example fract_to_rem_e6 = 500000 = 50% removal. 1000000 = 100% removal
    function removeLiquidity(uint256 _token_id, uint128 fract_to_rem_e6) public onlyByOwnGovCust returns (uint256, uint256) {
