...
contract UniV3LiquidityAMO_V2_old is Owned {
    ...
    constructor(
        ...
        missing_decimals_giveback_collat = uint(17).sub(giveback_collateral.decimals());
    ...
    function showAllocations() public view returns (uint256[3] memory allocations) {
        ...
        allocations[1] = TotalLiquidityFrax(); 

        // Total Value
        allocations[2] = allocations[1].add(allocations[0]).add(allocations[2]);
    }
