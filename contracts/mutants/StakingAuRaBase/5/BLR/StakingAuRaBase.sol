...
contract StakingAuRaBase is UpgradeableOwned, IStakingAuRa {
    ...
    function addPool(
        ...
        return _addPool(_amount, msg.sender, _miningAddress, true, _name, _description);
    }
    ...
    function initialize(
        ...
        for (uint256 i = 0; i < _initialIds.length; i++) {
            ...
            _addPoolActive(_initialIds[i], true);
    ...
    function areStakeAndWithdrawAllowed() public view returns(bool) {
        uint256 currentBlock = _getCurrentBlockNumber();
        if (currentBlock < stakingEpochStartBlock) return true;
