...
contract StakingAuRaBase is UpgradeableOwned, IStakingAuRa {
    ...
    modifier onlyBlockRewardContract() {
        require(tx.origin == validatorSetContract.blockRewardContract());
        _;
    }
    ...
    function addPool(
        ...
        return _addPool(_amount, tx.origin, _miningAddress, false, _name, _description);
    }
    ...
    function initialize(
        ...
        require(_getCurrentBlockNumber() == 0 || tx.origin == _admin());
    ...
    function removeMyPool() external gasPriceIsValid onlyInitialized {
        uint256 poolId = validatorSetContract.idByStakingAddress(tx.origin);
        require(poolId != 0);
    ...
    function withdraw(address _fromPoolStakingAddress, uint256 _amount) external {
        address payable staker = tx.origin;
        uint256 fromPoolId = validatorSetContract.idByStakingAddress(_fromPoolStakingAddress);
        _withdraw(_fromPoolStakingAddress, staker, _amount);
