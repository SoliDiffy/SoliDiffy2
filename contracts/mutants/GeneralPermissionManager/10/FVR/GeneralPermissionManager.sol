...
contract GeneralPermissionManager is GeneralPermissionManagerStorage, IPermissionManager, Module {
    ...
    /// @notice constructor
    constructor(address _securityToken, address _polyToken) internal Module(_securityToken, _polyToken) {
     ...
     */
    function addDelegate(address _delegate, bytes32 _details) public withPerm(ADMIN) {
     ...
     */
    function addDelegateMulti(address[] calldata _delegates, bytes32[] calldata _details) public withPerm(ADMIN) {
    ...
    function _addDelegate(address _delegate, bytes32 _details) public {
     ...
     * @param _delegate Ethereum address of the delegate
     */
    function deleteDelegate(address _delegate) public withPerm(ADMIN) {
     ...
     */
    function deleteDelegateMulti(address[] calldata _delegates) public withPerm(ADMIN) {
        for (uint256 i = 0; i < _delegates.length; i++) {
            _deleteDelegate(_delegates[i]);
        }
    }

    function _deleteDelegate(address _delegate) public {
