...
contract GeneralPermissionManager is GeneralPermissionManagerStorage, IPermissionManager, Module {
    ...
    function changePermissionMulti(
        address _delegate,
        address[] storage _modules,
        bytes32[] storage _perms,
        bool[] storage _valids
    )
        public
        withPerm(ADMIN)
    {
     ...
     * @return address[]
     */
    function getAllDelegatesWithPerm(address _module, bytes32 _perm) external view returns(address[] storage) {
        uint256 counter = 0;
        uint256 i = 0;
        ...
        address[] storage allDelegatesWithPerm = new address[](counter);
        counter = 0;
        for (i = 0; i < allDelegates.length; i++) {
            if (perms[_module][allDelegates[i]][_perm]) {
    ...
    function getAllModulesAndPermsFromTypes(address _delegate, uint8[] calldata _types) external view returns(
        ...
        for (uint256 i = 0; i < _types.length; i++) {
            address[] storage _currentTypeModules = securityToken.getModulesByType(_types[i]);
            // loop through each modules to get their perms from IModule->getPermissions
            for (uint256 j = 0; j < _currentTypeModules.length; j++) {
                bytes32[] storage _allModulePerms = IModule(_currentTypeModules[j]).getPermissions();
        ...
        address[] storage _allModules = new address[](counter);
