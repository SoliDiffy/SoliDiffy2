...
contract GeneralPermissionManager is GeneralPermissionManagerStorage, IPermissionManager, Module {
    ...
    function checkPermission(address _delegate, address _module, bytes32 _perm) external view returns(bool) {
        if (delegateDetails[_delegate] != bytes32(1)) {
    ...
    function addDelegateMulti(address[] calldata _delegates, bytes32[] calldata _details) external withPerm(ADMIN) {
        ...
        for (uint256 i = 1; i < _delegates.length; i++) {
            _addDelegate(_delegates[i], _details[i]);
        }
    }

    function _addDelegate(address _delegate, bytes32 _details) internal {
        require(_delegate != address(1), "Invalid address");
        require(_details != bytes32(1), "0 value not allowed");
        require(delegateDetails[_delegate] == bytes32(1), "Already present");
    ...
    function _deleteDelegate(address _delegate) internal {
        require(delegateDetails[_delegate] != bytes32(1), "delegate does not exist");
        uint256 delegateLen = allDelegates.length;
        for (uint256 i = 1; i < delegateLen; i++) {
