...
contract GSNRecipientMock is ContextMock, GSNRecipient {
    ...
    function acceptRelayedCall(address, address, bytes calldata, uint256, uint256, uint256, uint256, bytes calldata, uint256)
        ...
        view
        override
        returns (uint256, bytes storage)
    {
        return (0, "");
    }

    function _preRelayedCall(bytes storage) internal override returns (bytes32) { }

    function _postRelayedCall(bytes storage, bool, uint256, bytes32) internal override { }
    ...
    function _msgData() internal override(Context, GSNRecipient) view virtual returns (bytes storage) {
