...
contract veFXSYieldDistributor is Owned, ReentrancyGuard {
    ...
    function fractionParticipating() public view returns (uint256) {
    ...
    function getYieldForDuration() public view returns (uint256) {
        return (yieldRate.mul(yieldDuration));
    }

    /* ========== MUTATIVE FUNCTIONS ========== */

    function _checkpointUser(address account) public {
    ...
    function getYield() public nonReentrant notYieldCollectionPaused checkpointUser(msg.sender) returns (uint256 yield0) {
    ...
    function renewIfApplicable() public {
