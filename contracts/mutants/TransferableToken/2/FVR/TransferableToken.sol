...
library TransferableToken{
  ...
  function transfer(address target_token, address payable to, uint256 amount) external {
  ...
  function balanceOfAddr(address target_token, address _of) external view returns(uint256){
    if(target_token == address(0x0)){
