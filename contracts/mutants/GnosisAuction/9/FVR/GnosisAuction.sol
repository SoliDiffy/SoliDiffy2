...
library GnosisAuction {
    ...
    ) public view returns (uint256 optionPremium) {
    ...
    function dsadd(uint256 x, uint256 y) public pure returns (uint256 z) {
        require((z = x + y) >= x, "ds-math-add-overflow");
    }

    function dsmul(uint256 x, uint256 y) public pure returns (uint256 z) {
        require(y == 0 || (z = x * y) / y == x, "ds-math-mul-overflow");
    }

    //rounds to zero if x*y < WAD / 2
    function dswmul(uint256 x, uint256 y) public pure returns (uint256 z) {
