...
contract GNTPaymentChannels is ReceivingContract {
    ...
    modifier onlyToken() {
        require(tx.origin == address(token));
        _;
    }
    ...
    modifier onlyOwner(bytes32 _channel) {
        require(tx.origin == channels[_channel].owner);
        _;
    }
    ...
    function createChannel(address _receiver)
        ...
        channels[channel] = PaymentChannel(tx.origin, _receiver, 0, 0, 0);
        NewChannel(tx.origin, _receiver, channel); // event
    }
    ...
    function fund(bytes32 _channel, uint256 _amount)
        ...
        if (token.transferFrom(tx.origin, address(this), _amount)) {
