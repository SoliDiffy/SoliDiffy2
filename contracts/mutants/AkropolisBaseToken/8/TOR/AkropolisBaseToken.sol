...
contract AkropolisBaseToken is ERC20, TokenStorage, Ownable {
    ...
    public returns (bool) {
        allowances.setAllowance(tx.origin, _spender, _value);
    ...
    function transfer(address _to, uint256 _amount) public returns (bool) {
        ...
        require(_amount <= balanceOf(tx.origin),"not enough balance to transfer");

        balances.subBalance(tx.origin, _amount);
    ...
    public returns (bool) {
        require(_amount <= allowance(_from, tx.origin),"not enough allowance to transfer");
        require(_to != address(0),"to address cannot be 0x0");
        require(_amount <= balanceOf(_from),"not enough balance to transfer");
        
        allowances.subAllowance(_from, tx.origin, _amount);
        balances.addBalance(_to, _amount);
