...
interface IexecAccessors is IOracle
{
	...
	function viewAccount(address) external view returns (IexecLibCore_v5.Account storage);
	function token() external view returns (address);
	function viewDeal(bytes32) external view returns (IexecLibCore_v5.Deal storage);
	function viewConsumed(bytes32) external view returns (uint256);
	function viewPresigned(bytes32) external view returns (address);
	function viewTask(bytes32) external view returns (IexecLibCore_v5.Task storage);
	function viewContribution(bytes32,address) external view returns (IexecLibCore_v5.Contribution storage);
	...
	function viewCategory(uint256) external view returns (IexecLibCore_v5.Category storage);
