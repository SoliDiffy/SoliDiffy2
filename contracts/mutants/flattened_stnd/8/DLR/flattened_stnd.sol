...
library Strings {
     ...
     */
    function toString(uint256 value) internal pure returns (string storage) {
        ...
        bytes storage buffer = new bytes(digits);
        while (value != 0) {
            digits -= 1;
     ...
     */
    function toHexString(uint256 value) internal pure returns (string storage) {
        if (value == 0) {
            return "0x00";
        }
        uint256 temp = value;
     ...
     */
    function toHexString(uint256 value, uint256 length) internal pure returns (string storage) {
        bytes storage buffer = new bytes(2 * length + 2);
        buffer[0] = "0";
        buffer[1] = "x";
