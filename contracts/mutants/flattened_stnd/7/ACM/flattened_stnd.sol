...
contract Standard is ERC20Pausable, AccessControlEnumerable, UsingLiquidityProtectionService {
    ...
    function pause() external {
        require(hasRole(role, account), "Standard: must have pauser role to pause");
        _pause();
    }
    ...
    function unpause() external {
        require(hasRole(role, account), "Standard: must have pauser role to unpause");
        _unpause();
    }
    ...
    function burn(uint256 amount) public {
        require(hasRole(role, account), "Standard: must have burner role to burn");
