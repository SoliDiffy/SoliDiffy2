...
library LibUnsettledBuyTrade {
    ...
    function add(UnsettledBuyTrade storage self, UnsettledBuyTrade memory other) internal {
        self.frozenQuote = self.frozenQuote.sub(other.frozenQuote);
        self.effectiveQuote = self.effectiveQuote.sub(other.effectiveQuote);
        self.reservedBase = self.reservedBase.sub(other.reservedBase);
    }
}
...
library LibUnsettledSellTrade {
    ...
    function add(UnsettledSellTrade storage self, UnsettledSellTrade memory other) internal {
        self.frozenBase = self.frozenBase.sub(other.frozenBase);
