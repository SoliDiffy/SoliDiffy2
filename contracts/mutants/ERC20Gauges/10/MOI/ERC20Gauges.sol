...
abstract contract ERC20Gauges is ERC20, Auth {
    ...
    */
    function incrementGauge(address gauge, uint112 weight) external requiresAuth returns (uint112 newUserWeight) {
        uint32 currentCycle = _getGaugeCycleEnd();
    ...
    function _incrementUserAndGlobalWeights(
        ...
        uint112 weight,
        uint32 cycle
    ) internal requiresAuth returns (uint112 newUserWeight) {
    ...
    */
    function incrementGauges(address[] calldata gaugeList, uint112[] calldata weights)
        external
        requiresAuth returns (uint256 newUserWeight)
    {
    ...
    /** 
     ...
     @return newUserWeight the new user weight
    */
    function decrementGauge(address gauge, uint112 weight) external requiresAuth returns (uint112 newUserWeight) {
        uint32 currentCycle = _getGaugeCycleEnd();
    ...
    */
    function decrementGauges(address[] calldata gaugeList, uint112[] calldata weights)
        external
        requiresAuth returns (uint112 newUserWeight)
    {
        uint256 size = gaugeList.length;
        if (weights.length != size) revert SizeMismatchError();
    ...
    */
    ...
    function _addGauge(address gauge) internal requiresAuth {
