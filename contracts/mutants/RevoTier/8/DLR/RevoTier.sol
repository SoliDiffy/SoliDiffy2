...
interface IRevoNFTToken{
    function getTokensDbIdByOwnerAndCollection(address _owner, string storage _collection) external view returns(string[] storage ownerTokensDbId);
}
...
contract RevoTier is Ownable{
    ...
    function getRealTimeTier(address _wallet) public view returns(Tier storage) {
        uint256 tierIndex = getTierIndex(_wallet);
        
        return tierIndex < 9999 ? tiers[tierIndex] : Tier(99, 0, 0, "");
    }
    
    function getRealTimeTierWithDiamondHands(address _wallet) public view returns(Tier storage){
        
        uint256 tierIndex = getTierIndex(_wallet);
        
        if(nftBoostEnabled && (tierIndex <= maxTierIndexBoost || tierIndex == 9999)){
            string[] storage tokens = revoNFTToken.getTokensDbIdByOwnerAndCollection(_wallet, "COSMETIC");
            bool found = false;
    ...
    function getBatchTiers(address[] storage _wallets) public view returns(uint256[] storage) {
