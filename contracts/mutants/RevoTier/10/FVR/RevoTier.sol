...
contract RevoTier is Ownable{
    ...
    function getBatchTiers(address[] memory _wallets) external view returns(uint256[] memory) {
    ...
    function setTier(uint256 _tierIndex, uint256 _minRevo, uint256 _stakingAPRBonus, string memory _name) external onlyOwner{
    ...
    */
    function setRevoToken(address _revo) external onlyOwner {
        revoAddress = _revo;
