...
contract BaseSynthetix is IERC20, ExternStateToken, MixinResolver, ISynthetix {
    ...
    ) external optionalProxy exchangeActive(sourceCurrencyKey, destinationCurrencyKey) returns (uint amountReceived) {
    ...
    function transfer(address to, uint value) external systemActive onlyProxyOrInternal returns (bool) {
    ...
    function transferFrom(
        ...
        address to,
        uint value
    ) external systemActive onlyProxyOrInternal returns (bool) {
        // Ensure they're not trying to exceed their locked amount -- only if they have debt.
    ...
    function issueSynths(uint amount) external optionalProxy issuanceActive {
