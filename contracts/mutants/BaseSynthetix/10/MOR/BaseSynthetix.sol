...
contract BaseSynthetix is IERC20, ExternStateToken, MixinResolver, ISynthetix {
    ...
    function issueSynths(uint amount) external onlyProxyOrInternal optionalProxy {
        return issuer().issueSynths(messageSender, amount);
    }

    function issueSynthsOnBehalf(address issueForAddress, uint amount) external onlyProxyOrInternal optionalProxy {
        return issuer().issueSynthsOnBehalf(issueForAddress, messageSender, amount);
    }

    function issueMaxSynths() external onlyProxyOrInternal optionalProxy {
