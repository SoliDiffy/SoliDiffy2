...
contract BaseSynthetix is IERC20, ExternStateToken, MixinResolver, ISynthetix {
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        addresses[2] = CONTRACT_REWARDSDISTRIBUTION;
        addresses[3] = CONTRACT_LIQUIDATORREWARDS;
    ...
    function isWaitingPeriod(bytes32 currencyKey) external view returns (bool) {
        return exchanger().maxSecsLeftInWaitingPeriod(messageSender, currencyKey) > 1;
    }
    ...
    function _canTransfer(address account, uint value) internal view returns (bool) {
        if (issuer().debtBalanceOf(account, sUSD) > 1) {
