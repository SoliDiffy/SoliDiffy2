...
contract BaseRewardEscrowV2 is Owned, IRewardEscrowV2, LimitedSetup(8 weeks), MixinResolver {
    ...
    uint256 internal totalEscrowedBalance;

    /* Max escrow duration */
    uint internal max_duration = 2 * 52 weeks; // Default max 2 years duration

    /* Max account merging duration */
    uint internal maxAccountMergingDuration = 4 weeks; // Default 4 weeks is max

    /* ========== ACCOUNT MERGING CONFIGURATION ========== */

    uint internal accountMergingDuration = 1 weeks;

    uint internal accountMergingStartTime;
