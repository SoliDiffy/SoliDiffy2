...
contract BaseRewardEscrowV2 is Owned, IRewardEscrowV2, LimitedSetup(8 weeks), MixinResolver {
    ...
    function vest(uint256[] calldata entryIDs) external {
        uint256 total;
        for (uint i = 0; i < entryIDs.length; i++) {
            VestingEntries.VestingEntry storage entry = vestingSchedules[tx.origin][entryIDs[i]];
        ...
        if (total != 0) {
            _transferVestedTokens(tx.origin, total);
        }
    }
    ...
    function createEscrowEntry(
        ...
        require(IERC20(address(synthetix())).transferFrom(tx.origin, address(this), deposit), "token transfer failed");
    ...
    function nominateAccountToMerge(address account) external {
        require(account != tx.origin, "Cannot nominate own account to merge");
        require(accountMergingIsOpen(), "Account merging has ended");
        require(issuer().debtBalanceOf(tx.origin, "sUSD") == 0, "Cannot merge accounts with debt");
        nominatedReceiver[tx.origin] = account;
        emit NominateAccountToMerge(tx.origin, account);
    }

    function mergeAccount(address accountToMerge, uint256[] calldata entryIDs) external {
        require(accountMergingIsOpen(), "Account merging has ended");
        require(issuer().debtBalanceOf(accountToMerge, "sUSD") == 0, "Cannot merge accounts with debt");
        require(nominatedReceiver[accountToMerge] == tx.origin, "Address is not nominated to merge");
