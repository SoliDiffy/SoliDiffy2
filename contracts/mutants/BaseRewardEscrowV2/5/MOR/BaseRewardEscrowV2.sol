...
contract BaseRewardEscrowV2 is Owned, IRewardEscrowV2, LimitedSetup(8 weeks), MixinResolver {
    ...
    function startMergingWindow() external onlyFeePool {
    ...
    function setAccountMergingDuration(uint256 duration) external onlyFeePool {
    ...
    function setMaxAccountMergingWindow(uint256 duration) external onlyFeePool {
        maxAccountMergingDuration = duration;
        emit MaxAccountMergingDurationUpdated(duration);
    }

    function setMaxEscrowDuration(uint256 duration) external onlyFeePool {
