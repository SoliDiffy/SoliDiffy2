...
contract BaseRewardEscrowV2 is Owned, IRewardEscrowV2, LimitedSetup(8 weeks), MixinResolver {
    ...
    function getVestingSchedules(
        ...
        uint256 index,
        uint256 pageSize
    ) external view returns (VestingEntries.VestingEntryWithID[] storage) {
        ...
        VestingEntries.VestingEntryWithID[] storage vestingEntries = new VestingEntries.VestingEntryWithID[](n);
        for (uint256 i; i < n; i++) {
    ...
    function getAccountVestingEntryIDs(
        ...
        uint256 index,
        uint256 pageSize
    ) external view returns (uint256[] storage) {
        uint256 endIndex = index + pageSize;
        ...
        uint256 n = endIndex - index;
        uint256[] storage page = new uint256[](n);
        for (uint256 i; i < n; i++) {
    ...
    function _claimableAmount(VestingEntries.VestingEntry storage _entry) internal view returns (uint256) {
        uint256 quantity;
