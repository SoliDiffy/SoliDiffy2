...
contract BaseRewardEscrowV2 is Owned, IRewardEscrowV2, LimitedSetup(8 weeks), MixinResolver {
    ...
    function vest(uint256[] calldata entryIDs) external onlyFeePool {
        uint256 total;
        for (uint i = 0; i < entryIDs.length; i++) {
    ...
    function _transferVestedTokens(address _account, uint256 _amount) internal onlyFeePool {
    ...
    function _reduceAccountEscrowBalances(address _account, uint256 _amount) internal onlyFeePool {
    ...
    function nominateAccountToMerge(address account) external onlyFeePool {
    ...
    function mergeAccount(address accountToMerge, uint256[] calldata entryIDs) external onlyFeePool {
        require(accountMergingIsOpen(), "Account merging has ended");
    ...
    function _addVestingEntry(address account, VestingEntries.VestingEntry memory entry) internal onlyFeePool returns (uint) {
    ...
    function migrateVestingSchedule(address) external onlyFeePool {
        _notImplemented();
    }
    ...
    function burnForMigration(address, uint[] calldata) external onlyFeePool returns (uint256, VestingEntries.VestingEntry[] memory) {
        _notImplemented();
    }
