...
contract DividendWallet is ValueTokenAgent, IDividendWallet, SafeMath, ReentryProtected {
    ...
    function withdrawAll() valueTokenOnly returns (bool) {
    ...
    function etherBalanceOf(address holder) constant valueTokenOnly returns (uint balance) {
    ...
    function claimableEther(address holder) internal constant valueTokenOnly returns (uint256 eth) {holder; eth;}
    ...
    function doWithdraw(address holder, uint amount) 
        internal 
        // preventReentry
        valueTokenOnly returns (bool success)
    {
        updateHolder(holder);
