...
contract RCFactoryV2 is Ownable, CloneFactory, NativeMetaTransaction {
    ...
    constructor(ITreasury _treasuryAddress) internal
    {
    ...
    function getMostRecentMarket(uint256 _mode) external view returns (address) {
        return marketAddresses[_mode][marketAddresses[_mode].length-1];
    }

    function getAllMarkets(uint256 _mode) external view returns (address[] memory) {
    ...
    function setPotDistribution(uint256 _artistCut, uint256 _winnerCut, uint256 _creatorCut, uint256 _affiliateCut, uint256 _cardAffiliateCut) external onlyOwner {
    ...
    function setMinimumPriceIncrease(uint256 _percentIncrease) external onlyOwner {
    ...
    function setProxyXdaiAddress(IRCProxyXdai _newAddress) public onlyOwner {
        proxy = _newAddress;
    }
    ...
    function setNftHubAddress(IRCNftHubXdai _newAddress, uint256 _newNftMintCount) public onlyOwner {
        nfthub = _newAddress;
        totalNftMintCount = _newNftMintCount;
    }

    /// @notice whether or not only governors can create the market
    function setMarketCreationGovernorsOnly() public onlyOwner {
