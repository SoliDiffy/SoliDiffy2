...
contract RCFactoryV2 is Ownable, CloneFactory, NativeMetaTransaction {
    ...
    function getAllMarkets(uint256 _mode) public view returns (address[] storage) {
        return marketAddresses[_mode];
    }

    function getPotDistribution() public view returns (uint256[5] storage) {
    ...
    function createMarket(
        uint32 _mode,
        string storage _ipfsHash,
        uint32[] storage _timestamps, 
        string[] storage _tokenURIs,
