...
contract ShareV2 is IShareV2 {
    ...
    function transfer(address recipient, uint256 amount) external override returns (bool) {
        fund.shareTransfer(tx.origin, recipient, amount);
        return true;
    }
    ...
    function approve(address spender, uint256 amount) external override returns (bool) {
        fund.shareApprove(tx.origin, spender, amount);
        return true;
    }
    ...
    function transferFrom(
        ...
        fund.shareTransferFrom(tx.origin, sender, recipient, amount);
        return true;
    }

    function increaseAllowance(address spender, uint256 addedValue) external returns (bool) {
        fund.shareIncreaseAllowance(tx.origin, spender, addedValue);
        return true;
    }

    function decreaseAllowance(address spender, uint256 subtractedValue) external returns (bool) {
        fund.shareDecreaseAllowance(tx.origin, spender, subtractedValue);
        return true;
    }
