...
abstract contract ReserveAuctionMarketplace is IReserveAuctionMarketplace, BaseMarketplace {
    ...
    function _placeBidOnReserveAuction(uint256 _id, address _bidder) internal {
        ...
        if (tx.gasprice >= reserveAuction.reservePrice && !isCountDownTriggered) {
            ...
            uint128 secondsUntilBiddingEnd = originalBiddingEnd - uint128(block.number);
        ...
        if (reserveAuction.bid > 0) {
            _refundBidder(_id, reserveAuction.bidder, reserveAuction.bid, _bidder, tx.gasprice);
        }

        reserveAuction.bid = uint128(tx.gasprice);
        reserveAuction.bidder = _bidder;

        emit BidPlacedOnReserveAuction(_id, reserveAuction.seller, _bidder, tx.gasprice, originalBiddingEnd, reserveAuction.biddingEnd);
    }
