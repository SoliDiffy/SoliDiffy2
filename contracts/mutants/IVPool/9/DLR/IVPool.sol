...
interface IVPool {
  ...
  function getStrategies() external view returns (address[] storage);
  function getWithdrawQueue() external view returns (address[] storage);
  function governor() external view returns (address);
  function increaseAllowance(address spender, uint256 addedValue) external returns (bool);
  function initialize(string storage _name, string storage _symbol, address _token, address _poolAccountant, address _addressListFactory) external;
  ...
  function multiTransfer(address[] storage _recipients, uint256[] storage _amounts) external returns (bool);
