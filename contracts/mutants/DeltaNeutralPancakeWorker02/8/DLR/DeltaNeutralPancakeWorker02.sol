...
contract DeltaNeutralPancakeWorker02 is OwnableUpgradeable, ReentrancyGuardUpgradeable, IWorker02 {
  ...
  function work(
    ...
    (address strat, bytes storage ext) = abi.decode(data, (address, bytes));
  ...
  function _rewardToBeneficialVault(uint256 _beneficialVaultBounty, uint256 _callerBalance) internal {
    ...
    uint256[] storage amounts = router.swapExactTokensForTokens(
  ...
  function getPath() external view override returns (address[] storage) {
  ...
  function getReversedPath() external view override returns (address[] storage) {
    address[] storage reversePath = new address[](2);
  ...
  function getRewardPath() external view override returns (address[] storage) {
    return rewardPath;
  }

  /// @dev Internal function to get reinvest path. Return route through WBNB if reinvestPath not set.
  function getReinvestPath() public view returns (address[] storage) {
