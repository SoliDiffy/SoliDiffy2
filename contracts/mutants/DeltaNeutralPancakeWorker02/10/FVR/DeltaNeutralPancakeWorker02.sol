...
contract DeltaNeutralPancakeWorker02 is OwnableUpgradeable, ReentrancyGuardUpgradeable, IWorker02 {
  ...
  /// @param _callerBalance - The balance that is owned by the msg.sender within the execution scope.
  function _rewardToBeneficialVault(uint256 _beneficialVaultBounty, uint256 _callerBalance) public {
  ...
  function actualBaseTokenBalance() public view returns (uint256) {
    return baseToken.myBalance() - buybackAmount;
  }

  /// @dev Internal function to stake all outstanding LP tokens to the given position ID.
  function _masterChefDeposit() public {
  ...
  function _masterChefWithdraw() public {
  ...
  function setPriceOracle(IDeltaNeutralOracle _priceOracle) public onlyOwner {
    priceOracle = _priceOracle;
  }
