...
contract AvatarArtBase is Ownable, IERC721Receiver{
     ...
     * @dev Get BNU token 
     */
    function getBnuToken() external view returns(IERC20){
        return _bnuToken;
    }
     ...
     */
    function getAvatarArtNFT() external view returns(IERC721){
     ...
     */
    function setAvatarArtNFT(address newAddress) external onlyOwner{
     ...
     * @dev Set BNU token 
     */
    function setBnuToken(address newAddress) external onlyOwner{
        require(newAddress != address(0), "Zero address");
