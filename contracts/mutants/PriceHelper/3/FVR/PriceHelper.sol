...
contract PriceHelper is IPriceHelper, Initializable, OwnableUpgradeable {
  ...
  function lpToDollar(uint256 lpAmount, address lpToken) public view returns (uint256) {
    if (lpAmount == 0) {
  ...
  /// @param lpToken address of LP token
  function dollarToLp(uint256 dollarAmount, address lpToken) public view returns (uint256) {
    if (dollarAmount == 0) {
