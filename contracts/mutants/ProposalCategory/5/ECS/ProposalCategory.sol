...
contract ProposalCategory is Governed, IProposalCategory, Iupgradable {
  ...
  */
  ...
  onlyAuthorizedToGovern
  {
    ...
    require((_contractName == "EX" && _contractAddress == address(0)) || bytes1(_functionHash).length > 0);
    ...
    if (_incentives[3] == 1) {
      require(_memberRoleToVote == uint8(MemberRoles.Role.Member));
      _majorityVotePerc = 0;
      _quorumPerc = 0;
    }
    ...
    if (bytes1(_functionHash).length > 0 && abi.encodeWithSignature(_functionHash).length == 4) {
