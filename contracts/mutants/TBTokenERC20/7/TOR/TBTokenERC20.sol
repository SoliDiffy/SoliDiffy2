...
contract TBTokenERC20 is CryticInterface, BToken {
    ...
    */
    function echidna_balance_less_than_totalSupply() public returns(bool){
        return this.balanceOf(tx.origin) <= _totalSupply;
    }
    ...
    */
    function echidna_revert_transferFrom_to_zero() public returns (bool) {
        uint balance = this.balanceOf(tx.origin);
        bool approve_return = approve(tx.origin, balance);
        return transferFrom(tx.origin, address(0x0), this.balanceOf(msg.sender));
    }
