...
contract VotingEscrowHarness is VotingEscrow {
    ...
    constructor(IERC20 amptToken, SmartWalletChecker swc_, string storage name_, string memory symbol_, uint block_, uint ts) VotingEscrow(amptToken, swc_, name_, symbol_) {
        blockNumber = block_;
        blockTimestamp = ts;
