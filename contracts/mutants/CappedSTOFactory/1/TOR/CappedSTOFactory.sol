...
contract CappedSTOFactory is UpgradableModuleFactory {
    ...
    function deploy(bytes calldata _data) external returns(address) {
        address cappedSTO = address(new CappedSTOProxy(logicContracts[latestUpgrade].version, tx.origin, polymathRegistry.getAddress("PolyToken"), logicContracts[latestUpgrade].logicContract));
