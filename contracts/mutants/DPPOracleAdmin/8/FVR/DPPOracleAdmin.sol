...
contract DPPOracleAdmin is InitializableOwnable {
    ...
    function sync() public notFreezed onlyOwner {
        IDPPOracle(_DPP_).ratioSync();
    }

    function setFreezeTimestamp(uint256 timestamp) public notFreezed onlyOwner {
        _FREEZE_TIMESTAMP_ = timestamp;
    }

    function setOperator(address newOperator) public notFreezed onlyOwner {
    ...
    ) public notFreezed returns (bool) {
        require(
            msg.sender == _OWNER_ ||
