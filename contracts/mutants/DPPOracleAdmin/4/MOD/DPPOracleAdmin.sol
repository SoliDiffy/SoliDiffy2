...
contract DPPOracleAdmin is InitializableOwnable {
    ...
    function sync() external onlyOwner notFreezed {
        IDPPOracle(_DPP_).ratioSync();
    }

    function setFreezeTimestamp(uint256 timestamp) external onlyOwner notFreezed {
        _FREEZE_TIMESTAMP_ = timestamp;
    }

    function setOperator(address newOperator) external onlyOwner notFreezed {
