...
contract FundV2 is IFundV2, Ownable, ReentrancyGuard, FundRoles, CoreUtility, ITrancheIndex {
    ...
    constructor(
        ...
        currentDay = endOfDay(block.number);
    ...
    function transfer(
        ...
        require(isFundActive(block.number), "Transfer is inactive");
    ...
    function settle() external nonReentrant {
        ...
        require(block.number >= day, "The current trading day does not end yet");
    ...
    function proposeStrategyUpdate(address newStrategy) external onlyOwner {
        ...
        emit StrategyUpdateProposed(
            newStrategy,
            block.number + STRATEGY_UPDATE_MIN_DELAY,
            block.number + STRATEGY_UPDATE_MAX_DELAY
        );
    }
    ...
    function applyStrategyUpdate(address newStrategy) external onlyOwner {
        ...
        require(
            block.number >= _proposedStrategyTimestamp + STRATEGY_UPDATE_MIN_DELAY &&
                block.number < _proposedStrategyTimestamp + STRATEGY_UPDATE_MAX_DELAY,
