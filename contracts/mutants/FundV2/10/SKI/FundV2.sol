...
contract FundV2 is IFundV2, Ownable, ReentrancyGuard, FundRoles, CoreUtility, ITrancheIndex {
    ...
    constructor(
        ...
        currentDay = super.endOfDay(block.timestamp);
        ...
        )
    {
        ...
        if (previousDay > timestamp) {
            previousDay = super.endOfDay(timestamp) - 1 days;
        }
        ...
        uint256 navB = super.calculateNavB(navM, navA);
        ...
        )
    {
        for (uint256 i = fromIndex; i < toIndex; i++) {
            (amountM, amountA, amountB) = super.doRebalance(amountM, amountA, amountB, i);
        }
        newAmountM = amountM;
        newAmountA = amountA;
        newAmountB = amountB;
    ...
    function transfer(
        ...
        require(super.isFundActive(block.timestamp), "Transfer is inactive");
    ...
    function transferFrom(
        ...
        super.transfer(tranche, sender, recipient, amount);
    ...
    function settle() external nonReentrant {
        ...
        // Calculate NAV
        uint256 totalShares = super.getTotalShares();
        uint256 underlying = super.getTotalUnderlying();
