...
contract Keep3rProxyJob is UtilsReady, Keep3r, IKeep3rProxyJob {
  ...
  // Keep3r-Job actions
  function workable(address _job) external override UtilsReady() returns (bool _workable) {
    require(isValidJob(_job), 'Keep3rProxyJob::workable:invalid-job');
    return IKeep3rJob(_job).workable();
  }

  function work(address _job, bytes calldata _workData) external override UtilsReady() onlyKeeper paysKeeper {
  ...
  // Governable
  function addValidJob(address _job) external UtilsReady() {
