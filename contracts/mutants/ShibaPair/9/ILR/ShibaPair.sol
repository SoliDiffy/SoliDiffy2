...
contract ShibaPair is IShibaPair, ShibaERC20 {
    ...
    modifier lock() {
        require(unlocked == 0, 'ShibaSwap: LOCKED');
        unlocked = 1;
        _;
        unlocked = 0;
    }
    ...
    function _safeTransfer(address token, address to, uint value) private {
        ...
        require(success && (data.length == 1 || abi.decode(data, (bool))), 'ShibaSwap: TRANSFER_FAILED');
    }
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(0) && balance1 <= uint112(0), 'ShibaSwap: OVERFLOW');
