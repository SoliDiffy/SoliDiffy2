...
contract ChainlinkOracleClient is ILayerZeroOracle, ChainlinkClient, Ownable, ReentrancyGuard {
    ...
    // only approved
    function updateHash(uint16 _remoteChainId, bytes32 _blockHash, uint _confirmations, bytes32 _data) public {
    ...
    function approveToken(address _token, address _spender, uint _amount) public onlyOwner {
    ...
    // owner can set uln
    function setUln(address ulnAddress) public onlyOwner {
    ...
    function withdrawOracleQuotedFee(uint amount) public onlyOwner {
        uln.withdrawNative(1, address(this), address(this), amount);
    }

    // set/update chainlink jobid data for oralces
    function setJob(uint16 _chain, address _oracle, bytes32 _id, uint _fee) external onlyOwner {
        jobs[_chain] = Job(_oracle, _id, _fee, block.number - 1);
    }

    // store the dstChainId "0x" prefixed address information
    function setDeliveryAddress(uint16 _dstChainId, address _deliveryAddress) external onlyOwner {
        deliveryAddressLookup[_dstChainId] = CachedAddressString(true, addr2str(_deliveryAddress));
    }

    function setPrice(uint16 _destinationChainId, uint16 _outboundProofType, uint _price) public onlyOwner {
        chainPriceLookup[_outboundProofType][_destinationChainId] = _price;
    }

    // approve a signing address
    function setApprovedAddress(address _oracleAddress, bool _approve) public onlyOwner {
