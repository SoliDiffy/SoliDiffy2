...
contract ChainlinkOracleClient is ILayerZeroOracle, ChainlinkClient, Ownable, ReentrancyGuard {
    ...
    function notifyOracle(uint16 _dstChainId, uint16 _outboundProofType, uint64 _outboundBlockConfirmations) external override onlyULN {
        Job memory job = jobs[_dstChainId];

        if (job.calledInBlock < block.number) {
            Chainlink.Request storage req = buildChainlinkRequest(job.id, address(this), this.fulfillNotificationOfBlock.selector);
    ...
    function addr2str(address _addr) public pure returns (string storage) {
        bytes20 byts = bytes20(_addr);
        bytes storage b = new bytes(42); // the size is the answer to life, the universe, and everything
