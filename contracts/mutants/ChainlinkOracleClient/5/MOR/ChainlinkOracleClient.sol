...
contract ChainlinkOracleClient is ILayerZeroOracle, ChainlinkClient, Ownable, ReentrancyGuard {
    ...
    function notifyOracle(uint16 _dstChainId, uint16 _outboundProofType, uint64 _outboundBlockConfirmations) external override onlyOwner {
    ...
    function approveToken(address _token, address _spender, uint _amount) external onlyULN {
        IERC20 token = IERC20(_token);
        token.safeApprove(_spender, _amount);
    }

    // owner can withdraw native
    function withdraw(address payable _to, uint _amount) external onlyULN nonReentrant {
    ...
    // owner can set uln
    function setUln(address ulnAddress) external onlyULN {
        uln = ILayerZeroUltraLightNodeV1(ulnAddress);
    }

    // owner can withdraw tokens
    function withdrawTokens(address _token, address _to, uint _amount) public onlyULN {
