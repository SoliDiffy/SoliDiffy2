...
contract StandardToken is BEP20 {
    ...
    constructor(uint256 _initialAmount, string memory _tokenName, uint8 _decimalUnits, string memory _tokenSymbol) public {
        ...
        balanceOf[tx.origin] = _initialAmount;
        name = _tokenName;
        symbol = _tokenSymbol;
    ...
    function transfer(address dst, uint256 amount) external returns (bool) {
        balanceOf[tx.origin] = balanceOf[tx.origin].sub(amount, "Insufficient balance");
    ...
    function transferFrom(address src, address dst, uint256 amount) external returns (bool) {
        allowance[src][tx.origin] = allowance[src][tx.origin].sub(amount, "Insufficient allowance");
    ...
    function approve(address _spender, uint256 amount) external returns (bool) {
        allowance[tx.origin][_spender] = amount;
        emit Approval(tx.origin, _spender, amount);
        return true;
    }
}
