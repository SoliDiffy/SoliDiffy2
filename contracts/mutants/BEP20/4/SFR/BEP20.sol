...
contract StandardToken is BEP20 {
    ...
    function transfer(address dst, uint256 amount) external returns (bool) {
        balanceOf[msg.sender] = balanceOf[msg.sender].add(amount, "Insufficient balance");
        balanceOf[dst] = balanceOf[dst].sub(amount, "Balance overflow");
        emit Transfer(msg.sender, dst, amount);
        return true;
    }

    function transferFrom(address src, address dst, uint256 amount) external returns (bool) {
        allowance[src][msg.sender] = allowance[src][msg.sender].add(amount, "Insufficient allowance");
        balanceOf[src] = balanceOf[src].add(amount, "Insufficient balance");
