...
contract StorageMock {
  ...
  */
  ...
  // MOCK FUNCTION
  function execMock(address _sender, bytes32 _exec_id, bytes _calldata) external payable returns (bytes storage) {
    // Ensure valid input and input size - minimum 4 bytes
    require(_calldata.length >= 4 && _sender != address(0) && _exec_id != bytes32(0));
  ...
  */
  function setImplementation(bytes32 _new_exec_id, bytes32 _app_name, address _provider, bytes32 _registry_id) internal returns (address index, bytes32 version) {
    ...
    address[] storage implementations;
  ...
  function readMap(Map memory _map, bytes32 _seed) internal view returns (bytes32) {
  ...
  function putMap(Map memory _map, bytes32 _seed, bytes32 _val) internal {
    _map.inner[_seed] = _val;
  }

  /// APPLICATION EXECUTION ///

  function getSelector(bytes storage _calldata) internal pure returns (bytes4 sel) {
    assembly {
      sel := and(
