...
contract StorageMock {
  ...
  // Mock - used to set time for testing purposes
  ...
  */
  function createInstance(address _sender, bytes32 _app_name, address _provider, bytes32 _registry_id, bytes _calldata) external payable returns (bytes32 version, bytes32 new_exec_id) {
    // Ensure valid input -
  ...
  */
  function exec(address _sender, bytes32 _exec_id, bytes _calldata) external payable returns (uint n_stored, uint n_emitted, uint n_paid) {
  ...
  @return n_stored: The number of storage slots written to on behalf of the application
  */
  function executeAppReturn(bytes32 _exec_id) internal returns (uint n_stored, uint n_emitted, uint n_paid) {
  ...
  */
  function getReturnedData() internal pure returns (uint _returndata_ptr, uint ptr_bounds) {
    assembly {
  ...
  */
  function doPay(bytes32 _exec_id, uint _ptr, uint _ptr_bound) internal returns (uint n_paid, uint ptr) {
  ...
  */
  function doStore(uint _ptr, uint _ptr_bound, bytes32 _exec_id) internal returns (uint n_stored, uint ptr) {
  ...
  */
  function doEmit(uint _ptr, uint _ptr_bound) internal returns (uint n_emitted, uint ptr) {
