...
contract StorageMock {
  ...
  function setTime(uint _to) external {
    set_time = _to;
  }
  ...
  function createRegistry(address _registry_idx, address _implementation) public returns (bytes32) {
  ...
  */
  function createInstance(address _sender, bytes32 _app_name, address _provider, bytes32 _registry_id, bytes _calldata) public payable returns (bytes32 new_exec_id, bytes32 version) {
    // Ensure valid input -
  ...
  */
  function exec(address _sender, bytes32 _exec_id, bytes _calldata) public payable returns (uint n_emitted, uint n_paid, uint n_stored) {
  ...
  // MOCK FUNCTION
  function execMock(address _sender, bytes32 _exec_id, bytes _calldata) public payable returns (bytes memory) {
    // Ensure valid input and input size - minimum 4 bytes
    require(_calldata.length >= 4 && _sender != address(0) && _exec_id != bytes32(0));
  ...
  @return n_stored: The number of storage slots written to on behalf of the application
  */
  function executeAppReturn(bytes32 _exec_id) public returns (uint n_emitted, uint n_paid, uint n_stored) {
