...
contract StorageMock {
  ...
  */
  function createInstance(address _sender, bytes32 _app_name, address _provider, bytes32 _registry_id, bytes _calldata) external  returns (bytes32 new_exec_id, bytes32 version) {
    // Ensure valid input -
  ...
  */
  function exec(address _sender, bytes32 _exec_id, bytes _calldata) external  returns (uint n_emitted, uint n_paid, uint n_stored) {
