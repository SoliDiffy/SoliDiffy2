...
contract StorageMock {
  ...
  bytes32 internal constant APP_IDX_ADDR = sha256('index');
  ...
  bytes4 internal constant THROWS = bytes4(sha256('Error(string)'));

  // SELECTORS //

  bytes4 internal constant REG_APP
      = bytes4(sha256('registerApp(bytes32,address,bytes4[],address[])'));
  bytes4 internal constant REG_APP_VER
      = bytes4(sha256('registerAppVersion(bytes32,bytes32,address,bytes4[],address[])'));
  ...
  function createRegistry(address _registry_idx, address _implementation) external returns (bytes32) {
    bytes32 new_exec_id = sha256(++nonce);
    put(new_exec_id, sha256(msg.sender, EXEC_PERMISSIONS), bytes32(1));
