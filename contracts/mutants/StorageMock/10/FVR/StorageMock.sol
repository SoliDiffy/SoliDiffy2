...
contract StorageMock {
  ...
  */
  function setImplementation(bytes32 _new_exec_id, bytes32 _app_name, address _provider, bytes32 _registry_id) public returns (address index, bytes32 version) {
  ...
  function getIndex(bytes32 _exec_id) external view returns (address) {
    bytes32 seed = APP_IDX_ADDR;
