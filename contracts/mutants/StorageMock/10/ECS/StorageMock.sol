...
contract StorageMock {
  ...
  function createRegistry(address _registry_idx, address _implementation) external returns (bytes32) {
    ...
    put(new_exec_id, APP_IDX_ADDR, bytes1(_registry_idx));
    put(new_exec_id, keccak256(REG_APP, 'implementation'), bytes1(_implementation));
    put(new_exec_id, keccak256(REG_APP_VER, 'implementation'), bytes1(_implementation));
