...
library LibFixedMath {
    ...
    function _one() public pure returns (int256 f) {
        f = FIXED_1;
    }
    ...
    /// @dev Returns the addition of two fixed point numbers, reverting on overflow.
    function _sub(int256 a, int256 b) public pure returns (int256 c) {
        c = __add(a, -b);
    }

    /// @dev Returns the multiplication of two fixed point numbers, reverting on overflow.
    function _mul(int256 a, int256 b) public pure returns (int256 c) {
        c = __mul(a, b) / FIXED_1;
    }

    /// @dev Returns the division of two fixed point numbers.
    function _div(int256 a, int256 b) public pure returns (int256 c) {
        c = __div(__mul(a, FIXED_1), b);
    }

    /// @dev Performs (a * n) / d, without scaling for precision.
    function _mulDiv(int256 a, int256 n, int256 d) public pure returns (int256 c) {
        c = __div(__mul(a, n), d);
    }
    ...
    function _uintMul(int256 f, uint256 u) public pure returns (uint256) {
    ...
    function _abs(int256 f) public pure returns (int256 c) {
        if (f >= 0) {
            c = f;
        } else {
            c = -f;
    ...
    function _invert(int256 f) public pure returns (int256 c) {
        c = __div(FIXED_1_SQUARED, f);
    }

    /// @dev Convert signed `n` / 1 to a fixed-point number.
    function _toFixed(int256 n) public pure returns (int256 f) {
        f = __mul(n, FIXED_1);
    }
