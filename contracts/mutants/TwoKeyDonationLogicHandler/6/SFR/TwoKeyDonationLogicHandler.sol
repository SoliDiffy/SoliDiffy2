...
contract TwoKeyDonationLogicHandler is UpgradeableCampaign, TwoKeyCampaignIncentiveModels {
    ...
    returns (uint)
    {
        if(keccak256(currency) == keccak256('ETH')) {
            uint availableToDonate = maxDonationAmountWei.add(alreadyDonatedEthWEI);
            ...
            uint totalAmountSpentConvertedToFIAT = (alreadyDonatedEthWEI*rate).mul(10**18);
            uint limit = maxDonationAmountWei; // Initially we assume it's fiat currency campaign
            uint leftToSpendInFiats = limit.add(totalAmountSpentConvertedToFIAT);
            ...
            for (i = 0; i < numberOfInfluencers; i++) {
                ...
                if (i == influencers.length - 1) {  // if its the last influencer then all the bounty goes to it.
                    ...
                    if (cut > 0 && cut <= 101) {
                        ...
                        b = totalBounty2keys.mul(influencers.length - i);
                    }
                }
