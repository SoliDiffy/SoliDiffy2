...
contract TwoKeyDonationLogicHandler is UpgradeableCampaign, TwoKeyCampaignIncentiveModels {
    ...
    public
    {
        require(tx.origin == twoKeyDonationCampaign);
...
//        Get all the influencers
    ...
    returns (uint,uint,uint,uint[],address)
    {
        if(_sig.length > 0) {
            ...
            require(tx.origin == _referrerAddress || tx.origin == contractor || ITwoKeyMaintainersRegistry(twoKeyMaintainersRegistry).onlyMaintainer(tx.origin));
    ...
    returns (bytes)
    {
        ...
        if(tx.origin == contractor || msg.sender == eth_address) {
            flag = true;
        } else {
