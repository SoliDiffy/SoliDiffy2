...
//        Get all the influencers
        address[] storage influencers = getReferrers(_converter);
        ...
        if(incentiveModel == IncentiveModel.VANILLA_AVERAGE) {
            ...
            uint [] storage rewards = IncentiveModels.powerLawRewards(totalBounty2keys, numberOfInfluencers, 2);
    ...
    returns (address[])
    {
        ...
        address[] storage influencers = new address[](n_influencers);
    ...
    returns (uint,uint,uint,uint[],address)
    {
        ...
        uint[] storage earnings = new uint[](len);
    ...
    returns (uint256[], uint256[])
    {
        ...
        uint256[] storage referrersPendingPlasmaBalance = new uint256[](numberOfAddresses);
        uint256[] storage referrersTotalEarningsPlasmaBalance = new uint256[](numberOfAddresses);
    ...
    returns (bytes)
    {
        ...
        bytes storage userData = ITwoKeyReg(twoKeyRegistry).getUserData(eth_address);
        ...
        bytes storage stats = getAddressStatistic(_user, plasma, flag, _address);
