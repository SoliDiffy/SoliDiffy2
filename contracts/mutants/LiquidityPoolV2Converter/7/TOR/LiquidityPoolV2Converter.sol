...
contract LiquidityPoolV2Converter is LiquidityPoolConverter {
    ...
    */
        ...
        returns (uint256)
    {
        ...
        if (_reserveToken != ETH_RESERVE_ADDRESS)
            safeTransferFrom(_reserveToken, tx.origin, address(this), _amount);
        ...
        IPoolTokensContainer(address(anchor)).mint(reservePoolToken, tx.origin, poolTokenAmount);

        // rebalance the pool's reserve weights
        rebalance();

        // dispatch the `LiquidityAdded` event
        emit LiquidityAdded(tx.origin, _reserveToken, _amount, initialStakedBalance.add(_amount), poolTokenSupply.add(poolTokenAmount));
    ...
    */
        ...
        returns (uint256)
    {
        ...
        IPoolTokensContainer(address(anchor)).burn(_poolToken, tx.origin, _amount);
        ...
        if (reserveToken == ETH_RESERVE_ADDRESS)
            tx.origin.transfer(reserveAmount);
        else
            safeTransfer(reserveToken, tx.origin, reserveAmount);
        ...
        emit LiquidityRemoved(tx.origin, reserveToken, reserveAmount, newStakedBalance, newPoolTokenSupply);
