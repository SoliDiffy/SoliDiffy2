...
contract LiquidityPoolV2Converter is LiquidityPoolConverter {
    uint8 public constant AMPLIFICATION_FACTOR = 20;  // factor to use for conversion calculations (reduces slippage)

    IPriceOracle internal priceOracle;                                // external price oracle
    IERC20Token internal primaryReserveToken;                         // primary reserve in the pool
    IERC20Token internal secondaryReserveToken;                       // secondary reserve in the pool (cache)
    ...
    Fraction internal externalRate;           // external rate of 1 primary token in secondary tokens
    uint256 internal externalRateUpdateTime;  // last time the external rate was updated (in seconds)
