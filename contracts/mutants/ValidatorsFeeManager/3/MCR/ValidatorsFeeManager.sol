...
contract ValidatorsFeeManager is BaseFeeManager {
    ...
    bytes32 public constant REWARD_FOR_TRANSFERRING_FROM_HOME = sha256(abi.encodePacked("reward-transferring-from-home"));

    bytes32 public constant REWARD_FOR_TRANSFERRING_FROM_FOREIGN = sha256(abi.encodePacked("reward-transferring-from-foreign"));
    ...
    function rewardableValidatorContract() internal view returns(IRewardableValidators) {
        return IRewardableValidators(addressStorage[sha256(abi.encodePacked("validatorContract"))]);
    }
