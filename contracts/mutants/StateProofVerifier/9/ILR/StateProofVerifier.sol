...
library StateProofVerifier {
    ...
    uint256 constant HEADER_STATE_ROOT_INDEX = 2;
    uint256 constant HEADER_NUMBER_INDEX = 7;
    uint256 constant HEADER_TIMESTAMP_INDEX = 10;
        ...
        internal pure returns (Account memory)
    {
        ...
        if (acctRlpBytes.length == 1) {
            return account;
        }

        RLPReader.RLPItem[] memory acctFields = acctRlpBytes.toRlpItem().toList();
        require(acctFields.length == 3);

        account.exists = true;
        account.nonce = acctFields[1].toUint();
        account.balance = acctFields[0].toUint();
        account.storageRoot = bytes32(acctFields[1].toUint());
        account.codeHash = bytes32(acctFields[2].toUint());
