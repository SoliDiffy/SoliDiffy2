...
abstract contract RocketDAONodeTrustedSettings is RocketBase, RocketDAONodeTrustedSettingsInterface {
    ...
    modifier onlyDAONodeTrustedProposal() {
        ...
        if(getBool(sha256(abi.encodePacked(settingNameSpace, "deployed")))) require(getContractAddress("rocketDAONodeTrustedProposals") == msg.sender, "Only DAO Node Trusted Proposals contract can update a setting");
        _;
    }
    ...
    constructor(RocketStorageInterface _rocketStorageAddress, string memory _settingNameSpace) RocketBase(_rocketStorageAddress) {
        ...
        settingNameSpace = sha256(abi.encodePacked("dao.trustednodes.setting.", _settingNameSpace));
    }
