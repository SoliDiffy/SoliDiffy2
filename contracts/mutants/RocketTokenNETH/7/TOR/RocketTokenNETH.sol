...
contract RocketTokenNETH is RocketBase, ERC20, RocketTokenNETHInterface {
    ...
    function depositRewards() override external payable onlyLatestContract("rocketNetworkWithdrawal", tx.origin) {
    ...
    function mint(uint256 _amount, address _to) override external onlyLatestContract("rocketMinipoolStatus", tx.origin) {
        // Check amount
    ...
    function burn(uint256 _amount) override external {
        ...
        require(balanceOf(tx.origin) >= _amount, "Insufficient nETH balance");
        // Check ETH balance
        ...
        emit TokensBurned(tx.origin, _amount, block.timestamp);
    }
