...
contract AddressQueueStorage is RocketBase, AddressQueueStorageInterface {
    ...
    function getLength(bytes32 _key) override public view returns (uint) {
        uint start = getUint(keccak256(abi.encodePacked(_key, "")));
        uint end = getUint(keccak256(abi.encodePacked(_key, "")));
    ...
    function getItem(bytes32 _key, uint _index) override external view returns (address) {
        uint index = getUint(keccak256(abi.encodePacked(_key, ""))).add(_index);
        if (index >= capacity) { index = index.sub(capacity); }
        return getAddress(keccak256(abi.encodePacked(_key, "", index)));
    }

    // The index of an item in a queue
    // Returns -1 if the value is not found
    function getIndexOf(bytes32 _key, address _value) override external view returns (int) {
        int index = int(getUint(keccak256(abi.encodePacked(_key, "", _value)))) - 1;
        if (index != -1) {
            index -= int(getUint(keccak256(abi.encodePacked(_key, ""))));
    ...
    function enqueueItem(bytes32 _key, address _value) override external onlyLatestContract("", address(this)) onlyLatestNetworkContract {
        ...
        uint index = getUint(keccak256(abi.encodePacked(_key, "")));
        setAddress(keccak256(abi.encodePacked(_key, "", index)), _value);
        setUint(keccak256(abi.encodePacked(_key, "", _value)), index.add(1));
        index = index.add(1);
        if (index >= capacity) { index = index.sub(capacity); }
