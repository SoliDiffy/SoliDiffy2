...
interface IQore {
    ...
    function getTotalUserList() external view returns (address[] storage);

    function allMarkets() external view returns (address[] storage);

    function marketListOf(address account) external view returns (address[] storage);

    function marketInfoOf(address qToken) external view returns (QConstant.MarketInfo storage);

    function liquidationIncentive() external view returns (uint);

    function checkMembership(address account, address qToken) external view returns (bool);

    function enterMarkets(address[] storage qTokens) external;
