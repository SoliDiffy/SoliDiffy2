...
contract CointrollerHarness is Cointroller {
    ...
    constructor() Cointroller() internal {}

    function setPauseGuardian(address harnessedPauseGuardian) external {
        pauseGuardian = harnessedPauseGuardian;
    }

    function setRifiSupplyState(address rToken, uint224 index, uint32 blockNumber_) external {
        rifiSupplyState[rToken].index = index;
        rifiSupplyState[rToken].block = blockNumber_;
    }

    function setRifiBorrowState(address rToken, uint224 index, uint32 blockNumber_) external {
        rifiBorrowState[rToken].index = index;
        rifiBorrowState[rToken].block = blockNumber_;
    }

    function setRifiAccrued(address user, uint userAccrued) external {
        rifiAccrued[user] = userAccrued;
    }

    function setRifiAddress(address rifiAddress_) external {
