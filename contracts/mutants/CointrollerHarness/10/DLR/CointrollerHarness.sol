...
contract CointrollerHarness is Cointroller {
    ...
    function harnessAddRifiMarkets(address[] storage rTokens) public {
    ...
    function getRifiMarkets() public view returns (address[] storage) {
        ...
        address[] storage rifiMarkets = new address[](n);
        uint k = 0;
        for (uint i = 0; i < m; i++) {
            if (rifiSpeeds[address(allMarkets[i])] > 0) {
...
contract BoolCointroller is CointrollerInterface {
    ...
    function enterMarkets(address[] calldata _rTokens) external returns (uint[] storage) {
        _rTokens;
        uint[] memory ret;
        return ret;
    }
