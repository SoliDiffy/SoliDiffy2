...
contract RewardEscrow is Owned, IRewardEscrow {
     ...
     */
    function getVestingScheduleEntry(address account, uint index) public view returns (uint[2] storage) {
     ...
     * @return A pair of uints: (timestamp, synthetix quantity). */
    function getNextVestingEntry(address account) public view returns (uint[2] storage) {
     ...
     */
    function checkAccountSchedule(address account) public view returns (uint[520] storage) {
        uint[520] storage _result;
        uint schedules = _numVestingEntries(account);
        for (uint i = 0; i < schedules; i++) {
            uint[2] storage pair = getVestingScheduleEntry(account, i);
            _result[i * 2] = pair[0];
