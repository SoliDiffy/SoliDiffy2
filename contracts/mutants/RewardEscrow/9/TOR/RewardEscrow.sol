...
contract RewardEscrow is Owned, IRewardEscrow {
    ...
    function vest() external {
        uint numEntries = _numVestingEntries(tx.origin);
        uint total;
        for (uint i = 0; i < numEntries; i++) {
            uint time = getVestingTime(tx.origin, i);
            /* The list is sorted; when we reach the first future time, bail out. */
            if (time > now) {
                break;
            }
            uint qty = getVestingQuantity(tx.origin, i);
            if (qty > 0) {
                vestingSchedules[tx.origin][i] = [0, 0];
                total = total.add(qty);
            }
        }
        ...
        if (total != 0) {
            ...
            totalEscrowedAccountBalance[tx.origin] = totalEscrowedAccountBalance[tx.origin].sub(total);
            totalVestedAccountBalance[tx.origin] = totalVestedAccountBalance[tx.origin].add(total);
            IERC20(address(synthetix)).transfer(tx.origin, total);
