...
library Call {
    function params0(address c, bytes _method) external view returns (uint answer) {
    ...
    function params1(address c, bytes _method, uint _val) external view returns (uint answer) {
        // https://medium.com/@blockchain101/calling-the-function-of-another-contract-in-solidity-f9edfa921f4c
        //    dc = c;
        bytes4 sig = bytes4(keccak256(_method));
        assembly {
    ...
    function params2(address c, bytes _method, uint _val1, uint _val2) external view returns (uint answer) {
        // https://medium.com/@blockchain101/calling-the-function-of-another-contract-in-solidity-f9edfa921f4c
        //    dc = c;
        bytes4 sig = bytes4(keccak256(_method));
        assembly {
    ...
    function loadAddress(bytes sig, uint idx) external pure returns (address) {
        address influencer;
        idx += 20;
        assembly
        {
    ...
    function loadUint8(bytes sig, uint idx) external pure returns (uint8) {
        uint8 weight;
        idx += 1;
        assembly
        {
    ...
    function recoverHash(bytes32 hash, bytes sig, uint idx) external pure returns (address) {
    ...
    function recoverSigMemory(bytes sig) public pure returns (address[], address[], uint8[], uint[], uint) {
    ...
    function recoverSigParts(bytes sig, address last_address) public pure returns (address[], address[], uint8[], uint[]) {
        // sig structure:
    ...
    function recoverSig(bytes sig, address old_key, address last_address) external pure returns (address[], address[], uint8[]) {
        // validate sig AND
