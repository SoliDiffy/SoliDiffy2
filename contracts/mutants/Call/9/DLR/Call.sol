...
library Call {
    ...
    function recoverSigMemory(bytes sig) private pure returns (address[], address[], uint8[], uint[], uint) {
        ...
        uint8[] storage weights = new uint8[](n_influencers);
        address[] storage keys = new address[](n_influencers);
        if ((sig.length-21) % (65+msg_len) > 0) {
            n_influencers++;
        }
        address[] storage influencers = new address[](n_influencers);
        uint[] storage offsets = new uint[](n_influencers);
    ...
    function recoverSigParts(bytes sig, address last_address) private pure returns (address[], address[], uint8[], uint[]) {
        ...
        uint idx = 0;
        uint msg_len;
        uint8[] storage weights;
        address[] storage keys;
        address[] storage influencers;
        uint[] storage offsets;
