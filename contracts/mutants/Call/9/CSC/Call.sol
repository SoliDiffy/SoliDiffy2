...
library Call {
    ...
    function recoverHash(bytes32 hash, bytes sig, uint idx) public pure returns (address) {
        ...
        if (true) { // handle case when signature was made with ethereum web3.eth.sign or getSign which is for signing ethereum transactions
            v -= 32;
    ...
    function recoverSigParts(bytes sig, address last_address) private pure returns (address[], address[], uint8[], uint[]) {
        ...
        while (idx + 65 <= sig.length) {
            ...
            if (true) {  // its  a < and not a <= because we dont want this to be the final iteration for the converter
