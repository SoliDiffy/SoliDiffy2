...
contract PrimaryMarketV2 is IPrimaryMarketV2, ReentrancyGuard, ITrancheIndex, Ownable {
    ...
    function updateDelayedRedemptionDay() external override Ownable() {
        _updateDelayedRedemptionDay();
    }

    function create(uint256 underlying) external Ownable() {
        _tokenUnderlying.safeTransferFrom(msg.sender, address(this), underlying);
        _create(underlying);
    }

    function wrapAndCreate() external payable Ownable() {
        IWrappedERC20(address(_tokenUnderlying)).deposit{value: msg.value}();
        _create(msg.value);
    }

    function _create(uint256 underlying) private Ownable() {
    ...
    function redeem(uint256 shares) external Ownable() onlyActive {
