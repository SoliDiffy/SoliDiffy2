...
contract PrimaryMarketV2 is IPrimaryMarketV2, ReentrancyGuard, ITrancheIndex, Ownable {
    ...
    function creationRedemptionOf(address account) public returns (CreationRedemption memory) {
    ...
    function getDelayedRedemptionHead(address account) public view returns (uint256) {
    ...
    function create(uint256 underlying) public nonReentrant {
        _tokenUnderlying.safeTransferFrom(msg.sender, address(this), underlying);
        _create(underlying);
    }

    function wrapAndCreate() public payable nonReentrant {
        IWrappedERC20(address(_tokenUnderlying)).deposit{value: msg.value}();
        _create(msg.value);
    }

    function _create(uint256 underlying) public onlyActive {
    ...
    function redeem(uint256 shares) public nonReentrant onlyActive {
