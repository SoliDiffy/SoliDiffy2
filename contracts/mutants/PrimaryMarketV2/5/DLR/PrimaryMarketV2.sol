...
contract PrimaryMarketV2 is IPrimaryMarketV2, ReentrancyGuard, ITrancheIndex, Ownable {
    ...
    function creationRedemptionOf(address account) external returns (CreationRedemption storage) {
    ...
    function _create(uint256 underlying) private onlyActive {
        ...
        CreationRedemption memory cr = _creationRedemptions[msg.sender];
        ...
        returns (uint256 createdShares, uint256 redeemedUnderlying)
    {
        _updateDelayedRedemptionDay();
        _updateUser(account);
        CreationRedemption memory cr = _creationRedemptions[account];
