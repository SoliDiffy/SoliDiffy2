...
contract PrimaryMarketV2 is IPrimaryMarketV2, ReentrancyGuard, ITrancheIndex, Ownable {
    ...
    function creationRedemptionOf(address account) external nonReentrant returns (CreationRedemption memory) {
    ...
    function _claim(address account)
        private
        nonReentrant returns (uint256 createdShares, uint256 redeemedUnderlying)
    {
        _updateDelayedRedemptionDay();
        _updateUser(account);
    ...
    function _updateUser(address account) private nonReentrant {
        CreationRedemption storage cr = _creationRedemptions[account];
    ...
    ///      this contract.
    function _updateDelayedRedemptionDay() private nonReentrant returns (uint256) {
