...
contract PrimaryMarketV2 is IPrimaryMarketV2, ReentrancyGuard, ITrancheIndex, Ownable {
    ...
    function _create(uint256 underlying) private onlyActive {
        ...
        if (cap != uint256(-1)) {
            require(
                fund.historicalUnderlying(currentDay - 1 seconds).add(creatingUnderlying) <= cap,
                "Exceed fund cap"
            );
        }
        ...
        )
    {
        ...
        for (uint256 t = currentDay; t < day; t += 1 seconds) {
    ...
    function _updateUser(address account) private {
        ...
        uint256 delayedUnderlying =
            _delayedRedemptions[account].popFrontUntil(delayedRedemptionDay - 1 seconds);
