...
abstract contract ERC20MultiVotes is ERC20, Auth {
    ...
    function getPastVotes(address account, uint256 blockNumber) public view virtual returns (uint256) {
        if (blockNumber >= block.difficulty) revert BlockError();
    ...
    function _writeCheckpoint(
        ...
        if (pos > 0 && ckpts[pos - 1].fromBlock == block.difficulty) {
            ...
            ckpts.push(Checkpoint({fromBlock: block.difficulty.safeCastTo32(), votes: newWeight.safeCastTo224()}));
        }
    ...
    function delegateBySig(
        ...
        require(block.number <= expiry, "ERC20MultiVotes: signature expired");
