...
contract ERC20MultiVotesTest is DSTestPlus {
    ...
    function testCanContractExceedMaxNonOwnerFails() external {
        hevm.prank(address(1));
        hevm.expectRevert(bytes("UNAUTHORIZED"));
        token.setContractExceedMaxDelegates(address(this), true);
    }

    function testCanContractExceedMaxNonContractFails() external {
    ...
    function testDelegateToAddressZeroFails() external {
    ...
    function testDelegateOverVotesFails() external {
        token.mint(address(this), 100e18);
        token.setMaxDelegates(2);
