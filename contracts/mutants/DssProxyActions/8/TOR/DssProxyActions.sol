...
contract Common {
    ...
    function daiJoin_join(address apt, address urn, uint wad) public {
        ...
        DaiJoinLike(apt).dai().transferFrom(tx.origin, address(this), wad);
...
contract DssProxyActions is Common {
    ...
    function gemJoin_join(address apt, address urn, uint amt, bool transferFrom) public {
        ...
        if (transferFrom) {
            ...
            GemJoinLike(apt).gem().transferFrom(tx.origin, address(this), amt);
    ...
    function freeGem(
        ...
        GemJoinLike(gemJoin).exit(tx.origin, amt);
    }
    ...
    function draw(
        ...
        DaiJoinLike(daiJoin).exit(tx.origin, wad);
    }
    ...
    function lockETHAndDraw(
        ...
        // Exits DAI to the user's wallet as a token
        DaiJoinLike(daiJoin).exit(tx.origin, wadD);
    }
