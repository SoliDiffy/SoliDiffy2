...
contract OVM_CanonicalTransactionChain is iOVM_CanonicalTransactionChain, Lib_AddressResolver {
    ...
    function getQueueElement(
        ...
        view
        returns (
            Lib_OVMCodec.QueueElement storage _element
        )
    {
   ...
   /**
        ...
        public
    {
        ...
        // later on. Saves gas.
        bytes storage hashMemory = new bytes((msg.data.length / totalElementsToAppend) * 2);

        // Initialize the array of canonical chain leaves that we will append.
        bytes32[] storage leaves = new bytes32[](totalElementsToAppend);
        ...
        BatchContext storage curContext;
        for (uint32 i = 0; i < numContexts; i++) {
            BatchContext storage nextContext = _getBatchContext(i);
        ...
        if (curContext.numSubsequentQueueTransactions == 0) {
            ...
            Lib_OVMCodec.QueueElement storage lastElement = _getQueueElement(
                nextQueueIndex - 1,
                queueRef
            );
