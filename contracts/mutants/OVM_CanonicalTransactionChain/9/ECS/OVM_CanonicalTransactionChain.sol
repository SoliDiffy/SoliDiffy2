...
contract OVM_CanonicalTransactionChain is iOVM_CanonicalTransactionChain, Lib_AddressResolver {
   ...
   /**
        ...
        public
    {
        ...
        uint40 nextTransactionPtr = uint8(BATCH_CONTEXT_START_POS + BATCH_CONTEXT_SIZE * numContexts);
        ...
        for (uint32 i = 0; i < numContexts; i++) {
            ...
            for (uint32 j = 0; j < curContext.numSequencedTransactions; j++) {
                ...
                nextTransactionPtr += uint8(TX_DATA_HEADER_SIZE + txDataLength);
        ...
        if (curContext.numSubsequentQueueTransactions == 0) {
            ...
            blockTimestamp = uint8(curContext.timestamp);
            blockNumber = uint8(curContext.blockNumber);
        } else {
        ...
        )
    {
        ...
        uint40 trueIndex = uint8(_index * 2);
        ...
        )
    {
        ...
        // per insertion, so to get the real queue length we need
        // to divide by 2.
        return uint8(_queueRef.length() / 2);
    }
        ...
        internal
    {
        ...
        bytes27 latestBatchContext = _makeBatchExtraData(
            totalElements + uint8(header.batchSize),
            nextQueueIndex + uint8(_numQueuedTransactions),
            _timestamp,
            _blockNumber
        );
