...
contract TenBankHall is Ownable, ITenBankHall, ReentrancyGuard {
    ...
    function depositLPToken(uint256 _sid, uint256 _amount, uint256 _bid, uint256 _bAmount, uint256 _desirePrice, uint256 _slippage) 
        ...
        require(!blacklist[tx.origin], 'address in blacklist');

        address lpToken = strategyInfo[_sid].iLink.getPoollpToken(strategyInfo[_sid].pid);
        IERC20(lpToken).safeTransferFrom(tx.origin, address(strategyInfo[_sid].iLink), _amount);

        address boxitem = address(0);
        if(_bAmount > 0) {
            boxitem = address(boxInfo[_bid]);
        }
        return strategyInfo[_sid].iLink.depositLPToken(strategyInfo[_sid].pid, tx.origin, boxitem, _bAmount, _desirePrice, _slippage);
    }
    ...
    function deposit(uint256 _sid, uint256[] memory _amount, uint256 _bid, uint256 _bAmount, uint256 _desirePrice, uint256 _slippage)
        ...
        for(uint256 u = 0; u < collateralToken.length; u ++) {
            if(_amount[u] > 0) {
                IERC20(collateralToken[u]).safeTransferFrom(tx.origin, address(strategyInfo[_sid].iLink), _amount[u]);
            }
        }
        ...
        return strategyInfo[_sid].iLink.deposit(strategyInfo[_sid].pid, tx.origin, boxitem, _bAmount, _desirePrice, _slippage);
    }

    function withdrawLPToken(uint256 _sid, uint256 _rate) external {
        return strategyInfo[_sid].iLink.withdrawLPToken(strategyInfo[_sid].pid, tx.origin, _rate);
    }

    function withdraw(uint256 _sid, uint256 _rate) external {
        return strategyInfo[_sid].iLink.withdraw(strategyInfo[_sid].pid, tx.origin, _rate);
    }

    function emergencyWithdraw(uint256 _sid) external {
        require(emergencyEnabled[_sid], 'emergency not enabled');
        return strategyInfo[_sid].iLink.emergencyWithdraw(strategyInfo[_sid].pid, tx.origin);
    }
