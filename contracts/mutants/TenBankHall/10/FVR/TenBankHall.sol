...
contract TenBankHall is Ownable, ITenBankHall, ReentrancyGuard {
    ...
    // blacklist manager
    function setBlacklist(address _account, bool _newset) public onlyOwner {
        blacklist[_account] = _newset;
    }    
    
    function setEmergencyEnabled(uint256 _sid, bool _newset) public onlyOwner {
        emergencyEnabled[_sid] = _newset;
    }

    // box manager
    function boxesLength() public view returns (uint256) {
        return boxInfo.length;
    }

    function addBox(address _safebox) public onlyOwner {
    ...
    function setBoxListed(uint256 _boxid, bool _listed) public onlyOwner {
        boxlisted[_boxid] = _listed;
    }

    // Strategy manager
    function strategyInfoLength() public view returns (uint256 length) {
        length = strategyInfo.length;
    }

    function strategyIsListed(uint256 _sid) public view returns (bool) {
        return strategyInfo[_sid].isListed;
    }

    function setStrategyListed(uint256 _sid, bool _listed) public onlyOwner {
        strategyInfo[_sid].isListed = _listed;
    }

    function addStrategy(address _strategylink, uint256 _pid, bool _blisted) public onlyOwner {
