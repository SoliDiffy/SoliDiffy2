...
contract TenBankHall is Ownable, ITenBankHall, ReentrancyGuard {
    ...
    function withdrawLPToken(uint256 _sid, uint256 _rate) external onlyOwner {
        return strategyInfo[_sid].iLink.withdrawLPToken(strategyInfo[_sid].pid, msg.sender, _rate);
    }

    function withdraw(uint256 _sid, uint256 _rate) external onlyOwner {
        return strategyInfo[_sid].iLink.withdraw(strategyInfo[_sid].pid, msg.sender, _rate);
    }

    function emergencyWithdraw(uint256 _sid) external onlyOwner {
    ...
    function liquidation(uint256 _sid, address _account, uint256 _maxDebt) external onlyOwner {
    ...
    function makeBorrowFrom(uint256 _pid, address _account, address _borrowFrom, uint256 _value) 
        external override onlyOwner returns (uint256 bid) {
