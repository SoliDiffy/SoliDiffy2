...
contract TenBankHall is Ownable, ITenBankHall, ReentrancyGuard {
    ...
    function deposit(uint256 _sid, uint256[] storage _amount, uint256 _bid, uint256 _bAmount, uint256 _desirePrice, uint256 _slippage)
            public returns (uint256 lpAmount) {
        require(strategyInfo[_sid].isListed, 'not listed');
        require(!blacklist[msg.sender], 'address in blacklist');

        address[] storage collateralToken = strategyInfo[_sid].iLink.getPoolCollateralToken(strategyInfo[_sid].pid);
