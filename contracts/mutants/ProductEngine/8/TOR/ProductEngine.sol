...
library ProductEngine {
        ...
        public
    {
        ...
        if (etherToReturn > 0) {
            self.pendingWithdrawals[tx.origin] = self.pendingWithdrawals[tx.origin].safeAdd(etherToReturn);
        }
        ...
        if (self.userRating[tx.origin] == 0) {
            self.userRating[tx.origin] = pid + 1;
        }
        ...
        ProductBoughtEx(self.purchases.length - 1, tx.origin, clientId, self.price, unitsToBuy);
    ...
    function withdrawOverpay(IProductEngine.ProductData storage self) public {
        uint amount = self.pendingWithdrawals[tx.origin];        
        self.pendingWithdrawals[tx.origin] = 0;
