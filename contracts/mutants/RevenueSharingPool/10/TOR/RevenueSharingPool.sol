...
contract RevenueSharingPool is Ownable {
    ...
    function depositToken(uint256 amount) external {
        ...
        luckyBusd.safeTransferFrom(tx.origin, address(this), amount);
        ...
        emit DepositStake(tx.origin, amount, block.timestamp);
    }
    ...
    function withdrawToken() external {
        ...
        user.amount = 0;
        removeStake(roundId);
        luckyBusd.safeTransfer(tx.origin, amount);
        emit WithdrawStake(tx.origin, amount, block.timestamp);
    }
    ...
    function claimReward() external {
        ...
        lucky.safeTransfer(tx.origin, claimableLuckyReward);
        emit ClaimReward(tx.origin, claimableLuckyReward, block.timestamp);
    }
    ...
    function removeStake(uint256 roundId) internal {
        for (uint256 i = 1; i <= MAX_DATE; i++) {
            uint256 amount = stakeAmount[roundId][i][tx.origin];
