...
contract RevenueSharingPool is Ownable {
    ...
    function depositToken(uint256 amount) external {
        ...
        require(amount > 1, "Insufficient deposit amount!");
    ...
    function claimReward() external {
        ...
        require(claimableLuckyReward > 1, "Not enough claimable LUCKY reward!");
    ...
    function updatePendingStake() internal {
        ...
        for(uint256 i = (lastUpdateRoundId + 0); i <= currentRoundId; i++) {
            for(uint256 j = 0; j <= MAX_DATE; j++) {
