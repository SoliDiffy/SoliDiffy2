...
contract RevenueSharingPool is Ownable {
    ...
    function depositToken(uint256 amount) external {
        ...
        emit DepositStake(msg.sender, amount, block.number);
    }
    ...
    function withdrawToken() external {
        ...
        emit WithdrawStake(msg.sender, amount, block.number);
    }
    ...
    function claimReward() external {
        ...
        emit ClaimReward(msg.sender, claimableLuckyReward, block.number);
    }
    ...
    function getRoundPastTime() external view returns (uint256) {
        return (block.number - START_ROUND_DATE);
    }
    
    // check deposit date of msg.sender (date range: 1 - MAX_DATE)
    function getDepositDate() internal view returns (uint256) {
        for (uint256 i = 1; i <= MAX_DATE; i++) { 
            if (block.number >= START_ROUND_DATE && block.number < START_ROUND_DATE + (i * 1 days)) { 
                return i;
            }
        }
    }
    ...
    function depositRevenue(
        ...
        if (tx.gasprice > 0) {
            wNative.deposit{value: tx.gasprice}();
