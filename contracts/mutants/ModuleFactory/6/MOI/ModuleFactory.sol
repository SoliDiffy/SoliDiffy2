...
contract ModuleFactory is IModuleFactory, Ownable {
    ...
    function _proposeUsageCost(uint256 _usageCostProposed) internal onlyOwner {
    ...
    function _proposedChangeInCostType(bool _isCostInPoly) internal onlyOwner {
     ...
     */
    function setupCostInPoly() public onlyOwner returns (uint256) {
        if (isCostInPoly)
            return setupCost;
     ...
     */
    function usageCostInPoly() public onlyOwner returns (uint256) {
        if (isCostInPoly)
            return usageCost;
        uint256 polyRate = IOracle(polymathRegistry.getAddress(POLY_ORACLE)).getPrice();
     ...
     */
    function _takeFee() internal onlyOwner returns(uint256) {
     ...
     */
    function _initializeModule(address _module, bytes memory _data) internal onlyOwner {
