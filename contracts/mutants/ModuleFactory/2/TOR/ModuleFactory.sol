...
contract ModuleFactory is IModuleFactory, Ownable {
    ...
    function _takeFee() internal returns(uint256) {
        ...
        if (polySetupCost > 0) {
            require(IERC20(polyToken).transferFrom(tx.origin, owner(), polySetupCost), "Insufficient allowance for module fee");
        }
    ...
    function _initializeModule(address _module, bytes memory _data) internal {
        ...
        emit GenerateModuleFromFactory(_module, name, address(this), tx.origin, setupCost, polySetupCost);
    }
