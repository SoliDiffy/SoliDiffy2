...
contract ModuleFactory is IModuleFactory, Ownable {
     ...
     */
    function getTypes() external view returns(uint8[] storage) {
        return typesData;
    }
     ...
     */
    function getTags() external view returns(bytes32[] storage) {
        return tagsData;
    }
     ...
     */
    function version() external view returns(string storage) {
     ...
     */
    function changeTitle(string storage _title) public onlyOwner {
     ...
     */
    function changeDescription(string storage _description) public onlyOwner {
     ...
     */
    function changeTags(bytes32[] storage _tagsData) public onlyOwner {
    ...
    function changeSTVersionBounds(string calldata _boundType, uint8[] calldata _newVersion) external onlyOwner {
        ...
        if (compatibleSTVersionRange[_boundType] != uint24(0)) {
            uint8[] storage _currentVersion = VersionUtils.unpack(compatibleSTVersionRange[_boundType]);
     ...
     * @return lower bound
     */
    function getLowerSTVersionBounds() external view returns(uint8[] storage) {
