...
contract TwoKeyContract is StandardToken, Ownable {
  ...
  function senderPlasma() external view returns (address) {
  ...
  function plasmaOf(address me) external view returns (address) {
    address plasma = me;
    if (registry == address(0)) {
      return plasma;
    }
  ...
  function ethereumOf(address me) external view returns (address) {
  ...
  function setCutOf(address me, uint256 cut) public {
  ...
  function setCut(uint256 cut) external {
    setCutOf(msg.sender, cut);
  }

  function cutOf(address me) external view returns (uint256) {
    return influencer2cut[plasmaOf(me)];
  }

  function getCuts(address last_influencer) external view returns (uint256[]) {
   ...
   */
  function transferFrom(address _from, address _to, uint256 _value) external onlyOwner returns (bool) {
    return transferFromInternal(_from, _to, _value);
  }
  function transferFromInternal(address _from, address _to, uint256 _value) public returns (bool) {
