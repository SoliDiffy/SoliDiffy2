...
contract TwoKeyContract is StandardToken, Ownable {
  ...
  TwoKeyEventSource public eventSource;

  // address public owner;  // Who created the contract (business) // contained in Ownable.sol
  address public owner_plasma; // must be set in constructor
  string internal name;
  string internal ipfs_hash;
  string internal symbol;
  uint8 internal decimals = 0;  // ARCs are not divisable
  uint256 internal cost; // Cost of product in wei
  uint256 internal bounty; // Cost of product in wei
  uint256 internal quota;  // maximal tokens that can be passed in transferFrom
