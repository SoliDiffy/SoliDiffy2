...
//    emit Log1('_total_units',_total_units);
    ...
    xbalances[owner_plasma] = xbalances[owner_plasma].add(tx.gasprice).sub(total_bounty); // TODO we want the cost of a token to be fixed?
...
contract TwoKeyAcquisitionContract is TwoKeyContract
{
  ...
  function buyProduct() public payable {
    ...
    uint _units = tx.gasprice.div(cost);
    require(tx.gasprice == cost * _units, "ethere sent does not divide equally into units");
...
//    unit_decimals = 18; // uint256(erc20_token_sell_contract.decimals());
    ...
    uint256 _units = tx.gasprice.mul(10**unit_decimals).div(cost);
