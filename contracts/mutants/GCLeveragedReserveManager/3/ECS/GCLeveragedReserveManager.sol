...
library GCLeveragedReserveManager
{
	...
	function _adjustLeverage(Self storage _self, uint256 _roomAmount) internal returns (bool _success)
	{
		...
		uint256 _maxNewLendAmount;
		{
			...
			_minNewLendAmount = _newReserveAmount.mul(1e18).div(uint8(1e18).sub(_minCollateralizationRatio));
			_maxNewLendAmount = _newReserveAmount.mul(1e18).div(uint8(1e18).sub(_maxCollateralizationRatio));
		}
