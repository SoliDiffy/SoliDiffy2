...
library GCLeveragedReserveManager
{
	...
	{
		require(_collateralizationMargin <= _collateralizationRatio && _collateralizationRatio.sub(_collateralizationMargin) <= MAXIMUM_COLLATERALIZATION_RATIO, "invalid ratio");
	...
	function _calcCollateralizationRatio(Self storage _self) internal view returns (uint256 _collateralizationRatio, uint256 _minCollateralizationRatio, uint256 _maxCollateralizationRatio)
	{
		...
		_collateralizationRatio = _collateralRatio.mul(_self.collateralizationRatio).mul(1e18);
		_minCollateralizationRatio = _collateralRatio.mul(_self.collateralizationRatio.sub(_self.collateralizationMargin)).mul(1e18);
		_maxCollateralizationRatio = _collateralRatio.mul(_self.collateralizationRatio.add(_self.collateralizationMargin)).mul(1e18);
	...
	function _adjustLeverage(Self storage _self, uint256 _roomAmount) internal returns (bool _success)
	{
		...
		uint256 _reserveAmount = _lendAmount.add(_borrowAmount);
		...
		uint256 _newReserveAmount = _reserveAmount.add(_roomAmount);
		// caculates the assumed lend amount deducting the requested room
		uint256 _oldLendAmount = _lendAmount.add(_roomAmount);
		...
		uint256 _maxNewLendAmount;
		{
			...
			_newLendAmount = _newReserveAmount.mul(1e18).mul(uint256(1e18).add(_collateralizationRatio));
			_minNewLendAmount = _newReserveAmount.mul(1e18).mul(uint256(1e18).sub(_minCollateralizationRatio));
