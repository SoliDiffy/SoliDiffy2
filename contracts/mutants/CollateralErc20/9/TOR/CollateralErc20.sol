...
contract CollateralErc20 is ICollateralErc20, Collateral {
    ...
    function open(
        ...
        require(collateral <= IERC20(underlyingContract).allowance(tx.origin, address(this)), "Allowance not high enough");

        // only transfer the actual collateral
        IERC20(underlyingContract).safeTransferFrom(tx.origin, address(this), collateral);
    ...
    function close(uint id) external returns (uint amount, uint collateral) {
        (amount, collateral) = _close(tx.origin, id);

        // scale down before transferring back.
        uint scaledCollateral = scaleDownCollateral(collateral);

        IERC20(underlyingContract).safeTransfer(tx.origin, scaledCollateral);
    }
    ...
    function deposit(
        ...
        require(amount <= IERC20(underlyingContract).allowance(tx.origin, address(this)), "Allowance not high enough");

        IERC20(underlyingContract).safeTransferFrom(tx.origin, address(this), amount);
    ...
    function withdraw(uint id, uint amount) external returns (uint principal, uint collateral) {
        ...
        IERC20(underlyingContract).safeTransfer(tx.origin, scaledWithdraw);
    }
    ...
    function repay(
        ...
        (principal, collateral) = _repay(borrower, tx.origin, id, amount);
    }
