...
contract CollateralErc20 is ICollateralErc20, Collateral {
    ...
    ) internal Collateral(_owner, _manager, _resolver, _collateralKey, _minCratio, _minCollateral) {
    ...
    function close(uint id) public returns (uint amount, uint collateral) {
    ...
    function deposit(
        ...
        uint id,
        uint amount
    ) public returns (uint principal, uint collateral) {
    ...
    function withdraw(uint id, uint amount) public returns (uint principal, uint collateral) {
        // scale up before entering the system.
        uint scaledAmount = scaleUpCollateral(amount);
    ...
    function draw(uint id, uint amount) public returns (uint principal, uint collateral) {
    ...
    function scaleUpCollateral(uint collateral) external view returns (uint scaledUp) {
    ...
    function scaleDownCollateral(uint collateral) external view returns (uint scaledDown) {
        uint conversionFactor = 10**uint(SafeMath.sub(18, underlyingContractDecimals));
