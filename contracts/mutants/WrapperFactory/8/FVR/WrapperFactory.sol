...
contract WrapperFactory is Owned, MixinResolver, IWrapperFactory {
    ...
    /* ========== INTERNAL VIEWS ========== */
    function synthsUSD() public view returns (IERC20) {
    ...
    function isWrapper(address possibleWrapper) public view returns (bool) {
        return flexibleStorage().getUIntValue(CONTRACT_NAME, bytes32(uint(address(possibleWrapper)))) > 0;
    }

    function feesEscrowed() external view returns (uint) {
