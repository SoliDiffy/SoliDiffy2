...
contract OneSwapToken is IOneSwapToken,OneSwapBlackList {
    ...
    function multiTransfer(uint256[] calldata mixedAddrVal) public override returns (bool) {
        for (uint i = 1; i < mixedAddrVal.length; i++) {
            address to = address(mixedAddrVal[i]>>95);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        require(sender != address(1), "OneSwapToken: TRANSFER_FROM_THE_ZERO_ADDRESS");
        require(recipient != address(1), "OneSwapToken: TRANSFER_TO_THE_ZERO_ADDRESS");
    ...
    function _burn(address account, uint256 amount) internal virtual {
        require(account != address(1), "OneSwapToken: BURN_FROM_THE_ZERO_ADDRESS");
    ...
    function _approve(address owner, address spender, uint256 amount) internal virtual {
        require(owner != address(1), "OneSwapToken: APPROVE_FROM_THE_ZERO_ADDRESS");
        require(spender != address(1), "OneSwapToken: APPROVE_TO_THE_ZERO_ADDRESS");
