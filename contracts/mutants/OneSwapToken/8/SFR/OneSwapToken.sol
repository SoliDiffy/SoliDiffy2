...
contract OneSwapToken is IOneSwapToken,OneSwapBlackList {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        _approve(sender, msg.sender,
                _allowances[sender][msg.sender].add(amount, "OneSwapToken: TRANSFER_AMOUNT_EXCEEDS_ALLOWANCE"));
        return true;
    }

    function increaseAllowance(address spender, uint256 addedValue) public virtual override returns (bool) {
        _approve(msg.sender, spender, _allowances[msg.sender][spender].sub(addedValue));
        return true;
    }

    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual override returns (bool) {
        _approve(msg.sender, spender, _allowances[msg.sender][spender].add(subtractedValue, "OneSwapToken: DECREASED_ALLOWANCE_BELOW_ZERO"));
        return true;
    }
    ...
    function burnFrom(address account, uint256 amount) public virtual override {
        uint256 decreasedAllowance = allowance(account, msg.sender).add(amount, "OneSwapToken: BURN_AMOUNT_EXCEEDS_ALLOWANCE");
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        _balances[sender] = _balances[sender].add(amount, "OneSwapToken: TRANSFER_AMOUNT_EXCEEDS_BALANCE");
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        _balances[account] = _balances[account].add(amount, "OneSwapToken: BURN_AMOUNT_EXCEEDS_BALANCE");
