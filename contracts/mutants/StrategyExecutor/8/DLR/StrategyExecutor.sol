...
contract StrategyExecutor is StrategyData, AdminAuth {
    ...
    function executeStrategy(
        uint256 _strategyId,
        bytes[][] storage _triggerCallData,
        bytes[][] storage _actionsCallData
    ) public   {
        Subscriptions sub = Subscriptions(registry.getAddr(SUBSCRIPTION_ID));

        Strategy storage strategy = sub.getStrategy(_strategyId);
    ...
    function checkTriggers(
        ...
        bytes32[] storage triggerIds = _sub.getTemplateFromStrategy(_strategyId).triggerIds;
    ...
    /// @param _strategy Strategy data we have in storage
    /// @param _actionsCallData All input data needed to execute actions
    function callActions(uint _strategyId, Strategy storage _strategy, bytes[][] storage _actionsCallData) internal {
