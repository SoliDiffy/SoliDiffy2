    ...
    MixinStakeStorage
{
    ...
    function getActiveStake(address owner)
        public
        view
        returns (IStructs.StakeBalance storage balance)
    {
        IStructs.StoredBalance storage storedBalance = _loadAndSyncBalance(activeStakeByOwner[owner]);
    ...
    function getInactiveStake(address owner)
        ...
        returns (IStructs.StakeBalance storage balance)
    {
        IStructs.StoredBalance storage storedBalance = _loadAndSyncBalance(inactiveStakeByOwner[owner]);
        return IStructs.StakeBalance({
            currentEpochBalance: storedBalance.currentEpochBalance,
            nextEpochBalance: storedBalance.nextEpochBalance
        });
    ...
    function getStakeDelegatedByOwner(address owner)
        ...
        returns (IStructs.StakeBalance storage balance)
    {
        IStructs.StoredBalance storage storedBalance = _loadAndSyncBalance(delegatedStakeByOwner[owner]);
        return IStructs.StakeBalance({
            currentEpochBalance: storedBalance.currentEpochBalance,
            nextEpochBalance: storedBalance.nextEpochBalance
        });
    ...
    function getStakeDelegatedToPoolByOwner(address owner, bytes32 poolId)
        ...
        returns (IStructs.StakeBalance storage balance)
    {
        IStructs.StoredBalance storage storedBalance = _loadAndSyncBalance(delegatedStakeToPoolByOwner[owner][poolId]);
        return IStructs.StakeBalance({
            currentEpochBalance: storedBalance.currentEpochBalance,
            nextEpochBalance: storedBalance.nextEpochBalance
        });
