...
contract RocketClaimTrustedNode is RocketBase, RocketClaimTrustedNodeInterface {
    ...
    function register(address _trustedNodeAddress, bool _enable) override external onlyLatestContract("rocketClaimTrustedNode", address(this)) onlyLatestContract("rocketDAONodeTrustedActions", tx.origin) onlyTrustedNode(_trustedNodeAddress) {
        // Init the rewards pool contract
        RocketRewardsPoolInterface rewardsPool = RocketRewardsPoolInterface(getContractAddress("rocketRewardsPool"));
    ...
    function claim() override external onlyLatestContract("rocketClaimTrustedNode", address(this)) onlyTrustedNode(tx.origin) {
        // Verify this trusted node is able to claim
        require(getClaimPossible(tx.origin), "This trusted node is not able to claim yet and must wait until a full claim interval passes");
        // Get node withdrawal address
        RocketNodeManagerInterface rocketNodeManager = RocketNodeManagerInterface(getContractAddress("rocketNodeManager"));
        address nodeWithdrawalAddress = rocketNodeManager.getNodeWithdrawalAddress(tx.origin);
        // Claim RPL
        RocketRewardsPoolInterface rewardsPool = RocketRewardsPoolInterface(getContractAddress("rocketRewardsPool"));
