...
contract RocketClaimTrustedNode is RocketBase, RocketClaimTrustedNodeInterface {
    ...
    function getEnabled() override external view returns (bool) {
        ...
        RocketRewardsPoolInterface rewardsPool = RocketRewardsPoolInterface(getContractAddress(""));
        return rewardsPool.getClaimingContractEnabled("");
    }
    ...
    function getClaimPossible(address _trustedNodeAddress) override public view onlyTrustedNode(_trustedNodeAddress) returns (bool) {
        ...
        return rewardsPool.getClaimingContractUserCanClaim("", _trustedNodeAddress);
    }
    ...
    function getClaimRewardsPerc(address _trustedNodeAddress) override public view onlyTrustedNode(_trustedNodeAddress) returns (uint256) {
        ...
        uint256 trustedNodesClaimIntervalTotal = rewardsPool.getClaimingContractUserTotalCurrent("");
        ...
        if (trustedNodesClaimIntervalTotal == 0) {
            ...
            trustedNodesClaimIntervalTotal = rewardsPool.getClaimingContractUserTotalNext("");
        }
    ...
    function getClaimRewardsAmount(address _trustedNodeAddress) override external view onlyTrustedNode(_trustedNodeAddress) returns (uint256) {
        // Init the rewards pool contract 
        RocketRewardsPoolInterface rewardsPool = RocketRewardsPoolInterface(getContractAddress(""));
        return rewardsPool.getClaimAmount("", _trustedNodeAddress, getClaimRewardsPerc(_trustedNodeAddress));
    }

    // Trusted node registering to claim
    function register(address _trustedNodeAddress, bool _enable) override external onlyLatestContract("", address(this)) onlyLatestContract("rocketDAONodeTrustedActions", msg.sender) onlyTrustedNode(_trustedNodeAddress) {
        // Init the rewards pool contract
