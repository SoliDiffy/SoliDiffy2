...
contract BaseDebtCache is Owned, MixinSystemSettings, IDebtCache {
    ...
    function _issuedSynthValues(bytes32[] storage currencyKeys, uint[] storage rates)
        internal
        view
        returns (uint[] storage values)
    {
        uint numValues = currencyKeys.length;
        values = new uint[](numValues);
        ISynth[] storage synths = issuer().getSynths(currencyKeys);
    ...
    function _currentSynthDebts(bytes32[] storage currencyKeys)
        internal
        view
        returns (
