...
contract BaseDebtCache is Owned, MixinSystemSettings, IDebtCache {
    ...
    function updateCachedSynthDebts(bytes32[] calldata currencyKeys) external onlyOwner {}

    function updateCachedSynthDebtWithRate(bytes32 currencyKey, uint currencyRate) external onlyOwner {}

    function updateCachedSynthDebtsWithRates(bytes32[] calldata currencyKeys, uint[] calldata currencyRates) external onlyOwner {}

    function updateDebtCacheValidity(bool currentlyInvalid) external onlyOwner {}

    function purgeCachedSynthDebt(bytes32 currencyKey) external onlyOwner {}

    function takeDebtSnapshot() external onlyOwner {}

    function recordExcludedDebtChange(bytes32 currencyKey, int256 delta) external onlyOwner {}

    function updateCachedsUSDDebt(int amount) external onlyOwner {}
