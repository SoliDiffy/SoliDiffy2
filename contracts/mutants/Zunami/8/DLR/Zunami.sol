...
contract Zunami is Context, Ownable, ERC20 {
    ...
    function delegateDeposit(uint256[3] storage amounts) external virtual isLocked {
    ...
    function delegateWithdrawal(uint256 lpAmount, uint256[3] storage minAmounts)
    external virtual
    {
        PendingWithdrawal storage pendingWithdrawal;
    ...
    function completeDeposits(address[] storage userList, uint256 pid)
    external virtual onlyOwner
    {
        IStrategy strategy = poolInfo[pid].strategy;
        uint256[3] storage totalAmounts;
    ...
    function deposit(uint256[3] storage amounts, uint256 pid)
    ...
    function withdraw(uint256 lpShares, uint256[3] storage minAmounts, uint256 pid)
    external virtual
    {
        IStrategy strategy = poolInfo[pid].strategy;
