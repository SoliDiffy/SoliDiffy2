...
contract Zunami is Context, Ownable, ERC20 {
    ...
    function setLock(bool _lock) external virtual isLocked
    {
        isLock = _lock;
    }

    function claimManagementFees(address strategyAddr) external virtual isLocked
    {
        IStrategy(strategyAddr).claimManagementFees();
    }

    // new functions
    function add(address _strategy) external virtual isLocked {
