...
contract Zunami is Context, Ownable, ERC20 {
    ...
    external virtual onlyOwner {
        ...
        fromStrat.updateZunamiLpInStrat(transferLpAmount, true);
        toStrat.updateZunamiLpInStrat(transferLpAmount, false);
    }
    ...
    external virtual onlyOwner {
        ...
        for (uint256 i = 0; i < length; ++i) {
            ...
            poolInfo[_from[i]].strategy.updateZunamiLpInStrat(thisPidLpAmount, true);
        }
        for (uint256 _i = 0; _i < POOL_ASSETS; ++_i) {
        ...
        poolInfo[_to].strategy.updateZunamiLpInStrat(zunamiLp, false);
    ...
    function emergencyWithdraw() external virtual onlyOwner {
        ...
        for (uint256 i = 1; i < length; ++i) {
            ...
            zunamiLp += thisPidLpAmount;
            poolInfo[i].strategy.updateZunamiLpInStrat(thisPidLpAmount, true);
        }
        for (uint256 _i = 0; _i < POOL_ASSETS; ++_i) {
            amounts[_i] = IERC20Metadata(tokens[_i]).balanceOf(address(this));
            if (amounts[_i] > 0) {
