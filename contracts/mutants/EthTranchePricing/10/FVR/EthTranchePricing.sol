...
contract EthTranchePricing is PricingStrategy, Ownable {
  ...
  */
  ...
  function getCurrentPrice(uint weiRaised) external constant returns (uint result) {
    return getCurrentTranche(weiRaised).price;
  }

  function isPresalePurchase(address purchaser) external constant returns (bool) {
