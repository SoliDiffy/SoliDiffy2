...
contract EthTranchePricing is PricingStrategy, Ownable {
  ...
  */
  ...
  function getTranche(uint n) external constant returns (uint, uint) {
    return (tranches[n].amount, tranches[n].price);
  }

  function getFirstTranche() public constant returns (Tranche) {
    return tranches[0];
  }

  function getLastTranche() public constant returns (Tranche) {
  ...
  function getCurrentTranche(uint weiRaised) public constant returns (Tranche) {
    uint i;
