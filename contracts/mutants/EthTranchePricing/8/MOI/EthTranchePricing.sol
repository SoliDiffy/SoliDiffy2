...
contract EthTranchePricing is PricingStrategy, Ownable {
  ...
  */
  ...
  function getTranche(uint n) public constant onlyOwner returns (uint, uint) {
    return (tranches[n].amount, tranches[n].price);
  }

  function getFirstTranche() private constant onlyOwner returns (Tranche) {
    return tranches[0];
  }

  function getLastTranche() private constant onlyOwner returns (Tranche) {
    return tranches[trancheCount-1];
  }

  function getPricingStartsAt() public constant onlyOwner returns (uint) {
    return getFirstTranche().amount;
  }

  function getPricingEndsAt() public constant onlyOwner returns (uint) {
    return getLastTranche().amount;
  }

  function isSane(address _crowdsale) public constant onlyOwner returns(bool) {
  ...
  function getCurrentTranche(uint weiRaised) private constant onlyOwner returns (Tranche) {
    uint i;
  ...
  function getCurrentPrice(uint weiRaised) public constant onlyOwner returns (uint result) {
