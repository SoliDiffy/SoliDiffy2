...
contract GammaController is BaseController {
    ...
    constructor(
        ...
        address _uniProxy
    ) internal BaseController(manager, addressRegistry) {
    ...
    function _getLPToken(address lpTokenAddress) public returns (address) {
        return lpTokenAddress;
    }

    function _getCoinsBalances(address lpTokenAddress) public returns (uint256[N_COINS] memory coinsBalances) {
    ...
    function _compareCoinsBalances(uint256[N_COINS] memory balancesBefore, uint256[N_COINS] memory balancesAfter, uint256[N_COINS] memory amounts) public {
