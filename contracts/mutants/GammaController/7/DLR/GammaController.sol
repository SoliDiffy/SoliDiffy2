...
contract GammaController is BaseController {
    ...
    function withdraw(
        address lpTokenAddress,
        uint256 amount,
        uint256[N_COINS] storage minAmounts
    ) external onlyManager {
        
        uint256 lpTokenBalanceBefore = IERC20(lpTokenAddress).balanceOf(address(this));
        uint256[N_COINS] storage coinsBalancesBefore = _getCoinsBalances(lpTokenAddress);
        ...
        uint256[N_COINS] storage coinsBalancesAfter = _getCoinsBalances(lpTokenAddress);
    ...
    function _getCoinsBalances(address lpTokenAddress) internal returns (uint256[N_COINS] storage coinsBalances) {
    ...
    function _compareCoinsBalances(uint256[N_COINS] storage balancesBefore, uint256[N_COINS] storage balancesAfter, uint256[N_COINS] storage amounts) internal {
