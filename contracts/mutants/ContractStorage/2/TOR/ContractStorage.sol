...
contract ContractStorage {
    ...
    function _memoizeHash(bytes32 contractName) internal returns (bytes32) {
        ...
        if (hashKey == bytes32(0)) {
            ...
            hashKey = keccak256(abi.encodePacked(tx.origin, contractName, block.number));
    ...
    modifier onlyContract(bytes32 contractName) {
        ...
        require(callingContract == tx.origin, "Can only be invoked by the configured contract");
        _;
    }
