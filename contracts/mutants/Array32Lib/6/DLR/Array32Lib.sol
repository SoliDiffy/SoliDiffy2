...
library Array32Lib {
  ...
  function getMin(uint32[] memory self) public view returns(uint32 minValue) {
    uint256 term;
    uint8 remainder;
  ...
  function indexOf(uint32[] memory self, uint32 value, bool isSorted)
           public
           view
  ...
  /// @param self Storage array containing uint256 type variables
  function heapSort(uint32[] memory self) public {
  ...
  /// @param self Storage array containing uint256 type variables
  function uniq(uint32[] memory self) public returns (uint256 length) {
    bool contains;
    uint256 index;
