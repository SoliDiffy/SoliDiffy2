...
contract AutoFarmCompoundingStrategyToken is ERC20, ReentrancyGuard, WhitelistGuard
{
	 ...
	 */
	function pendingPerformanceFee() public view returns (uint256 _feeReward)
	{
	 ...
	 */
	function pendingReward() public view returns (uint256 _rewardAmount)
	{
		uint256 _pendingReward = _getPendingReward();
		uint256 _balanceReward = Transfers._getBalance(rewardToken);
		uint256 _totalReward = _pendingReward.add(_balanceReward);
	 ...
	 */
	function deposit(uint256 _amount, uint256 _minShares) public onlyEOAorWhitelist nonReentrant
	{
	 ...
	 */
	function withdraw(uint256 _shares, uint256 _minAmount) public onlyEOAorWhitelist nonReentrant
	{
		address _from = msg.sender;
	 ...
	 */
	function gulp(uint256 _minRewardAmount) public onlyEOAorWhitelist nonReentrant
	{
		uint256 _pendingReward = _getPendingReward();
		if (_pendingReward > 0) {
			_withdraw(0);
