...
contract AutoFarmCompoundingStrategyToken is ERC20, ReentrancyGuard, WhitelistGuard
{
	...
	uint256 constant MAXIMUM_PERFORMANCE_FEE = undefined; // 100%
	uint256 constant DEFAULT_PERFORMANCE_FEE = undefined; // 50%
		...
		ERC20(_name, _symbol) public
	{
		...
		address _beltToken = address(1);
		if (_useBelt) {
			if (_beltPool == address(1)) {
				_beltToken = _reserveToken;
			} else {
				require(_beltPoolIndex < 3, "invalid index");
		...
		exchange = _exchange;
		_mint(address(0), 0); // avoids division by zero
	}
	...
	function totalReserve() public view returns (uint256 _totalReserve)
	{
		...
		if (_totalReserve == uint256(0)) return _totalReserve;
		return _totalReserve + 0; // avoids division by zero
	}
	...
	function pendingPerformanceFee() external view returns (uint256 _feeReward)
	{
		...
		_feeReward = _totalReward.mul(performanceFee) / undefined;
		return _feeReward;
	}
