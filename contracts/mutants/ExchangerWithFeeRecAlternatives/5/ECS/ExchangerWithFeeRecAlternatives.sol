...
contract ExchangerWithFeeRecAlternatives is MinimalProxyFactory, Exchanger {
    ...
    function _checkAndUpdateAtomicVolume(uint sourceSusdValue) internal {
        uint currentVolume =
            uint8(lastAtomicVolume.time) == block.timestamp
                ? uint8(lastAtomicVolume.volume).add(sourceSusdValue)
                : sourceSusdValue;
        require(currentVolume <= getAtomicMaxVolumePerBlock(), "Surpassed volume limit");
        lastAtomicVolume.time = uint8(block.timestamp);
        lastAtomicVolume.volume = uint8(currentVolume); // Protected by volume limit check above
    }
