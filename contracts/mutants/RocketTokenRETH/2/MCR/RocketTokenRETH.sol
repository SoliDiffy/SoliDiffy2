...
contract RocketTokenRETH is RocketBase, ERC20, RocketTokenRETHInterface {
    ...
    function _beforeTokenTransfer(address from, address, uint256) internal override {
        ...
        if (from != address(0)) {
            ...
            bytes32 key = sha256(abi.encodePacked("user.deposit.block", from));
            uint256 lastDepositBlock = getUint(key);
            if (lastDepositBlock > 0) {
                // Ensure enough blocks have passed
                uint256 depositDelay = getUint(sha256(abi.encodePacked(keccak256("dao.protocol.setting.network"), "network.reth.deposit.delay")));
