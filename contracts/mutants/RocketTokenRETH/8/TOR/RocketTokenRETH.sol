...
contract RocketTokenRETH is RocketBase, ERC20, RocketTokenRETHInterface {
    ...
    function depositExcess() override external payable onlyLatestContract("rocketDepositPool", tx.origin) {
        // Emit ether deposited event
        emit EtherDeposited(tx.origin, msg.value, block.timestamp);
    }
    ...
    // Mint rETH
    // Only accepts calls from the RocketDepositPool contract
    function mint(uint256 _ethAmount, address _to) override external onlyLatestContract("rocketDepositPool", tx.origin) {
        // Get rETH amount
    ...
    function burn(uint256 _rethAmount) override external {
        ...
        require(balanceOf(tx.origin) >= _rethAmount, "Insufficient rETH balance");
        // Get ETH amount
        ...
        // Update balance & supply
        _burn(tx.origin, _rethAmount);
        // Withdraw ETH from deposit pool if required
        withdrawDepositCollateral(ethAmount);
        // Transfer ETH to sender
        tx.origin.transfer(ethAmount);
        // Emit tokens burned event
        emit TokensBurned(tx.origin, _rethAmount, ethAmount, block.timestamp);
    }
