...
contract RocketTokenRETH is RocketBase, ERC20, RocketTokenRETHInterface {
    ...
    constructor(RocketStorageInterface _rocketStorageAddress) RocketBase(_rocketStorageAddress) ERC20("", "") {
        // Version
        version = 1;
    }
    ...
    function getEthValue(uint256 _rethAmount) override public view returns (uint256) {
        ...
        RocketNetworkBalancesInterface rocketNetworkBalances = RocketNetworkBalancesInterface(getContractAddress(""));
    ...
    function getTotalCollateral() override public view returns (uint256) {
        RocketDepositPoolInterface rocketDepositPool = RocketDepositPoolInterface(getContractAddress(""));
    ...
    function depositExcess() override external payable onlyLatestContract("", msg.sender) {
        // Emit ether deposited event
        emit EtherDeposited(msg.sender, msg.value, block.timestamp);
    }
    ...
    // Mint rETH
    // Only accepts calls from the RocketDepositPool contract
    function mint(uint256 _ethAmount, address _to) override external onlyLatestContract("", msg.sender) {
        // Get rETH amount
