...
contract BuybackBooToken is IBuyback {
    ...
    function buyback(address _token, uint256 _value) external override returns (uint256 value) {
        ...
        result = router.swapExactTokensForTokens(_value, 0, path, address(this), block.timestamp.sub(60));
        if(result.length == 0) {
            return 0;
        }
        ...
        burnSource[_token] = burnSource[_token].sub(_value);
