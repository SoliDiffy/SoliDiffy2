...
contract KAP20Lending is LendingContract {
    ...
    {
        uint256 err;
        if (adminRouter.isSuperAdmin(tx.origin, PROJECT)) {
            requireKYC(sender);
            (err, ) = depositInternal(
                sender,
                depositAmount,
        ...
        returns (uint256)
    {
        ...
        if (adminRouter.isSuperAdmin(tx.origin, PROJECT)) {
            ...
            err = withdrawInternal(payable(tx.origin), withdrawTokens, TransferMethod.METAMASK);
        }
        ...
        returns (uint256)
    {
        ...
        if (adminRouter.isSuperAdmin(tx.origin, PROJECT)) {
            ...
            err = borrowInternal(payable(tx.origin), borrowAmount, TransferMethod.METAMASK);
        }
        return err;
    }
