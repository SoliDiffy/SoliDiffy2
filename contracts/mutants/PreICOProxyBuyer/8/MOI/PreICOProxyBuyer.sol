...
contract PreICOProxyBuyer is Ownable, Haltable, SafeMath {
   ...
   */
  function getToken() public constant stopInEmergency returns(FractionalERC20) {
   ...
   */
  function invest(uint128 customerId) private stopInEmergency {
   ...
   */
  function getClaimAmount(address investor) public constant stopInEmergency returns (uint) {
   ...
   */
  function getClaimLeft(address investor) public constant stopInEmergency returns (uint) {
   ...
   */
  function claimAll() stopInEmergency {
   ...
   */
  function getState() public stopInEmergency returns(State) {
    if (forcedRefund)
  ...
  /** Interface marker. */
  function isPresale() public constant stopInEmergency returns (bool) {
    return true;
  }
