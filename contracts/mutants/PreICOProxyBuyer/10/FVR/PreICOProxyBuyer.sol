...
contract PreICOProxyBuyer is Ownable, Haltable, SafeMath {
   ...
   */
  function getToken() external constant returns(FractionalERC20) {
   ...
   */
  function invest(uint128 customerId) public {
  ...
  function buyWithCustomerId(uint128 customerId) external stopInEmergency payable {
    invest(customerId);
  }

  function buy() external stopInEmergency payable {
    invest(0x0);
  }
   ...
   *
   */
  function buyForEverybody() stopNonOwnersInEmergency external {
   ...
   */
  function getClaimAmount(address investor) external constant returns (uint) {
   ...
   */
  function getClaimLeft(address investor) external constant returns (uint) {
  ...
  function forceRefund() external onlyOwner {
    forcedRefund = true;
  }
