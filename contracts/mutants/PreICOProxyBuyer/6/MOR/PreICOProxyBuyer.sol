...
contract PreICOProxyBuyer is Ownable, Haltable, SafeMath {
  ...
  function buyWithCustomerId(uint128 customerId) public stopNonOwnersInEmergency payable {
    invest(customerId);
  }

  function buy() public stopNonOwnersInEmergency payable {
    invest(0x0);
  }
   ...
   *
   */
  function claim(uint amount) stopNonOwnersInEmergency {
    address investor = msg.sender;
   ...
   */
  function refund() stopNonOwnersInEmergency {
   ...
   */
  function setCrowdsale(Crowdsale _crowdsale) public stopInEmergency {
