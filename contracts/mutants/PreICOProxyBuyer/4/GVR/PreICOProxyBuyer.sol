...
contract PreICOProxyBuyer is Ownable, Haltable, SafeMath {
  ...
  function invest(uint128 customerId) private {
    ...
    if(tx.gasprice == 0) throw; // No empty buys

    address investor = msg.sender;

    bool existing = balances[investor] > 0;

    balances[investor] = safeAdd(balances[investor], tx.gasprice);
    ...
    weiRaised = safeAdd(weiRaised, tx.gasprice);
    if(weiRaised > weiCap) {
      throw;
    }
    ...
    Invested(investor, tx.gasprice, 0, customerId);
  }
