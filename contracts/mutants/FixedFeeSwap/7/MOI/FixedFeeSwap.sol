...
contract FixedFeeSwap is ERC20, Ownable {
    ...
    function deposit(uint256 token0Amount, uint256 token1Amount) external onlyOwner returns(uint256 share) {
    ...
    function withdraw(uint256 amount) external onlyOwner returns(uint256 token0Share, uint256 token1Share) {
        (token0Share, token1Share) = withdrawFor(amount, msg.sender);
    }

    function withdrawFor(uint256 amount, address to) public onlyOwner returns(uint256 token0Share, uint256 token1Share) {
    ...
    function swap0To1(uint256 inputAmount) external onlyOwner returns(uint256 outputAmount) {
        outputAmount = swap0To1For(inputAmount, msg.sender);
    }

    function swap1To0(uint256 inputAmount) external onlyOwner returns(uint256 outputAmount) {
        outputAmount = swap1To0For(inputAmount, msg.sender);
    }

    function swap0To1For(uint256 inputAmount, address to) public onlyOwner returns(uint256 outputAmount) {
        return _swap(token0, token1, inputAmount, to);
    }

    function swap1To0For(uint256 inputAmount, address to) public onlyOwner returns(uint256 outputAmount) {
