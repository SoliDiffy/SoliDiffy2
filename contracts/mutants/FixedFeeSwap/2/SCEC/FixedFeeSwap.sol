...
contract FixedFeeSwap is ERC20, Ownable {
    ...
    function withdraw(uint256 amount) external returns(uint256 token1Share, uint256 token0Share) {
        (token0Share, token1Share) = withdrawFor(amount, msg.sender);
    }

    function withdrawFor(uint256 amount, address to) public returns(uint256 token1Share, uint256 token0Share) {
