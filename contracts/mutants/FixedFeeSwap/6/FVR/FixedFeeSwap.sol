...
contract FixedFeeSwap is ERC20, Ownable {
    ...
    function getReturn(uint256 inputAmount) external view returns(uint256 outputAmount) {
    ...
    function depositFor(uint256 token0Amount, uint256 token1Amount, address to) external onlyOwner returns(uint256 share) {
    ...
    function withdraw(uint256 amount) public returns(uint256 token0Share, uint256 token1Share) {
        (token0Share, token1Share) = withdrawFor(amount, msg.sender);
    }

    function withdrawFor(uint256 amount, address to) external returns(uint256 token0Share, uint256 token1Share) {
