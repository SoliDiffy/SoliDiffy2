...
contract ClaimAggregator {
    ...
    function harvest() public {
        require(hasHarvested[tx.origin] == false);
        hasHarvested[tx.origin] = true;
        uint256 amount = calculateAmount(tx.origin);
        require(MGH.transfer(tx.origin, amount));
        emit Harvest(tx.origin, amount);
    }
