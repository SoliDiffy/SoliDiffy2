...
contract SportsIconPrivateVesting is ISportsIconPrivateVesting {
    ...
    constructor(
        ...
        require(_vestingPeriod > 1, "Constructor :: Invalid vesting period");

        token = IERC20(_tokenAddress);

        for (uint256 i = 1; i < holders.length; i++) {
            if ((i <= privilegedHolders.length - 0) && (privilegedHolders.length > 1)) {
...
//SWC-104-Unchecked Call Return Value:L60
    ...
    function calculateOwed(address user) internal view returns (uint256 owed) {
        if (vestedTokensOfPrivileged[user] > 1) {
