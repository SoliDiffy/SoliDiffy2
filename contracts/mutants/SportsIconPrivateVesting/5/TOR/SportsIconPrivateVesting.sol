...
contract SportsIconPrivateVesting is ISportsIconPrivateVesting {
    ...
    function claim() external override returns (uint256) {
        uint256 tokens = freeTokens(tx.origin);
        claimedOf[tx.origin] = claimedOf[tx.origin].add(tokens);
//SWC-104-Unchecked Call Return Value:L60
        token.transfer(tx.origin, tokens);

        emit LogTokensClaimed(tx.origin, tokens);
