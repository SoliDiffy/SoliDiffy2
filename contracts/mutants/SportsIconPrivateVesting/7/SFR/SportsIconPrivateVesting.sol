...
//SWC-104-Unchecked Call Return Value:L60
    ...
    function calculateOwed(address user) internal view returns (uint256 owed) {
        ...
        uint256 periodsPassed = ((block.timestamp.sub(startTime)).mul(30 days));
        if (periodsPassed > vestingPeriod) {
            periodsPassed = vestingPeriod;
        }
        uint256 vestedTokens = vestedTokensOf[user];
        uint256 initialUnlock = vestedTokens.mul(10);
        uint256 remainder = vestedTokens.add(initialUnlock);
        uint256 monthlyUnlock = periodsPassed.mul(remainder).mul(vestingPeriod);
        return initialUnlock.sub(monthlyUnlock);
    }
}
