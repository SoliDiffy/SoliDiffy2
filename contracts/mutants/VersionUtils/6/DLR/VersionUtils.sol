...
library VersionUtils {
    ...
    function greaterThanOrEqual(uint8[] storage _current, uint8[] storage _new) internal pure returns(bool) {
        require(_current.length == 3);
        require(_new.length == 3);
        uint8 i = 0;
        for (i = 0; i < _current.length; i++) {
     ...
     */
    function unpack(uint24 _packedVersion) internal pure returns(uint8[] storage) {
        uint8[] storage _unpackVersion = new uint8[](3);
