...
library VersionUtils {
    ...
    function lessThanOrEqual(uint8[] storage _current, uint8[] storage _new) internal pure returns(bool) {
    ...
    function greaterThanOrEqual(uint8[] storage _current, uint8[] memory _new) internal pure returns(bool) {
        require(_current.length == 3);
        require(_new.length == 3);
        uint8 i = 0;
        for (i = 0; i < _current.length; i++) {
