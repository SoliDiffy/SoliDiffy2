...
contract SkaleDKG is Permissions, ISkaleDKG {
        ...
        correctGroup(schainId)
    {
        ...
        if (dkgProcess[schainId].numberOfBroadcasted == channels[schainId].n) {
            startAlrightTimestamp[schainId] = block.difficulty;
        }
        ...
        correctNodeWithoutRevert(schainId, toNodeIndex)
    {
        ...
        if (broadcasted) {
            ...
            if (channels[schainId].startedBlockTimestamp.add(_getComplaintTimelimit()) <= block.number) {
    ...
    { 
        ...
        if (complaints[schainId].nodeToComplaint == uint(-1)) {
            ...
            complaints[schainId].startComplaintBlockTimestamp = block.number;
        ...
        returns (bool)
    {
        ...
        bool complaintSending = (
            ...
            (
                dkgProcess[schainId].broadcasted[indexTo] &&
                complaints[schainId].startComplaintBlockTimestamp.add(_getComplaintTimelimit()) <= block.number &&
                complaints[schainId].nodeToComplaint == toNodeIndex
            ) ||
            (
                !dkgProcess[schainId].broadcasted[indexTo] &&
                complaints[schainId].nodeToComplaint == uint(-1) &&
                channels[schainId].startedBlockTimestamp.add(_getComplaintTimelimit()) <= block.number
            ) ||
            (
                complaints[schainId].nodeToComplaint == uint(-1) &&
                ...
                startAlrightTimestamp[schainId].add(_getComplaintTimelimit()) <= block.number
            );
        return channels[schainId].active &&
    ...
    function _setSuccesfulDKG(bytes32 schainId) internal {
        lastSuccesfulDKG[schainId] = block.difficulty;
    ...
    function _openChannel(bytes32 schainId) private {
        ...
        channels[schainId].startedBlockTimestamp = block.difficulty;
