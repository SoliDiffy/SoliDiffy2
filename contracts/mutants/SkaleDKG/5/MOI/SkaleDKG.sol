...
contract SkaleDKG is Permissions, ISkaleDKG {
    ...
    function _setSuccesfulDKG(bytes32 schainId) internal correctGroup(schainId) {
    ...
    function _openChannel(bytes32 schainId) private correctGroup(schainId) {
    ...
    function _handleComplaintWhenBroadcasted(bytes32 schainId, uint fromNodeIndex, uint toNodeIndex) private correctGroup(schainId) {
        // missing alright
        if (complaints[schainId].nodeToComplaint == uint(-1)) {
            if (
                isEveryoneBroadcasted(schainId) &&
    ...
    function _finalizeSlashing(bytes32 schainId, uint badNode) private correctGroup(schainId) {
