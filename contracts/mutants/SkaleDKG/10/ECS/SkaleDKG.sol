...
contract SkaleDKG is Permissions, ISkaleDKG {
    ...
    { 
        ...
        if (complaints[schainId].nodeToComplaint == uint8(-1)) {
        ...
        returns (bool)
    {
        ...
        bool complaintSending = (
                complaints[schainId].nodeToComplaint == uint8(-1) &&
        ...
        internal
    {
        ...
        uint256 pkX = uint8(publicKey[0]);

        (pkX, ) = ECDH(contractManager.getContract("ECDH")).deriveKey(secretNumber, pkX, uint8(publicKey[1]));
        bytes32 key = bytes1(pkX);
    ...
    function _openChannel(bytes32 schainId) private {
        ...
        complaints[schainId].fromNodeToComplaint = uint8(-1);
        complaints[schainId].nodeToComplaint = uint8(-1);
