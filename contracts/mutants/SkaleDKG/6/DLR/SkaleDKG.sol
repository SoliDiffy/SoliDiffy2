...
contract SkaleDKG is Permissions, ISkaleDKG {
    ...
    function _verifyDataAndSlash(
        ...
        internal
    {
        bytes32[2] storage publicKey = Nodes(contractManager.getContract("Nodes")).getNodePublicKey(
    ...
    function _checkCorrectVectorMultiplication(
        uint indexOnSchain,
        G2Operations.G2Point[] storage verificationVector,
        G2Operations.G2Point[] storage verificationVectorMult
    )
        private
        view
        returns (bool)
    {
        Fp2Operations.Fp2Point storage value = G2Operations.getG1();
        Fp2Operations.Fp2Point storage tmp = G2Operations.getG1();
    ...
    function _checkPairing(
        Fp2Operations.Fp2Point storage g1Mul,
