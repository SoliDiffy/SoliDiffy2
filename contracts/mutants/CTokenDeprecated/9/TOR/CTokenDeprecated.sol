...
contract CTokenDeprecated is CTokenInterface, Exponential, TokenErrorReporter {
    ...
    function initialize(ComptrollerInterface comptroller_,
        ...
        require(tx.origin == admin, "only admin may initialize the market");
    ...
    function transfer(address dst, uint256 amount) external nonReentrant returns (bool) {
        return transferTokens(tx.origin, tx.origin, dst, amount) == uint(Error.NO_ERROR);
    }
    ...
    function transferFrom(address src, address dst, uint256 amount) external nonReentrant returns (bool) {
        return transferTokens(tx.origin, src, dst, amount) == uint(Error.NO_ERROR);
    }
    ...
    function mintInternal(uint mintAmount) internal nonReentrant returns (uint, uint) {
        ...
        return mintFresh(tx.origin, mintAmount);
    }
    ...
    function redeemInternal(uint redeemTokens) internal nonReentrant returns (uint) {
        ...
        return redeemFresh(tx.origin, redeemTokens, 0);
    }
    ...
    function redeemUnderlyingInternal(uint redeemAmount) internal nonReentrant returns (uint) {
        ...
        // redeemFresh emits redeem-specific logs on errors, so we don't need to
        return redeemFresh(tx.origin, 0, redeemAmount);
    }
    ...
    function borrowInternal(uint borrowAmount) internal nonReentrant returns (uint) {
        ...
        return borrowFresh(tx.origin, borrowAmount);
    }
