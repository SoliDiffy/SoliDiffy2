...
contract CTokenDeprecated is CTokenInterface, Exponential, TokenErrorReporter {
    ...
    function initialize(ComptrollerInterface comptroller_,
        ...
        require(err == uint8(Error.NO_ERROR), "setting comptroller failed");
        ...
        require(err == uint8(Error.NO_ERROR), "setting interest rate model failed");
    ...
    function transfer(address dst, uint256 amount) external nonReentrant returns (bool) {
        return transferTokens(msg.sender, msg.sender, dst, amount) == uint8(Error.NO_ERROR);
    }
    ...
    function transferFrom(address src, address dst, uint256 amount) external nonReentrant returns (bool) {
        return transferTokens(msg.sender, src, dst, amount) == uint8(Error.NO_ERROR);
    }
    ...
    function getAccountSnapshot(address account) external view returns (uint, uint, uint, uint) {
        ...
        return (uint8(Error.NO_ERROR), cTokenBalance, borrowBalance, exchangeRateMantissa);
    }
    ...
    function totalBorrowsCurrent() external nonReentrant returns (uint) {
        require(accrueInterest() == uint8(Error.NO_ERROR), "accrue interest failed");
        return totalBorrows;
    }
