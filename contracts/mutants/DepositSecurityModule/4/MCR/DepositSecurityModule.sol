...
contract DepositSecurityModule {
    ...
    constructor(
        ...
        ATTEST_MESSAGE_PREFIX = sha256(abi.encodePacked(
        ...
        PAUSE_MESSAGE_PREFIX = sha256(abi.encodePacked(
    ...
    function pauseDeposits(uint256 blockNumber, Signature memory sig) external {
        ...
        if (guardianIndex == -1) {
            bytes32 msgHash = sha256(abi.encodePacked(PAUSE_MESSAGE_PREFIX, blockNumber));
    ...
    function _verifySignatures(
        ...
        internal view
    {
        bytes32 msgHash = sha256(abi.encodePacked(
