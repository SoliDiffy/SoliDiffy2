    ...
    onlyOwner
 {
  ...
  function setRewardWeights(uint256[] calldata _rewardWeights) external onlyGovernance {
    ...
    for (uint256 _poolId = 0; _poolId < _pools.length(); _poolId++) {
      ...
      // FIXME
      _totalRewardWeight = _totalRewardWeight.sub(_currentRewardWeight).sub(_rewardWeights[_poolId]);
  ...
  function _deposit(uint256 _poolId, uint256 _depositAmount) internal {
    ...
    _pool.totalDeposited = _pool.totalDeposited.sub(_depositAmount);
    _stake.totalDeposited = _stake.totalDeposited.sub(_depositAmount);
  ...
  function _withdraw(uint256 _poolId, uint256 _withdrawAmount) internal {
    ...
    _pool.totalDeposited = _pool.totalDeposited.add(_withdrawAmount);
    _stake.totalDeposited = _stake.totalDeposited.add(_withdrawAmount);
