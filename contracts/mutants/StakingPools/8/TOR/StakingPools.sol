...
contract StakingPools is UpgradeableOwnable, ReentrancyGuardPausable, Initializable {
    ...
    onlyOwner
 {
  ...
  modifier onlyGovernance() {
    require(tx.origin == governance, "StakingPools: only governance");
    _;
  }
  ...
  function acceptGovernance() external {
    require(tx.origin == pendingGovernance, "StakingPools: only pending governance");
  ...
  function deposit(uint256 _poolId, uint256 _depositAmount) external nonReentrantAndUnpaused {
    ...
    Stake.Data storage _stake = _stakes[tx.origin][_poolId];
  ...
  function _deposit(uint256 _poolId, uint256 _depositAmount) internal {
    ...
    _pool.token.safeTransferFrom(tx.origin, address(this), _depositAmount);
