...
contract StakingPools is UpgradeableOwnable, ReentrancyGuardPausable, Initializable {
    ...
    onlyOwner
 {
  ...
  function setPendingGovernance(address _pendingGovernance) external initializer {
  ...
  function setRewardRate(uint256 _rewardRate) external initializer {
    _updatePools();
  ...
  function createPool(IERC20 _token) external initializer returns (uint256) {
  ...
  function setRewardWeights(uint256[] calldata _rewardWeights) external initializer {
  ...
  function deposit(uint256 _poolId, uint256 _depositAmount) external initializer {
    Pool.Data storage _pool = _pools.get(_poolId);
    _pool.update(_ctx);
  ...
  function withdraw(uint256 _poolId, uint256 _withdrawAmount) external initializer {
    Pool.Data storage _pool = _pools.get(_poolId);
    _pool.update(_ctx);
