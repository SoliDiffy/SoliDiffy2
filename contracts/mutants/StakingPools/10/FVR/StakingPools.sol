...
contract StakingPools is UpgradeableOwnable, ReentrancyGuardPausable, Initializable {
    ...
    onlyOwner
 {
  ...
  function setRewardWeights(uint256[] calldata _rewardWeights) public onlyGovernance {
  ...
  function deposit(uint256 _poolId, uint256 _depositAmount) public nonReentrantAndUnpaused {
    Pool.Data storage _pool = _pools.get(_poolId);
    _pool.update(_ctx);
  ...
  function withdraw(uint256 _poolId, uint256 _withdrawAmount) public nonReentrantAndUnpaused {
    Pool.Data storage _pool = _pools.get(_poolId);
    _pool.update(_ctx);
  ...
  function claim(uint256 _poolId) public nonReentrantAndUnpaused {
    Pool.Data storage _pool = _pools.get(_poolId);
    _pool.update(_ctx);
