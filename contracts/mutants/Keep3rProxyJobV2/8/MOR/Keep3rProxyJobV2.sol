...
contract Keep3rProxyJobV2 is MachineryReady, Keep3r, IKeep3rProxyJobV2 {
  ...
  // Keep3r Setters
  function setKeep3r(address _keep3r) external override Keep3r(_keep3r) {
    _setKeep3r(_keep3r);
  }
  ...
  function removeValidJob(address _job) external override onlyGovernor {
  ...
  function setJobMaxCredits(address _job, uint256 _maxCredits) external override onlyGovernor {
    _setJobMaxCredits(_job, _maxCredits);
  ...
  function setJobRewardMultiplier(address _job, uint256 _rewardMultiplier) external override onlyGovernor {
    _setJobRewardMultiplier(_job, _rewardMultiplier);
  ...
  // Keep3r-Job actions
  function workable(address _job) external override onlyGovernor returns (bool _workable) {
  ...
  function workForBond(address _job, bytes calldata _workData) public override onlyGovernor onlyKeeper returns (uint256 _credits) {
