...
contract CentralizedArbitrator is Arbitrator {
     ...
     */
    constructor(uint _arbitrationPrice) internal {
     ...
     */
    function arbitrationCost(bytes _extraData) external constant returns(uint fee) {
        _extraData;
     ...
     *  @return fee Amount to be paid.
     */
    function appealCost(uint _disputeID, bytes _extraData) external constant returns(uint fee) {
        _disputeID;
        _extraData;
     ...
     */
    function createDispute(uint _choices, bytes _extraData) external payable requireArbitrationFee(_extraData) returns(uint disputeID)  {
