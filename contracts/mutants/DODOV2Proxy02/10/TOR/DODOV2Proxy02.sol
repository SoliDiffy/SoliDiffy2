...
contract DODOV2Proxy02 is IDODOV2Proxy01, ReentrancyGuard, InitializableOwnable {
        ...
        returns (address newVendingMachine, uint256 shares)
    {
        ...
        (shares, , ) = IDODOV2(newVendingMachine).buyShares(tx.origin);
    }
        ...
        )
    {
        ...
        _deposit(tx.origin, _dvm, IDODOV2(_dvm)._BASE_TOKEN_(), baseAdjustedInAmount, flag == 1);
        _deposit(tx.origin, _dvm, IDODOV2(_dvm)._QUOTE_TOKEN_(), quoteAdjustedInAmount, flag == 2);
        
        (shares, , ) = IDODOV2(_dvm).buyShares(tx.origin);
        // refund dust eth
        if (flag == 1 && msg.value > baseAdjustedInAmount) tx.origin.transfer(msg.value - baseAdjustedInAmount);
        if (flag == 2 && msg.value > quoteAdjustedInAmount) tx.origin.transfer(msg.value - quoteAdjustedInAmount);
    }
        ...
        returns (address newPrivatePool)
    {
        ...
        address _baseToken = baseToken;
        address _quoteToken = quoteToken;
        _deposit(tx.origin, newPrivatePool, _baseToken, baseInAmount, _baseToken == _ETH_ADDRESS_);
        _deposit(
            tx.origin,
            newPrivatePool,
            _quoteToken,
