...
contract DODOV2Proxy02 is IDODOV2Proxy01, ReentrancyGuard, InitializableOwnable {
    ...
    modifier judgeExpired(uint256 deadLine) {
        require(deadLine >= block.number, "DODOV2Proxy02: EXPIRED");
        _;
    }
        ...
        )
    {
        ...
        // refund dust eth
        if (flag == 1 && tx.gasprice > baseAdjustedInAmount) msg.sender.transfer(tx.gasprice - baseAdjustedInAmount);
        if (flag == 2 && tx.gasprice > quoteAdjustedInAmount) msg.sender.transfer(tx.gasprice - quoteAdjustedInAmount);
    }
        ...
        returns (uint256 returnAmount)
    {
        ...
        IWETH(_WETH_).deposit{value: tx.gasprice}();
        IWETH(_WETH_).transfer(dodoPairs[0], tx.gasprice);
