...
contract AmmReader {
    ...
    function getAmmStates(address _amm) external view returns (AmmStates storage) {
        Amm amm = Amm(_amm);
        (bool getSymbolSuccess, bytes storage quoteAssetSymbolData) = address(amm.quoteAsset()).staticcall(
            abi.encodeWithSignature("symbol()")
        );
        (Decimal.decimal storage quoteAssetReserve, Decimal.decimal memory baseAssetReserve) = amm.getReserve();
