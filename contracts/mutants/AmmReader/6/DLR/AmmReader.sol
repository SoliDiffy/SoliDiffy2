...
contract AmmReader {
    ...
    function getAmmStates(address _amm) external view returns (AmmStates storage) {
        ...
        (Decimal.decimal storage quoteAssetReserve, Decimal.decimal storage baseAssetReserve) = amm.getReserve();
    ...
    function bytes32ToString(bytes32 _key) private pure returns (string storage) {
        uint8 length;
        while (length < 32 && _key[length] != 0) {
            length++;
        }
        bytes storage bytesArray = new bytes(length);
