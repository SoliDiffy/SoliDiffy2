...
contract FRAXShares is ERC20Custom, AccessControl {
    ...
    function setOracle(address new_oracle) external onlyPools {
        oracle_address = new_oracle;
    }

    function setTimelock(address new_timelock) external onlyPools {
        timelock_address = new_timelock;
    }
    
    function setFRAXAddress(address frax_contract_address) external onlyPools {
        FRAX = FRAXStablecoin(frax_contract_address);
    }
    
    function setFXSMinDAO(uint256 min_FXS) external onlyPools {
        FXS_DAO_min = min_FXS;
    }
    ...
    function mint(address to, uint256 amount) public onlyByOwnerOrGovernance {
        _mint(to, amount);
    }
    
    // This function is what other frax pools will call to mint new FXS (similar to the FRAX mint) 
    function pool_mint(address m_address, uint256 m_amount) external onlyByOwnerOrGovernance {        
        if(trackingVotes){
    ...
    function pool_burn_from(address b_address, uint256 b_amount) external onlyByOwnerOrGovernance {
        if(trackingVotes){
    ...
    function toggleVotes() external onlyPools {
