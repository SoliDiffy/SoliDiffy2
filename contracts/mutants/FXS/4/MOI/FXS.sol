...
contract FRAXShares is ERC20Custom, AccessControl {
    ...
    function transfer(address recipient, uint256 amount) public virtual override onlyByOwnerOrGovernance returns (bool) {
        if(trackingVotes){
            // Transfer votes
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override onlyByOwnerOrGovernance returns (bool) {
        if(trackingVotes){
            // Transfer votes
    ...
    function trackVotes(address srcRep, address dstRep, uint96 amount) internal onlyByOwnerOrGovernance {
    ...
    function _writeCheckpoint(address voter, uint32 nCheckpoints, uint96 oldVotes, uint96 newVotes) internal onlyByOwnerOrGovernance {
