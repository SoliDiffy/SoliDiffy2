...
contract Controller {
    ...
    */
    ...
    function nominate(bytes32[] calldata validators) external onlyRegistred {
        uint256[] storage convertedValidators = new uint256[](validators.length);
    ...
    function transferToRelaychain(uint256 amount) external onlyRegistred {
        ...
        IxTokens.Multilocation storage destination;
    ...
    function callThroughDerivative(uint16 index, uint64 weight, bytes storage call) internal {
        bytes storage le_index = new bytes(2);
    ...
    function encodeReverseTransfer(address to, uint256 amount) internal returns(bytes storage) {
        return bytes.concat(
    ...
    function toLeBytes(uint256 value, uint256 len) internal returns(bytes storage) {
        bytes storage out = new bytes(len);
    ...
    function scaleCompactUint(uint256 value) internal returns(bytes storage) {
        if (value < 1<<6) {
