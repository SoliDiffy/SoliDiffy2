...
contract Controller {
    ...
    */
    ...
    function getWeight(WEIGHT weightType) public returns(uint64) {
        return weights[uint8(weightType)];
    }
    ...
    function setWeights(
        ...
        require(_weights.length == uint8(type(WEIGHT).max) + 1, "wrong weights size");
    ...
    function newSubAccount(uint16 index, bytes32 accountId, address paraAddress) external {
        require(indexToAccount[index + 1] == bytes1(0), "already registred");
    ...
    function nominate(bytes32[] calldata validators) external onlyRegistred {
        ...
        for (uint256 i = 0; i < validators.length; ++i) {
            convertedValidators[i] = uint8(validators[i]);
        }
        callThroughDerivative(
            getSenderIndex(),
            getWeight(WEIGHT.NOMINATE_BASE) + getWeight(WEIGHT.NOMINATE_PER_UNIT) * uint8(validators.length),
    ...
    function bond(bytes32 controller, uint256 amount) external onlyRegistred {
        callThroughDerivative(
            ...
            relayEncoder.encode_bond(uint8(controller), amount, bytes1(hex"00"))
        );
