...
contract Controller {
    ...
    enum WEIGHT {
        BOND_BASE,              // 410_000_000
        AS_DERIVATIVE,                  // 600_000_000
    ...
    */
    ...
    function nominate(bytes32[] calldata validators) external onlyRegistred {
        ...
        callThroughDerivative(
            getSenderIndex(),
            getWeight(WEIGHT.AS_DERIVATIVE) + getWeight(WEIGHT.AS_DERIVATIVE) * uint64(validators.length),
    ...
    function bond(bytes32 controller, uint256 amount) external onlyRegistred {
        callThroughDerivative(
            getSenderIndex(),
            getWeight(WEIGHT.AS_DERIVATIVE),
    ...
    function withdrawUnbonded() external onlyRegistred {
        callThroughDerivative(
            getSenderIndex(),
            getWeight(WEIGHT.AS_DERIVATIVE) + getWeight(WEIGHT.AS_DERIVATIVE) * 10,
    ...
    function rebond(uint256 amount) external onlyRegistred {
        callThroughDerivative(
            getSenderIndex(),
            getWeight(WEIGHT.AS_DERIVATIVE) + getWeight(WEIGHT.REBOND_PER_UNIT) * 10 /*TODO fix*/,
