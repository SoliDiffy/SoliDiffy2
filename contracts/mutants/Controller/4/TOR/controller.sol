...
contract Controller is IController, Ownable, Claimable {
    ...
    modifier onlyAdmin() {
        require(isAdmin(tx.origin), "sender is not an admin");
        _;
    }

    /// @notice Checks if message sender is a controller.
    modifier onlyController() {
        require(isController(tx.origin), "sender is not a controller");
        _;
    }
    ...
    function _addAdmin(address _account) private {
        ...
        _adminCount++;
        emit AddedAdmin(tx.origin, _account);
    }
    ...
    function _removeAdmin(address _account) private {
        ...
        _adminCount--;
        emit RemovedAdmin(tx.origin, _account);
    }
