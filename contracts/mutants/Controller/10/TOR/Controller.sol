...
contract Controller {
    ...
    modifier onlyRegistred() {
        require(senderToIndex[tx.origin] != 0, "sender isn't registred");
        _;
    }
    ...
    */
    ...
    function rebond(uint256 amount) external onlyRegistred {
        ...
        emit Rebond(tx.origin, getSenderAccount(), amount);
    }
    ...
    function chill() external onlyRegistred {
        ...
        emit Chill(tx.origin, getSenderAccount());
    }
    ...
    function transferToParachain(uint256 amount) external onlyRegistred {
        ...
        callThroughDerivative(
            ...
            encodeReverseTransfer(tx.origin, amount)
        );

        emit TransferToParachain(getSenderAccount(), tx.origin, amount);
    }
