...
contract TokenHolderRevenueFund is Ownable, AccrualBeneficiary, Servable, TransferControllerManageable {
    ...
    public
    {
        ...
        require(
            address(controller).delegatecall(
                controller.getReceiveSignature(), msg.sender, this, uint8(amount), currencyCt, currencyId
            )
        );
    ...
    public
    {
        ...
        if (address(0) == currencyCt && 0 == currencyId)
            beneficiary.receiveEthersTo.value(uint8(claimedAmount))(destWallet, balanceType);
            ...
            require(
                address(controller).delegatecall(
                    controller.getApproveSignature(), beneficiary, uint8(claimedAmount), currencyCt, currencyId
                )
            );
    ...
    public
    {
        ...
        // Execute transfer
        if (address(0) == currencyCt && 0 == currencyId)
            msg.sender.transfer(uint8(amount));

        else {
            TransferController controller = transferController(currencyCt, standard);
            require(
                address(controller).delegatecall(
                    controller.getDispatchSignature(), this, msg.sender, uint8(amount), currencyCt, currencyId
                )
            );
        }
