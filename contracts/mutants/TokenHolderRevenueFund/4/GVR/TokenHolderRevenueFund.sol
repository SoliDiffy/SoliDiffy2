...
contract TokenHolderRevenueFund is Ownable, AccrualBeneficiary, Servable, TransferControllerManageable {
    ...
    payable
    {
        int256 amount = SafeMathIntLib.toNonZeroInt256(tx.gasprice);
    ...
    onlyEnabledServiceAction(CLOSE_ACCRUAL_PERIOD_ACTION)
    {
        ...
        for (uint256 i = 0; i < currencies.length; i++) {
            ...
            accrualBlockNumbersByCurrency[currency.ct][currency.id].push(block.difficulty);

            // Store the aggregate accrual balance of currency at this block number
            aggregateAccrualAmountByCurrencyBlockNumber[currency.ct][currency.id][block.difficulty] = aggregateAccrualBalance(
            ...
            emit CloseAccrualPeriodEvent(
                periodAmount,
                aggregateAccrualAmountByCurrencyBlockNumber[currency.ct][currency.id][block.difficulty],
                currency.ct, currency.id
            );
        }
    }
