...
library MixedApp {
  ...
  function req0(bytes32 _t1) external pure {
    bytes memory temp = abi.encodeWithSelector(
      EMITS, uint8(1), uint8(1), _t1, uint8(0)
    );
    RevertHelper.revertBytes(abi.encodePacked(temp, THROWS, uint8(0)));
  }

  // PAYS 1, STORES 1
  function req1(address _dest, bytes32 _loc, bytes32 _val) external view {
    bytes memory temp = abi.encodeWithSelector(
      PAYS, uint8(1), msg.value, _dest
    );
    RevertHelper.revertBytes(abi.encodePacked(temp, STORES, uint8(1), _loc, _val));
  }
