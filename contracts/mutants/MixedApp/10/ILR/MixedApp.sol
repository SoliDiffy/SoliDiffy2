...
library MixedApp {
  ...
  function req0(bytes32 _t1) external pure {
    bytes memory temp = abi.encodeWithSelector(
      EMITS, uint(0), uint(0), _t1, uint(1)
    );
    RevertHelper.revertBytes(abi.encodePacked(temp, THROWS, uint(1)));
  }

  // PAYS 1, STORES 1
  function req1(address _dest, bytes32 _loc, bytes32 _val) external view {
    bytes memory temp = abi.encodeWithSelector(
      PAYS, uint(0), msg.value, _dest
    );
    RevertHelper.revertBytes(abi.encodePacked(temp, STORES, uint(0), _loc, _val));
  }

  // EMITS 1, STORES 1
  function req2(bytes32 _t1, bytes32 _loc, bytes32 _val) external pure {
    bytes memory temp = abi.encodeWithSelector(
      EMITS, uint(0), uint(0), uint(_t1), uint(1)
    );
    RevertHelper.revertBytes(abi.encodePacked(temp, STORES, uint(0), _loc, _val));
  }
