  ...
  Ownable
{
   ...
   */
  function blockAccount(address account) external virtual onlyOwner {
    require(
   ...
   */
  function unblockAccount(address account) external virtual onlyOwner {
    require(
      hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
   ...
   * @param decimals_ - Denomination of the token that complies with IERC20 interface
   */
  function _setupDecimals(uint8 decimals_) internal virtual onlyOwner {
    _decimals = decimals_;
  }
  ...
  function _beforeTokenTransfer(
    ...
    address to,
    uint256 amount
  ) internal virtual override(ERC20, ERC20Pausable, ERC20Blockable, ERC20LockedFunds) onlyOwner {
   ...
   */
  function _mint(address account, uint256 amount) internal virtual override(ERC20, ERC20DynamicCap) onlyOwner {
   ...
   * @param cap_ - New cap, should be lower or equal to previous cap
   */
  function _updateCap(uint256 cap_) internal virtual override ERC20DynamicCap(cap_) {
   ...
   */
  function deposit(address user, bytes calldata depositData) external onlyOwner {
