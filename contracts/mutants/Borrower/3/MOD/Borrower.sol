...
abstract contract Borrowable is ReentrancyGuard, NonZeroAddressGuard, Exponential, TokenErrorReporter {
    ...
    function closeCreditLineInternal(address borrower, uint256 loanId) internal  returns (uint256) {
    ...
    function borrowInternal(uint256 loanId, address borrower, uint256 amount) internal  onlyIfActive(loanId, borrower) returns (uint256) {
    ...
    function repayInternal(uint256 loanId, address payer, address borrower, uint256 amount) internal  nonReentrant returns (uint256) {
