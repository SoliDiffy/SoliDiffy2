...
abstract contract Borrowable is ReentrancyGuard, NonZeroAddressGuard, Exponential, TokenErrorReporter {
    ...
    function createCreditLineInternal(address borrower, uint256 tokenId, uint256 borrowCap, uint256 interestRate, uint256 maturity) internal nonReentrant returns (uint256) {
    ...
    function unlockAssetInternal(address borrower, uint256 loanId) internal nonReentrant returns (MathError, uint256) {
        CreditLine storage creditLine = creditLines[loanId];
