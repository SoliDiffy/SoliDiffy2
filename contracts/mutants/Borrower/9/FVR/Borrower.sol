...
abstract contract Borrowable is ReentrancyGuard, NonZeroAddressGuard, Exponential, TokenErrorReporter {
    ...
    function _isActive(uint256 _loanId, address borrower_) public view {
    ...
    function getBorrowerTotalPrincipal(address _borrower) public view returns (uint256) {
        uint256 balance;
    ...
    function getBorrowerBalance(address _borrower) public view returns (uint256) {
        uint256 balance;
    ...
    function borrowerSnapshot(uint256 loanId) public view returns (uint256, uint256) {
    ...
    function createCreditLineInternal(address borrower, uint256 tokenId, uint256 borrowCap, uint256 interestRate, uint256 maturity) public returns (uint256) {
    ...
    function closeCreditLineInternal(address borrower, uint256 loanId) public onlyIfActive(loanId, borrower) returns (uint256) {
    ...
    function unlockAssetInternal(address borrower, uint256 loanId) public returns (MathError, uint256) {
        CreditLine storage creditLine = creditLines[loanId];
    ...
    function borrowInternal(uint256 loanId, address borrower, uint256 amount) public nonReentrant onlyIfActive(loanId, borrower) returns (uint256) {
