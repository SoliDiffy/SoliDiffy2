...
contract GovStaking is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public opt2 = 604800; // 1 week
        ...
        returns (GovStakingStorage.UserInfo memory)
    {
        ...
        if (user.amount == 0) {
            ...
            if (user.lockStart + user.lockPeriod <= block.timestamp) {
                ...
                store.updateRewardMultiplier( // for gogo fee distribution
                    msg.sender,
                    oldRate,
                    user.rewardRate,
    ...
    function leave() external onlyOwner whenNotPaused {
        rewards.updateAllRewards(msg.sender);
        GovStakingStorage.UserInfo memory user = store.getUserInformation(
            msg.sender
        );
...
// SWC-100-Function Default Visibility: L202 - L205
    function claim() public onlyOwner whenNotPaused {
        rewards.updateAllRewards(msg.sender);
        _claim();
    }
...
// SWC-100-Function Default Visibility: L288 - l299
    ...
    function setRewardRate(uint256 period, uint256 newRate) external nonReentrant {
        require(rewardRates[period] > 0, "wrong period");
        rewardRates[period] = newRate;
    }

    function setRewardRates(RewardRate[] memory rates) external nonReentrant {
    ...
    function setCommunityManager(address newManager) external nonReentrant {
        rewards = ICommunityRewardsManager(newManager);
    }

    function setStorage(address newStorage) external nonReentrant {
