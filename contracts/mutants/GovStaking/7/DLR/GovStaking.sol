...
contract GovStaking is Ownable, ReentrancyGuard, Pausable {
    ...
    function enter(uint256 amount, uint256 extendingPeriod)
        ...
        nonReentrant
        whenNotPaused
        returns (GovStakingStorage.UserInfo storage)
    {
        ...
        GovStakingStorage.UserInfo storage user = store.getUserInformation(
            msg.sender
        );
...
// SWC-100-Function Default Visibility: L208 - L212
    ...
    function storeUser(GovStakingStorage.UserInfo storage user) internal {
        store.writeUser(
            msg.sender,
            user.amount,
            user.lockStart,
