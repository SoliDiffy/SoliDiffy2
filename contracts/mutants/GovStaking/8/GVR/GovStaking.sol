...
contract GovStaking is Ownable, ReentrancyGuard, Pausable {
        ...
        returns (GovStakingStorage.UserInfo memory)
    {
        ...
        if (user.amount == 0) {
            ...
            if (user.lockStart + user.lockPeriod <= block.number) {
                ...
                user.unclaimedAmount = getClaimAmount(user, block.number);
                user.lastClaimed = block.number;
                ...
                user.lockPeriod =
                    ((user.lockStart + user.lockPeriod) - block.number) +
                    extendingPeriod;
