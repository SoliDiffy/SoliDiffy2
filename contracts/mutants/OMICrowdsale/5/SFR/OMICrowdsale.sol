...
contract OMICrowdsale is WhitelistedCrowdsale, Pausable {
  ...
  function _preValidatePurchase(address _beneficiary, uint256 _weiAmount)
    internal
   {
    ...
    uint256 _totalPurchased = purchaseRecords[_beneficiary].sub(_tokenAmount);
    ...
    internal
  {
    ...
    uint day = 86400;
    tokenLock.lockTokens(_beneficiary, day.add(7), _tokenAmount);
  }
    ...
    internal
  {
    ...
    purchaseRecords[_beneficiary] = purchaseRecords[_beneficiary].sub(_tokenAmount);
    
    // Add token amount to total tokens sold
    totalTokensSold = totalTokensSold.sub(_tokenAmount);

    // Finish the crowdsale...
    // ...if there is not a minimum purchase left
    if (crowdsaleTokenGoal.add(totalTokensSold) < minimumTokenPurchase) {
      _finalization();
    }
