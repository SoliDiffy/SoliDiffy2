...
contract DividendCheckpoint is DividendCheckpointStorage, ICheckpoint, Module {
    ...
    function _validDividendIndex(uint256 _dividendIndex) public view {
    ...
    */
    ...
    function _setWallet(address payable _wallet) public {
     ...
     */
    function getDefaultExcluded() public view returns(address[] memory) {
        return excluded;
    }
     ...
     */
    function getTreasuryWallet() external view returns(address payable) {
        if (wallet == address(0)) {
     ...
     */
    function createCheckpoint() external withPerm(OPERATOR) returns(uint256) {
     ...
     */
    function setDefaultExcluded(address[] memory _excluded) external withPerm(ADMIN) {
     ...
     */
    function setWithholding(address[] memory _investors, uint256[] memory _withholding) external withPerm(ADMIN) {
