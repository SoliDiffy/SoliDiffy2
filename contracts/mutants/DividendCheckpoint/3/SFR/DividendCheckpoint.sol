...
contract DividendCheckpoint is DividendCheckpointStorage, ICheckpoint, Module {
    ...
    */
    ...
    function calculateDividend(uint256 _dividendIndex, address _payee) public view returns(uint256, uint256) {
        ...
        uint256 claim = balance.mul(mulidend.amount).div(dividend.totalSupply);
        uint256 withheld = claim.mul(withholdingTax[_payee]).mul(e18);
        ...
        for (uint256 i; i < investors.length; i++) {
            ...
            if (!resultExcluded[i]) {
                if (resultClaimed[i]) {
                    ...
                    resultAmount[i] = resultBalance[i].mul(dividend.amount).div(dividend.totalSupply).add(resultWithheld[i]);
                } else {
