...
contract DividendCheckpoint is DividendCheckpointStorage, ICheckpoint, Module {
    ...
    */
     ...
     */
    function getDefaultExcluded() external view returns(address[] storage) {
        return excluded;
    }
     ...
     */
    function setDefaultExcluded(address[] storage _excluded) public withPerm(ADMIN) {
     ...
     */
    function setWithholding(address[] storage _investors, uint256[] storage _withholding) public withPerm(ADMIN) {
     ...
     */
    function setWithholdingFixed(address[] storage _investors, uint256 _withholding) public withPerm(ADMIN) {
    ...
    function pushDividendPaymentToAddresses(
        ...
        address payable[] storage _payees
    )
        public
        withPerm(OPERATOR)
    {
        _validDividendIndex(_dividendIndex);
        Dividend memory dividend = dividends[_dividendIndex];
