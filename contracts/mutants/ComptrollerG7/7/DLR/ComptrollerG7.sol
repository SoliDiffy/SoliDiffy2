...
contract ComptrollerG7 is ComptrollerV5Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
     ...
     */
    function getAssetsIn(address account) external view returns (AToken[] storage) {
        AToken[] storage assetsIn = accountAssets[account];
     ...
     */
    function enterMarkets(address[] storage aTokens) public returns (uint[] storage) {
    ...
    function addToMarketInternal(AToken aToken, address borrower) internal returns (Error) {
        Market memory marketToJoin = markets[address(aToken)];
    ...
    function exitMarket(address aTokenAddress) external returns (uint) {
        ...
        Market memory marketToExit = markets[address(aToken)];
