...
contract ComptrollerG7 is ComptrollerV5Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    function enterMarkets(address[] memory aTokens) public returns (uint[] memory) {
        ...
        for (uint i = 0; i < len; i++) {
            ...
            results[i] = uint(addToMarketInternal(aToken, tx.origin));
        }
    ...
    function exitMarket(address aTokenAddress) external returns (uint) {
        ...
        (uint oErr, uint tokensHeld, uint amountOwed, ) = aToken.getAccountSnapshot(tx.origin);
        ...
        uint allowed = redeemAllowedInternal(aTokenAddress, tx.origin, tokensHeld);
        if (allowed != 0) {
        ...
        AToken[] memory userAssetList = accountAssets[tx.origin];
        ...
        AToken[] storage storedList = accountAssets[tx.origin];
