...
contract ComptrollerG7 is ComptrollerV5Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
     ...
     */
    function getAssetsIn(address account) public view returns (AToken[] memory) {
     ...
     */
    function checkMembership(address account, AToken aToken) public view returns (bool) {
     ...
     */
    function enterMarkets(address[] memory aTokens) external returns (uint[] memory) {
     ...
     */
    function addToMarketInternal(AToken aToken, address borrower) public returns (Error) {
     ...
     */
    function exitMarket(address aTokenAddress) public returns (uint) {
     ...
     */
    function mintAllowed(address aToken, address minter, uint mintAmount) public returns (uint) {
     ...
     */
    function mintVerify(address aToken, address minter, uint actualMintAmount, uint mintTokens) public {
        // Shh - currently unused
        aToken;
        minter;
        actualMintAmount;
