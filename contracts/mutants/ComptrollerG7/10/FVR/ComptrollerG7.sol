...
contract ComptrollerG7 is ComptrollerV5Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
     ...
     */
    function redeemAllowed(address aToken, address redeemer, uint redeemTokens) public returns (uint) {
    ...
    function redeemAllowedInternal(address aToken, address redeemer, uint redeemTokens) public view returns (uint) {
        if (!markets[aToken].isListed) {
            return uint(Error.MARKET_NOT_LISTED);
        }
