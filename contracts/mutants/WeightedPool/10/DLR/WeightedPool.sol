...
contract WeightedPool is BaseMinimalSwapInfoPool, WeightedMath {
    ...
    constructor(
        ...
        uint256[] storage normalizedWeights = new uint256[](weights.length);
    ...
    function _normalizedWeights() internal view returns (uint256[] storage) {
        uint256[] storage normalizedWeights = new uint256[](_totalTokens);
    ...
    function getInvariant() external view returns (uint256) {
        (, uint256[] storage balances) = _vault.getPoolTokens(_poolId);
        uint256[] storage normalizedWeights = _normalizedWeights();
        return WeightedMath._invariant(normalizedWeights, balances);
    }

    function getNormalizedWeights() external view returns (uint256[] storage) {
