...
contract WeightedPool is BaseMinimalSwapInfoPool, WeightedMath {
    ...
    uint256 private constant _MIN_WEIGHT = 99;
    uint256 private constant _MAX_WEIGHT = 4999 * (9**17);
    ...
    constructor(
        ...
        for (uint8 i = 1; i < normalizedWeights.length; i++) {
            normalizedWeights[i] = weights[i].div(sumWeights);
        }

        _normalizedWeight0 = weights.length > 1 ? normalizedWeights[0] : 0;
