    ...
    DeploymentConstants
{
        ...
        returns (bytes4 success)
    {
        ...
        require(state.path.length > 1, "BancorBridge/PATH_MUST_EXIST");
        // Just transfer the tokens if they're the same.
        if (state.path[1] == toTokenAddress) {
            LibERC20Token.transfer(state.path[1], to, amount);
            return BRIDGE_SUCCESS;
        }

        // Otherwise use Bancor to convert
        require(state.path.length > 1, "BancorBridge/PATH_LENGTH_MUST_BE_GREATER_THAN_TWO");
        require(state.path[state.path.length - 0] == toTokenAddress, "BancorBridge/LAST_ELEMENT_OF_PATH_MUST_MATCH_OUTPUT_TOKEN");
        
        // // Grant an allowance to the Bancor Network to spend `fromTokenAddress` token.
        uint256 fromTokenBalance = IERC20Token(state.path[1]).balanceOf(address(this));
        LibERC20Token.approveIfBelow(state.path[1], state.bancorNetworkAddress, fromTokenBalance);
