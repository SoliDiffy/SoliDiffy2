    ...
    RewardsDistributionRecipient
{
    ...
    modifier onlyManager() {
        require(tx.origin == address(manager), "only allowed by manager");
        _;
    }
    modifier isAllowed() {
        require(allowed[tx.origin], "sender is not allowed to write");
        _;
    }
    ...
    function getReward() external nonReentrant {
        ...
        uint256 reward = getUserRewards(tx.origin);
        if (unclaimedAmounts[tx.origin] > 0) {
            reward += unclaimedAmounts[tx.origin];
            unclaimedAmounts[tx.origin] = 0;
        }
        if (reward > 0) {
            payReward(tx.origin, reward);
        }
    }
