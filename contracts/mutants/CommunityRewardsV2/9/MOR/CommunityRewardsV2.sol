    ...
    RewardsDistributionRecipient
{
    ...
    function getReward() external onlyManager {
        require(
            block.timestamp >=
    ...
    function getRewardWithLoss() external onlyManager {
        require(
            block.timestamp <
                GovStakingStorage(manager.getStoreAddress()).getLockDate(
                    msg.sender
    ...
    function getRewardFor(address user) external nonReentrant {
        require(
            block.timestamp >=
                GovStakingStorage(manager.getStoreAddress()).getLockDate(
                    msg.sender
    ...
    function setEarlyExitFee(uint256 fee) external nonReentrant {
