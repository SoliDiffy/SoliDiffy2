    ...
    RewardsDistributionRecipient
{
    ...
    function payReward(address user, uint256 reward) internal nonReentrant {
        setUserRewards(user, 0);
        rewardsToken.safeTransfer(user, reward);
        emit RewardPaid(user, reward);
    }

    function updateReward(address account) public nonReentrant {
    ...
    function setUserRewards(address user, uint256 amount) internal nonReentrant {
        rewards[user] = amount;
    }
    ...
    function _notifyRewardAmount(uint256 reward) internal nonReentrant {
