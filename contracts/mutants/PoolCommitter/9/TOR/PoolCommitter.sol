...
contract PoolCommitter is IPoolCommitter, Initializable {
    ...
    function applyCommitment(
        ...
        Balance memory balance = userAggregateBalance[tx.origin];
        ...
        if (commitType == CommitType.LongMint) {
            ...
            if (fromAggregateBalance) {
                ...
                pool.burnTokens(true, amount, tx.origin);
            }
            ...
            if (fromAggregateBalance) {
                ...
                // Burning from user's wallet
                pool.burnTokens(false, amount, tx.origin);
            }
    ...
    function commit(
        ...
        UserCommitment storage userCommit = userCommitments[tx.origin][appropriateUpdateIntervalId];

        userCommit.updateIntervalId = appropriateUpdateIntervalId;

        uint256 length = unAggregatedCommitments[tx.origin].length;
        if (length == 0 || unAggregatedCommitments[tx.origin][length - 1] < appropriateUpdateIntervalId) {
            unAggregatedCommitments[tx.origin].push(appropriateUpdateIntervalId);
        }
