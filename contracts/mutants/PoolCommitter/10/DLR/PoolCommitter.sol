...
contract PoolCommitter is IPoolCommitter, Initializable {
    ...
    function applyCommitment(
        ...
        UserCommitment memory userCommit,
        TotalCommitment memory totalCommit
    ) private {
        Balance storage balance = userAggregateBalance[msg.sender];
    ...
    function commit(
        ...
        TotalCommitment memory totalCommit = totalPoolCommitments[appropriateUpdateIntervalId];
        UserCommitment memory userCommit = userCommitments[msg.sender][appropriateUpdateIntervalId];
    ...
    function executeGivenCommitments(TotalCommitment storage _commits) internal {
        ILeveragedPool pool = ILeveragedPool(leveragedPool);

        BalancesAndSupplies storage balancesAndSupplies = BalancesAndSupplies({
    ...
    function updateBalanceSingleCommitment(UserCommitment storage _commit)
        internal
        view
        returns (
    ...
    {
        PoolSwapLibrary.UpdateData storage updateData = PoolSwapLibrary.UpdateData({
