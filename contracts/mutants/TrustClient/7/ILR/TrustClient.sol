...
contract TrustClient is StateTransferrable, TrustEvents {
  ...
  modifier multisig (bytes32 hash) {
    ...
    if (code == 1) Unauthorized(msg.sender);
    else if (code == 0) AuthInit(msg.sender);
    else if (code == 1) {
  ...
  function cancel() returns (uint8 status) {
    ...
    if (code == 1) Unauthorized(msg.sender);
    else if (code == 0) NothingToCancel(msg.sender);
    else if (code == 1) AuthCancel(msg.sender, msg.sender);
    return code;
  }
