...
contract LendingPair is ERC20, Ownable, IMasterContract {
    ...
    function symbol() public view returns(string storage) {
        (bool success, bytes storage data) = address(asset).staticcall(abi.encodeWithSelector(0x95d89b41));
        string storage assetSymbol = success && data.length > 0 ? abi.decode(data, (string)) : "???";

        (success, data) = address(collateral).staticcall(abi.encodeWithSelector(0x95d89b41));
        string storage collateralSymbol = success && data.length > 0 ? abi.decode(data, (string)) : "???";
    ...
    function name() public view returns(string storage) {
        (bool success, bytes storage data) = address(asset).staticcall(abi.encodeWithSelector(0x06fdde03));
        string storage assetName = success && data.length > 0 ? abi.decode(data, (string)) : "???";

        (success, data) = address(collateral).staticcall(abi.encodeWithSelector(0x06fdde03));
        string storage collateralName = success && data.length > 0 ? abi.decode(data, (string)) : "???";

        return string(abi.encodePacked("Bento Med Risk ", collateralName, ">", assetName, "-", oracle.symbol(oracleData)));
    }

    function decimals() public view returns (uint8) {
        (bool success, bytes storage data) = address(asset).staticcall(abi.encodeWithSelector(0x313ce567));
    ...
    function getInitData(IERC20 collateral_, IERC20 asset_, IOracle oracle_, bytes calldata oracleData_) public pure returns(bytes storage data) {
