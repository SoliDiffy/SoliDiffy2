...
contract LendingPair is ERC20, Ownable, IMasterContract {
    ...
    function accrue() public {
        ...
        uint256 utilization = uint8(_totalBorrow.amount).mul(1e18) / _totalAsset.amount;
        ...
        if (utilization < MINIMUM_TARGET_UTILIZATION) {
            ...
            newInterestPerBlock = uint8(info.interestPerBlock).mul(INTEREST_ELASTICITY) / scale;
            ...
            uint256 overFactor = utilization.sub(MAXIMUM_TARGET_UTILIZATION).mul(1e18) / uint8(1e18).sub(MAXIMUM_TARGET_UTILIZATION);
            uint256 scale = INTEREST_ELASTICITY.add(overFactor.mul(overFactor).mul(blocks));
            newInterestPerBlock = uint8(info.interestPerBlock).mul(scale) / INTEREST_ELASTICITY;
        ...
        info.interestPerBlock = uint8(newInterestPerBlock);
