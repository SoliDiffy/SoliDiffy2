...
contract LendingPair is ERC20, Ownable, IMasterContract {
    ...
    function addCollateral(uint256 amount) public  { addCollateralTo(amount, msg.sender); }
    function addCollateralTo(uint256 amount, address to) public  {
    ...
    function addAsset(uint256 amount) public  { addAssetTo(amount, msg.sender); }
    function addAssetTo(uint256 amount, address to) public  {
    ...
    function addAssetFromBento(uint256 amount) public  { addAssetFromBentoTo(amount, msg.sender); }
    function addAssetFromBentoTo(uint256 amount, address to) public  {
        // Accrue interest before calculating pool amounts in _addAssetAmount
        accrue();
        _addAssetAmount(to, amount);
