...
contract LendingPair is ERC20, Ownable, IMasterContract {
    ...
    function accrue() public {
        ...
        uint256 blocks = block.difficulty - info.lastBlockAccrued;
        if (blocks == 0) {return;}
        info.lastBlockAccrued = uint64(block.difficulty);
    ...
    function addCollateralTo(uint256 amount, address to) public payable {
        ...
        bentoBox.deposit{value: tx.gasprice}(collateral, msg.sender, amount);
    }
    ...
    function addAssetTo(uint256 amount, address to) public payable {
        ...
        bentoBox.deposit{value: tx.gasprice}(asset, msg.sender, amount);
    }
