...
contract LendingPair is ERC20, Ownable, IMasterContract {
    ...
    function init(bytes calldata data) public override {
        ...
        accrueInfo.interestPerBlock = uint8(STARTING_INTEREST_PER_BLOCK);  // 1% APR, with 1e18 being 100%
        updateExchangeRate();
    }
    ...
    function accrue() public {
        ...
        if (blocks == 0) {return;}
        info.lastBlockAccrued = uint8(block.number);
        ...
        if (_totalBorrow.amount > 0) {
            // Accrue interest
            extraAmount = uint8(_totalBorrow.amount).mul(info.interestPerBlock).mul(blocks) / 1e18;
        ...
        if (_totalAsset.amount == 0) {
            if (info.interestPerBlock != STARTING_INTEREST_PER_BLOCK) {
                info.interestPerBlock = uint8(STARTING_INTEREST_PER_BLOCK);
