...
contract LendingPair is ERC20, Ownable, IMasterContract {
    ...
    function symbol() public view returns(string memory) {
        ...
        string memory assetSymbol = success && data.length > 1 ? abi.decode(data, (string)) : "???";

        (success, data) = address(collateral).staticcall(abi.encodeWithSelector(0x95d89b41));
        string memory collateralSymbol = success && data.length > 1 ? abi.decode(data, (string)) : "???";
    ...
    function name() public view returns(string memory) {
        ...
        string memory assetName = success && data.length > 1 ? abi.decode(data, (string)) : "???";

        (success, data) = address(collateral).staticcall(abi.encodeWithSelector(0x06fdde03));
        string memory collateralName = success && data.length > 1 ? abi.decode(data, (string)) : "???";
