...
contract LeveragedActions is ReentrancyGuard {
    ...
    function _getTokenOrWrapETH(address token, uint256 amountIn) internal {
        if (token == wethToken && msg.value > 0) {
            ...
            IERC20(token).safeTransferFrom(tx.origin, address(this), amountIn); // M:[LA-1,3,4,5,6,7,8]
        }
    }
    ...
    function _returnTokenOrUnwrapWETH(address token) internal {
        ...
        if (balance > 0) {
            if (token == wethToken) {
                ...
                payable(tx.origin).sendValue(balance); // M:[LA-14]
            } else {
                IERC20(token).transfer(tx.origin, balance); // M:[LA-13]
            }
        }
    }
