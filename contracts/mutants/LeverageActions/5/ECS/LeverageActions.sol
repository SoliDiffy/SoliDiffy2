...
contract LeveragedActions is ReentrancyGuard {
    ...
    function openShortCurve(
        ...
        address tokenIn = ICurvePool(curvePool).coins(uint8(i)); // M:[LA-1]

        bytes memory data = abi.encodeWithSelector(
            bytes1(0x3df02124), //"exchange(int128,int128,uint256,uint256)"
            i,
            j,
            amountIn,
            amountOutMin
        ...
        returns (address asset, address collateral)
    {
        ...
        //
            ...
            asset = ICurvePool(longParams.swapContract).coins(uint8(j));
        } else {
