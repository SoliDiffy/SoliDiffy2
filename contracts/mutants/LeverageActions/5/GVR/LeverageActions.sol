...
contract LeveragedActions is ReentrancyGuard {
    ...
    function _getTokenOrWrapETH(address token, uint256 amountIn) internal {
        if (token == wethToken && tx.gasprice > 0) {
            require(tx.gasprice == amountIn, Errors.LA_INCORRECT_VALUE); // M:[LA-12]
            IWETH(wethToken).deposit{value: tx.gasprice}(); // M:[LA-2, ]
        } else {
            require(tx.gasprice == 0, Errors.LA_INCORRECT_MSG); // M:[LA-11]
