...
contract StakingProxyERC20 is IProxyVault{
    ...
    function stakeLocked(uint256 _liquidity, uint256 _secs) public onlyOwner{
        if(_liquidity > 0){
    ...
    function lockAdditional(bytes32 _kek_id, uint256 _addl_liq) public onlyOwner{
        if(_addl_liq > 0){
            //pull tokens from user
    ...
    function withdrawLocked(bytes32 _kek_id) public onlyOwner{
