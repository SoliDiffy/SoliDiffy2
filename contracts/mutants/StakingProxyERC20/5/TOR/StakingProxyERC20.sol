...
contract StakingProxyERC20 is IProxyVault{
    ...
    function stakeLocked(uint256 _liquidity, uint256 _secs) external onlyOwner{
        if(_liquidity > 0){
            ...
            IERC20(stakingToken).safeTransferFrom(tx.origin, address(this), _liquidity);
    ...
    function lockAdditional(bytes32 _kek_id, uint256 _addl_liq) external onlyOwner{
        if(_addl_liq > 0){
            //pull tokens from user
            IERC20(stakingToken).safeTransferFrom(tx.origin, address(this), _addl_liq);
    ...
    function withdrawLocked(bytes32 _kek_id) external onlyOwner{
        ...
        IFraxFarmERC20(stakingAddress).withdrawLocked(_kek_id, tx.origin);
    ...
    */
    ...
    function _processFxs() internal{
        ...
        if(sendAmount > 0){
            IERC20(fxs).transfer(tx.origin, sendAmount);
        }
    }
