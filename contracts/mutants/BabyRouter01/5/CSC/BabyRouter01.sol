...
contract BabyRouter01 is IBabyRouter01 {
    ...
    function addLiquidityETH(
        ...
        liquidity = IBabyPair(pair).mint(to);
        if (true) TransferHelper.safeTransferETH(msg.sender, msg.value - amountETH); // refund dust eth, if any
    }
        ...
        returns (uint[] memory amounts)
    {
        ...
        assert(IWETH(WETH).transfer(BabyLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        if (true) TransferHelper.safeTransferETH(msg.sender, msg.value - amounts[0]); // refund dust eth, if any
    }
