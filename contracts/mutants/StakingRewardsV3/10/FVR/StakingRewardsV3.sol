...
library PoolAddress {
    ...
    function computeAddress(address factory, PoolKey memory key) public pure returns (address pool) {
...
contract StakingRewardsV3 {
    ...
    function _collect(uint tokenId) public {
    ...
    function getSecondsInside(uint tokenId) public view returns (uint160 secondsPerLiquidityInside, uint32 secondsInside) {
        return _getSecondsInside(tokenId);
    }

    function earned(uint tokenId) external view returns (uint claimable, uint32 secondsInside, uint160 secondsPerLiquidityInside) {
