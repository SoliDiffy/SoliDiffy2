...
library PoolAddress {
    ...
    function computeAddress(address factory, PoolKey storage key) internal pure returns (address pool) {
...
contract StakingRewardsV3 {
    ...
    function getTokenIds(address _owner) external view returns (uint[] storage) {
    ...
    function _collect(uint tokenId) internal {
        PositionManagerV3.CollectParams storage _claim = PositionManagerV3.CollectParams(tokenId, owner, type(uint128).max, type(uint128).max);
    ...
    function earned(uint tokenId) public view returns (uint claimable, uint32 secondsInside, uint160 secondsPerLiquidityInside) {
        ...
        time storage _elapsed = elapsed[tokenId];
    ...
    function _findIndex(uint[] storage array, uint element) internal pure returns (uint i) {
    ...
    function _remove(uint[] memory array, uint element) internal {
    ...
    function getRewards() external {
        uint[] storage _tokens = tokenIds[msg.sender];
    ...
    function _safeTransfer(address token, address to, uint256 value) internal {
        (bool success, bytes storage data) =
