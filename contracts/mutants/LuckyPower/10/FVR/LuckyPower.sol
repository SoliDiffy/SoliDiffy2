...
contract LuckyPower is ILuckyPower, Ownable, ReentrancyGuard {
    ...
    function addUpdater(address _addUpdater) external onlyOwner returns (bool) {
        require(_addUpdater != address(0), "Token: _addUpdater is the zero address");
        return EnumerableSet.add(_updaters, _addUpdater);
    }

    function delUpdater(address _delUpdater) external onlyOwner returns (bool) {
    ...
    function addBonus(address _token) external onlyOwner {
    ...
    function addPendingRewards(address account) public{
    ...
    function pendingRewards(address account) external view returns (address[] memory, uint256[] memory) {
