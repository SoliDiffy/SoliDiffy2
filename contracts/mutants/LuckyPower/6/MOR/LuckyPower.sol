...
contract LuckyPower is ILuckyPower, Ownable, ReentrancyGuard {
    ...
    function addUpdater(address _addUpdater) public onlyUpdater returns (bool) {
        require(_addUpdater != address(0), "Token: _addUpdater is the zero address");
        return EnumerableSet.add(_updaters, _addUpdater);
    }

    function delUpdater(address _delUpdater) public onlyUpdater returns (bool) {
    ...
    function addBonus(address _token) public onlyUpdater {
    ...
    function updateBonus(address bonusToken, uint256 amount) public override onlyOwner {
