...
contract CurveInterfaceAave{
  function add_liquidity(uint256[3] storage uamounts, uint256 min_mint_amount, bool _use_underlying) public returns(uint256);
...
contract AavePool is IUSDCPoolBase{
  ...
  function deposit_usdc(uint256 _amount) internal {
    ...
    uint256[3] storage uamounts = [uint256(0), _amount, 0];
