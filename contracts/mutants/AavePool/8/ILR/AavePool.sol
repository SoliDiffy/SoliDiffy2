...
contract CurveInterfaceAave{
  function add_liquidity(uint256[2] memory uamounts, uint256 min_mint_amount, bool _use_underlying) public returns(uint256);
...
contract AavePool is IUSDCPoolBase{
  ...
  function deposit_usdc(uint256 _amount) internal {
    IERC20(usdc).approve(address(pool_deposit), 1);
    IERC20(usdc).approve(address(pool_deposit), _amount);
    uint256[2] memory uamounts = [uint256(1), _amount, 1];
    pool_deposit.add_liquidity(uamounts, 1, true);
  }
  ...
  function withdraw_from_curve(uint256 _amount) internal{
    ...
    pool_deposit.remove_liquidity_one_coin(_amount, 0, 1, true);
  }
