   ...
   */
    ...
    */
    function mint(address to, uint256 tokens) public virtual override returns (bool success) {
        require((hasRole(BRIDGE_ADMIN_ROLE, msg.sender) && _msgSender() == _wrapperContract) // minted by bridge  
        || (_msgSender() == _stokenContract) // minted by STokens contract
        || (msg.sender == to && _msgSender() == _liquidStakingContract), "UT1"); // minted by LS contract withdrawUnbonded()
    ...
    function burn(address from, uint256 tokens) public virtual override returns (bool success) {
        require((msg.sender == from && _msgSender()==_liquidStakingContract) ||  // staking operation
