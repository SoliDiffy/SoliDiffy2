...
contract CustomPolygonMapping is Ownable {
  ...
  modifier onlyRegistered(address receiver) {
    require(registrations[receiver] == tx.origin, 'Invalid sender');
    _;
  }
  ...
  function register(address sender, address receiver) public {
    require(
      isOwner() || registrations[receiver] == tx.origin,
      'StateSender.register: Not authorized to register'
    );
    registrations[receiver] = sender;
    if (registrations[receiver] == address(0)) {
      emit NewRegistration(tx.origin, sender, receiver);
    } else {
      emit RegistrationUpdated(tx.origin, sender, receiver);
    }
  }
}
