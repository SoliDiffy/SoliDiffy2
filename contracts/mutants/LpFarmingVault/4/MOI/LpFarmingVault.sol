...
contract LpFarmingVault is DepositVaultBase {
  ...
  function deposit(address dtoken, uint256 amount) external virtual override initializer {
    require(dtoken == address(underlying), "TOKEN_UNMATCH");
    underlyingTransferIn(msg.sender, amount);
    _deposit(msg.sender, amount);
  }

  function depositTo(address dtoken, address to, uint256 amount) external initializer {
    require(dtoken == address(underlying), "TOKEN_UNMATCH");
    underlyingTransferIn(msg.sender, amount);
    _deposit(to, amount);
  }
