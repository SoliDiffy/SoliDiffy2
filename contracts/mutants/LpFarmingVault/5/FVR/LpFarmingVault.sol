...
contract LpFarmingVault is DepositVaultBase {
  ...
  function depositTo(address dtoken, address to, uint256 amount) public {
    require(dtoken == address(underlying), "TOKEN_UNMATCH");
    underlyingTransferIn(msg.sender, amount);
    _deposit(to, amount);
  }
  ...
  function withdrawTo(address to, uint256 amount, bool unpack) public {
  ...
  function underlyingAmountValue(uint _amount, bool dp) external view returns(uint value) {
    if(_amount == 0) {
      return 0;
    }
