...
contract ExpectedOutputGuardHandler is IOutputGuardHandler {
    ...
    function shouldVerifyArgumentEquals(OutputGuardModel.Data memory data) external {
        expectedData = data;
    }

    /** Mock the isValid() function return value */
    function mockIsValid(bool isValid) external {
        expectedIsValid = isValid;
    }

    /** Mock the getExitTarget() function return value */
    function mockGetExitTarget(address payable exitTarget) external {
        expectedExitTarget = exitTarget;
    }

    /** Mock the getConfirmSigAddress() function return value  */
    function mockGetConfirmSigAddress(address payable confirmSigAddress) external {
        expectedConfirmSigAddress = confirmSigAddress;
    }

    /** overrride */
    function isValid(OutputGuardModel.Data memory data) external view returns (bool) {
