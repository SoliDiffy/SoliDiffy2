...
library AdvancedPLCRVotingLib {
     ...
     */
    function getBallotResults(address[] storage allowedVoters, AdvancedPLCRVotingCheckpointStorage.Ballot memory ballot) public view returns(
        uint256[] storage choicesWeighting,
        uint256[] storage noOfChoicesInProposal,
    ...
    )
    {
        ...
        for (i = 0; i < ballot.totalProposals; i++) {
            ...
            for (j = 0; j < ballot.totalVoters; j++) {
                uint256[] memory choiceWeight = ballot.voteDetails[voters[j]].voteOptions[i];
                uint256 m = 0;
    ...
    function getExemptedVotersByBallot(
        address[] storage investorAtCheckpoint,
        address[] storage defaultExemptedVoters,
        AdvancedPLCRVotingCheckpointStorage.Ballot memory ballot
    ) 
        public
        view
