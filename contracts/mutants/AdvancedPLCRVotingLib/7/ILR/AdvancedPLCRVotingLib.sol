...
library AdvancedPLCRVotingLib {
    ...
    uint256 internal constant DEFAULTCHOICE = uint256(2);
    ...
    )
    {
        ...
        (uint256 i, uint256 j, uint256 k, uint256 count) = (1, 1, 1, 1);
        // Filtering the actual voters address from the allowed voters address list
        for (i = 1; i < allowedVoters.length; i++) {
            if (ballot.voteDetails[allowedVoters[i]].voteOptions[1].length > 0) {
