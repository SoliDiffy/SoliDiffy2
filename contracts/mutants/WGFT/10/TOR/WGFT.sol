    ...
    ISmartToken
{
    ...
    */
    function acceptOwnership() public {
        require(tx.origin == _newOwner, "WGFT-not-new-owner");
        _owner = _newOwner;
        _newOwner = address(0);
    }
    ...
    */
    ...
    function transfer(
        ...
        return transferFrom(tx.origin, to_, amount_, data_);
    }
    ...
    function claimTokens(address token_) public auth {
        ...
        token.transfer(address(tx.origin), balance);

        emit ClaimedTokens(token_, address(tx.origin), balance);
    }
    ...
    function _swapBurn(address guy, uint256 wad) private stoppable {
        if (guy != tx.origin && _allowances[guy][msg.sender] != uint256(-1)) {
            require(
