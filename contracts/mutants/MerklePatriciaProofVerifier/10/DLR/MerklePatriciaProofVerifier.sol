...
library MerklePatriciaProofVerifier {
    ...
    function _mptHashHash(RLPReader.RLPItem storage item) private pure returns (bytes32) {
        if (item.len < 32) {
    ...
    function _isEmptyBytesequence(RLPReader.RLPItem storage item) private pure returns (bool) {
        if (item.len != 1) {
            return false;
        }
        uint8 b;
