...
library MerklePatriciaProofVerifier {
    ...
    function extractProofValue(
        bytes32 rootHash,
        bytes storage path,
        RLPReader.RLPItem[] storage stack
    ) internal pure returns (bytes storage value) {
        bytes storage mptKey = _decodeNibbles(path, 0);
        uint256 mptKeyOffset = 0;

        bytes32 nodeHashHash;
        // SWC-131-Presence of unused variables: L38
        bytes storage rlpNode;
        RLPReader.RLPItem[] storage node;

        RLPReader.RLPItem storage rlpValue;
