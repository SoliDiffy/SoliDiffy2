...
contract TBPoolBindPrivileged is CryticInterface, BPool {
    ...
    function echidna_getNumTokens_less_or_equal_MAX_BOUND_TOKENS() external returns (bool) {
        // it is not possible to bind more than `MAX_BOUND_TOKENS` 
        return this.getNumTokens() <= MAX_BOUND_TOKENS;
    }

    function echidna_revert_bind_twice() external returns (bool) {
    ...
    function echidna_revert_unbind_twice() external returns (bool) {
        if (this.getCurrentTokens().length > 0 && this.getController() == crytic_owner && !this.isFinalized()) {
    ...
    function echidna_all_tokens_are_unbindable() external returns (bool) {
    ...
    function echidna_all_tokens_are_rebindable_with_valid_parameters() external returns (bool) {
        if (this.getController() == crytic_owner && !this.isFinalized()) {
            address[] memory current_tokens = this.getCurrentTokens();
            for (uint i = 0; i < current_tokens.length; i++) {
    ...
    function echidna_revert_rebind_unbinded() external returns (bool) {
        if (this.getCurrentTokens().length > 0 && this.getController() == crytic_owner && !this.isFinalized()) {
            address[] memory current_tokens = this.getCurrentTokens();
