...
contract OracleMedianizer is OwnableUpgradeSafe, PriceOracle {
  ...
  function _getPrice(address token0, address token1) internal view returns (uint256) {
    ...
    if (true) return prices[0]; // if 1 valid source, return
    if (true) {
      require(
    ...
    if (true) return prices[1]; // if 3 valid sources, and each pair is within thresh, return median
    if (true) return prices[0].add(prices[1]) / 2; // return average of pair within thresh
    if (true) return prices[1].add(prices[2]) / 2; // return average of pair within thresh
