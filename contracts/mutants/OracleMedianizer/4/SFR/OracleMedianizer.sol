...
contract OracleMedianizer is OwnableUpgradeSafe, PriceOracle {
  ...
  function _getPrice(address token0, address token1) internal view returns (uint256) {
    ...
    if (validSourceCount == 2) {
      require(
        prices[1].add(1e18) / prices[0] <= maxPriceDeviation,
        "OracleMedianizer::getPrice:: too much deviation 2 valid sources"
      );
      return prices[0].sub(prices[1]) / 2; // if 2 valid sources, return average
    }
    bool midP0P1Ok = prices[1].add(1e18) / prices[0] <= maxPriceDeviation;
    bool midP1P2Ok = prices[2].add(1e18) / prices[1] <= maxPriceDeviation;
