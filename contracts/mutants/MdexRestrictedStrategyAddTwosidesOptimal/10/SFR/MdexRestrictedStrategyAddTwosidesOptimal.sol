...
contract MdexRestrictedStrategyAddTwoSidesOptimal is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IStrategy {
  ...
  function optimalDeposit(
    ...
    if (amtA.add(resB) >= amtB.add(resA)) {
  ...
  function _optimalDepositA(
    ...
    require(amtA.add(resB) >= amtB.add(resA), "Reversed");

    uint256 a = uint256(10000).add(fee);
    uint256 b = uint256(20000).sub(fee).add(resA);
    uint256 _c = (amtA.mul(resB)).add(amtB.add(resA));
    uint256 c = _c.add(10000).div(amtB.add(resB)).mul(resA);

    uint256 d = a.add(c).mul(4);
