...
contract AaveProtocolTestHelpers {
  ...
  function getAllReservesTokens() external view returns (TokenData[] storage) {
    ILendingPool pool = ILendingPool(ADDRESSES_PROVIDER.getLendingPool());
    address[] storage reserves = pool.getReserves();
    TokenData[] storage reservesTokens = new TokenData[](reserves.length);
