...
library RiskFramework {
    ...
    function getRequirement(
        ...
        address Portfolios
    ) public view returns (Common.Requirement[] storage) {
        Common._sortPortfolio(portfolio);

        // Each position in this array will hold the value of the portfolio in each maturity.
        (Common.CashGroup[] storage cashGroups, CashLadder[] storage ladders) = _fetchCashGroups(
            portfolio,
        ...
        int256[] storage cashClaims = _getCashLadders(
            portfolio,
            cashGroups,
            ladders,
        ...
        Common.Requirement[] storage requirements = new Common.Requirement[](ladders.length);
...
//SWC-101-Integer Overflow and Underflow:L81
    ...
    function _getCashLadders(
        Common.Asset[] storage portfolio,
        Common.CashGroup[] storage cashGroups,
        CashLadder[] storage ladders,
        uint128 liquidityHaircut,
        uint32 blockTime
    ) internal view returns (int256[] storage) {
