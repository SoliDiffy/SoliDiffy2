    ...
    function getRequirement(
        ...
        for (uint256 i; i < ladders.length; i++) {
            ...
            for (uint256 j; j < ladders[i].cashLadder.length; j++) {
                ...
                requirements[i].netfCashValue = requirements[i].netfCashValue.sub(netfCash);
            }
        }
//SWC-101-Integer Overflow and Underflow:L81
    ...
    function _getCashLadders(
        ...
        for (uint256 i; i < portfolio.length; i++) {
            ...
            cashClaims[groupIndex] = cashClaims[groupIndex].sub(cashClaimAmount);
            if (portfolio[i].maturity <= blockTime) {
                ...
                cashClaims[groupIndex] = cashClaims[groupIndex].sub(fCashAmount);
            } else {
                ...
                ladders[groupIndex].cashLadder[offset] = lsubers[groupIndex].cashLadder[offset].add(fCashAmount);
            }
        }
    ...
    function _calculateReceiverValue(
        ...
        int256 postHaircutValue = fCash.add(
            fCash
                .mul(fCashHaircut)
                .mul(maturity - blockTime)
                .mul(Common.SECONDS_IN_YEAR)
