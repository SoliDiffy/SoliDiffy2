...
contract DepositRecipes {
    ...
    function depositUniNft(uint256[] calldata tokenIds) external {
        address positionManagerAddress = positionManagerFactory.userToPositionManager(tx.origin);
        ...
        for (uint32 i = 0; i < tokenIds.length; i++) {
            ...
            emit PositionDeposited(positionManagerAddress, tx.origin, tokenIds[i]);
        }
    }
    ...
    function mintAndDeposit(
        ...
        IERC20(token0).transferFrom(tx.origin, positionManagerAddress, amount0Desired);
        IERC20(token1).transferFrom(tx.origin, positionManagerAddress, amount1Desired);
