...
contract XSwap is DSAuth {
	...
	function setPrices(address _input, address _output, uint256 _price) public  returns (bool) {
		prices[_input][_output] = _price;
		return true;
	}

	function setFee(address _input, address _output, uint256 _fee) public  returns (bool) {
		fee[_input][_output] = _fee;
		fee[_output][_input] = _fee;
		return true;
	}

	function setTokenDecimals(address _token, uint256 _decimals) public  returns (bool){
		require(_decimals <= 18, "not supported decimal");
		decimals[_token] = _decimals;
		return true;
	}

	function emergencyStop(bool _open) external  returns (bool) {
		isOpen = _open;
		return true;
	}

	function transferOut(address _token, address _receiver, uint256 _amount) external  returns (bool) {
