...
contract XSwap is DSAuth {
	...
	// create new pairs
	function createPair(address _input, address _output, uint256 _priceInOut, uint256 _priceOutIn, uint256 _fee) external  returns (bool) {
		setPrices(_input, _output, _priceInOut, _priceOutIn);
		setFee(_input, _output, _fee);
		return true;
	}

	function setPrices(address _input, address _output, uint256 _priceInOut, uint256 _priceOutIn) public  returns (bool) {
