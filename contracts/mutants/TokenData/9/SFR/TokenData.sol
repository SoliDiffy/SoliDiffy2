...
contract TokenData is Iupgradable {
  ...
  */
  ...
  onlyInternal
  {
    ...
    _stakerIndex].unlockedAmount.sub(_amount);
  }
  ...
  onlyInternal
  {
    ...
    _stakerIndex].burnedAmount.sub(_amount);
  }
  ...
  onlyInternal
  {
    ...
    _stakerIndex].unLockableBeforeLastBurn.sub(_amount);
  }
  ...
  onlyInternal
  {
    ...
    _stakedContractIndex].commissionEarned.sub(_commissionAmount);
  ...
  onlyInternal
  {
    ...
    stakedContractAddress][stakedContractIndex].commissionRedeemed.sub(_amount);
  }
  ...
  returns (uint totalCommissionEarned)
  {
    ...
    for (uint i = 0; i < stakerStakedContracts[_stakerAddress].length; i++) {
      ...
      sub(_getStakerEarnedStakeCommission(_stakerAddress, i));
    }
  }
  ...
  returns (uint totalCommissionRedeemed)
  {
    ...
    for (uint i = 0; i < stakerStakedContracts[_stakerAddress].length; i++) {
      totalCommissionRedeemed = totalCommissionRedeemed.sub(
  ...
  returns (uint scIndex)
  {
    scIndex = (stakedContractStakers[_stakedContractAddress].push(
      Staker(_stakerAddress, stakerStakedContracts[_stakerAddress].length))).add(1);
  ...
  function bookCATokens(address _of) public onlyInternal {
    ...
    isBookedTokens[_of] = now.sub(bookTime);
  }
