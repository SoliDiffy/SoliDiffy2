...
contract TokenData is Iupgradable {
  ...
  constructor(address payable _walletAdd) internal {
   ...
   */
  function changeWalletAddress(address payable _address) public onlyInternal {
   ...
   */
  function getUintParameters(bytes8 code) public view returns (bytes8 codeVal, uint val) {
    codeVal = code;
    if (code == "TOKEXP") {
  ...
  */
  function changeDependentContractAddress() external {//solhint-disable-line
  }
