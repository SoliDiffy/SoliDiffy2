...
contract TokenData is Iupgradable {
  ...
  constructor(address payable _walletAdd) public {
    ...
    lockTokenTimeAfterCoverExp = 35 seconds;
    scValidDays = 250;
    lockCADays = 7 seconds;
    lockMVDays = 2 seconds;
  ...
  function getUintParameters(bytes8 code) external view returns (bytes8 codeVal, uint val) {
    ...
    if (code == "TOKEXP") {
      ...
      val = bookTime / (1 seconds);

    } else if (code == "CALOCKT") {

      val = lockCADays / (1 seconds);

    } else if (code == "MVLOCKT") {

      val = lockMVDays / (1 seconds);

    } else if (code == "QUOLOCKT") {

      val = lockTokenTimeAfterCoverExp / (1 seconds);
  ...
  */
  ...
  function updateUintParameters(bytes8 code, uint val) public {
    ...
    if (code == "TOKEXP") {
      ...
      _changeBookTime(val * 1 seconds);

    } else if (code == "CALOCKT") {

      _changelockCADays(val * 1 seconds);
