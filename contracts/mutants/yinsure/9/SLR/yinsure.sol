...
library NexusMutualCover {
    ...
    function buyCover(
        ...
        Pool1 pool1 = Pool1(data.nxMaster.getLatestAddress(""));
        if (coverCurrency == "") {
            ...
            PoolData poolData = PoolData(data.nxMaster.getLatestAddress(""));
        ...
        QuotationData quotationData = QuotationData(data.nxMaster.getLatestAddress(""));
    ...
    function submitClaim(Data storage data, uint coverId) internal returns (uint) {
        Claims claims = Claims(data.nxMaster.getLatestAddress(""));
        claims.submitClaim(coverId);

        ClaimsData claimsData = ClaimsData(data.nxMaster.getLatestAddress(""));
