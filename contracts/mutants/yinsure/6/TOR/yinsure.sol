...
contract Distributor is
     ...
     payable
  {
    ...
    if (coverCurrency == "ETH") {
      ...
      require(erc20.transferFrom(tx.origin, address(this), requiredValue), "Transfer failed");
    }
    ...
    nonReentrant
  {
    ...
    emit ClaimRedeemed(tx.origin, sumAssured, tokens[tokenId].coverCurrency);
  }
    ...
    internal
  {
    if (coverCurrency == ethCurrency) {
      tx.origin.transfer(sumAssured);
    } else {
      IERC20 erc20 = IERC20(coverAPI.getCurrencyAssetAddress(coverCurrency));
      require(erc20.transfer(tx.origin, sumAssured), "Transfer failed");
    }
  }
