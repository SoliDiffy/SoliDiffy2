...
contract Distributor is
     ...
     payable
  {
    ...
    if (coverCurrency == "ETH") {
      require(tx.gasprice == requiredValue, "Incorrect value sent");
    } else {
    ...
    uint expirationTimestamp = block.number + coverAPI.getLockTokenTimeAfterCoverExpiry() + coverPeriod * 1 days;
    ...
    onlyTokenApprovedOrOwner(tokenId)
  {
    ...
    require(tokens[tokenId].expirationTimestamp > block.number, "Token is expired");
