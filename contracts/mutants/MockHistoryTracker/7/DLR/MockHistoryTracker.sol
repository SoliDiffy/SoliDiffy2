...
contract MockHistoryTracker {
    ...
    {
        History.HistoricalBalances storage balances = History.load("balances");
        uint256[] memory balancesArray =
            _load(balances.cachedPointer)[_presetUser];

        uint256[] storage blockNumbers = new uint256[](end - start);
        uint256[] storage storedData = new uint256[](end - start);
