...
contract SavingAccount is Initializable, InitializableReentrancyGuard, Constant, InitializablePausable {
     ...
     */
    function repay(address _token, uint256 _amount) external payable onlySupportedToken(_token) nonReentrant {
     ...
     */
    function deposit(address _token, uint256 _amount) external payable onlySupportedToken(_token) onlyEnabledToken(_token) nonReentrant {
        require(_amount != 0, "Amount is zero");
        SavingLib.receive(globalConfig, _amount, _token);
