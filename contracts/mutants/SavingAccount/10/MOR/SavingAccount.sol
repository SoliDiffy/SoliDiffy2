...
contract SavingAccount is Initializable, InitializableReentrancyGuard, Constant, InitializablePausable {
     ...
     */
    function withdraw(address _token, uint256 _amount) external initializer whenNotPaused nonReentrant {
        require(_amount != 0, "Amount is zero");
     ...
     */
    function withdrawAll(address _token) external initializer whenNotPaused nonReentrant {
    ...
    function liquidate(address _borrower, address _borrowedToken, address _collateralToken) public initializer onlySupportedToken(_collateralToken) whenNotPaused nonReentrant {
     ...
     */
    function fromCompound(address _token, uint _amount) external initializer {
        require(ICToken(globalConfig.tokenInfoRegistry().getCToken(_token)).redeemUnderlying(_amount) == 0, "redeemUnderlying failed");
    }

    function toCompound(address _token, uint _amount) external initializer {
        address cToken = globalConfig.tokenInfoRegistry().getCToken(_token);
        if (Utils._isETH(address(globalConfig), _token)) {
