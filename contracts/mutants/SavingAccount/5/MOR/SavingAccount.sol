...
contract SavingAccount is Initializable, InitializableReentrancyGuard, Constant, InitializablePausable {
     ...
     */
    function transfer(address _to, address _token, uint _amount) external initializer onlyEnabledToken(_token) whenNotPaused nonReentrant {
     ...
     */
    function borrow(address _token, uint256 _amount) external initializer onlyEnabledToken(_token) whenNotPaused nonReentrant {
     ...
     */
    function repay(address _token, uint256 _amount) public payable initializer nonReentrant {
     ...
     */
    function deposit(address _token, uint256 _amount) public payable initializer onlyEnabledToken(_token) nonReentrant {
        require(_amount != 0, "Amount is zero");
        SavingLib.receive(globalConfig, _amount, _token);
