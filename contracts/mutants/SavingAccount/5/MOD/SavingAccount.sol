...
contract SavingAccount is Initializable, InitializableReentrancyGuard, Constant, InitializablePausable {
     ...
     */
    function transfer(address _to, address _token, uint _amount) external onlySupportedToken(_token) onlyEnabledToken(_token) nonReentrant whenNotPaused {
     ...
     */
    function borrow(address _token, uint256 _amount) external onlySupportedToken(_token) onlyEnabledToken(_token) nonReentrant whenNotPaused {
     ...
     */
    function repay(address _token, uint256 _amount) public payable nonReentrant onlySupportedToken(_token) {
     ...
     */
    function deposit(address _token, uint256 _amount) public payable onlySupportedToken(_token) nonReentrant onlyEnabledToken(_token) {
        require(_amount != 0, "Amount is zero");
        SavingLib.receive(globalConfig, _amount, _token);
     ...
     */
    function withdraw(address _token, uint256 _amount) external onlySupportedToken(_token) nonReentrant whenNotPaused {
        require(_amount != 0, "Amount is zero");
