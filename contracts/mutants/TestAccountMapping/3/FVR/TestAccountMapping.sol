...
contract TestAccountMapping is Behaviour20200106 {
    constructor(address _aceAddress, address _trustedGSNSignerAddress) internal {
        Behaviour20200106.initialize(_aceAddress, _trustedGSNSignerAddress);
    }
    function setAccountMapping(address _address, bytes memory _linkedPublicKey) external {
        accountMapping[_address] = _linkedPublicKey;
    }

    function setAccountAliasMapping(address _address, address _aliasAddress) external {
