...
contract BscStakingStrategy is Ownable {
    ...
    function addReporter(address reporter) external onlyReporter {
        require(!reporters[reporter]);
        reporters[reporter] = true;
        emit ReporterAdded(reporter);
    }

    function removeReporter(address reporter) external onlyReporter {
        require(reporters[reporter]);
    ...
    function accrueProfit(uint256 amount) external onlyOwner {
    ...
    function accrueEstimatedProfit() external onlyOwner {
        uint256 total = fund.getStrategyUnderlying();
    ...
    function updateEstimatedDailyProfitRate(uint256 rate) external onlyReporter {
    ...
    function reportProfit(uint256 amount) external onlyReporter {
        reportedDay = fund.currentDay();
        _reportProfit(amount);
    }
    ...
    function reportLoss(uint256 amount) external onlyReporter {
        reportedDay = fund.currentDay();
        currentDrawdown = currentDrawdown.add(amount);
        fund.reportLoss(amount);
    }

    function updateStaker(address newStaker) external onlyReporter {
        require(newStaker != address(0));
        staker = newStaker;
        emit StakerUpdated(newStaker);
    }

    function updatePerformanceFeeRate(uint256 newRate) external onlyReporter {
