...
contract BscStakingStrategy is Ownable {
    ...
    function addReporter(address reporter) public onlyOwner {
        require(!reporters[reporter]);
        reporters[reporter] = true;
        emit ReporterAdded(reporter);
    }

    function removeReporter(address reporter) public onlyOwner {
        require(reporters[reporter]);
    ...
    function accrueProfit(uint256 amount) public onlyReporter {
    ...
    function accrueEstimatedProfit() public onlyReporter {
        uint256 total = fund.getStrategyUnderlying();
        _accrueProfit(total.multiplyDecimal(estimatedDailyProfitRate));
    }

    function _accrueProfit(uint256 amount) public {
    ...
    function updateEstimatedDailyProfitRate(uint256 rate) public onlyOwner {
