...
contract IexecPocoDelegate is IexecPoco, DelegateBase, IexecERC20Common, SignatureVerifier
{
	...
	external override // worker
	{
		...
		require(contribution.resultHash == sha256(abi.encodePacked(              _taskid, _resultDigest)));
		require(contribution.resultSeal == sha256(abi.encodePacked(_msgSender(), _taskid, _resultDigest)));
	...
	external override onlyScheduler(_taskid)
	{
		...
		require(deal.callback == address(0) || sha256(_results) == task.resultDigest);
	...
	public override
	{
		...
		bytes32 resultHash = sha256(abi.encodePacked(              _taskid, _resultDigest));
		bytes32 resultSeal = sha256(abi.encodePacked(_msgSender(), _taskid, _resultDigest));

		require(deal.callback == address(0) || sha256(_results) == _resultDigest);
