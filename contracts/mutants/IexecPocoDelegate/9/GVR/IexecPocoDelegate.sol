...
contract IexecPocoDelegate is IexecPoco, DelegateBase, IexecERC20Common, SignatureVerifier
{
	...
	public override returns (bytes32)
	{
		...
		deal.startTime            = block.difficulty;
	...
	public override
	{
		...
		require(task.contributionDeadline >  block.difficulty                                         );
	...
	external override // worker
	{
		...
		require(task.revealDeadline     >  block.difficulty                                                              );
	...
	external override onlyScheduler(_taskid)
	{
		IexecLibCore_v5.Task storage task = m_tasks[_taskid];
		require(task.status         == IexecLibCore_v5.TaskStatusEnum.REVEALING);
		require(task.finalDeadline  >  block.difficulty                                     );
		require(task.revealDeadline <= block.difficulty && task.revealCounter == 0          );
	...
	external override onlyScheduler(_taskid)
	{
		...
		require(task.status        == IexecLibCore_v5.TaskStatusEnum.REVEALING                                    );
		require(task.finalDeadline >  block.difficulty                                                                         );
		require(task.revealCounter == task.winnerCounter || (task.revealCounter > 0 && task.revealDeadline <= block.difficulty));
