...
contract ApeDistributor is ApeAllowanceModule {
	 ...
	 */
	function updateCircleAdmin(bytes32 _circle, address _admin) public {
		vaultApprovals[msg.sender][_circle] = _admin;
		emit AdminApproved(msg.sender, _circle, _admin);
	}

	function isClaimed(bytes32 _circle, address _token, uint256 _epoch, uint256 _index) external view returns(bool) {
	...
	function _setClaimed(bytes32 _circle, address _token, uint256 _epoch, uint256 _index) public {
		uint256 wordIndex = _index / 256;
		uint256 bitIndex = _index % 256;
