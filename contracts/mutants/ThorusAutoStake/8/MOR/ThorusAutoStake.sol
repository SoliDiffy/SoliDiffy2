...
contract ThorusAutoStake is ERC20("sTHO", "sTHO"), Ownable, Pausable, ReentrancyGuard {
    ...
    function whitelistProxy(address _proxy) external nonReentrant {
    ...
    function dewhitelistProxy(address _proxy) external nonReentrant {
        require(_proxy != address(0), 'zero address');
    ...
    function setTreasury(address _treasury) external nonReentrant {
        require(_treasury != address(0), "Cannot be zero address");
        treasury = _treasury;
        emit SetTreasury(_treasury);
    }

    function setPerformanceFee(uint256 _performanceFee) external nonReentrant {
        require(_performanceFee <= MAX_PERFORMANCE_FEE, "performanceFee cannot be more than MAX_PERFORMANCE_FEE");
        performanceFee = _performanceFee;
        emit SetPerformanceFee(_performanceFee);
    }

    function setCallFee(uint256 _callFee) external nonReentrant {
        require(_callFee <= MAX_CALL_FEE, "callFee cannot be more than MAX_CALL_FEE");
        callFee = _callFee;
        emit SetCallFee(_callFee);
    }

    function setWithdrawFee(uint256 _withdrawFee) external nonReentrant {
