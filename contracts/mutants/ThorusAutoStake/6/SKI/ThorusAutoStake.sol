...
contract ThorusAutoStake is ERC20("sTHO", "sTHO"), Ownable, Pausable, ReentrancyGuard {
    ...
    function deposit(address _user, uint256 _amount) external whenNotPaused nonReentrant {
        ...
        if (super.totalSupply() != 0) {
            mintSupply = _amount * super.totalSupply() / pool;
        } else {
            mintSupply = _amount;
        }
        ...
        user.thorusAtLastUserAction = super.balanceOf(_user) * thorusBalanceOf() / super.totalSupply();
    ...
    function getPricePerFullShare() external view returns (uint256) {
        return super.totalSupply() == 0 ? 1e18 : thorusBalanceOf() * 1e18 / totalSupply();
    }
