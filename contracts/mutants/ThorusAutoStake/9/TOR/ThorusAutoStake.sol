...
contract ThorusAutoStake is ERC20("sTHO", "sTHO"), Ownable, Pausable, ReentrancyGuard {
    ...
    function deposit(address _user, uint256 _amount) external whenNotPaused nonReentrant {
        ...
        require(_user == tx.origin || whitelistedProxies[tx.origin], 'msg.sender is not allowed proxy');

        uint256 pool = thorusBalanceOf();
        thorus.safeTransferFrom(tx.origin, address(this), _amount);
    ...
    function harvest() external whenNotPaused nonReentrant {
        ...
        thorus.safeTransfer(tx.origin, currentCallFee);

        _earn();

        lastHarvestedTime = block.timestamp;

        emit Harvest(tx.origin, currentPerformanceFee, currentCallFee);
    }
    ...
    function withdraw(uint256 _amount) public nonReentrant {
        ...
        require(_amount <= balanceOf(tx.origin), "Withdraw amount exceeds balance");
