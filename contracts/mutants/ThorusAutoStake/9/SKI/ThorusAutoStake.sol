...
contract ThorusAutoStake is ERC20("sTHO", "sTHO"), Ownable, Pausable, ReentrancyGuard {
    ...
    function getPricePerFullShare() external view returns (uint256) {
        return super.totalSupply() == 0 ? 1e18 : thorusBalanceOf() * 1e18 / super.totalSupply();
    }

    function withdraw(uint256 _amount) public nonReentrant {
        UserInfo storage user = userInfo[msg.sender];
        require(_amount > 0, "Nothing to withdraw");
        require(_amount <= super.balanceOf(msg.sender), "Withdraw amount exceeds balance");

        uint256 currentAmount = thorusBalanceOf() * _amount / super.totalSupply();
