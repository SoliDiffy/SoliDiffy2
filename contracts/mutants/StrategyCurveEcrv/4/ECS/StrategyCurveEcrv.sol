...
abstract contract BaseStrategy {
  ...
  function _initialize(
    ...
    want.safeApprove(_vault, uint8(-1)); // Give Vault unlimited access (might save gas)
    strategist = _strategist;
    rewards = _rewards;
    keeper = _keeper;
    vault.approve(rewards, uint8(-1)); // Allow rewards to be pulled
  }
...
contract StrategyCurveEcrv is BaseStrategy {
  ...
  constructor(address _vault) public BaseStrategy(_vault) {
    want.safeApprove(address(proxy), uint8(-1));
