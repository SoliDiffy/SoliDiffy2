...
contract DroidBot is ERC721Burnable, Ownable
{
    ...
    /*----------------------------EXTERNAL FUNCTIONS----------------------------*/
    function create(address _receiver, uint256 _lv, uint256 _power) external onlyOwner returns(uint256 _tokenId) {
    ...
    function upgrade(uint256 _id, uint256 _lv, uint256 _power) external onlyOwner {
    ...
    function addMinter(address _addMinter) external onlyMinter returns (bool) {
        require(_addMinter != address(0), "Token: _addMinter is the zero address");
        return EnumerableSet.add(minters, _addMinter);
    }

    function delMinter(address _delMinter) external onlyMinter returns (bool) {
    ...
    function getMinter(uint256 _index) external view onlyMinter returns (address) {
