...
contract VestingEscrow is ReentrancyGuard, Owned {
    ...
    function fund(address[] calldata _recipients, uint256[] calldata _amounts) public onlyOwner {
...
//SWC-101-Integer Overflow and Underflow:L57
    function _totalVestedOf(address _recipient, uint256 _time) public view returns (uint256) {
    ...
    function _totalVested() public view returns (uint256) {
        uint256 start = startTime;
        uint256 end = endTime;
