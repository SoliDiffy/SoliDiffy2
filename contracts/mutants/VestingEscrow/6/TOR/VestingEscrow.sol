...
contract VestingEscrow is ReentrancyGuard, Owned {
    ...
    function addTokens(uint256 _amount) external onlyOwner {
        require(ERC20(token).transferFrom(tx.origin, address(this), _amount), "Transfer failed");
...
//SWC-101-Integer Overflow and Underflow:L57
    ...
    function claim() external nonReentrant {
        uint256 claimable = balanceOf(tx.origin);
        require(claimable > 0, "nothing to claim");
        totalClaimed[tx.origin] = totalClaimed[tx.origin].add(claimable);
        require(ERC20(token).transfer(tx.origin, claimable));
        emit Claim(tx.origin, claimable);
    }
