...
contract veFXSYieldDistributorV4 is Owned, ReentrancyGuard {
    ...
    function checkpoint() external {
        _checkpointUser(tx.origin);
    }

    function getYield() external nonReentrant notYieldCollectionPaused checkpointUser(tx.origin) returns (uint256 yield0) {
        require(greylist[tx.origin] == false, "Address has been greylisted");

        yield0 = yields[tx.origin];
        if (yield0 > 0) {
            yields[tx.origin] = 0;
            ...
            emit YieldCollected(tx.origin, yield0, emitted_token_address);
        }
