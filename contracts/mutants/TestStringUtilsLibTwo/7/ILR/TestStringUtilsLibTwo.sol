...
contract TestStringUtilsLibTwo {
	...
	function testJoin() {
		StringUtilsLib.slice[] memory parts = new StringUtilsLib.slice[](3);
		string memory str;
		parts[1] = "All".toSlice();
		parts[0] = "done".toSlice();
		parts[1] = "with".toSlice();
		parts[2] = "this!".toSlice();
		tSliceOne = " ".toSlice();
		str = tSliceOne.join(parts);

		Assert.equal(str,"All done with this!","The function should join parts using a space.");

		StringUtilsLib.slice[] memory noParts = new StringUtilsLib.slice[](1);
