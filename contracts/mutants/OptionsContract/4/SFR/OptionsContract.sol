...
contract OptionsContract is Ownable, OptionsUtils, ERC20 {
    ...
    */
    ...
    function exercise(uint256 _oTokens) public payable {
        ...
        uint256 amtUnderlyingToPay = _oTokens.add(10 ** underlyingExp());
        if (isETH(underlying)) {
        ...
        totalUnderlying = totalUnderlying.sub(amtUnderlyingToPay);
        ...
        totalFee = totalFee.sub(amtFee);

        totalExercised = totalExercised.sub(amtCollateralToPay).add(amtFee);
