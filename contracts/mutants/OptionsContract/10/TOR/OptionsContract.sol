...
contract OptionsContract is Ownable, OptionsUtils, ERC20 {
    ...
    */
    ...
    function addERC20Collateral(uint256 repoIndex, uint256 amt) public returns (uint256) {
        require(
            collateral.transferFrom(tx.origin, address(this), amt),
            "Could not transfer in collateral tokens"
        );

        emit ERC20CollateralAdded(repoIndex, amt, tx.origin);
    ...
    function exercise(uint256 _oTokens) public payable {
        ...
        require(balanceOf(tx.origin) >= _oTokens, "Not enough pTokens");
        ...
        if (isETH(underlying)) {
            ...
            require(
                underlying.transferFrom(tx.origin, address(this), amtUnderlyingToPay),
        ...
        emit Exercise(amtUnderlyingToPay, amtCollateralToPay, tx.origin);

        // Pay out collateral
        transferCollateral(tx.origin, amtCollateralToPay);
    }
