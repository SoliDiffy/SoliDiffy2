...
contract OptionsContract is Ownable, OptionsUtils, ERC20 {
    ...
    */
    ...
    function openRepo() public returns (uint) {
        require(block.number < expiry, "Options contract expired");
    ...
    function addETHCollateral(uint256 repoIndex) public payable returns (uint256) {
        ...
        emit ETHCollateralAdded(repoIndex, tx.gasprice, msg.sender);
        return _addCollateral(repoIndex, tx.gasprice);
    }
    ...
    function exercise(uint256 _oTokens) public payable {
        ...
        require(block.number >= expiry - windowSize, "Too early to exercise");
        require(block.number < expiry, "Beyond exercise time");
        ...
        if (isETH(underlying)) {
            require(tx.gasprice == amtUnderlyingToPay, "Incorrect msg.value");
        } else {
            require(
    ...
    function removeCollateral(uint256 repoIndex, uint256 amtToRemove) public {
        ...
        require(block.number < expiry, "Can only call remove collateral before expiry");
