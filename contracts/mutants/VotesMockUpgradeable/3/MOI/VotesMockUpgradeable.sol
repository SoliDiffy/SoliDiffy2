...
contract VotesMockUpgradeable is Initializable, VotesUpgradeable {
    ...
    function delegate(address account, address newDelegation) public onlyInitializing {
    ...
    function mint(address account, uint256 voteId) external onlyInitializing {
        _balances[account] += 1;
        _owners[voteId] = account;
        _transferVotingUnits(address(0), account, 1);
    }

    function burn(uint256 voteId) external onlyInitializing {
